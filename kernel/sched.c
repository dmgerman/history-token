...
mdefine_line|#define SCHED_LOAD_SCALE (1 &lt;&lt; SCHED_LOAD_SHIFT)
...
suffix:semicolon
macro_line|#ifdef CONFIG_SMP
multiline_comment|/* Mandatory scheduling domains */
id|DEFINE_PER_CPU
c_func
(paren
r_struct
...
id|migration_req_t
suffix:semicolon
multiline_comment|/*&n; * The task&squot;s runqueue lock must be held, and the new mask must be valid.&n; * Returns true if you have to wait for migration thread.&n; */
DECL|function|__set_cpus_allowed
r_static
r_int
id|__set_cpus_allowed
c_func
(paren
id|task_t
op_star
...
suffix:semicolon
multiline_comment|/*&n; * Return a low guess at the load of cpu. Update previous history if update&n; * is true&n; */
...
macro_line|#endif
multiline_comment|/*&n; * sched_balance_wake can be used with SMT architectures to wake a&n; * task onto an idle sibling if cpu is not idle. Returns cpu if&n; * cpu is idle or no siblings are idle, otherwise returns an idle&n; * sibling.&n; */
macro_line|#if defined(CONFIG_SMP) &amp;&amp; defined(ARCH_HAS_SCHED_WAKE_BALANCE)
DECL|function|sched_balance_wake
r_static
r_int
id|sched_balance_wake
c_func
(paren
r_int
id|cpu
...
suffix:semicolon
multiline_comment|/*&n;&t; * Fast-migrate the task if it&squot;s not running or&n;&t; * runnable currently. Do not violate hard affinity.&n;&t; */
r_do
(brace
r_if
c_cond
...
(brace
multiline_comment|/* Need to wait for migration thread. */
id|task_rq_unlock
c_func
(paren
id|rq
...
suffix:semicolon
multiline_comment|/* If we raced with sys_sched_setaffinity, don&squot;t&n;&t;&t; * restore mask. */
id|rq
op_assign
id|task_rq_lock
c_func
...
(brace
multiline_comment|/* Restore old mask: won&squot;t need migration&n;&t;&t;&t; * thread, since current cpu is allowed. */
id|BUG_ON
c_func
(paren
id|__set_cpus_allowed
...
l_int|0
suffix:semicolon
multiline_comment|/* Aggressive migration if we&squot;ve failed balancing */
r_if
c_cond
(paren
id|idle
...
id|NEWLY_IDLE
op_logical_or
id|domain-&gt;nr_balance_failed
OL
id|domain-&gt;cache_nice_tries
)paren
(brace
r_if
c_cond
...
id|out_balanced
suffix:semicolon
multiline_comment|/*&n;&t; * Don&squot;t modify when we newly become idle because that ruins our&n;&t; * statistics: its triggered by some value of nr_running (ie. 0).&n;&t; * Timer based balancing is a good statistic though.&n;&t; */
r_if
c_cond
(paren
id|idle
...
id|avg_load
suffix:semicolon
multiline_comment|/*&n;&t;&t; * Load is cumulative over SD_FLAG_IDLE domains, but&n;&t;&t; * spread over !SD_FLAG_IDLE domains. For example, 2&n;&t;&t; * processes running on an SMT CPU puts a load of 2 on&n;&t;&t; * that CPU, however 2 processes running on 2 CPUs puts&n;&t;&t; * a load of 1 on that domain.&n;&t;&t; *&n;&t;&t; * This should be configurable so as SMT siblings become&n;&t;&t; * more powerful, they can &quot;spread&quot; more load - for example,&n;&t;&t; * the above case might only count as a load of 1.7.&n;&t;&t; */
r_if
c_cond
(paren
op_logical_neg
...
l_int|2
suffix:semicolon
multiline_comment|/* Get rid of the scaling factor, rounding *up* as we divide */
op_star
id|imbalance
op_assign
(paren
...
id|out_balanced
suffix:semicolon
multiline_comment|/* How many tasks to actually move to equalise the imbalance */
op_star
id|imbalance
op_mul_assign
id|min
...
c_cond
(paren
id|domain-&gt;balance_interval
OL
id|domain-&gt;max_interval
)paren
id|domain-&gt;balance_interval
op_mul_assign
l_int|2
...
multiline_comment|/* hasn&squot;t been setup yet */
r_break
suffix:semicolon
r_if
c_cond
...
multiline_comment|/*&n; * active_load_balance is run by migration threads. It pushes a running&n; * task off the cpu. It can be required to correctly have at least 1 task&n; * running on each physical CPU where possible, and not have a physical /&n; * logical imbalance.&n; *&n; * Called with busiest locked.&n; */
...
r_return
suffix:semicolon
multiline_comment|/* sd-&gt;parent should never cause a NULL dereference, if it did so,&n; &t; * then push_cpu was set to a buggy value */
r_while
c_loop
(paren
op_logical_neg
...
r_return
suffix:semicolon
multiline_comment|/* Run through all this CPU&squot;s domains */
r_do
(brace
r_int
r_int
...
suffix:semicolon
multiline_comment|/*&n;&t; * The RT priorities are set via setscheduler(), but we still&n;&t; * allow the &squot;normal&squot; nice value to be set - but as expected&n;&t; * it wont have any effect on scheduling until the task is&n;&t; * not SCHED_NORMAL:&n;&t; */
r_if
c_cond
(paren
id|rt_task
...
multiline_comment|/* migrate_all_tasks - function to migrate all the tasks from the&n; * current cpu caller must have already scheduled this to the target&n; * cpu via set_cpus_allowed.  Machine is stopped.  */
...
id|node
suffix:semicolon
multiline_comment|/* We&squot;re nailed to this CPU. */
id|src_cpu
op_assign
id|smp_processor_id
c_func
...
suffix:semicolon
multiline_comment|/* Not required, but here for neatness. */
id|write_lock
c_func
(paren
op_amp
...
suffix:semicolon
multiline_comment|/* watch out for per node tasks, let&squot;s stay on this node */
id|node
op_assign
id|cpu_to_node
c_func
...
r_continue
suffix:semicolon
multiline_comment|/* Figure out where this task should go (attempting to&n;&t;&t; * keep it on-node), and check if it can be migrated&n;&t;&t; * as-is.  NOTE that kernel threads bound to more than&n;&t;&t; * one online cpu will be migrated. */
id|mask
op_assign
id|node_to_cpumask
c_func
...
multiline_comment|/* Set up groups */
r_for
c_loop
(paren
id|i
...
multiline_comment|/* Set up CPU groups */
id|for_each_cpu_mask
c_func
(paren
id|i
...
suffix:semicolon
macro_line|#endif /* defined(CONFIG_SMP) &amp;&amp; defined(CONFIG_PREEMPT) */
eof
