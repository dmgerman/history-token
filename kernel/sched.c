...
mdefine_line|#define CAN_MIGRATE_TASK(p,rq,this_cpu)&t;&t;&t;&t;&t;&bslash;&n;&t;((jiffies - (p)-&gt;sleep_timestamp &gt; cache_decay_ticks) &amp;&amp;&t;&bslash;&n;&t;&t;((p) != (rq)-&gt;curr) &amp;&amp;&t;&t;&t;&t;&t;&bslash;&n;&t;&t;&t;(tmp-&gt;cpus_allowed &amp; (1 &lt;&lt; (this_cpu))))
r_if
c_cond
(paren
op_logical_neg
...
macro_line|#if CONFIG_SMP
multiline_comment|/*&n; * Change a given task&squot;s CPU affinity. Migrate the process to a&n; * proper CPU and schedule it away if the current CPU is removed&n; * from the allowed bitmask.&n; */
...
multiline_comment|/*&n;&t; * We mark the process as nonrunnable, and kick it to&n;&t; * schedule away from its current CPU. We also add&n;&t; * the task to the migration queue and wake up the&n;&t; * target CPU&squot;s migration thread, so that it can pick&n;&t; * up this task and insert it into the local runqueue.&n;&t; */
id|p-&gt;state
op_assign
id|TASK_UNINTERRUPTIBLE
suffix:semicolon
...
multiline_comment|/* wait for all migration threads to start up. */
r_while
c_loop
(paren
op_logical_neg
