...
mdefine_line|#define can_schedule(p,cpu) &bslash;&n;&t;((p)-&gt;cpus_runnable &amp; (p)-&gt;cpus_allowed &amp; (1 &lt;&lt; cpu))
macro_line|#else
DECL|macro|idle_task
...
suffix:semicolon
multiline_comment|/*&n;&t; * fast path falls through. We have to clear cpus_runnable before&n;&t; * checking prev-&gt;state to avoid a wakeup race. Protect against&n;&t; * the task exiting early.&n;&t; */
id|task_lock
c_func
(paren
id|prev
...
(brace
multiline_comment|/* We won&squot;t go through the normal tail, so do this by hand */
id|prev-&gt;policy
op_and_assign
op_complement
id|SCHED_YIELD
...
multiline_comment|/*&n; * The core wakeup function.  Non-exclusive wakeups (nr_exclusive == 0) just wake everything&n; * up.  If it&squot;s an exclusive wakeup (nr_exclusive == small +ve number) then we wake all the&n; * non-exclusive tasks and one exclusive task.&n; *&n; * There are circumstances in which we can try to wake a task which has already&n; * started to run but is not in state TASK_RUNNING.  try_to_wake_up() returns zero&n; * in this (rare) case, and we handle it by contonuing to scan the queue.&n; */
...
id|SIGCHLD
suffix:semicolon
multiline_comment|/* We also take the runqueue_lock while altering task fields&n;&t; * which affect scheduling decisions */
id|spin_lock
c_func
(paren
op_amp
...
id|this_task-&gt;cap_effective
op_assign
id|CAP_INIT_EFF_SET
suffix:semicolon
id|this_task-&gt;cap_inheritable
op_assign
id|CAP_INIT_INH_SET
suffix:semicolon
id|this_task-&gt;cap_permitted
op_assign
id|CAP_FULL_SET
suffix:semicolon
id|this_task-&gt;keep_capabilities
op_assign
l_int|0
suffix:semicolon
id|memcpy
