...
multiline_comment|/*&n; * wake up processes in the wait queue depending on the state of a context bit in the flags&n; * - wakes up a process if the specified bit is set in the flags member&n; * - the context bit is cleared if the process is woken up&n; * - if the bit number is negative, then the loop stops at the first unset context bit encountered&n; * - returns the number of processes woken&n; */
DECL|function|__wake_up_ctx_common
r_static
r_inline
r_int
id|__wake_up_ctx_common
(paren
id|wait_queue_head_t
op_star
id|q
...
DECL|function|__wake_up_ctx_sync
r_int
id|__wake_up_ctx_sync
c_func
(paren
id|wait_queue_head_t
op_star
