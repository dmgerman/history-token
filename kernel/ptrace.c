...
multiline_comment|/*&n; * Turn a tracing stop into a normal stop now, since with no tracer there&n; * would be no way to wake it up with SIGCONT or SIGKILL.  If there was a&n; * signal sent that would resume the child, but didn&squot;t because it was in&n; * TASK_TRACED, resume it now.&n; * Requires that irqs be disabled.&n; */
