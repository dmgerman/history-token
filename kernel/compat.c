...
multiline_comment|/*&n; * We currently only need the following fields from the sigevent&n; * structure: sigev_value, sigev_signo, sig_notify and (sometimes&n; * sigev_notify_thread_id).  The others are handled in user mode.&n; * We also assume that copying sigev_value.sival_int is sufficient&n; * to keep all the bits of sigev_value.sival_ptr intact.&n; */
