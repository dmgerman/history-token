...
suffix:semicolon
multiline_comment|/* modutils uses these exported symbols to figure out if&n;   kallsyms support is present */
DECL|variable|__start___kallsyms
id|EXPORT_SYMBOL
c_func
(paren
id|__start___kallsyms
...
suffix:semicolon
DECL|variable|__stop___kallsyms
id|EXPORT_SYMBOL
c_func
(paren
id|__stop___kallsyms
...
suffix:semicolon
DECL|macro|MODLIST_SIZE
mdefine_line|#define MODLIST_SIZE 4096
multiline_comment|/*&n; * this function isn&squot;t smp safe but that&squot;s not really a problem; it&squot;s&n; * called from oops context only and any locking could actually prevent&n; * the oops from going out; the line that is generated is informational&n; * only and should NEVER prevent the real oops from going out. &n; */
...
macro_line|#if defined(CONFIG_MODULES) || defined(CONFIG_KALLSYMS)
DECL|macro|MAX_SYMBOL_SIZE
mdefine_line|#define MAX_SYMBOL_SIZE 512
r_static
r_void
DECL|function|address_to_exported_symbol
id|address_to_exported_symbol
c_func
(paren
r_int
r_int
...
(brace
multiline_comment|/* walk the symbol list of this module. Only symbols&n;&t;&t;   who&squot;s address is smaller than the searched for address&n;&t;&t;   are relevant; and only if it&squot;s better than the best so far */
r_for
c_loop
(paren
id|i
...
(brace
multiline_comment|/* static to not take up stackspace; if we race here too bad */
r_static
r_char
id|buffer
(braket
