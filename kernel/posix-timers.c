...
r_static
r_int
id|do_posix_clock_process_gettime
c_func
(paren
r_struct
id|timespec
...
r_static
r_int
id|do_posix_clock_thread_gettime
c_func
(paren
r_struct
id|timespec
...
multiline_comment|/*&n; * This function gets called when a POSIX.1b interval timer expires.  It&n; * is used as a callback from the kernel internal timer.  The&n; * run_timer_list code ALWAYS calls with interrutps on.&n;&n; * This code is for CLOCK_REALTIME* and CLOCK_MONOTONIC* timers.&n; */
...
(brace
multiline_comment|/* Single Unix specficiation says to return ENOTSUP but we do not have that */
r_return
op_minus
id|EINVAL
suffix:semicolon
)brace
DECL|function|process_ticks
r_static
r_int
r_int
id|process_ticks
...
suffix:semicolon
multiline_comment|/* The signal structure is shared between all threads */
id|ticks
op_assign
id|p-&gt;signal-&gt;utime
op_plus
id|p-&gt;signal-&gt;stime
suffix:semicolon
multiline_comment|/* Add up the cpu time for all the still running threads of this process */
id|t
op_assign
id|p
suffix:semicolon
...
multiline_comment|/*&n; * Single Unix Specification V3:&n; *&n; * Implementations shall also support the special clockid_t value&n; * CLOCK_THREAD_CPUTIME_ID, which represents the CPU-time clock of the calling&n; * thread when invoking one of the clock_*() or timer_*() functions. For these&n; * clock IDs, the values returned by clock_gettime() and specified by&n; * clock_settime() shall represent the amount of execution time of the thread&n; * associated with the clock.&n; */
DECL|function|do_posix_clock_thread_gettime
r_static
r_int
id|do_posix_clock_thread_gettime
c_func
...
multiline_comment|/*&n; * Single Unix Specification V3:&n; *&n; * Implementations shall also support the special clockid_t value&n; * CLOCK_PROCESS_CPUTIME_ID, which represents the CPU-time clock of the&n; * calling process when invoking one of the clock_*() or timer_*() functions.&n; * For these clock IDs, the values returned by clock_gettime() and specified&n; * by clock_settime() represent the amount of execution time of the process&n; * associated with the clock.&n; */
DECL|function|do_posix_clock_process_gettime
r_static
r_int
id|do_posix_clock_process_gettime
c_func
...
id|new_tp
suffix:semicolon
multiline_comment|/* Cannot set process specific clocks */
r_if
c_cond
(paren
id|which_clock
...
(brace
multiline_comment|/* Process process specific clocks */
r_if
c_cond
(paren
id|which_clock
...
multiline_comment|/* More process specific clocks could follow here */
r_return
op_minus
id|EINVAL
suffix:semicolon
...
id|rtn_tp
suffix:semicolon
multiline_comment|/* All process clocks have the resolution of CLOCK_PROCESS_CPUTIME_ID */
r_if
c_cond
(paren
id|which_clock
