...
id|running_timer
suffix:semicolon
DECL|member|run_timer_list_running
r_struct
id|list_head
op_star
id|run_timer_list_running
suffix:semicolon
DECL|member|tv1
id|tvec_root_t
id|tv1
...
multiline_comment|/*&n; * If a timer handler re-adds the timer with expires == jiffies, the timer&n; * running code can lock up.  So here we detect that situation and park the&n; * timer onto base-&gt;run_timer_list_running.  It will be added to the main timer&n; * structures later, by __run_timers().&n; */
...
c_cond
(paren
id|base-&gt;run_timer_list_running
)paren
(brace
id|vec
op_assign
...
multiline_comment|/***&n; * add_timer - start a timer&n; * @timer: the timer to be added&n; *&n; * The kernel will do a -&gt;function(-&gt;data) callback from the&n; * timer interrupt at the -&gt;expired point in the future. The&n; * current time is &squot;jiffies&squot;.&n; *&n; * The timer&squot;s -&gt;expired, -&gt;function (and if the handler uses it, -&gt;data)&n; * fields must be set prior calling this function.&n; *&n; * Timers with an -&gt;expired field in the past will be executed in the next&n; * timer tick. It&squot;s illegal to add an already pending timer.&n; */
...
multiline_comment|/***&n; * add_timer_on - start a timer on a particular CPU&n; * @timer: the timer to be added&n; * @cpu: the CPU to start it on&n; *&n; * This is not very scalable on SMP.&n; */
...
id|INITIAL_JIFFIES
suffix:semicolon
multiline_comment|/*&n;&t; * The tv indices are always larger by one compared to the&n;&t; * respective parts of timer_jiffies. If all lower indices are&n;&t; * zero at initialisation, this is achieved by an (otherwise&n;&t; * unneccessary) invocation of the timer cascade on the first&n;&t; * timer interrupt. If not, we need to take it into account&n;&t; * here:&n;&t; */
id|j
op_assign
(paren
id|base-&gt;tv1.index
