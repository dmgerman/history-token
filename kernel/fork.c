...
multiline_comment|/*&n; * Checks if the use count of an mm is non-zero and if so&n; * returns a reference to it after bumping up the use count.&n; * If the use count is zero, it means this mm is going away,&n; * so return NULL.&n; */
DECL|function|mmgrab
r_struct
id|mm_struct
op_star
id|mmgrab
...
(brace
multiline_comment|/* Warn about the old no longer supported case so that we see it */
r_if
c_cond
(paren
id|clone_flags
...
(paren
id|KERN_WARNING
l_string|&quot;%s trying to use CLONE_THREAD without CLONE_DETACH&bslash;n&quot;
comma
id|current-&gt;comm
)paren
suffix:semicolon
