...
multiline_comment|/* All the workqueues on the system, for hotplug cpu to add/remove&n;   threads to each one as cpus come/go.  Protected by cpucontrol&n;   sem. */
r_static
id|LIST_HEAD
c_func
(paren
...
suffix:semicolon
DECL|macro|add_workqueue
mdefine_line|#define add_workqueue(wq) list_add(&amp;(wq)-&gt;list, &amp;workqueues)
DECL|macro|del_workqueue
mdefine_line|#define del_workqueue(wq) list_del(&amp;(wq)-&gt;list)
macro_line|#else
DECL|macro|add_workqueue
mdefine_line|#define add_workqueue(wq)
DECL|macro|del_workqueue
mdefine_line|#define del_workqueue(wq)
