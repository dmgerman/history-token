multiline_comment|/* &n; * Cryptographic API.&n; *&n; * DES &amp; 3DES_EDE Cipher Algorithms.&n; *&n; * Originally released as descore by Dana L. How &lt;how@isl.stanford.edu&gt;.&n; * Modified by Raimar Falke &lt;rf13@inf.tu-dresden.de&gt; for the Linux-Kernel.&n; * Derived from Cryptoapi and Nettle implementations, adapted for in-place&n; * scatterlist interface.  Changed LGPL to GPL per section 3 of the LGPL.&n; *&n; * Copyright (c) 1992 Dana L. How.&n; * Copyright (c) Raimar Falke &lt;rf13@inf.tu-dresden.de&gt; &n; * Copyright (c) Gisle S&#xfffd;lensminde &lt;gisle@ii.uib.no&gt;&n; * Copyright (C) 2001 Niels M&#xfffd;ller.&n; * Copyright (c) 2002 James Morris &lt;jmorris@intercode.com.au&gt;&n; *&n; * This program is free software; you can redistribute it and/or modify&n; * it under the terms of the GNU General Public License as published by&n; * the Free Software Foundation; either version 2 of the License, or&n; * (at your option) any later version.&n; *&n; */
macro_line|#include &lt;linux/init.h&gt;
macro_line|#include &lt;linux/module.h&gt;
macro_line|#include &lt;linux/mm.h&gt;
macro_line|#include &lt;asm/scatterlist.h&gt;
macro_line|#include &lt;linux/crypto.h&gt;
DECL|macro|DES_KEY_SIZE
mdefine_line|#define DES_KEY_SIZE&t;&t;8
DECL|macro|DES_KEY_SIZE_WORDS
mdefine_line|#define DES_KEY_SIZE_WORDS&t;2
DECL|macro|DES_BLOCK_SIZE
mdefine_line|#define DES_BLOCK_SIZE&t;&t;8
DECL|macro|ROR
mdefine_line|#define ROR(d,c,o)&t;((d) = (d) &gt;&gt; (c) | (d) &lt;&lt; (o))
DECL|variable|des_keymap
r_const
r_static
id|__u32
id|des_keymap
(braket
)braket
op_assign
(brace
l_int|0x02080008
comma
l_int|0x02082000
comma
l_int|0x00002008
comma
l_int|0x00000000
comma
l_int|0x02002000
comma
l_int|0x00080008
comma
l_int|0x02080000
comma
l_int|0x02082008
comma
l_int|0x00000008
comma
l_int|0x02000000
comma
l_int|0x00082000
comma
l_int|0x00002008
comma
l_int|0x00082008
comma
l_int|0x02002008
comma
l_int|0x02000008
comma
l_int|0x02080000
comma
l_int|0x00002000
comma
l_int|0x00082008
comma
l_int|0x00080008
comma
l_int|0x02002000
comma
l_int|0x02082008
comma
l_int|0x02000008
comma
l_int|0x00000000
comma
l_int|0x00082000
comma
l_int|0x02000000
comma
l_int|0x00080000
comma
l_int|0x02002008
comma
l_int|0x02080008
comma
l_int|0x00080000
comma
l_int|0x00002000
comma
l_int|0x02082000
comma
l_int|0x00000008
comma
l_int|0x00080000
comma
l_int|0x00002000
comma
l_int|0x02000008
comma
l_int|0x02082008
comma
l_int|0x00002008
comma
l_int|0x02000000
comma
l_int|0x00000000
comma
l_int|0x00082000
comma
l_int|0x02080008
comma
l_int|0x02002008
comma
l_int|0x02002000
comma
l_int|0x00080008
comma
l_int|0x02082000
comma
l_int|0x00000008
comma
l_int|0x00080008
comma
l_int|0x02002000
comma
l_int|0x02082008
comma
l_int|0x00080000
comma
l_int|0x02080000
comma
l_int|0x02000008
comma
l_int|0x00082000
comma
l_int|0x00002008
comma
l_int|0x02002008
comma
l_int|0x02080000
comma
l_int|0x00000008
comma
l_int|0x02082000
comma
l_int|0x00082008
comma
l_int|0x00000000
comma
l_int|0x02000000
comma
l_int|0x02080008
comma
l_int|0x00002000
comma
l_int|0x00082008
comma
l_int|0x08000004
comma
l_int|0x00020004
comma
l_int|0x00000000
comma
l_int|0x08020200
comma
l_int|0x00020004
comma
l_int|0x00000200
comma
l_int|0x08000204
comma
l_int|0x00020000
comma
l_int|0x00000204
comma
l_int|0x08020204
comma
l_int|0x00020200
comma
l_int|0x08000000
comma
l_int|0x08000200
comma
l_int|0x08000004
comma
l_int|0x08020000
comma
l_int|0x00020204
comma
l_int|0x00020000
comma
l_int|0x08000204
comma
l_int|0x08020004
comma
l_int|0x00000000
comma
l_int|0x00000200
comma
l_int|0x00000004
comma
l_int|0x08020200
comma
l_int|0x08020004
comma
l_int|0x08020204
comma
l_int|0x08020000
comma
l_int|0x08000000
comma
l_int|0x00000204
comma
l_int|0x00000004
comma
l_int|0x00020200
comma
l_int|0x00020204
comma
l_int|0x08000200
comma
l_int|0x00000204
comma
l_int|0x08000000
comma
l_int|0x08000200
comma
l_int|0x00020204
comma
l_int|0x08020200
comma
l_int|0x00020004
comma
l_int|0x00000000
comma
l_int|0x08000200
comma
l_int|0x08000000
comma
l_int|0x00000200
comma
l_int|0x08020004
comma
l_int|0x00020000
comma
l_int|0x00020004
comma
l_int|0x08020204
comma
l_int|0x00020200
comma
l_int|0x00000004
comma
l_int|0x08020204
comma
l_int|0x00020200
comma
l_int|0x00020000
comma
l_int|0x08000204
comma
l_int|0x08000004
comma
l_int|0x08020000
comma
l_int|0x00020204
comma
l_int|0x00000000
comma
l_int|0x00000200
comma
l_int|0x08000004
comma
l_int|0x08000204
comma
l_int|0x08020200
comma
l_int|0x08020000
comma
l_int|0x00000204
comma
l_int|0x00000004
comma
l_int|0x08020004
comma
l_int|0x80040100
comma
l_int|0x01000100
comma
l_int|0x80000000
comma
l_int|0x81040100
comma
l_int|0x00000000
comma
l_int|0x01040000
comma
l_int|0x81000100
comma
l_int|0x80040000
comma
l_int|0x01040100
comma
l_int|0x81000000
comma
l_int|0x01000000
comma
l_int|0x80000100
comma
l_int|0x81000000
comma
l_int|0x80040100
comma
l_int|0x00040000
comma
l_int|0x01000000
comma
l_int|0x81040000
comma
l_int|0x00040100
comma
l_int|0x00000100
comma
l_int|0x80000000
comma
l_int|0x00040100
comma
l_int|0x81000100
comma
l_int|0x01040000
comma
l_int|0x00000100
comma
l_int|0x80000100
comma
l_int|0x00000000
comma
l_int|0x80040000
comma
l_int|0x01040100
comma
l_int|0x01000100
comma
l_int|0x81040000
comma
l_int|0x81040100
comma
l_int|0x00040000
comma
l_int|0x81040000
comma
l_int|0x80000100
comma
l_int|0x00040000
comma
l_int|0x81000000
comma
l_int|0x00040100
comma
l_int|0x01000100
comma
l_int|0x80000000
comma
l_int|0x01040000
comma
l_int|0x81000100
comma
l_int|0x00000000
comma
l_int|0x00000100
comma
l_int|0x80040000
comma
l_int|0x00000000
comma
l_int|0x81040000
comma
l_int|0x01040100
comma
l_int|0x00000100
comma
l_int|0x01000000
comma
l_int|0x81040100
comma
l_int|0x80040100
comma
l_int|0x00040000
comma
l_int|0x81040100
comma
l_int|0x80000000
comma
l_int|0x01000100
comma
l_int|0x80040100
comma
l_int|0x80040000
comma
l_int|0x00040100
comma
l_int|0x01040000
comma
l_int|0x81000100
comma
l_int|0x80000100
comma
l_int|0x01000000
comma
l_int|0x81000000
comma
l_int|0x01040100
comma
l_int|0x04010801
comma
l_int|0x00000000
comma
l_int|0x00010800
comma
l_int|0x04010000
comma
l_int|0x04000001
comma
l_int|0x00000801
comma
l_int|0x04000800
comma
l_int|0x00010800
comma
l_int|0x00000800
comma
l_int|0x04010001
comma
l_int|0x00000001
comma
l_int|0x04000800
comma
l_int|0x00010001
comma
l_int|0x04010800
comma
l_int|0x04010000
comma
l_int|0x00000001
comma
l_int|0x00010000
comma
l_int|0x04000801
comma
l_int|0x04010001
comma
l_int|0x00000800
comma
l_int|0x00010801
comma
l_int|0x04000000
comma
l_int|0x00000000
comma
l_int|0x00010001
comma
l_int|0x04000801
comma
l_int|0x00010801
comma
l_int|0x04010800
comma
l_int|0x04000001
comma
l_int|0x04000000
comma
l_int|0x00010000
comma
l_int|0x00000801
comma
l_int|0x04010801
comma
l_int|0x00010001
comma
l_int|0x04010800
comma
l_int|0x04000800
comma
l_int|0x00010801
comma
l_int|0x04010801
comma
l_int|0x00010001
comma
l_int|0x04000001
comma
l_int|0x00000000
comma
l_int|0x04000000
comma
l_int|0x00000801
comma
l_int|0x00010000
comma
l_int|0x04010001
comma
l_int|0x00000800
comma
l_int|0x04000000
comma
l_int|0x00010801
comma
l_int|0x04000801
comma
l_int|0x04010800
comma
l_int|0x00000800
comma
l_int|0x00000000
comma
l_int|0x04000001
comma
l_int|0x00000001
comma
l_int|0x04010801
comma
l_int|0x00010800
comma
l_int|0x04010000
comma
l_int|0x04010001
comma
l_int|0x00010000
comma
l_int|0x00000801
comma
l_int|0x04000800
comma
l_int|0x04000801
comma
l_int|0x00000001
comma
l_int|0x04010000
comma
l_int|0x00010800
comma
l_int|0x00000400
comma
l_int|0x00000020
comma
l_int|0x00100020
comma
l_int|0x40100000
comma
l_int|0x40100420
comma
l_int|0x40000400
comma
l_int|0x00000420
comma
l_int|0x00000000
comma
l_int|0x00100000
comma
l_int|0x40100020
comma
l_int|0x40000020
comma
l_int|0x00100400
comma
l_int|0x40000000
comma
l_int|0x00100420
comma
l_int|0x00100400
comma
l_int|0x40000020
comma
l_int|0x40100020
comma
l_int|0x00000400
comma
l_int|0x40000400
comma
l_int|0x40100420
comma
l_int|0x00000000
comma
l_int|0x00100020
comma
l_int|0x40100000
comma
l_int|0x00000420
comma
l_int|0x40100400
comma
l_int|0x40000420
comma
l_int|0x00100420
comma
l_int|0x40000000
comma
l_int|0x40000420
comma
l_int|0x40100400
comma
l_int|0x00000020
comma
l_int|0x00100000
comma
l_int|0x40000420
comma
l_int|0x00100400
comma
l_int|0x40100400
comma
l_int|0x40000020
comma
l_int|0x00000400
comma
l_int|0x00000020
comma
l_int|0x00100000
comma
l_int|0x40100400
comma
l_int|0x40100020
comma
l_int|0x40000420
comma
l_int|0x00000420
comma
l_int|0x00000000
comma
l_int|0x00000020
comma
l_int|0x40100000
comma
l_int|0x40000000
comma
l_int|0x00100020
comma
l_int|0x00000000
comma
l_int|0x40100020
comma
l_int|0x00100020
comma
l_int|0x00000420
comma
l_int|0x40000020
comma
l_int|0x00000400
comma
l_int|0x40100420
comma
l_int|0x00100000
comma
l_int|0x00100420
comma
l_int|0x40000000
comma
l_int|0x40000400
comma
l_int|0x40100420
comma
l_int|0x40100000
comma
l_int|0x00100420
comma
l_int|0x00100400
comma
l_int|0x40000400
comma
l_int|0x00800000
comma
l_int|0x00001000
comma
l_int|0x00000040
comma
l_int|0x00801042
comma
l_int|0x00801002
comma
l_int|0x00800040
comma
l_int|0x00001042
comma
l_int|0x00801000
comma
l_int|0x00001000
comma
l_int|0x00000002
comma
l_int|0x00800002
comma
l_int|0x00001040
comma
l_int|0x00800042
comma
l_int|0x00801002
comma
l_int|0x00801040
comma
l_int|0x00000000
comma
l_int|0x00001040
comma
l_int|0x00800000
comma
l_int|0x00001002
comma
l_int|0x00000042
comma
l_int|0x00800040
comma
l_int|0x00001042
comma
l_int|0x00000000
comma
l_int|0x00800002
comma
l_int|0x00000002
comma
l_int|0x00800042
comma
l_int|0x00801042
comma
l_int|0x00001002
comma
l_int|0x00801000
comma
l_int|0x00000040
comma
l_int|0x00000042
comma
l_int|0x00801040
comma
l_int|0x00801040
comma
l_int|0x00800042
comma
l_int|0x00001002
comma
l_int|0x00801000
comma
l_int|0x00001000
comma
l_int|0x00000002
comma
l_int|0x00800002
comma
l_int|0x00800040
comma
l_int|0x00800000
comma
l_int|0x00001040
comma
l_int|0x00801042
comma
l_int|0x00000000
comma
l_int|0x00001042
comma
l_int|0x00800000
comma
l_int|0x00000040
comma
l_int|0x00001002
comma
l_int|0x00800042
comma
l_int|0x00000040
comma
l_int|0x00000000
comma
l_int|0x00801042
comma
l_int|0x00801002
comma
l_int|0x00801040
comma
l_int|0x00000042
comma
l_int|0x00001000
comma
l_int|0x00001040
comma
l_int|0x00801002
comma
l_int|0x00800040
comma
l_int|0x00000042
comma
l_int|0x00000002
comma
l_int|0x00001042
comma
l_int|0x00801000
comma
l_int|0x00800002
comma
l_int|0x10400000
comma
l_int|0x00404010
comma
l_int|0x00000010
comma
l_int|0x10400010
comma
l_int|0x10004000
comma
l_int|0x00400000
comma
l_int|0x10400010
comma
l_int|0x00004010
comma
l_int|0x00400010
comma
l_int|0x00004000
comma
l_int|0x00404000
comma
l_int|0x10000000
comma
l_int|0x10404010
comma
l_int|0x10000010
comma
l_int|0x10000000
comma
l_int|0x10404000
comma
l_int|0x00000000
comma
l_int|0x10004000
comma
l_int|0x00404010
comma
l_int|0x00000010
comma
l_int|0x10000010
comma
l_int|0x10404010
comma
l_int|0x00004000
comma
l_int|0x10400000
comma
l_int|0x10404000
comma
l_int|0x00400010
comma
l_int|0x10004010
comma
l_int|0x00404000
comma
l_int|0x00004010
comma
l_int|0x00000000
comma
l_int|0x00400000
comma
l_int|0x10004010
comma
l_int|0x00404010
comma
l_int|0x00000010
comma
l_int|0x10000000
comma
l_int|0x00004000
comma
l_int|0x10000010
comma
l_int|0x10004000
comma
l_int|0x00404000
comma
l_int|0x10400010
comma
l_int|0x00000000
comma
l_int|0x00404010
comma
l_int|0x00004010
comma
l_int|0x10404000
comma
l_int|0x10004000
comma
l_int|0x00400000
comma
l_int|0x10404010
comma
l_int|0x10000000
comma
l_int|0x10004010
comma
l_int|0x10400000
comma
l_int|0x00400000
comma
l_int|0x10404010
comma
l_int|0x00004000
comma
l_int|0x00400010
comma
l_int|0x10400010
comma
l_int|0x00004010
comma
l_int|0x00400010
comma
l_int|0x00000000
comma
l_int|0x10404000
comma
l_int|0x10000010
comma
l_int|0x10400000
comma
l_int|0x10004010
comma
l_int|0x00000010
comma
l_int|0x00404000
comma
l_int|0x00208080
comma
l_int|0x00008000
comma
l_int|0x20200000
comma
l_int|0x20208080
comma
l_int|0x00200000
comma
l_int|0x20008080
comma
l_int|0x20008000
comma
l_int|0x20200000
comma
l_int|0x20008080
comma
l_int|0x00208080
comma
l_int|0x00208000
comma
l_int|0x20000080
comma
l_int|0x20200080
comma
l_int|0x00200000
comma
l_int|0x00000000
comma
l_int|0x20008000
comma
l_int|0x00008000
comma
l_int|0x20000000
comma
l_int|0x00200080
comma
l_int|0x00008080
comma
l_int|0x20208080
comma
l_int|0x00208000
comma
l_int|0x20000080
comma
l_int|0x00200080
comma
l_int|0x20000000
comma
l_int|0x00000080
comma
l_int|0x00008080
comma
l_int|0x20208000
comma
l_int|0x00000080
comma
l_int|0x20200080
comma
l_int|0x20208000
comma
l_int|0x00000000
comma
l_int|0x00000000
comma
l_int|0x20208080
comma
l_int|0x00200080
comma
l_int|0x20008000
comma
l_int|0x00208080
comma
l_int|0x00008000
comma
l_int|0x20000080
comma
l_int|0x00200080
comma
l_int|0x20208000
comma
l_int|0x00000080
comma
l_int|0x00008080
comma
l_int|0x20200000
comma
l_int|0x20008080
comma
l_int|0x20000000
comma
l_int|0x20200000
comma
l_int|0x00208000
comma
l_int|0x20208080
comma
l_int|0x00008080
comma
l_int|0x00208000
comma
l_int|0x20200080
comma
l_int|0x00200000
comma
l_int|0x20000080
comma
l_int|0x20008000
comma
l_int|0x00000000
comma
l_int|0x00008000
comma
l_int|0x00200000
comma
l_int|0x20200080
comma
l_int|0x00208080
comma
l_int|0x20000000
comma
l_int|0x20208000
comma
l_int|0x00000080
comma
l_int|0x20008080
comma
)brace
suffix:semicolon
DECL|variable|rotors
r_const
r_static
r_char
id|rotors
(braket
)braket
op_assign
(brace
l_int|34
comma
l_int|13
comma
l_int|5
comma
l_int|46
comma
l_int|47
comma
l_int|18
comma
l_int|32
comma
l_int|41
comma
l_int|11
comma
l_int|53
comma
l_int|33
comma
l_int|20
comma
l_int|14
comma
l_int|36
comma
l_int|30
comma
l_int|24
comma
l_int|49
comma
l_int|2
comma
l_int|15
comma
l_int|37
comma
l_int|42
comma
l_int|50
comma
l_int|0
comma
l_int|21
comma
l_int|38
comma
l_int|48
comma
l_int|6
comma
l_int|26
comma
l_int|39
comma
l_int|4
comma
l_int|52
comma
l_int|25
comma
l_int|12
comma
l_int|27
comma
l_int|31
comma
l_int|40
comma
l_int|1
comma
l_int|17
comma
l_int|28
comma
l_int|29
comma
l_int|23
comma
l_int|51
comma
l_int|35
comma
l_int|7
comma
l_int|3
comma
l_int|22
comma
l_int|9
comma
l_int|43
comma
l_int|41
comma
l_int|20
comma
l_int|12
comma
l_int|53
comma
l_int|54
comma
l_int|25
comma
l_int|39
comma
l_int|48
comma
l_int|18
comma
l_int|31
comma
l_int|40
comma
l_int|27
comma
l_int|21
comma
l_int|43
comma
l_int|37
comma
l_int|0
comma
l_int|1
comma
l_int|9
comma
l_int|22
comma
l_int|44
comma
l_int|49
comma
l_int|2
comma
l_int|7
comma
l_int|28
comma
l_int|45
comma
l_int|55
comma
l_int|13
comma
l_int|33
comma
l_int|46
comma
l_int|11
comma
l_int|6
comma
l_int|32
comma
l_int|19
comma
l_int|34
comma
l_int|38
comma
l_int|47
comma
l_int|8
comma
l_int|24
comma
l_int|35
comma
l_int|36
comma
l_int|30
comma
l_int|3
comma
l_int|42
comma
l_int|14
comma
l_int|10
comma
l_int|29
comma
l_int|16
comma
l_int|50
comma
l_int|55
comma
l_int|34
comma
l_int|26
comma
l_int|38
comma
l_int|11
comma
l_int|39
comma
l_int|53
comma
l_int|5
comma
l_int|32
comma
l_int|45
comma
l_int|54
comma
l_int|41
comma
l_int|35
comma
l_int|2
comma
l_int|51
comma
l_int|14
comma
l_int|15
comma
l_int|23
comma
l_int|36
comma
l_int|3
comma
l_int|8
comma
l_int|16
comma
l_int|21
comma
l_int|42
comma
l_int|6
comma
l_int|12
comma
l_int|27
comma
l_int|47
comma
l_int|31
comma
l_int|25
comma
l_int|20
comma
l_int|46
comma
l_int|33
comma
l_int|48
comma
l_int|52
comma
l_int|4
comma
l_int|22
comma
l_int|7
comma
l_int|49
comma
l_int|50
comma
l_int|44
comma
l_int|17
comma
l_int|1
comma
l_int|28
comma
l_int|24
comma
l_int|43
comma
l_int|30
comma
l_int|9
comma
l_int|12
comma
l_int|48
comma
l_int|40
comma
l_int|52
comma
l_int|25
comma
l_int|53
comma
l_int|38
comma
l_int|19
comma
l_int|46
comma
l_int|6
comma
l_int|11
comma
l_int|55
comma
l_int|49
comma
l_int|16
comma
l_int|10
comma
l_int|28
comma
l_int|29
comma
l_int|37
comma
l_int|50
comma
l_int|17
comma
l_int|22
comma
l_int|30
comma
l_int|35
comma
l_int|1
comma
l_int|20
comma
l_int|26
comma
l_int|41
comma
l_int|4
comma
l_int|45
comma
l_int|39
comma
l_int|34
comma
l_int|31
comma
l_int|47
comma
l_int|5
comma
l_int|13
comma
l_int|18
comma
l_int|36
comma
l_int|21
comma
l_int|8
comma
l_int|9
comma
l_int|3
comma
l_int|0
comma
l_int|15
comma
l_int|42
comma
l_int|7
comma
l_int|2
comma
l_int|44
comma
l_int|23
comma
l_int|26
comma
l_int|5
comma
l_int|54
comma
l_int|13
comma
l_int|39
comma
l_int|38
comma
l_int|52
comma
l_int|33
comma
l_int|31
comma
l_int|20
comma
l_int|25
comma
l_int|12
comma
l_int|8
comma
l_int|30
comma
l_int|24
comma
l_int|42
comma
l_int|43
comma
l_int|51
comma
l_int|9
comma
l_int|0
comma
l_int|36
comma
l_int|44
comma
l_int|49
comma
l_int|15
comma
l_int|34
comma
l_int|40
comma
l_int|55
comma
l_int|18
comma
l_int|6
comma
l_int|53
comma
l_int|48
comma
l_int|45
comma
l_int|4
comma
l_int|19
comma
l_int|27
comma
l_int|32
comma
l_int|50
comma
l_int|35
comma
l_int|22
comma
l_int|23
comma
l_int|17
comma
l_int|14
comma
l_int|29
comma
l_int|1
comma
l_int|21
comma
l_int|16
comma
l_int|3
comma
l_int|37
comma
l_int|40
comma
l_int|19
comma
l_int|11
comma
l_int|27
comma
l_int|53
comma
l_int|52
comma
l_int|13
comma
l_int|47
comma
l_int|45
comma
l_int|34
comma
l_int|39
comma
l_int|26
comma
l_int|22
comma
l_int|44
comma
l_int|7
comma
l_int|1
comma
l_int|2
comma
l_int|10
comma
l_int|23
comma
l_int|14
comma
l_int|50
comma
l_int|3
comma
l_int|8
comma
l_int|29
comma
l_int|48
comma
l_int|54
comma
l_int|12
comma
l_int|32
comma
l_int|20
comma
l_int|38
comma
l_int|5
comma
l_int|6
comma
l_int|18
comma
l_int|33
comma
l_int|41
comma
l_int|46
comma
l_int|9
comma
l_int|49
comma
l_int|36
comma
l_int|37
comma
l_int|0
comma
l_int|28
comma
l_int|43
comma
l_int|15
comma
l_int|35
comma
l_int|30
comma
l_int|17
comma
l_int|51
comma
l_int|54
comma
l_int|33
comma
l_int|25
comma
l_int|41
comma
l_int|38
comma
l_int|13
comma
l_int|27
comma
l_int|4
comma
l_int|6
comma
l_int|48
comma
l_int|53
comma
l_int|40
comma
l_int|36
comma
l_int|3
comma
l_int|21
comma
l_int|15
comma
l_int|16
comma
l_int|24
comma
l_int|37
comma
l_int|28
comma
l_int|9
comma
l_int|17
comma
l_int|22
comma
l_int|43
comma
l_int|5
comma
l_int|11
comma
l_int|26
comma
l_int|46
comma
l_int|34
comma
l_int|52
comma
l_int|19
comma
l_int|20
comma
l_int|32
comma
l_int|47
comma
l_int|55
comma
l_int|31
comma
l_int|23
comma
l_int|8
comma
l_int|50
comma
l_int|51
comma
l_int|14
comma
l_int|42
comma
l_int|2
comma
l_int|29
comma
l_int|49
comma
l_int|44
comma
l_int|0
comma
l_int|10
comma
l_int|11
comma
l_int|47
comma
l_int|39
comma
l_int|55
comma
l_int|52
comma
l_int|27
comma
l_int|41
comma
l_int|18
comma
l_int|20
comma
l_int|5
comma
l_int|38
comma
l_int|54
comma
l_int|50
comma
l_int|17
comma
l_int|35
comma
l_int|29
comma
l_int|30
comma
l_int|7
comma
l_int|51
comma
l_int|42
comma
l_int|23
comma
l_int|0
comma
l_int|36
comma
l_int|2
comma
l_int|19
comma
l_int|25
comma
l_int|40
comma
l_int|31
comma
l_int|48
comma
l_int|13
comma
l_int|33
comma
l_int|34
comma
l_int|46
comma
l_int|4
comma
l_int|12
comma
l_int|45
comma
l_int|37
comma
l_int|22
comma
l_int|9
comma
l_int|10
comma
l_int|28
comma
l_int|1
comma
l_int|16
comma
l_int|43
comma
l_int|8
comma
l_int|3
comma
l_int|14
comma
l_int|24
comma
l_int|18
comma
l_int|54
comma
l_int|46
comma
l_int|5
comma
l_int|6
comma
l_int|34
comma
l_int|48
comma
l_int|25
comma
l_int|27
comma
l_int|12
comma
l_int|45
comma
l_int|4
comma
l_int|2
comma
l_int|24
comma
l_int|42
comma
l_int|36
comma
l_int|37
comma
l_int|14
comma
l_int|3
comma
l_int|49
comma
l_int|30
comma
l_int|7
comma
l_int|43
comma
l_int|9
comma
l_int|26
comma
l_int|32
comma
l_int|47
comma
l_int|38
comma
l_int|55
comma
l_int|20
comma
l_int|40
comma
l_int|41
comma
l_int|53
comma
l_int|11
comma
l_int|19
comma
l_int|52
comma
l_int|44
comma
l_int|29
comma
l_int|16
comma
l_int|17
comma
l_int|35
comma
l_int|8
comma
l_int|23
comma
l_int|50
comma
l_int|15
comma
l_int|10
comma
l_int|21
comma
l_int|0
comma
l_int|32
comma
l_int|11
comma
l_int|31
comma
l_int|19
comma
l_int|20
comma
l_int|48
comma
l_int|5
comma
l_int|39
comma
l_int|41
comma
l_int|26
comma
l_int|6
comma
l_int|18
comma
l_int|16
comma
l_int|7
comma
l_int|1
comma
l_int|50
comma
l_int|51
comma
l_int|28
comma
l_int|17
comma
l_int|8
comma
l_int|44
comma
l_int|21
comma
l_int|2
comma
l_int|23
comma
l_int|40
comma
l_int|46
comma
l_int|4
comma
l_int|52
comma
l_int|12
comma
l_int|34
comma
l_int|54
comma
l_int|55
comma
l_int|38
comma
l_int|25
comma
l_int|33
comma
l_int|13
comma
l_int|3
comma
l_int|43
comma
l_int|30
comma
l_int|0
comma
l_int|49
comma
l_int|22
comma
l_int|37
comma
l_int|9
comma
l_int|29
comma
l_int|24
comma
l_int|35
comma
l_int|14
comma
l_int|46
comma
l_int|25
comma
l_int|45
comma
l_int|33
comma
l_int|34
comma
l_int|5
comma
l_int|19
comma
l_int|53
comma
l_int|55
comma
l_int|40
comma
l_int|20
comma
l_int|32
comma
l_int|30
comma
l_int|21
comma
l_int|15
comma
l_int|9
comma
l_int|10
comma
l_int|42
comma
l_int|0
comma
l_int|22
comma
l_int|3
comma
l_int|35
comma
l_int|16
comma
l_int|37
comma
l_int|54
comma
l_int|31
comma
l_int|18
comma
l_int|13
comma
l_int|26
comma
l_int|48
comma
l_int|11
comma
l_int|12
comma
l_int|52
comma
l_int|39
comma
l_int|47
comma
l_int|27
comma
l_int|17
comma
l_int|2
comma
l_int|44
comma
l_int|14
comma
l_int|8
comma
l_int|36
comma
l_int|51
comma
l_int|23
comma
l_int|43
comma
l_int|7
comma
l_int|49
comma
l_int|28
comma
l_int|31
comma
l_int|39
comma
l_int|6
comma
l_int|47
comma
l_int|48
comma
l_int|19
comma
l_int|33
comma
l_int|38
comma
l_int|12
comma
l_int|54
comma
l_int|34
comma
l_int|46
comma
l_int|44
comma
l_int|35
comma
l_int|29
comma
l_int|23
comma
l_int|24
comma
l_int|1
comma
l_int|14
comma
l_int|36
comma
l_int|17
comma
l_int|49
comma
l_int|30
comma
l_int|51
comma
l_int|11
comma
l_int|45
comma
l_int|32
comma
l_int|27
comma
l_int|40
comma
l_int|5
comma
l_int|25
comma
l_int|26
comma
l_int|13
comma
l_int|53
comma
l_int|4
comma
l_int|41
comma
l_int|0
comma
l_int|16
comma
l_int|3
comma
l_int|28
comma
l_int|22
comma
l_int|50
comma
l_int|10
comma
l_int|37
comma
l_int|2
comma
l_int|21
comma
l_int|8
comma
l_int|42
comma
l_int|45
comma
l_int|53
comma
l_int|20
comma
l_int|4
comma
l_int|5
comma
l_int|33
comma
l_int|47
comma
l_int|52
comma
l_int|26
comma
l_int|11
comma
l_int|48
comma
l_int|31
comma
l_int|3
comma
l_int|49
comma
l_int|43
comma
l_int|37
comma
l_int|7
comma
l_int|15
comma
l_int|28
comma
l_int|50
comma
l_int|0
comma
l_int|8
comma
l_int|44
comma
l_int|10
comma
l_int|25
comma
l_int|6
comma
l_int|46
comma
l_int|41
comma
l_int|54
comma
l_int|19
comma
l_int|39
comma
l_int|40
comma
l_int|27
comma
l_int|38
comma
l_int|18
comma
l_int|55
comma
l_int|14
comma
l_int|30
comma
l_int|17
comma
l_int|42
comma
l_int|36
comma
l_int|9
comma
l_int|24
comma
l_int|51
comma
l_int|16
comma
l_int|35
comma
l_int|22
comma
l_int|1
comma
l_int|6
comma
l_int|38
comma
l_int|34
comma
l_int|18
comma
l_int|19
comma
l_int|47
comma
l_int|4
comma
l_int|13
comma
l_int|40
comma
l_int|25
comma
l_int|5
comma
l_int|45
comma
l_int|17
comma
l_int|8
comma
l_int|2
comma
l_int|51
comma
l_int|21
comma
l_int|29
comma
l_int|42
comma
l_int|9
comma
l_int|14
comma
l_int|22
comma
l_int|3
comma
l_int|24
comma
l_int|39
comma
l_int|20
comma
l_int|31
comma
l_int|55
comma
l_int|11
comma
l_int|33
comma
l_int|53
comma
l_int|54
comma
l_int|41
comma
l_int|52
comma
l_int|32
comma
l_int|12
comma
l_int|28
comma
l_int|44
comma
l_int|0
comma
l_int|1
comma
l_int|50
comma
l_int|23
comma
l_int|7
comma
l_int|10
comma
l_int|30
comma
l_int|49
comma
l_int|36
comma
l_int|15
comma
l_int|20
comma
l_int|52
comma
l_int|48
comma
l_int|32
comma
l_int|33
comma
l_int|4
comma
l_int|18
comma
l_int|27
comma
l_int|54
comma
l_int|39
comma
l_int|19
comma
l_int|6
comma
l_int|0
comma
l_int|22
comma
l_int|16
comma
l_int|10
comma
l_int|35
comma
l_int|43
comma
l_int|1
comma
l_int|23
comma
l_int|28
comma
l_int|36
comma
l_int|17
comma
l_int|7
comma
l_int|53
comma
l_int|34
comma
l_int|45
comma
l_int|12
comma
l_int|25
comma
l_int|47
comma
l_int|38
comma
l_int|11
comma
l_int|55
comma
l_int|13
comma
l_int|46
comma
l_int|26
comma
l_int|42
comma
l_int|3
comma
l_int|14
comma
l_int|15
comma
l_int|9
comma
l_int|37
comma
l_int|21
comma
l_int|24
comma
l_int|44
comma
l_int|8
comma
l_int|50
comma
l_int|29
comma
l_int|27
comma
l_int|6
comma
l_int|55
comma
l_int|39
comma
l_int|40
comma
l_int|11
comma
l_int|25
comma
l_int|34
comma
l_int|4
comma
l_int|46
comma
l_int|26
comma
l_int|13
comma
l_int|7
comma
l_int|29
comma
l_int|23
comma
l_int|17
comma
l_int|42
comma
l_int|50
comma
l_int|8
comma
l_int|30
comma
l_int|35
comma
l_int|43
comma
l_int|24
comma
l_int|14
comma
l_int|31
comma
l_int|41
comma
l_int|52
comma
l_int|19
comma
l_int|32
comma
l_int|54
comma
l_int|45
comma
l_int|18
comma
l_int|5
comma
l_int|20
comma
l_int|53
comma
l_int|33
comma
l_int|49
comma
l_int|10
comma
l_int|21
comma
l_int|22
comma
l_int|16
comma
l_int|44
comma
l_int|28
comma
l_int|0
comma
l_int|51
comma
l_int|15
comma
l_int|2
comma
l_int|36
comma
)brace
suffix:semicolon
DECL|variable|parity
r_const
r_static
r_char
id|parity
(braket
)braket
op_assign
(brace
l_int|8
comma
l_int|1
comma
l_int|0
comma
l_int|8
comma
l_int|0
comma
l_int|8
comma
l_int|8
comma
l_int|0
comma
l_int|0
comma
l_int|8
comma
l_int|8
comma
l_int|0
comma
l_int|8
comma
l_int|0
comma
l_int|2
comma
l_int|8
comma
l_int|0
comma
l_int|8
comma
l_int|8
comma
l_int|0
comma
l_int|8
comma
l_int|0
comma
l_int|0
comma
l_int|8
comma
l_int|8
comma
l_int|0
comma
l_int|0
comma
l_int|8
comma
l_int|0
comma
l_int|8
comma
l_int|8
comma
l_int|3
comma
l_int|0
comma
l_int|8
comma
l_int|8
comma
l_int|0
comma
l_int|8
comma
l_int|0
comma
l_int|0
comma
l_int|8
comma
l_int|8
comma
l_int|0
comma
l_int|0
comma
l_int|8
comma
l_int|0
comma
l_int|8
comma
l_int|8
comma
l_int|0
comma
l_int|8
comma
l_int|0
comma
l_int|0
comma
l_int|8
comma
l_int|0
comma
l_int|8
comma
l_int|8
comma
l_int|0
comma
l_int|0
comma
l_int|8
comma
l_int|8
comma
l_int|0
comma
l_int|8
comma
l_int|0
comma
l_int|0
comma
l_int|8
comma
l_int|0
comma
l_int|8
comma
l_int|8
comma
l_int|0
comma
l_int|8
comma
l_int|0
comma
l_int|0
comma
l_int|8
comma
l_int|8
comma
l_int|0
comma
l_int|0
comma
l_int|8
comma
l_int|0
comma
l_int|8
comma
l_int|8
comma
l_int|0
comma
l_int|8
comma
l_int|0
comma
l_int|0
comma
l_int|8
comma
l_int|0
comma
l_int|8
comma
l_int|8
comma
l_int|0
comma
l_int|0
comma
l_int|8
comma
l_int|8
comma
l_int|0
comma
l_int|8
comma
l_int|0
comma
l_int|0
comma
l_int|8
comma
l_int|8
comma
l_int|0
comma
l_int|0
comma
l_int|8
comma
l_int|0
comma
l_int|8
comma
l_int|8
comma
l_int|0
comma
l_int|0
comma
l_int|8
comma
l_int|8
comma
l_int|0
comma
l_int|8
comma
l_int|0
comma
l_int|0
comma
l_int|8
comma
l_int|0
comma
l_int|8
comma
l_int|8
comma
l_int|0
comma
l_int|8
comma
l_int|0
comma
l_int|0
comma
l_int|8
comma
l_int|8
comma
l_int|0
comma
l_int|0
comma
l_int|8
comma
l_int|0
comma
l_int|8
comma
l_int|8
comma
l_int|0
comma
l_int|0
comma
l_int|8
comma
l_int|8
comma
l_int|0
comma
l_int|8
comma
l_int|0
comma
l_int|0
comma
l_int|8
comma
l_int|8
comma
l_int|0
comma
l_int|0
comma
l_int|8
comma
l_int|0
comma
l_int|8
comma
l_int|8
comma
l_int|0
comma
l_int|8
comma
l_int|0
comma
l_int|0
comma
l_int|8
comma
l_int|0
comma
l_int|8
comma
l_int|8
comma
l_int|0
comma
l_int|0
comma
l_int|8
comma
l_int|8
comma
l_int|0
comma
l_int|8
comma
l_int|0
comma
l_int|0
comma
l_int|8
comma
l_int|8
comma
l_int|0
comma
l_int|0
comma
l_int|8
comma
l_int|0
comma
l_int|8
comma
l_int|8
comma
l_int|0
comma
l_int|0
comma
l_int|8
comma
l_int|8
comma
l_int|0
comma
l_int|8
comma
l_int|0
comma
l_int|0
comma
l_int|8
comma
l_int|0
comma
l_int|8
comma
l_int|8
comma
l_int|0
comma
l_int|8
comma
l_int|0
comma
l_int|0
comma
l_int|8
comma
l_int|8
comma
l_int|0
comma
l_int|0
comma
l_int|8
comma
l_int|0
comma
l_int|8
comma
l_int|8
comma
l_int|0
comma
l_int|8
comma
l_int|0
comma
l_int|0
comma
l_int|8
comma
l_int|0
comma
l_int|8
comma
l_int|8
comma
l_int|0
comma
l_int|0
comma
l_int|8
comma
l_int|8
comma
l_int|0
comma
l_int|8
comma
l_int|0
comma
l_int|0
comma
l_int|8
comma
l_int|0
comma
l_int|8
comma
l_int|8
comma
l_int|0
comma
l_int|8
comma
l_int|0
comma
l_int|0
comma
l_int|8
comma
l_int|8
comma
l_int|0
comma
l_int|0
comma
l_int|8
comma
l_int|0
comma
l_int|8
comma
l_int|8
comma
l_int|0
comma
l_int|4
comma
l_int|8
comma
l_int|8
comma
l_int|0
comma
l_int|8
comma
l_int|0
comma
l_int|0
comma
l_int|8
comma
l_int|8
comma
l_int|0
comma
l_int|0
comma
l_int|8
comma
l_int|0
comma
l_int|8
comma
l_int|8
comma
l_int|0
comma
l_int|8
comma
l_int|5
comma
l_int|0
comma
l_int|8
comma
l_int|0
comma
l_int|8
comma
l_int|8
comma
l_int|0
comma
l_int|0
comma
l_int|8
comma
l_int|8
comma
l_int|0
comma
l_int|8
comma
l_int|0
comma
l_int|6
comma
l_int|8
comma
)brace
suffix:semicolon
DECL|struct|des_ctx
r_struct
id|des_ctx
(brace
DECL|member|iv
id|__u8
id|iv
(braket
id|DES_BLOCK_SIZE
)braket
suffix:semicolon
DECL|member|keyinfo
id|__u32
id|keyinfo
(braket
l_int|32
)braket
suffix:semicolon
)brace
suffix:semicolon
DECL|function|des_small_fips_encrypt
r_static
r_void
id|des_small_fips_encrypt
c_func
(paren
r_void
op_star
id|ctx
comma
id|__u8
op_star
id|dst
comma
id|__u8
op_star
id|src
)paren
(brace
id|__u32
op_star
id|keyinfo
op_assign
(paren
(paren
r_struct
id|des_ctx
op_star
)paren
id|ctx
)paren
op_member_access_from_pointer
id|keyinfo
suffix:semicolon
id|__u32
id|x
comma
id|y
comma
id|z
suffix:semicolon
id|x
op_assign
id|src
(braket
l_int|7
)braket
suffix:semicolon
id|x
op_lshift_assign
l_int|8
suffix:semicolon
id|x
op_or_assign
id|src
(braket
l_int|6
)braket
suffix:semicolon
id|x
op_lshift_assign
l_int|8
suffix:semicolon
id|x
op_or_assign
id|src
(braket
l_int|5
)braket
suffix:semicolon
id|x
op_lshift_assign
l_int|8
suffix:semicolon
id|x
op_or_assign
id|src
(braket
l_int|4
)braket
suffix:semicolon
id|y
op_assign
id|src
(braket
l_int|3
)braket
suffix:semicolon
id|y
op_lshift_assign
l_int|8
suffix:semicolon
id|y
op_or_assign
id|src
(braket
l_int|2
)braket
suffix:semicolon
id|y
op_lshift_assign
l_int|8
suffix:semicolon
id|y
op_or_assign
id|src
(braket
l_int|1
)braket
suffix:semicolon
id|y
op_lshift_assign
l_int|8
suffix:semicolon
id|y
op_or_assign
id|src
(braket
l_int|0
)braket
suffix:semicolon
id|z
op_assign
(paren
(paren
id|x
op_rshift
l_int|004
)paren
op_xor
id|y
)paren
op_amp
l_int|0x0F0F0F0FL
suffix:semicolon
id|x
op_xor_assign
id|z
op_lshift
l_int|004
suffix:semicolon
id|y
op_xor_assign
id|z
suffix:semicolon
id|z
op_assign
(paren
(paren
id|y
op_rshift
l_int|020
)paren
op_xor
id|x
)paren
op_amp
l_int|0x0000FFFFL
suffix:semicolon
id|y
op_xor_assign
id|z
op_lshift
l_int|020
suffix:semicolon
id|x
op_xor_assign
id|z
suffix:semicolon
id|z
op_assign
(paren
(paren
id|x
op_rshift
l_int|002
)paren
op_xor
id|y
)paren
op_amp
l_int|0x33333333L
suffix:semicolon
id|x
op_xor_assign
id|z
op_lshift
l_int|002
suffix:semicolon
id|y
op_xor_assign
id|z
suffix:semicolon
id|z
op_assign
(paren
(paren
id|y
op_rshift
l_int|010
)paren
op_xor
id|x
)paren
op_amp
l_int|0x00FF00FFL
suffix:semicolon
id|y
op_xor_assign
id|z
op_lshift
l_int|010
suffix:semicolon
id|x
op_xor_assign
id|z
suffix:semicolon
id|x
op_assign
id|x
op_rshift
l_int|1
op_or
id|x
op_lshift
l_int|31
suffix:semicolon
id|z
op_assign
(paren
id|x
op_xor
id|y
)paren
op_amp
l_int|0x55555555L
suffix:semicolon
id|y
op_xor_assign
id|z
suffix:semicolon
id|x
op_xor_assign
id|z
suffix:semicolon
id|y
op_assign
id|y
op_rshift
l_int|1
op_or
id|y
op_lshift
l_int|31
suffix:semicolon
id|z
op_assign
id|keyinfo
(braket
l_int|0
)braket
suffix:semicolon
id|z
op_xor_assign
id|y
suffix:semicolon
id|x
op_xor_assign
op_star
(paren
id|__u32
op_star
)paren
(paren
(paren
id|u8
op_star
)paren
(paren
id|des_keymap
op_plus
l_int|192
)paren
op_plus
(paren
l_int|0xFC
op_amp
id|z
)paren
)paren
suffix:semicolon
id|z
op_rshift_assign
l_int|8
suffix:semicolon
id|x
op_xor_assign
op_star
(paren
id|__u32
op_star
)paren
(paren
(paren
id|u8
op_star
)paren
(paren
id|des_keymap
op_plus
l_int|128
)paren
op_plus
(paren
l_int|0xFC
op_amp
id|z
)paren
)paren
suffix:semicolon
id|z
op_rshift_assign
l_int|8
suffix:semicolon
id|x
op_xor_assign
op_star
(paren
id|__u32
op_star
)paren
(paren
(paren
id|u8
op_star
)paren
(paren
id|des_keymap
op_plus
l_int|64
)paren
op_plus
(paren
l_int|0xFC
op_amp
id|z
)paren
)paren
suffix:semicolon
id|z
op_rshift_assign
l_int|8
suffix:semicolon
id|x
op_xor_assign
op_star
(paren
id|__u32
op_star
)paren
(paren
(paren
id|u8
op_star
)paren
id|des_keymap
op_plus
(paren
l_int|0xFC
op_amp
id|z
)paren
)paren
suffix:semicolon
id|z
op_assign
id|keyinfo
(braket
l_int|1
)braket
suffix:semicolon
id|z
op_xor_assign
id|y
suffix:semicolon
id|z
op_assign
id|z
op_lshift
l_int|4
op_or
id|z
op_rshift
l_int|28
suffix:semicolon
id|x
op_xor_assign
op_star
(paren
id|__u32
op_star
)paren
(paren
(paren
id|u8
op_star
)paren
(paren
id|des_keymap
op_plus
l_int|448
)paren
op_plus
(paren
l_int|0xFC
op_amp
id|z
)paren
)paren
suffix:semicolon
id|z
op_rshift_assign
l_int|8
suffix:semicolon
id|x
op_xor_assign
op_star
(paren
id|__u32
op_star
)paren
(paren
(paren
id|u8
op_star
)paren
(paren
id|des_keymap
op_plus
l_int|384
)paren
op_plus
(paren
l_int|0xFC
op_amp
id|z
)paren
)paren
suffix:semicolon
id|z
op_rshift_assign
l_int|8
suffix:semicolon
id|x
op_xor_assign
op_star
(paren
id|__u32
op_star
)paren
(paren
(paren
id|u8
op_star
)paren
(paren
id|des_keymap
op_plus
l_int|320
)paren
op_plus
(paren
l_int|0xFC
op_amp
id|z
)paren
)paren
suffix:semicolon
id|z
op_rshift_assign
l_int|8
suffix:semicolon
id|x
op_xor_assign
op_star
(paren
id|__u32
op_star
)paren
(paren
(paren
id|u8
op_star
)paren
(paren
id|des_keymap
op_plus
l_int|256
)paren
op_plus
(paren
l_int|0xFC
op_amp
id|z
)paren
)paren
suffix:semicolon
id|z
op_assign
id|keyinfo
(braket
l_int|2
)braket
suffix:semicolon
id|z
op_xor_assign
id|x
suffix:semicolon
id|y
op_xor_assign
op_star
(paren
id|__u32
op_star
)paren
(paren
(paren
id|u8
op_star
)paren
(paren
id|des_keymap
op_plus
l_int|192
)paren
op_plus
(paren
l_int|0xFC
op_amp
id|z
)paren
)paren
suffix:semicolon
id|z
op_rshift_assign
l_int|8
suffix:semicolon
id|y
op_xor_assign
op_star
(paren
id|__u32
op_star
)paren
(paren
(paren
id|u8
op_star
)paren
(paren
id|des_keymap
op_plus
l_int|128
)paren
op_plus
(paren
l_int|0xFC
op_amp
id|z
)paren
)paren
suffix:semicolon
id|z
op_rshift_assign
l_int|8
suffix:semicolon
id|y
op_xor_assign
op_star
(paren
id|__u32
op_star
)paren
(paren
(paren
id|u8
op_star
)paren
(paren
id|des_keymap
op_plus
l_int|64
)paren
op_plus
(paren
l_int|0xFC
op_amp
id|z
)paren
)paren
suffix:semicolon
id|z
op_rshift_assign
l_int|8
suffix:semicolon
id|y
op_xor_assign
op_star
(paren
id|__u32
op_star
)paren
(paren
(paren
id|u8
op_star
)paren
id|des_keymap
op_plus
(paren
l_int|0xFC
op_amp
id|z
)paren
)paren
suffix:semicolon
id|z
op_assign
id|keyinfo
(braket
l_int|3
)braket
suffix:semicolon
id|z
op_xor_assign
id|x
suffix:semicolon
id|z
op_assign
id|z
op_lshift
l_int|4
op_or
id|z
op_rshift
l_int|28
suffix:semicolon
id|y
op_xor_assign
op_star
(paren
id|__u32
op_star
)paren
(paren
(paren
id|u8
op_star
)paren
(paren
id|des_keymap
op_plus
l_int|448
)paren
op_plus
(paren
l_int|0xFC
op_amp
id|z
)paren
)paren
suffix:semicolon
id|z
op_rshift_assign
l_int|8
suffix:semicolon
id|y
op_xor_assign
op_star
(paren
id|__u32
op_star
)paren
(paren
(paren
id|u8
op_star
)paren
(paren
id|des_keymap
op_plus
l_int|384
)paren
op_plus
(paren
l_int|0xFC
op_amp
id|z
)paren
)paren
suffix:semicolon
id|z
op_rshift_assign
l_int|8
suffix:semicolon
id|y
op_xor_assign
op_star
(paren
id|__u32
op_star
)paren
(paren
(paren
id|u8
op_star
)paren
(paren
id|des_keymap
op_plus
l_int|320
)paren
op_plus
(paren
l_int|0xFC
op_amp
id|z
)paren
)paren
suffix:semicolon
id|z
op_rshift_assign
l_int|8
suffix:semicolon
id|y
op_xor_assign
op_star
(paren
id|__u32
op_star
)paren
(paren
(paren
id|u8
op_star
)paren
(paren
id|des_keymap
op_plus
l_int|256
)paren
op_plus
(paren
l_int|0xFC
op_amp
id|z
)paren
)paren
suffix:semicolon
id|z
op_assign
id|keyinfo
(braket
l_int|4
)braket
suffix:semicolon
id|z
op_xor_assign
id|y
suffix:semicolon
id|x
op_xor_assign
op_star
(paren
id|__u32
op_star
)paren
(paren
(paren
id|u8
op_star
)paren
(paren
id|des_keymap
op_plus
l_int|192
)paren
op_plus
(paren
l_int|0xFC
op_amp
id|z
)paren
)paren
suffix:semicolon
id|z
op_rshift_assign
l_int|8
suffix:semicolon
id|x
op_xor_assign
op_star
(paren
id|__u32
op_star
)paren
(paren
(paren
id|u8
op_star
)paren
(paren
id|des_keymap
op_plus
l_int|128
)paren
op_plus
(paren
l_int|0xFC
op_amp
id|z
)paren
)paren
suffix:semicolon
id|z
op_rshift_assign
l_int|8
suffix:semicolon
id|x
op_xor_assign
op_star
(paren
id|__u32
op_star
)paren
(paren
(paren
id|u8
op_star
)paren
(paren
id|des_keymap
op_plus
l_int|64
)paren
op_plus
(paren
l_int|0xFC
op_amp
id|z
)paren
)paren
suffix:semicolon
id|z
op_rshift_assign
l_int|8
suffix:semicolon
id|x
op_xor_assign
op_star
(paren
id|__u32
op_star
)paren
(paren
(paren
id|u8
op_star
)paren
id|des_keymap
op_plus
(paren
l_int|0xFC
op_amp
id|z
)paren
)paren
suffix:semicolon
id|z
op_assign
id|keyinfo
(braket
l_int|5
)braket
suffix:semicolon
id|z
op_xor_assign
id|y
suffix:semicolon
id|z
op_assign
id|z
op_lshift
l_int|4
op_or
id|z
op_rshift
l_int|28
suffix:semicolon
id|x
op_xor_assign
op_star
(paren
id|__u32
op_star
)paren
(paren
(paren
id|u8
op_star
)paren
(paren
id|des_keymap
op_plus
l_int|448
)paren
op_plus
(paren
l_int|0xFC
op_amp
id|z
)paren
)paren
suffix:semicolon
id|z
op_rshift_assign
l_int|8
suffix:semicolon
id|x
op_xor_assign
op_star
(paren
id|__u32
op_star
)paren
(paren
(paren
id|u8
op_star
)paren
(paren
id|des_keymap
op_plus
l_int|384
)paren
op_plus
(paren
l_int|0xFC
op_amp
id|z
)paren
)paren
suffix:semicolon
id|z
op_rshift_assign
l_int|8
suffix:semicolon
id|x
op_xor_assign
op_star
(paren
id|__u32
op_star
)paren
(paren
(paren
id|u8
op_star
)paren
(paren
id|des_keymap
op_plus
l_int|320
)paren
op_plus
(paren
l_int|0xFC
op_amp
id|z
)paren
)paren
suffix:semicolon
id|z
op_rshift_assign
l_int|8
suffix:semicolon
id|x
op_xor_assign
op_star
(paren
id|__u32
op_star
)paren
(paren
(paren
id|u8
op_star
)paren
(paren
id|des_keymap
op_plus
l_int|256
)paren
op_plus
(paren
l_int|0xFC
op_amp
id|z
)paren
)paren
suffix:semicolon
id|z
op_assign
id|keyinfo
(braket
l_int|6
)braket
suffix:semicolon
id|z
op_xor_assign
id|x
suffix:semicolon
id|y
op_xor_assign
op_star
(paren
id|__u32
op_star
)paren
(paren
(paren
id|u8
op_star
)paren
(paren
id|des_keymap
op_plus
l_int|192
)paren
op_plus
(paren
l_int|0xFC
op_amp
id|z
)paren
)paren
suffix:semicolon
id|z
op_rshift_assign
l_int|8
suffix:semicolon
id|y
op_xor_assign
op_star
(paren
id|__u32
op_star
)paren
(paren
(paren
id|u8
op_star
)paren
(paren
id|des_keymap
op_plus
l_int|128
)paren
op_plus
(paren
l_int|0xFC
op_amp
id|z
)paren
)paren
suffix:semicolon
id|z
op_rshift_assign
l_int|8
suffix:semicolon
id|y
op_xor_assign
op_star
(paren
id|__u32
op_star
)paren
(paren
(paren
id|u8
op_star
)paren
(paren
id|des_keymap
op_plus
l_int|64
)paren
op_plus
(paren
l_int|0xFC
op_amp
id|z
)paren
)paren
suffix:semicolon
id|z
op_rshift_assign
l_int|8
suffix:semicolon
id|y
op_xor_assign
op_star
(paren
id|__u32
op_star
)paren
(paren
(paren
id|u8
op_star
)paren
id|des_keymap
op_plus
(paren
l_int|0xFC
op_amp
id|z
)paren
)paren
suffix:semicolon
id|z
op_assign
id|keyinfo
(braket
l_int|7
)braket
suffix:semicolon
id|z
op_xor_assign
id|x
suffix:semicolon
id|z
op_assign
id|z
op_lshift
l_int|4
op_or
id|z
op_rshift
l_int|28
suffix:semicolon
id|y
op_xor_assign
op_star
(paren
id|__u32
op_star
)paren
(paren
(paren
id|u8
op_star
)paren
(paren
id|des_keymap
op_plus
l_int|448
)paren
op_plus
(paren
l_int|0xFC
op_amp
id|z
)paren
)paren
suffix:semicolon
id|z
op_rshift_assign
l_int|8
suffix:semicolon
id|y
op_xor_assign
op_star
(paren
id|__u32
op_star
)paren
(paren
(paren
id|u8
op_star
)paren
(paren
id|des_keymap
op_plus
l_int|384
)paren
op_plus
(paren
l_int|0xFC
op_amp
id|z
)paren
)paren
suffix:semicolon
id|z
op_rshift_assign
l_int|8
suffix:semicolon
id|y
op_xor_assign
op_star
(paren
id|__u32
op_star
)paren
(paren
(paren
id|u8
op_star
)paren
(paren
id|des_keymap
op_plus
l_int|320
)paren
op_plus
(paren
l_int|0xFC
op_amp
id|z
)paren
)paren
suffix:semicolon
id|z
op_rshift_assign
l_int|8
suffix:semicolon
id|y
op_xor_assign
op_star
(paren
id|__u32
op_star
)paren
(paren
(paren
id|u8
op_star
)paren
(paren
id|des_keymap
op_plus
l_int|256
)paren
op_plus
(paren
l_int|0xFC
op_amp
id|z
)paren
)paren
suffix:semicolon
id|z
op_assign
id|keyinfo
(braket
l_int|8
)braket
suffix:semicolon
id|z
op_xor_assign
id|y
suffix:semicolon
id|x
op_xor_assign
op_star
(paren
id|__u32
op_star
)paren
(paren
(paren
id|u8
op_star
)paren
(paren
id|des_keymap
op_plus
l_int|192
)paren
op_plus
(paren
l_int|0xFC
op_amp
id|z
)paren
)paren
suffix:semicolon
id|z
op_rshift_assign
l_int|8
suffix:semicolon
id|x
op_xor_assign
op_star
(paren
id|__u32
op_star
)paren
(paren
(paren
id|u8
op_star
)paren
(paren
id|des_keymap
op_plus
l_int|128
)paren
op_plus
(paren
l_int|0xFC
op_amp
id|z
)paren
)paren
suffix:semicolon
id|z
op_rshift_assign
l_int|8
suffix:semicolon
id|x
op_xor_assign
op_star
(paren
id|__u32
op_star
)paren
(paren
(paren
id|u8
op_star
)paren
(paren
id|des_keymap
op_plus
l_int|64
)paren
op_plus
(paren
l_int|0xFC
op_amp
id|z
)paren
)paren
suffix:semicolon
id|z
op_rshift_assign
l_int|8
suffix:semicolon
id|x
op_xor_assign
op_star
(paren
id|__u32
op_star
)paren
(paren
(paren
id|u8
op_star
)paren
id|des_keymap
op_plus
(paren
l_int|0xFC
op_amp
id|z
)paren
)paren
suffix:semicolon
id|z
op_assign
id|keyinfo
(braket
l_int|9
)braket
suffix:semicolon
id|z
op_xor_assign
id|y
suffix:semicolon
id|z
op_assign
id|z
op_lshift
l_int|4
op_or
id|z
op_rshift
l_int|28
suffix:semicolon
id|x
op_xor_assign
op_star
(paren
id|__u32
op_star
)paren
(paren
(paren
id|u8
op_star
)paren
(paren
id|des_keymap
op_plus
l_int|448
)paren
op_plus
(paren
l_int|0xFC
op_amp
id|z
)paren
)paren
suffix:semicolon
id|z
op_rshift_assign
l_int|8
suffix:semicolon
id|x
op_xor_assign
op_star
(paren
id|__u32
op_star
)paren
(paren
(paren
id|u8
op_star
)paren
(paren
id|des_keymap
op_plus
l_int|384
)paren
op_plus
(paren
l_int|0xFC
op_amp
id|z
)paren
)paren
suffix:semicolon
id|z
op_rshift_assign
l_int|8
suffix:semicolon
id|x
op_xor_assign
op_star
(paren
id|__u32
op_star
)paren
(paren
(paren
id|u8
op_star
)paren
(paren
id|des_keymap
op_plus
l_int|320
)paren
op_plus
(paren
l_int|0xFC
op_amp
id|z
)paren
)paren
suffix:semicolon
id|z
op_rshift_assign
l_int|8
suffix:semicolon
id|x
op_xor_assign
op_star
(paren
id|__u32
op_star
)paren
(paren
(paren
id|u8
op_star
)paren
(paren
id|des_keymap
op_plus
l_int|256
)paren
op_plus
(paren
l_int|0xFC
op_amp
id|z
)paren
)paren
suffix:semicolon
id|z
op_assign
id|keyinfo
(braket
l_int|10
)braket
suffix:semicolon
id|z
op_xor_assign
id|x
suffix:semicolon
id|y
op_xor_assign
op_star
(paren
id|__u32
op_star
)paren
(paren
(paren
id|u8
op_star
)paren
(paren
id|des_keymap
op_plus
l_int|192
)paren
op_plus
(paren
l_int|0xFC
op_amp
id|z
)paren
)paren
suffix:semicolon
id|z
op_rshift_assign
l_int|8
suffix:semicolon
id|y
op_xor_assign
op_star
(paren
id|__u32
op_star
)paren
(paren
(paren
id|u8
op_star
)paren
(paren
id|des_keymap
op_plus
l_int|128
)paren
op_plus
(paren
l_int|0xFC
op_amp
id|z
)paren
)paren
suffix:semicolon
id|z
op_rshift_assign
l_int|8
suffix:semicolon
id|y
op_xor_assign
op_star
(paren
id|__u32
op_star
)paren
(paren
(paren
id|u8
op_star
)paren
(paren
id|des_keymap
op_plus
l_int|64
)paren
op_plus
(paren
l_int|0xFC
op_amp
id|z
)paren
)paren
suffix:semicolon
id|z
op_rshift_assign
l_int|8
suffix:semicolon
id|y
op_xor_assign
op_star
(paren
id|__u32
op_star
)paren
(paren
(paren
id|u8
op_star
)paren
id|des_keymap
op_plus
(paren
l_int|0xFC
op_amp
id|z
)paren
)paren
suffix:semicolon
id|z
op_assign
id|keyinfo
(braket
l_int|11
)braket
suffix:semicolon
id|z
op_xor_assign
id|x
suffix:semicolon
id|z
op_assign
id|z
op_lshift
l_int|4
op_or
id|z
op_rshift
l_int|28
suffix:semicolon
id|y
op_xor_assign
op_star
(paren
id|__u32
op_star
)paren
(paren
(paren
id|u8
op_star
)paren
(paren
id|des_keymap
op_plus
l_int|448
)paren
op_plus
(paren
l_int|0xFC
op_amp
id|z
)paren
)paren
suffix:semicolon
id|z
op_rshift_assign
l_int|8
suffix:semicolon
id|y
op_xor_assign
op_star
(paren
id|__u32
op_star
)paren
(paren
(paren
id|u8
op_star
)paren
(paren
id|des_keymap
op_plus
l_int|384
)paren
op_plus
(paren
l_int|0xFC
op_amp
id|z
)paren
)paren
suffix:semicolon
id|z
op_rshift_assign
l_int|8
suffix:semicolon
id|y
op_xor_assign
op_star
(paren
id|__u32
op_star
)paren
(paren
(paren
id|u8
op_star
)paren
(paren
id|des_keymap
op_plus
l_int|320
)paren
op_plus
(paren
l_int|0xFC
op_amp
id|z
)paren
)paren
suffix:semicolon
id|z
op_rshift_assign
l_int|8
suffix:semicolon
id|y
op_xor_assign
op_star
(paren
id|__u32
op_star
)paren
(paren
(paren
id|u8
op_star
)paren
(paren
id|des_keymap
op_plus
l_int|256
)paren
op_plus
(paren
l_int|0xFC
op_amp
id|z
)paren
)paren
suffix:semicolon
id|z
op_assign
id|keyinfo
(braket
l_int|12
)braket
suffix:semicolon
id|z
op_xor_assign
id|y
suffix:semicolon
id|x
op_xor_assign
op_star
(paren
id|__u32
op_star
)paren
(paren
(paren
id|u8
op_star
)paren
(paren
id|des_keymap
op_plus
l_int|192
)paren
op_plus
(paren
l_int|0xFC
op_amp
id|z
)paren
)paren
suffix:semicolon
id|z
op_rshift_assign
l_int|8
suffix:semicolon
id|x
op_xor_assign
op_star
(paren
id|__u32
op_star
)paren
(paren
(paren
id|u8
op_star
)paren
(paren
id|des_keymap
op_plus
l_int|128
)paren
op_plus
(paren
l_int|0xFC
op_amp
id|z
)paren
)paren
suffix:semicolon
id|z
op_rshift_assign
l_int|8
suffix:semicolon
id|x
op_xor_assign
op_star
(paren
id|__u32
op_star
)paren
(paren
(paren
id|u8
op_star
)paren
(paren
id|des_keymap
op_plus
l_int|64
)paren
op_plus
(paren
l_int|0xFC
op_amp
id|z
)paren
)paren
suffix:semicolon
id|z
op_rshift_assign
l_int|8
suffix:semicolon
id|x
op_xor_assign
op_star
(paren
id|__u32
op_star
)paren
(paren
(paren
id|u8
op_star
)paren
id|des_keymap
op_plus
(paren
l_int|0xFC
op_amp
id|z
)paren
)paren
suffix:semicolon
id|z
op_assign
id|keyinfo
(braket
l_int|13
)braket
suffix:semicolon
id|z
op_xor_assign
id|y
suffix:semicolon
id|z
op_assign
id|z
op_lshift
l_int|4
op_or
id|z
op_rshift
l_int|28
suffix:semicolon
id|x
op_xor_assign
op_star
(paren
id|__u32
op_star
)paren
(paren
(paren
id|u8
op_star
)paren
(paren
id|des_keymap
op_plus
l_int|448
)paren
op_plus
(paren
l_int|0xFC
op_amp
id|z
)paren
)paren
suffix:semicolon
id|z
op_rshift_assign
l_int|8
suffix:semicolon
id|x
op_xor_assign
op_star
(paren
id|__u32
op_star
)paren
(paren
(paren
id|u8
op_star
)paren
(paren
id|des_keymap
op_plus
l_int|384
)paren
op_plus
(paren
l_int|0xFC
op_amp
id|z
)paren
)paren
suffix:semicolon
id|z
op_rshift_assign
l_int|8
suffix:semicolon
id|x
op_xor_assign
op_star
(paren
id|__u32
op_star
)paren
(paren
(paren
id|u8
op_star
)paren
(paren
id|des_keymap
op_plus
l_int|320
)paren
op_plus
(paren
l_int|0xFC
op_amp
id|z
)paren
)paren
suffix:semicolon
id|z
op_rshift_assign
l_int|8
suffix:semicolon
id|x
op_xor_assign
op_star
(paren
id|__u32
op_star
)paren
(paren
(paren
id|u8
op_star
)paren
(paren
id|des_keymap
op_plus
l_int|256
)paren
op_plus
(paren
l_int|0xFC
op_amp
id|z
)paren
)paren
suffix:semicolon
id|z
op_assign
id|keyinfo
(braket
l_int|14
)braket
suffix:semicolon
id|z
op_xor_assign
id|x
suffix:semicolon
id|y
op_xor_assign
op_star
(paren
id|__u32
op_star
)paren
(paren
(paren
id|u8
op_star
)paren
(paren
id|des_keymap
op_plus
l_int|192
)paren
op_plus
(paren
l_int|0xFC
op_amp
id|z
)paren
)paren
suffix:semicolon
id|z
op_rshift_assign
l_int|8
suffix:semicolon
id|y
op_xor_assign
op_star
(paren
id|__u32
op_star
)paren
(paren
(paren
id|u8
op_star
)paren
(paren
id|des_keymap
op_plus
l_int|128
)paren
op_plus
(paren
l_int|0xFC
op_amp
id|z
)paren
)paren
suffix:semicolon
id|z
op_rshift_assign
l_int|8
suffix:semicolon
id|y
op_xor_assign
op_star
(paren
id|__u32
op_star
)paren
(paren
(paren
id|u8
op_star
)paren
(paren
id|des_keymap
op_plus
l_int|64
)paren
op_plus
(paren
l_int|0xFC
op_amp
id|z
)paren
)paren
suffix:semicolon
id|z
op_rshift_assign
l_int|8
suffix:semicolon
id|y
op_xor_assign
op_star
(paren
id|__u32
op_star
)paren
(paren
(paren
id|u8
op_star
)paren
id|des_keymap
op_plus
(paren
l_int|0xFC
op_amp
id|z
)paren
)paren
suffix:semicolon
id|z
op_assign
id|keyinfo
(braket
l_int|15
)braket
suffix:semicolon
id|z
op_xor_assign
id|x
suffix:semicolon
id|z
op_assign
id|z
op_lshift
l_int|4
op_or
id|z
op_rshift
l_int|28
suffix:semicolon
id|y
op_xor_assign
op_star
(paren
id|__u32
op_star
)paren
(paren
(paren
id|u8
op_star
)paren
(paren
id|des_keymap
op_plus
l_int|448
)paren
op_plus
(paren
l_int|0xFC
op_amp
id|z
)paren
)paren
suffix:semicolon
id|z
op_rshift_assign
l_int|8
suffix:semicolon
id|y
op_xor_assign
op_star
(paren
id|__u32
op_star
)paren
(paren
(paren
id|u8
op_star
)paren
(paren
id|des_keymap
op_plus
l_int|384
)paren
op_plus
(paren
l_int|0xFC
op_amp
id|z
)paren
)paren
suffix:semicolon
id|z
op_rshift_assign
l_int|8
suffix:semicolon
id|y
op_xor_assign
op_star
(paren
id|__u32
op_star
)paren
(paren
(paren
id|u8
op_star
)paren
(paren
id|des_keymap
op_plus
l_int|320
)paren
op_plus
(paren
l_int|0xFC
op_amp
id|z
)paren
)paren
suffix:semicolon
id|z
op_rshift_assign
l_int|8
suffix:semicolon
id|y
op_xor_assign
op_star
(paren
id|__u32
op_star
)paren
(paren
(paren
id|u8
op_star
)paren
(paren
id|des_keymap
op_plus
l_int|256
)paren
op_plus
(paren
l_int|0xFC
op_amp
id|z
)paren
)paren
suffix:semicolon
id|z
op_assign
id|keyinfo
(braket
l_int|16
)braket
suffix:semicolon
id|z
op_xor_assign
id|y
suffix:semicolon
id|x
op_xor_assign
op_star
(paren
id|__u32
op_star
)paren
(paren
(paren
id|u8
op_star
)paren
(paren
id|des_keymap
op_plus
l_int|192
)paren
op_plus
(paren
l_int|0xFC
op_amp
id|z
)paren
)paren
suffix:semicolon
id|z
op_rshift_assign
l_int|8
suffix:semicolon
id|x
op_xor_assign
op_star
(paren
id|__u32
op_star
)paren
(paren
(paren
id|u8
op_star
)paren
(paren
id|des_keymap
op_plus
l_int|128
)paren
op_plus
(paren
l_int|0xFC
op_amp
id|z
)paren
)paren
suffix:semicolon
id|z
op_rshift_assign
l_int|8
suffix:semicolon
id|x
op_xor_assign
op_star
(paren
id|__u32
op_star
)paren
(paren
(paren
id|u8
op_star
)paren
(paren
id|des_keymap
op_plus
l_int|64
)paren
op_plus
(paren
l_int|0xFC
op_amp
id|z
)paren
)paren
suffix:semicolon
id|z
op_rshift_assign
l_int|8
suffix:semicolon
id|x
op_xor_assign
op_star
(paren
id|__u32
op_star
)paren
(paren
(paren
id|u8
op_star
)paren
id|des_keymap
op_plus
(paren
l_int|0xFC
op_amp
id|z
)paren
)paren
suffix:semicolon
id|z
op_assign
id|keyinfo
(braket
l_int|17
)braket
suffix:semicolon
id|z
op_xor_assign
id|y
suffix:semicolon
id|z
op_assign
id|z
op_lshift
l_int|4
op_or
id|z
op_rshift
l_int|28
suffix:semicolon
id|x
op_xor_assign
op_star
(paren
id|__u32
op_star
)paren
(paren
(paren
id|u8
op_star
)paren
(paren
id|des_keymap
op_plus
l_int|448
)paren
op_plus
(paren
l_int|0xFC
op_amp
id|z
)paren
)paren
suffix:semicolon
id|z
op_rshift_assign
l_int|8
suffix:semicolon
id|x
op_xor_assign
op_star
(paren
id|__u32
op_star
)paren
(paren
(paren
id|u8
op_star
)paren
(paren
id|des_keymap
op_plus
l_int|384
)paren
op_plus
(paren
l_int|0xFC
op_amp
id|z
)paren
)paren
suffix:semicolon
id|z
op_rshift_assign
l_int|8
suffix:semicolon
id|x
op_xor_assign
op_star
(paren
id|__u32
op_star
)paren
(paren
(paren
id|u8
op_star
)paren
(paren
id|des_keymap
op_plus
l_int|320
)paren
op_plus
(paren
l_int|0xFC
op_amp
id|z
)paren
)paren
suffix:semicolon
id|z
op_rshift_assign
l_int|8
suffix:semicolon
id|x
op_xor_assign
op_star
(paren
id|__u32
op_star
)paren
(paren
(paren
id|u8
op_star
)paren
(paren
id|des_keymap
op_plus
l_int|256
)paren
op_plus
(paren
l_int|0xFC
op_amp
id|z
)paren
)paren
suffix:semicolon
id|z
op_assign
id|keyinfo
(braket
l_int|18
)braket
suffix:semicolon
id|z
op_xor_assign
id|x
suffix:semicolon
id|y
op_xor_assign
op_star
(paren
id|__u32
op_star
)paren
(paren
(paren
id|u8
op_star
)paren
(paren
id|des_keymap
op_plus
l_int|192
)paren
op_plus
(paren
l_int|0xFC
op_amp
id|z
)paren
)paren
suffix:semicolon
id|z
op_rshift_assign
l_int|8
suffix:semicolon
id|y
op_xor_assign
op_star
(paren
id|__u32
op_star
)paren
(paren
(paren
id|u8
op_star
)paren
(paren
id|des_keymap
op_plus
l_int|128
)paren
op_plus
(paren
l_int|0xFC
op_amp
id|z
)paren
)paren
suffix:semicolon
id|z
op_rshift_assign
l_int|8
suffix:semicolon
id|y
op_xor_assign
op_star
(paren
id|__u32
op_star
)paren
(paren
(paren
id|u8
op_star
)paren
(paren
id|des_keymap
op_plus
l_int|64
)paren
op_plus
(paren
l_int|0xFC
op_amp
id|z
)paren
)paren
suffix:semicolon
id|z
op_rshift_assign
l_int|8
suffix:semicolon
id|y
op_xor_assign
op_star
(paren
id|__u32
op_star
)paren
(paren
(paren
id|u8
op_star
)paren
id|des_keymap
op_plus
(paren
l_int|0xFC
op_amp
id|z
)paren
)paren
suffix:semicolon
id|z
op_assign
id|keyinfo
(braket
l_int|19
)braket
suffix:semicolon
id|z
op_xor_assign
id|x
suffix:semicolon
id|z
op_assign
id|z
op_lshift
l_int|4
op_or
id|z
op_rshift
l_int|28
suffix:semicolon
id|y
op_xor_assign
op_star
(paren
id|__u32
op_star
)paren
(paren
(paren
id|u8
op_star
)paren
(paren
id|des_keymap
op_plus
l_int|448
)paren
op_plus
(paren
l_int|0xFC
op_amp
id|z
)paren
)paren
suffix:semicolon
id|z
op_rshift_assign
l_int|8
suffix:semicolon
id|y
op_xor_assign
op_star
(paren
id|__u32
op_star
)paren
(paren
(paren
id|u8
op_star
)paren
(paren
id|des_keymap
op_plus
l_int|384
)paren
op_plus
(paren
l_int|0xFC
op_amp
id|z
)paren
)paren
suffix:semicolon
id|z
op_rshift_assign
l_int|8
suffix:semicolon
id|y
op_xor_assign
op_star
(paren
id|__u32
op_star
)paren
(paren
(paren
id|u8
op_star
)paren
(paren
id|des_keymap
op_plus
l_int|320
)paren
op_plus
(paren
l_int|0xFC
op_amp
id|z
)paren
)paren
suffix:semicolon
id|z
op_rshift_assign
l_int|8
suffix:semicolon
id|y
op_xor_assign
op_star
(paren
id|__u32
op_star
)paren
(paren
(paren
id|u8
op_star
)paren
(paren
id|des_keymap
op_plus
l_int|256
)paren
op_plus
(paren
l_int|0xFC
op_amp
id|z
)paren
)paren
suffix:semicolon
id|z
op_assign
id|keyinfo
(braket
l_int|20
)braket
suffix:semicolon
id|z
op_xor_assign
id|y
suffix:semicolon
id|x
op_xor_assign
op_star
(paren
id|__u32
op_star
)paren
(paren
(paren
id|u8
op_star
)paren
(paren
id|des_keymap
op_plus
l_int|192
)paren
op_plus
(paren
l_int|0xFC
op_amp
id|z
)paren
)paren
suffix:semicolon
id|z
op_rshift_assign
l_int|8
suffix:semicolon
id|x
op_xor_assign
op_star
(paren
id|__u32
op_star
)paren
(paren
(paren
id|u8
op_star
)paren
(paren
id|des_keymap
op_plus
l_int|128
)paren
op_plus
(paren
l_int|0xFC
op_amp
id|z
)paren
)paren
suffix:semicolon
id|z
op_rshift_assign
l_int|8
suffix:semicolon
id|x
op_xor_assign
op_star
(paren
id|__u32
op_star
)paren
(paren
(paren
id|u8
op_star
)paren
(paren
id|des_keymap
op_plus
l_int|64
)paren
op_plus
(paren
l_int|0xFC
op_amp
id|z
)paren
)paren
suffix:semicolon
id|z
op_rshift_assign
l_int|8
suffix:semicolon
id|x
op_xor_assign
op_star
(paren
id|__u32
op_star
)paren
(paren
(paren
id|u8
op_star
)paren
id|des_keymap
op_plus
(paren
l_int|0xFC
op_amp
id|z
)paren
)paren
suffix:semicolon
id|z
op_assign
id|keyinfo
(braket
l_int|21
)braket
suffix:semicolon
id|z
op_xor_assign
id|y
suffix:semicolon
id|z
op_assign
id|z
op_lshift
l_int|4
op_or
id|z
op_rshift
l_int|28
suffix:semicolon
id|x
op_xor_assign
op_star
(paren
id|__u32
op_star
)paren
(paren
(paren
id|u8
op_star
)paren
(paren
id|des_keymap
op_plus
l_int|448
)paren
op_plus
(paren
l_int|0xFC
op_amp
id|z
)paren
)paren
suffix:semicolon
id|z
op_rshift_assign
l_int|8
suffix:semicolon
id|x
op_xor_assign
op_star
(paren
id|__u32
op_star
)paren
(paren
(paren
id|u8
op_star
)paren
(paren
id|des_keymap
op_plus
l_int|384
)paren
op_plus
(paren
l_int|0xFC
op_amp
id|z
)paren
)paren
suffix:semicolon
id|z
op_rshift_assign
l_int|8
suffix:semicolon
id|x
op_xor_assign
op_star
(paren
id|__u32
op_star
)paren
(paren
(paren
id|u8
op_star
)paren
(paren
id|des_keymap
op_plus
l_int|320
)paren
op_plus
(paren
l_int|0xFC
op_amp
id|z
)paren
)paren
suffix:semicolon
id|z
op_rshift_assign
l_int|8
suffix:semicolon
id|x
op_xor_assign
op_star
(paren
id|__u32
op_star
)paren
(paren
(paren
id|u8
op_star
)paren
(paren
id|des_keymap
op_plus
l_int|256
)paren
op_plus
(paren
l_int|0xFC
op_amp
id|z
)paren
)paren
suffix:semicolon
id|z
op_assign
id|keyinfo
(braket
l_int|22
)braket
suffix:semicolon
id|z
op_xor_assign
id|x
suffix:semicolon
id|y
op_xor_assign
op_star
(paren
id|__u32
op_star
)paren
(paren
(paren
id|u8
op_star
)paren
(paren
id|des_keymap
op_plus
l_int|192
)paren
op_plus
(paren
l_int|0xFC
op_amp
id|z
)paren
)paren
suffix:semicolon
id|z
op_rshift_assign
l_int|8
suffix:semicolon
id|y
op_xor_assign
op_star
(paren
id|__u32
op_star
)paren
(paren
(paren
id|u8
op_star
)paren
(paren
id|des_keymap
op_plus
l_int|128
)paren
op_plus
(paren
l_int|0xFC
op_amp
id|z
)paren
)paren
suffix:semicolon
id|z
op_rshift_assign
l_int|8
suffix:semicolon
id|y
op_xor_assign
op_star
(paren
id|__u32
op_star
)paren
(paren
(paren
id|u8
op_star
)paren
(paren
id|des_keymap
op_plus
l_int|64
)paren
op_plus
(paren
l_int|0xFC
op_amp
id|z
)paren
)paren
suffix:semicolon
id|z
op_rshift_assign
l_int|8
suffix:semicolon
id|y
op_xor_assign
op_star
(paren
id|__u32
op_star
)paren
(paren
(paren
id|u8
op_star
)paren
id|des_keymap
op_plus
(paren
l_int|0xFC
op_amp
id|z
)paren
)paren
suffix:semicolon
id|z
op_assign
id|keyinfo
(braket
l_int|23
)braket
suffix:semicolon
id|z
op_xor_assign
id|x
suffix:semicolon
id|z
op_assign
id|z
op_lshift
l_int|4
op_or
id|z
op_rshift
l_int|28
suffix:semicolon
id|y
op_xor_assign
op_star
(paren
id|__u32
op_star
)paren
(paren
(paren
id|u8
op_star
)paren
(paren
id|des_keymap
op_plus
l_int|448
)paren
op_plus
(paren
l_int|0xFC
op_amp
id|z
)paren
)paren
suffix:semicolon
id|z
op_rshift_assign
l_int|8
suffix:semicolon
id|y
op_xor_assign
op_star
(paren
id|__u32
op_star
)paren
(paren
(paren
id|u8
op_star
)paren
(paren
id|des_keymap
op_plus
l_int|384
)paren
op_plus
(paren
l_int|0xFC
op_amp
id|z
)paren
)paren
suffix:semicolon
id|z
op_rshift_assign
l_int|8
suffix:semicolon
id|y
op_xor_assign
op_star
(paren
id|__u32
op_star
)paren
(paren
(paren
id|u8
op_star
)paren
(paren
id|des_keymap
op_plus
l_int|320
)paren
op_plus
(paren
l_int|0xFC
op_amp
id|z
)paren
)paren
suffix:semicolon
id|z
op_rshift_assign
l_int|8
suffix:semicolon
id|y
op_xor_assign
op_star
(paren
id|__u32
op_star
)paren
(paren
(paren
id|u8
op_star
)paren
(paren
id|des_keymap
op_plus
l_int|256
)paren
op_plus
(paren
l_int|0xFC
op_amp
id|z
)paren
)paren
suffix:semicolon
id|z
op_assign
id|keyinfo
(braket
l_int|24
)braket
suffix:semicolon
id|z
op_xor_assign
id|y
suffix:semicolon
id|x
op_xor_assign
op_star
(paren
id|__u32
op_star
)paren
(paren
(paren
id|u8
op_star
)paren
(paren
id|des_keymap
op_plus
l_int|192
)paren
op_plus
(paren
l_int|0xFC
op_amp
id|z
)paren
)paren
suffix:semicolon
id|z
op_rshift_assign
l_int|8
suffix:semicolon
id|x
op_xor_assign
op_star
(paren
id|__u32
op_star
)paren
(paren
(paren
id|u8
op_star
)paren
(paren
id|des_keymap
op_plus
l_int|128
)paren
op_plus
(paren
l_int|0xFC
op_amp
id|z
)paren
)paren
suffix:semicolon
id|z
op_rshift_assign
l_int|8
suffix:semicolon
id|x
op_xor_assign
op_star
(paren
id|__u32
op_star
)paren
(paren
(paren
id|u8
op_star
)paren
(paren
id|des_keymap
op_plus
l_int|64
)paren
op_plus
(paren
l_int|0xFC
op_amp
id|z
)paren
)paren
suffix:semicolon
id|z
op_rshift_assign
l_int|8
suffix:semicolon
id|x
op_xor_assign
op_star
(paren
id|__u32
op_star
)paren
(paren
(paren
id|u8
op_star
)paren
id|des_keymap
op_plus
(paren
l_int|0xFC
op_amp
id|z
)paren
)paren
suffix:semicolon
id|z
op_assign
id|keyinfo
(braket
l_int|25
)braket
suffix:semicolon
id|z
op_xor_assign
id|y
suffix:semicolon
id|z
op_assign
id|z
op_lshift
l_int|4
op_or
id|z
op_rshift
l_int|28
suffix:semicolon
id|x
op_xor_assign
op_star
(paren
id|__u32
op_star
)paren
(paren
(paren
id|u8
op_star
)paren
(paren
id|des_keymap
op_plus
l_int|448
)paren
op_plus
(paren
l_int|0xFC
op_amp
id|z
)paren
)paren
suffix:semicolon
id|z
op_rshift_assign
l_int|8
suffix:semicolon
id|x
op_xor_assign
op_star
(paren
id|__u32
op_star
)paren
(paren
(paren
id|u8
op_star
)paren
(paren
id|des_keymap
op_plus
l_int|384
)paren
op_plus
(paren
l_int|0xFC
op_amp
id|z
)paren
)paren
suffix:semicolon
id|z
op_rshift_assign
l_int|8
suffix:semicolon
id|x
op_xor_assign
op_star
(paren
id|__u32
op_star
)paren
(paren
(paren
id|u8
op_star
)paren
(paren
id|des_keymap
op_plus
l_int|320
)paren
op_plus
(paren
l_int|0xFC
op_amp
id|z
)paren
)paren
suffix:semicolon
id|z
op_rshift_assign
l_int|8
suffix:semicolon
id|x
op_xor_assign
op_star
(paren
id|__u32
op_star
)paren
(paren
(paren
id|u8
op_star
)paren
(paren
id|des_keymap
op_plus
l_int|256
)paren
op_plus
(paren
l_int|0xFC
op_amp
id|z
)paren
)paren
suffix:semicolon
id|z
op_assign
id|keyinfo
(braket
l_int|26
)braket
suffix:semicolon
id|z
op_xor_assign
id|x
suffix:semicolon
id|y
op_xor_assign
op_star
(paren
id|__u32
op_star
)paren
(paren
(paren
id|u8
op_star
)paren
(paren
id|des_keymap
op_plus
l_int|192
)paren
op_plus
(paren
l_int|0xFC
op_amp
id|z
)paren
)paren
suffix:semicolon
id|z
op_rshift_assign
l_int|8
suffix:semicolon
id|y
op_xor_assign
op_star
(paren
id|__u32
op_star
)paren
(paren
(paren
id|u8
op_star
)paren
(paren
id|des_keymap
op_plus
l_int|128
)paren
op_plus
(paren
l_int|0xFC
op_amp
id|z
)paren
)paren
suffix:semicolon
id|z
op_rshift_assign
l_int|8
suffix:semicolon
id|y
op_xor_assign
op_star
(paren
id|__u32
op_star
)paren
(paren
(paren
id|u8
op_star
)paren
(paren
id|des_keymap
op_plus
l_int|64
)paren
op_plus
(paren
l_int|0xFC
op_amp
id|z
)paren
)paren
suffix:semicolon
id|z
op_rshift_assign
l_int|8
suffix:semicolon
id|y
op_xor_assign
op_star
(paren
id|__u32
op_star
)paren
(paren
(paren
id|u8
op_star
)paren
id|des_keymap
op_plus
(paren
l_int|0xFC
op_amp
id|z
)paren
)paren
suffix:semicolon
id|z
op_assign
id|keyinfo
(braket
l_int|27
)braket
suffix:semicolon
id|z
op_xor_assign
id|x
suffix:semicolon
id|z
op_assign
id|z
op_lshift
l_int|4
op_or
id|z
op_rshift
l_int|28
suffix:semicolon
id|y
op_xor_assign
op_star
(paren
id|__u32
op_star
)paren
(paren
(paren
id|u8
op_star
)paren
(paren
id|des_keymap
op_plus
l_int|448
)paren
op_plus
(paren
l_int|0xFC
op_amp
id|z
)paren
)paren
suffix:semicolon
id|z
op_rshift_assign
l_int|8
suffix:semicolon
id|y
op_xor_assign
op_star
(paren
id|__u32
op_star
)paren
(paren
(paren
id|u8
op_star
)paren
(paren
id|des_keymap
op_plus
l_int|384
)paren
op_plus
(paren
l_int|0xFC
op_amp
id|z
)paren
)paren
suffix:semicolon
id|z
op_rshift_assign
l_int|8
suffix:semicolon
id|y
op_xor_assign
op_star
(paren
id|__u32
op_star
)paren
(paren
(paren
id|u8
op_star
)paren
(paren
id|des_keymap
op_plus
l_int|320
)paren
op_plus
(paren
l_int|0xFC
op_amp
id|z
)paren
)paren
suffix:semicolon
id|z
op_rshift_assign
l_int|8
suffix:semicolon
id|y
op_xor_assign
op_star
(paren
id|__u32
op_star
)paren
(paren
(paren
id|u8
op_star
)paren
(paren
id|des_keymap
op_plus
l_int|256
)paren
op_plus
(paren
l_int|0xFC
op_amp
id|z
)paren
)paren
suffix:semicolon
id|z
op_assign
id|keyinfo
(braket
l_int|28
)braket
suffix:semicolon
id|z
op_xor_assign
id|y
suffix:semicolon
id|x
op_xor_assign
op_star
(paren
id|__u32
op_star
)paren
(paren
(paren
id|u8
op_star
)paren
(paren
id|des_keymap
op_plus
l_int|192
)paren
op_plus
(paren
l_int|0xFC
op_amp
id|z
)paren
)paren
suffix:semicolon
id|z
op_rshift_assign
l_int|8
suffix:semicolon
id|x
op_xor_assign
op_star
(paren
id|__u32
op_star
)paren
(paren
(paren
id|u8
op_star
)paren
(paren
id|des_keymap
op_plus
l_int|128
)paren
op_plus
(paren
l_int|0xFC
op_amp
id|z
)paren
)paren
suffix:semicolon
id|z
op_rshift_assign
l_int|8
suffix:semicolon
id|x
op_xor_assign
op_star
(paren
id|__u32
op_star
)paren
(paren
(paren
id|u8
op_star
)paren
(paren
id|des_keymap
op_plus
l_int|64
)paren
op_plus
(paren
l_int|0xFC
op_amp
id|z
)paren
)paren
suffix:semicolon
id|z
op_rshift_assign
l_int|8
suffix:semicolon
id|x
op_xor_assign
op_star
(paren
id|__u32
op_star
)paren
(paren
(paren
id|u8
op_star
)paren
id|des_keymap
op_plus
(paren
l_int|0xFC
op_amp
id|z
)paren
)paren
suffix:semicolon
id|z
op_assign
id|keyinfo
(braket
l_int|29
)braket
suffix:semicolon
id|z
op_xor_assign
id|y
suffix:semicolon
id|z
op_assign
id|z
op_lshift
l_int|4
op_or
id|z
op_rshift
l_int|28
suffix:semicolon
id|x
op_xor_assign
op_star
(paren
id|__u32
op_star
)paren
(paren
(paren
id|u8
op_star
)paren
(paren
id|des_keymap
op_plus
l_int|448
)paren
op_plus
(paren
l_int|0xFC
op_amp
id|z
)paren
)paren
suffix:semicolon
id|z
op_rshift_assign
l_int|8
suffix:semicolon
id|x
op_xor_assign
op_star
(paren
id|__u32
op_star
)paren
(paren
(paren
id|u8
op_star
)paren
(paren
id|des_keymap
op_plus
l_int|384
)paren
op_plus
(paren
l_int|0xFC
op_amp
id|z
)paren
)paren
suffix:semicolon
id|z
op_rshift_assign
l_int|8
suffix:semicolon
id|x
op_xor_assign
op_star
(paren
id|__u32
op_star
)paren
(paren
(paren
id|u8
op_star
)paren
(paren
id|des_keymap
op_plus
l_int|320
)paren
op_plus
(paren
l_int|0xFC
op_amp
id|z
)paren
)paren
suffix:semicolon
id|z
op_rshift_assign
l_int|8
suffix:semicolon
id|x
op_xor_assign
op_star
(paren
id|__u32
op_star
)paren
(paren
(paren
id|u8
op_star
)paren
(paren
id|des_keymap
op_plus
l_int|256
)paren
op_plus
(paren
l_int|0xFC
op_amp
id|z
)paren
)paren
suffix:semicolon
id|z
op_assign
id|keyinfo
(braket
l_int|30
)braket
suffix:semicolon
id|z
op_xor_assign
id|x
suffix:semicolon
id|y
op_xor_assign
op_star
(paren
id|__u32
op_star
)paren
(paren
(paren
id|u8
op_star
)paren
(paren
id|des_keymap
op_plus
l_int|192
)paren
op_plus
(paren
l_int|0xFC
op_amp
id|z
)paren
)paren
suffix:semicolon
id|z
op_rshift_assign
l_int|8
suffix:semicolon
id|y
op_xor_assign
op_star
(paren
id|__u32
op_star
)paren
(paren
(paren
id|u8
op_star
)paren
(paren
id|des_keymap
op_plus
l_int|128
)paren
op_plus
(paren
l_int|0xFC
op_amp
id|z
)paren
)paren
suffix:semicolon
id|z
op_rshift_assign
l_int|8
suffix:semicolon
id|y
op_xor_assign
op_star
(paren
id|__u32
op_star
)paren
(paren
(paren
id|u8
op_star
)paren
(paren
id|des_keymap
op_plus
l_int|64
)paren
op_plus
(paren
l_int|0xFC
op_amp
id|z
)paren
)paren
suffix:semicolon
id|z
op_rshift_assign
l_int|8
suffix:semicolon
id|y
op_xor_assign
op_star
(paren
id|__u32
op_star
)paren
(paren
(paren
id|u8
op_star
)paren
id|des_keymap
op_plus
(paren
l_int|0xFC
op_amp
id|z
)paren
)paren
suffix:semicolon
id|z
op_assign
id|keyinfo
(braket
l_int|31
)braket
suffix:semicolon
id|z
op_xor_assign
id|x
suffix:semicolon
id|z
op_assign
id|z
op_lshift
l_int|4
op_or
id|z
op_rshift
l_int|28
suffix:semicolon
id|y
op_xor_assign
op_star
(paren
id|__u32
op_star
)paren
(paren
(paren
id|u8
op_star
)paren
(paren
id|des_keymap
op_plus
l_int|448
)paren
op_plus
(paren
l_int|0xFC
op_amp
id|z
)paren
)paren
suffix:semicolon
id|z
op_rshift_assign
l_int|8
suffix:semicolon
id|y
op_xor_assign
op_star
(paren
id|__u32
op_star
)paren
(paren
(paren
id|u8
op_star
)paren
(paren
id|des_keymap
op_plus
l_int|384
)paren
op_plus
(paren
l_int|0xFC
op_amp
id|z
)paren
)paren
suffix:semicolon
id|z
op_rshift_assign
l_int|8
suffix:semicolon
id|y
op_xor_assign
op_star
(paren
id|__u32
op_star
)paren
(paren
(paren
id|u8
op_star
)paren
(paren
id|des_keymap
op_plus
l_int|320
)paren
op_plus
(paren
l_int|0xFC
op_amp
id|z
)paren
)paren
suffix:semicolon
id|z
op_rshift_assign
l_int|8
suffix:semicolon
id|y
op_xor_assign
op_star
(paren
id|__u32
op_star
)paren
(paren
(paren
id|u8
op_star
)paren
(paren
id|des_keymap
op_plus
l_int|256
)paren
op_plus
(paren
l_int|0xFC
op_amp
id|z
)paren
)paren
suffix:semicolon
id|x
op_assign
id|x
op_lshift
l_int|1
op_or
id|x
op_rshift
l_int|31
suffix:semicolon
id|z
op_assign
(paren
id|x
op_xor
id|y
)paren
op_amp
l_int|0x55555555L
suffix:semicolon
id|y
op_xor_assign
id|z
suffix:semicolon
id|x
op_xor_assign
id|z
suffix:semicolon
id|y
op_assign
id|y
op_lshift
l_int|1
op_or
id|y
op_rshift
l_int|31
suffix:semicolon
id|z
op_assign
(paren
(paren
id|x
op_rshift
l_int|010
)paren
op_xor
id|y
)paren
op_amp
l_int|0x00FF00FFL
suffix:semicolon
id|x
op_xor_assign
id|z
op_lshift
l_int|010
suffix:semicolon
id|y
op_xor_assign
id|z
suffix:semicolon
id|z
op_assign
(paren
(paren
id|y
op_rshift
l_int|002
)paren
op_xor
id|x
)paren
op_amp
l_int|0x33333333L
suffix:semicolon
id|y
op_xor_assign
id|z
op_lshift
l_int|002
suffix:semicolon
id|x
op_xor_assign
id|z
suffix:semicolon
id|z
op_assign
(paren
(paren
id|x
op_rshift
l_int|020
)paren
op_xor
id|y
)paren
op_amp
l_int|0x0000FFFFL
suffix:semicolon
id|x
op_xor_assign
id|z
op_lshift
l_int|020
suffix:semicolon
id|y
op_xor_assign
id|z
suffix:semicolon
id|z
op_assign
(paren
(paren
id|y
op_rshift
l_int|004
)paren
op_xor
id|x
)paren
op_amp
l_int|0x0F0F0F0FL
suffix:semicolon
id|y
op_xor_assign
id|z
op_lshift
l_int|004
suffix:semicolon
id|x
op_xor_assign
id|z
suffix:semicolon
id|dst
(braket
l_int|0
)braket
op_assign
id|x
suffix:semicolon
id|x
op_rshift_assign
l_int|8
suffix:semicolon
id|dst
(braket
l_int|1
)braket
op_assign
id|x
suffix:semicolon
id|x
op_rshift_assign
l_int|8
suffix:semicolon
id|dst
(braket
l_int|2
)braket
op_assign
id|x
suffix:semicolon
id|x
op_rshift_assign
l_int|8
suffix:semicolon
id|dst
(braket
l_int|3
)braket
op_assign
id|x
suffix:semicolon
id|dst
(braket
l_int|4
)braket
op_assign
id|y
suffix:semicolon
id|y
op_rshift_assign
l_int|8
suffix:semicolon
id|dst
(braket
l_int|5
)braket
op_assign
id|y
suffix:semicolon
id|y
op_rshift_assign
l_int|8
suffix:semicolon
id|dst
(braket
l_int|6
)braket
op_assign
id|y
suffix:semicolon
id|y
op_rshift_assign
l_int|8
suffix:semicolon
id|dst
(braket
l_int|7
)braket
op_assign
id|y
suffix:semicolon
r_return
suffix:semicolon
)brace
DECL|function|des_small_fips_decrypt
r_static
r_void
id|des_small_fips_decrypt
c_func
(paren
r_void
op_star
id|ctx
comma
id|__u8
op_star
id|dst
comma
id|__u8
op_star
id|src
)paren
(brace
id|__u32
op_star
id|keyinfo
op_assign
(paren
(paren
r_struct
id|des_ctx
op_star
)paren
id|ctx
)paren
op_member_access_from_pointer
id|keyinfo
suffix:semicolon
id|__u32
id|x
comma
id|y
comma
id|z
suffix:semicolon
id|x
op_assign
id|src
(braket
l_int|7
)braket
suffix:semicolon
id|x
op_lshift_assign
l_int|8
suffix:semicolon
id|x
op_or_assign
id|src
(braket
l_int|6
)braket
suffix:semicolon
id|x
op_lshift_assign
l_int|8
suffix:semicolon
id|x
op_or_assign
id|src
(braket
l_int|5
)braket
suffix:semicolon
id|x
op_lshift_assign
l_int|8
suffix:semicolon
id|x
op_or_assign
id|src
(braket
l_int|4
)braket
suffix:semicolon
id|y
op_assign
id|src
(braket
l_int|3
)braket
suffix:semicolon
id|y
op_lshift_assign
l_int|8
suffix:semicolon
id|y
op_or_assign
id|src
(braket
l_int|2
)braket
suffix:semicolon
id|y
op_lshift_assign
l_int|8
suffix:semicolon
id|y
op_or_assign
id|src
(braket
l_int|1
)braket
suffix:semicolon
id|y
op_lshift_assign
l_int|8
suffix:semicolon
id|y
op_or_assign
id|src
(braket
l_int|0
)braket
suffix:semicolon
id|z
op_assign
(paren
(paren
id|x
op_rshift
l_int|004
)paren
op_xor
id|y
)paren
op_amp
l_int|0x0F0F0F0FL
suffix:semicolon
id|x
op_xor_assign
id|z
op_lshift
l_int|004
suffix:semicolon
id|y
op_xor_assign
id|z
suffix:semicolon
id|z
op_assign
(paren
(paren
id|y
op_rshift
l_int|020
)paren
op_xor
id|x
)paren
op_amp
l_int|0x0000FFFFL
suffix:semicolon
id|y
op_xor_assign
id|z
op_lshift
l_int|020
suffix:semicolon
id|x
op_xor_assign
id|z
suffix:semicolon
id|z
op_assign
(paren
(paren
id|x
op_rshift
l_int|002
)paren
op_xor
id|y
)paren
op_amp
l_int|0x33333333L
suffix:semicolon
id|x
op_xor_assign
id|z
op_lshift
l_int|002
suffix:semicolon
id|y
op_xor_assign
id|z
suffix:semicolon
id|z
op_assign
(paren
(paren
id|y
op_rshift
l_int|010
)paren
op_xor
id|x
)paren
op_amp
l_int|0x00FF00FFL
suffix:semicolon
id|y
op_xor_assign
id|z
op_lshift
l_int|010
suffix:semicolon
id|x
op_xor_assign
id|z
suffix:semicolon
id|x
op_assign
id|x
op_rshift
l_int|1
op_or
id|x
op_lshift
l_int|31
suffix:semicolon
id|z
op_assign
(paren
id|x
op_xor
id|y
)paren
op_amp
l_int|0x55555555L
suffix:semicolon
id|y
op_xor_assign
id|z
suffix:semicolon
id|x
op_xor_assign
id|z
suffix:semicolon
id|y
op_assign
id|y
op_rshift
l_int|1
op_or
id|y
op_lshift
l_int|31
suffix:semicolon
id|z
op_assign
id|keyinfo
(braket
l_int|31
)braket
suffix:semicolon
id|z
op_xor_assign
id|y
suffix:semicolon
id|z
op_assign
id|z
op_lshift
l_int|4
op_or
id|z
op_rshift
l_int|28
suffix:semicolon
id|x
op_xor_assign
op_star
(paren
id|__u32
op_star
)paren
(paren
(paren
id|u8
op_star
)paren
(paren
id|des_keymap
op_plus
l_int|448
)paren
op_plus
(paren
l_int|0xFC
op_amp
id|z
)paren
)paren
suffix:semicolon
id|z
op_rshift_assign
l_int|8
suffix:semicolon
id|x
op_xor_assign
op_star
(paren
id|__u32
op_star
)paren
(paren
(paren
id|u8
op_star
)paren
(paren
id|des_keymap
op_plus
l_int|384
)paren
op_plus
(paren
l_int|0xFC
op_amp
id|z
)paren
)paren
suffix:semicolon
id|z
op_rshift_assign
l_int|8
suffix:semicolon
id|x
op_xor_assign
op_star
(paren
id|__u32
op_star
)paren
(paren
(paren
id|u8
op_star
)paren
(paren
id|des_keymap
op_plus
l_int|320
)paren
op_plus
(paren
l_int|0xFC
op_amp
id|z
)paren
)paren
suffix:semicolon
id|z
op_rshift_assign
l_int|8
suffix:semicolon
id|x
op_xor_assign
op_star
(paren
id|__u32
op_star
)paren
(paren
(paren
id|u8
op_star
)paren
(paren
id|des_keymap
op_plus
l_int|256
)paren
op_plus
(paren
l_int|0xFC
op_amp
id|z
)paren
)paren
suffix:semicolon
id|z
op_assign
id|keyinfo
(braket
l_int|30
)braket
suffix:semicolon
id|z
op_xor_assign
id|y
suffix:semicolon
id|x
op_xor_assign
op_star
(paren
id|__u32
op_star
)paren
(paren
(paren
id|u8
op_star
)paren
(paren
id|des_keymap
op_plus
l_int|192
)paren
op_plus
(paren
l_int|0xFC
op_amp
id|z
)paren
)paren
suffix:semicolon
id|z
op_rshift_assign
l_int|8
suffix:semicolon
id|x
op_xor_assign
op_star
(paren
id|__u32
op_star
)paren
(paren
(paren
id|u8
op_star
)paren
(paren
id|des_keymap
op_plus
l_int|128
)paren
op_plus
(paren
l_int|0xFC
op_amp
id|z
)paren
)paren
suffix:semicolon
id|z
op_rshift_assign
l_int|8
suffix:semicolon
id|x
op_xor_assign
op_star
(paren
id|__u32
op_star
)paren
(paren
(paren
id|u8
op_star
)paren
(paren
id|des_keymap
op_plus
l_int|64
)paren
op_plus
(paren
l_int|0xFC
op_amp
id|z
)paren
)paren
suffix:semicolon
id|z
op_rshift_assign
l_int|8
suffix:semicolon
id|x
op_xor_assign
op_star
(paren
id|__u32
op_star
)paren
(paren
(paren
id|u8
op_star
)paren
id|des_keymap
op_plus
(paren
l_int|0xFC
op_amp
id|z
)paren
)paren
suffix:semicolon
id|z
op_assign
id|keyinfo
(braket
l_int|29
)braket
suffix:semicolon
id|z
op_xor_assign
id|x
suffix:semicolon
id|z
op_assign
id|z
op_lshift
l_int|4
op_or
id|z
op_rshift
l_int|28
suffix:semicolon
id|y
op_xor_assign
op_star
(paren
id|__u32
op_star
)paren
(paren
(paren
id|u8
op_star
)paren
(paren
id|des_keymap
op_plus
l_int|448
)paren
op_plus
(paren
l_int|0xFC
op_amp
id|z
)paren
)paren
suffix:semicolon
id|z
op_rshift_assign
l_int|8
suffix:semicolon
id|y
op_xor_assign
op_star
(paren
id|__u32
op_star
)paren
(paren
(paren
id|u8
op_star
)paren
(paren
id|des_keymap
op_plus
l_int|384
)paren
op_plus
(paren
l_int|0xFC
op_amp
id|z
)paren
)paren
suffix:semicolon
id|z
op_rshift_assign
l_int|8
suffix:semicolon
id|y
op_xor_assign
op_star
(paren
id|__u32
op_star
)paren
(paren
(paren
id|u8
op_star
)paren
(paren
id|des_keymap
op_plus
l_int|320
)paren
op_plus
(paren
l_int|0xFC
op_amp
id|z
)paren
)paren
suffix:semicolon
id|z
op_rshift_assign
l_int|8
suffix:semicolon
id|y
op_xor_assign
op_star
(paren
id|__u32
op_star
)paren
(paren
(paren
id|u8
op_star
)paren
(paren
id|des_keymap
op_plus
l_int|256
)paren
op_plus
(paren
l_int|0xFC
op_amp
id|z
)paren
)paren
suffix:semicolon
id|z
op_assign
id|keyinfo
(braket
l_int|28
)braket
suffix:semicolon
id|z
op_xor_assign
id|x
suffix:semicolon
id|y
op_xor_assign
op_star
(paren
id|__u32
op_star
)paren
(paren
(paren
id|u8
op_star
)paren
(paren
id|des_keymap
op_plus
l_int|192
)paren
op_plus
(paren
l_int|0xFC
op_amp
id|z
)paren
)paren
suffix:semicolon
id|z
op_rshift_assign
l_int|8
suffix:semicolon
id|y
op_xor_assign
op_star
(paren
id|__u32
op_star
)paren
(paren
(paren
id|u8
op_star
)paren
(paren
id|des_keymap
op_plus
l_int|128
)paren
op_plus
(paren
l_int|0xFC
op_amp
id|z
)paren
)paren
suffix:semicolon
id|z
op_rshift_assign
l_int|8
suffix:semicolon
id|y
op_xor_assign
op_star
(paren
id|__u32
op_star
)paren
(paren
(paren
id|u8
op_star
)paren
(paren
id|des_keymap
op_plus
l_int|64
)paren
op_plus
(paren
l_int|0xFC
op_amp
id|z
)paren
)paren
suffix:semicolon
id|z
op_rshift_assign
l_int|8
suffix:semicolon
id|y
op_xor_assign
op_star
(paren
id|__u32
op_star
)paren
(paren
(paren
id|u8
op_star
)paren
id|des_keymap
op_plus
(paren
l_int|0xFC
op_amp
id|z
)paren
)paren
suffix:semicolon
id|z
op_assign
id|keyinfo
(braket
l_int|27
)braket
suffix:semicolon
id|z
op_xor_assign
id|y
suffix:semicolon
id|z
op_assign
id|z
op_lshift
l_int|4
op_or
id|z
op_rshift
l_int|28
suffix:semicolon
id|x
op_xor_assign
op_star
(paren
id|__u32
op_star
)paren
(paren
(paren
id|u8
op_star
)paren
(paren
id|des_keymap
op_plus
l_int|448
)paren
op_plus
(paren
l_int|0xFC
op_amp
id|z
)paren
)paren
suffix:semicolon
id|z
op_rshift_assign
l_int|8
suffix:semicolon
id|x
op_xor_assign
op_star
(paren
id|__u32
op_star
)paren
(paren
(paren
id|u8
op_star
)paren
(paren
id|des_keymap
op_plus
l_int|384
)paren
op_plus
(paren
l_int|0xFC
op_amp
id|z
)paren
)paren
suffix:semicolon
id|z
op_rshift_assign
l_int|8
suffix:semicolon
id|x
op_xor_assign
op_star
(paren
id|__u32
op_star
)paren
(paren
(paren
id|u8
op_star
)paren
(paren
id|des_keymap
op_plus
l_int|320
)paren
op_plus
(paren
l_int|0xFC
op_amp
id|z
)paren
)paren
suffix:semicolon
id|z
op_rshift_assign
l_int|8
suffix:semicolon
id|x
op_xor_assign
op_star
(paren
id|__u32
op_star
)paren
(paren
(paren
id|u8
op_star
)paren
(paren
id|des_keymap
op_plus
l_int|256
)paren
op_plus
(paren
l_int|0xFC
op_amp
id|z
)paren
)paren
suffix:semicolon
id|z
op_assign
id|keyinfo
(braket
l_int|26
)braket
suffix:semicolon
id|z
op_xor_assign
id|y
suffix:semicolon
id|x
op_xor_assign
op_star
(paren
id|__u32
op_star
)paren
(paren
(paren
id|u8
op_star
)paren
(paren
id|des_keymap
op_plus
l_int|192
)paren
op_plus
(paren
l_int|0xFC
op_amp
id|z
)paren
)paren
suffix:semicolon
id|z
op_rshift_assign
l_int|8
suffix:semicolon
id|x
op_xor_assign
op_star
(paren
id|__u32
op_star
)paren
(paren
(paren
id|u8
op_star
)paren
(paren
id|des_keymap
op_plus
l_int|128
)paren
op_plus
(paren
l_int|0xFC
op_amp
id|z
)paren
)paren
suffix:semicolon
id|z
op_rshift_assign
l_int|8
suffix:semicolon
id|x
op_xor_assign
op_star
(paren
id|__u32
op_star
)paren
(paren
(paren
id|u8
op_star
)paren
(paren
id|des_keymap
op_plus
l_int|64
)paren
op_plus
(paren
l_int|0xFC
op_amp
id|z
)paren
)paren
suffix:semicolon
id|z
op_rshift_assign
l_int|8
suffix:semicolon
id|x
op_xor_assign
op_star
(paren
id|__u32
op_star
)paren
(paren
(paren
id|u8
op_star
)paren
id|des_keymap
op_plus
(paren
l_int|0xFC
op_amp
id|z
)paren
)paren
suffix:semicolon
id|z
op_assign
id|keyinfo
(braket
l_int|25
)braket
suffix:semicolon
id|z
op_xor_assign
id|x
suffix:semicolon
id|z
op_assign
id|z
op_lshift
l_int|4
op_or
id|z
op_rshift
l_int|28
suffix:semicolon
id|y
op_xor_assign
op_star
(paren
id|__u32
op_star
)paren
(paren
(paren
id|u8
op_star
)paren
(paren
id|des_keymap
op_plus
l_int|448
)paren
op_plus
(paren
l_int|0xFC
op_amp
id|z
)paren
)paren
suffix:semicolon
id|z
op_rshift_assign
l_int|8
suffix:semicolon
id|y
op_xor_assign
op_star
(paren
id|__u32
op_star
)paren
(paren
(paren
id|u8
op_star
)paren
(paren
id|des_keymap
op_plus
l_int|384
)paren
op_plus
(paren
l_int|0xFC
op_amp
id|z
)paren
)paren
suffix:semicolon
id|z
op_rshift_assign
l_int|8
suffix:semicolon
id|y
op_xor_assign
op_star
(paren
id|__u32
op_star
)paren
(paren
(paren
id|u8
op_star
)paren
(paren
id|des_keymap
op_plus
l_int|320
)paren
op_plus
(paren
l_int|0xFC
op_amp
id|z
)paren
)paren
suffix:semicolon
id|z
op_rshift_assign
l_int|8
suffix:semicolon
id|y
op_xor_assign
op_star
(paren
id|__u32
op_star
)paren
(paren
(paren
id|u8
op_star
)paren
(paren
id|des_keymap
op_plus
l_int|256
)paren
op_plus
(paren
l_int|0xFC
op_amp
id|z
)paren
)paren
suffix:semicolon
id|z
op_assign
id|keyinfo
(braket
l_int|24
)braket
suffix:semicolon
id|z
op_xor_assign
id|x
suffix:semicolon
id|y
op_xor_assign
op_star
(paren
id|__u32
op_star
)paren
(paren
(paren
id|u8
op_star
)paren
(paren
id|des_keymap
op_plus
l_int|192
)paren
op_plus
(paren
l_int|0xFC
op_amp
id|z
)paren
)paren
suffix:semicolon
id|z
op_rshift_assign
l_int|8
suffix:semicolon
id|y
op_xor_assign
op_star
(paren
id|__u32
op_star
)paren
(paren
(paren
id|u8
op_star
)paren
(paren
id|des_keymap
op_plus
l_int|128
)paren
op_plus
(paren
l_int|0xFC
op_amp
id|z
)paren
)paren
suffix:semicolon
id|z
op_rshift_assign
l_int|8
suffix:semicolon
id|y
op_xor_assign
op_star
(paren
id|__u32
op_star
)paren
(paren
(paren
id|u8
op_star
)paren
(paren
id|des_keymap
op_plus
l_int|64
)paren
op_plus
(paren
l_int|0xFC
op_amp
id|z
)paren
)paren
suffix:semicolon
id|z
op_rshift_assign
l_int|8
suffix:semicolon
id|y
op_xor_assign
op_star
(paren
id|__u32
op_star
)paren
(paren
(paren
id|u8
op_star
)paren
id|des_keymap
op_plus
(paren
l_int|0xFC
op_amp
id|z
)paren
)paren
suffix:semicolon
id|z
op_assign
id|keyinfo
(braket
l_int|23
)braket
suffix:semicolon
id|z
op_xor_assign
id|y
suffix:semicolon
id|z
op_assign
id|z
op_lshift
l_int|4
op_or
id|z
op_rshift
l_int|28
suffix:semicolon
id|x
op_xor_assign
op_star
(paren
id|__u32
op_star
)paren
(paren
(paren
id|u8
op_star
)paren
(paren
id|des_keymap
op_plus
l_int|448
)paren
op_plus
(paren
l_int|0xFC
op_amp
id|z
)paren
)paren
suffix:semicolon
id|z
op_rshift_assign
l_int|8
suffix:semicolon
id|x
op_xor_assign
op_star
(paren
id|__u32
op_star
)paren
(paren
(paren
id|u8
op_star
)paren
(paren
id|des_keymap
op_plus
l_int|384
)paren
op_plus
(paren
l_int|0xFC
op_amp
id|z
)paren
)paren
suffix:semicolon
id|z
op_rshift_assign
l_int|8
suffix:semicolon
id|x
op_xor_assign
op_star
(paren
id|__u32
op_star
)paren
(paren
(paren
id|u8
op_star
)paren
(paren
id|des_keymap
op_plus
l_int|320
)paren
op_plus
(paren
l_int|0xFC
op_amp
id|z
)paren
)paren
suffix:semicolon
id|z
op_rshift_assign
l_int|8
suffix:semicolon
id|x
op_xor_assign
op_star
(paren
id|__u32
op_star
)paren
(paren
(paren
id|u8
op_star
)paren
(paren
id|des_keymap
op_plus
l_int|256
)paren
op_plus
(paren
l_int|0xFC
op_amp
id|z
)paren
)paren
suffix:semicolon
id|z
op_assign
id|keyinfo
(braket
l_int|22
)braket
suffix:semicolon
id|z
op_xor_assign
id|y
suffix:semicolon
id|x
op_xor_assign
op_star
(paren
id|__u32
op_star
)paren
(paren
(paren
id|u8
op_star
)paren
(paren
id|des_keymap
op_plus
l_int|192
)paren
op_plus
(paren
l_int|0xFC
op_amp
id|z
)paren
)paren
suffix:semicolon
id|z
op_rshift_assign
l_int|8
suffix:semicolon
id|x
op_xor_assign
op_star
(paren
id|__u32
op_star
)paren
(paren
(paren
id|u8
op_star
)paren
(paren
id|des_keymap
op_plus
l_int|128
)paren
op_plus
(paren
l_int|0xFC
op_amp
id|z
)paren
)paren
suffix:semicolon
id|z
op_rshift_assign
l_int|8
suffix:semicolon
id|x
op_xor_assign
op_star
(paren
id|__u32
op_star
)paren
(paren
(paren
id|u8
op_star
)paren
(paren
id|des_keymap
op_plus
l_int|64
)paren
op_plus
(paren
l_int|0xFC
op_amp
id|z
)paren
)paren
suffix:semicolon
id|z
op_rshift_assign
l_int|8
suffix:semicolon
id|x
op_xor_assign
op_star
(paren
id|__u32
op_star
)paren
(paren
(paren
id|u8
op_star
)paren
id|des_keymap
op_plus
(paren
l_int|0xFC
op_amp
id|z
)paren
)paren
suffix:semicolon
id|z
op_assign
id|keyinfo
(braket
l_int|21
)braket
suffix:semicolon
id|z
op_xor_assign
id|x
suffix:semicolon
id|z
op_assign
id|z
op_lshift
l_int|4
op_or
id|z
op_rshift
l_int|28
suffix:semicolon
id|y
op_xor_assign
op_star
(paren
id|__u32
op_star
)paren
(paren
(paren
id|u8
op_star
)paren
(paren
id|des_keymap
op_plus
l_int|448
)paren
op_plus
(paren
l_int|0xFC
op_amp
id|z
)paren
)paren
suffix:semicolon
id|z
op_rshift_assign
l_int|8
suffix:semicolon
id|y
op_xor_assign
op_star
(paren
id|__u32
op_star
)paren
(paren
(paren
id|u8
op_star
)paren
(paren
id|des_keymap
op_plus
l_int|384
)paren
op_plus
(paren
l_int|0xFC
op_amp
id|z
)paren
)paren
suffix:semicolon
id|z
op_rshift_assign
l_int|8
suffix:semicolon
id|y
op_xor_assign
op_star
(paren
id|__u32
op_star
)paren
(paren
(paren
id|u8
op_star
)paren
(paren
id|des_keymap
op_plus
l_int|320
)paren
op_plus
(paren
l_int|0xFC
op_amp
id|z
)paren
)paren
suffix:semicolon
id|z
op_rshift_assign
l_int|8
suffix:semicolon
id|y
op_xor_assign
op_star
(paren
id|__u32
op_star
)paren
(paren
(paren
id|u8
op_star
)paren
(paren
id|des_keymap
op_plus
l_int|256
)paren
op_plus
(paren
l_int|0xFC
op_amp
id|z
)paren
)paren
suffix:semicolon
id|z
op_assign
id|keyinfo
(braket
l_int|20
)braket
suffix:semicolon
id|z
op_xor_assign
id|x
suffix:semicolon
id|y
op_xor_assign
op_star
(paren
id|__u32
op_star
)paren
(paren
(paren
id|u8
op_star
)paren
(paren
id|des_keymap
op_plus
l_int|192
)paren
op_plus
(paren
l_int|0xFC
op_amp
id|z
)paren
)paren
suffix:semicolon
id|z
op_rshift_assign
l_int|8
suffix:semicolon
id|y
op_xor_assign
op_star
(paren
id|__u32
op_star
)paren
(paren
(paren
id|u8
op_star
)paren
(paren
id|des_keymap
op_plus
l_int|128
)paren
op_plus
(paren
l_int|0xFC
op_amp
id|z
)paren
)paren
suffix:semicolon
id|z
op_rshift_assign
l_int|8
suffix:semicolon
id|y
op_xor_assign
op_star
(paren
id|__u32
op_star
)paren
(paren
(paren
id|u8
op_star
)paren
(paren
id|des_keymap
op_plus
l_int|64
)paren
op_plus
(paren
l_int|0xFC
op_amp
id|z
)paren
)paren
suffix:semicolon
id|z
op_rshift_assign
l_int|8
suffix:semicolon
id|y
op_xor_assign
op_star
(paren
id|__u32
op_star
)paren
(paren
(paren
id|u8
op_star
)paren
id|des_keymap
op_plus
(paren
l_int|0xFC
op_amp
id|z
)paren
)paren
suffix:semicolon
id|z
op_assign
id|keyinfo
(braket
l_int|19
)braket
suffix:semicolon
id|z
op_xor_assign
id|y
suffix:semicolon
id|z
op_assign
id|z
op_lshift
l_int|4
op_or
id|z
op_rshift
l_int|28
suffix:semicolon
id|x
op_xor_assign
op_star
(paren
id|__u32
op_star
)paren
(paren
(paren
id|u8
op_star
)paren
(paren
id|des_keymap
op_plus
l_int|448
)paren
op_plus
(paren
l_int|0xFC
op_amp
id|z
)paren
)paren
suffix:semicolon
id|z
op_rshift_assign
l_int|8
suffix:semicolon
id|x
op_xor_assign
op_star
(paren
id|__u32
op_star
)paren
(paren
(paren
id|u8
op_star
)paren
(paren
id|des_keymap
op_plus
l_int|384
)paren
op_plus
(paren
l_int|0xFC
op_amp
id|z
)paren
)paren
suffix:semicolon
id|z
op_rshift_assign
l_int|8
suffix:semicolon
id|x
op_xor_assign
op_star
(paren
id|__u32
op_star
)paren
(paren
(paren
id|u8
op_star
)paren
(paren
id|des_keymap
op_plus
l_int|320
)paren
op_plus
(paren
l_int|0xFC
op_amp
id|z
)paren
)paren
suffix:semicolon
id|z
op_rshift_assign
l_int|8
suffix:semicolon
id|x
op_xor_assign
op_star
(paren
id|__u32
op_star
)paren
(paren
(paren
id|u8
op_star
)paren
(paren
id|des_keymap
op_plus
l_int|256
)paren
op_plus
(paren
l_int|0xFC
op_amp
id|z
)paren
)paren
suffix:semicolon
id|z
op_assign
id|keyinfo
(braket
l_int|18
)braket
suffix:semicolon
id|z
op_xor_assign
id|y
suffix:semicolon
id|x
op_xor_assign
op_star
(paren
id|__u32
op_star
)paren
(paren
(paren
id|u8
op_star
)paren
(paren
id|des_keymap
op_plus
l_int|192
)paren
op_plus
(paren
l_int|0xFC
op_amp
id|z
)paren
)paren
suffix:semicolon
id|z
op_rshift_assign
l_int|8
suffix:semicolon
id|x
op_xor_assign
op_star
(paren
id|__u32
op_star
)paren
(paren
(paren
id|u8
op_star
)paren
(paren
id|des_keymap
op_plus
l_int|128
)paren
op_plus
(paren
l_int|0xFC
op_amp
id|z
)paren
)paren
suffix:semicolon
id|z
op_rshift_assign
l_int|8
suffix:semicolon
id|x
op_xor_assign
op_star
(paren
id|__u32
op_star
)paren
(paren
(paren
id|u8
op_star
)paren
(paren
id|des_keymap
op_plus
l_int|64
)paren
op_plus
(paren
l_int|0xFC
op_amp
id|z
)paren
)paren
suffix:semicolon
id|z
op_rshift_assign
l_int|8
suffix:semicolon
id|x
op_xor_assign
op_star
(paren
id|__u32
op_star
)paren
(paren
(paren
id|u8
op_star
)paren
id|des_keymap
op_plus
(paren
l_int|0xFC
op_amp
id|z
)paren
)paren
suffix:semicolon
id|z
op_assign
id|keyinfo
(braket
l_int|17
)braket
suffix:semicolon
id|z
op_xor_assign
id|x
suffix:semicolon
id|z
op_assign
id|z
op_lshift
l_int|4
op_or
id|z
op_rshift
l_int|28
suffix:semicolon
id|y
op_xor_assign
op_star
(paren
id|__u32
op_star
)paren
(paren
(paren
id|u8
op_star
)paren
(paren
id|des_keymap
op_plus
l_int|448
)paren
op_plus
(paren
l_int|0xFC
op_amp
id|z
)paren
)paren
suffix:semicolon
id|z
op_rshift_assign
l_int|8
suffix:semicolon
id|y
op_xor_assign
op_star
(paren
id|__u32
op_star
)paren
(paren
(paren
id|u8
op_star
)paren
(paren
id|des_keymap
op_plus
l_int|384
)paren
op_plus
(paren
l_int|0xFC
op_amp
id|z
)paren
)paren
suffix:semicolon
id|z
op_rshift_assign
l_int|8
suffix:semicolon
id|y
op_xor_assign
op_star
(paren
id|__u32
op_star
)paren
(paren
(paren
id|u8
op_star
)paren
(paren
id|des_keymap
op_plus
l_int|320
)paren
op_plus
(paren
l_int|0xFC
op_amp
id|z
)paren
)paren
suffix:semicolon
id|z
op_rshift_assign
l_int|8
suffix:semicolon
id|y
op_xor_assign
op_star
(paren
id|__u32
op_star
)paren
(paren
(paren
id|u8
op_star
)paren
(paren
id|des_keymap
op_plus
l_int|256
)paren
op_plus
(paren
l_int|0xFC
op_amp
id|z
)paren
)paren
suffix:semicolon
id|z
op_assign
id|keyinfo
(braket
l_int|16
)braket
suffix:semicolon
id|z
op_xor_assign
id|x
suffix:semicolon
id|y
op_xor_assign
op_star
(paren
id|__u32
op_star
)paren
(paren
(paren
id|u8
op_star
)paren
(paren
id|des_keymap
op_plus
l_int|192
)paren
op_plus
(paren
l_int|0xFC
op_amp
id|z
)paren
)paren
suffix:semicolon
id|z
op_rshift_assign
l_int|8
suffix:semicolon
id|y
op_xor_assign
op_star
(paren
id|__u32
op_star
)paren
(paren
(paren
id|u8
op_star
)paren
(paren
id|des_keymap
op_plus
l_int|128
)paren
op_plus
(paren
l_int|0xFC
op_amp
id|z
)paren
)paren
suffix:semicolon
id|z
op_rshift_assign
l_int|8
suffix:semicolon
id|y
op_xor_assign
op_star
(paren
id|__u32
op_star
)paren
(paren
(paren
id|u8
op_star
)paren
(paren
id|des_keymap
op_plus
l_int|64
)paren
op_plus
(paren
l_int|0xFC
op_amp
id|z
)paren
)paren
suffix:semicolon
id|z
op_rshift_assign
l_int|8
suffix:semicolon
id|y
op_xor_assign
op_star
(paren
id|__u32
op_star
)paren
(paren
(paren
id|u8
op_star
)paren
id|des_keymap
op_plus
(paren
l_int|0xFC
op_amp
id|z
)paren
)paren
suffix:semicolon
id|z
op_assign
id|keyinfo
(braket
l_int|15
)braket
suffix:semicolon
id|z
op_xor_assign
id|y
suffix:semicolon
id|z
op_assign
id|z
op_lshift
l_int|4
op_or
id|z
op_rshift
l_int|28
suffix:semicolon
id|x
op_xor_assign
op_star
(paren
id|__u32
op_star
)paren
(paren
(paren
id|u8
op_star
)paren
(paren
id|des_keymap
op_plus
l_int|448
)paren
op_plus
(paren
l_int|0xFC
op_amp
id|z
)paren
)paren
suffix:semicolon
id|z
op_rshift_assign
l_int|8
suffix:semicolon
id|x
op_xor_assign
op_star
(paren
id|__u32
op_star
)paren
(paren
(paren
id|u8
op_star
)paren
(paren
id|des_keymap
op_plus
l_int|384
)paren
op_plus
(paren
l_int|0xFC
op_amp
id|z
)paren
)paren
suffix:semicolon
id|z
op_rshift_assign
l_int|8
suffix:semicolon
id|x
op_xor_assign
op_star
(paren
id|__u32
op_star
)paren
(paren
(paren
id|u8
op_star
)paren
(paren
id|des_keymap
op_plus
l_int|320
)paren
op_plus
(paren
l_int|0xFC
op_amp
id|z
)paren
)paren
suffix:semicolon
id|z
op_rshift_assign
l_int|8
suffix:semicolon
id|x
op_xor_assign
op_star
(paren
id|__u32
op_star
)paren
(paren
(paren
id|u8
op_star
)paren
(paren
id|des_keymap
op_plus
l_int|256
)paren
op_plus
(paren
l_int|0xFC
op_amp
id|z
)paren
)paren
suffix:semicolon
id|z
op_assign
id|keyinfo
(braket
l_int|14
)braket
suffix:semicolon
id|z
op_xor_assign
id|y
suffix:semicolon
id|x
op_xor_assign
op_star
(paren
id|__u32
op_star
)paren
(paren
(paren
id|u8
op_star
)paren
(paren
id|des_keymap
op_plus
l_int|192
)paren
op_plus
(paren
l_int|0xFC
op_amp
id|z
)paren
)paren
suffix:semicolon
id|z
op_rshift_assign
l_int|8
suffix:semicolon
id|x
op_xor_assign
op_star
(paren
id|__u32
op_star
)paren
(paren
(paren
id|u8
op_star
)paren
(paren
id|des_keymap
op_plus
l_int|128
)paren
op_plus
(paren
l_int|0xFC
op_amp
id|z
)paren
)paren
suffix:semicolon
id|z
op_rshift_assign
l_int|8
suffix:semicolon
id|x
op_xor_assign
op_star
(paren
id|__u32
op_star
)paren
(paren
(paren
id|u8
op_star
)paren
(paren
id|des_keymap
op_plus
l_int|64
)paren
op_plus
(paren
l_int|0xFC
op_amp
id|z
)paren
)paren
suffix:semicolon
id|z
op_rshift_assign
l_int|8
suffix:semicolon
id|x
op_xor_assign
op_star
(paren
id|__u32
op_star
)paren
(paren
(paren
id|u8
op_star
)paren
id|des_keymap
op_plus
(paren
l_int|0xFC
op_amp
id|z
)paren
)paren
suffix:semicolon
id|z
op_assign
id|keyinfo
(braket
l_int|13
)braket
suffix:semicolon
id|z
op_xor_assign
id|x
suffix:semicolon
id|z
op_assign
id|z
op_lshift
l_int|4
op_or
id|z
op_rshift
l_int|28
suffix:semicolon
id|y
op_xor_assign
op_star
(paren
id|__u32
op_star
)paren
(paren
(paren
id|u8
op_star
)paren
(paren
id|des_keymap
op_plus
l_int|448
)paren
op_plus
(paren
l_int|0xFC
op_amp
id|z
)paren
)paren
suffix:semicolon
id|z
op_rshift_assign
l_int|8
suffix:semicolon
id|y
op_xor_assign
op_star
(paren
id|__u32
op_star
)paren
(paren
(paren
id|u8
op_star
)paren
(paren
id|des_keymap
op_plus
l_int|384
)paren
op_plus
(paren
l_int|0xFC
op_amp
id|z
)paren
)paren
suffix:semicolon
id|z
op_rshift_assign
l_int|8
suffix:semicolon
id|y
op_xor_assign
op_star
(paren
id|__u32
op_star
)paren
(paren
(paren
id|u8
op_star
)paren
(paren
id|des_keymap
op_plus
l_int|320
)paren
op_plus
(paren
l_int|0xFC
op_amp
id|z
)paren
)paren
suffix:semicolon
id|z
op_rshift_assign
l_int|8
suffix:semicolon
id|y
op_xor_assign
op_star
(paren
id|__u32
op_star
)paren
(paren
(paren
id|u8
op_star
)paren
(paren
id|des_keymap
op_plus
l_int|256
)paren
op_plus
(paren
l_int|0xFC
op_amp
id|z
)paren
)paren
suffix:semicolon
id|z
op_assign
id|keyinfo
(braket
l_int|12
)braket
suffix:semicolon
id|z
op_xor_assign
id|x
suffix:semicolon
id|y
op_xor_assign
op_star
(paren
id|__u32
op_star
)paren
(paren
(paren
id|u8
op_star
)paren
(paren
id|des_keymap
op_plus
l_int|192
)paren
op_plus
(paren
l_int|0xFC
op_amp
id|z
)paren
)paren
suffix:semicolon
id|z
op_rshift_assign
l_int|8
suffix:semicolon
id|y
op_xor_assign
op_star
(paren
id|__u32
op_star
)paren
(paren
(paren
id|u8
op_star
)paren
(paren
id|des_keymap
op_plus
l_int|128
)paren
op_plus
(paren
l_int|0xFC
op_amp
id|z
)paren
)paren
suffix:semicolon
id|z
op_rshift_assign
l_int|8
suffix:semicolon
id|y
op_xor_assign
op_star
(paren
id|__u32
op_star
)paren
(paren
(paren
id|u8
op_star
)paren
(paren
id|des_keymap
op_plus
l_int|64
)paren
op_plus
(paren
l_int|0xFC
op_amp
id|z
)paren
)paren
suffix:semicolon
id|z
op_rshift_assign
l_int|8
suffix:semicolon
id|y
op_xor_assign
op_star
(paren
id|__u32
op_star
)paren
(paren
(paren
id|u8
op_star
)paren
id|des_keymap
op_plus
(paren
l_int|0xFC
op_amp
id|z
)paren
)paren
suffix:semicolon
id|z
op_assign
id|keyinfo
(braket
l_int|11
)braket
suffix:semicolon
id|z
op_xor_assign
id|y
suffix:semicolon
id|z
op_assign
id|z
op_lshift
l_int|4
op_or
id|z
op_rshift
l_int|28
suffix:semicolon
id|x
op_xor_assign
op_star
(paren
id|__u32
op_star
)paren
(paren
(paren
id|u8
op_star
)paren
(paren
id|des_keymap
op_plus
l_int|448
)paren
op_plus
(paren
l_int|0xFC
op_amp
id|z
)paren
)paren
suffix:semicolon
id|z
op_rshift_assign
l_int|8
suffix:semicolon
id|x
op_xor_assign
op_star
(paren
id|__u32
op_star
)paren
(paren
(paren
id|u8
op_star
)paren
(paren
id|des_keymap
op_plus
l_int|384
)paren
op_plus
(paren
l_int|0xFC
op_amp
id|z
)paren
)paren
suffix:semicolon
id|z
op_rshift_assign
l_int|8
suffix:semicolon
id|x
op_xor_assign
op_star
(paren
id|__u32
op_star
)paren
(paren
(paren
id|u8
op_star
)paren
(paren
id|des_keymap
op_plus
l_int|320
)paren
op_plus
(paren
l_int|0xFC
op_amp
id|z
)paren
)paren
suffix:semicolon
id|z
op_rshift_assign
l_int|8
suffix:semicolon
id|x
op_xor_assign
op_star
(paren
id|__u32
op_star
)paren
(paren
(paren
id|u8
op_star
)paren
(paren
id|des_keymap
op_plus
l_int|256
)paren
op_plus
(paren
l_int|0xFC
op_amp
id|z
)paren
)paren
suffix:semicolon
id|z
op_assign
id|keyinfo
(braket
l_int|10
)braket
suffix:semicolon
id|z
op_xor_assign
id|y
suffix:semicolon
id|x
op_xor_assign
op_star
(paren
id|__u32
op_star
)paren
(paren
(paren
id|u8
op_star
)paren
(paren
id|des_keymap
op_plus
l_int|192
)paren
op_plus
(paren
l_int|0xFC
op_amp
id|z
)paren
)paren
suffix:semicolon
id|z
op_rshift_assign
l_int|8
suffix:semicolon
id|x
op_xor_assign
op_star
(paren
id|__u32
op_star
)paren
(paren
(paren
id|u8
op_star
)paren
(paren
id|des_keymap
op_plus
l_int|128
)paren
op_plus
(paren
l_int|0xFC
op_amp
id|z
)paren
)paren
suffix:semicolon
id|z
op_rshift_assign
l_int|8
suffix:semicolon
id|x
op_xor_assign
op_star
(paren
id|__u32
op_star
)paren
(paren
(paren
id|u8
op_star
)paren
(paren
id|des_keymap
op_plus
l_int|64
)paren
op_plus
(paren
l_int|0xFC
op_amp
id|z
)paren
)paren
suffix:semicolon
id|z
op_rshift_assign
l_int|8
suffix:semicolon
id|x
op_xor_assign
op_star
(paren
id|__u32
op_star
)paren
(paren
(paren
id|u8
op_star
)paren
id|des_keymap
op_plus
(paren
l_int|0xFC
op_amp
id|z
)paren
)paren
suffix:semicolon
id|z
op_assign
id|keyinfo
(braket
l_int|9
)braket
suffix:semicolon
id|z
op_xor_assign
id|x
suffix:semicolon
id|z
op_assign
id|z
op_lshift
l_int|4
op_or
id|z
op_rshift
l_int|28
suffix:semicolon
id|y
op_xor_assign
op_star
(paren
id|__u32
op_star
)paren
(paren
(paren
id|u8
op_star
)paren
(paren
id|des_keymap
op_plus
l_int|448
)paren
op_plus
(paren
l_int|0xFC
op_amp
id|z
)paren
)paren
suffix:semicolon
id|z
op_rshift_assign
l_int|8
suffix:semicolon
id|y
op_xor_assign
op_star
(paren
id|__u32
op_star
)paren
(paren
(paren
id|u8
op_star
)paren
(paren
id|des_keymap
op_plus
l_int|384
)paren
op_plus
(paren
l_int|0xFC
op_amp
id|z
)paren
)paren
suffix:semicolon
id|z
op_rshift_assign
l_int|8
suffix:semicolon
id|y
op_xor_assign
op_star
(paren
id|__u32
op_star
)paren
(paren
(paren
id|u8
op_star
)paren
(paren
id|des_keymap
op_plus
l_int|320
)paren
op_plus
(paren
l_int|0xFC
op_amp
id|z
)paren
)paren
suffix:semicolon
id|z
op_rshift_assign
l_int|8
suffix:semicolon
id|y
op_xor_assign
op_star
(paren
id|__u32
op_star
)paren
(paren
(paren
id|u8
op_star
)paren
(paren
id|des_keymap
op_plus
l_int|256
)paren
op_plus
(paren
l_int|0xFC
op_amp
id|z
)paren
)paren
suffix:semicolon
id|z
op_assign
id|keyinfo
(braket
l_int|8
)braket
suffix:semicolon
id|z
op_xor_assign
id|x
suffix:semicolon
id|y
op_xor_assign
op_star
(paren
id|__u32
op_star
)paren
(paren
(paren
id|u8
op_star
)paren
(paren
id|des_keymap
op_plus
l_int|192
)paren
op_plus
(paren
l_int|0xFC
op_amp
id|z
)paren
)paren
suffix:semicolon
id|z
op_rshift_assign
l_int|8
suffix:semicolon
id|y
op_xor_assign
op_star
(paren
id|__u32
op_star
)paren
(paren
(paren
id|u8
op_star
)paren
(paren
id|des_keymap
op_plus
l_int|128
)paren
op_plus
(paren
l_int|0xFC
op_amp
id|z
)paren
)paren
suffix:semicolon
id|z
op_rshift_assign
l_int|8
suffix:semicolon
id|y
op_xor_assign
op_star
(paren
id|__u32
op_star
)paren
(paren
(paren
id|u8
op_star
)paren
(paren
id|des_keymap
op_plus
l_int|64
)paren
op_plus
(paren
l_int|0xFC
op_amp
id|z
)paren
)paren
suffix:semicolon
id|z
op_rshift_assign
l_int|8
suffix:semicolon
id|y
op_xor_assign
op_star
(paren
id|__u32
op_star
)paren
(paren
(paren
id|u8
op_star
)paren
id|des_keymap
op_plus
(paren
l_int|0xFC
op_amp
id|z
)paren
)paren
suffix:semicolon
id|z
op_assign
id|keyinfo
(braket
l_int|7
)braket
suffix:semicolon
id|z
op_xor_assign
id|y
suffix:semicolon
id|z
op_assign
id|z
op_lshift
l_int|4
op_or
id|z
op_rshift
l_int|28
suffix:semicolon
id|x
op_xor_assign
op_star
(paren
id|__u32
op_star
)paren
(paren
(paren
id|u8
op_star
)paren
(paren
id|des_keymap
op_plus
l_int|448
)paren
op_plus
(paren
l_int|0xFC
op_amp
id|z
)paren
)paren
suffix:semicolon
id|z
op_rshift_assign
l_int|8
suffix:semicolon
id|x
op_xor_assign
op_star
(paren
id|__u32
op_star
)paren
(paren
(paren
id|u8
op_star
)paren
(paren
id|des_keymap
op_plus
l_int|384
)paren
op_plus
(paren
l_int|0xFC
op_amp
id|z
)paren
)paren
suffix:semicolon
id|z
op_rshift_assign
l_int|8
suffix:semicolon
id|x
op_xor_assign
op_star
(paren
id|__u32
op_star
)paren
(paren
(paren
id|u8
op_star
)paren
(paren
id|des_keymap
op_plus
l_int|320
)paren
op_plus
(paren
l_int|0xFC
op_amp
id|z
)paren
)paren
suffix:semicolon
id|z
op_rshift_assign
l_int|8
suffix:semicolon
id|x
op_xor_assign
op_star
(paren
id|__u32
op_star
)paren
(paren
(paren
id|u8
op_star
)paren
(paren
id|des_keymap
op_plus
l_int|256
)paren
op_plus
(paren
l_int|0xFC
op_amp
id|z
)paren
)paren
suffix:semicolon
id|z
op_assign
id|keyinfo
(braket
l_int|6
)braket
suffix:semicolon
id|z
op_xor_assign
id|y
suffix:semicolon
id|x
op_xor_assign
op_star
(paren
id|__u32
op_star
)paren
(paren
(paren
id|u8
op_star
)paren
(paren
id|des_keymap
op_plus
l_int|192
)paren
op_plus
(paren
l_int|0xFC
op_amp
id|z
)paren
)paren
suffix:semicolon
id|z
op_rshift_assign
l_int|8
suffix:semicolon
id|x
op_xor_assign
op_star
(paren
id|__u32
op_star
)paren
(paren
(paren
id|u8
op_star
)paren
(paren
id|des_keymap
op_plus
l_int|128
)paren
op_plus
(paren
l_int|0xFC
op_amp
id|z
)paren
)paren
suffix:semicolon
id|z
op_rshift_assign
l_int|8
suffix:semicolon
id|x
op_xor_assign
op_star
(paren
id|__u32
op_star
)paren
(paren
(paren
id|u8
op_star
)paren
(paren
id|des_keymap
op_plus
l_int|64
)paren
op_plus
(paren
l_int|0xFC
op_amp
id|z
)paren
)paren
suffix:semicolon
id|z
op_rshift_assign
l_int|8
suffix:semicolon
id|x
op_xor_assign
op_star
(paren
id|__u32
op_star
)paren
(paren
(paren
id|u8
op_star
)paren
id|des_keymap
op_plus
(paren
l_int|0xFC
op_amp
id|z
)paren
)paren
suffix:semicolon
id|z
op_assign
id|keyinfo
(braket
l_int|5
)braket
suffix:semicolon
id|z
op_xor_assign
id|x
suffix:semicolon
id|z
op_assign
id|z
op_lshift
l_int|4
op_or
id|z
op_rshift
l_int|28
suffix:semicolon
id|y
op_xor_assign
op_star
(paren
id|__u32
op_star
)paren
(paren
(paren
id|u8
op_star
)paren
(paren
id|des_keymap
op_plus
l_int|448
)paren
op_plus
(paren
l_int|0xFC
op_amp
id|z
)paren
)paren
suffix:semicolon
id|z
op_rshift_assign
l_int|8
suffix:semicolon
id|y
op_xor_assign
op_star
(paren
id|__u32
op_star
)paren
(paren
(paren
id|u8
op_star
)paren
(paren
id|des_keymap
op_plus
l_int|384
)paren
op_plus
(paren
l_int|0xFC
op_amp
id|z
)paren
)paren
suffix:semicolon
id|z
op_rshift_assign
l_int|8
suffix:semicolon
id|y
op_xor_assign
op_star
(paren
id|__u32
op_star
)paren
(paren
(paren
id|u8
op_star
)paren
(paren
id|des_keymap
op_plus
l_int|320
)paren
op_plus
(paren
l_int|0xFC
op_amp
id|z
)paren
)paren
suffix:semicolon
id|z
op_rshift_assign
l_int|8
suffix:semicolon
id|y
op_xor_assign
op_star
(paren
id|__u32
op_star
)paren
(paren
(paren
id|u8
op_star
)paren
(paren
id|des_keymap
op_plus
l_int|256
)paren
op_plus
(paren
l_int|0xFC
op_amp
id|z
)paren
)paren
suffix:semicolon
id|z
op_assign
id|keyinfo
(braket
l_int|4
)braket
suffix:semicolon
id|z
op_xor_assign
id|x
suffix:semicolon
id|y
op_xor_assign
op_star
(paren
id|__u32
op_star
)paren
(paren
(paren
id|u8
op_star
)paren
(paren
id|des_keymap
op_plus
l_int|192
)paren
op_plus
(paren
l_int|0xFC
op_amp
id|z
)paren
)paren
suffix:semicolon
id|z
op_rshift_assign
l_int|8
suffix:semicolon
id|y
op_xor_assign
op_star
(paren
id|__u32
op_star
)paren
(paren
(paren
id|u8
op_star
)paren
(paren
id|des_keymap
op_plus
l_int|128
)paren
op_plus
(paren
l_int|0xFC
op_amp
id|z
)paren
)paren
suffix:semicolon
id|z
op_rshift_assign
l_int|8
suffix:semicolon
id|y
op_xor_assign
op_star
(paren
id|__u32
op_star
)paren
(paren
(paren
id|u8
op_star
)paren
(paren
id|des_keymap
op_plus
l_int|64
)paren
op_plus
(paren
l_int|0xFC
op_amp
id|z
)paren
)paren
suffix:semicolon
id|z
op_rshift_assign
l_int|8
suffix:semicolon
id|y
op_xor_assign
op_star
(paren
id|__u32
op_star
)paren
(paren
(paren
id|u8
op_star
)paren
id|des_keymap
op_plus
(paren
l_int|0xFC
op_amp
id|z
)paren
)paren
suffix:semicolon
id|z
op_assign
id|keyinfo
(braket
l_int|3
)braket
suffix:semicolon
id|z
op_xor_assign
id|y
suffix:semicolon
id|z
op_assign
id|z
op_lshift
l_int|4
op_or
id|z
op_rshift
l_int|28
suffix:semicolon
id|x
op_xor_assign
op_star
(paren
id|__u32
op_star
)paren
(paren
(paren
id|u8
op_star
)paren
(paren
id|des_keymap
op_plus
l_int|448
)paren
op_plus
(paren
l_int|0xFC
op_amp
id|z
)paren
)paren
suffix:semicolon
id|z
op_rshift_assign
l_int|8
suffix:semicolon
id|x
op_xor_assign
op_star
(paren
id|__u32
op_star
)paren
(paren
(paren
id|u8
op_star
)paren
(paren
id|des_keymap
op_plus
l_int|384
)paren
op_plus
(paren
l_int|0xFC
op_amp
id|z
)paren
)paren
suffix:semicolon
id|z
op_rshift_assign
l_int|8
suffix:semicolon
id|x
op_xor_assign
op_star
(paren
id|__u32
op_star
)paren
(paren
(paren
id|u8
op_star
)paren
(paren
id|des_keymap
op_plus
l_int|320
)paren
op_plus
(paren
l_int|0xFC
op_amp
id|z
)paren
)paren
suffix:semicolon
id|z
op_rshift_assign
l_int|8
suffix:semicolon
id|x
op_xor_assign
op_star
(paren
id|__u32
op_star
)paren
(paren
(paren
id|u8
op_star
)paren
(paren
id|des_keymap
op_plus
l_int|256
)paren
op_plus
(paren
l_int|0xFC
op_amp
id|z
)paren
)paren
suffix:semicolon
id|z
op_assign
id|keyinfo
(braket
l_int|2
)braket
suffix:semicolon
id|z
op_xor_assign
id|y
suffix:semicolon
id|x
op_xor_assign
op_star
(paren
id|__u32
op_star
)paren
(paren
(paren
id|u8
op_star
)paren
(paren
id|des_keymap
op_plus
l_int|192
)paren
op_plus
(paren
l_int|0xFC
op_amp
id|z
)paren
)paren
suffix:semicolon
id|z
op_rshift_assign
l_int|8
suffix:semicolon
id|x
op_xor_assign
op_star
(paren
id|__u32
op_star
)paren
(paren
(paren
id|u8
op_star
)paren
(paren
id|des_keymap
op_plus
l_int|128
)paren
op_plus
(paren
l_int|0xFC
op_amp
id|z
)paren
)paren
suffix:semicolon
id|z
op_rshift_assign
l_int|8
suffix:semicolon
id|x
op_xor_assign
op_star
(paren
id|__u32
op_star
)paren
(paren
(paren
id|u8
op_star
)paren
(paren
id|des_keymap
op_plus
l_int|64
)paren
op_plus
(paren
l_int|0xFC
op_amp
id|z
)paren
)paren
suffix:semicolon
id|z
op_rshift_assign
l_int|8
suffix:semicolon
id|x
op_xor_assign
op_star
(paren
id|__u32
op_star
)paren
(paren
(paren
id|u8
op_star
)paren
id|des_keymap
op_plus
(paren
l_int|0xFC
op_amp
id|z
)paren
)paren
suffix:semicolon
id|z
op_assign
id|keyinfo
(braket
l_int|1
)braket
suffix:semicolon
id|z
op_xor_assign
id|x
suffix:semicolon
id|z
op_assign
id|z
op_lshift
l_int|4
op_or
id|z
op_rshift
l_int|28
suffix:semicolon
id|y
op_xor_assign
op_star
(paren
id|__u32
op_star
)paren
(paren
(paren
id|u8
op_star
)paren
(paren
id|des_keymap
op_plus
l_int|448
)paren
op_plus
(paren
l_int|0xFC
op_amp
id|z
)paren
)paren
suffix:semicolon
id|z
op_rshift_assign
l_int|8
suffix:semicolon
id|y
op_xor_assign
op_star
(paren
id|__u32
op_star
)paren
(paren
(paren
id|u8
op_star
)paren
(paren
id|des_keymap
op_plus
l_int|384
)paren
op_plus
(paren
l_int|0xFC
op_amp
id|z
)paren
)paren
suffix:semicolon
id|z
op_rshift_assign
l_int|8
suffix:semicolon
id|y
op_xor_assign
op_star
(paren
id|__u32
op_star
)paren
(paren
(paren
id|u8
op_star
)paren
(paren
id|des_keymap
op_plus
l_int|320
)paren
op_plus
(paren
l_int|0xFC
op_amp
id|z
)paren
)paren
suffix:semicolon
id|z
op_rshift_assign
l_int|8
suffix:semicolon
id|y
op_xor_assign
op_star
(paren
id|__u32
op_star
)paren
(paren
(paren
id|u8
op_star
)paren
(paren
id|des_keymap
op_plus
l_int|256
)paren
op_plus
(paren
l_int|0xFC
op_amp
id|z
)paren
)paren
suffix:semicolon
id|z
op_assign
id|keyinfo
(braket
l_int|0
)braket
suffix:semicolon
id|z
op_xor_assign
id|x
suffix:semicolon
id|y
op_xor_assign
op_star
(paren
id|__u32
op_star
)paren
(paren
(paren
id|u8
op_star
)paren
(paren
id|des_keymap
op_plus
l_int|192
)paren
op_plus
(paren
l_int|0xFC
op_amp
id|z
)paren
)paren
suffix:semicolon
id|z
op_rshift_assign
l_int|8
suffix:semicolon
id|y
op_xor_assign
op_star
(paren
id|__u32
op_star
)paren
(paren
(paren
id|u8
op_star
)paren
(paren
id|des_keymap
op_plus
l_int|128
)paren
op_plus
(paren
l_int|0xFC
op_amp
id|z
)paren
)paren
suffix:semicolon
id|z
op_rshift_assign
l_int|8
suffix:semicolon
id|y
op_xor_assign
op_star
(paren
id|__u32
op_star
)paren
(paren
(paren
id|u8
op_star
)paren
(paren
id|des_keymap
op_plus
l_int|64
)paren
op_plus
(paren
l_int|0xFC
op_amp
id|z
)paren
)paren
suffix:semicolon
id|z
op_rshift_assign
l_int|8
suffix:semicolon
id|y
op_xor_assign
op_star
(paren
id|__u32
op_star
)paren
(paren
(paren
id|u8
op_star
)paren
id|des_keymap
op_plus
(paren
l_int|0xFC
op_amp
id|z
)paren
)paren
suffix:semicolon
id|x
op_assign
id|x
op_lshift
l_int|1
op_or
id|x
op_rshift
l_int|31
suffix:semicolon
id|z
op_assign
(paren
id|x
op_xor
id|y
)paren
op_amp
l_int|0x55555555L
suffix:semicolon
id|y
op_xor_assign
id|z
suffix:semicolon
id|x
op_xor_assign
id|z
suffix:semicolon
id|y
op_assign
id|y
op_lshift
l_int|1
op_or
id|y
op_rshift
l_int|31
suffix:semicolon
id|z
op_assign
(paren
(paren
id|x
op_rshift
l_int|010
)paren
op_xor
id|y
)paren
op_amp
l_int|0x00FF00FFL
suffix:semicolon
id|x
op_xor_assign
id|z
op_lshift
l_int|010
suffix:semicolon
id|y
op_xor_assign
id|z
suffix:semicolon
id|z
op_assign
(paren
(paren
id|y
op_rshift
l_int|002
)paren
op_xor
id|x
)paren
op_amp
l_int|0x33333333L
suffix:semicolon
id|y
op_xor_assign
id|z
op_lshift
l_int|002
suffix:semicolon
id|x
op_xor_assign
id|z
suffix:semicolon
id|z
op_assign
(paren
(paren
id|x
op_rshift
l_int|020
)paren
op_xor
id|y
)paren
op_amp
l_int|0x0000FFFFL
suffix:semicolon
id|x
op_xor_assign
id|z
op_lshift
l_int|020
suffix:semicolon
id|y
op_xor_assign
id|z
suffix:semicolon
id|z
op_assign
(paren
(paren
id|y
op_rshift
l_int|004
)paren
op_xor
id|x
)paren
op_amp
l_int|0x0F0F0F0FL
suffix:semicolon
id|y
op_xor_assign
id|z
op_lshift
l_int|004
suffix:semicolon
id|x
op_xor_assign
id|z
suffix:semicolon
id|dst
(braket
l_int|0
)braket
op_assign
id|x
suffix:semicolon
id|x
op_rshift_assign
l_int|8
suffix:semicolon
id|dst
(braket
l_int|1
)braket
op_assign
id|x
suffix:semicolon
id|x
op_rshift_assign
l_int|8
suffix:semicolon
id|dst
(braket
l_int|2
)braket
op_assign
id|x
suffix:semicolon
id|x
op_rshift_assign
l_int|8
suffix:semicolon
id|dst
(braket
l_int|3
)braket
op_assign
id|x
suffix:semicolon
id|dst
(braket
l_int|4
)braket
op_assign
id|y
suffix:semicolon
id|y
op_rshift_assign
l_int|8
suffix:semicolon
id|dst
(braket
l_int|5
)braket
op_assign
id|y
suffix:semicolon
id|y
op_rshift_assign
l_int|8
suffix:semicolon
id|dst
(braket
l_int|6
)braket
op_assign
id|y
suffix:semicolon
id|y
op_rshift_assign
l_int|8
suffix:semicolon
id|dst
(braket
l_int|7
)braket
op_assign
id|y
suffix:semicolon
r_return
suffix:semicolon
)brace
multiline_comment|/*&n; * RFC2451: Weak key checks SHOULD be performed.&n; */
DECL|function|des_setkey
r_static
r_int
id|des_setkey
c_func
(paren
r_void
op_star
id|ctx
comma
r_const
id|__u8
op_star
id|key
comma
r_int
id|keylen
comma
r_int
op_star
id|flags
)paren
(brace
r_struct
id|des_ctx
op_star
id|dctx
op_assign
id|ctx
suffix:semicolon
r_const
id|__u8
op_star
id|k
suffix:semicolon
id|__u8
op_star
id|b0
comma
op_star
id|b1
suffix:semicolon
id|__u32
id|n
comma
id|w
suffix:semicolon
id|__u32
op_star
id|method
suffix:semicolon
id|__u8
id|bits0
(braket
l_int|56
)braket
comma
id|bits1
(braket
l_int|56
)braket
suffix:semicolon
r_if
c_cond
(paren
id|keylen
op_ne
id|DES_KEY_SIZE
)paren
(brace
op_star
id|flags
op_or_assign
id|CRYPTO_BAD_KEY_LEN
suffix:semicolon
r_return
op_minus
id|EINVAL
suffix:semicolon
)brace
id|n
op_assign
id|parity
(braket
id|key
(braket
l_int|0
)braket
)braket
suffix:semicolon
id|n
op_lshift_assign
l_int|4
suffix:semicolon
id|n
op_or_assign
id|parity
(braket
id|key
(braket
l_int|1
)braket
)braket
suffix:semicolon
id|n
op_lshift_assign
l_int|4
suffix:semicolon
id|n
op_or_assign
id|parity
(braket
id|key
(braket
l_int|2
)braket
)braket
suffix:semicolon
id|n
op_lshift_assign
l_int|4
suffix:semicolon
id|n
op_or_assign
id|parity
(braket
id|key
(braket
l_int|3
)braket
)braket
suffix:semicolon
id|n
op_lshift_assign
l_int|4
suffix:semicolon
id|n
op_or_assign
id|parity
(braket
id|key
(braket
l_int|4
)braket
)braket
suffix:semicolon
id|n
op_lshift_assign
l_int|4
suffix:semicolon
id|n
op_or_assign
id|parity
(braket
id|key
(braket
l_int|5
)braket
)braket
suffix:semicolon
id|n
op_lshift_assign
l_int|4
suffix:semicolon
id|n
op_or_assign
id|parity
(braket
id|key
(braket
l_int|6
)braket
)braket
suffix:semicolon
id|n
op_lshift_assign
l_int|4
suffix:semicolon
id|n
op_or_assign
id|parity
(braket
id|key
(braket
l_int|7
)braket
)braket
suffix:semicolon
id|w
op_assign
l_int|0x88888888L
suffix:semicolon
r_if
c_cond
(paren
(paren
op_star
id|flags
op_amp
id|CRYPTO_WEAK_KEY_CHECK
)paren
op_logical_and
op_logical_neg
(paren
(paren
id|n
op_minus
(paren
id|w
op_rshift
l_int|3
)paren
)paren
op_amp
id|w
)paren
)paren
(brace
multiline_comment|/* 1 in 10^10 keys passes this test */
r_if
c_cond
(paren
id|n
OL
l_int|0x41415151
)paren
(brace
r_if
c_cond
(paren
id|n
OL
l_int|0x31312121
)paren
(brace
r_if
c_cond
(paren
id|n
OL
l_int|0x14141515
)paren
(brace
multiline_comment|/* 01 01 01 01 01 01 01 01 */
r_if
c_cond
(paren
id|n
op_eq
l_int|0x11111111
)paren
r_goto
id|weak
suffix:semicolon
multiline_comment|/* 01 1F 01 1F 01 0E 01 0E */
r_if
c_cond
(paren
id|n
op_eq
l_int|0x13131212
)paren
r_goto
id|weak
suffix:semicolon
)brace
r_else
(brace
multiline_comment|/* 01 E0 01 E0 01 F1 01 F1 */
r_if
c_cond
(paren
id|n
op_eq
l_int|0x14141515
)paren
r_goto
id|weak
suffix:semicolon
multiline_comment|/* 01 FE 01 FE 01 FE 01 FE */
r_if
c_cond
(paren
id|n
op_eq
l_int|0x16161616
)paren
r_goto
id|weak
suffix:semicolon
)brace
)brace
r_else
(brace
r_if
c_cond
(paren
id|n
OL
l_int|0x34342525
)paren
(brace
multiline_comment|/* 1F 01 1F 01 0E 01 0E 01 */
r_if
c_cond
(paren
id|n
op_eq
l_int|0x31312121
)paren
r_goto
id|weak
suffix:semicolon
multiline_comment|/* 1F 1F 1F 1F 0E 0E 0E 0E (?) */
r_if
c_cond
(paren
id|n
op_eq
l_int|0x33332222
)paren
r_goto
id|weak
suffix:semicolon
)brace
r_else
(brace
multiline_comment|/* 1F E0 1F E0 0E F1 0E F1 */
r_if
c_cond
(paren
id|n
op_eq
l_int|0x34342525
)paren
r_goto
id|weak
suffix:semicolon
multiline_comment|/* 1F FE 1F FE 0E FE 0E FE */
r_if
c_cond
(paren
id|n
op_eq
l_int|0x36362626
)paren
r_goto
id|weak
suffix:semicolon
)brace
)brace
)brace
r_else
(brace
r_if
c_cond
(paren
id|n
OL
l_int|0x61616161
)paren
(brace
r_if
c_cond
(paren
id|n
OL
l_int|0x44445555
)paren
(brace
multiline_comment|/* E0 01 E0 01 F1 01 F1 01 */
r_if
c_cond
(paren
id|n
op_eq
l_int|0x41415151
)paren
r_goto
id|weak
suffix:semicolon
multiline_comment|/* E0 1F E0 1F F1 0E F1 0E */
r_if
c_cond
(paren
id|n
op_eq
l_int|0x43435252
)paren
r_goto
id|weak
suffix:semicolon
)brace
r_else
(brace
multiline_comment|/* E0 E0 E0 E0 F1 F1 F1 F1 (?) */
r_if
c_cond
(paren
id|n
op_eq
l_int|0x44445555
)paren
r_goto
id|weak
suffix:semicolon
multiline_comment|/* E0 FE E0 FE F1 FE F1 FE */
r_if
c_cond
(paren
id|n
op_eq
l_int|0x46465656
)paren
r_goto
id|weak
suffix:semicolon
)brace
)brace
r_else
(brace
r_if
c_cond
(paren
id|n
OL
l_int|0x64646565
)paren
(brace
multiline_comment|/* FE 01 FE 01 FE 01 FE 01 */
r_if
c_cond
(paren
id|n
op_eq
l_int|0x61616161
)paren
r_goto
id|weak
suffix:semicolon
multiline_comment|/* FE 1F FE 1F FE 0E FE 0E */
r_if
c_cond
(paren
id|n
op_eq
l_int|0x63636262
)paren
r_goto
id|weak
suffix:semicolon
)brace
r_else
(brace
multiline_comment|/* FE E0 FE E0 FE F1 FE F1 */
r_if
c_cond
(paren
id|n
op_eq
l_int|0x64646565
)paren
r_goto
id|weak
suffix:semicolon
multiline_comment|/* FE FE FE FE FE FE FE FE */
r_if
c_cond
(paren
id|n
op_eq
l_int|0x66666666
)paren
r_goto
id|weak
suffix:semicolon
)brace
)brace
)brace
r_goto
id|not_weak
suffix:semicolon
id|weak
suffix:colon
op_star
id|flags
op_or_assign
id|CRYPTO_WEAK_KEY
suffix:semicolon
r_return
op_minus
id|EINVAL
suffix:semicolon
)brace
id|not_weak
suffix:colon
multiline_comment|/* explode the bits */
id|n
op_assign
l_int|56
suffix:semicolon
id|b0
op_assign
id|bits0
suffix:semicolon
id|b1
op_assign
id|bits1
suffix:semicolon
r_do
(brace
id|w
op_assign
(paren
l_int|256
op_or
op_star
id|key
op_increment
)paren
op_lshift
l_int|2
suffix:semicolon
r_do
(brace
op_decrement
id|n
suffix:semicolon
id|b1
(braket
id|n
)braket
op_assign
l_int|8
op_amp
id|w
suffix:semicolon
id|w
op_rshift_assign
l_int|1
suffix:semicolon
id|b0
(braket
id|n
)braket
op_assign
l_int|4
op_amp
id|w
suffix:semicolon
)brace
r_while
c_loop
(paren
id|w
op_ge
l_int|16
)paren
suffix:semicolon
)brace
r_while
c_loop
(paren
id|n
)paren
suffix:semicolon
multiline_comment|/* put the bits in the correct places */
id|n
op_assign
l_int|16
suffix:semicolon
id|k
op_assign
id|rotors
suffix:semicolon
id|method
op_assign
id|dctx-&gt;keyinfo
suffix:semicolon
r_do
(brace
id|w
op_assign
(paren
id|b1
(braket
id|k
(braket
l_int|0
)braket
)braket
op_or
id|b0
(braket
id|k
(braket
l_int|1
)braket
)braket
)paren
op_lshift
l_int|4
suffix:semicolon
id|w
op_or_assign
(paren
id|b1
(braket
id|k
(braket
l_int|2
)braket
)braket
op_or
id|b0
(braket
id|k
(braket
l_int|3
)braket
)braket
)paren
op_lshift
l_int|2
suffix:semicolon
id|w
op_or_assign
id|b1
(braket
id|k
(braket
l_int|4
)braket
)braket
op_or
id|b0
(braket
id|k
(braket
l_int|5
)braket
)braket
suffix:semicolon
id|w
op_lshift_assign
l_int|8
suffix:semicolon
id|w
op_or_assign
(paren
id|b1
(braket
id|k
(braket
l_int|6
)braket
)braket
op_or
id|b0
(braket
id|k
(braket
l_int|7
)braket
)braket
)paren
op_lshift
l_int|4
suffix:semicolon
id|w
op_or_assign
(paren
id|b1
(braket
id|k
(braket
l_int|8
)braket
)braket
op_or
id|b0
(braket
id|k
(braket
l_int|9
)braket
)braket
)paren
op_lshift
l_int|2
suffix:semicolon
id|w
op_or_assign
id|b1
(braket
id|k
(braket
l_int|10
)braket
)braket
op_or
id|b0
(braket
id|k
(braket
l_int|11
)braket
)braket
suffix:semicolon
id|w
op_lshift_assign
l_int|8
suffix:semicolon
id|w
op_or_assign
(paren
id|b1
(braket
id|k
(braket
l_int|12
)braket
)braket
op_or
id|b0
(braket
id|k
(braket
l_int|13
)braket
)braket
)paren
op_lshift
l_int|4
suffix:semicolon
id|w
op_or_assign
(paren
id|b1
(braket
id|k
(braket
l_int|14
)braket
)braket
op_or
id|b0
(braket
id|k
(braket
l_int|15
)braket
)braket
)paren
op_lshift
l_int|2
suffix:semicolon
id|w
op_or_assign
id|b1
(braket
id|k
(braket
l_int|16
)braket
)braket
op_or
id|b0
(braket
id|k
(braket
l_int|17
)braket
)braket
suffix:semicolon
id|w
op_lshift_assign
l_int|8
suffix:semicolon
id|w
op_or_assign
(paren
id|b1
(braket
id|k
(braket
l_int|18
)braket
)braket
op_or
id|b0
(braket
id|k
(braket
l_int|19
)braket
)braket
)paren
op_lshift
l_int|4
suffix:semicolon
id|w
op_or_assign
(paren
id|b1
(braket
id|k
(braket
l_int|20
)braket
)braket
op_or
id|b0
(braket
id|k
(braket
l_int|21
)braket
)braket
)paren
op_lshift
l_int|2
suffix:semicolon
id|w
op_or_assign
id|b1
(braket
id|k
(braket
l_int|22
)braket
)braket
op_or
id|b0
(braket
id|k
(braket
l_int|23
)braket
)braket
suffix:semicolon
id|method
(braket
l_int|0
)braket
op_assign
id|w
suffix:semicolon
id|w
op_assign
(paren
id|b1
(braket
id|k
(braket
l_int|0
op_plus
l_int|24
)braket
)braket
op_or
id|b0
(braket
id|k
(braket
l_int|1
op_plus
l_int|24
)braket
)braket
)paren
op_lshift
l_int|4
suffix:semicolon
id|w
op_or_assign
(paren
id|b1
(braket
id|k
(braket
l_int|2
op_plus
l_int|24
)braket
)braket
op_or
id|b0
(braket
id|k
(braket
l_int|3
op_plus
l_int|24
)braket
)braket
)paren
op_lshift
l_int|2
suffix:semicolon
id|w
op_or_assign
id|b1
(braket
id|k
(braket
l_int|4
op_plus
l_int|24
)braket
)braket
op_or
id|b0
(braket
id|k
(braket
l_int|5
op_plus
l_int|24
)braket
)braket
suffix:semicolon
id|w
op_lshift_assign
l_int|8
suffix:semicolon
id|w
op_or_assign
(paren
id|b1
(braket
id|k
(braket
l_int|6
op_plus
l_int|24
)braket
)braket
op_or
id|b0
(braket
id|k
(braket
l_int|7
op_plus
l_int|24
)braket
)braket
)paren
op_lshift
l_int|4
suffix:semicolon
id|w
op_or_assign
(paren
id|b1
(braket
id|k
(braket
l_int|8
op_plus
l_int|24
)braket
)braket
op_or
id|b0
(braket
id|k
(braket
l_int|9
op_plus
l_int|24
)braket
)braket
)paren
op_lshift
l_int|2
suffix:semicolon
id|w
op_or_assign
id|b1
(braket
id|k
(braket
l_int|10
op_plus
l_int|24
)braket
)braket
op_or
id|b0
(braket
id|k
(braket
l_int|11
op_plus
l_int|24
)braket
)braket
suffix:semicolon
id|w
op_lshift_assign
l_int|8
suffix:semicolon
id|w
op_or_assign
(paren
id|b1
(braket
id|k
(braket
l_int|12
op_plus
l_int|24
)braket
)braket
op_or
id|b0
(braket
id|k
(braket
l_int|13
op_plus
l_int|24
)braket
)braket
)paren
op_lshift
l_int|4
suffix:semicolon
id|w
op_or_assign
(paren
id|b1
(braket
id|k
(braket
l_int|14
op_plus
l_int|24
)braket
)braket
op_or
id|b0
(braket
id|k
(braket
l_int|15
op_plus
l_int|24
)braket
)braket
)paren
op_lshift
l_int|2
suffix:semicolon
id|w
op_or_assign
id|b1
(braket
id|k
(braket
l_int|16
op_plus
l_int|24
)braket
)braket
op_or
id|b0
(braket
id|k
(braket
l_int|17
op_plus
l_int|24
)braket
)braket
suffix:semicolon
id|w
op_lshift_assign
l_int|8
suffix:semicolon
id|w
op_or_assign
(paren
id|b1
(braket
id|k
(braket
l_int|18
op_plus
l_int|24
)braket
)braket
op_or
id|b0
(braket
id|k
(braket
l_int|19
op_plus
l_int|24
)braket
)braket
)paren
op_lshift
l_int|4
suffix:semicolon
id|w
op_or_assign
(paren
id|b1
(braket
id|k
(braket
l_int|20
op_plus
l_int|24
)braket
)braket
op_or
id|b0
(braket
id|k
(braket
l_int|21
op_plus
l_int|24
)braket
)braket
)paren
op_lshift
l_int|2
suffix:semicolon
id|w
op_or_assign
id|b1
(braket
id|k
(braket
l_int|22
op_plus
l_int|24
)braket
)braket
op_or
id|b0
(braket
id|k
(braket
l_int|23
op_plus
l_int|24
)braket
)braket
suffix:semicolon
id|ROR
c_func
(paren
id|w
comma
l_int|4
comma
l_int|28
)paren
suffix:semicolon
multiline_comment|/* could be eliminated */
id|method
(braket
l_int|1
)braket
op_assign
id|w
suffix:semicolon
id|k
op_add_assign
l_int|48
suffix:semicolon
id|method
op_add_assign
l_int|2
suffix:semicolon
)brace
r_while
c_loop
(paren
op_decrement
id|n
)paren
suffix:semicolon
r_return
l_int|0
suffix:semicolon
)brace
DECL|variable|alg
r_static
r_struct
id|crypto_alg
id|alg
op_assign
(brace
dot
id|cra_id
op_assign
id|CRYPTO_ALG_DES
comma
dot
id|cra_name
op_assign
l_string|&quot;des&quot;
comma
dot
id|cra_blocksize
op_assign
id|DES_BLOCK_SIZE
comma
dot
id|cra_ctxsize
op_assign
r_sizeof
(paren
r_struct
id|des_ctx
)paren
comma
dot
id|cra_u
op_assign
(brace
dot
id|cipher
op_assign
(brace
dot
id|cia_keysize
op_assign
id|DES_KEY_SIZE
comma
dot
id|cia_ivsize
op_assign
id|DES_BLOCK_SIZE
comma
dot
id|cia_setkey
op_assign
id|des_setkey
comma
dot
id|cia_encrypt
op_assign
id|des_small_fips_encrypt
comma
dot
id|cia_decrypt
op_assign
id|des_small_fips_decrypt
)brace
)brace
)brace
suffix:semicolon
DECL|function|init
r_static
r_int
id|__init
id|init
c_func
(paren
r_void
)paren
(brace
id|INIT_LIST_HEAD
c_func
(paren
op_amp
id|alg.cra_list
)paren
suffix:semicolon
r_return
id|crypto_register_alg
c_func
(paren
op_amp
id|alg
)paren
suffix:semicolon
)brace
DECL|function|fini
r_static
r_void
id|__exit
id|fini
c_func
(paren
r_void
)paren
(brace
id|crypto_unregister_alg
c_func
(paren
op_amp
id|alg
)paren
suffix:semicolon
)brace
DECL|variable|init
id|module_init
c_func
(paren
id|init
)paren
suffix:semicolon
DECL|variable|fini
id|module_exit
c_func
(paren
id|fini
)paren
suffix:semicolon
id|MODULE_LICENSE
c_func
(paren
l_string|&quot;GPL&quot;
)paren
suffix:semicolon
id|MODULE_DESCRIPTION
c_func
(paren
l_string|&quot;DES &amp; Triple DES EDE Cipher Algorithms&quot;
)paren
suffix:semicolon
macro_line|#if 0
multiline_comment|/*&n; * RFC2451:&n; *&n; * However, if the first two or last two independent 64-bit keys are&n; * equal (k1 == k2 or k2 == k3), then the 3DES operation is simply the&n; * same as DES.  Implementers MUST reject keys that exhibit this&n; * property.&n; */
macro_line|#endif
eof
