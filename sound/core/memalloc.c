...
r_static
r_inline
DECL|function|snd_malloc_dev_pages_fallback
r_void
op_star
id|snd_malloc_dev_pages_fallback
c_func
(paren
r_struct
id|device
...
r_static
r_inline
DECL|function|snd_free_isa_pages
r_void
...
multiline_comment|/**&n; * snd_dma_alloc_pages_fallback - allocate the buffer area according to the given type with fallback&n; * @dev: the buffer device info&n; * @size: the buffer size to allocate&n; * @dmab: buffer allocation record to store the allocated data&n; *&n; * Calls the memory-allocator function for the corresponding&n; * buffer type.  When no space is left, this function reduces the size and&n; * tries to allocate again.  The size actually allocated is stored in&n; * res_size argument.&n; * &n; * Returns zero if the buffer with the given size is allocated successfuly,&n; * other a negative value at error.&n; */
...
(paren
id|KERN_WARNING
l_string|&quot;snd-page-alloc: releasing the used block (type=%d, id=0x%x&bslash;n&quot;
comma
id|mem-&gt;dev.type
comma
id|mem-&gt;dev.id
...
(paren
id|KERN_WARNING
l_string|&quot;snd-page-alloc: can&squot;t malloc memlist&bslash;n&quot;
)paren
suffix:semicolon
r_break
suffix:semicolon
