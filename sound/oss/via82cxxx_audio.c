...
DECL|macro|VIA_MAX_BUFFER_DMA_PAGES
mdefine_line|#define VIA_MAX_BUFFER_DMA_PAGES&t;32
multiline_comment|/* buffering default values in ms */
DECL|macro|VIA_DEFAULT_FRAG_TIME
mdefine_line|#define VIA_DEFAULT_FRAG_TIME&t;&t;20
DECL|macro|VIA_DEFAULT_BUFFER_TIME
mdefine_line|#define VIA_DEFAULT_BUFFER_TIME&t;&t;500
multiline_comment|/* the hardware has a 256 fragment limit */
DECL|macro|VIA_MIN_FRAG_NUMBER
mdefine_line|#define VIA_MIN_FRAG_NUMBER&t;&t;2
DECL|macro|VIA_MAX_FRAG_NUMBER
mdefine_line|#define VIA_MAX_FRAG_NUMBER&t;&t;128
DECL|macro|VIA_MAX_FRAG_SIZE
mdefine_line|#define VIA_MAX_FRAG_SIZE&t;&t;PAGE_SIZE
DECL|macro|VIA_MIN_FRAG_SIZE
mdefine_line|#define VIA_MIN_FRAG_SIZE&t;&t;(VIA_MAX_BUFFER_DMA_PAGES * PAGE_SIZE / VIA_MAX_FRAG_NUMBER)
multiline_comment|/* 82C686 function 5 (audio codec) PCI configuration registers */
DECL|macro|VIA_ACLINK_STATUS
mdefine_line|#define VIA_ACLINK_STATUS&t;0x40
...
DECL|macro|VIA_PCM_LEFTVOL
mdefine_line|#define VIA_PCM_LEFTVOL&t;&t;&t;0x02
DECL|macro|VIA_PCM_RIGHTVOL
mdefine_line|#define VIA_PCM_RIGHTVOL&t;&t;0x03
DECL|macro|VIA_PCM_TABLE_ADDR
mdefine_line|#define VIA_PCM_TABLE_ADDR&t;&t;0x04
DECL|macro|VIA_PCM_STOPRATE
mdefine_line|#define VIA_PCM_STOPRATE&t;&t;0x08&t;/* 8233+ */
DECL|macro|VIA_PCM_BLOCK_COUNT
mdefine_line|#define VIA_PCM_BLOCK_COUNT&t;&t;0x0C
...
multiline_comment|/* Six channel audio output on 8233 */
DECL|macro|VIA_BASE0_MULTI_OUT_CHAN
mdefine_line|#define VIA_BASE0_MULTI_OUT_CHAN&t;&t;0x40
DECL|macro|VIA_BASE0_MULTI_OUT_CHAN_STATUS
mdefine_line|#define VIA_BASE0_MULTI_OUT_CHAN_STATUS&t;&t;0x40
DECL|macro|VIA_BASE0_MULTI_OUT_CHAN_CTRL
mdefine_line|#define VIA_BASE0_MULTI_OUT_CHAN_CTRL&t;&t;0x41
DECL|macro|VIA_BASE0_MULTI_OUT_CHAN_TYPE
mdefine_line|#define VIA_BASE0_MULTI_OUT_CHAN_TYPE&t;&t;0x42
...
multiline_comment|/* Newer VIA we need to monitor the low 3 bits of each channel. This&n;   mask covers the channels we don&squot;t yet use as well &n; */
DECL|macro|VIA_NEW_INTR_MASK
mdefine_line|#define VIA_NEW_INTR_MASK&t;&t;0x77077777UL
...
DECL|macro|VIA_SGD_PAUSED
mdefine_line|#define VIA_SGD_PAUSED&t;&t;&t;(1 &lt;&lt; 6)
...
l_int|1
suffix:semicolon
DECL|member|is_multi
r_int
id|is_multi
suffix:colon
l_int|1
suffix:semicolon
multiline_comment|/* 8233 6 channel */
DECL|member|pcm_fmt
id|u8
id|pcm_fmt
suffix:semicolon
...
l_int|1
suffix:semicolon
multiline_comment|/* Has legacy ports */
DECL|member|intmask
r_int
id|intmask
suffix:colon
l_int|1
suffix:semicolon
multiline_comment|/* Needs int bits */
DECL|member|sixchannel
r_int
id|sixchannel
suffix:colon
l_int|1
suffix:semicolon
multiline_comment|/* 8233/35 with 6 channel support */
DECL|member|volume
r_int
id|volume
suffix:colon
...
l_int|1
suffix:semicolon
DECL|member|mixer_vol
r_int
id|mixer_vol
suffix:semicolon
multiline_comment|/* 8233/35 volume  - not yet implemented */
...
id|open_sem
suffix:semicolon
multiline_comment|/* The 8233/8235 have 4 DX audio channels, two record and&n;&t;   one six channel out. We bind ch_in to DX 1, ch_out to multichannel&n;&t;   and ch_fm to DX 2. DX 3 and REC0/REC1 are unused at the&n;&t;   moment */
DECL|member|ch_in
r_struct
id|via_channel
id|ch_in
...
c_func
(paren
l_string|&quot;Start with intmask %d&bslash;n&quot;
comma
id|chan-&gt;intmask
)paren
suffix:semicolon
...
c_func
(paren
l_string|&quot;About to start from %d to %d&bslash;n&quot;
comma
id|inl
c_func
(paren
...
c_func
(paren
l_string|&quot;Status is now %02X&bslash;n&quot;
comma
id|inb
c_func
(paren
...
c_func
(paren
l_string|&quot;Control is now %02X&bslash;n&quot;
comma
id|inb
c_func
(paren
...
(paren
id|KERN_WARNING
l_string|&quot;via82cxxx warning: SG stopped or paused&bslash;n&quot;
)paren
suffix:semicolon
r_return
l_int|0
...
(brace
multiline_comment|/* Thomas Sailer:&n;&t; * EAGAIN is supposed to be used if IO is pending,&n;&t; * not if there is contention on some internal&n;&t; * synchronization primitive which should be&n;&t; * held only for a short time anyway&n;&t; */
id|nonblock
op_assign
l_int|0
suffix:semicolon
...
multiline_comment|/*&n;&t; * clear any enabled interrupt bits&n;&t; */
id|tmp
op_assign
id|inb
(paren
...
id|card-&gt;baseaddr
op_plus
id|VIA_BASE0_MULTI_OUT_CHAN_TYPE
)paren
suffix:semicolon
id|new_tmp
op_assign
...
suffix:semicolon
multiline_comment|/*&n;&t; * clear any existing flags&n;&t; */
id|via_chan_status_clear
(paren
id|card-&gt;baseaddr
op_plus
...
l_int|0x8000
suffix:semicolon
multiline_comment|/* The mute volume? -- Seems to work! */
id|DPRINTK
(paren
...
suffix:semicolon
multiline_comment|/* Save current state */
id|dacp
op_assign
id|via_ac97_read_reg
c_func
...
suffix:semicolon
multiline_comment|/* Mute - largely reduces popping */
id|via_ac97_write_reg
c_func
(paren
id|ac97
...
suffix:semicolon
multiline_comment|/* Set new rate */
id|via_ac97_write_reg
(paren
id|ac97
comma
...
suffix:semicolon
multiline_comment|/* Power DAC back up */
id|via_ac97_write_reg
c_func
(paren
id|ac97
...
suffix:semicolon
multiline_comment|/* reduces popping */
...
multiline_comment|/* Turn on the IRQ bits */
r_if
c_cond
(paren
id|chan
...
c_func
(paren
l_string|&quot;Using multichannel for pcm out&bslash;n&quot;
)paren
suffix:semicolon
)brace
r_else
...
multiline_comment|/**&n; *      via_chan_init - Initialize PCM channel&n; *      @card: Private audio chip info&n; *      @chan: Channel to be initialized&n; *&n; *      Performs some of the preparations necessary to begin&n; *      using a PCM channel.&n; *&n; *      Currently the preparations consist of&n; *      setting the PCM channel to a known state.&n; */
...
id|DPRINTK
(paren
l_string|&quot;dmabuf_pg #%d (h=%lx, v2p=%lx, a=%p)&bslash;n&quot;
comma
id|i
comma
(paren
...
id|DPRINTK
(paren
l_string|&quot;dmabuf #%d (32(h)=%lx)&bslash;n&quot;
comma
id|i
comma
(paren
...
multiline_comment|/* overwrite the last buffer information */
id|chan-&gt;sgtable
(braket
id|chan-&gt;frag_number
op_minus
...
suffix:semicolon
multiline_comment|/* load no rate adaption, stereo 16bit, set up ring slots */
r_if
c_cond
(paren
id|card-&gt;sixchannel
...
DECL|function|via_chan_buffer_free
r_static
r_void
id|via_chan_buffer_free
(paren
...
id|VIA_PCM_REC_FIFO
suffix:semicolon
multiline_comment|/* set interrupt select bits where applicable (PCM in &amp; out channels) */
r_if
c_cond
(paren
op_logical_neg
...
c_func
(paren
l_string|&quot;SET FMT - %02x %02x&bslash;n&quot;
comma
id|chan-&gt;intmask
comma
id|chan-&gt;is_multi
...
id|m
suffix:semicolon
multiline_comment|/*&n;&t;&t; *&t;Channel 0x4 is up to 6 x 16bit and has to be&n;&t;&t; *&t;programmed differently &n;&t;&t; */
r_if
c_cond
(paren
id|chan-&gt;is_multi
...
l_int|0
suffix:semicolon
multiline_comment|/*&n;&t;&t;&t; *&t;Load the type bit for num channels&n;&t;&t;&t; *&t;and 8/16bit&n;&t;&t;&t; */
r_if
c_cond
(paren
id|chan-&gt;pcm_fmt
...
suffix:semicolon
multiline_comment|/*&n;&t;&t;&t; *&t;Set the channel steering&n;&t;&t;&t; *&t;Mono&n;&t;&t;&t; *&t;&t;Channel 0 to slot 3&n;&t;&t;&t; *&t;&t;Channel 0 to slot 4&n;&t;&t;&t; *&t;Stereo&n;&t;&t;&t; *&t;&t;Channel 0 to slot 3&n;&t;&t;&t; *&t;&t;Channel 1 to slot 4&n;&t;&t;&t; */
r_switch
c_cond
(paren
id|chan-&gt;channels
...
r_else
(brace
multiline_comment|/*&n;&t;&t;&t; *&t;New style, turn off channel volume&n;&t;&t;&t; *&t;control, set bits in the right register&n;&t;&t;&t; */
id|outb
c_func
(paren
l_int|0x0
...
DECL|function|via_chan_set_buffering
r_static
r_int
id|via_chan_set_buffering
(paren
r_struct
id|via_info
op_star
...
suffix:semicolon
multiline_comment|/* in both cases the buffer cannot be changed */
r_if
c_cond
(paren
id|chan-&gt;is_active
...
multiline_comment|/* called outside SETFRAGMENT */
multiline_comment|/* set defaults or do nothing */
r_if
c_cond
(paren
id|val
...
id|DPRINTK
(paren
l_string|&quot;setting default values %d %d&bslash;n&quot;
comma
id|chan-&gt;frag_size
comma
id|chan-&gt;frag_number
...
id|DPRINTK
(paren
l_string|&quot;using user values %d %d&bslash;n&quot;
comma
id|chan-&gt;frag_size
comma
id|chan-&gt;frag_number
...
multiline_comment|/* quake3 wants frag_number to be a power of two */
id|shift
op_assign
l_int|0
suffix:semicolon
...
multiline_comment|/**&n; *&t;via_chan_flush_frag - Flush partially-full playback buffer to hardware&n; *&t;@chan: Channel whose DMA table will be flushed&n; *&n; *&t;Flushes partially-full playback buffer to hardware.&n; */
...
id|DPRINTK
(paren
l_string|&quot;MAYBE START %s&bslash;n&quot;
comma
id|chan-&gt;name
)paren
suffix:semicolon
...
suffix:semicolon
macro_line|#if 0
multiline_comment|/*&n;&t; *&t;Intercept volume control on 8233 and 8235&n;&t; */
r_if
c_cond
(paren
id|card-&gt;volume
...
macro_line|#endif
multiline_comment|/*&n;         * Reset AC97 controller: enable, disable, enable,&n;         * pausing after each command for good luck.  Only&n;&t; * do this if the codec is not ready, because it causes&n;&t; * loud pops and such due to such a hard codec reset.&n;         */
id|pci_read_config_byte
(paren
id|pdev
comma
id|VIA_ACLINK_STATUS
comma
op_amp
id|tmp8
)paren
...
multiline_comment|/* Make sure VRA is enabled, in case we didn&squot;t do a&n;&t; * complete codec reset, above&n;&t; */
id|pci_read_config_byte
(paren
id|pdev
comma
...
id|chan-&gt;frag_size
suffix:semicolon
multiline_comment|/* FIXME - signed overflow is undefined */
r_if
c_cond
(paren
id|chan-&gt;bytes
...
(brace
multiline_comment|/* If we are recording, then n_frags represents the number&n;&t;&t; * of fragments waiting to be handled by userspace.&n;&t;&t; * If we are playback, then n_frags represents the number&n;&t;&t; * of fragments remaining to be filled by userspace.&n;&t;&t; * We increment here.  If we reach max number of fragments,&n;&t;&t; * this indicates an underrun/overrun.  For this case under OSS,&n;&t;&t; * we stop the record/playback process.&n;&t;&t; */
r_if
c_cond
(paren
id|atomic_read
...
id|DPRINTK
(paren
l_string|&quot;%s intr, channel n_frags == %d, missed %d&bslash;n&quot;
comma
id|chan-&gt;name
comma
id|atomic_read
...
DECL|function|via_new_interrupt
r_static
id|irqreturn_t
id|via_new_interrupt
c_func
(paren
r_int
id|irq
...
id|status32
op_amp
id|VIA_NEW_INTR_MASK
)paren
)paren
r_return
id|IRQ_NONE
suffix:semicolon
multiline_comment|/*&n;&t; * goes away completely on UP&n;&t; */
id|spin_lock
(paren
op_amp
id|card-&gt;lock
...
multiline_comment|/* VIA requires this is done */
id|pci_write_config_byte
c_func
(paren
id|card-&gt;pdev
...
(brace
multiline_comment|/* make sure FM irq is not routed to us */
id|pci_read_config_byte
(paren
id|card-&gt;pdev
comma
...
multiline_comment|/* Thomas Sailer:&n;&t; * But also to ourselves, release semaphore if we do so */
r_if
c_cond
(paren
id|need_resched
...
id|chan-&gt;sw_ptr
suffix:semicolon
multiline_comment|/* n_frags represents the number of fragments waiting&n;&t; * to be copied to userland.  sleep until at least&n;&t; * one buffer has been read from the audio hardware.&n;&t; */
id|add_wait_queue
c_func
(paren
op_amp
...
id|DPRINTK
(paren
l_string|&quot;Flushed block %u, sw_ptr now %u, n_frags now %d&bslash;n&quot;
comma
id|n
comma
id|chan-&gt;sw_ptr
...
multiline_comment|/* grab current channel fragment pointer.  In the case of&n;&t; * playback, this is pointing to the next fragment that&n;&t; * should receive data from userland.&n;&t; */
id|n
op_assign
id|chan-&gt;sw_ptr
suffix:semicolon
multiline_comment|/* n_frags represents the number of fragments remaining&n;&t; * to be filled by userspace.  Sleep until&n;&t; * at least one fragment is available for our use.&n;&t; */
id|add_wait_queue
c_func
(paren
op_amp
...
id|DPRINTK
(paren
l_string|&quot;Sleeping on page %d, tmp==%d, ir==%d&bslash;n&quot;
comma
id|n
comma
id|tmp
...
id|out
suffix:semicolon
multiline_comment|/* Now that we have at least one fragment we can write to, fill the buffer&n;&t; * as much as possible with data from userspace.&n;&t; */
r_while
c_loop
(paren
(paren
...
multiline_comment|/* If we didn&squot;t fill up the buffer with data, stop now.&n;         * Put a &squot;stop&squot; marker in the DMA table too, to tell the&n;         * audio hardware to stop if it gets here.&n;         */
r_if
c_cond
(paren
id|chan-&gt;slop_len
...
multiline_comment|/*&n;         * If we get to this point, we have filled a buffer with&n;         * audio data, flush the buffer to audio hardware.&n;         */
...
id|DPRINTK
(paren
l_string|&quot;regs==S=%02X C=%02X TP=%02X BP=%08X RT=%08X SG=%08X CC=%08X SS=%08X&bslash;n&quot;
comma
id|inb
(paren
id|card-&gt;baseaddr
...
id|DPRINTK
(paren
l_string|&quot;FRAGS %d FRAGNUM %d&bslash;n&quot;
comma
id|atomic_read
c_func
(paren
...
r_break
suffix:semicolon
multiline_comment|/* query or set number of channels (1=mono, 2=stereo, 4/6 for multichannel) */
r_case
...
id|val
op_assign
id|card-&gt;ch_in.channels
suffix:semicolon
r_else
id|val
op_assign
id|card-&gt;ch_out.channels
suffix:semicolon
)brace
id|DPRINTK
(paren
...
suffix:semicolon
id|card-&gt;ch_in.frag_number
op_assign
l_int|0
suffix:semicolon
id|card-&gt;ch_in.frag_size
op_assign
l_int|0
suffix:semicolon
id|atomic_set
...
suffix:semicolon
id|card-&gt;ch_out.frag_number
op_assign
l_int|0
suffix:semicolon
id|card-&gt;ch_out.frag_size
op_assign
l_int|0
suffix:semicolon
id|atomic_set
...
r_break
suffix:semicolon
multiline_comment|/* obtain buffer fragment size */
r_case
...
l_int|2
suffix:semicolon
singleline_comment|// TO DO - use FIFO: via_capture_fifo(card, 1);
id|via_chan_pcm_fmt
(paren
id|chan
comma
...
(brace
multiline_comment|/* if in duplex mode make the recording and playback channels&n;&t;&t;&t;   have the same settings */
id|chan-&gt;pcm_fmt
op_assign
id|VIA_PCM_FMT_16BIT
op_or
...
(paren
id|KERN_INFO
l_string|&quot;Via 686a/8233/8235 audio driver &quot;
id|VIA_VERSION
l_string|&quot;&bslash;n&quot;
)paren
suffix:semicolon
...
l_int|1
suffix:semicolon
multiline_comment|/* Overkill for now, but more flexible done right */
id|card-&gt;intmask
op_assign
id|id-&gt;driver_data
suffix:semicolon
...
id|KERN_INFO
id|PFX
l_string|&quot;Six channel audio available&bslash;n&quot;
)paren
suffix:semicolon
)brace
r_if
