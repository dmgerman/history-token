...
singleline_comment|//#define DEBUG_MMAP
singleline_comment|//#define DEBUG_MMIO
...
op_assign
l_int|0x80
multiline_comment|/* SDATA_IN Map Register */
)brace
suffix:semicolon
...
suffix:semicolon
multiline_comment|/* These are capabilities (and bugs) the chipsets _can_ have */
r_static
r_struct
(brace
DECL|member|nr_ac97
...
suffix:semicolon
DECL|macro|CAP_MMIO
mdefine_line|#define CAP_MMIO                 0x0001
DECL|macro|CAP_20BIT_AUDIO_SUPPORT
mdefine_line|#define CAP_20BIT_AUDIO_SUPPORT  0x0002
DECL|member|flags
id|u_int16_t
id|flags
suffix:semicolon
DECL|variable|card_cap
)brace
id|card_cap
(braket
)braket
...
comma
multiline_comment|/* NVIDIA_NFORCE */
multiline_comment|/*@FIXME to be verified*/
(brace
l_int|2
comma
...
comma
id|PCI_DEVICE_ID_NVIDIA_MCP1_AUDIO
comma
id|PCI_ANY_ID
comma
id|PCI_ANY_ID
...
id|pci_id
suffix:semicolon
DECL|member|pci_id_internal
id|u16
id|pci_id_internal
suffix:semicolon
multiline_comment|/* used to access card_cap[] */
...
id|channel
suffix:semicolon
multiline_comment|/* 1:1 to states[] but diff. lifetime */
DECL|member|chandma
id|dma_addr_t
id|chandma
suffix:semicolon
...
id|irq
suffix:semicolon
DECL|member|ac97base_mmio_phys
r_int
r_int
id|ac97base_mmio_phys
suffix:semicolon
DECL|member|iobase_mmio_phys
r_int
r_int
id|iobase_mmio_phys
suffix:semicolon
...
mdefine_line|#define I810_IOREADW(card, off)&t;&t;I810_IOREAD(w, u16, card, off)
...
mdefine_line|#define I810_IOWRITEW(val, card, off)&t;I810_IOWRITE(w, val, card, off)
DECL|macro|I810_IOWRITEB
mdefine_line|#define I810_IOWRITEB(val, card, off)&t;I810_IOWRITE(b, val, card, off)
DECL|macro|GET_CIV
mdefine_line|#define GET_CIV(card, port) MODULOP2(I810_IOREADB((card), (port) + OFF_CIV), SG_LEN)
DECL|macro|GET_LVI
mdefine_line|#define GET_LVI(card, port) MODULOP2(I810_IOREADB((card), (port) + OFF_LVI), SG_LEN)
multiline_comment|/* set LVI from CIV */
DECL|macro|CIV_TO_LVI
mdefine_line|#define CIV_TO_LVI(card, port, off) &bslash;&n;&t;I810_IOWRITEB(MODULOP2(GET_CIV((card), (port)) + (off), SG_LEN), (card), (port) + OFF_LVI)
...
suffix:semicolon
multiline_comment|/* No codec, no setup */
r_if
c_cond
(paren
id|codec
...
id|ADC_RUNNING
suffix:semicolon
singleline_comment|// Interrupt enable, LVI enable, DMA enable
id|I810_IOWRITEB
c_func
(paren
l_int|0x10
...
multiline_comment|/* Do not process partial fragments. */
id|fragsize
op_assign
id|dmabuf-&gt;fragsize
suffix:semicolon
...
multiline_comment|/* MASKP2(swptr, fragsize) - 1 is the tail of our transfer */
id|x
op_assign
id|MODULOP2
c_func
...
c_func
(paren
l_string|&quot;ADC HWP %d,%d,%d&bslash;n&quot;
comma
id|hwptr
comma
id|dmabuf-&gt;hwptr
...
DECL|function|i810_get_free_write_space
r_static
r_inline
r_int
...
DECL|function|i810_get_available_read_data
r_static
r_inline
r_int
...
DECL|function|fill_partial_frag
r_static
r_inline
r_void
id|fill_partial_frag
c_func
(paren
r_struct
id|dmabuf
...
suffix:semicolon
multiline_comment|/* &n;&t; * This will make sure that our LVI is correct, that our&n;&t; * pointer is updated, and that the DAC is running.  We&n;&t; * have to force the setting of dmabuf-&gt;trigger to avoid&n;&t; * any possible deadlocks.&n;&t; */
id|dmabuf-&gt;trigger
op_assign
id|PCM_ENABLE_OUTPUT
suffix:semicolon
...
(brace
multiline_comment|/* only wake_up() waiters if this interrupt signals&n;&t;&t;&t; * us being beyond a userfragsize of data open or&n;&t;&t;&t; * available, and i810_update_ptr() does that for&n;&t;&t;&t; * us&n;&t;&t;&t; */
id|i810_update_ptr
c_func
(paren
id|state
...
(brace
multiline_comment|/* wake_up() unconditionally on LVI and DCH */
id|i810_update_ptr
c_func
(paren
id|state
...
suffix:semicolon
macro_line|#endif
multiline_comment|/* silently ignore invalid PCM_ENABLE_xxx bits,&n;&t;&t; * like the other drivers do&n;&t;&t; */
r_if
c_cond
(paren
op_logical_neg
...
singleline_comment|//return put_user((val &amp; 0xcfff), p);
r_return
id|put_user
c_func
(paren
...
suffix:semicolon
multiline_comment|/* Put the ACLink in 2 channel mode by default */
id|i
op_assign
id|I810_IOREADL
c_func
...
DECL|function|i810_ac97_get_mmio
r_static
id|u16
id|i810_ac97_get_mmio
c_func
...
(paren
id|KERN_DEBUG
l_string|&quot;i810_audio: ac97_get_mmio(%d) -&gt; 0x%04X&bslash;n&quot;
comma
(paren
(paren
r_int
...
DECL|function|i810_ac97_get_io
r_static
id|u16
id|i810_ac97_get_io
c_func
...
DECL|function|i810_ac97_set_mmio
r_static
r_void
id|i810_ac97_set_mmio
c_func
...
(paren
id|KERN_DEBUG
l_string|&quot;i810_audio: ac97_set_mmio(0x%04X, %d)&bslash;n&quot;
comma
(paren
id|u32
)paren
...
DECL|function|i810_ac97_set_io
r_static
r_void
id|i810_ac97_set_io
c_func
...
multiline_comment|/* At this point we deassert AC_RESET # */
id|I810_IOWRITEL
c_func
(paren
id|reg
...
suffix:semicolon
multiline_comment|/* We must now allow time for the Codec initialisation.&n;&t;   600mS is the specified time */
r_for
c_loop
(paren
id|i
...
(paren
id|KERN_INFO
l_string|&quot;i810_audio: Primary codec has ID %d&bslash;n&quot;
comma
id|primary_codec_id
)paren
suffix:semicolon
...
(paren
id|KERN_INFO
l_string|&quot;i810_audio: Codec not ready.. wait.. &quot;
)paren
suffix:semicolon
id|set_current_state
c_func
...
suffix:semicolon
multiline_comment|/* actually 600mS by the spec */
r_if
c_cond
(paren
id|i810_ac97_exists
...
(paren
id|KERN_INFO
l_string|&quot;i810_audio: Defaulting to base 2 channel mode.&bslash;n&quot;
)paren
suffix:semicolon
id|reg
op_assign
...
l_int|NULL
suffix:semicolon
multiline_comment|/*@FIXME I don&squot;t know, if I&squot;m playing to safe here... (jsaw) */
r_if
c_cond
(paren
(paren
...
(brace
multiline_comment|/* codec reset */
id|printk
c_func
(paren
id|KERN_INFO
l_string|&quot;i810_audio: Resetting connection %d&bslash;n&quot;
comma
id|num_ac97
)paren
suffix:semicolon
...
suffix:semicolon
multiline_comment|/* If we have the SDATA_IN Map Register, as on ICH4, we&n;&t;&t;   do not loop thru all possible codec IDs but thru all &n;&t;&t;   possible IO channels. Bit 0:1 of SDM then holds the &n;&t;&t;   last codec ID spoken to. &n;&t;&t;*/
r_if
c_cond
(paren
id|ich_use_mmio
...
(paren
id|KERN_INFO
l_string|&quot;i810_audio: Connection %d with codec id %d&bslash;n&quot;
comma
id|num_ac97
comma
id|ac97_id
...
multiline_comment|/* Turn on the amplifier */
id|codec
op_member_access_from_pointer
id|codec_write
c_func
...
multiline_comment|/* if chipset could have mmio capability, check it */
r_if
c_cond
(paren
id|card_cap
...
(paren
id|KERN_ERR
l_string|&quot;i810_audio: No I/O resources available.&bslash;n&quot;
)paren
suffix:semicolon
r_goto
id|out_mem
...
(paren
id|KERN_INFO
l_string|&quot;i810: %s found at IO 0x%04lx and 0x%04lx, &quot;
l_string|&quot;MEM 0x%04lx and 0x%04lx, IRQ %d&bslash;n&quot;
comma
id|card_names
(braket
id|pci_id-&gt;driver_data
...
(paren
id|KERN_ERR
l_string|&quot;i810: cannot allocate channel DMA memory&bslash;n&quot;
)paren
suffix:semicolon
r_goto
id|out_mem
...
(brace
multiline_comment|/* We may dispose of this altogether some time soon, so... */
r_struct
id|i810_channel
op_star
id|cp
...
(brace
multiline_comment|/*@FIXME can ioremap fail? don&squot;t know (jsaw) */
r_if
c_cond
(paren
id|request_mem_region
...
(paren
id|KERN_INFO
l_string|&quot;i810: %s mmio at 0x%04lx and 0x%04lx&bslash;n&quot;
comma
id|card_names
(braket
id|pci_id-&gt;driver_data
...
mdefine_line|#define I810_MODULE_NAME &quot;i810_audio&quot;
...
c_func
(paren
l_string|&quot;i810_audio: S/PDIF can only be locked to 32000, 44100, or 48000Hz.&bslash;n&quot;
)paren
suffix:semicolon
id|spdif_locked
op_assign
