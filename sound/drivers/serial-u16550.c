multiline_comment|/*&n; *   serial.c&n; *   Copyright (c) by Jaroslav Kysela &lt;perex@suse.cz&gt;,&n; *                    Isaku Yamahata &lt;yamahata@private.email.ne.jp&gt;,&n; *&t;&t;      George Hansper &lt;ghansper@apana.org.au&gt;,&n; *&t;&t;      Hannu Savolainen&n; *&n; *   This code is based on the code from ALSA 0.5.9, but heavily rewritten.&n; *&n; * Sat Mar 31 17:27:57 PST 2001 tim.mann@compaq.com &n; *      Added support for the Midiator MS-124T and for the MS-124W in&n; *      Single Addressed (S/A) or Multiple Burst (M/B) mode, with&n; *      power derived either parasitically from the serial port or&n; *      from a separate power supply.&n; * &n; *      The new snd_adaptor module parameter allows you to select&n; *      either the default Roland Soundcanvas support (0), which was&n; *      previously included in this driver but was not documented,&n; *      Midiator MS-124T support (1), Midiator MS-124W S/A mode&n; *      support (2), or MS-124W M/B mode support (3).  For the&n; *      Midiator MS-124W, you must set the physical M-S and A-B&n; *      switches on the Midiator to match the driver mode you select.&n; *  &n; *      - In Roland Soundcanvas mode, multiple ALSA raw MIDI&n; *      substreams are supported (midiCnD0-midiCnD15).  Whenever you&n; *      write to a different substream, the driver sends the&n; *      nonstandard MIDI command sequence F5 NN, where NN is the&n; *      substream number plus 1.  Roland modules use this command to&n; *      switch between different &quot;parts&quot;, so this feature lets you&n; *      treat each part as a distinct raw MIDI substream.  The driver&n; *      provides no way to send F5 00 (no selection) or to not send&n; *      the F5 NN command sequence at all; perhaps it ought to.&n; * &n; *      - In MS-124T mode, one raw MIDI substream is supported&n; *      (midiCnD0); the snd_outs module parameter is automatically set&n; *      to 1.  The driver sends the same data to all four MIDI Out&n; *      connectors.  Set the A-B switch and the snd_speed module&n; *      parameter to match (A=19200, B=9600).&n; * &n; *      Usage example for MS-124T, with A-B switch in A position:&n; *        setserial /dev/ttyS0 uart none&n; *        /sbin/modprobe snd-serial-u16550 snd_port=0x3f8 snd_irq=4 &bslash;&n; *            snd_adaptor=1 snd_speed=19200&n; *&n; *      - In MS-124W S/A mode, one raw MIDI substream is supported&n; *      (midiCnD0); the snd_outs module parameter is automatically set&n; *      to 1.  The driver sends the same data to all four MIDI Out&n; *      connectors at full MIDI speed.&n; * &n; *      Usage example for S/A mode:&n; *        setserial /dev/ttyS0 uart none&n; *        /sbin/modprobe snd-serial-u16550 snd_port=0x3f8 snd_irq=4 &bslash;&n; *            snd_adaptor=2&n; *&n; *      - In MS-124W M/B mode, the driver supports 16 ALSA raw MIDI&n; *      substreams; the snd_outs module parameter is automatically set&n; *      to 16.  The substream number gives a bitmask of which MIDI Out&n; *      connectors the data should be sent to, with midiCnD1 sending&n; *      to Out 1, midiCnD2 to Out 2, midiCnD4 to Out 3, and midiCnD8&n; *      to Out 4.  Thus midiCnD15 sends the data to all 4 ports.  As a&n; *      special case, midiCnD0 also sends to all ports, since it is&n; *      not useful to send the data to no ports.  M/B mode has extra&n; *      overhead to select the MIDI Out for each byte, so the&n; *      aggregate data rate across all four MIDI Outs is at most one&n; *      byte every 520 us, as compared with the full MIDI data rate of&n; *      one byte every 320 us per port.&n; * &n; *      Usage example for M/B mode:&n; *        setserial /dev/ttyS0 uart none&n; *        /sbin/insmod snd-serial-u16550 snd_port=0x3f8 snd_irq=4 &bslash;&n; *            snd_adaptor=3&n; *&n; *      - The MS-124W hardware&squot;s M/A mode is currently not supported.&n; *      This mode allows the MIDI Outs to act independently at double&n; *      the aggregate throughput of M/B, but does not allow sending&n; *      the same byte simultaneously to multiple MIDI Outs.  The M/A&n; *      protocol requires the driver to twiddle the modem control&n; *      lines under timing constraints, so it would be a bit more&n; *      complicated to implement than the other modes.&n; *&n; *      - Midiator models other than MS-124W and MS-124T are currently&n; *      not supported.  Note that the suffix letter is significant;&n; *      the MS-124 and MS-124B are not compatible, nor are the other&n; *      known models MS-101, MS-101B, MS-103, and MS-114.  I do have&n; *      documentation that partially covers these models, but no units&n; *      to experiment with.  The MS-124W support is tested with a real&n; *      unit.  The MS-124T support is untested, but should work.&n; *&n; *   This program is free software; you can redistribute it and/or modify&n; *   it under the terms of the GNU General Public License as published by&n; *   the Free Software Foundation; either version 2 of the License, or&n; *   (at your option) any later version.&n; *&n; *   This program is distributed in the hope that it will be useful,&n; *   but WITHOUT ANY WARRANTY; without even the implied warranty of&n; *   MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the&n; *   GNU General Public License for more details.&n; *&n; *   You should have received a copy of the GNU General Public License&n; *   along with this program; if not, write to the Free Software&n; *   Foundation, Inc., 59 Temple Place, Suite 330, Boston, MA  02111-1307 USA&n; *&n; */
