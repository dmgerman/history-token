...
multiline_comment|/**&n; * snd_ac97_bus - create an AC97 bus component&n; * @card: the card instance&n; * @_bus: the template of AC97 bus, callbacks and&n; *         the private data.&n; * @rbus: the pointer to store the new AC97 bus instance.&n; *&n; * Creates an AC97 bus component.  An ac97_bus_t instance is newly&n; * allocated and initialized from the template (_bus).&n; *&n; * The template must include the valid callbacks (at least read and&n; * write), the bus number (num), and the private data (private_data).&n; * The other callbacks, wait and reset, are not mandatory.&n; * &n; * The clock is set to 48000.  If another clock is needed, set&n; * bus-&gt;clock manually.&n; *&n; * The AC97 bus instance is registered as a low-level device, so you don&squot;t&n; * have to release it manually.&n; *&n; * Returns zero if successful, or a negative error code on failure.&n; */
...
multiline_comment|/**&n; * snd_ac97_mixer - create an Codec97 component&n; * @bus: the AC97 bus which codec is attached to&n; * @_ac97: the template of ac97, including index, callbacks and&n; *         the private data.&n; * @rac97: the pointer to store the new ac97 instance.&n; *&n; * Creates an Codec97 component.  An ac97_t instance is newly&n; * allocated and initialized from the template (_ac97).  The codec&n; * is then initialized by the standard procedure.&n; *&n; * The template must include the valid callbacks (at least read and&n; * write), the codec number (num) and address (addr), and the private&n; * data (private_data).  The other callbacks, wait and reset, are not&n; * mandatory.&n; * &n; * The ac97 instance is registered as a low-level device, so you don&squot;t&n; * have to release it manually.&n; *&n; * Returns zero if successful, or a negative error code on failure.&n; */
