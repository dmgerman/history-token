...
mdefine_line|#define USE_AES_IEC958
...
DECL|macro|CM_VCO
mdefine_line|#define CM_VCO&t;&t;&t;0x08000000&t;/* Voice Control? CMI8738 */
DECL|macro|CM_MCBINT
mdefine_line|#define CM_MCBINT&t;&t;0x04000000&t;/* Master Control Block abort cond.? */
...
DECL|macro|CM_XDO46
mdefine_line|#define CM_XDO46&t;&t;0x00000080&t;/* Modell 033? Direct programming EEPROM (read data register) */
DECL|macro|CM_LHBTOG
mdefine_line|#define CM_LHBTOG&t;&t;0x00000040&t;/* High/Low status from DMA ctrl register */
DECL|macro|CM_LEG_HDMA
mdefine_line|#define CM_LEG_HDMA&t;&t;0x00000020&t;/* Legacy is in High DMA channel */
DECL|macro|CM_LEG_STEREO
mdefine_line|#define CM_LEG_STEREO&t;&t;0x00000010&t;/* Legacy is in Stereo mode */
...
DECL|macro|CM_REFFREQ_XIN
mdefine_line|#define CM_REFFREQ_XIN&t;&t;(315*1000*1000)/22&t;/* 14.31818 Mhz reference clock frequency pin XIN */
DECL|macro|CM_ADCMULT_XIN
mdefine_line|#define CM_ADCMULT_XIN&t;&t;512&t;&t;&t;/* Guessed (487 best for 44.1kHz, not for 88/176kHz) */
DECL|macro|CM_TOLERANCE_RATE
mdefine_line|#define CM_TOLERANCE_RATE&t;0.001&t;&t;&t;/* Tolerance sample rate pitch (1000ppm) */
DECL|macro|CM_MAXIMUM_RATE
mdefine_line|#define CM_MAXIMUM_RATE&t;&t;80000000&t;&t;/* Note more than 80MHz */
...
multiline_comment|/*&n; * For CMI-8338 (0x28 - 0x2b) .. is this valid for CMI-8738&n; * or identical with AC97 codec?&n; */
DECL|macro|CM_REG_EXTERN_CODEC
mdefine_line|#define CM_REG_EXTERN_CODEC&t;CM_REG_AC97
multiline_comment|/*&n; * MPU401 pci port index address 0x40 - 0x4f (CMI-8738 spec ver. 0.6)&n; */
DECL|macro|CM_REG_MPU_PCI
mdefine_line|#define CM_REG_MPU_PCI&t;&t;0x40
multiline_comment|/*&n; * FM pci port index address 0x50 - 0x5f (CMI-8738 spec ver. 0.6)&n; */
DECL|macro|CM_REG_FM_PCI
mdefine_line|#define CM_REG_FM_PCI&t;&t;0x50
multiline_comment|/*&n; * for CMI-8338 .. this is not valid for CMI-8738.&n; */
DECL|macro|CM_REG_EXTENT_IND
mdefine_line|#define CM_REG_EXTENT_IND&t;0xf0
DECL|macro|CM_VPHONE_MASK
mdefine_line|#define CM_VPHONE_MASK&t;&t;0xe0&t;/* Phone volume control (0-3) &lt;&lt; 5 */
DECL|macro|CM_VPHONE_SHIFT
mdefine_line|#define CM_VPHONE_SHIFT&t;&t;5
DECL|macro|CM_VPHOM
mdefine_line|#define CM_VPHOM&t;&t;0x10&t;/* Phone mute control */
DECL|macro|CM_VSPKM
mdefine_line|#define CM_VSPKM&t;&t;0x08&t;/* Speaker mute control, default high */
DECL|macro|CM_RLOOPREN
mdefine_line|#define CM_RLOOPREN&t;&t;0x04    /* Rec. R-channel enable */
DECL|macro|CM_RLOOPLEN
mdefine_line|#define CM_RLOOPLEN&t;&t;0x02&t;/* Rec. L-channel enable */
multiline_comment|/*&n; * CMI-8338 spec ver 0.5 (this is not valid for CMI-8738):&n; * the 8 registers 0xf8 - 0xff are used for programming m/n counter by the PLL&n; * unit (readonly?).&n; */
DECL|macro|CM_REG_PLL
mdefine_line|#define CM_REG_PLL&t;&t;0xf8
...
DECL|macro|CM_OPEN_MCHAN
mdefine_line|#define CM_OPEN_MCHAN&t;0x80
...
DECL|macro|CM_OPEN_PLAYBACK_MULTI
mdefine_line|#define CM_OPEN_PLAYBACK_MULTI&t;(CM_CH_PLAY | CM_OPEN_DAC | CM_OPEN_MCHAN)
...
mdefine_line|#define CM_SAVED_MIXERS&t;&t;ARRAY_SIZE(cm_saved_mixer)
...
id|dig_pcm_status
suffix:semicolon
macro_line|#ifdef USE_AES_IEC958
DECL|member|spdif_channel
...
macro_line|#ifdef USE_VAR48KRATE
multiline_comment|/*&n; * Determine PLL values for frequency setup, maybe the CMI8338 (CMI8738???)&n; * does it this way .. maybe not.  Never get any information from C-Media about&n; * that &lt;werner@suse.de&gt;.&n; */
DECL|function|snd_cmipci_pll_rmn
r_static
r_int
id|snd_cmipci_pll_rmn
c_func
(paren
r_int
r_int
...
id|rate
suffix:semicolon
multiline_comment|/*&n;&t;&t;&t; * If we found one, remember this,&n;&t;&t;&t; * and try to find a closer one&n;&t;&t;&t; */
r_if
c_cond
(paren
id|delta
...
multiline_comment|/*&n; * Program pll register bits, I assume that the 8 registers 0xf8 upto 0xff&n; * are mapped onto the 8 ADC/DAC sampling frequency which can be choosen&n; * at the register CM_REG_FUNCTRL1 (0x04).&n; * Problem: other ways are also possible (any information about that?)&n; */
DECL|function|snd_cmipci_set_pll
r_static
r_void
id|snd_cmipci_set_pll
c_func
(paren
id|cmipci_t
op_star
...
id|slot
suffix:semicolon
multiline_comment|/*&n;&t; * Guess that this programs at reg. 0x04 the pos 15:13/12:10&n;&t; * for DSFC/ASFC (000 upto 111).&n;&t; */
multiline_comment|/* FIXME: Init (Do we&squot;ve to set an other register first before programming?) */
multiline_comment|/* FIXME: Is this correct? Or shouldn&squot;t the m/n/r values be used for that? */
id|snd_cmipci_write_b
c_func
(paren
id|cm
...
suffix:semicolon
multiline_comment|/* FIXME: Setup (Do we&squot;ve to set an other register first to enable this?) */
)brace
...
DECL|function|snd_cmipci_playback2_hw_params
r_static
r_int
id|snd_cmipci_playback2_hw_params
c_func
(paren
id|snd_pcm_substream_t
op_star
...
multiline_comment|/* reserve the channel A */
id|cm-&gt;opened
(braket
id|CM_CH_PLAY
)braket
op_assign
id|CM_OPEN_PLAYBACK_MULTI
suffix:semicolon
id|up
c_func
(paren
...
DECL|function|snd_cmipci_ch_reset
r_static
r_void
id|snd_cmipci_ch_reset
c_func
(paren
id|cmipci_t
op_star
...
(brace
multiline_comment|/* multi-channels */
id|rec-&gt;dma_size
op_assign
(paren
id|rec-&gt;dma_size
...
macro_line|# ifndef USE_AES_IEC958
...
macro_line|# else  /* if USE_AES_IEC958 */
multiline_comment|/*&n; * The bitstream handling&n; */
DECL|struct|iec958_stru_bitstream
r_typedef
r_struct
id|iec958_stru_bitstream
(brace
DECL|member|data
id|u32
op_star
id|data
suffix:semicolon
multiline_comment|/* Holds the current position */
DECL|member|left
id|u32
id|left
suffix:semicolon
multiline_comment|/* Bits left in current 32bit frame */
DECL|member|word
id|u32
id|word
suffix:semicolon
multiline_comment|/* The 32bit frame of the current position */
DECL|member|bits
id|u32
id|bits
suffix:semicolon
multiline_comment|/* All bits together */
DECL|member|err
r_int
id|err
suffix:semicolon
multiline_comment|/* Error condition */
DECL|typedef|iec958_bitstream_t
)brace
id|iec958_bitstream_t
suffix:semicolon
DECL|variable|bs
r_static
id|iec958_bitstream_t
id|bs
suffix:semicolon
multiline_comment|/* Initialize ptr on the buffer */
DECL|function|iec958_init_bitstream
r_static
r_void
id|iec958_init_bitstream
c_func
(paren
id|u8
op_star
...
id|buf
suffix:semicolon
multiline_comment|/* Set initial position */
id|bs.word
op_assign
op_star
id|bs.data
suffix:semicolon
multiline_comment|/* The first 32bit frame */
id|bs.left
op_assign
l_int|32
suffix:semicolon
multiline_comment|/* has exactly 32bits */
id|bs.bits
op_assign
id|size
suffix:semicolon
...
multiline_comment|/* Remove ptr on the buffer */
DECL|function|iec958_clear_bitstream
r_static
r_void
id|iec958_clear_bitstream
c_func
(paren
r_void
)paren
...
multiline_comment|/* Get bits from bitstream (max 32) */
DECL|function|iec958_getbits
r_static
r_inline
id|u32
id|iec958_getbits
...
id|bits
suffix:semicolon
macro_line|#  ifdef WORDS_BIGENDIAN
r_if
c_cond
(paren
id|bits
...
(brace
multiline_comment|/* Within 32bit frame */
id|res
op_assign
(paren
id|bs.word
...
multiline_comment|/* We may cross the frame boundary */
id|res
op_assign
(paren
id|bs.word
...
suffix:semicolon
multiline_comment|/* Next 32bit frame */
r_if
c_cond
(paren
id|bits
)paren
multiline_comment|/* Add remaining bits, if any */
id|res
op_assign
(paren
id|res
...
suffix:semicolon
macro_line|#  else  /* not WORDS_BIGENDIAN */
r_if
c_cond
(paren
id|bits
...
macro_line|#  endif /* not WORDS_BIGENDIAN */
id|bs.left
op_assign
(paren
l_int|32
...
DECL|function|iec958_bits_avail
r_static
r_inline
id|u32
id|iec958_bits_avail
c_func
(paren
r_void
)paren
...
multiline_comment|/*&n; * Determine parity for time slots 4 upto 30&n; * to be sure that bit 4 upt 31 will carry&n; * an even number of ones and zeros.&n; */
DECL|function|iec958_parity
r_static
id|u32
id|iec958_parity
c_func
...
multiline_comment|/*&n; * Compose 32bit iec958 subframe, two sub frames&n; * build one frame with two channels.&n; *&n; * bit 0-3  = preamble&n; *     4-7  = AUX (=0)&n; *     8-27 = data (12-27 for 16bit, 8-27 for 20bit, and 24bit without AUX)&n; *     28   = validity (0 for valid data, else &squot;in error&squot;)&n; *     29   = user data (0)&n; *     30   = channel status (24 bytes for 192 frames)&n; *     31   = parity&n; */
DECL|function|iec958_subframe
r_static
r_inline
id|u32
id|iec958_subframe
...
(brace
multiline_comment|/* Does this work for LE systems ??? */
r_if
c_cond
(paren
id|status
...
multiline_comment|/*&n;&t; * Set one of the 192 bits of the channel status (AES3 and higher)&n;&t; */
r_if
c_cond
(paren
id|status
...
l_int|0x03
suffix:semicolon
multiline_comment|/* Block start, &squot;Z&squot; */
r_else
r_if
c_cond
(paren
...
l_int|0x05
suffix:semicolon
multiline_comment|/* odd sub frame, &squot;Y&squot; */
r_else
id|data
op_or_assign
l_int|0x09
suffix:semicolon
multiline_comment|/* even sub frame, &squot;X&squot; */
multiline_comment|/*&n;&t; * sub frame counter: 2 sub frame are one audio frame&n;&t; * and 192 frames are one block&n;&t; */
id|cm-&gt;spdif_counter
op_assign
(paren
op_increment
...
macro_line|# endif /* if USE_AES_IEC958 */
...
suffix:semicolon
multiline_comment|/* Q: Does this function know about 24bit silence? */
r_if
c_cond
(paren
id|snd_pcm_format_set_silence
...
(brace
multiline_comment|/* chip model &gt;= 37 */
r_if
c_cond
(paren
id|snd_pcm_format_width
...
singleline_comment|// DEFINE_BIT_SWITCH_ARG(spdo_48k, CM_REG_MISC_CTRL, CM_SPDF_AC97|CM_SPDIF48K, 0, 1);
id|DEFINE_BIT_SWITCH_ARG
c_func
(paren
id|spdif_loop
...
suffix:semicolon
multiline_comment|/* for non-multichannel chips */
DECL|variable|__devinitdata
r_static
id|snd_kcontrol_new_t
id|snd_cmipci_nomulti_switch
id|__devinitdata
op_assign
id|DEFINE_MIXER_SWITCH
c_func
...
comma
singleline_comment|//&t;DEFINE_MIXER_SWITCH(&quot;IEC958 In/Out 48KHz&quot;, spdo_48k),
id|DEFINE_MIXER_SWITCH
c_func
(paren
l_string|&quot;IEC958 Loop&quot;
...
suffix:semicolon
macro_line|#endif
multiline_comment|/* Set Bus Master Request */
id|snd_cmipci_set_bit
c_func
(paren
id|cm
...
suffix:semicolon
multiline_comment|/* Assume TX and compatible chip set (Autodetection required for VX chip sets) */
r_switch
c_cond
(paren
id|pci-&gt;device
...
id|PCI_DEVICE_ID_CMEDIA_CM8738B
suffix:colon
multiline_comment|/* PCI_VENDOR_ID_INTEL, PCI_DEVICE_ID_INTEL_82437VX */
r_if
c_cond
(paren
op_logical_neg
...
suffix:semicolon
multiline_comment|/*&n;&t; * (Re-)Enable external switch spdo_48k&n;&t; */
id|snd_cmipci_set_bit
c_func
(paren
id|cm
...
(paren
id|KERN_WARNING
l_string|&quot;cmipci: cannot reserve joystick ports&bslash;n&quot;
)paren
suffix:semicolon
id|snd_cmipci_clear_bit
c_func
