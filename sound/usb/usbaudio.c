...
suffix:semicolon
multiline_comment|/*&n; * convert a sampling rate into USB format (fs/1000 in Q10.14)&n; * this will overflow at approx 2MSPS&n; */
DECL|function|get_usb_rate
r_inline
r_static
r_int
id|get_usb_rate
...
multiline_comment|/*&n; * prepare urb for capture sync pipe&n; *&n; * fill the length and offset of each urb descriptor.&n; * the fixed 10.14 frequency is passed through the pipe.&n; */
...
multiline_comment|/*&n; * prepare urb for playback sync pipe&n; *&n; * set up the offset and length to receive the current frequency.&n; */
...
multiline_comment|/*&n; * process after playback sync complete&n; *&n; * retrieve the current 10.14 frequency from pipe, and set it.&n; * the value is referred in prepare_playback_urb().&n; */
...
(paren
id|KERN_WARNING
l_string|&quot;requested frequency %u (nominal %u) out of range!&bslash;n&quot;
comma
id|f
comma
id|subs-&gt;freqn
...
id|total_packs
suffix:semicolon
multiline_comment|/* calculate the frequency in 10.14 format */
id|subs-&gt;freqn
op_assign
id|subs-&gt;freqm
op_assign
...
suffix:semicolon
singleline_comment|// printk(&quot;ok, getting back rate to %d&bslash;n&quot;, runtime-&gt;rate);
)brace
...
multiline_comment|/*&n; * allocate a buffer.&n; *&n; * so far we use a physically linear buffer although packetize transfer&n; * doesn&squot;t need a continuous area.&n; * if sg buffer is supported on the later version of alsa, we&squot;ll follow&n; * that.&n; */
...
suffix:semicolon
multiline_comment|/* FIXME: we need more constraints to restrict the format type,&n;&t; * channels and rates according to the audioformat list!&n;&t; */
)brace
...
multiline_comment|/*&n; * parse descriptor buffer and return the pointer starting the given&n; * descriptor type and interface.&n; * if altsetting is not -1, seek the buffer until the matching alternate&n; * setting is found.&n; */
...
id|retire_capture_urb
suffix:semicolon
id|subs-&gt;ops.prepare_sync
op_assign
id|prepare_capture_sync_urb
suffix:semicolon
id|subs-&gt;ops.retire_sync
op_assign
id|retire_capture_sync_urb
suffix:semicolon
)brace
...
suffix:semicolon
multiline_comment|/* FIXME: needed support for TYPE II and III */
r_if
c_cond
(paren
id|format_type
...
(paren
id|KERN_ERR
l_string|&quot;%d:%u:%d : non-supported sample bit %d in %d bytes&bslash;n&quot;
comma
id|dev-&gt;devnum
comma
id|iface_no
...
(brace
multiline_comment|/* FIXME: correct?  or S32_LE? */
id|subs-&gt;formats
op_or_assign
...
(brace
multiline_comment|/*&n;&t;&t;&t; * build the rate table and bitmap flags&n;&t;&t;&t; */
r_int
id|r
comma
id|idx
...
macro_line|#if 0 
singleline_comment|// FIXME - we need to define constraint
r_if
c_cond
(paren
id|c
...
id|SNDRV_PCM_KNOT
suffix:semicolon
multiline_comment|/* unconventional rate */
macro_line|#endif
)brace
)brace
r_else
...
DECL|function|snd_usb_audio_stream_new
r_static
r_int
id|snd_usb_audio_stream_new
c_func
(paren
id|snd_usb_audio_t
op_star
...
multiline_comment|/*&n; * parse audio control descriptor and create pcm streams&n; */
DECL|function|snd_usb_create_pcm
r_static
r_int
id|snd_usb_create_pcm
c_func
(paren
id|snd_usb_audio_t
op_star
...
(paren
id|KERN_ERR
l_string|&quot;%d:%u:%d: skipping - no valid interface.&bslash;n&quot;
comma
id|dev-&gt;devnum
comma
id|ctrlif
...
(paren
id|KERN_ERR
l_string|&quot;%d:%u:%d ep%d : have no bandwith at alt[0]&bslash;n&quot;
comma
id|dev-&gt;devnum
comma
id|ctrlif
...
(paren
id|KERN_ERR
l_string|&quot;%d:%u:%d : has no endpoint&bslash;n&quot;
comma
id|dev-&gt;devnum
comma
id|ctrlif
...
multiline_comment|/* note: this requires the data endpoint to be ep0 and&n;&t;&t; * the optional sync ep to be ep1, which seems to be the case&n;&t;&t; */
r_if
c_cond
(paren
id|iface-&gt;altsetting
...
(paren
id|KERN_INFO
l_string|&quot;adding an input interface %d:%u:%d&bslash;n&quot;
comma
id|dev-&gt;devnum
comma
id|ctrlif
...
(paren
id|KERN_INFO
l_string|&quot;adding an output interface %d:%u:%d&bslash;n&quot;
comma
id|dev-&gt;devnum
comma
id|ctrlif
...
multiline_comment|/* all endpoints are parsed.  now create pcm streams */
r_for
c_loop
(paren
id|i
...
multiline_comment|/*&n; * allocate and get description buffer&n; * must be freed later.&n; */
DECL|function|alloc_desc_buffer
r_static
r_int
id|alloc_desc_buffer
c_func
(paren
r_struct
id|usb_device
...
(paren
id|KERN_ERR
l_string|&quot;%d:%d: cannot get first 8 bytes&bslash;n&quot;
comma
id|index
comma
id|dev-&gt;devnum
...
(paren
id|KERN_ERR
l_string|&quot;%d:%d: invalid config desc&bslash;n&quot;
comma
id|index
comma
id|dev-&gt;devnum
...
(paren
id|KERN_ERR
l_string|&quot;cannot malloc descriptor (size = %d)&bslash;n&quot;
comma
id|buflen
)paren
suffix:semicolon
...
(paren
id|KERN_ERR
l_string|&quot;%d:%d: cannot get DT_CONFIG: error %d&bslash;n&quot;
comma
id|index
comma
id|dev-&gt;devnum
...
(paren
id|KERN_ERR
l_string|&quot;cannot create a card instance %d&bslash;n&quot;
comma
id|i
)paren
suffix:semicolon
