...
multiline_comment|/*&n; * Check whether delayed processing was scheduled for our current CPU,&n; * and then manually invoke NAPI polling to pump data off the card.&n; *&n; * In cases where there is bi-directional communications, reading only&n; * one message at a time can lead to packets being dropped by the&n; * network adapter, forcing superfluous retries and possibly timeouts.&n; * Thus, we set our budget to greater than 1.&n; */
