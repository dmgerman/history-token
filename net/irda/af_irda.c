...
multiline_comment|/*&n; * Function irda_data_wait (sk)&n; *&n; *    Sleep until data has arrive. But check for races..&n; *&n; *    The caller is expected to deal with the situation when we return&n; *    due to pending signals. And even if not, the peeked skb might have&n; *    been already dequeued due to concurrent operation.&n; *    Currently irda_recvmsg_stream() is the only caller and is ok.&n; * Return 0 if condition packet has arrived, -ERESTARTSYS if signal_pending()&n; * Only used once in irda_recvmsg_stream() -&gt; inline&n; */
