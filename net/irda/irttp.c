...
suffix:semicolon
multiline_comment|/************************ GLOBAL PROCEDURES ************************/
...
(brace
multiline_comment|/* Set new value for timer */
id|mod_timer
c_func
(paren
op_amp
...
multiline_comment|/*&n; * Function irttp_todo_expired (data)&n; *&n; *    Todo timer has expired!&n; *&n; * One of the restriction of the timer is that it is run only on the timer&n; * interrupt which run every 10ms. This mean that even if you set the timer&n; * with a delay of 0, it may take up to 10ms before it&squot;s run.&n; * So, to minimise latency and keep cache fresh, we try to avoid using&n; * it as much as possible.&n; * Note : we can&squot;t use tasklets, because they can&squot;t be asynchronously&n; * killed (need user context), and we can&squot;t guarantee that here...&n; * Jean II&n; */
...
suffix:semicolon
multiline_comment|/* Try to make some progress, especially on Tx side - Jean II */
id|irttp_run_rx_queue
c_func
(paren
id|self
...
multiline_comment|/* Finish cleanup */
id|irttp_close_tsap
c_func
(paren
id|self
...
multiline_comment|/*&n; * Function irttp_reassemble (self)&n; *&n; *    Makes a new (continuous) skb of all the fragments in the fragment&n; *    queue&n; *&n; */
...
multiline_comment|/*************************** CLIENT CALLS ***************************/
multiline_comment|/************************** LMP CALLBACKS **************************/
multiline_comment|/* Everything is happily mixed up. Waiting for next clean up - Jean II */
...
suffix:semicolon
multiline_comment|/* Initialise todo timer */
id|init_timer
c_func
(paren
op_amp
...
suffix:semicolon
multiline_comment|/* This one won&squot;t be cleaned up if we are disconnect_pend + close_pend&n;&t; * and we receive a disconnect_indication */
r_if
c_cond
(paren
id|self-&gt;disconnect_skb
...
multiline_comment|/* self-&gt;tx_sdu_busy is the state of the client.&n;&t;&t; * Update state after notifying client to avoid&n;&t;&t; * race condition with irttp_flow_indication().&n;&t;&t; * If the queue empty itself after our test but before&n;&t;&t; * we set the flag, we will fix ourselves below in&n;&t;&t; * irttp_run_tx_queue().&n;&t;&t; * Jean II */
id|self-&gt;tx_sdu_busy
op_assign
id|TRUE
suffix:semicolon
...
r_return
suffix:semicolon
multiline_comment|/* Try to send out frames as long as we have credits&n;&t; * and as long as LAP is not full. If LAP is full, it will&n;&t; * poll us through irttp_flow_indication() - Jean II */
r_while
c_loop
(paren
(paren
...
multiline_comment|/* Check if we can accept more frames from client.&n;&t; * We don&squot;t want to wait until the todo timer to do that, and we&n;&t; * can&squot;t use tasklets (grr...), so we are obliged to give control&n;&t; * to client. That&squot;s ok, this test will be true not too often&n;&t; * (max once per LAP window) and we are called from places&n;&t; * where we can spend a bit of time doing stuff. - Jean II */
r_if
c_cond
(paren
(paren
...
suffix:semicolon
multiline_comment|/* self-&gt;tx_sdu_busy is the state of the client.&n;&t;&t; * We don&squot;t really have a race here, but it&squot;s always safer&n;&t;&t; * to update our state after the client - Jean II */
id|self-&gt;tx_sdu_busy
op_assign
id|FALSE
suffix:semicolon
...
multiline_comment|/* Push data to the higher layer.&n;&t; * We do it synchronously because running the todo timer for each&n;&t; * receive packet would be too much overhead and latency.&n;&t; * By passing control to the higher layer, we run the risk that&n;&t; * it may take time or grab a lock. Most often, the higher layer&n;&t; * will only put packet in a queue.&n;&t; * Anyway, packets are only dripping through the IrDA, so we can&n;&t; * have time before the next packet.&n;&t; * Further, we are run from NET_BH, so the worse that can happen is&n;&t; * us missing the optimal time to send back the PF bit in LAP.&n;&t; * Jean II */
id|irttp_run_rx_queue
c_func
(paren
id|self
...
(brace
multiline_comment|/* Restart pushing stuff to LAP */
id|irttp_run_tx_queue
c_func
(paren
id|self
...
suffix:semicolon
multiline_comment|/* Note : we don&squot;t want to schedule the todo timer&n;&t;&t; * because it has horrible latency. No tasklets&n;&t;&t; * because the tasklet API is broken. - Jean II */
)brace
r_return
l_int|0
suffix:semicolon
...
suffix:semicolon
multiline_comment|/* We are &quot;polled&quot; directly from LAP, and the LAP want to fill&n;&t; * its Tx window. We want to do our best to send it data, so that&n;&t; * we maximise the window. On the other hand, we want to limit the&n;&t; * amount of work here so that LAP doesn&squot;t hang forever waiting&n;&t; * for packets. - Jean II */
multiline_comment|/* Try to send some packets. Currently, LAP calls us every time&n;&t; * there is one free slot, so we will send only one packet.&n;&t; * This allow the scheduler to do its round robin - Jean II */
id|irttp_run_tx_queue
c_func
(paren
id|self
...
suffix:semicolon
multiline_comment|/* Note regarding the interraction with higher layer.&n;&t; * irttp_run_tx_queue() may call the client when its queue&n;&t; * start to empty, via notify.flow_indication(). Initially.&n;&t; * I wanted this to happen in a tasklet, to avoid client&n;&t; * grabbing the CPU, but we can&squot;t use tasklets safely. And timer&n;&t; * is definitely too slow.&n;&t; * This will happen only once per LAP window, and usually at&n;&t; * the third packet (unless window is smaller). LAP is still&n;&t; * doing mtt and sending first packet so it&squot;s sort of OK&n;&t; * to do that. Jean II */
multiline_comment|/* If we need to send disconnect. try to do it now */
r_if
c_cond
(paren
id|self-&gt;disconnect_pend
...
id|FALSE
suffix:semicolon
multiline_comment|/* Client say he can accept more data, try to free our&n;&t;&t; * queues ASAP - Jean II */
id|irttp_run_rx_queue
c_func
(paren
id|self
...
multiline_comment|/* Note : we don&squot;t need to check if self-&gt;rx_queue is full and the&n;&t; * state of self-&gt;rx_sdu_busy because the disconnect response will&n;&t; * be sent at the LMP level (so even if the peer has its Tx queue&n;&t; * full of data). - Jean II */
id|IRDA_DEBUG
c_func
(paren
l_int|1
...
(brace
multiline_comment|/* This bit will tell us if it&squot;s the last fragment or not */
id|more
op_assign
id|skb-&gt;data
(braket
...
suffix:semicolon
multiline_comment|/* Do we have too much credits to send to peer ? */
r_if
c_cond
(paren
(paren
...
(brace
multiline_comment|/* Send explicit credit frame */
id|irttp_give_credit
c_func
(paren
id|self
...
suffix:semicolon
multiline_comment|/* Note : do *NOT* check if tx_queue is non-empty, that&n;&t;&t; * will produce deadlocks. I repeat : send a credit frame&n;&t;&t; * even if we have something to send in our Tx queue.&n;&t;&t; * If we have credits, it means that our Tx queue is blocked.&n;&t;&t; *&n;&t;&t; * Let&squot;s suppose the peer can&squot;t keep up with our Tx. He will&n;&t;&t; * flow control us by not sending us any credits, and we&n;&t;&t; * will stop Tx and start accumulating credits here.&n;&t;&t; * Up to the point where the peer will stop its Tx queue,&n;&t;&t; * for lack of credits.&n;&t;&t; * Let&squot;s assume the peer application is single threaded.&n;&t;&t; * It will block on Tx and never consume any Rx buffer.&n;&t;&t; * Deadlock. Guaranteed. - Jean II&n;&t;&t; */
)brace
multiline_comment|/* Reset lock */
id|self-&gt;rx_queue_lock
op_assign
...
id|seq
comma
l_string|&quot;  Used by (%s)&bslash;n&bslash;n&quot;
comma
id|self-&gt;notify.name
)paren
suffix:semicolon
