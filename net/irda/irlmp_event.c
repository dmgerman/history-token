...
(brace
multiline_comment|/*&n;&t;IRDA_ASSERT(self != NULL, return;);&n;&t;IRDA_DEBUG(4, &quot;%s(), LMP LSAP = %s&bslash;n&quot;, __FUNCTION__, irlsap_state[state]);&n;&t;*/
id|self-&gt;lsap_state
op_assign
id|state
suffix:semicolon
...
multiline_comment|/*&n; * Send an event on all LSAPs attached to this LAP.&n; */
r_static
r_inline
r_void
DECL|function|irlmp_do_all_lsap_event
...
id|lsap_next
suffix:semicolon
multiline_comment|/* Note : this function use the new hashbin_find_next()&n;&t; * function, instead of the old hashbin_get_next().&n;&t; * This make sure that we are always pointing one lsap&n;&t; * ahead, so that if the current lsap is removed as the&n;&t; * result of sending the event, we don&squot;t care.&n;&t; * Also, as we store the context ourselves, if an enumeration&n;&t; * of the same lsap hashbin happens as the result of sending the&n;&t; * event, we don&squot;t care.&n;&t; * The only problem is if the next lsap is removed. In that case,&n;&t; * hashbin_find_next() will return NULL and we will abort the&n;&t; * enumeration. - Jean II */
multiline_comment|/* Also : we don&squot;t accept any skb in input. We can *NOT* pass&n;&t; * the same skb to multiple clients safely, we would need to&n;&t; * skb_clone() it. - Jean II */
id|lsap
op_assign
(paren
r_struct
...
suffix:semicolon
multiline_comment|/* Tell LSAPs that they can start sending data */
id|irlmp_do_all_lsap_event
c_func
(paren
id|self-&gt;lsaps
