...
suffix:semicolon
multiline_comment|/* Small helper function that checks if the chunk length&n; * is of the appropriate length.  The &squot;required_length&squot; argument&n; * is set to be the size of a specific chunk we are testing.&n; * Return Values:  1 = Valid length&n; * &t;&t;   0 = Invalid length&n; *&n; */
r_static
r_inline
r_int
DECL|function|sctp_chunk_length_valid
id|sctp_chunk_length_valid
c_func
(paren
r_struct
id|sctp_chunk
...
id|len
suffix:semicolon
multiline_comment|/* 6.10 Bundling&n;&t; * An endpoint MUST NOT bundle INIT, INIT ACK or&n;&t; * SHUTDOWN COMPLETE with any other chunks.&n;&t; * &n;&t; * IG Section 2.11.2&n;&t; * Furthermore, we require that the receiver of an INIT chunk MUST&n;&t; * enforce these rules by silently discarding an arriving packet&n;&t; * with an INIT chunk that is bundled with other chunks.&n;&t; */
r_if
c_cond
(paren
op_logical_neg
...
suffix:semicolon
multiline_comment|/* Make sure that the INIT chunk has a valid length.&n;&t; * Normally, this would cause an ABORT with a Protocol Violation&n;&t; * error, but since we don&squot;t have an association, we&squot;ll&n;&t; * just discard the packet.&n;&t; */
r_if
c_cond
(paren
op_logical_neg
...
suffix:semicolon
multiline_comment|/* Make sure that the INIT-ACK chunk has a valid length */
r_if
c_cond
(paren
op_logical_neg
...
suffix:semicolon
multiline_comment|/* Make sure that the COOKIE_ECHO chunk has a valid length.&n;&t; * In this case, we check that we have enough for at least a&n;&t; * chunk header.  More detailed verification is done&n;&t; * in sctp_unpack_cookie().&n;&t; */
r_if
c_cond
(paren
op_logical_neg
...
suffix:semicolon
multiline_comment|/* Verify that the chunk length for the COOKIE-ACK is OK.&n;&t; * If we don&squot;t do this, any bundled chunks may be junked.&n;&t; */
r_if
c_cond
(paren
op_logical_neg
...
suffix:semicolon
multiline_comment|/* Make sure that the HEARTBEAT chunk has a valid length. */
r_if
c_cond
(paren
op_logical_neg
...
suffix:semicolon
multiline_comment|/* Make sure that the HEARTBEAT-ACK chunk has a valid length.  */
r_if
c_cond
(paren
op_logical_neg
...
suffix:semicolon
multiline_comment|/* Make sure that the INIT chunk has a valid length.&n;&t; * In this case, we generate a protocol violation since we have&n;&t; * an association established.&n;&t; */
r_if
c_cond
(paren
op_logical_neg
...
id|err_chk_p
suffix:semicolon
multiline_comment|/* Make sure that the chunk has a valid length from the protocol&n;&t; * perspective.  In this case check to make sure we have at least&n;&t; * enough for the chunk header.  Cookie length verification is&n;&t; * done later.&n;&t; */
r_if
c_cond
(paren
op_logical_neg
...
suffix:semicolon
multiline_comment|/* Make sure that the ABORT chunk has a valid length.&n;&t; * Since this is an ABORT chunk, we have to discard it&n;&t; * because of the following text:&n;&t; * RFC 2960, Section 3.3.7&n;&t; *    If an endpoint receives an ABORT with a format error or for an&n;&t; *    association that doesn&squot;t exist, it MUST silently discard it.&n;&t; * Becasue the length is &quot;invalid&quot;, we can&squot;t really discard just&n;&t; * as we do not know its true length.  So, to be safe, discard the&n;&t; * packet.&n;&t; */
r_if
c_cond
(paren
op_logical_neg
...
suffix:semicolon
multiline_comment|/* Make sure that the ERROR chunk has a valid length.&n;&t; * The parameter walking depends on this as well.&n;&t; */
r_if
c_cond
(paren
op_logical_neg
...
multiline_comment|/* It is possible to have malformed error causes, and that&n;&t; * will cause us to end the walk early.  However, since&n;&t; * we are discarding the packet, there should be no adverse&n;&t; * affects.&n;&t; */
r_return
id|sctp_sf_pdiscard
c_func
(paren
...
suffix:semicolon
multiline_comment|/* See if we have an error cause code in the chunk.  */
id|len
op_assign
id|ntohs
c_func
...
multiline_comment|/*&n; * Process an incoming ICMP as an ABORT.  (COOKIE-WAIT state)&n; */
DECL|function|sctp_sf_cookie_wait_icmp_abort
id|sctp_disposition_t
...
multiline_comment|/*&n; * Process an ABORT.  (COOKIE-ECHOED state)&n; */
...
multiline_comment|/*&n; * Stop T1 timer and abort association with &quot;INIT failed&quot;.&n; *&n; * This is common code called by several sctp_sf_*_abort() functions above.&n; */
DECL|function|sctp_stop_t1_and_abort
r_void
id|sctp_stop_t1_and_abort
c_func
(paren
...
suffix:semicolon
multiline_comment|/* Make sure that the SHUTDOWN chunk has a valid length. */
r_if
c_cond
(paren
op_logical_neg
...
id|reply
suffix:semicolon
multiline_comment|/* Since we are not going to really process this INIT, there&n;&t; * is no point in verifying chunk boundries.  Just generate&n;&t; * the SHUTDOWN ACK.&n;&t; */
id|reply
op_assign
...
suffix:semicolon
multiline_comment|/* Make sure that the SACK chunk has a valid length. */
r_if
c_cond
(paren
op_logical_neg
...
suffix:semicolon
multiline_comment|/* Make sure that the ERROR chunk has a valid length. */
r_if
c_cond
(paren
op_logical_neg
...
suffix:semicolon
multiline_comment|/* Make sure that the SHUTDOWN_ACK chunk has a valid length. */
r_if
c_cond
(paren
op_logical_neg
...
r_do
(brace
multiline_comment|/* Break out if chunk length is less then minimal. */
r_if
c_cond
(paren
id|ntohs
...
l_int|1
suffix:semicolon
multiline_comment|/* RFC 2960, Section 3.3.7&n;&t;&t; *   Moreover, under any circumstances, an endpoint that&n;&t;&t; *   receives an ABORT  MUST NOT respond to that ABORT by&n;&t;&t; *   sending an ABORT of its own.&n;&t;&t; */
r_if
c_cond
(paren
id|SCTP_CID_ABORT
...
suffix:semicolon
multiline_comment|/* If the chunk length is invalid, we don&squot;t want to process&n;&t;&t; * the reset of the packet.&n;&t;&t; */
r_if
c_cond
(paren
op_logical_neg
...
multiline_comment|/* Make sure that the ASCONF ADDIP chunk has a valid length.  */
r_if
c_cond
(paren
op_logical_neg
...
multiline_comment|/* Make sure that the ADDIP chunk has a valid length.  */
r_if
c_cond
(paren
op_logical_neg
...
multiline_comment|/* Make sure that the FORWARD_TSN chunk has valid length.  */
r_if
c_cond
(paren
op_logical_neg
...
multiline_comment|/* Make sure that the FORWARD_TSN chunk has a valid length.  */
r_if
c_cond
(paren
op_logical_neg
...
suffix:semicolon
multiline_comment|/* Make sure that the chunk has a valid length.&n;&t; * Since we don&squot;t know the chunk type, we use a general&n;&t; * chunkhdr structure to make a comparison.&n;&t; */
r_if
c_cond
(paren
op_logical_neg
...
multiline_comment|/*&n; * Handle a protocol violation when the chunk length is invalid.&n; * &quot;Invalid&quot; length is identified as smaller then the minimal length a&n; * given chunk can be.  For example, a SACK chunk has invalid length&n; * if it&squot;s length is set to be smaller then the size of sctp_sack_chunk_t.&n; *&n; * We inform the other end by sending an ABORT with a Protocol Violation&n; * error code. &n; *&n; * Section: Not specified&n; * Verification Tag:  Nothing to do&n; * Inputs&n; * (endpoint, asoc, chunk)&n; *&n; * Outputs&n; * (reply_msg, msg_up, counters)&n; *&n; * Generate an  ABORT chunk and terminate the association.&n; */
DECL|function|sctp_sf_violation_chunklen
id|sctp_disposition_t
id|sctp_sf_violation_chunklen
c_func
(paren
...
op_assign
l_string|&quot;The following chunk had invalid length:&quot;
suffix:semicolon
multiline_comment|/* Make the abort chunk. */
m_abort
op_assign
...
id|reply
suffix:semicolon
multiline_comment|/* There are 2 ways of getting here:&n;&t; *    1) called in response to a SHUTDOWN chunk&n;&t; *    2) called when SCTP_EVENT_NO_PENDING_TSN event is issued.&n;&t; *&n;&t; * For the case (2), the arg parameter is set to NULL.  We need&n;&t; * to check that we have a chunk before accessing it&squot;s fields.&n;&t; */
r_if
c_cond
(paren
id|chunk
