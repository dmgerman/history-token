multiline_comment|/*&n; * INET&t;&t;An implementation of the TCP/IP protocol suite for the LINUX&n; *&t;&t;operating system.  INET is implemented using the  BSD Socket&n; *&t;&t;interface as the means of communication with the user level.&n; *&n; *&t;&t;ipv4 proc support&n; *&n; *&t;&t;Arnaldo Carvalho de Melo &lt;acme@conectiva.com.br&gt;, 2002/10/10&n; *&n; *&t;&t;This program is free software; you can redistribute it and/or&n; *&t;&t;modify it under the terms of the GNU General Public License as&n; *&t;&t;published by the Free Software Foundation; version 2 of the&n; *&t;&t;License&n; */
macro_line|#include &lt;linux/config.h&gt;
macro_line|#include &lt;linux/init.h&gt;
macro_line|#include &lt;linux/netdevice.h&gt;
macro_line|#include &lt;net/neighbour.h&gt;
macro_line|#include &lt;net/arp.h&gt;
macro_line|#include &lt;net/udp.h&gt;
macro_line|#include &lt;linux/rtnetlink.h&gt;
macro_line|#include &lt;linux/route.h&gt;
macro_line|#include &lt;net/ip_fib.h&gt;
macro_line|#include &lt;linux/seq_file.h&gt;
macro_line|#include &lt;linux/proc_fs.h&gt;
r_extern
r_int
id|raw_get_info
c_func
(paren
r_char
op_star
comma
r_char
op_star
op_star
comma
id|off_t
comma
r_int
)paren
suffix:semicolon
r_extern
r_int
id|snmp_get_info
c_func
(paren
r_char
op_star
comma
r_char
op_star
op_star
comma
id|off_t
comma
r_int
)paren
suffix:semicolon
r_extern
r_int
id|netstat_get_info
c_func
(paren
r_char
op_star
comma
r_char
op_star
op_star
comma
id|off_t
comma
r_int
)paren
suffix:semicolon
r_extern
r_int
id|afinet_get_info
c_func
(paren
r_char
op_star
comma
r_char
op_star
op_star
comma
id|off_t
comma
r_int
)paren
suffix:semicolon
r_extern
r_int
id|tcp_get_info
c_func
(paren
r_char
op_star
comma
r_char
op_star
op_star
comma
id|off_t
comma
r_int
)paren
suffix:semicolon
macro_line|#ifdef CONFIG_PROC_FS
macro_line|#ifdef CONFIG_AX25
multiline_comment|/* ------------------------------------------------------------------------ */
multiline_comment|/*&n; *&t;ax25 -&gt; ASCII conversion&n; */
DECL|function|ax2asc2
r_static
r_char
op_star
id|ax2asc2
c_func
(paren
id|ax25_address
op_star
id|a
comma
r_char
op_star
id|buf
)paren
(brace
r_char
id|c
comma
op_star
id|s
suffix:semicolon
r_int
id|n
suffix:semicolon
r_for
c_loop
(paren
id|n
op_assign
l_int|0
comma
id|s
op_assign
id|buf
suffix:semicolon
id|n
OL
l_int|6
suffix:semicolon
id|n
op_increment
)paren
(brace
id|c
op_assign
(paren
id|a-&gt;ax25_call
(braket
id|n
)braket
op_rshift
l_int|1
)paren
op_amp
l_int|0x7F
suffix:semicolon
r_if
c_cond
(paren
id|c
op_ne
l_char|&squot; &squot;
)paren
op_star
id|s
op_increment
op_assign
id|c
suffix:semicolon
)brace
op_star
id|s
op_increment
op_assign
l_char|&squot;-&squot;
suffix:semicolon
r_if
c_cond
(paren
(paren
id|n
op_assign
(paren
(paren
id|a-&gt;ax25_call
(braket
l_int|6
)braket
op_rshift
l_int|1
)paren
op_amp
l_int|0x0F
)paren
)paren
OG
l_int|9
)paren
(brace
op_star
id|s
op_increment
op_assign
l_char|&squot;1&squot;
suffix:semicolon
id|n
op_sub_assign
l_int|10
suffix:semicolon
)brace
op_star
id|s
op_increment
op_assign
id|n
op_plus
l_char|&squot;0&squot;
suffix:semicolon
op_star
id|s
op_increment
op_assign
l_char|&squot;&bslash;0&squot;
suffix:semicolon
r_if
c_cond
(paren
op_star
id|buf
op_eq
l_char|&squot;&bslash;0&squot;
op_logical_or
op_star
id|buf
op_eq
l_char|&squot;-&squot;
)paren
r_return
l_string|&quot;*&quot;
suffix:semicolon
r_return
id|buf
suffix:semicolon
)brace
macro_line|#endif /* CONFIG_AX25 */
DECL|struct|arp_iter_state
r_struct
id|arp_iter_state
(brace
DECL|member|is_pneigh
id|loff_t
id|is_pneigh
suffix:colon
l_int|1
comma
DECL|member|bucket
id|bucket
suffix:colon
l_int|6
comma
DECL|member|pos
id|pos
suffix:colon
r_sizeof
(paren
id|loff_t
)paren
op_star
l_int|8
op_minus
l_int|7
suffix:semicolon
)brace
suffix:semicolon
DECL|function|neigh_get_bucket
r_static
id|__inline__
r_struct
id|neighbour
op_star
id|neigh_get_bucket
c_func
(paren
id|loff_t
op_star
id|pos
)paren
(brace
r_struct
id|neighbour
op_star
id|n
op_assign
l_int|NULL
suffix:semicolon
r_struct
id|arp_iter_state
op_star
id|state
op_assign
(paren
r_struct
id|arp_iter_state
op_star
)paren
id|pos
suffix:semicolon
id|loff_t
id|l
op_assign
id|state-&gt;pos
suffix:semicolon
r_int
id|i
comma
id|bucket
op_assign
id|state-&gt;bucket
suffix:semicolon
r_for
c_loop
(paren
suffix:semicolon
id|bucket
op_le
id|NEIGH_HASHMASK
suffix:semicolon
op_increment
id|bucket
)paren
r_for
c_loop
(paren
id|i
op_assign
l_int|0
comma
id|n
op_assign
id|arp_tbl.hash_buckets
(braket
id|bucket
)braket
suffix:semicolon
id|n
suffix:semicolon
op_increment
id|i
comma
id|n
op_assign
id|n-&gt;next
)paren
multiline_comment|/* Do not confuse users &quot;arp -a&quot; with magic entries */
r_if
c_cond
(paren
(paren
id|n-&gt;nud_state
op_amp
op_complement
id|NUD_NOARP
)paren
op_logical_and
op_logical_neg
id|l
op_decrement
)paren
(brace
id|state-&gt;pos
op_assign
id|i
suffix:semicolon
id|state-&gt;bucket
op_assign
id|bucket
suffix:semicolon
r_goto
id|out
suffix:semicolon
)brace
id|out
suffix:colon
r_return
id|n
suffix:semicolon
)brace
DECL|function|pneigh_get_bucket
r_static
id|__inline__
r_struct
id|pneigh_entry
op_star
id|pneigh_get_bucket
c_func
(paren
id|loff_t
op_star
id|pos
)paren
(brace
r_struct
id|pneigh_entry
op_star
id|n
op_assign
l_int|NULL
suffix:semicolon
r_struct
id|arp_iter_state
op_star
id|state
op_assign
(paren
r_struct
id|arp_iter_state
op_star
)paren
id|pos
suffix:semicolon
id|loff_t
id|l
op_assign
id|state-&gt;pos
suffix:semicolon
r_int
id|i
comma
id|bucket
op_assign
id|state-&gt;bucket
suffix:semicolon
r_for
c_loop
(paren
suffix:semicolon
id|bucket
op_le
id|PNEIGH_HASHMASK
suffix:semicolon
op_increment
id|bucket
)paren
r_for
c_loop
(paren
id|i
op_assign
l_int|0
comma
id|n
op_assign
id|arp_tbl.phash_buckets
(braket
id|bucket
)braket
suffix:semicolon
id|n
suffix:semicolon
op_increment
id|i
comma
id|n
op_assign
id|n-&gt;next
)paren
r_if
c_cond
(paren
op_logical_neg
id|l
op_decrement
)paren
(brace
id|state-&gt;pos
op_assign
id|i
suffix:semicolon
id|state-&gt;bucket
op_assign
id|bucket
suffix:semicolon
r_goto
id|out
suffix:semicolon
)brace
id|out
suffix:colon
r_return
id|n
suffix:semicolon
)brace
DECL|function|arp_get_bucket
r_static
id|__inline__
r_void
op_star
id|arp_get_bucket
c_func
(paren
r_struct
id|seq_file
op_star
id|seq
comma
id|loff_t
op_star
id|pos
)paren
(brace
r_void
op_star
id|rc
op_assign
id|neigh_get_bucket
c_func
(paren
id|pos
)paren
suffix:semicolon
r_if
c_cond
(paren
op_logical_neg
id|rc
)paren
(brace
r_struct
id|arp_iter_state
op_star
id|state
op_assign
(paren
r_struct
id|arp_iter_state
op_star
)paren
id|pos
suffix:semicolon
id|read_unlock_bh
c_func
(paren
op_amp
id|arp_tbl.lock
)paren
suffix:semicolon
id|state-&gt;is_pneigh
op_assign
l_int|1
suffix:semicolon
id|state-&gt;bucket
op_assign
l_int|0
suffix:semicolon
id|state-&gt;pos
op_assign
l_int|0
suffix:semicolon
multiline_comment|/* HACK: till there is state we can pass to seq_show...  */
id|seq
op_member_access_from_pointer
r_private
op_assign
(paren
r_void
op_star
)paren
l_int|1
suffix:semicolon
id|rc
op_assign
id|pneigh_get_bucket
c_func
(paren
id|pos
)paren
suffix:semicolon
)brace
r_return
id|rc
suffix:semicolon
)brace
DECL|function|arp_seq_start
r_static
r_void
op_star
id|arp_seq_start
c_func
(paren
r_struct
id|seq_file
op_star
id|seq
comma
id|loff_t
op_star
id|pos
)paren
(brace
id|read_lock_bh
c_func
(paren
op_amp
id|arp_tbl.lock
)paren
suffix:semicolon
r_return
op_star
id|pos
ques
c_cond
id|arp_get_bucket
c_func
(paren
id|seq
comma
id|pos
)paren
suffix:colon
(paren
r_void
op_star
)paren
l_int|1
suffix:semicolon
)brace
DECL|function|arp_seq_next
r_static
r_void
op_star
id|arp_seq_next
c_func
(paren
r_struct
id|seq_file
op_star
id|seq
comma
r_void
op_star
id|v
comma
id|loff_t
op_star
id|pos
)paren
(brace
r_void
op_star
id|rc
suffix:semicolon
r_struct
id|arp_iter_state
op_star
id|state
suffix:semicolon
r_if
c_cond
(paren
id|v
op_eq
(paren
r_void
op_star
)paren
l_int|1
)paren
(brace
id|rc
op_assign
id|arp_get_bucket
c_func
(paren
id|seq
comma
id|pos
)paren
suffix:semicolon
r_goto
id|out
suffix:semicolon
)brace
id|state
op_assign
(paren
r_struct
id|arp_iter_state
op_star
)paren
id|pos
suffix:semicolon
r_if
c_cond
(paren
op_logical_neg
id|state-&gt;is_pneigh
)paren
(brace
r_struct
id|neighbour
op_star
id|n
op_assign
id|v
suffix:semicolon
id|rc
op_assign
id|n
op_assign
id|n-&gt;next
suffix:semicolon
r_if
c_cond
(paren
id|n
)paren
r_goto
id|out
suffix:semicolon
id|state-&gt;pos
op_assign
l_int|0
suffix:semicolon
op_increment
id|state-&gt;bucket
suffix:semicolon
id|rc
op_assign
id|neigh_get_bucket
c_func
(paren
id|pos
)paren
suffix:semicolon
r_if
c_cond
(paren
id|rc
)paren
r_goto
id|out
suffix:semicolon
id|read_unlock_bh
c_func
(paren
op_amp
id|arp_tbl.lock
)paren
suffix:semicolon
multiline_comment|/* HACK: till there is state we can pass to seq_show...  */
id|seq
op_member_access_from_pointer
r_private
op_assign
(paren
r_void
op_star
)paren
l_int|1
suffix:semicolon
id|state-&gt;is_pneigh
op_assign
l_int|1
suffix:semicolon
id|state-&gt;bucket
op_assign
l_int|0
suffix:semicolon
id|state-&gt;pos
op_assign
l_int|0
suffix:semicolon
id|rc
op_assign
id|pneigh_get_bucket
c_func
(paren
id|pos
)paren
suffix:semicolon
)brace
r_else
(brace
r_struct
id|pneigh_entry
op_star
id|pn
op_assign
id|v
suffix:semicolon
id|pn
op_assign
id|pn-&gt;next
suffix:semicolon
r_if
c_cond
(paren
op_logical_neg
id|pn
)paren
(brace
op_increment
id|state-&gt;bucket
suffix:semicolon
id|state-&gt;pos
op_assign
l_int|0
suffix:semicolon
id|pn
op_assign
id|pneigh_get_bucket
c_func
(paren
id|pos
)paren
suffix:semicolon
)brace
id|rc
op_assign
id|pn
suffix:semicolon
)brace
id|out
suffix:colon
op_increment
op_star
id|pos
suffix:semicolon
r_return
id|rc
suffix:semicolon
)brace
DECL|function|arp_seq_stop
r_static
r_void
id|arp_seq_stop
c_func
(paren
r_struct
id|seq_file
op_star
id|seq
comma
r_void
op_star
id|v
)paren
(brace
r_if
c_cond
(paren
op_logical_neg
id|seq
op_member_access_from_pointer
r_private
)paren
id|read_unlock_bh
c_func
(paren
op_amp
id|arp_tbl.lock
)paren
suffix:semicolon
)brace
DECL|macro|HBUFFERLEN
mdefine_line|#define HBUFFERLEN 30
DECL|function|arp_format_neigh_entry
r_static
id|__inline__
r_void
id|arp_format_neigh_entry
c_func
(paren
r_struct
id|seq_file
op_star
id|seq
comma
r_struct
id|neighbour
op_star
id|n
)paren
(brace
r_char
id|hbuffer
(braket
id|HBUFFERLEN
)braket
suffix:semicolon
r_const
r_char
id|hexbuf
(braket
)braket
op_assign
l_string|&quot;0123456789ABCDEF&quot;
suffix:semicolon
r_int
id|k
comma
id|j
suffix:semicolon
r_char
id|tbuf
(braket
l_int|16
)braket
suffix:semicolon
r_struct
id|net_device
op_star
id|dev
op_assign
id|n-&gt;dev
suffix:semicolon
r_int
id|hatype
op_assign
id|dev-&gt;type
suffix:semicolon
id|read_lock
c_func
(paren
op_amp
id|n-&gt;lock
)paren
suffix:semicolon
multiline_comment|/* Convert hardware address to XX:XX:XX:XX ... form. */
macro_line|#ifdef CONFIG_AX25
r_if
c_cond
(paren
id|hatype
op_eq
id|ARPHRD_AX25
op_logical_or
id|hatype
op_eq
id|ARPHRD_NETROM
)paren
id|ax2asc2
c_func
(paren
(paren
id|ax25_address
op_star
)paren
id|n-&gt;ha
comma
id|hbuffer
)paren
suffix:semicolon
r_else
(brace
macro_line|#endif
r_for
c_loop
(paren
id|k
op_assign
l_int|0
comma
id|j
op_assign
l_int|0
suffix:semicolon
id|k
OL
id|HBUFFERLEN
op_minus
l_int|3
op_logical_and
id|j
OL
id|dev-&gt;addr_len
suffix:semicolon
id|j
op_increment
)paren
(brace
id|hbuffer
(braket
id|k
op_increment
)braket
op_assign
id|hexbuf
(braket
(paren
id|n-&gt;ha
(braket
id|j
)braket
op_rshift
l_int|4
)paren
op_amp
l_int|15
)braket
suffix:semicolon
id|hbuffer
(braket
id|k
op_increment
)braket
op_assign
id|hexbuf
(braket
id|n-&gt;ha
(braket
id|j
)braket
op_amp
l_int|15
)braket
suffix:semicolon
id|hbuffer
(braket
id|k
op_increment
)braket
op_assign
l_char|&squot;:&squot;
suffix:semicolon
)brace
id|hbuffer
(braket
op_decrement
id|k
)braket
op_assign
l_int|0
suffix:semicolon
macro_line|#ifdef CONFIG_AX25
)brace
macro_line|#endif
id|sprintf
c_func
(paren
id|tbuf
comma
l_string|&quot;%u.%u.%u.%u&quot;
comma
id|NIPQUAD
c_func
(paren
op_star
(paren
id|u32
op_star
)paren
id|n-&gt;primary_key
)paren
)paren
suffix:semicolon
id|seq_printf
c_func
(paren
id|seq
comma
l_string|&quot;%-16s 0x%-10x0x%-10x%s     *        %s&bslash;n&quot;
comma
id|tbuf
comma
id|hatype
comma
id|arp_state_to_flags
c_func
(paren
id|n
)paren
comma
id|hbuffer
comma
id|dev-&gt;name
)paren
suffix:semicolon
id|read_unlock
c_func
(paren
op_amp
id|n-&gt;lock
)paren
suffix:semicolon
)brace
DECL|function|arp_format_pneigh_entry
r_static
id|__inline__
r_void
id|arp_format_pneigh_entry
c_func
(paren
r_struct
id|seq_file
op_star
id|seq
comma
r_struct
id|pneigh_entry
op_star
id|n
)paren
(brace
r_struct
id|net_device
op_star
id|dev
op_assign
id|n-&gt;dev
suffix:semicolon
r_int
id|hatype
op_assign
id|dev
ques
c_cond
id|dev-&gt;type
suffix:colon
l_int|0
suffix:semicolon
r_char
id|tbuf
(braket
l_int|16
)braket
suffix:semicolon
id|sprintf
c_func
(paren
id|tbuf
comma
l_string|&quot;%u.%u.%u.%u&quot;
comma
id|NIPQUAD
c_func
(paren
op_star
(paren
id|u32
op_star
)paren
id|n-&gt;key
)paren
)paren
suffix:semicolon
id|seq_printf
c_func
(paren
id|seq
comma
l_string|&quot;%-16s 0x%-10x0x%-10x%s     *        %s&bslash;n&quot;
comma
id|tbuf
comma
id|hatype
comma
id|ATF_PUBL
op_or
id|ATF_PERM
comma
l_string|&quot;00:00:00:00:00:00&quot;
comma
id|dev
ques
c_cond
id|dev-&gt;name
suffix:colon
l_string|&quot;*&quot;
)paren
suffix:semicolon
)brace
DECL|function|arp_seq_show
r_static
r_int
id|arp_seq_show
c_func
(paren
r_struct
id|seq_file
op_star
id|seq
comma
r_void
op_star
id|v
)paren
(brace
r_if
c_cond
(paren
id|v
op_eq
(paren
r_void
op_star
)paren
l_int|1
)paren
id|seq_puts
c_func
(paren
id|seq
comma
l_string|&quot;IP address       HW type     Flags       &quot;
l_string|&quot;HW address            Mask     Device&bslash;n&quot;
)paren
suffix:semicolon
r_else
(brace
r_if
c_cond
(paren
id|seq
op_member_access_from_pointer
r_private
)paren
id|arp_format_pneigh_entry
c_func
(paren
id|seq
comma
id|v
)paren
suffix:semicolon
r_else
id|arp_format_neigh_entry
c_func
(paren
id|seq
comma
id|v
)paren
suffix:semicolon
)brace
r_return
l_int|0
suffix:semicolon
)brace
multiline_comment|/* ------------------------------------------------------------------------ */
DECL|function|fib_seq_start
r_static
r_void
op_star
id|fib_seq_start
c_func
(paren
r_struct
id|seq_file
op_star
id|seq
comma
id|loff_t
op_star
id|pos
)paren
(brace
r_return
op_star
id|pos
ques
c_cond
l_int|NULL
suffix:colon
(paren
r_void
op_star
)paren
l_int|1
suffix:semicolon
)brace
DECL|function|fib_seq_next
r_static
r_void
op_star
id|fib_seq_next
c_func
(paren
r_struct
id|seq_file
op_star
id|seq
comma
r_void
op_star
id|v
comma
id|loff_t
op_star
id|pos
)paren
(brace
r_return
l_int|NULL
suffix:semicolon
)brace
DECL|function|fib_seq_stop
r_static
r_void
id|fib_seq_stop
c_func
(paren
r_struct
id|seq_file
op_star
id|seq
comma
r_void
op_star
id|v
)paren
(brace
)brace
multiline_comment|/* &n; *&t;This outputs /proc/net/route.&n; *&n; *&t;It always works in backward compatibility mode.&n; *&t;The format of the file is not supposed to be changed.&n; */
DECL|function|fib_seq_show
r_static
r_int
id|fib_seq_show
c_func
(paren
r_struct
id|seq_file
op_star
id|seq
comma
r_void
op_star
id|v
)paren
(brace
id|seq_printf
c_func
(paren
id|seq
comma
l_string|&quot;%-127s&bslash;n&quot;
comma
l_string|&quot;Iface&bslash;tDestination&bslash;tGateway &quot;
l_string|&quot;&bslash;tFlags&bslash;tRefCnt&bslash;tUse&bslash;tMetric&bslash;tMask&bslash;t&bslash;tMTU&quot;
l_string|&quot;&bslash;tWindow&bslash;tIRTT&quot;
)paren
suffix:semicolon
r_if
c_cond
(paren
id|ip_fib_main_table
)paren
id|ip_fib_main_table
op_member_access_from_pointer
id|tb_seq_show
c_func
(paren
id|ip_fib_main_table
comma
id|seq
)paren
suffix:semicolon
r_return
l_int|0
suffix:semicolon
)brace
multiline_comment|/* ------------------------------------------------------------------------ */
DECL|macro|UDP_HASH_POS_BITS
mdefine_line|#define UDP_HASH_POS_BITS (sizeof(loff_t) * 8 - 8)
DECL|macro|UDP_HASH_BITS
mdefine_line|#define UDP_HASH_BITS (((loff_t)127) &lt;&lt; UDP_HASH_POS_BITS)
DECL|macro|UDP_HASH_BUCKET
mdefine_line|#define UDP_HASH_BUCKET(p) ((p &amp; UDP_HASH_BITS) &gt;&gt; UDP_HASH_POS_BITS)
DECL|function|udp_get_bucket
r_static
id|__inline__
r_struct
id|sock
op_star
id|udp_get_bucket
c_func
(paren
r_struct
id|seq_file
op_star
id|seq
comma
id|loff_t
op_star
id|pos
)paren
(brace
r_struct
id|sock
op_star
id|sk
op_assign
l_int|NULL
suffix:semicolon
id|loff_t
id|ppos
op_assign
op_star
id|pos
op_amp
op_complement
id|UDP_HASH_BITS
comma
id|l
op_assign
id|ppos
suffix:semicolon
id|loff_t
id|bucket
op_assign
id|UDP_HASH_BUCKET
c_func
(paren
op_star
id|pos
)paren
suffix:semicolon
r_for
c_loop
(paren
suffix:semicolon
id|bucket
OL
id|UDP_HTABLE_SIZE
suffix:semicolon
op_increment
id|bucket
)paren
r_for
c_loop
(paren
id|sk
op_assign
id|udp_hash
(braket
id|bucket
)braket
suffix:semicolon
id|sk
suffix:semicolon
id|sk
op_assign
id|sk-&gt;next
)paren
(brace
r_if
c_cond
(paren
id|sk-&gt;family
op_ne
id|PF_INET
)paren
r_continue
suffix:semicolon
r_if
c_cond
(paren
id|l
op_decrement
)paren
r_continue
suffix:semicolon
op_star
id|pos
op_assign
(paren
id|bucket
op_lshift
id|UDP_HASH_POS_BITS
)paren
op_or
id|ppos
suffix:semicolon
multiline_comment|/*&n;&t;&t;&t; * temporary HACK till we have a solution to&n;&t;&t;&t; * get more state passed to seq_show -acme&n;&t;&t;&t; */
id|seq
op_member_access_from_pointer
r_private
op_assign
(paren
r_void
op_star
)paren
(paren
r_int
)paren
id|bucket
suffix:semicolon
r_goto
id|out
suffix:semicolon
)brace
id|out
suffix:colon
r_return
id|sk
suffix:semicolon
)brace
DECL|function|udp_seq_start
r_static
r_void
op_star
id|udp_seq_start
c_func
(paren
r_struct
id|seq_file
op_star
id|seq
comma
id|loff_t
op_star
id|pos
)paren
(brace
id|read_lock
c_func
(paren
op_amp
id|udp_hash_lock
)paren
suffix:semicolon
r_return
op_star
id|pos
ques
c_cond
id|udp_get_bucket
c_func
(paren
id|seq
comma
id|pos
)paren
suffix:colon
(paren
r_void
op_star
)paren
l_int|1
suffix:semicolon
)brace
DECL|function|udp_seq_next
r_static
r_void
op_star
id|udp_seq_next
c_func
(paren
r_struct
id|seq_file
op_star
id|seq
comma
r_void
op_star
id|v
comma
id|loff_t
op_star
id|pos
)paren
(brace
r_int
id|next_bucket
suffix:semicolon
r_struct
id|sock
op_star
id|sk
suffix:semicolon
r_if
c_cond
(paren
id|v
op_eq
(paren
r_void
op_star
)paren
l_int|1
)paren
(brace
id|sk
op_assign
id|udp_get_bucket
c_func
(paren
id|seq
comma
id|pos
)paren
suffix:semicolon
r_goto
id|out
suffix:semicolon
)brace
id|sk
op_assign
id|v
suffix:semicolon
id|sk
op_assign
id|sk-&gt;next
suffix:semicolon
r_if
c_cond
(paren
id|sk
)paren
r_goto
id|out
suffix:semicolon
id|next_bucket
op_assign
id|UDP_HASH_BUCKET
c_func
(paren
op_star
id|pos
)paren
op_plus
l_int|1
suffix:semicolon
r_if
c_cond
(paren
id|next_bucket
op_ge
id|UDP_HTABLE_SIZE
)paren
r_goto
id|out
suffix:semicolon
op_star
id|pos
op_assign
(paren
id|loff_t
)paren
id|next_bucket
op_lshift
id|UDP_HASH_POS_BITS
suffix:semicolon
id|sk
op_assign
id|udp_get_bucket
c_func
(paren
id|seq
comma
id|pos
)paren
suffix:semicolon
id|out
suffix:colon
op_increment
op_star
id|pos
suffix:semicolon
r_return
id|sk
suffix:semicolon
)brace
DECL|function|udp_seq_stop
r_static
r_void
id|udp_seq_stop
c_func
(paren
r_struct
id|seq_file
op_star
id|seq
comma
r_void
op_star
id|v
)paren
(brace
id|read_unlock
c_func
(paren
op_amp
id|udp_hash_lock
)paren
suffix:semicolon
)brace
DECL|function|udp_format_sock
r_static
r_void
id|udp_format_sock
c_func
(paren
r_struct
id|sock
op_star
id|sp
comma
r_char
op_star
id|tmpbuf
comma
r_int
id|bucket
)paren
(brace
r_struct
id|inet_opt
op_star
id|inet
op_assign
id|inet_sk
c_func
(paren
id|sp
)paren
suffix:semicolon
r_int
r_int
id|dest
op_assign
id|inet-&gt;daddr
suffix:semicolon
r_int
r_int
id|src
op_assign
id|inet-&gt;rcv_saddr
suffix:semicolon
id|__u16
id|destp
op_assign
id|ntohs
c_func
(paren
id|inet-&gt;dport
)paren
suffix:semicolon
id|__u16
id|srcp
op_assign
id|ntohs
c_func
(paren
id|inet-&gt;sport
)paren
suffix:semicolon
id|sprintf
c_func
(paren
id|tmpbuf
comma
l_string|&quot;%4d: %08X:%04X %08X:%04X&quot;
l_string|&quot; %02X %08X:%08X %02X:%08lX %08X %5d %8d %lu %d %p&quot;
comma
id|bucket
comma
id|src
comma
id|srcp
comma
id|dest
comma
id|destp
comma
id|sp-&gt;state
comma
id|atomic_read
c_func
(paren
op_amp
id|sp-&gt;wmem_alloc
)paren
comma
id|atomic_read
c_func
(paren
op_amp
id|sp-&gt;rmem_alloc
)paren
comma
l_int|0
comma
l_int|0L
comma
l_int|0
comma
id|sock_i_uid
c_func
(paren
id|sp
)paren
comma
l_int|0
comma
id|sock_i_ino
c_func
(paren
id|sp
)paren
comma
id|atomic_read
c_func
(paren
op_amp
id|sp-&gt;refcnt
)paren
comma
id|sp
)paren
suffix:semicolon
)brace
DECL|function|udp_seq_show
r_static
r_int
id|udp_seq_show
c_func
(paren
r_struct
id|seq_file
op_star
id|seq
comma
r_void
op_star
id|v
)paren
(brace
r_if
c_cond
(paren
id|v
op_eq
(paren
r_void
op_star
)paren
l_int|1
)paren
id|seq_printf
c_func
(paren
id|seq
comma
l_string|&quot;%-127s&bslash;n&quot;
comma
l_string|&quot;  sl  local_address rem_address   st tx_queue &quot;
l_string|&quot;rx_queue tr tm-&gt;when retrnsmt   uid  timeout &quot;
l_string|&quot;inode&quot;
)paren
suffix:semicolon
r_else
(brace
r_char
id|tmpbuf
(braket
l_int|129
)braket
suffix:semicolon
id|udp_format_sock
c_func
(paren
id|v
comma
id|tmpbuf
comma
(paren
r_int
)paren
id|seq
op_member_access_from_pointer
r_private
)paren
suffix:semicolon
id|seq_printf
c_func
(paren
id|seq
comma
l_string|&quot;%-127s&bslash;n&quot;
comma
id|tmpbuf
)paren
suffix:semicolon
)brace
r_return
l_int|0
suffix:semicolon
)brace
multiline_comment|/* ------------------------------------------------------------------------ */
DECL|variable|arp_seq_ops
r_static
r_struct
id|seq_operations
id|arp_seq_ops
op_assign
(brace
dot
id|start
op_assign
id|arp_seq_start
comma
dot
id|next
op_assign
id|arp_seq_next
comma
dot
id|stop
op_assign
id|arp_seq_stop
comma
dot
id|show
op_assign
id|arp_seq_show
comma
)brace
suffix:semicolon
DECL|variable|fib_seq_ops
r_static
r_struct
id|seq_operations
id|fib_seq_ops
op_assign
(brace
dot
id|start
op_assign
id|fib_seq_start
comma
dot
id|next
op_assign
id|fib_seq_next
comma
dot
id|stop
op_assign
id|fib_seq_stop
comma
dot
id|show
op_assign
id|fib_seq_show
comma
)brace
suffix:semicolon
DECL|variable|udp_seq_ops
r_static
r_struct
id|seq_operations
id|udp_seq_ops
op_assign
(brace
dot
id|start
op_assign
id|udp_seq_start
comma
dot
id|next
op_assign
id|udp_seq_next
comma
dot
id|stop
op_assign
id|udp_seq_stop
comma
dot
id|show
op_assign
id|udp_seq_show
comma
)brace
suffix:semicolon
DECL|function|arp_seq_open
r_static
r_int
id|arp_seq_open
c_func
(paren
r_struct
id|inode
op_star
id|inode
comma
r_struct
id|file
op_star
id|file
)paren
(brace
r_return
id|seq_open
c_func
(paren
id|file
comma
op_amp
id|arp_seq_ops
)paren
suffix:semicolon
)brace
DECL|function|fib_seq_open
r_static
r_int
id|fib_seq_open
c_func
(paren
r_struct
id|inode
op_star
id|inode
comma
r_struct
id|file
op_star
id|file
)paren
(brace
r_return
id|seq_open
c_func
(paren
id|file
comma
op_amp
id|fib_seq_ops
)paren
suffix:semicolon
)brace
DECL|function|udp_seq_open
r_static
r_int
id|udp_seq_open
c_func
(paren
r_struct
id|inode
op_star
id|inode
comma
r_struct
id|file
op_star
id|file
)paren
(brace
r_return
id|seq_open
c_func
(paren
id|file
comma
op_amp
id|udp_seq_ops
)paren
suffix:semicolon
)brace
DECL|variable|arp_seq_fops
r_static
r_struct
id|file_operations
id|arp_seq_fops
op_assign
(brace
dot
id|open
op_assign
id|arp_seq_open
comma
dot
id|read
op_assign
id|seq_read
comma
dot
id|llseek
op_assign
id|seq_lseek
comma
dot
id|release
op_assign
id|seq_release
comma
)brace
suffix:semicolon
DECL|variable|fib_seq_fops
r_static
r_struct
id|file_operations
id|fib_seq_fops
op_assign
(brace
dot
id|open
op_assign
id|fib_seq_open
comma
dot
id|read
op_assign
id|seq_read
comma
dot
id|llseek
op_assign
id|seq_lseek
comma
dot
id|release
op_assign
id|seq_release
comma
)brace
suffix:semicolon
DECL|variable|udp_seq_fops
r_static
r_struct
id|file_operations
id|udp_seq_fops
op_assign
(brace
dot
id|open
op_assign
id|udp_seq_open
comma
dot
id|read
op_assign
id|seq_read
comma
dot
id|llseek
op_assign
id|seq_lseek
comma
dot
id|release
op_assign
id|seq_release
comma
)brace
suffix:semicolon
multiline_comment|/* ------------------------------------------------------------------------ */
DECL|function|ipv4_proc_init
r_int
id|__init
id|ipv4_proc_init
c_func
(paren
r_void
)paren
(brace
r_struct
id|proc_dir_entry
op_star
id|p
suffix:semicolon
r_int
id|rc
op_assign
l_int|0
suffix:semicolon
r_if
c_cond
(paren
op_logical_neg
id|proc_net_create
c_func
(paren
l_string|&quot;raw&quot;
comma
l_int|0
comma
id|raw_get_info
)paren
)paren
r_goto
id|out_raw
suffix:semicolon
r_if
c_cond
(paren
op_logical_neg
id|proc_net_create
c_func
(paren
l_string|&quot;netstat&quot;
comma
l_int|0
comma
id|netstat_get_info
)paren
)paren
r_goto
id|out_netstat
suffix:semicolon
r_if
c_cond
(paren
op_logical_neg
id|proc_net_create
c_func
(paren
l_string|&quot;snmp&quot;
comma
l_int|0
comma
id|snmp_get_info
)paren
)paren
r_goto
id|out_snmp
suffix:semicolon
r_if
c_cond
(paren
op_logical_neg
id|proc_net_create
c_func
(paren
l_string|&quot;sockstat&quot;
comma
l_int|0
comma
id|afinet_get_info
)paren
)paren
r_goto
id|out_sockstat
suffix:semicolon
r_if
c_cond
(paren
op_logical_neg
id|proc_net_create
c_func
(paren
l_string|&quot;tcp&quot;
comma
l_int|0
comma
id|tcp_get_info
)paren
)paren
r_goto
id|out_tcp
suffix:semicolon
id|p
op_assign
id|create_proc_entry
c_func
(paren
l_string|&quot;udp&quot;
comma
id|S_IRUGO
comma
id|proc_net
)paren
suffix:semicolon
r_if
c_cond
(paren
op_logical_neg
id|p
)paren
r_goto
id|out_udp
suffix:semicolon
id|p-&gt;proc_fops
op_assign
op_amp
id|udp_seq_fops
suffix:semicolon
id|p
op_assign
id|create_proc_entry
c_func
(paren
l_string|&quot;arp&quot;
comma
id|S_IRUGO
comma
id|proc_net
)paren
suffix:semicolon
r_if
c_cond
(paren
op_logical_neg
id|p
)paren
r_goto
id|out_arp
suffix:semicolon
id|p-&gt;proc_fops
op_assign
op_amp
id|arp_seq_fops
suffix:semicolon
id|p
op_assign
id|create_proc_entry
c_func
(paren
l_string|&quot;route&quot;
comma
id|S_IRUGO
comma
id|proc_net
)paren
suffix:semicolon
r_if
c_cond
(paren
op_logical_neg
id|p
)paren
r_goto
id|out_route
suffix:semicolon
id|p-&gt;proc_fops
op_assign
op_amp
id|fib_seq_fops
suffix:semicolon
id|out
suffix:colon
r_return
id|rc
suffix:semicolon
id|out_route
suffix:colon
id|remove_proc_entry
c_func
(paren
l_string|&quot;route&quot;
comma
id|proc_net
)paren
suffix:semicolon
id|out_arp
suffix:colon
id|remove_proc_entry
c_func
(paren
l_string|&quot;udp&quot;
comma
id|proc_net
)paren
suffix:semicolon
id|out_udp
suffix:colon
id|proc_net_remove
c_func
(paren
l_string|&quot;tcp&quot;
)paren
suffix:semicolon
id|out_tcp
suffix:colon
id|proc_net_remove
c_func
(paren
l_string|&quot;sockstat&quot;
)paren
suffix:semicolon
id|out_sockstat
suffix:colon
id|proc_net_remove
c_func
(paren
l_string|&quot;snmp&quot;
)paren
suffix:semicolon
id|out_snmp
suffix:colon
id|proc_net_remove
c_func
(paren
l_string|&quot;netstat&quot;
)paren
suffix:semicolon
id|out_netstat
suffix:colon
id|proc_net_remove
c_func
(paren
l_string|&quot;raw&quot;
)paren
suffix:semicolon
id|out_raw
suffix:colon
id|rc
op_assign
op_minus
id|ENOMEM
suffix:semicolon
r_goto
id|out
suffix:semicolon
)brace
macro_line|#else /* CONFIG_PROC_FS */
DECL|function|ipv4_proc_init
r_int
id|__init
id|ipv4_proc_init
c_func
(paren
r_void
)paren
(brace
r_return
l_int|0
suffix:semicolon
)brace
macro_line|#endif /* CONFIG_PROC_FS */
eof
