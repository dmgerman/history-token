...
suffix:semicolon
multiline_comment|/*&n; * Pressure flag: try to collapse.&n; * Technical note: it is used by multiple contexts non atomically.&n; * All the sk_stream_mem_schedule() is of this nature: accounting&n; * is strict, actions are advisory and have some latency.&n; */
...
DECL|variable|tcp_enter_memory_pressure
id|EXPORT_SYMBOL
c_func
(paren
id|tcp_enter_memory_pressure
...
DECL|function|do_tcp_sendpages
r_static
id|ssize_t
id|do_tcp_sendpages
c_func
...
id|sock-&gt;sk
suffix:semicolon
DECL|macro|TCP_ZC_CSUM_FLAGS
mdefine_line|#define TCP_ZC_CSUM_FLAGS (NETIF_F_IP_CSUM | NETIF_F_NO_CSUM | NETIF_F_HW_CSUM)
r_if
c_cond
(paren
op_logical_neg
...
suffix:semicolon
DECL|macro|TCP_ZC_CSUM_FLAGS
macro_line|#undef TCP_ZC_CSUM_FLAGS
id|lock_sock
c_func
(paren
id|sk
...
id|new_segment
suffix:colon
multiline_comment|/* Allocate new segment. If the interface is SG,&n;&t;&t;&t;&t; * allocate skb fitting to single page.&n;&t;&t;&t;&t; */
r_if
c_cond
(paren
op_logical_neg
...
id|wait_for_memory
suffix:semicolon
multiline_comment|/*&n;&t;&t;&t;&t; * Check whether we can use HW checksum.&n;&t;&t;&t;&t; */
r_if
c_cond
(paren
id|sk-&gt;sk_route_caps
...
multiline_comment|/* Try to append data to the end of skb. */
r_if
c_cond
(paren
id|copy
...
id|seglen
suffix:semicolon
multiline_comment|/* Where to copy to? */
r_if
c_cond
(paren
id|skb_tailroom
...
(brace
multiline_comment|/* We have some space in skb head. Superb! */
r_if
c_cond
(paren
id|copy
...
(brace
multiline_comment|/* We can extend the last page&n;&t;&t;&t;&t;&t; * fragment. */
id|merge
op_assign
l_int|1
suffix:semicolon
...
(brace
multiline_comment|/* Need to add new fragment and cannot&n;&t;&t;&t;&t;&t; * do this because interface is non-SG,&n;&t;&t;&t;&t;&t; * or because all the page slots are&n;&t;&t;&t;&t;&t; * busy. */
id|tcp_mark_push
c_func
(paren
id|tp
...
(brace
multiline_comment|/* If page is cached, align&n;&t;&t;&t;&t;&t; * offset to L1 cache boundary&n;&t;&t;&t;&t;&t; */
id|off
op_assign
(paren
id|off
...
(brace
multiline_comment|/* Allocate new cache page. */
r_if
c_cond
(paren
op_logical_neg
...
id|off
suffix:semicolon
multiline_comment|/* Time to copy data. We are close to&n;&t;&t;&t;&t; * the end! */
id|err
op_assign
id|skb_copy_to_page
c_func
...
(brace
multiline_comment|/* If this page was new, give it to the&n;&t;&t;&t;&t;&t; * socket so it does not get leaked.&n;&t;&t;&t;&t;&t; */
r_if
c_cond
(paren
op_logical_neg
...
multiline_comment|/* Update the skb. */
r_if
c_cond
(paren
id|merge
...
(brace
multiline_comment|/* Delayed ACKs frequently hit locked sockets during bulk&n;&t;&t;    * receive. */
r_if
c_cond
(paren
id|tp-&gt;ack.blocked
...
multiline_comment|/*&n; * This routine provides an alternative to tcp_recvmsg() for routines&n; * that would like to handle copying from skbuffs directly in &squot;sendfile&squot;&n; * fashion.&n; * Note:&n; *&t;- It is assumed that the socket was locked by the caller.&n; *&t;- The routine does not block.&n; *&t;- At present, there is no support for reading OOB data&n; *&t;  or for &squot;peeking&squot; the socket using this routine&n; *&t;  (although both would be easy to implement).&n; */
DECL|function|tcp_read_sock
r_int
id|tcp_read_sock
c_func
(paren
...
id|offset
suffix:semicolon
multiline_comment|/* Stop reading if we hit a patch of urgent data */
r_if
c_cond
(paren
id|tp-&gt;urg_data
...
(paren
id|KERN_INFO
l_string|&quot;recvmsg bug: copied %X &quot;
l_string|&quot;seq %X&bslash;n&quot;
comma
op_star
id|seq
comma
...
(paren
id|KERN_DEBUG
l_string|&quot;TCP(%s:%d): Application bug, race in MSG_PEEK.&bslash;n&quot;
comma
id|current-&gt;comm
comma
id|current-&gt;pid
...
id|found_fin_ok
suffix:colon
multiline_comment|/* Process the FIN. */
op_increment
op_star
id|seq
suffix:semicolon
...
multiline_comment|/*&n; *&t;Shutdown the sending side of a connection. Much like close except&n; *&t;that we don&squot;t receive shut down or set_sock_flag(sk, SOCK_DEAD).&n; */
...
(paren
id|KERN_INFO
l_string|&quot;TCP: too many of orphaned &quot;
...
id|TCP_MAXSEG
suffix:colon
multiline_comment|/* Values greater than interface MTU won&squot;t take effect. However&n;&t;&t; * at the point when this call is done we typically don&squot;t yet&n;&t;&t; * know which interface is going to be used */
r_if
c_cond
(paren
id|val
...
(brace
multiline_comment|/* Translate value in seconds to number of&n;&t;&t;&t; * retransmits */
r_while
c_loop
(paren
id|tp-&gt;defer_accept
...
id|thash_entries
suffix:semicolon
DECL|function|set_thash_entries
r_static
r_int
id|__init
id|set_thash_entries
c_func
(paren
r_char
op_star
...
(paren
id|KERN_INFO
l_string|&quot;TCP: Hash tables configured &quot;
