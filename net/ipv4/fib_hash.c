...
mdefine_line|#define FZ_MAX_DIVISOR ((PAGE_SIZE&lt;&lt;MAX_ORDER) / sizeof(struct fib_node *))
...
mdefine_line|#define FIB_SCAN_KEY(f, fp, key) &bslash;&n;for ( ; ((f) = *(fp)) != NULL &amp;&amp; ((f)-&gt;fn_key == (key)); (fp) = &amp;(f)-&gt;fn_next)
...
macro_line|#else
DECL|macro|FIB_SCAN_TOS
mdefine_line|#define FIB_SCAN_TOS(f, fp, key, tos) &bslash;&n;for ( ; ((f) = *(fp)) != NULL &amp;&amp; ((f)-&gt;fn_key == (key)) &amp;&amp; &bslash;&n;     (f)-&gt;fn_tos == (tos) ; (fp) = &amp;(f)-&gt;fn_next)
macro_line|#endif
r_static
r_void
id|rtmsg_fib
