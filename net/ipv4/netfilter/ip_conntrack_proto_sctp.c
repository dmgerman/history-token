...
mdefine_line|#define for_each_sctp_chunk(skb, sch, offset, count)&t;&bslash;&n;for (offset = skb-&gt;nh.iph-&gt;ihl * 4 + sizeof (sctp_sctphdr_t), count = 0;&t;&bslash;&n;&t;offset &lt; skb-&gt;len &amp;&amp; !skb_copy_bits(skb, offset, &amp;sch, sizeof(sch));&t;&bslash;&n;&t;offset += (htons(sch.length) + 3) &amp; ~3, count++)
