...
comma
multiline_comment|/*&n; *&t;sSS -&gt; sSR&t;Standard open.&n; *&t;sSR -&gt; sSR&t;Retransmitted SYN/ACK.&n; *&t;sES -&gt; sIG&t;Late retransmitted SYN/ACK?&n; *&t;sFW -&gt; sIG&t;Might be SYN/ACK answering ignored SYN&n; *&t;sCW -&gt; sIG&n; *&t;sLA -&gt; sIG&n; *&t;sTW -&gt; sIG&n; *&t;sCL -&gt; sIG&n; */
multiline_comment|/* &t;     sNO, sSS, sSR, sES, sFW, sCW, sLA, sTW, sCL, sLI&t;*/
multiline_comment|/*fin*/
(brace
id|sIV
...
comma
multiline_comment|/*&n; *&t;sSS -&gt; sIV&t;Might be a half-open connection.&n; *&t;sSR -&gt; sSR&t;Might answer late resent SYN.&n; *&t;sES -&gt; sES&t;:-)&n; *&t;sFW -&gt; sCW&t;Normal close request answered by ACK.&n; *&t;sCW -&gt; sCW&n; *&t;sLA -&gt; sTW&t;Last ACK detected.&n; *&t;sTW -&gt; sTW&t;Retransmitted last ACK.&n; *&t;sCL -&gt; sCL&n; */
multiline_comment|/* &t;     sNO, sSS, sSR, sES, sFW, sCW, sLA, sTW, sCL, sLI&t;*/
multiline_comment|/*rst*/
(brace
id|sIV
...
multiline_comment|/* TCP connection tracking based on &squot;Real Stateful TCP Packet Filtering&n;   in IP Filter&squot; by Guido van Rooij.&n;   &n;   http://www.nluug.nl/events/sane2000/papers.html&n;   http://www.iae.nl/users/guido/papers/tcp_filtering.ps.gz&n;   &n;   The boundaries and the conditions are changed according to RFC793:&n;   the packet must intersect the window (i.e. segments may be&n;   after the right or before the left edge) and thus receivers may ACK&n;   segments after the right edge of the window.&n;&n;   &t;td_maxend = max(sack + max(win,1)) seen in reply packets&n;&t;td_maxwin = max(max(win, 1)) + (sack - ack) seen in sent packets&n;&t;td_maxwin += seq + len - sender.td_maxend&n;&t;&t;&t;if seq + len &gt; sender.td_maxend&n;&t;td_end    = max(seq + len) seen in sent packets&n;   &n;   I.   Upper bound for valid data:&t;seq &lt;= sender.td_maxend&n;   II.  Lower bound for valid data:&t;seq + len &gt;= sender.td_end - receiver.td_maxwin&n;   III.&t;Upper bound for valid ack:      sack &lt;= receiver.td_end&n;   IV.&t;Lower bound for valid ack:&t;ack &gt;= receiver.td_end - MAXACKWINDOW&n;   &t;&n;   where sack is the highest right edge of sack block found in the packet.&n;   &t;&n;   The upper bound limit for a valid ack is not ignored - &n;   we doesn&squot;t have to deal with fragments. &n;*/
...
multiline_comment|/*&n;&t;&t; * Packets contains no data: we assume it is valid&n;&t;&t; * and check the ack value only.&n;&t;&t; * However RST segments are always validated by their&n;&t;&t; * SEQ number, except when seq == 0 (reset sent answering&n;&t;&t; * SYN.&n;&t;&t; */
id|seq
op_assign
id|end
op_assign
...
id|end
suffix:semicolon
multiline_comment|/*&n;&t;&t; * Update receiver data.&n;&t;&t; */
r_if
c_cond
(paren
id|after
...
l_string|&quot;BUG&quot;
suffix:colon
l_string|&quot;ACK is under the lower bound (possible overly delayed ACK)&quot;
suffix:colon
l_string|&quot;ACK is over the upper bound (ACKed data not seen yet)&quot;
suffix:colon
l_string|&quot;SEQ is under the lower bound (already ACKed data retransmitted)&quot;
suffix:colon
...
suffix:colon
multiline_comment|/* Either SYN in ORIGINAL&n;&t;&t; * or SYN/ACK in REPLY. */
r_if
c_cond
(paren
id|index
...
l_int|NULL
comma
l_string|&quot;ip_ct_tcp: invalid SYN&quot;
)paren
suffix:semicolon
r_return
op_minus
...
(brace
multiline_comment|/* RST sent to invalid SYN we had let trough&n;&t;&t;&t; * SYN was in window then, tear down connection.&n;&t;&t;&t; * We skip window checking, because packet might ACK&n;&t;&t;&t; * segments we ignored in the SYN. */
r_goto
id|in_window
suffix:semicolon
)brace
