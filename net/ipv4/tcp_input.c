...
multiline_comment|/* Increase initial CWND conservatively: if estimated&n; * RTT is low enough (&lt;20msec) or if we have some preset ssthresh.&n; *&n; * Numbers are taken from RFC2414.&n; */
...
multiline_comment|/* Out of window. F.e. zero window probe.&n;&t; *&n;&t; * Note: it is highly possible that we may open window and enqueue&n;&t; * this segment now. However, this will be known only after we queue&n;&t; * it, which will result in queue full of successive 1 byte BSD&n;&t; * window probes, it is SWS in fact. So, always reject it and send ACK.&n;&t; */
r_if
c_cond
(paren
op_logical_neg
