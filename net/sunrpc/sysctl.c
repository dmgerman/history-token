multiline_comment|/*&n; * linux/net/sunrpc/sysctl.c&n; *&n; * Sysctl interface to sunrpc module.&n; *&n; * I would prefer to register the sunrpc table below sys/net, but that&squot;s&n; * impossible at the moment.&n; */
macro_line|#include &lt;linux/config.h&gt;
macro_line|#include &lt;linux/types.h&gt;
macro_line|#include &lt;linux/linkage.h&gt;
macro_line|#include &lt;linux/ctype.h&gt;
macro_line|#include &lt;linux/fs.h&gt;
macro_line|#include &lt;linux/sysctl.h&gt;
macro_line|#include &lt;linux/module.h&gt;
macro_line|#include &lt;asm/uaccess.h&gt;
macro_line|#include &lt;linux/sunrpc/types.h&gt;
macro_line|#include &lt;linux/sunrpc/sched.h&gt;
macro_line|#include &lt;linux/sunrpc/stats.h&gt;
macro_line|#include &lt;linux/sunrpc/xprt.h&gt;
multiline_comment|/*&n; * Declare the debug flags here&n; */
DECL|variable|rpc_debug
r_int
r_int
id|rpc_debug
suffix:semicolon
DECL|variable|nfs_debug
r_int
r_int
id|nfs_debug
suffix:semicolon
DECL|variable|nfsd_debug
r_int
r_int
id|nfsd_debug
suffix:semicolon
DECL|variable|nlm_debug
r_int
r_int
id|nlm_debug
suffix:semicolon
macro_line|#ifdef RPC_DEBUG
DECL|variable|sunrpc_table_header
r_static
r_struct
id|ctl_table_header
op_star
id|sunrpc_table_header
suffix:semicolon
DECL|variable|sunrpc_table
r_static
id|ctl_table
id|sunrpc_table
(braket
)braket
suffix:semicolon
r_void
DECL|function|rpc_register_sysctl
id|rpc_register_sysctl
c_func
(paren
r_void
)paren
(brace
r_if
c_cond
(paren
op_logical_neg
id|sunrpc_table_header
)paren
(brace
id|sunrpc_table_header
op_assign
id|register_sysctl_table
c_func
(paren
id|sunrpc_table
comma
l_int|1
)paren
suffix:semicolon
macro_line|#ifdef CONFIG_PROC_FS
r_if
c_cond
(paren
id|sunrpc_table
(braket
l_int|0
)braket
dot
id|de
)paren
id|sunrpc_table
(braket
l_int|0
)braket
dot
id|de-&gt;owner
op_assign
id|THIS_MODULE
suffix:semicolon
macro_line|#endif
)brace
)brace
r_void
DECL|function|rpc_unregister_sysctl
id|rpc_unregister_sysctl
c_func
(paren
r_void
)paren
(brace
r_if
c_cond
(paren
id|sunrpc_table_header
)paren
(brace
id|unregister_sysctl_table
c_func
(paren
id|sunrpc_table_header
)paren
suffix:semicolon
id|sunrpc_table_header
op_assign
l_int|NULL
suffix:semicolon
)brace
)brace
r_static
r_int
DECL|function|proc_dodebug
id|proc_dodebug
c_func
(paren
id|ctl_table
op_star
id|table
comma
r_int
id|write
comma
r_struct
id|file
op_star
id|file
comma
r_void
id|__user
op_star
id|buffer
comma
r_int
op_star
id|lenp
)paren
(brace
r_char
id|tmpbuf
(braket
l_int|20
)braket
comma
id|c
comma
op_star
id|s
suffix:semicolon
r_char
id|__user
op_star
id|p
suffix:semicolon
r_int
r_int
id|value
suffix:semicolon
r_int
id|left
comma
id|len
suffix:semicolon
r_if
c_cond
(paren
(paren
id|file-&gt;f_pos
op_logical_and
op_logical_neg
id|write
)paren
op_logical_or
op_logical_neg
op_star
id|lenp
)paren
(brace
op_star
id|lenp
op_assign
l_int|0
suffix:semicolon
r_return
l_int|0
suffix:semicolon
)brace
id|left
op_assign
op_star
id|lenp
suffix:semicolon
r_if
c_cond
(paren
id|write
)paren
(brace
r_if
c_cond
(paren
op_logical_neg
id|access_ok
c_func
(paren
id|VERIFY_READ
comma
id|buffer
comma
id|left
)paren
)paren
r_return
op_minus
id|EFAULT
suffix:semicolon
id|p
op_assign
id|buffer
suffix:semicolon
r_while
c_loop
(paren
id|left
op_logical_and
id|__get_user
c_func
(paren
id|c
comma
id|p
)paren
op_ge
l_int|0
op_logical_and
id|isspace
c_func
(paren
id|c
)paren
)paren
id|left
op_decrement
comma
id|p
op_increment
suffix:semicolon
r_if
c_cond
(paren
op_logical_neg
id|left
)paren
r_goto
id|done
suffix:semicolon
r_if
c_cond
(paren
id|left
OG
r_sizeof
(paren
id|tmpbuf
)paren
op_minus
l_int|1
)paren
r_return
op_minus
id|EINVAL
suffix:semicolon
r_if
c_cond
(paren
id|copy_from_user
c_func
(paren
id|tmpbuf
comma
id|p
comma
id|left
)paren
)paren
r_return
op_minus
id|EFAULT
suffix:semicolon
id|tmpbuf
(braket
id|left
)braket
op_assign
l_char|&squot;&bslash;0&squot;
suffix:semicolon
r_for
c_loop
(paren
id|s
op_assign
id|tmpbuf
comma
id|value
op_assign
l_int|0
suffix:semicolon
l_char|&squot;0&squot;
op_le
op_star
id|s
op_logical_and
op_star
id|s
op_le
l_char|&squot;9&squot;
suffix:semicolon
id|s
op_increment
comma
id|left
op_decrement
)paren
id|value
op_assign
l_int|10
op_star
id|value
op_plus
(paren
op_star
id|s
op_minus
l_char|&squot;0&squot;
)paren
suffix:semicolon
r_if
c_cond
(paren
op_star
id|s
op_logical_and
op_logical_neg
id|isspace
c_func
(paren
op_star
id|s
)paren
)paren
r_return
op_minus
id|EINVAL
suffix:semicolon
r_while
c_loop
(paren
id|left
op_logical_and
id|isspace
c_func
(paren
op_star
id|s
)paren
)paren
id|left
op_decrement
comma
id|s
op_increment
suffix:semicolon
op_star
(paren
r_int
r_int
op_star
)paren
id|table-&gt;data
op_assign
id|value
suffix:semicolon
multiline_comment|/* Display the RPC tasks on writing to rpc_debug */
r_if
c_cond
(paren
id|table-&gt;ctl_name
op_eq
id|CTL_RPCDEBUG
)paren
(brace
id|rpc_show_tasks
c_func
(paren
)paren
suffix:semicolon
)brace
)brace
r_else
(brace
r_if
c_cond
(paren
op_logical_neg
id|access_ok
c_func
(paren
id|VERIFY_WRITE
comma
id|buffer
comma
id|left
)paren
)paren
r_return
op_minus
id|EFAULT
suffix:semicolon
id|len
op_assign
id|sprintf
c_func
(paren
id|tmpbuf
comma
l_string|&quot;%d&quot;
comma
op_star
(paren
r_int
r_int
op_star
)paren
id|table-&gt;data
)paren
suffix:semicolon
r_if
c_cond
(paren
id|len
OG
id|left
)paren
id|len
op_assign
id|left
suffix:semicolon
r_if
c_cond
(paren
id|__copy_to_user
c_func
(paren
id|buffer
comma
id|tmpbuf
comma
id|len
)paren
)paren
r_return
op_minus
id|EFAULT
suffix:semicolon
r_if
c_cond
(paren
(paren
id|left
op_sub_assign
id|len
)paren
OG
l_int|0
)paren
(brace
r_if
c_cond
(paren
id|put_user
c_func
(paren
l_char|&squot;&bslash;n&squot;
comma
(paren
r_char
id|__user
op_star
)paren
id|buffer
op_plus
id|len
)paren
)paren
r_return
op_minus
id|EFAULT
suffix:semicolon
id|left
op_decrement
suffix:semicolon
)brace
)brace
id|done
suffix:colon
op_star
id|lenp
op_sub_assign
id|left
suffix:semicolon
id|file-&gt;f_pos
op_add_assign
op_star
id|lenp
suffix:semicolon
r_return
l_int|0
suffix:semicolon
)brace
DECL|variable|min_slot_table_size
r_static
r_int
r_int
id|min_slot_table_size
op_assign
id|RPC_MIN_SLOT_TABLE
suffix:semicolon
DECL|variable|max_slot_table_size
r_static
r_int
r_int
id|max_slot_table_size
op_assign
id|RPC_MAX_SLOT_TABLE
suffix:semicolon
DECL|variable|debug_table
r_static
id|ctl_table
id|debug_table
(braket
)braket
op_assign
(brace
(brace
dot
id|ctl_name
op_assign
id|CTL_RPCDEBUG
comma
dot
id|procname
op_assign
l_string|&quot;rpc_debug&quot;
comma
dot
id|data
op_assign
op_amp
id|rpc_debug
comma
dot
id|maxlen
op_assign
r_sizeof
(paren
r_int
)paren
comma
dot
id|mode
op_assign
l_int|0644
comma
dot
id|proc_handler
op_assign
op_amp
id|proc_dodebug
)brace
comma
(brace
dot
id|ctl_name
op_assign
id|CTL_NFSDEBUG
comma
dot
id|procname
op_assign
l_string|&quot;nfs_debug&quot;
comma
dot
id|data
op_assign
op_amp
id|nfs_debug
comma
dot
id|maxlen
op_assign
r_sizeof
(paren
r_int
)paren
comma
dot
id|mode
op_assign
l_int|0644
comma
dot
id|proc_handler
op_assign
op_amp
id|proc_dodebug
)brace
comma
(brace
dot
id|ctl_name
op_assign
id|CTL_NFSDDEBUG
comma
dot
id|procname
op_assign
l_string|&quot;nfsd_debug&quot;
comma
dot
id|data
op_assign
op_amp
id|nfsd_debug
comma
dot
id|maxlen
op_assign
r_sizeof
(paren
r_int
)paren
comma
dot
id|mode
op_assign
l_int|0644
comma
dot
id|proc_handler
op_assign
op_amp
id|proc_dodebug
)brace
comma
(brace
dot
id|ctl_name
op_assign
id|CTL_NLMDEBUG
comma
dot
id|procname
op_assign
l_string|&quot;nlm_debug&quot;
comma
dot
id|data
op_assign
op_amp
id|nlm_debug
comma
dot
id|maxlen
op_assign
r_sizeof
(paren
r_int
)paren
comma
dot
id|mode
op_assign
l_int|0644
comma
dot
id|proc_handler
op_assign
op_amp
id|proc_dodebug
)brace
comma
(brace
dot
id|ctl_name
op_assign
id|CTL_SLOTTABLE_UDP
comma
dot
id|procname
op_assign
l_string|&quot;udp_slot_table_entries&quot;
comma
dot
id|data
op_assign
op_amp
id|xprt_udp_slot_table_entries
comma
dot
id|maxlen
op_assign
r_sizeof
(paren
r_int
r_int
)paren
comma
dot
id|mode
op_assign
l_int|0644
comma
dot
id|proc_handler
op_assign
op_amp
id|proc_dointvec_minmax
comma
dot
id|strategy
op_assign
op_amp
id|sysctl_intvec
comma
dot
id|extra1
op_assign
op_amp
id|min_slot_table_size
comma
dot
id|extra2
op_assign
op_amp
id|max_slot_table_size
)brace
comma
(brace
dot
id|ctl_name
op_assign
id|CTL_SLOTTABLE_TCP
comma
dot
id|procname
op_assign
l_string|&quot;tcp_slot_table_entries&quot;
comma
dot
id|data
op_assign
op_amp
id|xprt_tcp_slot_table_entries
comma
dot
id|maxlen
op_assign
r_sizeof
(paren
r_int
r_int
)paren
comma
dot
id|mode
op_assign
l_int|0644
comma
dot
id|proc_handler
op_assign
op_amp
id|proc_dointvec_minmax
comma
dot
id|strategy
op_assign
op_amp
id|sysctl_intvec
comma
dot
id|extra1
op_assign
op_amp
id|min_slot_table_size
comma
dot
id|extra2
op_assign
op_amp
id|max_slot_table_size
)brace
comma
(brace
dot
id|ctl_name
op_assign
l_int|0
)brace
)brace
suffix:semicolon
DECL|variable|sunrpc_table
r_static
id|ctl_table
id|sunrpc_table
(braket
)braket
op_assign
(brace
(brace
dot
id|ctl_name
op_assign
id|CTL_SUNRPC
comma
dot
id|procname
op_assign
l_string|&quot;sunrpc&quot;
comma
dot
id|mode
op_assign
l_int|0555
comma
dot
id|child
op_assign
id|debug_table
)brace
comma
(brace
dot
id|ctl_name
op_assign
l_int|0
)brace
)brace
suffix:semicolon
macro_line|#endif
eof
