...
suffix:semicolon
multiline_comment|/*&n; * Use to link togheter all the &quot;struct eventpoll&quot;. We need to link&n; * all the available eventpoll structures to be able to perform proper&n; * cleanup in case a file that is stored inside epoll is closed without&n; * previously being removed.&n; */
...
id|eplist
suffix:semicolon
multiline_comment|/*&n; * Serialize the access to &quot;eplist&quot; and also to ep_notify_file_close().&n; * It is read-held when we want to be sure that a given file will not&n; * vanish while we&squot;re doing f_op-&gt;poll(). When &quot;ep-&gt;lock&quot; is taken,&n; * it will nest inside this semaphore.&n; */
DECL|variable|epsem
r_struct
id|rw_semaphore
id|epsem
...
suffix:semicolon
multiline_comment|/*&n;&t;&t; * The ep_find() function increases the &quot;struct epitem&quot; usage count&n;&t;&t; * so we have to do an ep_remove() + ep_release_epitem().&n;&t;&t; */
r_while
c_loop
(paren
(paren
...
multiline_comment|/*&n;&t; * Walks through the whole hash by freeing each &quot;struct epitem&quot;. At this&n;&t; * point we are sure no poll callbacks will be lingering around, so we can&n;&t; * avoid the lock on &quot;ep-&gt;lock&quot;.&n;&t; */
r_for
c_loop
(paren
id|i
...
suffix:semicolon
multiline_comment|/* Initialize the rwsem used to access &quot;eplist&quot; */
id|init_rwsem
c_func
(paren
op_amp
