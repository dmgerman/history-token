multiline_comment|/*&n; *  linux/fs/hpfs/inode.c&n; *&n; *  Mikulas Patocka (mikulas@artax.karlin.mff.cuni.cz), 1998-1999&n; *&n; *  inode VFS functions&n; */
macro_line|#include &lt;linux/fs.h&gt;
macro_line|#include &lt;linux/time.h&gt;
macro_line|#include &lt;linux/smp_lock.h&gt;
macro_line|#include &lt;linux/buffer_head.h&gt;
macro_line|#include &quot;hpfs_fn.h&quot;
DECL|variable|hpfs_file_ops
r_static
r_struct
id|file_operations
id|hpfs_file_ops
op_assign
(brace
dot
id|llseek
op_assign
id|generic_file_llseek
comma
dot
id|read
op_assign
id|generic_file_read
comma
dot
id|write
op_assign
id|hpfs_file_write
comma
dot
id|mmap
op_assign
id|generic_file_mmap
comma
dot
id|open
op_assign
id|hpfs_open
comma
dot
id|release
op_assign
id|hpfs_file_release
comma
dot
id|fsync
op_assign
id|hpfs_file_fsync
comma
dot
id|sendfile
op_assign
id|generic_file_sendfile
comma
)brace
suffix:semicolon
DECL|variable|hpfs_file_iops
r_static
r_struct
id|inode_operations
id|hpfs_file_iops
op_assign
(brace
dot
id|truncate
op_assign
id|hpfs_truncate
comma
dot
id|setattr
op_assign
id|hpfs_notify_change
comma
)brace
suffix:semicolon
DECL|variable|hpfs_dir_ops
r_static
r_struct
id|file_operations
id|hpfs_dir_ops
op_assign
(brace
dot
id|llseek
op_assign
id|hpfs_dir_lseek
comma
dot
id|read
op_assign
id|generic_read_dir
comma
dot
id|readdir
op_assign
id|hpfs_readdir
comma
dot
id|open
op_assign
id|hpfs_open
comma
dot
id|release
op_assign
id|hpfs_dir_release
comma
dot
id|fsync
op_assign
id|hpfs_file_fsync
comma
)brace
suffix:semicolon
DECL|variable|hpfs_dir_iops
r_static
r_struct
id|inode_operations
id|hpfs_dir_iops
op_assign
(brace
dot
id|create
op_assign
id|hpfs_create
comma
dot
id|lookup
op_assign
id|hpfs_lookup
comma
dot
id|unlink
op_assign
id|hpfs_unlink
comma
dot
id|symlink
op_assign
id|hpfs_symlink
comma
dot
id|mkdir
op_assign
id|hpfs_mkdir
comma
dot
id|rmdir
op_assign
id|hpfs_rmdir
comma
dot
id|mknod
op_assign
id|hpfs_mknod
comma
dot
id|rename
op_assign
id|hpfs_rename
comma
dot
id|setattr
op_assign
id|hpfs_notify_change
comma
)brace
suffix:semicolon
DECL|variable|hpfs_symlink_aops
r_struct
id|address_space_operations
id|hpfs_symlink_aops
op_assign
(brace
dot
id|readpage
op_assign
id|hpfs_symlink_readpage
)brace
suffix:semicolon
DECL|function|hpfs_init_inode
r_void
id|hpfs_init_inode
c_func
(paren
r_struct
id|inode
op_star
id|i
)paren
(brace
r_struct
id|super_block
op_star
id|sb
op_assign
id|i-&gt;i_sb
suffix:semicolon
r_struct
id|hpfs_inode_info
op_star
id|hpfs_inode
op_assign
id|hpfs_i
c_func
(paren
id|i
)paren
suffix:semicolon
id|i-&gt;i_uid
op_assign
id|hpfs_sb
c_func
(paren
id|sb
)paren
op_member_access_from_pointer
id|sb_uid
suffix:semicolon
id|i-&gt;i_gid
op_assign
id|hpfs_sb
c_func
(paren
id|sb
)paren
op_member_access_from_pointer
id|sb_gid
suffix:semicolon
id|i-&gt;i_mode
op_assign
id|hpfs_sb
c_func
(paren
id|sb
)paren
op_member_access_from_pointer
id|sb_mode
suffix:semicolon
id|hpfs_inode-&gt;i_conv
op_assign
id|hpfs_sb
c_func
(paren
id|sb
)paren
op_member_access_from_pointer
id|sb_conv
suffix:semicolon
id|i-&gt;i_blksize
op_assign
l_int|512
suffix:semicolon
id|i-&gt;i_size
op_assign
op_minus
l_int|1
suffix:semicolon
id|i-&gt;i_blocks
op_assign
op_minus
l_int|1
suffix:semicolon
id|hpfs_inode-&gt;i_dno
op_assign
l_int|0
suffix:semicolon
id|hpfs_inode-&gt;i_n_secs
op_assign
l_int|0
suffix:semicolon
id|hpfs_inode-&gt;i_file_sec
op_assign
l_int|0
suffix:semicolon
id|hpfs_inode-&gt;i_disk_sec
op_assign
l_int|0
suffix:semicolon
id|hpfs_inode-&gt;i_dpos
op_assign
l_int|0
suffix:semicolon
id|hpfs_inode-&gt;i_dsubdno
op_assign
l_int|0
suffix:semicolon
id|hpfs_inode-&gt;i_ea_mode
op_assign
l_int|0
suffix:semicolon
id|hpfs_inode-&gt;i_ea_uid
op_assign
l_int|0
suffix:semicolon
id|hpfs_inode-&gt;i_ea_gid
op_assign
l_int|0
suffix:semicolon
id|hpfs_inode-&gt;i_ea_size
op_assign
l_int|0
suffix:semicolon
id|hpfs_inode-&gt;i_rddir_off
op_assign
l_int|NULL
suffix:semicolon
id|hpfs_inode-&gt;i_dirty
op_assign
l_int|0
suffix:semicolon
id|i-&gt;i_ctime.tv_sec
op_assign
id|i-&gt;i_ctime.tv_nsec
op_assign
l_int|0
suffix:semicolon
id|i-&gt;i_mtime.tv_sec
op_assign
id|i-&gt;i_mtime.tv_nsec
op_assign
l_int|0
suffix:semicolon
id|i-&gt;i_atime.tv_sec
op_assign
id|i-&gt;i_atime.tv_nsec
op_assign
l_int|0
suffix:semicolon
)brace
DECL|function|hpfs_read_inode
r_void
id|hpfs_read_inode
c_func
(paren
r_struct
id|inode
op_star
id|i
)paren
(brace
r_struct
id|buffer_head
op_star
id|bh
suffix:semicolon
r_struct
id|fnode
op_star
id|fnode
suffix:semicolon
r_struct
id|super_block
op_star
id|sb
op_assign
id|i-&gt;i_sb
suffix:semicolon
r_struct
id|hpfs_inode_info
op_star
id|hpfs_inode
op_assign
id|hpfs_i
c_func
(paren
id|i
)paren
suffix:semicolon
r_int
r_char
op_star
id|ea
suffix:semicolon
r_int
id|ea_size
suffix:semicolon
id|hpfs_init_inode
c_func
(paren
id|i
)paren
suffix:semicolon
r_if
c_cond
(paren
op_logical_neg
id|hpfs_sb
c_func
(paren
id|i-&gt;i_sb
)paren
op_member_access_from_pointer
id|sb_rd_inode
)paren
id|hpfs_error
c_func
(paren
id|i-&gt;i_sb
comma
l_string|&quot;read_inode: sb_rd_inode == 0&quot;
)paren
suffix:semicolon
r_if
c_cond
(paren
id|hpfs_sb
c_func
(paren
id|i-&gt;i_sb
)paren
op_member_access_from_pointer
id|sb_rd_inode
op_eq
l_int|2
)paren
(brace
id|i-&gt;i_mode
op_or_assign
id|S_IFREG
suffix:semicolon
id|i-&gt;i_mode
op_and_assign
op_complement
l_int|0111
suffix:semicolon
id|i-&gt;i_op
op_assign
op_amp
id|hpfs_file_iops
suffix:semicolon
id|i-&gt;i_fop
op_assign
op_amp
id|hpfs_file_ops
suffix:semicolon
id|i-&gt;i_nlink
op_assign
l_int|1
suffix:semicolon
r_return
suffix:semicolon
)brace
r_if
c_cond
(paren
op_logical_neg
(paren
id|fnode
op_assign
id|hpfs_map_fnode
c_func
(paren
id|sb
comma
id|i-&gt;i_ino
comma
op_amp
id|bh
)paren
)paren
)paren
(brace
multiline_comment|/*i-&gt;i_mode |= S_IFREG;&n;&t;&t;i-&gt;i_mode &amp;= ~0111;&n;&t;&t;i-&gt;i_op = &amp;hpfs_file_iops;&n;&t;&t;i-&gt;i_fop = &amp;hpfs_file_ops;&n;&t;&t;i-&gt;i_nlink = 0;*/
id|make_bad_inode
c_func
(paren
id|i
)paren
suffix:semicolon
r_return
suffix:semicolon
)brace
r_if
c_cond
(paren
id|hpfs_sb
c_func
(paren
id|i-&gt;i_sb
)paren
op_member_access_from_pointer
id|sb_eas
)paren
(brace
r_if
c_cond
(paren
(paren
id|ea
op_assign
id|hpfs_get_ea
c_func
(paren
id|i-&gt;i_sb
comma
id|fnode
comma
l_string|&quot;UID&quot;
comma
op_amp
id|ea_size
)paren
)paren
)paren
(brace
r_if
c_cond
(paren
id|ea_size
op_eq
l_int|2
)paren
(brace
id|i-&gt;i_uid
op_assign
id|le16_to_cpu
c_func
(paren
op_star
(paren
id|u16
op_star
)paren
id|ea
)paren
suffix:semicolon
id|hpfs_inode-&gt;i_ea_uid
op_assign
l_int|1
suffix:semicolon
)brace
id|kfree
c_func
(paren
id|ea
)paren
suffix:semicolon
)brace
r_if
c_cond
(paren
(paren
id|ea
op_assign
id|hpfs_get_ea
c_func
(paren
id|i-&gt;i_sb
comma
id|fnode
comma
l_string|&quot;GID&quot;
comma
op_amp
id|ea_size
)paren
)paren
)paren
(brace
r_if
c_cond
(paren
id|ea_size
op_eq
l_int|2
)paren
(brace
id|i-&gt;i_gid
op_assign
id|le16_to_cpu
c_func
(paren
op_star
(paren
id|u16
op_star
)paren
id|ea
)paren
suffix:semicolon
id|hpfs_inode-&gt;i_ea_gid
op_assign
l_int|1
suffix:semicolon
)brace
id|kfree
c_func
(paren
id|ea
)paren
suffix:semicolon
)brace
r_if
c_cond
(paren
(paren
id|ea
op_assign
id|hpfs_get_ea
c_func
(paren
id|i-&gt;i_sb
comma
id|fnode
comma
l_string|&quot;SYMLINK&quot;
comma
op_amp
id|ea_size
)paren
)paren
)paren
(brace
id|kfree
c_func
(paren
id|ea
)paren
suffix:semicolon
id|i-&gt;i_mode
op_assign
id|S_IFLNK
op_or
l_int|0777
suffix:semicolon
id|i-&gt;i_op
op_assign
op_amp
id|page_symlink_inode_operations
suffix:semicolon
id|i-&gt;i_data.a_ops
op_assign
op_amp
id|hpfs_symlink_aops
suffix:semicolon
id|i-&gt;i_nlink
op_assign
l_int|1
suffix:semicolon
id|i-&gt;i_size
op_assign
id|ea_size
suffix:semicolon
id|i-&gt;i_blocks
op_assign
l_int|1
suffix:semicolon
id|brelse
c_func
(paren
id|bh
)paren
suffix:semicolon
r_return
suffix:semicolon
)brace
r_if
c_cond
(paren
(paren
id|ea
op_assign
id|hpfs_get_ea
c_func
(paren
id|i-&gt;i_sb
comma
id|fnode
comma
l_string|&quot;MODE&quot;
comma
op_amp
id|ea_size
)paren
)paren
)paren
(brace
r_int
id|rdev
op_assign
l_int|0
suffix:semicolon
id|umode_t
id|mode
op_assign
id|hpfs_sb
c_func
(paren
id|sb
)paren
op_member_access_from_pointer
id|sb_mode
suffix:semicolon
r_if
c_cond
(paren
id|ea_size
op_eq
l_int|2
)paren
(brace
id|mode
op_assign
id|le16_to_cpu
c_func
(paren
op_star
(paren
id|u16
op_star
)paren
id|ea
)paren
suffix:semicolon
id|hpfs_inode-&gt;i_ea_mode
op_assign
l_int|1
suffix:semicolon
)brace
id|kfree
c_func
(paren
id|ea
)paren
suffix:semicolon
id|i-&gt;i_mode
op_assign
id|mode
suffix:semicolon
r_if
c_cond
(paren
id|S_ISBLK
c_func
(paren
id|mode
)paren
op_logical_or
id|S_ISCHR
c_func
(paren
id|mode
)paren
)paren
(brace
r_if
c_cond
(paren
(paren
id|ea
op_assign
id|hpfs_get_ea
c_func
(paren
id|i-&gt;i_sb
comma
id|fnode
comma
l_string|&quot;DEV&quot;
comma
op_amp
id|ea_size
)paren
)paren
)paren
(brace
r_if
c_cond
(paren
id|ea_size
op_eq
l_int|4
)paren
id|rdev
op_assign
id|le32_to_cpu
c_func
(paren
op_star
(paren
id|u32
op_star
)paren
id|ea
)paren
suffix:semicolon
id|kfree
c_func
(paren
id|ea
)paren
suffix:semicolon
)brace
)brace
r_if
c_cond
(paren
id|S_ISBLK
c_func
(paren
id|mode
)paren
op_logical_or
id|S_ISCHR
c_func
(paren
id|mode
)paren
op_logical_or
id|S_ISFIFO
c_func
(paren
id|mode
)paren
op_logical_or
id|S_ISSOCK
c_func
(paren
id|mode
)paren
)paren
(brace
id|brelse
c_func
(paren
id|bh
)paren
suffix:semicolon
id|i-&gt;i_nlink
op_assign
l_int|1
suffix:semicolon
id|i-&gt;i_size
op_assign
l_int|0
suffix:semicolon
id|i-&gt;i_blocks
op_assign
l_int|1
suffix:semicolon
id|init_special_inode
c_func
(paren
id|i
comma
id|mode
comma
id|new_decode_dev
c_func
(paren
id|rdev
)paren
)paren
suffix:semicolon
r_return
suffix:semicolon
)brace
)brace
)brace
r_if
c_cond
(paren
id|fnode-&gt;dirflag
)paren
(brace
r_int
id|n_dnodes
comma
id|n_subdirs
suffix:semicolon
id|i-&gt;i_mode
op_or_assign
id|S_IFDIR
suffix:semicolon
id|i-&gt;i_op
op_assign
op_amp
id|hpfs_dir_iops
suffix:semicolon
id|i-&gt;i_fop
op_assign
op_amp
id|hpfs_dir_ops
suffix:semicolon
id|hpfs_inode-&gt;i_parent_dir
op_assign
id|fnode-&gt;up
suffix:semicolon
id|hpfs_inode-&gt;i_dno
op_assign
id|fnode-&gt;u.external
(braket
l_int|0
)braket
dot
id|disk_secno
suffix:semicolon
r_if
c_cond
(paren
id|hpfs_sb
c_func
(paren
id|sb
)paren
op_member_access_from_pointer
id|sb_chk
op_ge
l_int|2
)paren
(brace
r_struct
id|buffer_head
op_star
id|bh0
suffix:semicolon
r_if
c_cond
(paren
id|hpfs_map_fnode
c_func
(paren
id|sb
comma
id|hpfs_inode-&gt;i_parent_dir
comma
op_amp
id|bh0
)paren
)paren
id|brelse
c_func
(paren
id|bh0
)paren
suffix:semicolon
)brace
id|n_dnodes
op_assign
l_int|0
suffix:semicolon
id|n_subdirs
op_assign
l_int|0
suffix:semicolon
id|hpfs_count_dnodes
c_func
(paren
id|i-&gt;i_sb
comma
id|hpfs_inode-&gt;i_dno
comma
op_amp
id|n_dnodes
comma
op_amp
id|n_subdirs
comma
l_int|NULL
)paren
suffix:semicolon
id|i-&gt;i_blocks
op_assign
l_int|4
op_star
id|n_dnodes
suffix:semicolon
id|i-&gt;i_size
op_assign
l_int|2048
op_star
id|n_dnodes
suffix:semicolon
id|i-&gt;i_nlink
op_assign
l_int|2
op_plus
id|n_subdirs
suffix:semicolon
)brace
r_else
(brace
id|i-&gt;i_mode
op_or_assign
id|S_IFREG
suffix:semicolon
r_if
c_cond
(paren
op_logical_neg
id|hpfs_inode-&gt;i_ea_mode
)paren
id|i-&gt;i_mode
op_and_assign
op_complement
l_int|0111
suffix:semicolon
id|i-&gt;i_op
op_assign
op_amp
id|hpfs_file_iops
suffix:semicolon
id|i-&gt;i_fop
op_assign
op_amp
id|hpfs_file_ops
suffix:semicolon
id|i-&gt;i_nlink
op_assign
l_int|1
suffix:semicolon
id|i-&gt;i_size
op_assign
id|fnode-&gt;file_size
suffix:semicolon
id|i-&gt;i_blocks
op_assign
(paren
(paren
id|i-&gt;i_size
op_plus
l_int|511
)paren
op_rshift
l_int|9
)paren
op_plus
l_int|1
suffix:semicolon
id|i-&gt;i_data.a_ops
op_assign
op_amp
id|hpfs_aops
suffix:semicolon
id|hpfs_i
c_func
(paren
id|i
)paren
op_member_access_from_pointer
id|mmu_private
op_assign
id|i-&gt;i_size
suffix:semicolon
)brace
id|brelse
c_func
(paren
id|bh
)paren
suffix:semicolon
)brace
DECL|function|hpfs_write_inode_ea
r_void
id|hpfs_write_inode_ea
c_func
(paren
r_struct
id|inode
op_star
id|i
comma
r_struct
id|fnode
op_star
id|fnode
)paren
(brace
r_struct
id|hpfs_inode_info
op_star
id|hpfs_inode
op_assign
id|hpfs_i
c_func
(paren
id|i
)paren
suffix:semicolon
r_if
c_cond
(paren
id|fnode-&gt;acl_size_l
op_logical_or
id|fnode-&gt;acl_size_s
)paren
(brace
multiline_comment|/* Some unknown structures like ACL may be in fnode,&n;&t;&t;   we&squot;d better not overwrite them */
id|hpfs_error
c_func
(paren
id|i-&gt;i_sb
comma
l_string|&quot;fnode %08x has some unknown HPFS386 stuctures&quot;
comma
id|i-&gt;i_ino
)paren
suffix:semicolon
)brace
r_else
r_if
c_cond
(paren
id|hpfs_sb
c_func
(paren
id|i-&gt;i_sb
)paren
op_member_access_from_pointer
id|sb_eas
op_ge
l_int|2
)paren
(brace
id|u32
id|ea
suffix:semicolon
r_if
c_cond
(paren
(paren
id|i-&gt;i_uid
op_ne
id|hpfs_sb
c_func
(paren
id|i-&gt;i_sb
)paren
op_member_access_from_pointer
id|sb_uid
)paren
op_logical_or
id|hpfs_inode-&gt;i_ea_uid
)paren
(brace
id|ea
op_assign
id|cpu_to_le32
c_func
(paren
id|i-&gt;i_uid
)paren
suffix:semicolon
id|hpfs_set_ea
c_func
(paren
id|i
comma
id|fnode
comma
l_string|&quot;UID&quot;
comma
(paren
r_char
op_star
)paren
op_amp
id|ea
comma
l_int|2
)paren
suffix:semicolon
id|hpfs_inode-&gt;i_ea_uid
op_assign
l_int|1
suffix:semicolon
)brace
r_if
c_cond
(paren
(paren
id|i-&gt;i_gid
op_ne
id|hpfs_sb
c_func
(paren
id|i-&gt;i_sb
)paren
op_member_access_from_pointer
id|sb_gid
)paren
op_logical_or
id|hpfs_inode-&gt;i_ea_gid
)paren
(brace
id|ea
op_assign
id|cpu_to_le32
c_func
(paren
id|i-&gt;i_gid
)paren
suffix:semicolon
id|hpfs_set_ea
c_func
(paren
id|i
comma
id|fnode
comma
l_string|&quot;GID&quot;
comma
(paren
r_char
op_star
)paren
op_amp
id|ea
comma
l_int|2
)paren
suffix:semicolon
id|hpfs_inode-&gt;i_ea_gid
op_assign
l_int|1
suffix:semicolon
)brace
r_if
c_cond
(paren
op_logical_neg
id|S_ISLNK
c_func
(paren
id|i-&gt;i_mode
)paren
)paren
r_if
c_cond
(paren
(paren
id|i-&gt;i_mode
op_ne
(paren
(paren
id|hpfs_sb
c_func
(paren
id|i-&gt;i_sb
)paren
op_member_access_from_pointer
id|sb_mode
op_amp
op_complement
(paren
id|S_ISDIR
c_func
(paren
id|i-&gt;i_mode
)paren
ques
c_cond
l_int|0
suffix:colon
l_int|0111
)paren
)paren
op_or
(paren
id|S_ISDIR
c_func
(paren
id|i-&gt;i_mode
)paren
ques
c_cond
id|S_IFDIR
suffix:colon
id|S_IFREG
)paren
)paren
op_logical_and
id|i-&gt;i_mode
op_ne
(paren
(paren
id|hpfs_sb
c_func
(paren
id|i-&gt;i_sb
)paren
op_member_access_from_pointer
id|sb_mode
op_amp
op_complement
(paren
id|S_ISDIR
c_func
(paren
id|i-&gt;i_mode
)paren
ques
c_cond
l_int|0222
suffix:colon
l_int|0333
)paren
)paren
op_or
(paren
id|S_ISDIR
c_func
(paren
id|i-&gt;i_mode
)paren
ques
c_cond
id|S_IFDIR
suffix:colon
id|S_IFREG
)paren
)paren
)paren
op_logical_or
id|hpfs_inode-&gt;i_ea_mode
)paren
(brace
id|ea
op_assign
id|cpu_to_le32
c_func
(paren
id|i-&gt;i_mode
)paren
suffix:semicolon
id|hpfs_set_ea
c_func
(paren
id|i
comma
id|fnode
comma
l_string|&quot;MODE&quot;
comma
(paren
r_char
op_star
)paren
op_amp
id|ea
comma
l_int|2
)paren
suffix:semicolon
id|hpfs_inode-&gt;i_ea_mode
op_assign
l_int|1
suffix:semicolon
)brace
r_if
c_cond
(paren
id|S_ISBLK
c_func
(paren
id|i-&gt;i_mode
)paren
op_logical_or
id|S_ISCHR
c_func
(paren
id|i-&gt;i_mode
)paren
)paren
(brace
id|ea
op_assign
id|cpu_to_le32
c_func
(paren
id|new_encode_dev
c_func
(paren
id|i-&gt;i_rdev
)paren
)paren
suffix:semicolon
id|hpfs_set_ea
c_func
(paren
id|i
comma
id|fnode
comma
l_string|&quot;DEV&quot;
comma
(paren
r_char
op_star
)paren
op_amp
id|ea
comma
l_int|4
)paren
suffix:semicolon
)brace
)brace
)brace
DECL|function|hpfs_write_inode
r_void
id|hpfs_write_inode
c_func
(paren
r_struct
id|inode
op_star
id|i
)paren
(brace
r_struct
id|hpfs_inode_info
op_star
id|hpfs_inode
op_assign
id|hpfs_i
c_func
(paren
id|i
)paren
suffix:semicolon
r_struct
id|inode
op_star
id|parent
suffix:semicolon
r_if
c_cond
(paren
op_logical_neg
id|i-&gt;i_nlink
)paren
r_return
suffix:semicolon
r_if
c_cond
(paren
id|i-&gt;i_ino
op_eq
id|hpfs_sb
c_func
(paren
id|i-&gt;i_sb
)paren
op_member_access_from_pointer
id|sb_root
)paren
r_return
suffix:semicolon
r_if
c_cond
(paren
id|hpfs_inode-&gt;i_rddir_off
op_logical_and
op_logical_neg
id|atomic_read
c_func
(paren
op_amp
id|i-&gt;i_count
)paren
)paren
(brace
r_if
c_cond
(paren
op_star
id|hpfs_inode-&gt;i_rddir_off
)paren
id|printk
c_func
(paren
l_string|&quot;HPFS: write_inode: some position still there&bslash;n&quot;
)paren
suffix:semicolon
id|kfree
c_func
(paren
id|hpfs_inode-&gt;i_rddir_off
)paren
suffix:semicolon
id|hpfs_inode-&gt;i_rddir_off
op_assign
l_int|NULL
suffix:semicolon
)brace
id|hpfs_lock_iget
c_func
(paren
id|i-&gt;i_sb
comma
l_int|1
)paren
suffix:semicolon
id|parent
op_assign
id|iget
c_func
(paren
id|i-&gt;i_sb
comma
id|hpfs_inode-&gt;i_parent_dir
)paren
suffix:semicolon
r_if
c_cond
(paren
id|parent
)paren
(brace
id|hpfs_unlock_iget
c_func
(paren
id|i-&gt;i_sb
)paren
suffix:semicolon
id|hpfs_inode-&gt;i_dirty
op_assign
l_int|0
suffix:semicolon
id|hpfs_lock_inode
c_func
(paren
id|parent
)paren
suffix:semicolon
id|hpfs_write_inode_nolock
c_func
(paren
id|i
)paren
suffix:semicolon
id|hpfs_unlock_inode
c_func
(paren
id|parent
)paren
suffix:semicolon
id|iput
c_func
(paren
id|parent
)paren
suffix:semicolon
)brace
r_else
(brace
id|hpfs_unlock_iget
c_func
(paren
id|i-&gt;i_sb
)paren
suffix:semicolon
id|mark_inode_dirty
c_func
(paren
id|i
)paren
suffix:semicolon
)brace
)brace
DECL|function|hpfs_write_inode_nolock
r_void
id|hpfs_write_inode_nolock
c_func
(paren
r_struct
id|inode
op_star
id|i
)paren
(brace
r_struct
id|hpfs_inode_info
op_star
id|hpfs_inode
op_assign
id|hpfs_i
c_func
(paren
id|i
)paren
suffix:semicolon
r_struct
id|buffer_head
op_star
id|bh
suffix:semicolon
r_struct
id|fnode
op_star
id|fnode
suffix:semicolon
r_struct
id|quad_buffer_head
id|qbh
suffix:semicolon
r_struct
id|hpfs_dirent
op_star
id|de
suffix:semicolon
r_if
c_cond
(paren
id|i-&gt;i_ino
op_eq
id|hpfs_sb
c_func
(paren
id|i-&gt;i_sb
)paren
op_member_access_from_pointer
id|sb_root
)paren
r_return
suffix:semicolon
r_if
c_cond
(paren
op_logical_neg
(paren
id|fnode
op_assign
id|hpfs_map_fnode
c_func
(paren
id|i-&gt;i_sb
comma
id|i-&gt;i_ino
comma
op_amp
id|bh
)paren
)paren
)paren
r_return
suffix:semicolon
r_if
c_cond
(paren
id|i-&gt;i_ino
op_ne
id|hpfs_sb
c_func
(paren
id|i-&gt;i_sb
)paren
op_member_access_from_pointer
id|sb_root
)paren
(brace
r_if
c_cond
(paren
op_logical_neg
(paren
id|de
op_assign
id|map_fnode_dirent
c_func
(paren
id|i-&gt;i_sb
comma
id|i-&gt;i_ino
comma
id|fnode
comma
op_amp
id|qbh
)paren
)paren
)paren
(brace
id|brelse
c_func
(paren
id|bh
)paren
suffix:semicolon
r_return
suffix:semicolon
)brace
)brace
r_else
id|de
op_assign
l_int|NULL
suffix:semicolon
r_if
c_cond
(paren
id|S_ISREG
c_func
(paren
id|i-&gt;i_mode
)paren
)paren
(brace
id|fnode-&gt;file_size
op_assign
id|de-&gt;file_size
op_assign
id|i-&gt;i_size
suffix:semicolon
)brace
r_else
r_if
c_cond
(paren
id|S_ISDIR
c_func
(paren
id|i-&gt;i_mode
)paren
)paren
(brace
id|fnode-&gt;file_size
op_assign
id|de-&gt;file_size
op_assign
l_int|0
suffix:semicolon
)brace
id|hpfs_write_inode_ea
c_func
(paren
id|i
comma
id|fnode
)paren
suffix:semicolon
r_if
c_cond
(paren
id|de
)paren
(brace
id|de-&gt;write_date
op_assign
id|gmt_to_local
c_func
(paren
id|i-&gt;i_sb
comma
id|i-&gt;i_mtime.tv_sec
)paren
suffix:semicolon
id|de-&gt;read_date
op_assign
id|gmt_to_local
c_func
(paren
id|i-&gt;i_sb
comma
id|i-&gt;i_atime.tv_sec
)paren
suffix:semicolon
id|de-&gt;creation_date
op_assign
id|gmt_to_local
c_func
(paren
id|i-&gt;i_sb
comma
id|i-&gt;i_ctime.tv_sec
)paren
suffix:semicolon
id|de-&gt;read_only
op_assign
op_logical_neg
(paren
id|i-&gt;i_mode
op_amp
l_int|0222
)paren
suffix:semicolon
id|de-&gt;ea_size
op_assign
id|hpfs_inode-&gt;i_ea_size
suffix:semicolon
id|hpfs_mark_4buffers_dirty
c_func
(paren
op_amp
id|qbh
)paren
suffix:semicolon
id|hpfs_brelse4
c_func
(paren
op_amp
id|qbh
)paren
suffix:semicolon
)brace
r_if
c_cond
(paren
id|S_ISDIR
c_func
(paren
id|i-&gt;i_mode
)paren
)paren
(brace
r_if
c_cond
(paren
(paren
id|de
op_assign
id|map_dirent
c_func
(paren
id|i
comma
id|hpfs_inode-&gt;i_dno
comma
l_string|&quot;&bslash;001&bslash;001&quot;
comma
l_int|2
comma
l_int|NULL
comma
op_amp
id|qbh
)paren
)paren
)paren
(brace
id|de-&gt;write_date
op_assign
id|gmt_to_local
c_func
(paren
id|i-&gt;i_sb
comma
id|i-&gt;i_mtime.tv_sec
)paren
suffix:semicolon
id|de-&gt;read_date
op_assign
id|gmt_to_local
c_func
(paren
id|i-&gt;i_sb
comma
id|i-&gt;i_atime.tv_sec
)paren
suffix:semicolon
id|de-&gt;creation_date
op_assign
id|gmt_to_local
c_func
(paren
id|i-&gt;i_sb
comma
id|i-&gt;i_ctime.tv_sec
)paren
suffix:semicolon
id|de-&gt;read_only
op_assign
op_logical_neg
(paren
id|i-&gt;i_mode
op_amp
l_int|0222
)paren
suffix:semicolon
id|de-&gt;ea_size
op_assign
multiline_comment|/*hpfs_inode-&gt;i_ea_size*/
l_int|0
suffix:semicolon
id|de-&gt;file_size
op_assign
l_int|0
suffix:semicolon
id|hpfs_mark_4buffers_dirty
c_func
(paren
op_amp
id|qbh
)paren
suffix:semicolon
id|hpfs_brelse4
c_func
(paren
op_amp
id|qbh
)paren
suffix:semicolon
)brace
r_else
id|hpfs_error
c_func
(paren
id|i-&gt;i_sb
comma
l_string|&quot;directory %08x doesn&squot;t have &squot;.&squot; entry&quot;
comma
id|i-&gt;i_ino
)paren
suffix:semicolon
)brace
id|mark_buffer_dirty
c_func
(paren
id|bh
)paren
suffix:semicolon
id|brelse
c_func
(paren
id|bh
)paren
suffix:semicolon
)brace
DECL|function|hpfs_notify_change
r_int
id|hpfs_notify_change
c_func
(paren
r_struct
id|dentry
op_star
id|dentry
comma
r_struct
id|iattr
op_star
id|attr
)paren
(brace
r_struct
id|inode
op_star
id|inode
op_assign
id|dentry-&gt;d_inode
suffix:semicolon
r_int
id|error
op_assign
l_int|0
suffix:semicolon
id|lock_kernel
c_func
(paren
)paren
suffix:semicolon
r_if
c_cond
(paren
(paren
(paren
id|attr-&gt;ia_valid
op_amp
id|ATTR_SIZE
)paren
op_logical_and
id|attr-&gt;ia_size
OG
id|inode-&gt;i_size
)paren
op_logical_or
(paren
id|hpfs_sb
c_func
(paren
id|inode-&gt;i_sb
)paren
op_member_access_from_pointer
id|sb_root
op_eq
id|inode-&gt;i_ino
)paren
)paren
(brace
id|error
op_assign
op_minus
id|EINVAL
suffix:semicolon
)brace
r_else
r_if
c_cond
(paren
(paren
id|error
op_assign
id|inode_change_ok
c_func
(paren
id|inode
comma
id|attr
)paren
)paren
)paren
(brace
)brace
r_else
r_if
c_cond
(paren
(paren
id|error
op_assign
id|inode_setattr
c_func
(paren
id|inode
comma
id|attr
)paren
)paren
)paren
(brace
)brace
r_else
(brace
id|hpfs_write_inode
c_func
(paren
id|inode
)paren
suffix:semicolon
)brace
id|unlock_kernel
c_func
(paren
)paren
suffix:semicolon
r_return
id|error
suffix:semicolon
)brace
DECL|function|hpfs_write_if_changed
r_void
id|hpfs_write_if_changed
c_func
(paren
r_struct
id|inode
op_star
id|inode
)paren
(brace
r_struct
id|hpfs_inode_info
op_star
id|hpfs_inode
op_assign
id|hpfs_i
c_func
(paren
id|inode
)paren
suffix:semicolon
r_if
c_cond
(paren
id|hpfs_inode-&gt;i_dirty
)paren
id|hpfs_write_inode
c_func
(paren
id|inode
)paren
suffix:semicolon
)brace
DECL|function|hpfs_delete_inode
r_void
id|hpfs_delete_inode
c_func
(paren
r_struct
id|inode
op_star
id|inode
)paren
(brace
id|lock_kernel
c_func
(paren
)paren
suffix:semicolon
id|hpfs_remove_fnode
c_func
(paren
id|inode-&gt;i_sb
comma
id|inode-&gt;i_ino
)paren
suffix:semicolon
id|unlock_kernel
c_func
(paren
)paren
suffix:semicolon
id|clear_inode
c_func
(paren
id|inode
)paren
suffix:semicolon
)brace
eof
