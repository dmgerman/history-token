...
multiline_comment|/**&n; *&t;remove_super&t;-&t;makes superblock unreachable&n; *&t;@s:&t;superblock in question&n; *&n; *&t;Removes superblock from the lists, and unlocks it.  @s should have&n; *&t;no active references by that time and after remove_super() it&squot;s&n; *&t;essentially in rundown mode - all remaining references are temporary,&n; *&t;no new references of any sort are going to appear and all holders&n; *&t;of temporary ones will eventually drop them.  At that point superblock&n; *&t;itself will be destroyed; all its contents is already gone.&n; */
