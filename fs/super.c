...
suffix:semicolon
multiline_comment|/* Yes, it sucks. As soon as we get refcounting... */
multiline_comment|/* Almost there - next two lines will go away RSN */
id|lock_super
c_func
(paren
id|s
...
(brace
multiline_comment|/*&n; * We are heavily relying on mount_sem here. We _will_ get rid of that&n; * ugliness RSN (and then atomicity of -&gt;s_active will play), but first&n; * we need to get rid of &quot;reuse&quot; branch of get_empty_super() and that&n; * requires reference counters. Chicken and egg problem, but fortunately&n; * we can use the fact that right now all accesses to -&gt;s_active are&n; * under mount_sem.&n; */
r_if
c_cond
(paren
id|atomic_read
