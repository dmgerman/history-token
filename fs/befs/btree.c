...
multiline_comment|/* Note:&n; * &n; * The book states 2 confusing things about befs b+trees. First, &n; * it states that the overflow feild of node headers is used by internal nodes &n; * to point to another node that &quot;effectivly continues this one&quot;. Here is what &n; * I belive that means. Each key in internal nodes points to another node that&n; * contains key values less than itself. Inspection reveals that the last key &n; * in the internal node is not the last key in the index. Keys that are &n; * greater than the last key in the internal node go into the overflow node. &n; * I imagine there is a performance reason for this.&n; *&n; * Second, it states that the header of a btree node is sufficient to &n; * distinguish internal nodes from leaf nodes. Without saying exactly how. &n; * After figuring out the first, it becomes obvious that internal nodes have&n; * overflow nodes and leafnodes do not.&n; */
...
suffix:semicolon
multiline_comment|/**&n; * befs_bt_read_super - read in btree superblock convert to cpu byteorder&n; * @sb: Filesystem superblock&n; * @ds: Datastream to read from&n; * @sup: Buffer in which to place the btree superblock&n; *&n; * Calls befs_read_datastream to read in the btree superblock and&n; * makes sure it is in cpu byteorder, byteswapping if nessisary.&n; *&n; * On success, returns BEFS_OK and *@sup contains the btree superblock,&n; * in cpu byte order.&n; *&n; * On failure, BEFS_ERR is returned.&n; */
r_static
r_int
...
multiline_comment|/**&n; * befs_bt_read_node - read in btree node and convert to cpu byteorder&n; * @sb: Filesystem superblock&n; * @ds: Datastream to read from&n; * @node: Buffer in which to place the btree node&n; * @node_off: Starting offset (in bytes) of the node in @ds&n; *&n; * Calls befs_read_datastream to read in the indicated btree node and&n; * makes sure its header feilds are in cpu byteorder, byteswapping if &n; * nessisary.&n; * Note: node-&gt;bh must be NULL when this function called first&n; * time. Don&squot;t forget brelse(node-&gt;bh) after last call.&n; *&n; * On success, returns BEFS_OK and *@node contains the btree node that&n; * starts at @node_off, with the node-&gt;head fields in cpu byte order.&n; *&n; * On failure, BEFS_ERR is returned.&n; */
r_static
r_int
