...
multiline_comment|/*&n; * An execve() will automatically &quot;de-thread&quot; the process.&n; * - if a master thread (PID==TGID) is doing this, then all subsidiary threads&n; *   will be killed (otherwise there will end up being two independent thread&n; *   groups with the same TGID).&n; * - if a subsidary thread is doing this, then it just leaves the thread group&n; */
...
(brace
multiline_comment|/* subsidiary thread - just escapes the group */
id|list_del_init
c_func
(paren
op_amp
...
r_else
(brace
multiline_comment|/* master thread - kill all subsidiary threads */
id|info.si_signo
op_assign
id|SIGKILL
suffix:semicolon
...
suffix:semicolon
multiline_comment|/* give the subsidiary threads a chance to clean themselves up */
r_if
c_cond
(paren
id|pause
