...
multiline_comment|/*&n; * Got race?&n; * We will need to arrange for the VFS layer to provide an atomic open.&n; * Until then, this create/open method is prone to inefficiency and race&n; * conditions due to the lookup, create, and open VFS calls from sys_open()&n; * placed on the wire.&n; *&n; * Given the above sorry state of affairs, I&squot;m simply sending an OPEN, a&n; * possible SETATTR, and then a CLOSE &n; * The file will be opened again in the subsequent VFS open call&n; * (nfs4_proc_file_open).&n; *&n; * The open for read will just hang around to be used by any process that&n; * opens the file O_RDONLY. This will all be resolved with the VFS changes.&n; */
r_static
r_int
