...
suffix:semicolon
multiline_comment|/* If we were signalled we still need to ensure that&n;&t; * we clean up any state on the server. We therefore&n;&t; * record the lock call as having succeeded in order to&n;&t; * ensure that locks_remove_posix() cleans it out when&n;&t; * the process exits.&n;&t; */
r_if
c_cond
(paren
id|status
