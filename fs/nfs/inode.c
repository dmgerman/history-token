...
multiline_comment|/*&n; *  Right now we are using get_sb_nodev, but we ought to switch to&n; *  get_anon_super() with appropriate comparison function.  The only&n; *  question being, when two NFS mounts are the same?  Identical IP&n; *  of server + identical root fhandle?  Trond?&n; */
...
(brace
multiline_comment|/* the latter will be needed */
id|kfree
c_func
(paren
id|server
