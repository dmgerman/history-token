...
multiline_comment|/* Symlink caching in the page cache is even more simplistic&n; * and straight-forward than readdir caching.&n; *&n; * We place the length at the beginning of the page, in host byte order,&n; * followed by the string.  The XDR response verification will NUL-terminate&n; * it.  In the very end of page we store pointer to struct page in question,&n; * simplifying nfs_put_link() (if inode got invalidated we can&squot;t find the page&n; * to be freed via pagecache lookup).&n; */
...
suffix:semicolon
multiline_comment|/* this must be page-sized */
