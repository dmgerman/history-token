multiline_comment|/*&n; * JFFS -- Journaling Flash File System, Linux implementation.&n; *&n; * Copyright (C) 1999, 2000  Axis Communications, Inc.&n; *&n; * Created by Finn Hakansson &lt;finn@axis.com&gt;.&n; *&n; * This is free software; you can redistribute it and/or modify it&n; * under the terms of the GNU General Public License as published by&n; * the Free Software Foundation; either version 2 of the License, or&n; * (at your option) any later version.&n; *&n; * $Id: intrep.c,v 1.102 2001/09/23 23:28:36 dwmw2 Exp $&n; *&n; * Ported to Linux 2.3.x and MTD:&n; * Copyright (C) 2000  Alexander Larsson (alex@cendio.se), Cendio Systems AB&n; *&n; */
...
(paren
id|KERN_NOTICE
l_string|&quot;flash_safe_write(%p, %08x, %p, %08x)&bslash;n&quot;
comma
id|mtd
comma
(paren
...
r_static
r_int
DECL|function|flash_safe_writev
id|flash_safe_writev
c_func
(paren
r_struct
id|mtd_info
...
(paren
id|KERN_NOTICE
l_string|&quot;flash_safe_writev(%p, %08x, %p)&bslash;n&quot;
comma
id|mtd
comma
(paren
...
multiline_comment|/* Not implemented writev. Repeatedly use write - on the not so&n;&t;   unreasonable assumption that the mtd driver doesn&squot;t care how&n;&t;   many write cycles we use. */
id|res
op_assign
l_int|0
suffix:semicolon
...
c_func
(paren
l_string|&quot;Didn&squot;t write all bytes in flash_safe_writev(). Returned %d&bslash;n&quot;
comma
id|res
)paren
suffix:semicolon
...
multiline_comment|/* If res is non-zero, retlen_a is undefined, but we don&squot;t&n;&t;&t;   care because in that case it&squot;s not going to be &n;&t;&t;   returned anyway.&n;&t;&t;*/
id|to
op_add_assign
id|retlen_a
suffix:semicolon
...
multiline_comment|/* write as many 64-byte chunks as we can */
r_while
c_loop
(paren
id|size
...
(paren
id|KERN_NOTICE
l_string|&quot;kmalloc failed in jffs_checksum_flash()&bslash;n&quot;
)paren
suffix:semicolon
r_return
op_minus
...
(brace
multiline_comment|/* scan_flash() wants us to try once more. A flipping &n;&t;&t;&t;   bits sector was detect in the middle of the scan flash.&n;&t;&t;&t;   Clean up old allocated memory before going in.&n;&t;&t;&t;*/
id|D1
c_func
(paren
id|printk
c_func
(paren
l_string|&quot;jffs_build_fs: Cleaning up all control structures,&quot;
l_string|&quot; reallocating them and trying mount again.&bslash;n&quot;
)paren
)paren
suffix:semicolon
id|jffs_cleanup_control
...
(brace
DECL|macro|NUM_REREADS
mdefine_line|#define NUM_REREADS             4 /* see note above */
DECL|macro|READ_AHEAD_BYTES
mdefine_line|#define READ_AHEAD_BYTES        4096 /* must be a multiple of 4, &n;&t;&t;&t;&t;&t;usually set to kernel page size */
id|__u8
op_star
id|read_buf1
suffix:semicolon
...
id|fmc-&gt;flash_size
suffix:semicolon
multiline_comment|/* Allocate read buffers */
id|read_buf1
op_assign
(paren
id|__u8
...
c_func
(paren
l_string|&quot;check_partly_erased_sector():checking sector which contains&quot;
l_string|&quot; offset 0x%x for flipping bits..&bslash;n&quot;
comma
(paren
id|__u32
)paren
...
(brace
multiline_comment|/* buffers MUST match, double word for word! */
r_if
c_cond
(paren
op_star
...
(brace
multiline_comment|/* flipping bits detected, time to erase sector */
multiline_comment|/* This will help us log some statistics etc. */
id|D1
c_func
(paren
id|printk
c_func
(paren
l_string|&quot;Flipping bits detected in re-read round:%i of %i&bslash;n&quot;
comma
id|cnt
comma
id|NUM_REREADS
...
c_func
(paren
l_string|&quot;check_partly_erased_sectors:flipping bits detected&quot;
l_string|&quot; @offset:0x%x(0x%x!=0x%x)&bslash;n&quot;
comma
(paren
id|__u32
)paren
...
suffix:semicolon
multiline_comment|/* calculate start of present sector */
id|offset
op_assign
(paren
(paren
...
c_func
(paren
l_string|&quot;check_partly_erased_sector():erasing sector starting 0x%x.&bslash;n&quot;
comma
id|offset
)paren
)paren
...
l_string|&quot;offset = %u, erase_size = %d&bslash;n&quot;
comma
id|offset
comma
id|fmc-&gt;sector_size
...
c_func
(paren
l_string|&quot;JFFS: Erase of flash sector @0x%x successful.&bslash;n&quot;
comma
id|offset
)paren
)paren
suffix:semicolon
multiline_comment|/* skip ahead to the next sector */
id|pos
op_assign
(paren
(paren
...
c_func
(paren
l_string|&quot;check_partly_erased_sector():Done checking all sectors till offset 0x%x for flipping bits.&bslash;n&quot;
comma
(paren
id|__u32
)paren
...
multiline_comment|/* end check_partly_erased_sectors() */
...
id|free_chunk_size2
suffix:semicolon
DECL|macro|NUMFREEALLOWED
mdefine_line|#define NUMFREEALLOWED     2        /* 2 chunks of at least erase size space allowed */
r_int
id|num_free_space
op_assign
l_int|0
suffix:semicolon
multiline_comment|/* Flag err if more than TWO&n;&t;&t;&t;&t;       free blocks found. This is NOT allowed&n;&t;&t;&t;&t;       by the current jffs design.&n;&t;&t;&t;&t;    */
r_int
id|num_free_spc_not_accp
op_assign
l_int|0
suffix:semicolon
multiline_comment|/* For debugging purposed keep count &n;&t;&t;&t;&t;&t;of how much free space was rejected and&n;&t;&t;&t;&t;&t;marked dirty&n;&t;&t;&t;&t;     */
id|D1
c_func
(paren
id|printk
...
suffix:semicolon
multiline_comment|/*&n;&t;  check and make sure that any sector does not suffer&n;&t;  from the &quot;partly erased, bit flipping syndrome&quot; (TM Vipin :)&n;&t;  If so, offending sectors will be erased.&n;&t;*/
r_if
c_cond
(paren
id|check_partly_erased_sectors
...
id|EIO
suffix:semicolon
multiline_comment|/* bad, bad, bad error. Cannot continue.*/
)brace
multiline_comment|/* Allocate read buffer */
id|read_buf
op_assign
...
suffix:colon
multiline_comment|/* We have found 0xffffffff at this position.  We have to&n;&t;&t;&t;   scan the rest of the flash till the end or till&n;&t;&t;&t;   something else than 0xffffffff is found.&n;&t;&t;           Keep going till we do not find JFFS_EMPTY_BITMASK &n;&t;&t;&t;   anymore */
id|D1
c_func
(paren
id|printk
c_func
(paren
l_string|&quot;jffs_scan_flash(): 0xffffffff at pos 0x%lx.&bslash;n&quot;
comma
(paren
r_int
)paren
...
c_func
(paren
l_string|&quot;jffs_scan_flash():0xffffffff ended at pos 0x%lx.&bslash;n&quot;
comma
(paren
r_int
)paren
...
suffix:semicolon
multiline_comment|/* end of last sector */
r_if
c_cond
(paren
id|start
...
(brace
multiline_comment|/* free space started in the previous sector! */
r_if
c_cond
(paren
(paren
...
(brace
multiline_comment|/*&n;&t;&t;&t;&t;&t;&t;  Count it in if we are still under NUMFREEALLOWED *and* it is &n;&t;&t;&t;&t;&t;&t;  at least 1 erase sector in length. This will keep us from &n;&t;&t;&t;&t;&t;&t;  picking any little ole&squot; space as &quot;free&quot;.&n;&t;&t;&t;&t;&t;&t;*/
id|D1
c_func
(paren
id|printk
c_func
(paren
l_string|&quot;Reducing end of free space to 0x%x from 0x%x&bslash;n&quot;
comma
(paren
r_int
r_int
...
c_func
(paren
l_string|&quot;Free space accepted: Starting 0x%x for 0x%x bytes&bslash;n&quot;
comma
(paren
r_int
r_int
...
suffix:semicolon
multiline_comment|/* below, space from &quot;start&quot; to &quot;pos&quot; will be marked dirty. */
id|start
op_assign
id|test_start
suffix:semicolon
multiline_comment|/* Being in here means that we have found at least an entire &n;&t;&t;&t;&t;&t;&t;   erase sector size of free space ending on a sector boundary.&n;&t;&t;&t;&t;&t;&t;   Keep track of free spaces accepted.&n;&t;&t;&t;&t;&t;&t;*/
id|num_free_space
op_increment
suffix:semicolon
)brace
...
c_func
(paren
l_string|&quot;Free space (#%i) found but *Not* accepted: Starting&quot;
l_string|&quot; 0x%x for 0x%x bytes&bslash;n&quot;
comma
id|num_free_spc_not_accp
comma
(paren
...
c_func
(paren
l_string|&quot;Dirty space: Starting 0x%x for 0x%x bytes&bslash;n&quot;
comma
(paren
r_int
r_int
...
r_else
(brace
multiline_comment|/* &quot;Flipping bits&quot; detected. This means that our scan for them&n;&t;&t;&t;&t;&t;   did not catch this offset. See check_partly_erased_sectors() for&n;&t;&t;&t;&t;&t;   more info.&n;&t;&t;&t;&t;&t;*/
id|D1
c_func
(paren
id|printk
c_func
(paren
l_string|&quot;jffs_scan_flash():wants to allocate dirty flash &quot;
l_string|&quot;space for 0 bytes.&bslash;n&quot;
)paren
)paren
suffix:semicolon
id|D1
...
c_func
(paren
l_string|&quot;jffs_scan_flash(): Flipping bits! We will free &quot;
l_string|&quot;all allocated memory, erase this sector and remount&bslash;n&quot;
)paren
)paren
suffix:semicolon
multiline_comment|/* calculate start of present sector */
...
c_func
(paren
l_string|&quot;jffs_scan_flash():erasing sector starting 0x%x.&bslash;n&quot;
comma
id|offset
)paren
)paren
...
id|EAGAIN
suffix:semicolon
multiline_comment|/* erased offending sector. Try mount one more time please. */
)brace
)brace
r_else
(brace
multiline_comment|/* Being in here means that we have found free space that ends on an erase sector&n;&t;&t;&t;&t;   boundary.&n;&t;&t;&t;&t;   Count it in if we are still under NUMFREEALLOWED *and* it is at least 1 erase &n;&t;&t;&t;&t;   sector in length. This will keep us from picking any little ole&squot; space as &quot;free&quot;.&n;&t;&t;&t;&t; */
r_if
c_cond
(paren
(paren
...
(brace
multiline_comment|/* We really don&squot;t do anything to mark space as free, except *not* &n;&t;&t;&t;&t;&t;      mark it dirty and just advance the &quot;pos&quot; location pointer. &n;&t;&t;&t;&t;&t;      It will automatically be picked up as free space.&n;&t;&t;&t;&t;&t;    */
id|num_free_space
op_increment
suffix:semicolon
id|D1
...
suffix:semicolon
multiline_comment|/* Mark this space as dirty. We already have our free space. */
id|D1
c_func
(paren
id|printk
...
(paren
id|KERN_WARNING
l_string|&quot;jffs_scan_flash(): Found free space &quot;
l_string|&quot;number %i. Only %i free space is allowed.&bslash;n&quot;
comma
id|num_free_space
comma
id|NUMFREEALLOWED
...
suffix:colon
multiline_comment|/* We have found 0x00000000 at this position.  Scan as far&n;&t;&t;&t;   as possible to find out how much is dirty.  */
id|D1
c_func
(paren
id|printk
c_func
(paren
l_string|&quot;jffs_scan_flash(): 0x00000000 at pos 0x%lx.&bslash;n&quot;
comma
(paren
r_int
)paren
...
id|JFFS_EMPTY_BITMASK
suffix:colon
multiline_comment|/* handle these in the main switch() loop */
r_goto
id|cont_scan
suffix:semicolon
r_default
...
id|cont_scan
suffix:colon
multiline_comment|/* First, mark as dirty the region&n;&t;&t;&t;   which really does contain crap. */
id|jffs_fmalloced
c_func
(paren
id|fmc
...
c_func
(paren
l_string|&quot;jffs_checksum_flash() failed to calculate a checksum&bslash;n&quot;
)paren
suffix:semicolon
id|jffs_fmalloced
c_func
...
(paren
id|KERN_WARNING
l_string|&quot;jffs_scan_flash(): Did not find even a single &quot;
l_string|&quot;chunk of free space. This is BAD!&bslash;n&quot;
)paren
suffix:semicolon
)brace
...
suffix:semicolon
multiline_comment|/* This is to trap the &quot;free size accounting screwed error. */
id|free_chunk_size1
op_assign
id|jffs_free_size1
c_func
...
(paren
id|KERN_WARNING
l_string|&quot;jffs_scan_falsh():Free size accounting screwed&bslash;n&quot;
)paren
suffix:semicolon
id|printk
c_func
(paren
id|KERN_WARNING
l_string|&quot;jfffs_scan_flash():free_chunk_size1 == 0x%x, &quot;
l_string|&quot;free_chunk_size2 == 0x%x, fmc-&gt;free_size == 0x%x&bslash;n&quot;
comma
id|free_chunk_size1
comma
id|free_chunk_size2
...
l_int|1
suffix:semicolon
multiline_comment|/* Do NOT mount f/s so that we can inspect what happened.&n;&t;&t;&t;      Mounting this  screwed up f/s will screw us up anyway.&n;&t;&t;&t;    */
)brace
r_return
l_int|0
suffix:semicolon
multiline_comment|/* as far as we are concerned, we are happy! */
)brace
...
suffix:semicolon
multiline_comment|/* if the file was deleted, set the deleted bit in the raw inode */
r_if
c_cond
(paren
id|f-&gt;deleted
...
suffix:semicolon
multiline_comment|/* The actual raw JFFS node */
id|node_iovec
(braket
l_int|0
)braket
...
l_int|1
suffix:semicolon
multiline_comment|/* Get name and size if there is one */
r_if
c_cond
(paren
id|raw_inode-&gt;nsize
...
suffix:semicolon
multiline_comment|/* Add some extra padding if necessary */
id|node_iovec
(braket
id|iovec_cnt
)braket
...
multiline_comment|/* Get data and size if there is any */
r_if
c_cond
(paren
id|raw_inode-&gt;dsize
...
op_increment
suffix:semicolon
multiline_comment|/* No need to pad this because we&squot;re not actually putting&n;&t;&t;   anything after it.&n;&t;&t;*/
)brace
r_if
c_cond
(paren
...
l_string|&quot;requested %i, wrote %i&bslash;n&quot;
comma
id|total_size
comma
id|err
...
(paren
id|KERN_ERR
l_string|&quot;JFFS: jffs_rewrite_data: Now retrying &quot;
l_string|&quot;rewrite. (raw inode)&bslash;n&quot;
)paren
suffix:semicolon
r_goto
...
id|KERN_ERR
l_string|&quot;JFFS: jffs_rewrite_data: Now retrying &quot;
l_string|&quot;rewrite. (name)&bslash;n&quot;
)paren
suffix:semicolon
r_goto
id|retry
...
(paren
id|KERN_NOTICE
l_string|&quot;thread_should_wake(): Not waking. Insufficient dirty space&bslash;n&quot;
)paren
)paren
suffix:semicolon
r_return
...
macro_line|#if 1
multiline_comment|/* If there is too much RAM used by the various structures, GC */
r_if
c_cond
(paren
...
(brace
multiline_comment|/* FIXME: Provide proof that this test can be satisfied. We&n;&t;&t;   don&squot;t want a filesystem doing endless GC just because this&n;&t;&t;   condition cannot ever be false.&n;&t;&t;*/
id|D2
c_func
(paren
id|printk
...
(paren
id|KERN_NOTICE
l_string|&quot;thread_should_wake(): Waking due to number of nodes&bslash;n&quot;
)paren
)paren
suffix:semicolon
r_return
...
(paren
id|KERN_NOTICE
l_string|&quot;thread_should_wake(): Waking due to insufficent free space&bslash;n&quot;
)paren
)paren
suffix:semicolon
r_return
...
(paren
id|KERN_NOTICE
l_string|&quot;thread_should_wake(): Waking due to excessive dirty space&bslash;n&quot;
)paren
)paren
suffix:semicolon
r_return
