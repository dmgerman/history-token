...
multiline_comment|/* people who call journal_begin with a page locked must call this&n;** BEFORE calling journal_begin&n;*/
DECL|function|prevent_flush_page_lock
r_static
r_int
...
id|inode-&gt;i_sb
suffix:semicolon
multiline_comment|/* we don&squot;t care if the inode has a stale pointer from an old&n;  ** transaction&n;  */
r_if
c_cond
(paren
op_logical_neg
...
multiline_comment|/* this last part is really important.  The address space operations have&n;  ** the page locked before they call the journal functions.  So it is possible&n;  ** for one process to be waiting in flush_pages_before_commit for a &n;  ** page, then for the process with the page locked to call journal_begin.&n;  **&n;  ** We&squot;ll deadlock because the process flushing pages will never notice&n;  ** the process with the page locked has called prevent_flush_page_lock.&n;  ** So, we wake up the page waiters, even though the page is still locked.&n;  ** The process waiting in flush_pages_before_commit must check the&n;  ** pl-&gt;do_not_lock flag, and stop trying to lock the page.&n;  */
id|wake_up
c_func
(paren
op_amp
...
multiline_comment|/* people who call journal_end with a page locked must call this&n;** AFTER calling journal_end&n;*/
DECL|function|allow_flush_page_lock
r_static
r_int
id|allow_flush_page_lock
c_func
(paren
r_struct
id|page
...
suffix:semicolon
singleline_comment|// FIXME: b_blocknr == 0 here. but b_data contains correct data
singleline_comment|// from tail. ll_rw_block will skip uptodate buffers
id|bh_result-&gt;b_blocknr
op_assign
l_int|0
suffix:semicolon
...
id|reiserfs_warning
(paren
l_string|&quot;vs-13042: reiserfs_read_inode2: %K not found&bslash;n&quot;
comma
op_amp
id|key
)paren
...
id|inode
suffix:semicolon
singleline_comment|//    if (comp_short_keys (INODE_PKEY (inode), key)) {
r_if
c_cond
(paren
id|is_bad_inode
...
id|reiserfs_warning
(paren
l_string|&quot;vs-13048: reiserfs_iget: &quot;
l_string|&quot;bad_inode. Stat data of (%lu %lu) not found&bslash;n&quot;
comma
id|key-&gt;on_disk_key.k_dir_id
comma
id|key-&gt;on_disk_key.k_objectid
...
multiline_comment|/* not a perfect generation count, as object ids can be reused, but this&n;    ** is as good as reiserfs can do right now&n;    */
id|inode-&gt;i_generation
op_assign
id|INODE_PKEY
(paren
...
suffix:semicolon
multiline_comment|/* async end_io handler decs this */
