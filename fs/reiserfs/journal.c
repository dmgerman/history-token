...
DECL|macro|WRITERS_QUEUED
mdefine_line|#define WRITERS_QUEUED 2       /* set when log is full due to too many&n;&t;&t;&t;&t;* writers&n;&t;&t;&t;&t;*/
r_static
r_int
id|do_journal_end
c_func
...
multiline_comment|/* wait on everything written so far before writing the commit */
r_for
c_loop
(paren
id|i
...
id|jl
suffix:semicolon
multiline_comment|/* flush for 256 transactions or 256 blocks, whichever comes first */
r_for
c_loop
(paren
id|i
...
suffix:semicolon
multiline_comment|/* we&squot;re only being called from kreiserfsd, it makes no sense to do&n;&t;** an async commit so that kreiserfsd can do it later&n;&t;*/
id|do_journal_end
c_func
(paren
op_amp
...
suffix:semicolon
multiline_comment|/* honor the flush wishes from the caller, simple commits can&n;  ** be done outside the journal lock, they are done below&n;  */
r_if
c_cond
(paren
id|flush
