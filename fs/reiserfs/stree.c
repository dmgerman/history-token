...
DECL|macro|SEARCH_BY_KEY_READA
mdefine_line|#define SEARCH_BY_KEY_READA 16
...
(brace
multiline_comment|/*&n;&t; * note, this needs attention if we are getting rid of the BKL&n;&t; * you have to make sure the prepared bit isn&squot;t set on this buffer&n;&t; */
r_if
c_cond
(paren
op_logical_neg
...
suffix:semicolon
multiline_comment|/* if we are going to read leaf nodes, try for read ahead as well */
r_if
c_cond
(paren
(paren
id|p_s_search_path-&gt;reada
op_amp
id|PATH_READA
)paren
op_logical_and
...
op_increment
suffix:semicolon
multiline_comment|/*&n;&t;&t; * check to make sure we&squot;re in the same object&n;&t;&t; */
id|le_key
op_assign
id|B_N_PDELIM_KEY
c_func
...
c_func
(paren
id|le_key-&gt;k_objectid
)paren
op_ne
id|p_s_key-&gt;on_disk_key.k_objectid
)paren
(brace
r_break
suffix:semicolon
...
multiline_comment|/* Calculate number of bytes which will be deleted or cut during balance */
...
id|p_s_inode
comma
multiline_comment|/* inode is here just to update i_blocks and quotas */
r_struct
id|buffer_head
op_star
id|p_s_un_bh
...
suffix:semicolon
multiline_comment|/* hack so the quota code doesn&squot;t have to guess if the file&n;    ** has a tail.  On tail insert, we allocate quota for 1 unformatted node.&n;    ** We test the offset because the tail might have been&n;    ** split into multiple items, and we only want to decrement for&n;    ** the unfm node once&n;    */
r_if
c_cond
(paren
op_logical_neg
...
id|bh-&gt;b_this_page
suffix:semicolon
multiline_comment|/* we want to unmap the buffers that contain the tail, and&n;&t;&t;** all the buffers after it (since the tail must be at the&n;&t;&t;** end of the file).  We don&squot;t want to unmap file data&n;&t;&t;** before the tail, since it might be dirty and waiting to&n;&t;&t;** reach disk&n;&t;&t;*/
id|cur_index
op_add_assign
id|bh-&gt;b_size
suffix:semicolon
...
id|retval2
suffix:semicolon
multiline_comment|/* For direct items, we only change the quota when deleting the last&n;    ** item.&n;    */
id|p_le_ih
op_assign
id|PATH_PITEM_HEAD
(paren
...
(brace
multiline_comment|/* we&squot;ve done an indirect-&gt;direct conversion.  when the data block&n;&t;** was freed, it was removed from the list of blocks that must&n;&t;** be flushed before the transaction commits, make sure to&n;&t;** unmap and invalidate it&n;&t;*/
id|unmap_buffers
c_func
(paren
id|page
...
multiline_comment|/*&n;     * are we doing a full truncate or delete, if so&n;     * kick in the reada code&n;     */
r_if
c_cond
(paren
id|n_new_file_size
...
id|inode
comma
multiline_comment|/* Inode item belongs to */
r_const
r_char
op_star
id|p_c_body
...
suffix:semicolon
macro_line|#endif
multiline_comment|/* DQUOT_* can schedule, must check before the fix_nodes */
r_if
c_cond
(paren
id|fs_changed
...
(brace
multiline_comment|/* Do we count quotas for item? */
id|fs_gen
op_assign
id|get_generation
c_func
...
suffix:semicolon
multiline_comment|/* hack so the quota code doesn&squot;t have to guess if the file has&n;&t; ** a tail, links are always tails, so there&squot;s no guessing needed&n;&t; */
r_if
c_cond
(paren
op_logical_neg
...
suffix:semicolon
macro_line|#endif
multiline_comment|/* We can&squot;t dirty inode here. It would be immediately written but&n;&t; * appropriate stat item isn&squot;t inserted yet... */
r_if
c_cond
(paren
id|DQUOT_ALLOC_SPACE_NODIRTY
...
suffix:semicolon
macro_line|#endif
multiline_comment|/* DQUOT_* can schedule, must check to be sure calling fix_nodes is safe */
r_if
c_cond
(paren
id|inode
