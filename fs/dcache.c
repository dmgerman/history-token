...
suffix:semicolon
DECL|variable|max_dentries
r_static
r_int
id|max_dentries
suffix:semicolon
DECL|variable|hashtable_end
r_static
r_void
op_star
id|hashtable_end
suffix:semicolon
DECL|function|is_bucket
r_static
r_inline
r_int
id|is_bucket
...
multiline_comment|/**&n; * d_lookup - search for a dentry&n; * @parent: parent dentry&n; * @name: qstr of name we wish to find&n; *&n; * Searches the children of the parent dentry for the name in question. If&n; * the dentry is found its reference count is incremented and the dentry&n; * is returned. The caller must use d_put to free the entry when it has&n; * finished using it. %NULL is returned on failure.&n; *&n; * d_lookup is now, dcache_lock free. The hash list is protected using RCU.&n; * Memory barriers are used while updating and doing lockless traversal. &n; * To avoid races with d_move while rename is happening, d_move_count is &n; * used. &n; *&n; * Overflows in memcmp(), while d_move, are avoided by keeping the length&n; * and name pointer in one structure pointed by d_qstr.&n; *&n; * rcu_read_lock() and rcu_read_unlock() are used to disable preemption while&n; * lookup is going on.&n; *&n; * d_lru list is not updated, which can leave non-zero d_count dentries&n; * around in d_lru list.&n; */
...
suffix:semicolon
multiline_comment|/* lookup is terminated when flow reaches any bucket head */
r_for
c_loop
(paren
id|tmp
...
r_break
suffix:semicolon
multiline_comment|/* to avoid race if dentry keep coming back to original&n;&t;&t; * bucket due to double moves&n;&t;&t; */
r_if
c_cond
(paren
id|unlikely
...
r_break
suffix:semicolon
multiline_comment|/*&n;&t;&t; * We must take a snapshot of d_move_count followed by&n;&t;&t; * read memory barrier before any search key comparison &n;&t;&t; */
id|move_count
op_assign
id|dentry-&gt;d_move_count
suffix:semicolon
...
suffix:semicolon
multiline_comment|/*&n;&t;&t; * If dentry is moved, fail the lookup&n;&t;&t; */
r_if
c_cond
(paren
id|unlikely
...
suffix:semicolon
multiline_comment|/* approximate maximum number of dentries in one hash bucket */
id|max_dentries
op_assign
(paren
id|mempages
