...
multiline_comment|/*&n; * Mark the locks for reclaiming.&n; * FIXME: In 2.5 we don&squot;t want to iterate through any global file_lock_list.&n; *        Maintain NLM lock reclaiming lists in the nlm_host instead.&n; */
r_static
DECL|function|nlmclnt_mark_reclaim
r_void
id|nlmclnt_mark_reclaim
c_func
(paren
r_struct
id|nlm_host
...
multiline_comment|/*&n; * Someone has sent us an SM_NOTIFY. Ensure we bind to the new port number,&n; * that we mark locks for reclaiming, and that we bump the pseudo NSM state.&n; */
r_static
r_inline
DECL|function|nlmclnt_prepare_reclaim
r_void
id|nlmclnt_prepare_reclaim
c_func
(paren
r_struct
id|nlm_host
...
c_func
(paren
l_string|&quot;NLM: reclaiming locks for host %s&quot;
comma
id|host-&gt;h_name
)paren
suffix:semicolon
)brace
multiline_comment|/*&n; * Reclaim all locks on server host. We do this by spawning a separate&n; * reclaimer thread.&n; */
r_void
...
suffix:semicolon
multiline_comment|/* First, reclaim all locks that have been marked. */
id|restart
suffix:colon
id|list_for_each
c_func
