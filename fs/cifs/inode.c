...
multiline_comment|/* no sense revalidating inode info on file that only we can write */
r_if
c_cond
(paren
id|CIFS_I
...
id|direntry-&gt;d_inode-&gt;i_size
suffix:semicolon
multiline_comment|/* BB we need to write out dirty pages here if any before getting possibly &n;&t;&t;stale time from server */
r_if
c_cond
(paren
id|cifs_sb-&gt;tcon-&gt;ses-&gt;capabilities
...
multiline_comment|/* if not oplocked, we invalidate inode pages if mtime &n;&t;   or file size has changed on server */
r_if
c_cond
(paren
id|timespec_equal
...
r_else
(brace
multiline_comment|/* file has changed on server */
id|cFYI
c_func
(paren
l_int|1
comma
(paren
l_string|&quot;Server copy changed, invalidating remote inode &quot;
)paren
)paren
suffix:semicolon
...
id|cifs_sb-&gt;tcon
suffix:semicolon
multiline_comment|/* To avoid spurious oplock breaks from server, in the case&n;&t;&t;of inodes that we already have open, avoid doing path&n;&t;&t;based setting of file size if we can do it by handle.&n;&t;&t;This keeps our caching token (oplock) and avoids&n;&t;&t;timeouts when the local oplock break takes longer to flush&n;&t;&t;writebehind data than the SMB timeout for the SetPathInfo &n;&t;&t;request would allow */
id|read_lock
c_func
(paren
op_amp
