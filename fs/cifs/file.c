...
(brace
multiline_comment|/* time to set mode which we can not&n;                 set earlier due to problems creating new read-only files */
r_if
c_cond
(paren
...
multiline_comment|/* in the meantime we could set the r/o dos attribute &n;                       when perms are e.g.:   &n;                            mode &amp; 0222 == 0 */
)brace
)brace
)brace
)brace
...
multiline_comment|/* BB fix and add missing call to cifs_writepage_sync here */
id|inode-&gt;i_ctime
op_assign
id|inode-&gt;i_mtime
op_assign
id|CURRENT_TIME
suffix:semicolon
multiline_comment|/* BB is this right? */
r_if
c_cond
(paren
(paren
...
suffix:semicolon
multiline_comment|/*&t;status = cifs_updatepage(file, page, offset, to-offset); */
multiline_comment|/* BB add - do we really need to lock the kernel here for so long ? */
id|unlock_kernel
c_func
(paren
)paren
...
r_static
r_int
DECL|function|cifs_readpage_sync
id|cifs_readpage_sync
c_func
(paren
r_struct
id|file
...
multiline_comment|/* BB finish adding missing here */
id|cFYI
c_func
(paren
l_int|1
comma
(paren
l_string|&quot;&bslash;nCount is %d total read %d bytes read %d &quot;
comma
id|count
comma
id|total_read
...
suffix:semicolon
multiline_comment|/* tmp_inode-&gt;i_mtime =&n;&t;   cifs_NTtimeToUnix(pfindData-&gt;LastModificationTime);&n;&t;   tmp_inode-&gt;i_ctime =&n;&t;   cifs_NTtimeToUnix(pfindData-&gt;LastStatusChange); */
id|tmp_inode-&gt;i_mode
op_assign
id|le64_to_cpu
c_func
