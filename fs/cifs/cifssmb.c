...
comma
(paren
l_string|&quot;Reading %d bytes on fid %d&quot;
comma
id|count
comma
id|netfid
...
id|byte_count
suffix:semicolon
multiline_comment|/* cFYI(1,(&quot;write at %lld %d bytes&quot;,offset,count));*/
id|rc
op_assign
id|smb_init
c_func
...
l_int|0
suffix:semicolon
multiline_comment|/* Can increase buffer size if buffer is big enough in some cases - ie we &n;&t;can send more if LARGE_WRITE_X capability returned by the server and if&n;&t;our buffer is big enough or if we convert to iovecs on socket writes&n;&t;and eliminate the copy to the CIFS buffer */
r_if
c_cond
(paren
id|tcon-&gt;ses-&gt;capabilities
...
suffix:semicolon
multiline_comment|/* BB fix this for sends &gt; 64K */
...
comma
(paren
l_string|&quot;In GetSrvInodeNum for %s&quot;
comma
id|searchName
)paren
)paren
...
c_func
(paren
id|SMB_QUERY_FILE_INTERNAL_INFO
)paren
suffix:semicolon
id|pSMB-&gt;Reserved4
op_assign
...
comma
(paren
l_string|&quot;error %d in QueryInternalInfo&quot;
comma
id|rc
)paren
)paren
...
multiline_comment|/* BB also check enough total bytes returned */
multiline_comment|/* If rc should we check for EOPNOSUPP and&n;&t;&t;&t;disable the srvino flag? or in caller? */
id|rc
op_assign
op_minus
id|EIO
...
id|pfinfo
suffix:semicolon
multiline_comment|/* BB Do we need a cast or hash here ? */
r_if
c_cond
(paren
id|count
...
comma
(paren
l_string|&quot;Illegal size ret in QryIntrnlInf&quot;
)paren
)paren
suffix:semicolon
id|rc
...
r_int
DECL|function|CIFSSMBQFSPosixInfo
...
(brace
multiline_comment|/* level 0x201  SMB_QUERY_CIFS_POSIX_INFO */
id|TRANSACTION2_QFSI_REQ
op_star
id|pSMB
op_assign
...
c_cond
(paren
id|response_data-&gt;UserBlocksAvail
op_eq
op_minus
l_int|1
)paren
...
c_cond
(paren
id|response_data-&gt;TotalFileNodes
op_ne
op_minus
l_int|1
)paren
...
c_cond
(paren
id|response_data-&gt;FreeFileNodes
op_ne
op_minus
l_int|1
)paren
...
multiline_comment|/* Some legacy servers such as NT4 require that the file times be set on &n;   an open handle, rather than by pathname - this is awkward due to&n;   potential access conflicts on the open, but it is unavoidable for these&n;   old servers since the only other choice is to go from 100 nanosecond DCE&n;   time and resort to the original setpathinfo level which takes the ancient&n;   DOS time format with 2 second granularity */
r_int
DECL|function|CIFSSMBSetFileTimes
...
comma
(paren
l_string|&quot;Set Times (via SetFileInfo)&quot;
)paren
)paren
suffix:semicolon
id|rc
...
id|rc
suffix:semicolon
multiline_comment|/* At this point there is no need to override the current pid&n;&t;with the pid of the opener, but that could change if we someday&n;&t;use an existing handle (rather than opening one on the fly) */
multiline_comment|/* pSMB-&gt;hdr.Pid = cpu_to_le16((__u16)pid_of_opener);&n;&t;pSMB-&gt;hdr.PidHigh = cpu_to_le16((__u16)(pid_of_opener &gt;&gt; 16));*/
id|params
op_assign
l_int|6
suffix:semicolon
...
comma
(paren
l_string|&quot;Send error in Set Time (SetFileInfo) = %d&quot;
comma
id|rc
)paren
)paren
...
multiline_comment|/* Can not be used to set time stamps yet (due to old DOS time format) */
multiline_comment|/* Can be used to set attributes */
macro_line|#if 0  /* Possibly not needed - since it turns out that strangely NT4 has a bug&n;&t;  handling it anyway and NT4 was what we thought it would be needed for&n;&t;  Do not delete it until we prove whether needed for Win9x though */
r_int
...
comma
(paren
l_string|&quot;Error in LegacySetAttr = %d&quot;
comma
id|rc
)paren
)paren
...
macro_line|#endif /* temporarily unneeded SetAttr legacy function */
r_int
