...
c_cond
(paren
id|fl-&gt;fl_ops-&gt;fl_release_private
)paren
id|fl-&gt;fl_ops
...
c_cond
(paren
id|fl-&gt;fl_lmops-&gt;fl_release_private
)paren
id|fl-&gt;fl_lmops
op_member_access_from_pointer
id|fl_release_private
...
macro_line|#endif
multiline_comment|/* default lease lock manager operations */
DECL|function|lease_break_callback
r_static
r_void
id|lease_break_callback
c_func
(paren
r_struct
id|file_lock
...
DECL|function|lease_release_private_callback
r_static
r_void
id|lease_release_private_callback
c_func
(paren
r_struct
id|file_lock
...
suffix:semicolon
id|fl-&gt;fl_file-&gt;f_owner.signum
op_assign
l_int|0
suffix:semicolon
)brace
DECL|function|lease_mylease_callback
r_static
r_int
id|lease_mylease_callback
c_func
(paren
r_struct
id|file_lock
...
DECL|variable|lease_manager_ops
r_struct
id|lock_manager_operations
id|lease_manager_ops
op_assign
(brace
dot
id|fl_break
...
suffix:semicolon
multiline_comment|/*&n; * Initialize a lease, use the default lock manager operations&n; */
DECL|function|lease_init
r_static
r_int
id|lease_init
c_func
...
multiline_comment|/*&n; * Check whether two locks have the same owner.&n; */
r_static
r_inline
r_int
...
id|fl1-&gt;fl_lmops
op_logical_and
id|fl1-&gt;fl_lmops-&gt;fl_compare_owner
)paren
r_return
id|fl2-&gt;fl_lmops
op_eq
...
multiline_comment|/**&n; * posix_lock_file_wait - Apply a POSIX-style lock to a file&n; * @filp: The file to apply the lock to&n; * @fl: The lock to be applied&n; *&n; * Add a POSIX style lock to a file.&n; * We merge adjacent &amp; overlapping locks whenever possible.&n; * POSIX locks are sorted by owner task, then by starting address&n; */
DECL|function|posix_lock_file_wait
r_int
...
DECL|variable|posix_lock_file_wait
id|EXPORT_SYMBOL
c_func
(paren
id|posix_lock_file_wait
...
multiline_comment|/**&n;*&t;remove_lease - let time_out_leases remove the lease.&n;*&t;@@file_lock: the lease to remove&n;*/
DECL|function|remove_lease
r_void
id|remove_lease
c_func
(paren
...
multiline_comment|/**&n; *&t;__setlease&t;-&t;sets a lease on an open file&n; *&t;@filp: file pointer&n; *&t;@arg: type of lease to obtain&n; *&t;@flp: input - file_lock to use, output - file_lock inserted&n; *&n; *&t;The (input) flp-&gt;fl_lmops-&gt;fl_break function is required&n; *&t;by break_lease().&n; *&n; *&t;Called with kernel lock held.&n; */
DECL|function|__setlease
r_int
id|__setlease
c_func
(paren
...
multiline_comment|/**&n; *&t;setlease        -       sets a lease on an open file&n; *&t;@filp: file pointer&n; *&t;@arg: type of lease to obtain&n; *&t;@lease: file_lock to use&n; *&n; *&t;Call this to establish a lease on the file.&n; *&t;The fl_lmops fl_break function is required by break_lease&n; */
DECL|function|setlease
r_int
id|setlease
c_func
(paren
...
(brace
multiline_comment|/* remove lease just inserted by __setlease */
id|flp-&gt;fl_type
op_assign
id|F_UNLCK
op_or
...
multiline_comment|/**&n; * flock_lock_file_wait - Apply a FLOCK-style lock to a file&n; * @filp: The file to apply the lock to&n; * @fl: The lock to be applied&n; *&n; * Add a FLOCK style lock to a file.&n; */
DECL|function|flock_lock_file_wait
r_int
...
DECL|variable|flock_lock_file_wait
id|EXPORT_SYMBOL
c_func
(paren
id|flock_lock_file_wait
...
id|lock.fl_ops
op_logical_and
id|lock.fl_ops-&gt;fl_release_private
)paren
id|lock.fl_ops
op_member_access_from_pointer
id|fl_release_private
...
multiline_comment|/* When getting ready for executing a binary, we make sure that current&n; * has a files_struct on its own. Before dropping the old files_struct,&n; * we take over ownership of all locks for all file descriptors we own.&n; * Note that we may accidentally steal a lock for a file that a sibling&n; * has created since the unshare_files() call.&n; */
