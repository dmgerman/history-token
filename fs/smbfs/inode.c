multiline_comment|/*&n; *  inode.c&n; *&n; *  Copyright (C) 1995, 1996 by Paal-Kr. Engstad and Volker Lendecke&n; *  Copyright (C) 1997 by Volker Lendecke&n; *&n; *  Please add a note about your changes to smbfs in the ChangeLog file.&n; */
macro_line|#include &lt;linux/config.h&gt;
macro_line|#include &lt;linux/module.h&gt;
macro_line|#include &lt;linux/time.h&gt;
macro_line|#include &lt;linux/kernel.h&gt;
macro_line|#include &lt;linux/mm.h&gt;
macro_line|#include &lt;linux/string.h&gt;
macro_line|#include &lt;linux/stat.h&gt;
macro_line|#include &lt;linux/errno.h&gt;
macro_line|#include &lt;linux/slab.h&gt;
macro_line|#include &lt;linux/init.h&gt;
macro_line|#include &lt;linux/file.h&gt;
macro_line|#include &lt;linux/dcache.h&gt;
macro_line|#include &lt;linux/smp_lock.h&gt;
macro_line|#include &lt;linux/nls.h&gt;
macro_line|#include &lt;linux/seq_file.h&gt;
macro_line|#include &lt;linux/mount.h&gt;
macro_line|#include &lt;linux/net.h&gt;
macro_line|#include &lt;linux/vfs.h&gt;
macro_line|#include &lt;linux/highuid.h&gt;
macro_line|#include &lt;linux/smb_fs.h&gt;
macro_line|#include &lt;linux/smbno.h&gt;
macro_line|#include &lt;linux/smb_mount.h&gt;
macro_line|#include &lt;asm/system.h&gt;
macro_line|#include &lt;asm/uaccess.h&gt;
macro_line|#include &quot;smb_debug.h&quot;
macro_line|#include &quot;getopt.h&quot;
macro_line|#include &quot;proto.h&quot;
multiline_comment|/* Always pick a default string */
macro_line|#ifdef CONFIG_SMB_NLS_REMOTE
DECL|macro|SMB_NLS_REMOTE
mdefine_line|#define SMB_NLS_REMOTE CONFIG_SMB_NLS_REMOTE
macro_line|#else
DECL|macro|SMB_NLS_REMOTE
mdefine_line|#define SMB_NLS_REMOTE &quot;&quot;
macro_line|#endif
DECL|macro|SMB_TTL_DEFAULT
mdefine_line|#define SMB_TTL_DEFAULT 1000
r_static
r_void
id|smb_delete_inode
c_func
(paren
r_struct
id|inode
op_star
)paren
suffix:semicolon
r_static
r_void
id|smb_put_super
c_func
(paren
r_struct
id|super_block
op_star
)paren
suffix:semicolon
r_static
r_int
id|smb_statfs
c_func
(paren
r_struct
id|super_block
op_star
comma
r_struct
id|kstatfs
op_star
)paren
suffix:semicolon
r_static
r_int
id|smb_show_options
c_func
(paren
r_struct
id|seq_file
op_star
comma
r_struct
id|vfsmount
op_star
)paren
suffix:semicolon
DECL|variable|smb_inode_cachep
r_static
id|kmem_cache_t
op_star
id|smb_inode_cachep
suffix:semicolon
DECL|function|smb_alloc_inode
r_static
r_struct
id|inode
op_star
id|smb_alloc_inode
c_func
(paren
r_struct
id|super_block
op_star
id|sb
)paren
(brace
r_struct
id|smb_inode_info
op_star
id|ei
suffix:semicolon
id|ei
op_assign
(paren
r_struct
id|smb_inode_info
op_star
)paren
id|kmem_cache_alloc
c_func
(paren
id|smb_inode_cachep
comma
id|SLAB_KERNEL
)paren
suffix:semicolon
r_if
c_cond
(paren
op_logical_neg
id|ei
)paren
r_return
l_int|NULL
suffix:semicolon
r_return
op_amp
id|ei-&gt;vfs_inode
suffix:semicolon
)brace
DECL|function|smb_destroy_inode
r_static
r_void
id|smb_destroy_inode
c_func
(paren
r_struct
id|inode
op_star
id|inode
)paren
(brace
id|kmem_cache_free
c_func
(paren
id|smb_inode_cachep
comma
id|SMB_I
c_func
(paren
id|inode
)paren
)paren
suffix:semicolon
)brace
DECL|function|init_once
r_static
r_void
id|init_once
c_func
(paren
r_void
op_star
id|foo
comma
id|kmem_cache_t
op_star
id|cachep
comma
r_int
r_int
id|flags
)paren
(brace
r_struct
id|smb_inode_info
op_star
id|ei
op_assign
(paren
r_struct
id|smb_inode_info
op_star
)paren
id|foo
suffix:semicolon
r_int
r_int
id|flagmask
op_assign
id|SLAB_CTOR_VERIFY
op_or
id|SLAB_CTOR_CONSTRUCTOR
suffix:semicolon
r_if
c_cond
(paren
(paren
id|flags
op_amp
id|flagmask
)paren
op_eq
id|SLAB_CTOR_CONSTRUCTOR
)paren
id|inode_init_once
c_func
(paren
op_amp
id|ei-&gt;vfs_inode
)paren
suffix:semicolon
)brace
DECL|function|init_inodecache
r_static
r_int
id|init_inodecache
c_func
(paren
r_void
)paren
(brace
id|smb_inode_cachep
op_assign
id|kmem_cache_create
c_func
(paren
l_string|&quot;smb_inode_cache&quot;
comma
r_sizeof
(paren
r_struct
id|smb_inode_info
)paren
comma
l_int|0
comma
id|SLAB_RECLAIM_ACCOUNT
comma
id|init_once
comma
l_int|NULL
)paren
suffix:semicolon
r_if
c_cond
(paren
id|smb_inode_cachep
op_eq
l_int|NULL
)paren
r_return
op_minus
id|ENOMEM
suffix:semicolon
r_return
l_int|0
suffix:semicolon
)brace
DECL|function|destroy_inodecache
r_static
r_void
id|destroy_inodecache
c_func
(paren
r_void
)paren
(brace
r_if
c_cond
(paren
id|kmem_cache_destroy
c_func
(paren
id|smb_inode_cachep
)paren
)paren
id|printk
c_func
(paren
id|KERN_INFO
l_string|&quot;smb_inode_cache: not all structures were freed&bslash;n&quot;
)paren
suffix:semicolon
)brace
DECL|function|smb_remount
r_static
r_int
id|smb_remount
c_func
(paren
r_struct
id|super_block
op_star
id|sb
comma
r_int
op_star
id|flags
comma
r_char
op_star
id|data
)paren
(brace
op_star
id|flags
op_or_assign
id|MS_NODIRATIME
suffix:semicolon
r_return
l_int|0
suffix:semicolon
)brace
DECL|variable|smb_sops
r_static
r_struct
id|super_operations
id|smb_sops
op_assign
(brace
dot
id|alloc_inode
op_assign
id|smb_alloc_inode
comma
dot
id|destroy_inode
op_assign
id|smb_destroy_inode
comma
dot
id|drop_inode
op_assign
id|generic_delete_inode
comma
dot
id|delete_inode
op_assign
id|smb_delete_inode
comma
dot
id|put_super
op_assign
id|smb_put_super
comma
dot
id|statfs
op_assign
id|smb_statfs
comma
dot
id|show_options
op_assign
id|smb_show_options
comma
dot
id|remount_fs
op_assign
id|smb_remount
comma
)brace
suffix:semicolon
multiline_comment|/* We are always generating a new inode here */
r_struct
id|inode
op_star
DECL|function|smb_iget
id|smb_iget
c_func
(paren
r_struct
id|super_block
op_star
id|sb
comma
r_struct
id|smb_fattr
op_star
id|fattr
)paren
(brace
r_struct
id|smb_sb_info
op_star
id|server
op_assign
id|SMB_SB
c_func
(paren
id|sb
)paren
suffix:semicolon
r_struct
id|inode
op_star
id|result
suffix:semicolon
id|DEBUG1
c_func
(paren
l_string|&quot;smb_iget: %p&bslash;n&quot;
comma
id|fattr
)paren
suffix:semicolon
id|result
op_assign
id|new_inode
c_func
(paren
id|sb
)paren
suffix:semicolon
r_if
c_cond
(paren
op_logical_neg
id|result
)paren
r_return
id|result
suffix:semicolon
id|result-&gt;i_ino
op_assign
id|fattr-&gt;f_ino
suffix:semicolon
id|SMB_I
c_func
(paren
id|result
)paren
op_member_access_from_pointer
id|open
op_assign
l_int|0
suffix:semicolon
id|SMB_I
c_func
(paren
id|result
)paren
op_member_access_from_pointer
id|fileid
op_assign
l_int|0
suffix:semicolon
id|SMB_I
c_func
(paren
id|result
)paren
op_member_access_from_pointer
id|access
op_assign
l_int|0
suffix:semicolon
id|SMB_I
c_func
(paren
id|result
)paren
op_member_access_from_pointer
id|flags
op_assign
l_int|0
suffix:semicolon
id|SMB_I
c_func
(paren
id|result
)paren
op_member_access_from_pointer
id|closed
op_assign
l_int|0
suffix:semicolon
id|SMB_I
c_func
(paren
id|result
)paren
op_member_access_from_pointer
id|openers
op_assign
l_int|0
suffix:semicolon
id|smb_set_inode_attr
c_func
(paren
id|result
comma
id|fattr
)paren
suffix:semicolon
r_if
c_cond
(paren
id|S_ISREG
c_func
(paren
id|result-&gt;i_mode
)paren
)paren
(brace
id|result-&gt;i_op
op_assign
op_amp
id|smb_file_inode_operations
suffix:semicolon
id|result-&gt;i_fop
op_assign
op_amp
id|smb_file_operations
suffix:semicolon
id|result-&gt;i_data.a_ops
op_assign
op_amp
id|smb_file_aops
suffix:semicolon
)brace
r_else
r_if
c_cond
(paren
id|S_ISDIR
c_func
(paren
id|result-&gt;i_mode
)paren
)paren
(brace
r_if
c_cond
(paren
id|server-&gt;opt.capabilities
op_amp
id|SMB_CAP_UNIX
)paren
id|result-&gt;i_op
op_assign
op_amp
id|smb_dir_inode_operations_unix
suffix:semicolon
r_else
id|result-&gt;i_op
op_assign
op_amp
id|smb_dir_inode_operations
suffix:semicolon
id|result-&gt;i_fop
op_assign
op_amp
id|smb_dir_operations
suffix:semicolon
)brace
r_else
r_if
c_cond
(paren
id|S_ISLNK
c_func
(paren
id|result-&gt;i_mode
)paren
)paren
(brace
id|result-&gt;i_op
op_assign
op_amp
id|smb_link_inode_operations
suffix:semicolon
)brace
r_else
(brace
id|init_special_inode
c_func
(paren
id|result
comma
id|result-&gt;i_mode
comma
id|fattr-&gt;f_rdev
)paren
suffix:semicolon
)brace
id|insert_inode_hash
c_func
(paren
id|result
)paren
suffix:semicolon
r_return
id|result
suffix:semicolon
)brace
multiline_comment|/*&n; * Copy the inode data to a smb_fattr structure.&n; */
r_void
DECL|function|smb_get_inode_attr
id|smb_get_inode_attr
c_func
(paren
r_struct
id|inode
op_star
id|inode
comma
r_struct
id|smb_fattr
op_star
id|fattr
)paren
(brace
id|memset
c_func
(paren
id|fattr
comma
l_int|0
comma
r_sizeof
(paren
r_struct
id|smb_fattr
)paren
)paren
suffix:semicolon
id|fattr-&gt;f_mode
op_assign
id|inode-&gt;i_mode
suffix:semicolon
id|fattr-&gt;f_nlink
op_assign
id|inode-&gt;i_nlink
suffix:semicolon
id|fattr-&gt;f_ino
op_assign
id|inode-&gt;i_ino
suffix:semicolon
id|fattr-&gt;f_uid
op_assign
id|inode-&gt;i_uid
suffix:semicolon
id|fattr-&gt;f_gid
op_assign
id|inode-&gt;i_gid
suffix:semicolon
id|fattr-&gt;f_size
op_assign
id|inode-&gt;i_size
suffix:semicolon
id|fattr-&gt;f_mtime
op_assign
id|inode-&gt;i_mtime
suffix:semicolon
id|fattr-&gt;f_ctime
op_assign
id|inode-&gt;i_ctime
suffix:semicolon
id|fattr-&gt;f_atime
op_assign
id|inode-&gt;i_atime
suffix:semicolon
id|fattr-&gt;f_blksize
op_assign
id|inode-&gt;i_blksize
suffix:semicolon
id|fattr-&gt;f_blocks
op_assign
id|inode-&gt;i_blocks
suffix:semicolon
id|fattr-&gt;attr
op_assign
id|SMB_I
c_func
(paren
id|inode
)paren
op_member_access_from_pointer
id|attr
suffix:semicolon
multiline_comment|/*&n;&t; * Keep the attributes in sync with the inode permissions.&n;&t; */
r_if
c_cond
(paren
id|fattr-&gt;f_mode
op_amp
id|S_IWUSR
)paren
id|fattr-&gt;attr
op_and_assign
op_complement
id|aRONLY
suffix:semicolon
r_else
id|fattr-&gt;attr
op_or_assign
id|aRONLY
suffix:semicolon
)brace
multiline_comment|/*&n; * Update the inode, possibly causing it to invalidate its pages if mtime/size&n; * is different from last time.&n; */
r_void
DECL|function|smb_set_inode_attr
id|smb_set_inode_attr
c_func
(paren
r_struct
id|inode
op_star
id|inode
comma
r_struct
id|smb_fattr
op_star
id|fattr
)paren
(brace
r_struct
id|smb_inode_info
op_star
id|ei
op_assign
id|SMB_I
c_func
(paren
id|inode
)paren
suffix:semicolon
multiline_comment|/*&n;&t; * A size change should have a different mtime, or same mtime&n;&t; * but different size.&n;&t; */
id|time_t
id|last_time
op_assign
id|inode-&gt;i_mtime.tv_sec
suffix:semicolon
id|loff_t
id|last_sz
op_assign
id|inode-&gt;i_size
suffix:semicolon
id|inode-&gt;i_mode
op_assign
id|fattr-&gt;f_mode
suffix:semicolon
id|inode-&gt;i_nlink
op_assign
id|fattr-&gt;f_nlink
suffix:semicolon
id|inode-&gt;i_uid
op_assign
id|fattr-&gt;f_uid
suffix:semicolon
id|inode-&gt;i_gid
op_assign
id|fattr-&gt;f_gid
suffix:semicolon
id|inode-&gt;i_ctime
op_assign
id|fattr-&gt;f_ctime
suffix:semicolon
id|inode-&gt;i_blksize
op_assign
id|fattr-&gt;f_blksize
suffix:semicolon
id|inode-&gt;i_blocks
op_assign
id|fattr-&gt;f_blocks
suffix:semicolon
id|inode-&gt;i_size
op_assign
id|fattr-&gt;f_size
suffix:semicolon
id|inode-&gt;i_mtime
op_assign
id|fattr-&gt;f_mtime
suffix:semicolon
id|inode-&gt;i_atime
op_assign
id|fattr-&gt;f_atime
suffix:semicolon
id|ei-&gt;attr
op_assign
id|fattr-&gt;attr
suffix:semicolon
multiline_comment|/*&n;&t; * Update the &quot;last time refreshed&quot; field for revalidation.&n;&t; */
id|ei-&gt;oldmtime
op_assign
id|jiffies
suffix:semicolon
r_if
c_cond
(paren
id|inode-&gt;i_mtime.tv_sec
op_ne
id|last_time
op_logical_or
id|inode-&gt;i_size
op_ne
id|last_sz
)paren
(brace
id|VERBOSE
c_func
(paren
l_string|&quot;%ld changed, old=%ld, new=%ld, oz=%ld, nz=%ld&bslash;n&quot;
comma
id|inode-&gt;i_ino
comma
(paren
r_int
)paren
id|last_time
comma
(paren
r_int
)paren
id|inode-&gt;i_mtime
comma
(paren
r_int
)paren
id|last_sz
comma
(paren
r_int
)paren
id|inode-&gt;i_size
)paren
suffix:semicolon
r_if
c_cond
(paren
op_logical_neg
id|S_ISDIR
c_func
(paren
id|inode-&gt;i_mode
)paren
)paren
id|invalidate_remote_inode
c_func
(paren
id|inode
)paren
suffix:semicolon
)brace
)brace
multiline_comment|/*&n; * This is called if the connection has gone bad ...&n; * try to kill off all the current inodes.&n; */
r_void
DECL|function|smb_invalidate_inodes
id|smb_invalidate_inodes
c_func
(paren
r_struct
id|smb_sb_info
op_star
id|server
)paren
(brace
id|VERBOSE
c_func
(paren
l_string|&quot;&bslash;n&quot;
)paren
suffix:semicolon
id|shrink_dcache_sb
c_func
(paren
id|SB_of
c_func
(paren
id|server
)paren
)paren
suffix:semicolon
id|invalidate_inodes
c_func
(paren
id|SB_of
c_func
(paren
id|server
)paren
)paren
suffix:semicolon
)brace
multiline_comment|/*&n; * This is called to update the inode attributes after&n; * we&squot;ve made changes to a file or directory.&n; */
r_static
r_int
DECL|function|smb_refresh_inode
id|smb_refresh_inode
c_func
(paren
r_struct
id|dentry
op_star
id|dentry
)paren
(brace
r_struct
id|inode
op_star
id|inode
op_assign
id|dentry-&gt;d_inode
suffix:semicolon
r_int
id|error
suffix:semicolon
r_struct
id|smb_fattr
id|fattr
suffix:semicolon
id|error
op_assign
id|smb_proc_getattr
c_func
(paren
id|dentry
comma
op_amp
id|fattr
)paren
suffix:semicolon
r_if
c_cond
(paren
op_logical_neg
id|error
)paren
(brace
id|smb_renew_times
c_func
(paren
id|dentry
)paren
suffix:semicolon
multiline_comment|/*&n;&t;&t; * Check whether the type part of the mode changed,&n;&t;&t; * and don&squot;t update the attributes if it did.&n;&t;&t; *&n;&t;&t; * And don&squot;t dick with the root inode&n;&t;&t; */
r_if
c_cond
(paren
id|inode-&gt;i_ino
op_eq
l_int|2
)paren
r_return
id|error
suffix:semicolon
r_if
c_cond
(paren
id|S_ISLNK
c_func
(paren
id|inode-&gt;i_mode
)paren
)paren
r_return
id|error
suffix:semicolon
multiline_comment|/* VFS will deal with it */
r_if
c_cond
(paren
(paren
id|inode-&gt;i_mode
op_amp
id|S_IFMT
)paren
op_eq
(paren
id|fattr.f_mode
op_amp
id|S_IFMT
)paren
)paren
(brace
id|smb_set_inode_attr
c_func
(paren
id|inode
comma
op_amp
id|fattr
)paren
suffix:semicolon
)brace
r_else
(brace
multiline_comment|/*&n;&t;&t;&t; * Big trouble! The inode has become a new object,&n;&t;&t;&t; * so any operations attempted on it are invalid.&n;&t;&t;&t; *&n;&t;&t;&t; * To limit damage, mark the inode as bad so that&n;&t;&t;&t; * subsequent lookup validations will fail.&n;&t;&t;&t; */
id|PARANOIA
c_func
(paren
l_string|&quot;%s/%s changed mode, %07o to %07o&bslash;n&quot;
comma
id|DENTRY_PATH
c_func
(paren
id|dentry
)paren
comma
id|inode-&gt;i_mode
comma
id|fattr.f_mode
)paren
suffix:semicolon
id|fattr.f_mode
op_assign
id|inode-&gt;i_mode
suffix:semicolon
multiline_comment|/* save mode */
id|make_bad_inode
c_func
(paren
id|inode
)paren
suffix:semicolon
id|inode-&gt;i_mode
op_assign
id|fattr.f_mode
suffix:semicolon
multiline_comment|/* restore mode */
multiline_comment|/*&n;&t;&t;&t; * No need to worry about unhashing the dentry: the&n;&t;&t;&t; * lookup validation will see that the inode is bad.&n;&t;&t;&t; * But we do want to invalidate the caches ...&n;&t;&t;&t; */
r_if
c_cond
(paren
op_logical_neg
id|S_ISDIR
c_func
(paren
id|inode-&gt;i_mode
)paren
)paren
id|invalidate_remote_inode
c_func
(paren
id|inode
)paren
suffix:semicolon
r_else
id|smb_invalid_dir_cache
c_func
(paren
id|inode
)paren
suffix:semicolon
id|error
op_assign
op_minus
id|EIO
suffix:semicolon
)brace
)brace
r_return
id|error
suffix:semicolon
)brace
multiline_comment|/*&n; * This is called when we want to check whether the inode&n; * has changed on the server.  If it has changed, we must&n; * invalidate our local caches.&n; */
r_int
DECL|function|smb_revalidate_inode
id|smb_revalidate_inode
c_func
(paren
r_struct
id|dentry
op_star
id|dentry
)paren
(brace
r_struct
id|smb_sb_info
op_star
id|s
op_assign
id|server_from_dentry
c_func
(paren
id|dentry
)paren
suffix:semicolon
r_struct
id|inode
op_star
id|inode
op_assign
id|dentry-&gt;d_inode
suffix:semicolon
r_int
id|error
op_assign
l_int|0
suffix:semicolon
id|DEBUG1
c_func
(paren
l_string|&quot;smb_revalidate_inode&bslash;n&quot;
)paren
suffix:semicolon
id|lock_kernel
c_func
(paren
)paren
suffix:semicolon
multiline_comment|/*&n;&t; * Check whether we&squot;ve recently refreshed the inode.&n;&t; */
r_if
c_cond
(paren
id|time_before
c_func
(paren
id|jiffies
comma
id|SMB_I
c_func
(paren
id|inode
)paren
op_member_access_from_pointer
id|oldmtime
op_plus
id|SMB_MAX_AGE
c_func
(paren
id|s
)paren
)paren
)paren
(brace
id|VERBOSE
c_func
(paren
l_string|&quot;up-to-date, ino=%ld, jiffies=%lu, oldtime=%lu&bslash;n&quot;
comma
id|inode-&gt;i_ino
comma
id|jiffies
comma
id|SMB_I
c_func
(paren
id|inode
)paren
op_member_access_from_pointer
id|oldmtime
)paren
suffix:semicolon
r_goto
id|out
suffix:semicolon
)brace
id|error
op_assign
id|smb_refresh_inode
c_func
(paren
id|dentry
)paren
suffix:semicolon
id|out
suffix:colon
id|unlock_kernel
c_func
(paren
)paren
suffix:semicolon
r_return
id|error
suffix:semicolon
)brace
multiline_comment|/*&n; * This routine is called when i_nlink == 0 and i_count goes to 0.&n; * All blocking cleanup operations need to go here to avoid races.&n; */
r_static
r_void
DECL|function|smb_delete_inode
id|smb_delete_inode
c_func
(paren
r_struct
id|inode
op_star
id|ino
)paren
(brace
id|DEBUG1
c_func
(paren
l_string|&quot;ino=%ld&bslash;n&quot;
comma
id|ino-&gt;i_ino
)paren
suffix:semicolon
id|lock_kernel
c_func
(paren
)paren
suffix:semicolon
r_if
c_cond
(paren
id|smb_close
c_func
(paren
id|ino
)paren
)paren
id|PARANOIA
c_func
(paren
l_string|&quot;could not close inode %ld&bslash;n&quot;
comma
id|ino-&gt;i_ino
)paren
suffix:semicolon
id|unlock_kernel
c_func
(paren
)paren
suffix:semicolon
id|clear_inode
c_func
(paren
id|ino
)paren
suffix:semicolon
)brace
DECL|variable|opts
r_static
r_struct
id|option
id|opts
(braket
)braket
op_assign
(brace
(brace
l_string|&quot;version&quot;
comma
l_int|0
comma
l_char|&squot;v&squot;
)brace
comma
(brace
l_string|&quot;win95&quot;
comma
id|SMB_MOUNT_WIN95
comma
l_int|1
)brace
comma
(brace
l_string|&quot;oldattr&quot;
comma
id|SMB_MOUNT_OLDATTR
comma
l_int|1
)brace
comma
(brace
l_string|&quot;dirattr&quot;
comma
id|SMB_MOUNT_DIRATTR
comma
l_int|1
)brace
comma
(brace
l_string|&quot;case&quot;
comma
id|SMB_MOUNT_CASE
comma
l_int|1
)brace
comma
(brace
l_string|&quot;uid&quot;
comma
l_int|0
comma
l_char|&squot;u&squot;
)brace
comma
(brace
l_string|&quot;gid&quot;
comma
l_int|0
comma
l_char|&squot;g&squot;
)brace
comma
(brace
l_string|&quot;file_mode&quot;
comma
l_int|0
comma
l_char|&squot;f&squot;
)brace
comma
(brace
l_string|&quot;dir_mode&quot;
comma
l_int|0
comma
l_char|&squot;d&squot;
)brace
comma
(brace
l_string|&quot;iocharset&quot;
comma
l_int|0
comma
l_char|&squot;i&squot;
)brace
comma
(brace
l_string|&quot;codepage&quot;
comma
l_int|0
comma
l_char|&squot;c&squot;
)brace
comma
(brace
l_string|&quot;ttl&quot;
comma
l_int|0
comma
l_char|&squot;t&squot;
)brace
comma
(brace
l_int|NULL
comma
l_int|0
comma
l_int|0
)brace
)brace
suffix:semicolon
r_static
r_int
DECL|function|parse_options
id|parse_options
c_func
(paren
r_struct
id|smb_mount_data_kernel
op_star
id|mnt
comma
r_char
op_star
id|options
)paren
(brace
r_int
id|c
suffix:semicolon
r_int
r_int
id|flags
suffix:semicolon
r_int
r_int
id|value
suffix:semicolon
r_char
op_star
id|optarg
suffix:semicolon
r_char
op_star
id|optopt
suffix:semicolon
id|flags
op_assign
l_int|0
suffix:semicolon
r_while
c_loop
(paren
(paren
id|c
op_assign
id|smb_getopt
c_func
(paren
l_string|&quot;smbfs&quot;
comma
op_amp
id|options
comma
id|opts
comma
op_amp
id|optopt
comma
op_amp
id|optarg
comma
op_amp
id|flags
comma
op_amp
id|value
)paren
)paren
OG
l_int|0
)paren
(brace
id|VERBOSE
c_func
(paren
l_string|&quot;&squot;%s&squot; -&gt; &squot;%s&squot;&bslash;n&quot;
comma
id|optopt
comma
id|optarg
ques
c_cond
id|optarg
suffix:colon
l_string|&quot;&lt;none&gt;&quot;
)paren
suffix:semicolon
r_switch
c_cond
(paren
id|c
)paren
(brace
r_case
l_int|1
suffix:colon
multiline_comment|/* got a &quot;flag&quot; option */
r_break
suffix:semicolon
r_case
l_char|&squot;v&squot;
suffix:colon
r_if
c_cond
(paren
id|value
op_ne
id|SMB_MOUNT_VERSION
)paren
(brace
id|printk
(paren
l_string|&quot;smbfs: Bad mount version %ld, expected %d&bslash;n&quot;
comma
id|value
comma
id|SMB_MOUNT_VERSION
)paren
suffix:semicolon
r_return
l_int|0
suffix:semicolon
)brace
id|mnt-&gt;version
op_assign
id|value
suffix:semicolon
r_break
suffix:semicolon
r_case
l_char|&squot;u&squot;
suffix:colon
id|mnt-&gt;uid
op_assign
id|value
suffix:semicolon
id|flags
op_or_assign
id|SMB_MOUNT_UID
suffix:semicolon
r_break
suffix:semicolon
r_case
l_char|&squot;g&squot;
suffix:colon
id|mnt-&gt;gid
op_assign
id|value
suffix:semicolon
id|flags
op_or_assign
id|SMB_MOUNT_GID
suffix:semicolon
r_break
suffix:semicolon
r_case
l_char|&squot;f&squot;
suffix:colon
id|mnt-&gt;file_mode
op_assign
(paren
id|value
op_amp
id|S_IRWXUGO
)paren
op_or
id|S_IFREG
suffix:semicolon
id|flags
op_or_assign
id|SMB_MOUNT_FMODE
suffix:semicolon
r_break
suffix:semicolon
r_case
l_char|&squot;d&squot;
suffix:colon
id|mnt-&gt;dir_mode
op_assign
(paren
id|value
op_amp
id|S_IRWXUGO
)paren
op_or
id|S_IFDIR
suffix:semicolon
id|flags
op_or_assign
id|SMB_MOUNT_DMODE
suffix:semicolon
r_break
suffix:semicolon
r_case
l_char|&squot;i&squot;
suffix:colon
id|strlcpy
c_func
(paren
id|mnt-&gt;codepage.local_name
comma
id|optarg
comma
id|SMB_NLS_MAXNAMELEN
)paren
suffix:semicolon
r_break
suffix:semicolon
r_case
l_char|&squot;c&squot;
suffix:colon
id|strlcpy
c_func
(paren
id|mnt-&gt;codepage.remote_name
comma
id|optarg
comma
id|SMB_NLS_MAXNAMELEN
)paren
suffix:semicolon
r_break
suffix:semicolon
r_case
l_char|&squot;t&squot;
suffix:colon
id|mnt-&gt;ttl
op_assign
id|value
suffix:semicolon
r_break
suffix:semicolon
r_default
suffix:colon
id|printk
(paren
l_string|&quot;smbfs: Unrecognized mount option %s&bslash;n&quot;
comma
id|optopt
)paren
suffix:semicolon
r_return
op_minus
l_int|1
suffix:semicolon
)brace
)brace
id|mnt-&gt;flags
op_assign
id|flags
suffix:semicolon
r_return
id|c
suffix:semicolon
)brace
multiline_comment|/*&n; * smb_show_options() is for displaying mount options in /proc/mounts.&n; * It tries to avoid showing settings that were not changed from their&n; * defaults.&n; */
r_static
r_int
DECL|function|smb_show_options
id|smb_show_options
c_func
(paren
r_struct
id|seq_file
op_star
id|s
comma
r_struct
id|vfsmount
op_star
id|m
)paren
(brace
r_struct
id|smb_mount_data_kernel
op_star
id|mnt
op_assign
id|SMB_SB
c_func
(paren
id|m-&gt;mnt_sb
)paren
op_member_access_from_pointer
id|mnt
suffix:semicolon
r_int
id|i
suffix:semicolon
r_for
c_loop
(paren
id|i
op_assign
l_int|0
suffix:semicolon
id|opts
(braket
id|i
)braket
dot
id|name
op_ne
l_int|NULL
suffix:semicolon
id|i
op_increment
)paren
r_if
c_cond
(paren
id|mnt-&gt;flags
op_amp
id|opts
(braket
id|i
)braket
dot
id|flag
)paren
id|seq_printf
c_func
(paren
id|s
comma
l_string|&quot;,%s&quot;
comma
id|opts
(braket
id|i
)braket
dot
id|name
)paren
suffix:semicolon
r_if
c_cond
(paren
id|mnt-&gt;flags
op_amp
id|SMB_MOUNT_UID
)paren
id|seq_printf
c_func
(paren
id|s
comma
l_string|&quot;,uid=%d&quot;
comma
id|mnt-&gt;uid
)paren
suffix:semicolon
r_if
c_cond
(paren
id|mnt-&gt;flags
op_amp
id|SMB_MOUNT_GID
)paren
id|seq_printf
c_func
(paren
id|s
comma
l_string|&quot;,gid=%d&quot;
comma
id|mnt-&gt;gid
)paren
suffix:semicolon
r_if
c_cond
(paren
id|mnt-&gt;mounted_uid
op_ne
l_int|0
)paren
id|seq_printf
c_func
(paren
id|s
comma
l_string|&quot;,mounted_uid=%d&quot;
comma
id|mnt-&gt;mounted_uid
)paren
suffix:semicolon
multiline_comment|/* &n;&t; * Defaults for file_mode and dir_mode are unknown to us; they&n;&t; * depend on the current umask of the user doing the mount.&n;&t; */
r_if
c_cond
(paren
id|mnt-&gt;flags
op_amp
id|SMB_MOUNT_FMODE
)paren
id|seq_printf
c_func
(paren
id|s
comma
l_string|&quot;,file_mode=%04o&quot;
comma
id|mnt-&gt;file_mode
op_amp
id|S_IRWXUGO
)paren
suffix:semicolon
r_if
c_cond
(paren
id|mnt-&gt;flags
op_amp
id|SMB_MOUNT_DMODE
)paren
id|seq_printf
c_func
(paren
id|s
comma
l_string|&quot;,dir_mode=%04o&quot;
comma
id|mnt-&gt;dir_mode
op_amp
id|S_IRWXUGO
)paren
suffix:semicolon
r_if
c_cond
(paren
id|strcmp
c_func
(paren
id|mnt-&gt;codepage.local_name
comma
id|CONFIG_NLS_DEFAULT
)paren
)paren
id|seq_printf
c_func
(paren
id|s
comma
l_string|&quot;,iocharset=%s&quot;
comma
id|mnt-&gt;codepage.local_name
)paren
suffix:semicolon
r_if
c_cond
(paren
id|strcmp
c_func
(paren
id|mnt-&gt;codepage.remote_name
comma
id|SMB_NLS_REMOTE
)paren
)paren
id|seq_printf
c_func
(paren
id|s
comma
l_string|&quot;,codepage=%s&quot;
comma
id|mnt-&gt;codepage.remote_name
)paren
suffix:semicolon
r_if
c_cond
(paren
id|mnt-&gt;ttl
op_ne
id|SMB_TTL_DEFAULT
)paren
id|seq_printf
c_func
(paren
id|s
comma
l_string|&quot;,ttl=%d&quot;
comma
id|mnt-&gt;ttl
)paren
suffix:semicolon
r_return
l_int|0
suffix:semicolon
)brace
r_static
r_void
DECL|function|smb_unload_nls
id|smb_unload_nls
c_func
(paren
r_struct
id|smb_sb_info
op_star
id|server
)paren
(brace
r_if
c_cond
(paren
id|server-&gt;remote_nls
)paren
(brace
id|unload_nls
c_func
(paren
id|server-&gt;remote_nls
)paren
suffix:semicolon
id|server-&gt;remote_nls
op_assign
l_int|NULL
suffix:semicolon
)brace
r_if
c_cond
(paren
id|server-&gt;local_nls
)paren
(brace
id|unload_nls
c_func
(paren
id|server-&gt;local_nls
)paren
suffix:semicolon
id|server-&gt;local_nls
op_assign
l_int|NULL
suffix:semicolon
)brace
)brace
r_static
r_void
DECL|function|smb_put_super
id|smb_put_super
c_func
(paren
r_struct
id|super_block
op_star
id|sb
)paren
(brace
r_struct
id|smb_sb_info
op_star
id|server
op_assign
id|SMB_SB
c_func
(paren
id|sb
)paren
suffix:semicolon
id|smb_lock_server
c_func
(paren
id|server
)paren
suffix:semicolon
id|server-&gt;state
op_assign
id|CONN_INVALID
suffix:semicolon
id|smbiod_unregister_server
c_func
(paren
id|server
)paren
suffix:semicolon
id|smb_close_socket
c_func
(paren
id|server
)paren
suffix:semicolon
r_if
c_cond
(paren
id|server-&gt;conn_pid
)paren
id|kill_proc
c_func
(paren
id|server-&gt;conn_pid
comma
id|SIGTERM
comma
l_int|1
)paren
suffix:semicolon
id|smb_kfree
c_func
(paren
id|server-&gt;ops
)paren
suffix:semicolon
id|smb_unload_nls
c_func
(paren
id|server
)paren
suffix:semicolon
id|sb-&gt;s_fs_info
op_assign
l_int|NULL
suffix:semicolon
id|smb_unlock_server
c_func
(paren
id|server
)paren
suffix:semicolon
id|smb_kfree
c_func
(paren
id|server
)paren
suffix:semicolon
)brace
DECL|function|smb_fill_super
r_int
id|smb_fill_super
c_func
(paren
r_struct
id|super_block
op_star
id|sb
comma
r_void
op_star
id|raw_data
comma
r_int
id|silent
)paren
(brace
r_struct
id|smb_sb_info
op_star
id|server
suffix:semicolon
r_struct
id|smb_mount_data_kernel
op_star
id|mnt
suffix:semicolon
r_struct
id|smb_mount_data
op_star
id|oldmnt
suffix:semicolon
r_struct
id|inode
op_star
id|root_inode
suffix:semicolon
r_struct
id|smb_fattr
id|root
suffix:semicolon
r_int
id|ver
suffix:semicolon
r_void
op_star
id|mem
suffix:semicolon
r_if
c_cond
(paren
op_logical_neg
id|raw_data
)paren
r_goto
id|out_no_data
suffix:semicolon
id|oldmnt
op_assign
(paren
r_struct
id|smb_mount_data
op_star
)paren
id|raw_data
suffix:semicolon
id|ver
op_assign
id|oldmnt-&gt;version
suffix:semicolon
r_if
c_cond
(paren
id|ver
op_ne
id|SMB_MOUNT_OLDVERSION
op_logical_and
id|cpu_to_be32
c_func
(paren
id|ver
)paren
op_ne
id|SMB_MOUNT_ASCII
)paren
r_goto
id|out_wrong_data
suffix:semicolon
id|sb-&gt;s_flags
op_or_assign
id|MS_NODIRATIME
suffix:semicolon
id|sb-&gt;s_blocksize
op_assign
l_int|1024
suffix:semicolon
multiline_comment|/* Eh...  Is this correct? */
id|sb-&gt;s_blocksize_bits
op_assign
l_int|10
suffix:semicolon
id|sb-&gt;s_magic
op_assign
id|SMB_SUPER_MAGIC
suffix:semicolon
id|sb-&gt;s_op
op_assign
op_amp
id|smb_sops
suffix:semicolon
id|server
op_assign
id|smb_kmalloc
c_func
(paren
r_sizeof
(paren
r_struct
id|smb_sb_info
)paren
comma
id|GFP_KERNEL
)paren
suffix:semicolon
r_if
c_cond
(paren
op_logical_neg
id|server
)paren
r_goto
id|out_no_server
suffix:semicolon
id|sb-&gt;s_fs_info
op_assign
id|server
suffix:semicolon
id|memset
c_func
(paren
id|server
comma
l_int|0
comma
r_sizeof
(paren
r_struct
id|smb_sb_info
)paren
)paren
suffix:semicolon
id|server-&gt;super_block
op_assign
id|sb
suffix:semicolon
id|server-&gt;mnt
op_assign
l_int|NULL
suffix:semicolon
id|server-&gt;sock_file
op_assign
l_int|NULL
suffix:semicolon
id|init_waitqueue_head
c_func
(paren
op_amp
id|server-&gt;conn_wq
)paren
suffix:semicolon
id|init_MUTEX
c_func
(paren
op_amp
id|server-&gt;sem
)paren
suffix:semicolon
id|INIT_LIST_HEAD
c_func
(paren
op_amp
id|server-&gt;entry
)paren
suffix:semicolon
id|INIT_LIST_HEAD
c_func
(paren
op_amp
id|server-&gt;xmitq
)paren
suffix:semicolon
id|INIT_LIST_HEAD
c_func
(paren
op_amp
id|server-&gt;recvq
)paren
suffix:semicolon
id|server-&gt;conn_error
op_assign
l_int|0
suffix:semicolon
id|server-&gt;conn_pid
op_assign
l_int|0
suffix:semicolon
id|server-&gt;state
op_assign
id|CONN_INVALID
suffix:semicolon
multiline_comment|/* no connection yet */
id|server-&gt;generation
op_assign
l_int|0
suffix:semicolon
multiline_comment|/* Allocate the global temp buffer and some superblock helper structs */
multiline_comment|/* FIXME: move these to the smb_sb_info struct */
id|VERBOSE
c_func
(paren
l_string|&quot;alloc chunk = %d&bslash;n&quot;
comma
r_sizeof
(paren
r_struct
id|smb_ops
)paren
op_plus
r_sizeof
(paren
r_struct
id|smb_mount_data_kernel
)paren
)paren
suffix:semicolon
id|mem
op_assign
id|smb_kmalloc
c_func
(paren
r_sizeof
(paren
r_struct
id|smb_ops
)paren
op_plus
r_sizeof
(paren
r_struct
id|smb_mount_data_kernel
)paren
comma
id|GFP_KERNEL
)paren
suffix:semicolon
r_if
c_cond
(paren
op_logical_neg
id|mem
)paren
r_goto
id|out_no_mem
suffix:semicolon
id|server-&gt;ops
op_assign
id|mem
suffix:semicolon
id|smb_install_null_ops
c_func
(paren
id|server-&gt;ops
)paren
suffix:semicolon
id|server-&gt;mnt
op_assign
id|mem
op_plus
r_sizeof
(paren
r_struct
id|smb_ops
)paren
suffix:semicolon
multiline_comment|/* Setup NLS stuff */
id|server-&gt;remote_nls
op_assign
l_int|NULL
suffix:semicolon
id|server-&gt;local_nls
op_assign
l_int|NULL
suffix:semicolon
id|mnt
op_assign
id|server-&gt;mnt
suffix:semicolon
id|memset
c_func
(paren
id|mnt
comma
l_int|0
comma
r_sizeof
(paren
r_struct
id|smb_mount_data_kernel
)paren
)paren
suffix:semicolon
id|strlcpy
c_func
(paren
id|mnt-&gt;codepage.local_name
comma
id|CONFIG_NLS_DEFAULT
comma
id|SMB_NLS_MAXNAMELEN
)paren
suffix:semicolon
id|strlcpy
c_func
(paren
id|mnt-&gt;codepage.remote_name
comma
id|SMB_NLS_REMOTE
comma
id|SMB_NLS_MAXNAMELEN
)paren
suffix:semicolon
id|mnt-&gt;ttl
op_assign
id|SMB_TTL_DEFAULT
suffix:semicolon
r_if
c_cond
(paren
id|ver
op_eq
id|SMB_MOUNT_OLDVERSION
)paren
(brace
id|mnt-&gt;version
op_assign
id|oldmnt-&gt;version
suffix:semicolon
id|SET_UID
c_func
(paren
id|mnt-&gt;uid
comma
id|oldmnt-&gt;uid
)paren
suffix:semicolon
id|SET_GID
c_func
(paren
id|mnt-&gt;gid
comma
id|oldmnt-&gt;gid
)paren
suffix:semicolon
id|mnt-&gt;file_mode
op_assign
(paren
id|oldmnt-&gt;file_mode
op_amp
id|S_IRWXUGO
)paren
op_or
id|S_IFREG
suffix:semicolon
id|mnt-&gt;dir_mode
op_assign
(paren
id|oldmnt-&gt;dir_mode
op_amp
id|S_IRWXUGO
)paren
op_or
id|S_IFDIR
suffix:semicolon
id|mnt-&gt;flags
op_assign
(paren
id|oldmnt-&gt;file_mode
op_rshift
l_int|9
)paren
op_or
id|SMB_MOUNT_UID
op_or
id|SMB_MOUNT_GID
op_or
id|SMB_MOUNT_FMODE
op_or
id|SMB_MOUNT_DMODE
suffix:semicolon
)brace
r_else
(brace
id|mnt-&gt;file_mode
op_assign
id|S_IRWXU
op_or
id|S_IRGRP
op_or
id|S_IXGRP
op_or
id|S_IROTH
op_or
id|S_IXOTH
op_or
id|S_IFREG
suffix:semicolon
id|mnt-&gt;dir_mode
op_assign
id|S_IRWXU
op_or
id|S_IRGRP
op_or
id|S_IXGRP
op_or
id|S_IROTH
op_or
id|S_IXOTH
op_or
id|S_IFDIR
suffix:semicolon
r_if
c_cond
(paren
id|parse_options
c_func
(paren
id|mnt
comma
id|raw_data
)paren
)paren
r_goto
id|out_bad_option
suffix:semicolon
)brace
id|mnt-&gt;mounted_uid
op_assign
id|current-&gt;uid
suffix:semicolon
id|smb_setcodepage
c_func
(paren
id|server
comma
op_amp
id|mnt-&gt;codepage
)paren
suffix:semicolon
multiline_comment|/*&n;&t; * Display the enabled options&n;&t; * Note: smb_proc_getattr uses these in 2.4 (but was changed in 2.2)&n;&t; */
r_if
c_cond
(paren
id|mnt-&gt;flags
op_amp
id|SMB_MOUNT_OLDATTR
)paren
id|printk
c_func
(paren
l_string|&quot;SMBFS: Using core getattr (Win 95 speedup)&bslash;n&quot;
)paren
suffix:semicolon
r_else
r_if
c_cond
(paren
id|mnt-&gt;flags
op_amp
id|SMB_MOUNT_DIRATTR
)paren
id|printk
c_func
(paren
l_string|&quot;SMBFS: Using dir ff getattr&bslash;n&quot;
)paren
suffix:semicolon
r_if
c_cond
(paren
id|smbiod_register_server
c_func
(paren
id|server
)paren
OL
l_int|0
)paren
(brace
id|printk
c_func
(paren
id|KERN_ERR
l_string|&quot;smbfs: failed to start smbiod&bslash;n&quot;
)paren
suffix:semicolon
r_goto
id|out_no_smbiod
suffix:semicolon
)brace
multiline_comment|/*&n;&t; * Keep the super block locked while we get the root inode.&n;&t; */
id|smb_init_root_dirent
c_func
(paren
id|server
comma
op_amp
id|root
)paren
suffix:semicolon
id|root_inode
op_assign
id|smb_iget
c_func
(paren
id|sb
comma
op_amp
id|root
)paren
suffix:semicolon
r_if
c_cond
(paren
op_logical_neg
id|root_inode
)paren
r_goto
id|out_no_root
suffix:semicolon
id|sb-&gt;s_root
op_assign
id|d_alloc_root
c_func
(paren
id|root_inode
)paren
suffix:semicolon
r_if
c_cond
(paren
op_logical_neg
id|sb-&gt;s_root
)paren
r_goto
id|out_no_root
suffix:semicolon
id|smb_new_dentry
c_func
(paren
id|sb-&gt;s_root
)paren
suffix:semicolon
r_return
l_int|0
suffix:semicolon
id|out_no_root
suffix:colon
id|iput
c_func
(paren
id|root_inode
)paren
suffix:semicolon
id|out_no_smbiod
suffix:colon
id|smb_unload_nls
c_func
(paren
id|server
)paren
suffix:semicolon
id|out_bad_option
suffix:colon
id|smb_kfree
c_func
(paren
id|mem
)paren
suffix:semicolon
id|out_no_mem
suffix:colon
r_if
c_cond
(paren
op_logical_neg
id|server-&gt;mnt
)paren
id|printk
c_func
(paren
id|KERN_ERR
l_string|&quot;smb_fill_super: allocation failure&bslash;n&quot;
)paren
suffix:semicolon
id|sb-&gt;s_fs_info
op_assign
l_int|NULL
suffix:semicolon
id|smb_kfree
c_func
(paren
id|server
)paren
suffix:semicolon
r_goto
id|out_fail
suffix:semicolon
id|out_wrong_data
suffix:colon
id|printk
c_func
(paren
id|KERN_ERR
l_string|&quot;smbfs: mount_data version %d is not supported&bslash;n&quot;
comma
id|ver
)paren
suffix:semicolon
r_goto
id|out_fail
suffix:semicolon
id|out_no_data
suffix:colon
id|printk
c_func
(paren
id|KERN_ERR
l_string|&quot;smb_fill_super: missing data argument&bslash;n&quot;
)paren
suffix:semicolon
id|out_fail
suffix:colon
r_return
op_minus
id|EINVAL
suffix:semicolon
id|out_no_server
suffix:colon
id|printk
c_func
(paren
id|KERN_ERR
l_string|&quot;smb_fill_super: cannot allocate struct smb_sb_info&bslash;n&quot;
)paren
suffix:semicolon
r_return
op_minus
id|ENOMEM
suffix:semicolon
)brace
r_static
r_int
DECL|function|smb_statfs
id|smb_statfs
c_func
(paren
r_struct
id|super_block
op_star
id|sb
comma
r_struct
id|kstatfs
op_star
id|buf
)paren
(brace
r_int
id|result
suffix:semicolon
id|lock_kernel
c_func
(paren
)paren
suffix:semicolon
id|result
op_assign
id|smb_proc_dskattr
c_func
(paren
id|sb
comma
id|buf
)paren
suffix:semicolon
id|unlock_kernel
c_func
(paren
)paren
suffix:semicolon
id|buf-&gt;f_type
op_assign
id|SMB_SUPER_MAGIC
suffix:semicolon
id|buf-&gt;f_namelen
op_assign
id|SMB_MAXPATHLEN
suffix:semicolon
r_return
id|result
suffix:semicolon
)brace
DECL|function|smb_getattr
r_int
id|smb_getattr
c_func
(paren
r_struct
id|vfsmount
op_star
id|mnt
comma
r_struct
id|dentry
op_star
id|dentry
comma
r_struct
id|kstat
op_star
id|stat
)paren
(brace
r_int
id|err
op_assign
id|smb_revalidate_inode
c_func
(paren
id|dentry
)paren
suffix:semicolon
r_if
c_cond
(paren
op_logical_neg
id|err
)paren
id|generic_fillattr
c_func
(paren
id|dentry-&gt;d_inode
comma
id|stat
)paren
suffix:semicolon
r_return
id|err
suffix:semicolon
)brace
r_int
DECL|function|smb_notify_change
id|smb_notify_change
c_func
(paren
r_struct
id|dentry
op_star
id|dentry
comma
r_struct
id|iattr
op_star
id|attr
)paren
(brace
r_struct
id|inode
op_star
id|inode
op_assign
id|dentry-&gt;d_inode
suffix:semicolon
r_struct
id|smb_sb_info
op_star
id|server
op_assign
id|server_from_dentry
c_func
(paren
id|dentry
)paren
suffix:semicolon
r_int
r_int
id|mask
op_assign
(paren
id|S_IFREG
op_or
id|S_IFDIR
op_or
id|S_IRWXUGO
)paren
suffix:semicolon
r_int
id|error
comma
id|changed
comma
id|refresh
op_assign
l_int|0
suffix:semicolon
r_struct
id|smb_fattr
id|fattr
suffix:semicolon
id|lock_kernel
c_func
(paren
)paren
suffix:semicolon
id|error
op_assign
id|smb_revalidate_inode
c_func
(paren
id|dentry
)paren
suffix:semicolon
r_if
c_cond
(paren
id|error
)paren
r_goto
id|out
suffix:semicolon
r_if
c_cond
(paren
(paren
id|error
op_assign
id|inode_change_ok
c_func
(paren
id|inode
comma
id|attr
)paren
)paren
OL
l_int|0
)paren
r_goto
id|out
suffix:semicolon
id|error
op_assign
op_minus
id|EPERM
suffix:semicolon
r_if
c_cond
(paren
(paren
id|attr-&gt;ia_valid
op_amp
id|ATTR_UID
)paren
op_logical_and
(paren
id|attr-&gt;ia_uid
op_ne
id|server-&gt;mnt-&gt;uid
)paren
)paren
r_goto
id|out
suffix:semicolon
r_if
c_cond
(paren
(paren
id|attr-&gt;ia_valid
op_amp
id|ATTR_GID
)paren
op_logical_and
(paren
id|attr-&gt;ia_uid
op_ne
id|server-&gt;mnt-&gt;gid
)paren
)paren
r_goto
id|out
suffix:semicolon
r_if
c_cond
(paren
(paren
id|attr-&gt;ia_valid
op_amp
id|ATTR_MODE
)paren
op_logical_and
(paren
id|attr-&gt;ia_mode
op_amp
op_complement
id|mask
)paren
)paren
r_goto
id|out
suffix:semicolon
r_if
c_cond
(paren
(paren
id|attr-&gt;ia_valid
op_amp
id|ATTR_SIZE
)paren
op_ne
l_int|0
)paren
(brace
id|VERBOSE
c_func
(paren
l_string|&quot;changing %s/%s, old size=%ld, new size=%ld&bslash;n&quot;
comma
id|DENTRY_PATH
c_func
(paren
id|dentry
)paren
comma
(paren
r_int
)paren
id|inode-&gt;i_size
comma
(paren
r_int
)paren
id|attr-&gt;ia_size
)paren
suffix:semicolon
id|filemap_fdatawrite
c_func
(paren
id|inode-&gt;i_mapping
)paren
suffix:semicolon
id|filemap_fdatawait
c_func
(paren
id|inode-&gt;i_mapping
)paren
suffix:semicolon
id|error
op_assign
id|smb_open
c_func
(paren
id|dentry
comma
id|O_WRONLY
)paren
suffix:semicolon
r_if
c_cond
(paren
id|error
)paren
r_goto
id|out
suffix:semicolon
id|error
op_assign
id|server-&gt;ops
op_member_access_from_pointer
id|truncate
c_func
(paren
id|inode
comma
id|attr-&gt;ia_size
)paren
suffix:semicolon
r_if
c_cond
(paren
id|error
)paren
r_goto
id|out
suffix:semicolon
id|error
op_assign
id|vmtruncate
c_func
(paren
id|inode
comma
id|attr-&gt;ia_size
)paren
suffix:semicolon
r_if
c_cond
(paren
id|error
)paren
r_goto
id|out
suffix:semicolon
id|refresh
op_assign
l_int|1
suffix:semicolon
)brace
r_if
c_cond
(paren
id|server-&gt;opt.capabilities
op_amp
id|SMB_CAP_UNIX
)paren
(brace
multiline_comment|/* For now we don&squot;t want to set the size with setattr_unix */
id|attr-&gt;ia_valid
op_and_assign
op_complement
id|ATTR_SIZE
suffix:semicolon
multiline_comment|/* FIXME: only call if we actually want to set something? */
id|error
op_assign
id|smb_proc_setattr_unix
c_func
(paren
id|dentry
comma
id|attr
comma
l_int|0
comma
l_int|0
)paren
suffix:semicolon
r_if
c_cond
(paren
op_logical_neg
id|error
)paren
id|refresh
op_assign
l_int|1
suffix:semicolon
r_goto
id|out
suffix:semicolon
)brace
multiline_comment|/*&n;&t; * Initialize the fattr and check for changed fields.&n;&t; * Note: CTIME under SMB is creation time rather than&n;&t; * change time, so we don&squot;t attempt to change it.&n;&t; */
id|smb_get_inode_attr
c_func
(paren
id|inode
comma
op_amp
id|fattr
)paren
suffix:semicolon
id|changed
op_assign
l_int|0
suffix:semicolon
r_if
c_cond
(paren
(paren
id|attr-&gt;ia_valid
op_amp
id|ATTR_MTIME
)paren
op_ne
l_int|0
)paren
(brace
id|fattr.f_mtime
op_assign
id|attr-&gt;ia_mtime
suffix:semicolon
id|changed
op_assign
l_int|1
suffix:semicolon
)brace
r_if
c_cond
(paren
(paren
id|attr-&gt;ia_valid
op_amp
id|ATTR_ATIME
)paren
op_ne
l_int|0
)paren
(brace
id|fattr.f_atime
op_assign
id|attr-&gt;ia_atime
suffix:semicolon
multiline_comment|/* Earlier protocols don&squot;t have an access time */
r_if
c_cond
(paren
id|server-&gt;opt.protocol
op_ge
id|SMB_PROTOCOL_LANMAN2
)paren
id|changed
op_assign
l_int|1
suffix:semicolon
)brace
r_if
c_cond
(paren
id|changed
)paren
(brace
id|error
op_assign
id|smb_proc_settime
c_func
(paren
id|dentry
comma
op_amp
id|fattr
)paren
suffix:semicolon
r_if
c_cond
(paren
id|error
)paren
r_goto
id|out
suffix:semicolon
id|refresh
op_assign
l_int|1
suffix:semicolon
)brace
multiline_comment|/*&n;&t; * Check for mode changes ... we&squot;re extremely limited in&n;&t; * what can be set for SMB servers: just the read-only bit.&n;&t; */
r_if
c_cond
(paren
(paren
id|attr-&gt;ia_valid
op_amp
id|ATTR_MODE
)paren
op_ne
l_int|0
)paren
(brace
id|VERBOSE
c_func
(paren
l_string|&quot;%s/%s mode change, old=%x, new=%x&bslash;n&quot;
comma
id|DENTRY_PATH
c_func
(paren
id|dentry
)paren
comma
id|fattr.f_mode
comma
id|attr-&gt;ia_mode
)paren
suffix:semicolon
id|changed
op_assign
l_int|0
suffix:semicolon
r_if
c_cond
(paren
id|attr-&gt;ia_mode
op_amp
id|S_IWUSR
)paren
(brace
r_if
c_cond
(paren
id|fattr.attr
op_amp
id|aRONLY
)paren
(brace
id|fattr.attr
op_and_assign
op_complement
id|aRONLY
suffix:semicolon
id|changed
op_assign
l_int|1
suffix:semicolon
)brace
)brace
r_else
(brace
r_if
c_cond
(paren
op_logical_neg
(paren
id|fattr.attr
op_amp
id|aRONLY
)paren
)paren
(brace
id|fattr.attr
op_or_assign
id|aRONLY
suffix:semicolon
id|changed
op_assign
l_int|1
suffix:semicolon
)brace
)brace
r_if
c_cond
(paren
id|changed
)paren
(brace
id|error
op_assign
id|smb_proc_setattr
c_func
(paren
id|dentry
comma
op_amp
id|fattr
)paren
suffix:semicolon
r_if
c_cond
(paren
id|error
)paren
r_goto
id|out
suffix:semicolon
id|refresh
op_assign
l_int|1
suffix:semicolon
)brace
)brace
id|error
op_assign
l_int|0
suffix:semicolon
id|out
suffix:colon
r_if
c_cond
(paren
id|refresh
)paren
id|smb_refresh_inode
c_func
(paren
id|dentry
)paren
suffix:semicolon
id|unlock_kernel
c_func
(paren
)paren
suffix:semicolon
r_return
id|error
suffix:semicolon
)brace
macro_line|#ifdef DEBUG_SMB_MALLOC
DECL|variable|smb_malloced
r_int
id|smb_malloced
suffix:semicolon
DECL|variable|smb_current_kmalloced
r_int
id|smb_current_kmalloced
suffix:semicolon
DECL|variable|smb_current_vmalloced
r_int
id|smb_current_vmalloced
suffix:semicolon
macro_line|#endif
DECL|function|smb_get_sb
r_static
r_struct
id|super_block
op_star
id|smb_get_sb
c_func
(paren
r_struct
id|file_system_type
op_star
id|fs_type
comma
r_int
id|flags
comma
r_const
r_char
op_star
id|dev_name
comma
r_void
op_star
id|data
)paren
(brace
r_return
id|get_sb_nodev
c_func
(paren
id|fs_type
comma
id|flags
comma
id|data
comma
id|smb_fill_super
)paren
suffix:semicolon
)brace
DECL|variable|smb_fs_type
r_static
r_struct
id|file_system_type
id|smb_fs_type
op_assign
(brace
dot
id|owner
op_assign
id|THIS_MODULE
comma
dot
id|name
op_assign
l_string|&quot;smbfs&quot;
comma
dot
id|get_sb
op_assign
id|smb_get_sb
comma
dot
id|kill_sb
op_assign
id|kill_anon_super
comma
dot
id|fs_flags
op_assign
id|FS_BINARY_MOUNTDATA
comma
)brace
suffix:semicolon
DECL|function|init_smb_fs
r_static
r_int
id|__init
id|init_smb_fs
c_func
(paren
r_void
)paren
(brace
r_int
id|err
suffix:semicolon
id|DEBUG1
c_func
(paren
l_string|&quot;registering ...&bslash;n&quot;
)paren
suffix:semicolon
macro_line|#ifdef DEBUG_SMB_MALLOC
id|smb_malloced
op_assign
l_int|0
suffix:semicolon
id|smb_current_kmalloced
op_assign
l_int|0
suffix:semicolon
id|smb_current_vmalloced
op_assign
l_int|0
suffix:semicolon
macro_line|#endif
id|err
op_assign
id|init_inodecache
c_func
(paren
)paren
suffix:semicolon
r_if
c_cond
(paren
id|err
)paren
r_goto
id|out_inode
suffix:semicolon
id|err
op_assign
id|smb_init_request_cache
c_func
(paren
)paren
suffix:semicolon
r_if
c_cond
(paren
id|err
)paren
r_goto
id|out_request
suffix:semicolon
id|err
op_assign
id|register_filesystem
c_func
(paren
op_amp
id|smb_fs_type
)paren
suffix:semicolon
r_if
c_cond
(paren
id|err
)paren
r_goto
id|out
suffix:semicolon
r_return
l_int|0
suffix:semicolon
id|out
suffix:colon
id|smb_destroy_request_cache
c_func
(paren
)paren
suffix:semicolon
id|out_request
suffix:colon
id|destroy_inodecache
c_func
(paren
)paren
suffix:semicolon
id|out_inode
suffix:colon
r_return
id|err
suffix:semicolon
)brace
DECL|function|exit_smb_fs
r_static
r_void
id|__exit
id|exit_smb_fs
c_func
(paren
r_void
)paren
(brace
id|DEBUG1
c_func
(paren
l_string|&quot;unregistering ...&bslash;n&quot;
)paren
suffix:semicolon
id|unregister_filesystem
c_func
(paren
op_amp
id|smb_fs_type
)paren
suffix:semicolon
id|smb_destroy_request_cache
c_func
(paren
)paren
suffix:semicolon
id|destroy_inodecache
c_func
(paren
)paren
suffix:semicolon
macro_line|#ifdef DEBUG_SMB_MALLOC
id|printk
c_func
(paren
id|KERN_DEBUG
l_string|&quot;smb_malloced: %d&bslash;n&quot;
comma
id|smb_malloced
)paren
suffix:semicolon
id|printk
c_func
(paren
id|KERN_DEBUG
l_string|&quot;smb_current_kmalloced: %d&bslash;n&quot;
comma
id|smb_current_kmalloced
)paren
suffix:semicolon
id|printk
c_func
(paren
id|KERN_DEBUG
l_string|&quot;smb_current_vmalloced: %d&bslash;n&quot;
comma
id|smb_current_vmalloced
)paren
suffix:semicolon
macro_line|#endif
)brace
id|module_init
c_func
(paren
id|init_smb_fs
)paren
id|module_exit
c_func
(paren
id|exit_smb_fs
)paren
id|MODULE_LICENSE
c_func
(paren
l_string|&quot;GPL&quot;
)paren
suffix:semicolon
eof
