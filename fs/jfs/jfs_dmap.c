...
(paren
id|KERN_ERR
l_string|&quot;dbUnmount: db_active[%d] = %d&bslash;n&quot;
comma
id|i
comma
id|atomic_read
...
multiline_comment|/*&n; * NAME:&t;dbNextAG()&n; *&n; * FUNCTION:    find the preferred allocation group for new allocations.&n; *&n; *&t;&t;Within the allocation groups, we maintain a preferred&n; *&t;&t;allocation group which consists of a group with at least&n; *&t;&t;average free space.  It is the preferred group that we target&n; *&t;&t;new inode allocation towards.  The tie-in between inode&n; *&t;&t;allocation and block allocation occurs as we allocate the&n; *&t;&t;first (data) block of an inode and specify the inode (block)&n; *&t;&t;as the allocation hint for this block.&n; *&n; *&t;&t;We try to avoid having more than one open file growing in&n; *&t;&t;an allocation group, as this will lead to fragmentation.&n; *&t;&t;This differs from the old OS/2 method of trying to keep&n; *&t;&t;empty ags around for large allocations.&n; *&n; * PARAMETERS:&n; *      ipbmap&t;-  pointer to in-core inode for the block map.&n; *&n; * RETURN VALUES:&n; *      the preferred allocation group number.&n; */
...
suffix:semicolon
multiline_comment|/* determine the average number of free blocks within the ags. */
id|avgfree
op_assign
(paren
id|u32
...
id|bmp-&gt;db_numag
suffix:semicolon
multiline_comment|/*&n;&t; * if the current preferred ag does not have an active allocator&n;&t; * and has at least average freespace, return it&n;&t; */
id|agpref
op_assign
id|bmp-&gt;db_agpref
suffix:semicolon
...
multiline_comment|/* open file is currently growing in this ag */
r_continue
suffix:semicolon
r_if
c_cond
...
multiline_comment|/*&n;&t; * If no inactive ag was found with average freespace, use the&n;&t; * next best&n;&t; */
r_if
c_cond
(paren
id|next_best
...
multiline_comment|/*&n;&t; * If no hint, let dbNextAG recommend an allocation group&n;&t; */
r_if
c_cond
(paren
id|hint
...
id|pref_ag
suffix:semicolon
multiline_comment|/* we would like to allocate close to the hint.  adjust the&n;&t; * hint to the block following the hint since the allocators&n;&t; * will start looking for free space starting at this point.&n;&t; */
id|blkno
op_assign
id|hint
op_plus
...
id|bmp-&gt;db_agl2size
suffix:semicolon
multiline_comment|/* check if blkno crosses over into a new allocation group.&n;&t; * if so, check if we should allow allocations within this&n;&t; * allocation group.&n;&t; */
r_if
c_cond
(paren
(paren
...
multiline_comment|/* check if the AG is currenly being written to.&n;&t;&t; * if so, call dbNextAG() to find a non-busy&n;&t;&t; * AG with sufficient free space.&n;&t;&t; */
r_if
c_cond
(paren
id|atomic_read
...
(brace
multiline_comment|/*&n;&t;&t;&t; * Someone else is writing in this allocation&n;&t;&t;&t; * group.  To avoid fragmenting, try another ag&n;&t;&t;&t; */
id|release_metapage
c_func
(paren
id|mp
...
multiline_comment|/* try to satisfy the allocation request with blocks within&n;&t;&t; * the same dmap as the hint.&n;&t;&t; */
r_if
c_cond
(paren
(paren
...
multiline_comment|/* try to satisfy the allocation request with blocks within&n;&t; * the same allocation group as the hint.&n;&t; */
id|IWRITE_LOCK
c_func
(paren
id|ipbmap
...
id|pref_ag
suffix:colon
multiline_comment|/*&n;&t; * Let dbNextAG recommend a preferred allocation group&n;&t; */
id|agno
op_assign
id|dbNextAG
c_func
...
suffix:semicolon
multiline_comment|/* Try to allocate within this allocation group.  if that fails, try to&n;&t; * allocate anywhere in the map.&n;&t; */
r_if
c_cond
(paren
(paren
...
comma
l_string|&quot;dbAllocNext: Corrupt dmap page&quot;
)paren
suffix:semicolon
r_return
op_minus
...
id|bmp-&gt;db_ipbmap-&gt;i_sb
comma
l_string|&quot;dbAllocNear: Corrupt dmap page&quot;
)paren
suffix:semicolon
r_return
op_minus
...
id|bmp-&gt;db_ipbmap-&gt;i_sb
comma
l_string|&quot;dbAllocAG: Corrupt dmapctl page&quot;
)paren
suffix:semicolon
id|release_metapage
c_func
...
id|bmp-&gt;db_ipbmap-&gt;i_sb
comma
l_string|&quot;dbFindCtl: Corrupt dmapctl page&quot;
)paren
suffix:semicolon
id|release_metapage
c_func
...
id|bmp-&gt;db_ipbmap-&gt;i_sb
comma
l_string|&quot;dbAdjCtl: Corrupt dmapctl page&quot;
)paren
suffix:semicolon
id|release_metapage
c_func
