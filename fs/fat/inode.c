multiline_comment|/*&n; *  linux/fs/fat/inode.c&n; *&n; *  Written 1992,1993 by Werner Almesberger&n; *  VFAT extensions by Gordon Chaffee, merged with msdos fs by Henrik Storner&n; *  Rewritten for the constant inumbers support by Al Viro&n; *&n; *  Fixes:&n; *&n; *&t;Max Cohan: Fixed invalid FSINFO offset when info_sector is 0&n; */
macro_line|#include &lt;linux/module.h&gt;
macro_line|#include &lt;linux/init.h&gt;
macro_line|#include &lt;linux/time.h&gt;
macro_line|#include &lt;linux/slab.h&gt;
macro_line|#include &lt;linux/smp_lock.h&gt;
macro_line|#include &lt;linux/seq_file.h&gt;
macro_line|#include &lt;linux/msdos_fs.h&gt;
macro_line|#include &lt;linux/pagemap.h&gt;
macro_line|#include &lt;linux/buffer_head.h&gt;
macro_line|#include &lt;linux/mount.h&gt;
macro_line|#include &lt;linux/vfs.h&gt;
macro_line|#include &lt;linux/parser.h&gt;
macro_line|#include &lt;asm/unaligned.h&gt;
macro_line|#ifndef CONFIG_FAT_DEFAULT_IOCHARSET
multiline_comment|/* if user don&squot;t select VFAT, this is undefined. */
DECL|macro|CONFIG_FAT_DEFAULT_IOCHARSET
mdefine_line|#define CONFIG_FAT_DEFAULT_IOCHARSET&t;&quot;&quot;
macro_line|#endif
DECL|variable|fat_default_codepage
r_static
r_int
id|fat_default_codepage
op_assign
id|CONFIG_FAT_DEFAULT_CODEPAGE
suffix:semicolon
DECL|variable|fat_default_iocharset
r_static
r_char
id|fat_default_iocharset
(braket
)braket
op_assign
id|CONFIG_FAT_DEFAULT_IOCHARSET
suffix:semicolon
DECL|function|fat_add_cluster
r_static
r_int
id|fat_add_cluster
c_func
(paren
r_struct
id|inode
op_star
id|inode
)paren
(brace
r_int
id|err
comma
id|cluster
suffix:semicolon
id|err
op_assign
id|fat_alloc_clusters
c_func
(paren
id|inode
comma
op_amp
id|cluster
comma
l_int|1
)paren
suffix:semicolon
r_if
c_cond
(paren
id|err
)paren
r_return
id|err
suffix:semicolon
multiline_comment|/* FIXME: this cluster should be added after data of this&n;&t; * cluster is writed */
id|err
op_assign
id|fat_chain_add
c_func
(paren
id|inode
comma
id|cluster
comma
l_int|1
)paren
suffix:semicolon
r_if
c_cond
(paren
id|err
)paren
id|fat_free_clusters
c_func
(paren
id|inode
comma
id|cluster
)paren
suffix:semicolon
r_return
id|err
suffix:semicolon
)brace
DECL|function|fat_get_block
r_static
r_int
id|fat_get_block
c_func
(paren
r_struct
id|inode
op_star
id|inode
comma
id|sector_t
id|iblock
comma
r_struct
id|buffer_head
op_star
id|bh_result
comma
r_int
id|create
)paren
(brace
r_struct
id|super_block
op_star
id|sb
op_assign
id|inode-&gt;i_sb
suffix:semicolon
id|sector_t
id|phys
suffix:semicolon
r_int
id|err
suffix:semicolon
id|err
op_assign
id|fat_bmap
c_func
(paren
id|inode
comma
id|iblock
comma
op_amp
id|phys
)paren
suffix:semicolon
r_if
c_cond
(paren
id|err
)paren
r_return
id|err
suffix:semicolon
r_if
c_cond
(paren
id|phys
)paren
(brace
id|map_bh
c_func
(paren
id|bh_result
comma
id|sb
comma
id|phys
)paren
suffix:semicolon
r_return
l_int|0
suffix:semicolon
)brace
r_if
c_cond
(paren
op_logical_neg
id|create
)paren
r_return
l_int|0
suffix:semicolon
r_if
c_cond
(paren
id|iblock
op_ne
id|MSDOS_I
c_func
(paren
id|inode
)paren
op_member_access_from_pointer
id|mmu_private
op_rshift
id|sb-&gt;s_blocksize_bits
)paren
(brace
id|fat_fs_panic
c_func
(paren
id|sb
comma
l_string|&quot;corrupted file size (i_pos %lld, %lld)&quot;
comma
id|MSDOS_I
c_func
(paren
id|inode
)paren
op_member_access_from_pointer
id|i_pos
comma
id|MSDOS_I
c_func
(paren
id|inode
)paren
op_member_access_from_pointer
id|mmu_private
)paren
suffix:semicolon
r_return
op_minus
id|EIO
suffix:semicolon
)brace
r_if
c_cond
(paren
op_logical_neg
(paren
(paren
r_int
r_int
)paren
id|iblock
op_amp
(paren
id|MSDOS_SB
c_func
(paren
id|sb
)paren
op_member_access_from_pointer
id|sec_per_clus
op_minus
l_int|1
)paren
)paren
)paren
(brace
id|err
op_assign
id|fat_add_cluster
c_func
(paren
id|inode
)paren
suffix:semicolon
r_if
c_cond
(paren
id|err
)paren
r_return
id|err
suffix:semicolon
)brace
id|MSDOS_I
c_func
(paren
id|inode
)paren
op_member_access_from_pointer
id|mmu_private
op_add_assign
id|sb-&gt;s_blocksize
suffix:semicolon
id|err
op_assign
id|fat_bmap
c_func
(paren
id|inode
comma
id|iblock
comma
op_amp
id|phys
)paren
suffix:semicolon
r_if
c_cond
(paren
id|err
)paren
r_return
id|err
suffix:semicolon
r_if
c_cond
(paren
op_logical_neg
id|phys
)paren
id|BUG
c_func
(paren
)paren
suffix:semicolon
id|set_buffer_new
c_func
(paren
id|bh_result
)paren
suffix:semicolon
id|map_bh
c_func
(paren
id|bh_result
comma
id|sb
comma
id|phys
)paren
suffix:semicolon
r_return
l_int|0
suffix:semicolon
)brace
DECL|function|fat_writepage
r_static
r_int
id|fat_writepage
c_func
(paren
r_struct
id|page
op_star
id|page
comma
r_struct
id|writeback_control
op_star
id|wbc
)paren
(brace
r_return
id|block_write_full_page
c_func
(paren
id|page
comma
id|fat_get_block
comma
id|wbc
)paren
suffix:semicolon
)brace
DECL|function|fat_readpage
r_static
r_int
id|fat_readpage
c_func
(paren
r_struct
id|file
op_star
id|file
comma
r_struct
id|page
op_star
id|page
)paren
(brace
r_return
id|block_read_full_page
c_func
(paren
id|page
comma
id|fat_get_block
)paren
suffix:semicolon
)brace
DECL|function|fat_prepare_write
r_static
r_int
id|fat_prepare_write
c_func
(paren
r_struct
id|file
op_star
id|file
comma
r_struct
id|page
op_star
id|page
comma
r_int
id|from
comma
r_int
id|to
)paren
(brace
r_return
id|cont_prepare_write
c_func
(paren
id|page
comma
id|from
comma
id|to
comma
id|fat_get_block
comma
op_amp
id|MSDOS_I
c_func
(paren
id|page-&gt;mapping-&gt;host
)paren
op_member_access_from_pointer
id|mmu_private
)paren
suffix:semicolon
)brace
DECL|function|_fat_bmap
r_static
id|sector_t
id|_fat_bmap
c_func
(paren
r_struct
id|address_space
op_star
id|mapping
comma
id|sector_t
id|block
)paren
(brace
r_return
id|generic_block_bmap
c_func
(paren
id|mapping
comma
id|block
comma
id|fat_get_block
)paren
suffix:semicolon
)brace
DECL|variable|fat_aops
r_static
r_struct
id|address_space_operations
id|fat_aops
op_assign
(brace
dot
id|readpage
op_assign
id|fat_readpage
comma
dot
id|writepage
op_assign
id|fat_writepage
comma
dot
id|sync_page
op_assign
id|block_sync_page
comma
dot
id|prepare_write
op_assign
id|fat_prepare_write
comma
dot
id|commit_write
op_assign
id|generic_commit_write
comma
dot
id|bmap
op_assign
id|_fat_bmap
)brace
suffix:semicolon
multiline_comment|/*&n; * New FAT inode stuff. We do the following:&n; *&t;a) i_ino is constant and has nothing with on-disk location.&n; *&t;b) FAT manages its own cache of directory entries.&n; *&t;c) *This* cache is indexed by on-disk location.&n; *&t;d) inode has an associated directory entry, all right, but&n; *&t;&t;it may be unhashed.&n; *&t;e) currently entries are stored within struct inode. That should&n; *&t;&t;change.&n; *&t;f) we deal with races in the following way:&n; *&t;&t;1. readdir() and lookup() do FAT-dir-cache lookup.&n; *&t;&t;2. rename() unhashes the F-d-c entry and rehashes it in&n; *&t;&t;&t;a new place.&n; *&t;&t;3. unlink() and rmdir() unhash F-d-c entry.&n; *&t;&t;4. fat_write_inode() checks whether the thing is unhashed.&n; *&t;&t;&t;If it is we silently return. If it isn&squot;t we do bread(),&n; *&t;&t;&t;check if the location is still valid and retry if it&n; *&t;&t;&t;isn&squot;t. Otherwise we do changes.&n; *&t;&t;5. Spinlock is used to protect hash/unhash/location check/lookup&n; *&t;&t;6. fat_clear_inode() unhashes the F-d-c entry.&n; *&t;&t;7. lookup() and readdir() do igrab() if they find a F-d-c entry&n; *&t;&t;&t;and consider negative result as cache miss.&n; */
DECL|function|fat_hash_init
r_static
r_void
id|fat_hash_init
c_func
(paren
r_struct
id|super_block
op_star
id|sb
)paren
(brace
r_struct
id|msdos_sb_info
op_star
id|sbi
op_assign
id|MSDOS_SB
c_func
(paren
id|sb
)paren
suffix:semicolon
r_int
id|i
suffix:semicolon
id|spin_lock_init
c_func
(paren
op_amp
id|sbi-&gt;inode_hash_lock
)paren
suffix:semicolon
r_for
c_loop
(paren
id|i
op_assign
l_int|0
suffix:semicolon
id|i
OL
id|FAT_HASH_SIZE
suffix:semicolon
id|i
op_increment
)paren
id|INIT_HLIST_HEAD
c_func
(paren
op_amp
id|sbi-&gt;inode_hashtable
(braket
id|i
)braket
)paren
suffix:semicolon
)brace
DECL|function|fat_hash
r_static
r_inline
r_int
r_int
id|fat_hash
c_func
(paren
r_struct
id|super_block
op_star
id|sb
comma
id|loff_t
id|i_pos
)paren
(brace
r_int
r_int
id|tmp
op_assign
(paren
r_int
r_int
)paren
id|i_pos
op_or
(paren
r_int
r_int
)paren
id|sb
suffix:semicolon
id|tmp
op_assign
id|tmp
op_plus
(paren
id|tmp
op_rshift
id|FAT_HASH_BITS
)paren
op_plus
(paren
id|tmp
op_rshift
id|FAT_HASH_BITS
op_star
l_int|2
)paren
suffix:semicolon
r_return
id|tmp
op_amp
id|FAT_HASH_MASK
suffix:semicolon
)brace
DECL|function|fat_attach
r_void
id|fat_attach
c_func
(paren
r_struct
id|inode
op_star
id|inode
comma
id|loff_t
id|i_pos
)paren
(brace
r_struct
id|super_block
op_star
id|sb
op_assign
id|inode-&gt;i_sb
suffix:semicolon
r_struct
id|msdos_sb_info
op_star
id|sbi
op_assign
id|MSDOS_SB
c_func
(paren
id|sb
)paren
suffix:semicolon
id|spin_lock
c_func
(paren
op_amp
id|sbi-&gt;inode_hash_lock
)paren
suffix:semicolon
id|MSDOS_I
c_func
(paren
id|inode
)paren
op_member_access_from_pointer
id|i_pos
op_assign
id|i_pos
suffix:semicolon
id|hlist_add_head
c_func
(paren
op_amp
id|MSDOS_I
c_func
(paren
id|inode
)paren
op_member_access_from_pointer
id|i_fat_hash
comma
id|sbi-&gt;inode_hashtable
op_plus
id|fat_hash
c_func
(paren
id|sb
comma
id|i_pos
)paren
)paren
suffix:semicolon
id|spin_unlock
c_func
(paren
op_amp
id|sbi-&gt;inode_hash_lock
)paren
suffix:semicolon
)brace
DECL|variable|fat_attach
id|EXPORT_SYMBOL
c_func
(paren
id|fat_attach
)paren
suffix:semicolon
DECL|function|fat_detach
r_void
id|fat_detach
c_func
(paren
r_struct
id|inode
op_star
id|inode
)paren
(brace
r_struct
id|msdos_sb_info
op_star
id|sbi
op_assign
id|MSDOS_SB
c_func
(paren
id|inode-&gt;i_sb
)paren
suffix:semicolon
id|spin_lock
c_func
(paren
op_amp
id|sbi-&gt;inode_hash_lock
)paren
suffix:semicolon
id|MSDOS_I
c_func
(paren
id|inode
)paren
op_member_access_from_pointer
id|i_pos
op_assign
l_int|0
suffix:semicolon
id|hlist_del_init
c_func
(paren
op_amp
id|MSDOS_I
c_func
(paren
id|inode
)paren
op_member_access_from_pointer
id|i_fat_hash
)paren
suffix:semicolon
id|spin_unlock
c_func
(paren
op_amp
id|sbi-&gt;inode_hash_lock
)paren
suffix:semicolon
)brace
DECL|variable|fat_detach
id|EXPORT_SYMBOL
c_func
(paren
id|fat_detach
)paren
suffix:semicolon
DECL|function|fat_iget
r_struct
id|inode
op_star
id|fat_iget
c_func
(paren
r_struct
id|super_block
op_star
id|sb
comma
id|loff_t
id|i_pos
)paren
(brace
r_struct
id|msdos_sb_info
op_star
id|sbi
op_assign
id|MSDOS_SB
c_func
(paren
id|sb
)paren
suffix:semicolon
r_struct
id|hlist_head
op_star
id|head
op_assign
id|sbi-&gt;inode_hashtable
op_plus
id|fat_hash
c_func
(paren
id|sb
comma
id|i_pos
)paren
suffix:semicolon
r_struct
id|hlist_node
op_star
id|_p
suffix:semicolon
r_struct
id|msdos_inode_info
op_star
id|i
suffix:semicolon
r_struct
id|inode
op_star
id|inode
op_assign
l_int|NULL
suffix:semicolon
id|spin_lock
c_func
(paren
op_amp
id|sbi-&gt;inode_hash_lock
)paren
suffix:semicolon
id|hlist_for_each_entry
c_func
(paren
id|i
comma
id|_p
comma
id|head
comma
id|i_fat_hash
)paren
(brace
id|BUG_ON
c_func
(paren
id|i-&gt;vfs_inode.i_sb
op_ne
id|sb
)paren
suffix:semicolon
r_if
c_cond
(paren
id|i-&gt;i_pos
op_ne
id|i_pos
)paren
r_continue
suffix:semicolon
id|inode
op_assign
id|igrab
c_func
(paren
op_amp
id|i-&gt;vfs_inode
)paren
suffix:semicolon
r_if
c_cond
(paren
id|inode
)paren
r_break
suffix:semicolon
)brace
id|spin_unlock
c_func
(paren
op_amp
id|sbi-&gt;inode_hash_lock
)paren
suffix:semicolon
r_return
id|inode
suffix:semicolon
)brace
DECL|function|is_exec
r_static
r_int
id|is_exec
c_func
(paren
r_int
r_char
op_star
id|extension
)paren
(brace
r_int
r_char
op_star
id|exe_extensions
op_assign
l_string|&quot;EXECOMBAT&quot;
comma
op_star
id|walk
suffix:semicolon
r_for
c_loop
(paren
id|walk
op_assign
id|exe_extensions
suffix:semicolon
op_star
id|walk
suffix:semicolon
id|walk
op_add_assign
l_int|3
)paren
r_if
c_cond
(paren
op_logical_neg
id|strncmp
c_func
(paren
id|extension
comma
id|walk
comma
l_int|3
)paren
)paren
r_return
l_int|1
suffix:semicolon
r_return
l_int|0
suffix:semicolon
)brace
DECL|function|fat_calc_dir_size
r_static
r_int
id|fat_calc_dir_size
c_func
(paren
r_struct
id|inode
op_star
id|inode
)paren
(brace
r_struct
id|msdos_sb_info
op_star
id|sbi
op_assign
id|MSDOS_SB
c_func
(paren
id|inode-&gt;i_sb
)paren
suffix:semicolon
r_int
id|ret
comma
id|fclus
comma
id|dclus
suffix:semicolon
id|inode-&gt;i_size
op_assign
l_int|0
suffix:semicolon
r_if
c_cond
(paren
id|MSDOS_I
c_func
(paren
id|inode
)paren
op_member_access_from_pointer
id|i_start
op_eq
l_int|0
)paren
r_return
l_int|0
suffix:semicolon
id|ret
op_assign
id|fat_get_cluster
c_func
(paren
id|inode
comma
id|FAT_ENT_EOF
comma
op_amp
id|fclus
comma
op_amp
id|dclus
)paren
suffix:semicolon
r_if
c_cond
(paren
id|ret
OL
l_int|0
)paren
r_return
id|ret
suffix:semicolon
id|inode-&gt;i_size
op_assign
(paren
id|fclus
op_plus
l_int|1
)paren
op_lshift
id|sbi-&gt;cluster_bits
suffix:semicolon
r_return
l_int|0
suffix:semicolon
)brace
multiline_comment|/* doesn&squot;t deal with root inode */
DECL|function|fat_fill_inode
r_static
r_int
id|fat_fill_inode
c_func
(paren
r_struct
id|inode
op_star
id|inode
comma
r_struct
id|msdos_dir_entry
op_star
id|de
)paren
(brace
r_struct
id|msdos_sb_info
op_star
id|sbi
op_assign
id|MSDOS_SB
c_func
(paren
id|inode-&gt;i_sb
)paren
suffix:semicolon
r_int
id|error
suffix:semicolon
id|MSDOS_I
c_func
(paren
id|inode
)paren
op_member_access_from_pointer
id|i_pos
op_assign
l_int|0
suffix:semicolon
id|inode-&gt;i_uid
op_assign
id|sbi-&gt;options.fs_uid
suffix:semicolon
id|inode-&gt;i_gid
op_assign
id|sbi-&gt;options.fs_gid
suffix:semicolon
id|inode-&gt;i_version
op_increment
suffix:semicolon
id|inode-&gt;i_generation
op_assign
id|get_seconds
c_func
(paren
)paren
suffix:semicolon
r_if
c_cond
(paren
(paren
id|de-&gt;attr
op_amp
id|ATTR_DIR
)paren
op_logical_and
op_logical_neg
id|IS_FREE
c_func
(paren
id|de-&gt;name
)paren
)paren
(brace
id|inode-&gt;i_generation
op_and_assign
op_complement
l_int|1
suffix:semicolon
id|inode-&gt;i_mode
op_assign
id|MSDOS_MKMODE
c_func
(paren
id|de-&gt;attr
comma
id|S_IRWXUGO
op_amp
op_complement
id|sbi-&gt;options.fs_dmask
)paren
op_or
id|S_IFDIR
suffix:semicolon
id|inode-&gt;i_op
op_assign
id|sbi-&gt;dir_ops
suffix:semicolon
id|inode-&gt;i_fop
op_assign
op_amp
id|fat_dir_operations
suffix:semicolon
id|MSDOS_I
c_func
(paren
id|inode
)paren
op_member_access_from_pointer
id|i_start
op_assign
id|le16_to_cpu
c_func
(paren
id|de-&gt;start
)paren
suffix:semicolon
r_if
c_cond
(paren
id|sbi-&gt;fat_bits
op_eq
l_int|32
)paren
id|MSDOS_I
c_func
(paren
id|inode
)paren
op_member_access_from_pointer
id|i_start
op_or_assign
(paren
id|le16_to_cpu
c_func
(paren
id|de-&gt;starthi
)paren
op_lshift
l_int|16
)paren
suffix:semicolon
id|MSDOS_I
c_func
(paren
id|inode
)paren
op_member_access_from_pointer
id|i_logstart
op_assign
id|MSDOS_I
c_func
(paren
id|inode
)paren
op_member_access_from_pointer
id|i_start
suffix:semicolon
id|error
op_assign
id|fat_calc_dir_size
c_func
(paren
id|inode
)paren
suffix:semicolon
r_if
c_cond
(paren
id|error
OL
l_int|0
)paren
r_return
id|error
suffix:semicolon
id|MSDOS_I
c_func
(paren
id|inode
)paren
op_member_access_from_pointer
id|mmu_private
op_assign
id|inode-&gt;i_size
suffix:semicolon
id|inode-&gt;i_nlink
op_assign
id|fat_subdirs
c_func
(paren
id|inode
)paren
suffix:semicolon
)brace
r_else
(brace
multiline_comment|/* not a directory */
id|inode-&gt;i_generation
op_or_assign
l_int|1
suffix:semicolon
id|inode-&gt;i_mode
op_assign
id|MSDOS_MKMODE
c_func
(paren
id|de-&gt;attr
comma
(paren
(paren
id|sbi-&gt;options.showexec
op_logical_and
op_logical_neg
id|is_exec
c_func
(paren
id|de-&gt;ext
)paren
)paren
ques
c_cond
id|S_IRUGO
op_or
id|S_IWUGO
suffix:colon
id|S_IRWXUGO
)paren
op_amp
op_complement
id|sbi-&gt;options.fs_fmask
)paren
op_or
id|S_IFREG
suffix:semicolon
id|MSDOS_I
c_func
(paren
id|inode
)paren
op_member_access_from_pointer
id|i_start
op_assign
id|le16_to_cpu
c_func
(paren
id|de-&gt;start
)paren
suffix:semicolon
r_if
c_cond
(paren
id|sbi-&gt;fat_bits
op_eq
l_int|32
)paren
id|MSDOS_I
c_func
(paren
id|inode
)paren
op_member_access_from_pointer
id|i_start
op_or_assign
(paren
id|le16_to_cpu
c_func
(paren
id|de-&gt;starthi
)paren
op_lshift
l_int|16
)paren
suffix:semicolon
id|MSDOS_I
c_func
(paren
id|inode
)paren
op_member_access_from_pointer
id|i_logstart
op_assign
id|MSDOS_I
c_func
(paren
id|inode
)paren
op_member_access_from_pointer
id|i_start
suffix:semicolon
id|inode-&gt;i_size
op_assign
id|le32_to_cpu
c_func
(paren
id|de-&gt;size
)paren
suffix:semicolon
id|inode-&gt;i_op
op_assign
op_amp
id|fat_file_inode_operations
suffix:semicolon
id|inode-&gt;i_fop
op_assign
op_amp
id|fat_file_operations
suffix:semicolon
id|inode-&gt;i_mapping-&gt;a_ops
op_assign
op_amp
id|fat_aops
suffix:semicolon
id|MSDOS_I
c_func
(paren
id|inode
)paren
op_member_access_from_pointer
id|mmu_private
op_assign
id|inode-&gt;i_size
suffix:semicolon
)brace
r_if
c_cond
(paren
id|de-&gt;attr
op_amp
id|ATTR_SYS
)paren
(brace
r_if
c_cond
(paren
id|sbi-&gt;options.sys_immutable
)paren
id|inode-&gt;i_flags
op_or_assign
id|S_IMMUTABLE
suffix:semicolon
)brace
id|MSDOS_I
c_func
(paren
id|inode
)paren
op_member_access_from_pointer
id|i_attrs
op_assign
id|de-&gt;attr
op_amp
id|ATTR_UNUSED
suffix:semicolon
multiline_comment|/* this is as close to the truth as we can get ... */
id|inode-&gt;i_blksize
op_assign
id|sbi-&gt;cluster_size
suffix:semicolon
id|inode-&gt;i_blocks
op_assign
(paren
(paren
id|inode-&gt;i_size
op_plus
(paren
id|sbi-&gt;cluster_size
op_minus
l_int|1
)paren
)paren
op_amp
op_complement
(paren
(paren
id|loff_t
)paren
id|sbi-&gt;cluster_size
op_minus
l_int|1
)paren
)paren
op_rshift
l_int|9
suffix:semicolon
id|inode-&gt;i_mtime.tv_sec
op_assign
id|inode-&gt;i_atime.tv_sec
op_assign
id|date_dos2unix
c_func
(paren
id|le16_to_cpu
c_func
(paren
id|de-&gt;time
)paren
comma
id|le16_to_cpu
c_func
(paren
id|de-&gt;date
)paren
)paren
suffix:semicolon
id|inode-&gt;i_mtime.tv_nsec
op_assign
id|inode-&gt;i_atime.tv_nsec
op_assign
l_int|0
suffix:semicolon
r_if
c_cond
(paren
id|sbi-&gt;options.isvfat
)paren
(brace
r_int
id|secs
op_assign
id|de-&gt;ctime_cs
op_div
l_int|100
suffix:semicolon
r_int
id|csecs
op_assign
id|de-&gt;ctime_cs
op_mod
l_int|100
suffix:semicolon
id|inode-&gt;i_ctime.tv_sec
op_assign
id|date_dos2unix
c_func
(paren
id|le16_to_cpu
c_func
(paren
id|de-&gt;ctime
)paren
comma
id|le16_to_cpu
c_func
(paren
id|de-&gt;cdate
)paren
)paren
op_plus
id|secs
suffix:semicolon
id|inode-&gt;i_ctime.tv_nsec
op_assign
id|csecs
op_star
l_int|10000000
suffix:semicolon
)brace
r_else
id|inode-&gt;i_ctime
op_assign
id|inode-&gt;i_mtime
suffix:semicolon
r_return
l_int|0
suffix:semicolon
)brace
DECL|function|fat_build_inode
r_struct
id|inode
op_star
id|fat_build_inode
c_func
(paren
r_struct
id|super_block
op_star
id|sb
comma
r_struct
id|msdos_dir_entry
op_star
id|de
comma
id|loff_t
id|i_pos
)paren
(brace
r_struct
id|inode
op_star
id|inode
suffix:semicolon
r_int
id|err
suffix:semicolon
id|inode
op_assign
id|fat_iget
c_func
(paren
id|sb
comma
id|i_pos
)paren
suffix:semicolon
r_if
c_cond
(paren
id|inode
)paren
r_goto
id|out
suffix:semicolon
id|inode
op_assign
id|new_inode
c_func
(paren
id|sb
)paren
suffix:semicolon
r_if
c_cond
(paren
op_logical_neg
id|inode
)paren
(brace
id|inode
op_assign
id|ERR_PTR
c_func
(paren
op_minus
id|ENOMEM
)paren
suffix:semicolon
r_goto
id|out
suffix:semicolon
)brace
id|inode-&gt;i_ino
op_assign
id|iunique
c_func
(paren
id|sb
comma
id|MSDOS_ROOT_INO
)paren
suffix:semicolon
id|inode-&gt;i_version
op_assign
l_int|1
suffix:semicolon
id|err
op_assign
id|fat_fill_inode
c_func
(paren
id|inode
comma
id|de
)paren
suffix:semicolon
r_if
c_cond
(paren
id|err
)paren
(brace
id|iput
c_func
(paren
id|inode
)paren
suffix:semicolon
id|inode
op_assign
id|ERR_PTR
c_func
(paren
id|err
)paren
suffix:semicolon
r_goto
id|out
suffix:semicolon
)brace
id|fat_attach
c_func
(paren
id|inode
comma
id|i_pos
)paren
suffix:semicolon
id|insert_inode_hash
c_func
(paren
id|inode
)paren
suffix:semicolon
id|out
suffix:colon
r_return
id|inode
suffix:semicolon
)brace
DECL|variable|fat_build_inode
id|EXPORT_SYMBOL
c_func
(paren
id|fat_build_inode
)paren
suffix:semicolon
DECL|function|fat_delete_inode
r_static
r_void
id|fat_delete_inode
c_func
(paren
r_struct
id|inode
op_star
id|inode
)paren
(brace
r_if
c_cond
(paren
op_logical_neg
id|is_bad_inode
c_func
(paren
id|inode
)paren
)paren
(brace
id|inode-&gt;i_size
op_assign
l_int|0
suffix:semicolon
id|fat_truncate
c_func
(paren
id|inode
)paren
suffix:semicolon
)brace
id|clear_inode
c_func
(paren
id|inode
)paren
suffix:semicolon
)brace
DECL|function|fat_clear_inode
r_static
r_void
id|fat_clear_inode
c_func
(paren
r_struct
id|inode
op_star
id|inode
)paren
(brace
r_struct
id|msdos_sb_info
op_star
id|sbi
op_assign
id|MSDOS_SB
c_func
(paren
id|inode-&gt;i_sb
)paren
suffix:semicolon
r_if
c_cond
(paren
id|is_bad_inode
c_func
(paren
id|inode
)paren
)paren
r_return
suffix:semicolon
id|lock_kernel
c_func
(paren
)paren
suffix:semicolon
id|spin_lock
c_func
(paren
op_amp
id|sbi-&gt;inode_hash_lock
)paren
suffix:semicolon
id|fat_cache_inval_inode
c_func
(paren
id|inode
)paren
suffix:semicolon
id|hlist_del_init
c_func
(paren
op_amp
id|MSDOS_I
c_func
(paren
id|inode
)paren
op_member_access_from_pointer
id|i_fat_hash
)paren
suffix:semicolon
id|spin_unlock
c_func
(paren
op_amp
id|sbi-&gt;inode_hash_lock
)paren
suffix:semicolon
id|unlock_kernel
c_func
(paren
)paren
suffix:semicolon
)brace
DECL|function|fat_put_super
r_static
r_void
id|fat_put_super
c_func
(paren
r_struct
id|super_block
op_star
id|sb
)paren
(brace
r_struct
id|msdos_sb_info
op_star
id|sbi
op_assign
id|MSDOS_SB
c_func
(paren
id|sb
)paren
suffix:semicolon
r_if
c_cond
(paren
op_logical_neg
(paren
id|sb-&gt;s_flags
op_amp
id|MS_RDONLY
)paren
)paren
id|fat_clusters_flush
c_func
(paren
id|sb
)paren
suffix:semicolon
r_if
c_cond
(paren
id|sbi-&gt;nls_disk
)paren
(brace
id|unload_nls
c_func
(paren
id|sbi-&gt;nls_disk
)paren
suffix:semicolon
id|sbi-&gt;nls_disk
op_assign
l_int|NULL
suffix:semicolon
id|sbi-&gt;options.codepage
op_assign
id|fat_default_codepage
suffix:semicolon
)brace
r_if
c_cond
(paren
id|sbi-&gt;nls_io
)paren
(brace
id|unload_nls
c_func
(paren
id|sbi-&gt;nls_io
)paren
suffix:semicolon
id|sbi-&gt;nls_io
op_assign
l_int|NULL
suffix:semicolon
)brace
r_if
c_cond
(paren
id|sbi-&gt;options.iocharset
op_ne
id|fat_default_iocharset
)paren
(brace
id|kfree
c_func
(paren
id|sbi-&gt;options.iocharset
)paren
suffix:semicolon
id|sbi-&gt;options.iocharset
op_assign
id|fat_default_iocharset
suffix:semicolon
)brace
id|sb-&gt;s_fs_info
op_assign
l_int|NULL
suffix:semicolon
id|kfree
c_func
(paren
id|sbi
)paren
suffix:semicolon
)brace
DECL|variable|fat_inode_cachep
r_static
id|kmem_cache_t
op_star
id|fat_inode_cachep
suffix:semicolon
DECL|function|fat_alloc_inode
r_static
r_struct
id|inode
op_star
id|fat_alloc_inode
c_func
(paren
r_struct
id|super_block
op_star
id|sb
)paren
(brace
r_struct
id|msdos_inode_info
op_star
id|ei
suffix:semicolon
id|ei
op_assign
id|kmem_cache_alloc
c_func
(paren
id|fat_inode_cachep
comma
id|SLAB_KERNEL
)paren
suffix:semicolon
r_if
c_cond
(paren
op_logical_neg
id|ei
)paren
r_return
l_int|NULL
suffix:semicolon
r_return
op_amp
id|ei-&gt;vfs_inode
suffix:semicolon
)brace
DECL|function|fat_destroy_inode
r_static
r_void
id|fat_destroy_inode
c_func
(paren
r_struct
id|inode
op_star
id|inode
)paren
(brace
id|kmem_cache_free
c_func
(paren
id|fat_inode_cachep
comma
id|MSDOS_I
c_func
(paren
id|inode
)paren
)paren
suffix:semicolon
)brace
DECL|function|init_once
r_static
r_void
id|init_once
c_func
(paren
r_void
op_star
id|foo
comma
id|kmem_cache_t
op_star
id|cachep
comma
r_int
r_int
id|flags
)paren
(brace
r_struct
id|msdos_inode_info
op_star
id|ei
op_assign
(paren
r_struct
id|msdos_inode_info
op_star
)paren
id|foo
suffix:semicolon
r_if
c_cond
(paren
(paren
id|flags
op_amp
(paren
id|SLAB_CTOR_VERIFY
op_or
id|SLAB_CTOR_CONSTRUCTOR
)paren
)paren
op_eq
id|SLAB_CTOR_CONSTRUCTOR
)paren
(brace
id|spin_lock_init
c_func
(paren
op_amp
id|ei-&gt;cache_lru_lock
)paren
suffix:semicolon
id|ei-&gt;nr_caches
op_assign
l_int|0
suffix:semicolon
id|ei-&gt;cache_valid_id
op_assign
id|FAT_CACHE_VALID
op_plus
l_int|1
suffix:semicolon
id|INIT_LIST_HEAD
c_func
(paren
op_amp
id|ei-&gt;cache_lru
)paren
suffix:semicolon
id|INIT_HLIST_NODE
c_func
(paren
op_amp
id|ei-&gt;i_fat_hash
)paren
suffix:semicolon
id|inode_init_once
c_func
(paren
op_amp
id|ei-&gt;vfs_inode
)paren
suffix:semicolon
)brace
)brace
DECL|function|fat_init_inodecache
r_static
r_int
id|__init
id|fat_init_inodecache
c_func
(paren
r_void
)paren
(brace
id|fat_inode_cachep
op_assign
id|kmem_cache_create
c_func
(paren
l_string|&quot;fat_inode_cache&quot;
comma
r_sizeof
(paren
r_struct
id|msdos_inode_info
)paren
comma
l_int|0
comma
id|SLAB_RECLAIM_ACCOUNT
comma
id|init_once
comma
l_int|NULL
)paren
suffix:semicolon
r_if
c_cond
(paren
id|fat_inode_cachep
op_eq
l_int|NULL
)paren
r_return
op_minus
id|ENOMEM
suffix:semicolon
r_return
l_int|0
suffix:semicolon
)brace
DECL|function|fat_destroy_inodecache
r_static
r_void
id|__exit
id|fat_destroy_inodecache
c_func
(paren
r_void
)paren
(brace
r_if
c_cond
(paren
id|kmem_cache_destroy
c_func
(paren
id|fat_inode_cachep
)paren
)paren
id|printk
c_func
(paren
id|KERN_INFO
l_string|&quot;fat_inode_cache: not all structures were freed&bslash;n&quot;
)paren
suffix:semicolon
)brace
DECL|function|fat_remount
r_static
r_int
id|fat_remount
c_func
(paren
r_struct
id|super_block
op_star
id|sb
comma
r_int
op_star
id|flags
comma
r_char
op_star
id|data
)paren
(brace
op_star
id|flags
op_or_assign
id|MS_NODIRATIME
suffix:semicolon
r_return
l_int|0
suffix:semicolon
)brace
DECL|function|fat_statfs
r_static
r_int
id|fat_statfs
c_func
(paren
r_struct
id|super_block
op_star
id|sb
comma
r_struct
id|kstatfs
op_star
id|buf
)paren
(brace
r_struct
id|msdos_sb_info
op_star
id|sbi
op_assign
id|MSDOS_SB
c_func
(paren
id|sb
)paren
suffix:semicolon
multiline_comment|/* If the count of free cluster is still unknown, counts it here. */
r_if
c_cond
(paren
id|sbi-&gt;free_clusters
op_eq
op_minus
l_int|1
)paren
(brace
r_int
id|err
op_assign
id|fat_count_free_clusters
c_func
(paren
id|sb
)paren
suffix:semicolon
r_if
c_cond
(paren
id|err
)paren
r_return
id|err
suffix:semicolon
)brace
id|buf-&gt;f_type
op_assign
id|sb-&gt;s_magic
suffix:semicolon
id|buf-&gt;f_bsize
op_assign
id|sbi-&gt;cluster_size
suffix:semicolon
id|buf-&gt;f_blocks
op_assign
id|sbi-&gt;max_cluster
op_minus
id|FAT_START_ENT
suffix:semicolon
id|buf-&gt;f_bfree
op_assign
id|sbi-&gt;free_clusters
suffix:semicolon
id|buf-&gt;f_bavail
op_assign
id|sbi-&gt;free_clusters
suffix:semicolon
id|buf-&gt;f_namelen
op_assign
id|sbi-&gt;options.isvfat
ques
c_cond
l_int|260
suffix:colon
l_int|12
suffix:semicolon
r_return
l_int|0
suffix:semicolon
)brace
DECL|function|fat_write_inode
r_static
r_int
id|fat_write_inode
c_func
(paren
r_struct
id|inode
op_star
id|inode
comma
r_int
id|wait
)paren
(brace
r_struct
id|super_block
op_star
id|sb
op_assign
id|inode-&gt;i_sb
suffix:semicolon
r_struct
id|msdos_sb_info
op_star
id|sbi
op_assign
id|MSDOS_SB
c_func
(paren
id|sb
)paren
suffix:semicolon
r_struct
id|buffer_head
op_star
id|bh
suffix:semicolon
r_struct
id|msdos_dir_entry
op_star
id|raw_entry
suffix:semicolon
id|loff_t
id|i_pos
suffix:semicolon
id|retry
suffix:colon
id|i_pos
op_assign
id|MSDOS_I
c_func
(paren
id|inode
)paren
op_member_access_from_pointer
id|i_pos
suffix:semicolon
r_if
c_cond
(paren
id|inode-&gt;i_ino
op_eq
id|MSDOS_ROOT_INO
op_logical_or
op_logical_neg
id|i_pos
)paren
(brace
r_return
l_int|0
suffix:semicolon
)brace
id|lock_kernel
c_func
(paren
)paren
suffix:semicolon
r_if
c_cond
(paren
op_logical_neg
(paren
id|bh
op_assign
id|sb_bread
c_func
(paren
id|sb
comma
id|i_pos
op_rshift
id|sbi-&gt;dir_per_block_bits
)paren
)paren
)paren
(brace
id|printk
c_func
(paren
id|KERN_ERR
l_string|&quot;FAT: unable to read inode block &quot;
l_string|&quot;for updating (i_pos %lld)&bslash;n&quot;
comma
id|i_pos
)paren
suffix:semicolon
id|unlock_kernel
c_func
(paren
)paren
suffix:semicolon
r_return
op_minus
id|EIO
suffix:semicolon
)brace
id|spin_lock
c_func
(paren
op_amp
id|sbi-&gt;inode_hash_lock
)paren
suffix:semicolon
r_if
c_cond
(paren
id|i_pos
op_ne
id|MSDOS_I
c_func
(paren
id|inode
)paren
op_member_access_from_pointer
id|i_pos
)paren
(brace
id|spin_unlock
c_func
(paren
op_amp
id|sbi-&gt;inode_hash_lock
)paren
suffix:semicolon
id|brelse
c_func
(paren
id|bh
)paren
suffix:semicolon
id|unlock_kernel
c_func
(paren
)paren
suffix:semicolon
r_goto
id|retry
suffix:semicolon
)brace
id|raw_entry
op_assign
op_amp
(paren
(paren
r_struct
id|msdos_dir_entry
op_star
)paren
(paren
id|bh-&gt;b_data
)paren
)paren
(braket
id|i_pos
op_amp
(paren
id|sbi-&gt;dir_per_block
op_minus
l_int|1
)paren
)braket
suffix:semicolon
r_if
c_cond
(paren
id|S_ISDIR
c_func
(paren
id|inode-&gt;i_mode
)paren
)paren
id|raw_entry-&gt;size
op_assign
l_int|0
suffix:semicolon
r_else
id|raw_entry-&gt;size
op_assign
id|cpu_to_le32
c_func
(paren
id|inode-&gt;i_size
)paren
suffix:semicolon
id|raw_entry-&gt;attr
op_assign
id|fat_attr
c_func
(paren
id|inode
)paren
suffix:semicolon
id|raw_entry-&gt;start
op_assign
id|cpu_to_le16
c_func
(paren
id|MSDOS_I
c_func
(paren
id|inode
)paren
op_member_access_from_pointer
id|i_logstart
)paren
suffix:semicolon
id|raw_entry-&gt;starthi
op_assign
id|cpu_to_le16
c_func
(paren
id|MSDOS_I
c_func
(paren
id|inode
)paren
op_member_access_from_pointer
id|i_logstart
op_rshift
l_int|16
)paren
suffix:semicolon
id|fat_date_unix2dos
c_func
(paren
id|inode-&gt;i_mtime.tv_sec
comma
op_amp
id|raw_entry-&gt;time
comma
op_amp
id|raw_entry-&gt;date
)paren
suffix:semicolon
r_if
c_cond
(paren
id|sbi-&gt;options.isvfat
)paren
(brace
id|fat_date_unix2dos
c_func
(paren
id|inode-&gt;i_ctime.tv_sec
comma
op_amp
id|raw_entry-&gt;ctime
comma
op_amp
id|raw_entry-&gt;cdate
)paren
suffix:semicolon
id|raw_entry-&gt;ctime_cs
op_assign
(paren
id|inode-&gt;i_ctime.tv_sec
op_amp
l_int|1
)paren
op_star
l_int|100
op_plus
id|inode-&gt;i_ctime.tv_nsec
op_div
l_int|10000000
suffix:semicolon
)brace
id|spin_unlock
c_func
(paren
op_amp
id|sbi-&gt;inode_hash_lock
)paren
suffix:semicolon
id|mark_buffer_dirty
c_func
(paren
id|bh
)paren
suffix:semicolon
r_if
c_cond
(paren
id|wait
)paren
id|sync_dirty_buffer
c_func
(paren
id|bh
)paren
suffix:semicolon
id|brelse
c_func
(paren
id|bh
)paren
suffix:semicolon
id|unlock_kernel
c_func
(paren
)paren
suffix:semicolon
r_return
l_int|0
suffix:semicolon
)brace
DECL|function|fat_sync_inode
r_int
id|fat_sync_inode
c_func
(paren
r_struct
id|inode
op_star
id|inode
)paren
(brace
r_return
id|fat_write_inode
c_func
(paren
id|inode
comma
l_int|1
)paren
suffix:semicolon
)brace
DECL|variable|fat_sync_inode
id|EXPORT_SYMBOL
c_func
(paren
id|fat_sync_inode
)paren
suffix:semicolon
r_static
r_int
id|fat_show_options
c_func
(paren
r_struct
id|seq_file
op_star
id|m
comma
r_struct
id|vfsmount
op_star
id|mnt
)paren
suffix:semicolon
DECL|variable|fat_sops
r_static
r_struct
id|super_operations
id|fat_sops
op_assign
(brace
dot
id|alloc_inode
op_assign
id|fat_alloc_inode
comma
dot
id|destroy_inode
op_assign
id|fat_destroy_inode
comma
dot
id|write_inode
op_assign
id|fat_write_inode
comma
dot
id|delete_inode
op_assign
id|fat_delete_inode
comma
dot
id|put_super
op_assign
id|fat_put_super
comma
dot
id|statfs
op_assign
id|fat_statfs
comma
dot
id|clear_inode
op_assign
id|fat_clear_inode
comma
dot
id|remount_fs
op_assign
id|fat_remount
comma
dot
id|read_inode
op_assign
id|make_bad_inode
comma
dot
id|show_options
op_assign
id|fat_show_options
comma
)brace
suffix:semicolon
multiline_comment|/*&n; * a FAT file handle with fhtype 3 is&n; *  0/  i_ino - for fast, reliable lookup if still in the cache&n; *  1/  i_generation - to see if i_ino is still valid&n; *          bit 0 == 0 iff directory&n; *  2/  i_pos(8-39) - if ino has changed, but still in cache&n; *  3/  i_pos(4-7)|i_logstart - to semi-verify inode found at i_pos&n; *  4/  i_pos(0-3)|parent-&gt;i_logstart - maybe used to hunt for the file on disc&n; *&n; * Hack for NFSv2: Maximum FAT entry number is 28bits and maximum&n; * i_pos is 40bits (blocknr(32) + dir offset(8)), so two 4bits&n; * of i_logstart is used to store the directory entry offset.&n; */
r_static
r_struct
id|dentry
op_star
DECL|function|fat_decode_fh
id|fat_decode_fh
c_func
(paren
r_struct
id|super_block
op_star
id|sb
comma
id|__u32
op_star
id|fh
comma
r_int
id|len
comma
r_int
id|fhtype
comma
r_int
(paren
op_star
id|acceptable
)paren
(paren
r_void
op_star
id|context
comma
r_struct
id|dentry
op_star
id|de
)paren
comma
r_void
op_star
id|context
)paren
(brace
r_if
c_cond
(paren
id|fhtype
op_ne
l_int|3
)paren
r_return
id|ERR_PTR
c_func
(paren
op_minus
id|ESTALE
)paren
suffix:semicolon
r_if
c_cond
(paren
id|len
OL
l_int|5
)paren
r_return
id|ERR_PTR
c_func
(paren
op_minus
id|ESTALE
)paren
suffix:semicolon
r_return
id|sb-&gt;s_export_op
op_member_access_from_pointer
id|find_exported_dentry
c_func
(paren
id|sb
comma
id|fh
comma
l_int|NULL
comma
id|acceptable
comma
id|context
)paren
suffix:semicolon
)brace
DECL|function|fat_get_dentry
r_static
r_struct
id|dentry
op_star
id|fat_get_dentry
c_func
(paren
r_struct
id|super_block
op_star
id|sb
comma
r_void
op_star
id|inump
)paren
(brace
r_struct
id|inode
op_star
id|inode
op_assign
l_int|NULL
suffix:semicolon
r_struct
id|dentry
op_star
id|result
suffix:semicolon
id|__u32
op_star
id|fh
op_assign
id|inump
suffix:semicolon
id|inode
op_assign
id|iget
c_func
(paren
id|sb
comma
id|fh
(braket
l_int|0
)braket
)paren
suffix:semicolon
r_if
c_cond
(paren
op_logical_neg
id|inode
op_logical_or
id|is_bad_inode
c_func
(paren
id|inode
)paren
op_logical_or
id|inode-&gt;i_generation
op_ne
id|fh
(braket
l_int|1
)braket
)paren
(brace
r_if
c_cond
(paren
id|inode
)paren
id|iput
c_func
(paren
id|inode
)paren
suffix:semicolon
id|inode
op_assign
l_int|NULL
suffix:semicolon
)brace
r_if
c_cond
(paren
op_logical_neg
id|inode
)paren
(brace
id|loff_t
id|i_pos
suffix:semicolon
r_int
id|i_logstart
op_assign
id|fh
(braket
l_int|3
)braket
op_amp
l_int|0x0fffffff
suffix:semicolon
id|i_pos
op_assign
(paren
id|loff_t
)paren
id|fh
(braket
l_int|2
)braket
op_lshift
l_int|8
suffix:semicolon
id|i_pos
op_or_assign
(paren
(paren
id|fh
(braket
l_int|3
)braket
op_rshift
l_int|24
)paren
op_amp
l_int|0xf0
)paren
op_or
(paren
id|fh
(braket
l_int|4
)braket
op_rshift
l_int|28
)paren
suffix:semicolon
multiline_comment|/* try 2 - see if i_pos is in F-d-c&n;&t;&t; * require i_logstart to be the same&n;&t;&t; * Will fail if you truncate and then re-write&n;&t;&t; */
id|inode
op_assign
id|fat_iget
c_func
(paren
id|sb
comma
id|i_pos
)paren
suffix:semicolon
r_if
c_cond
(paren
id|inode
op_logical_and
id|MSDOS_I
c_func
(paren
id|inode
)paren
op_member_access_from_pointer
id|i_logstart
op_ne
id|i_logstart
)paren
(brace
id|iput
c_func
(paren
id|inode
)paren
suffix:semicolon
id|inode
op_assign
l_int|NULL
suffix:semicolon
)brace
)brace
r_if
c_cond
(paren
op_logical_neg
id|inode
)paren
(brace
multiline_comment|/* For now, do nothing&n;&t;&t; * What we could do is:&n;&t;&t; * follow the file starting at fh[4], and record&n;&t;&t; * the &quot;..&quot; entry, and the name of the fh[2] entry.&n;&t;&t; * The follow the &quot;..&quot; file finding the next step up.&n;&t;&t; * This way we build a path to the root of&n;&t;&t; * the tree. If this works, we lookup the path and so&n;&t;&t; * get this inode into the cache.&n;&t;&t; * Finally try the fat_iget lookup again&n;&t;&t; * If that fails, then weare totally out of luck&n;&t;&t; * But all that is for another day&n;&t;&t; */
)brace
r_if
c_cond
(paren
op_logical_neg
id|inode
)paren
r_return
id|ERR_PTR
c_func
(paren
op_minus
id|ESTALE
)paren
suffix:semicolon
multiline_comment|/* now to find a dentry.&n;&t; * If possible, get a well-connected one&n;&t; */
id|result
op_assign
id|d_alloc_anon
c_func
(paren
id|inode
)paren
suffix:semicolon
r_if
c_cond
(paren
id|result
op_eq
l_int|NULL
)paren
(brace
id|iput
c_func
(paren
id|inode
)paren
suffix:semicolon
r_return
id|ERR_PTR
c_func
(paren
op_minus
id|ENOMEM
)paren
suffix:semicolon
)brace
id|result-&gt;d_op
op_assign
id|sb-&gt;s_root-&gt;d_op
suffix:semicolon
r_return
id|result
suffix:semicolon
)brace
r_static
r_int
DECL|function|fat_encode_fh
id|fat_encode_fh
c_func
(paren
r_struct
id|dentry
op_star
id|de
comma
id|__u32
op_star
id|fh
comma
r_int
op_star
id|lenp
comma
r_int
id|connectable
)paren
(brace
r_int
id|len
op_assign
op_star
id|lenp
suffix:semicolon
r_struct
id|inode
op_star
id|inode
op_assign
id|de-&gt;d_inode
suffix:semicolon
id|u32
id|ipos_h
comma
id|ipos_m
comma
id|ipos_l
suffix:semicolon
r_if
c_cond
(paren
id|len
OL
l_int|5
)paren
r_return
l_int|255
suffix:semicolon
multiline_comment|/* no room */
id|ipos_h
op_assign
id|MSDOS_I
c_func
(paren
id|inode
)paren
op_member_access_from_pointer
id|i_pos
op_rshift
l_int|8
suffix:semicolon
id|ipos_m
op_assign
(paren
id|MSDOS_I
c_func
(paren
id|inode
)paren
op_member_access_from_pointer
id|i_pos
op_amp
l_int|0xf0
)paren
op_lshift
l_int|24
suffix:semicolon
id|ipos_l
op_assign
(paren
id|MSDOS_I
c_func
(paren
id|inode
)paren
op_member_access_from_pointer
id|i_pos
op_amp
l_int|0x0f
)paren
op_lshift
l_int|28
suffix:semicolon
op_star
id|lenp
op_assign
l_int|5
suffix:semicolon
id|fh
(braket
l_int|0
)braket
op_assign
id|inode-&gt;i_ino
suffix:semicolon
id|fh
(braket
l_int|1
)braket
op_assign
id|inode-&gt;i_generation
suffix:semicolon
id|fh
(braket
l_int|2
)braket
op_assign
id|ipos_h
suffix:semicolon
id|fh
(braket
l_int|3
)braket
op_assign
id|ipos_m
op_or
id|MSDOS_I
c_func
(paren
id|inode
)paren
op_member_access_from_pointer
id|i_logstart
suffix:semicolon
id|spin_lock
c_func
(paren
op_amp
id|de-&gt;d_lock
)paren
suffix:semicolon
id|fh
(braket
l_int|4
)braket
op_assign
id|ipos_l
op_or
id|MSDOS_I
c_func
(paren
id|de-&gt;d_parent-&gt;d_inode
)paren
op_member_access_from_pointer
id|i_logstart
suffix:semicolon
id|spin_unlock
c_func
(paren
op_amp
id|de-&gt;d_lock
)paren
suffix:semicolon
r_return
l_int|3
suffix:semicolon
)brace
DECL|function|fat_get_parent
r_static
r_struct
id|dentry
op_star
id|fat_get_parent
c_func
(paren
r_struct
id|dentry
op_star
id|child
)paren
(brace
r_struct
id|buffer_head
op_star
id|bh
suffix:semicolon
r_struct
id|msdos_dir_entry
op_star
id|de
suffix:semicolon
id|loff_t
id|i_pos
suffix:semicolon
r_struct
id|dentry
op_star
id|parent
suffix:semicolon
r_struct
id|inode
op_star
id|inode
suffix:semicolon
r_int
id|err
suffix:semicolon
id|lock_kernel
c_func
(paren
)paren
suffix:semicolon
id|err
op_assign
id|fat_get_dotdot_entry
c_func
(paren
id|child-&gt;d_inode
comma
op_amp
id|bh
comma
op_amp
id|de
comma
op_amp
id|i_pos
)paren
suffix:semicolon
r_if
c_cond
(paren
id|err
)paren
(brace
id|parent
op_assign
id|ERR_PTR
c_func
(paren
id|err
)paren
suffix:semicolon
r_goto
id|out
suffix:semicolon
)brace
id|inode
op_assign
id|fat_build_inode
c_func
(paren
id|child-&gt;d_sb
comma
id|de
comma
id|i_pos
)paren
suffix:semicolon
id|brelse
c_func
(paren
id|bh
)paren
suffix:semicolon
r_if
c_cond
(paren
id|IS_ERR
c_func
(paren
id|inode
)paren
)paren
(brace
id|parent
op_assign
id|ERR_PTR
c_func
(paren
id|PTR_ERR
c_func
(paren
id|inode
)paren
)paren
suffix:semicolon
r_goto
id|out
suffix:semicolon
)brace
id|parent
op_assign
id|d_alloc_anon
c_func
(paren
id|inode
)paren
suffix:semicolon
r_if
c_cond
(paren
op_logical_neg
id|parent
)paren
(brace
id|iput
c_func
(paren
id|inode
)paren
suffix:semicolon
id|parent
op_assign
id|ERR_PTR
c_func
(paren
op_minus
id|ENOMEM
)paren
suffix:semicolon
)brace
id|out
suffix:colon
id|unlock_kernel
c_func
(paren
)paren
suffix:semicolon
r_return
id|parent
suffix:semicolon
)brace
DECL|variable|fat_export_ops
r_static
r_struct
id|export_operations
id|fat_export_ops
op_assign
(brace
dot
id|decode_fh
op_assign
id|fat_decode_fh
comma
dot
id|encode_fh
op_assign
id|fat_encode_fh
comma
dot
id|get_dentry
op_assign
id|fat_get_dentry
comma
dot
id|get_parent
op_assign
id|fat_get_parent
comma
)brace
suffix:semicolon
DECL|function|fat_show_options
r_static
r_int
id|fat_show_options
c_func
(paren
r_struct
id|seq_file
op_star
id|m
comma
r_struct
id|vfsmount
op_star
id|mnt
)paren
(brace
r_struct
id|msdos_sb_info
op_star
id|sbi
op_assign
id|MSDOS_SB
c_func
(paren
id|mnt-&gt;mnt_sb
)paren
suffix:semicolon
r_struct
id|fat_mount_options
op_star
id|opts
op_assign
op_amp
id|sbi-&gt;options
suffix:semicolon
r_int
id|isvfat
op_assign
id|opts-&gt;isvfat
suffix:semicolon
r_if
c_cond
(paren
id|opts-&gt;fs_uid
op_ne
l_int|0
)paren
id|seq_printf
c_func
(paren
id|m
comma
l_string|&quot;,uid=%u&quot;
comma
id|opts-&gt;fs_uid
)paren
suffix:semicolon
r_if
c_cond
(paren
id|opts-&gt;fs_gid
op_ne
l_int|0
)paren
id|seq_printf
c_func
(paren
id|m
comma
l_string|&quot;,gid=%u&quot;
comma
id|opts-&gt;fs_gid
)paren
suffix:semicolon
id|seq_printf
c_func
(paren
id|m
comma
l_string|&quot;,fmask=%04o&quot;
comma
id|opts-&gt;fs_fmask
)paren
suffix:semicolon
id|seq_printf
c_func
(paren
id|m
comma
l_string|&quot;,dmask=%04o&quot;
comma
id|opts-&gt;fs_dmask
)paren
suffix:semicolon
r_if
c_cond
(paren
id|sbi-&gt;nls_disk
)paren
id|seq_printf
c_func
(paren
id|m
comma
l_string|&quot;,codepage=%s&quot;
comma
id|sbi-&gt;nls_disk-&gt;charset
)paren
suffix:semicolon
r_if
c_cond
(paren
id|isvfat
)paren
(brace
r_if
c_cond
(paren
id|sbi-&gt;nls_io
)paren
id|seq_printf
c_func
(paren
id|m
comma
l_string|&quot;,iocharset=%s&quot;
comma
id|sbi-&gt;nls_io-&gt;charset
)paren
suffix:semicolon
r_switch
c_cond
(paren
id|opts-&gt;shortname
)paren
(brace
r_case
id|VFAT_SFN_DISPLAY_WIN95
op_or
id|VFAT_SFN_CREATE_WIN95
suffix:colon
id|seq_puts
c_func
(paren
id|m
comma
l_string|&quot;,shortname=win95&quot;
)paren
suffix:semicolon
r_break
suffix:semicolon
r_case
id|VFAT_SFN_DISPLAY_WINNT
op_or
id|VFAT_SFN_CREATE_WINNT
suffix:colon
id|seq_puts
c_func
(paren
id|m
comma
l_string|&quot;,shortname=winnt&quot;
)paren
suffix:semicolon
r_break
suffix:semicolon
r_case
id|VFAT_SFN_DISPLAY_WINNT
op_or
id|VFAT_SFN_CREATE_WIN95
suffix:colon
id|seq_puts
c_func
(paren
id|m
comma
l_string|&quot;,shortname=mixed&quot;
)paren
suffix:semicolon
r_break
suffix:semicolon
r_case
id|VFAT_SFN_DISPLAY_LOWER
op_or
id|VFAT_SFN_CREATE_WIN95
suffix:colon
multiline_comment|/* seq_puts(m, &quot;,shortname=lower&quot;); */
r_break
suffix:semicolon
r_default
suffix:colon
id|seq_puts
c_func
(paren
id|m
comma
l_string|&quot;,shortname=unknown&quot;
)paren
suffix:semicolon
r_break
suffix:semicolon
)brace
)brace
r_if
c_cond
(paren
id|opts-&gt;name_check
op_ne
l_char|&squot;n&squot;
)paren
id|seq_printf
c_func
(paren
id|m
comma
l_string|&quot;,check=%c&quot;
comma
id|opts-&gt;name_check
)paren
suffix:semicolon
r_if
c_cond
(paren
id|opts-&gt;quiet
)paren
id|seq_puts
c_func
(paren
id|m
comma
l_string|&quot;,quiet&quot;
)paren
suffix:semicolon
r_if
c_cond
(paren
id|opts-&gt;showexec
)paren
id|seq_puts
c_func
(paren
id|m
comma
l_string|&quot;,showexec&quot;
)paren
suffix:semicolon
r_if
c_cond
(paren
id|opts-&gt;sys_immutable
)paren
id|seq_puts
c_func
(paren
id|m
comma
l_string|&quot;,sys_immutable&quot;
)paren
suffix:semicolon
r_if
c_cond
(paren
op_logical_neg
id|isvfat
)paren
(brace
r_if
c_cond
(paren
id|opts-&gt;dotsOK
)paren
id|seq_puts
c_func
(paren
id|m
comma
l_string|&quot;,dotsOK=yes&quot;
)paren
suffix:semicolon
r_if
c_cond
(paren
id|opts-&gt;nocase
)paren
id|seq_puts
c_func
(paren
id|m
comma
l_string|&quot;,nocase&quot;
)paren
suffix:semicolon
)brace
r_else
(brace
r_if
c_cond
(paren
id|opts-&gt;utf8
)paren
id|seq_puts
c_func
(paren
id|m
comma
l_string|&quot;,utf8&quot;
)paren
suffix:semicolon
r_if
c_cond
(paren
id|opts-&gt;unicode_xlate
)paren
id|seq_puts
c_func
(paren
id|m
comma
l_string|&quot;,uni_xlate&quot;
)paren
suffix:semicolon
r_if
c_cond
(paren
op_logical_neg
id|opts-&gt;numtail
)paren
id|seq_puts
c_func
(paren
id|m
comma
l_string|&quot;,nonumtail&quot;
)paren
suffix:semicolon
)brace
r_return
l_int|0
suffix:semicolon
)brace
r_enum
(brace
DECL|enumerator|Opt_check_n
DECL|enumerator|Opt_check_r
DECL|enumerator|Opt_check_s
DECL|enumerator|Opt_uid
DECL|enumerator|Opt_gid
id|Opt_check_n
comma
id|Opt_check_r
comma
id|Opt_check_s
comma
id|Opt_uid
comma
id|Opt_gid
comma
DECL|enumerator|Opt_umask
DECL|enumerator|Opt_dmask
DECL|enumerator|Opt_fmask
DECL|enumerator|Opt_codepage
DECL|enumerator|Opt_nocase
id|Opt_umask
comma
id|Opt_dmask
comma
id|Opt_fmask
comma
id|Opt_codepage
comma
id|Opt_nocase
comma
DECL|enumerator|Opt_quiet
DECL|enumerator|Opt_showexec
DECL|enumerator|Opt_debug
DECL|enumerator|Opt_immutable
id|Opt_quiet
comma
id|Opt_showexec
comma
id|Opt_debug
comma
id|Opt_immutable
comma
DECL|enumerator|Opt_dots
DECL|enumerator|Opt_nodots
id|Opt_dots
comma
id|Opt_nodots
comma
DECL|enumerator|Opt_charset
DECL|enumerator|Opt_shortname_lower
DECL|enumerator|Opt_shortname_win95
id|Opt_charset
comma
id|Opt_shortname_lower
comma
id|Opt_shortname_win95
comma
DECL|enumerator|Opt_shortname_winnt
DECL|enumerator|Opt_shortname_mixed
DECL|enumerator|Opt_utf8_no
DECL|enumerator|Opt_utf8_yes
id|Opt_shortname_winnt
comma
id|Opt_shortname_mixed
comma
id|Opt_utf8_no
comma
id|Opt_utf8_yes
comma
DECL|enumerator|Opt_uni_xl_no
DECL|enumerator|Opt_uni_xl_yes
DECL|enumerator|Opt_nonumtail_no
DECL|enumerator|Opt_nonumtail_yes
id|Opt_uni_xl_no
comma
id|Opt_uni_xl_yes
comma
id|Opt_nonumtail_no
comma
id|Opt_nonumtail_yes
comma
DECL|enumerator|Opt_obsolate
DECL|enumerator|Opt_err
id|Opt_obsolate
comma
id|Opt_err
comma
)brace
suffix:semicolon
DECL|variable|fat_tokens
r_static
id|match_table_t
id|fat_tokens
op_assign
(brace
(brace
id|Opt_check_r
comma
l_string|&quot;check=relaxed&quot;
)brace
comma
(brace
id|Opt_check_s
comma
l_string|&quot;check=strict&quot;
)brace
comma
(brace
id|Opt_check_n
comma
l_string|&quot;check=normal&quot;
)brace
comma
(brace
id|Opt_check_r
comma
l_string|&quot;check=r&quot;
)brace
comma
(brace
id|Opt_check_s
comma
l_string|&quot;check=s&quot;
)brace
comma
(brace
id|Opt_check_n
comma
l_string|&quot;check=n&quot;
)brace
comma
(brace
id|Opt_uid
comma
l_string|&quot;uid=%u&quot;
)brace
comma
(brace
id|Opt_gid
comma
l_string|&quot;gid=%u&quot;
)brace
comma
(brace
id|Opt_umask
comma
l_string|&quot;umask=%o&quot;
)brace
comma
(brace
id|Opt_dmask
comma
l_string|&quot;dmask=%o&quot;
)brace
comma
(brace
id|Opt_fmask
comma
l_string|&quot;fmask=%o&quot;
)brace
comma
(brace
id|Opt_codepage
comma
l_string|&quot;codepage=%u&quot;
)brace
comma
(brace
id|Opt_nocase
comma
l_string|&quot;nocase&quot;
)brace
comma
(brace
id|Opt_quiet
comma
l_string|&quot;quiet&quot;
)brace
comma
(brace
id|Opt_showexec
comma
l_string|&quot;showexec&quot;
)brace
comma
(brace
id|Opt_debug
comma
l_string|&quot;debug&quot;
)brace
comma
(brace
id|Opt_immutable
comma
l_string|&quot;sys_immutable&quot;
)brace
comma
(brace
id|Opt_obsolate
comma
l_string|&quot;conv=binary&quot;
)brace
comma
(brace
id|Opt_obsolate
comma
l_string|&quot;conv=text&quot;
)brace
comma
(brace
id|Opt_obsolate
comma
l_string|&quot;conv=auto&quot;
)brace
comma
(brace
id|Opt_obsolate
comma
l_string|&quot;conv=b&quot;
)brace
comma
(brace
id|Opt_obsolate
comma
l_string|&quot;conv=t&quot;
)brace
comma
(brace
id|Opt_obsolate
comma
l_string|&quot;conv=a&quot;
)brace
comma
(brace
id|Opt_obsolate
comma
l_string|&quot;fat=%u&quot;
)brace
comma
(brace
id|Opt_obsolate
comma
l_string|&quot;blocksize=%u&quot;
)brace
comma
(brace
id|Opt_obsolate
comma
l_string|&quot;cvf_format=%20s&quot;
)brace
comma
(brace
id|Opt_obsolate
comma
l_string|&quot;cvf_options=%100s&quot;
)brace
comma
(brace
id|Opt_obsolate
comma
l_string|&quot;posix&quot;
)brace
comma
(brace
id|Opt_err
comma
l_int|NULL
)brace
)brace
suffix:semicolon
DECL|variable|msdos_tokens
r_static
id|match_table_t
id|msdos_tokens
op_assign
(brace
(brace
id|Opt_nodots
comma
l_string|&quot;nodots&quot;
)brace
comma
(brace
id|Opt_nodots
comma
l_string|&quot;dotsOK=no&quot;
)brace
comma
(brace
id|Opt_dots
comma
l_string|&quot;dots&quot;
)brace
comma
(brace
id|Opt_dots
comma
l_string|&quot;dotsOK=yes&quot;
)brace
comma
(brace
id|Opt_err
comma
l_int|NULL
)brace
)brace
suffix:semicolon
DECL|variable|vfat_tokens
r_static
id|match_table_t
id|vfat_tokens
op_assign
(brace
(brace
id|Opt_charset
comma
l_string|&quot;iocharset=%s&quot;
)brace
comma
(brace
id|Opt_shortname_lower
comma
l_string|&quot;shortname=lower&quot;
)brace
comma
(brace
id|Opt_shortname_win95
comma
l_string|&quot;shortname=win95&quot;
)brace
comma
(brace
id|Opt_shortname_winnt
comma
l_string|&quot;shortname=winnt&quot;
)brace
comma
(brace
id|Opt_shortname_mixed
comma
l_string|&quot;shortname=mixed&quot;
)brace
comma
(brace
id|Opt_utf8_no
comma
l_string|&quot;utf8=0&quot;
)brace
comma
multiline_comment|/* 0 or no or false */
(brace
id|Opt_utf8_no
comma
l_string|&quot;utf8=no&quot;
)brace
comma
(brace
id|Opt_utf8_no
comma
l_string|&quot;utf8=false&quot;
)brace
comma
(brace
id|Opt_utf8_yes
comma
l_string|&quot;utf8=1&quot;
)brace
comma
multiline_comment|/* empty or 1 or yes or true */
(brace
id|Opt_utf8_yes
comma
l_string|&quot;utf8=yes&quot;
)brace
comma
(brace
id|Opt_utf8_yes
comma
l_string|&quot;utf8=true&quot;
)brace
comma
(brace
id|Opt_utf8_yes
comma
l_string|&quot;utf8&quot;
)brace
comma
(brace
id|Opt_uni_xl_no
comma
l_string|&quot;uni_xlate=0&quot;
)brace
comma
multiline_comment|/* 0 or no or false */
(brace
id|Opt_uni_xl_no
comma
l_string|&quot;uni_xlate=no&quot;
)brace
comma
(brace
id|Opt_uni_xl_no
comma
l_string|&quot;uni_xlate=false&quot;
)brace
comma
(brace
id|Opt_uni_xl_yes
comma
l_string|&quot;uni_xlate=1&quot;
)brace
comma
multiline_comment|/* empty or 1 or yes or true */
(brace
id|Opt_uni_xl_yes
comma
l_string|&quot;uni_xlate=yes&quot;
)brace
comma
(brace
id|Opt_uni_xl_yes
comma
l_string|&quot;uni_xlate=true&quot;
)brace
comma
(brace
id|Opt_uni_xl_yes
comma
l_string|&quot;uni_xlate&quot;
)brace
comma
(brace
id|Opt_nonumtail_no
comma
l_string|&quot;nonumtail=0&quot;
)brace
comma
multiline_comment|/* 0 or no or false */
(brace
id|Opt_nonumtail_no
comma
l_string|&quot;nonumtail=no&quot;
)brace
comma
(brace
id|Opt_nonumtail_no
comma
l_string|&quot;nonumtail=false&quot;
)brace
comma
(brace
id|Opt_nonumtail_yes
comma
l_string|&quot;nonumtail=1&quot;
)brace
comma
multiline_comment|/* empty or 1 or yes or true */
(brace
id|Opt_nonumtail_yes
comma
l_string|&quot;nonumtail=yes&quot;
)brace
comma
(brace
id|Opt_nonumtail_yes
comma
l_string|&quot;nonumtail=true&quot;
)brace
comma
(brace
id|Opt_nonumtail_yes
comma
l_string|&quot;nonumtail&quot;
)brace
comma
(brace
id|Opt_err
comma
l_int|NULL
)brace
)brace
suffix:semicolon
DECL|function|parse_options
r_static
r_int
id|parse_options
c_func
(paren
r_char
op_star
id|options
comma
r_int
id|is_vfat
comma
r_int
op_star
id|debug
comma
r_struct
id|fat_mount_options
op_star
id|opts
)paren
(brace
r_char
op_star
id|p
suffix:semicolon
id|substring_t
id|args
(braket
id|MAX_OPT_ARGS
)braket
suffix:semicolon
r_int
id|option
suffix:semicolon
r_char
op_star
id|iocharset
suffix:semicolon
id|opts-&gt;isvfat
op_assign
id|is_vfat
suffix:semicolon
id|opts-&gt;fs_uid
op_assign
id|current-&gt;uid
suffix:semicolon
id|opts-&gt;fs_gid
op_assign
id|current-&gt;gid
suffix:semicolon
id|opts-&gt;fs_fmask
op_assign
id|opts-&gt;fs_dmask
op_assign
id|current-&gt;fs-&gt;umask
suffix:semicolon
id|opts-&gt;codepage
op_assign
id|fat_default_codepage
suffix:semicolon
id|opts-&gt;iocharset
op_assign
id|fat_default_iocharset
suffix:semicolon
r_if
c_cond
(paren
id|is_vfat
)paren
id|opts-&gt;shortname
op_assign
id|VFAT_SFN_DISPLAY_LOWER
op_or
id|VFAT_SFN_CREATE_WIN95
suffix:semicolon
r_else
id|opts-&gt;shortname
op_assign
l_int|0
suffix:semicolon
id|opts-&gt;name_check
op_assign
l_char|&squot;n&squot;
suffix:semicolon
id|opts-&gt;quiet
op_assign
id|opts-&gt;showexec
op_assign
id|opts-&gt;sys_immutable
op_assign
id|opts-&gt;dotsOK
op_assign
l_int|0
suffix:semicolon
id|opts-&gt;utf8
op_assign
id|opts-&gt;unicode_xlate
op_assign
l_int|0
suffix:semicolon
id|opts-&gt;numtail
op_assign
l_int|1
suffix:semicolon
id|opts-&gt;nocase
op_assign
l_int|0
suffix:semicolon
op_star
id|debug
op_assign
l_int|0
suffix:semicolon
r_if
c_cond
(paren
op_logical_neg
id|options
)paren
r_return
l_int|0
suffix:semicolon
r_while
c_loop
(paren
(paren
id|p
op_assign
id|strsep
c_func
(paren
op_amp
id|options
comma
l_string|&quot;,&quot;
)paren
)paren
op_ne
l_int|NULL
)paren
(brace
r_int
id|token
suffix:semicolon
r_if
c_cond
(paren
op_logical_neg
op_star
id|p
)paren
r_continue
suffix:semicolon
id|token
op_assign
id|match_token
c_func
(paren
id|p
comma
id|fat_tokens
comma
id|args
)paren
suffix:semicolon
r_if
c_cond
(paren
id|token
op_eq
id|Opt_err
)paren
(brace
r_if
c_cond
(paren
id|is_vfat
)paren
id|token
op_assign
id|match_token
c_func
(paren
id|p
comma
id|vfat_tokens
comma
id|args
)paren
suffix:semicolon
r_else
id|token
op_assign
id|match_token
c_func
(paren
id|p
comma
id|msdos_tokens
comma
id|args
)paren
suffix:semicolon
)brace
r_switch
c_cond
(paren
id|token
)paren
(brace
r_case
id|Opt_check_s
suffix:colon
id|opts-&gt;name_check
op_assign
l_char|&squot;s&squot;
suffix:semicolon
r_break
suffix:semicolon
r_case
id|Opt_check_r
suffix:colon
id|opts-&gt;name_check
op_assign
l_char|&squot;r&squot;
suffix:semicolon
r_break
suffix:semicolon
r_case
id|Opt_check_n
suffix:colon
id|opts-&gt;name_check
op_assign
l_char|&squot;n&squot;
suffix:semicolon
r_break
suffix:semicolon
r_case
id|Opt_nocase
suffix:colon
r_if
c_cond
(paren
op_logical_neg
id|is_vfat
)paren
id|opts-&gt;nocase
op_assign
l_int|1
suffix:semicolon
r_else
(brace
multiline_comment|/* for backward compatibility */
id|opts-&gt;shortname
op_assign
id|VFAT_SFN_DISPLAY_WIN95
op_or
id|VFAT_SFN_CREATE_WIN95
suffix:semicolon
)brace
r_break
suffix:semicolon
r_case
id|Opt_quiet
suffix:colon
id|opts-&gt;quiet
op_assign
l_int|1
suffix:semicolon
r_break
suffix:semicolon
r_case
id|Opt_showexec
suffix:colon
id|opts-&gt;showexec
op_assign
l_int|1
suffix:semicolon
r_break
suffix:semicolon
r_case
id|Opt_debug
suffix:colon
op_star
id|debug
op_assign
l_int|1
suffix:semicolon
r_break
suffix:semicolon
r_case
id|Opt_immutable
suffix:colon
id|opts-&gt;sys_immutable
op_assign
l_int|1
suffix:semicolon
r_break
suffix:semicolon
r_case
id|Opt_uid
suffix:colon
r_if
c_cond
(paren
id|match_int
c_func
(paren
op_amp
id|args
(braket
l_int|0
)braket
comma
op_amp
id|option
)paren
)paren
r_return
l_int|0
suffix:semicolon
id|opts-&gt;fs_uid
op_assign
id|option
suffix:semicolon
r_break
suffix:semicolon
r_case
id|Opt_gid
suffix:colon
r_if
c_cond
(paren
id|match_int
c_func
(paren
op_amp
id|args
(braket
l_int|0
)braket
comma
op_amp
id|option
)paren
)paren
r_return
l_int|0
suffix:semicolon
id|opts-&gt;fs_gid
op_assign
id|option
suffix:semicolon
r_break
suffix:semicolon
r_case
id|Opt_umask
suffix:colon
r_if
c_cond
(paren
id|match_octal
c_func
(paren
op_amp
id|args
(braket
l_int|0
)braket
comma
op_amp
id|option
)paren
)paren
r_return
l_int|0
suffix:semicolon
id|opts-&gt;fs_fmask
op_assign
id|opts-&gt;fs_dmask
op_assign
id|option
suffix:semicolon
r_break
suffix:semicolon
r_case
id|Opt_dmask
suffix:colon
r_if
c_cond
(paren
id|match_octal
c_func
(paren
op_amp
id|args
(braket
l_int|0
)braket
comma
op_amp
id|option
)paren
)paren
r_return
l_int|0
suffix:semicolon
id|opts-&gt;fs_dmask
op_assign
id|option
suffix:semicolon
r_break
suffix:semicolon
r_case
id|Opt_fmask
suffix:colon
r_if
c_cond
(paren
id|match_octal
c_func
(paren
op_amp
id|args
(braket
l_int|0
)braket
comma
op_amp
id|option
)paren
)paren
r_return
l_int|0
suffix:semicolon
id|opts-&gt;fs_fmask
op_assign
id|option
suffix:semicolon
r_break
suffix:semicolon
r_case
id|Opt_codepage
suffix:colon
r_if
c_cond
(paren
id|match_int
c_func
(paren
op_amp
id|args
(braket
l_int|0
)braket
comma
op_amp
id|option
)paren
)paren
r_return
l_int|0
suffix:semicolon
id|opts-&gt;codepage
op_assign
id|option
suffix:semicolon
r_break
suffix:semicolon
multiline_comment|/* msdos specific */
r_case
id|Opt_dots
suffix:colon
id|opts-&gt;dotsOK
op_assign
l_int|1
suffix:semicolon
r_break
suffix:semicolon
r_case
id|Opt_nodots
suffix:colon
id|opts-&gt;dotsOK
op_assign
l_int|0
suffix:semicolon
r_break
suffix:semicolon
multiline_comment|/* vfat specific */
r_case
id|Opt_charset
suffix:colon
r_if
c_cond
(paren
id|opts-&gt;iocharset
op_ne
id|fat_default_iocharset
)paren
id|kfree
c_func
(paren
id|opts-&gt;iocharset
)paren
suffix:semicolon
id|iocharset
op_assign
id|match_strdup
c_func
(paren
op_amp
id|args
(braket
l_int|0
)braket
)paren
suffix:semicolon
r_if
c_cond
(paren
op_logical_neg
id|iocharset
)paren
r_return
op_minus
id|ENOMEM
suffix:semicolon
id|opts-&gt;iocharset
op_assign
id|iocharset
suffix:semicolon
r_break
suffix:semicolon
r_case
id|Opt_shortname_lower
suffix:colon
id|opts-&gt;shortname
op_assign
id|VFAT_SFN_DISPLAY_LOWER
op_or
id|VFAT_SFN_CREATE_WIN95
suffix:semicolon
r_break
suffix:semicolon
r_case
id|Opt_shortname_win95
suffix:colon
id|opts-&gt;shortname
op_assign
id|VFAT_SFN_DISPLAY_WIN95
op_or
id|VFAT_SFN_CREATE_WIN95
suffix:semicolon
r_break
suffix:semicolon
r_case
id|Opt_shortname_winnt
suffix:colon
id|opts-&gt;shortname
op_assign
id|VFAT_SFN_DISPLAY_WINNT
op_or
id|VFAT_SFN_CREATE_WINNT
suffix:semicolon
r_break
suffix:semicolon
r_case
id|Opt_shortname_mixed
suffix:colon
id|opts-&gt;shortname
op_assign
id|VFAT_SFN_DISPLAY_WINNT
op_or
id|VFAT_SFN_CREATE_WIN95
suffix:semicolon
r_break
suffix:semicolon
r_case
id|Opt_utf8_no
suffix:colon
multiline_comment|/* 0 or no or false */
id|opts-&gt;utf8
op_assign
l_int|0
suffix:semicolon
r_break
suffix:semicolon
r_case
id|Opt_utf8_yes
suffix:colon
multiline_comment|/* empty or 1 or yes or true */
id|opts-&gt;utf8
op_assign
l_int|1
suffix:semicolon
r_break
suffix:semicolon
r_case
id|Opt_uni_xl_no
suffix:colon
multiline_comment|/* 0 or no or false */
id|opts-&gt;unicode_xlate
op_assign
l_int|0
suffix:semicolon
r_break
suffix:semicolon
r_case
id|Opt_uni_xl_yes
suffix:colon
multiline_comment|/* empty or 1 or yes or true */
id|opts-&gt;unicode_xlate
op_assign
l_int|1
suffix:semicolon
r_break
suffix:semicolon
r_case
id|Opt_nonumtail_no
suffix:colon
multiline_comment|/* 0 or no or false */
id|opts-&gt;numtail
op_assign
l_int|1
suffix:semicolon
multiline_comment|/* negated option */
r_break
suffix:semicolon
r_case
id|Opt_nonumtail_yes
suffix:colon
multiline_comment|/* empty or 1 or yes or true */
id|opts-&gt;numtail
op_assign
l_int|0
suffix:semicolon
multiline_comment|/* negated option */
r_break
suffix:semicolon
multiline_comment|/* obsolete mount options */
r_case
id|Opt_obsolate
suffix:colon
id|printk
c_func
(paren
id|KERN_INFO
l_string|&quot;FAT: &bslash;&quot;%s&bslash;&quot; option is obsolete, &quot;
l_string|&quot;not supported now&bslash;n&quot;
comma
id|p
)paren
suffix:semicolon
r_break
suffix:semicolon
multiline_comment|/* unknown option */
r_default
suffix:colon
id|printk
c_func
(paren
id|KERN_ERR
l_string|&quot;FAT: Unrecognized mount option &bslash;&quot;%s&bslash;&quot; &quot;
l_string|&quot;or missing value&bslash;n&quot;
comma
id|p
)paren
suffix:semicolon
r_return
op_minus
id|EINVAL
suffix:semicolon
)brace
)brace
multiline_comment|/* UTF8 doesn&squot;t provide FAT semantics */
r_if
c_cond
(paren
op_logical_neg
id|strcmp
c_func
(paren
id|opts-&gt;iocharset
comma
l_string|&quot;utf8&quot;
)paren
)paren
(brace
id|printk
c_func
(paren
id|KERN_ERR
l_string|&quot;FAT: utf8 is not a recommended IO charset&quot;
l_string|&quot; for FAT filesystems, filesystem will be case sensitive!&bslash;n&quot;
)paren
suffix:semicolon
)brace
r_if
c_cond
(paren
id|opts-&gt;unicode_xlate
)paren
id|opts-&gt;utf8
op_assign
l_int|0
suffix:semicolon
r_return
l_int|0
suffix:semicolon
)brace
DECL|function|fat_read_root
r_static
r_int
id|fat_read_root
c_func
(paren
r_struct
id|inode
op_star
id|inode
)paren
(brace
r_struct
id|super_block
op_star
id|sb
op_assign
id|inode-&gt;i_sb
suffix:semicolon
r_struct
id|msdos_sb_info
op_star
id|sbi
op_assign
id|MSDOS_SB
c_func
(paren
id|sb
)paren
suffix:semicolon
r_int
id|error
suffix:semicolon
id|MSDOS_I
c_func
(paren
id|inode
)paren
op_member_access_from_pointer
id|i_pos
op_assign
l_int|0
suffix:semicolon
id|inode-&gt;i_uid
op_assign
id|sbi-&gt;options.fs_uid
suffix:semicolon
id|inode-&gt;i_gid
op_assign
id|sbi-&gt;options.fs_gid
suffix:semicolon
id|inode-&gt;i_version
op_increment
suffix:semicolon
id|inode-&gt;i_generation
op_assign
l_int|0
suffix:semicolon
id|inode-&gt;i_mode
op_assign
(paren
id|S_IRWXUGO
op_amp
op_complement
id|sbi-&gt;options.fs_dmask
)paren
op_or
id|S_IFDIR
suffix:semicolon
id|inode-&gt;i_op
op_assign
id|sbi-&gt;dir_ops
suffix:semicolon
id|inode-&gt;i_fop
op_assign
op_amp
id|fat_dir_operations
suffix:semicolon
r_if
c_cond
(paren
id|sbi-&gt;fat_bits
op_eq
l_int|32
)paren
(brace
id|MSDOS_I
c_func
(paren
id|inode
)paren
op_member_access_from_pointer
id|i_start
op_assign
id|sbi-&gt;root_cluster
suffix:semicolon
id|error
op_assign
id|fat_calc_dir_size
c_func
(paren
id|inode
)paren
suffix:semicolon
r_if
c_cond
(paren
id|error
OL
l_int|0
)paren
r_return
id|error
suffix:semicolon
)brace
r_else
(brace
id|MSDOS_I
c_func
(paren
id|inode
)paren
op_member_access_from_pointer
id|i_start
op_assign
l_int|0
suffix:semicolon
id|inode-&gt;i_size
op_assign
id|sbi-&gt;dir_entries
op_star
r_sizeof
(paren
r_struct
id|msdos_dir_entry
)paren
suffix:semicolon
)brace
id|inode-&gt;i_blksize
op_assign
id|sbi-&gt;cluster_size
suffix:semicolon
id|inode-&gt;i_blocks
op_assign
(paren
(paren
id|inode-&gt;i_size
op_plus
(paren
id|sbi-&gt;cluster_size
op_minus
l_int|1
)paren
)paren
op_amp
op_complement
(paren
(paren
id|loff_t
)paren
id|sbi-&gt;cluster_size
op_minus
l_int|1
)paren
)paren
op_rshift
l_int|9
suffix:semicolon
id|MSDOS_I
c_func
(paren
id|inode
)paren
op_member_access_from_pointer
id|i_logstart
op_assign
l_int|0
suffix:semicolon
id|MSDOS_I
c_func
(paren
id|inode
)paren
op_member_access_from_pointer
id|mmu_private
op_assign
id|inode-&gt;i_size
suffix:semicolon
id|MSDOS_I
c_func
(paren
id|inode
)paren
op_member_access_from_pointer
id|i_attrs
op_assign
id|ATTR_NONE
suffix:semicolon
id|inode-&gt;i_mtime.tv_sec
op_assign
id|inode-&gt;i_atime.tv_sec
op_assign
id|inode-&gt;i_ctime.tv_sec
op_assign
l_int|0
suffix:semicolon
id|inode-&gt;i_mtime.tv_nsec
op_assign
id|inode-&gt;i_atime.tv_nsec
op_assign
id|inode-&gt;i_ctime.tv_nsec
op_assign
l_int|0
suffix:semicolon
id|inode-&gt;i_nlink
op_assign
id|fat_subdirs
c_func
(paren
id|inode
)paren
op_plus
l_int|2
suffix:semicolon
r_return
l_int|0
suffix:semicolon
)brace
multiline_comment|/*&n; * Read the super block of an MS-DOS FS.&n; */
DECL|function|fat_fill_super
r_int
id|fat_fill_super
c_func
(paren
r_struct
id|super_block
op_star
id|sb
comma
r_void
op_star
id|data
comma
r_int
id|silent
comma
r_struct
id|inode_operations
op_star
id|fs_dir_inode_ops
comma
r_int
id|isvfat
)paren
(brace
r_struct
id|inode
op_star
id|root_inode
op_assign
l_int|NULL
suffix:semicolon
r_struct
id|buffer_head
op_star
id|bh
suffix:semicolon
r_struct
id|fat_boot_sector
op_star
id|b
suffix:semicolon
r_struct
id|msdos_sb_info
op_star
id|sbi
suffix:semicolon
id|u16
id|logical_sector_size
suffix:semicolon
id|u32
id|total_sectors
comma
id|total_clusters
comma
id|fat_clusters
comma
id|rootdir_sectors
suffix:semicolon
r_int
id|debug
suffix:semicolon
r_int
r_int
id|media
suffix:semicolon
r_int
id|error
suffix:semicolon
r_char
id|buf
(braket
l_int|50
)braket
suffix:semicolon
id|sbi
op_assign
id|kmalloc
c_func
(paren
r_sizeof
(paren
r_struct
id|msdos_sb_info
)paren
comma
id|GFP_KERNEL
)paren
suffix:semicolon
r_if
c_cond
(paren
op_logical_neg
id|sbi
)paren
r_return
op_minus
id|ENOMEM
suffix:semicolon
id|sb-&gt;s_fs_info
op_assign
id|sbi
suffix:semicolon
id|memset
c_func
(paren
id|sbi
comma
l_int|0
comma
r_sizeof
(paren
r_struct
id|msdos_sb_info
)paren
)paren
suffix:semicolon
id|sb-&gt;s_flags
op_or_assign
id|MS_NODIRATIME
suffix:semicolon
id|sb-&gt;s_magic
op_assign
id|MSDOS_SUPER_MAGIC
suffix:semicolon
id|sb-&gt;s_op
op_assign
op_amp
id|fat_sops
suffix:semicolon
id|sb-&gt;s_export_op
op_assign
op_amp
id|fat_export_ops
suffix:semicolon
id|sbi-&gt;dir_ops
op_assign
id|fs_dir_inode_ops
suffix:semicolon
id|error
op_assign
id|parse_options
c_func
(paren
id|data
comma
id|isvfat
comma
op_amp
id|debug
comma
op_amp
id|sbi-&gt;options
)paren
suffix:semicolon
r_if
c_cond
(paren
id|error
)paren
r_goto
id|out_fail
suffix:semicolon
id|error
op_assign
op_minus
id|EIO
suffix:semicolon
id|sb_min_blocksize
c_func
(paren
id|sb
comma
l_int|512
)paren
suffix:semicolon
id|bh
op_assign
id|sb_bread
c_func
(paren
id|sb
comma
l_int|0
)paren
suffix:semicolon
r_if
c_cond
(paren
id|bh
op_eq
l_int|NULL
)paren
(brace
id|printk
c_func
(paren
id|KERN_ERR
l_string|&quot;FAT: unable to read boot sector&bslash;n&quot;
)paren
suffix:semicolon
r_goto
id|out_fail
suffix:semicolon
)brace
id|b
op_assign
(paren
r_struct
id|fat_boot_sector
op_star
)paren
id|bh-&gt;b_data
suffix:semicolon
r_if
c_cond
(paren
op_logical_neg
id|b-&gt;reserved
)paren
(brace
r_if
c_cond
(paren
op_logical_neg
id|silent
)paren
id|printk
c_func
(paren
id|KERN_ERR
l_string|&quot;FAT: bogus number of reserved sectors&bslash;n&quot;
)paren
suffix:semicolon
id|brelse
c_func
(paren
id|bh
)paren
suffix:semicolon
r_goto
id|out_invalid
suffix:semicolon
)brace
r_if
c_cond
(paren
op_logical_neg
id|b-&gt;fats
)paren
(brace
r_if
c_cond
(paren
op_logical_neg
id|silent
)paren
id|printk
c_func
(paren
id|KERN_ERR
l_string|&quot;FAT: bogus number of FAT structure&bslash;n&quot;
)paren
suffix:semicolon
id|brelse
c_func
(paren
id|bh
)paren
suffix:semicolon
r_goto
id|out_invalid
suffix:semicolon
)brace
multiline_comment|/*&n;&t; * Earlier we checked here that b-&gt;secs_track and b-&gt;head are nonzero,&n;&t; * but it turns out valid FAT filesystems can have zero there.&n;&t; */
id|media
op_assign
id|b-&gt;media
suffix:semicolon
r_if
c_cond
(paren
op_logical_neg
id|FAT_VALID_MEDIA
c_func
(paren
id|media
)paren
)paren
(brace
r_if
c_cond
(paren
op_logical_neg
id|silent
)paren
id|printk
c_func
(paren
id|KERN_ERR
l_string|&quot;FAT: invalid media value (0x%02x)&bslash;n&quot;
comma
id|media
)paren
suffix:semicolon
id|brelse
c_func
(paren
id|bh
)paren
suffix:semicolon
r_goto
id|out_invalid
suffix:semicolon
)brace
id|logical_sector_size
op_assign
id|le16_to_cpu
c_func
(paren
id|get_unaligned
c_func
(paren
(paren
id|__le16
op_star
)paren
op_amp
id|b-&gt;sector_size
)paren
)paren
suffix:semicolon
r_if
c_cond
(paren
op_logical_neg
id|logical_sector_size
op_logical_or
(paren
id|logical_sector_size
op_amp
(paren
id|logical_sector_size
op_minus
l_int|1
)paren
)paren
op_logical_or
(paren
id|logical_sector_size
OL
l_int|512
)paren
op_logical_or
(paren
id|PAGE_CACHE_SIZE
OL
id|logical_sector_size
)paren
)paren
(brace
r_if
c_cond
(paren
op_logical_neg
id|silent
)paren
id|printk
c_func
(paren
id|KERN_ERR
l_string|&quot;FAT: bogus logical sector size %u&bslash;n&quot;
comma
id|logical_sector_size
)paren
suffix:semicolon
id|brelse
c_func
(paren
id|bh
)paren
suffix:semicolon
r_goto
id|out_invalid
suffix:semicolon
)brace
id|sbi-&gt;sec_per_clus
op_assign
id|b-&gt;sec_per_clus
suffix:semicolon
r_if
c_cond
(paren
op_logical_neg
id|sbi-&gt;sec_per_clus
op_logical_or
(paren
id|sbi-&gt;sec_per_clus
op_amp
(paren
id|sbi-&gt;sec_per_clus
op_minus
l_int|1
)paren
)paren
)paren
(brace
r_if
c_cond
(paren
op_logical_neg
id|silent
)paren
id|printk
c_func
(paren
id|KERN_ERR
l_string|&quot;FAT: bogus sectors per cluster %u&bslash;n&quot;
comma
id|sbi-&gt;sec_per_clus
)paren
suffix:semicolon
id|brelse
c_func
(paren
id|bh
)paren
suffix:semicolon
r_goto
id|out_invalid
suffix:semicolon
)brace
r_if
c_cond
(paren
id|logical_sector_size
OL
id|sb-&gt;s_blocksize
)paren
(brace
id|printk
c_func
(paren
id|KERN_ERR
l_string|&quot;FAT: logical sector size too small for device&quot;
l_string|&quot; (logical sector size = %u)&bslash;n&quot;
comma
id|logical_sector_size
)paren
suffix:semicolon
id|brelse
c_func
(paren
id|bh
)paren
suffix:semicolon
r_goto
id|out_fail
suffix:semicolon
)brace
r_if
c_cond
(paren
id|logical_sector_size
OG
id|sb-&gt;s_blocksize
)paren
(brace
id|brelse
c_func
(paren
id|bh
)paren
suffix:semicolon
r_if
c_cond
(paren
op_logical_neg
id|sb_set_blocksize
c_func
(paren
id|sb
comma
id|logical_sector_size
)paren
)paren
(brace
id|printk
c_func
(paren
id|KERN_ERR
l_string|&quot;FAT: unable to set blocksize %u&bslash;n&quot;
comma
id|logical_sector_size
)paren
suffix:semicolon
r_goto
id|out_fail
suffix:semicolon
)brace
id|bh
op_assign
id|sb_bread
c_func
(paren
id|sb
comma
l_int|0
)paren
suffix:semicolon
r_if
c_cond
(paren
id|bh
op_eq
l_int|NULL
)paren
(brace
id|printk
c_func
(paren
id|KERN_ERR
l_string|&quot;FAT: unable to read boot sector&quot;
l_string|&quot; (logical sector size = %lu)&bslash;n&quot;
comma
id|sb-&gt;s_blocksize
)paren
suffix:semicolon
r_goto
id|out_fail
suffix:semicolon
)brace
id|b
op_assign
(paren
r_struct
id|fat_boot_sector
op_star
)paren
id|bh-&gt;b_data
suffix:semicolon
)brace
id|sbi-&gt;cluster_size
op_assign
id|sb-&gt;s_blocksize
op_star
id|sbi-&gt;sec_per_clus
suffix:semicolon
id|sbi-&gt;cluster_bits
op_assign
id|ffs
c_func
(paren
id|sbi-&gt;cluster_size
)paren
op_minus
l_int|1
suffix:semicolon
id|sbi-&gt;fats
op_assign
id|b-&gt;fats
suffix:semicolon
id|sbi-&gt;fat_bits
op_assign
l_int|0
suffix:semicolon
multiline_comment|/* Don&squot;t know yet */
id|sbi-&gt;fat_start
op_assign
id|le16_to_cpu
c_func
(paren
id|b-&gt;reserved
)paren
suffix:semicolon
id|sbi-&gt;fat_length
op_assign
id|le16_to_cpu
c_func
(paren
id|b-&gt;fat_length
)paren
suffix:semicolon
id|sbi-&gt;root_cluster
op_assign
l_int|0
suffix:semicolon
id|sbi-&gt;free_clusters
op_assign
op_minus
l_int|1
suffix:semicolon
multiline_comment|/* Don&squot;t know yet */
id|sbi-&gt;prev_free
op_assign
id|FAT_START_ENT
suffix:semicolon
r_if
c_cond
(paren
op_logical_neg
id|sbi-&gt;fat_length
op_logical_and
id|b-&gt;fat32_length
)paren
(brace
r_struct
id|fat_boot_fsinfo
op_star
id|fsinfo
suffix:semicolon
r_struct
id|buffer_head
op_star
id|fsinfo_bh
suffix:semicolon
multiline_comment|/* Must be FAT32 */
id|sbi-&gt;fat_bits
op_assign
l_int|32
suffix:semicolon
id|sbi-&gt;fat_length
op_assign
id|le32_to_cpu
c_func
(paren
id|b-&gt;fat32_length
)paren
suffix:semicolon
id|sbi-&gt;root_cluster
op_assign
id|le32_to_cpu
c_func
(paren
id|b-&gt;root_cluster
)paren
suffix:semicolon
id|sb-&gt;s_maxbytes
op_assign
l_int|0xffffffff
suffix:semicolon
multiline_comment|/* MC - if info_sector is 0, don&squot;t multiply by 0 */
id|sbi-&gt;fsinfo_sector
op_assign
id|le16_to_cpu
c_func
(paren
id|b-&gt;info_sector
)paren
suffix:semicolon
r_if
c_cond
(paren
id|sbi-&gt;fsinfo_sector
op_eq
l_int|0
)paren
id|sbi-&gt;fsinfo_sector
op_assign
l_int|1
suffix:semicolon
id|fsinfo_bh
op_assign
id|sb_bread
c_func
(paren
id|sb
comma
id|sbi-&gt;fsinfo_sector
)paren
suffix:semicolon
r_if
c_cond
(paren
id|fsinfo_bh
op_eq
l_int|NULL
)paren
(brace
id|printk
c_func
(paren
id|KERN_ERR
l_string|&quot;FAT: bread failed, FSINFO block&quot;
l_string|&quot; (sector = %lu)&bslash;n&quot;
comma
id|sbi-&gt;fsinfo_sector
)paren
suffix:semicolon
id|brelse
c_func
(paren
id|bh
)paren
suffix:semicolon
r_goto
id|out_fail
suffix:semicolon
)brace
id|fsinfo
op_assign
(paren
r_struct
id|fat_boot_fsinfo
op_star
)paren
id|fsinfo_bh-&gt;b_data
suffix:semicolon
r_if
c_cond
(paren
op_logical_neg
id|IS_FSINFO
c_func
(paren
id|fsinfo
)paren
)paren
(brace
id|printk
c_func
(paren
id|KERN_WARNING
l_string|&quot;FAT: Did not find valid FSINFO signature.&bslash;n&quot;
l_string|&quot;     Found signature1 0x%08x signature2 0x%08x&quot;
l_string|&quot; (sector = %lu)&bslash;n&quot;
comma
id|le32_to_cpu
c_func
(paren
id|fsinfo-&gt;signature1
)paren
comma
id|le32_to_cpu
c_func
(paren
id|fsinfo-&gt;signature2
)paren
comma
id|sbi-&gt;fsinfo_sector
)paren
suffix:semicolon
)brace
r_else
(brace
id|sbi-&gt;free_clusters
op_assign
id|le32_to_cpu
c_func
(paren
id|fsinfo-&gt;free_clusters
)paren
suffix:semicolon
id|sbi-&gt;prev_free
op_assign
id|le32_to_cpu
c_func
(paren
id|fsinfo-&gt;next_cluster
)paren
suffix:semicolon
)brace
id|brelse
c_func
(paren
id|fsinfo_bh
)paren
suffix:semicolon
)brace
id|sbi-&gt;dir_per_block
op_assign
id|sb-&gt;s_blocksize
op_div
r_sizeof
(paren
r_struct
id|msdos_dir_entry
)paren
suffix:semicolon
id|sbi-&gt;dir_per_block_bits
op_assign
id|ffs
c_func
(paren
id|sbi-&gt;dir_per_block
)paren
op_minus
l_int|1
suffix:semicolon
id|sbi-&gt;dir_start
op_assign
id|sbi-&gt;fat_start
op_plus
id|sbi-&gt;fats
op_star
id|sbi-&gt;fat_length
suffix:semicolon
id|sbi-&gt;dir_entries
op_assign
id|le16_to_cpu
c_func
(paren
id|get_unaligned
c_func
(paren
(paren
id|__le16
op_star
)paren
op_amp
id|b-&gt;dir_entries
)paren
)paren
suffix:semicolon
r_if
c_cond
(paren
id|sbi-&gt;dir_entries
op_amp
(paren
id|sbi-&gt;dir_per_block
op_minus
l_int|1
)paren
)paren
(brace
r_if
c_cond
(paren
op_logical_neg
id|silent
)paren
id|printk
c_func
(paren
id|KERN_ERR
l_string|&quot;FAT: bogus directroy-entries per block&quot;
l_string|&quot; (%u)&bslash;n&quot;
comma
id|sbi-&gt;dir_entries
)paren
suffix:semicolon
id|brelse
c_func
(paren
id|bh
)paren
suffix:semicolon
r_goto
id|out_invalid
suffix:semicolon
)brace
id|rootdir_sectors
op_assign
id|sbi-&gt;dir_entries
op_star
r_sizeof
(paren
r_struct
id|msdos_dir_entry
)paren
op_div
id|sb-&gt;s_blocksize
suffix:semicolon
id|sbi-&gt;data_start
op_assign
id|sbi-&gt;dir_start
op_plus
id|rootdir_sectors
suffix:semicolon
id|total_sectors
op_assign
id|le16_to_cpu
c_func
(paren
id|get_unaligned
c_func
(paren
(paren
id|__le16
op_star
)paren
op_amp
id|b-&gt;sectors
)paren
)paren
suffix:semicolon
r_if
c_cond
(paren
id|total_sectors
op_eq
l_int|0
)paren
id|total_sectors
op_assign
id|le32_to_cpu
c_func
(paren
id|b-&gt;total_sect
)paren
suffix:semicolon
id|total_clusters
op_assign
(paren
id|total_sectors
op_minus
id|sbi-&gt;data_start
)paren
op_div
id|sbi-&gt;sec_per_clus
suffix:semicolon
r_if
c_cond
(paren
id|sbi-&gt;fat_bits
op_ne
l_int|32
)paren
id|sbi-&gt;fat_bits
op_assign
(paren
id|total_clusters
OG
id|MAX_FAT12
)paren
ques
c_cond
l_int|16
suffix:colon
l_int|12
suffix:semicolon
multiline_comment|/* check that FAT table does not overflow */
id|fat_clusters
op_assign
id|sbi-&gt;fat_length
op_star
id|sb-&gt;s_blocksize
op_star
l_int|8
op_div
id|sbi-&gt;fat_bits
suffix:semicolon
id|total_clusters
op_assign
id|min
c_func
(paren
id|total_clusters
comma
id|fat_clusters
op_minus
id|FAT_START_ENT
)paren
suffix:semicolon
r_if
c_cond
(paren
id|total_clusters
OG
id|MAX_FAT
c_func
(paren
id|sb
)paren
)paren
(brace
r_if
c_cond
(paren
op_logical_neg
id|silent
)paren
id|printk
c_func
(paren
id|KERN_ERR
l_string|&quot;FAT: count of clusters too big (%u)&bslash;n&quot;
comma
id|total_clusters
)paren
suffix:semicolon
id|brelse
c_func
(paren
id|bh
)paren
suffix:semicolon
r_goto
id|out_invalid
suffix:semicolon
)brace
id|sbi-&gt;max_cluster
op_assign
id|total_clusters
op_plus
id|FAT_START_ENT
suffix:semicolon
multiline_comment|/* check the free_clusters, it&squot;s not necessarily correct */
r_if
c_cond
(paren
id|sbi-&gt;free_clusters
op_ne
op_minus
l_int|1
op_logical_and
id|sbi-&gt;free_clusters
OG
id|total_clusters
)paren
id|sbi-&gt;free_clusters
op_assign
op_minus
l_int|1
suffix:semicolon
multiline_comment|/* check the prev_free, it&squot;s not necessarily correct */
id|sbi-&gt;prev_free
op_mod_assign
id|sbi-&gt;max_cluster
suffix:semicolon
r_if
c_cond
(paren
id|sbi-&gt;prev_free
OL
id|FAT_START_ENT
)paren
id|sbi-&gt;prev_free
op_assign
id|FAT_START_ENT
suffix:semicolon
id|brelse
c_func
(paren
id|bh
)paren
suffix:semicolon
multiline_comment|/* set up enough so that it can read an inode */
id|fat_hash_init
c_func
(paren
id|sb
)paren
suffix:semicolon
id|fat_ent_access_init
c_func
(paren
id|sb
)paren
suffix:semicolon
multiline_comment|/*&n;&t; * The low byte of FAT&squot;s first entry must have same value with&n;&t; * media-field.  But in real world, too many devices is&n;&t; * writing wrong value.  So, removed that validity check.&n;&t; *&n;&t; * if (FAT_FIRST_ENT(sb, media) != first)&n;&t; */
id|error
op_assign
op_minus
id|EINVAL
suffix:semicolon
id|sprintf
c_func
(paren
id|buf
comma
l_string|&quot;cp%d&quot;
comma
id|sbi-&gt;options.codepage
)paren
suffix:semicolon
id|sbi-&gt;nls_disk
op_assign
id|load_nls
c_func
(paren
id|buf
)paren
suffix:semicolon
r_if
c_cond
(paren
op_logical_neg
id|sbi-&gt;nls_disk
)paren
(brace
id|printk
c_func
(paren
id|KERN_ERR
l_string|&quot;FAT: codepage %s not found&bslash;n&quot;
comma
id|buf
)paren
suffix:semicolon
r_goto
id|out_fail
suffix:semicolon
)brace
multiline_comment|/* FIXME: utf8 is using iocharset for upper/lower conversion */
r_if
c_cond
(paren
id|sbi-&gt;options.isvfat
)paren
(brace
id|sbi-&gt;nls_io
op_assign
id|load_nls
c_func
(paren
id|sbi-&gt;options.iocharset
)paren
suffix:semicolon
r_if
c_cond
(paren
op_logical_neg
id|sbi-&gt;nls_io
)paren
(brace
id|printk
c_func
(paren
id|KERN_ERR
l_string|&quot;FAT: IO charset %s not found&bslash;n&quot;
comma
id|sbi-&gt;options.iocharset
)paren
suffix:semicolon
r_goto
id|out_fail
suffix:semicolon
)brace
)brace
id|error
op_assign
op_minus
id|ENOMEM
suffix:semicolon
id|root_inode
op_assign
id|new_inode
c_func
(paren
id|sb
)paren
suffix:semicolon
r_if
c_cond
(paren
op_logical_neg
id|root_inode
)paren
r_goto
id|out_fail
suffix:semicolon
id|root_inode-&gt;i_ino
op_assign
id|MSDOS_ROOT_INO
suffix:semicolon
id|root_inode-&gt;i_version
op_assign
l_int|1
suffix:semicolon
id|error
op_assign
id|fat_read_root
c_func
(paren
id|root_inode
)paren
suffix:semicolon
r_if
c_cond
(paren
id|error
OL
l_int|0
)paren
r_goto
id|out_fail
suffix:semicolon
id|error
op_assign
op_minus
id|ENOMEM
suffix:semicolon
id|insert_inode_hash
c_func
(paren
id|root_inode
)paren
suffix:semicolon
id|sb-&gt;s_root
op_assign
id|d_alloc_root
c_func
(paren
id|root_inode
)paren
suffix:semicolon
r_if
c_cond
(paren
op_logical_neg
id|sb-&gt;s_root
)paren
(brace
id|printk
c_func
(paren
id|KERN_ERR
l_string|&quot;FAT: get root inode failed&bslash;n&quot;
)paren
suffix:semicolon
r_goto
id|out_fail
suffix:semicolon
)brace
r_return
l_int|0
suffix:semicolon
id|out_invalid
suffix:colon
id|error
op_assign
op_minus
id|EINVAL
suffix:semicolon
r_if
c_cond
(paren
op_logical_neg
id|silent
)paren
id|printk
c_func
(paren
id|KERN_INFO
l_string|&quot;VFS: Can&squot;t find a valid FAT filesystem&quot;
l_string|&quot; on dev %s.&bslash;n&quot;
comma
id|sb-&gt;s_id
)paren
suffix:semicolon
id|out_fail
suffix:colon
r_if
c_cond
(paren
id|root_inode
)paren
id|iput
c_func
(paren
id|root_inode
)paren
suffix:semicolon
r_if
c_cond
(paren
id|sbi-&gt;nls_io
)paren
id|unload_nls
c_func
(paren
id|sbi-&gt;nls_io
)paren
suffix:semicolon
r_if
c_cond
(paren
id|sbi-&gt;nls_disk
)paren
id|unload_nls
c_func
(paren
id|sbi-&gt;nls_disk
)paren
suffix:semicolon
r_if
c_cond
(paren
id|sbi-&gt;options.iocharset
op_ne
id|fat_default_iocharset
)paren
id|kfree
c_func
(paren
id|sbi-&gt;options.iocharset
)paren
suffix:semicolon
id|sb-&gt;s_fs_info
op_assign
l_int|NULL
suffix:semicolon
id|kfree
c_func
(paren
id|sbi
)paren
suffix:semicolon
r_return
id|error
suffix:semicolon
)brace
DECL|variable|fat_fill_super
id|EXPORT_SYMBOL
c_func
(paren
id|fat_fill_super
)paren
suffix:semicolon
r_int
id|__init
id|fat_cache_init
c_func
(paren
r_void
)paren
suffix:semicolon
r_void
id|__exit
id|fat_cache_destroy
c_func
(paren
r_void
)paren
suffix:semicolon
DECL|function|init_fat_fs
r_static
r_int
id|__init
id|init_fat_fs
c_func
(paren
r_void
)paren
(brace
r_int
id|ret
suffix:semicolon
id|ret
op_assign
id|fat_cache_init
c_func
(paren
)paren
suffix:semicolon
r_if
c_cond
(paren
id|ret
OL
l_int|0
)paren
r_return
id|ret
suffix:semicolon
r_return
id|fat_init_inodecache
c_func
(paren
)paren
suffix:semicolon
)brace
DECL|function|exit_fat_fs
r_static
r_void
id|__exit
id|exit_fat_fs
c_func
(paren
r_void
)paren
(brace
id|fat_cache_destroy
c_func
(paren
)paren
suffix:semicolon
id|fat_destroy_inodecache
c_func
(paren
)paren
suffix:semicolon
)brace
id|module_init
c_func
(paren
id|init_fat_fs
)paren
id|module_exit
c_func
(paren
id|exit_fat_fs
)paren
id|MODULE_LICENSE
c_func
(paren
l_string|&quot;GPL&quot;
)paren
suffix:semicolon
eof
