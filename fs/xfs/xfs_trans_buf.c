...
multiline_comment|/*&n; * This call is used to indicate that the buffer is going to&n; * be staled and was an inode buffer. This means it gets&n; * special processing during unpin - where any inodes &n; * associated with the buffer should be removed from ail.&n; * There is also special processing during recovery,&n; * any replay of the inodes in the buffer needs to be&n; * prevented as the buffer may have been reused.&n; */
r_void
DECL|function|xfs_trans_stale_inode_buf
