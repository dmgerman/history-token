...
DECL|macro|qdprintk
macro_line|# define qdprintk(s, args...)&t;&t;printk(s, ## args)
macro_line|#else
DECL|macro|qdprintk
macro_line|# define qdprintk(s, args...)&t;&t;do { } while (0)
macro_line|#endif
id|STATIC
r_int
...
c_func
(paren
l_string|&quot;cannot remove, quota on: flags=%x&bslash;n&quot;
comma
id|flags
)paren
suffix:semicolon
...
c_func
(paren
l_string|&quot;cannot set state, invalid op: op=%x flags=%x&bslash;n&quot;
comma
id|op
comma
id|flags
...
suffix:semicolon
multiline_comment|/*&n;&t; * Only root file system can have quotas enabled on disk but not&n;&t; * in core. Note that quota utilities (like quotaoff) _expect_&n;&t; * errno == EEXIST here.&n;&t; */
r_if
c_cond
(paren
id|mp-&gt;m_dev
...
suffix:semicolon
multiline_comment|/*&n;&t; * Root file system may or may not have quotas on in core.&n;&t; * We have to perform the quotaoff accordingly.&n;&t; */
r_if
c_cond
(paren
id|mp-&gt;m_dev
...
(brace
multiline_comment|/*&n;&t;&t;&t; * This can happen only with grp+usr quota&n;&t;&t;&t; * combination. Note: 1) accounting cannot be turned&n;&t;&t;&t; * off without enforcement also getting turned off.&n;&t;&t;&t; * 2) Every flag that exists in mpqflags MUST exist&n;&t;&t;&t; * in sbqflags (but not vice versa).&n;&t;&t;&t; * which means at this point sbqflags = UQ+GQ+..,&n;&t;&t;&t; * and mpqflags = UQ or GQ.&n;&t;&t;&t; */
id|ASSERT
c_func
(paren
id|sbflags
...
c_func
(paren
l_string|&quot;quotaoff, sbflags=%x flags=%x m_qflags=%x&bslash;n&quot;
comma
id|sbflags
comma
id|flags
...
suffix:semicolon
multiline_comment|/* XXX TBD Finish this for group quota support */
multiline_comment|/* We need to update the SB and mp separately */
r_return
id|XFS_ERROR
c_func
(paren
...
multiline_comment|/*&n;&t; * Nothing to do? Don&squot;t complain.&n;&t; * This happens when we&squot;re just turning off quota enforcement.&n;&t; */
r_if
c_cond
(paren
(paren
...
multiline_comment|/*&n; * This does two separate functions:&n; * Switch on quotas for the root file system. This will take effect only&n; * on reboot.&n; * Switch on (a given) quota enforcement for both root and non-root filesystems.&n; * This takes effect immediately.&n; */
id|STATIC
r_int
...
suffix:semicolon
multiline_comment|/*&n;&t; * If caller wants to turn on accounting on /, but accounting&n;&t; * is already turned on, ignore ACCTing flags.&n;&t; * Switching on quota accounting for non-root filesystems&n;&t; * must be done at mount time.&n;&t; */
id|accflags
op_assign
id|flags
op_amp
...
multiline_comment|/* Only rootfs can turn on quotas with a delayed effect */
id|ASSERT
c_func
(paren
op_logical_neg
...
suffix:semicolon
multiline_comment|/*&n;&t; * Change superblock version (if needed) for the root filesystem&n;&t; */
r_if
c_cond
(paren
id|rootfs
...
c_func
(paren
l_string|&quot;Old superblock version %x&bslash;n&quot;
comma
...
c_func
(paren
l_string|&quot;Converted to version %x&bslash;n&quot;
comma
id|mp-&gt;m_sb.sb_versionnum
)paren
suffix:semicolon
...
suffix:semicolon
multiline_comment|/*&n;&t; * If we had just turned on quotas (ondisk) for rootfs, or if we aren&squot;t&n;&t; * trying to switch on quota enforcement, we are done.&n;&t; */
r_if
c_cond
(paren
id|delay
...
suffix:semicolon
multiline_comment|/*&n;&t; * Switch on quota enforcement in core. This applies to both root&n;&t; * and non-root file systems.&n;&t; */
id|mutex_lock
c_func
(paren
op_amp
...
suffix:semicolon
multiline_comment|/*&n;&t; * If the qflags are different on disk, as can be the case when&n;&t; * root filesystem&squot;s quotas are being turned on, return them in the&n;&t; * HI 8 bits.&n;&t; */
r_if
c_cond
(paren
id|mp-&gt;m_dev
...
c_func
(paren
l_string|&quot;-----------DQTEST DQUOT----------------&bslash;n&quot;
)paren
suffix:semicolon
id|printk
c_func
...
c_func
(paren
l_string|&quot;quotacheck failed for %d, error = %d&bslash;nreason = %s&bslash;n&quot;
comma
id|INT_GET
c_func
(paren
...
c_func
(paren
l_string|&quot;quotacheck failed for %d (%s) [%d != %d]&bslash;n&quot;
comma
id|INT_GET
c_func
(paren
...
c_func
(paren
l_string|&quot;%d [%s] [0x%p] BLK TIMER NOT STARTED&bslash;n&quot;
comma
id|d-&gt;d_id
comma
id|DQFLAGTO_TYPESTR
...
c_func
(paren
l_string|&quot;%d [%s] [0x%p] INO TIMER NOT STARTED&bslash;n&quot;
comma
id|d-&gt;d_id
comma
id|DQFLAGTO_TYPESTR
...
c_func
(paren
l_string|&quot;%d [%s] [0x%p] qchecked&bslash;n&quot;
comma
id|d-&gt;d_id
comma
id|XFS_QM_ISUDQ
...
c_func
(paren
l_string|&quot;Bulkstat returned error 0x%x&bslash;n&quot;
comma
id|error
)paren
suffix:semicolon
...
c_func
(paren
l_string|&quot;Checking results against system dquots&bslash;n&quot;
)paren
suffix:semicolon
r_for
c_loop
...
c_func
(paren
l_string|&quot;**************&t;quotacheck failed  **************&bslash;n&quot;
)paren
suffix:semicolon
id|printk
c_func
(paren
l_string|&quot;failures = %d&bslash;n&quot;
comma
id|qmtest_nfails
)paren
suffix:semicolon
...
c_func
(paren
l_string|&quot;**************&t;quotacheck successful! **************&bslash;n&quot;
)paren
suffix:semicolon
)brace
id|kmem_free
