...
l_int|1
suffix:semicolon
multiline_comment|/*&n;&t; * XXXsup We could keep reference counts on usr and grp quotas&n;&t; * inside XQM separately, and avoid having two hashtables even&n;&t; * when only one &squot;type&squot; is active in the system.&n;&t; */
...
multiline_comment|/*&n; * This is called from xfs_mountfs to start quotas and initialize all&n; * necessary data structures like quotainfo, and in the rootfs&squot;s case&n; * xfs_Gqm. This is also responsible for running a quotacheck as necessary.&n; * We are guaranteed that the superblock is consistently read in at this&n; * point.&n; */
r_int
...
l_int|0
suffix:semicolon
multiline_comment|/*&n;&t; * If a non-root file system had quotas running earlier, but decided&n;&t; * to mount without -o quota/pquota options, revoke the quotachecked&n;&t; * license, and bail out.&n;&t; */
r_if
c_cond
(paren
op_logical_neg
...
suffix:semicolon
macro_line|#endif
multiline_comment|/*&n;&t; * If this is the root file system, mark flags in mount struct first.&n;&t; * We couldn&squot;t do this earlier because we didn&squot;t have the superblock&n;&t; * read in.&n;&t; */
r_if
c_cond
(paren
id|mp-&gt;m_dev
...
id|XFS_GQUOTA_ACTIVE
suffix:semicolon
multiline_comment|/*&n;&t;&t; * The quotainode of the root file system may or may not&n;&t;&t; * exist at this point.&n;&t;&t; */
)brace
id|ASSERT
c_func
(paren
...
c_cond
(paren
id|mp-&gt;m_rootip-&gt;i_udquot
op_logical_or
id|mp-&gt;m_rootip-&gt;i_gdquot
)paren
...
op_logical_and
(paren
id|mp-&gt;m_rbmip-&gt;i_udquot
op_logical_or
id|mp-&gt;m_rbmip-&gt;i_gdquot
)paren
)paren
id|xfs_qm_dqdettach_inode
c_func
...
op_logical_and
(paren
id|mp-&gt;m_rsumip-&gt;i_udquot
op_logical_or
id|mp-&gt;m_rsumip-&gt;i_gdquot
)paren
)paren
id|xfs_qm_dqdettach_inode
c_func
...
suffix:semicolon
multiline_comment|/*&n;&t;&t; * If this is the root filesystem doing a quotacheck,&n;&t;&t; * we should do periodic bflushes. This is because there&squot;s&n;&t;&t; * no bflushd at this point.&n;&t;&t; */
r_if
c_cond
(paren
id|mp-&gt;m_flags
op_amp
id|XFS_MOUNT_ROOTQCHECK
)paren
(brace
r_if
c_cond
...
multiline_comment|/* XXX XFS_STATS */
r_goto
id|done
suffix:semicolon
)brace
...
multiline_comment|/*&n;&t;&t;&t; * Don&squot;t worry about the udquot that we may have&n;&t;&t;&t; * attached above. It&squot;ll get dettached, if not already.&n;&t;&t;&t; */
r_goto
id|done
suffix:semicolon
id|nquotas
...
r_void
DECL|function|xfs_qm_dqdettach_inode
id|xfs_qm_dqdettach_inode
c_func
(paren
id|xfs_inode_t
...
multiline_comment|/*&n; * This is called by xfs_sync and flags arg determines the caller,&n; * and its motives, as done in xfs_sync.&n; *&n; * vfs_sync: SYNC_FSDATA|SYNC_ATTR|SYNC_BDFLUSH 0x31&n; * syscall sync: SYNC_FSDATA|SYNC_ATTR|SYNC_DELWRI 0x25&n; * umountroot : SYNC_WAIT | SYNC_CLOSE | SYNC_ATTR | SYNC_FSDATA&n; */
r_int
...
suffix:semicolon
multiline_comment|/*&n;&t;&t; * When quotachecking the root filesystem,&n;&t;&t; * we may not have bdflush, and we may fill&n;&t;&t; * up all available freebufs.&n;&t;&t; * The workaround here is to push on the&n;&t;&t; * log and do a bflush on the rootdev&n;&t;&t; * periodically.&n;&t;&t; */
r_if
c_cond
(paren
id|mp-&gt;m_flags
...
multiline_comment|/*&n;&t; * Adjust the timers since we just changed usages&n;&t; */
r_if
c_cond
(paren
op_logical_neg
...
id|tp
comma
multiline_comment|/* transaction pointer - NULL */
id|xfs_ino_t
id|ino
comma
...
multiline_comment|/*&n;&t; * Create the two inodes, if they don&squot;t exist already. The changes&n;&t; * made above will get added to a transaction and logged in one of&n;&t; * the qino_alloc calls below.&n;&t; */
r_if
c_cond
(paren
id|XFS_IS_UQUOTA_ON
...
c_func
(paren
l_string|&quot;Shake free 0x%x&bslash;n&quot;
comma
id|howmany
)paren
suffix:semicolon
...
c_func
(paren
id|xfsstats.xs_qm_dqinact_reclaims
)paren
suffix:semicolon
id|nextdqp
op_assign
...
suffix:semicolon
multiline_comment|/*&n;&t;&t;&t; * We&squot;ll be doing a dqflush, and it is&n;&t;&t;&t; * possible to fill up the entire buffer cache&n;&t;&t;&t; * with dirty delayed write buffers when doing&n;&t;&t;&t; * this on a root filesystem, if bdflush isn&squot;t&n;&t;&t;&t; * running. So, do a flush periodically.&n;&t;&t;&t; */
r_if
c_cond
(paren
id|dqp-&gt;q_mount-&gt;m_flags
op_amp
id|XFS_MOUNT_ROOTQCHECK
)paren
(brace
...
c_func
(paren
id|xfsstats.xs_qm_dqshake_reclaims
)paren
suffix:semicolon
id|xfs_qm_dqdestroy
c_func
...
c_func
(paren
id|xfsstats.xs_qm_dqreclaim_misses
)paren
suffix:semicolon
)brace
...
c_func
(paren
l_string|&quot;FREELIST destroy 0x%p&bslash;n&quot;
comma
id|dqp
)paren
suffix:semicolon
...
macro_line|#ifdef QUOTADEBUG
r_void
DECL|function|xfs_qm_freelist_print
