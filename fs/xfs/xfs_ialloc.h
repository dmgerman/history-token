...
mdefine_line|#define&t;__XFS_IALLOC_H__
r_struct
id|xfs_buf
suffix:semicolon
r_struct
...
mdefine_line|#define&t;XFS_IALLOC_INODES(mp)&t;xfs_ialloc_inodes(mp)
macro_line|#else
DECL|macro|XFS_IALLOC_INODES
mdefine_line|#define&t;XFS_IALLOC_INODES(mp)&t;((mp)-&gt;m_ialloc_inos)
macro_line|#endif
...
mdefine_line|#define&t;XFS_IALLOC_BLOCKS(mp)&t;xfs_ialloc_blocks(mp)
macro_line|#else
DECL|macro|XFS_IALLOC_BLOCKS
mdefine_line|#define&t;XFS_IALLOC_BLOCKS(mp)&t;((mp)-&gt;m_ialloc_blks)
macro_line|#endif
...
mdefine_line|#define&t;XFS_INODE_BIG_CLUSTER_SIZE&t;8192
DECL|macro|XFS_INODE_SMALL_CLUSTER_SIZE
mdefine_line|#define&t;XFS_INODE_SMALL_CLUSTER_SIZE&t;4096
DECL|macro|XFS_INODE_CLUSTER_SIZE
mdefine_line|#define&t;XFS_INODE_CLUSTER_SIZE(mp)&t;(mp)-&gt;m_inode_cluster_size
...
mdefine_line|#define&t;XFS_MAKE_IPTR(mp,b,o)&t;&t;xfs_make_iptr(mp,b,o)
macro_line|#else
DECL|macro|XFS_MAKE_IPTR
mdefine_line|#define&t;XFS_MAKE_IPTR(mp,b,o) &bslash;&n;&t;((xfs_dinode_t *)(xfs_buf_offset(b, (o) &lt;&lt; (mp)-&gt;m_sb.sb_inodelog)))
macro_line|#endif
...
mdefine_line|#define&t;XFS_IALLOC_FIND_FREE(fp)&t;xfs_ialloc_find_free(fp)
macro_line|#else
DECL|macro|XFS_IALLOC_FIND_FREE
mdefine_line|#define&t;XFS_IALLOC_FIND_FREE(fp)&t;xfs_lowbit64(*(fp))
macro_line|#endif
macro_line|#ifdef __KERNEL__
multiline_comment|/*&n; * Prototypes for visible xfs_ialloc.c routines.&n; */
multiline_comment|/*&n; * Allocate an inode on disk.&n; * Mode is used to tell whether the new inode will need space, and whether&n; * it is a directory.&n; *&n; * To work within the constraint of one allocation per transaction,&n; * xfs_dialloc() is designed to be called twice if it has to do an&n; * allocation to make more free inodes.  If an inode is&n; * available without an allocation, agbp would be set to the current&n; * agbp and alloc_done set to false.&n; * If an allocation needed to be done, agbp would be set to the&n; * inode header of the allocation group and alloc_done set to true.&n; * The caller should then commit the current transaction and allocate a new&n; * transaction.  xfs_dialloc() should then be called again with&n; * the agbp value returned from the previous call.&n; *&n; * Once we successfully pick an inode its number is returned and the&n; * on-disk data structures are updated.  The inode itself is not read&n; * in, since doing so would break ordering constraints with xfs_reclaim.&n; *&n; * *agbp should be set to NULL on the first call, *alloc_done set to FALSE.&n; */
r_int
