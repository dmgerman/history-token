...
DECL|macro|PUSH
mdefine_line|#define PUSH(low, high)&t;((void) ((top-&gt;lo = (low)), (top-&gt;hi = (high)), ++top))
DECL|macro|POP
mdefine_line|#define&t;POP(low, high)&t;((void) (--top, (low = top-&gt;lo), (high = top-&gt;hi)))
DECL|macro|STACK_NOT_EMPTY
mdefine_line|#define&t;STACK_NOT_EMPTY&t;(stack &lt; top)
multiline_comment|/* Order size using quicksort.  This implementation incorporates&n;   four optimizations discussed in Sedgewick:&n;&n;   1. Non-recursive, using an explicit stack of pointer that store the&n;      next array partition to sort.  To save time, this maximum amount&n;      of space required to store an array of SIZE_MAX is allocated on the&n;      stack.  Assuming a 32-bit (64 bit) integer for size_t, this needs&n;      only 32 * sizeof(stack_node) == 256 bytes (for 64 bit: 1024 bytes).&n;      Pretty cheap, actually.&n;&n;   2. Chose the pivot element using a median-of-three decision tree.&n;      This reduces the probability of selecting a bad pivot value and&n;      eliminates certain extraneous comparisons.&n;&n;   3. Only quicksorts TOTAL_ELEMS / MAX_THRESH partitions, leaving&n;      insertion sort to order the MAX_THRESH items within each partition.&n;      This is a big win, since insertion sort is faster for small, mostly&n;      sorted array segments.&n;&n;   4. The larger of the two sub-partitions is always pushed onto the&n;      stack first, with the algorithm then concentrating on the&n;      smaller partition.  This *guarantees* no more than log (total_elems)&n;      stack size is needed (actually O(1) in this case)!  */
r_void
DECL|function|qsort
id|qsort
(paren
