...
id|lsn
comma
multiline_comment|/* lsn looking for */
id|xfs_log_callback_t
op_star
id|cb
)paren
...
multiline_comment|/* This assert does not take into account padding from striped log writes *&n;&t;&t;ASSERT((tail_cycle == (cycle + 1)) ||&n;&t;&t;       ((bytes + log-&gt;l_roundoff) &gt;= tail_bytes));&n;*/
id|free_bytes
op_assign
id|log-&gt;l_logsize
suffix:semicolon
...
l_int|0
suffix:semicolon
multiline_comment|/* filled in by xlog_recover */
id|log-&gt;l_grant_reserve_bytes
op_assign
l_int|0
suffix:semicolon
...
l_int|0
suffix:semicolon
multiline_comment|/* XFS_LI_QUOTAOFF logitems */
id|xlog_get_iclog_buffer_size
c_func
(paren
id|mp
...
(brace
multiline_comment|/* count of 0 is already accounted for up in&n;&t;&t; * xlog_state_sync_all().  Once in this routine,&n;&t;&t; * operations on the iclog are single threaded.&n;&t;&t; *&n;&t;&t; * Difference between rounded up size and size&n;&t;&t; */
id|count
op_assign
id|iclog-&gt;ic_offset
op_amp
...
suffix:semicolon
multiline_comment|/* Count is already rounded up to a BBSIZE above */
id|count
op_assign
id|iclog-&gt;ic_offset
op_plus
...
macro_line|#endif
multiline_comment|/*&n;&t; * Transition from DIRTY to ACTIVE if applicable. NOP if&n;&t; * STATE_IOERROR.&n;&t; */
id|xlog_state_clean_log
c_func
(paren
id|log
...
multiline_comment|/*&n;&t; * Someone could be sleeping on the next iclog even though it is&n;&t; * in the ACTIVE state.&t; We kick off one thread to force the&n;&t; * iclog buffer out.&n;&t; */
r_if
c_cond
(paren
id|iclog-&gt;ic_next-&gt;ic_state
op_amp
(paren
id|XLOG_STATE_ACTIVE
op_or
...
multiline_comment|/*&n; * If the lsn is not found or the iclog with the lsn is in the callback&n; * state, we need to call the function directly.  This is done outside&n; * this function&squot;s scope.  Otherwise, we insert the callback at the end&n; * of the iclog&squot;s callback list.&n; */
r_int
DECL|function|xlog_state_lsn_is_synced
id|xlog_state_lsn_is_synced
c_func
(paren
id|xlog_t
...
multiline_comment|/* insert callback onto end of list */
id|cb-&gt;cb_next
op_assign
l_int|0
suffix:semicolon
...
multiline_comment|/* xlog_state_lsn_is_synced */
