...
suffix:semicolon
multiline_comment|/*&n;&t;&t;&t; * XXX:casey - This may result in unnecessary auditing.&n;&t;&t;&t; */
id|privileged
op_assign
id|capable
c_func
...
suffix:semicolon
multiline_comment|/*&n;&t;&t;&t;&t; * We&squot;ll dqrele olddquot at the end.&n;&t;&t;&t;&t; */
)brace
id|ip-&gt;i_d.di_uid
op_assign
id|uid
...
id|XFS_DIFLAG_REALTIME
suffix:semicolon
multiline_comment|/* This is replicated in the io core for&n;&t;&t;&t;&t; * CXFS use&n;&t;&t;&t;&t; */
...
suffix:semicolon
multiline_comment|/*&n;&t; * release any dquot(s) inode had kept before chown&n;&t; */
r_if
c_cond
(paren
id|olddquot1
...
multiline_comment|/*&n;&t; * If we&squot;re invalidating, always flush since we want to&n;&t; * tear things down.  Otherwise, don&squot;t flush anything if&n;&t; * we&squot;re not dirty.&n;&t; */
r_if
c_cond
(paren
id|flag
...
r_else
(brace
multiline_comment|/*&n;&t;&t; * In the non-invalidating case, calls to fsync() do not&n;&t;&t; * flush all the dirty mmap&squot;d pages.  That requires a&n;&t;&t; * call to msync().&n;&t;&t; */
id|VOP_FLUSH_PAGES
c_func
(paren
id|vp
...
(brace
multiline_comment|/*&n;&t;&t;&t; * Timestamps/size haven&squot;t changed since last inode&n;&t;&t;&t; * flush or inode transaction commit.  That means&n;&t;&t;&t; * either nothing got written or a transaction&n;&t;&t;&t; * committed which caught the updates.&t;If the&n;&t;&t;&t; * latter happened and the transaction hasn&squot;t&n;&t;&t;&t; * hit the disk yet, the inode will be still&n;&t;&t;&t; * be pinned.  If it is, force the log.&n;&t;&t;&t; */
r_if
c_cond
(paren
id|xfs_ipincount
...
r_else
(brace
multiline_comment|/*&n;&t;&t;&t; * Kick off a transaction to log the inode&n;&t;&t;&t; * core to get the updates.  Make it&n;&t;&t;&t; * sync if FSYNC_WAIT is passed in (which&n;&t;&t;&t; * is done by everybody but specfs).  The&n;&t;&t;&t; * sync transaction will also force the log.&n;&t;&t;&t; */
id|xfs_iunlock
c_func
(paren
id|ip
...
suffix:semicolon
multiline_comment|/*&n;&t;&t;&t; * Note - it&squot;s possible that we might have pushed&n;&t;&t;&t; * ourselves out of the way during trans_reserve&n;&t;&t;&t; * which would flush the inode.&t; But there&squot;s no&n;&t;&t;&t; * guarantee that the inode buffer has actually&n;&t;&t;&t; * gone out yet (it&squot;s delwri).&t;Plus the buffer&n;&t;&t;&t; * could be pinned anyway if it&squot;s part of an&n;&t;&t;&t; * inode in another recent transaction.&t; So we&n;&t;&t;&t; * play it safe and fire off the transaction anyway.&n;&t;&t;&t; */
id|xfs_trans_ijoin
c_func
(paren
id|tp
...
r_else
(brace
multiline_comment|/*&n;&t;&t; * We don&squot;t care about the timestamps here.  We&n;&t;&t; * only care about the size field growing on us&n;&t;&t; * and forcing any space allocation transactions.&n;&t;&t; * We have to flush changes to the size fields&n;&t;&t; * otherwise we could write out data that&n;&t;&t; * becomes inaccessible after a crash.&n;&t;&t; */
id|xfs_ilock
c_func
(paren
id|ip
...
(brace
multiline_comment|/*&n;&t;&t;&t; * Force the log if the inode is pinned.&n;&t;&t;&t; * That ensures that all transactions committed&n;&t;&t;&t; * against the inode hit the disk.  This may do&n;&t;&t;&t; * too much work but it&squot;s safe.&n;&t;&t;&t; */
r_if
c_cond
(paren
id|xfs_ipincount
...
r_else
(brace
multiline_comment|/*&n;&t;&t;&t; * Kick off a sync transaction to log the inode&n;&t;&t;&t; * core.  The transaction has to be sync since&n;&t;&t;&t; * we need these updates to guarantee that the&n;&t;&t;&t; * data written will be seen.  The sync&n;&t;&t;&t; * transaction will also force the log.&n;&t;&t;&t; */
id|xfs_iunlock
c_func
(paren
id|ip
...
id|STATIC
r_void
DECL|function|xfs_ctrunc_trace
id|xfs_ctrunc_trace
c_func
(paren
r_int
...
macro_line|#endif /* XFS_RW_TRACE */
DECL|macro|XFS_CREATE_NEW_MAXTRIES
mdefine_line|#define XFS_CREATE_NEW_MAXTRIES 10000
multiline_comment|/*&n; * xfs_create (create a new file).&n; *   It might still find name exists out there, though.&n; *&t;But vpp, doens&squot;t point at a vnode.&n; */
id|STATIC
r_int
...
id|e_inum_saved
suffix:semicolon
multiline_comment|/* for retry trap code */
r_int
id|namelen
suffix:semicolon
id|ASSERT
...
suffix:semicolon
multiline_comment|/*&n;&t; * At this point we cannot do an xfs_iget() of the entry named&n;&t; * since we are already holding a log reservation and it could&n;&t; * be in xfs_inactive waiting for a log reservation.  We&squot;d just&n;&t; * end up waiting forever for the inactive to complete.&t; Instead&n;&t; * we just look to see if there is an entry with the name.  In&n;&t; * the case where there is not, we didn&squot;t need the inode anyway.&n;&t; * In the case where there is an entry, we&squot;ll get it later after&n;&t; * dropping our transaction.&n;&t; */
id|error
op_assign
id|xfs_dir_lookup_int
c_func
...
suffix:semicolon
multiline_comment|/*&n;&t;&t; * Reserve disk quota and the inode.&n;&t;&t; */
r_if
c_cond
(paren
id|XFS_IS_QUOTA_ON
...
suffix:semicolon
multiline_comment|/*&n;&t;&t; * At this point, we&squot;ve gotten a newly allocated inode.&n;&t;&t; * It is locked (and joined to the transaction).&n;&t;&t; */
id|ASSERT
c_func
(paren
id|ismrlocked
...
suffix:semicolon
multiline_comment|/*&n;&t;&t; * Now we join the directory inode to the transaction.&n;&t;&t; * We do not do it earlier because xfs_dir_ialloc&n;&t;&t; * might commit the previous transaction (and release&n;&t;&t; * all the locks).&n;&t;&t; */
id|VN_HOLD
c_func
(paren
id|dir_vp
...
suffix:semicolon
multiline_comment|/*&n;&t;&t; * If this is a synchronous mount, make sure that the&n;&t;&t; * create transaction goes to disk before returning to&n;&t;&t; * the user.&n;&t;&t; */
r_if
c_cond
(paren
id|mp-&gt;m_flags
...
op_increment
suffix:semicolon
multiline_comment|/*&n;&t;&t; * Attach the dquot(s) to the inodes and modify them incore.&n;&t;&t; * These ids of the inode couldn&squot;t have changed since the new&n;&t;&t; * inode has been locked ever since it was created.&n;&t;&t; */
r_if
c_cond
(paren
id|XFS_IS_QUOTA_ON
...
id|e_inum
suffix:semicolon
multiline_comment|/*&n;&t;&t; * The file already exists, so we&squot;re in the wrong&n;&t;&t; * transaction for this operation.  Cancel the old&n;&t;&t; * transaction and start a new one.  We have to drop&n;&t;&t; * our locks in doing this. But, we don&squot;t care&n;&t;&t; * if the directory changes. We have already checked&n;&t;&t; * if the dir exists and is EXEC by the user.&n;&t;&t; * After all, we already have the vnode held.&n;&t;&t; *&n;&t;&t; * All we need to do is truncate it.&n;&t;&t; */
id|xfs_trans_cancel
c_func
(paren
id|tp
...
l_int|NULL
suffix:semicolon
multiline_comment|/*&n;&t;&t; * Now that we&squot;ve dropped our log reservation, get a&n;&t;&t; * reference to the inode we are re-creating.  If we&n;&t;&t; * have to drop the directory lock in the call and&n;&t;&t; * the entry is removed, then just start over.&n;&t;&t; */
id|error
op_assign
id|xfs_dir_lookup_int
c_func
...
l_int|NULL
suffix:semicolon
multiline_comment|/*&n;&t;&t; * Done with directory. Don&squot;t care about it&n;&t;&t; * anymore.&n;&t;&t; */
multiline_comment|/*&n;&t;&t; * Since we&squot;re at a good, clean point, check for any&n;&t;&t; * obvious problems and get out if they occur.&n;&t;&t; */
id|vp
op_assign
id|XFS_ITOV
c_func
...
multiline_comment|/* Need the behaviour lock before the iolock as we are&n;&t;&t; * potentially going to make a VOP call on vp. */
id|VN_BHV_READ_LOCK
c_func
(paren
op_amp
...
suffix:semicolon
multiline_comment|/*&n;&t;&t; * We need to do the xfs_itruncate_start call before&n;&t;&t; * reserving any log space in the transaction.&n;&t;&t; */
id|xfs_ilock
c_func
(paren
id|ip
...
multiline_comment|/* Once we got the I/O lock we can drop the behaviour&n;&t;&t; * lock.&n;&t;&t; */
...
multiline_comment|/*&n;&t;&t; * Now lock inode.&n;&t;&t; */
id|xfs_ilock
c_func
(paren
id|ip
...
(brace
multiline_comment|/*&n;&t;&t;&t; * Truncate the file.  The timestamps must&n;&t;&t;&t; * be updated whether the file is changed&n;&t;&t;&t; * or not.&n;&t;&t;&t; */
id|ASSERT
c_func
(paren
id|vap-&gt;va_size
...
suffix:semicolon
multiline_comment|/*&n;&t;&t;&t;&t; * always signal sync xaction.&t;We&squot;re&n;&t;&t;&t;&t; * truncating an existing file so the&n;&t;&t;&t;&t; * xaction must be sync regardless&n;&t;&t;&t;&t; * of whether the filesystem is wsync&n;&t;&t;&t;&t; * to make the truncate persistent&n;&t;&t;&t;&t; * in the face of a crash.&n;&t;&t;&t;&t; */
id|error
op_assign
id|xfs_itruncate_finish
c_func
...
(brace
multiline_comment|/*&n;&t;&t;&t; * If we truncated the file, then the inode will&n;&t;&t;&t; * have been held within the previous transaction&n;&t;&t;&t; * and must be unlocked now.&n;&t;&t;&t; */
id|xfs_iunlock
c_func
(paren
id|ip
...
(brace
multiline_comment|/*&n;&t;&t; * If we truncated the file, then the inode will&n;&t;&t; * have been held within the transaction and must&n;&t;&t; * be unlocked now.&n;&t;&t; */
id|xfs_iunlock
c_func
(paren
id|ip
...
suffix:semicolon
macro_line|#endif
multiline_comment|/*&n; * The following routine will lock the inodes associated with the&n; * directory and the named entry in the directory. The locks are&n; * acquired in increasing inode number.&n; *&n; * If the entry is &quot;..&quot;, then only the directory is locked. The&n; * vnode ref count will still include that from the .. entry in&n; * this case.&n; *&n; * The inode passed in will have been looked up using xfs_get_dir_entry().&n; * Since that lookup the directory lock will have been dropped, so&n; * we need to validate that the inode given is still pointed to by the&n; * directory.  We use the directory inode in memory generation count&n; * as an optimization to tell if a new lookup is necessary.  If the&n; * directory no longer points to the given inode with the given name,&n; * then we drop the directory lock, set the entry_changed parameter to 1,&n; * and return.&t;It is up to the caller to drop the reference to the inode.&n; *&n; * There is a dealock we need to worry about. If the locked directory is&n; * in the AIL, it might be blocking up the log. The next inode we lock&n; * could be already locked by another thread waiting for log space (e.g&n; * a permanent log reservation with a long running transaction (see&n; * xfs_itruncate_finish)). To solve this, we must check if the directory&n; * is in the ail and use lock_nowait. If we can&squot;t lock, we need to&n; * drop the inode lock on the directory and try again. xfs_iunlock will&n; * potentially push the tail if we were holding up the log.&n; */
id|STATIC
r_int
...
multiline_comment|/*&n;&t; * If the inode we found in the first pass is no longer&n;&t; * the entry with the given name, then drop our transaction and&n;&t; * inode reference and start over.&n;&t; */
r_if
c_cond
(paren
id|entry_changed
...
multiline_comment|/*&n;&t; * Return error when removing . and ..&n;&t; */
r_if
c_cond
(paren
id|name
...
multiline_comment|/*&n;&t; * Make sure that nothing with the given name exists in the&n;&t; * target directory.&n;&t; */
id|error
op_assign
id|xfs_dir_lookup_int
c_func
...
suffix:semicolon
multiline_comment|/*&n;&t; * Since we&squot;ve already started a transaction, we cannot allow&n;&t; * the lookup to do a vn_get().&n;&t; */
id|error
op_assign
id|xfs_dir_lookup_int
c_func
...
l_int|0
suffix:semicolon
multiline_comment|/*&n;&t; * use bcopy because the inode is a long long and there&squot;s no&n;&t; * assurance that xfid-&gt;fid_ino is properly aligned.&n;&t; */
id|bcopy
c_func
(paren
op_amp
...
(brace
multiline_comment|/*&n;&t;&t; * In the write case, we may have added a new entry to&n;&t;&t; * the reference cache.&t; This might store a pointer to&n;&t;&t; * an inode to be released in this inode.  If it is there,&n;&t;&t; * clear the pointer and release the inode after unlocking&n;&t;&t; * this one.&n;&t;&t; */
id|release_ip
op_assign
id|ip-&gt;i_release
suffix:semicolon
...
suffix:semicolon
macro_line|#if 0
multiline_comment|/* not turning this on until some&n;&t;&t;&t;&t; * performance analysis is done&n;&t;&t;&t;&t; */
r_if
c_cond
(paren
id|flags
...
suffix:semicolon
multiline_comment|/*&n;&t; * Need to zero the stuff we&squot;re not freeing, on disk.&n;&t; * If its specrt (realtime &amp; can&squot;t use unwritten extents) then&n;&t; * we actually need to zero the extent edges.  Otherwise xfs_bunmapi&n;&t; * will take care of it for us.&n;&t; */
r_if
c_cond
(paren
id|specrt
