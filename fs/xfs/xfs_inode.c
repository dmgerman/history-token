...
mdefine_line|#define xfs_validate_extents(ep, nrecs, fmt)
...
id|real_size
suffix:semicolon
multiline_comment|/*&n;&t; * If the size is unreasonable, then something&n;&t; * is wrong and we just bail out rather than crash in&n;&t; * kmem_alloc() or bcopy() below.&n;&t; */
r_if
c_cond
(paren
id|size
...
suffix:semicolon
multiline_comment|/*&n;&t; * If the number of extents is unreasonable, then something&n;&t; * is wrong and we just bail out rather than crash in&n;&t; * kmem_alloc() or bcopy() below.&n;&t; */
r_if
c_cond
(paren
id|size
...
multiline_comment|/*&n; * xfs_iextents_copy()&n; *&n; * This is called to copy the REAL extents (as opposed to the delayed&n; * allocation extents) from the inode into the given buffer.  It&n; * returns the number of bytes copied into the buffer.&n; *&n; * If there are no delayed allocation extents, then we can just&n; * bcopy() the extents into the buffer.&t; Otherwise, we need to&n; * examine each extent in turn and skip those which are delayed.&n; */
r_int
...
(brace
multiline_comment|/*&n;&t;&t; * There are no delayed allocation extents,&n;&t;&t; * so just copy everything.&n;&t;&t; */
id|ASSERT
c_func
(paren
id|ifp-&gt;if_bytes
...
multiline_comment|/*&n; * xfs_ibusy_check -- Checks whether inode reference count allows unmount&n; *&n; * The value returned is one if the reference count would prevent an unmount.&n; */
r_int
DECL|function|xfs_ibusy_check
id|xfs_ibusy_check
c_func
(paren
id|xfs_inode_t
