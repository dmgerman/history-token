...
multiline_comment|/*&n; * xfs sync routine for internal use&n; *&n; * This routine supports all of the flags defined for the generic VFS_SYNC&n; * interface as explained above under xys_sync.  In the interests of not&n; * changing interfaces within the 6.5 family, additional internallly-&n; * required functions are specified within a separate xflags parameter,&n; * only available by calling this routine.&n; *&n; * xflags:&n; *&t;XFS_XSYNC_RELOC - Sync for relocation.  Don&squot;t try to get behavior&n; *                        locks as this will cause you to hang.  Not all&n; *                        combinations of flags are necessarily supported&n; *                        when this is specified.&n; */
r_int
