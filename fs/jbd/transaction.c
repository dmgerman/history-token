...
multiline_comment|/**&n; * int journal_get_write_access() - notify intent to modify a buffer for metadata (not data) update.&n; * @handle: transaction to add buffer modifications to&n; * @bh:     bh to be used for metadata writes&n; * @credits: variable that will receive credits for the buffer&n; *&n; * Returns an error code or 0 on success.&n; *&n; * In full data journalling mode the buffer may be of type BJ_AsyncData,&n; * because we&squot;re write()ing a buffer which is also part of a shared mapping.&n; */
