...
(brace
multiline_comment|/* We can&squot;t reliably test the buffer state if we found&n;&t;&t; * it already locked, so just wait for the lock and&n;&t;&t; * retry. */
id|unlock_journal
c_func
(paren
id|journal
...
multiline_comment|/* It has been modified by a later transaction: add it to the&n;&t; * new transaction&squot;s metadata list. */
id|was_dirty
op_assign
id|test_clear_buffer_jbddirty
c_func
