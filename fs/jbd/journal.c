...
suffix:semicolon
DECL|variable|journal_start_commit
id|EXPORT_SYMBOL
c_func
(paren
...
suffix:semicolon
DECL|variable|journal_force_commit_nested
id|EXPORT_SYMBOL
c_func
(paren
...
(brace
multiline_comment|/*&n;&t;&t; * The simpler the better. Flushing journal isn&squot;t a&n;&t;&t; * good idea, because that depends on threads that may&n;&t;&t; * be already stopped.&n;&t;&t; */
id|jbd_debug
c_func
(paren
l_int|1
...
r_else
(brace
multiline_comment|/*&n;&t;&t; * We assume on resume that commits are already there,&n;&t;&t; * so we don&squot;t sleep&n;&t;&t; */
id|DEFINE_WAIT
c_func
(paren
id|wait
...
suffix:semicolon
multiline_comment|/*&n;&t; * Were we woken up by a commit wakeup event?&n;&t; */
id|transaction
op_assign
id|journal-&gt;j_running_transaction
suffix:semicolon
...
multiline_comment|/*&n; * Called under j_state_lock.  Returns true if a transaction was started.&n; */
...
(brace
multiline_comment|/*&n;&t;&t; * We want a new commit: OK, mark the request and wakup the&n;&t;&t; * commit thread.  We do _not_ do the commit ourselves.&n;&t;&t; */
id|journal-&gt;j_commit_request
op_assign
id|target
suffix:semicolon
...
multiline_comment|/*&n; * Force and wait upon a commit if the calling process is not within&n; * transaction.  This is used for forcing out undo-protected data which contains&n; * bitmaps, when the fs is running out of space.&n; *&n; * We can only force the running transaction if we don&squot;t have an active handle;&n; * otherwise, we will deadlock.&n; *&n; * Returns true if a transaction was started.&n; */
DECL|function|journal_force_commit_nested
r_int
id|journal_force_commit_nested
c_func
(paren
...
l_int|0
suffix:semicolon
multiline_comment|/* Nothing to retry */
)brace
id|tid
op_assign
id|transaction-&gt;t_tid
...
multiline_comment|/*&n; * Start a commit of the current running transaction (if any).  Returns true&n; * if a transaction was started, and fills its tid in at *ptid&n; */
DECL|function|journal_start_commit
r_int
id|journal_start_commit
c_func
(paren
...
id|tid
op_assign
id|journal-&gt;j_running_transaction-&gt;t_tid
suffix:semicolon
id|ret
op_assign
id|__log_start_commit
...
id|bh-&gt;b_data
suffix:semicolon
multiline_comment|/* journal descriptor can store up to n blocks -bzzz */
id|n
op_assign
id|journal-&gt;j_blocksize
op_div
...
(paren
id|KERN_ERR
l_string|&quot;%s: Cant allocate bhs for commit thread&bslash;n&quot;
comma
id|__FUNCTION__
)paren
suffix:semicolon
...
multiline_comment|/*&n; * Grab a ref against this buffer_head&squot;s journal_head.  If it ended up not&n; * having a journal_head, return NULL&n; */
DECL|function|journal_grab_journal_head
r_struct
id|journal_head
op_star
...
id|KERN_WARNING
l_string|&quot;%s: freeing &quot;
l_string|&quot;b_frozen_data&bslash;n&quot;
comma
id|__FUNCTION__
)paren
suffix:semicolon
...
macro_line|#endif
DECL|variable|jbd_handle_cache
id|kmem_cache_t
op_star
id|jbd_handle_cache
suffix:semicolon
DECL|function|journal_init_handle_cache
r_static
r_int
id|__init
id|journal_init_handle_cache
c_func
(paren
r_void
)paren
...
(paren
id|KERN_EMERG
l_string|&quot;JBD: failed to create handle cache&bslash;n&quot;
)paren
suffix:semicolon
r_return
op_minus
...
DECL|function|journal_destroy_handle_cache
r_static
r_void
id|journal_destroy_handle_cache
c_func
(paren
r_void
)paren
