...
multiline_comment|/* Done it all: now write the commit record.  We should have&n; * cleaned up our previous buffers by now, so if we are in abort&n; * mode we can now just skip the rest of the journal write&n; * entirely.&n; *&n; * Returns 1 if the journal needs to be aborted or 0 on success&n; */
DECL|function|journal_write_commit_record
r_static
r_int
id|journal_write_commit_record
c_func
(paren
id|journal_t
op_star
...
suffix:semicolon
multiline_comment|/* is it possible for another commit to fail at roughly&n;&t; * the same time as this one?  If so, we don&squot;t want to&n;&t; * trust the barrier flag in the super, but instead want&n;&t; * to remember if we sent a barrier request&n;&t; */
r_if
c_cond
(paren
id|ret
...
suffix:semicolon
multiline_comment|/*&n;&t; * First, drop modified flag: all accesses to the buffers&n;&t; * will be tracked for a new trasaction only -bzzz&n;&t; */
id|spin_lock
c_func
(paren
op_amp
...
id|jh
op_assign
id|commit_transaction-&gt;t_buffers-&gt;b_tnext
suffix:semicolon
r_do
(brace
id|J_ASSERT_JH
