...
suffix:semicolon
multiline_comment|/**&n; * ntfs_write_mst_block - write a @page to the backing store&n; * @wbc:&t;writeback control structure&n; * @page:&t;page cache page to write out&n; *&n; * This function is for writing pages belonging to non-resident, mst protected&n; * attributes to their backing store.  The only supported attribute is the&n; * index allocation attribute.  Both directory inodes and index inodes are&n; * supported.&n; *&n; * The page must remain locked for the duration of the write because we apply&n; * the mst fixups, write, and then undo the fixups, so if we were to unlock the&n; * page before undoing the fixups, any other user of the page will see the&n; * page contents as corrupt.&n; *&n; * Return 0 on success and -errno on error.&n; *&n; * Based on ntfs_write_block(), ntfs_mft_writepage(), and&n; * write_mft_record_nolock().&n; */
...
id|vol-&gt;sb
comma
l_string|&quot;Writing ntfs records without existing &quot;
...
l_int|0
suffix:semicolon
multiline_comment|/* Need this to silence a stupid gcc warning. */
id|rec_is_dirty
op_assign
id|FALSE
suffix:semicolon
...
id|vol-&gt;sb
comma
l_string|&quot;Writing ntfs records without &quot;
...
id|vol-&gt;sb
comma
l_string|&quot;Failed to apply mst &quot;
l_string|&quot;fixups (inode 0x%lx, &quot;
l_string|&quot;attribute type 0x%x, page &quot;
l_string|&quot;index 0x%lx)!  Umount and &quot;
l_string|&quot;run chkdsk.&quot;
comma
id|vi-&gt;i_ino
comma
...
id|vol-&gt;sb
comma
l_string|&quot;I/O error while writing ntfs record &quot;
l_string|&quot;(inode 0x%lx, attribute type 0x%x, page &quot;
l_string|&quot;index 0x%lx)!  Umount and run chkdsk.&quot;
comma
id|vi-&gt;i_ino
comma
id|ni-&gt;type
...
suffix:semicolon
macro_line|#ifdef NTFS_RW
multiline_comment|/**&n; * mark_ntfs_record_dirty - mark an ntfs record dirty&n; * @ni:&t;&t;ntfs inode to which the ntfs record to be marked dirty belongs&n; * @page:&t;page containing the ntfs record to mark dirty&n; * @rec_start:&t;byte offset within @page at which the ntfs record begins&n; *&n; * If the ntfs record is the same size as the page cache page @page, set all&n; * buffers in the page dirty.  Otherwise, set only the buffers in which the&n; * ntfs record is located dirty.&n; *&n; * Also, set the page containing the ntfs record dirty, which also marks the&n; * vfs inode the ntfs record belongs to dirty (I_DIRTY_PAGES).&n; */
