...
multiline_comment|/**&n; * ntfs_write_mst_block - write a @page to the backing store&n; * @wbc:&t;writeback control structure&n; * @page:&t;page cache page to write out&n; *&n; * This function is for writing pages belonging to non-resident, mst protected&n; * attributes to their backing store.  The only supported attributes are index&n; * allocation and $MFT/$DATA.  Both directory inodes and index inodes are&n; * supported for the index allocation case.&n; *&n; * The page must remain locked for the duration of the write because we apply&n; * the mst fixups, write, and then undo the fixups, so if we were to unlock the&n; * page before undoing the fixups, any other user of the page will see the&n; * page contents as corrupt.&n; *&n; * We clear the page uptodate flag for the duration of the function to ensure&n; * exclusion for the $MFT/$DATA case against someone mapping an mft record we&n; * are about to apply the mst fixups to.&n; *&n; * Return 0 on success and -errno on error.&n; *&n; * Based on ntfs_write_block(), ntfs_mft_writepage(), and&n; * write_mft_record_nolock().&n; */
