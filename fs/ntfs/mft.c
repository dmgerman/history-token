...
multiline_comment|/**&n; * map_mft_record - map, pin and lock an mft record&n; * @rw:&t;&t;map for read (rw = READ) or write (rw = WRITE)&n; * @ni:&t;&t;ntfs inode whose MFT record to map&n; *&n; * First, take the mrec_lock semaphore for reading or writing, depending on&n; * the value or @rw. We might now be sleeping, while waiting for the semaphore&n; * if it was already locked by someone else.&n; *&n; * Then increment the map reference count and return the mft. If this is the&n; * first invocation, the page of the record is first mapped using&n; * map_mft_record_page().&n; *&n; * This in turn uses ntfs_map_page() to get the page containing the wanted mft&n; * record (it in turn calls read_cache_page() which reads it in from disk if&n; * necessary, increments the use count on the page so that it cannot disappear&n; * under us and returns a reference to the page cache page).&n; *&n; * If read_cache_page() invokes ntfs_mst_readpage() to load the page from disk,&n; * it sets PG_locked and clears PG_uptodate on the page. Once I/O has&n; * completed and the post-read mst fixups on each mft record in the page have&n; * been performed, the page gets PG_uptodate set and PG_locked cleared (this is&n; * done in our asynchronous I/O completion handler end_buffer_read_mft_async()).&n; * ntfs_map_page() waits for PG_locked to become clear and checks if&n; * PG_uptodate is set and returns an error code if not. This provides&n; * sufficient protection against races when reading/using the page.&n; *&n; * However there is the write mapping to think about. Doing the above described&n; * checking here will be fine, because when initiating the write we will set&n; * PG_locked and clear PG_uptodate making sure nobody is touching the page&n; * contents. Doing the locking this way means that the commit to disk code in&n; * the page cache code paths is automatically sufficiently locked with us as&n; * we will not touch a page that has been locked or is not uptodate. The only&n; * locking problem then is them locking the page while we are accessing it.&n; *&n; * So that code will end up having to own the mrec_lock of all mft&n; * records/inodes present in the page before I/O can proceed. Grr. In that&n; * case we wouldn&squot;t need need to bother with PG_locked and PG_uptodate as&n; * nobody will be accessing anything without owning the mrec_lock semaphore.&n; * But we do need to use them because of the read_cache_page() invokation and&n; * the code becomes so much simpler this way that it is well worth it.&n; *&n; * The mft record is now ours and we return a pointer to it. You need to check&n; * the returned pointer with IS_ERR() and if that is true, PTR_ERR() will return&n; * the error code.&n; *&n; * NOTE: Caller is responsible for setting the mft record dirty before calling&n; * unmap_mft_record(). This is obviously only necessary if the caller really&n; * modified the mft record...&n; * Q: Do we want to recycle one of the VFS inode state bits instead?&n; * A: No, the inode ones mean we want to change the mft record, not we want to&n; * write it out.&n; */
...
c_func
(paren
l_string|&quot;Entering for mft_no 0x%lx, mapping for %s.&quot;
comma
id|ni-&gt;mft_no
comma
id|rw
