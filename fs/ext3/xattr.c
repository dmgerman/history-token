...
multiline_comment|/*&n; * ext3_xattr_block_get()&n; *&n; * routine looks for attribute in EA block and returns it&squot;s value and size&n; */
r_int
...
multiline_comment|/*&n; * ext3_xattr_ibody_get()&n; *&n; * routine looks for attribute in inode body and returns it&squot;s value and size&n; */
r_int
...
multiline_comment|/* can&squot;t find EA */
id|brelse
c_func
(paren
id|iloc.bh
...
suffix:semicolon
multiline_comment|/* try to find attribute in inode body */
id|err
op_assign
id|ext3_xattr_ibody_get
c_func
...
multiline_comment|/* search was unsuccessful, try to find EA in dedicated block */
id|err
op_assign
id|ext3_xattr_block_get
c_func
...
multiline_comment|/* ext3_xattr_block_list()&n; *&n; * generate list of attributes stored in EA block&n; */
r_int
...
multiline_comment|/* ext3_xattr_ibody_list()&n; *&n; * generate list of attributes stored in inode body&n; */
r_int
...
id|inode-&gt;i_sb
comma
l_string|&quot;ext3_xattr_ibody_list&quot;
comma
l_string|&quot;inode %ld&quot;
comma
id|inode-&gt;i_ino
...
suffix:semicolon
multiline_comment|/* get list of attributes stored in inode body */
id|error
op_assign
id|ext3_xattr_ibody_list
c_func
...
(brace
multiline_comment|/* some error occured while collecting&n;&t;&t; * attributes in inode body */
id|size
op_assign
l_int|0
suffix:semicolon
...
id|error
suffix:semicolon
multiline_comment|/* get list of attributes stored in dedicated block */
r_if
c_cond
(paren
id|buffer
...
suffix:semicolon
multiline_comment|/* listing was successful, so we return len */
r_if
c_cond
(paren
id|error
...
multiline_comment|/*&n; * ext3_xattr_ibody_find()&n; *&n; * search attribute and calculate free space in inode body&n; * NOTE: free space includes space our attribute hold&n; */
r_int
...
id|inode-&gt;i_sb
comma
l_string|&quot;ext3_xattr_ibody_find&quot;
comma
l_string|&quot;inode %ld&quot;
comma
id|inode-&gt;i_ino
...
multiline_comment|/*&n; * ext3_xattr_block_find()&n; *&n; * search attribute and calculate free space in EA block (if it allocated)&n; * NOTE: free space includes space our attribute hold&n; */
r_int
...
multiline_comment|/*&n; * ext3_xattr_ibody_set()&n; *&n; * this routine add/remove/replace attribute in inode body&n; */
r_int
...
(brace
multiline_comment|/* inode had no attributes before */
op_star
(paren
(paren
id|__u32
...
r_else
(brace
multiline_comment|/* we calculate all but our attribute&n;&t;&t;&t; * because it will be removed before changing */
id|free
op_sub_assign
id|EXT3_XATTR_LEN
c_func
...
multiline_comment|/* optimization: can we simple replace old value ? */
r_if
c_cond
(paren
id|here
...
(brace
multiline_comment|/* time to remove old value */
r_struct
id|ext3_xattr_entry
op_star
id|e
...
suffix:semicolon
multiline_comment|/* recalculate offsets */
id|e
op_assign
(paren
r_struct
...
c_func
(paren
l_string|&quot;ALERT at %s:%d: 0x%p + %d &gt; 0x%p&bslash;n&quot;
comma
id|__FILE__
comma
id|__LINE__
...
id|value_len
suffix:semicolon
multiline_comment|/* use last to create new entry */
id|last-&gt;e_name_len
op_assign
id|strlen
c_func
...
c_func
(paren
l_string|&quot;ALERT at %s:%d: 0x%p + %d + %zd &gt; 0x%p&bslash;n&quot;
comma
id|__FILE__
comma
id|__LINE__
...
suffix:semicolon
DECL|macro|EX_FOUND_IN_IBODY
mdefine_line|#define EX_FOUND_IN_IBODY&t;1
DECL|macro|EX_FOUND_IN_BLOCK
mdefine_line|#define EX_FOUND_IN_BLOCK&t;2
multiline_comment|/* try to find attribute in inode body */
id|err
op_assign
id|ext3_xattr_ibody_find
...
(brace
multiline_comment|/* found EA in inode */
id|where
op_assign
id|EX_FOUND_IN_IBODY
suffix:semicolon
)brace
r_else
r_if
...
(brace
multiline_comment|/* there is no such attribute in inode body */
multiline_comment|/* try to find attribute in dedicated block */
id|err
op_assign
id|ext3_xattr_block_find
c_func
...
(brace
multiline_comment|/* not found EA in block */
r_goto
id|finish
suffix:semicolon
)brace
...
(brace
multiline_comment|/* found EA in block */
id|where
op_assign
id|EX_FOUND_IN_BLOCK
suffix:semicolon
)brace
)brace
r_else
...
id|finish
suffix:semicolon
multiline_comment|/* check flags: may replace? may create ? */
r_if
c_cond
(paren
id|where
...
multiline_comment|/* check if we have enough space to store attribute */
id|total
op_assign
id|EXT3_XATTR_LEN
c_func
...
(brace
multiline_comment|/* have no enough space */
id|err
op_assign
op_minus
id|ENOSPC
...
multiline_comment|/* there are two cases when we want to remove EA from original storage:&n;&t; * a) EA is stored in the inode, but new value doesn&squot;t fit&n;&t; * b) EA is stored in the block, but new value fit in inode&n;&t; */
r_if
c_cond
(paren
id|where
...
suffix:semicolon
multiline_comment|/* try to store EA in inode body */
id|err
op_assign
id|ext3_xattr_ibody_set
c_func
...
(brace
multiline_comment|/* can&squot;t store EA in inode body: try to store in block */
id|err
op_assign
id|ext3_xattr_block_set
c_func
...
multiline_comment|/*&n; * ext3_xattr_block_set()&n; *&n; * this routine add/remove/replace attribute in EA block&n; */
r_int
