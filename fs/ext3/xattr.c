multiline_comment|/*&n; * linux/fs/ext3/xattr.c&n; *&n; * Copyright (C) 2001 by Andreas Gruenbacher, &lt;a.gruenbacher@computer.org&gt;&n; *&n; * Fix by Harrison Xing &lt;harrison@mountainviewdata.com&gt;.&n; * Ext3 code with a lot of help from Eric Jarman &lt;ejarman@acm.org&gt;.&n; * Extended attributes for symlinks and special files added per&n; *  suggestion of Luka Renko &lt;luka.renko@hermes.si&gt;.&n; */
multiline_comment|/*&n; * Extended attributes are stored on disk blocks allocated outside of&n; * any inode. The i_file_acl field is then made to point to this allocated&n; * block. If all extended attributes of an inode are identical, these&n; * inodes may share the same extended attribute block. Such situations&n; * are automatically detected by keeping a cache of recent attribute block&n; * numbers and hashes over the block&squot;s contents in memory.&n; *&n; *&n; * Extended attribute block layout:&n; *&n; *   +------------------+&n; *   | header           |&n; *   &#xfffd; entry 1          | |&n; *   | entry 2          | | growing downwards&n; *   | entry 3          | v&n; *   | four null bytes  |&n; *   | . . .            |&n; *   | value 1          | ^&n; *   | value 3          | | growing upwards&n; *   | value 2          | |&n; *   +------------------+&n; *&n; * The block header is followed by multiple entry descriptors. These entry&n; * descriptors are variable in size, and alligned to EXT3_XATTR_PAD&n; * byte boundaries. The entry descriptors are sorted by attribute name,&n; * so that two extended attribute blocks can be compared efficiently.&n; *&n; * Attribute values are aligned to the end of the block, stored in&n; * no specific order. They are also padded to EXT3_XATTR_PAD byte&n; * boundaries. No additional gaps are left between them.&n; *&n; * Locking strategy&n; * ----------------&n; * The VFS holdsinode-&gt;i_sem semaphore when any of the xattr inode&n; * operations are called, so we are guaranteed that only one&n; * processes accesses extended attributes of an inode at any time.&n; *&n; * For writing we also grab the ext3_xattr_sem semaphore. This ensures that&n; * only a single process is modifying an extended attribute block, even&n; * if the block is shared among inodes.&n; */
...
id|ext3_xattr_cache
suffix:semicolon
multiline_comment|/*&n; * If a file system does not share extended attributes among inodes,&n; * we should not need the ext3_xattr_sem semaphore. However, the&n; * filesystem may still contain shared blocks, so we always take&n; * the lock.&n; */
r_static
id|DECLARE_MUTEX
c_func
(paren
...
multiline_comment|/*&n; * Second half of ext3_xattr_set(): Update the file system.&n; */
r_static
r_int
DECL|function|ext3_xattr_set2
id|ext3_xattr_set2
c_func
(paren
id|handle_t
