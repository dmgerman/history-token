...
multiline_comment|/*&n; * Work out how many blocks we need to progress with the next chunk of a&n; * truncate transaction.&n; */
DECL|function|blocks_for_truncate
r_static
r_int
r_int
id|blocks_for_truncate
c_func
(paren
r_struct
id|inode
...
suffix:semicolon
multiline_comment|/* Give ourselves just enough room to cope with inodes in which&n;&t; * i_blocks is corrupt: we&squot;ve seen disk corruptions in the past&n;&t; * which resulted in random data in an inode which looked enough&n;&t; * like a regular file for ext3 to try to delete it.  Things&n;&t; * will go a bit crazy if that happens, but at least we should&n;&t; * try not to panic the whole kernel. */
r_if
c_cond
(paren
id|needed
...
l_int|2
suffix:semicolon
multiline_comment|/* But we need to bound the transaction so we don&squot;t overflow the&n;&t; * journal. */
r_if
c_cond
(paren
id|needed
...
suffix:semicolon
multiline_comment|/* lazy initialize the block allocation info here if necessary */
r_if
c_cond
(paren
id|S_ISREG
...
suffix:semicolon
multiline_comment|/* i_disksize growing is protected by truncate_sem&n;&t; * don&squot;t forget to protect it if you&squot;re about to implement&n;&t; * concurrent ext3_get_block() -bzzz */
r_if
c_cond
(paren
op_logical_neg
...
DECL|macro|DIO_CREDITS
mdefine_line|#define DIO_CREDITS (EXT3_RESERVE_TRANS_BLOCKS + 32)
r_static
r_int
DECL|function|ext3_direct_io_get_blocks
...
(brace
multiline_comment|/*&n;&t;&t; * Huge direct-io writes can hold off commits for long&n;&t;&t; * periods of time.  Let this commit run.&n;&t;&t; */
id|ext3_journal_stop
c_func
(paren
id|handle
...
(brace
multiline_comment|/*&n;&t;&t; * Getting low on buffer credits...&n;&t;&t; */
id|ret
op_assign
id|ext3_journal_extend
c_func
...
(brace
multiline_comment|/*&n;&t;&t;&t; * Couldn&squot;t extend the transaction.  Start a new one.&n;&t;&t;&t; */
id|ret
op_assign
id|ext3_journal_restart
c_func
...
DECL|function|ext3_writepages_get_block
r_static
r_int
id|ext3_writepages_get_block
c_func
(paren
r_struct
id|inode
...
DECL|function|ext3_ordered_commit_write
r_static
r_int
id|ext3_ordered_commit_write
c_func
(paren
r_struct
id|file
...
(brace
multiline_comment|/*&n;&t;&t; * generic_commit_write() will run mark_inode_dirty() if i_size&n;&t;&t; * changes.  So let&squot;s piggyback the i_disksize mark_inode_dirty&n;&t;&t; * into that.&n;&t;&t; */
id|loff_t
id|new_i_size
suffix:semicolon
id|new_i_size
...
DECL|function|ext3_writeback_commit_write
r_static
r_int
id|ext3_writeback_commit_write
c_func
(paren
r_struct
id|file
...
DECL|function|ext3_journalled_commit_write
r_static
r_int
id|ext3_journalled_commit_write
c_func
(paren
r_struct
id|file
...
id|pos
suffix:semicolon
multiline_comment|/*&n;&t; * Here we duplicate the generic_commit_write() functionality&n;&t; */
id|pos
op_assign
(paren
(paren
...
DECL|function|journal_dirty_data_fn
r_static
r_int
id|journal_dirty_data_fn
c_func
(paren
id|handle_t
op_star
...
suffix:semicolon
multiline_comment|/*&n;&t; * We give up here if we&squot;re reentered, because it might be for a&n;&t; * different filesystem.&n;&t; */
r_if
c_cond
(paren
id|ext3_journal_current_handle
...
r_static
r_int
DECL|function|ext3_writeback_writepage_helper
id|ext3_writeback_writepage_helper
c_func
(paren
r_struct
id|page
...
r_static
r_int
DECL|function|ext3_writeback_writepages
id|ext3_writeback_writepages
c_func
(paren
r_struct
id|address_space
...
suffix:semicolon
multiline_comment|/*&n;&t; * Need to reaquire the handle since ext3_writepages_get_block()&n;&t; * can restart the handle&n;&t; */
id|handle
op_assign
id|journal_current_handle
c_func
...
DECL|function|ext3_writeback_writepage
r_static
r_int
id|ext3_writeback_writepage
c_func
(paren
r_struct
id|page
...
DECL|function|ext3_journalled_writepage
r_static
r_int
id|ext3_journalled_writepage
c_func
(paren
r_struct
id|page
...
(brace
multiline_comment|/*&n;&t;&t; * It&squot;s mmapped pagecache.  Add buffers and journal it.  There&n;&t;&t; * doesn&squot;t seem much point in redirtying the page here.&n;&t;&t; */
id|ClearPageChecked
c_func
(paren
id|page
...
r_else
(brace
multiline_comment|/*&n;&t;&t; * It may be a page full of checkpoint-mode buffers.  We don&squot;t&n;&t;&t; * really know unless we go poke around in the buffer_heads.&n;&t;&t; * But block_write_full_page will do the right thing.&n;&t;&t; */
id|ret
op_assign
id|block_write_full_page
c_func
...
suffix:semicolon
multiline_comment|/*&n;&t; * If it&squot;s a full truncate we just forget about the pending dirtying&n;&t; */
r_if
c_cond
(paren
id|offset
...
multiline_comment|/*&n; * Pages can be marked dirty completely asynchronously from ext3&squot;s journalling&n; * activity.  By filemap_sync_pte(), try_to_unmap_one(), etc.  We cannot do&n; * much here because -&gt;set_page_dirty is called under VFS locks.  The page is&n; * not necessarily locked.&n; *&n; * We cannot just dirty the page and leave attached buffers clean, because the&n; * buffers&squot; dirty state is &quot;definitive&quot;.  We cannot just set the buffers dirty&n; * or jbddirty because all the journalling code will explode.&n; *&n; * So what we do is to mark the page &quot;pending dirty&quot; and next time writepage&n; * is called, propagate that into the buffers appropriately.&n; */
DECL|function|ext3_journalled_set_page_dirty
r_static
r_int
id|ext3_journalled_set_page_dirty
c_func
(paren
r_struct
id|page
...
DECL|variable|ext3_ordered_aops
r_static
r_struct
id|address_space_operations
id|ext3_ordered_aops
op_assign
(brace
dot
id|readpage
...
suffix:semicolon
DECL|variable|ext3_journalled_aops
r_static
r_struct
id|address_space_operations
id|ext3_journalled_aops
op_assign
(brace
dot
id|readpage
...
suffix:semicolon
multiline_comment|/*&n;&t; * For &quot;nobh&quot; option,  we can only work if we don&squot;t need to&n;&t; * read-in the page - otherwise we create buffers to do the IO.&n;&t; */
r_if
c_cond
(paren
op_logical_neg
...
DECL|function|ext3_get_inode_block
r_static
r_int
r_int
id|ext3_get_inode_block
c_func
(paren
r_struct
id|super_block
...
(brace
multiline_comment|/* someone brought it uptodate while we waited */
id|unlock_buffer
c_func
(paren
id|bh
...
suffix:semicolon
multiline_comment|/* Is the inode bitmap in cache? */
id|desc
op_assign
...
id|make_io
suffix:semicolon
multiline_comment|/*&n;&t;&t;&t; * If the inode bitmap isn&squot;t in cache then the&n;&t;&t;&t; * optimisation may end up performing two reads instead&n;&t;&t;&t; * of one, so skip it.&n;&t;&t;&t; */
r_if
c_cond
(paren
op_logical_neg
...
(brace
multiline_comment|/* all other inodes are free, so skip I/O */
id|memset
c_func
(paren
id|bh-&gt;b_data
...
(brace
multiline_comment|/*&n;&t;&t; * When mke2fs creates big inodes it does not zero out&n;&t;&t; * the unused bytes above EXT3_GOOD_OLD_INODE_SIZE,&n;&t;&t; * so ignore those first few inodes.&n;&t;&t; */
id|ei-&gt;i_extra_isize
op_assign
id|le16_to_cpu
c_func
...
(brace
multiline_comment|/* The extra space is currently unused. Use it. */
id|ei-&gt;i_extra_isize
op_assign
r_sizeof
(paren
...
id|handle
suffix:semicolon
multiline_comment|/* (user+group)*(old+new) structure, inode write (sb,&n;&t;&t; * inode block, ? - but truncate inode update has it) */
id|handle
op_assign
id|ext3_journal_start
c_func
...
multiline_comment|/* We know that structure was already allocated during DQUOT_INIT so&n;&t; * we will be updating only the data blocks + inodes */
id|ret
op_add_assign
l_int|2
op_star
