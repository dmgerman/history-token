...
DECL|macro|MB_CACHE_WRITER
mdefine_line|#define MB_CACHE_WRITER ((unsigned short)~0U &gt;&gt; 1)
DECL|variable|mb_cache_queue
...
(brace
multiline_comment|/* Wake up all processes queuing for this cache entry. */
r_if
c_cond
(paren
id|ce-&gt;e_queued
...
multiline_comment|/*&n; * mb_cache_entry_get()&n; *&n; * Get a cache entry  by device / block number. (There can only be one entry&n; * in the cache per device and block.) Returns NULL if no such cache entry&n; * exists. The returned cache entry is locked for exclusive access (&quot;single&n; * writer&quot;).&n; */
r_struct
id|mb_cache_entry
op_star
...
suffix:semicolon
multiline_comment|/* Incrementing before holding the lock gives readers&n;&t;&t;&t;   priority over writers. */
id|ce-&gt;e_used
op_increment
suffix:semicolon
r_while
