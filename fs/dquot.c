...
multiline_comment|/* No locks needed here as ANY_DQUOT_DIRTY is used just by sync and so the&n; * worst what can happen is that dquot is not written by concurrent sync... */
...
r_continue
suffix:semicolon
multiline_comment|/* Now we have active dquot from which someone is holding reference so we&n;&t;&t; * can safely just increase use count */
id|atomic_inc
c_func
(paren
op_amp
