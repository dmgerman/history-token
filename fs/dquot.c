...
multiline_comment|/*&n; * Dquot List Management:&n; * The quota code uses three lists for dquot management: the inuse_list,&n; * free_dquots, and dquot_hash[] array. A single dquot structure may be&n; * on all three lists, depending on its current state.&n; *&n; * All dquots are placed to the end of inuse_list when first created, and this&n; * list is used for the sync and invalidate operations, which must look&n; * at every dquot.&n; *&n; * Unused dquots (dq_count == 0) are added to the free_dquots list when&n; * freed, and this list is searched whenever we need an available dquot.&n; * Dquots are removed from the list as soon as they are used again, and&n; * nr_free_dquots gives the number of dquots on the list. When dquot is&n; * invalidated it&squot;s completely released from memory.&n; *&n; * Dquots with a specific identity (device, type and id) are placed on&n; * one of the dquot_hash[] hash chains. The provides an efficient search&n; * mechanism to locate a specific dquot.&n; */
...
r_continue
suffix:semicolon
multiline_comment|/* Raise use count so quota won&squot;t be invalidated. We can&squot;t use dqduplicate() as it does too many tests */
id|dquot-&gt;dq_count
op_increment
suffix:semicolon
r_if
...
(brace
multiline_comment|/* We can&squot;t block while changing quota structure... */
r_if
c_cond
(paren
id|id
...
suffix:semicolon
multiline_comment|/* We copy data to preserve them from changing */
id|dqput
c_func
(paren
id|dquot
...
macro_line|#if 0&t;/* We are not going to support filesystems without i_blocks... */
