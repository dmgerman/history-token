...
multiline_comment|/*&n; * Speculatively reserve an inode in a blockgroup which used to have some&n; * spare ones.  Later, when we come to actually claim the inode in the bitmap&n; * it may be that it was taken.  In that case the allocator will undo this&n; * reservation and try again.&n; *&n; * The inode allocator does not physically alter the superblock.  But we still&n; * set sb-&gt;s_dirt, because the superblock was &quot;logically&quot; altered - we need to&n; * go and add up the free inodes counts again and flush out the superblock.&n; */
DECL|function|ext2_reserve_inode
r_static
r_void
id|ext2_reserve_inode
c_func
(paren
r_struct
id|super_block
