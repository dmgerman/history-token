...
multiline_comment|/*&n; * [Sep 2001 AV] Single-semaphore locking scheme (kudos to David Holland)&n; * implemented.  Let&squot;s see if raised priority of -&gt;s_vfs_rename_sem gives&n; * any extra contention...&n; */
...
suffix:semicolon
macro_line|#endif
multiline_comment|/**&n; * generic_permission  -  check for access rights on a Posix-like filesystem&n; * @inode:&t;inode to check access rights for&n; * @mask:&t;right to check for (%MAY_READ, %MAY_WRITE, %MAY_EXEC)&n; * @check_acl:&t;optional callback to check for Posix ACLs&n; *&n; * Used to check for read/write/execute permissions on a file.&n; * We use &quot;fsuid&quot; for this, letting us set arbitrary permissions&n; * for filesystem access without changing the &quot;normal&quot; uids which&n; * are used for other things..&n; */
DECL|function|generic_permission
r_int
...
multiline_comment|/* Ordinary permission routines do not understand MAY_APPEND. */
id|submask
op_assign
id|mask
op_amp
...
multiline_comment|/*&n; * Short-cut version of permission(), for calling by&n; * path_walk(), when dcache lock is held.  Combines parts&n; * of permission() and generic_permission(), and tests ONLY for&n; * MAY_EXEC permission.&n; *&n; * If appropriate, check DAC only.  If not appropriate, or&n; * short-cut DAC fails, then call permission() to do more&n; * complete permission check.&n; */
DECL|function|exec_permission_lite
r_static
r_inline
r_int
id|exec_permission_lite
c_func
(paren
r_struct
id|inode
...
DECL|function|__do_follow_link
r_static
r_inline
r_int
id|__do_follow_link
...
c_cond
(paren
id|dentry-&gt;d_inode-&gt;i_op-&gt;put_link
)paren
id|dentry-&gt;d_inode-&gt;i_op
op_member_access_from_pointer
id|put_link
...
multiline_comment|/* no need for dcache_lock, as serialization is taken care in&n; * namespace.c&n; */
...
suffix:semicolon
multiline_comment|/*&n; *  It&squot;s more convoluted than I&squot;d like it to be, but... it&squot;s still fairly&n; *  small and for now I&squot;d prefer to have fast path as straight as possible.&n; *  It _is_ time-critical.&n; */
DECL|function|do_lookup
r_static
r_int
id|do_lookup
c_func
...
DECL|function|__link_path_walk
r_static
id|fastcall
r_int
id|__link_path_walk
...
id|return_reval
suffix:colon
multiline_comment|/*&n;&t;&t; * We bypassed the ordinary revalidation routines.&n;&t;&t; * We may need to check the cached dentry for staleness.&n;&t;&t; */
r_if
c_cond
(paren
id|nd-&gt;dentry
...
op_logical_and
(paren
id|nd-&gt;dentry-&gt;d_sb-&gt;s_type-&gt;fs_flags
op_amp
id|FS_REVAL_DOT
)paren
)paren
...
id|ESTALE
suffix:semicolon
multiline_comment|/* Note: we do not d_invalidate() */
r_if
c_cond
(paren
op_logical_neg
...
multiline_comment|/*&n; * Wrapper to retry pathname resolution whenever the underlying&n; * file system returns an ESTALE.&n; *&n; * Retry the whole path once, forcing real lookup requests&n; * instead of relying on the dcache.&n; */
...
id|result
suffix:semicolon
multiline_comment|/* make sure the stuff we saved doesn&squot;t go away */
id|dget
c_func
(paren
id|save.dentry
...
multiline_comment|/*&n; * p1 and p2 should be directories on the same fs.&n; */
DECL|function|lock_rename
r_struct
id|dentry
op_star
id|lock_rename
...
(paren
op_amp
id|p1-&gt;d_inode-&gt;i_sb-&gt;s_vfs_rename_sem
)paren
suffix:semicolon
r_for
c_loop
...
suffix:semicolon
multiline_comment|/* Allow the LSM permission hook to distinguish append &n;&t;   access from general write access. */
r_if
c_cond
(paren
id|flag
...
multiline_comment|/* source should not be ancestor of target */
id|error
op_assign
op_minus
id|EINVAL
...
id|exit4
suffix:semicolon
multiline_comment|/* target should not be an ancestor of source */
id|error
op_assign
op_minus
id|ENOTEMPTY
...
multiline_comment|/*&n; * A helper for -&gt;readlink().  This should be used *ONLY* for symlinks that&n; * have -&gt;follow_link() touching nd only in nd_set_link().  Using (or not&n; * using) it for any given inode is up to filesystem.&n; */
DECL|function|generic_readlink
r_int
id|generic_readlink
c_func
(paren
...
DECL|function|page_follow_link_light
r_int
...
suffix:semicolon
DECL|variable|page_follow_link_light
id|EXPORT_SYMBOL
c_func
(paren
id|page_follow_link_light
...
suffix:semicolon
DECL|variable|generic_permission
id|EXPORT_SYMBOL
c_func
(paren
id|generic_permission
