multiline_comment|/*&n; *  linux/fs/ufs/super.c&n; *&n; * Copyright (C) 1998&n; * Daniel Pirkl &lt;daniel.pirkl@email.cz&gt;&n; * Charles University, Faculty of Mathematics and Physics&n; */
multiline_comment|/* Derived from&n; *&n; *  linux/fs/ext2/super.c&n; *&n; * Copyright (C) 1992, 1993, 1994, 1995&n; * Remy Card (card@masi.ibp.fr)&n; * Laboratoire MASI - Institut Blaise Pascal&n; * Universite Pierre et Marie Curie (Paris VI)&n; *&n; *  from&n; *&n; *  linux/fs/minix/inode.c&n; *&n; *  Copyright (C) 1991, 1992  Linus Torvalds&n; *&n; *  Big-endian to little-endian byte-swapping/bitmaps by&n; *        David S. Miller (davem@caip.rutgers.edu), 1995&n; */
multiline_comment|/*&n; * Inspired by&n; *&n; *  linux/fs/ufs/super.c&n; *&n; * Copyright (C) 1996&n; * Adrian Rodriguez (adrian@franklins-tower.rutgers.edu)&n; * Laboratory for Computer Science Research Computing Facility&n; * Rutgers, The State University of New Jersey&n; *&n; * Copyright (C) 1996  Eddie C. Dost  (ecd@skynet.be)&n; *&n; * Kernel module support added on 96/04/26 by&n; * Stefan Reinauer &lt;stepan@home.culture.mipt.ru&gt;&n; *&n; * Module usage counts added on 96/04/29 by&n; * Gertjan van Wingerde &lt;gertjan@cs.vu.nl&gt;&n; *&n; * Clean swab support on 19970406 by&n; * Francois-Rene Rideau &lt;fare@tunes.org&gt;&n; *&n; * 4.4BSD (FreeBSD) support added on February 1st 1998 by&n; * Niels Kristian Bech Jensen &lt;nkbj@image.dk&gt; partially based&n; * on code by Martin von Loewis &lt;martin@mira.isdn.cs.tu-berlin.de&gt;.&n; *&n; * NeXTstep support added on February 5th 1998 by&n; * Niels Kristian Bech Jensen &lt;nkbj@image.dk&gt;.&n; *&n; * write support Daniel Pirkl &lt;daniel.pirkl@email.cz&gt; 1998&n; * &n; * HP/UX hfs filesystem support added by&n; * Martin K. Petersen &lt;mkp@mkp.net&gt;, August 1999&n; *&n; * UFS2 (of FreeBSD 5.x) support added by&n; * Niraj Kumar &lt;niraj17@iitbombay.org&gt;, Jan 2004&n; *&n; */
macro_line|#include &lt;linux/config.h&gt;
macro_line|#include &lt;linux/module.h&gt;
macro_line|#include &lt;linux/bitops.h&gt;
macro_line|#include &lt;stdarg.h&gt;
macro_line|#include &lt;asm/uaccess.h&gt;
macro_line|#include &lt;asm/system.h&gt;
macro_line|#include &lt;linux/errno.h&gt;
macro_line|#include &lt;linux/fs.h&gt;
macro_line|#include &lt;linux/ufs_fs.h&gt;
macro_line|#include &lt;linux/slab.h&gt;
macro_line|#include &lt;linux/time.h&gt;
macro_line|#include &lt;linux/stat.h&gt;
macro_line|#include &lt;linux/string.h&gt;
macro_line|#include &lt;linux/blkdev.h&gt;
macro_line|#include &lt;linux/init.h&gt;
macro_line|#include &lt;linux/parser.h&gt;
macro_line|#include &lt;linux/smp_lock.h&gt;
macro_line|#include &lt;linux/buffer_head.h&gt;
macro_line|#include &lt;linux/vfs.h&gt;
macro_line|#include &quot;swab.h&quot;
macro_line|#include &quot;util.h&quot;
DECL|macro|UFS_SUPER_DEBUG
macro_line|#undef UFS_SUPER_DEBUG
DECL|macro|UFS_SUPER_DEBUG_MORE
macro_line|#undef UFS_SUPER_DEBUG_MORE
DECL|macro|UFS_SUPER_DEBUG_MORE
macro_line|#undef UFS_SUPER_DEBUG_MORE
macro_line|#ifdef UFS_SUPER_DEBUG
DECL|macro|UFSD
mdefine_line|#define UFSD(x) printk(&quot;(%s, %d), %s: &quot;, __FILE__, __LINE__, __FUNCTION__); printk x;
macro_line|#else
DECL|macro|UFSD
mdefine_line|#define UFSD(x)
macro_line|#endif
macro_line|#ifdef UFS_SUPER_DEBUG_MORE
multiline_comment|/*&n; * Print contents of ufs_super_block, useful for debugging&n; */
DECL|function|ufs_print_super_stuff
r_void
id|ufs_print_super_stuff
c_func
(paren
r_struct
id|super_block
op_star
id|sb
comma
r_struct
id|ufs_super_block_first
op_star
id|usb1
comma
r_struct
id|ufs_super_block_second
op_star
id|usb2
comma
r_struct
id|ufs_super_block_third
op_star
id|usb3
)paren
(brace
id|printk
c_func
(paren
l_string|&quot;ufs_print_super_stuff&bslash;n&quot;
)paren
suffix:semicolon
id|printk
c_func
(paren
l_string|&quot;size of usb:     %u&bslash;n&quot;
comma
r_sizeof
(paren
r_struct
id|ufs_super_block
)paren
)paren
suffix:semicolon
id|printk
c_func
(paren
l_string|&quot;  magic:         0x%x&bslash;n&quot;
comma
id|fs32_to_cpu
c_func
(paren
id|sb
comma
id|usb3-&gt;fs_magic
)paren
)paren
suffix:semicolon
id|printk
c_func
(paren
l_string|&quot;  sblkno:        %u&bslash;n&quot;
comma
id|fs32_to_cpu
c_func
(paren
id|sb
comma
id|usb1-&gt;fs_sblkno
)paren
)paren
suffix:semicolon
id|printk
c_func
(paren
l_string|&quot;  cblkno:        %u&bslash;n&quot;
comma
id|fs32_to_cpu
c_func
(paren
id|sb
comma
id|usb1-&gt;fs_cblkno
)paren
)paren
suffix:semicolon
id|printk
c_func
(paren
l_string|&quot;  iblkno:        %u&bslash;n&quot;
comma
id|fs32_to_cpu
c_func
(paren
id|sb
comma
id|usb1-&gt;fs_iblkno
)paren
)paren
suffix:semicolon
id|printk
c_func
(paren
l_string|&quot;  dblkno:        %u&bslash;n&quot;
comma
id|fs32_to_cpu
c_func
(paren
id|sb
comma
id|usb1-&gt;fs_dblkno
)paren
)paren
suffix:semicolon
id|printk
c_func
(paren
l_string|&quot;  cgoffset:      %u&bslash;n&quot;
comma
id|fs32_to_cpu
c_func
(paren
id|sb
comma
id|usb1-&gt;fs_cgoffset
)paren
)paren
suffix:semicolon
id|printk
c_func
(paren
l_string|&quot;  ~cgmask:       0x%x&bslash;n&quot;
comma
op_complement
id|fs32_to_cpu
c_func
(paren
id|sb
comma
id|usb1-&gt;fs_cgmask
)paren
)paren
suffix:semicolon
id|printk
c_func
(paren
l_string|&quot;  size:          %u&bslash;n&quot;
comma
id|fs32_to_cpu
c_func
(paren
id|sb
comma
id|usb1-&gt;fs_size
)paren
)paren
suffix:semicolon
id|printk
c_func
(paren
l_string|&quot;  dsize:         %u&bslash;n&quot;
comma
id|fs32_to_cpu
c_func
(paren
id|sb
comma
id|usb1-&gt;fs_dsize
)paren
)paren
suffix:semicolon
id|printk
c_func
(paren
l_string|&quot;  ncg:           %u&bslash;n&quot;
comma
id|fs32_to_cpu
c_func
(paren
id|sb
comma
id|usb1-&gt;fs_ncg
)paren
)paren
suffix:semicolon
id|printk
c_func
(paren
l_string|&quot;  bsize:         %u&bslash;n&quot;
comma
id|fs32_to_cpu
c_func
(paren
id|sb
comma
id|usb1-&gt;fs_bsize
)paren
)paren
suffix:semicolon
id|printk
c_func
(paren
l_string|&quot;  fsize:         %u&bslash;n&quot;
comma
id|fs32_to_cpu
c_func
(paren
id|sb
comma
id|usb1-&gt;fs_fsize
)paren
)paren
suffix:semicolon
id|printk
c_func
(paren
l_string|&quot;  frag:          %u&bslash;n&quot;
comma
id|fs32_to_cpu
c_func
(paren
id|sb
comma
id|usb1-&gt;fs_frag
)paren
)paren
suffix:semicolon
id|printk
c_func
(paren
l_string|&quot;  fragshift:     %u&bslash;n&quot;
comma
id|fs32_to_cpu
c_func
(paren
id|sb
comma
id|usb1-&gt;fs_fragshift
)paren
)paren
suffix:semicolon
id|printk
c_func
(paren
l_string|&quot;  ~fmask:        %u&bslash;n&quot;
comma
op_complement
id|fs32_to_cpu
c_func
(paren
id|sb
comma
id|usb1-&gt;fs_fmask
)paren
)paren
suffix:semicolon
id|printk
c_func
(paren
l_string|&quot;  fshift:        %u&bslash;n&quot;
comma
id|fs32_to_cpu
c_func
(paren
id|sb
comma
id|usb1-&gt;fs_fshift
)paren
)paren
suffix:semicolon
id|printk
c_func
(paren
l_string|&quot;  sbsize:        %u&bslash;n&quot;
comma
id|fs32_to_cpu
c_func
(paren
id|sb
comma
id|usb1-&gt;fs_sbsize
)paren
)paren
suffix:semicolon
id|printk
c_func
(paren
l_string|&quot;  spc:           %u&bslash;n&quot;
comma
id|fs32_to_cpu
c_func
(paren
id|sb
comma
id|usb1-&gt;fs_spc
)paren
)paren
suffix:semicolon
id|printk
c_func
(paren
l_string|&quot;  cpg:           %u&bslash;n&quot;
comma
id|fs32_to_cpu
c_func
(paren
id|sb
comma
id|usb1-&gt;fs_cpg
)paren
)paren
suffix:semicolon
id|printk
c_func
(paren
l_string|&quot;  ipg:           %u&bslash;n&quot;
comma
id|fs32_to_cpu
c_func
(paren
id|sb
comma
id|usb1-&gt;fs_ipg
)paren
)paren
suffix:semicolon
id|printk
c_func
(paren
l_string|&quot;  fpg:           %u&bslash;n&quot;
comma
id|fs32_to_cpu
c_func
(paren
id|sb
comma
id|usb1-&gt;fs_fpg
)paren
)paren
suffix:semicolon
id|printk
c_func
(paren
l_string|&quot;  csaddr:        %u&bslash;n&quot;
comma
id|fs32_to_cpu
c_func
(paren
id|sb
comma
id|usb1-&gt;fs_csaddr
)paren
)paren
suffix:semicolon
id|printk
c_func
(paren
l_string|&quot;  cssize:        %u&bslash;n&quot;
comma
id|fs32_to_cpu
c_func
(paren
id|sb
comma
id|usb1-&gt;fs_cssize
)paren
)paren
suffix:semicolon
id|printk
c_func
(paren
l_string|&quot;  cgsize:        %u&bslash;n&quot;
comma
id|fs32_to_cpu
c_func
(paren
id|sb
comma
id|usb1-&gt;fs_cgsize
)paren
)paren
suffix:semicolon
id|printk
c_func
(paren
l_string|&quot;  fstodb:        %u&bslash;n&quot;
comma
id|fs32_to_cpu
c_func
(paren
id|sb
comma
id|usb1-&gt;fs_fsbtodb
)paren
)paren
suffix:semicolon
id|printk
c_func
(paren
l_string|&quot;  contigsumsize: %d&bslash;n&quot;
comma
id|fs32_to_cpu
c_func
(paren
id|sb
comma
id|usb3-&gt;fs_u2.fs_44.fs_contigsumsize
)paren
)paren
suffix:semicolon
id|printk
c_func
(paren
l_string|&quot;  postblformat:  %u&bslash;n&quot;
comma
id|fs32_to_cpu
c_func
(paren
id|sb
comma
id|usb3-&gt;fs_postblformat
)paren
)paren
suffix:semicolon
id|printk
c_func
(paren
l_string|&quot;  nrpos:         %u&bslash;n&quot;
comma
id|fs32_to_cpu
c_func
(paren
id|sb
comma
id|usb3-&gt;fs_nrpos
)paren
)paren
suffix:semicolon
id|printk
c_func
(paren
l_string|&quot;  ndir           %u&bslash;n&quot;
comma
id|fs32_to_cpu
c_func
(paren
id|sb
comma
id|usb1-&gt;fs_cstotal.cs_ndir
)paren
)paren
suffix:semicolon
id|printk
c_func
(paren
l_string|&quot;  nifree         %u&bslash;n&quot;
comma
id|fs32_to_cpu
c_func
(paren
id|sb
comma
id|usb1-&gt;fs_cstotal.cs_nifree
)paren
)paren
suffix:semicolon
id|printk
c_func
(paren
l_string|&quot;  nbfree         %u&bslash;n&quot;
comma
id|fs32_to_cpu
c_func
(paren
id|sb
comma
id|usb1-&gt;fs_cstotal.cs_nbfree
)paren
)paren
suffix:semicolon
id|printk
c_func
(paren
l_string|&quot;  nffree         %u&bslash;n&quot;
comma
id|fs32_to_cpu
c_func
(paren
id|sb
comma
id|usb1-&gt;fs_cstotal.cs_nffree
)paren
)paren
suffix:semicolon
id|printk
c_func
(paren
l_string|&quot;&bslash;n&quot;
)paren
suffix:semicolon
)brace
multiline_comment|/*&n; * Print contents of ufs2 ufs_super_block, useful for debugging&n; */
DECL|function|ufs2_print_super_stuff
r_void
id|ufs2_print_super_stuff
c_func
(paren
r_struct
id|super_block
op_star
id|sb
comma
r_struct
id|ufs_super_block
op_star
id|usb
)paren
(brace
id|printk
c_func
(paren
l_string|&quot;ufs_print_super_stuff&bslash;n&quot;
)paren
suffix:semicolon
id|printk
c_func
(paren
l_string|&quot;size of usb:     %u&bslash;n&quot;
comma
r_sizeof
(paren
r_struct
id|ufs_super_block
)paren
)paren
suffix:semicolon
id|printk
c_func
(paren
l_string|&quot;  magic:         0x%x&bslash;n&quot;
comma
id|fs32_to_cpu
c_func
(paren
id|sb
comma
id|usb-&gt;fs_magic
)paren
)paren
suffix:semicolon
id|printk
c_func
(paren
l_string|&quot;  fs_size:   %u&bslash;n&quot;
comma
id|fs64_to_cpu
c_func
(paren
id|sb
comma
id|usb-&gt;fs_u11.fs_u2.fs_size
)paren
)paren
suffix:semicolon
id|printk
c_func
(paren
l_string|&quot;  fs_dsize:  %u&bslash;n&quot;
comma
id|fs64_to_cpu
c_func
(paren
id|sb
comma
id|usb-&gt;fs_u11.fs_u2.fs_dsize
)paren
)paren
suffix:semicolon
id|printk
c_func
(paren
l_string|&quot;  bsize:         %u&bslash;n&quot;
comma
id|fs32_to_cpu
c_func
(paren
id|usb
comma
id|usb-&gt;fs_bsize
)paren
)paren
suffix:semicolon
id|printk
c_func
(paren
l_string|&quot;  fsize:         %u&bslash;n&quot;
comma
id|fs32_to_cpu
c_func
(paren
id|usb
comma
id|usb-&gt;fs_fsize
)paren
)paren
suffix:semicolon
id|printk
c_func
(paren
l_string|&quot;  fs_volname:  %s&bslash;n&quot;
comma
id|usb-&gt;fs_u11.fs_u2.fs_volname
)paren
suffix:semicolon
id|printk
c_func
(paren
l_string|&quot;  fs_fsmnt:  %s&bslash;n&quot;
comma
id|usb-&gt;fs_u11.fs_u2.fs_fsmnt
)paren
suffix:semicolon
id|printk
c_func
(paren
l_string|&quot;  fs_sblockloc: %u&bslash;n&quot;
comma
id|fs64_to_cpu
c_func
(paren
id|sb
comma
id|usb-&gt;fs_u11.fs_u2.fs_sblockloc
)paren
)paren
suffix:semicolon
id|printk
c_func
(paren
l_string|&quot;  cs_ndir(No of dirs):  %u&bslash;n&quot;
comma
id|fs64_to_cpu
c_func
(paren
id|sb
comma
id|usb-&gt;fs_u11.fs_u2.fs_cstotal.cs_ndir
)paren
)paren
suffix:semicolon
id|printk
c_func
(paren
l_string|&quot;  cs_nbfree(No of free blocks):  %u&bslash;n&quot;
comma
id|fs64_to_cpu
c_func
(paren
id|sb
comma
id|usb-&gt;fs_u11.fs_u2.fs_cstotal.cs_nbfree
)paren
)paren
suffix:semicolon
id|printk
c_func
(paren
l_string|&quot;&bslash;n&quot;
)paren
suffix:semicolon
)brace
multiline_comment|/*&n; * Print contents of ufs_cylinder_group, useful for debugging&n; */
DECL|function|ufs_print_cylinder_stuff
r_void
id|ufs_print_cylinder_stuff
c_func
(paren
r_struct
id|super_block
op_star
id|sb
comma
r_struct
id|ufs_cylinder_group
op_star
id|cg
)paren
(brace
id|printk
c_func
(paren
l_string|&quot;&bslash;nufs_print_cylinder_stuff&bslash;n&quot;
)paren
suffix:semicolon
id|printk
c_func
(paren
l_string|&quot;size of ucg: %u&bslash;n&quot;
comma
r_sizeof
(paren
r_struct
id|ufs_cylinder_group
)paren
)paren
suffix:semicolon
id|printk
c_func
(paren
l_string|&quot;  magic:        %x&bslash;n&quot;
comma
id|fs32_to_cpu
c_func
(paren
id|sb
comma
id|cg-&gt;cg_magic
)paren
)paren
suffix:semicolon
id|printk
c_func
(paren
l_string|&quot;  time:         %u&bslash;n&quot;
comma
id|fs32_to_cpu
c_func
(paren
id|sb
comma
id|cg-&gt;cg_time
)paren
)paren
suffix:semicolon
id|printk
c_func
(paren
l_string|&quot;  cgx:          %u&bslash;n&quot;
comma
id|fs32_to_cpu
c_func
(paren
id|sb
comma
id|cg-&gt;cg_cgx
)paren
)paren
suffix:semicolon
id|printk
c_func
(paren
l_string|&quot;  ncyl:         %u&bslash;n&quot;
comma
id|fs16_to_cpu
c_func
(paren
id|sb
comma
id|cg-&gt;cg_ncyl
)paren
)paren
suffix:semicolon
id|printk
c_func
(paren
l_string|&quot;  niblk:        %u&bslash;n&quot;
comma
id|fs16_to_cpu
c_func
(paren
id|sb
comma
id|cg-&gt;cg_niblk
)paren
)paren
suffix:semicolon
id|printk
c_func
(paren
l_string|&quot;  ndblk:        %u&bslash;n&quot;
comma
id|fs32_to_cpu
c_func
(paren
id|sb
comma
id|cg-&gt;cg_ndblk
)paren
)paren
suffix:semicolon
id|printk
c_func
(paren
l_string|&quot;  cs_ndir:      %u&bslash;n&quot;
comma
id|fs32_to_cpu
c_func
(paren
id|sb
comma
id|cg-&gt;cg_cs.cs_ndir
)paren
)paren
suffix:semicolon
id|printk
c_func
(paren
l_string|&quot;  cs_nbfree:    %u&bslash;n&quot;
comma
id|fs32_to_cpu
c_func
(paren
id|sb
comma
id|cg-&gt;cg_cs.cs_nbfree
)paren
)paren
suffix:semicolon
id|printk
c_func
(paren
l_string|&quot;  cs_nifree:    %u&bslash;n&quot;
comma
id|fs32_to_cpu
c_func
(paren
id|sb
comma
id|cg-&gt;cg_cs.cs_nifree
)paren
)paren
suffix:semicolon
id|printk
c_func
(paren
l_string|&quot;  cs_nffree:    %u&bslash;n&quot;
comma
id|fs32_to_cpu
c_func
(paren
id|sb
comma
id|cg-&gt;cg_cs.cs_nffree
)paren
)paren
suffix:semicolon
id|printk
c_func
(paren
l_string|&quot;  rotor:        %u&bslash;n&quot;
comma
id|fs32_to_cpu
c_func
(paren
id|sb
comma
id|cg-&gt;cg_rotor
)paren
)paren
suffix:semicolon
id|printk
c_func
(paren
l_string|&quot;  frotor:       %u&bslash;n&quot;
comma
id|fs32_to_cpu
c_func
(paren
id|sb
comma
id|cg-&gt;cg_frotor
)paren
)paren
suffix:semicolon
id|printk
c_func
(paren
l_string|&quot;  irotor:       %u&bslash;n&quot;
comma
id|fs32_to_cpu
c_func
(paren
id|sb
comma
id|cg-&gt;cg_irotor
)paren
)paren
suffix:semicolon
id|printk
c_func
(paren
l_string|&quot;  frsum:        %u, %u, %u, %u, %u, %u, %u, %u&bslash;n&quot;
comma
id|fs32_to_cpu
c_func
(paren
id|sb
comma
id|cg-&gt;cg_frsum
(braket
l_int|0
)braket
)paren
comma
id|fs32_to_cpu
c_func
(paren
id|sb
comma
id|cg-&gt;cg_frsum
(braket
l_int|1
)braket
)paren
comma
id|fs32_to_cpu
c_func
(paren
id|sb
comma
id|cg-&gt;cg_frsum
(braket
l_int|2
)braket
)paren
comma
id|fs32_to_cpu
c_func
(paren
id|sb
comma
id|cg-&gt;cg_frsum
(braket
l_int|3
)braket
)paren
comma
id|fs32_to_cpu
c_func
(paren
id|sb
comma
id|cg-&gt;cg_frsum
(braket
l_int|4
)braket
)paren
comma
id|fs32_to_cpu
c_func
(paren
id|sb
comma
id|cg-&gt;cg_frsum
(braket
l_int|5
)braket
)paren
comma
id|fs32_to_cpu
c_func
(paren
id|sb
comma
id|cg-&gt;cg_frsum
(braket
l_int|6
)braket
)paren
comma
id|fs32_to_cpu
c_func
(paren
id|sb
comma
id|cg-&gt;cg_frsum
(braket
l_int|7
)braket
)paren
)paren
suffix:semicolon
id|printk
c_func
(paren
l_string|&quot;  btotoff:      %u&bslash;n&quot;
comma
id|fs32_to_cpu
c_func
(paren
id|sb
comma
id|cg-&gt;cg_btotoff
)paren
)paren
suffix:semicolon
id|printk
c_func
(paren
l_string|&quot;  boff:         %u&bslash;n&quot;
comma
id|fs32_to_cpu
c_func
(paren
id|sb
comma
id|cg-&gt;cg_boff
)paren
)paren
suffix:semicolon
id|printk
c_func
(paren
l_string|&quot;  iuseoff:      %u&bslash;n&quot;
comma
id|fs32_to_cpu
c_func
(paren
id|sb
comma
id|cg-&gt;cg_iusedoff
)paren
)paren
suffix:semicolon
id|printk
c_func
(paren
l_string|&quot;  freeoff:      %u&bslash;n&quot;
comma
id|fs32_to_cpu
c_func
(paren
id|sb
comma
id|cg-&gt;cg_freeoff
)paren
)paren
suffix:semicolon
id|printk
c_func
(paren
l_string|&quot;  nextfreeoff:  %u&bslash;n&quot;
comma
id|fs32_to_cpu
c_func
(paren
id|sb
comma
id|cg-&gt;cg_nextfreeoff
)paren
)paren
suffix:semicolon
id|printk
c_func
(paren
l_string|&quot;  clustersumoff %u&bslash;n&quot;
comma
id|fs32_to_cpu
c_func
(paren
id|sb
comma
id|cg-&gt;cg_u.cg_44.cg_clustersumoff
)paren
)paren
suffix:semicolon
id|printk
c_func
(paren
l_string|&quot;  clusteroff    %u&bslash;n&quot;
comma
id|fs32_to_cpu
c_func
(paren
id|sb
comma
id|cg-&gt;cg_u.cg_44.cg_clusteroff
)paren
)paren
suffix:semicolon
id|printk
c_func
(paren
l_string|&quot;  nclusterblks  %u&bslash;n&quot;
comma
id|fs32_to_cpu
c_func
(paren
id|sb
comma
id|cg-&gt;cg_u.cg_44.cg_nclusterblks
)paren
)paren
suffix:semicolon
id|printk
c_func
(paren
l_string|&quot;&bslash;n&quot;
)paren
suffix:semicolon
)brace
macro_line|#endif /* UFS_SUPER_DEBUG_MORE */
DECL|variable|ufs_super_ops
r_static
r_struct
id|super_operations
id|ufs_super_ops
suffix:semicolon
DECL|variable|error_buf
r_static
r_char
id|error_buf
(braket
l_int|1024
)braket
suffix:semicolon
DECL|function|ufs_error
r_void
id|ufs_error
(paren
r_struct
id|super_block
op_star
id|sb
comma
r_const
r_char
op_star
id|function
comma
r_const
r_char
op_star
id|fmt
comma
dot
dot
dot
)paren
(brace
r_struct
id|ufs_sb_private_info
op_star
id|uspi
suffix:semicolon
r_struct
id|ufs_super_block_first
op_star
id|usb1
suffix:semicolon
id|va_list
id|args
suffix:semicolon
id|uspi
op_assign
id|UFS_SB
c_func
(paren
id|sb
)paren
op_member_access_from_pointer
id|s_uspi
suffix:semicolon
id|usb1
op_assign
id|ubh_get_usb_first
c_func
(paren
id|USPI_UBH
)paren
suffix:semicolon
r_if
c_cond
(paren
op_logical_neg
(paren
id|sb-&gt;s_flags
op_amp
id|MS_RDONLY
)paren
)paren
(brace
id|usb1-&gt;fs_clean
op_assign
id|UFS_FSBAD
suffix:semicolon
id|ubh_mark_buffer_dirty
c_func
(paren
id|USPI_UBH
)paren
suffix:semicolon
id|sb-&gt;s_dirt
op_assign
l_int|1
suffix:semicolon
id|sb-&gt;s_flags
op_or_assign
id|MS_RDONLY
suffix:semicolon
)brace
id|va_start
(paren
id|args
comma
id|fmt
)paren
suffix:semicolon
id|vsprintf
(paren
id|error_buf
comma
id|fmt
comma
id|args
)paren
suffix:semicolon
id|va_end
(paren
id|args
)paren
suffix:semicolon
r_switch
c_cond
(paren
id|UFS_SB
c_func
(paren
id|sb
)paren
op_member_access_from_pointer
id|s_mount_opt
op_amp
id|UFS_MOUNT_ONERROR
)paren
(brace
r_case
id|UFS_MOUNT_ONERROR_PANIC
suffix:colon
id|panic
(paren
l_string|&quot;UFS-fs panic (device %s): %s: %s&bslash;n&quot;
comma
id|sb-&gt;s_id
comma
id|function
comma
id|error_buf
)paren
suffix:semicolon
r_case
id|UFS_MOUNT_ONERROR_LOCK
suffix:colon
r_case
id|UFS_MOUNT_ONERROR_UMOUNT
suffix:colon
r_case
id|UFS_MOUNT_ONERROR_REPAIR
suffix:colon
id|printk
(paren
id|KERN_CRIT
l_string|&quot;UFS-fs error (device %s): %s: %s&bslash;n&quot;
comma
id|sb-&gt;s_id
comma
id|function
comma
id|error_buf
)paren
suffix:semicolon
)brace
)brace
DECL|function|ufs_panic
r_void
id|ufs_panic
(paren
r_struct
id|super_block
op_star
id|sb
comma
r_const
r_char
op_star
id|function
comma
r_const
r_char
op_star
id|fmt
comma
dot
dot
dot
)paren
(brace
r_struct
id|ufs_sb_private_info
op_star
id|uspi
suffix:semicolon
r_struct
id|ufs_super_block_first
op_star
id|usb1
suffix:semicolon
id|va_list
id|args
suffix:semicolon
id|uspi
op_assign
id|UFS_SB
c_func
(paren
id|sb
)paren
op_member_access_from_pointer
id|s_uspi
suffix:semicolon
id|usb1
op_assign
id|ubh_get_usb_first
c_func
(paren
id|USPI_UBH
)paren
suffix:semicolon
r_if
c_cond
(paren
op_logical_neg
(paren
id|sb-&gt;s_flags
op_amp
id|MS_RDONLY
)paren
)paren
(brace
id|usb1-&gt;fs_clean
op_assign
id|UFS_FSBAD
suffix:semicolon
id|ubh_mark_buffer_dirty
c_func
(paren
id|USPI_UBH
)paren
suffix:semicolon
id|sb-&gt;s_dirt
op_assign
l_int|1
suffix:semicolon
)brace
id|va_start
(paren
id|args
comma
id|fmt
)paren
suffix:semicolon
id|vsprintf
(paren
id|error_buf
comma
id|fmt
comma
id|args
)paren
suffix:semicolon
id|va_end
(paren
id|args
)paren
suffix:semicolon
id|sb-&gt;s_flags
op_or_assign
id|MS_RDONLY
suffix:semicolon
id|printk
(paren
id|KERN_CRIT
l_string|&quot;UFS-fs panic (device %s): %s: %s&bslash;n&quot;
comma
id|sb-&gt;s_id
comma
id|function
comma
id|error_buf
)paren
suffix:semicolon
)brace
DECL|function|ufs_warning
r_void
id|ufs_warning
(paren
r_struct
id|super_block
op_star
id|sb
comma
r_const
r_char
op_star
id|function
comma
r_const
r_char
op_star
id|fmt
comma
dot
dot
dot
)paren
(brace
id|va_list
id|args
suffix:semicolon
id|va_start
(paren
id|args
comma
id|fmt
)paren
suffix:semicolon
id|vsprintf
(paren
id|error_buf
comma
id|fmt
comma
id|args
)paren
suffix:semicolon
id|va_end
(paren
id|args
)paren
suffix:semicolon
id|printk
(paren
id|KERN_WARNING
l_string|&quot;UFS-fs warning (device %s): %s: %s&bslash;n&quot;
comma
id|sb-&gt;s_id
comma
id|function
comma
id|error_buf
)paren
suffix:semicolon
)brace
r_enum
(brace
DECL|enumerator|Opt_type_old
DECL|enumerator|Opt_type_sunx86
DECL|enumerator|Opt_type_sun
DECL|enumerator|Opt_type_44bsd
id|Opt_type_old
comma
id|Opt_type_sunx86
comma
id|Opt_type_sun
comma
id|Opt_type_44bsd
comma
DECL|enumerator|Opt_type_ufs2
DECL|enumerator|Opt_type_hp
DECL|enumerator|Opt_type_nextstepcd
DECL|enumerator|Opt_type_nextstep
id|Opt_type_ufs2
comma
id|Opt_type_hp
comma
id|Opt_type_nextstepcd
comma
id|Opt_type_nextstep
comma
DECL|enumerator|Opt_type_openstep
DECL|enumerator|Opt_onerror_panic
DECL|enumerator|Opt_onerror_lock
id|Opt_type_openstep
comma
id|Opt_onerror_panic
comma
id|Opt_onerror_lock
comma
DECL|enumerator|Opt_onerror_umount
DECL|enumerator|Opt_onerror_repair
DECL|enumerator|Opt_err
id|Opt_onerror_umount
comma
id|Opt_onerror_repair
comma
id|Opt_err
)brace
suffix:semicolon
DECL|variable|tokens
r_static
id|match_table_t
id|tokens
op_assign
(brace
(brace
id|Opt_type_old
comma
l_string|&quot;ufstype=old&quot;
)brace
comma
(brace
id|Opt_type_sunx86
comma
l_string|&quot;ufstype=sunx86&quot;
)brace
comma
(brace
id|Opt_type_sun
comma
l_string|&quot;ufstype=sun&quot;
)brace
comma
(brace
id|Opt_type_44bsd
comma
l_string|&quot;ufstype=44bsd&quot;
)brace
comma
(brace
id|Opt_type_ufs2
comma
l_string|&quot;ufstype=ufs2&quot;
)brace
comma
(brace
id|Opt_type_ufs2
comma
l_string|&quot;ufstype=5xbsd&quot;
)brace
comma
(brace
id|Opt_type_hp
comma
l_string|&quot;ufstype=hp&quot;
)brace
comma
(brace
id|Opt_type_nextstepcd
comma
l_string|&quot;ufstype=nextstep-cd&quot;
)brace
comma
(brace
id|Opt_type_nextstep
comma
l_string|&quot;ufstype=nextstep&quot;
)brace
comma
(brace
id|Opt_type_openstep
comma
l_string|&quot;ufstype=openstep&quot;
)brace
comma
(brace
id|Opt_onerror_panic
comma
l_string|&quot;onerror=panic&quot;
)brace
comma
(brace
id|Opt_onerror_lock
comma
l_string|&quot;onerror=lock&quot;
)brace
comma
(brace
id|Opt_onerror_umount
comma
l_string|&quot;onerror=umount&quot;
)brace
comma
(brace
id|Opt_onerror_repair
comma
l_string|&quot;onerror=repair&quot;
)brace
comma
(brace
id|Opt_err
comma
l_int|NULL
)brace
)brace
suffix:semicolon
DECL|function|ufs_parse_options
r_static
r_int
id|ufs_parse_options
(paren
r_char
op_star
id|options
comma
r_int
op_star
id|mount_options
)paren
(brace
r_char
op_star
id|p
suffix:semicolon
id|UFSD
c_func
(paren
(paren
l_string|&quot;ENTER&bslash;n&quot;
)paren
)paren
r_if
c_cond
(paren
op_logical_neg
id|options
)paren
r_return
l_int|1
suffix:semicolon
r_while
c_loop
(paren
(paren
id|p
op_assign
id|strsep
c_func
(paren
op_amp
id|options
comma
l_string|&quot;,&quot;
)paren
)paren
op_ne
l_int|NULL
)paren
(brace
id|substring_t
id|args
(braket
id|MAX_OPT_ARGS
)braket
suffix:semicolon
r_int
id|token
suffix:semicolon
r_if
c_cond
(paren
op_logical_neg
op_star
id|p
)paren
r_continue
suffix:semicolon
id|token
op_assign
id|match_token
c_func
(paren
id|p
comma
id|tokens
comma
id|args
)paren
suffix:semicolon
r_switch
c_cond
(paren
id|token
)paren
(brace
r_case
id|Opt_type_old
suffix:colon
id|ufs_clear_opt
(paren
op_star
id|mount_options
comma
id|UFSTYPE
)paren
suffix:semicolon
id|ufs_set_opt
(paren
op_star
id|mount_options
comma
id|UFSTYPE_OLD
)paren
suffix:semicolon
r_break
suffix:semicolon
r_case
id|Opt_type_sunx86
suffix:colon
id|ufs_clear_opt
(paren
op_star
id|mount_options
comma
id|UFSTYPE
)paren
suffix:semicolon
id|ufs_set_opt
(paren
op_star
id|mount_options
comma
id|UFSTYPE_SUNx86
)paren
suffix:semicolon
r_break
suffix:semicolon
r_case
id|Opt_type_sun
suffix:colon
id|ufs_clear_opt
(paren
op_star
id|mount_options
comma
id|UFSTYPE
)paren
suffix:semicolon
id|ufs_set_opt
(paren
op_star
id|mount_options
comma
id|UFSTYPE_SUN
)paren
suffix:semicolon
r_break
suffix:semicolon
r_case
id|Opt_type_44bsd
suffix:colon
id|ufs_clear_opt
(paren
op_star
id|mount_options
comma
id|UFSTYPE
)paren
suffix:semicolon
id|ufs_set_opt
(paren
op_star
id|mount_options
comma
id|UFSTYPE_44BSD
)paren
suffix:semicolon
r_break
suffix:semicolon
r_case
id|Opt_type_ufs2
suffix:colon
id|ufs_clear_opt
c_func
(paren
op_star
id|mount_options
comma
id|UFSTYPE
)paren
suffix:semicolon
id|ufs_set_opt
c_func
(paren
op_star
id|mount_options
comma
id|UFSTYPE_UFS2
)paren
suffix:semicolon
r_break
suffix:semicolon
r_case
id|Opt_type_hp
suffix:colon
id|ufs_clear_opt
(paren
op_star
id|mount_options
comma
id|UFSTYPE
)paren
suffix:semicolon
id|ufs_set_opt
(paren
op_star
id|mount_options
comma
id|UFSTYPE_HP
)paren
suffix:semicolon
r_break
suffix:semicolon
r_case
id|Opt_type_nextstepcd
suffix:colon
id|ufs_clear_opt
(paren
op_star
id|mount_options
comma
id|UFSTYPE
)paren
suffix:semicolon
id|ufs_set_opt
(paren
op_star
id|mount_options
comma
id|UFSTYPE_NEXTSTEP_CD
)paren
suffix:semicolon
r_break
suffix:semicolon
r_case
id|Opt_type_nextstep
suffix:colon
id|ufs_clear_opt
(paren
op_star
id|mount_options
comma
id|UFSTYPE
)paren
suffix:semicolon
id|ufs_set_opt
(paren
op_star
id|mount_options
comma
id|UFSTYPE_NEXTSTEP
)paren
suffix:semicolon
r_break
suffix:semicolon
r_case
id|Opt_type_openstep
suffix:colon
id|ufs_clear_opt
(paren
op_star
id|mount_options
comma
id|UFSTYPE
)paren
suffix:semicolon
id|ufs_set_opt
(paren
op_star
id|mount_options
comma
id|UFSTYPE_OPENSTEP
)paren
suffix:semicolon
r_break
suffix:semicolon
r_case
id|Opt_onerror_panic
suffix:colon
id|ufs_clear_opt
(paren
op_star
id|mount_options
comma
id|ONERROR
)paren
suffix:semicolon
id|ufs_set_opt
(paren
op_star
id|mount_options
comma
id|ONERROR_PANIC
)paren
suffix:semicolon
r_break
suffix:semicolon
r_case
id|Opt_onerror_lock
suffix:colon
id|ufs_clear_opt
(paren
op_star
id|mount_options
comma
id|ONERROR
)paren
suffix:semicolon
id|ufs_set_opt
(paren
op_star
id|mount_options
comma
id|ONERROR_LOCK
)paren
suffix:semicolon
r_break
suffix:semicolon
r_case
id|Opt_onerror_umount
suffix:colon
id|ufs_clear_opt
(paren
op_star
id|mount_options
comma
id|ONERROR
)paren
suffix:semicolon
id|ufs_set_opt
(paren
op_star
id|mount_options
comma
id|ONERROR_UMOUNT
)paren
suffix:semicolon
r_break
suffix:semicolon
r_case
id|Opt_onerror_repair
suffix:colon
id|printk
c_func
(paren
l_string|&quot;UFS-fs: Unable to do repair on error, &quot;
l_string|&quot;will lock lock instead&bslash;n&quot;
)paren
suffix:semicolon
id|ufs_clear_opt
(paren
op_star
id|mount_options
comma
id|ONERROR
)paren
suffix:semicolon
id|ufs_set_opt
(paren
op_star
id|mount_options
comma
id|ONERROR_REPAIR
)paren
suffix:semicolon
r_break
suffix:semicolon
r_default
suffix:colon
id|printk
c_func
(paren
l_string|&quot;UFS-fs: Invalid option: &bslash;&quot;%s&bslash;&quot; &quot;
l_string|&quot;or missing value&bslash;n&quot;
comma
id|p
)paren
suffix:semicolon
r_return
l_int|0
suffix:semicolon
)brace
)brace
r_return
l_int|1
suffix:semicolon
)brace
multiline_comment|/*&n; * Read on-disk structures associated with cylinder groups&n; */
DECL|function|ufs_read_cylinder_structures
r_static
r_int
id|ufs_read_cylinder_structures
(paren
r_struct
id|super_block
op_star
id|sb
)paren
(brace
r_struct
id|ufs_sb_info
op_star
id|sbi
op_assign
id|UFS_SB
c_func
(paren
id|sb
)paren
suffix:semicolon
r_struct
id|ufs_sb_private_info
op_star
id|uspi
suffix:semicolon
r_struct
id|ufs_super_block
op_star
id|usb
suffix:semicolon
r_struct
id|ufs_buffer_head
op_star
id|ubh
suffix:semicolon
r_int
r_char
op_star
id|base
comma
op_star
id|space
suffix:semicolon
r_int
id|size
comma
id|blks
comma
id|i
suffix:semicolon
r_int
id|flags
op_assign
l_int|0
suffix:semicolon
id|UFSD
c_func
(paren
(paren
l_string|&quot;ENTER&bslash;n&quot;
)paren
)paren
id|uspi
op_assign
id|sbi-&gt;s_uspi
suffix:semicolon
id|usb
op_assign
(paren
r_struct
id|ufs_super_block
op_star
)paren
(paren
(paren
r_struct
id|ufs_buffer_head
op_star
)paren
id|uspi
)paren
op_member_access_from_pointer
id|bh
(braket
l_int|0
)braket
op_member_access_from_pointer
id|b_data
suffix:semicolon
id|flags
op_assign
id|UFS_SB
c_func
(paren
id|sb
)paren
op_member_access_from_pointer
id|s_flags
suffix:semicolon
multiline_comment|/*&n;&t; * Read cs structures from (usually) first data block&n;&t; * on the device. &n;&t; */
id|size
op_assign
id|uspi-&gt;s_cssize
suffix:semicolon
id|blks
op_assign
(paren
id|size
op_plus
id|uspi-&gt;s_fsize
op_minus
l_int|1
)paren
op_rshift
id|uspi-&gt;s_fshift
suffix:semicolon
id|base
op_assign
id|space
op_assign
id|kmalloc
c_func
(paren
id|size
comma
id|GFP_KERNEL
)paren
suffix:semicolon
r_if
c_cond
(paren
op_logical_neg
id|base
)paren
r_goto
id|failed
suffix:semicolon
r_for
c_loop
(paren
id|i
op_assign
l_int|0
suffix:semicolon
id|i
OL
id|blks
suffix:semicolon
id|i
op_add_assign
id|uspi-&gt;s_fpb
)paren
(brace
id|size
op_assign
id|uspi-&gt;s_bsize
suffix:semicolon
r_if
c_cond
(paren
id|i
op_plus
id|uspi-&gt;s_fpb
OG
id|blks
)paren
id|size
op_assign
(paren
id|blks
op_minus
id|i
)paren
op_star
id|uspi-&gt;s_fsize
suffix:semicolon
r_if
c_cond
(paren
(paren
id|flags
op_amp
id|UFS_TYPE_MASK
)paren
op_eq
id|UFS_TYPE_UFS2
)paren
(brace
id|ubh
op_assign
id|ubh_bread
c_func
(paren
id|sb
comma
id|fs64_to_cpu
c_func
(paren
id|sb
comma
id|usb-&gt;fs_u11.fs_u2.fs_csaddr
)paren
op_plus
id|i
comma
id|size
)paren
suffix:semicolon
r_if
c_cond
(paren
op_logical_neg
id|ubh
)paren
r_goto
id|failed
suffix:semicolon
id|ubh_ubhcpymem
(paren
id|space
comma
id|ubh
comma
id|size
)paren
suffix:semicolon
id|sbi-&gt;s_csp
(braket
id|ufs_fragstoblks
c_func
(paren
id|i
)paren
)braket
op_assign
(paren
r_struct
id|ufs_csum
op_star
)paren
id|space
suffix:semicolon
)brace
r_else
(brace
id|ubh
op_assign
id|ubh_bread
c_func
(paren
id|sb
comma
id|uspi-&gt;s_csaddr
op_plus
id|i
comma
id|size
)paren
suffix:semicolon
r_if
c_cond
(paren
op_logical_neg
id|ubh
)paren
r_goto
id|failed
suffix:semicolon
id|ubh_ubhcpymem
c_func
(paren
id|space
comma
id|ubh
comma
id|size
)paren
suffix:semicolon
id|sbi-&gt;s_csp
(braket
id|ufs_fragstoblks
c_func
(paren
id|i
)paren
)braket
op_assign
(paren
r_struct
id|ufs_csum
op_star
)paren
id|space
suffix:semicolon
)brace
id|space
op_add_assign
id|size
suffix:semicolon
id|ubh_brelse
(paren
id|ubh
)paren
suffix:semicolon
id|ubh
op_assign
l_int|NULL
suffix:semicolon
)brace
multiline_comment|/*&n;&t; * Read cylinder group (we read only first fragment from block&n;&t; * at this time) and prepare internal data structures for cg caching.&n;&t; */
r_if
c_cond
(paren
op_logical_neg
(paren
id|sbi-&gt;s_ucg
op_assign
id|kmalloc
(paren
r_sizeof
(paren
r_struct
id|buffer_head
op_star
)paren
op_star
id|uspi-&gt;s_ncg
comma
id|GFP_KERNEL
)paren
)paren
)paren
r_goto
id|failed
suffix:semicolon
r_for
c_loop
(paren
id|i
op_assign
l_int|0
suffix:semicolon
id|i
OL
id|uspi-&gt;s_ncg
suffix:semicolon
id|i
op_increment
)paren
id|sbi-&gt;s_ucg
(braket
id|i
)braket
op_assign
l_int|NULL
suffix:semicolon
r_for
c_loop
(paren
id|i
op_assign
l_int|0
suffix:semicolon
id|i
OL
id|UFS_MAX_GROUP_LOADED
suffix:semicolon
id|i
op_increment
)paren
(brace
id|sbi-&gt;s_ucpi
(braket
id|i
)braket
op_assign
l_int|NULL
suffix:semicolon
id|sbi-&gt;s_cgno
(braket
id|i
)braket
op_assign
id|UFS_CGNO_EMPTY
suffix:semicolon
)brace
r_for
c_loop
(paren
id|i
op_assign
l_int|0
suffix:semicolon
id|i
OL
id|uspi-&gt;s_ncg
suffix:semicolon
id|i
op_increment
)paren
(brace
id|UFSD
c_func
(paren
(paren
l_string|&quot;read cg %u&bslash;n&quot;
comma
id|i
)paren
)paren
r_if
c_cond
(paren
op_logical_neg
(paren
id|sbi-&gt;s_ucg
(braket
id|i
)braket
op_assign
id|sb_bread
c_func
(paren
id|sb
comma
id|ufs_cgcmin
c_func
(paren
id|i
)paren
)paren
)paren
)paren
r_goto
id|failed
suffix:semicolon
r_if
c_cond
(paren
op_logical_neg
id|ufs_cg_chkmagic
(paren
id|sb
comma
(paren
r_struct
id|ufs_cylinder_group
op_star
)paren
id|sbi-&gt;s_ucg
(braket
id|i
)braket
op_member_access_from_pointer
id|b_data
)paren
)paren
r_goto
id|failed
suffix:semicolon
macro_line|#ifdef UFS_SUPER_DEBUG_MORE
id|ufs_print_cylinder_stuff
c_func
(paren
id|sb
comma
(paren
r_struct
id|ufs_cylinder_group
op_star
)paren
id|sbi-&gt;s_ucg
(braket
id|i
)braket
op_member_access_from_pointer
id|b_data
)paren
suffix:semicolon
macro_line|#endif
)brace
r_for
c_loop
(paren
id|i
op_assign
l_int|0
suffix:semicolon
id|i
OL
id|UFS_MAX_GROUP_LOADED
suffix:semicolon
id|i
op_increment
)paren
(brace
r_if
c_cond
(paren
op_logical_neg
(paren
id|sbi-&gt;s_ucpi
(braket
id|i
)braket
op_assign
id|kmalloc
(paren
r_sizeof
(paren
r_struct
id|ufs_cg_private_info
)paren
comma
id|GFP_KERNEL
)paren
)paren
)paren
r_goto
id|failed
suffix:semicolon
id|sbi-&gt;s_cgno
(braket
id|i
)braket
op_assign
id|UFS_CGNO_EMPTY
suffix:semicolon
)brace
id|sbi-&gt;s_cg_loaded
op_assign
l_int|0
suffix:semicolon
id|UFSD
c_func
(paren
(paren
l_string|&quot;EXIT&bslash;n&quot;
)paren
)paren
r_return
l_int|1
suffix:semicolon
id|failed
suffix:colon
r_if
c_cond
(paren
id|base
)paren
id|kfree
(paren
id|base
)paren
suffix:semicolon
r_if
c_cond
(paren
id|sbi-&gt;s_ucg
)paren
(brace
r_for
c_loop
(paren
id|i
op_assign
l_int|0
suffix:semicolon
id|i
OL
id|uspi-&gt;s_ncg
suffix:semicolon
id|i
op_increment
)paren
r_if
c_cond
(paren
id|sbi-&gt;s_ucg
(braket
id|i
)braket
)paren
id|brelse
(paren
id|sbi-&gt;s_ucg
(braket
id|i
)braket
)paren
suffix:semicolon
id|kfree
(paren
id|sbi-&gt;s_ucg
)paren
suffix:semicolon
r_for
c_loop
(paren
id|i
op_assign
l_int|0
suffix:semicolon
id|i
OL
id|UFS_MAX_GROUP_LOADED
suffix:semicolon
id|i
op_increment
)paren
r_if
c_cond
(paren
id|sbi-&gt;s_ucpi
(braket
id|i
)braket
)paren
id|kfree
(paren
id|sbi-&gt;s_ucpi
(braket
id|i
)braket
)paren
suffix:semicolon
)brace
id|UFSD
c_func
(paren
(paren
l_string|&quot;EXIT (FAILED)&bslash;n&quot;
)paren
)paren
r_return
l_int|0
suffix:semicolon
)brace
multiline_comment|/*&n; * Put on-disk structures associated with cylinder groups and &n; * write them back to disk&n; */
DECL|function|ufs_put_cylinder_structures
r_static
r_void
id|ufs_put_cylinder_structures
(paren
r_struct
id|super_block
op_star
id|sb
)paren
(brace
r_struct
id|ufs_sb_info
op_star
id|sbi
op_assign
id|UFS_SB
c_func
(paren
id|sb
)paren
suffix:semicolon
r_struct
id|ufs_sb_private_info
op_star
id|uspi
suffix:semicolon
r_struct
id|ufs_buffer_head
op_star
id|ubh
suffix:semicolon
r_int
r_char
op_star
id|base
comma
op_star
id|space
suffix:semicolon
r_int
id|blks
comma
id|size
comma
id|i
suffix:semicolon
id|UFSD
c_func
(paren
(paren
l_string|&quot;ENTER&bslash;n&quot;
)paren
)paren
id|uspi
op_assign
id|sbi-&gt;s_uspi
suffix:semicolon
id|size
op_assign
id|uspi-&gt;s_cssize
suffix:semicolon
id|blks
op_assign
(paren
id|size
op_plus
id|uspi-&gt;s_fsize
op_minus
l_int|1
)paren
op_rshift
id|uspi-&gt;s_fshift
suffix:semicolon
id|base
op_assign
id|space
op_assign
(paren
r_char
op_star
)paren
id|sbi-&gt;s_csp
(braket
l_int|0
)braket
suffix:semicolon
r_for
c_loop
(paren
id|i
op_assign
l_int|0
suffix:semicolon
id|i
OL
id|blks
suffix:semicolon
id|i
op_add_assign
id|uspi-&gt;s_fpb
)paren
(brace
id|size
op_assign
id|uspi-&gt;s_bsize
suffix:semicolon
r_if
c_cond
(paren
id|i
op_plus
id|uspi-&gt;s_fpb
OG
id|blks
)paren
id|size
op_assign
(paren
id|blks
op_minus
id|i
)paren
op_star
id|uspi-&gt;s_fsize
suffix:semicolon
id|ubh
op_assign
id|ubh_bread
c_func
(paren
id|sb
comma
id|uspi-&gt;s_csaddr
op_plus
id|i
comma
id|size
)paren
suffix:semicolon
id|ubh_memcpyubh
(paren
id|ubh
comma
id|space
comma
id|size
)paren
suffix:semicolon
id|space
op_add_assign
id|size
suffix:semicolon
id|ubh_mark_buffer_uptodate
(paren
id|ubh
comma
l_int|1
)paren
suffix:semicolon
id|ubh_mark_buffer_dirty
(paren
id|ubh
)paren
suffix:semicolon
id|ubh_brelse
(paren
id|ubh
)paren
suffix:semicolon
)brace
r_for
c_loop
(paren
id|i
op_assign
l_int|0
suffix:semicolon
id|i
OL
id|sbi-&gt;s_cg_loaded
suffix:semicolon
id|i
op_increment
)paren
(brace
id|ufs_put_cylinder
(paren
id|sb
comma
id|i
)paren
suffix:semicolon
id|kfree
(paren
id|sbi-&gt;s_ucpi
(braket
id|i
)braket
)paren
suffix:semicolon
)brace
r_for
c_loop
(paren
suffix:semicolon
id|i
OL
id|UFS_MAX_GROUP_LOADED
suffix:semicolon
id|i
op_increment
)paren
id|kfree
(paren
id|sbi-&gt;s_ucpi
(braket
id|i
)braket
)paren
suffix:semicolon
r_for
c_loop
(paren
id|i
op_assign
l_int|0
suffix:semicolon
id|i
OL
id|uspi-&gt;s_ncg
suffix:semicolon
id|i
op_increment
)paren
id|brelse
(paren
id|sbi-&gt;s_ucg
(braket
id|i
)braket
)paren
suffix:semicolon
id|kfree
(paren
id|sbi-&gt;s_ucg
)paren
suffix:semicolon
id|kfree
(paren
id|base
)paren
suffix:semicolon
id|UFSD
c_func
(paren
(paren
l_string|&quot;EXIT&bslash;n&quot;
)paren
)paren
)brace
DECL|function|ufs_fill_super
r_static
r_int
id|ufs_fill_super
c_func
(paren
r_struct
id|super_block
op_star
id|sb
comma
r_void
op_star
id|data
comma
r_int
id|silent
)paren
(brace
r_struct
id|ufs_sb_info
op_star
id|sbi
suffix:semicolon
r_struct
id|ufs_sb_private_info
op_star
id|uspi
suffix:semicolon
r_struct
id|ufs_super_block_first
op_star
id|usb1
suffix:semicolon
r_struct
id|ufs_super_block_second
op_star
id|usb2
suffix:semicolon
r_struct
id|ufs_super_block_third
op_star
id|usb3
suffix:semicolon
r_struct
id|ufs_super_block
op_star
id|usb
suffix:semicolon
r_struct
id|ufs_buffer_head
op_star
id|ubh
suffix:semicolon
r_struct
id|inode
op_star
id|inode
suffix:semicolon
r_int
id|block_size
comma
id|super_block_size
suffix:semicolon
r_int
id|flags
suffix:semicolon
id|uspi
op_assign
l_int|NULL
suffix:semicolon
id|ubh
op_assign
l_int|NULL
suffix:semicolon
id|flags
op_assign
l_int|0
suffix:semicolon
id|UFSD
c_func
(paren
(paren
l_string|&quot;ENTER&bslash;n&quot;
)paren
)paren
id|sbi
op_assign
id|kmalloc
c_func
(paren
r_sizeof
(paren
r_struct
id|ufs_sb_info
)paren
comma
id|GFP_KERNEL
)paren
suffix:semicolon
r_if
c_cond
(paren
op_logical_neg
id|sbi
)paren
r_goto
id|failed_nomem
suffix:semicolon
id|sb-&gt;s_fs_info
op_assign
id|sbi
suffix:semicolon
id|memset
c_func
(paren
id|sbi
comma
l_int|0
comma
r_sizeof
(paren
r_struct
id|ufs_sb_info
)paren
)paren
suffix:semicolon
id|UFSD
c_func
(paren
(paren
l_string|&quot;flag %u&bslash;n&quot;
comma
(paren
r_int
)paren
(paren
id|sb-&gt;s_flags
op_amp
id|MS_RDONLY
)paren
)paren
)paren
macro_line|#ifndef CONFIG_UFS_FS_WRITE
r_if
c_cond
(paren
op_logical_neg
(paren
id|sb-&gt;s_flags
op_amp
id|MS_RDONLY
)paren
)paren
(brace
id|printk
c_func
(paren
l_string|&quot;ufs was compiled with read-only support, &quot;
l_string|&quot;can&squot;t be mounted as read-write&bslash;n&quot;
)paren
suffix:semicolon
r_goto
id|failed
suffix:semicolon
)brace
macro_line|#endif
multiline_comment|/*&n;&t; * Set default mount options&n;&t; * Parse mount options&n;&t; */
id|sbi-&gt;s_mount_opt
op_assign
l_int|0
suffix:semicolon
id|ufs_set_opt
(paren
id|sbi-&gt;s_mount_opt
comma
id|ONERROR_LOCK
)paren
suffix:semicolon
r_if
c_cond
(paren
op_logical_neg
id|ufs_parse_options
(paren
(paren
r_char
op_star
)paren
id|data
comma
op_amp
id|sbi-&gt;s_mount_opt
)paren
)paren
(brace
id|printk
c_func
(paren
l_string|&quot;wrong mount options&bslash;n&quot;
)paren
suffix:semicolon
r_goto
id|failed
suffix:semicolon
)brace
r_if
c_cond
(paren
op_logical_neg
(paren
id|sbi-&gt;s_mount_opt
op_amp
id|UFS_MOUNT_UFSTYPE
)paren
)paren
(brace
r_if
c_cond
(paren
op_logical_neg
id|silent
)paren
id|printk
c_func
(paren
l_string|&quot;You didn&squot;t specify the type of your ufs filesystem&bslash;n&bslash;n&quot;
l_string|&quot;mount -t ufs -o ufstype=&quot;
l_string|&quot;sun|sunx86|44bsd|ufs2|5xbsd|old|hp|nextstep|netxstep-cd|openstep ...&bslash;n&bslash;n&quot;
l_string|&quot;&gt;&gt;&gt;WARNING&lt;&lt;&lt; Wrong ufstype may corrupt your filesystem, &quot;
l_string|&quot;default is ufstype=old&bslash;n&quot;
)paren
suffix:semicolon
id|ufs_set_opt
(paren
id|sbi-&gt;s_mount_opt
comma
id|UFSTYPE_OLD
)paren
suffix:semicolon
)brace
id|sbi-&gt;s_uspi
op_assign
id|uspi
op_assign
id|kmalloc
(paren
r_sizeof
(paren
r_struct
id|ufs_sb_private_info
)paren
comma
id|GFP_KERNEL
)paren
suffix:semicolon
r_if
c_cond
(paren
op_logical_neg
id|uspi
)paren
r_goto
id|failed
suffix:semicolon
multiline_comment|/* Keep 2Gig file limit. Some UFS variants need to override &n;&t;   this but as I don&squot;t know which I&squot;ll let those in the know loosen&n;&t;   the rules */
r_switch
c_cond
(paren
id|sbi-&gt;s_mount_opt
op_amp
id|UFS_MOUNT_UFSTYPE
)paren
(brace
r_case
id|UFS_MOUNT_UFSTYPE_44BSD
suffix:colon
id|UFSD
c_func
(paren
(paren
l_string|&quot;ufstype=44bsd&bslash;n&quot;
)paren
)paren
id|uspi-&gt;s_fsize
op_assign
id|block_size
op_assign
l_int|512
suffix:semicolon
id|uspi-&gt;s_fmask
op_assign
op_complement
(paren
l_int|512
op_minus
l_int|1
)paren
suffix:semicolon
id|uspi-&gt;s_fshift
op_assign
l_int|9
suffix:semicolon
id|uspi-&gt;s_sbsize
op_assign
id|super_block_size
op_assign
l_int|1536
suffix:semicolon
id|uspi-&gt;s_sbbase
op_assign
l_int|0
suffix:semicolon
id|flags
op_or_assign
id|UFS_DE_44BSD
op_or
id|UFS_UID_44BSD
op_or
id|UFS_ST_44BSD
op_or
id|UFS_CG_44BSD
suffix:semicolon
r_break
suffix:semicolon
r_case
id|UFS_MOUNT_UFSTYPE_UFS2
suffix:colon
id|UFSD
c_func
(paren
(paren
l_string|&quot;ufstype=ufs2&bslash;n&quot;
)paren
)paren
id|uspi-&gt;s_fsize
op_assign
id|block_size
op_assign
l_int|512
suffix:semicolon
id|uspi-&gt;s_fmask
op_assign
op_complement
(paren
l_int|512
op_minus
l_int|1
)paren
suffix:semicolon
id|uspi-&gt;s_fshift
op_assign
l_int|9
suffix:semicolon
id|uspi-&gt;s_sbsize
op_assign
id|super_block_size
op_assign
l_int|1536
suffix:semicolon
id|uspi-&gt;s_sbbase
op_assign
l_int|0
suffix:semicolon
id|flags
op_or_assign
id|UFS_TYPE_UFS2
op_or
id|UFS_DE_44BSD
op_or
id|UFS_UID_44BSD
op_or
id|UFS_ST_44BSD
op_or
id|UFS_CG_44BSD
suffix:semicolon
r_if
c_cond
(paren
op_logical_neg
(paren
id|sb-&gt;s_flags
op_amp
id|MS_RDONLY
)paren
)paren
(brace
id|printk
c_func
(paren
id|KERN_INFO
l_string|&quot;ufstype=ufs2 is supported read-only&bslash;n&quot;
)paren
suffix:semicolon
id|sb-&gt;s_flags
op_or_assign
id|MS_RDONLY
suffix:semicolon
)brace
r_break
suffix:semicolon
r_case
id|UFS_MOUNT_UFSTYPE_SUN
suffix:colon
id|UFSD
c_func
(paren
(paren
l_string|&quot;ufstype=sun&bslash;n&quot;
)paren
)paren
id|uspi-&gt;s_fsize
op_assign
id|block_size
op_assign
l_int|1024
suffix:semicolon
id|uspi-&gt;s_fmask
op_assign
op_complement
(paren
l_int|1024
op_minus
l_int|1
)paren
suffix:semicolon
id|uspi-&gt;s_fshift
op_assign
l_int|10
suffix:semicolon
id|uspi-&gt;s_sbsize
op_assign
id|super_block_size
op_assign
l_int|2048
suffix:semicolon
id|uspi-&gt;s_sbbase
op_assign
l_int|0
suffix:semicolon
id|uspi-&gt;s_maxsymlinklen
op_assign
l_int|56
suffix:semicolon
id|flags
op_or_assign
id|UFS_DE_OLD
op_or
id|UFS_UID_EFT
op_or
id|UFS_ST_SUN
op_or
id|UFS_CG_SUN
suffix:semicolon
r_break
suffix:semicolon
r_case
id|UFS_MOUNT_UFSTYPE_SUNx86
suffix:colon
id|UFSD
c_func
(paren
(paren
l_string|&quot;ufstype=sunx86&bslash;n&quot;
)paren
)paren
id|uspi-&gt;s_fsize
op_assign
id|block_size
op_assign
l_int|1024
suffix:semicolon
id|uspi-&gt;s_fmask
op_assign
op_complement
(paren
l_int|1024
op_minus
l_int|1
)paren
suffix:semicolon
id|uspi-&gt;s_fshift
op_assign
l_int|10
suffix:semicolon
id|uspi-&gt;s_sbsize
op_assign
id|super_block_size
op_assign
l_int|2048
suffix:semicolon
id|uspi-&gt;s_sbbase
op_assign
l_int|0
suffix:semicolon
id|uspi-&gt;s_maxsymlinklen
op_assign
l_int|56
suffix:semicolon
id|flags
op_or_assign
id|UFS_DE_OLD
op_or
id|UFS_UID_EFT
op_or
id|UFS_ST_SUNx86
op_or
id|UFS_CG_SUN
suffix:semicolon
r_break
suffix:semicolon
r_case
id|UFS_MOUNT_UFSTYPE_OLD
suffix:colon
id|UFSD
c_func
(paren
(paren
l_string|&quot;ufstype=old&bslash;n&quot;
)paren
)paren
id|uspi-&gt;s_fsize
op_assign
id|block_size
op_assign
l_int|1024
suffix:semicolon
id|uspi-&gt;s_fmask
op_assign
op_complement
(paren
l_int|1024
op_minus
l_int|1
)paren
suffix:semicolon
id|uspi-&gt;s_fshift
op_assign
l_int|10
suffix:semicolon
id|uspi-&gt;s_sbsize
op_assign
id|super_block_size
op_assign
l_int|2048
suffix:semicolon
id|uspi-&gt;s_sbbase
op_assign
l_int|0
suffix:semicolon
id|flags
op_or_assign
id|UFS_DE_OLD
op_or
id|UFS_UID_OLD
op_or
id|UFS_ST_OLD
op_or
id|UFS_CG_OLD
suffix:semicolon
r_if
c_cond
(paren
op_logical_neg
(paren
id|sb-&gt;s_flags
op_amp
id|MS_RDONLY
)paren
)paren
(brace
r_if
c_cond
(paren
op_logical_neg
id|silent
)paren
id|printk
c_func
(paren
id|KERN_INFO
l_string|&quot;ufstype=old is supported read-only&bslash;n&quot;
)paren
suffix:semicolon
id|sb-&gt;s_flags
op_or_assign
id|MS_RDONLY
suffix:semicolon
)brace
r_break
suffix:semicolon
r_case
id|UFS_MOUNT_UFSTYPE_NEXTSTEP
suffix:colon
id|UFSD
c_func
(paren
(paren
l_string|&quot;ufstype=nextstep&bslash;n&quot;
)paren
)paren
id|uspi-&gt;s_fsize
op_assign
id|block_size
op_assign
l_int|1024
suffix:semicolon
id|uspi-&gt;s_fmask
op_assign
op_complement
(paren
l_int|1024
op_minus
l_int|1
)paren
suffix:semicolon
id|uspi-&gt;s_fshift
op_assign
l_int|10
suffix:semicolon
id|uspi-&gt;s_sbsize
op_assign
id|super_block_size
op_assign
l_int|2048
suffix:semicolon
id|uspi-&gt;s_sbbase
op_assign
l_int|0
suffix:semicolon
id|flags
op_or_assign
id|UFS_DE_OLD
op_or
id|UFS_UID_OLD
op_or
id|UFS_ST_OLD
op_or
id|UFS_CG_OLD
suffix:semicolon
r_if
c_cond
(paren
op_logical_neg
(paren
id|sb-&gt;s_flags
op_amp
id|MS_RDONLY
)paren
)paren
(brace
r_if
c_cond
(paren
op_logical_neg
id|silent
)paren
id|printk
c_func
(paren
id|KERN_INFO
l_string|&quot;ufstype=nextstep is supported read-only&bslash;n&quot;
)paren
suffix:semicolon
id|sb-&gt;s_flags
op_or_assign
id|MS_RDONLY
suffix:semicolon
)brace
r_break
suffix:semicolon
r_case
id|UFS_MOUNT_UFSTYPE_NEXTSTEP_CD
suffix:colon
id|UFSD
c_func
(paren
(paren
l_string|&quot;ufstype=nextstep-cd&bslash;n&quot;
)paren
)paren
id|uspi-&gt;s_fsize
op_assign
id|block_size
op_assign
l_int|2048
suffix:semicolon
id|uspi-&gt;s_fmask
op_assign
op_complement
(paren
l_int|2048
op_minus
l_int|1
)paren
suffix:semicolon
id|uspi-&gt;s_fshift
op_assign
l_int|11
suffix:semicolon
id|uspi-&gt;s_sbsize
op_assign
id|super_block_size
op_assign
l_int|2048
suffix:semicolon
id|uspi-&gt;s_sbbase
op_assign
l_int|0
suffix:semicolon
id|flags
op_or_assign
id|UFS_DE_OLD
op_or
id|UFS_UID_OLD
op_or
id|UFS_ST_OLD
op_or
id|UFS_CG_OLD
suffix:semicolon
r_if
c_cond
(paren
op_logical_neg
(paren
id|sb-&gt;s_flags
op_amp
id|MS_RDONLY
)paren
)paren
(brace
r_if
c_cond
(paren
op_logical_neg
id|silent
)paren
id|printk
c_func
(paren
id|KERN_INFO
l_string|&quot;ufstype=nextstep-cd is supported read-only&bslash;n&quot;
)paren
suffix:semicolon
id|sb-&gt;s_flags
op_or_assign
id|MS_RDONLY
suffix:semicolon
)brace
r_break
suffix:semicolon
r_case
id|UFS_MOUNT_UFSTYPE_OPENSTEP
suffix:colon
id|UFSD
c_func
(paren
(paren
l_string|&quot;ufstype=openstep&bslash;n&quot;
)paren
)paren
id|uspi-&gt;s_fsize
op_assign
id|block_size
op_assign
l_int|1024
suffix:semicolon
id|uspi-&gt;s_fmask
op_assign
op_complement
(paren
l_int|1024
op_minus
l_int|1
)paren
suffix:semicolon
id|uspi-&gt;s_fshift
op_assign
l_int|10
suffix:semicolon
id|uspi-&gt;s_sbsize
op_assign
id|super_block_size
op_assign
l_int|2048
suffix:semicolon
id|uspi-&gt;s_sbbase
op_assign
l_int|0
suffix:semicolon
id|flags
op_or_assign
id|UFS_DE_44BSD
op_or
id|UFS_UID_44BSD
op_or
id|UFS_ST_44BSD
op_or
id|UFS_CG_44BSD
suffix:semicolon
r_if
c_cond
(paren
op_logical_neg
(paren
id|sb-&gt;s_flags
op_amp
id|MS_RDONLY
)paren
)paren
(brace
r_if
c_cond
(paren
op_logical_neg
id|silent
)paren
id|printk
c_func
(paren
id|KERN_INFO
l_string|&quot;ufstype=openstep is supported read-only&bslash;n&quot;
)paren
suffix:semicolon
id|sb-&gt;s_flags
op_or_assign
id|MS_RDONLY
suffix:semicolon
)brace
r_break
suffix:semicolon
r_case
id|UFS_MOUNT_UFSTYPE_HP
suffix:colon
id|UFSD
c_func
(paren
(paren
l_string|&quot;ufstype=hp&bslash;n&quot;
)paren
)paren
id|uspi-&gt;s_fsize
op_assign
id|block_size
op_assign
l_int|1024
suffix:semicolon
id|uspi-&gt;s_fmask
op_assign
op_complement
(paren
l_int|1024
op_minus
l_int|1
)paren
suffix:semicolon
id|uspi-&gt;s_fshift
op_assign
l_int|10
suffix:semicolon
id|uspi-&gt;s_sbsize
op_assign
id|super_block_size
op_assign
l_int|2048
suffix:semicolon
id|uspi-&gt;s_sbbase
op_assign
l_int|0
suffix:semicolon
id|flags
op_or_assign
id|UFS_DE_OLD
op_or
id|UFS_UID_OLD
op_or
id|UFS_ST_OLD
op_or
id|UFS_CG_OLD
suffix:semicolon
r_if
c_cond
(paren
op_logical_neg
(paren
id|sb-&gt;s_flags
op_amp
id|MS_RDONLY
)paren
)paren
(brace
r_if
c_cond
(paren
op_logical_neg
id|silent
)paren
id|printk
c_func
(paren
id|KERN_INFO
l_string|&quot;ufstype=hp is supported read-only&bslash;n&quot;
)paren
suffix:semicolon
id|sb-&gt;s_flags
op_or_assign
id|MS_RDONLY
suffix:semicolon
)brace
r_break
suffix:semicolon
r_default
suffix:colon
(brace
)brace
r_if
c_cond
(paren
op_logical_neg
id|silent
)paren
id|printk
c_func
(paren
l_string|&quot;unknown ufstype&bslash;n&quot;
)paren
suffix:semicolon
r_goto
id|failed
suffix:semicolon
)brace
id|again
suffix:colon
r_if
c_cond
(paren
op_logical_neg
id|sb_set_blocksize
c_func
(paren
id|sb
comma
id|block_size
)paren
)paren
(brace
id|printk
c_func
(paren
id|KERN_ERR
l_string|&quot;UFS: failed to set blocksize&bslash;n&quot;
)paren
suffix:semicolon
r_goto
id|failed
suffix:semicolon
)brace
multiline_comment|/*&n;&t; * read ufs super block from device&n;&t; */
r_if
c_cond
(paren
(paren
id|flags
op_amp
id|UFS_TYPE_MASK
)paren
op_eq
id|UFS_TYPE_UFS2
)paren
(brace
id|ubh
op_assign
id|ubh_bread_uspi
c_func
(paren
id|uspi
comma
id|sb
comma
id|uspi-&gt;s_sbbase
op_plus
id|SBLOCK_UFS2
op_div
id|block_size
comma
id|super_block_size
)paren
suffix:semicolon
)brace
r_else
(brace
id|ubh
op_assign
id|ubh_bread_uspi
c_func
(paren
id|uspi
comma
id|sb
comma
id|uspi-&gt;s_sbbase
op_plus
id|UFS_SBLOCK
op_div
id|block_size
comma
id|super_block_size
)paren
suffix:semicolon
)brace
r_if
c_cond
(paren
op_logical_neg
id|ubh
)paren
r_goto
id|failed
suffix:semicolon
id|usb1
op_assign
id|ubh_get_usb_first
c_func
(paren
id|USPI_UBH
)paren
suffix:semicolon
id|usb2
op_assign
id|ubh_get_usb_second
c_func
(paren
id|USPI_UBH
)paren
suffix:semicolon
id|usb3
op_assign
id|ubh_get_usb_third
c_func
(paren
id|USPI_UBH
)paren
suffix:semicolon
id|usb
op_assign
(paren
r_struct
id|ufs_super_block
op_star
)paren
(paren
(paren
r_struct
id|ufs_buffer_head
op_star
)paren
id|uspi
)paren
op_member_access_from_pointer
id|bh
(braket
l_int|0
)braket
op_member_access_from_pointer
id|b_data
suffix:semicolon
multiline_comment|/*&n;&t; * Check ufs magic number&n;&t; */
id|sbi-&gt;s_bytesex
op_assign
id|BYTESEX_LE
suffix:semicolon
r_switch
c_cond
(paren
(paren
id|uspi-&gt;fs_magic
op_assign
id|fs32_to_cpu
c_func
(paren
id|sb
comma
id|usb3-&gt;fs_magic
)paren
)paren
)paren
(brace
r_case
id|UFS_MAGIC
suffix:colon
r_case
id|UFS2_MAGIC
suffix:colon
r_case
id|UFS_MAGIC_LFN
suffix:colon
r_case
id|UFS_MAGIC_FEA
suffix:colon
r_case
id|UFS_MAGIC_4GB
suffix:colon
r_goto
id|magic_found
suffix:semicolon
)brace
id|sbi-&gt;s_bytesex
op_assign
id|BYTESEX_BE
suffix:semicolon
r_switch
c_cond
(paren
(paren
id|uspi-&gt;fs_magic
op_assign
id|fs32_to_cpu
c_func
(paren
id|sb
comma
id|usb3-&gt;fs_magic
)paren
)paren
)paren
(brace
r_case
id|UFS_MAGIC
suffix:colon
r_case
id|UFS2_MAGIC
suffix:colon
r_case
id|UFS_MAGIC_LFN
suffix:colon
r_case
id|UFS_MAGIC_FEA
suffix:colon
r_case
id|UFS_MAGIC_4GB
suffix:colon
r_goto
id|magic_found
suffix:semicolon
)brace
r_if
c_cond
(paren
(paren
(paren
(paren
id|sbi-&gt;s_mount_opt
op_amp
id|UFS_MOUNT_UFSTYPE
)paren
op_eq
id|UFS_MOUNT_UFSTYPE_NEXTSTEP
)paren
op_logical_or
(paren
(paren
id|sbi-&gt;s_mount_opt
op_amp
id|UFS_MOUNT_UFSTYPE
)paren
op_eq
id|UFS_MOUNT_UFSTYPE_NEXTSTEP_CD
)paren
op_logical_or
(paren
(paren
id|sbi-&gt;s_mount_opt
op_amp
id|UFS_MOUNT_UFSTYPE
)paren
op_eq
id|UFS_MOUNT_UFSTYPE_OPENSTEP
)paren
)paren
op_logical_and
id|uspi-&gt;s_sbbase
OL
l_int|256
)paren
(brace
id|ubh_brelse_uspi
c_func
(paren
id|uspi
)paren
suffix:semicolon
id|ubh
op_assign
l_int|NULL
suffix:semicolon
id|uspi-&gt;s_sbbase
op_add_assign
l_int|8
suffix:semicolon
r_goto
id|again
suffix:semicolon
)brace
r_if
c_cond
(paren
op_logical_neg
id|silent
)paren
id|printk
c_func
(paren
l_string|&quot;ufs_read_super: bad magic number&bslash;n&quot;
)paren
suffix:semicolon
r_goto
id|failed
suffix:semicolon
id|magic_found
suffix:colon
multiline_comment|/*&n;&t; * Check block and fragment sizes&n;&t; */
id|uspi-&gt;s_bsize
op_assign
id|fs32_to_cpu
c_func
(paren
id|sb
comma
id|usb1-&gt;fs_bsize
)paren
suffix:semicolon
id|uspi-&gt;s_fsize
op_assign
id|fs32_to_cpu
c_func
(paren
id|sb
comma
id|usb1-&gt;fs_fsize
)paren
suffix:semicolon
id|uspi-&gt;s_sbsize
op_assign
id|fs32_to_cpu
c_func
(paren
id|sb
comma
id|usb1-&gt;fs_sbsize
)paren
suffix:semicolon
id|uspi-&gt;s_fmask
op_assign
id|fs32_to_cpu
c_func
(paren
id|sb
comma
id|usb1-&gt;fs_fmask
)paren
suffix:semicolon
id|uspi-&gt;s_fshift
op_assign
id|fs32_to_cpu
c_func
(paren
id|sb
comma
id|usb1-&gt;fs_fshift
)paren
suffix:semicolon
r_if
c_cond
(paren
id|uspi-&gt;s_fsize
op_amp
(paren
id|uspi-&gt;s_fsize
op_minus
l_int|1
)paren
)paren
(brace
id|printk
c_func
(paren
id|KERN_ERR
l_string|&quot;ufs_read_super: fragment size %u is not a power of 2&bslash;n&quot;
comma
id|uspi-&gt;s_fsize
)paren
suffix:semicolon
r_goto
id|failed
suffix:semicolon
)brace
r_if
c_cond
(paren
id|uspi-&gt;s_fsize
OL
l_int|512
)paren
(brace
id|printk
c_func
(paren
id|KERN_ERR
l_string|&quot;ufs_read_super: fragment size %u is too small&bslash;n&quot;
comma
id|uspi-&gt;s_fsize
)paren
suffix:semicolon
r_goto
id|failed
suffix:semicolon
)brace
r_if
c_cond
(paren
id|uspi-&gt;s_fsize
OG
l_int|4096
)paren
(brace
id|printk
c_func
(paren
id|KERN_ERR
l_string|&quot;ufs_read_super: fragment size %u is too large&bslash;n&quot;
comma
id|uspi-&gt;s_fsize
)paren
suffix:semicolon
r_goto
id|failed
suffix:semicolon
)brace
r_if
c_cond
(paren
id|uspi-&gt;s_bsize
op_amp
(paren
id|uspi-&gt;s_bsize
op_minus
l_int|1
)paren
)paren
(brace
id|printk
c_func
(paren
id|KERN_ERR
l_string|&quot;ufs_read_super: block size %u is not a power of 2&bslash;n&quot;
comma
id|uspi-&gt;s_bsize
)paren
suffix:semicolon
r_goto
id|failed
suffix:semicolon
)brace
r_if
c_cond
(paren
id|uspi-&gt;s_bsize
OL
l_int|4096
)paren
(brace
id|printk
c_func
(paren
id|KERN_ERR
l_string|&quot;ufs_read_super: block size %u is too small&bslash;n&quot;
comma
id|uspi-&gt;s_bsize
)paren
suffix:semicolon
r_goto
id|failed
suffix:semicolon
)brace
r_if
c_cond
(paren
id|uspi-&gt;s_bsize
op_div
id|uspi-&gt;s_fsize
OG
l_int|8
)paren
(brace
id|printk
c_func
(paren
id|KERN_ERR
l_string|&quot;ufs_read_super: too many fragments per block (%u)&bslash;n&quot;
comma
id|uspi-&gt;s_bsize
op_div
id|uspi-&gt;s_fsize
)paren
suffix:semicolon
r_goto
id|failed
suffix:semicolon
)brace
r_if
c_cond
(paren
id|uspi-&gt;s_fsize
op_ne
id|block_size
op_logical_or
id|uspi-&gt;s_sbsize
op_ne
id|super_block_size
)paren
(brace
id|ubh_brelse_uspi
c_func
(paren
id|uspi
)paren
suffix:semicolon
id|ubh
op_assign
l_int|NULL
suffix:semicolon
id|block_size
op_assign
id|uspi-&gt;s_fsize
suffix:semicolon
id|super_block_size
op_assign
id|uspi-&gt;s_sbsize
suffix:semicolon
id|UFSD
c_func
(paren
(paren
l_string|&quot;another value of block_size or super_block_size %u, %u&bslash;n&quot;
comma
id|block_size
comma
id|super_block_size
)paren
)paren
r_goto
id|again
suffix:semicolon
)brace
macro_line|#ifdef UFS_SUPER_DEBUG_MORE
r_if
c_cond
(paren
(paren
id|flags
op_amp
id|UFS_TYPE_MASK
)paren
op_eq
id|UFS_TYPE_UFS2
)paren
id|ufs2_print_super_stuff
c_func
(paren
id|sb
comma
id|usb
)paren
suffix:semicolon
r_else
id|ufs_print_super_stuff
c_func
(paren
id|sb
comma
id|usb1
comma
id|usb2
comma
id|usb3
)paren
suffix:semicolon
macro_line|#endif
multiline_comment|/*&n;&t; * Check, if file system was correctly unmounted.&n;&t; * If not, make it read only.&n;&t; */
r_if
c_cond
(paren
(paren
(paren
id|flags
op_amp
id|UFS_ST_MASK
)paren
op_eq
id|UFS_ST_44BSD
)paren
op_logical_or
(paren
(paren
id|flags
op_amp
id|UFS_ST_MASK
)paren
op_eq
id|UFS_ST_OLD
)paren
op_logical_or
(paren
(paren
(paren
id|flags
op_amp
id|UFS_ST_MASK
)paren
op_eq
id|UFS_ST_SUN
op_logical_or
(paren
id|flags
op_amp
id|UFS_ST_MASK
)paren
op_eq
id|UFS_ST_SUNx86
)paren
op_logical_and
(paren
id|ufs_get_fs_state
c_func
(paren
id|sb
comma
id|usb1
comma
id|usb3
)paren
op_eq
(paren
id|UFS_FSOK
op_minus
id|fs32_to_cpu
c_func
(paren
id|sb
comma
id|usb1-&gt;fs_time
)paren
)paren
)paren
)paren
)paren
(brace
r_switch
c_cond
(paren
id|usb1-&gt;fs_clean
)paren
(brace
r_case
id|UFS_FSCLEAN
suffix:colon
id|UFSD
c_func
(paren
(paren
l_string|&quot;fs is clean&bslash;n&quot;
)paren
)paren
r_break
suffix:semicolon
r_case
id|UFS_FSSTABLE
suffix:colon
id|UFSD
c_func
(paren
(paren
l_string|&quot;fs is stable&bslash;n&quot;
)paren
)paren
r_break
suffix:semicolon
r_case
id|UFS_FSOSF1
suffix:colon
id|UFSD
c_func
(paren
(paren
l_string|&quot;fs is DEC OSF/1&bslash;n&quot;
)paren
)paren
r_break
suffix:semicolon
r_case
id|UFS_FSACTIVE
suffix:colon
id|printk
c_func
(paren
l_string|&quot;ufs_read_super: fs is active&bslash;n&quot;
)paren
suffix:semicolon
id|sb-&gt;s_flags
op_or_assign
id|MS_RDONLY
suffix:semicolon
r_break
suffix:semicolon
r_case
id|UFS_FSBAD
suffix:colon
id|printk
c_func
(paren
l_string|&quot;ufs_read_super: fs is bad&bslash;n&quot;
)paren
suffix:semicolon
id|sb-&gt;s_flags
op_or_assign
id|MS_RDONLY
suffix:semicolon
r_break
suffix:semicolon
r_default
suffix:colon
id|printk
c_func
(paren
l_string|&quot;ufs_read_super: can&squot;t grok fs_clean 0x%x&bslash;n&quot;
comma
id|usb1-&gt;fs_clean
)paren
suffix:semicolon
id|sb-&gt;s_flags
op_or_assign
id|MS_RDONLY
suffix:semicolon
r_break
suffix:semicolon
)brace
)brace
r_else
(brace
id|printk
c_func
(paren
l_string|&quot;ufs_read_super: fs needs fsck&bslash;n&quot;
)paren
suffix:semicolon
id|sb-&gt;s_flags
op_or_assign
id|MS_RDONLY
suffix:semicolon
)brace
multiline_comment|/*&n;&t; * Read ufs_super_block into internal data structures&n;&t; */
id|sb-&gt;s_op
op_assign
op_amp
id|ufs_super_ops
suffix:semicolon
id|sb-&gt;dq_op
op_assign
l_int|NULL
suffix:semicolon
multiline_comment|/***/
id|sb-&gt;s_magic
op_assign
id|fs32_to_cpu
c_func
(paren
id|sb
comma
id|usb3-&gt;fs_magic
)paren
suffix:semicolon
id|uspi-&gt;s_sblkno
op_assign
id|fs32_to_cpu
c_func
(paren
id|sb
comma
id|usb1-&gt;fs_sblkno
)paren
suffix:semicolon
id|uspi-&gt;s_cblkno
op_assign
id|fs32_to_cpu
c_func
(paren
id|sb
comma
id|usb1-&gt;fs_cblkno
)paren
suffix:semicolon
id|uspi-&gt;s_iblkno
op_assign
id|fs32_to_cpu
c_func
(paren
id|sb
comma
id|usb1-&gt;fs_iblkno
)paren
suffix:semicolon
id|uspi-&gt;s_dblkno
op_assign
id|fs32_to_cpu
c_func
(paren
id|sb
comma
id|usb1-&gt;fs_dblkno
)paren
suffix:semicolon
id|uspi-&gt;s_cgoffset
op_assign
id|fs32_to_cpu
c_func
(paren
id|sb
comma
id|usb1-&gt;fs_cgoffset
)paren
suffix:semicolon
id|uspi-&gt;s_cgmask
op_assign
id|fs32_to_cpu
c_func
(paren
id|sb
comma
id|usb1-&gt;fs_cgmask
)paren
suffix:semicolon
r_if
c_cond
(paren
(paren
id|flags
op_amp
id|UFS_TYPE_MASK
)paren
op_eq
id|UFS_TYPE_UFS2
)paren
(brace
id|uspi-&gt;s_u2_size
op_assign
id|fs64_to_cpu
c_func
(paren
id|sb
comma
id|usb-&gt;fs_u11.fs_u2.fs_size
)paren
suffix:semicolon
id|uspi-&gt;s_u2_dsize
op_assign
id|fs64_to_cpu
c_func
(paren
id|sb
comma
id|usb-&gt;fs_u11.fs_u2.fs_dsize
)paren
suffix:semicolon
)brace
r_else
(brace
id|uspi-&gt;s_size
op_assign
id|fs32_to_cpu
c_func
(paren
id|sb
comma
id|usb1-&gt;fs_size
)paren
suffix:semicolon
id|uspi-&gt;s_dsize
op_assign
id|fs32_to_cpu
c_func
(paren
id|sb
comma
id|usb1-&gt;fs_dsize
)paren
suffix:semicolon
)brace
id|uspi-&gt;s_ncg
op_assign
id|fs32_to_cpu
c_func
(paren
id|sb
comma
id|usb1-&gt;fs_ncg
)paren
suffix:semicolon
multiline_comment|/* s_bsize already set */
multiline_comment|/* s_fsize already set */
id|uspi-&gt;s_fpb
op_assign
id|fs32_to_cpu
c_func
(paren
id|sb
comma
id|usb1-&gt;fs_frag
)paren
suffix:semicolon
id|uspi-&gt;s_minfree
op_assign
id|fs32_to_cpu
c_func
(paren
id|sb
comma
id|usb1-&gt;fs_minfree
)paren
suffix:semicolon
id|uspi-&gt;s_bmask
op_assign
id|fs32_to_cpu
c_func
(paren
id|sb
comma
id|usb1-&gt;fs_bmask
)paren
suffix:semicolon
id|uspi-&gt;s_fmask
op_assign
id|fs32_to_cpu
c_func
(paren
id|sb
comma
id|usb1-&gt;fs_fmask
)paren
suffix:semicolon
id|uspi-&gt;s_bshift
op_assign
id|fs32_to_cpu
c_func
(paren
id|sb
comma
id|usb1-&gt;fs_bshift
)paren
suffix:semicolon
id|uspi-&gt;s_fshift
op_assign
id|fs32_to_cpu
c_func
(paren
id|sb
comma
id|usb1-&gt;fs_fshift
)paren
suffix:semicolon
id|UFSD
c_func
(paren
(paren
l_string|&quot;uspi-&gt;s_bshift = %d,uspi-&gt;s_fshift = %d&quot;
comma
id|uspi-&gt;s_bshift
comma
id|uspi-&gt;s_fshift
)paren
)paren
suffix:semicolon
id|uspi-&gt;s_fpbshift
op_assign
id|fs32_to_cpu
c_func
(paren
id|sb
comma
id|usb1-&gt;fs_fragshift
)paren
suffix:semicolon
id|uspi-&gt;s_fsbtodb
op_assign
id|fs32_to_cpu
c_func
(paren
id|sb
comma
id|usb1-&gt;fs_fsbtodb
)paren
suffix:semicolon
multiline_comment|/* s_sbsize already set */
id|uspi-&gt;s_csmask
op_assign
id|fs32_to_cpu
c_func
(paren
id|sb
comma
id|usb1-&gt;fs_csmask
)paren
suffix:semicolon
id|uspi-&gt;s_csshift
op_assign
id|fs32_to_cpu
c_func
(paren
id|sb
comma
id|usb1-&gt;fs_csshift
)paren
suffix:semicolon
id|uspi-&gt;s_nindir
op_assign
id|fs32_to_cpu
c_func
(paren
id|sb
comma
id|usb1-&gt;fs_nindir
)paren
suffix:semicolon
id|uspi-&gt;s_inopb
op_assign
id|fs32_to_cpu
c_func
(paren
id|sb
comma
id|usb1-&gt;fs_inopb
)paren
suffix:semicolon
id|uspi-&gt;s_nspf
op_assign
id|fs32_to_cpu
c_func
(paren
id|sb
comma
id|usb1-&gt;fs_nspf
)paren
suffix:semicolon
id|uspi-&gt;s_npsect
op_assign
id|ufs_get_fs_npsect
c_func
(paren
id|sb
comma
id|usb1
comma
id|usb3
)paren
suffix:semicolon
id|uspi-&gt;s_interleave
op_assign
id|fs32_to_cpu
c_func
(paren
id|sb
comma
id|usb1-&gt;fs_interleave
)paren
suffix:semicolon
id|uspi-&gt;s_trackskew
op_assign
id|fs32_to_cpu
c_func
(paren
id|sb
comma
id|usb1-&gt;fs_trackskew
)paren
suffix:semicolon
id|uspi-&gt;s_csaddr
op_assign
id|fs32_to_cpu
c_func
(paren
id|sb
comma
id|usb1-&gt;fs_csaddr
)paren
suffix:semicolon
id|uspi-&gt;s_cssize
op_assign
id|fs32_to_cpu
c_func
(paren
id|sb
comma
id|usb1-&gt;fs_cssize
)paren
suffix:semicolon
id|uspi-&gt;s_cgsize
op_assign
id|fs32_to_cpu
c_func
(paren
id|sb
comma
id|usb1-&gt;fs_cgsize
)paren
suffix:semicolon
id|uspi-&gt;s_ntrak
op_assign
id|fs32_to_cpu
c_func
(paren
id|sb
comma
id|usb1-&gt;fs_ntrak
)paren
suffix:semicolon
id|uspi-&gt;s_nsect
op_assign
id|fs32_to_cpu
c_func
(paren
id|sb
comma
id|usb1-&gt;fs_nsect
)paren
suffix:semicolon
id|uspi-&gt;s_spc
op_assign
id|fs32_to_cpu
c_func
(paren
id|sb
comma
id|usb1-&gt;fs_spc
)paren
suffix:semicolon
id|uspi-&gt;s_ipg
op_assign
id|fs32_to_cpu
c_func
(paren
id|sb
comma
id|usb1-&gt;fs_ipg
)paren
suffix:semicolon
id|uspi-&gt;s_fpg
op_assign
id|fs32_to_cpu
c_func
(paren
id|sb
comma
id|usb1-&gt;fs_fpg
)paren
suffix:semicolon
id|uspi-&gt;s_cpc
op_assign
id|fs32_to_cpu
c_func
(paren
id|sb
comma
id|usb2-&gt;fs_cpc
)paren
suffix:semicolon
id|uspi-&gt;s_contigsumsize
op_assign
id|fs32_to_cpu
c_func
(paren
id|sb
comma
id|usb3-&gt;fs_u2.fs_44.fs_contigsumsize
)paren
suffix:semicolon
id|uspi-&gt;s_qbmask
op_assign
id|ufs_get_fs_qbmask
c_func
(paren
id|sb
comma
id|usb3
)paren
suffix:semicolon
id|uspi-&gt;s_qfmask
op_assign
id|ufs_get_fs_qfmask
c_func
(paren
id|sb
comma
id|usb3
)paren
suffix:semicolon
id|uspi-&gt;s_postblformat
op_assign
id|fs32_to_cpu
c_func
(paren
id|sb
comma
id|usb3-&gt;fs_postblformat
)paren
suffix:semicolon
id|uspi-&gt;s_nrpos
op_assign
id|fs32_to_cpu
c_func
(paren
id|sb
comma
id|usb3-&gt;fs_nrpos
)paren
suffix:semicolon
id|uspi-&gt;s_postbloff
op_assign
id|fs32_to_cpu
c_func
(paren
id|sb
comma
id|usb3-&gt;fs_postbloff
)paren
suffix:semicolon
id|uspi-&gt;s_rotbloff
op_assign
id|fs32_to_cpu
c_func
(paren
id|sb
comma
id|usb3-&gt;fs_rotbloff
)paren
suffix:semicolon
multiline_comment|/*&n;&t; * Compute another frequently used values&n;&t; */
id|uspi-&gt;s_fpbmask
op_assign
id|uspi-&gt;s_fpb
op_minus
l_int|1
suffix:semicolon
r_if
c_cond
(paren
(paren
id|flags
op_amp
id|UFS_TYPE_MASK
)paren
op_eq
id|UFS_TYPE_UFS2
)paren
(brace
id|uspi-&gt;s_apbshift
op_assign
id|uspi-&gt;s_bshift
op_minus
l_int|3
suffix:semicolon
)brace
r_else
(brace
id|uspi-&gt;s_apbshift
op_assign
id|uspi-&gt;s_bshift
op_minus
l_int|2
suffix:semicolon
)brace
id|uspi-&gt;s_2apbshift
op_assign
id|uspi-&gt;s_apbshift
op_star
l_int|2
suffix:semicolon
id|uspi-&gt;s_3apbshift
op_assign
id|uspi-&gt;s_apbshift
op_star
l_int|3
suffix:semicolon
id|uspi-&gt;s_apb
op_assign
l_int|1
op_lshift
id|uspi-&gt;s_apbshift
suffix:semicolon
id|uspi-&gt;s_2apb
op_assign
l_int|1
op_lshift
id|uspi-&gt;s_2apbshift
suffix:semicolon
id|uspi-&gt;s_3apb
op_assign
l_int|1
op_lshift
id|uspi-&gt;s_3apbshift
suffix:semicolon
id|uspi-&gt;s_apbmask
op_assign
id|uspi-&gt;s_apb
op_minus
l_int|1
suffix:semicolon
id|uspi-&gt;s_nspfshift
op_assign
id|uspi-&gt;s_fshift
op_minus
id|UFS_SECTOR_BITS
suffix:semicolon
id|uspi-&gt;s_nspb
op_assign
id|uspi-&gt;s_nspf
op_lshift
id|uspi-&gt;s_fpbshift
suffix:semicolon
id|uspi-&gt;s_inopf
op_assign
id|uspi-&gt;s_inopb
op_rshift
id|uspi-&gt;s_fpbshift
suffix:semicolon
id|uspi-&gt;s_bpf
op_assign
id|uspi-&gt;s_fsize
op_lshift
l_int|3
suffix:semicolon
id|uspi-&gt;s_bpfshift
op_assign
id|uspi-&gt;s_fshift
op_plus
l_int|3
suffix:semicolon
id|uspi-&gt;s_bpfmask
op_assign
id|uspi-&gt;s_bpf
op_minus
l_int|1
suffix:semicolon
r_if
c_cond
(paren
(paren
id|sbi-&gt;s_mount_opt
op_amp
id|UFS_MOUNT_UFSTYPE
)paren
op_eq
id|UFS_MOUNT_UFSTYPE_44BSD
)paren
id|uspi-&gt;s_maxsymlinklen
op_assign
id|fs32_to_cpu
c_func
(paren
id|sb
comma
id|usb3-&gt;fs_u2.fs_44.fs_maxsymlinklen
)paren
suffix:semicolon
id|sbi-&gt;s_flags
op_assign
id|flags
suffix:semicolon
id|inode
op_assign
id|iget
c_func
(paren
id|sb
comma
id|UFS_ROOTINO
)paren
suffix:semicolon
r_if
c_cond
(paren
op_logical_neg
id|inode
op_logical_or
id|is_bad_inode
c_func
(paren
id|inode
)paren
)paren
r_goto
id|failed
suffix:semicolon
id|sb-&gt;s_root
op_assign
id|d_alloc_root
c_func
(paren
id|inode
)paren
suffix:semicolon
r_if
c_cond
(paren
op_logical_neg
id|sb-&gt;s_root
)paren
r_goto
id|dalloc_failed
suffix:semicolon
multiline_comment|/*&n;&t; * Read cylinder group structures&n;&t; */
r_if
c_cond
(paren
op_logical_neg
(paren
id|sb-&gt;s_flags
op_amp
id|MS_RDONLY
)paren
)paren
r_if
c_cond
(paren
op_logical_neg
id|ufs_read_cylinder_structures
c_func
(paren
id|sb
)paren
)paren
r_goto
id|failed
suffix:semicolon
id|UFSD
c_func
(paren
(paren
l_string|&quot;EXIT&bslash;n&quot;
)paren
)paren
r_return
l_int|0
suffix:semicolon
id|dalloc_failed
suffix:colon
id|iput
c_func
(paren
id|inode
)paren
suffix:semicolon
id|failed
suffix:colon
r_if
c_cond
(paren
id|ubh
)paren
id|ubh_brelse_uspi
(paren
id|uspi
)paren
suffix:semicolon
r_if
c_cond
(paren
id|uspi
)paren
id|kfree
(paren
id|uspi
)paren
suffix:semicolon
r_if
c_cond
(paren
id|sbi
)paren
id|kfree
c_func
(paren
id|sbi
)paren
suffix:semicolon
id|sb-&gt;s_fs_info
op_assign
l_int|NULL
suffix:semicolon
id|UFSD
c_func
(paren
(paren
l_string|&quot;EXIT (FAILED)&bslash;n&quot;
)paren
)paren
r_return
op_minus
id|EINVAL
suffix:semicolon
id|failed_nomem
suffix:colon
id|UFSD
c_func
(paren
(paren
l_string|&quot;EXIT (NOMEM)&bslash;n&quot;
)paren
)paren
r_return
op_minus
id|ENOMEM
suffix:semicolon
)brace
DECL|function|ufs_write_super
r_static
r_void
id|ufs_write_super
(paren
r_struct
id|super_block
op_star
id|sb
)paren
(brace
r_struct
id|ufs_sb_private_info
op_star
id|uspi
suffix:semicolon
r_struct
id|ufs_super_block_first
op_star
id|usb1
suffix:semicolon
r_struct
id|ufs_super_block_third
op_star
id|usb3
suffix:semicolon
r_int
id|flags
suffix:semicolon
id|lock_kernel
c_func
(paren
)paren
suffix:semicolon
id|UFSD
c_func
(paren
(paren
l_string|&quot;ENTER&bslash;n&quot;
)paren
)paren
id|flags
op_assign
id|UFS_SB
c_func
(paren
id|sb
)paren
op_member_access_from_pointer
id|s_flags
suffix:semicolon
id|uspi
op_assign
id|UFS_SB
c_func
(paren
id|sb
)paren
op_member_access_from_pointer
id|s_uspi
suffix:semicolon
id|usb1
op_assign
id|ubh_get_usb_first
c_func
(paren
id|USPI_UBH
)paren
suffix:semicolon
id|usb3
op_assign
id|ubh_get_usb_third
c_func
(paren
id|USPI_UBH
)paren
suffix:semicolon
r_if
c_cond
(paren
op_logical_neg
(paren
id|sb-&gt;s_flags
op_amp
id|MS_RDONLY
)paren
)paren
(brace
id|usb1-&gt;fs_time
op_assign
id|cpu_to_fs32
c_func
(paren
id|sb
comma
id|get_seconds
c_func
(paren
)paren
)paren
suffix:semicolon
r_if
c_cond
(paren
(paren
id|flags
op_amp
id|UFS_ST_MASK
)paren
op_eq
id|UFS_ST_SUN
op_logical_or
(paren
id|flags
op_amp
id|UFS_ST_MASK
)paren
op_eq
id|UFS_ST_SUNx86
)paren
id|ufs_set_fs_state
c_func
(paren
id|sb
comma
id|usb1
comma
id|usb3
comma
id|UFS_FSOK
op_minus
id|fs32_to_cpu
c_func
(paren
id|sb
comma
id|usb1-&gt;fs_time
)paren
)paren
suffix:semicolon
id|ubh_mark_buffer_dirty
(paren
id|USPI_UBH
)paren
suffix:semicolon
)brace
id|sb-&gt;s_dirt
op_assign
l_int|0
suffix:semicolon
id|UFSD
c_func
(paren
(paren
l_string|&quot;EXIT&bslash;n&quot;
)paren
)paren
id|unlock_kernel
c_func
(paren
)paren
suffix:semicolon
)brace
DECL|function|ufs_put_super
r_static
r_void
id|ufs_put_super
(paren
r_struct
id|super_block
op_star
id|sb
)paren
(brace
r_struct
id|ufs_sb_info
op_star
id|sbi
op_assign
id|UFS_SB
c_func
(paren
id|sb
)paren
suffix:semicolon
id|UFSD
c_func
(paren
(paren
l_string|&quot;ENTER&bslash;n&quot;
)paren
)paren
r_if
c_cond
(paren
op_logical_neg
(paren
id|sb-&gt;s_flags
op_amp
id|MS_RDONLY
)paren
)paren
id|ufs_put_cylinder_structures
(paren
id|sb
)paren
suffix:semicolon
id|ubh_brelse_uspi
(paren
id|sbi-&gt;s_uspi
)paren
suffix:semicolon
id|kfree
(paren
id|sbi-&gt;s_uspi
)paren
suffix:semicolon
id|kfree
(paren
id|sbi
)paren
suffix:semicolon
id|sb-&gt;s_fs_info
op_assign
l_int|NULL
suffix:semicolon
r_return
suffix:semicolon
)brace
DECL|function|ufs_remount
r_static
r_int
id|ufs_remount
(paren
r_struct
id|super_block
op_star
id|sb
comma
r_int
op_star
id|mount_flags
comma
r_char
op_star
id|data
)paren
(brace
r_struct
id|ufs_sb_private_info
op_star
id|uspi
suffix:semicolon
r_struct
id|ufs_super_block_first
op_star
id|usb1
suffix:semicolon
r_struct
id|ufs_super_block_third
op_star
id|usb3
suffix:semicolon
r_int
id|new_mount_opt
comma
id|ufstype
suffix:semicolon
r_int
id|flags
suffix:semicolon
id|uspi
op_assign
id|UFS_SB
c_func
(paren
id|sb
)paren
op_member_access_from_pointer
id|s_uspi
suffix:semicolon
id|flags
op_assign
id|UFS_SB
c_func
(paren
id|sb
)paren
op_member_access_from_pointer
id|s_flags
suffix:semicolon
id|usb1
op_assign
id|ubh_get_usb_first
c_func
(paren
id|USPI_UBH
)paren
suffix:semicolon
id|usb3
op_assign
id|ubh_get_usb_third
c_func
(paren
id|USPI_UBH
)paren
suffix:semicolon
multiline_comment|/*&n;&t; * Allow the &quot;check&quot; option to be passed as a remount option.&n;&t; * It is not possible to change ufstype option during remount&n;&t; */
id|ufstype
op_assign
id|UFS_SB
c_func
(paren
id|sb
)paren
op_member_access_from_pointer
id|s_mount_opt
op_amp
id|UFS_MOUNT_UFSTYPE
suffix:semicolon
id|new_mount_opt
op_assign
l_int|0
suffix:semicolon
id|ufs_set_opt
(paren
id|new_mount_opt
comma
id|ONERROR_LOCK
)paren
suffix:semicolon
r_if
c_cond
(paren
op_logical_neg
id|ufs_parse_options
(paren
id|data
comma
op_amp
id|new_mount_opt
)paren
)paren
r_return
op_minus
id|EINVAL
suffix:semicolon
r_if
c_cond
(paren
op_logical_neg
(paren
id|new_mount_opt
op_amp
id|UFS_MOUNT_UFSTYPE
)paren
)paren
(brace
id|new_mount_opt
op_or_assign
id|ufstype
suffix:semicolon
)brace
r_else
r_if
c_cond
(paren
(paren
id|new_mount_opt
op_amp
id|UFS_MOUNT_UFSTYPE
)paren
op_ne
id|ufstype
)paren
(brace
id|printk
c_func
(paren
l_string|&quot;ufstype can&squot;t be changed during remount&bslash;n&quot;
)paren
suffix:semicolon
r_return
op_minus
id|EINVAL
suffix:semicolon
)brace
r_if
c_cond
(paren
(paren
op_star
id|mount_flags
op_amp
id|MS_RDONLY
)paren
op_eq
(paren
id|sb-&gt;s_flags
op_amp
id|MS_RDONLY
)paren
)paren
(brace
id|UFS_SB
c_func
(paren
id|sb
)paren
op_member_access_from_pointer
id|s_mount_opt
op_assign
id|new_mount_opt
suffix:semicolon
r_return
l_int|0
suffix:semicolon
)brace
multiline_comment|/*&n;&t; * fs was mouted as rw, remounting ro&n;&t; */
r_if
c_cond
(paren
op_star
id|mount_flags
op_amp
id|MS_RDONLY
)paren
(brace
id|ufs_put_cylinder_structures
c_func
(paren
id|sb
)paren
suffix:semicolon
id|usb1-&gt;fs_time
op_assign
id|cpu_to_fs32
c_func
(paren
id|sb
comma
id|get_seconds
c_func
(paren
)paren
)paren
suffix:semicolon
r_if
c_cond
(paren
(paren
id|flags
op_amp
id|UFS_ST_MASK
)paren
op_eq
id|UFS_ST_SUN
op_logical_or
(paren
id|flags
op_amp
id|UFS_ST_MASK
)paren
op_eq
id|UFS_ST_SUNx86
)paren
id|ufs_set_fs_state
c_func
(paren
id|sb
comma
id|usb1
comma
id|usb3
comma
id|UFS_FSOK
op_minus
id|fs32_to_cpu
c_func
(paren
id|sb
comma
id|usb1-&gt;fs_time
)paren
)paren
suffix:semicolon
id|ubh_mark_buffer_dirty
(paren
id|USPI_UBH
)paren
suffix:semicolon
id|sb-&gt;s_dirt
op_assign
l_int|0
suffix:semicolon
id|sb-&gt;s_flags
op_or_assign
id|MS_RDONLY
suffix:semicolon
)brace
multiline_comment|/*&n;&t; * fs was mounted as ro, remounting rw&n;&t; */
r_else
(brace
macro_line|#ifndef CONFIG_UFS_FS_WRITE
id|printk
c_func
(paren
l_string|&quot;ufs was compiled with read-only support, &quot;
l_string|&quot;can&squot;t be mounted as read-write&bslash;n&quot;
)paren
suffix:semicolon
r_return
op_minus
id|EINVAL
suffix:semicolon
macro_line|#else
r_if
c_cond
(paren
id|ufstype
op_ne
id|UFS_MOUNT_UFSTYPE_SUN
op_logical_and
id|ufstype
op_ne
id|UFS_MOUNT_UFSTYPE_44BSD
op_logical_and
id|ufstype
op_ne
id|UFS_MOUNT_UFSTYPE_SUNx86
)paren
(brace
id|printk
c_func
(paren
l_string|&quot;this ufstype is read-only supported&bslash;n&quot;
)paren
suffix:semicolon
r_return
op_minus
id|EINVAL
suffix:semicolon
)brace
r_if
c_cond
(paren
op_logical_neg
id|ufs_read_cylinder_structures
(paren
id|sb
)paren
)paren
(brace
id|printk
c_func
(paren
l_string|&quot;failed during remounting&bslash;n&quot;
)paren
suffix:semicolon
r_return
op_minus
id|EPERM
suffix:semicolon
)brace
id|sb-&gt;s_flags
op_and_assign
op_complement
id|MS_RDONLY
suffix:semicolon
macro_line|#endif
)brace
id|UFS_SB
c_func
(paren
id|sb
)paren
op_member_access_from_pointer
id|s_mount_opt
op_assign
id|new_mount_opt
suffix:semicolon
r_return
l_int|0
suffix:semicolon
)brace
DECL|function|ufs_statfs
r_static
r_int
id|ufs_statfs
(paren
r_struct
id|super_block
op_star
id|sb
comma
r_struct
id|kstatfs
op_star
id|buf
)paren
(brace
r_struct
id|ufs_sb_private_info
op_star
id|uspi
suffix:semicolon
r_struct
id|ufs_super_block_first
op_star
id|usb1
suffix:semicolon
r_struct
id|ufs_super_block
op_star
id|usb
suffix:semicolon
r_int
id|flags
op_assign
l_int|0
suffix:semicolon
id|lock_kernel
c_func
(paren
)paren
suffix:semicolon
id|uspi
op_assign
id|UFS_SB
c_func
(paren
id|sb
)paren
op_member_access_from_pointer
id|s_uspi
suffix:semicolon
id|usb1
op_assign
id|ubh_get_usb_first
(paren
id|USPI_UBH
)paren
suffix:semicolon
id|usb
op_assign
(paren
r_struct
id|ufs_super_block
op_star
)paren
(paren
(paren
r_struct
id|ufs_buffer_head
op_star
)paren
id|uspi
)paren
op_member_access_from_pointer
id|bh
(braket
l_int|0
)braket
op_member_access_from_pointer
id|b_data
suffix:semicolon
id|flags
op_assign
id|UFS_SB
c_func
(paren
id|sb
)paren
op_member_access_from_pointer
id|s_flags
suffix:semicolon
r_if
c_cond
(paren
(paren
id|flags
op_amp
id|UFS_TYPE_MASK
)paren
op_eq
id|UFS_TYPE_UFS2
)paren
(brace
id|buf-&gt;f_type
op_assign
id|UFS2_MAGIC
suffix:semicolon
id|buf-&gt;f_blocks
op_assign
id|fs64_to_cpu
c_func
(paren
id|sb
comma
id|usb-&gt;fs_u11.fs_u2.fs_dsize
)paren
suffix:semicolon
id|buf-&gt;f_bfree
op_assign
id|ufs_blkstofrags
c_func
(paren
id|fs64_to_cpu
c_func
(paren
id|sb
comma
id|usb-&gt;fs_u11.fs_u2.fs_cstotal.cs_nbfree
)paren
)paren
op_plus
id|fs64_to_cpu
c_func
(paren
id|sb
comma
id|usb-&gt;fs_u11.fs_u2.fs_cstotal.cs_nffree
)paren
suffix:semicolon
id|buf-&gt;f_ffree
op_assign
id|fs64_to_cpu
c_func
(paren
id|sb
comma
id|usb-&gt;fs_u11.fs_u2.fs_cstotal.cs_nifree
)paren
suffix:semicolon
)brace
r_else
(brace
id|buf-&gt;f_type
op_assign
id|UFS_MAGIC
suffix:semicolon
id|buf-&gt;f_blocks
op_assign
id|uspi-&gt;s_dsize
suffix:semicolon
id|buf-&gt;f_bfree
op_assign
id|ufs_blkstofrags
c_func
(paren
id|fs32_to_cpu
c_func
(paren
id|sb
comma
id|usb1-&gt;fs_cstotal.cs_nbfree
)paren
)paren
op_plus
id|fs32_to_cpu
c_func
(paren
id|sb
comma
id|usb1-&gt;fs_cstotal.cs_nffree
)paren
suffix:semicolon
id|buf-&gt;f_ffree
op_assign
id|fs32_to_cpu
c_func
(paren
id|sb
comma
id|usb1-&gt;fs_cstotal.cs_nifree
)paren
suffix:semicolon
)brace
id|buf-&gt;f_bsize
op_assign
id|sb-&gt;s_blocksize
suffix:semicolon
id|buf-&gt;f_bavail
op_assign
(paren
id|buf-&gt;f_bfree
OG
(paren
(paren
(paren
r_int
)paren
id|buf-&gt;f_blocks
op_div
l_int|100
)paren
op_star
id|uspi-&gt;s_minfree
)paren
)paren
ques
c_cond
(paren
id|buf-&gt;f_bfree
op_minus
(paren
(paren
(paren
r_int
)paren
id|buf-&gt;f_blocks
op_div
l_int|100
)paren
op_star
id|uspi-&gt;s_minfree
)paren
)paren
suffix:colon
l_int|0
suffix:semicolon
id|buf-&gt;f_files
op_assign
id|uspi-&gt;s_ncg
op_star
id|uspi-&gt;s_ipg
suffix:semicolon
id|buf-&gt;f_namelen
op_assign
id|UFS_MAXNAMLEN
suffix:semicolon
id|unlock_kernel
c_func
(paren
)paren
suffix:semicolon
r_return
l_int|0
suffix:semicolon
)brace
DECL|variable|ufs_inode_cachep
r_static
id|kmem_cache_t
op_star
id|ufs_inode_cachep
suffix:semicolon
DECL|function|ufs_alloc_inode
r_static
r_struct
id|inode
op_star
id|ufs_alloc_inode
c_func
(paren
r_struct
id|super_block
op_star
id|sb
)paren
(brace
r_struct
id|ufs_inode_info
op_star
id|ei
suffix:semicolon
id|ei
op_assign
(paren
r_struct
id|ufs_inode_info
op_star
)paren
id|kmem_cache_alloc
c_func
(paren
id|ufs_inode_cachep
comma
id|SLAB_KERNEL
)paren
suffix:semicolon
r_if
c_cond
(paren
op_logical_neg
id|ei
)paren
r_return
l_int|NULL
suffix:semicolon
id|ei-&gt;vfs_inode.i_version
op_assign
l_int|1
suffix:semicolon
r_return
op_amp
id|ei-&gt;vfs_inode
suffix:semicolon
)brace
DECL|function|ufs_destroy_inode
r_static
r_void
id|ufs_destroy_inode
c_func
(paren
r_struct
id|inode
op_star
id|inode
)paren
(brace
id|kmem_cache_free
c_func
(paren
id|ufs_inode_cachep
comma
id|UFS_I
c_func
(paren
id|inode
)paren
)paren
suffix:semicolon
)brace
DECL|function|init_once
r_static
r_void
id|init_once
c_func
(paren
r_void
op_star
id|foo
comma
id|kmem_cache_t
op_star
id|cachep
comma
r_int
r_int
id|flags
)paren
(brace
r_struct
id|ufs_inode_info
op_star
id|ei
op_assign
(paren
r_struct
id|ufs_inode_info
op_star
)paren
id|foo
suffix:semicolon
r_if
c_cond
(paren
(paren
id|flags
op_amp
(paren
id|SLAB_CTOR_VERIFY
op_or
id|SLAB_CTOR_CONSTRUCTOR
)paren
)paren
op_eq
id|SLAB_CTOR_CONSTRUCTOR
)paren
id|inode_init_once
c_func
(paren
op_amp
id|ei-&gt;vfs_inode
)paren
suffix:semicolon
)brace
DECL|function|init_inodecache
r_static
r_int
id|init_inodecache
c_func
(paren
r_void
)paren
(brace
id|ufs_inode_cachep
op_assign
id|kmem_cache_create
c_func
(paren
l_string|&quot;ufs_inode_cache&quot;
comma
r_sizeof
(paren
r_struct
id|ufs_inode_info
)paren
comma
l_int|0
comma
id|SLAB_RECLAIM_ACCOUNT
comma
id|init_once
comma
l_int|NULL
)paren
suffix:semicolon
r_if
c_cond
(paren
id|ufs_inode_cachep
op_eq
l_int|NULL
)paren
r_return
op_minus
id|ENOMEM
suffix:semicolon
r_return
l_int|0
suffix:semicolon
)brace
DECL|function|destroy_inodecache
r_static
r_void
id|destroy_inodecache
c_func
(paren
r_void
)paren
(brace
r_if
c_cond
(paren
id|kmem_cache_destroy
c_func
(paren
id|ufs_inode_cachep
)paren
)paren
id|printk
c_func
(paren
id|KERN_INFO
l_string|&quot;ufs_inode_cache: not all structures were freed&bslash;n&quot;
)paren
suffix:semicolon
)brace
macro_line|#ifdef CONFIG_QUOTA
r_static
id|ssize_t
id|ufs_quota_read
c_func
(paren
r_struct
id|super_block
op_star
comma
r_int
comma
r_char
op_star
comma
r_int
comma
id|loff_t
)paren
suffix:semicolon
r_static
id|ssize_t
id|ufs_quota_write
c_func
(paren
r_struct
id|super_block
op_star
comma
r_int
comma
r_const
r_char
op_star
comma
r_int
comma
id|loff_t
)paren
suffix:semicolon
macro_line|#endif
DECL|variable|ufs_super_ops
r_static
r_struct
id|super_operations
id|ufs_super_ops
op_assign
(brace
dot
id|alloc_inode
op_assign
id|ufs_alloc_inode
comma
dot
id|destroy_inode
op_assign
id|ufs_destroy_inode
comma
dot
id|read_inode
op_assign
id|ufs_read_inode
comma
dot
id|write_inode
op_assign
id|ufs_write_inode
comma
dot
id|delete_inode
op_assign
id|ufs_delete_inode
comma
dot
id|put_super
op_assign
id|ufs_put_super
comma
dot
id|write_super
op_assign
id|ufs_write_super
comma
dot
id|statfs
op_assign
id|ufs_statfs
comma
dot
id|remount_fs
op_assign
id|ufs_remount
comma
macro_line|#ifdef CONFIG_QUOTA
dot
id|quota_read
op_assign
id|ufs_quota_read
comma
dot
id|quota_write
op_assign
id|ufs_quota_write
comma
macro_line|#endif
)brace
suffix:semicolon
macro_line|#ifdef CONFIG_QUOTA
multiline_comment|/* Read data from quotafile - avoid pagecache and such because we cannot afford&n; * acquiring the locks... As quota files are never truncated and quota code&n; * itself serializes the operations (and noone else should touch the files)&n; * we don&squot;t have to be afraid of races */
DECL|function|ufs_quota_read
r_static
id|ssize_t
id|ufs_quota_read
c_func
(paren
r_struct
id|super_block
op_star
id|sb
comma
r_int
id|type
comma
r_char
op_star
id|data
comma
r_int
id|len
comma
id|loff_t
id|off
)paren
(brace
r_struct
id|inode
op_star
id|inode
op_assign
id|sb_dqopt
c_func
(paren
id|sb
)paren
op_member_access_from_pointer
id|files
(braket
id|type
)braket
suffix:semicolon
id|sector_t
id|blk
op_assign
id|off
op_rshift
id|sb-&gt;s_blocksize_bits
suffix:semicolon
r_int
id|err
op_assign
l_int|0
suffix:semicolon
r_int
id|offset
op_assign
id|off
op_amp
(paren
id|sb-&gt;s_blocksize
op_minus
l_int|1
)paren
suffix:semicolon
r_int
id|tocopy
suffix:semicolon
r_int
id|toread
suffix:semicolon
r_struct
id|buffer_head
op_star
id|bh
suffix:semicolon
id|loff_t
id|i_size
op_assign
id|i_size_read
c_func
(paren
id|inode
)paren
suffix:semicolon
r_if
c_cond
(paren
id|off
OG
id|i_size
)paren
r_return
l_int|0
suffix:semicolon
r_if
c_cond
(paren
id|off
op_plus
id|len
OG
id|i_size
)paren
id|len
op_assign
id|i_size
op_minus
id|off
suffix:semicolon
id|toread
op_assign
id|len
suffix:semicolon
r_while
c_loop
(paren
id|toread
OG
l_int|0
)paren
(brace
id|tocopy
op_assign
id|sb-&gt;s_blocksize
op_minus
id|offset
OL
id|toread
ques
c_cond
id|sb-&gt;s_blocksize
op_minus
id|offset
suffix:colon
id|toread
suffix:semicolon
id|bh
op_assign
id|ufs_bread
c_func
(paren
id|inode
comma
id|blk
comma
l_int|0
comma
op_amp
id|err
)paren
suffix:semicolon
r_if
c_cond
(paren
id|err
)paren
r_return
id|err
suffix:semicolon
r_if
c_cond
(paren
op_logical_neg
id|bh
)paren
multiline_comment|/* A hole? */
id|memset
c_func
(paren
id|data
comma
l_int|0
comma
id|tocopy
)paren
suffix:semicolon
r_else
(brace
id|memcpy
c_func
(paren
id|data
comma
id|bh-&gt;b_data
op_plus
id|offset
comma
id|tocopy
)paren
suffix:semicolon
id|brelse
c_func
(paren
id|bh
)paren
suffix:semicolon
)brace
id|offset
op_assign
l_int|0
suffix:semicolon
id|toread
op_sub_assign
id|tocopy
suffix:semicolon
id|data
op_add_assign
id|tocopy
suffix:semicolon
id|blk
op_increment
suffix:semicolon
)brace
r_return
id|len
suffix:semicolon
)brace
multiline_comment|/* Write to quotafile */
DECL|function|ufs_quota_write
r_static
id|ssize_t
id|ufs_quota_write
c_func
(paren
r_struct
id|super_block
op_star
id|sb
comma
r_int
id|type
comma
r_const
r_char
op_star
id|data
comma
r_int
id|len
comma
id|loff_t
id|off
)paren
(brace
r_struct
id|inode
op_star
id|inode
op_assign
id|sb_dqopt
c_func
(paren
id|sb
)paren
op_member_access_from_pointer
id|files
(braket
id|type
)braket
suffix:semicolon
id|sector_t
id|blk
op_assign
id|off
op_rshift
id|sb-&gt;s_blocksize_bits
suffix:semicolon
r_int
id|err
op_assign
l_int|0
suffix:semicolon
r_int
id|offset
op_assign
id|off
op_amp
(paren
id|sb-&gt;s_blocksize
op_minus
l_int|1
)paren
suffix:semicolon
r_int
id|tocopy
suffix:semicolon
r_int
id|towrite
op_assign
id|len
suffix:semicolon
r_struct
id|buffer_head
op_star
id|bh
suffix:semicolon
id|down
c_func
(paren
op_amp
id|inode-&gt;i_sem
)paren
suffix:semicolon
r_while
c_loop
(paren
id|towrite
OG
l_int|0
)paren
(brace
id|tocopy
op_assign
id|sb-&gt;s_blocksize
op_minus
id|offset
OL
id|towrite
ques
c_cond
id|sb-&gt;s_blocksize
op_minus
id|offset
suffix:colon
id|towrite
suffix:semicolon
id|bh
op_assign
id|ufs_bread
c_func
(paren
id|inode
comma
id|blk
comma
l_int|1
comma
op_amp
id|err
)paren
suffix:semicolon
r_if
c_cond
(paren
op_logical_neg
id|bh
)paren
r_goto
id|out
suffix:semicolon
id|lock_buffer
c_func
(paren
id|bh
)paren
suffix:semicolon
id|memcpy
c_func
(paren
id|bh-&gt;b_data
op_plus
id|offset
comma
id|data
comma
id|tocopy
)paren
suffix:semicolon
id|flush_dcache_page
c_func
(paren
id|bh-&gt;b_page
)paren
suffix:semicolon
id|set_buffer_uptodate
c_func
(paren
id|bh
)paren
suffix:semicolon
id|mark_buffer_dirty
c_func
(paren
id|bh
)paren
suffix:semicolon
id|unlock_buffer
c_func
(paren
id|bh
)paren
suffix:semicolon
id|brelse
c_func
(paren
id|bh
)paren
suffix:semicolon
id|offset
op_assign
l_int|0
suffix:semicolon
id|towrite
op_sub_assign
id|tocopy
suffix:semicolon
id|data
op_add_assign
id|tocopy
suffix:semicolon
id|blk
op_increment
suffix:semicolon
)brace
id|out
suffix:colon
r_if
c_cond
(paren
id|len
op_eq
id|towrite
)paren
r_return
id|err
suffix:semicolon
r_if
c_cond
(paren
id|inode-&gt;i_size
OL
id|off
op_plus
id|len
op_minus
id|towrite
)paren
id|i_size_write
c_func
(paren
id|inode
comma
id|off
op_plus
id|len
op_minus
id|towrite
)paren
suffix:semicolon
id|inode-&gt;i_version
op_increment
suffix:semicolon
id|inode-&gt;i_mtime
op_assign
id|inode-&gt;i_ctime
op_assign
id|CURRENT_TIME_SEC
suffix:semicolon
id|mark_inode_dirty
c_func
(paren
id|inode
)paren
suffix:semicolon
id|up
c_func
(paren
op_amp
id|inode-&gt;i_sem
)paren
suffix:semicolon
r_return
id|len
op_minus
id|towrite
suffix:semicolon
)brace
macro_line|#endif
DECL|function|ufs_get_sb
r_static
r_struct
id|super_block
op_star
id|ufs_get_sb
c_func
(paren
r_struct
id|file_system_type
op_star
id|fs_type
comma
r_int
id|flags
comma
r_const
r_char
op_star
id|dev_name
comma
r_void
op_star
id|data
)paren
(brace
r_return
id|get_sb_bdev
c_func
(paren
id|fs_type
comma
id|flags
comma
id|dev_name
comma
id|data
comma
id|ufs_fill_super
)paren
suffix:semicolon
)brace
DECL|variable|ufs_fs_type
r_static
r_struct
id|file_system_type
id|ufs_fs_type
op_assign
(brace
dot
id|owner
op_assign
id|THIS_MODULE
comma
dot
id|name
op_assign
l_string|&quot;ufs&quot;
comma
dot
id|get_sb
op_assign
id|ufs_get_sb
comma
dot
id|kill_sb
op_assign
id|kill_block_super
comma
dot
id|fs_flags
op_assign
id|FS_REQUIRES_DEV
comma
)brace
suffix:semicolon
DECL|function|init_ufs_fs
r_static
r_int
id|__init
id|init_ufs_fs
c_func
(paren
r_void
)paren
(brace
r_int
id|err
op_assign
id|init_inodecache
c_func
(paren
)paren
suffix:semicolon
r_if
c_cond
(paren
id|err
)paren
r_goto
id|out1
suffix:semicolon
id|err
op_assign
id|register_filesystem
c_func
(paren
op_amp
id|ufs_fs_type
)paren
suffix:semicolon
r_if
c_cond
(paren
id|err
)paren
r_goto
id|out
suffix:semicolon
r_return
l_int|0
suffix:semicolon
id|out
suffix:colon
id|destroy_inodecache
c_func
(paren
)paren
suffix:semicolon
id|out1
suffix:colon
r_return
id|err
suffix:semicolon
)brace
DECL|function|exit_ufs_fs
r_static
r_void
id|__exit
id|exit_ufs_fs
c_func
(paren
r_void
)paren
(brace
id|unregister_filesystem
c_func
(paren
op_amp
id|ufs_fs_type
)paren
suffix:semicolon
id|destroy_inodecache
c_func
(paren
)paren
suffix:semicolon
)brace
id|module_init
c_func
(paren
id|init_ufs_fs
)paren
id|module_exit
c_func
(paren
id|exit_ufs_fs
)paren
id|MODULE_LICENSE
c_func
(paren
l_string|&quot;GPL&quot;
)paren
suffix:semicolon
eof
