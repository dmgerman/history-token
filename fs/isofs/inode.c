...
suffix:semicolon
DECL|variable|isofs_inode_cachep
r_static
id|kmem_cache_t
op_star
id|isofs_inode_cachep
suffix:semicolon
DECL|function|isofs_alloc_inode
r_static
r_struct
id|inode
op_star
id|isofs_alloc_inode
c_func
(paren
r_struct
id|super_block
...
DECL|function|isofs_destroy_inode
r_static
r_void
id|isofs_destroy_inode
c_func
(paren
r_struct
id|inode
...
(paren
id|KERN_INFO
l_string|&quot;iso_inode_cache: not all structures were freed&bslash;n&quot;
)paren
suffix:semicolon
)brace
DECL|function|isofs_remount
r_static
r_int
id|isofs_remount
c_func
...
(brace
multiline_comment|/* we probably want a lot more here */
op_star
id|flags
op_or_assign
id|MS_RDONLY
...
DECL|enumerator|Opt_map_a
DECL|enumerator|Opt_map_n
DECL|enumerator|Opt_map_o
DECL|enumerator|Opt_mode
DECL|enumerator|Opt_nojoliet
id|Opt_iocharset
comma
id|Opt_map_a
comma
...
id|Opt_nojoliet
comma
DECL|enumerator|Opt_norock
DECL|enumerator|Opt_sb
DECL|enumerator|Opt_session
DECL|enumerator|Opt_uid
DECL|enumerator|Opt_unhide
DECL|enumerator|Opt_utf8
...
id|Opt_err
comma
DECL|enumerator|Opt_nocompress
id|Opt_nocompress
comma
)brace
suffix:semicolon
...
(paren
id|KERN_DEBUG
l_string|&quot;First datazone:%ld&bslash;n&quot;
comma
id|sbi-&gt;s_firstdatazone
)paren
suffix:semicolon
...
multiline_comment|/*&n; * Get a set of blocks; filling in buffer_heads if already allocated&n; * or getblk() if they are not.  Returns the number of blocks inserted&n; * (0 == error.)&n; */
DECL|function|isofs_get_blocks
r_int
id|isofs_get_blocks
c_func
(paren
...
id|firstext
op_assign
id|ei-&gt;i_first_extent
suffix:semicolon
id|sect_size
op_assign
id|ei-&gt;i_section_size
op_rshift
...
id|nextblk
op_assign
id|ei-&gt;i_next_section_block
suffix:semicolon
id|nextoff
op_assign
id|ei-&gt;i_next_section_offset
suffix:semicolon
id|section
op_assign
l_int|0
...
(brace
multiline_comment|/* If we are *way* beyond the end of the file, print a message.&n;&t;&t; * Access beyond the end of the file up to the next page boundary&n;&t;&t; * is normal, however because of the way the page cache works.&n;&t;&t; * In this case, we just return 0 so that we can properly fill&n;&t;&t; * the page with useless information without generating any&n;&t;&t; * I/O errors.&n;&t;&t; */
r_if
c_cond
(paren
id|b_off
...
c_func
(paren
l_string|&quot;isofs_get_blocks: block &gt;= EOF (%ld, %ld)&bslash;n&quot;
comma
id|iblock
comma
(paren
...
c_func
(paren
l_string|&quot;isofs_get_blocks: More than 100 file sections ?!?, aborting...&bslash;n&quot;
)paren
suffix:semicolon
id|printk
c_func
(paren
l_string|&quot;isofs_get_blocks: block=%ld firstext=%u sect_size=%u &quot;
l_string|&quot;nextblk=%lu nextoff=%lu&bslash;n&quot;
comma
id|iblock
comma
id|firstext
...
op_increment
suffix:semicolon
multiline_comment|/* Next buffer head */
id|b_off
op_increment
suffix:semicolon
multiline_comment|/* Next buffer offset */
id|nblocks
op_decrement
suffix:semicolon
id|rv
...
multiline_comment|/*&n; * Used by the standard interfaces.&n; */
...
l_int|0
suffix:semicolon
multiline_comment|/* The first 16 blocks are reserved as the System Area.  Thus,&n;&t; * no inodes can appear in block 0.  We use this to flag that&n;&t; * this is the last section. */
id|ei-&gt;i_next_section_block
op_assign
l_int|0
suffix:semicolon
...
l_string|&quot;isofs_read_level3_size: inode=%lu&bslash;n&quot;
comma
id|inode-&gt;i_ino
)paren
suffix:semicolon
...
suffix:semicolon
multiline_comment|/* Set the number of blocks for stat() - should be done before RR */
id|inode-&gt;i_blksize
op_assign
id|PAGE_CACHE_SIZE
suffix:semicolon
multiline_comment|/* For stat() only */
id|inode-&gt;i_blocks
op_assign
(paren
id|inode-&gt;i_size
...
DECL|struct|isofs_iget5_callback_data
r_struct
id|isofs_iget5_callback_data
(brace
DECL|member|block
r_int
r_int
...
suffix:semicolon
DECL|function|isofs_iget5_test
r_static
r_int
id|isofs_iget5_test
c_func
...
multiline_comment|/* Store, in the inode&squot;s containing structure, the block and block&n; * offset that point to the underlying meta-data for the inode.  The&n; * code below is otherwise similar to the iget() code in&n; * include/linux/fs.h */
DECL|function|isofs_iget
r_struct
id|inode
op_star
id|isofs_iget
