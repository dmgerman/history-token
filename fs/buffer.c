...
multiline_comment|/* Anti-deadlock ordering:&n; *&t;lru_list_lock &gt; hash_table_lock &gt; unused_list_lock&n; */
DECL|macro|BH_ENTRY
...
multiline_comment|/*&n; * The buffers have been marked clean and locked.  Just submit the dang&n; * things.. &n; */
DECL|function|write_locked_buffers
r_static
r_void
id|write_locked_buffers
c_func
(paren
r_struct
id|buffer_head
...
multiline_comment|/*&n; * Write some buffers from the head of the dirty queue.&n; *&n; * This must be called with the LRU lock held, and will&n; * return without it!&n; */
DECL|macro|NRSYNC
mdefine_line|#define NRSYNC (32)
DECL|function|write_some_buffers
r_static
r_int
...
multiline_comment|/*&n; * Write out all buffers on the dirty list.&n; */
DECL|function|write_unlocked_buffers
r_static
r_void
id|write_unlocked_buffers
c_func
(paren
r_struct
id|block_device
...
multiline_comment|/*&n; * Wait for a buffer on the proper list.&n; *&n; * This must be called with the LRU lock held, and&n; * will return with it released.&n; */
DECL|function|wait_for_buffers
r_static
r_int
id|wait_for_buffers
c_func
...
DECL|function|wait_for_some_buffers
r_static
r_inline
r_void
...
DECL|function|wait_for_locked_buffers
r_static
r_int
id|wait_for_locked_buffers
c_func
(paren
r_struct
id|block_device
...
l_int|0
suffix:semicolon
multiline_comment|/* One pass for no-wait, three for wait:&n;&t; * 0) write out all dirty, unlocked buffers;&n;&t; * 1) wait for all dirty locked buffers;&n;&t; * 2) write out all dirty, unlocked buffers;&n;&t; * 2) wait for completion by waiting for all buffers to unlock.&n;&t; */
id|write_unlocked_buffers
c_func
(paren
id|bdev
...
DECL|function|sync_all_buffers
r_int
id|sync_all_buffers
c_func
(paren
...
DECL|function|__insert_into_hash_list
r_static
r_inline
r_void
id|__insert_into_hash_list
c_func
(paren
r_struct
id|buffer_head
...
suffix:semicolon
multiline_comment|/* All buffers in the lru lists are mapped */
r_if
c_cond
(paren
op_logical_neg
...
c_func
(paren
l_string|&quot;invalidate: dirty buffer&bslash;n&quot;
)paren
suffix:semicolon
r_if
c_cond
...
c_func
(paren
l_string|&quot;invalidate: busy buffer&bslash;n&quot;
)paren
suffix:semicolon
id|write_unlock
c_func
...
id|retry
suffix:semicolon
multiline_comment|/* Get rid of the page cache */
...
r_return
suffix:semicolon
multiline_comment|/* If we&squot;re getting into imbalance, start write-out */
id|spin_lock
c_func
(paren
op_amp
...
(paren
id|KERN_ERR
l_string|&quot;%s: zeroing uptodate buffer!&bslash;n&quot;
comma
id|__FUNCTION__
)paren
suffix:semicolon
...
multiline_comment|/*&n; * Create the page-cache page that contains the requested block&n; */
...
DECL|function|hash_page_buffers
r_static
r_void
id|hash_page_buffers
c_func
(paren
r_struct
id|page
...
multiline_comment|/* Insert the buffer into the hash lists if necessary */
r_if
c_cond
(paren
op_logical_neg
...
suffix:semicolon
multiline_comment|/* Size must be within 512 bytes and PAGE_SIZE */
r_if
c_cond
(paren
id|size
...
l_int|0
suffix:semicolon
multiline_comment|/* Hash in the buffers on the hash list */
id|hash_page_buffers
c_func
(paren
id|page
...
suffix:semicolon
multiline_comment|/* We hashed up this page, so increment buffermem */
id|atomic_inc
c_func
(paren
op_amp
...
r_continue
suffix:semicolon
multiline_comment|/* Don&squot;t start IO first time around.. */
r_if
c_cond
(paren
op_logical_neg
...
r_continue
suffix:semicolon
multiline_comment|/* Second time through we start actively writing out.. */
r_if
c_cond
(paren
id|test_and_set_bit
...
mdefine_line|#define BUFFER_BUSY_BITS&t;((1&lt;&lt;BH_Dirty) | (1&lt;&lt;BH_Lock))
...
id|bh
suffix:semicolon
multiline_comment|/* if this buffer was hashed, this page counts as buffermem */
r_if
c_cond
(paren
id|bh-&gt;b_pprev
...
(brace
multiline_comment|/* no IO or waiting next time */
id|gfp_mask
op_assign
l_int|0
suffix:semicolon
...
l_string|&quot;%d locked, %d dirty&bslash;n&quot;
comma
id|buf_types
(braket
id|nlist
...
multiline_comment|/*&n; * kupdate&n; */
DECL|variable|kupdate_timer
r_static
r_struct
id|timer_list
id|kupdate_timer
suffix:semicolon
DECL|function|kupdate_handler
r_static
r_void
id|kupdate_handler
c_func
...
suffix:semicolon
id|kupdate_timer.expires
op_assign
id|jiffies
op_plus
id|bdf_prm.b_un.interval
suffix:semicolon
id|kupdate_timer.data
op_assign
l_int|0
suffix:semicolon
id|kupdate_timer.function
op_assign
id|kupdate_handler
suffix:semicolon
id|add_timer
