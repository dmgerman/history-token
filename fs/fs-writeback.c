...
multiline_comment|/*&n; * Write a single inode&squot;s dirty pages and inode data out to disk.&n; * If `wait&squot; is set, wait on the writeout.&n; *&n; * The whole writeout design is quite complex and fragile.  We want to avoid&n; * starvation of particular inodes when others are being redirtied, prevent&n; * livelocks, etc.&n; *&n; * So what we do is to move all pages which are to be written from dirty_pages&n; * onto io_pages.  And keep on writing io_pages until it&squot;s empty.  Refusing to&n; * move more pages onto io_pages until io_pages is empty.  Once that point has&n; * been reached, we are ready to take another pass across the inode&squot;s dirty&n; * pages.&n; *&n; * Called under inode_lock.&n; */
r_static
r_void
...
(brace
multiline_comment|/* Needs more writeback */
id|inode-&gt;i_state
op_or_assign
id|I_DIRTY_PAGES
suffix:semicolon
