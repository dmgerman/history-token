...
multiline_comment|/*&n; * Write out a list of dirty inodes.&n; *&n; * If `sync&squot; is true, wait on writeout of the last mapping which we write.&n; *&n; * If older_than_this is non-NULL, then only write out mappings which&n; * had their first dirtying at a time earlier than *older_than_this.&n; *&n; * Called under inode_lock.&n; */
