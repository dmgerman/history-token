...
DECL|macro|NP_NONE
mdefine_line|#define NP_NONE&t;&t;((void*)NOTIFY_NONE)
DECL|macro|NP_WOKENUP
mdefine_line|#define NP_WOKENUP&t;((void*)NOTIFY_WOKENUP)
DECL|macro|NP_REMOVED
mdefine_line|#define NP_REMOVED&t;((void*)NOTIFY_REMOVED)
...
id|notify_owner
suffix:semicolon
multiline_comment|/* != 0 means notification registered */
DECL|member|notify
r_struct
id|sigevent
id|notify
suffix:semicolon
DECL|member|notify_filp
r_struct
id|file
op_star
id|notify_filp
...
suffix:semicolon
DECL|variable|mqueue_notify_fops
r_static
r_struct
id|file_operations
id|mqueue_notify_fops
suffix:semicolon
...
(brace
id|info-&gt;notify_filp-&gt;private_data
op_assign
(paren
r_void
op_star
...
multiline_comment|/*&n; * File descriptor based notification, intended to be used to implement&n; * SIGEV_THREAD:&n; * SIGEV_THREAD means that a notification function should be called in the&n; * context of a new thread. The kernel can&squot;t do that. Therefore mq_notify&n; * calls with SIGEV_THREAD return a new file descriptor. A user space helper&n; * must create a new thread and then read from the given file descriptor.&n; * The read always returns one byte. If it&squot;s NOTIFY_WOKENUP, then it must&n; * call the notification function. If it&squot;s NOTIFY_REMOVED, then the&n; * notification was removed. The file descriptor supports poll, thus one&n; * supervisor thread can manage multiple message queue notifications.&n; *&n; * The implementation must support multiple outstanding notifications:&n; * It&squot;s possible that a new notification is added and signaled before user&n; * space calls mqueue_notify_read for the previous notification.&n; * Therefore the notification state is stored in the private_data field of&n; * the file descriptor.&n; */
DECL|function|mqueue_notify_poll
r_static
r_int
r_int
id|mqueue_notify_poll
c_func
(paren
r_struct
id|file
...
DECL|function|mqueue_notify_read
r_static
id|ssize_t
id|mqueue_notify_read
c_func
(paren
r_struct
id|file
...
DECL|function|mqueue_notify_release
r_static
r_int
id|mqueue_notify_release
c_func
(paren
r_struct
id|inode
...
multiline_comment|/* pipelined_send() - send a message directly to the task waiting in&n; * sys_mq_timedreceive() (without inserting message into a queue). */
...
multiline_comment|/*&n; * Notes: the case when user wants us to deregister (with NULL as pointer&n; * or SIGEV_NONE) and he isn&squot;t currently owner of notification will be&n; * silently discarded. It isn&squot;t explicitly defined in the POSIX.&n; */
