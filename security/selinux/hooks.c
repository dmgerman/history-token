...
multiline_comment|/*&n; * Check that a process has enough memory to allocate a new virtual&n; * mapping. 0 means there is enough memory for the allocation to&n; * succeed and -ENOMEM implies there is not.&n; *&n; * Note that secondary_ops-&gt;capable and task_has_perm_noaudit return 0&n; * if the capability is granted, but __vm_enough_memory requires 1 if&n; * the capability is granted.&n; *&n; * Do not audit the selinux permission check, as this is applied to all&n; * processes that allocate mappings.&n; */
...
multiline_comment|/*&n; * called after apply_creds without the task lock held&n; */
DECL|function|selinux_bprm_post_apply_creds
r_static
r_void
id|selinux_bprm_post_apply_creds
c_func
(paren
r_struct
id|linux_binprm
...
suffix:semicolon
multiline_comment|/* Check whether the new SID can inherit signal state&n;&t;   from the old SID.  If not, clear itimers to avoid&n;&t;   subsequent signal generation and flush and unblock&n;&t;   signals. This must occur _after_ the task SID has&n;&t;  been updated so that any kill done after the flush&n;&t;  will be checked against the new SID. */
id|rc
op_assign
id|avc_has_perm
c_func
...
multiline_comment|/* Check whether the new SID can inherit resource limits&n;&t;   from the old SID.  If not, reset all soft limits to&n;&t;   the lower of the current task&squot;s hard limit and the init&n;&t;   task&squot;s soft limit.  Note that the setting of hard limits&n;&t;   (even to lower them) can be controlled by the setrlimit&n;&t;   check. The inclusion of the init task&squot;s soft limit into&n;&t;   the computation is to avoid resetting soft limits higher&n;&t;   than the default soft limit for cases where the default&n;&t;   is lower than the hard limit, e.g. RLIMIT_CORE or&n;&t;   RLIMIT_STACK.*/
id|rc
op_assign
id|avc_has_perm
c_func
...
(brace
multiline_comment|/*&n;&t;&t; * We are making executable an anonymous mapping or a&n;&t;&t; * private file mapping that will also be writable.&n;&t;&t; * This has an additional check.&n;&t;&t; */
r_int
id|rc
op_assign
id|task_has_perm
...
(brace
multiline_comment|/*&n;&t;&t; * We are making executable a file mapping that has&n;&t;&t; * had some COW done. Since pages might have been written,&n;&t;&t; * check ability to execute the possibly modified content.&n;&t;&t; * This typically should only occur for text relocations.&n;&t;&t; */
r_int
id|rc
op_assign
id|file_has_perm
...
id|err
suffix:semicolon
multiline_comment|/*&n;&t; * If a TCP socket, check name_connect permission for the port.&n;&t; */
id|isec
op_assign
id|SOCK_INODE
c_func
...
(brace
multiline_comment|/* SELinux only allows a process to change its own&n;&t;&t;   security attributes. */
r_return
op_minus
id|EACCES
suffix:semicolon
...
id|EINVAL
suffix:semicolon
multiline_comment|/* Only allow single threaded processes to change context */
r_if
c_cond
(paren
id|atomic_read
...
id|error
suffix:semicolon
multiline_comment|/* Check for ptracing, and update the task SID if ok.&n;&t;&t;   Otherwise, leave SID unchanged and fail. */
id|task_lock
c_func
(paren
id|p
