multiline_comment|/* &n; * lib/reed_solomon/rslib.c&n; *&n; * Overview:&n; *   Generic Reed Solomon encoder / decoder library&n; *   &n; * Copyright (C) 2004 Thomas Gleixner (tglx@linutronix.de)&n; *&n; * Reed Solomon code lifted from reed solomon library written by Phil Karn&n; * Copyright 2002 Phil Karn, KA9Q&n; *&n; * $Id: rslib.c,v 1.4 2004/10/05 22:07:53 gleixner Exp $&n; *&n; * This program is free software; you can redistribute it and/or modify&n; * it under the terms of the GNU General Public License version 2 as&n; * published by the Free Software Foundation.&n; *&n; * Description:&n; *&t;&n; * The generic Reed Solomon library provides runtime configurable&n; * encoding / decoding of RS codes.&n; * Each user must call init_rs to get a pointer to a rs_control&n; * structure for the given rs parameters. This structure is either&n; * generated or a already available matching control structure is used.&n; * If a structure is generated then the polynominal arrays for&n; * fast encoding / decoding are built. This can take some time so&n; * make sure not to call this function from a timecritical path.&n; * Usually a module / driver should initialize the neccecary &n; * rs_control structure on module / driver init and release it&n; * on exit.&n; * The encoding puts the calculated syndrome into a given syndrom &n; * buffer. &n; * The decoding is a two step process. The first step calculates&n; * the syndrome over the received (data + syndrom) and calls the&n; * second stage, which does the decoding / error correction itself.&n; * Many hw encoders provide a syndrom calculation over the received&n; * data + syndrom and can call the second stage directly.&n; *&n; */
