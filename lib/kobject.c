...
multiline_comment|/**&n; * kobject_get_path - generate and return the path associated with a given kobj&n; * and kset pair.  The result must be freed by the caller with kfree().&n; *&n; * @kset:&t;kset in question, with which to build the path&n; * @kobj:&t;kobject in question, with which to build the path&n; * @gfp_mask:&t;the allocation type used to allocate the path&n; */
