...
multiline_comment|/*&n; * galileo_pcibios_(read/write) -&n; *&n; * reads/write a dword/word/byte register from the configuration space&n; * of a device.&n; *&n; * Inputs :&n; * bus - bus number&n; * devfn - device function index&n; * offset - register offset in the configuration space&n; * size - size of value (1=byte,2=word,4-dword)&n; * val - value to be written / read&n; *&n; * Outputs :&n; * PCIBIOS_SUCCESSFUL when operation was succesfull&n; * PCIBIOS_DEVICE_NOT_FOUND when the bus or dev is errorneous&n; * PCIBIOS_BAD_REGISTER_NUMBER when accessing non aligned&n; */
DECL|function|galileo_pcibios_read
r_static
r_int
id|galileo_pcibios_read
c_func
...
DECL|function|galileo_pcibios_write
r_static
r_int
id|galileo_pcibios_write
c_func
...
suffix:semicolon
singleline_comment|//if (busnum == 1) pci1WriteConfigReg (offset,bus-&gt;dev,val);
r_return
id|PCIBIOS_SUCCESSFUL
suffix:semicolon
)brace
...
suffix:semicolon
singleline_comment|//if (busnum == 1) tmp = pci1ReadConfigReg (offset,bus-&gt;dev);
r_if
c_cond
(paren
id|size
...
suffix:semicolon
singleline_comment|//if (busnum == 1) pci1WriteConfigReg (offset,bus-&gt;dev,tmp);
)brace
r_return
id|PCIBIOS_SUCCESSFUL
suffix:semicolon
