...
id|va
suffix:semicolon
multiline_comment|/* this gives us the real physical address of the first page */
op_star
id|dma_handle
op_assign
id|pa
...
suffix:semicolon
multiline_comment|/* set refcount=1 on all pages in an order&gt;0 allocation so that vfree() will actually free&n;&t; * all pages that were allocated.&n;&t; */
r_if
c_cond
(paren
id|order
...
multiline_comment|/* we need to ensure that there are no cachelines in use, or worse dirty in this area&n;&t; * - can&squot;t do until after virtual address mappings are created&n;&t; */
