...
(brace
multiline_comment|/* This is usually used needed to map code in small model, so it needs to &n;&t;&t;   be in the first 31bit. Limit it to that.&n;&t;&t;   This means we need to move the unmapped base down for this case. This can &n;&t;&t;   give conflicts with the heap, but we assume that glibc malloc knows how &n;&t;&t;   to fall back to mmap. Give it 1GB of playground for now. -AK */
r_if
c_cond
(paren
op_logical_neg
