...
id|PROT_EXEC
suffix:semicolon
multiline_comment|/* noexec=on|off&n;Control non executable mappings for 64bit processes.&n;&n;on&t;Enable&n;off&t;Disable&n;noforce (default) Don&squot;t enable by default for heap/stack/data, &n;&t;but allow PROT_EXEC to be effective&n;&n;*/
...
suffix:semicolon
multiline_comment|/* noexec32=opt{,opt} &n;&n;Control the no exec default for 32bit processes. Can be also overwritten&n;per executable using ELF header flags (e.g. needed for the X server)&n;Requires noexec=on or noexec=noforce to be effective.&n;&n;Valid options: &n;   all,on    Heap,stack,data is non executable. &t;&n;   off       (default) Heap,stack,data is executable&n;   stack     Stack is non executable, heap/data is.&n;   force     Don&squot;t imply PROT_EXEC for PROT_READ &n;   compat    (default) Imply PROT_EXEC for PROT_READ&n;&n;*/
DECL|function|nonx32_setup
r_static
r_int
id|__init
id|nonx32_setup
...
suffix:semicolon
multiline_comment|/*&n; * Great future plan:&n; * Declare PDA itself and support (irqstack,tss,pml4) as per cpu data.&n; * Always point %gs to its beginning&n; */
...
suffix:semicolon
macro_line|#endif
multiline_comment|/* We don&squot;t support CPU hotplug, so only allocate as much as needed here */
r_int
id|maxi
op_assign
id|max_t
