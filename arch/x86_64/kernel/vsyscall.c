multiline_comment|/*&n; *  linux/arch/x86_64/kernel/vsyscall.c&n; *&n; *  Copyright (C) 2001 Andrea Arcangeli &lt;andrea@suse.de&gt; SuSE&n; *  Copyright 2003 Andi Kleen, SuSE Labs.&n; *&n; *  Thanks to hpa@transmeta.com for some useful hint.&n; *  Special thanks to Ingo Molnar for his early experience with&n; *  a different vsyscall implementation for Linux/IA32 and for the name.&n; *&n; *  vsyscall 1 is located at -10Mbyte, vsyscall 2 is located&n; *  at virtual address -10Mbyte+1024bytes etc... There are at max 4&n; *  vsyscalls. One vsyscall can reserve more than 1 slot to avoid&n; *  jumping out of line if necessary. We cannot add more with this&n; *  mechanism because older kernels won&squot;t return -ENOSYS.&n; *  If we want more than four we need a vDSO.&n; *&n; *  Note: the concept clashes with user mode linux. If you use UML and&n; *  want per guest time just set the kernel.vsyscall64 sysctl to 0.&n; */
...
DECL|macro|SYSCALL
mdefine_line|#define SYSCALL 0x050f
DECL|macro|NOP2
mdefine_line|#define NOP2    0x9090
multiline_comment|/*&n; * NOP out syscall in vsyscall page when not needed.&n; */
DECL|function|vsyscall_sysctl_change
r_static
r_int
...
id|ret
suffix:semicolon
multiline_comment|/* gcc has some trouble with __va(__pa()), so just do it this&n;&t;   way. */
id|map1
op_assign
id|ioremap
c_func
...
DECL|function|vsyscall_sysctl_nostrat
r_static
r_int
id|vsyscall_sysctl_nostrat
c_func
(paren
id|ctl_table
op_star
...
suffix:semicolon
DECL|variable|kernel_root_table2
r_static
id|ctl_table
id|kernel_root_table2
(braket
)braket
op_assign
(brace
