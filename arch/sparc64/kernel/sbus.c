...
multiline_comment|/* These should be allocated on an SMP_CACHE_BYTES&n; * aligned boundary for optimal performance.&n; *&n; * On SYSIO, using an 8K page size we have 1GB of SBUS&n; * DMA space mapped.  We divide this space into equally&n; * sized clusters. We allocate a DMA mapping from the&n; * cluster that matches the order of the allocation, or&n; * if the order is greater than the number of clusters,&n; * we try to allocate from the last cluster.&n; */
...
r_else
(brace
multiline_comment|/* Used cluster in the way */
id|cluster
op_assign
l_int|NULL
suffix:semicolon
...
l_int|0
suffix:semicolon
multiline_comment|/* Multiple cluster allocations must not wrap */
id|cluster
op_assign
l_int|NULL
suffix:semicolon
...
multiline_comment|/* ent/iopte points to the last cluster entry we&squot;re going to use,&n;&t; * so save our place for the next allocation.&n;&t; */
r_if
c_cond
(paren
(paren
...
DECL|function|sbus_dma_sync_single_for_cpu
r_void
...
DECL|function|sbus_dma_sync_single_for_device
r_void
...
DECL|function|sbus_dma_sync_sg_for_cpu
r_void
...
DECL|function|sbus_dma_sync_sg_for_device
r_void
