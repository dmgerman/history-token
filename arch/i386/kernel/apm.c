multiline_comment|/* -*- linux-c -*-&n; * APM BIOS driver for Linux&n; * Copyright 1994-2001 Stephen Rothwell (sfr@canb.auug.org.au)&n; *&n; * Initial development of this driver was funded by NEC Australia P/L&n; *&t;and NEC Corporation&n; *&n; * This program is free software; you can redistribute it and/or modify it&n; * under the terms of the GNU General Public License as published by the&n; * Free Software Foundation; either version 2, or (at your option) any&n; * later version.&n; *&n; * This program is distributed in the hope that it will be useful, but&n; * WITHOUT ANY WARRANTY; without even the implied warranty of&n; * MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the GNU&n; * General Public License for more details.&n; *&n; * October 1995, Rik Faith (faith@cs.unc.edu):&n; *    Minor enhancements and updates (to the patch set) for 1.3.x&n; *    Documentation&n; * January 1996, Rik Faith (faith@cs.unc.edu):&n; *    Make /proc/apm easy to format (bump driver version)&n; * March 1996, Rik Faith (faith@cs.unc.edu):&n; *    Prohibit APM BIOS calls unless apm_enabled.&n; *    (Thanks to Ulrich Windl &lt;Ulrich.Windl@rz.uni-regensburg.de&gt;)&n; * April 1996, Stephen Rothwell (sfr@canb.auug.org.au)&n; *    Version 1.0 and 1.1&n; * May 1996, Version 1.2&n; * Feb 1998, Version 1.3&n; * Feb 1998, Version 1.4&n; * Aug 1998, Version 1.5&n; * Sep 1998, Version 1.6&n; * Nov 1998, Version 1.7&n; * Jan 1999, Version 1.8&n; * Jan 1999, Version 1.9&n; * Oct 1999, Version 1.10&n; * Nov 1999, Version 1.11&n; * Jan 2000, Version 1.12&n; * Feb 2000, Version 1.13&n; * Nov 2000, Version 1.14&n; * Oct 2001, Version 1.15&n; * Jan 2002, Version 1.16&n; * Oct 2002, Version 1.16ac&n; *&n; * History:&n; *    0.6b: first version in official kernel, Linux 1.3.46&n; *    0.7: changed /proc/apm format, Linux 1.3.58&n; *    0.8: fixed gcc 2.7.[12] compilation problems, Linux 1.3.59&n; *    0.9: only call bios if bios is present, Linux 1.3.72&n; *    1.0: use fixed device number, consolidate /proc/apm into this file,&n; *         Linux 1.3.85&n; *    1.1: support user-space standby and suspend, power off after system&n; *         halted, Linux 1.3.98&n; *    1.2: When resetting RTC after resume, take care so that the time&n; *         is only incorrect by 30-60mS (vs. 1S previously) (Gabor J. Toth&n; *         &lt;jtoth@princeton.edu&gt;); improve interaction between&n; *         screen-blanking and gpm (Stephen Rothwell); Linux 1.99.4&n; *    1.2a:Simple change to stop mysterious bug reports with SMP also added&n; *&t;   levels to the printk calls. APM is not defined for SMP machines.&n; *         The new replacment for it is, but Linux doesn&squot;t yet support this.&n; *         Alan Cox Linux 2.1.55&n; *    1.3: Set up a valid data descriptor 0x40 for buggy BIOS&squot;s&n; *    1.4: Upgraded to support APM 1.2. Integrated ThinkPad suspend patch by&n; *         Dean Gaudet &lt;dgaudet@arctic.org&gt;.&n; *         C. Scott Ananian &lt;cananian@alumni.princeton.edu&gt; Linux 2.1.87&n; *    1.5: Fix segment register reloading (in case of bad segments saved&n; *         across BIOS call).&n; *         Stephen Rothwell&n; *    1.6: Cope with complier/assembler differences.&n; *         Only try to turn off the first display device.&n; *         Fix OOPS at power off with no APM BIOS by Jan Echternach&n; *                   &lt;echter@informatik.uni-rostock.de&gt;&n; *         Stephen Rothwell&n; *    1.7: Modify driver&squot;s cached copy of the disabled/disengaged flags&n; *         to reflect current state of APM BIOS.&n; *         Chris Rankin &lt;rankinc@bellsouth.net&gt;&n; *         Reset interrupt 0 timer to 100Hz after suspend&n; *         Chad Miller &lt;cmiller@surfsouth.com&gt;&n; *         Add CONFIG_APM_IGNORE_SUSPEND_BOUNCE&n; *         Richard Gooch &lt;rgooch@atnf.csiro.au&gt;&n; *         Allow boot time disabling of APM&n; *         Make boot messages far less verbose by default&n; *         Make asm safer&n; *         Stephen Rothwell&n; *    1.8: Add CONFIG_APM_RTC_IS_GMT&n; *         Richard Gooch &lt;rgooch@atnf.csiro.au&gt;&n; *         change APM_NOINTS to CONFIG_APM_ALLOW_INTS&n; *         remove dependency on CONFIG_PROC_FS&n; *         Stephen Rothwell&n; *    1.9: Fix small typo.  &lt;laslo@wodip.opole.pl&gt;&n; *         Try to cope with BIOS&squot;s that need to have all display&n; *         devices blanked and not just the first one.&n; *         Ross Paterson &lt;ross@soi.city.ac.uk&gt;&n; *         Fix segment limit setting it has always been wrong as&n; *         the segments needed to have byte granularity.&n; *         Mark a few things __init.&n; *         Add hack to allow power off of SMP systems by popular request.&n; *         Use CONFIG_SMP instead of __SMP__&n; *         Ignore BOUNCES for three seconds.&n; *         Stephen Rothwell&n; *   1.10: Fix for Thinkpad return code.&n; *         Merge 2.2 and 2.3 drivers.&n; *         Remove APM dependencies in arch/i386/kernel/process.c&n; *         Remove APM dependencies in drivers/char/sysrq.c&n; *         Reset time across standby.&n; *         Allow more inititialisation on SMP.&n; *         Remove CONFIG_APM_POWER_OFF and make it boot time&n; *         configurable (default on).&n; *         Make debug only a boot time parameter (remove APM_DEBUG).&n; *         Try to blank all devices on any error.&n; *   1.11: Remove APM dependencies in drivers/char/console.c&n; *         Check nr_running to detect if we are idle (from&n; *         Borislav Deianov &lt;borislav@lix.polytechnique.fr&gt;)&n; *         Fix for bioses that don&squot;t zero the top part of the&n; *         entrypoint offset (Mario Sitta &lt;sitta@al.unipmn.it&gt;)&n; *         (reported by Panos Katsaloulis &lt;teras@writeme.com&gt;).&n; *         Real mode power off patch (Walter Hofmann&n; *         &lt;Walter.Hofmann@physik.stud.uni-erlangen.de&gt;).&n; *   1.12: Remove CONFIG_SMP as the compiler will optimize&n; *         the code away anyway (smp_num_cpus == 1 in UP)&n; *         noted by Artur Skawina &lt;skawina@geocities.com&gt;.&n; *         Make power off under SMP work again.&n; *         Fix thinko with initial engaging of BIOS.&n; *         Make sure power off only happens on CPU 0&n; *         (Paul &quot;Rusty&quot; Russell &lt;rusty@rustcorp.com.au&gt;).&n; *         Do error notification to user mode if BIOS calls fail.&n; *         Move entrypoint offset fix to ...boot/setup.S&n; *         where it belongs (Cosmos &lt;gis88564@cis.nctu.edu.tw&gt;).&n; *         Remove smp-power-off. SMP users must now specify&n; *         &quot;apm=power-off&quot; on the kernel command line. Suggested&n; *         by Jim Avera &lt;jima@hal.com&gt;, modified by Alan Cox&n; *         &lt;alan@lxorguk.ukuu.org.uk&gt;.&n; *         Register the /proc/apm entry even on SMP so that&n; *         scripts that check for it before doing power off&n; *         work (Jim Avera &lt;jima@hal.com&gt;).&n; *   1.13: Changes for new pm_ interfaces (Andy Henroid&n; *         &lt;andy_henroid@yahoo.com&gt;).&n; *         Modularize the code.&n; *         Fix the Thinkpad (again) :-( (CONFIG_APM_IGNORE_MULTIPLE_SUSPENDS&n; *         is now the way life works).&n; *         Fix thinko in suspend() (wrong return).&n; *         Notify drivers on critical suspend.&n; *         Make kapmd absorb more idle time (Pavel Machek &lt;pavel@suse.cz&gt;&n; *         modified by sfr).&n; *         Disable interrupts while we are suspended (Andy Henroid&n; *         &lt;andy_henroid@yahoo.com&gt; fixed by sfr).&n; *         Make power off work on SMP again (Tony Hoyle&n; *         &lt;tmh@magenta-logic.com&gt; and &lt;zlatko@iskon.hr&gt;) modified by sfr.&n; *         Remove CONFIG_APM_SUSPEND_BOUNCE.  The bounce ignore&n; *         interval is now configurable.&n; *   1.14: Make connection version persist across module unload/load.&n; *         Enable and engage power management earlier.&n; *         Disengage power management on module unload.&n; *         Changed to use the sysrq-register hack for registering the&n; *         power off function called by magic sysrq based upon discussions&n; *         in irc://irc.openprojects.net/#kernelnewbies&n; *         (Crutcher Dunnavant &lt;crutcher+kernel@datastacks.com&gt;).&n; *         Make CONFIG_APM_REAL_MODE_POWER_OFF run time configurable.&n; *         (Arjan van de Ven &lt;arjanv@redhat.com&gt;) modified by sfr.&n; *         Work around byte swap bug in one of the Vaio&squot;s BIOS&squot;s&n; *         (Marc Boucher &lt;marc@mbsi.ca&gt;).&n; *         Exposed the disable flag to dmi so that we can handle known&n; *         broken APM (Alan Cox &lt;alan@redhat.com&gt;).&n; *   1.14ac: If the BIOS says &quot;I slowed the CPU down&quot; then don&squot;t spin&n; *         calling it - instead idle. (Alan Cox &lt;alan@redhat.com&gt;)&n; *         If an APM idle fails log it and idle sensibly&n; *   1.15: Don&squot;t queue events to clients who open the device O_WRONLY.&n; *         Don&squot;t expect replies from clients who open the device O_RDONLY.&n; *         (Idea from Thomas Hood &lt;jdthood@mail.com&gt;)&n; *         Minor waitqueue cleanups. (John Fremlin &lt;chief@bandits.org&gt;)&n; *   1.16: Fix idle calling. (Andreas Steinmetz &lt;ast@domdv.de&gt; et al.)&n; *         Notify listeners of standby or suspend events before notifying&n; *         drivers. Return EBUSY to ioctl() if suspend is rejected.&n; *         (Russell King &lt;rmk@arm.linux.org.uk&gt; and Thomas Hood)&n; *         Ignore first resume after we generate our own resume event&n; *         after a suspend (Thomas Hood &lt;jdthood@mail.com&gt;)&n; *         Daemonize now gets rid of our controlling terminal (sfr).&n; *         CONFIG_APM_CPU_IDLE now just affects the default value of&n; *         idle_threshold (sfr).&n; *         Change name of kernel apm daemon (as it no longer idles) (sfr).&n; *   1.16ac: Fix up SMP support somewhat. You can now force SMP on and we&n; *&t;   make _all_ APM calls on the CPU#0. Fix unsafe sign bug.&n; *&t;   TODO: determine if its &quot;boot CPU&quot; or &quot;CPU0&quot; we want to lock to.&n; *&n; * APM 1.1 Reference:&n; *&n; *   Intel Corporation, Microsoft Corporation. Advanced Power Management&n; *   (APM) BIOS Interface Specification, Revision 1.1, September 1993.&n; *   Intel Order Number 241704-001.  Microsoft Part Number 781-110-X01.&n; *&n; * [This document is available free from Intel by calling 800.628.8686 (fax&n; * 916.356.6100) or 800.548.4725; or via anonymous ftp from&n; * ftp://ftp.intel.com/pub/IAL/software_specs/apmv11.doc.  It is also&n; * available from Microsoft by calling 206.882.8080.]&n; *&n; * APM 1.2 Reference:&n; *   Intel Corporation, Microsoft Corporation. Advanced Power Management&n; *   (APM) BIOS Interface Specification, Revision 1.2, February 1996.&n; *&n; * [This document is available from Microsoft at:&n; *    http://www.microsoft.com/hwdev/busbios/amp_12.htm]&n; */
macro_line|#include &lt;linux/config.h&gt;
macro_line|#include &lt;linux/module.h&gt;
macro_line|#include &lt;linux/poll.h&gt;
macro_line|#include &lt;linux/types.h&gt;
macro_line|#include &lt;linux/stddef.h&gt;
macro_line|#include &lt;linux/timer.h&gt;
macro_line|#include &lt;linux/fcntl.h&gt;
macro_line|#include &lt;linux/slab.h&gt;
macro_line|#include &lt;linux/stat.h&gt;
macro_line|#include &lt;linux/proc_fs.h&gt;
macro_line|#include &lt;linux/miscdevice.h&gt;
macro_line|#include &lt;linux/apm_bios.h&gt;
macro_line|#include &lt;linux/init.h&gt;
macro_line|#include &lt;linux/time.h&gt;
macro_line|#include &lt;linux/sched.h&gt;
macro_line|#include &lt;linux/pm.h&gt;
macro_line|#include &lt;linux/device.h&gt;
macro_line|#include &lt;linux/kernel.h&gt;
macro_line|#include &lt;linux/smp.h&gt;
macro_line|#include &lt;linux/smp_lock.h&gt;
macro_line|#include &lt;linux/dmi.h&gt;
macro_line|#include &lt;linux/suspend.h&gt;
macro_line|#include &lt;asm/system.h&gt;
macro_line|#include &lt;asm/uaccess.h&gt;
macro_line|#include &lt;asm/desc.h&gt;
macro_line|#include &quot;io_ports.h&quot;
r_extern
id|spinlock_t
id|i8253_lock
suffix:semicolon
r_extern
r_int
r_int
id|get_cmos_time
c_func
(paren
r_void
)paren
suffix:semicolon
r_extern
r_void
id|machine_real_restart
c_func
(paren
r_int
r_char
op_star
comma
r_int
)paren
suffix:semicolon
macro_line|#if defined(CONFIG_APM_DISPLAY_BLANK) &amp;&amp; defined(CONFIG_VT)
r_extern
r_int
(paren
op_star
id|console_blank_hook
)paren
(paren
r_int
)paren
suffix:semicolon
macro_line|#endif
multiline_comment|/*&n; * The apm_bios device is one of the misc char devices.&n; * This is its minor number.&n; */
DECL|macro|APM_MINOR_DEV
mdefine_line|#define&t;APM_MINOR_DEV&t;134
multiline_comment|/*&n; * See Documentation/Config.help for the configuration options.&n; *&n; * Various options can be changed at boot time as follows:&n; * (We allow underscores for compatibility with the modules code)&n; *&t;apm=on/off&t;&t;&t;enable/disable APM&n; *&t;    [no-]allow[-_]ints&t;&t;allow interrupts during BIOS calls&n; *&t;    [no-]broken[-_]psr&t;&t;BIOS has a broken GetPowerStatus call&n; *&t;    [no-]realmode[-_]power[-_]off&t;switch to real mode before&n; *&t;    &t;&t;&t;&t;&t;powering off&n; *&t;    [no-]debug&t;&t;&t;log some debugging messages&n; *&t;    [no-]power[-_]off&t;&t;power off on shutdown&n; *&t;    [no-]smp&t;&t;&t;Use apm even on an SMP box&n; *&t;    bounce[-_]interval=&lt;n&gt;&t;number of ticks to ignore suspend&n; *&t;    &t;&t;&t;&t;bounces&n; *          idle[-_]threshold=&lt;n&gt;       System idle percentage above which to&n; *                                      make APM BIOS idle calls. Set it to&n; *                                      100 to disable.&n; *          idle[-_]period=&lt;n&gt;          Period (in 1/100s of a second) over&n; *                                      which the idle percentage is&n; *                                      calculated.&n; */
multiline_comment|/* KNOWN PROBLEM MACHINES:&n; *&n; * U: TI 4000M TravelMate: BIOS is *NOT* APM compliant&n; *                         [Confirmed by TI representative]&n; * ?: ACER 486DX4/75: uses dseg 0040, in violation of APM specification&n; *                    [Confirmed by BIOS disassembly]&n; *                    [This may work now ...]&n; * P: Toshiba 1950S: battery life information only gets updated after resume&n; * P: Midwest Micro Soundbook Elite DX2/66 monochrome: screen blanking&n; * &t;broken in BIOS [Reported by Garst R. Reese &lt;reese@isn.net&gt;]&n; * ?: AcerNote-950: oops on reading /proc/apm - workaround is a WIP&n; * &t;Neale Banks &lt;neale@lowendale.com.au&gt; December 2000&n; *&n; * Legend: U = unusable with APM patches&n; *         P = partially usable with APM patches&n; */
multiline_comment|/*&n; * Define as 1 to make the driver always call the APM BIOS busy&n; * routine even if the clock was not reported as slowed by the&n; * idle routine.  Otherwise, define as 0.&n; */
DECL|macro|ALWAYS_CALL_BUSY
mdefine_line|#define ALWAYS_CALL_BUSY   1
multiline_comment|/*&n; * Define to make the APM BIOS calls zero all data segment registers (so&n; * that an incorrect BIOS implementation will cause a kernel panic if it&n; * tries to write to arbitrary memory).&n; */
DECL|macro|APM_ZERO_SEGS
mdefine_line|#define APM_ZERO_SEGS
macro_line|#include &quot;apm.h&quot;
multiline_comment|/*&n; * Define to make all _set_limit calls use 64k limits.  The APM 1.1 BIOS is&n; * supposed to provide limit information that it recognizes.  Many machines&n; * do this correctly, but many others do not restrict themselves to their&n; * claimed limit.  When this happens, they will cause a segmentation&n; * violation in the kernel at boot time.  Most BIOS&squot;s, however, will&n; * respect a 64k limit, so we use that.  If you want to be pedantic and&n; * hold your BIOS to its claims, then undefine this.&n; */
DECL|macro|APM_RELAX_SEGMENTS
mdefine_line|#define APM_RELAX_SEGMENTS
multiline_comment|/*&n; * Define to re-initialize the interrupt 0 timer to 100 Hz after a suspend.&n; * This patched by Chad Miller &lt;cmiller@surfsouth.com&gt;, original code by&n; * David Chen &lt;chen@ctpa04.mit.edu&gt;&n; */
DECL|macro|INIT_TIMER_AFTER_SUSPEND
macro_line|#undef INIT_TIMER_AFTER_SUSPEND
macro_line|#ifdef INIT_TIMER_AFTER_SUSPEND
macro_line|#include &lt;linux/timex.h&gt;
macro_line|#include &lt;asm/io.h&gt;
macro_line|#include &lt;linux/delay.h&gt;
macro_line|#endif
multiline_comment|/*&n; * Need to poll the APM BIOS every second&n; */
DECL|macro|APM_CHECK_TIMEOUT
mdefine_line|#define APM_CHECK_TIMEOUT&t;(HZ)
multiline_comment|/*&n; * Ignore suspend events for this amount of time after a resume&n; */
DECL|macro|DEFAULT_BOUNCE_INTERVAL
mdefine_line|#define DEFAULT_BOUNCE_INTERVAL&t;&t;(3 * HZ)
multiline_comment|/*&n; * Maximum number of events stored&n; */
DECL|macro|APM_MAX_EVENTS
mdefine_line|#define APM_MAX_EVENTS&t;&t;20
multiline_comment|/*&n; * The per-file APM data&n; */
DECL|struct|apm_user
r_struct
id|apm_user
(brace
DECL|member|magic
r_int
id|magic
suffix:semicolon
DECL|member|next
r_struct
id|apm_user
op_star
id|next
suffix:semicolon
DECL|member|suser
r_int
id|suser
suffix:colon
l_int|1
suffix:semicolon
DECL|member|writer
r_int
id|writer
suffix:colon
l_int|1
suffix:semicolon
DECL|member|reader
r_int
id|reader
suffix:colon
l_int|1
suffix:semicolon
DECL|member|suspend_wait
r_int
id|suspend_wait
suffix:colon
l_int|1
suffix:semicolon
DECL|member|suspend_result
r_int
id|suspend_result
suffix:semicolon
DECL|member|suspends_pending
r_int
id|suspends_pending
suffix:semicolon
DECL|member|standbys_pending
r_int
id|standbys_pending
suffix:semicolon
DECL|member|suspends_read
r_int
id|suspends_read
suffix:semicolon
DECL|member|standbys_read
r_int
id|standbys_read
suffix:semicolon
DECL|member|event_head
r_int
id|event_head
suffix:semicolon
DECL|member|event_tail
r_int
id|event_tail
suffix:semicolon
DECL|member|events
id|apm_event_t
id|events
(braket
id|APM_MAX_EVENTS
)braket
suffix:semicolon
)brace
suffix:semicolon
multiline_comment|/*&n; * The magic number in apm_user&n; */
DECL|macro|APM_BIOS_MAGIC
mdefine_line|#define APM_BIOS_MAGIC&t;&t;0x4101
multiline_comment|/*&n; * idle percentage above which bios idle calls are done&n; */
macro_line|#ifdef CONFIG_APM_CPU_IDLE
DECL|macro|DEFAULT_IDLE_THRESHOLD
mdefine_line|#define DEFAULT_IDLE_THRESHOLD&t;95
macro_line|#else
DECL|macro|DEFAULT_IDLE_THRESHOLD
mdefine_line|#define DEFAULT_IDLE_THRESHOLD&t;100
macro_line|#endif
DECL|macro|DEFAULT_IDLE_PERIOD
mdefine_line|#define DEFAULT_IDLE_PERIOD&t;(100 / 3)
multiline_comment|/*&n; * Local variables&n; */
r_static
r_struct
(brace
DECL|member|offset
r_int
r_int
id|offset
suffix:semicolon
DECL|member|segment
r_int
r_int
id|segment
suffix:semicolon
DECL|variable|apm_bios_entry
)brace
id|apm_bios_entry
suffix:semicolon
DECL|variable|clock_slowed
r_static
r_int
id|clock_slowed
suffix:semicolon
DECL|variable|idle_threshold
r_static
r_int
id|idle_threshold
op_assign
id|DEFAULT_IDLE_THRESHOLD
suffix:semicolon
DECL|variable|idle_period
r_static
r_int
id|idle_period
op_assign
id|DEFAULT_IDLE_PERIOD
suffix:semicolon
DECL|variable|set_pm_idle
r_static
r_int
id|set_pm_idle
suffix:semicolon
DECL|variable|suspends_pending
r_static
r_int
id|suspends_pending
suffix:semicolon
DECL|variable|standbys_pending
r_static
r_int
id|standbys_pending
suffix:semicolon
DECL|variable|ignore_sys_suspend
r_static
r_int
id|ignore_sys_suspend
suffix:semicolon
DECL|variable|ignore_normal_resume
r_static
r_int
id|ignore_normal_resume
suffix:semicolon
DECL|variable|bounce_interval
r_static
r_int
id|bounce_interval
op_assign
id|DEFAULT_BOUNCE_INTERVAL
suffix:semicolon
macro_line|#ifdef CONFIG_APM_RTC_IS_GMT
DECL|macro|clock_cmos_diff
macro_line|#&t;define&t;clock_cmos_diff&t;0
DECL|macro|got_clock_diff
macro_line|#&t;define&t;got_clock_diff&t;1
macro_line|#else
DECL|variable|clock_cmos_diff
r_static
r_int
id|clock_cmos_diff
suffix:semicolon
DECL|variable|got_clock_diff
r_static
r_int
id|got_clock_diff
suffix:semicolon
macro_line|#endif
DECL|variable|debug
r_static
r_int
id|debug
suffix:semicolon
DECL|variable|smp
r_static
r_int
id|smp
suffix:semicolon
DECL|variable|apm_disabled
r_static
r_int
id|apm_disabled
op_assign
op_minus
l_int|1
suffix:semicolon
macro_line|#ifdef CONFIG_SMP
DECL|variable|power_off
r_static
r_int
id|power_off
suffix:semicolon
macro_line|#else
DECL|variable|power_off
r_static
r_int
id|power_off
op_assign
l_int|1
suffix:semicolon
macro_line|#endif
macro_line|#ifdef CONFIG_APM_REAL_MODE_POWER_OFF
DECL|variable|realmode_power_off
r_static
r_int
id|realmode_power_off
op_assign
l_int|1
suffix:semicolon
macro_line|#else
DECL|variable|realmode_power_off
r_static
r_int
id|realmode_power_off
suffix:semicolon
macro_line|#endif
DECL|variable|exit_kapmd
r_static
r_int
id|exit_kapmd
suffix:semicolon
DECL|variable|kapmd_running
r_static
r_int
id|kapmd_running
suffix:semicolon
macro_line|#ifdef CONFIG_APM_ALLOW_INTS
DECL|variable|allow_ints
r_static
r_int
id|allow_ints
op_assign
l_int|1
suffix:semicolon
macro_line|#else
DECL|variable|allow_ints
r_static
r_int
id|allow_ints
suffix:semicolon
macro_line|#endif
DECL|variable|broken_psr
r_static
r_int
id|broken_psr
suffix:semicolon
r_static
id|DECLARE_WAIT_QUEUE_HEAD
c_func
(paren
id|apm_waitqueue
)paren
suffix:semicolon
r_static
id|DECLARE_WAIT_QUEUE_HEAD
c_func
(paren
id|apm_suspend_waitqueue
)paren
suffix:semicolon
DECL|variable|user_list
r_static
r_struct
id|apm_user
op_star
id|user_list
suffix:semicolon
r_static
id|DEFINE_SPINLOCK
c_func
(paren
id|user_list_lock
)paren
suffix:semicolon
DECL|variable|bad_bios_desc
r_static
r_struct
id|desc_struct
id|bad_bios_desc
op_assign
(brace
l_int|0
comma
l_int|0x00409200
)brace
suffix:semicolon
DECL|variable|driver_version
r_static
r_char
id|driver_version
(braket
)braket
op_assign
l_string|&quot;1.16ac&quot;
suffix:semicolon
multiline_comment|/* no spaces */
multiline_comment|/*&n; *&t;APM event names taken from the APM 1.2 specification. These are&n; *&t;the message codes that the BIOS uses to tell us about events&n; */
DECL|variable|apm_event_name
r_static
r_char
op_star
id|apm_event_name
(braket
)braket
op_assign
(brace
l_string|&quot;system standby&quot;
comma
l_string|&quot;system suspend&quot;
comma
l_string|&quot;normal resume&quot;
comma
l_string|&quot;critical resume&quot;
comma
l_string|&quot;low battery&quot;
comma
l_string|&quot;power status change&quot;
comma
l_string|&quot;update time&quot;
comma
l_string|&quot;critical suspend&quot;
comma
l_string|&quot;user standby&quot;
comma
l_string|&quot;user suspend&quot;
comma
l_string|&quot;system standby resume&quot;
comma
l_string|&quot;capabilities change&quot;
)brace
suffix:semicolon
DECL|macro|NR_APM_EVENT_NAME
mdefine_line|#define NR_APM_EVENT_NAME&t;&bslash;&n;&t;&t;(sizeof(apm_event_name) / sizeof(apm_event_name[0]))
DECL|struct|lookup_t
r_typedef
r_struct
id|lookup_t
(brace
DECL|member|key
r_int
id|key
suffix:semicolon
DECL|member|msg
r_char
op_star
id|msg
suffix:semicolon
DECL|typedef|lookup_t
)brace
id|lookup_t
suffix:semicolon
multiline_comment|/*&n; *&t;The BIOS returns a set of standard error codes in AX when the&n; *&t;carry flag is set.&n; */
DECL|variable|error_table
r_static
r_const
id|lookup_t
id|error_table
(braket
)braket
op_assign
(brace
multiline_comment|/* N/A&t;{ APM_SUCCESS,&t;&t;&quot;Operation succeeded&quot; }, */
(brace
id|APM_DISABLED
comma
l_string|&quot;Power management disabled&quot;
)brace
comma
(brace
id|APM_CONNECTED
comma
l_string|&quot;Real mode interface already connected&quot;
)brace
comma
(brace
id|APM_NOT_CONNECTED
comma
l_string|&quot;Interface not connected&quot;
)brace
comma
(brace
id|APM_16_CONNECTED
comma
l_string|&quot;16 bit interface already connected&quot;
)brace
comma
multiline_comment|/* N/A&t;{ APM_16_UNSUPPORTED,&t;&quot;16 bit interface not supported&quot; }, */
(brace
id|APM_32_CONNECTED
comma
l_string|&quot;32 bit interface already connected&quot;
)brace
comma
(brace
id|APM_32_UNSUPPORTED
comma
l_string|&quot;32 bit interface not supported&quot;
)brace
comma
(brace
id|APM_BAD_DEVICE
comma
l_string|&quot;Unrecognized device ID&quot;
)brace
comma
(brace
id|APM_BAD_PARAM
comma
l_string|&quot;Parameter out of range&quot;
)brace
comma
(brace
id|APM_NOT_ENGAGED
comma
l_string|&quot;Interface not engaged&quot;
)brace
comma
(brace
id|APM_BAD_FUNCTION
comma
l_string|&quot;Function not supported&quot;
)brace
comma
(brace
id|APM_RESUME_DISABLED
comma
l_string|&quot;Resume timer disabled&quot;
)brace
comma
(brace
id|APM_BAD_STATE
comma
l_string|&quot;Unable to enter requested state&quot;
)brace
comma
multiline_comment|/* N/A&t;{ APM_NO_EVENTS,&t;&quot;No events pending&quot; }, */
(brace
id|APM_NO_ERROR
comma
l_string|&quot;BIOS did not set a return code&quot;
)brace
comma
(brace
id|APM_NOT_PRESENT
comma
l_string|&quot;No APM present&quot;
)brace
)brace
suffix:semicolon
DECL|macro|ERROR_COUNT
mdefine_line|#define ERROR_COUNT&t;(sizeof(error_table)/sizeof(lookup_t))
multiline_comment|/**&n; *&t;apm_error&t;-&t;display an APM error&n; *&t;@str: information string&n; *&t;@err: APM BIOS return code&n; *&n; *&t;Write a meaningful log entry to the kernel log in the event of&n; *&t;an APM error.&n; */
DECL|function|apm_error
r_static
r_void
id|apm_error
c_func
(paren
r_char
op_star
id|str
comma
r_int
id|err
)paren
(brace
r_int
id|i
suffix:semicolon
r_for
c_loop
(paren
id|i
op_assign
l_int|0
suffix:semicolon
id|i
OL
id|ERROR_COUNT
suffix:semicolon
id|i
op_increment
)paren
r_if
c_cond
(paren
id|error_table
(braket
id|i
)braket
dot
id|key
op_eq
id|err
)paren
r_break
suffix:semicolon
r_if
c_cond
(paren
id|i
OL
id|ERROR_COUNT
)paren
id|printk
c_func
(paren
id|KERN_NOTICE
l_string|&quot;apm: %s: %s&bslash;n&quot;
comma
id|str
comma
id|error_table
(braket
id|i
)braket
dot
id|msg
)paren
suffix:semicolon
r_else
id|printk
c_func
(paren
id|KERN_NOTICE
l_string|&quot;apm: %s: unknown error code %#2.2x&bslash;n&quot;
comma
id|str
comma
id|err
)paren
suffix:semicolon
)brace
multiline_comment|/*&n; * Lock APM functionality to physical CPU 0&n; */
macro_line|#ifdef CONFIG_SMP
DECL|function|apm_save_cpus
r_static
id|cpumask_t
id|apm_save_cpus
c_func
(paren
r_void
)paren
(brace
id|cpumask_t
id|x
op_assign
id|current-&gt;cpus_allowed
suffix:semicolon
multiline_comment|/* Some bioses don&squot;t like being called from CPU != 0 */
id|set_cpus_allowed
c_func
(paren
id|current
comma
id|cpumask_of_cpu
c_func
(paren
l_int|0
)paren
)paren
suffix:semicolon
id|BUG_ON
c_func
(paren
id|smp_processor_id
c_func
(paren
)paren
op_ne
l_int|0
)paren
suffix:semicolon
r_return
id|x
suffix:semicolon
)brace
DECL|function|apm_restore_cpus
r_static
r_inline
r_void
id|apm_restore_cpus
c_func
(paren
id|cpumask_t
id|mask
)paren
(brace
id|set_cpus_allowed
c_func
(paren
id|current
comma
id|mask
)paren
suffix:semicolon
)brace
macro_line|#else
multiline_comment|/*&n; *&t;No CPU lockdown needed on a uniprocessor&n; */
DECL|macro|apm_save_cpus
mdefine_line|#define apm_save_cpus()&t;&t;(current-&gt;cpus_allowed)
DECL|macro|apm_restore_cpus
mdefine_line|#define apm_restore_cpus(x)&t;(void)(x)
macro_line|#endif
multiline_comment|/*&n; * These are the actual BIOS calls.  Depending on APM_ZERO_SEGS and&n; * apm_info.allow_ints, we are being really paranoid here!  Not only&n; * are interrupts disabled, but all the segment registers (except SS)&n; * are saved and zeroed this means that if the BIOS tries to reference&n; * any data without explicitly loading the segment registers, the kernel&n; * will fault immediately rather than have some unforeseen circumstances&n; * for the rest of the kernel.  And it will be very obvious!  :-) Doing&n; * this depends on CS referring to the same physical memory as DS so that&n; * DS can be zeroed before the call. Unfortunately, we can&squot;t do anything&n; * about the stack segment/pointer.  Also, we tell the compiler that&n; * everything could change.&n; *&n; * Also, we KNOW that for the non error case of apm_bios_call, there&n; * is no useful data returned in the low order 8 bits of eax.&n; */
DECL|macro|APM_DO_CLI
mdefine_line|#define APM_DO_CLI&t;&bslash;&n;&t;if (apm_info.allow_ints) &bslash;&n;&t;&t;local_irq_enable(); &bslash;&n;&t;else &bslash;&n;&t;&t;local_irq_disable();
macro_line|#ifdef APM_ZERO_SEGS
DECL|macro|APM_DECL_SEGS
macro_line|#&t;define APM_DECL_SEGS &bslash;&n;&t;&t;unsigned int saved_fs; unsigned int saved_gs;
DECL|macro|APM_DO_SAVE_SEGS
macro_line|#&t;define APM_DO_SAVE_SEGS &bslash;&n;&t;&t;savesegment(fs, saved_fs); savesegment(gs, saved_gs)
DECL|macro|APM_DO_RESTORE_SEGS
macro_line|#&t;define APM_DO_RESTORE_SEGS &bslash;&n;&t;&t;loadsegment(fs, saved_fs); loadsegment(gs, saved_gs)
macro_line|#else
DECL|macro|APM_DECL_SEGS
macro_line|#&t;define APM_DECL_SEGS
DECL|macro|APM_DO_SAVE_SEGS
macro_line|#&t;define APM_DO_SAVE_SEGS
DECL|macro|APM_DO_RESTORE_SEGS
macro_line|#&t;define APM_DO_RESTORE_SEGS
macro_line|#endif
multiline_comment|/**&n; *&t;apm_bios_call&t;-&t;Make an APM BIOS 32bit call&n; *&t;@func: APM function to execute&n; *&t;@ebx_in: EBX register for call entry&n; *&t;@ecx_in: ECX register for call entry&n; *&t;@eax: EAX register return&n; *&t;@ebx: EBX register return&n; *&t;@ecx: ECX register return&n; *&t;@edx: EDX register return&n; *&t;@esi: ESI register return&n; *&n; *&t;Make an APM call using the 32bit protected mode interface. The&n; *&t;caller is responsible for knowing if APM BIOS is configured and&n; *&t;enabled. This call can disable interrupts for a long period of&n; *&t;time on some laptops.  The return value is in AH and the carry&n; *&t;flag is loaded into AL.  If there is an error, then the error&n; *&t;code is returned in AH (bits 8-15 of eax) and this function&n; *&t;returns non-zero.&n; */
DECL|function|apm_bios_call
r_static
id|u8
id|apm_bios_call
c_func
(paren
id|u32
id|func
comma
id|u32
id|ebx_in
comma
id|u32
id|ecx_in
comma
id|u32
op_star
id|eax
comma
id|u32
op_star
id|ebx
comma
id|u32
op_star
id|ecx
comma
id|u32
op_star
id|edx
comma
id|u32
op_star
id|esi
)paren
(brace
id|APM_DECL_SEGS
r_int
r_int
id|flags
suffix:semicolon
id|cpumask_t
id|cpus
suffix:semicolon
r_int
id|cpu
suffix:semicolon
r_struct
id|desc_struct
id|save_desc_40
suffix:semicolon
id|cpus
op_assign
id|apm_save_cpus
c_func
(paren
)paren
suffix:semicolon
id|cpu
op_assign
id|get_cpu
c_func
(paren
)paren
suffix:semicolon
id|save_desc_40
op_assign
id|per_cpu
c_func
(paren
id|cpu_gdt_table
comma
id|cpu
)paren
(braket
l_int|0x40
op_div
l_int|8
)braket
suffix:semicolon
id|per_cpu
c_func
(paren
id|cpu_gdt_table
comma
id|cpu
)paren
(braket
l_int|0x40
op_div
l_int|8
)braket
op_assign
id|bad_bios_desc
suffix:semicolon
id|local_save_flags
c_func
(paren
id|flags
)paren
suffix:semicolon
id|APM_DO_CLI
suffix:semicolon
id|APM_DO_SAVE_SEGS
suffix:semicolon
id|apm_bios_call_asm
c_func
(paren
id|func
comma
id|ebx_in
comma
id|ecx_in
comma
id|eax
comma
id|ebx
comma
id|ecx
comma
id|edx
comma
id|esi
)paren
suffix:semicolon
id|APM_DO_RESTORE_SEGS
suffix:semicolon
id|local_irq_restore
c_func
(paren
id|flags
)paren
suffix:semicolon
id|per_cpu
c_func
(paren
id|cpu_gdt_table
comma
id|cpu
)paren
(braket
l_int|0x40
op_div
l_int|8
)braket
op_assign
id|save_desc_40
suffix:semicolon
id|put_cpu
c_func
(paren
)paren
suffix:semicolon
id|apm_restore_cpus
c_func
(paren
id|cpus
)paren
suffix:semicolon
r_return
op_star
id|eax
op_amp
l_int|0xff
suffix:semicolon
)brace
multiline_comment|/**&n; *&t;apm_bios_call_simple&t;-&t;make a simple APM BIOS 32bit call&n; *&t;@func: APM function to invoke&n; *&t;@ebx_in: EBX register value for BIOS call&n; *&t;@ecx_in: ECX register value for BIOS call&n; *&t;@eax: EAX register on return from the BIOS call&n; *&n; *&t;Make a BIOS call that does only returns one value, or just status.&n; *&t;If there is an error, then the error code is returned in AH&n; *&t;(bits 8-15 of eax) and this function returns non-zero. This is&n; *&t;used for simpler BIOS operations. This call may hold interrupts&n; *&t;off for a long time on some laptops.&n; */
DECL|function|apm_bios_call_simple
r_static
id|u8
id|apm_bios_call_simple
c_func
(paren
id|u32
id|func
comma
id|u32
id|ebx_in
comma
id|u32
id|ecx_in
comma
id|u32
op_star
id|eax
)paren
(brace
id|u8
id|error
suffix:semicolon
id|APM_DECL_SEGS
r_int
r_int
id|flags
suffix:semicolon
id|cpumask_t
id|cpus
suffix:semicolon
r_int
id|cpu
suffix:semicolon
r_struct
id|desc_struct
id|save_desc_40
suffix:semicolon
id|cpus
op_assign
id|apm_save_cpus
c_func
(paren
)paren
suffix:semicolon
id|cpu
op_assign
id|get_cpu
c_func
(paren
)paren
suffix:semicolon
id|save_desc_40
op_assign
id|per_cpu
c_func
(paren
id|cpu_gdt_table
comma
id|cpu
)paren
(braket
l_int|0x40
op_div
l_int|8
)braket
suffix:semicolon
id|per_cpu
c_func
(paren
id|cpu_gdt_table
comma
id|cpu
)paren
(braket
l_int|0x40
op_div
l_int|8
)braket
op_assign
id|bad_bios_desc
suffix:semicolon
id|local_save_flags
c_func
(paren
id|flags
)paren
suffix:semicolon
id|APM_DO_CLI
suffix:semicolon
id|APM_DO_SAVE_SEGS
suffix:semicolon
id|error
op_assign
id|apm_bios_call_simple_asm
c_func
(paren
id|func
comma
id|ebx_in
comma
id|ecx_in
comma
id|eax
)paren
suffix:semicolon
id|APM_DO_RESTORE_SEGS
suffix:semicolon
id|local_irq_restore
c_func
(paren
id|flags
)paren
suffix:semicolon
id|__get_cpu_var
c_func
(paren
id|cpu_gdt_table
)paren
(braket
l_int|0x40
op_div
l_int|8
)braket
op_assign
id|save_desc_40
suffix:semicolon
id|put_cpu
c_func
(paren
)paren
suffix:semicolon
id|apm_restore_cpus
c_func
(paren
id|cpus
)paren
suffix:semicolon
r_return
id|error
suffix:semicolon
)brace
multiline_comment|/**&n; *&t;apm_driver_version&t;-&t;APM driver version&n; *&t;@val:&t;loaded with the APM version on return&n; *&n; *&t;Retrieve the APM version supported by the BIOS. This is only&n; *&t;supported for APM 1.1 or higher. An error indicates APM 1.0 is&n; *&t;probably present.&n; *&n; *&t;On entry val should point to a value indicating the APM driver&n; *&t;version with the high byte being the major and the low byte the&n; *&t;minor number both in BCD&n; *&n; *&t;On return it will hold the BIOS revision supported in the&n; *&t;same format.&n; */
DECL|function|apm_driver_version
r_static
r_int
id|apm_driver_version
c_func
(paren
id|u_short
op_star
id|val
)paren
(brace
id|u32
id|eax
suffix:semicolon
r_if
c_cond
(paren
id|apm_bios_call_simple
c_func
(paren
id|APM_FUNC_VERSION
comma
l_int|0
comma
op_star
id|val
comma
op_amp
id|eax
)paren
)paren
r_return
(paren
id|eax
op_rshift
l_int|8
)paren
op_amp
l_int|0xff
suffix:semicolon
op_star
id|val
op_assign
id|eax
suffix:semicolon
r_return
id|APM_SUCCESS
suffix:semicolon
)brace
multiline_comment|/**&n; *&t;apm_get_event&t;-&t;get an APM event from the BIOS&n; *&t;@event: pointer to the event&n; *&t;@info: point to the event information&n; *&n; *&t;The APM BIOS provides a polled information for event&n; *&t;reporting. The BIOS expects to be polled at least every second&n; *&t;when events are pending. When a message is found the caller should&n; *&t;poll until no more messages are present.  However, this causes&n; *&t;problems on some laptops where a suspend event notification is&n; *&t;not cleared until it is acknowledged.&n; *&n; *&t;Additional information is returned in the info pointer, providing&n; *&t;that APM 1.2 is in use. If no messges are pending the value 0x80&n; *&t;is returned (No power management events pending).&n; */
DECL|function|apm_get_event
r_static
r_int
id|apm_get_event
c_func
(paren
id|apm_event_t
op_star
id|event
comma
id|apm_eventinfo_t
op_star
id|info
)paren
(brace
id|u32
id|eax
suffix:semicolon
id|u32
id|ebx
suffix:semicolon
id|u32
id|ecx
suffix:semicolon
id|u32
id|dummy
suffix:semicolon
r_if
c_cond
(paren
id|apm_bios_call
c_func
(paren
id|APM_FUNC_GET_EVENT
comma
l_int|0
comma
l_int|0
comma
op_amp
id|eax
comma
op_amp
id|ebx
comma
op_amp
id|ecx
comma
op_amp
id|dummy
comma
op_amp
id|dummy
)paren
)paren
r_return
(paren
id|eax
op_rshift
l_int|8
)paren
op_amp
l_int|0xff
suffix:semicolon
op_star
id|event
op_assign
id|ebx
suffix:semicolon
r_if
c_cond
(paren
id|apm_info.connection_version
OL
l_int|0x0102
)paren
op_star
id|info
op_assign
op_complement
l_int|0
suffix:semicolon
multiline_comment|/* indicate info not valid */
r_else
op_star
id|info
op_assign
id|ecx
suffix:semicolon
r_return
id|APM_SUCCESS
suffix:semicolon
)brace
multiline_comment|/**&n; *&t;set_power_state&t;-&t;set the power management state&n; *&t;@what: which items to transition&n; *&t;@state: state to transition to&n; *&n; *&t;Request an APM change of state for one or more system devices. The&n; *&t;processor state must be transitioned last of all. what holds the&n; *&t;class of device in the upper byte and the device number (0xFF for&n; *&t;all) for the object to be transitioned.&n; *&n; *&t;The state holds the state to transition to, which may in fact&n; *&t;be an acceptance of a BIOS requested state change.&n; */
DECL|function|set_power_state
r_static
r_int
id|set_power_state
c_func
(paren
id|u_short
id|what
comma
id|u_short
id|state
)paren
(brace
id|u32
id|eax
suffix:semicolon
r_if
c_cond
(paren
id|apm_bios_call_simple
c_func
(paren
id|APM_FUNC_SET_STATE
comma
id|what
comma
id|state
comma
op_amp
id|eax
)paren
)paren
r_return
(paren
id|eax
op_rshift
l_int|8
)paren
op_amp
l_int|0xff
suffix:semicolon
r_return
id|APM_SUCCESS
suffix:semicolon
)brace
multiline_comment|/**&n; *&t;set_system_power_state - set system wide power state&n; *&t;@state: which state to enter&n; *&n; *&t;Transition the entire system into a new APM power state.&n; */
DECL|function|set_system_power_state
r_static
r_int
id|set_system_power_state
c_func
(paren
id|u_short
id|state
)paren
(brace
r_return
id|set_power_state
c_func
(paren
id|APM_DEVICE_ALL
comma
id|state
)paren
suffix:semicolon
)brace
multiline_comment|/**&n; *&t;apm_do_idle&t;-&t;perform power saving&n; *&n; *&t;This function notifies the BIOS that the processor is (in the view&n; *&t;of the OS) idle. It returns -1 in the event that the BIOS refuses&n; *&t;to handle the idle request. On a success the function returns 1&n; *&t;if the BIOS did clock slowing or 0 otherwise.&n; */
DECL|function|apm_do_idle
r_static
r_int
id|apm_do_idle
c_func
(paren
r_void
)paren
(brace
id|u32
id|eax
suffix:semicolon
r_if
c_cond
(paren
id|apm_bios_call_simple
c_func
(paren
id|APM_FUNC_IDLE
comma
l_int|0
comma
l_int|0
comma
op_amp
id|eax
)paren
)paren
(brace
r_static
r_int
r_int
id|t
suffix:semicolon
multiline_comment|/* This always fails on some SMP boards running UP kernels.&n;&t;&t; * Only report the failure the first 5 times.&n;&t;&t; */
r_if
c_cond
(paren
op_increment
id|t
OL
l_int|5
)paren
(brace
id|printk
c_func
(paren
id|KERN_DEBUG
l_string|&quot;apm_do_idle failed (%d)&bslash;n&quot;
comma
(paren
id|eax
op_rshift
l_int|8
)paren
op_amp
l_int|0xff
)paren
suffix:semicolon
id|t
op_assign
id|jiffies
suffix:semicolon
)brace
r_return
op_minus
l_int|1
suffix:semicolon
)brace
id|clock_slowed
op_assign
(paren
id|apm_info.bios.flags
op_amp
id|APM_IDLE_SLOWS_CLOCK
)paren
op_ne
l_int|0
suffix:semicolon
r_return
id|clock_slowed
suffix:semicolon
)brace
multiline_comment|/**&n; *&t;apm_do_busy&t;-&t;inform the BIOS the CPU is busy&n; *&n; *&t;Request that the BIOS brings the CPU back to full performance. &n; */
DECL|function|apm_do_busy
r_static
r_void
id|apm_do_busy
c_func
(paren
r_void
)paren
(brace
id|u32
id|dummy
suffix:semicolon
r_if
c_cond
(paren
id|clock_slowed
op_logical_or
id|ALWAYS_CALL_BUSY
)paren
(brace
(paren
r_void
)paren
id|apm_bios_call_simple
c_func
(paren
id|APM_FUNC_BUSY
comma
l_int|0
comma
l_int|0
comma
op_amp
id|dummy
)paren
suffix:semicolon
id|clock_slowed
op_assign
l_int|0
suffix:semicolon
)brace
)brace
multiline_comment|/*&n; * If no process has really been interested in&n; * the CPU for some time, we want to call BIOS&n; * power management - we probably want&n; * to conserve power.&n; */
DECL|macro|IDLE_CALC_LIMIT
mdefine_line|#define IDLE_CALC_LIMIT   (HZ * 100)
DECL|macro|IDLE_LEAKY_MAX
mdefine_line|#define IDLE_LEAKY_MAX    16
DECL|variable|original_pm_idle
r_static
r_void
(paren
op_star
id|original_pm_idle
)paren
(paren
r_void
)paren
suffix:semicolon
r_extern
r_void
id|default_idle
c_func
(paren
r_void
)paren
suffix:semicolon
multiline_comment|/**&n; * apm_cpu_idle&t;&t;-&t;cpu idling for APM capable Linux&n; *&n; * This is the idling function the kernel executes when APM is available. It &n; * tries to do BIOS powermanagement based on the average system idle time.&n; * Furthermore it calls the system default idle routine.&n; */
DECL|function|apm_cpu_idle
r_static
r_void
id|apm_cpu_idle
c_func
(paren
r_void
)paren
(brace
r_static
r_int
id|use_apm_idle
suffix:semicolon
multiline_comment|/* = 0 */
r_static
r_int
r_int
id|last_jiffies
suffix:semicolon
multiline_comment|/* = 0 */
r_static
r_int
r_int
id|last_stime
suffix:semicolon
multiline_comment|/* = 0 */
r_int
id|apm_idle_done
op_assign
l_int|0
suffix:semicolon
r_int
r_int
id|jiffies_since_last_check
op_assign
id|jiffies
op_minus
id|last_jiffies
suffix:semicolon
r_int
r_int
id|bucket
suffix:semicolon
id|recalc
suffix:colon
r_if
c_cond
(paren
id|jiffies_since_last_check
OG
id|IDLE_CALC_LIMIT
)paren
(brace
id|use_apm_idle
op_assign
l_int|0
suffix:semicolon
id|last_jiffies
op_assign
id|jiffies
suffix:semicolon
id|last_stime
op_assign
id|current-&gt;stime
suffix:semicolon
)brace
r_else
r_if
c_cond
(paren
id|jiffies_since_last_check
OG
id|idle_period
)paren
(brace
r_int
r_int
id|idle_percentage
suffix:semicolon
id|idle_percentage
op_assign
id|current-&gt;stime
op_minus
id|last_stime
suffix:semicolon
id|idle_percentage
op_mul_assign
l_int|100
suffix:semicolon
id|idle_percentage
op_div_assign
id|jiffies_since_last_check
suffix:semicolon
id|use_apm_idle
op_assign
(paren
id|idle_percentage
OG
id|idle_threshold
)paren
suffix:semicolon
r_if
c_cond
(paren
id|apm_info.forbid_idle
)paren
id|use_apm_idle
op_assign
l_int|0
suffix:semicolon
id|last_jiffies
op_assign
id|jiffies
suffix:semicolon
id|last_stime
op_assign
id|current-&gt;stime
suffix:semicolon
)brace
id|bucket
op_assign
id|IDLE_LEAKY_MAX
suffix:semicolon
r_while
c_loop
(paren
op_logical_neg
id|need_resched
c_func
(paren
)paren
)paren
(brace
r_if
c_cond
(paren
id|use_apm_idle
)paren
(brace
r_int
r_int
id|t
suffix:semicolon
id|t
op_assign
id|jiffies
suffix:semicolon
r_switch
c_cond
(paren
id|apm_do_idle
c_func
(paren
)paren
)paren
(brace
r_case
l_int|0
suffix:colon
id|apm_idle_done
op_assign
l_int|1
suffix:semicolon
r_if
c_cond
(paren
id|t
op_ne
id|jiffies
)paren
(brace
r_if
c_cond
(paren
id|bucket
)paren
(brace
id|bucket
op_assign
id|IDLE_LEAKY_MAX
suffix:semicolon
r_continue
suffix:semicolon
)brace
)brace
r_else
r_if
c_cond
(paren
id|bucket
)paren
(brace
id|bucket
op_decrement
suffix:semicolon
r_continue
suffix:semicolon
)brace
r_break
suffix:semicolon
r_case
l_int|1
suffix:colon
id|apm_idle_done
op_assign
l_int|1
suffix:semicolon
r_break
suffix:semicolon
r_default
suffix:colon
multiline_comment|/* BIOS refused */
r_break
suffix:semicolon
)brace
)brace
r_if
c_cond
(paren
id|original_pm_idle
)paren
id|original_pm_idle
c_func
(paren
)paren
suffix:semicolon
r_else
id|default_idle
c_func
(paren
)paren
suffix:semicolon
id|jiffies_since_last_check
op_assign
id|jiffies
op_minus
id|last_jiffies
suffix:semicolon
r_if
c_cond
(paren
id|jiffies_since_last_check
OG
id|idle_period
)paren
r_goto
id|recalc
suffix:semicolon
)brace
r_if
c_cond
(paren
id|apm_idle_done
)paren
id|apm_do_busy
c_func
(paren
)paren
suffix:semicolon
)brace
multiline_comment|/**&n; *&t;apm_power_off&t;-&t;ask the BIOS to power off&n; *&n; *&t;Handle the power off sequence. This is the one piece of code we&n; *&t;will execute even on SMP machines. In order to deal with BIOS&n; *&t;bugs we support real mode APM BIOS power off calls. We also make&n; *&t;the SMP call on CPU0 as some systems will only honour this call&n; *&t;on their first cpu.&n; */
DECL|function|apm_power_off
r_static
r_void
id|apm_power_off
c_func
(paren
r_void
)paren
(brace
r_int
r_char
id|po_bios_call
(braket
)braket
op_assign
(brace
l_int|0xb8
comma
l_int|0x00
comma
l_int|0x10
comma
multiline_comment|/* movw  $0x1000,ax  */
l_int|0x8e
comma
l_int|0xd0
comma
multiline_comment|/* movw  ax,ss       */
l_int|0xbc
comma
l_int|0x00
comma
l_int|0xf0
comma
multiline_comment|/* movw  $0xf000,sp  */
l_int|0xb8
comma
l_int|0x07
comma
l_int|0x53
comma
multiline_comment|/* movw  $0x5307,ax  */
l_int|0xbb
comma
l_int|0x01
comma
l_int|0x00
comma
multiline_comment|/* movw  $0x0001,bx  */
l_int|0xb9
comma
l_int|0x03
comma
l_int|0x00
comma
multiline_comment|/* movw  $0x0003,cx  */
l_int|0xcd
comma
l_int|0x15
multiline_comment|/* int   $0x15       */
)brace
suffix:semicolon
multiline_comment|/*&n;&t; * This may be called on an SMP machine.&n;&t; */
macro_line|#ifdef CONFIG_SMP
multiline_comment|/* Some bioses don&squot;t like being called from CPU != 0 */
id|set_cpus_allowed
c_func
(paren
id|current
comma
id|cpumask_of_cpu
c_func
(paren
l_int|0
)paren
)paren
suffix:semicolon
id|BUG_ON
c_func
(paren
id|smp_processor_id
c_func
(paren
)paren
op_ne
l_int|0
)paren
suffix:semicolon
macro_line|#endif
r_if
c_cond
(paren
id|apm_info.realmode_power_off
)paren
(brace
(paren
r_void
)paren
id|apm_save_cpus
c_func
(paren
)paren
suffix:semicolon
id|machine_real_restart
c_func
(paren
id|po_bios_call
comma
r_sizeof
(paren
id|po_bios_call
)paren
)paren
suffix:semicolon
)brace
r_else
(paren
r_void
)paren
id|set_system_power_state
c_func
(paren
id|APM_STATE_OFF
)paren
suffix:semicolon
)brace
macro_line|#ifdef CONFIG_APM_DO_ENABLE
multiline_comment|/**&n; *&t;apm_enable_power_management - enable BIOS APM power management&n; *&t;@enable: enable yes/no&n; *&n; *&t;Enable or disable the APM BIOS power services. &n; */
DECL|function|apm_enable_power_management
r_static
r_int
id|apm_enable_power_management
c_func
(paren
r_int
id|enable
)paren
(brace
id|u32
id|eax
suffix:semicolon
r_if
c_cond
(paren
(paren
id|enable
op_eq
l_int|0
)paren
op_logical_and
(paren
id|apm_info.bios.flags
op_amp
id|APM_BIOS_DISENGAGED
)paren
)paren
r_return
id|APM_NOT_ENGAGED
suffix:semicolon
r_if
c_cond
(paren
id|apm_bios_call_simple
c_func
(paren
id|APM_FUNC_ENABLE_PM
comma
id|APM_DEVICE_BALL
comma
id|enable
comma
op_amp
id|eax
)paren
)paren
r_return
(paren
id|eax
op_rshift
l_int|8
)paren
op_amp
l_int|0xff
suffix:semicolon
r_if
c_cond
(paren
id|enable
)paren
id|apm_info.bios.flags
op_and_assign
op_complement
id|APM_BIOS_DISABLED
suffix:semicolon
r_else
id|apm_info.bios.flags
op_or_assign
id|APM_BIOS_DISABLED
suffix:semicolon
r_return
id|APM_SUCCESS
suffix:semicolon
)brace
macro_line|#endif
multiline_comment|/**&n; *&t;apm_get_power_status&t;-&t;get current power state&n; *&t;@status: returned status&n; *&t;@bat: battery info&n; *&t;@life: estimated life&n; *&n; *&t;Obtain the current power status from the APM BIOS. We return a&n; *&t;status which gives the rough battery status, and current power&n; *&t;source. The bat value returned give an estimate as a percentage&n; *&t;of life and a status value for the battery. The estimated life&n; *&t;if reported is a lifetime in secodnds/minutes at current powwer&n; *&t;consumption.&n; */
DECL|function|apm_get_power_status
r_static
r_int
id|apm_get_power_status
c_func
(paren
id|u_short
op_star
id|status
comma
id|u_short
op_star
id|bat
comma
id|u_short
op_star
id|life
)paren
(brace
id|u32
id|eax
suffix:semicolon
id|u32
id|ebx
suffix:semicolon
id|u32
id|ecx
suffix:semicolon
id|u32
id|edx
suffix:semicolon
id|u32
id|dummy
suffix:semicolon
r_if
c_cond
(paren
id|apm_info.get_power_status_broken
)paren
r_return
id|APM_32_UNSUPPORTED
suffix:semicolon
r_if
c_cond
(paren
id|apm_bios_call
c_func
(paren
id|APM_FUNC_GET_STATUS
comma
id|APM_DEVICE_ALL
comma
l_int|0
comma
op_amp
id|eax
comma
op_amp
id|ebx
comma
op_amp
id|ecx
comma
op_amp
id|edx
comma
op_amp
id|dummy
)paren
)paren
r_return
(paren
id|eax
op_rshift
l_int|8
)paren
op_amp
l_int|0xff
suffix:semicolon
op_star
id|status
op_assign
id|ebx
suffix:semicolon
op_star
id|bat
op_assign
id|ecx
suffix:semicolon
r_if
c_cond
(paren
id|apm_info.get_power_status_swabinminutes
)paren
(brace
op_star
id|life
op_assign
id|swab16
c_func
(paren
(paren
id|u16
)paren
id|edx
)paren
suffix:semicolon
op_star
id|life
op_or_assign
l_int|0x8000
suffix:semicolon
)brace
r_else
op_star
id|life
op_assign
id|edx
suffix:semicolon
r_return
id|APM_SUCCESS
suffix:semicolon
)brace
macro_line|#if 0
r_static
r_int
id|apm_get_battery_status
c_func
(paren
id|u_short
id|which
comma
id|u_short
op_star
id|status
comma
id|u_short
op_star
id|bat
comma
id|u_short
op_star
id|life
comma
id|u_short
op_star
id|nbat
)paren
(brace
id|u32
id|eax
suffix:semicolon
id|u32
id|ebx
suffix:semicolon
id|u32
id|ecx
suffix:semicolon
id|u32
id|edx
suffix:semicolon
id|u32
id|esi
suffix:semicolon
r_if
c_cond
(paren
id|apm_info.connection_version
OL
l_int|0x0102
)paren
(brace
multiline_comment|/* pretend we only have one battery. */
r_if
c_cond
(paren
id|which
op_ne
l_int|1
)paren
r_return
id|APM_BAD_DEVICE
suffix:semicolon
op_star
id|nbat
op_assign
l_int|1
suffix:semicolon
r_return
id|apm_get_power_status
c_func
(paren
id|status
comma
id|bat
comma
id|life
)paren
suffix:semicolon
)brace
r_if
c_cond
(paren
id|apm_bios_call
c_func
(paren
id|APM_FUNC_GET_STATUS
comma
(paren
l_int|0x8000
op_or
(paren
id|which
)paren
)paren
comma
l_int|0
comma
op_amp
id|eax
comma
op_amp
id|ebx
comma
op_amp
id|ecx
comma
op_amp
id|edx
comma
op_amp
id|esi
)paren
)paren
r_return
(paren
id|eax
op_rshift
l_int|8
)paren
op_amp
l_int|0xff
suffix:semicolon
op_star
id|status
op_assign
id|ebx
suffix:semicolon
op_star
id|bat
op_assign
id|ecx
suffix:semicolon
op_star
id|life
op_assign
id|edx
suffix:semicolon
op_star
id|nbat
op_assign
id|esi
suffix:semicolon
r_return
id|APM_SUCCESS
suffix:semicolon
)brace
macro_line|#endif
multiline_comment|/**&n; *&t;apm_engage_power_management&t;-&t;enable PM on a device&n; *&t;@device: identity of device&n; *&t;@enable: on/off&n; *&n; *&t;Activate or deactive power management on either a specific device&n; *&t;or the entire system (%APM_DEVICE_ALL).&n; */
DECL|function|apm_engage_power_management
r_static
r_int
id|apm_engage_power_management
c_func
(paren
id|u_short
id|device
comma
r_int
id|enable
)paren
(brace
id|u32
id|eax
suffix:semicolon
r_if
c_cond
(paren
(paren
id|enable
op_eq
l_int|0
)paren
op_logical_and
(paren
id|device
op_eq
id|APM_DEVICE_ALL
)paren
op_logical_and
(paren
id|apm_info.bios.flags
op_amp
id|APM_BIOS_DISABLED
)paren
)paren
r_return
id|APM_DISABLED
suffix:semicolon
r_if
c_cond
(paren
id|apm_bios_call_simple
c_func
(paren
id|APM_FUNC_ENGAGE_PM
comma
id|device
comma
id|enable
comma
op_amp
id|eax
)paren
)paren
r_return
(paren
id|eax
op_rshift
l_int|8
)paren
op_amp
l_int|0xff
suffix:semicolon
r_if
c_cond
(paren
id|device
op_eq
id|APM_DEVICE_ALL
)paren
(brace
r_if
c_cond
(paren
id|enable
)paren
id|apm_info.bios.flags
op_and_assign
op_complement
id|APM_BIOS_DISENGAGED
suffix:semicolon
r_else
id|apm_info.bios.flags
op_or_assign
id|APM_BIOS_DISENGAGED
suffix:semicolon
)brace
r_return
id|APM_SUCCESS
suffix:semicolon
)brace
macro_line|#if defined(CONFIG_APM_DISPLAY_BLANK) &amp;&amp; defined(CONFIG_VT)
multiline_comment|/**&n; *&t;apm_console_blank&t;-&t;blank the display&n; *&t;@blank: on/off&n; *&n; *&t;Attempt to blank the console, firstly by blanking just video device&n; *&t;zero, and if that fails (some BIOSes don&squot;t support it) then it blanks&n; *&t;all video devices. Typically the BIOS will do laptop backlight and&n; *&t;monitor powerdown for us.&n; */
DECL|function|apm_console_blank
r_static
r_int
id|apm_console_blank
c_func
(paren
r_int
id|blank
)paren
(brace
r_int
id|error
suffix:semicolon
id|u_short
id|state
suffix:semicolon
id|state
op_assign
id|blank
ques
c_cond
id|APM_STATE_STANDBY
suffix:colon
id|APM_STATE_READY
suffix:semicolon
multiline_comment|/* Blank the first display device */
id|error
op_assign
id|set_power_state
c_func
(paren
l_int|0x100
comma
id|state
)paren
suffix:semicolon
r_if
c_cond
(paren
(paren
id|error
op_ne
id|APM_SUCCESS
)paren
op_logical_and
(paren
id|error
op_ne
id|APM_NO_ERROR
)paren
)paren
(brace
multiline_comment|/* try to blank them all instead */
id|error
op_assign
id|set_power_state
c_func
(paren
l_int|0x1ff
comma
id|state
)paren
suffix:semicolon
r_if
c_cond
(paren
(paren
id|error
op_ne
id|APM_SUCCESS
)paren
op_logical_and
(paren
id|error
op_ne
id|APM_NO_ERROR
)paren
)paren
multiline_comment|/* try to blank device one instead */
id|error
op_assign
id|set_power_state
c_func
(paren
l_int|0x101
comma
id|state
)paren
suffix:semicolon
)brace
r_if
c_cond
(paren
(paren
id|error
op_eq
id|APM_SUCCESS
)paren
op_logical_or
(paren
id|error
op_eq
id|APM_NO_ERROR
)paren
)paren
r_return
l_int|1
suffix:semicolon
r_if
c_cond
(paren
id|error
op_eq
id|APM_NOT_ENGAGED
)paren
(brace
r_static
r_int
id|tried
suffix:semicolon
r_int
id|eng_error
suffix:semicolon
r_if
c_cond
(paren
id|tried
op_increment
op_eq
l_int|0
)paren
(brace
id|eng_error
op_assign
id|apm_engage_power_management
c_func
(paren
id|APM_DEVICE_ALL
comma
l_int|1
)paren
suffix:semicolon
r_if
c_cond
(paren
id|eng_error
)paren
(brace
id|apm_error
c_func
(paren
l_string|&quot;set display&quot;
comma
id|error
)paren
suffix:semicolon
id|apm_error
c_func
(paren
l_string|&quot;engage interface&quot;
comma
id|eng_error
)paren
suffix:semicolon
r_return
l_int|0
suffix:semicolon
)brace
r_else
r_return
id|apm_console_blank
c_func
(paren
id|blank
)paren
suffix:semicolon
)brace
)brace
id|apm_error
c_func
(paren
l_string|&quot;set display&quot;
comma
id|error
)paren
suffix:semicolon
r_return
l_int|0
suffix:semicolon
)brace
macro_line|#endif
DECL|function|queue_empty
r_static
r_int
id|queue_empty
c_func
(paren
r_struct
id|apm_user
op_star
id|as
)paren
(brace
r_return
id|as-&gt;event_head
op_eq
id|as-&gt;event_tail
suffix:semicolon
)brace
DECL|function|get_queued_event
r_static
id|apm_event_t
id|get_queued_event
c_func
(paren
r_struct
id|apm_user
op_star
id|as
)paren
(brace
id|as-&gt;event_tail
op_assign
(paren
id|as-&gt;event_tail
op_plus
l_int|1
)paren
op_mod
id|APM_MAX_EVENTS
suffix:semicolon
r_return
id|as-&gt;events
(braket
id|as-&gt;event_tail
)braket
suffix:semicolon
)brace
DECL|function|queue_event
r_static
r_void
id|queue_event
c_func
(paren
id|apm_event_t
id|event
comma
r_struct
id|apm_user
op_star
id|sender
)paren
(brace
r_struct
id|apm_user
op_star
id|as
suffix:semicolon
id|spin_lock
c_func
(paren
op_amp
id|user_list_lock
)paren
suffix:semicolon
r_if
c_cond
(paren
id|user_list
op_eq
l_int|NULL
)paren
r_goto
id|out
suffix:semicolon
r_for
c_loop
(paren
id|as
op_assign
id|user_list
suffix:semicolon
id|as
op_ne
l_int|NULL
suffix:semicolon
id|as
op_assign
id|as-&gt;next
)paren
(brace
r_if
c_cond
(paren
(paren
id|as
op_eq
id|sender
)paren
op_logical_or
(paren
op_logical_neg
id|as-&gt;reader
)paren
)paren
r_continue
suffix:semicolon
id|as-&gt;event_head
op_assign
(paren
id|as-&gt;event_head
op_plus
l_int|1
)paren
op_mod
id|APM_MAX_EVENTS
suffix:semicolon
r_if
c_cond
(paren
id|as-&gt;event_head
op_eq
id|as-&gt;event_tail
)paren
(brace
r_static
r_int
id|notified
suffix:semicolon
r_if
c_cond
(paren
id|notified
op_increment
op_eq
l_int|0
)paren
id|printk
c_func
(paren
id|KERN_ERR
l_string|&quot;apm: an event queue overflowed&bslash;n&quot;
)paren
suffix:semicolon
id|as-&gt;event_tail
op_assign
(paren
id|as-&gt;event_tail
op_plus
l_int|1
)paren
op_mod
id|APM_MAX_EVENTS
suffix:semicolon
)brace
id|as-&gt;events
(braket
id|as-&gt;event_head
)braket
op_assign
id|event
suffix:semicolon
r_if
c_cond
(paren
(paren
op_logical_neg
id|as-&gt;suser
)paren
op_logical_or
(paren
op_logical_neg
id|as-&gt;writer
)paren
)paren
r_continue
suffix:semicolon
r_switch
c_cond
(paren
id|event
)paren
(brace
r_case
id|APM_SYS_SUSPEND
suffix:colon
r_case
id|APM_USER_SUSPEND
suffix:colon
id|as-&gt;suspends_pending
op_increment
suffix:semicolon
id|suspends_pending
op_increment
suffix:semicolon
r_break
suffix:semicolon
r_case
id|APM_SYS_STANDBY
suffix:colon
r_case
id|APM_USER_STANDBY
suffix:colon
id|as-&gt;standbys_pending
op_increment
suffix:semicolon
id|standbys_pending
op_increment
suffix:semicolon
r_break
suffix:semicolon
)brace
)brace
id|wake_up_interruptible
c_func
(paren
op_amp
id|apm_waitqueue
)paren
suffix:semicolon
id|out
suffix:colon
id|spin_unlock
c_func
(paren
op_amp
id|user_list_lock
)paren
suffix:semicolon
)brace
DECL|function|set_time
r_static
r_void
id|set_time
c_func
(paren
r_void
)paren
(brace
r_if
c_cond
(paren
id|got_clock_diff
)paren
(brace
multiline_comment|/* Must know time zone in order to set clock */
id|xtime.tv_sec
op_assign
id|get_cmos_time
c_func
(paren
)paren
op_plus
id|clock_cmos_diff
suffix:semicolon
id|xtime.tv_nsec
op_assign
l_int|0
suffix:semicolon
)brace
)brace
DECL|function|get_time_diff
r_static
r_void
id|get_time_diff
c_func
(paren
r_void
)paren
(brace
macro_line|#ifndef CONFIG_APM_RTC_IS_GMT
multiline_comment|/*&n;&t; * Estimate time zone so that set_time can update the clock&n;&t; */
id|clock_cmos_diff
op_assign
op_minus
id|get_cmos_time
c_func
(paren
)paren
suffix:semicolon
id|clock_cmos_diff
op_add_assign
id|get_seconds
c_func
(paren
)paren
suffix:semicolon
id|got_clock_diff
op_assign
l_int|1
suffix:semicolon
macro_line|#endif
)brace
DECL|function|reinit_timer
r_static
r_void
id|reinit_timer
c_func
(paren
r_void
)paren
(brace
macro_line|#ifdef INIT_TIMER_AFTER_SUSPEND
r_int
r_int
id|flags
suffix:semicolon
r_extern
id|spinlock_t
id|i8253_lock
suffix:semicolon
id|spin_lock_irqsave
c_func
(paren
op_amp
id|i8253_lock
comma
id|flags
)paren
suffix:semicolon
multiline_comment|/* set the clock to 100 Hz */
id|outb_p
c_func
(paren
l_int|0x34
comma
id|PIT_MODE
)paren
suffix:semicolon
multiline_comment|/* binary, mode 2, LSB/MSB, ch 0 */
id|udelay
c_func
(paren
l_int|10
)paren
suffix:semicolon
id|outb_p
c_func
(paren
id|LATCH
op_amp
l_int|0xff
comma
id|PIT_CH0
)paren
suffix:semicolon
multiline_comment|/* LSB */
id|udelay
c_func
(paren
l_int|10
)paren
suffix:semicolon
id|outb
c_func
(paren
id|LATCH
op_rshift
l_int|8
comma
id|PIT_CH0
)paren
suffix:semicolon
multiline_comment|/* MSB */
id|udelay
c_func
(paren
l_int|10
)paren
suffix:semicolon
id|spin_unlock_irqrestore
c_func
(paren
op_amp
id|i8253_lock
comma
id|flags
)paren
suffix:semicolon
macro_line|#endif
)brace
DECL|function|suspend
r_static
r_int
id|suspend
c_func
(paren
r_int
id|vetoable
)paren
(brace
r_int
id|err
suffix:semicolon
r_struct
id|apm_user
op_star
id|as
suffix:semicolon
r_if
c_cond
(paren
id|pm_send_all
c_func
(paren
id|PM_SUSPEND
comma
(paren
r_void
op_star
)paren
l_int|3
)paren
)paren
(brace
multiline_comment|/* Vetoed */
r_if
c_cond
(paren
id|vetoable
)paren
(brace
r_if
c_cond
(paren
id|apm_info.connection_version
OG
l_int|0x100
)paren
id|set_system_power_state
c_func
(paren
id|APM_STATE_REJECT
)paren
suffix:semicolon
id|err
op_assign
op_minus
id|EBUSY
suffix:semicolon
id|ignore_sys_suspend
op_assign
l_int|0
suffix:semicolon
id|printk
c_func
(paren
id|KERN_WARNING
l_string|&quot;apm: suspend was vetoed.&bslash;n&quot;
)paren
suffix:semicolon
r_goto
id|out
suffix:semicolon
)brace
id|printk
c_func
(paren
id|KERN_CRIT
l_string|&quot;apm: suspend was vetoed, but suspending anyway.&bslash;n&quot;
)paren
suffix:semicolon
)brace
id|device_suspend
c_func
(paren
id|PMSG_SUSPEND
)paren
suffix:semicolon
id|device_power_down
c_func
(paren
id|PMSG_SUSPEND
)paren
suffix:semicolon
multiline_comment|/* serialize with the timer interrupt */
id|write_seqlock_irq
c_func
(paren
op_amp
id|xtime_lock
)paren
suffix:semicolon
multiline_comment|/* protect against access to timer chip registers */
id|spin_lock
c_func
(paren
op_amp
id|i8253_lock
)paren
suffix:semicolon
id|get_time_diff
c_func
(paren
)paren
suffix:semicolon
multiline_comment|/*&n;&t; * Irq spinlock must be dropped around set_system_power_state.&n;&t; * We&squot;ll undo any timer changes due to interrupts below.&n;&t; */
id|spin_unlock
c_func
(paren
op_amp
id|i8253_lock
)paren
suffix:semicolon
id|write_sequnlock_irq
c_func
(paren
op_amp
id|xtime_lock
)paren
suffix:semicolon
id|save_processor_state
c_func
(paren
)paren
suffix:semicolon
id|err
op_assign
id|set_system_power_state
c_func
(paren
id|APM_STATE_SUSPEND
)paren
suffix:semicolon
id|restore_processor_state
c_func
(paren
)paren
suffix:semicolon
id|write_seqlock_irq
c_func
(paren
op_amp
id|xtime_lock
)paren
suffix:semicolon
id|spin_lock
c_func
(paren
op_amp
id|i8253_lock
)paren
suffix:semicolon
id|reinit_timer
c_func
(paren
)paren
suffix:semicolon
id|set_time
c_func
(paren
)paren
suffix:semicolon
id|ignore_normal_resume
op_assign
l_int|1
suffix:semicolon
id|spin_unlock
c_func
(paren
op_amp
id|i8253_lock
)paren
suffix:semicolon
id|write_sequnlock_irq
c_func
(paren
op_amp
id|xtime_lock
)paren
suffix:semicolon
r_if
c_cond
(paren
id|err
op_eq
id|APM_NO_ERROR
)paren
id|err
op_assign
id|APM_SUCCESS
suffix:semicolon
r_if
c_cond
(paren
id|err
op_ne
id|APM_SUCCESS
)paren
id|apm_error
c_func
(paren
l_string|&quot;suspend&quot;
comma
id|err
)paren
suffix:semicolon
id|err
op_assign
(paren
id|err
op_eq
id|APM_SUCCESS
)paren
ques
c_cond
l_int|0
suffix:colon
op_minus
id|EIO
suffix:semicolon
id|device_power_up
c_func
(paren
)paren
suffix:semicolon
id|device_resume
c_func
(paren
)paren
suffix:semicolon
id|pm_send_all
c_func
(paren
id|PM_RESUME
comma
(paren
r_void
op_star
)paren
l_int|0
)paren
suffix:semicolon
id|queue_event
c_func
(paren
id|APM_NORMAL_RESUME
comma
l_int|NULL
)paren
suffix:semicolon
id|out
suffix:colon
id|spin_lock
c_func
(paren
op_amp
id|user_list_lock
)paren
suffix:semicolon
r_for
c_loop
(paren
id|as
op_assign
id|user_list
suffix:semicolon
id|as
op_ne
l_int|NULL
suffix:semicolon
id|as
op_assign
id|as-&gt;next
)paren
(brace
id|as-&gt;suspend_wait
op_assign
l_int|0
suffix:semicolon
id|as-&gt;suspend_result
op_assign
id|err
suffix:semicolon
)brace
id|spin_unlock
c_func
(paren
op_amp
id|user_list_lock
)paren
suffix:semicolon
id|wake_up_interruptible
c_func
(paren
op_amp
id|apm_suspend_waitqueue
)paren
suffix:semicolon
r_return
id|err
suffix:semicolon
)brace
DECL|function|standby
r_static
r_void
id|standby
c_func
(paren
r_void
)paren
(brace
r_int
id|err
suffix:semicolon
id|device_power_down
c_func
(paren
id|PMSG_SUSPEND
)paren
suffix:semicolon
multiline_comment|/* serialize with the timer interrupt */
id|write_seqlock_irq
c_func
(paren
op_amp
id|xtime_lock
)paren
suffix:semicolon
multiline_comment|/* If needed, notify drivers here */
id|get_time_diff
c_func
(paren
)paren
suffix:semicolon
id|write_sequnlock_irq
c_func
(paren
op_amp
id|xtime_lock
)paren
suffix:semicolon
id|err
op_assign
id|set_system_power_state
c_func
(paren
id|APM_STATE_STANDBY
)paren
suffix:semicolon
r_if
c_cond
(paren
(paren
id|err
op_ne
id|APM_SUCCESS
)paren
op_logical_and
(paren
id|err
op_ne
id|APM_NO_ERROR
)paren
)paren
id|apm_error
c_func
(paren
l_string|&quot;standby&quot;
comma
id|err
)paren
suffix:semicolon
id|device_power_up
c_func
(paren
)paren
suffix:semicolon
)brace
DECL|function|get_event
r_static
id|apm_event_t
id|get_event
c_func
(paren
r_void
)paren
(brace
r_int
id|error
suffix:semicolon
id|apm_event_t
id|event
suffix:semicolon
id|apm_eventinfo_t
id|info
suffix:semicolon
r_static
r_int
id|notified
suffix:semicolon
multiline_comment|/* we don&squot;t use the eventinfo */
id|error
op_assign
id|apm_get_event
c_func
(paren
op_amp
id|event
comma
op_amp
id|info
)paren
suffix:semicolon
r_if
c_cond
(paren
id|error
op_eq
id|APM_SUCCESS
)paren
r_return
id|event
suffix:semicolon
r_if
c_cond
(paren
(paren
id|error
op_ne
id|APM_NO_EVENTS
)paren
op_logical_and
(paren
id|notified
op_increment
op_eq
l_int|0
)paren
)paren
id|apm_error
c_func
(paren
l_string|&quot;get_event&quot;
comma
id|error
)paren
suffix:semicolon
r_return
l_int|0
suffix:semicolon
)brace
DECL|function|check_events
r_static
r_void
id|check_events
c_func
(paren
r_void
)paren
(brace
id|apm_event_t
id|event
suffix:semicolon
r_static
r_int
r_int
id|last_resume
suffix:semicolon
r_static
r_int
id|ignore_bounce
suffix:semicolon
r_while
c_loop
(paren
(paren
id|event
op_assign
id|get_event
c_func
(paren
)paren
)paren
op_ne
l_int|0
)paren
(brace
r_if
c_cond
(paren
id|debug
)paren
(brace
r_if
c_cond
(paren
id|event
op_le
id|NR_APM_EVENT_NAME
)paren
id|printk
c_func
(paren
id|KERN_DEBUG
l_string|&quot;apm: received %s notify&bslash;n&quot;
comma
id|apm_event_name
(braket
id|event
op_minus
l_int|1
)braket
)paren
suffix:semicolon
r_else
id|printk
c_func
(paren
id|KERN_DEBUG
l_string|&quot;apm: received unknown &quot;
l_string|&quot;event 0x%02x&bslash;n&quot;
comma
id|event
)paren
suffix:semicolon
)brace
r_if
c_cond
(paren
id|ignore_bounce
op_logical_and
(paren
(paren
id|jiffies
op_minus
id|last_resume
)paren
OG
id|bounce_interval
)paren
)paren
id|ignore_bounce
op_assign
l_int|0
suffix:semicolon
r_switch
c_cond
(paren
id|event
)paren
(brace
r_case
id|APM_SYS_STANDBY
suffix:colon
r_case
id|APM_USER_STANDBY
suffix:colon
id|queue_event
c_func
(paren
id|event
comma
l_int|NULL
)paren
suffix:semicolon
r_if
c_cond
(paren
id|standbys_pending
op_le
l_int|0
)paren
id|standby
c_func
(paren
)paren
suffix:semicolon
r_break
suffix:semicolon
r_case
id|APM_USER_SUSPEND
suffix:colon
macro_line|#ifdef CONFIG_APM_IGNORE_USER_SUSPEND
r_if
c_cond
(paren
id|apm_info.connection_version
OG
l_int|0x100
)paren
id|set_system_power_state
c_func
(paren
id|APM_STATE_REJECT
)paren
suffix:semicolon
r_break
suffix:semicolon
macro_line|#endif
r_case
id|APM_SYS_SUSPEND
suffix:colon
r_if
c_cond
(paren
id|ignore_bounce
)paren
(brace
r_if
c_cond
(paren
id|apm_info.connection_version
OG
l_int|0x100
)paren
id|set_system_power_state
c_func
(paren
id|APM_STATE_REJECT
)paren
suffix:semicolon
r_break
suffix:semicolon
)brace
multiline_comment|/*&n;&t;&t;&t; * If we are already processing a SUSPEND,&n;&t;&t;&t; * then further SUSPEND events from the BIOS&n;&t;&t;&t; * will be ignored.  We also return here to&n;&t;&t;&t; * cope with the fact that the Thinkpads keep&n;&t;&t;&t; * sending a SUSPEND event until something else&n;&t;&t;&t; * happens!&n;&t;&t;&t; */
r_if
c_cond
(paren
id|ignore_sys_suspend
)paren
r_return
suffix:semicolon
id|ignore_sys_suspend
op_assign
l_int|1
suffix:semicolon
id|queue_event
c_func
(paren
id|event
comma
l_int|NULL
)paren
suffix:semicolon
r_if
c_cond
(paren
id|suspends_pending
op_le
l_int|0
)paren
(paren
r_void
)paren
id|suspend
c_func
(paren
l_int|1
)paren
suffix:semicolon
r_break
suffix:semicolon
r_case
id|APM_NORMAL_RESUME
suffix:colon
r_case
id|APM_CRITICAL_RESUME
suffix:colon
r_case
id|APM_STANDBY_RESUME
suffix:colon
id|ignore_sys_suspend
op_assign
l_int|0
suffix:semicolon
id|last_resume
op_assign
id|jiffies
suffix:semicolon
id|ignore_bounce
op_assign
l_int|1
suffix:semicolon
r_if
c_cond
(paren
(paren
id|event
op_ne
id|APM_NORMAL_RESUME
)paren
op_logical_or
(paren
id|ignore_normal_resume
op_eq
l_int|0
)paren
)paren
(brace
id|write_seqlock_irq
c_func
(paren
op_amp
id|xtime_lock
)paren
suffix:semicolon
id|set_time
c_func
(paren
)paren
suffix:semicolon
id|write_sequnlock_irq
c_func
(paren
op_amp
id|xtime_lock
)paren
suffix:semicolon
id|device_resume
c_func
(paren
)paren
suffix:semicolon
id|pm_send_all
c_func
(paren
id|PM_RESUME
comma
(paren
r_void
op_star
)paren
l_int|0
)paren
suffix:semicolon
id|queue_event
c_func
(paren
id|event
comma
l_int|NULL
)paren
suffix:semicolon
)brace
id|ignore_normal_resume
op_assign
l_int|0
suffix:semicolon
r_break
suffix:semicolon
r_case
id|APM_CAPABILITY_CHANGE
suffix:colon
r_case
id|APM_LOW_BATTERY
suffix:colon
r_case
id|APM_POWER_STATUS_CHANGE
suffix:colon
id|queue_event
c_func
(paren
id|event
comma
l_int|NULL
)paren
suffix:semicolon
multiline_comment|/* If needed, notify drivers here */
r_break
suffix:semicolon
r_case
id|APM_UPDATE_TIME
suffix:colon
id|write_seqlock_irq
c_func
(paren
op_amp
id|xtime_lock
)paren
suffix:semicolon
id|set_time
c_func
(paren
)paren
suffix:semicolon
id|write_sequnlock_irq
c_func
(paren
op_amp
id|xtime_lock
)paren
suffix:semicolon
r_break
suffix:semicolon
r_case
id|APM_CRITICAL_SUSPEND
suffix:colon
multiline_comment|/*&n;&t;&t;&t; * We are not allowed to reject a critical suspend.&n;&t;&t;&t; */
(paren
r_void
)paren
id|suspend
c_func
(paren
l_int|0
)paren
suffix:semicolon
r_break
suffix:semicolon
)brace
)brace
)brace
DECL|function|apm_event_handler
r_static
r_void
id|apm_event_handler
c_func
(paren
r_void
)paren
(brace
r_static
r_int
id|pending_count
op_assign
l_int|4
suffix:semicolon
r_int
id|err
suffix:semicolon
r_if
c_cond
(paren
(paren
id|standbys_pending
OG
l_int|0
)paren
op_logical_or
(paren
id|suspends_pending
OG
l_int|0
)paren
)paren
(brace
r_if
c_cond
(paren
(paren
id|apm_info.connection_version
OG
l_int|0x100
)paren
op_logical_and
(paren
id|pending_count
op_decrement
op_le
l_int|0
)paren
)paren
(brace
id|pending_count
op_assign
l_int|4
suffix:semicolon
r_if
c_cond
(paren
id|debug
)paren
id|printk
c_func
(paren
id|KERN_DEBUG
l_string|&quot;apm: setting state busy&bslash;n&quot;
)paren
suffix:semicolon
id|err
op_assign
id|set_system_power_state
c_func
(paren
id|APM_STATE_BUSY
)paren
suffix:semicolon
r_if
c_cond
(paren
id|err
)paren
id|apm_error
c_func
(paren
l_string|&quot;busy&quot;
comma
id|err
)paren
suffix:semicolon
)brace
)brace
r_else
id|pending_count
op_assign
l_int|4
suffix:semicolon
id|check_events
c_func
(paren
)paren
suffix:semicolon
)brace
multiline_comment|/*&n; * This is the APM thread main loop.&n; */
DECL|function|apm_mainloop
r_static
r_void
id|apm_mainloop
c_func
(paren
r_void
)paren
(brace
id|DECLARE_WAITQUEUE
c_func
(paren
id|wait
comma
id|current
)paren
suffix:semicolon
id|add_wait_queue
c_func
(paren
op_amp
id|apm_waitqueue
comma
op_amp
id|wait
)paren
suffix:semicolon
id|set_current_state
c_func
(paren
id|TASK_INTERRUPTIBLE
)paren
suffix:semicolon
r_for
c_loop
(paren
suffix:semicolon
suffix:semicolon
)paren
(brace
id|schedule_timeout
c_func
(paren
id|APM_CHECK_TIMEOUT
)paren
suffix:semicolon
r_if
c_cond
(paren
id|exit_kapmd
)paren
r_break
suffix:semicolon
multiline_comment|/*&n;&t;&t; * Ok, check all events, check for idle (and mark us sleeping&n;&t;&t; * so as not to count towards the load average)..&n;&t;&t; */
id|set_current_state
c_func
(paren
id|TASK_INTERRUPTIBLE
)paren
suffix:semicolon
id|apm_event_handler
c_func
(paren
)paren
suffix:semicolon
)brace
id|remove_wait_queue
c_func
(paren
op_amp
id|apm_waitqueue
comma
op_amp
id|wait
)paren
suffix:semicolon
)brace
DECL|function|check_apm_user
r_static
r_int
id|check_apm_user
c_func
(paren
r_struct
id|apm_user
op_star
id|as
comma
r_const
r_char
op_star
id|func
)paren
(brace
r_if
c_cond
(paren
(paren
id|as
op_eq
l_int|NULL
)paren
op_logical_or
(paren
id|as-&gt;magic
op_ne
id|APM_BIOS_MAGIC
)paren
)paren
(brace
id|printk
c_func
(paren
id|KERN_ERR
l_string|&quot;apm: %s passed bad filp&bslash;n&quot;
comma
id|func
)paren
suffix:semicolon
r_return
l_int|1
suffix:semicolon
)brace
r_return
l_int|0
suffix:semicolon
)brace
DECL|function|do_read
r_static
id|ssize_t
id|do_read
c_func
(paren
r_struct
id|file
op_star
id|fp
comma
r_char
id|__user
op_star
id|buf
comma
r_int
id|count
comma
id|loff_t
op_star
id|ppos
)paren
(brace
r_struct
id|apm_user
op_star
id|as
suffix:semicolon
r_int
id|i
suffix:semicolon
id|apm_event_t
id|event
suffix:semicolon
id|as
op_assign
id|fp-&gt;private_data
suffix:semicolon
r_if
c_cond
(paren
id|check_apm_user
c_func
(paren
id|as
comma
l_string|&quot;read&quot;
)paren
)paren
r_return
op_minus
id|EIO
suffix:semicolon
r_if
c_cond
(paren
(paren
r_int
)paren
id|count
OL
r_sizeof
(paren
id|apm_event_t
)paren
)paren
r_return
op_minus
id|EINVAL
suffix:semicolon
r_if
c_cond
(paren
(paren
id|queue_empty
c_func
(paren
id|as
)paren
)paren
op_logical_and
(paren
id|fp-&gt;f_flags
op_amp
id|O_NONBLOCK
)paren
)paren
r_return
op_minus
id|EAGAIN
suffix:semicolon
id|wait_event_interruptible
c_func
(paren
id|apm_waitqueue
comma
op_logical_neg
id|queue_empty
c_func
(paren
id|as
)paren
)paren
suffix:semicolon
id|i
op_assign
id|count
suffix:semicolon
r_while
c_loop
(paren
(paren
id|i
op_ge
r_sizeof
(paren
id|event
)paren
)paren
op_logical_and
op_logical_neg
id|queue_empty
c_func
(paren
id|as
)paren
)paren
(brace
id|event
op_assign
id|get_queued_event
c_func
(paren
id|as
)paren
suffix:semicolon
r_if
c_cond
(paren
id|copy_to_user
c_func
(paren
id|buf
comma
op_amp
id|event
comma
r_sizeof
(paren
id|event
)paren
)paren
)paren
(brace
r_if
c_cond
(paren
id|i
OL
id|count
)paren
r_break
suffix:semicolon
r_return
op_minus
id|EFAULT
suffix:semicolon
)brace
r_switch
c_cond
(paren
id|event
)paren
(brace
r_case
id|APM_SYS_SUSPEND
suffix:colon
r_case
id|APM_USER_SUSPEND
suffix:colon
id|as-&gt;suspends_read
op_increment
suffix:semicolon
r_break
suffix:semicolon
r_case
id|APM_SYS_STANDBY
suffix:colon
r_case
id|APM_USER_STANDBY
suffix:colon
id|as-&gt;standbys_read
op_increment
suffix:semicolon
r_break
suffix:semicolon
)brace
id|buf
op_add_assign
r_sizeof
(paren
id|event
)paren
suffix:semicolon
id|i
op_sub_assign
r_sizeof
(paren
id|event
)paren
suffix:semicolon
)brace
r_if
c_cond
(paren
id|i
OL
id|count
)paren
r_return
id|count
op_minus
id|i
suffix:semicolon
r_if
c_cond
(paren
id|signal_pending
c_func
(paren
id|current
)paren
)paren
r_return
op_minus
id|ERESTARTSYS
suffix:semicolon
r_return
l_int|0
suffix:semicolon
)brace
DECL|function|do_poll
r_static
r_int
r_int
id|do_poll
c_func
(paren
r_struct
id|file
op_star
id|fp
comma
id|poll_table
op_star
id|wait
)paren
(brace
r_struct
id|apm_user
op_star
id|as
suffix:semicolon
id|as
op_assign
id|fp-&gt;private_data
suffix:semicolon
r_if
c_cond
(paren
id|check_apm_user
c_func
(paren
id|as
comma
l_string|&quot;poll&quot;
)paren
)paren
r_return
l_int|0
suffix:semicolon
id|poll_wait
c_func
(paren
id|fp
comma
op_amp
id|apm_waitqueue
comma
id|wait
)paren
suffix:semicolon
r_if
c_cond
(paren
op_logical_neg
id|queue_empty
c_func
(paren
id|as
)paren
)paren
r_return
id|POLLIN
op_or
id|POLLRDNORM
suffix:semicolon
r_return
l_int|0
suffix:semicolon
)brace
DECL|function|do_ioctl
r_static
r_int
id|do_ioctl
c_func
(paren
r_struct
id|inode
op_star
id|inode
comma
r_struct
id|file
op_star
id|filp
comma
id|u_int
id|cmd
comma
id|u_long
id|arg
)paren
(brace
r_struct
id|apm_user
op_star
id|as
suffix:semicolon
id|as
op_assign
id|filp-&gt;private_data
suffix:semicolon
r_if
c_cond
(paren
id|check_apm_user
c_func
(paren
id|as
comma
l_string|&quot;ioctl&quot;
)paren
)paren
r_return
op_minus
id|EIO
suffix:semicolon
r_if
c_cond
(paren
(paren
op_logical_neg
id|as-&gt;suser
)paren
op_logical_or
(paren
op_logical_neg
id|as-&gt;writer
)paren
)paren
r_return
op_minus
id|EPERM
suffix:semicolon
r_switch
c_cond
(paren
id|cmd
)paren
(brace
r_case
id|APM_IOC_STANDBY
suffix:colon
r_if
c_cond
(paren
id|as-&gt;standbys_read
OG
l_int|0
)paren
(brace
id|as-&gt;standbys_read
op_decrement
suffix:semicolon
id|as-&gt;standbys_pending
op_decrement
suffix:semicolon
id|standbys_pending
op_decrement
suffix:semicolon
)brace
r_else
id|queue_event
c_func
(paren
id|APM_USER_STANDBY
comma
id|as
)paren
suffix:semicolon
r_if
c_cond
(paren
id|standbys_pending
op_le
l_int|0
)paren
id|standby
c_func
(paren
)paren
suffix:semicolon
r_break
suffix:semicolon
r_case
id|APM_IOC_SUSPEND
suffix:colon
r_if
c_cond
(paren
id|as-&gt;suspends_read
OG
l_int|0
)paren
(brace
id|as-&gt;suspends_read
op_decrement
suffix:semicolon
id|as-&gt;suspends_pending
op_decrement
suffix:semicolon
id|suspends_pending
op_decrement
suffix:semicolon
)brace
r_else
id|queue_event
c_func
(paren
id|APM_USER_SUSPEND
comma
id|as
)paren
suffix:semicolon
r_if
c_cond
(paren
id|suspends_pending
op_le
l_int|0
)paren
(brace
r_return
id|suspend
c_func
(paren
l_int|1
)paren
suffix:semicolon
)brace
r_else
(brace
id|as-&gt;suspend_wait
op_assign
l_int|1
suffix:semicolon
id|wait_event_interruptible
c_func
(paren
id|apm_suspend_waitqueue
comma
id|as-&gt;suspend_wait
op_eq
l_int|0
)paren
suffix:semicolon
r_return
id|as-&gt;suspend_result
suffix:semicolon
)brace
r_break
suffix:semicolon
r_default
suffix:colon
r_return
op_minus
id|EINVAL
suffix:semicolon
)brace
r_return
l_int|0
suffix:semicolon
)brace
DECL|function|do_release
r_static
r_int
id|do_release
c_func
(paren
r_struct
id|inode
op_star
id|inode
comma
r_struct
id|file
op_star
id|filp
)paren
(brace
r_struct
id|apm_user
op_star
id|as
suffix:semicolon
id|as
op_assign
id|filp-&gt;private_data
suffix:semicolon
r_if
c_cond
(paren
id|check_apm_user
c_func
(paren
id|as
comma
l_string|&quot;release&quot;
)paren
)paren
r_return
l_int|0
suffix:semicolon
id|filp-&gt;private_data
op_assign
l_int|NULL
suffix:semicolon
r_if
c_cond
(paren
id|as-&gt;standbys_pending
OG
l_int|0
)paren
(brace
id|standbys_pending
op_sub_assign
id|as-&gt;standbys_pending
suffix:semicolon
r_if
c_cond
(paren
id|standbys_pending
op_le
l_int|0
)paren
id|standby
c_func
(paren
)paren
suffix:semicolon
)brace
r_if
c_cond
(paren
id|as-&gt;suspends_pending
OG
l_int|0
)paren
(brace
id|suspends_pending
op_sub_assign
id|as-&gt;suspends_pending
suffix:semicolon
r_if
c_cond
(paren
id|suspends_pending
op_le
l_int|0
)paren
(paren
r_void
)paren
id|suspend
c_func
(paren
l_int|1
)paren
suffix:semicolon
)brace
id|spin_lock
c_func
(paren
op_amp
id|user_list_lock
)paren
suffix:semicolon
r_if
c_cond
(paren
id|user_list
op_eq
id|as
)paren
id|user_list
op_assign
id|as-&gt;next
suffix:semicolon
r_else
(brace
r_struct
id|apm_user
op_star
id|as1
suffix:semicolon
r_for
c_loop
(paren
id|as1
op_assign
id|user_list
suffix:semicolon
(paren
id|as1
op_ne
l_int|NULL
)paren
op_logical_and
(paren
id|as1-&gt;next
op_ne
id|as
)paren
suffix:semicolon
id|as1
op_assign
id|as1-&gt;next
)paren
suffix:semicolon
r_if
c_cond
(paren
id|as1
op_eq
l_int|NULL
)paren
id|printk
c_func
(paren
id|KERN_ERR
l_string|&quot;apm: filp not in user list&bslash;n&quot;
)paren
suffix:semicolon
r_else
id|as1-&gt;next
op_assign
id|as-&gt;next
suffix:semicolon
)brace
id|spin_unlock
c_func
(paren
op_amp
id|user_list_lock
)paren
suffix:semicolon
id|kfree
c_func
(paren
id|as
)paren
suffix:semicolon
r_return
l_int|0
suffix:semicolon
)brace
DECL|function|do_open
r_static
r_int
id|do_open
c_func
(paren
r_struct
id|inode
op_star
id|inode
comma
r_struct
id|file
op_star
id|filp
)paren
(brace
r_struct
id|apm_user
op_star
id|as
suffix:semicolon
id|as
op_assign
(paren
r_struct
id|apm_user
op_star
)paren
id|kmalloc
c_func
(paren
r_sizeof
(paren
op_star
id|as
)paren
comma
id|GFP_KERNEL
)paren
suffix:semicolon
r_if
c_cond
(paren
id|as
op_eq
l_int|NULL
)paren
(brace
id|printk
c_func
(paren
id|KERN_ERR
l_string|&quot;apm: cannot allocate struct of size %d bytes&bslash;n&quot;
comma
r_sizeof
(paren
op_star
id|as
)paren
)paren
suffix:semicolon
r_return
op_minus
id|ENOMEM
suffix:semicolon
)brace
id|as-&gt;magic
op_assign
id|APM_BIOS_MAGIC
suffix:semicolon
id|as-&gt;event_tail
op_assign
id|as-&gt;event_head
op_assign
l_int|0
suffix:semicolon
id|as-&gt;suspends_pending
op_assign
id|as-&gt;standbys_pending
op_assign
l_int|0
suffix:semicolon
id|as-&gt;suspends_read
op_assign
id|as-&gt;standbys_read
op_assign
l_int|0
suffix:semicolon
multiline_comment|/*&n;&t; * XXX - this is a tiny bit broken, when we consider BSD&n;         * process accounting. If the device is opened by root, we&n;&t; * instantly flag that we used superuser privs. Who knows,&n;&t; * we might close the device immediately without doing a&n;&t; * privileged operation -- cevans&n;&t; */
id|as-&gt;suser
op_assign
id|capable
c_func
(paren
id|CAP_SYS_ADMIN
)paren
suffix:semicolon
id|as-&gt;writer
op_assign
(paren
id|filp-&gt;f_mode
op_amp
id|FMODE_WRITE
)paren
op_eq
id|FMODE_WRITE
suffix:semicolon
id|as-&gt;reader
op_assign
(paren
id|filp-&gt;f_mode
op_amp
id|FMODE_READ
)paren
op_eq
id|FMODE_READ
suffix:semicolon
id|spin_lock
c_func
(paren
op_amp
id|user_list_lock
)paren
suffix:semicolon
id|as-&gt;next
op_assign
id|user_list
suffix:semicolon
id|user_list
op_assign
id|as
suffix:semicolon
id|spin_unlock
c_func
(paren
op_amp
id|user_list_lock
)paren
suffix:semicolon
id|filp-&gt;private_data
op_assign
id|as
suffix:semicolon
r_return
l_int|0
suffix:semicolon
)brace
DECL|function|apm_get_info
r_static
r_int
id|apm_get_info
c_func
(paren
r_char
op_star
id|buf
comma
r_char
op_star
op_star
id|start
comma
id|off_t
id|fpos
comma
r_int
id|length
)paren
(brace
r_char
op_star
id|p
suffix:semicolon
r_int
r_int
id|bx
suffix:semicolon
r_int
r_int
id|cx
suffix:semicolon
r_int
r_int
id|dx
suffix:semicolon
r_int
id|error
suffix:semicolon
r_int
r_int
id|ac_line_status
op_assign
l_int|0xff
suffix:semicolon
r_int
r_int
id|battery_status
op_assign
l_int|0xff
suffix:semicolon
r_int
r_int
id|battery_flag
op_assign
l_int|0xff
suffix:semicolon
r_int
id|percentage
op_assign
op_minus
l_int|1
suffix:semicolon
r_int
id|time_units
op_assign
op_minus
l_int|1
suffix:semicolon
r_char
op_star
id|units
op_assign
l_string|&quot;?&quot;
suffix:semicolon
id|p
op_assign
id|buf
suffix:semicolon
r_if
c_cond
(paren
(paren
id|num_online_cpus
c_func
(paren
)paren
op_eq
l_int|1
)paren
op_logical_and
op_logical_neg
(paren
id|error
op_assign
id|apm_get_power_status
c_func
(paren
op_amp
id|bx
comma
op_amp
id|cx
comma
op_amp
id|dx
)paren
)paren
)paren
(brace
id|ac_line_status
op_assign
(paren
id|bx
op_rshift
l_int|8
)paren
op_amp
l_int|0xff
suffix:semicolon
id|battery_status
op_assign
id|bx
op_amp
l_int|0xff
suffix:semicolon
r_if
c_cond
(paren
(paren
id|cx
op_amp
l_int|0xff
)paren
op_ne
l_int|0xff
)paren
id|percentage
op_assign
id|cx
op_amp
l_int|0xff
suffix:semicolon
r_if
c_cond
(paren
id|apm_info.connection_version
OG
l_int|0x100
)paren
(brace
id|battery_flag
op_assign
(paren
id|cx
op_rshift
l_int|8
)paren
op_amp
l_int|0xff
suffix:semicolon
r_if
c_cond
(paren
id|dx
op_ne
l_int|0xffff
)paren
(brace
id|units
op_assign
(paren
id|dx
op_amp
l_int|0x8000
)paren
ques
c_cond
l_string|&quot;min&quot;
suffix:colon
l_string|&quot;sec&quot;
suffix:semicolon
id|time_units
op_assign
id|dx
op_amp
l_int|0x7fff
suffix:semicolon
)brace
)brace
)brace
multiline_comment|/* Arguments, with symbols from linux/apm_bios.h.  Information is&n;&t;   from the Get Power Status (0x0a) call unless otherwise noted.&n;&n;&t;   0) Linux driver version (this will change if format changes)&n;&t;   1) APM BIOS Version.  Usually 1.0, 1.1 or 1.2.&n;&t;   2) APM flags from APM Installation Check (0x00):&n;&t;      bit 0: APM_16_BIT_SUPPORT&n;&t;      bit 1: APM_32_BIT_SUPPORT&n;&t;      bit 2: APM_IDLE_SLOWS_CLOCK&n;&t;      bit 3: APM_BIOS_DISABLED&n;&t;      bit 4: APM_BIOS_DISENGAGED&n;&t;   3) AC line status&n;&t;      0x00: Off-line&n;&t;      0x01: On-line&n;&t;      0x02: On backup power (BIOS &gt;= 1.1 only)&n;&t;      0xff: Unknown&n;&t;   4) Battery status&n;&t;      0x00: High&n;&t;      0x01: Low&n;&t;      0x02: Critical&n;&t;      0x03: Charging&n;&t;      0x04: Selected battery not present (BIOS &gt;= 1.2 only)&n;&t;      0xff: Unknown&n;&t;   5) Battery flag&n;&t;      bit 0: High&n;&t;      bit 1: Low&n;&t;      bit 2: Critical&n;&t;      bit 3: Charging&n;&t;      bit 7: No system battery&n;&t;      0xff: Unknown&n;&t;   6) Remaining battery life (percentage of charge):&n;&t;      0-100: valid&n;&t;      -1: Unknown&n;&t;   7) Remaining battery life (time units):&n;&t;      Number of remaining minutes or seconds&n;&t;      -1: Unknown&n;&t;   8) min = minutes; sec = seconds */
id|p
op_add_assign
id|sprintf
c_func
(paren
id|p
comma
l_string|&quot;%s %d.%d 0x%02x 0x%02x 0x%02x 0x%02x %d%% %d %s&bslash;n&quot;
comma
id|driver_version
comma
(paren
id|apm_info.bios.version
op_rshift
l_int|8
)paren
op_amp
l_int|0xff
comma
id|apm_info.bios.version
op_amp
l_int|0xff
comma
id|apm_info.bios.flags
comma
id|ac_line_status
comma
id|battery_status
comma
id|battery_flag
comma
id|percentage
comma
id|time_units
comma
id|units
)paren
suffix:semicolon
r_return
id|p
op_minus
id|buf
suffix:semicolon
)brace
DECL|function|apm
r_static
r_int
id|apm
c_func
(paren
r_void
op_star
id|unused
)paren
(brace
r_int
r_int
id|bx
suffix:semicolon
r_int
r_int
id|cx
suffix:semicolon
r_int
r_int
id|dx
suffix:semicolon
r_int
id|error
suffix:semicolon
r_char
op_star
id|power_stat
suffix:semicolon
r_char
op_star
id|bat_stat
suffix:semicolon
id|kapmd_running
op_assign
l_int|1
suffix:semicolon
id|daemonize
c_func
(paren
l_string|&quot;kapmd&quot;
)paren
suffix:semicolon
id|current-&gt;flags
op_or_assign
id|PF_NOFREEZE
suffix:semicolon
macro_line|#ifdef CONFIG_SMP
multiline_comment|/* 2002/08/01 - WT&n;&t; * This is to avoid random crashes at boot time during initialization&n;&t; * on SMP systems in case of &quot;apm=power-off&quot; mode. Seen on ASUS A7M266D.&n;&t; * Some bioses don&squot;t like being called from CPU != 0.&n;&t; * Method suggested by Ingo Molnar.&n;&t; */
id|set_cpus_allowed
c_func
(paren
id|current
comma
id|cpumask_of_cpu
c_func
(paren
l_int|0
)paren
)paren
suffix:semicolon
id|BUG_ON
c_func
(paren
id|smp_processor_id
c_func
(paren
)paren
op_ne
l_int|0
)paren
suffix:semicolon
macro_line|#endif
r_if
c_cond
(paren
id|apm_info.connection_version
op_eq
l_int|0
)paren
(brace
id|apm_info.connection_version
op_assign
id|apm_info.bios.version
suffix:semicolon
r_if
c_cond
(paren
id|apm_info.connection_version
OG
l_int|0x100
)paren
(brace
multiline_comment|/*&n;&t;&t;&t; * We only support BIOSs up to version 1.2&n;&t;&t;&t; */
r_if
c_cond
(paren
id|apm_info.connection_version
OG
l_int|0x0102
)paren
id|apm_info.connection_version
op_assign
l_int|0x0102
suffix:semicolon
id|error
op_assign
id|apm_driver_version
c_func
(paren
op_amp
id|apm_info.connection_version
)paren
suffix:semicolon
r_if
c_cond
(paren
id|error
op_ne
id|APM_SUCCESS
)paren
(brace
id|apm_error
c_func
(paren
l_string|&quot;driver version&quot;
comma
id|error
)paren
suffix:semicolon
multiline_comment|/* Fall back to an APM 1.0 connection. */
id|apm_info.connection_version
op_assign
l_int|0x100
suffix:semicolon
)brace
)brace
)brace
r_if
c_cond
(paren
id|debug
)paren
id|printk
c_func
(paren
id|KERN_INFO
l_string|&quot;apm: Connection version %d.%d&bslash;n&quot;
comma
(paren
id|apm_info.connection_version
op_rshift
l_int|8
)paren
op_amp
l_int|0xff
comma
id|apm_info.connection_version
op_amp
l_int|0xff
)paren
suffix:semicolon
macro_line|#ifdef CONFIG_APM_DO_ENABLE
r_if
c_cond
(paren
id|apm_info.bios.flags
op_amp
id|APM_BIOS_DISABLED
)paren
(brace
multiline_comment|/*&n;&t;&t; * This call causes my NEC UltraLite Versa 33/C to hang if it&n;&t;&t; * is booted with PM disabled but not in the docking station.&n;&t;&t; * Unfortunate ...&n;&t;&t; */
id|error
op_assign
id|apm_enable_power_management
c_func
(paren
l_int|1
)paren
suffix:semicolon
r_if
c_cond
(paren
id|error
)paren
(brace
id|apm_error
c_func
(paren
l_string|&quot;enable power management&quot;
comma
id|error
)paren
suffix:semicolon
r_return
op_minus
l_int|1
suffix:semicolon
)brace
)brace
macro_line|#endif
r_if
c_cond
(paren
(paren
id|apm_info.bios.flags
op_amp
id|APM_BIOS_DISENGAGED
)paren
op_logical_and
(paren
id|apm_info.connection_version
OG
l_int|0x0100
)paren
)paren
(brace
id|error
op_assign
id|apm_engage_power_management
c_func
(paren
id|APM_DEVICE_ALL
comma
l_int|1
)paren
suffix:semicolon
r_if
c_cond
(paren
id|error
)paren
(brace
id|apm_error
c_func
(paren
l_string|&quot;engage power management&quot;
comma
id|error
)paren
suffix:semicolon
r_return
op_minus
l_int|1
suffix:semicolon
)brace
)brace
r_if
c_cond
(paren
id|debug
op_logical_and
(paren
id|num_online_cpus
c_func
(paren
)paren
op_eq
l_int|1
op_logical_or
id|smp
)paren
)paren
(brace
id|error
op_assign
id|apm_get_power_status
c_func
(paren
op_amp
id|bx
comma
op_amp
id|cx
comma
op_amp
id|dx
)paren
suffix:semicolon
r_if
c_cond
(paren
id|error
)paren
id|printk
c_func
(paren
id|KERN_INFO
l_string|&quot;apm: power status not available&bslash;n&quot;
)paren
suffix:semicolon
r_else
(brace
r_switch
c_cond
(paren
(paren
id|bx
op_rshift
l_int|8
)paren
op_amp
l_int|0xff
)paren
(brace
r_case
l_int|0
suffix:colon
id|power_stat
op_assign
l_string|&quot;off line&quot;
suffix:semicolon
r_break
suffix:semicolon
r_case
l_int|1
suffix:colon
id|power_stat
op_assign
l_string|&quot;on line&quot;
suffix:semicolon
r_break
suffix:semicolon
r_case
l_int|2
suffix:colon
id|power_stat
op_assign
l_string|&quot;on backup power&quot;
suffix:semicolon
r_break
suffix:semicolon
r_default
suffix:colon
id|power_stat
op_assign
l_string|&quot;unknown&quot;
suffix:semicolon
r_break
suffix:semicolon
)brace
r_switch
c_cond
(paren
id|bx
op_amp
l_int|0xff
)paren
(brace
r_case
l_int|0
suffix:colon
id|bat_stat
op_assign
l_string|&quot;high&quot;
suffix:semicolon
r_break
suffix:semicolon
r_case
l_int|1
suffix:colon
id|bat_stat
op_assign
l_string|&quot;low&quot;
suffix:semicolon
r_break
suffix:semicolon
r_case
l_int|2
suffix:colon
id|bat_stat
op_assign
l_string|&quot;critical&quot;
suffix:semicolon
r_break
suffix:semicolon
r_case
l_int|3
suffix:colon
id|bat_stat
op_assign
l_string|&quot;charging&quot;
suffix:semicolon
r_break
suffix:semicolon
r_default
suffix:colon
id|bat_stat
op_assign
l_string|&quot;unknown&quot;
suffix:semicolon
r_break
suffix:semicolon
)brace
id|printk
c_func
(paren
id|KERN_INFO
l_string|&quot;apm: AC %s, battery status %s, battery life &quot;
comma
id|power_stat
comma
id|bat_stat
)paren
suffix:semicolon
r_if
c_cond
(paren
(paren
id|cx
op_amp
l_int|0xff
)paren
op_eq
l_int|0xff
)paren
id|printk
c_func
(paren
l_string|&quot;unknown&bslash;n&quot;
)paren
suffix:semicolon
r_else
id|printk
c_func
(paren
l_string|&quot;%d%%&bslash;n&quot;
comma
id|cx
op_amp
l_int|0xff
)paren
suffix:semicolon
r_if
c_cond
(paren
id|apm_info.connection_version
OG
l_int|0x100
)paren
(brace
id|printk
c_func
(paren
id|KERN_INFO
l_string|&quot;apm: battery flag 0x%02x, battery life &quot;
comma
(paren
id|cx
op_rshift
l_int|8
)paren
op_amp
l_int|0xff
)paren
suffix:semicolon
r_if
c_cond
(paren
id|dx
op_eq
l_int|0xffff
)paren
id|printk
c_func
(paren
l_string|&quot;unknown&bslash;n&quot;
)paren
suffix:semicolon
r_else
id|printk
c_func
(paren
l_string|&quot;%d %s&bslash;n&quot;
comma
id|dx
op_amp
l_int|0x7fff
comma
(paren
id|dx
op_amp
l_int|0x8000
)paren
ques
c_cond
l_string|&quot;minutes&quot;
suffix:colon
l_string|&quot;seconds&quot;
)paren
suffix:semicolon
)brace
)brace
)brace
multiline_comment|/* Install our power off handler.. */
r_if
c_cond
(paren
id|power_off
)paren
id|pm_power_off
op_assign
id|apm_power_off
suffix:semicolon
r_if
c_cond
(paren
id|num_online_cpus
c_func
(paren
)paren
op_eq
l_int|1
op_logical_or
id|smp
)paren
(brace
macro_line|#if defined(CONFIG_APM_DISPLAY_BLANK) &amp;&amp; defined(CONFIG_VT)
id|console_blank_hook
op_assign
id|apm_console_blank
suffix:semicolon
macro_line|#endif
id|apm_mainloop
c_func
(paren
)paren
suffix:semicolon
macro_line|#if defined(CONFIG_APM_DISPLAY_BLANK) &amp;&amp; defined(CONFIG_VT)
id|console_blank_hook
op_assign
l_int|NULL
suffix:semicolon
macro_line|#endif
)brace
id|kapmd_running
op_assign
l_int|0
suffix:semicolon
r_return
l_int|0
suffix:semicolon
)brace
macro_line|#ifndef MODULE
DECL|function|apm_setup
r_static
r_int
id|__init
id|apm_setup
c_func
(paren
r_char
op_star
id|str
)paren
(brace
r_int
id|invert
suffix:semicolon
r_while
c_loop
(paren
(paren
id|str
op_ne
l_int|NULL
)paren
op_logical_and
(paren
op_star
id|str
op_ne
l_char|&squot;&bslash;0&squot;
)paren
)paren
(brace
r_if
c_cond
(paren
id|strncmp
c_func
(paren
id|str
comma
l_string|&quot;off&quot;
comma
l_int|3
)paren
op_eq
l_int|0
)paren
id|apm_disabled
op_assign
l_int|1
suffix:semicolon
r_if
c_cond
(paren
id|strncmp
c_func
(paren
id|str
comma
l_string|&quot;on&quot;
comma
l_int|2
)paren
op_eq
l_int|0
)paren
id|apm_disabled
op_assign
l_int|0
suffix:semicolon
r_if
c_cond
(paren
(paren
id|strncmp
c_func
(paren
id|str
comma
l_string|&quot;bounce-interval=&quot;
comma
l_int|16
)paren
op_eq
l_int|0
)paren
op_logical_or
(paren
id|strncmp
c_func
(paren
id|str
comma
l_string|&quot;bounce_interval=&quot;
comma
l_int|16
)paren
op_eq
l_int|0
)paren
)paren
id|bounce_interval
op_assign
id|simple_strtol
c_func
(paren
id|str
op_plus
l_int|16
comma
l_int|NULL
comma
l_int|0
)paren
suffix:semicolon
r_if
c_cond
(paren
(paren
id|strncmp
c_func
(paren
id|str
comma
l_string|&quot;idle-threshold=&quot;
comma
l_int|15
)paren
op_eq
l_int|0
)paren
op_logical_or
(paren
id|strncmp
c_func
(paren
id|str
comma
l_string|&quot;idle_threshold=&quot;
comma
l_int|15
)paren
op_eq
l_int|0
)paren
)paren
id|idle_threshold
op_assign
id|simple_strtol
c_func
(paren
id|str
op_plus
l_int|15
comma
l_int|NULL
comma
l_int|0
)paren
suffix:semicolon
r_if
c_cond
(paren
(paren
id|strncmp
c_func
(paren
id|str
comma
l_string|&quot;idle-period=&quot;
comma
l_int|12
)paren
op_eq
l_int|0
)paren
op_logical_or
(paren
id|strncmp
c_func
(paren
id|str
comma
l_string|&quot;idle_period=&quot;
comma
l_int|12
)paren
op_eq
l_int|0
)paren
)paren
id|idle_period
op_assign
id|simple_strtol
c_func
(paren
id|str
op_plus
l_int|12
comma
l_int|NULL
comma
l_int|0
)paren
suffix:semicolon
id|invert
op_assign
(paren
id|strncmp
c_func
(paren
id|str
comma
l_string|&quot;no-&quot;
comma
l_int|3
)paren
op_eq
l_int|0
)paren
op_logical_or
(paren
id|strncmp
c_func
(paren
id|str
comma
l_string|&quot;no_&quot;
comma
l_int|3
)paren
op_eq
l_int|0
)paren
suffix:semicolon
r_if
c_cond
(paren
id|invert
)paren
id|str
op_add_assign
l_int|3
suffix:semicolon
r_if
c_cond
(paren
id|strncmp
c_func
(paren
id|str
comma
l_string|&quot;debug&quot;
comma
l_int|5
)paren
op_eq
l_int|0
)paren
id|debug
op_assign
op_logical_neg
id|invert
suffix:semicolon
r_if
c_cond
(paren
(paren
id|strncmp
c_func
(paren
id|str
comma
l_string|&quot;power-off&quot;
comma
l_int|9
)paren
op_eq
l_int|0
)paren
op_logical_or
(paren
id|strncmp
c_func
(paren
id|str
comma
l_string|&quot;power_off&quot;
comma
l_int|9
)paren
op_eq
l_int|0
)paren
)paren
id|power_off
op_assign
op_logical_neg
id|invert
suffix:semicolon
r_if
c_cond
(paren
id|strncmp
c_func
(paren
id|str
comma
l_string|&quot;smp&quot;
comma
l_int|3
)paren
op_eq
l_int|0
)paren
(brace
id|smp
op_assign
op_logical_neg
id|invert
suffix:semicolon
id|idle_threshold
op_assign
l_int|100
suffix:semicolon
)brace
r_if
c_cond
(paren
(paren
id|strncmp
c_func
(paren
id|str
comma
l_string|&quot;allow-ints&quot;
comma
l_int|10
)paren
op_eq
l_int|0
)paren
op_logical_or
(paren
id|strncmp
c_func
(paren
id|str
comma
l_string|&quot;allow_ints&quot;
comma
l_int|10
)paren
op_eq
l_int|0
)paren
)paren
id|apm_info.allow_ints
op_assign
op_logical_neg
id|invert
suffix:semicolon
r_if
c_cond
(paren
(paren
id|strncmp
c_func
(paren
id|str
comma
l_string|&quot;broken-psr&quot;
comma
l_int|10
)paren
op_eq
l_int|0
)paren
op_logical_or
(paren
id|strncmp
c_func
(paren
id|str
comma
l_string|&quot;broken_psr&quot;
comma
l_int|10
)paren
op_eq
l_int|0
)paren
)paren
id|apm_info.get_power_status_broken
op_assign
op_logical_neg
id|invert
suffix:semicolon
r_if
c_cond
(paren
(paren
id|strncmp
c_func
(paren
id|str
comma
l_string|&quot;realmode-power-off&quot;
comma
l_int|18
)paren
op_eq
l_int|0
)paren
op_logical_or
(paren
id|strncmp
c_func
(paren
id|str
comma
l_string|&quot;realmode_power_off&quot;
comma
l_int|18
)paren
op_eq
l_int|0
)paren
)paren
id|apm_info.realmode_power_off
op_assign
op_logical_neg
id|invert
suffix:semicolon
id|str
op_assign
id|strchr
c_func
(paren
id|str
comma
l_char|&squot;,&squot;
)paren
suffix:semicolon
r_if
c_cond
(paren
id|str
op_ne
l_int|NULL
)paren
id|str
op_add_assign
id|strspn
c_func
(paren
id|str
comma
l_string|&quot;, &bslash;t&quot;
)paren
suffix:semicolon
)brace
r_return
l_int|1
suffix:semicolon
)brace
id|__setup
c_func
(paren
l_string|&quot;apm=&quot;
comma
id|apm_setup
)paren
suffix:semicolon
macro_line|#endif
DECL|variable|apm_bios_fops
r_static
r_struct
id|file_operations
id|apm_bios_fops
op_assign
(brace
dot
id|owner
op_assign
id|THIS_MODULE
comma
dot
id|read
op_assign
id|do_read
comma
dot
id|poll
op_assign
id|do_poll
comma
dot
id|ioctl
op_assign
id|do_ioctl
comma
dot
id|open
op_assign
id|do_open
comma
dot
id|release
op_assign
id|do_release
comma
)brace
suffix:semicolon
DECL|variable|apm_device
r_static
r_struct
id|miscdevice
id|apm_device
op_assign
(brace
id|APM_MINOR_DEV
comma
l_string|&quot;apm_bios&quot;
comma
op_amp
id|apm_bios_fops
)brace
suffix:semicolon
multiline_comment|/* Simple &quot;print if true&quot; callback */
DECL|function|print_if_true
r_static
r_int
id|__init
id|print_if_true
c_func
(paren
r_struct
id|dmi_system_id
op_star
id|d
)paren
(brace
id|printk
c_func
(paren
l_string|&quot;%s&bslash;n&quot;
comma
id|d-&gt;ident
)paren
suffix:semicolon
r_return
l_int|0
suffix:semicolon
)brace
multiline_comment|/*&n; * Some Bioses enable the PS/2 mouse (touchpad) at resume, even if it was&n; * disabled before the suspend. Linux used to get terribly confused by that.&n; */
DECL|function|broken_ps2_resume
r_static
r_int
id|__init
id|broken_ps2_resume
c_func
(paren
r_struct
id|dmi_system_id
op_star
id|d
)paren
(brace
id|printk
c_func
(paren
id|KERN_INFO
l_string|&quot;%s machine detected. Mousepad Resume Bug workaround hopefully not needed.&bslash;n&quot;
comma
id|d-&gt;ident
)paren
suffix:semicolon
r_return
l_int|0
suffix:semicolon
)brace
multiline_comment|/* Some bioses have a broken protected mode poweroff and need to use realmode */
DECL|function|set_realmode_power_off
r_static
r_int
id|__init
id|set_realmode_power_off
c_func
(paren
r_struct
id|dmi_system_id
op_star
id|d
)paren
(brace
r_if
c_cond
(paren
id|apm_info.realmode_power_off
op_eq
l_int|0
)paren
(brace
id|apm_info.realmode_power_off
op_assign
l_int|1
suffix:semicolon
id|printk
c_func
(paren
id|KERN_INFO
l_string|&quot;%s bios detected. Using realmode poweroff only.&bslash;n&quot;
comma
id|d-&gt;ident
)paren
suffix:semicolon
)brace
r_return
l_int|0
suffix:semicolon
)brace
multiline_comment|/* Some laptops require interrupts to be enabled during APM calls */
DECL|function|set_apm_ints
r_static
r_int
id|__init
id|set_apm_ints
c_func
(paren
r_struct
id|dmi_system_id
op_star
id|d
)paren
(brace
r_if
c_cond
(paren
id|apm_info.allow_ints
op_eq
l_int|0
)paren
(brace
id|apm_info.allow_ints
op_assign
l_int|1
suffix:semicolon
id|printk
c_func
(paren
id|KERN_INFO
l_string|&quot;%s machine detected. Enabling interrupts during APM calls.&bslash;n&quot;
comma
id|d-&gt;ident
)paren
suffix:semicolon
)brace
r_return
l_int|0
suffix:semicolon
)brace
multiline_comment|/* Some APM bioses corrupt memory or just plain do not work */
DECL|function|apm_is_horked
r_static
r_int
id|__init
id|apm_is_horked
c_func
(paren
r_struct
id|dmi_system_id
op_star
id|d
)paren
(brace
r_if
c_cond
(paren
id|apm_info.disabled
op_eq
l_int|0
)paren
(brace
id|apm_info.disabled
op_assign
l_int|1
suffix:semicolon
id|printk
c_func
(paren
id|KERN_INFO
l_string|&quot;%s machine detected. Disabling APM.&bslash;n&quot;
comma
id|d-&gt;ident
)paren
suffix:semicolon
)brace
r_return
l_int|0
suffix:semicolon
)brace
DECL|function|apm_is_horked_d850md
r_static
r_int
id|__init
id|apm_is_horked_d850md
c_func
(paren
r_struct
id|dmi_system_id
op_star
id|d
)paren
(brace
r_if
c_cond
(paren
id|apm_info.disabled
op_eq
l_int|0
)paren
(brace
id|apm_info.disabled
op_assign
l_int|1
suffix:semicolon
id|printk
c_func
(paren
id|KERN_INFO
l_string|&quot;%s machine detected. Disabling APM.&bslash;n&quot;
comma
id|d-&gt;ident
)paren
suffix:semicolon
id|printk
c_func
(paren
id|KERN_INFO
l_string|&quot;This bug is fixed in bios P15 which is available for &bslash;n&quot;
)paren
suffix:semicolon
id|printk
c_func
(paren
id|KERN_INFO
l_string|&quot;download from support.intel.com &bslash;n&quot;
)paren
suffix:semicolon
)brace
r_return
l_int|0
suffix:semicolon
)brace
multiline_comment|/* Some APM bioses hang on APM idle calls */
DECL|function|apm_likes_to_melt
r_static
r_int
id|__init
id|apm_likes_to_melt
c_func
(paren
r_struct
id|dmi_system_id
op_star
id|d
)paren
(brace
r_if
c_cond
(paren
id|apm_info.forbid_idle
op_eq
l_int|0
)paren
(brace
id|apm_info.forbid_idle
op_assign
l_int|1
suffix:semicolon
id|printk
c_func
(paren
id|KERN_INFO
l_string|&quot;%s machine detected. Disabling APM idle calls.&bslash;n&quot;
comma
id|d-&gt;ident
)paren
suffix:semicolon
)brace
r_return
l_int|0
suffix:semicolon
)brace
multiline_comment|/*&n; *  Check for clue free BIOS implementations who use&n; *  the following QA technique&n; *&n; *      [ Write BIOS Code ]&lt;------&n; *               |                ^&n; *      &lt; Does it Compile &gt;----N--&n; *               |Y               ^&n; *&t;&lt; Does it Boot Win98 &gt;-N--&n; *               |Y&n; *           [Ship It]&n; *&n; *&t;Phoenix A04  08/24/2000 is known bad (Dell Inspiron 5000e)&n; *&t;Phoenix A07  09/29/2000 is known good (Dell Inspiron 5000)&n; */
DECL|function|broken_apm_power
r_static
r_int
id|__init
id|broken_apm_power
c_func
(paren
r_struct
id|dmi_system_id
op_star
id|d
)paren
(brace
id|apm_info.get_power_status_broken
op_assign
l_int|1
suffix:semicolon
id|printk
c_func
(paren
id|KERN_WARNING
l_string|&quot;BIOS strings suggest APM bugs, disabling power status reporting.&bslash;n&quot;
)paren
suffix:semicolon
r_return
l_int|0
suffix:semicolon
)brace
multiline_comment|/*&n; * This bios swaps the APM minute reporting bytes over (Many sony laptops&n; * have this problem).&n; */
DECL|function|swab_apm_power_in_minutes
r_static
r_int
id|__init
id|swab_apm_power_in_minutes
c_func
(paren
r_struct
id|dmi_system_id
op_star
id|d
)paren
(brace
id|apm_info.get_power_status_swabinminutes
op_assign
l_int|1
suffix:semicolon
id|printk
c_func
(paren
id|KERN_WARNING
l_string|&quot;BIOS strings suggest APM reports battery life in minutes and wrong byte order.&bslash;n&quot;
)paren
suffix:semicolon
r_return
l_int|0
suffix:semicolon
)brace
DECL|variable|apm_dmi_table
r_static
r_struct
id|dmi_system_id
id|__initdata
id|apm_dmi_table
(braket
)braket
op_assign
(brace
(brace
id|print_if_true
comma
id|KERN_WARNING
l_string|&quot;IBM T23 - BIOS 1.03b+ and controller firmware 1.02+ may be needed for Linux APM.&quot;
comma
(brace
id|DMI_MATCH
c_func
(paren
id|DMI_SYS_VENDOR
comma
l_string|&quot;IBM&quot;
)paren
comma
id|DMI_MATCH
c_func
(paren
id|DMI_BIOS_VERSION
comma
l_string|&quot;1AET38WW (1.01b)&quot;
)paren
comma
)brace
comma
)brace
comma
(brace
multiline_comment|/* Handle problems with APM on the C600 */
id|broken_ps2_resume
comma
l_string|&quot;Dell Latitude C600&quot;
comma
(brace
id|DMI_MATCH
c_func
(paren
id|DMI_SYS_VENDOR
comma
l_string|&quot;Dell&quot;
)paren
comma
id|DMI_MATCH
c_func
(paren
id|DMI_PRODUCT_NAME
comma
l_string|&quot;Latitude C600&quot;
)paren
comma
)brace
comma
)brace
comma
(brace
multiline_comment|/* Allow interrupts during suspend on Dell Latitude laptops*/
id|set_apm_ints
comma
l_string|&quot;Dell Latitude&quot;
comma
(brace
id|DMI_MATCH
c_func
(paren
id|DMI_SYS_VENDOR
comma
l_string|&quot;Dell Computer Corporation&quot;
)paren
comma
id|DMI_MATCH
c_func
(paren
id|DMI_PRODUCT_NAME
comma
l_string|&quot;Latitude C510&quot;
)paren
comma
)brace
)brace
comma
(brace
multiline_comment|/* APM crashes */
id|apm_is_horked
comma
l_string|&quot;Dell Inspiron 2500&quot;
comma
(brace
id|DMI_MATCH
c_func
(paren
id|DMI_SYS_VENDOR
comma
l_string|&quot;Dell Computer Corporation&quot;
)paren
comma
id|DMI_MATCH
c_func
(paren
id|DMI_PRODUCT_NAME
comma
l_string|&quot;Inspiron 2500&quot;
)paren
comma
id|DMI_MATCH
c_func
(paren
id|DMI_BIOS_VENDOR
comma
l_string|&quot;Phoenix Technologies LTD&quot;
)paren
comma
id|DMI_MATCH
c_func
(paren
id|DMI_BIOS_VERSION
comma
l_string|&quot;A11&quot;
)paren
comma
)brace
comma
)brace
comma
(brace
multiline_comment|/* Allow interrupts during suspend on Dell Inspiron laptops*/
id|set_apm_ints
comma
l_string|&quot;Dell Inspiron&quot;
comma
(brace
id|DMI_MATCH
c_func
(paren
id|DMI_SYS_VENDOR
comma
l_string|&quot;Dell Computer Corporation&quot;
)paren
comma
id|DMI_MATCH
c_func
(paren
id|DMI_PRODUCT_NAME
comma
l_string|&quot;Inspiron 4000&quot;
)paren
comma
)brace
comma
)brace
comma
(brace
multiline_comment|/* Handle problems with APM on Inspiron 5000e */
id|broken_apm_power
comma
l_string|&quot;Dell Inspiron 5000e&quot;
comma
(brace
id|DMI_MATCH
c_func
(paren
id|DMI_BIOS_VENDOR
comma
l_string|&quot;Phoenix Technologies LTD&quot;
)paren
comma
id|DMI_MATCH
c_func
(paren
id|DMI_BIOS_VERSION
comma
l_string|&quot;A04&quot;
)paren
comma
id|DMI_MATCH
c_func
(paren
id|DMI_BIOS_DATE
comma
l_string|&quot;08/24/2000&quot;
)paren
comma
)brace
comma
)brace
comma
(brace
multiline_comment|/* Handle problems with APM on Inspiron 2500 */
id|broken_apm_power
comma
l_string|&quot;Dell Inspiron 2500&quot;
comma
(brace
id|DMI_MATCH
c_func
(paren
id|DMI_BIOS_VENDOR
comma
l_string|&quot;Phoenix Technologies LTD&quot;
)paren
comma
id|DMI_MATCH
c_func
(paren
id|DMI_BIOS_VERSION
comma
l_string|&quot;A12&quot;
)paren
comma
id|DMI_MATCH
c_func
(paren
id|DMI_BIOS_DATE
comma
l_string|&quot;02/04/2002&quot;
)paren
comma
)brace
comma
)brace
comma
(brace
multiline_comment|/* APM crashes */
id|apm_is_horked
comma
l_string|&quot;Dell Dimension 4100&quot;
comma
(brace
id|DMI_MATCH
c_func
(paren
id|DMI_SYS_VENDOR
comma
l_string|&quot;Dell Computer Corporation&quot;
)paren
comma
id|DMI_MATCH
c_func
(paren
id|DMI_PRODUCT_NAME
comma
l_string|&quot;XPS-Z&quot;
)paren
comma
id|DMI_MATCH
c_func
(paren
id|DMI_BIOS_VENDOR
comma
l_string|&quot;Intel Corp.&quot;
)paren
comma
id|DMI_MATCH
c_func
(paren
id|DMI_BIOS_VERSION
comma
l_string|&quot;A11&quot;
)paren
comma
)brace
comma
)brace
comma
(brace
multiline_comment|/* Allow interrupts during suspend on Compaq Laptops*/
id|set_apm_ints
comma
l_string|&quot;Compaq 12XL125&quot;
comma
(brace
id|DMI_MATCH
c_func
(paren
id|DMI_SYS_VENDOR
comma
l_string|&quot;Compaq&quot;
)paren
comma
id|DMI_MATCH
c_func
(paren
id|DMI_PRODUCT_NAME
comma
l_string|&quot;Compaq PC&quot;
)paren
comma
id|DMI_MATCH
c_func
(paren
id|DMI_BIOS_VENDOR
comma
l_string|&quot;Phoenix Technologies LTD&quot;
)paren
comma
id|DMI_MATCH
c_func
(paren
id|DMI_BIOS_VERSION
comma
l_string|&quot;4.06&quot;
)paren
comma
)brace
comma
)brace
comma
(brace
multiline_comment|/* Allow interrupts during APM or the clock goes slow */
id|set_apm_ints
comma
l_string|&quot;ASUSTeK&quot;
comma
(brace
id|DMI_MATCH
c_func
(paren
id|DMI_SYS_VENDOR
comma
l_string|&quot;ASUSTeK Computer Inc.&quot;
)paren
comma
id|DMI_MATCH
c_func
(paren
id|DMI_PRODUCT_NAME
comma
l_string|&quot;L8400K series Notebook PC&quot;
)paren
comma
)brace
comma
)brace
comma
(brace
multiline_comment|/* APM blows on shutdown */
id|apm_is_horked
comma
l_string|&quot;ABIT KX7-333[R]&quot;
comma
(brace
id|DMI_MATCH
c_func
(paren
id|DMI_BOARD_VENDOR
comma
l_string|&quot;ABIT&quot;
)paren
comma
id|DMI_MATCH
c_func
(paren
id|DMI_BOARD_NAME
comma
l_string|&quot;VT8367-8233A (KX7-333[R])&quot;
)paren
comma
)brace
comma
)brace
comma
(brace
multiline_comment|/* APM crashes */
id|apm_is_horked
comma
l_string|&quot;Trigem Delhi3&quot;
comma
(brace
id|DMI_MATCH
c_func
(paren
id|DMI_SYS_VENDOR
comma
l_string|&quot;TriGem Computer, Inc&quot;
)paren
comma
id|DMI_MATCH
c_func
(paren
id|DMI_PRODUCT_NAME
comma
l_string|&quot;Delhi3&quot;
)paren
comma
)brace
comma
)brace
comma
(brace
multiline_comment|/* APM crashes */
id|apm_is_horked
comma
l_string|&quot;Fujitsu-Siemens&quot;
comma
(brace
id|DMI_MATCH
c_func
(paren
id|DMI_BIOS_VENDOR
comma
l_string|&quot;hoenix/FUJITSU SIEMENS&quot;
)paren
comma
id|DMI_MATCH
c_func
(paren
id|DMI_BIOS_VERSION
comma
l_string|&quot;Version1.01&quot;
)paren
comma
)brace
comma
)brace
comma
(brace
multiline_comment|/* APM crashes */
id|apm_is_horked_d850md
comma
l_string|&quot;Intel D850MD&quot;
comma
(brace
id|DMI_MATCH
c_func
(paren
id|DMI_BIOS_VENDOR
comma
l_string|&quot;Intel Corp.&quot;
)paren
comma
id|DMI_MATCH
c_func
(paren
id|DMI_BIOS_VERSION
comma
l_string|&quot;MV85010A.86A.0016.P07.0201251536&quot;
)paren
comma
)brace
comma
)brace
comma
(brace
multiline_comment|/* APM crashes */
id|apm_is_horked
comma
l_string|&quot;Intel D810EMO&quot;
comma
(brace
id|DMI_MATCH
c_func
(paren
id|DMI_BIOS_VENDOR
comma
l_string|&quot;Intel Corp.&quot;
)paren
comma
id|DMI_MATCH
c_func
(paren
id|DMI_BIOS_VERSION
comma
l_string|&quot;MO81010A.86A.0008.P04.0004170800&quot;
)paren
comma
)brace
comma
)brace
comma
(brace
multiline_comment|/* APM crashes */
id|apm_is_horked
comma
l_string|&quot;Dell XPS-Z&quot;
comma
(brace
id|DMI_MATCH
c_func
(paren
id|DMI_BIOS_VENDOR
comma
l_string|&quot;Intel Corp.&quot;
)paren
comma
id|DMI_MATCH
c_func
(paren
id|DMI_BIOS_VERSION
comma
l_string|&quot;A11&quot;
)paren
comma
id|DMI_MATCH
c_func
(paren
id|DMI_PRODUCT_NAME
comma
l_string|&quot;XPS-Z&quot;
)paren
comma
)brace
comma
)brace
comma
(brace
multiline_comment|/* APM crashes */
id|apm_is_horked
comma
l_string|&quot;Sharp PC-PJ/AX&quot;
comma
(brace
id|DMI_MATCH
c_func
(paren
id|DMI_SYS_VENDOR
comma
l_string|&quot;SHARP&quot;
)paren
comma
id|DMI_MATCH
c_func
(paren
id|DMI_PRODUCT_NAME
comma
l_string|&quot;PC-PJ/AX&quot;
)paren
comma
id|DMI_MATCH
c_func
(paren
id|DMI_BIOS_VENDOR
comma
l_string|&quot;SystemSoft&quot;
)paren
comma
id|DMI_MATCH
c_func
(paren
id|DMI_BIOS_VERSION
comma
l_string|&quot;Version R2.08&quot;
)paren
comma
)brace
comma
)brace
comma
(brace
multiline_comment|/* APM crashes */
id|apm_is_horked
comma
l_string|&quot;Dell Inspiron 2500&quot;
comma
(brace
id|DMI_MATCH
c_func
(paren
id|DMI_SYS_VENDOR
comma
l_string|&quot;Dell Computer Corporation&quot;
)paren
comma
id|DMI_MATCH
c_func
(paren
id|DMI_PRODUCT_NAME
comma
l_string|&quot;Inspiron 2500&quot;
)paren
comma
id|DMI_MATCH
c_func
(paren
id|DMI_BIOS_VENDOR
comma
l_string|&quot;Phoenix Technologies LTD&quot;
)paren
comma
id|DMI_MATCH
c_func
(paren
id|DMI_BIOS_VERSION
comma
l_string|&quot;A11&quot;
)paren
comma
)brace
comma
)brace
comma
(brace
multiline_comment|/* APM idle hangs */
id|apm_likes_to_melt
comma
l_string|&quot;Jabil AMD&quot;
comma
(brace
id|DMI_MATCH
c_func
(paren
id|DMI_BIOS_VENDOR
comma
l_string|&quot;American Megatrends Inc.&quot;
)paren
comma
id|DMI_MATCH
c_func
(paren
id|DMI_BIOS_VERSION
comma
l_string|&quot;0AASNP06&quot;
)paren
comma
)brace
comma
)brace
comma
(brace
multiline_comment|/* APM idle hangs */
id|apm_likes_to_melt
comma
l_string|&quot;AMI Bios&quot;
comma
(brace
id|DMI_MATCH
c_func
(paren
id|DMI_BIOS_VENDOR
comma
l_string|&quot;American Megatrends Inc.&quot;
)paren
comma
id|DMI_MATCH
c_func
(paren
id|DMI_BIOS_VERSION
comma
l_string|&quot;0AASNP05&quot;
)paren
comma
)brace
comma
)brace
comma
(brace
multiline_comment|/* Handle problems with APM on Sony Vaio PCG-N505X(DE) */
id|swab_apm_power_in_minutes
comma
l_string|&quot;Sony VAIO&quot;
comma
(brace
id|DMI_MATCH
c_func
(paren
id|DMI_BIOS_VENDOR
comma
l_string|&quot;Phoenix Technologies LTD&quot;
)paren
comma
id|DMI_MATCH
c_func
(paren
id|DMI_BIOS_VERSION
comma
l_string|&quot;R0206H&quot;
)paren
comma
id|DMI_MATCH
c_func
(paren
id|DMI_BIOS_DATE
comma
l_string|&quot;08/23/99&quot;
)paren
comma
)brace
comma
)brace
comma
(brace
multiline_comment|/* Handle problems with APM on Sony Vaio PCG-N505VX */
id|swab_apm_power_in_minutes
comma
l_string|&quot;Sony VAIO&quot;
comma
(brace
id|DMI_MATCH
c_func
(paren
id|DMI_BIOS_VENDOR
comma
l_string|&quot;Phoenix Technologies LTD&quot;
)paren
comma
id|DMI_MATCH
c_func
(paren
id|DMI_BIOS_VERSION
comma
l_string|&quot;W2K06H0&quot;
)paren
comma
id|DMI_MATCH
c_func
(paren
id|DMI_BIOS_DATE
comma
l_string|&quot;02/03/00&quot;
)paren
comma
)brace
comma
)brace
comma
(brace
multiline_comment|/* Handle problems with APM on Sony Vaio PCG-XG29 */
id|swab_apm_power_in_minutes
comma
l_string|&quot;Sony VAIO&quot;
comma
(brace
id|DMI_MATCH
c_func
(paren
id|DMI_BIOS_VENDOR
comma
l_string|&quot;Phoenix Technologies LTD&quot;
)paren
comma
id|DMI_MATCH
c_func
(paren
id|DMI_BIOS_VERSION
comma
l_string|&quot;R0117A0&quot;
)paren
comma
id|DMI_MATCH
c_func
(paren
id|DMI_BIOS_DATE
comma
l_string|&quot;04/25/00&quot;
)paren
comma
)brace
comma
)brace
comma
(brace
multiline_comment|/* Handle problems with APM on Sony Vaio PCG-Z600NE */
id|swab_apm_power_in_minutes
comma
l_string|&quot;Sony VAIO&quot;
comma
(brace
id|DMI_MATCH
c_func
(paren
id|DMI_BIOS_VENDOR
comma
l_string|&quot;Phoenix Technologies LTD&quot;
)paren
comma
id|DMI_MATCH
c_func
(paren
id|DMI_BIOS_VERSION
comma
l_string|&quot;R0121Z1&quot;
)paren
comma
id|DMI_MATCH
c_func
(paren
id|DMI_BIOS_DATE
comma
l_string|&quot;05/11/00&quot;
)paren
comma
)brace
comma
)brace
comma
(brace
multiline_comment|/* Handle problems with APM on Sony Vaio PCG-Z600NE */
id|swab_apm_power_in_minutes
comma
l_string|&quot;Sony VAIO&quot;
comma
(brace
id|DMI_MATCH
c_func
(paren
id|DMI_BIOS_VENDOR
comma
l_string|&quot;Phoenix Technologies LTD&quot;
)paren
comma
id|DMI_MATCH
c_func
(paren
id|DMI_BIOS_VERSION
comma
l_string|&quot;WME01Z1&quot;
)paren
comma
id|DMI_MATCH
c_func
(paren
id|DMI_BIOS_DATE
comma
l_string|&quot;08/11/00&quot;
)paren
comma
)brace
comma
)brace
comma
(brace
multiline_comment|/* Handle problems with APM on Sony Vaio PCG-Z600LEK(DE) */
id|swab_apm_power_in_minutes
comma
l_string|&quot;Sony VAIO&quot;
comma
(brace
id|DMI_MATCH
c_func
(paren
id|DMI_BIOS_VENDOR
comma
l_string|&quot;Phoenix Technologies LTD&quot;
)paren
comma
id|DMI_MATCH
c_func
(paren
id|DMI_BIOS_VERSION
comma
l_string|&quot;R0206Z3&quot;
)paren
comma
id|DMI_MATCH
c_func
(paren
id|DMI_BIOS_DATE
comma
l_string|&quot;12/25/00&quot;
)paren
comma
)brace
comma
)brace
comma
(brace
multiline_comment|/* Handle problems with APM on Sony Vaio PCG-Z505LS */
id|swab_apm_power_in_minutes
comma
l_string|&quot;Sony VAIO&quot;
comma
(brace
id|DMI_MATCH
c_func
(paren
id|DMI_BIOS_VENDOR
comma
l_string|&quot;Phoenix Technologies LTD&quot;
)paren
comma
id|DMI_MATCH
c_func
(paren
id|DMI_BIOS_VERSION
comma
l_string|&quot;R0203D0&quot;
)paren
comma
id|DMI_MATCH
c_func
(paren
id|DMI_BIOS_DATE
comma
l_string|&quot;05/12/00&quot;
)paren
comma
)brace
comma
)brace
comma
(brace
multiline_comment|/* Handle problems with APM on Sony Vaio PCG-Z505LS */
id|swab_apm_power_in_minutes
comma
l_string|&quot;Sony VAIO&quot;
comma
(brace
id|DMI_MATCH
c_func
(paren
id|DMI_BIOS_VENDOR
comma
l_string|&quot;Phoenix Technologies LTD&quot;
)paren
comma
id|DMI_MATCH
c_func
(paren
id|DMI_BIOS_VERSION
comma
l_string|&quot;R0203Z3&quot;
)paren
comma
id|DMI_MATCH
c_func
(paren
id|DMI_BIOS_DATE
comma
l_string|&quot;08/25/00&quot;
)paren
comma
)brace
comma
)brace
comma
(brace
multiline_comment|/* Handle problems with APM on Sony Vaio PCG-Z505LS (with updated BIOS) */
id|swab_apm_power_in_minutes
comma
l_string|&quot;Sony VAIO&quot;
comma
(brace
id|DMI_MATCH
c_func
(paren
id|DMI_BIOS_VENDOR
comma
l_string|&quot;Phoenix Technologies LTD&quot;
)paren
comma
id|DMI_MATCH
c_func
(paren
id|DMI_BIOS_VERSION
comma
l_string|&quot;R0209Z3&quot;
)paren
comma
id|DMI_MATCH
c_func
(paren
id|DMI_BIOS_DATE
comma
l_string|&quot;05/12/01&quot;
)paren
comma
)brace
comma
)brace
comma
(brace
multiline_comment|/* Handle problems with APM on Sony Vaio PCG-F104K */
id|swab_apm_power_in_minutes
comma
l_string|&quot;Sony VAIO&quot;
comma
(brace
id|DMI_MATCH
c_func
(paren
id|DMI_BIOS_VENDOR
comma
l_string|&quot;Phoenix Technologies LTD&quot;
)paren
comma
id|DMI_MATCH
c_func
(paren
id|DMI_BIOS_VERSION
comma
l_string|&quot;R0204K2&quot;
)paren
comma
id|DMI_MATCH
c_func
(paren
id|DMI_BIOS_DATE
comma
l_string|&quot;08/28/00&quot;
)paren
comma
)brace
comma
)brace
comma
(brace
multiline_comment|/* Handle problems with APM on Sony Vaio PCG-C1VN/C1VE */
id|swab_apm_power_in_minutes
comma
l_string|&quot;Sony VAIO&quot;
comma
(brace
id|DMI_MATCH
c_func
(paren
id|DMI_BIOS_VENDOR
comma
l_string|&quot;Phoenix Technologies LTD&quot;
)paren
comma
id|DMI_MATCH
c_func
(paren
id|DMI_BIOS_VERSION
comma
l_string|&quot;R0208P1&quot;
)paren
comma
id|DMI_MATCH
c_func
(paren
id|DMI_BIOS_DATE
comma
l_string|&quot;11/09/00&quot;
)paren
comma
)brace
comma
)brace
comma
(brace
multiline_comment|/* Handle problems with APM on Sony Vaio PCG-C1VE */
id|swab_apm_power_in_minutes
comma
l_string|&quot;Sony VAIO&quot;
comma
(brace
id|DMI_MATCH
c_func
(paren
id|DMI_BIOS_VENDOR
comma
l_string|&quot;Phoenix Technologies LTD&quot;
)paren
comma
id|DMI_MATCH
c_func
(paren
id|DMI_BIOS_VERSION
comma
l_string|&quot;R0204P1&quot;
)paren
comma
id|DMI_MATCH
c_func
(paren
id|DMI_BIOS_DATE
comma
l_string|&quot;09/12/00&quot;
)paren
comma
)brace
comma
)brace
comma
(brace
multiline_comment|/* Handle problems with APM on Sony Vaio PCG-C1VE */
id|swab_apm_power_in_minutes
comma
l_string|&quot;Sony VAIO&quot;
comma
(brace
id|DMI_MATCH
c_func
(paren
id|DMI_BIOS_VENDOR
comma
l_string|&quot;Phoenix Technologies LTD&quot;
)paren
comma
id|DMI_MATCH
c_func
(paren
id|DMI_BIOS_VERSION
comma
l_string|&quot;WXPO1Z3&quot;
)paren
comma
id|DMI_MATCH
c_func
(paren
id|DMI_BIOS_DATE
comma
l_string|&quot;10/26/01&quot;
)paren
comma
)brace
comma
)brace
comma
(brace
multiline_comment|/* broken PM poweroff bios */
id|set_realmode_power_off
comma
l_string|&quot;Award Software v4.60 PGMA&quot;
comma
(brace
id|DMI_MATCH
c_func
(paren
id|DMI_BIOS_VENDOR
comma
l_string|&quot;Award Software International, Inc.&quot;
)paren
comma
id|DMI_MATCH
c_func
(paren
id|DMI_BIOS_VERSION
comma
l_string|&quot;4.60 PGMA&quot;
)paren
comma
id|DMI_MATCH
c_func
(paren
id|DMI_BIOS_DATE
comma
l_string|&quot;134526184&quot;
)paren
comma
)brace
comma
)brace
comma
multiline_comment|/* Generic per vendor APM settings  */
(brace
multiline_comment|/* Allow interrupts during suspend on IBM laptops */
id|set_apm_ints
comma
l_string|&quot;IBM&quot;
comma
(brace
id|DMI_MATCH
c_func
(paren
id|DMI_SYS_VENDOR
comma
l_string|&quot;IBM&quot;
)paren
comma
)brace
comma
)brace
comma
(brace
)brace
)brace
suffix:semicolon
multiline_comment|/*&n; * Just start the APM thread. We do NOT want to do APM BIOS&n; * calls from anything but the APM thread, if for no other reason&n; * than the fact that we don&squot;t trust the APM BIOS. This way,&n; * most common APM BIOS problems that lead to protection errors&n; * etc will have at least some level of being contained...&n; *&n; * In short, if something bad happens, at least we have a choice&n; * of just killing the apm thread..&n; */
DECL|function|apm_init
r_static
r_int
id|__init
id|apm_init
c_func
(paren
r_void
)paren
(brace
r_struct
id|proc_dir_entry
op_star
id|apm_proc
suffix:semicolon
r_int
id|ret
suffix:semicolon
r_int
id|i
suffix:semicolon
id|dmi_check_system
c_func
(paren
id|apm_dmi_table
)paren
suffix:semicolon
r_if
c_cond
(paren
id|apm_info.bios.version
op_eq
l_int|0
)paren
(brace
id|printk
c_func
(paren
id|KERN_INFO
l_string|&quot;apm: BIOS not found.&bslash;n&quot;
)paren
suffix:semicolon
r_return
op_minus
id|ENODEV
suffix:semicolon
)brace
id|printk
c_func
(paren
id|KERN_INFO
l_string|&quot;apm: BIOS version %d.%d Flags 0x%02x (Driver version %s)&bslash;n&quot;
comma
(paren
(paren
id|apm_info.bios.version
op_rshift
l_int|8
)paren
op_amp
l_int|0xff
)paren
comma
(paren
id|apm_info.bios.version
op_amp
l_int|0xff
)paren
comma
id|apm_info.bios.flags
comma
id|driver_version
)paren
suffix:semicolon
r_if
c_cond
(paren
(paren
id|apm_info.bios.flags
op_amp
id|APM_32_BIT_SUPPORT
)paren
op_eq
l_int|0
)paren
(brace
id|printk
c_func
(paren
id|KERN_INFO
l_string|&quot;apm: no 32 bit BIOS support&bslash;n&quot;
)paren
suffix:semicolon
r_return
op_minus
id|ENODEV
suffix:semicolon
)brace
r_if
c_cond
(paren
id|allow_ints
)paren
id|apm_info.allow_ints
op_assign
l_int|1
suffix:semicolon
r_if
c_cond
(paren
id|broken_psr
)paren
id|apm_info.get_power_status_broken
op_assign
l_int|1
suffix:semicolon
r_if
c_cond
(paren
id|realmode_power_off
)paren
id|apm_info.realmode_power_off
op_assign
l_int|1
suffix:semicolon
multiline_comment|/* User can override, but default is to trust DMI */
r_if
c_cond
(paren
id|apm_disabled
op_ne
op_minus
l_int|1
)paren
id|apm_info.disabled
op_assign
id|apm_disabled
suffix:semicolon
multiline_comment|/*&n;&t; * Fix for the Compaq Contura 3/25c which reports BIOS version 0.1&n;&t; * but is reportedly a 1.0 BIOS.&n;&t; */
r_if
c_cond
(paren
id|apm_info.bios.version
op_eq
l_int|0x001
)paren
id|apm_info.bios.version
op_assign
l_int|0x100
suffix:semicolon
multiline_comment|/* BIOS &lt; 1.2 doesn&squot;t set cseg_16_len */
r_if
c_cond
(paren
id|apm_info.bios.version
OL
l_int|0x102
)paren
id|apm_info.bios.cseg_16_len
op_assign
l_int|0
suffix:semicolon
multiline_comment|/* 64k */
r_if
c_cond
(paren
id|debug
)paren
(brace
id|printk
c_func
(paren
id|KERN_INFO
l_string|&quot;apm: entry %x:%lx cseg16 %x dseg %x&quot;
comma
id|apm_info.bios.cseg
comma
id|apm_info.bios.offset
comma
id|apm_info.bios.cseg_16
comma
id|apm_info.bios.dseg
)paren
suffix:semicolon
r_if
c_cond
(paren
id|apm_info.bios.version
OG
l_int|0x100
)paren
id|printk
c_func
(paren
l_string|&quot; cseg len %x, dseg len %x&quot;
comma
id|apm_info.bios.cseg_len
comma
id|apm_info.bios.dseg_len
)paren
suffix:semicolon
r_if
c_cond
(paren
id|apm_info.bios.version
OG
l_int|0x101
)paren
id|printk
c_func
(paren
l_string|&quot; cseg16 len %x&quot;
comma
id|apm_info.bios.cseg_16_len
)paren
suffix:semicolon
id|printk
c_func
(paren
l_string|&quot;&bslash;n&quot;
)paren
suffix:semicolon
)brace
r_if
c_cond
(paren
id|apm_info.disabled
)paren
(brace
id|printk
c_func
(paren
id|KERN_NOTICE
l_string|&quot;apm: disabled on user request.&bslash;n&quot;
)paren
suffix:semicolon
r_return
op_minus
id|ENODEV
suffix:semicolon
)brace
r_if
c_cond
(paren
(paren
id|num_online_cpus
c_func
(paren
)paren
OG
l_int|1
)paren
op_logical_and
op_logical_neg
id|power_off
op_logical_and
op_logical_neg
id|smp
)paren
(brace
id|printk
c_func
(paren
id|KERN_NOTICE
l_string|&quot;apm: disabled - APM is not SMP safe.&bslash;n&quot;
)paren
suffix:semicolon
id|apm_info.disabled
op_assign
l_int|1
suffix:semicolon
r_return
op_minus
id|ENODEV
suffix:semicolon
)brace
r_if
c_cond
(paren
id|PM_IS_ACTIVE
c_func
(paren
)paren
)paren
(brace
id|printk
c_func
(paren
id|KERN_NOTICE
l_string|&quot;apm: overridden by ACPI.&bslash;n&quot;
)paren
suffix:semicolon
id|apm_info.disabled
op_assign
l_int|1
suffix:semicolon
r_return
op_minus
id|ENODEV
suffix:semicolon
)brace
id|pm_active
op_assign
l_int|1
suffix:semicolon
multiline_comment|/*&n;&t; * Set up a segment that references the real mode segment 0x40&n;&t; * that extends up to the end of page zero (that we have reserved).&n;&t; * This is for buggy BIOS&squot;s that refer to (real mode) segment 0x40&n;&t; * even though they are called in protected mode.&n;&t; */
id|set_base
c_func
(paren
id|bad_bios_desc
comma
id|__va
c_func
(paren
(paren
r_int
r_int
)paren
l_int|0x40
op_lshift
l_int|4
)paren
)paren
suffix:semicolon
id|_set_limit
c_func
(paren
(paren
r_char
op_star
)paren
op_amp
id|bad_bios_desc
comma
l_int|4095
op_minus
(paren
l_int|0x40
op_lshift
l_int|4
)paren
)paren
suffix:semicolon
id|apm_bios_entry.offset
op_assign
id|apm_info.bios.offset
suffix:semicolon
id|apm_bios_entry.segment
op_assign
id|APM_CS
suffix:semicolon
r_for
c_loop
(paren
id|i
op_assign
l_int|0
suffix:semicolon
id|i
OL
id|NR_CPUS
suffix:semicolon
id|i
op_increment
)paren
(brace
id|set_base
c_func
(paren
id|per_cpu
c_func
(paren
id|cpu_gdt_table
comma
id|i
)paren
(braket
id|APM_CS
op_rshift
l_int|3
)braket
comma
id|__va
c_func
(paren
(paren
r_int
r_int
)paren
id|apm_info.bios.cseg
op_lshift
l_int|4
)paren
)paren
suffix:semicolon
id|set_base
c_func
(paren
id|per_cpu
c_func
(paren
id|cpu_gdt_table
comma
id|i
)paren
(braket
id|APM_CS_16
op_rshift
l_int|3
)braket
comma
id|__va
c_func
(paren
(paren
r_int
r_int
)paren
id|apm_info.bios.cseg_16
op_lshift
l_int|4
)paren
)paren
suffix:semicolon
id|set_base
c_func
(paren
id|per_cpu
c_func
(paren
id|cpu_gdt_table
comma
id|i
)paren
(braket
id|APM_DS
op_rshift
l_int|3
)braket
comma
id|__va
c_func
(paren
(paren
r_int
r_int
)paren
id|apm_info.bios.dseg
op_lshift
l_int|4
)paren
)paren
suffix:semicolon
macro_line|#ifndef APM_RELAX_SEGMENTS
r_if
c_cond
(paren
id|apm_info.bios.version
op_eq
l_int|0x100
)paren
(brace
macro_line|#endif
multiline_comment|/* For ASUS motherboard, Award BIOS rev 110 (and others?) */
id|_set_limit
c_func
(paren
(paren
r_char
op_star
)paren
op_amp
id|per_cpu
c_func
(paren
id|cpu_gdt_table
comma
id|i
)paren
(braket
id|APM_CS
op_rshift
l_int|3
)braket
comma
l_int|64
op_star
l_int|1024
op_minus
l_int|1
)paren
suffix:semicolon
multiline_comment|/* For some unknown machine. */
id|_set_limit
c_func
(paren
(paren
r_char
op_star
)paren
op_amp
id|per_cpu
c_func
(paren
id|cpu_gdt_table
comma
id|i
)paren
(braket
id|APM_CS_16
op_rshift
l_int|3
)braket
comma
l_int|64
op_star
l_int|1024
op_minus
l_int|1
)paren
suffix:semicolon
multiline_comment|/* For the DEC Hinote Ultra CT475 (and others?) */
id|_set_limit
c_func
(paren
(paren
r_char
op_star
)paren
op_amp
id|per_cpu
c_func
(paren
id|cpu_gdt_table
comma
id|i
)paren
(braket
id|APM_DS
op_rshift
l_int|3
)braket
comma
l_int|64
op_star
l_int|1024
op_minus
l_int|1
)paren
suffix:semicolon
macro_line|#ifndef APM_RELAX_SEGMENTS
)brace
r_else
(brace
id|_set_limit
c_func
(paren
(paren
r_char
op_star
)paren
op_amp
id|per_cpu
c_func
(paren
id|cpu_gdt_table
comma
id|i
)paren
(braket
id|APM_CS
op_rshift
l_int|3
)braket
comma
(paren
id|apm_info.bios.cseg_len
op_minus
l_int|1
)paren
op_amp
l_int|0xffff
)paren
suffix:semicolon
id|_set_limit
c_func
(paren
(paren
r_char
op_star
)paren
op_amp
id|per_cpu
c_func
(paren
id|cpu_gdt_table
comma
id|i
)paren
(braket
id|APM_CS_16
op_rshift
l_int|3
)braket
comma
(paren
id|apm_info.bios.cseg_16_len
op_minus
l_int|1
)paren
op_amp
l_int|0xffff
)paren
suffix:semicolon
id|_set_limit
c_func
(paren
(paren
r_char
op_star
)paren
op_amp
id|per_cpu
c_func
(paren
id|cpu_gdt_table
comma
id|i
)paren
(braket
id|APM_DS
op_rshift
l_int|3
)braket
comma
(paren
id|apm_info.bios.dseg_len
op_minus
l_int|1
)paren
op_amp
l_int|0xffff
)paren
suffix:semicolon
multiline_comment|/* workaround for broken BIOSes */
r_if
c_cond
(paren
id|apm_info.bios.cseg_len
op_le
id|apm_info.bios.offset
)paren
id|_set_limit
c_func
(paren
(paren
r_char
op_star
)paren
op_amp
id|per_cpu
c_func
(paren
id|cpu_gdt_table
comma
id|i
)paren
(braket
id|APM_CS
op_rshift
l_int|3
)braket
comma
l_int|64
op_star
l_int|1024
op_minus
l_int|1
)paren
suffix:semicolon
r_if
c_cond
(paren
id|apm_info.bios.dseg_len
op_le
l_int|0x40
)paren
(brace
multiline_comment|/* 0x40 * 4kB == 64kB */
multiline_comment|/* for the BIOS that assumes granularity = 1 */
id|per_cpu
c_func
(paren
id|cpu_gdt_table
comma
id|i
)paren
(braket
id|APM_DS
op_rshift
l_int|3
)braket
dot
id|b
op_or_assign
l_int|0x800000
suffix:semicolon
id|printk
c_func
(paren
id|KERN_NOTICE
l_string|&quot;apm: we set the granularity of dseg.&bslash;n&quot;
)paren
suffix:semicolon
)brace
)brace
macro_line|#endif
)brace
id|apm_proc
op_assign
id|create_proc_info_entry
c_func
(paren
l_string|&quot;apm&quot;
comma
l_int|0
comma
l_int|NULL
comma
id|apm_get_info
)paren
suffix:semicolon
r_if
c_cond
(paren
id|apm_proc
)paren
id|apm_proc-&gt;owner
op_assign
id|THIS_MODULE
suffix:semicolon
id|ret
op_assign
id|kernel_thread
c_func
(paren
id|apm
comma
l_int|NULL
comma
id|CLONE_KERNEL
op_or
id|SIGCHLD
)paren
suffix:semicolon
r_if
c_cond
(paren
id|ret
OL
l_int|0
)paren
(brace
id|printk
c_func
(paren
id|KERN_ERR
l_string|&quot;apm: disabled - Unable to start kernel thread.&bslash;n&quot;
)paren
suffix:semicolon
r_return
op_minus
id|ENOMEM
suffix:semicolon
)brace
r_if
c_cond
(paren
id|num_online_cpus
c_func
(paren
)paren
OG
l_int|1
op_logical_and
op_logical_neg
id|smp
)paren
(brace
id|printk
c_func
(paren
id|KERN_NOTICE
l_string|&quot;apm: disabled - APM is not SMP safe (power off active).&bslash;n&quot;
)paren
suffix:semicolon
r_return
l_int|0
suffix:semicolon
)brace
id|misc_register
c_func
(paren
op_amp
id|apm_device
)paren
suffix:semicolon
r_if
c_cond
(paren
id|HZ
op_ne
l_int|100
)paren
id|idle_period
op_assign
(paren
id|idle_period
op_star
id|HZ
)paren
op_div
l_int|100
suffix:semicolon
r_if
c_cond
(paren
id|idle_threshold
OL
l_int|100
)paren
(brace
id|original_pm_idle
op_assign
id|pm_idle
suffix:semicolon
id|pm_idle
op_assign
id|apm_cpu_idle
suffix:semicolon
id|set_pm_idle
op_assign
l_int|1
suffix:semicolon
)brace
r_return
l_int|0
suffix:semicolon
)brace
DECL|function|apm_exit
r_static
r_void
id|__exit
id|apm_exit
c_func
(paren
r_void
)paren
(brace
r_int
id|error
suffix:semicolon
r_if
c_cond
(paren
id|set_pm_idle
)paren
(brace
id|pm_idle
op_assign
id|original_pm_idle
suffix:semicolon
multiline_comment|/*&n;&t;&t; * We are about to unload the current idle thread pm callback&n;&t;&t; * (pm_idle), Wait for all processors to update cached/local&n;&t;&t; * copies of pm_idle before proceeding.&n;&t;&t; */
id|cpu_idle_wait
c_func
(paren
)paren
suffix:semicolon
)brace
r_if
c_cond
(paren
(paren
(paren
id|apm_info.bios.flags
op_amp
id|APM_BIOS_DISENGAGED
)paren
op_eq
l_int|0
)paren
op_logical_and
(paren
id|apm_info.connection_version
OG
l_int|0x0100
)paren
)paren
(brace
id|error
op_assign
id|apm_engage_power_management
c_func
(paren
id|APM_DEVICE_ALL
comma
l_int|0
)paren
suffix:semicolon
r_if
c_cond
(paren
id|error
)paren
id|apm_error
c_func
(paren
l_string|&quot;disengage power management&quot;
comma
id|error
)paren
suffix:semicolon
)brace
id|misc_deregister
c_func
(paren
op_amp
id|apm_device
)paren
suffix:semicolon
id|remove_proc_entry
c_func
(paren
l_string|&quot;apm&quot;
comma
l_int|NULL
)paren
suffix:semicolon
r_if
c_cond
(paren
id|power_off
)paren
id|pm_power_off
op_assign
l_int|NULL
suffix:semicolon
id|exit_kapmd
op_assign
l_int|1
suffix:semicolon
r_while
c_loop
(paren
id|kapmd_running
)paren
id|schedule
c_func
(paren
)paren
suffix:semicolon
id|pm_active
op_assign
l_int|0
suffix:semicolon
)brace
DECL|variable|apm_init
id|module_init
c_func
(paren
id|apm_init
)paren
suffix:semicolon
DECL|variable|apm_exit
id|module_exit
c_func
(paren
id|apm_exit
)paren
suffix:semicolon
id|MODULE_AUTHOR
c_func
(paren
l_string|&quot;Stephen Rothwell&quot;
)paren
suffix:semicolon
id|MODULE_DESCRIPTION
c_func
(paren
l_string|&quot;Advanced Power Management&quot;
)paren
suffix:semicolon
id|MODULE_LICENSE
c_func
(paren
l_string|&quot;GPL&quot;
)paren
suffix:semicolon
id|module_param
c_func
(paren
id|debug
comma
r_bool
comma
l_int|0644
)paren
suffix:semicolon
id|MODULE_PARM_DESC
c_func
(paren
id|debug
comma
l_string|&quot;Enable debug mode&quot;
)paren
suffix:semicolon
id|module_param
c_func
(paren
id|power_off
comma
r_bool
comma
l_int|0444
)paren
suffix:semicolon
id|MODULE_PARM_DESC
c_func
(paren
id|power_off
comma
l_string|&quot;Enable power off&quot;
)paren
suffix:semicolon
id|module_param
c_func
(paren
id|bounce_interval
comma
r_int
comma
l_int|0444
)paren
suffix:semicolon
id|MODULE_PARM_DESC
c_func
(paren
id|bounce_interval
comma
l_string|&quot;Set the number of ticks to ignore suspend bounces&quot;
)paren
suffix:semicolon
id|module_param
c_func
(paren
id|allow_ints
comma
r_bool
comma
l_int|0444
)paren
suffix:semicolon
id|MODULE_PARM_DESC
c_func
(paren
id|allow_ints
comma
l_string|&quot;Allow interrupts during BIOS calls&quot;
)paren
suffix:semicolon
id|module_param
c_func
(paren
id|broken_psr
comma
r_bool
comma
l_int|0444
)paren
suffix:semicolon
id|MODULE_PARM_DESC
c_func
(paren
id|broken_psr
comma
l_string|&quot;BIOS has a broken GetPowerStatus call&quot;
)paren
suffix:semicolon
id|module_param
c_func
(paren
id|realmode_power_off
comma
r_bool
comma
l_int|0444
)paren
suffix:semicolon
id|MODULE_PARM_DESC
c_func
(paren
id|realmode_power_off
comma
l_string|&quot;Switch to real mode before powering off&quot;
)paren
suffix:semicolon
id|module_param
c_func
(paren
id|idle_threshold
comma
r_int
comma
l_int|0444
)paren
suffix:semicolon
id|MODULE_PARM_DESC
c_func
(paren
id|idle_threshold
comma
l_string|&quot;System idle percentage above which to make APM BIOS idle calls&quot;
)paren
suffix:semicolon
id|module_param
c_func
(paren
id|idle_period
comma
r_int
comma
l_int|0444
)paren
suffix:semicolon
id|MODULE_PARM_DESC
c_func
(paren
id|idle_period
comma
l_string|&quot;Period (in sec/100) over which to caculate the idle percentage&quot;
)paren
suffix:semicolon
id|module_param
c_func
(paren
id|smp
comma
r_bool
comma
l_int|0444
)paren
suffix:semicolon
id|MODULE_PARM_DESC
c_func
(paren
id|smp
comma
l_string|&quot;Set this to enable APM use on an SMP platform. Use with caution on older systems&quot;
)paren
suffix:semicolon
DECL|variable|APM_MINOR_DEV
id|MODULE_ALIAS_MISCDEV
c_func
(paren
id|APM_MINOR_DEV
)paren
suffix:semicolon
eof
