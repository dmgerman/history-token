...
(brace
singleline_comment|// Pointless. Use send_IPI_mask to do this instead
r_int
id|cpu
suffix:semicolon
r_if
...
suffix:semicolon
multiline_comment|/* We have do to this one last to make sure that the IPI service&n;&t; * code desn&squot;t get confused if it gets an unexpected repeat&n;&t; * trigger of an old IPI while we&squot;re still setting up the new&n;&t; * one. */
id|atomic_set
c_func
(paren
op_amp
...
suffix:semicolon
multiline_comment|/* It is now safe to reuse the &quot;call_data&quot; global, but we need&n;&t; * to keep local bottom-halves disabled until after waiters have&n;&t; * been acknowledged to prevent reuse of the per-cpu call data&n;&t; * entry. */
id|spin_unlock
c_func
(paren
op_amp
...
suffix:semicolon
multiline_comment|/*&n;&t; * Notify initiating CPU that I&squot;ve grabbed the data and am about&n;&t; * to execute the function (and avoid servicing any single IPI&n;&t; * twice)&n;&t; */
r_if
c_cond
(paren
id|test_and_set_bit
