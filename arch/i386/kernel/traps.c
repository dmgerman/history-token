...
suffix:semicolon
multiline_comment|/*&n; * Sometimes, we know that we&squot;re disabling interrupts for too long.&n; * This happens during long writes to slow console devices, and may&n; * happen in other places.&n; *&n; * To prevent the NMI watchdog from firing when we&squot;re doing these things,&n; * touch_nmi_watchdog() may be used to reset the NMI watchdog timer&n; * back to its full interval (five seconds).&n; */
