multiline_comment|/*&n; *&t;Pentium 4/Xeon CPU on demand clock modulation/speed scaling&n; *&t;(C) 2002 Zwane Mwaikambo &lt;zwane@commfireservices.com&gt;&n; *&t;(C) 2002 Arjan van de Ven &lt;arjanv@redhat.com&gt;&n; *&t;(C) 2002 Tora T. Engstad&n; *&t;All Rights Reserved&n; *&n; *&t;This program is free software; you can redistribute it and/or&n; *      modify it under the terms of the GNU General Public License&n; *      as published by the Free Software Foundation; either version&n; *      2 of the License, or (at your option) any later version.&n; *&n; *      The author(s) of this software shall not be held liable for damages&n; *      of any nature resulting due to the use of this software. This&n; *      software is provided AS-IS with no warranties.&n; *&t;&n; *&t;Date&t;&t;Errata&t;&t;&t;Description&n; *&t;20020525&t;N44, O17&t;12.5% or 25% DC causes lockup&n; *&n; */
macro_line|#include &lt;linux/kernel.h&gt;
macro_line|#include &lt;linux/module.h&gt; 
macro_line|#include &lt;linux/init.h&gt;
macro_line|#include &lt;linux/smp.h&gt;
macro_line|#include &lt;linux/cpufreq.h&gt;
macro_line|#include &lt;linux/slab.h&gt;
macro_line|#include &lt;linux/sched.h&gt;
macro_line|#include &lt;asm/processor.h&gt; 
macro_line|#include &lt;asm/msr.h&gt;
macro_line|#include &lt;asm/timex.h&gt;
DECL|macro|PFX
mdefine_line|#define PFX&t;&quot;cpufreq: &quot;
multiline_comment|/*&n; * Duty Cycle (3bits), note DC_DISABLE is not specified in&n; * intel docs i just use it to mean disable&n; */
r_enum
(brace
DECL|enumerator|DC_RESV
DECL|enumerator|DC_DFLT
DECL|enumerator|DC_25PT
DECL|enumerator|DC_38PT
DECL|enumerator|DC_50PT
id|DC_RESV
comma
id|DC_DFLT
comma
id|DC_25PT
comma
id|DC_38PT
comma
id|DC_50PT
comma
DECL|enumerator|DC_64PT
DECL|enumerator|DC_75PT
DECL|enumerator|DC_88PT
DECL|enumerator|DC_DISABLE
id|DC_64PT
comma
id|DC_75PT
comma
id|DC_88PT
comma
id|DC_DISABLE
)brace
suffix:semicolon
DECL|macro|DC_ENTRIES
mdefine_line|#define DC_ENTRIES&t;8
DECL|variable|has_N44_O17_errata
r_static
r_int
id|has_N44_O17_errata
suffix:semicolon
DECL|variable|stock_freq
r_static
r_int
id|stock_freq
suffix:semicolon
id|MODULE_PARM
c_func
(paren
id|stock_freq
comma
l_string|&quot;i&quot;
)paren
suffix:semicolon
DECL|variable|cpufreq_p4_driver
r_static
r_struct
id|cpufreq_driver
op_star
id|cpufreq_p4_driver
suffix:semicolon
DECL|variable|cpufreq_p4_old_state
r_static
r_int
r_int
id|cpufreq_p4_old_state
op_assign
l_int|0
suffix:semicolon
DECL|function|cpufreq_p4_setdc
r_static
r_int
id|cpufreq_p4_setdc
c_func
(paren
r_int
r_int
id|cpu
comma
r_int
r_int
id|newstate
)paren
(brace
id|u32
id|l
comma
id|h
suffix:semicolon
r_int
r_int
id|cpus_allowed
suffix:semicolon
r_struct
id|cpufreq_freqs
id|freqs
suffix:semicolon
r_if
c_cond
(paren
op_logical_neg
id|cpu_online
c_func
(paren
id|cpu
)paren
op_logical_or
(paren
id|newstate
OG
id|DC_DISABLE
)paren
op_logical_or
(paren
id|newstate
op_eq
id|DC_RESV
)paren
)paren
r_return
op_minus
id|EINVAL
suffix:semicolon
id|cpu
op_assign
id|cpu
op_rshift
l_int|1
suffix:semicolon
multiline_comment|/* physical CPU #nr */
multiline_comment|/* notifiers */
id|freqs.old
op_assign
id|stock_freq
op_star
id|cpufreq_p4_old_state
op_div
l_int|8
suffix:semicolon
id|freqs
dot
r_new
op_assign
id|stock_freq
op_star
id|newstate
op_div
l_int|8
suffix:semicolon
id|freqs.cpu
op_assign
l_int|2
op_star
id|cpu
suffix:semicolon
id|cpufreq_notify_transition
c_func
(paren
op_amp
id|freqs
comma
id|CPUFREQ_PRECHANGE
)paren
suffix:semicolon
id|freqs.cpu
op_increment
suffix:semicolon
id|cpufreq_notify_transition
c_func
(paren
op_amp
id|freqs
comma
id|CPUFREQ_PRECHANGE
)paren
suffix:semicolon
multiline_comment|/* switch to physical CPU where state is to be changed*/
id|cpus_allowed
op_assign
id|current-&gt;cpus_allowed
suffix:semicolon
id|set_cpus_allowed
c_func
(paren
id|current
comma
l_int|3
op_lshift
(paren
l_int|2
op_star
id|cpu
)paren
)paren
suffix:semicolon
id|BUG_ON
c_func
(paren
id|cpu
op_ne
(paren
id|smp_processor_id
c_func
(paren
)paren
op_rshift
l_int|1
)paren
)paren
suffix:semicolon
id|rdmsr
c_func
(paren
id|MSR_IA32_THERM_STATUS
comma
id|l
comma
id|h
)paren
suffix:semicolon
r_if
c_cond
(paren
id|l
op_amp
l_int|0x01
)paren
id|printk
c_func
(paren
id|KERN_DEBUG
id|PFX
l_string|&quot;CPU#%d currently thermal throttled&bslash;n&quot;
comma
id|cpu
)paren
suffix:semicolon
r_if
c_cond
(paren
id|has_N44_O17_errata
op_logical_and
(paren
id|newstate
op_eq
id|DC_25PT
op_logical_or
id|newstate
op_eq
id|DC_DFLT
)paren
)paren
id|newstate
op_assign
id|DC_38PT
suffix:semicolon
id|rdmsr
c_func
(paren
id|MSR_IA32_THERM_CONTROL
comma
id|l
comma
id|h
)paren
suffix:semicolon
r_if
c_cond
(paren
id|newstate
op_eq
id|DC_DISABLE
)paren
(brace
id|printk
c_func
(paren
id|KERN_INFO
id|PFX
l_string|&quot;CPU#%d,%d disabling modulation&bslash;n&quot;
comma
id|cpu
comma
(paren
id|cpu
op_plus
l_int|1
)paren
)paren
suffix:semicolon
id|wrmsr
c_func
(paren
id|MSR_IA32_THERM_CONTROL
comma
id|l
op_amp
op_complement
(paren
l_int|1
op_lshift
l_int|4
)paren
comma
id|h
)paren
suffix:semicolon
)brace
r_else
(brace
id|printk
c_func
(paren
id|KERN_INFO
id|PFX
l_string|&quot;CPU#%d,%d setting duty cycle to %d%%&bslash;n&quot;
comma
id|cpu
comma
(paren
id|cpu
op_plus
l_int|1
)paren
comma
(paren
(paren
l_int|125
op_star
id|newstate
)paren
op_div
l_int|10
)paren
)paren
suffix:semicolon
multiline_comment|/* bits 63 - 5&t;: reserved &n;&t;&t; * bit  4&t;: enable/disable&n;&t;&t; * bits 3-1&t;: duty cycle&n;&t;&t; * bit  0&t;: reserved&n;&t;&t; */
id|l
op_assign
(paren
id|l
op_amp
op_complement
l_int|14
)paren
suffix:semicolon
id|l
op_assign
id|l
op_or
(paren
l_int|1
op_lshift
l_int|4
)paren
op_or
(paren
(paren
id|newstate
op_amp
l_int|0x7
)paren
op_lshift
l_int|1
)paren
suffix:semicolon
id|wrmsr
c_func
(paren
id|MSR_IA32_THERM_CONTROL
comma
id|l
comma
id|h
)paren
suffix:semicolon
)brace
id|set_cpus_allowed
c_func
(paren
id|current
comma
id|cpus_allowed
)paren
suffix:semicolon
multiline_comment|/* notifiers */
id|cpufreq_notify_transition
c_func
(paren
op_amp
id|freqs
comma
id|CPUFREQ_POSTCHANGE
)paren
suffix:semicolon
id|freqs.cpu
op_decrement
suffix:semicolon
id|cpufreq_notify_transition
c_func
(paren
op_amp
id|freqs
comma
id|CPUFREQ_POSTCHANGE
)paren
suffix:semicolon
id|cpufreq_p4_old_state
op_assign
id|newstate
suffix:semicolon
r_return
l_int|0
suffix:semicolon
)brace
DECL|function|cpufreq_p4_setpolicy
r_static
r_void
id|cpufreq_p4_setpolicy
c_func
(paren
r_struct
id|cpufreq_policy
op_star
id|policy
)paren
(brace
r_int
r_int
id|i
suffix:semicolon
r_int
r_int
id|newstate
op_assign
l_int|0
suffix:semicolon
r_int
r_int
id|number_states
op_assign
l_int|0
suffix:semicolon
r_if
c_cond
(paren
op_logical_neg
id|cpufreq_p4_driver
op_logical_or
op_logical_neg
id|stock_freq
op_logical_or
op_logical_neg
id|policy
)paren
r_return
suffix:semicolon
r_if
c_cond
(paren
id|policy-&gt;policy
op_eq
id|CPUFREQ_POLICY_POWERSAVE
)paren
(brace
r_for
c_loop
(paren
id|i
op_assign
l_int|8
suffix:semicolon
id|i
OG
l_int|0
suffix:semicolon
id|i
op_increment
)paren
r_if
c_cond
(paren
(paren
id|policy-&gt;min
op_le
(paren
(paren
id|stock_freq
op_div
l_int|8
)paren
op_star
id|i
)paren
)paren
op_logical_and
(paren
id|policy-&gt;max
op_ge
(paren
(paren
id|stock_freq
op_div
l_int|8
)paren
op_star
id|i
)paren
)paren
)paren
(brace
id|newstate
op_assign
id|i
suffix:semicolon
id|number_states
op_increment
suffix:semicolon
)brace
)brace
r_else
(brace
r_for
c_loop
(paren
id|i
op_assign
l_int|0
suffix:semicolon
id|i
op_le
l_int|8
suffix:semicolon
id|i
op_decrement
)paren
r_if
c_cond
(paren
(paren
id|policy-&gt;min
op_le
(paren
(paren
id|stock_freq
op_div
l_int|8
)paren
op_star
id|i
)paren
)paren
op_logical_and
(paren
id|policy-&gt;max
op_ge
(paren
(paren
id|stock_freq
op_div
l_int|8
)paren
op_star
id|i
)paren
)paren
)paren
(brace
id|newstate
op_assign
id|i
suffix:semicolon
id|number_states
op_increment
suffix:semicolon
)brace
)brace
multiline_comment|/* if (number_states == 1) */
(brace
r_if
c_cond
(paren
id|policy-&gt;cpu
op_eq
id|CPUFREQ_ALL_CPUS
)paren
(brace
r_for
c_loop
(paren
id|i
op_assign
l_int|0
suffix:semicolon
id|i
OL
(paren
id|NR_CPUS
op_div
l_int|2
)paren
suffix:semicolon
id|i
op_increment
)paren
r_if
c_cond
(paren
id|cpu_online
c_func
(paren
l_int|2
op_star
id|i
)paren
)paren
id|cpufreq_p4_setdc
c_func
(paren
(paren
l_int|2
op_star
id|i
)paren
comma
id|newstate
)paren
suffix:semicolon
)brace
r_else
(brace
id|cpufreq_p4_setdc
c_func
(paren
id|policy-&gt;cpu
comma
id|newstate
)paren
suffix:semicolon
)brace
)brace
multiline_comment|/* else {&n;&t;&t;if (policy-&gt;policy == CPUFREQ_POLICY_POWERSAVE) {&n;&t;&t;&t;min_state = newstate;&n;&t;&t;&t;max_state = newstate + (number_states - 1);&n;&t;&t;} else {&n;&t;&t;&t;max_state = newstate;&n;&t;&t;&t;min_state = newstate - (number_states - 1);&n;&t;&t;}&n;&t;} */
)brace
DECL|function|cpufreq_p4_verify
r_static
r_void
id|cpufreq_p4_verify
c_func
(paren
r_struct
id|cpufreq_policy
op_star
id|policy
)paren
(brace
r_int
r_int
id|number_states
op_assign
l_int|0
suffix:semicolon
r_int
r_int
id|i
suffix:semicolon
r_if
c_cond
(paren
op_logical_neg
id|cpufreq_p4_driver
op_logical_or
op_logical_neg
id|stock_freq
op_logical_or
op_logical_neg
id|policy
)paren
r_return
suffix:semicolon
r_if
c_cond
(paren
op_logical_neg
id|cpu_online
c_func
(paren
id|policy-&gt;cpu
)paren
)paren
id|policy-&gt;cpu
op_assign
id|CPUFREQ_ALL_CPUS
suffix:semicolon
id|cpufreq_verify_within_limits
c_func
(paren
id|policy
comma
(paren
id|stock_freq
op_div
l_int|8
)paren
comma
id|stock_freq
)paren
suffix:semicolon
multiline_comment|/* is there at least one state within limit? */
r_for
c_loop
(paren
id|i
op_assign
l_int|1
suffix:semicolon
id|i
op_le
l_int|8
suffix:semicolon
id|i
op_increment
)paren
r_if
c_cond
(paren
(paren
id|policy-&gt;min
op_le
(paren
(paren
id|stock_freq
op_div
l_int|8
)paren
op_star
id|i
)paren
)paren
op_logical_and
(paren
id|policy-&gt;max
op_ge
(paren
(paren
id|stock_freq
op_div
l_int|8
)paren
op_star
id|i
)paren
)paren
)paren
id|number_states
op_increment
suffix:semicolon
r_if
c_cond
(paren
id|number_states
)paren
r_return
suffix:semicolon
id|policy-&gt;max
op_assign
(paren
id|stock_freq
op_div
l_int|8
)paren
op_star
(paren
(paren
(paren
r_int
r_int
)paren
(paren
(paren
id|policy-&gt;max
op_star
l_int|8
)paren
op_div
id|stock_freq
)paren
)paren
op_plus
l_int|1
)paren
suffix:semicolon
r_return
suffix:semicolon
)brace
DECL|function|cpufreq_p4_init
r_int
id|__init
id|cpufreq_p4_init
c_func
(paren
r_void
)paren
(brace
r_struct
id|cpuinfo_x86
op_star
id|c
op_assign
id|cpu_data
suffix:semicolon
r_int
id|cpuid
suffix:semicolon
r_int
id|ret
suffix:semicolon
r_struct
id|cpufreq_driver
op_star
id|driver
suffix:semicolon
r_int
r_int
id|i
suffix:semicolon
multiline_comment|/*&n;&t; * THERM_CONTROL is architectural for IA32 now, so &n;&t; * we can rely on the capability checks&n;&t; */
r_if
c_cond
(paren
id|c-&gt;x86_vendor
op_ne
id|X86_VENDOR_INTEL
)paren
r_return
op_minus
id|ENODEV
suffix:semicolon
r_if
c_cond
(paren
op_logical_neg
id|test_bit
c_func
(paren
id|X86_FEATURE_ACPI
comma
id|c-&gt;x86_capability
)paren
op_logical_or
op_logical_neg
id|test_bit
c_func
(paren
id|X86_FEATURE_ACC
comma
id|c-&gt;x86_capability
)paren
)paren
r_return
op_minus
id|ENODEV
suffix:semicolon
multiline_comment|/* Errata workarounds */
id|cpuid
op_assign
(paren
id|c-&gt;x86
op_lshift
l_int|8
)paren
op_or
(paren
id|c-&gt;x86_model
op_lshift
l_int|4
)paren
op_or
id|c-&gt;x86_mask
suffix:semicolon
r_switch
c_cond
(paren
id|cpuid
)paren
(brace
r_case
l_int|0x0f07
suffix:colon
r_case
l_int|0x0f0a
suffix:colon
r_case
l_int|0x0f11
suffix:colon
r_case
l_int|0x0f12
suffix:colon
id|has_N44_O17_errata
op_assign
l_int|1
suffix:semicolon
r_default
suffix:colon
r_break
suffix:semicolon
)brace
id|printk
c_func
(paren
id|KERN_INFO
id|PFX
l_string|&quot;P4/Xeon(TM) CPU On-Demand Clock Modulation available&bslash;n&quot;
)paren
suffix:semicolon
id|driver
op_assign
id|kmalloc
c_func
(paren
r_sizeof
(paren
r_struct
id|cpufreq_driver
)paren
op_plus
id|NR_CPUS
op_star
r_sizeof
(paren
r_struct
id|cpufreq_freqs
)paren
comma
id|GFP_KERNEL
)paren
suffix:semicolon
r_if
c_cond
(paren
op_logical_neg
id|driver
)paren
r_return
op_minus
id|ENOMEM
suffix:semicolon
id|driver-&gt;policy
op_assign
(paren
r_struct
id|cpufreq_policy
op_star
)paren
(paren
id|driver
op_plus
r_sizeof
(paren
r_struct
id|cpufreq_driver
)paren
)paren
suffix:semicolon
r_if
c_cond
(paren
op_logical_neg
id|stock_freq
)paren
id|stock_freq
op_assign
id|cpu_khz
suffix:semicolon
macro_line|#ifdef CONFIG_CPU_FREQ_24_API
id|driver-&gt;cpu_min_freq
op_assign
id|stock_freq
op_div
l_int|8
suffix:semicolon
r_for
c_loop
(paren
id|i
op_assign
l_int|0
suffix:semicolon
id|i
OL
id|NR_CPUS
suffix:semicolon
id|i
op_increment
)paren
id|driver-&gt;cpu_cur_freq
(braket
id|i
)braket
op_assign
id|stock_freq
suffix:semicolon
macro_line|#endif
id|cpufreq_p4_old_state
op_assign
id|DC_DISABLE
suffix:semicolon
id|driver-&gt;verify
op_assign
op_amp
id|cpufreq_p4_verify
suffix:semicolon
id|driver-&gt;setpolicy
op_assign
op_amp
id|cpufreq_p4_setpolicy
suffix:semicolon
r_for
c_loop
(paren
id|i
op_assign
l_int|0
suffix:semicolon
id|i
OL
id|NR_CPUS
suffix:semicolon
id|i
op_increment
)paren
(brace
r_if
c_cond
(paren
id|has_N44_O17_errata
)paren
id|driver-&gt;policy
(braket
id|i
)braket
dot
id|min
op_assign
(paren
id|stock_freq
op_star
l_int|3
)paren
op_div
l_int|8
suffix:semicolon
r_else
id|driver-&gt;policy
(braket
id|i
)braket
dot
id|min
op_assign
id|stock_freq
op_div
l_int|8
suffix:semicolon
id|driver-&gt;policy
(braket
id|i
)braket
dot
id|max
op_assign
id|stock_freq
suffix:semicolon
id|driver-&gt;policy
(braket
id|i
)braket
dot
id|policy
op_assign
id|CPUFREQ_POLICY_PERFORMANCE
suffix:semicolon
id|driver-&gt;policy
(braket
id|i
)braket
dot
id|max_cpu_freq
op_assign
id|stock_freq
suffix:semicolon
id|driver-&gt;policy
(braket
id|i
)braket
dot
id|cpu
op_assign
id|i
suffix:semicolon
)brace
id|ret
op_assign
id|cpufreq_register
c_func
(paren
id|driver
)paren
suffix:semicolon
r_if
c_cond
(paren
id|ret
)paren
(brace
id|kfree
c_func
(paren
id|driver
)paren
suffix:semicolon
r_return
id|ret
suffix:semicolon
)brace
id|cpufreq_p4_driver
op_assign
id|driver
suffix:semicolon
r_return
l_int|0
suffix:semicolon
)brace
DECL|function|cpufreq_p4_exit
r_void
id|__exit
id|cpufreq_p4_exit
c_func
(paren
r_void
)paren
(brace
id|u32
id|l
comma
id|h
suffix:semicolon
r_if
c_cond
(paren
id|cpufreq_p4_driver
)paren
(brace
id|cpufreq_unregister
c_func
(paren
)paren
suffix:semicolon
multiline_comment|/* return back to a non modulated state */
id|rdmsr
c_func
(paren
id|MSR_IA32_THERM_CONTROL
comma
id|l
comma
id|h
)paren
suffix:semicolon
id|wrmsr
c_func
(paren
id|MSR_IA32_THERM_CONTROL
comma
id|l
op_amp
op_complement
(paren
l_int|1
op_lshift
l_int|4
)paren
comma
id|h
)paren
suffix:semicolon
id|kfree
c_func
(paren
id|cpufreq_p4_driver
)paren
suffix:semicolon
)brace
)brace
id|MODULE_AUTHOR
(paren
l_string|&quot;Zwane Mwaikambo &lt;zwane@commfireservices.com&gt;&quot;
)paren
suffix:semicolon
id|MODULE_DESCRIPTION
(paren
l_string|&quot;cpufreq driver for Pentium(TM) 4/Xeon(TM)&quot;
)paren
suffix:semicolon
id|MODULE_LICENSE
(paren
l_string|&quot;GPL&quot;
)paren
suffix:semicolon
DECL|variable|cpufreq_p4_init
id|module_init
c_func
(paren
id|cpufreq_p4_init
)paren
suffix:semicolon
DECL|variable|cpufreq_p4_exit
id|module_exit
c_func
(paren
id|cpufreq_p4_exit
)paren
suffix:semicolon
eof
