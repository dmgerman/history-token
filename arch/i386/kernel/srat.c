...
multiline_comment|/*&n; * The SRAT table always lists ascending addresses, so can always&n; * assume that the first &quot;start&quot; address that you see is the real&n; * start of the node, and that the current &quot;end&quot; address is after&n; * the previous one.&n; */
DECL|function|node_read_chunk
r_static
id|__init
r_void
id|node_read_chunk
...
(brace
multiline_comment|/*&n;&t; * Only add present memory as told by the e820.&n;&t; * There is no guarantee from the SRAT that the memory it&n;&t; * enumerates is present at boot time because it represents&n;&t; * *possible* memory hotplug areas the same as normal RAM.&n;&t; */
r_if
c_cond
(paren
id|memory_chunk-&gt;start_pfn
op_ge
id|max_pfn
)paren
(brace
...
(paren
id|KERN_INFO
l_string|&quot;Ignoring SRAT pfns: 0x%08lx -&gt; %08lx&bslash;n&quot;
comma
id|memory_chunk-&gt;start_pfn
comma
id|memory_chunk-&gt;end_pfn
)paren
suffix:semicolon
r_return
suffix:semicolon
