...
multiline_comment|/* ------ Calibrate the TSC based on HPET timer -------&n; * Return 2^32 * (1 / (TSC clocks per usec)) for do_fast_gettimeoffset().&n; * calibrate_tsc() calibrates the processor TSC by comparing&n; * it to the HPET timer of known frequency.&n; * Too much 64-bit arithmetic here to do this cleanly in C&n; */
