...
mdefine_line|#define COMPATIBLE_IOCTL(cmd) { cmd, (unsigned long)sys_ioctl, 0 }
DECL|macro|HANDLE_IOCTL
mdefine_line|#define HANDLE_IOCTL(cmd,handler) { cmd, (unsigned long)handler, 0 }
...
op_assign
(brace
multiline_comment|/* List here explicitly which ioctl&squot;s need translation,&n;     * all others default to calling sys_ioctl().&n;     */
