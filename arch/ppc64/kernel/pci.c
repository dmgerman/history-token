...
id|bus
suffix:semicolon
multiline_comment|/* For now, override phys_mem_access_prot. If we need it,&n;&t; * later, we may move that initialization to each ppc_md&n;&t; */
...
suffix:semicolon
multiline_comment|/* Write combine is always 0 on non-memory space mappings. On&n;&t; * memory space, if the user didn&squot;t pass 1, we check for a&n;&t; * &quot;prefetchable&quot; resource. This is a bit hackish, but we use&n;&t; * this to workaround the inability of /sysfs to provide a write&n;&t; * combine bit&n;&t; */
r_if
c_cond
(paren
id|mmap_state
...
c_func
(paren
l_string|&quot;PCI map for %s:%lx, prot: %lx&bslash;n&quot;
comma
id|pci_name
c_func
(paren
...
multiline_comment|/*&n; * This one is used by /dev/mem and fbdev who have no clue about the&n; * PCI device, it tries to find the PCI device first and calls the&n; * above routine&n; */
DECL|function|pci_phys_mem_access_prot
id|pgprot_t
id|pci_phys_mem_access_prot
c_func
(paren
...
c_func
(paren
l_string|&quot;non-PCI map for %lx, prot: %lx&bslash;n&quot;
comma
id|offset
comma
id|prot
