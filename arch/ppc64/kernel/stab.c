multiline_comment|/*&n; * PowerPC64 Segment Translation Support.&n; *&n; * Dave Engebretsen and Mike Corrigan {engebret|mikejc}@us.ibm.com&n; *    Copyright (c) 2001 Dave Engebretsen&n; *&n; * Copyright (C) 2002 Anton Blanchard &lt;anton@au.ibm.com&gt;, IBM&n; * &n; *      This program is free software; you can redistribute it and/or&n; *      modify it under the terms of the GNU General Public License&n; *      as published by the Free Software Foundation; either version&n; *      2 of the License, or (at your option) any later version.&n; */
macro_line|#include &lt;linux/config.h&gt;
macro_line|#include &lt;asm/pgtable.h&gt;
macro_line|#include &lt;asm/mmu.h&gt;
macro_line|#include &lt;asm/mmu_context.h&gt;
macro_line|#include &lt;asm/paca.h&gt;
macro_line|#include &lt;asm/naca.h&gt;
macro_line|#include &lt;asm/cputable.h&gt;
r_static
r_int
id|make_ste
c_func
(paren
r_int
r_int
id|stab
comma
r_int
r_int
id|esid
comma
r_int
r_int
id|vsid
)paren
suffix:semicolon
r_static
r_void
id|make_slbe
c_func
(paren
r_int
r_int
id|esid
comma
r_int
r_int
id|vsid
comma
r_int
id|large
comma
r_int
id|kernel_segment
)paren
suffix:semicolon
multiline_comment|/*&n; * Build an entry for the base kernel segment and put it into&n; * the segment table or SLB.  All other segment table or SLB&n; * entries are faulted in.&n; */
DECL|function|stab_initialize
r_void
id|stab_initialize
c_func
(paren
r_int
r_int
id|stab
)paren
(brace
r_int
r_int
id|esid
comma
id|vsid
suffix:semicolon
r_int
id|seg0_largepages
op_assign
l_int|0
suffix:semicolon
id|esid
op_assign
id|GET_ESID
c_func
(paren
id|KERNELBASE
)paren
suffix:semicolon
id|vsid
op_assign
id|get_kernel_vsid
c_func
(paren
id|esid
op_lshift
id|SID_SHIFT
)paren
suffix:semicolon
r_if
c_cond
(paren
id|cur_cpu_spec-&gt;cpu_features
op_amp
id|CPU_FTR_16M_PAGE
)paren
id|seg0_largepages
op_assign
l_int|1
suffix:semicolon
r_if
c_cond
(paren
id|cur_cpu_spec-&gt;cpu_features
op_amp
id|CPU_FTR_SLB
)paren
(brace
multiline_comment|/* Invalidate the entire SLB &amp; all the ERATS */
macro_line|#ifdef CONFIG_PPC_ISERIES
id|asm
r_volatile
(paren
l_string|&quot;isync; slbia; isync&quot;
op_scope_resolution
suffix:colon
l_string|&quot;memory&quot;
)paren
suffix:semicolon
multiline_comment|/*&n;&t;&t; * The hypervisor loads SLB entry 0, but we need to increment&n;&t;&t; * next_round_robin to avoid overwriting it&n;&t;&t; */
id|get_paca
c_func
(paren
)paren
op_member_access_from_pointer
id|xStab_data.next_round_robin
op_assign
l_int|1
suffix:semicolon
macro_line|#else
id|asm
r_volatile
(paren
l_string|&quot;isync&quot;
op_scope_resolution
suffix:colon
l_string|&quot;memory&quot;
)paren
suffix:semicolon
id|asm
r_volatile
(paren
l_string|&quot;slbmte  %0,%0&quot;
op_scope_resolution
l_string|&quot;r&quot;
(paren
l_int|0
)paren
suffix:colon
l_string|&quot;memory&quot;
)paren
suffix:semicolon
id|asm
r_volatile
(paren
l_string|&quot;isync; slbia; isync&quot;
op_scope_resolution
suffix:colon
l_string|&quot;memory&quot;
)paren
suffix:semicolon
id|make_slbe
c_func
(paren
id|esid
comma
id|vsid
comma
id|seg0_largepages
comma
l_int|1
)paren
suffix:semicolon
id|asm
r_volatile
(paren
l_string|&quot;isync&quot;
op_scope_resolution
suffix:colon
l_string|&quot;memory&quot;
)paren
suffix:semicolon
macro_line|#endif
multiline_comment|/*&n;&t;&t; * Bolt in the first vmalloc segment. Since modules end&n;&t;&t; * up there it gets hit very heavily.&n;&t;&t; */
id|esid
op_assign
id|GET_ESID
c_func
(paren
id|VMALLOCBASE
)paren
suffix:semicolon
id|vsid
op_assign
id|get_kernel_vsid
c_func
(paren
id|VMALLOCBASE
)paren
suffix:semicolon
id|make_slbe
c_func
(paren
id|esid
comma
id|vsid
comma
l_int|0
comma
l_int|1
)paren
suffix:semicolon
)brace
r_else
(brace
id|asm
r_volatile
(paren
l_string|&quot;isync; slbia; isync&quot;
op_scope_resolution
suffix:colon
l_string|&quot;memory&quot;
)paren
suffix:semicolon
id|make_ste
c_func
(paren
id|stab
comma
id|esid
comma
id|vsid
)paren
suffix:semicolon
multiline_comment|/* Order update */
id|asm
r_volatile
(paren
l_string|&quot;sync&quot;
op_scope_resolution
suffix:colon
l_string|&quot;memory&quot;
)paren
suffix:semicolon
)brace
)brace
multiline_comment|/* Both the segment table and SLB code uses the following cache */
DECL|macro|NR_STAB_CACHE_ENTRIES
mdefine_line|#define NR_STAB_CACHE_ENTRIES 8
id|DEFINE_PER_CPU
c_func
(paren
r_int
comma
id|stab_cache_ptr
)paren
suffix:semicolon
id|DEFINE_PER_CPU
c_func
(paren
r_int
comma
id|stab_cache
(braket
id|NR_STAB_CACHE_ENTRIES
)braket
)paren
suffix:semicolon
multiline_comment|/*&n; * Segment table stuff&n; */
multiline_comment|/*&n; * Create a segment table entry for the given esid/vsid pair.&n; */
DECL|function|make_ste
r_static
r_int
id|make_ste
c_func
(paren
r_int
r_int
id|stab
comma
r_int
r_int
id|esid
comma
r_int
r_int
id|vsid
)paren
(brace
r_int
r_int
id|entry
comma
id|group
comma
id|old_esid
comma
id|castout_entry
comma
id|i
suffix:semicolon
r_int
r_int
id|global_entry
suffix:semicolon
id|STE
op_star
id|ste
comma
op_star
id|castout_ste
suffix:semicolon
r_int
r_int
id|kernel_segment
op_assign
(paren
id|REGION_ID
c_func
(paren
id|esid
op_lshift
id|SID_SHIFT
)paren
op_ne
id|USER_REGION_ID
)paren
suffix:semicolon
multiline_comment|/* Search the primary group first. */
id|global_entry
op_assign
(paren
id|esid
op_amp
l_int|0x1f
)paren
op_lshift
l_int|3
suffix:semicolon
id|ste
op_assign
(paren
id|STE
op_star
)paren
(paren
id|stab
op_or
(paren
(paren
id|esid
op_amp
l_int|0x1f
)paren
op_lshift
l_int|7
)paren
)paren
suffix:semicolon
multiline_comment|/* Find an empty entry, if one exists. */
r_for
c_loop
(paren
id|group
op_assign
l_int|0
suffix:semicolon
id|group
OL
l_int|2
suffix:semicolon
id|group
op_increment
)paren
(brace
r_for
c_loop
(paren
id|entry
op_assign
l_int|0
suffix:semicolon
id|entry
OL
l_int|8
suffix:semicolon
id|entry
op_increment
comma
id|ste
op_increment
)paren
(brace
r_if
c_cond
(paren
op_logical_neg
(paren
id|ste-&gt;dw0.dw0.v
)paren
)paren
(brace
id|ste-&gt;dw0.dword0
op_assign
l_int|0
suffix:semicolon
id|ste-&gt;dw1.dword1
op_assign
l_int|0
suffix:semicolon
id|ste-&gt;dw1.dw1.vsid
op_assign
id|vsid
suffix:semicolon
id|ste-&gt;dw0.dw0.esid
op_assign
id|esid
suffix:semicolon
id|ste-&gt;dw0.dw0.kp
op_assign
l_int|1
suffix:semicolon
r_if
c_cond
(paren
op_logical_neg
id|kernel_segment
)paren
id|ste-&gt;dw0.dw0.ks
op_assign
l_int|1
suffix:semicolon
id|asm
r_volatile
(paren
l_string|&quot;eieio&quot;
op_scope_resolution
suffix:colon
l_string|&quot;memory&quot;
)paren
suffix:semicolon
id|ste-&gt;dw0.dw0.v
op_assign
l_int|1
suffix:semicolon
r_return
(paren
id|global_entry
op_or
id|entry
)paren
suffix:semicolon
)brace
)brace
multiline_comment|/* Now search the secondary group. */
id|global_entry
op_assign
(paren
(paren
op_complement
id|esid
)paren
op_amp
l_int|0x1f
)paren
op_lshift
l_int|3
suffix:semicolon
id|ste
op_assign
(paren
id|STE
op_star
)paren
(paren
id|stab
op_or
(paren
(paren
(paren
op_complement
id|esid
)paren
op_amp
l_int|0x1f
)paren
op_lshift
l_int|7
)paren
)paren
suffix:semicolon
)brace
multiline_comment|/*&n;&t; * Could not find empty entry, pick one with a round robin selection.&n;&t; * Search all entries in the two groups.&n;&t; */
id|castout_entry
op_assign
id|get_paca
c_func
(paren
)paren
op_member_access_from_pointer
id|xStab_data.next_round_robin
suffix:semicolon
r_for
c_loop
(paren
id|i
op_assign
l_int|0
suffix:semicolon
id|i
OL
l_int|16
suffix:semicolon
id|i
op_increment
)paren
(brace
r_if
c_cond
(paren
id|castout_entry
OL
l_int|8
)paren
(brace
id|global_entry
op_assign
(paren
id|esid
op_amp
l_int|0x1f
)paren
op_lshift
l_int|3
suffix:semicolon
id|ste
op_assign
(paren
id|STE
op_star
)paren
(paren
id|stab
op_or
(paren
(paren
id|esid
op_amp
l_int|0x1f
)paren
op_lshift
l_int|7
)paren
)paren
suffix:semicolon
id|castout_ste
op_assign
id|ste
op_plus
id|castout_entry
suffix:semicolon
)brace
r_else
(brace
id|global_entry
op_assign
(paren
(paren
op_complement
id|esid
)paren
op_amp
l_int|0x1f
)paren
op_lshift
l_int|3
suffix:semicolon
id|ste
op_assign
(paren
id|STE
op_star
)paren
(paren
id|stab
op_or
(paren
(paren
(paren
op_complement
id|esid
)paren
op_amp
l_int|0x1f
)paren
op_lshift
l_int|7
)paren
)paren
suffix:semicolon
id|castout_ste
op_assign
id|ste
op_plus
(paren
id|castout_entry
op_minus
l_int|8
)paren
suffix:semicolon
)brace
multiline_comment|/* Dont cast out the first kernel segment */
r_if
c_cond
(paren
id|castout_ste-&gt;dw0.dw0.esid
op_ne
id|GET_ESID
c_func
(paren
id|KERNELBASE
)paren
)paren
r_break
suffix:semicolon
id|castout_entry
op_assign
(paren
id|castout_entry
op_plus
l_int|1
)paren
op_amp
l_int|0xf
suffix:semicolon
)brace
id|get_paca
c_func
(paren
)paren
op_member_access_from_pointer
id|xStab_data.next_round_robin
op_assign
(paren
id|castout_entry
op_plus
l_int|1
)paren
op_amp
l_int|0xf
suffix:semicolon
multiline_comment|/* Modify the old entry to the new value. */
multiline_comment|/* Force previous translations to complete. DRENG */
id|asm
r_volatile
(paren
l_string|&quot;isync&quot;
suffix:colon
suffix:colon
suffix:colon
l_string|&quot;memory&quot;
)paren
suffix:semicolon
id|castout_ste-&gt;dw0.dw0.v
op_assign
l_int|0
suffix:semicolon
id|asm
r_volatile
(paren
l_string|&quot;sync&quot;
suffix:colon
suffix:colon
suffix:colon
l_string|&quot;memory&quot;
)paren
suffix:semicolon
multiline_comment|/* Order update */
id|castout_ste-&gt;dw0.dword0
op_assign
l_int|0
suffix:semicolon
id|castout_ste-&gt;dw1.dword1
op_assign
l_int|0
suffix:semicolon
id|castout_ste-&gt;dw1.dw1.vsid
op_assign
id|vsid
suffix:semicolon
id|old_esid
op_assign
id|castout_ste-&gt;dw0.dw0.esid
suffix:semicolon
id|castout_ste-&gt;dw0.dw0.esid
op_assign
id|esid
suffix:semicolon
id|castout_ste-&gt;dw0.dw0.kp
op_assign
l_int|1
suffix:semicolon
r_if
c_cond
(paren
op_logical_neg
id|kernel_segment
)paren
id|castout_ste-&gt;dw0.dw0.ks
op_assign
l_int|1
suffix:semicolon
id|asm
r_volatile
(paren
l_string|&quot;eieio&quot;
suffix:colon
suffix:colon
suffix:colon
l_string|&quot;memory&quot;
)paren
suffix:semicolon
multiline_comment|/* Order update */
id|castout_ste-&gt;dw0.dw0.v
op_assign
l_int|1
suffix:semicolon
id|asm
r_volatile
(paren
l_string|&quot;slbie  %0&quot;
suffix:colon
suffix:colon
l_string|&quot;r&quot;
(paren
id|old_esid
op_lshift
id|SID_SHIFT
)paren
)paren
suffix:semicolon
multiline_comment|/* Ensure completion of slbie */
id|asm
r_volatile
(paren
l_string|&quot;sync&quot;
suffix:colon
suffix:colon
suffix:colon
l_string|&quot;memory&quot;
)paren
suffix:semicolon
r_return
(paren
id|global_entry
op_or
(paren
id|castout_entry
op_amp
l_int|0x7
)paren
)paren
suffix:semicolon
)brace
DECL|function|__ste_allocate
r_static
r_inline
r_void
id|__ste_allocate
c_func
(paren
r_int
r_int
id|esid
comma
r_int
r_int
id|vsid
)paren
(brace
r_int
r_char
id|stab_entry
suffix:semicolon
r_int
r_int
id|offset
suffix:semicolon
r_int
id|region_id
op_assign
id|REGION_ID
c_func
(paren
id|esid
op_lshift
id|SID_SHIFT
)paren
suffix:semicolon
id|stab_entry
op_assign
id|make_ste
c_func
(paren
id|get_paca
c_func
(paren
)paren
op_member_access_from_pointer
id|xStab_data.virt
comma
id|esid
comma
id|vsid
)paren
suffix:semicolon
r_if
c_cond
(paren
id|region_id
op_ne
id|USER_REGION_ID
)paren
r_return
suffix:semicolon
id|offset
op_assign
id|__get_cpu_var
c_func
(paren
id|stab_cache_ptr
)paren
suffix:semicolon
r_if
c_cond
(paren
id|offset
OL
id|NR_STAB_CACHE_ENTRIES
)paren
id|__get_cpu_var
c_func
(paren
id|stab_cache
(braket
id|offset
op_increment
)braket
)paren
op_assign
id|stab_entry
suffix:semicolon
r_else
id|offset
op_assign
id|NR_STAB_CACHE_ENTRIES
op_plus
l_int|1
suffix:semicolon
id|__get_cpu_var
c_func
(paren
id|stab_cache_ptr
)paren
op_assign
id|offset
suffix:semicolon
)brace
multiline_comment|/*&n; * Allocate a segment table entry for the given ea.&n; */
DECL|function|ste_allocate
r_int
id|ste_allocate
c_func
(paren
r_int
r_int
id|ea
)paren
(brace
r_int
r_int
id|vsid
comma
id|esid
suffix:semicolon
id|mm_context_t
id|context
suffix:semicolon
multiline_comment|/* Check for invalid effective addresses. */
r_if
c_cond
(paren
op_logical_neg
id|IS_VALID_EA
c_func
(paren
id|ea
)paren
)paren
r_return
l_int|1
suffix:semicolon
multiline_comment|/* Kernel or user address? */
r_if
c_cond
(paren
id|REGION_ID
c_func
(paren
id|ea
)paren
op_ge
id|KERNEL_REGION_ID
)paren
(brace
id|vsid
op_assign
id|get_kernel_vsid
c_func
(paren
id|ea
)paren
suffix:semicolon
id|context
op_assign
id|KERNEL_CONTEXT
c_func
(paren
id|ea
)paren
suffix:semicolon
)brace
r_else
(brace
r_if
c_cond
(paren
op_logical_neg
id|current-&gt;mm
)paren
r_return
l_int|1
suffix:semicolon
id|context
op_assign
id|current-&gt;mm-&gt;context
suffix:semicolon
id|vsid
op_assign
id|get_vsid
c_func
(paren
id|context.id
comma
id|ea
)paren
suffix:semicolon
)brace
id|esid
op_assign
id|GET_ESID
c_func
(paren
id|ea
)paren
suffix:semicolon
id|__ste_allocate
c_func
(paren
id|esid
comma
id|vsid
)paren
suffix:semicolon
multiline_comment|/* Order update */
id|asm
r_volatile
(paren
l_string|&quot;sync&quot;
op_scope_resolution
suffix:colon
l_string|&quot;memory&quot;
)paren
suffix:semicolon
r_return
l_int|0
suffix:semicolon
)brace
multiline_comment|/*&n; * preload some userspace segments into the segment table.&n; */
DECL|function|preload_stab
r_static
r_void
id|preload_stab
c_func
(paren
r_struct
id|task_struct
op_star
id|tsk
comma
r_struct
id|mm_struct
op_star
id|mm
)paren
(brace
r_int
r_int
id|pc
op_assign
id|KSTK_EIP
c_func
(paren
id|tsk
)paren
suffix:semicolon
r_int
r_int
id|stack
op_assign
id|KSTK_ESP
c_func
(paren
id|tsk
)paren
suffix:semicolon
r_int
r_int
id|unmapped_base
suffix:semicolon
r_int
r_int
id|pc_esid
op_assign
id|GET_ESID
c_func
(paren
id|pc
)paren
suffix:semicolon
r_int
r_int
id|stack_esid
op_assign
id|GET_ESID
c_func
(paren
id|stack
)paren
suffix:semicolon
r_int
r_int
id|unmapped_base_esid
suffix:semicolon
r_int
r_int
id|vsid
suffix:semicolon
r_if
c_cond
(paren
id|test_tsk_thread_flag
c_func
(paren
id|tsk
comma
id|TIF_32BIT
)paren
)paren
id|unmapped_base
op_assign
id|TASK_UNMAPPED_BASE_USER32
suffix:semicolon
r_else
id|unmapped_base
op_assign
id|TASK_UNMAPPED_BASE_USER64
suffix:semicolon
id|unmapped_base_esid
op_assign
id|GET_ESID
c_func
(paren
id|unmapped_base
)paren
suffix:semicolon
r_if
c_cond
(paren
op_logical_neg
id|IS_VALID_EA
c_func
(paren
id|pc
)paren
op_logical_or
(paren
id|REGION_ID
c_func
(paren
id|pc
)paren
op_ge
id|KERNEL_REGION_ID
)paren
)paren
r_return
suffix:semicolon
id|vsid
op_assign
id|get_vsid
c_func
(paren
id|mm-&gt;context.id
comma
id|pc
)paren
suffix:semicolon
id|__ste_allocate
c_func
(paren
id|pc_esid
comma
id|vsid
)paren
suffix:semicolon
r_if
c_cond
(paren
id|pc_esid
op_eq
id|stack_esid
)paren
r_return
suffix:semicolon
r_if
c_cond
(paren
op_logical_neg
id|IS_VALID_EA
c_func
(paren
id|stack
)paren
op_logical_or
(paren
id|REGION_ID
c_func
(paren
id|stack
)paren
op_ge
id|KERNEL_REGION_ID
)paren
)paren
r_return
suffix:semicolon
id|vsid
op_assign
id|get_vsid
c_func
(paren
id|mm-&gt;context.id
comma
id|stack
)paren
suffix:semicolon
id|__ste_allocate
c_func
(paren
id|stack_esid
comma
id|vsid
)paren
suffix:semicolon
r_if
c_cond
(paren
id|pc_esid
op_eq
id|unmapped_base_esid
op_logical_or
id|stack_esid
op_eq
id|unmapped_base_esid
)paren
r_return
suffix:semicolon
r_if
c_cond
(paren
op_logical_neg
id|IS_VALID_EA
c_func
(paren
id|unmapped_base
)paren
op_logical_or
(paren
id|REGION_ID
c_func
(paren
id|unmapped_base
)paren
op_ge
id|KERNEL_REGION_ID
)paren
)paren
r_return
suffix:semicolon
id|vsid
op_assign
id|get_vsid
c_func
(paren
id|mm-&gt;context.id
comma
id|unmapped_base
)paren
suffix:semicolon
id|__ste_allocate
c_func
(paren
id|unmapped_base_esid
comma
id|vsid
)paren
suffix:semicolon
multiline_comment|/* Order update */
id|asm
r_volatile
(paren
l_string|&quot;sync&quot;
suffix:colon
suffix:colon
suffix:colon
l_string|&quot;memory&quot;
)paren
suffix:semicolon
)brace
multiline_comment|/* Flush all user entries from the segment table of the current processor. */
DECL|function|flush_stab
r_void
id|flush_stab
c_func
(paren
r_struct
id|task_struct
op_star
id|tsk
comma
r_struct
id|mm_struct
op_star
id|mm
)paren
(brace
id|STE
op_star
id|stab
op_assign
(paren
id|STE
op_star
)paren
id|get_paca
c_func
(paren
)paren
op_member_access_from_pointer
id|xStab_data.virt
suffix:semicolon
id|STE
op_star
id|ste
suffix:semicolon
r_int
r_int
id|offset
op_assign
id|__get_cpu_var
c_func
(paren
id|stab_cache_ptr
)paren
suffix:semicolon
multiline_comment|/* Force previous translations to complete. DRENG */
id|asm
r_volatile
(paren
l_string|&quot;isync&quot;
suffix:colon
suffix:colon
suffix:colon
l_string|&quot;memory&quot;
)paren
suffix:semicolon
r_if
c_cond
(paren
id|offset
op_le
id|NR_STAB_CACHE_ENTRIES
)paren
(brace
r_int
id|i
suffix:semicolon
r_for
c_loop
(paren
id|i
op_assign
l_int|0
suffix:semicolon
id|i
OL
id|offset
suffix:semicolon
id|i
op_increment
)paren
(brace
id|ste
op_assign
id|stab
op_plus
id|__get_cpu_var
c_func
(paren
id|stab_cache
(braket
id|i
)braket
)paren
suffix:semicolon
id|ste-&gt;dw0.dw0.v
op_assign
l_int|0
suffix:semicolon
)brace
)brace
r_else
(brace
r_int
r_int
id|entry
suffix:semicolon
multiline_comment|/* Invalidate all entries. */
id|ste
op_assign
id|stab
suffix:semicolon
multiline_comment|/* Never flush the first entry. */
id|ste
op_add_assign
l_int|1
suffix:semicolon
r_for
c_loop
(paren
id|entry
op_assign
l_int|1
suffix:semicolon
id|entry
OL
(paren
id|PAGE_SIZE
op_div
r_sizeof
(paren
id|STE
)paren
)paren
suffix:semicolon
id|entry
op_increment
comma
id|ste
op_increment
)paren
(brace
r_int
r_int
id|ea
suffix:semicolon
id|ea
op_assign
id|ste-&gt;dw0.dw0.esid
op_lshift
id|SID_SHIFT
suffix:semicolon
r_if
c_cond
(paren
id|ea
OL
id|KERNELBASE
)paren
(brace
id|ste-&gt;dw0.dw0.v
op_assign
l_int|0
suffix:semicolon
)brace
)brace
)brace
id|asm
r_volatile
(paren
l_string|&quot;sync; slbia; sync&quot;
op_scope_resolution
suffix:colon
l_string|&quot;memory&quot;
)paren
suffix:semicolon
id|__get_cpu_var
c_func
(paren
id|stab_cache_ptr
)paren
op_assign
l_int|0
suffix:semicolon
id|preload_stab
c_func
(paren
id|tsk
comma
id|mm
)paren
suffix:semicolon
)brace
multiline_comment|/*&n; * SLB stuff&n; */
multiline_comment|/*&n; * Create a segment buffer entry for the given esid/vsid pair.&n; *&n; * NOTE: A context syncronising instruction is required before and after&n; * this, in the common case we use exception entry and rfid.&n; */
DECL|function|make_slbe
r_static
r_void
id|make_slbe
c_func
(paren
r_int
r_int
id|esid
comma
r_int
r_int
id|vsid
comma
r_int
id|large
comma
r_int
id|kernel_segment
)paren
(brace
r_int
r_int
id|entry
comma
id|castout_entry
suffix:semicolon
r_union
(brace
r_int
r_int
id|word0
suffix:semicolon
id|slb_dword0
id|data
suffix:semicolon
)brace
id|esid_data
suffix:semicolon
r_union
(brace
r_int
r_int
id|word0
suffix:semicolon
id|slb_dword1
id|data
suffix:semicolon
)brace
id|vsid_data
suffix:semicolon
r_struct
id|paca_struct
op_star
id|lpaca
op_assign
id|get_paca
c_func
(paren
)paren
suffix:semicolon
multiline_comment|/*&n;&t; * We take the next entry, round robin. Previously we tried&n;&t; * to find a free slot first but that took too long. Unfortunately&n;&t; * we dont have any LRU information to help us choose a slot.&n;&t; */
multiline_comment|/* &n;&t; * Never cast out the segment for our kernel stack. Since we&n;&t; * dont invalidate the ERAT we could have a valid translation&n;&t; * for the kernel stack during the first part of exception exit &n;&t; * which gets invalidated due to a tlbie from another cpu at a&n;&t; * non recoverable point (after setting srr0/1) - Anton&n;&t; *&n;&t; * paca Ksave is always valid (even when on the interrupt stack)&n;&t; * so we use that.&n;&t; */
id|castout_entry
op_assign
id|lpaca-&gt;xStab_data.next_round_robin
suffix:semicolon
r_do
(brace
id|entry
op_assign
id|castout_entry
suffix:semicolon
id|castout_entry
op_increment
suffix:semicolon
multiline_comment|/*&n;&t;&t; * We bolt in the first kernel segment and the first&n;&t;&t; * vmalloc segment.&n;&t;&t; */
r_if
c_cond
(paren
id|castout_entry
op_ge
id|SLB_NUM_ENTRIES
)paren
id|castout_entry
op_assign
l_int|2
suffix:semicolon
id|asm
r_volatile
(paren
l_string|&quot;slbmfee  %0,%1&quot;
suffix:colon
l_string|&quot;=r&quot;
(paren
id|esid_data
)paren
suffix:colon
l_string|&quot;r&quot;
(paren
id|entry
)paren
)paren
suffix:semicolon
)brace
r_while
c_loop
(paren
id|esid_data.data.v
op_logical_and
id|esid_data.data.esid
op_eq
id|GET_ESID
c_func
(paren
id|lpaca-&gt;xKsave
)paren
)paren
suffix:semicolon
id|lpaca-&gt;xStab_data.next_round_robin
op_assign
id|castout_entry
suffix:semicolon
multiline_comment|/* slbie not needed as the previous mapping is still valid. */
multiline_comment|/* &n;&t; * Write the new SLB entry.&n;&t; */
id|vsid_data.word0
op_assign
l_int|0
suffix:semicolon
id|vsid_data.data.vsid
op_assign
id|vsid
suffix:semicolon
id|vsid_data.data.kp
op_assign
l_int|1
suffix:semicolon
r_if
c_cond
(paren
id|large
)paren
id|vsid_data.data.l
op_assign
l_int|1
suffix:semicolon
r_if
c_cond
(paren
id|kernel_segment
)paren
id|vsid_data.data.c
op_assign
l_int|1
suffix:semicolon
r_else
id|vsid_data.data.ks
op_assign
l_int|1
suffix:semicolon
id|esid_data.word0
op_assign
l_int|0
suffix:semicolon
id|esid_data.data.esid
op_assign
id|esid
suffix:semicolon
id|esid_data.data.v
op_assign
l_int|1
suffix:semicolon
id|esid_data.data.index
op_assign
id|entry
suffix:semicolon
multiline_comment|/*&n;&t; * No need for an isync before or after this slbmte. The exception&n;&t; * we enter with and the rfid we exit with are context synchronizing.&n;&t; */
id|asm
r_volatile
(paren
l_string|&quot;slbmte  %0,%1&quot;
suffix:colon
suffix:colon
l_string|&quot;r&quot;
(paren
id|vsid_data
)paren
comma
l_string|&quot;r&quot;
(paren
id|esid_data
)paren
)paren
suffix:semicolon
)brace
DECL|function|__slb_allocate
r_static
r_inline
r_void
id|__slb_allocate
c_func
(paren
r_int
r_int
id|esid
comma
r_int
r_int
id|vsid
comma
id|mm_context_t
id|context
)paren
(brace
r_int
id|large
op_assign
l_int|0
suffix:semicolon
r_int
id|region_id
op_assign
id|REGION_ID
c_func
(paren
id|esid
op_lshift
id|SID_SHIFT
)paren
suffix:semicolon
r_int
r_int
id|offset
suffix:semicolon
r_if
c_cond
(paren
id|cur_cpu_spec-&gt;cpu_features
op_amp
id|CPU_FTR_16M_PAGE
)paren
(brace
r_if
c_cond
(paren
id|region_id
op_eq
id|KERNEL_REGION_ID
)paren
id|large
op_assign
l_int|1
suffix:semicolon
r_else
r_if
c_cond
(paren
id|region_id
op_eq
id|USER_REGION_ID
)paren
id|large
op_assign
id|in_hugepage_area
c_func
(paren
id|context
comma
id|esid
op_lshift
id|SID_SHIFT
)paren
suffix:semicolon
)brace
id|make_slbe
c_func
(paren
id|esid
comma
id|vsid
comma
id|large
comma
id|region_id
op_ne
id|USER_REGION_ID
)paren
suffix:semicolon
r_if
c_cond
(paren
id|region_id
op_ne
id|USER_REGION_ID
)paren
r_return
suffix:semicolon
id|offset
op_assign
id|__get_cpu_var
c_func
(paren
id|stab_cache_ptr
)paren
suffix:semicolon
r_if
c_cond
(paren
id|offset
OL
id|NR_STAB_CACHE_ENTRIES
)paren
id|__get_cpu_var
c_func
(paren
id|stab_cache
(braket
id|offset
op_increment
)braket
)paren
op_assign
id|esid
suffix:semicolon
r_else
id|offset
op_assign
id|NR_STAB_CACHE_ENTRIES
op_plus
l_int|1
suffix:semicolon
id|__get_cpu_var
c_func
(paren
id|stab_cache_ptr
)paren
op_assign
id|offset
suffix:semicolon
)brace
multiline_comment|/*&n; * Allocate a segment table entry for the given ea.&n; */
DECL|function|slb_allocate
r_int
id|slb_allocate
c_func
(paren
r_int
r_int
id|ea
)paren
(brace
r_int
r_int
id|vsid
comma
id|esid
suffix:semicolon
id|mm_context_t
id|context
suffix:semicolon
multiline_comment|/* Check for invalid effective addresses. */
r_if
c_cond
(paren
id|unlikely
c_func
(paren
op_logical_neg
id|IS_VALID_EA
c_func
(paren
id|ea
)paren
)paren
)paren
r_return
l_int|1
suffix:semicolon
multiline_comment|/* Kernel or user address? */
r_if
c_cond
(paren
id|REGION_ID
c_func
(paren
id|ea
)paren
op_ge
id|KERNEL_REGION_ID
)paren
(brace
id|context
op_assign
id|KERNEL_CONTEXT
c_func
(paren
id|ea
)paren
suffix:semicolon
id|vsid
op_assign
id|get_kernel_vsid
c_func
(paren
id|ea
)paren
suffix:semicolon
)brace
r_else
(brace
r_if
c_cond
(paren
id|unlikely
c_func
(paren
op_logical_neg
id|current-&gt;mm
)paren
)paren
r_return
l_int|1
suffix:semicolon
id|context
op_assign
id|current-&gt;mm-&gt;context
suffix:semicolon
id|vsid
op_assign
id|get_vsid
c_func
(paren
id|context.id
comma
id|ea
)paren
suffix:semicolon
)brace
id|esid
op_assign
id|GET_ESID
c_func
(paren
id|ea
)paren
suffix:semicolon
id|BUG_ON
c_func
(paren
(paren
id|esid
op_lshift
id|SID_SHIFT
)paren
op_eq
id|VMALLOCBASE
)paren
suffix:semicolon
id|__slb_allocate
c_func
(paren
id|esid
comma
id|vsid
comma
id|context
)paren
suffix:semicolon
r_return
l_int|0
suffix:semicolon
)brace
multiline_comment|/*&n; * preload some userspace segments into the SLB.&n; */
DECL|function|preload_slb
r_static
r_void
id|preload_slb
c_func
(paren
r_struct
id|task_struct
op_star
id|tsk
comma
r_struct
id|mm_struct
op_star
id|mm
)paren
(brace
r_int
r_int
id|pc
op_assign
id|KSTK_EIP
c_func
(paren
id|tsk
)paren
suffix:semicolon
r_int
r_int
id|stack
op_assign
id|KSTK_ESP
c_func
(paren
id|tsk
)paren
suffix:semicolon
r_int
r_int
id|unmapped_base
suffix:semicolon
r_int
r_int
id|pc_esid
op_assign
id|GET_ESID
c_func
(paren
id|pc
)paren
suffix:semicolon
r_int
r_int
id|stack_esid
op_assign
id|GET_ESID
c_func
(paren
id|stack
)paren
suffix:semicolon
r_int
r_int
id|unmapped_base_esid
suffix:semicolon
r_int
r_int
id|vsid
suffix:semicolon
r_if
c_cond
(paren
id|test_tsk_thread_flag
c_func
(paren
id|tsk
comma
id|TIF_32BIT
)paren
)paren
id|unmapped_base
op_assign
id|TASK_UNMAPPED_BASE_USER32
suffix:semicolon
r_else
id|unmapped_base
op_assign
id|TASK_UNMAPPED_BASE_USER64
suffix:semicolon
id|unmapped_base_esid
op_assign
id|GET_ESID
c_func
(paren
id|unmapped_base
)paren
suffix:semicolon
r_if
c_cond
(paren
op_logical_neg
id|IS_VALID_EA
c_func
(paren
id|pc
)paren
op_logical_or
(paren
id|REGION_ID
c_func
(paren
id|pc
)paren
op_ge
id|KERNEL_REGION_ID
)paren
)paren
r_return
suffix:semicolon
id|vsid
op_assign
id|get_vsid
c_func
(paren
id|mm-&gt;context.id
comma
id|pc
)paren
suffix:semicolon
id|__slb_allocate
c_func
(paren
id|pc_esid
comma
id|vsid
comma
id|mm-&gt;context
)paren
suffix:semicolon
r_if
c_cond
(paren
id|pc_esid
op_eq
id|stack_esid
)paren
r_return
suffix:semicolon
r_if
c_cond
(paren
op_logical_neg
id|IS_VALID_EA
c_func
(paren
id|stack
)paren
op_logical_or
(paren
id|REGION_ID
c_func
(paren
id|stack
)paren
op_ge
id|KERNEL_REGION_ID
)paren
)paren
r_return
suffix:semicolon
id|vsid
op_assign
id|get_vsid
c_func
(paren
id|mm-&gt;context.id
comma
id|stack
)paren
suffix:semicolon
id|__slb_allocate
c_func
(paren
id|stack_esid
comma
id|vsid
comma
id|mm-&gt;context
)paren
suffix:semicolon
r_if
c_cond
(paren
id|pc_esid
op_eq
id|unmapped_base_esid
op_logical_or
id|stack_esid
op_eq
id|unmapped_base_esid
)paren
r_return
suffix:semicolon
r_if
c_cond
(paren
op_logical_neg
id|IS_VALID_EA
c_func
(paren
id|unmapped_base
)paren
op_logical_or
(paren
id|REGION_ID
c_func
(paren
id|unmapped_base
)paren
op_ge
id|KERNEL_REGION_ID
)paren
)paren
r_return
suffix:semicolon
id|vsid
op_assign
id|get_vsid
c_func
(paren
id|mm-&gt;context.id
comma
id|unmapped_base
)paren
suffix:semicolon
id|__slb_allocate
c_func
(paren
id|unmapped_base_esid
comma
id|vsid
comma
id|mm-&gt;context
)paren
suffix:semicolon
)brace
multiline_comment|/* Flush all user entries from the segment table of the current processor. */
DECL|function|flush_slb
r_void
id|flush_slb
c_func
(paren
r_struct
id|task_struct
op_star
id|tsk
comma
r_struct
id|mm_struct
op_star
id|mm
)paren
(brace
r_int
r_int
id|offset
op_assign
id|__get_cpu_var
c_func
(paren
id|stab_cache_ptr
)paren
suffix:semicolon
r_union
(brace
r_int
r_int
id|word0
suffix:semicolon
id|slb_dword0
id|data
suffix:semicolon
)brace
id|esid_data
suffix:semicolon
r_int
r_int
id|esid
comma
id|vsid
suffix:semicolon
id|WARN_ON
c_func
(paren
op_logical_neg
id|irqs_disabled
c_func
(paren
)paren
)paren
suffix:semicolon
r_if
c_cond
(paren
id|offset
op_le
id|NR_STAB_CACHE_ENTRIES
)paren
(brace
r_int
id|i
suffix:semicolon
id|asm
r_volatile
(paren
l_string|&quot;isync&quot;
suffix:colon
suffix:colon
suffix:colon
l_string|&quot;memory&quot;
)paren
suffix:semicolon
r_for
c_loop
(paren
id|i
op_assign
l_int|0
suffix:semicolon
id|i
OL
id|offset
suffix:semicolon
id|i
op_increment
)paren
(brace
id|esid_data.word0
op_assign
l_int|0
suffix:semicolon
id|esid_data.data.esid
op_assign
id|__get_cpu_var
c_func
(paren
id|stab_cache
(braket
id|i
)braket
)paren
suffix:semicolon
id|BUG_ON
c_func
(paren
id|esid_data.data.esid
op_eq
id|GET_ESID
c_func
(paren
id|VMALLOCBASE
)paren
)paren
suffix:semicolon
id|asm
r_volatile
(paren
l_string|&quot;slbie %0&quot;
suffix:colon
suffix:colon
l_string|&quot;r&quot;
(paren
id|esid_data
)paren
)paren
suffix:semicolon
)brace
id|asm
r_volatile
(paren
l_string|&quot;isync&quot;
suffix:colon
suffix:colon
suffix:colon
l_string|&quot;memory&quot;
)paren
suffix:semicolon
)brace
r_else
(brace
id|asm
r_volatile
(paren
l_string|&quot;isync; slbia; isync&quot;
suffix:colon
suffix:colon
suffix:colon
l_string|&quot;memory&quot;
)paren
suffix:semicolon
multiline_comment|/*&n;&t;&t; * Bolt in the first vmalloc segment. Since modules end&n;&t;&t; * up there it gets hit very heavily. We must not touch&n;&t;&t; * the vmalloc region between the slbia and here, thats&n;&t;&t; * why we require interrupts off.&n;&t;&t; */
id|esid
op_assign
id|GET_ESID
c_func
(paren
id|VMALLOCBASE
)paren
suffix:semicolon
id|vsid
op_assign
id|get_kernel_vsid
c_func
(paren
id|VMALLOCBASE
)paren
suffix:semicolon
id|get_paca
c_func
(paren
)paren
op_member_access_from_pointer
id|xStab_data.next_round_robin
op_assign
l_int|1
suffix:semicolon
id|make_slbe
c_func
(paren
id|esid
comma
id|vsid
comma
l_int|0
comma
l_int|1
)paren
suffix:semicolon
)brace
multiline_comment|/* Workaround POWER5 &lt; DD2.1 issue */
r_if
c_cond
(paren
id|offset
op_eq
l_int|1
op_logical_or
id|offset
OG
id|NR_STAB_CACHE_ENTRIES
)paren
(brace
multiline_comment|/* &n;&t;&t; * flush segment in EEH region, we dont normally access&n;&t;&t; * addresses in this region.&n;&t;&t; */
id|esid_data.word0
op_assign
l_int|0
suffix:semicolon
id|esid_data.data.esid
op_assign
id|EEH_REGION_ID
suffix:semicolon
id|asm
r_volatile
(paren
l_string|&quot;slbie %0&quot;
suffix:colon
suffix:colon
l_string|&quot;r&quot;
(paren
id|esid_data
)paren
)paren
suffix:semicolon
)brace
id|__get_cpu_var
c_func
(paren
id|stab_cache_ptr
)paren
op_assign
l_int|0
suffix:semicolon
id|preload_slb
c_func
(paren
id|tsk
comma
id|mm
)paren
suffix:semicolon
)brace
eof
