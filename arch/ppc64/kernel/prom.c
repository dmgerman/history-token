...
id|of_chosen
suffix:semicolon
multiline_comment|/*&n; * Wrapper for allocating memory for various data that needs to be&n; * attached to device nodes as they are processed at boot or when&n; * added to the device tree later (e.g. DLPAR).  At boot there is&n; * already a region reserved so we just increment *mem_start by size;&n; * otherwise we call kmalloc.&n; */
...
multiline_comment|/*&n; * Plug a device node into the tree and global list.&n; */
DECL|function|of_attach_node
r_void
id|of_attach_node
c_func
(paren
...
DECL|function|prom_reconfig_notifier
r_static
r_int
...
(paren
id|KERN_ERR
l_string|&quot;finish_node returned %d&bslash;n&quot;
comma
id|err
)paren
suffix:semicolon
...
DECL|variable|prom_reconfig_nb
r_static
r_struct
id|notifier_block
id|prom_reconfig_nb
...
l_int|10
comma
multiline_comment|/* This one needs to run first */
)brace
suffix:semicolon
DECL|function|prom_reconfig_setup
r_static
r_int
id|__init
id|prom_reconfig_setup
c_func
(paren
r_void
)paren
...
DECL|variable|prom_reconfig_setup
id|__initcall
c_func
(paren
id|prom_reconfig_setup
