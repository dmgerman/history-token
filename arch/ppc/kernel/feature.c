...
DECL|macro|KL_FCR
mdefine_line|#define KL_FCR(r)&t;&t;(keylargo_base + ((r) &gt;&gt; 2))
DECL|macro|KL_IN
mdefine_line|#define KL_IN(r)&t;&t;(in_le32(KL_FCR(r)))
DECL|macro|KL_OUT
mdefine_line|#define KL_OUT(r,v)&t;&t;(out_le32(KL_FCR(r), (v)))
DECL|macro|KL_BIS
mdefine_line|#define KL_BIS(r,v)&t;&t;(KL_OUT((r), KL_IN(r) | (v)))
DECL|macro|KL_BIC
mdefine_line|#define KL_BIC(r,v)&t;&t;(KL_OUT((r), KL_IN(r) &amp; ~(v)))
DECL|macro|KL_GPIO_IN
mdefine_line|#define KL_GPIO_IN(r)&t;&t;(in_8(((volatile u8 *)keylargo_base)+(r)))
DECL|macro|KL_GPIO_OUT
mdefine_line|#define KL_GPIO_OUT(r,v)&t;(out_8(((volatile u8 *)keylargo_base)+(r), (v)))
DECL|macro|KL_LOCK
mdefine_line|#define KL_LOCK()&t;&t;spin_lock_irqsave(&amp;keylargo-&gt;lock, flags)
DECL|macro|KL_UNLOCK
mdefine_line|#define KL_UNLOCK()&t;&t;spin_unlock_irqrestore(&amp;keylargo-&gt;lock, flags)
...
id|uninorth_fw
suffix:semicolon
multiline_comment|/* Feature bits for Apple motherboards */
...
multiline_comment|/* This table currently lacks most oldworld machines, but&n; * they currently don&squot;t need it so...&n; * &n; * Todo: The whole feature_xxx mecanism need to be redone&n; * some way to be able to handle the new kind of features&n; * exposed by core99. At this point, the main &quot;tables&quot; will&n; * probably be in this table, which will have to be filled with&n; * all known machines&n; */
multiline_comment|/* Warning: Don&squot;t change ordering of entries as some machines&n; * adverstise beeing compatible with several models. In those&n; * case, the &quot;highest&quot; has to be first&n; */
...
comma
multiline_comment|/* QuickSilver G4 */
(brace
...
l_int|0
multiline_comment|/*FTR_HAS_FW_POWER*/
)brace
comma
(brace
...
(paren
id|KERN_ERR
l_string|&quot;feature.c: Can&squot;t find device-tree root !&bslash;n&quot;
)paren
suffix:semicolon
r_return
suffix:semicolon
...
c_func
(paren
l_string|&quot;PowerMac model: %s&bslash;n&quot;
comma
id|model
)paren
suffix:semicolon
...
c_func
(paren
l_string|&quot;Uni-N revision: %d, KeyLargo revision: %d %s&bslash;n&quot;
comma
id|uninorth_rev
comma
id|keylargo_rev
...
macro_line|#endif
macro_line|#ifdef CONFIG_PMAC_PBOOK
multiline_comment|/* On PowerBooks, we disable the serial ports by default, they&n;&t; * will be re-enabled by the driver&n;&t; */
...
multiline_comment|/* XXX We use the keylargo spinlock, but we never&n;&t; * have uninorth without keylargo, so...&n;&t; */
id|KL_LOCK
c_func
(paren
)paren
...
id|flags
suffix:semicolon
multiline_comment|/* TODO: should probably handle save/restore of PCI config space here&n;&t; */
r_if
c_cond
(paren
op_logical_neg
...
r_void
DECL|function|feature_set_modem_power
id|feature_set_modem_power
c_func
(paren
r_struct
...
(brace
multiline_comment|/* Assert modem reset */
id|KL_GPIO_OUT
c_func
(paren
...
suffix:semicolon
multiline_comment|/* Release modem reset */
id|KL_GPIO_OUT
c_func
(paren
id|KL_GPIO_MODEM_RESET
