...
(paren
id|KERN_ERR
l_string|&quot;mf.c: unable to kmalloc %d bytes&bslash;n&quot;
comma
r_sizeof
(paren
r_struct
...
singleline_comment|//&t;    memcpy(cmdline, &quot;Bad cmdline&quot;, 11);
singleline_comment|//&t;}
)brace
id|pci_unmap_single
c_func
...
(brace
singleline_comment|//    unsigned long usec, tsec;
id|u32
id|dataWord1
op_assign
op_star
...
suffix:semicolon
singleline_comment|// Now THIS is a nasty hack!
singleline_comment|// It ensures that the first two calls to mf_getRtcTime get different
singleline_comment|// answers.  That way the loop in init_time (time.c) will not think
singleline_comment|// the clock is stuck.
r_if
c_cond
(paren
id|lastsec
