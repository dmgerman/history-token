...
multiline_comment|/*&n; * Calculate the user-level address that would have been in ar.bsp had the user executed a&n; * &quot;cover&quot; instruction right before entering the kernel.&n; */
r_int
r_int
DECL|function|ia64_get_user_bsp
...
multiline_comment|/*&n; * Synchronize (i.e, write) the RSE backing store living in kernel space to the VM of the&n; * indicated child process.&n; *&n; * If new_bsp is non-zero, the bsp will (effectively) be updated to the new value upon&n; * resumption of the child process.  This is accomplished by setting the loadrs value to&n; * zero and the bspstore value to the new bsp value.&n; *&n; * When new_bsp and flush_user_rbs are both 0, the register backing store in kernel space&n; * is written to user space and the loadrs and bspstore values are left alone.&n; *&n; * When new_bsp is zero and flush_user_rbs is 1 (non-zero), loadrs is set to 0, and the&n; * bspstore value is set to the old bsp value.  This will cause the stacked registers (r32&n; * and up) to be obtained entirely from the child&squot;s memory space rather than from the&n; * kernel.  (This makes it easier to write code for modifying the stacked registers in&n; * multi-threaded programs.)&n; *&n; * Note: I had originally written this function without the flush_user_rbs parameter; it&n; * was written so that loadrs would always be set to zero.  But I had problems with&n; * certain system calls apparently causing a portion of the RBS to be zeroed.  (I still&n; * don&squot;t understand why this was happening.) Anyway, it&squot;d definitely less intrusive to&n; * leave loadrs and bspstore alone if possible.&n; */
r_static
r_int
...
id|ret
suffix:semicolon
multiline_comment|/*&n;&t; * Return early if nothing to do.  Note that new_bsp will be zero if the caller&n;&t; * wants to force synchronization without changing bsp.&n;&t; */
r_if
c_cond
(paren
id|user_bsp
