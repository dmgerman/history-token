multiline_comment|/*&n; * Kernel support for the ptrace() and syscall tracing interfaces.&n; *&n; * Copyright (C) 1999-2002 Hewlett-Packard Co&n; *&t;David Mosberger-Tang &lt;davidm@hpl.hp.com&gt;&n; *&n; * Derived from the x86 and Alpha versions.  Most of the code in here&n; * could actually be factored into a common set of routines.&n; */
...
multiline_comment|/*&n; * Simulate user-level &quot;flushrs&quot;.  Note: we can&squot;t just add pt-&gt;loadrs&gt;&gt;16 to&n; * pt-&gt;ar_bspstore because the kernel backing store and the user-level backing store may&n; * have different alignments (and therefore a different number of intervening rnat slots).&n; */
r_static
r_void
DECL|function|user_flushrs
id|user_flushrs
(paren
r_struct
id|task_struct
...
r_inline
r_void
DECL|function|sync_user_rbs_one_thread
id|sync_user_rbs_one_thread
(paren
r_struct
id|task_struct
op_star
...
macro_line|#else
DECL|macro|collect_task
macro_line|# define collect_task(list, p, make_writable)&t;sync_user_rbs_one_thread(p, make_writable)
DECL|macro|finish_task
macro_line|# define finish_task(list, make_writable)&t;(NULL)
macro_line|#endif
multiline_comment|/*&n; * Synchronize the RSE backing store of CHILD and all tasks that share the address space&n; * with it.  CHILD_URBS_END is the address of the end of the register backing store of&n; * CHILD.  If MAKE_WRITABLE is set, a user-level &quot;flushrs&quot; is simulated such that the VM&n; * can be written via ptrace() and the tasks will pick up the newly written values.  It&n; * would be OK to unconditionally simulate a &quot;flushrs&quot;, but this would be more intrusive&n; * than strictly necessary (e.g., it would make it impossible to obtain the original value&n; * of ar.bspstore).&n; */
r_static
r_void
DECL|function|threads_sync_user_rbs
id|threads_sync_user_rbs
(paren
r_struct
id|task_struct
op_star
...
r_else
(brace
multiline_comment|/*&n;&t;&t; * Note: we can&squot;t call ia64_sync_user_rbs() while holding the&n;&t;&t; * tasklist_lock because that may cause a dead-lock: ia64_sync_user_rbs()&n;&t;&t; * may indirectly call tlb_flush_all(), which triggers an IPI.&n;&t;&t; * Furthermore, tasklist_lock is acquired by fork() with interrupts&n;&t;&t; * disabled, so with the right timing, the IPI never completes, hence&n;&t;&t; * tasklist_lock never gets released, hence fork() never completes...&n;&t;&t; */
r_struct
id|task_list
op_star
id|list
...
suffix:semicolon
multiline_comment|/* set the flag in the child thread only */
)brace
...
id|PT_AR_BSP
suffix:colon
multiline_comment|/*&n;&t;&t;&t; * By convention, we use PT_AR_BSP to refer to the end of the user-level&n;&t;&t;&t; * backing store.  Use ia64_rse_skip_regs(PT_AR_BSP, -CFM.sof) to get&n;&t;&t;&t; * the real value of ar.bsp at the time the kernel was entered.&n;&t;&t;&t; */
id|urbs_end
op_assign
id|ia64_get_user_rbs_end
c_func
...
suffix:semicolon
multiline_comment|/* fp scratch regs(10-15) */
id|retval
op_or_assign
id|__copy_to_user
c_func
