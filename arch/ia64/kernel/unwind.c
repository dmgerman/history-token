...
mdefine_line|#define alloc_labeled_state()&t;kmalloc(sizeof(struct unw_labeled_state), GFP_ATOMIC)
DECL|macro|free_labeled_state
mdefine_line|#define free_labeled_state(usr)&t;kfree(usr)
...
id|r0
suffix:semicolon
multiline_comment|/* constant 0 for r0 */
...
id|kernel_table
suffix:semicolon
multiline_comment|/* unwind table describing the gate page (kernel code that is mapped into user space): */
DECL|member|gate_table_size
r_int
id|gate_table_size
suffix:semicolon
DECL|member|gate_table
r_int
r_int
op_star
id|gate_table
...
l_int|0
suffix:semicolon
multiline_comment|/* read r0 always returns 0 */
op_star
id|nat
op_assign
l_int|0
...
l_int|0
suffix:semicolon
multiline_comment|/* if register is a NaT, *addr may contain kernel data! */
op_star
id|nat
op_assign
l_int|1
...
multiline_comment|/* Make a copy of the state stack.  Non-recursive to avoid stack overflows.  */
r_static
r_struct
id|unw_reg_state
op_star
DECL|function|dup_state_stack
id|dup_state_stack
(paren
r_struct
id|unw_reg_state
...
multiline_comment|/* Free all stacked register states (but not RS itself).  */
r_static
r_void
DECL|function|free_state_stack
id|free_state_stack
(paren
r_struct
id|unw_reg_state
...
suffix:semicolon
multiline_comment|/* insert into list of labeled states: */
id|ls-&gt;next
op_assign
id|sr-&gt;labeled_states
suffix:semicolon
...
suffix:semicolon
multiline_comment|/* free labeled register states &amp; stack: */
id|STAT
c_func
(paren
id|parse_start
...
l_int|0
comma
l_string|&quot;unwind.%s: UNW_INSN_MOVE_CONST bad val=%ld&bslash;n&quot;
comma
id|__FUNCTION__
comma
id|val
...
suffix:semicolon
multiline_comment|/*&n;&t; * Subtle stuff here: we _could_ unwind through the switch_stack frame but we&n;&t; * don&squot;t want to do that because it would be slow as each preserved register would&n;&t; * have to be processed.  Instead, what we do here is zero out the frame info and&n;&t; * start the unwind process at the function that created the switch_stack frame.&n;&t; * When a preserved value in switch_stack needs to be accessed, run_script() will&n;&t; * initialize the appropriate pointer on demand.&n;&t; */
id|memset
c_func
(paren
id|info
...
l_string|&quot;  rbs = [0x%lx-0x%lx)&bslash;n&quot;
l_string|&quot;  stk = [0x%lx-0x%lx)&bslash;n&quot;
l_string|&quot;  pr     0x%lx&bslash;n&quot;
l_string|&quot;  sw     0x%lx&bslash;n&quot;
l_string|&quot;  sp     0x%lx&bslash;n&quot;
comma
id|__FUNCTION__
comma
(paren
...
r_void
DECL|function|unw_init_from_interruption
...
c_func
(paren
l_string|&quot;%s: failed to find gate DSO&squot;s unwind table!&bslash;n&quot;
comma
id|__FUNCTION__
)paren
suffix:semicolon
...
l_int|8
suffix:semicolon
multiline_comment|/* reserve space for &quot;end of table&quot; marker */
id|unw.gate_table
op_assign
id|kmalloc
c_func
...
(paren
id|KERN_ERR
l_string|&quot;%s: unable to create unwind data for gate page!&bslash;n&quot;
comma
id|__FUNCTION__
)paren
suffix:semicolon
...
multiline_comment|/*&n; * DEPRECATED DEPRECATED DEPRECATED DEPRECATED DEPRECATED DEPRECATED DEPRECATED&n; *&n; *&t;This system call has been deprecated.  The new and improved way to get&n; *&t;at the kernel&squot;s unwind info is via the gate DSO.  The address of the&n; *&t;ELF header for this DSO is passed to user-level via AT_SYSINFO_EHDR.&n; *&n; * DEPRECATED DEPRECATED DEPRECATED DEPRECATED DEPRECATED DEPRECATED DEPRECATED&n; *&n; * This system call copies the unwind data into the buffer pointed to by BUF and returns&n; * the size of the unwind data.  If BUF_SIZE is smaller than the size of the unwind data&n; * or if BUF is NULL, nothing is copied, but the system call still returns the size of the&n; * unwind data.&n; *&n; * The first portion of the unwind data contains an unwind table and rest contains the&n; * associated unwind info (in no particular order).  The unwind table consists of a table&n; * of entries of the form:&n; *&n; *&t;u64 start;&t;(64-bit address of start of function)&n; *&t;u64 end;&t;(64-bit address of start of function)&n; *&t;u64 info;&t;(BUF-relative offset to unwind info)&n; *&n; * The end of the unwind table is indicated by an entry with a START address of zero.&n; *&n; * Please see the IA-64 Software Conventions and Runtime Architecture manual for details&n; * on the format of the unwind info.&n; *&n; * ERRORS&n; *&t;EFAULT&t;BUF points outside your accessible address space.&n; */
id|asmlinkage
r_int
DECL|function|sys_getunwind
id|sys_getunwind
(paren
r_void
id|__user
