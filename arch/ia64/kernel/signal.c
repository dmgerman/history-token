multiline_comment|/*&n; * Architecture-specific signal handling support.&n; *&n; * Copyright (C) 1999-2004 Hewlett-Packard Co&n; *&t;David Mosberger-Tang &lt;davidm@hpl.hp.com&gt;&n; *&n; * Derived from i386 and Alpha versions.&n; */
...
r_static
r_int
DECL|function|force_sigsegv_info
id|force_sigsegv_info
(paren
r_int
id|sig
comma
...
(brace
multiline_comment|/*&n;&t;&t; * Acquiring siglock around the sa_handler-update is almost&n;&t;&t; * certainly overkill, but this isn&squot;t a&n;&t;&t; * performance-critical path and I&squot;d rather play it safe&n;&t;&t; * here than having to debug a nasty race if and when&n;&t;&t; * something changes in kernel/signal.c that would make it&n;&t;&t; * no longer safe to modify sa_handler without holding the&n;&t;&t; * lock.&n;&t;&t; */
...
multiline_comment|/* Set a delayed signal that was detected in MCA/INIT/NMI/PMI context where it&n; * could not be delivered.  It is important that the target process is not&n; * allowed to do any more work in user space.  Possible cases for the target&n; * process:&n; *&n; * - It is sleeping and will wake up soon.  Store the data in the current task,&n; *   the signal will be sent when the current task returns from the next&n; *   interrupt.&n; *&n; * - It is running in user context.  Store the data in the current task, the&n; *   signal will be sent when the current task returns from the next interrupt.&n; *&n; * - It is running in kernel context on this or another cpu and will return to&n; *   user context.  Store the data in the target task, the signal will be sent&n; *   to itself when the target task returns to user space.&n; *&n; * - It is running in kernel context on this cpu and will sleep before&n; *   returning to user context.  Because this is also the current task, the&n; *   signal will not get delivered and the task could sleep indefinitely.&n; *   Store the data in the idle task for this cpu, the signal will be sent&n; *   after the idle task processes its next interrupt.&n; *&n; * To cover all cases, store the data in the target task, the current task and&n; * the idle task on this cpu.  Whatever happens, the signal will be delivered&n; * to the target task before it can do any useful user space work.  Multiple&n; * deliveries have no unwanted side effects.&n; *&n; * Note: This code is executed in MCA/INIT/NMI/PMI context, with interrupts&n; * disabled.  It must not take any locks nor use kernel structures or services&n; * that require locks.&n; */
multiline_comment|/* To ensure that we get the right pid, check its start time.  To avoid extra&n; * include files in thread_info.h, convert the task start_time to unsigned long,&n; * giving us a cycle time of &gt; 580 years.&n; */
r_static
r_inline
r_int
r_int
DECL|function|start_time_ul
id|start_time_ul
c_func
(paren
r_const
...
(brace
r_return
id|t-&gt;start_time.tv_sec
op_star
id|NSEC_PER_SEC
op_plus
id|t-&gt;start_time.tv_nsec
suffix:semicolon
)brace
r_void
DECL|function|set_sigdelayed
id|set_sigdelayed
c_func
(paren
id|pid_t
...
r_return
suffix:semicolon
id|t-&gt;thread_info-&gt;sigdelayed.signo
op_assign
id|signo
suffix:semicolon
id|t-&gt;thread_info-&gt;sigdelayed.code
op_assign
id|code
suffix:semicolon
id|t-&gt;thread_info-&gt;sigdelayed.addr
op_assign
id|addr
suffix:semicolon
id|t-&gt;thread_info-&gt;sigdelayed.start_time
op_assign
id|start_time
suffix:semicolon
id|t-&gt;thread_info-&gt;sigdelayed.pid
op_assign
id|pid
suffix:semicolon
id|wmb
...
multiline_comment|/* Called from entry.S when it detects TIF_SIGDELAYED, a delayed signal that&n; * was detected in MCA/INIT/NMI/PMI context where it could not be delivered.&n; */
r_void
DECL|function|do_sigdelayed
id|do_sigdelayed
c_func
(paren
r_void
