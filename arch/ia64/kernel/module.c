...
(brace
multiline_comment|/* Calls to init code from core are bad news */
r_if
c_cond
(paren
id|in_core
...
(paren
id|KERN_ERR
l_string|&quot;%s: init symbol 0x%lx used in module code at %p&bslash;n&quot;
comma
id|mod-&gt;name
comma
id|val
...
(brace
multiline_comment|/*&n;&t;&t;&t;&t; * Init section may have been allocated far away from core,&n;&t;&t;&t;&t; * if the branch won&squot;t reach, then allocate a plt for it.&n;&t;&t;&t;&t; */
r_if
c_cond
(paren
id|in_init
...
multiline_comment|/*&n; * Modules contain a single unwind table which covers both the core and the init text&n; * sections but since the two are not contiguous, we need to split this table up such that&n; * we can register (and unregister) each &quot;segment&quot; seperately.  Fortunately, this sounds&n; * more complicated than it really is.&n; */
r_static
r_void
...
l_int|0
suffix:semicolon
multiline_comment|/* First, count how many init and core unwind-table entries there are.  */
r_for
c_loop
(paren
id|e1
...
id|num_core
suffix:semicolon
multiline_comment|/*&n;&t; * Second, sort the table such that all unwind-table entries for the init and core&n;&t; * text sections are nicely separated.  We do this with a stupid bubble sort&n;&t; * (unwind tables don&squot;t get ridiculously huge).&n;&t; */
r_for
c_loop
(paren
id|e1
...
multiline_comment|/*&n;&t; * Third, locate the init and core segments in the unwind table:&n;&t; */
r_if
c_cond
(paren
id|in_init
...
c_func
(paren
l_string|&quot;%s: name=%s, gp=%lx, num_init=%lu, num_core=%lu&bslash;n&quot;
comma
id|__FUNCTION__
comma
id|mod-&gt;name
...
suffix:semicolon
multiline_comment|/*&n;&t; * Fourth, register both tables (if not empty).&n;&t; */
r_if
c_cond
(paren
id|num_core
...
c_func
(paren
l_string|&quot;%s:  core: handle=%p [%p-%p)&bslash;n&quot;
comma
id|__FUNCTION__
comma
id|mod-&gt;arch.core_unw_table
...
c_func
(paren
l_string|&quot;%s:  init: handle=%p [%p-%p)&bslash;n&quot;
comma
id|__FUNCTION__
comma
id|mod-&gt;arch.init_unw_table
