...
suffix:semicolon
multiline_comment|/* PCI pin to GSI routing information.  This info typically comes from ACPI. */
r_static
r_struct
(brace
...
(paren
id|KERN_DEBUG
l_string|&quot;%s: routing vector %d to %x&bslash;n&quot;
comma
id|__FUNCTION__
comma
id|vector
...
id|delivery
suffix:semicolon
multiline_comment|/*&n;&t; * In override, it may not provide addr/gsi_base.  GSI is enough to&n;&t; * locate iosapic addr, gsi_base and rte_index by examining&n;&t; * gsi_base and num_rte of registered iosapics (tbd)&n;&t; */
macro_line|#ifndef&t;OVERRIDE_DEBUG
r_if
c_cond
(paren
id|iosapic_address
...
multiline_comment|/*&n; * Map PCI pin to the corresponding GSI.&n; * If no such mapping exists, return -1.&n; */
r_static
r_int
DECL|function|pci_pin_to_gsi
id|pci_pin_to_gsi
(paren
r_int
id|bus
...
multiline_comment|/*&n; * Map PCI pin to the corresponding IA-64 interrupt vector.  If no such mapping exists,&n; * try to allocate a new vector.  If it fails, return -1.&n; */
r_static
r_int
...
c_func
(paren
l_string|&quot;PCI: no interrupt route for %02x:%02x pin %c&bslash;n&quot;
comma
id|bus
comma
id|slot
...
(brace
multiline_comment|/* we should allocate a vector for this interrupt line */
id|index
op_assign
id|find_iosapic
c_func
...
c_func
(paren
l_string|&quot;PCI: GSI 0x%x has no IOSAPIC mapping&bslash;n&quot;
comma
id|gsi
)paren
suffix:semicolon
...
c_func
(paren
l_string|&quot;PCI: (%02x:%02x INT%c) -&gt; GSI 0x%x -&gt; vector %d&bslash;n&quot;
comma
id|bus
comma
id|slot
...
c_func
(paren
l_string|&quot;IOSAPIC: version %x.%x, address 0x%lx, GSIs 0x%x-0x%x&bslash;n&quot;
comma
(paren
id|ver
op_amp
...
id|IOSAPIC_LOWEST_PRIORITY
comma
multiline_comment|/* IOSAPIC_POL_HIGH, IOSAPIC_EDGE */
l_int|1
comma
l_int|1
comma
...
c_func
(paren
l_string|&quot;ISA: IRQ %u -&gt; GSI 0x%x (high,edge) -&gt; CPU 0x%04x vector %d&bslash;n&quot;
comma
id|isa_irq
comma
id|isa_irq
...
multiline_comment|/*&n; * Set allocated interrupt vector to dev-&gt;irq and&n; * program IOSAPIC to deliver interrupts&n; */
r_void
DECL|function|iosapic_fixup_pci_interrupt
id|iosapic_fixup_pci_interrupt
(paren
r_struct
id|pci_dev
op_star
...
op_decrement
suffix:semicolon
multiline_comment|/* interrupt pins are numberd starting from 1 */
id|vector
op_assign
id|pci_pin_to_vector
c_func
...
(paren
id|KERN_WARNING
l_string|&quot;PCI: using PPB (%s INT%c) to get vector %d&bslash;n&quot;
comma
id|dev-&gt;slot_name
comma
l_char|&squot;A&squot;
...
(paren
id|KERN_WARNING
l_string|&quot;PCI: Couldn&squot;t map irq for (%s INT%c)&bslash;n&quot;
comma
id|dev-&gt;slot_name
comma
l_char|&squot;A&squot;
...
macro_line|#ifdef CONFIG_SMP
multiline_comment|/*&n;&t;&t;&t; * For platforms that do not support interrupt redirect&n;&t;&t;&t; * via the XTP interface, we can round-robin the PCI&n;&t;&t;&t; * device interrupts to the processors&n;&t;&t;&t; */
r_if
c_cond
(paren
op_logical_neg
...
r_else
(brace
multiline_comment|/*&n;&t;&t;&t;&t; * Direct the interrupt vector to the current cpu,&n;&t;&t;&t;&t; * platform redirection will distribute them.&n;&t;&t;&t;&t; */
id|dest
op_assign
(paren
id|ia64_get_lid
...
c_func
(paren
l_string|&quot;PCI-&gt;APIC IRQ transform: (%s INT%c) -&gt; CPU 0x%04x vector %d&bslash;n&quot;
comma
id|dev-&gt;slot_name
comma
l_char|&squot;A&squot;
...
c_func
(paren
l_string|&quot;%s: acpi_get_prt failed&bslash;n&quot;
comma
id|__FILE__
)paren
suffix:semicolon
...
(brace
multiline_comment|/* fixup dev-&gt;irq and program IOSAPIC */
id|iosapic_fixup_pci_interrupt
c_func
(paren
id|dev
