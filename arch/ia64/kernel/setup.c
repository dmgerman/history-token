...
multiline_comment|/*&n; * There are two places in the performance critical path of&n; * the exception handling code where we need to know the physical&n; * address of the swapper_pg_dir structure.  This routine&n; * patches the &quot;movl&quot; instructions to load the value needed.&n; */
r_static
r_void
id|__init
DECL|function|patch_ivt_with_phys_swapper_pg_dir
id|patch_ivt_with_phys_swapper_pg_dir
c_func
(paren
r_void
)paren
