multiline_comment|/* $Id$&n; *&n; * This file is subject to the terms and conditions of the GNU General Public&n; * License.  See the file &quot;COPYING&quot; in the main directory of this archive&n; * for more details.&n; *&n; * Copyright (C) 1992 - 1997, 2000 Silicon Graphics, Inc.&n; * Copyright (C) 2000 by Colin Ngam&n; */
macro_line|#include &lt;linux/types.h&gt;
macro_line|#include &lt;linux/config.h&gt;
macro_line|#include &lt;linux/ctype.h&gt;
macro_line|#include &lt;asm/sn/sgi.h&gt;
macro_line|#include &lt;asm/sn/iograph.h&gt;
macro_line|#include &lt;asm/sn/invent.h&gt;
macro_line|#include &lt;asm/sn/hcl.h&gt;
macro_line|#include &lt;asm/sn/labelcl.h&gt;
macro_line|#include &lt;asm/sn/agent.h&gt;
macro_line|#include &lt;asm/sn/klconfig.h&gt;
macro_line|#include &lt;asm/sn/nodepda.h&gt;
macro_line|#include &lt;asm/sn/module.h&gt;
macro_line|#include &lt;asm/sn/router.h&gt;
macro_line|#include &lt;asm/sn/xtalk/xbow.h&gt;
DECL|macro|printf
mdefine_line|#define printf printk
DECL|variable|hasmetarouter
r_int
id|hasmetarouter
suffix:semicolon
DECL|macro|LDEBUG
mdefine_line|#define LDEBUG 0
DECL|macro|NIC_UNKNOWN
mdefine_line|#define NIC_UNKNOWN ((nic_t) -1)
DECL|macro|DEBUG_KLGRAPH
macro_line|#undef DEBUG_KLGRAPH
macro_line|#ifdef DEBUG_KLGRAPH
DECL|macro|DBG
mdefine_line|#define DBG(x...) printk(x)
macro_line|#else
DECL|macro|DBG
mdefine_line|#define DBG(x...)
macro_line|#endif /* DEBUG_KLGRAPH */
r_static
r_void
id|sort_nic_names
c_func
(paren
id|lboard_t
op_star
)paren
suffix:semicolon
id|lboard_t
op_star
DECL|function|find_lboard
id|find_lboard
c_func
(paren
id|lboard_t
op_star
id|start
comma
r_int
r_char
id|brd_type
)paren
(brace
multiline_comment|/* Search all boards stored on this node. */
r_while
c_loop
(paren
id|start
)paren
(brace
r_if
c_cond
(paren
id|start-&gt;brd_type
op_eq
id|brd_type
)paren
r_return
id|start
suffix:semicolon
id|start
op_assign
id|KLCF_NEXT
c_func
(paren
id|start
)paren
suffix:semicolon
)brace
multiline_comment|/* Didn&squot;t find it. */
r_return
(paren
id|lboard_t
op_star
)paren
l_int|NULL
suffix:semicolon
)brace
id|lboard_t
op_star
DECL|function|find_lboard_class
id|find_lboard_class
c_func
(paren
id|lboard_t
op_star
id|start
comma
r_int
r_char
id|brd_type
)paren
(brace
multiline_comment|/* Search all boards stored on this node. */
r_while
c_loop
(paren
id|start
)paren
(brace
r_if
c_cond
(paren
id|KLCLASS
c_func
(paren
id|start-&gt;brd_type
)paren
op_eq
id|KLCLASS
c_func
(paren
id|brd_type
)paren
)paren
r_return
id|start
suffix:semicolon
id|start
op_assign
id|KLCF_NEXT
c_func
(paren
id|start
)paren
suffix:semicolon
)brace
multiline_comment|/* Didn&squot;t find it. */
r_return
(paren
id|lboard_t
op_star
)paren
l_int|NULL
suffix:semicolon
)brace
id|klinfo_t
op_star
DECL|function|find_component
id|find_component
c_func
(paren
id|lboard_t
op_star
id|brd
comma
id|klinfo_t
op_star
id|kli
comma
r_int
r_char
id|struct_type
)paren
(brace
r_int
id|index
comma
id|j
suffix:semicolon
r_if
c_cond
(paren
id|kli
op_eq
(paren
id|klinfo_t
op_star
)paren
l_int|NULL
)paren
(brace
id|index
op_assign
l_int|0
suffix:semicolon
)brace
r_else
(brace
r_for
c_loop
(paren
id|j
op_assign
l_int|0
suffix:semicolon
id|j
OL
id|KLCF_NUM_COMPS
c_func
(paren
id|brd
)paren
suffix:semicolon
id|j
op_increment
)paren
(brace
r_if
c_cond
(paren
id|kli
op_eq
id|KLCF_COMP
c_func
(paren
id|brd
comma
id|j
)paren
)paren
r_break
suffix:semicolon
)brace
id|index
op_assign
id|j
suffix:semicolon
r_if
c_cond
(paren
id|index
op_eq
id|KLCF_NUM_COMPS
c_func
(paren
id|brd
)paren
)paren
(brace
id|printf
c_func
(paren
l_string|&quot;find_component: Bad pointer: 0x%p&bslash;n&quot;
comma
id|kli
)paren
suffix:semicolon
r_return
(paren
id|klinfo_t
op_star
)paren
l_int|NULL
suffix:semicolon
)brace
id|index
op_increment
suffix:semicolon
multiline_comment|/* next component */
)brace
r_for
c_loop
(paren
suffix:semicolon
id|index
OL
id|KLCF_NUM_COMPS
c_func
(paren
id|brd
)paren
suffix:semicolon
id|index
op_increment
)paren
(brace
id|kli
op_assign
id|KLCF_COMP
c_func
(paren
id|brd
comma
id|index
)paren
suffix:semicolon
id|DBG
c_func
(paren
l_string|&quot;find_component: brd %p kli %p  request type = 0x%x kli type 0x%x&bslash;n&quot;
comma
id|brd
comma
id|kli
comma
id|kli-&gt;struct_type
comma
id|KLCF_COMP_TYPE
c_func
(paren
id|kli
)paren
)paren
suffix:semicolon
r_if
c_cond
(paren
id|KLCF_COMP_TYPE
c_func
(paren
id|kli
)paren
op_eq
id|struct_type
)paren
r_return
id|kli
suffix:semicolon
)brace
multiline_comment|/* Didn&squot;t find it. */
r_return
(paren
id|klinfo_t
op_star
)paren
l_int|NULL
suffix:semicolon
)brace
id|klinfo_t
op_star
DECL|function|find_first_component
id|find_first_component
c_func
(paren
id|lboard_t
op_star
id|brd
comma
r_int
r_char
id|struct_type
)paren
(brace
r_return
id|find_component
c_func
(paren
id|brd
comma
(paren
id|klinfo_t
op_star
)paren
l_int|NULL
comma
id|struct_type
)paren
suffix:semicolon
)brace
id|lboard_t
op_star
DECL|function|find_lboard_modslot
id|find_lboard_modslot
c_func
(paren
id|lboard_t
op_star
id|start
comma
id|moduleid_t
id|mod
comma
id|slotid_t
id|slot
)paren
(brace
multiline_comment|/* Search all boards stored on this node. */
r_while
c_loop
(paren
id|start
)paren
(brace
r_if
c_cond
(paren
id|MODULE_MATCH
c_func
(paren
id|start-&gt;brd_module
comma
id|mod
)paren
op_logical_and
(paren
id|start-&gt;brd_slot
op_eq
id|slot
)paren
)paren
r_return
id|start
suffix:semicolon
id|start
op_assign
id|KLCF_NEXT
c_func
(paren
id|start
)paren
suffix:semicolon
)brace
multiline_comment|/* Didn&squot;t find it. */
r_return
(paren
id|lboard_t
op_star
)paren
l_int|NULL
suffix:semicolon
)brace
id|lboard_t
op_star
DECL|function|find_lboard_module
id|find_lboard_module
c_func
(paren
id|lboard_t
op_star
id|start
comma
id|moduleid_t
id|mod
)paren
(brace
multiline_comment|/* Search all boards stored on this node. */
r_while
c_loop
(paren
id|start
)paren
(brace
r_if
c_cond
(paren
id|MODULE_MATCH
c_func
(paren
id|start-&gt;brd_module
comma
id|mod
)paren
)paren
r_return
id|start
suffix:semicolon
id|start
op_assign
id|KLCF_NEXT
c_func
(paren
id|start
)paren
suffix:semicolon
)brace
multiline_comment|/* Didn&squot;t find it. */
r_return
(paren
id|lboard_t
op_star
)paren
l_int|NULL
suffix:semicolon
)brace
id|lboard_t
op_star
DECL|function|find_lboard_module_class
id|find_lboard_module_class
c_func
(paren
id|lboard_t
op_star
id|start
comma
id|moduleid_t
id|mod
comma
r_int
r_char
id|brd_type
)paren
(brace
r_while
c_loop
(paren
id|start
)paren
(brace
id|DBG
c_func
(paren
l_string|&quot;find_lboard_module_class: lboard 0x%p, start-&gt;brd_module 0x%x, mod 0x%x, start-&gt;brd_type 0x%x, brd_type 0x%x&bslash;n&quot;
comma
id|start
comma
id|start-&gt;brd_module
comma
id|mod
comma
id|start-&gt;brd_type
comma
id|brd_type
)paren
suffix:semicolon
r_if
c_cond
(paren
id|MODULE_MATCH
c_func
(paren
id|start-&gt;brd_module
comma
id|mod
)paren
op_logical_and
(paren
id|KLCLASS
c_func
(paren
id|start-&gt;brd_type
)paren
op_eq
id|KLCLASS
c_func
(paren
id|brd_type
)paren
)paren
)paren
r_return
id|start
suffix:semicolon
id|start
op_assign
id|KLCF_NEXT
c_func
(paren
id|start
)paren
suffix:semicolon
)brace
multiline_comment|/* Didn&squot;t find it. */
r_return
(paren
id|lboard_t
op_star
)paren
l_int|NULL
suffix:semicolon
)brace
macro_line|#ifndef CONFIG_IA64_SGI_IO
DECL|macro|tolower
mdefine_line|#define tolower(c)&t;(isupper(c) ? (c) - &squot;A&squot; + &squot;a&squot; : (c))
DECL|macro|toupper
mdefine_line|#define toupper(c)&t;(islower(c) ? (c) - &squot;a&squot; + &squot;A&squot; : (c))
macro_line|#endif
multiline_comment|/*&n; * Convert a NIC name to a name for use in the hardware graph.&n; */
r_void
DECL|function|nic_name_convert
id|nic_name_convert
c_func
(paren
r_char
op_star
id|old_name
comma
r_char
op_star
id|new_name
)paren
(brace
r_int
id|i
suffix:semicolon
r_char
id|c
suffix:semicolon
r_char
op_star
id|compare_ptr
suffix:semicolon
r_if
c_cond
(paren
(paren
id|old_name
(braket
l_int|0
)braket
op_eq
l_char|&squot;&bslash;0&squot;
)paren
op_logical_or
(paren
id|old_name
(braket
l_int|1
)braket
op_eq
l_char|&squot;&bslash;0&squot;
)paren
)paren
(brace
id|strcpy
c_func
(paren
id|new_name
comma
id|EDGE_LBL_XWIDGET
)paren
suffix:semicolon
)brace
r_else
(brace
r_for
c_loop
(paren
id|i
op_assign
l_int|0
suffix:semicolon
id|i
OL
id|strlen
c_func
(paren
id|old_name
)paren
suffix:semicolon
id|i
op_increment
)paren
(brace
id|c
op_assign
id|old_name
(braket
id|i
)braket
suffix:semicolon
r_if
c_cond
(paren
id|isalpha
c_func
(paren
id|c
)paren
)paren
id|new_name
(braket
id|i
)braket
op_assign
id|tolower
c_func
(paren
id|c
)paren
suffix:semicolon
r_else
r_if
c_cond
(paren
id|isdigit
c_func
(paren
id|c
)paren
)paren
id|new_name
(braket
id|i
)braket
op_assign
id|c
suffix:semicolon
r_else
id|new_name
(braket
id|i
)braket
op_assign
l_char|&squot;_&squot;
suffix:semicolon
)brace
id|new_name
(braket
id|i
)braket
op_assign
l_char|&squot;&bslash;0&squot;
suffix:semicolon
)brace
multiline_comment|/* XXX -&n;         * Since a bunch of boards made it out with weird names like&n;         * IO6-fibbbed and IO6P2, we need to look for IO6 in a name and&n;         * replace it with &quot;baseio&quot; to avoid confusion in the field.&n;&t; * We also have to make sure we don&squot;t report media_io instead of&n;&t; * baseio.&n;         */
multiline_comment|/* Skip underscores at the beginning of the name */
r_for
c_loop
(paren
id|compare_ptr
op_assign
id|new_name
suffix:semicolon
(paren
op_star
id|compare_ptr
)paren
op_eq
l_char|&squot;_&squot;
suffix:semicolon
id|compare_ptr
op_increment
)paren
suffix:semicolon
multiline_comment|/*&n;&t; * Check for some names we need to replace.  Early boards&n;&t; * had junk following the name so check only the first&n;&t; * characters.&n;&t; */
r_if
c_cond
(paren
op_logical_neg
id|strncmp
c_func
(paren
id|new_name
comma
l_string|&quot;io6&quot;
comma
l_int|3
)paren
op_logical_or
op_logical_neg
id|strncmp
c_func
(paren
id|new_name
comma
l_string|&quot;mio&quot;
comma
l_int|3
)paren
op_logical_or
op_logical_neg
id|strncmp
c_func
(paren
id|new_name
comma
l_string|&quot;media_io&quot;
comma
l_int|8
)paren
)paren
id|strcpy
c_func
(paren
id|new_name
comma
l_string|&quot;baseio&quot;
)paren
suffix:semicolon
macro_line|#if !defined(CONFIG_SGI_IP35) &amp;&amp; !defined(CONFIG_IA64_SGI_SN1) &amp;&amp; !defined(CONFIG_IA64_GENERIC)
r_else
r_if
c_cond
(paren
op_logical_neg
id|strncmp
c_func
(paren
id|new_name
comma
l_string|&quot;ip29&quot;
comma
l_int|4
)paren
)paren
id|strcpy
c_func
(paren
id|new_name
comma
id|SN00_MOTHERBOARD
)paren
suffix:semicolon
macro_line|#endif
r_else
r_if
c_cond
(paren
op_logical_neg
id|strncmp
c_func
(paren
id|new_name
comma
l_string|&quot;divo&quot;
comma
l_int|4
)paren
)paren
id|strcpy
c_func
(paren
id|new_name
comma
l_string|&quot;divo&quot;
)paren
suffix:semicolon
)brace
multiline_comment|/* Check if the given board corresponds to the global &n; * master io6&n; */
r_int
DECL|function|is_master_baseio
id|is_master_baseio
c_func
(paren
id|nasid_t
id|nasid
comma
id|moduleid_t
id|module
comma
id|slotid_t
id|slot
)paren
(brace
id|lboard_t
op_star
id|board
suffix:semicolon
macro_line|#if CONFIG_SGI_IP35 || CONFIG_IA64_SGI_SN1 || CONFIG_IA64_GENERIC
multiline_comment|/* BRINGUP: If this works then look for callers of is_master_baseio()&n; * (e.g. iograph.c) and let them pass in a slot if they want&n; */
id|board
op_assign
id|find_lboard_module
c_func
(paren
(paren
id|lboard_t
op_star
)paren
id|KL_CONFIG_INFO
c_func
(paren
id|nasid
)paren
comma
id|module
)paren
suffix:semicolon
macro_line|#else
id|board
op_assign
id|find_lboard_modslot
c_func
(paren
(paren
id|lboard_t
op_star
)paren
id|KL_CONFIG_INFO
c_func
(paren
id|nasid
)paren
comma
id|module
comma
id|slot
)paren
suffix:semicolon
macro_line|#endif
macro_line|#ifndef _STANDALONE
(brace
id|cnodeid_t
id|cnode
op_assign
id|NASID_TO_COMPACT_NODEID
c_func
(paren
id|nasid
)paren
suffix:semicolon
r_if
c_cond
(paren
op_logical_neg
id|board
op_logical_and
(paren
id|NODEPDA
c_func
(paren
id|cnode
)paren
op_member_access_from_pointer
id|xbow_peer
op_ne
id|INVALID_NASID
)paren
)paren
macro_line|#if CONFIG_SGI_IP35 || CONFIG_IA64_SGI_SN1 || CONFIG_IA64_GENERIC
id|board
op_assign
id|find_lboard_module
c_func
(paren
(paren
id|lboard_t
op_star
)paren
id|KL_CONFIG_INFO
c_func
(paren
id|NODEPDA
c_func
(paren
id|cnode
)paren
op_member_access_from_pointer
id|xbow_peer
)paren
comma
id|module
)paren
suffix:semicolon
macro_line|#else
id|board
op_assign
id|find_lboard_modslot
c_func
(paren
(paren
id|lboard_t
op_star
)paren
id|KL_CONFIG_INFO
c_func
(paren
id|NODEPDA
c_func
(paren
id|cnode
)paren
op_member_access_from_pointer
id|xbow_peer
)paren
comma
id|module
comma
id|slot
)paren
suffix:semicolon
macro_line|#endif
)brace
macro_line|#endif
r_if
c_cond
(paren
op_logical_neg
id|board
)paren
r_return
l_int|0
suffix:semicolon
r_return
id|board-&gt;brd_flags
op_amp
id|GLOBAL_MASTER_IO6
suffix:semicolon
)brace
multiline_comment|/*&n; * Find the lboard structure and get the board name.&n; * If we can&squot;t find the structure or it&squot;s too low a revision,&n; * use default name.&n; */
id|lboard_t
op_star
DECL|function|get_board_name
id|get_board_name
c_func
(paren
id|nasid_t
id|nasid
comma
id|moduleid_t
id|mod
comma
id|slotid_t
id|slot
comma
r_char
op_star
id|name
)paren
(brace
id|lboard_t
op_star
id|brd
suffix:semicolon
id|brd
op_assign
id|find_lboard_modslot
c_func
(paren
(paren
id|lboard_t
op_star
)paren
id|KL_CONFIG_INFO
c_func
(paren
id|nasid
)paren
comma
id|mod
comma
id|slot
)paren
suffix:semicolon
macro_line|#ifndef _STANDALONE
(brace
id|cnodeid_t
id|cnode
op_assign
id|NASID_TO_COMPACT_NODEID
c_func
(paren
id|nasid
)paren
suffix:semicolon
r_if
c_cond
(paren
op_logical_neg
id|brd
op_logical_and
(paren
id|NODEPDA
c_func
(paren
id|cnode
)paren
op_member_access_from_pointer
id|xbow_peer
op_ne
id|INVALID_NASID
)paren
)paren
id|brd
op_assign
id|find_lboard_modslot
c_func
(paren
(paren
id|lboard_t
op_star
)paren
id|KL_CONFIG_INFO
c_func
(paren
id|NODEPDA
c_func
(paren
id|cnode
)paren
op_member_access_from_pointer
id|xbow_peer
)paren
comma
id|mod
comma
id|slot
)paren
suffix:semicolon
)brace
macro_line|#endif
r_if
c_cond
(paren
op_logical_neg
id|brd
op_logical_or
(paren
id|brd-&gt;brd_sversion
OL
l_int|2
)paren
)paren
(brace
id|strcpy
c_func
(paren
id|name
comma
id|EDGE_LBL_XWIDGET
)paren
suffix:semicolon
)brace
r_else
(brace
id|nic_name_convert
c_func
(paren
id|brd-&gt;brd_name
comma
id|name
)paren
suffix:semicolon
)brace
multiline_comment|/*&n; &t; * PV # 540860&n;&t; * If the name is not &squot;baseio&squot; or SN00 MOTHERBOARD&n;&t; * get the lowest of all the names in the nic string.&n;&t; * This is needed for boards like divo, which can have&n;&t; * a bunch of daughter cards, but would like to be called&n;&t; * divo. We could do this for baseio and SN00 MOTHERBOARD&n; &t; * but it has some special case names that we would not&n; &t; * like to disturb at this point.&n;&t; */
multiline_comment|/* gfx boards don&squot;t need any of this name scrambling */
r_if
c_cond
(paren
id|brd
op_logical_and
(paren
id|KLCLASS
c_func
(paren
id|brd-&gt;brd_type
)paren
op_eq
id|KLCLASS_GFX
)paren
)paren
(brace
r_return
id|brd
suffix:semicolon
)brace
r_if
c_cond
(paren
op_logical_neg
(paren
op_logical_neg
id|strcmp
c_func
(paren
id|name
comma
l_string|&quot;baseio&quot;
)paren
)paren
)paren
(brace
r_if
c_cond
(paren
id|brd
)paren
(brace
id|sort_nic_names
c_func
(paren
id|brd
)paren
suffix:semicolon
multiline_comment|/* Convert to small case, &squot;-&squot; to &squot;_&squot; etc */
id|nic_name_convert
c_func
(paren
id|brd-&gt;brd_name
comma
id|name
)paren
suffix:semicolon
)brace
)brace
r_return
id|brd
suffix:semicolon
)brace
r_int
DECL|function|get_cpu_slice
id|get_cpu_slice
c_func
(paren
id|cpuid_t
id|cpu
)paren
(brace
id|klcpu_t
op_star
id|acpu
suffix:semicolon
r_if
c_cond
(paren
(paren
id|acpu
op_assign
id|get_cpuinfo
c_func
(paren
id|cpu
)paren
)paren
op_eq
l_int|NULL
)paren
r_return
op_minus
l_int|1
suffix:semicolon
r_return
id|acpu-&gt;cpu_info.physid
suffix:semicolon
)brace
multiline_comment|/*&n; * get_actual_nasid&n; *&n; *&t;Completely disabled brds have their klconfig on &n; *&t;some other nasid as they have no memory. But their&n; *&t;actual nasid is hidden in the klconfig. Use this&n; *&t;routine to get it. Works for normal boards too.&n; */
id|nasid_t
DECL|function|get_actual_nasid
id|get_actual_nasid
c_func
(paren
id|lboard_t
op_star
id|brd
)paren
(brace
id|klhub_t
op_star
id|hub
suffix:semicolon
r_if
c_cond
(paren
op_logical_neg
id|brd
)paren
r_return
id|INVALID_NASID
suffix:semicolon
multiline_comment|/* find out if we are a completely disabled brd. */
id|hub
op_assign
(paren
id|klhub_t
op_star
)paren
id|find_first_component
c_func
(paren
id|brd
comma
id|KLSTRUCT_HUB
)paren
suffix:semicolon
r_if
c_cond
(paren
op_logical_neg
id|hub
)paren
r_return
id|INVALID_NASID
suffix:semicolon
r_if
c_cond
(paren
op_logical_neg
(paren
id|hub-&gt;hub_info.flags
op_amp
id|KLINFO_ENABLE
)paren
)paren
multiline_comment|/* disabled node brd */
r_return
id|hub-&gt;hub_info.physid
suffix:semicolon
r_else
r_return
id|brd-&gt;brd_nasid
suffix:semicolon
)brace
r_int
DECL|function|xbow_port_io_enabled
id|xbow_port_io_enabled
c_func
(paren
id|nasid_t
id|nasid
comma
r_int
id|link
)paren
(brace
id|lboard_t
op_star
id|brd
suffix:semicolon
id|klxbow_t
op_star
id|xbow_p
suffix:semicolon
multiline_comment|/*&n;&t; * look for boards that might contain an xbow or xbridge&n;&t; */
macro_line|#if SN0
id|brd
op_assign
id|find_lboard
c_func
(paren
(paren
id|lboard_t
op_star
)paren
id|KL_CONFIG_INFO
c_func
(paren
id|nasid
)paren
comma
id|KLTYPE_MIDPLANE8
)paren
suffix:semicolon
macro_line|#else
id|brd
op_assign
id|find_lboard
c_func
(paren
(paren
id|lboard_t
op_star
)paren
id|KL_CONFIG_INFO
c_func
(paren
id|nasid
)paren
comma
id|KLTYPE_PBRICK_XBOW
)paren
suffix:semicolon
macro_line|#endif
r_if
c_cond
(paren
id|brd
op_eq
l_int|NULL
)paren
r_return
l_int|0
suffix:semicolon
r_if
c_cond
(paren
(paren
id|xbow_p
op_assign
(paren
id|klxbow_t
op_star
)paren
id|find_component
c_func
(paren
id|brd
comma
l_int|NULL
comma
id|KLSTRUCT_XBOW
)paren
)paren
op_eq
l_int|NULL
)paren
r_return
l_int|0
suffix:semicolon
r_if
c_cond
(paren
op_logical_neg
id|XBOW_PORT_TYPE_IO
c_func
(paren
id|xbow_p
comma
id|link
)paren
op_logical_or
op_logical_neg
id|XBOW_PORT_IS_ENABLED
c_func
(paren
id|xbow_p
comma
id|link
)paren
)paren
r_return
l_int|0
suffix:semicolon
id|printf
c_func
(paren
l_string|&quot;xbow_port_io_enabled:  brd 0x%p xbow_p 0x%p &bslash;n&quot;
comma
id|brd
comma
id|xbow_p
)paren
suffix:semicolon
r_return
l_int|1
suffix:semicolon
)brace
r_void
DECL|function|board_to_path
id|board_to_path
c_func
(paren
id|lboard_t
op_star
id|brd
comma
r_char
op_star
id|path
)paren
(brace
id|moduleid_t
id|modnum
suffix:semicolon
r_char
op_star
id|board_name
suffix:semicolon
macro_line|#if !defined(CONFIG_SGI_IP35) &amp;&amp; !defined(CONFIG_IA64_SGI_SN1) &amp;&amp; !defined(CONFIG_IA64_GENERIC)
id|slotid_t
id|slot
suffix:semicolon
r_char
id|slot_name
(braket
id|SLOTNUM_MAXLENGTH
)braket
suffix:semicolon
macro_line|#endif
id|ASSERT
c_func
(paren
id|brd
)paren
suffix:semicolon
r_switch
c_cond
(paren
id|KLCLASS
c_func
(paren
id|brd-&gt;brd_type
)paren
)paren
(brace
r_case
id|KLCLASS_NODE
suffix:colon
id|board_name
op_assign
id|EDGE_LBL_NODE
suffix:semicolon
r_break
suffix:semicolon
r_case
id|KLCLASS_ROUTER
suffix:colon
r_if
c_cond
(paren
id|brd-&gt;brd_type
op_eq
id|KLTYPE_META_ROUTER
)paren
(brace
id|board_name
op_assign
id|EDGE_LBL_META_ROUTER
suffix:semicolon
id|hasmetarouter
op_increment
suffix:semicolon
)brace
r_else
id|board_name
op_assign
id|EDGE_LBL_ROUTER
suffix:semicolon
r_break
suffix:semicolon
r_case
id|KLCLASS_MIDPLANE
suffix:colon
id|board_name
op_assign
id|EDGE_LBL_MIDPLANE
suffix:semicolon
r_break
suffix:semicolon
r_case
id|KLCLASS_IO
suffix:colon
id|board_name
op_assign
id|EDGE_LBL_IO
suffix:semicolon
r_break
suffix:semicolon
r_case
id|KLCLASS_IOBRICK
suffix:colon
r_if
c_cond
(paren
id|brd-&gt;brd_type
op_eq
id|KLTYPE_PBRICK
)paren
id|board_name
op_assign
id|EDGE_LBL_PBRICK
suffix:semicolon
r_else
r_if
c_cond
(paren
id|brd-&gt;brd_type
op_eq
id|KLTYPE_IBRICK
)paren
id|board_name
op_assign
id|EDGE_LBL_IBRICK
suffix:semicolon
r_else
r_if
c_cond
(paren
id|brd-&gt;brd_type
op_eq
id|KLTYPE_XBRICK
)paren
id|board_name
op_assign
id|EDGE_LBL_XBRICK
suffix:semicolon
r_else
id|board_name
op_assign
id|EDGE_LBL_IOBRICK
suffix:semicolon
r_break
suffix:semicolon
r_default
suffix:colon
id|board_name
op_assign
id|EDGE_LBL_UNKNOWN
suffix:semicolon
)brace
id|modnum
op_assign
id|brd-&gt;brd_module
suffix:semicolon
macro_line|#if defined(SN0)
id|slot
op_assign
id|brd-&gt;brd_slot
suffix:semicolon
id|get_slotname
c_func
(paren
id|slot
comma
id|slot_name
)paren
suffix:semicolon
id|ASSERT
c_func
(paren
id|modnum
op_ge
l_int|0
)paren
suffix:semicolon
id|sprintf
c_func
(paren
id|path
comma
l_string|&quot;%H/&quot;
id|EDGE_LBL_SLOT
l_string|&quot;/%s/%s&quot;
comma
id|modnum
comma
id|slot_name
comma
id|board_name
)paren
suffix:semicolon
macro_line|#else
id|ASSERT
c_func
(paren
id|modnum
op_ne
id|MODULE_UNKNOWN
op_logical_and
id|modnum
op_ne
id|INVALID_MODULE
)paren
suffix:semicolon
macro_line|#ifdef BRINGUP /* fix IP35 hwgraph */
id|sprintf
c_func
(paren
id|path
comma
id|EDGE_LBL_MODULE
l_string|&quot;/%x/%s&quot;
comma
id|modnum
comma
id|board_name
)paren
suffix:semicolon
macro_line|#else
id|sprintf
c_func
(paren
id|path
comma
l_string|&quot;%H/%s&quot;
comma
id|modnum
comma
id|board_name
)paren
suffix:semicolon
macro_line|#endif
macro_line|#endif
)brace
multiline_comment|/*&n; * Get the module number for a NASID.&n; */
id|moduleid_t
DECL|function|get_module_id
id|get_module_id
c_func
(paren
id|nasid_t
id|nasid
)paren
(brace
id|lboard_t
op_star
id|brd
suffix:semicolon
id|brd
op_assign
id|find_lboard
c_func
(paren
(paren
id|lboard_t
op_star
)paren
id|KL_CONFIG_INFO
c_func
(paren
id|nasid
)paren
comma
id|KLTYPE_IP27
)paren
suffix:semicolon
r_if
c_cond
(paren
op_logical_neg
id|brd
)paren
r_return
id|INVALID_MODULE
suffix:semicolon
r_else
r_return
id|brd-&gt;brd_module
suffix:semicolon
)brace
macro_line|#ifndef CONFIG_IA64_SGI_IO
macro_line|#if 1
multiline_comment|/*&n; *  find_gfxpipe(#)&n; *&n; *  XXXmacko&n; *  This is only used by graphics drivers, and should be moved&n; *  over to gfx/kern/graphics/SN0 as soon as it&squot;s convenient.&n; */
DECL|variable|graphics_pipe_list
r_static
id|klgfx_t
op_star
id|graphics_pipe_list
op_assign
l_int|NULL
suffix:semicolon
DECL|variable|hwgraph_all_gfxids
r_static
id|devfs_handle_t
id|hwgraph_all_gfxids
op_assign
id|GRAPH_VERTEX_NONE
suffix:semicolon
r_void
DECL|function|setup_gfxpipe_link
id|setup_gfxpipe_link
c_func
(paren
id|devfs_handle_t
id|vhdl
comma
r_int
id|pipenum
)paren
(brace
r_char
id|idbuf
(braket
l_int|8
)braket
suffix:semicolon
r_extern
id|graph_hdl_t
id|hwgraph
suffix:semicolon
id|graph_info_add_LBL
c_func
(paren
id|hwgraph
comma
id|vhdl
comma
id|INFO_LBL_GFXID
comma
id|INFO_DESC_EXPORT
comma
(paren
id|arbitrary_info_t
)paren
id|pipenum
)paren
suffix:semicolon
r_if
c_cond
(paren
id|hwgraph_all_gfxids
op_eq
id|GRAPH_VERTEX_NONE
)paren
id|hwgraph_path_add
c_func
(paren
id|hwgraph_root
comma
id|EDGE_LBL_GFX
comma
op_amp
id|hwgraph_all_gfxids
)paren
suffix:semicolon
id|sprintf
c_func
(paren
id|idbuf
comma
l_string|&quot;%d&quot;
comma
id|pipenum
)paren
suffix:semicolon
id|hwgraph_edge_add
c_func
(paren
id|hwgraph_all_gfxids
comma
id|vhdl
comma
id|idbuf
)paren
suffix:semicolon
)brace
macro_line|#endif
multiline_comment|/* &n; * find the pipenum&squot;th logical graphics pipe (KLCLASS_GFX)&n; */
id|lboard_t
op_star
DECL|function|find_gfxpipe
id|find_gfxpipe
c_func
(paren
r_int
id|pipenum
)paren
(brace
id|gda_t
op_star
id|gdap
suffix:semicolon
id|cnodeid_t
id|cnode
suffix:semicolon
id|nasid_t
id|nasid
suffix:semicolon
id|lboard_t
op_star
id|lb
suffix:semicolon
id|klgfx_t
op_star
id|kg
comma
op_star
op_star
id|pkg
suffix:semicolon
r_int
id|i
suffix:semicolon
id|gdap
op_assign
(paren
id|gda_t
op_star
)paren
id|GDA_ADDR
c_func
(paren
id|get_nasid
c_func
(paren
)paren
)paren
suffix:semicolon
r_if
c_cond
(paren
id|gdap-&gt;g_magic
op_ne
id|GDA_MAGIC
)paren
r_return
l_int|NULL
suffix:semicolon
r_if
c_cond
(paren
op_logical_neg
id|graphics_pipe_list
)paren
(brace
multiline_comment|/* for all nodes */
r_for
c_loop
(paren
id|cnode
op_assign
l_int|0
suffix:semicolon
id|cnode
OL
id|MAX_COMPACT_NODES
suffix:semicolon
id|cnode
op_increment
)paren
(brace
id|nasid
op_assign
id|gdap-&gt;g_nasidtable
(braket
id|cnode
)braket
suffix:semicolon
r_if
c_cond
(paren
id|nasid
op_eq
id|INVALID_NASID
)paren
r_continue
suffix:semicolon
id|lb
op_assign
id|KL_CONFIG_INFO
c_func
(paren
id|nasid
)paren
suffix:semicolon
r_while
c_loop
(paren
id|lb
op_assign
id|find_lboard_class
c_func
(paren
id|lb
comma
id|KLCLASS_GFX
)paren
)paren
(brace
id|moduleid_t
id|kgm
comma
id|pkgm
suffix:semicolon
r_int
id|kgs
comma
id|pkgs
suffix:semicolon
macro_line|#if defined(DEBUG) &amp;&amp; (defined(CONFIG_SGI_IP35) || defined(CONFIG_IA64_SGI_SN1) || defined(CONFIG_IA64_GENERIC)) &amp;&amp; defined(BRINGUP)
id|printf
c_func
(paren
l_string|&quot;find_gfxpipe(): PIPE: %s mod %M slot %d&bslash;n&quot;
comma
id|lb
ques
c_cond
id|lb-&gt;brd_name
suffix:colon
l_string|&quot;!LBRD&quot;
comma
id|lb-&gt;brd_module
comma
id|lb-&gt;brd_slot
)paren
suffix:semicolon
macro_line|#endif
multiline_comment|/* insert lb into list */
r_if
c_cond
(paren
op_logical_neg
(paren
id|kg
op_assign
(paren
id|klgfx_t
op_star
)paren
id|find_first_component
c_func
(paren
id|lb
comma
id|KLSTRUCT_GFX
)paren
)paren
)paren
(brace
id|lb
op_assign
id|KLCF_NEXT
c_func
(paren
id|lb
)paren
suffix:semicolon
r_continue
suffix:semicolon
)brace
multiline_comment|/* set moduleslot now that we have brd_module set */
id|kg-&gt;moduleslot
op_assign
(paren
id|lb-&gt;brd_module
op_lshift
l_int|8
)paren
op_or
id|SLOTNUM_GETSLOT
c_func
(paren
id|lb-&gt;brd_slot
)paren
suffix:semicolon
multiline_comment|/* make sure board has device flag set */
id|kg-&gt;gfx_info.flags
op_or_assign
id|KLINFO_DEVICE
suffix:semicolon
r_if
c_cond
(paren
id|kg-&gt;cookie
OL
id|KLGFX_COOKIE
)paren
(brace
id|kg-&gt;gfx_next_pipe
op_assign
l_int|NULL
suffix:semicolon
id|kg-&gt;cookie
op_assign
id|KLGFX_COOKIE
suffix:semicolon
)brace
id|kgm
op_assign
id|kg-&gt;moduleslot
op_rshift
l_int|8
suffix:semicolon
id|kgs
op_assign
id|kg-&gt;moduleslot
op_amp
l_int|0xff
suffix:semicolon
id|pkg
op_assign
op_amp
id|graphics_pipe_list
suffix:semicolon
r_while
c_loop
(paren
op_star
id|pkg
)paren
(brace
id|pkgm
op_assign
(paren
op_star
id|pkg
)paren
op_member_access_from_pointer
id|moduleslot
op_rshift
l_int|8
suffix:semicolon
id|pkgs
op_assign
(paren
op_star
id|pkg
)paren
op_member_access_from_pointer
id|moduleslot
op_amp
l_int|0xff
suffix:semicolon
r_if
c_cond
(paren
op_logical_neg
(paren
id|MODULE_CMP
c_func
(paren
id|kgm
comma
id|pkgm
)paren
OG
l_int|0
op_logical_or
(paren
id|MODULE_CMP
c_func
(paren
id|kgm
comma
id|pkgm
)paren
op_eq
l_int|0
op_logical_and
id|kgs
OG
id|pkgs
)paren
)paren
)paren
r_break
suffix:semicolon
id|pkg
op_assign
op_amp
(paren
op_star
id|pkg
)paren
op_member_access_from_pointer
id|gfx_next_pipe
suffix:semicolon
)brace
id|kg-&gt;gfx_next_pipe
op_assign
op_star
id|pkg
suffix:semicolon
op_star
id|pkg
op_assign
id|kg
suffix:semicolon
id|lb
op_assign
id|KLCF_NEXT
c_func
(paren
id|lb
)paren
suffix:semicolon
)brace
)brace
macro_line|#ifdef FIND_GFXPIPE_DEBUG
id|i
op_assign
l_int|0
suffix:semicolon
id|kg
op_assign
id|graphics_pipe_list
suffix:semicolon
r_while
c_loop
(paren
id|kg
)paren
(brace
id|lboard_t
op_star
id|lb
suffix:semicolon
macro_line|#if defined(CONFIG_SGI_IP35) || defined(CONFIG_IA64_SGI_SN1) || defined(CONFIG_IA64_GENERIC)
id|lb
op_assign
id|find_lboard_class
c_func
(paren
id|KL_CONFIG_INFO
c_func
(paren
id|kg-&gt;gfx_info.nasid
)paren
comma
id|KLCLASS_GFX
)paren
suffix:semicolon
macro_line|#else
macro_line|#error Need to figure out how to find graphics boards ...
macro_line|#endif
macro_line|#if defined(SUPPORT_PRINTING_M_FORMAT)
id|printf
c_func
(paren
l_string|&quot;find_gfxpipe(): %s pipe %d mod %M slot %d&bslash;n&quot;
comma
id|lb
ques
c_cond
id|lb-&gt;brd_name
suffix:colon
l_string|&quot;!LBRD&quot;
comma
id|i
comma
(paren
id|kg-&gt;moduleslot
op_rshift
l_int|8
)paren
comma
(paren
id|kg-&gt;moduleslot
op_amp
l_int|0xff
)paren
)paren
suffix:semicolon
macro_line|#else
id|printf
c_func
(paren
l_string|&quot;find_gfxpipe(): %s pipe %d mod 0x%x slot %d&bslash;n&quot;
comma
id|lb
ques
c_cond
id|lb-&gt;brd_name
suffix:colon
l_string|&quot;!LBRD&quot;
comma
id|i
comma
(paren
id|kg-&gt;moduleslot
op_rshift
l_int|8
)paren
comma
(paren
id|kg-&gt;moduleslot
op_amp
l_int|0xff
)paren
)paren
suffix:semicolon
macro_line|#endif
id|kg
op_assign
id|kg-&gt;gfx_next_pipe
suffix:semicolon
id|i
op_increment
suffix:semicolon
)brace
macro_line|#endif
)brace
id|i
op_assign
l_int|0
suffix:semicolon
id|kg
op_assign
id|graphics_pipe_list
suffix:semicolon
r_while
c_loop
(paren
id|kg
op_logical_and
(paren
id|i
OL
id|pipenum
)paren
)paren
(brace
id|kg
op_assign
id|kg-&gt;gfx_next_pipe
suffix:semicolon
id|i
op_increment
suffix:semicolon
)brace
r_if
c_cond
(paren
op_logical_neg
id|kg
)paren
r_return
l_int|NULL
suffix:semicolon
macro_line|#if defined(SN0)
r_return
id|find_lboard_modslot
c_func
(paren
id|KL_CONFIG_INFO
c_func
(paren
id|kg-&gt;gfx_info.nasid
)paren
comma
(paren
id|kg-&gt;moduleslot
op_rshift
l_int|8
)paren
comma
id|SLOTNUM_XTALK_CLASS
op_or
(paren
id|kg-&gt;moduleslot
op_amp
l_int|0xff
)paren
)paren
suffix:semicolon
macro_line|#elif defined(CONFIG_SGI_IP35) || defined(CONFIG_IA64_SGI_SN1) || defined(CONFIG_IA64_GENERIC)
r_return
id|find_lboard_class
c_func
(paren
id|KL_CONFIG_INFO
c_func
(paren
id|kg-&gt;gfx_info.nasid
)paren
comma
id|KLCLASS_GFX
)paren
suffix:semicolon
macro_line|#else
macro_line|#error Need to figure out how to find graphics boards ...
macro_line|#endif
)brace
macro_line|#endif
DECL|macro|MHZ
mdefine_line|#define MHZ&t;1000000
macro_line|#ifndef CONFIG_IA64_SGI_IO
id|uint
DECL|function|cpu_cycles_adjust
id|cpu_cycles_adjust
c_func
(paren
id|uint
id|orig_cycles
)paren
(brace
id|klcpu_t
op_star
id|acpu
suffix:semicolon
id|uint
id|speed
suffix:semicolon
id|acpu
op_assign
id|nasid_slice_to_cpuinfo
c_func
(paren
id|get_nasid
c_func
(paren
)paren
comma
id|get_slice
c_func
(paren
)paren
)paren
suffix:semicolon
r_if
c_cond
(paren
id|acpu
op_eq
l_int|NULL
)paren
r_return
id|orig_cycles
suffix:semicolon
multiline_comment|/*&n;&t; * cpu cycles seem to be half of the real value, hack and mult by 2&n;&t; * for now.&n;&t; */
id|speed
op_assign
(paren
id|orig_cycles
op_star
l_int|2
)paren
op_div
id|MHZ
suffix:semicolon
multiline_comment|/*&n;&t; * if the cpu thinks its running at some random speed nowhere close &n;&t; * the programmed speed, do nothing.&n;&t; */
r_if
c_cond
(paren
(paren
id|speed
OL
(paren
id|acpu-&gt;cpu_speed
op_minus
l_int|2
)paren
)paren
op_logical_or
(paren
id|speed
OG
(paren
id|acpu-&gt;cpu_speed
op_plus
l_int|2
)paren
)paren
)paren
r_return
id|orig_cycles
suffix:semicolon
r_return
(paren
id|acpu-&gt;cpu_speed
op_star
id|MHZ
op_div
l_int|2
)paren
suffix:semicolon
)brace
macro_line|#endif /* CONFIG_IA64_SGI_IO */
multiline_comment|/* Get the canonical hardware graph name for the given pci component&n; * on the given io board.&n; */
r_void
DECL|function|device_component_canonical_name_get
id|device_component_canonical_name_get
c_func
(paren
id|lboard_t
op_star
id|brd
comma
id|klinfo_t
op_star
id|component
comma
r_char
op_star
id|name
)paren
(brace
id|moduleid_t
id|modnum
suffix:semicolon
id|slotid_t
id|slot
suffix:semicolon
r_char
id|board_name
(braket
l_int|20
)braket
suffix:semicolon
macro_line|#ifdef SN0
r_char
id|slot_name
(braket
id|SLOTNUM_MAXLENGTH
)braket
suffix:semicolon
macro_line|#endif
id|ASSERT
c_func
(paren
id|brd
)paren
suffix:semicolon
multiline_comment|/* Get the module number of this board */
id|modnum
op_assign
id|brd-&gt;brd_module
suffix:semicolon
multiline_comment|/* Convert the [ CLASS | TYPE ] kind of slotid&n;&t; * into a string &n;&t; */
id|slot
op_assign
id|brd-&gt;brd_slot
suffix:semicolon
macro_line|#ifdef SN0
id|get_slotname
c_func
(paren
id|slot
comma
id|slot_name
)paren
suffix:semicolon
id|ASSERT
c_func
(paren
id|modnum
op_ge
l_int|0
)paren
suffix:semicolon
macro_line|#else
id|ASSERT
c_func
(paren
id|modnum
op_ne
id|MODULE_UNKNOWN
op_logical_and
id|modnum
op_ne
id|INVALID_MODULE
)paren
suffix:semicolon
macro_line|#endif
multiline_comment|/* Get the io board name  */
r_if
c_cond
(paren
op_logical_neg
id|brd
op_logical_or
(paren
id|brd-&gt;brd_sversion
OL
l_int|2
)paren
)paren
(brace
id|strcpy
c_func
(paren
id|name
comma
id|EDGE_LBL_XWIDGET
)paren
suffix:semicolon
)brace
r_else
(brace
id|nic_name_convert
c_func
(paren
id|brd-&gt;brd_name
comma
id|board_name
)paren
suffix:semicolon
)brace
multiline_comment|/* Give out the canonical  name of the pci device*/
macro_line|#ifdef SN0
id|sprintf
c_func
(paren
id|name
comma
l_string|&quot;/hw/&quot;
id|EDGE_LBL_MODULE
l_string|&quot;/%M/&quot;
id|EDGE_LBL_SLOT
l_string|&quot;/%s/%s/&quot;
id|EDGE_LBL_PCI
l_string|&quot;/%d&quot;
comma
id|modnum
comma
id|slot_name
comma
id|board_name
comma
id|KLCF_BRIDGE_W_ID
c_func
(paren
id|component
)paren
)paren
suffix:semicolon
macro_line|#elif defined (CONFIG_SGI_IP35)  || defined(CONFIG_IA64_SGI_SN1) || defined(CONFIG_IA64_GENERIC)
id|sprintf
c_func
(paren
id|name
comma
l_string|&quot;/dev/hw/&quot;
id|EDGE_LBL_MODULE
l_string|&quot;/%x/&quot;
id|EDGE_LBL_SLOT
l_string|&quot;/%s/&quot;
id|EDGE_LBL_PCI
l_string|&quot;/%d&quot;
comma
id|modnum
comma
id|board_name
comma
id|KLCF_BRIDGE_W_ID
c_func
(paren
id|component
)paren
)paren
suffix:semicolon
macro_line|#endif
)brace
multiline_comment|/*&n; * Get the serial number of the main  component of a board&n; * Returns 0 if a valid serial number is found&n; * 1 otherwise.&n; * Assumptions: Nic manufacturing string  has the following format&n; *&t;&t;&t;*Serial:&lt;serial_number&gt;;*&n; */
r_static
r_int
DECL|function|component_serial_number_get
id|component_serial_number_get
c_func
(paren
id|lboard_t
op_star
id|board
comma
id|klconf_off_t
id|mfg_nic_offset
comma
r_char
op_star
id|serial_number
comma
r_char
op_star
id|key_pattern
)paren
(brace
r_char
op_star
id|mfg_nic_string
suffix:semicolon
r_char
op_star
id|serial_string
comma
op_star
id|str
suffix:semicolon
r_int
id|i
suffix:semicolon
r_char
op_star
id|serial_pattern
op_assign
l_string|&quot;Serial:&quot;
suffix:semicolon
multiline_comment|/* We have an error on a null mfg nic offset */
r_if
c_cond
(paren
op_logical_neg
id|mfg_nic_offset
)paren
r_return
l_int|1
suffix:semicolon
multiline_comment|/* Get the hub&squot;s manufacturing nic information&n;&t; * which is in the form of a pre-formatted string&n;&t; */
id|mfg_nic_string
op_assign
(paren
r_char
op_star
)paren
id|NODE_OFFSET_TO_K0
c_func
(paren
id|NASID_GET
c_func
(paren
id|board
)paren
comma
id|mfg_nic_offset
)paren
suffix:semicolon
multiline_comment|/* There is no manufacturing nic info */
r_if
c_cond
(paren
op_logical_neg
id|mfg_nic_string
)paren
r_return
l_int|1
suffix:semicolon
id|str
op_assign
id|mfg_nic_string
suffix:semicolon
multiline_comment|/* Look for the key pattern first (if it is  specified)&n;&t; * and then print the serial number corresponding to that.&n;&t; */
r_if
c_cond
(paren
id|strcmp
c_func
(paren
id|key_pattern
comma
l_string|&quot;&quot;
)paren
op_logical_and
op_logical_neg
(paren
id|str
op_assign
id|strstr
c_func
(paren
id|mfg_nic_string
comma
id|key_pattern
)paren
)paren
)paren
r_return
l_int|1
suffix:semicolon
multiline_comment|/* There is no serial number info in the manufacturing&n;&t; * nic info&n;&t; */
r_if
c_cond
(paren
op_logical_neg
(paren
id|serial_string
op_assign
id|strstr
c_func
(paren
id|str
comma
id|serial_pattern
)paren
)paren
)paren
r_return
l_int|1
suffix:semicolon
id|serial_string
op_assign
id|serial_string
op_plus
id|strlen
c_func
(paren
id|serial_pattern
)paren
suffix:semicolon
multiline_comment|/*  Copy the serial number information from the klconfig */
id|i
op_assign
l_int|0
suffix:semicolon
r_while
c_loop
(paren
id|serial_string
(braket
id|i
)braket
op_ne
l_char|&squot;;&squot;
)paren
(brace
id|serial_number
(braket
id|i
)braket
op_assign
id|serial_string
(braket
id|i
)braket
suffix:semicolon
id|i
op_increment
suffix:semicolon
)brace
id|serial_number
(braket
id|i
)braket
op_assign
l_int|0
suffix:semicolon
r_return
l_int|0
suffix:semicolon
)brace
multiline_comment|/*&n; * Get the serial number of a board&n; * Returns 0 if a valid serial number is found&n; * 1 otherwise.&n; */
r_int
DECL|function|board_serial_number_get
id|board_serial_number_get
c_func
(paren
id|lboard_t
op_star
id|board
comma
r_char
op_star
id|serial_number
)paren
(brace
id|ASSERT
c_func
(paren
id|board
op_logical_and
id|serial_number
)paren
suffix:semicolon
r_if
c_cond
(paren
op_logical_neg
id|board
op_logical_or
op_logical_neg
id|serial_number
)paren
r_return
l_int|1
suffix:semicolon
id|strcpy
c_func
(paren
id|serial_number
comma
l_string|&quot;&quot;
)paren
suffix:semicolon
r_switch
c_cond
(paren
id|KLCLASS
c_func
(paren
id|board-&gt;brd_type
)paren
)paren
(brace
r_case
id|KLCLASS_CPU
suffix:colon
(brace
multiline_comment|/* Node board */
id|klhub_t
op_star
id|hub
suffix:semicolon
multiline_comment|/* Get the hub component information */
id|hub
op_assign
(paren
id|klhub_t
op_star
)paren
id|find_first_component
c_func
(paren
id|board
comma
id|KLSTRUCT_HUB
)paren
suffix:semicolon
multiline_comment|/* If we don&squot;t have a hub component on an IP27&n;&t;&t; * then we have a weird klconfig.&n;&t;&t; */
r_if
c_cond
(paren
op_logical_neg
id|hub
)paren
r_return
l_int|1
suffix:semicolon
multiline_comment|/* Get the serial number information from&n;&t;&t; * the hub&squot;s manufacturing nic info&n;&t;&t; */
r_if
c_cond
(paren
id|component_serial_number_get
c_func
(paren
id|board
comma
id|hub-&gt;hub_mfg_nic
comma
id|serial_number
comma
macro_line|#if defined(CONFIG_SGI_IP35) || defined(CONFIG_IA64_SGI_SN1) || defined(CONFIG_IA64_GENERIC)
l_string|&quot;IP35&quot;
)paren
)paren
macro_line|#else
l_string|&quot;IP27&quot;
)paren
)paren
multiline_comment|/* Try with IP31 key if IP27 key fails */
r_if
c_cond
(paren
id|component_serial_number_get
c_func
(paren
id|board
comma
id|hub-&gt;hub_mfg_nic
comma
id|serial_number
comma
l_string|&quot;IP31&quot;
)paren
)paren
macro_line|#endif /* CONFIG_SGI_IP35 || CONFIG_IA64_SGI_SN1 */
r_return
l_int|1
suffix:semicolon
r_break
suffix:semicolon
)brace
r_case
id|KLCLASS_IO
suffix:colon
(brace
multiline_comment|/* IO board */
r_if
c_cond
(paren
id|KLTYPE
c_func
(paren
id|board-&gt;brd_type
)paren
op_eq
id|KLTYPE_TPU
)paren
(brace
multiline_comment|/* Special case for TPU boards */
id|kltpu_t
op_star
id|tpu
suffix:semicolon
multiline_comment|/* Get the tpu component information */
id|tpu
op_assign
(paren
id|kltpu_t
op_star
)paren
id|find_first_component
c_func
(paren
id|board
comma
id|KLSTRUCT_TPU
)paren
suffix:semicolon
multiline_comment|/* If we don&squot;t have a tpu component on a tpu board&n;&t;&t;&t; * then we have a weird klconfig.&n;&t;&t;&t; */
r_if
c_cond
(paren
op_logical_neg
id|tpu
)paren
r_return
l_int|1
suffix:semicolon
multiline_comment|/* Get the serial number information from&n;&t;&t;&t; * the tpu&squot;s manufacturing nic info&n;&t;&t;&t; */
r_if
c_cond
(paren
id|component_serial_number_get
c_func
(paren
id|board
comma
id|tpu-&gt;tpu_mfg_nic
comma
id|serial_number
comma
l_string|&quot;&quot;
)paren
)paren
r_return
l_int|1
suffix:semicolon
r_break
suffix:semicolon
)brace
r_else
r_if
c_cond
(paren
(paren
id|KLTYPE
c_func
(paren
id|board-&gt;brd_type
)paren
op_eq
id|KLTYPE_GSN_A
)paren
op_logical_or
(paren
id|KLTYPE
c_func
(paren
id|board-&gt;brd_type
)paren
op_eq
id|KLTYPE_GSN_B
)paren
)paren
(brace
multiline_comment|/* Special case for GSN boards */
id|klgsn_t
op_star
id|gsn
suffix:semicolon
multiline_comment|/* Get the gsn component information */
id|gsn
op_assign
(paren
id|klgsn_t
op_star
)paren
id|find_first_component
c_func
(paren
id|board
comma
(paren
(paren
id|KLTYPE
c_func
(paren
id|board-&gt;brd_type
)paren
op_eq
id|KLTYPE_GSN_A
)paren
ques
c_cond
id|KLSTRUCT_GSN_A
suffix:colon
id|KLSTRUCT_GSN_B
)paren
)paren
suffix:semicolon
multiline_comment|/* If we don&squot;t have a gsn component on a gsn board&n;&t;&t;&t; * then we have a weird klconfig.&n;&t;&t;&t; */
r_if
c_cond
(paren
op_logical_neg
id|gsn
)paren
r_return
l_int|1
suffix:semicolon
multiline_comment|/* Get the serial number information from&n;&t;&t;&t; * the gsn&squot;s manufacturing nic info&n;&t;&t;&t; */
r_if
c_cond
(paren
id|component_serial_number_get
c_func
(paren
id|board
comma
id|gsn-&gt;gsn_mfg_nic
comma
id|serial_number
comma
l_string|&quot;&quot;
)paren
)paren
r_return
l_int|1
suffix:semicolon
r_break
suffix:semicolon
)brace
r_else
(brace
id|klbri_t
op_star
id|bridge
suffix:semicolon
multiline_comment|/* Get the bridge component information */
id|bridge
op_assign
(paren
id|klbri_t
op_star
)paren
id|find_first_component
c_func
(paren
id|board
comma
id|KLSTRUCT_BRI
)paren
suffix:semicolon
multiline_comment|/* If we don&squot;t have a bridge component on an IO board&n;&t;&t;&t; * then we have a weird klconfig.&n;&t;&t;&t; */
r_if
c_cond
(paren
op_logical_neg
id|bridge
)paren
r_return
l_int|1
suffix:semicolon
multiline_comment|/* Get the serial number information from&n;&t;&t; &t; * the bridge&squot;s manufacturing nic info&n;&t;&t;&t; */
r_if
c_cond
(paren
id|component_serial_number_get
c_func
(paren
id|board
comma
id|bridge-&gt;bri_mfg_nic
comma
id|serial_number
comma
l_string|&quot;&quot;
)paren
)paren
r_return
l_int|1
suffix:semicolon
r_break
suffix:semicolon
)brace
)brace
r_case
id|KLCLASS_ROUTER
suffix:colon
(brace
multiline_comment|/* Router board */
id|klrou_t
op_star
id|router
suffix:semicolon
multiline_comment|/* Get the router component information */
id|router
op_assign
(paren
id|klrou_t
op_star
)paren
id|find_first_component
c_func
(paren
id|board
comma
id|KLSTRUCT_ROU
)paren
suffix:semicolon
multiline_comment|/* If we don&squot;t have a router component on a router board&n;&t;&t; * then we have a weird klconfig.&n;&t;&t; */
r_if
c_cond
(paren
op_logical_neg
id|router
)paren
r_return
l_int|1
suffix:semicolon
multiline_comment|/* Get the serial number information from&n;&t;&t; * the router&squot;s manufacturing nic info&n;&t;&t; */
r_if
c_cond
(paren
id|component_serial_number_get
c_func
(paren
id|board
comma
id|router-&gt;rou_mfg_nic
comma
id|serial_number
comma
l_string|&quot;&quot;
)paren
)paren
r_return
l_int|1
suffix:semicolon
r_break
suffix:semicolon
)brace
r_case
id|KLCLASS_GFX
suffix:colon
(brace
multiline_comment|/* Gfx board */
id|klgfx_t
op_star
id|graphics
suffix:semicolon
multiline_comment|/* Get the graphics component information */
id|graphics
op_assign
(paren
id|klgfx_t
op_star
)paren
id|find_first_component
c_func
(paren
id|board
comma
id|KLSTRUCT_GFX
)paren
suffix:semicolon
multiline_comment|/* If we don&squot;t have a gfx component on a gfx board&n;&t;&t; * then we have a weird klconfig.&n;&t;&t; */
r_if
c_cond
(paren
op_logical_neg
id|graphics
)paren
r_return
l_int|1
suffix:semicolon
multiline_comment|/* Get the serial number information from&n;&t;&t; * the graphics&squot;s manufacturing nic info&n;&t;&t; */
r_if
c_cond
(paren
id|component_serial_number_get
c_func
(paren
id|board
comma
id|graphics-&gt;gfx_mfg_nic
comma
id|serial_number
comma
l_string|&quot;&quot;
)paren
)paren
r_return
l_int|1
suffix:semicolon
r_break
suffix:semicolon
)brace
r_default
suffix:colon
id|strcpy
c_func
(paren
id|serial_number
comma
l_string|&quot;&quot;
)paren
suffix:semicolon
r_break
suffix:semicolon
)brace
r_return
l_int|0
suffix:semicolon
)brace
macro_line|#include &quot;asm/sn/sn_private.h&quot;
macro_line|#ifndef CONFIG_IA64_SGI_IO
multiline_comment|/*&n; * Given a physical address get the name of memory dimm bank&n; * in a hwgraph name format.&n; */
r_void
DECL|function|membank_pathname_get
id|membank_pathname_get
c_func
(paren
id|paddr_t
id|paddr
comma
r_char
op_star
id|name
)paren
(brace
id|cnodeid_t
id|cnode
suffix:semicolon
r_char
id|slotname
(braket
id|SLOTNUM_MAXLENGTH
)braket
suffix:semicolon
id|cnode
op_assign
id|paddr_cnode
c_func
(paren
id|paddr
)paren
suffix:semicolon
multiline_comment|/* Make sure that we have a valid name buffer */
r_if
c_cond
(paren
op_logical_neg
id|name
)paren
r_return
suffix:semicolon
id|name
(braket
l_int|0
)braket
op_assign
l_int|0
suffix:semicolon
multiline_comment|/* Make sure that the cnode is valid */
r_if
c_cond
(paren
(paren
id|cnode
op_eq
id|CNODEID_NONE
)paren
op_logical_or
(paren
id|cnode
OG
id|numnodes
)paren
)paren
r_return
suffix:semicolon
multiline_comment|/* Given a slotid(class:type) get the slotname */
macro_line|#if defined (SN0)
id|get_slotname
c_func
(paren
id|NODE_SLOTID
c_func
(paren
id|cnode
)paren
comma
id|slotname
)paren
suffix:semicolon
id|sprintf
c_func
(paren
id|name
comma
l_string|&quot;/hw/&quot;
id|EDGE_LBL_MODULE
l_string|&quot;/%M/&quot;
id|EDGE_LBL_SLOT
l_string|&quot;/%s/&quot;
id|EDGE_LBL_NODE
l_string|&quot;/&quot;
id|EDGE_LBL_MEMORY
l_string|&quot;/dimm_bank/%d&quot;
comma
id|NODE_MODULEID
c_func
(paren
id|cnode
)paren
comma
id|slotname
comma
id|paddr_dimm
c_func
(paren
id|paddr
)paren
)paren
suffix:semicolon
macro_line|#elif defined (CONFIG_SGI_IP35) || defined(CONFIG_IA64_SGI_SN1) || defined(CONFIG_IA64_GENERIC)
id|sprintf
c_func
(paren
id|name
comma
l_string|&quot;/dev/hw/&quot;
id|EDGE_LBL_MODULE
l_string|&quot;/%M/&quot;
id|EDGE_LBL_NODE
l_string|&quot;/&quot;
id|EDGE_LBL_MEMORY
l_string|&quot;/dimm_bank/%d&quot;
comma
id|NODE_MODULEID
c_func
(paren
id|cnode
)paren
comma
id|paddr_dimm
c_func
(paren
id|paddr
)paren
)paren
suffix:semicolon
macro_line|#endif
)brace
r_int
DECL|function|membank_check_mixed_hidensity
id|membank_check_mixed_hidensity
c_func
(paren
id|nasid_t
id|nasid
)paren
(brace
id|lboard_t
op_star
id|brd
suffix:semicolon
id|klmembnk_t
op_star
id|mem
suffix:semicolon
r_int
id|min_size
op_assign
l_int|1024
comma
id|max_size
op_assign
l_int|0
suffix:semicolon
r_int
id|bank
comma
id|mem_size
suffix:semicolon
id|brd
op_assign
id|find_lboard
c_func
(paren
(paren
id|lboard_t
op_star
)paren
id|KL_CONFIG_INFO
c_func
(paren
id|nasid
)paren
comma
id|KLTYPE_IP27
)paren
suffix:semicolon
id|ASSERT
c_func
(paren
id|brd
)paren
suffix:semicolon
id|mem
op_assign
(paren
id|klmembnk_t
op_star
)paren
id|find_first_component
c_func
(paren
id|brd
comma
id|KLSTRUCT_MEMBNK
)paren
suffix:semicolon
id|ASSERT
c_func
(paren
id|mem
)paren
suffix:semicolon
r_for
c_loop
(paren
id|mem_size
op_assign
l_int|0
comma
id|bank
op_assign
l_int|0
suffix:semicolon
id|bank
OL
id|MD_MEM_BANKS
suffix:semicolon
id|bank
op_increment
)paren
(brace
id|mem_size
op_assign
id|KLCONFIG_MEMBNK_SIZE
c_func
(paren
id|mem
comma
id|bank
)paren
suffix:semicolon
r_if
c_cond
(paren
id|mem_size
OL
id|min_size
)paren
id|min_size
op_assign
id|mem_size
suffix:semicolon
r_if
c_cond
(paren
id|mem_size
OG
id|max_size
)paren
id|max_size
op_assign
id|mem_size
suffix:semicolon
)brace
r_if
c_cond
(paren
(paren
id|max_size
op_eq
l_int|512
)paren
op_logical_and
(paren
id|max_size
op_ne
id|min_size
)paren
)paren
r_return
l_int|1
suffix:semicolon
r_return
l_int|0
suffix:semicolon
)brace
r_int
DECL|function|mem_mixed_hidensity_banks
id|mem_mixed_hidensity_banks
c_func
(paren
r_void
)paren
(brace
id|cnodeid_t
id|cnode
suffix:semicolon
id|nasid_t
id|nasid
suffix:semicolon
r_for
c_loop
(paren
id|cnode
op_assign
l_int|0
suffix:semicolon
id|cnode
OL
id|maxnodes
suffix:semicolon
id|cnode
op_increment
)paren
(brace
id|nasid
op_assign
id|COMPACT_TO_NASID_NODEID
c_func
(paren
id|cnode
)paren
suffix:semicolon
r_if
c_cond
(paren
id|nasid
op_eq
id|INVALID_NASID
)paren
r_continue
suffix:semicolon
r_if
c_cond
(paren
id|membank_check_mixed_hidensity
c_func
(paren
id|nasid
)paren
)paren
r_return
l_int|1
suffix:semicolon
)brace
r_return
l_int|0
suffix:semicolon
)brace
macro_line|#endif /* CONFIG_IA64_SGI_IO */
id|xwidgetnum_t
DECL|function|nodevertex_widgetnum_get
id|nodevertex_widgetnum_get
c_func
(paren
id|devfs_handle_t
id|node_vtx
)paren
(brace
id|hubinfo_t
id|hubinfo_p
suffix:semicolon
id|hwgraph_info_get_LBL
c_func
(paren
id|node_vtx
comma
id|INFO_LBL_NODE_INFO
comma
(paren
id|arbitrary_info_t
op_star
)paren
op_amp
id|hubinfo_p
)paren
suffix:semicolon
r_return
id|hubinfo_p-&gt;h_widgetid
suffix:semicolon
)brace
id|devfs_handle_t
DECL|function|nodevertex_xbow_peer_get
id|nodevertex_xbow_peer_get
c_func
(paren
id|devfs_handle_t
id|node_vtx
)paren
(brace
id|hubinfo_t
id|hubinfo_p
suffix:semicolon
id|nasid_t
id|xbow_peer_nasid
suffix:semicolon
id|cnodeid_t
id|xbow_peer
suffix:semicolon
id|hwgraph_info_get_LBL
c_func
(paren
id|node_vtx
comma
id|INFO_LBL_NODE_INFO
comma
(paren
id|arbitrary_info_t
op_star
)paren
op_amp
id|hubinfo_p
)paren
suffix:semicolon
id|xbow_peer_nasid
op_assign
id|hubinfo_p-&gt;h_nodepda-&gt;xbow_peer
suffix:semicolon
r_if
c_cond
(paren
id|xbow_peer_nasid
op_eq
id|INVALID_NASID
)paren
(brace
r_return
(paren
(paren
id|devfs_handle_t
)paren
op_minus
l_int|1
)paren
suffix:semicolon
)brace
id|xbow_peer
op_assign
id|NASID_TO_COMPACT_NODEID
c_func
(paren
id|xbow_peer_nasid
)paren
suffix:semicolon
r_return
id|NODEPDA
c_func
(paren
id|xbow_peer
)paren
op_member_access_from_pointer
id|node_vertex
suffix:semicolon
)brace
multiline_comment|/* NIC Sorting Support */
DECL|macro|MAX_NICS_PER_STRING
mdefine_line|#define MAX_NICS_PER_STRING &t;32
DECL|macro|MAX_NIC_NAME_LEN
mdefine_line|#define MAX_NIC_NAME_LEN&t;32
r_static
r_char
op_star
DECL|function|get_nic_string
id|get_nic_string
c_func
(paren
id|lboard_t
op_star
id|lb
)paren
(brace
r_int
id|i
suffix:semicolon
id|klinfo_t
op_star
id|k
op_assign
l_int|NULL
suffix:semicolon
id|klconf_off_t
id|mfg_off
op_assign
l_int|0
suffix:semicolon
r_char
op_star
id|mfg_nic
op_assign
l_int|NULL
suffix:semicolon
r_for
c_loop
(paren
id|i
op_assign
l_int|0
suffix:semicolon
id|i
OL
id|KLCF_NUM_COMPS
c_func
(paren
id|lb
)paren
suffix:semicolon
id|i
op_increment
)paren
(brace
id|k
op_assign
id|KLCF_COMP
c_func
(paren
id|lb
comma
id|i
)paren
suffix:semicolon
r_switch
c_cond
(paren
id|k-&gt;struct_type
)paren
(brace
r_case
id|KLSTRUCT_BRI
suffix:colon
id|mfg_off
op_assign
(paren
(paren
id|klbri_t
op_star
)paren
id|k
)paren
op_member_access_from_pointer
id|bri_mfg_nic
suffix:semicolon
r_break
suffix:semicolon
r_case
id|KLSTRUCT_HUB
suffix:colon
id|mfg_off
op_assign
(paren
(paren
id|klhub_t
op_star
)paren
id|k
)paren
op_member_access_from_pointer
id|hub_mfg_nic
suffix:semicolon
r_break
suffix:semicolon
r_case
id|KLSTRUCT_ROU
suffix:colon
id|mfg_off
op_assign
(paren
(paren
id|klrou_t
op_star
)paren
id|k
)paren
op_member_access_from_pointer
id|rou_mfg_nic
suffix:semicolon
r_break
suffix:semicolon
r_case
id|KLSTRUCT_GFX
suffix:colon
id|mfg_off
op_assign
(paren
(paren
id|klgfx_t
op_star
)paren
id|k
)paren
op_member_access_from_pointer
id|gfx_mfg_nic
suffix:semicolon
r_break
suffix:semicolon
r_case
id|KLSTRUCT_TPU
suffix:colon
id|mfg_off
op_assign
(paren
(paren
id|kltpu_t
op_star
)paren
id|k
)paren
op_member_access_from_pointer
id|tpu_mfg_nic
suffix:semicolon
r_break
suffix:semicolon
r_case
id|KLSTRUCT_GSN_A
suffix:colon
r_case
id|KLSTRUCT_GSN_B
suffix:colon
id|mfg_off
op_assign
(paren
(paren
id|klgsn_t
op_star
)paren
id|k
)paren
op_member_access_from_pointer
id|gsn_mfg_nic
suffix:semicolon
r_break
suffix:semicolon
r_case
id|KLSTRUCT_XTHD
suffix:colon
id|mfg_off
op_assign
(paren
(paren
id|klxthd_t
op_star
)paren
id|k
)paren
op_member_access_from_pointer
id|xthd_mfg_nic
suffix:semicolon
r_break
suffix:semicolon
r_default
suffix:colon
id|mfg_off
op_assign
l_int|0
suffix:semicolon
r_break
suffix:semicolon
)brace
r_if
c_cond
(paren
id|mfg_off
)paren
r_break
suffix:semicolon
)brace
r_if
c_cond
(paren
(paren
id|mfg_off
)paren
op_logical_and
(paren
id|k
)paren
)paren
id|mfg_nic
op_assign
(paren
r_char
op_star
)paren
id|NODE_OFFSET_TO_K0
c_func
(paren
id|k-&gt;nasid
comma
id|mfg_off
)paren
suffix:semicolon
r_return
id|mfg_nic
suffix:semicolon
)brace
r_char
op_star
DECL|function|get_first_string
id|get_first_string
c_func
(paren
r_char
op_star
op_star
id|ptrs
comma
r_int
id|n
)paren
(brace
r_int
id|i
suffix:semicolon
r_char
op_star
id|tmpptr
suffix:semicolon
r_if
c_cond
(paren
(paren
id|ptrs
op_eq
l_int|NULL
)paren
op_logical_or
(paren
id|n
op_eq
l_int|0
)paren
)paren
r_return
l_int|NULL
suffix:semicolon
id|tmpptr
op_assign
id|ptrs
(braket
l_int|0
)braket
suffix:semicolon
r_if
c_cond
(paren
id|n
op_eq
l_int|1
)paren
r_return
id|tmpptr
suffix:semicolon
r_for
c_loop
(paren
id|i
op_assign
l_int|0
suffix:semicolon
id|i
OL
id|n
suffix:semicolon
id|i
op_increment
)paren
(brace
r_if
c_cond
(paren
id|strcmp
c_func
(paren
id|tmpptr
comma
id|ptrs
(braket
id|i
)braket
)paren
OG
l_int|0
)paren
id|tmpptr
op_assign
id|ptrs
(braket
id|i
)braket
suffix:semicolon
)brace
r_return
id|tmpptr
suffix:semicolon
)brace
r_int
DECL|function|get_ptrs
id|get_ptrs
c_func
(paren
r_char
op_star
id|idata
comma
r_char
op_star
op_star
id|ptrs
comma
r_int
id|n
comma
r_char
op_star
id|label
)paren
(brace
r_int
id|i
op_assign
l_int|0
suffix:semicolon
r_char
op_star
id|tmp
op_assign
id|idata
suffix:semicolon
r_if
c_cond
(paren
(paren
id|ptrs
op_eq
l_int|NULL
)paren
op_logical_or
(paren
id|idata
op_eq
l_int|NULL
)paren
op_logical_or
(paren
id|label
op_eq
l_int|NULL
)paren
op_logical_or
(paren
id|n
op_eq
l_int|0
)paren
)paren
r_return
l_int|0
suffix:semicolon
r_while
c_loop
(paren
(paren
id|tmp
op_assign
id|strstr
c_func
(paren
id|tmp
comma
id|label
)paren
)paren
)paren
(brace
id|tmp
op_add_assign
id|strlen
c_func
(paren
id|label
)paren
suffix:semicolon
multiline_comment|/* check for empty name field, and last NULL ptr */
r_if
c_cond
(paren
(paren
id|i
OL
(paren
id|n
op_minus
l_int|1
)paren
)paren
op_logical_and
(paren
op_star
id|tmp
op_ne
l_char|&squot;;&squot;
)paren
)paren
(brace
id|ptrs
(braket
id|i
op_increment
)braket
op_assign
id|tmp
suffix:semicolon
)brace
)brace
id|ptrs
(braket
id|i
)braket
op_assign
l_int|NULL
suffix:semicolon
r_return
id|i
suffix:semicolon
)brace
multiline_comment|/*&n; * sort_nic_names&n; *&n; * &t;Does not really do sorting. Find the alphabetically lowest&n; *&t;name among all the nic names found in a nic string.&n; *&n; * Return:&n; *&t;Nothing&n; *&n; * Side Effects:&n; *&n; *&t;lb-&gt;brd_name gets the new name found&n; */
r_static
r_void
DECL|function|sort_nic_names
id|sort_nic_names
c_func
(paren
id|lboard_t
op_star
id|lb
)paren
(brace
r_char
op_star
id|nic_str
suffix:semicolon
r_char
op_star
id|ptrs
(braket
id|MAX_NICS_PER_STRING
)braket
suffix:semicolon
r_char
id|name
(braket
id|MAX_NIC_NAME_LEN
)braket
suffix:semicolon
r_char
op_star
id|tmp
comma
op_star
id|tmp1
suffix:semicolon
op_star
id|name
op_assign
l_int|0
suffix:semicolon
multiline_comment|/* Get the nic pointer from the lb */
r_if
c_cond
(paren
(paren
id|nic_str
op_assign
id|get_nic_string
c_func
(paren
id|lb
)paren
)paren
op_eq
l_int|NULL
)paren
r_return
suffix:semicolon
id|tmp
op_assign
id|get_first_string
c_func
(paren
id|ptrs
comma
id|get_ptrs
c_func
(paren
id|nic_str
comma
id|ptrs
comma
id|MAX_NICS_PER_STRING
comma
l_string|&quot;Name:&quot;
)paren
)paren
suffix:semicolon
r_if
c_cond
(paren
id|tmp
op_eq
l_int|NULL
)paren
r_return
suffix:semicolon
r_if
c_cond
(paren
(paren
id|tmp1
op_assign
id|strchr
c_func
(paren
id|tmp
comma
l_char|&squot;;&squot;
)paren
)paren
)paren
(brace
id|strncpy
c_func
(paren
id|name
comma
id|tmp
comma
id|tmp1
op_minus
id|tmp
)paren
suffix:semicolon
id|name
(braket
id|tmp1
op_minus
id|tmp
)braket
op_assign
l_int|0
suffix:semicolon
)brace
r_else
(brace
id|strncpy
c_func
(paren
id|name
comma
id|tmp
comma
(paren
r_sizeof
(paren
id|name
)paren
op_minus
l_int|1
)paren
)paren
suffix:semicolon
id|name
(braket
r_sizeof
(paren
id|name
)paren
op_minus
l_int|1
)braket
op_assign
l_int|0
suffix:semicolon
)brace
id|strcpy
c_func
(paren
id|lb-&gt;brd_name
comma
id|name
)paren
suffix:semicolon
)brace
macro_line|#if defined(CONFIG_SGI_IP35) || defined(CONFIG_IA64_SGI_SN1) || defined(CONFIG_IA64_GENERIC)
DECL|variable|brick_types
r_char
id|brick_types
(braket
id|MAX_BRICK_TYPES
op_plus
l_int|1
)braket
op_assign
l_string|&quot;crikxdp789012345&quot;
suffix:semicolon
multiline_comment|/*&n; * Format a module id for printing.&n; */
r_void
DECL|function|format_module_id
id|format_module_id
c_func
(paren
r_char
op_star
id|buffer
comma
id|moduleid_t
id|m
comma
r_int
id|fmt
)paren
(brace
r_int
id|rack
comma
id|position
suffix:semicolon
r_char
id|brickchar
suffix:semicolon
id|rack
op_assign
id|MODULE_GET_RACK
c_func
(paren
id|m
)paren
suffix:semicolon
id|ASSERT
c_func
(paren
id|MODULE_GET_BTYPE
c_func
(paren
id|m
)paren
OL
id|MAX_BRICK_TYPES
)paren
suffix:semicolon
id|brickchar
op_assign
id|MODULE_GET_BTCHAR
c_func
(paren
id|m
)paren
suffix:semicolon
id|position
op_assign
id|MODULE_GET_BPOS
c_func
(paren
id|m
)paren
suffix:semicolon
r_if
c_cond
(paren
id|fmt
op_eq
id|MODULE_FORMAT_BRIEF
)paren
(brace
multiline_comment|/* Brief module number format, eg. 002c15 */
multiline_comment|/* Decompress the rack number */
op_star
id|buffer
op_increment
op_assign
l_char|&squot;0&squot;
op_plus
id|RACK_GET_CLASS
c_func
(paren
id|rack
)paren
suffix:semicolon
op_star
id|buffer
op_increment
op_assign
l_char|&squot;0&squot;
op_plus
id|RACK_GET_GROUP
c_func
(paren
id|rack
)paren
suffix:semicolon
op_star
id|buffer
op_increment
op_assign
l_char|&squot;0&squot;
op_plus
id|RACK_GET_NUM
c_func
(paren
id|rack
)paren
suffix:semicolon
multiline_comment|/* Add the brick type */
op_star
id|buffer
op_increment
op_assign
id|brickchar
suffix:semicolon
)brace
r_else
r_if
c_cond
(paren
id|fmt
op_eq
id|MODULE_FORMAT_LONG
)paren
(brace
multiline_comment|/* Fuller hwgraph format, eg. rack/002/bay/15 */
id|strcpy
c_func
(paren
id|buffer
comma
id|EDGE_LBL_RACK
l_string|&quot;/&quot;
)paren
suffix:semicolon
id|buffer
op_add_assign
id|strlen
c_func
(paren
id|buffer
)paren
suffix:semicolon
op_star
id|buffer
op_increment
op_assign
l_char|&squot;0&squot;
op_plus
id|RACK_GET_CLASS
c_func
(paren
id|rack
)paren
suffix:semicolon
op_star
id|buffer
op_increment
op_assign
l_char|&squot;0&squot;
op_plus
id|RACK_GET_GROUP
c_func
(paren
id|rack
)paren
suffix:semicolon
op_star
id|buffer
op_increment
op_assign
l_char|&squot;0&squot;
op_plus
id|RACK_GET_NUM
c_func
(paren
id|rack
)paren
suffix:semicolon
id|strcpy
c_func
(paren
id|buffer
comma
l_string|&quot;/&quot;
id|EDGE_LBL_RPOS
l_string|&quot;/&quot;
)paren
suffix:semicolon
id|buffer
op_add_assign
id|strlen
c_func
(paren
id|buffer
)paren
suffix:semicolon
)brace
multiline_comment|/* Add the bay position, using at least two digits */
r_if
c_cond
(paren
id|position
OL
l_int|10
)paren
op_star
id|buffer
op_increment
op_assign
l_char|&squot;0&squot;
suffix:semicolon
id|sprintf
c_func
(paren
id|buffer
comma
l_string|&quot;%d&quot;
comma
id|position
)paren
suffix:semicolon
)brace
multiline_comment|/*&n; * Parse a module id, in either brief or long form.&n; * Returns &lt; 0 on error.&n; * The long form does not include a brick type, so it defaults to 0 (CBrick)&n; */
r_int
DECL|function|parse_module_id
id|parse_module_id
c_func
(paren
r_char
op_star
id|buffer
)paren
(brace
r_int
r_int
id|v
comma
id|rack
comma
id|bay
comma
id|type
comma
id|form
suffix:semicolon
id|moduleid_t
id|m
suffix:semicolon
r_char
id|c
suffix:semicolon
r_if
c_cond
(paren
id|strstr
c_func
(paren
id|buffer
comma
id|EDGE_LBL_RACK
l_string|&quot;/&quot;
)paren
op_eq
id|buffer
)paren
(brace
id|form
op_assign
id|MODULE_FORMAT_LONG
suffix:semicolon
id|buffer
op_add_assign
id|strlen
c_func
(paren
id|EDGE_LBL_RACK
l_string|&quot;/&quot;
)paren
suffix:semicolon
multiline_comment|/* A long module ID must be exactly 5 non-template chars. */
r_if
c_cond
(paren
id|strlen
c_func
(paren
id|buffer
)paren
op_ne
id|strlen
c_func
(paren
l_string|&quot;/&quot;
id|EDGE_LBL_RPOS
l_string|&quot;/&quot;
)paren
op_plus
l_int|5
)paren
r_return
op_minus
l_int|1
suffix:semicolon
)brace
r_else
(brace
id|form
op_assign
id|MODULE_FORMAT_BRIEF
suffix:semicolon
multiline_comment|/* A brief module id must be exactly 6 characters */
r_if
c_cond
(paren
id|strlen
c_func
(paren
id|buffer
)paren
op_ne
l_int|6
)paren
r_return
op_minus
l_int|2
suffix:semicolon
)brace
multiline_comment|/* The rack number must be exactly 3 digits */
r_if
c_cond
(paren
op_logical_neg
(paren
id|isdigit
c_func
(paren
id|buffer
(braket
l_int|0
)braket
)paren
op_logical_and
id|isdigit
c_func
(paren
id|buffer
(braket
l_int|1
)braket
)paren
op_logical_and
id|isdigit
c_func
(paren
id|buffer
(braket
l_int|2
)braket
)paren
)paren
)paren
r_return
op_minus
l_int|3
suffix:semicolon
id|rack
op_assign
l_int|0
suffix:semicolon
id|v
op_assign
op_star
id|buffer
op_increment
op_minus
l_char|&squot;0&squot;
suffix:semicolon
r_if
c_cond
(paren
id|v
OG
id|RACK_CLASS_MASK
c_func
(paren
id|rack
)paren
op_rshift
id|RACK_CLASS_SHFT
c_func
(paren
id|rack
)paren
)paren
r_return
op_minus
l_int|4
suffix:semicolon
id|RACK_ADD_CLASS
c_func
(paren
id|rack
comma
id|v
)paren
suffix:semicolon
id|v
op_assign
op_star
id|buffer
op_increment
op_minus
l_char|&squot;0&squot;
suffix:semicolon
r_if
c_cond
(paren
id|v
OG
id|RACK_GROUP_MASK
c_func
(paren
id|rack
)paren
op_rshift
id|RACK_GROUP_SHFT
c_func
(paren
id|rack
)paren
)paren
r_return
op_minus
l_int|5
suffix:semicolon
id|RACK_ADD_GROUP
c_func
(paren
id|rack
comma
id|v
)paren
suffix:semicolon
id|v
op_assign
op_star
id|buffer
op_increment
op_minus
l_char|&squot;0&squot;
suffix:semicolon
multiline_comment|/* rack numbers are 1-based */
r_if
c_cond
(paren
id|v
op_minus
l_int|1
OG
id|RACK_NUM_MASK
c_func
(paren
id|rack
)paren
op_rshift
id|RACK_NUM_SHFT
c_func
(paren
id|rack
)paren
)paren
r_return
op_minus
l_int|6
suffix:semicolon
id|RACK_ADD_NUM
c_func
(paren
id|rack
comma
id|v
)paren
suffix:semicolon
r_if
c_cond
(paren
id|form
op_eq
id|MODULE_FORMAT_BRIEF
)paren
(brace
multiline_comment|/* Next should be a module type character.  Accept ucase or lcase. */
id|c
op_assign
op_star
id|buffer
op_increment
suffix:semicolon
r_if
c_cond
(paren
op_logical_neg
id|isalpha
c_func
(paren
id|c
)paren
)paren
r_return
op_minus
l_int|7
suffix:semicolon
multiline_comment|/* strchr() returns a pointer into brick_types[], or NULL */
id|type
op_assign
(paren
r_int
r_int
)paren
(paren
id|strchr
c_func
(paren
id|brick_types
comma
id|tolower
c_func
(paren
id|c
)paren
)paren
op_minus
id|brick_types
)paren
suffix:semicolon
r_if
c_cond
(paren
id|type
OG
id|MODULE_BTYPE_MASK
op_rshift
id|MODULE_BTYPE_SHFT
)paren
r_return
op_minus
l_int|8
suffix:semicolon
)brace
r_else
(brace
multiline_comment|/* Hardcode the module type, and skip over the boilerplate */
id|type
op_assign
id|MODULE_CBRICK
suffix:semicolon
r_if
c_cond
(paren
id|strstr
c_func
(paren
id|buffer
comma
l_string|&quot;/&quot;
id|EDGE_LBL_RPOS
l_string|&quot;/&quot;
)paren
op_ne
id|buffer
)paren
r_return
op_minus
l_int|9
suffix:semicolon
id|buffer
op_add_assign
id|strlen
c_func
(paren
l_string|&quot;/&quot;
id|EDGE_LBL_RPOS
l_string|&quot;/&quot;
)paren
suffix:semicolon
)brace
multiline_comment|/* The bay number is last.  Make sure it&squot;s exactly two digits */
r_if
c_cond
(paren
op_logical_neg
(paren
id|isdigit
c_func
(paren
id|buffer
(braket
l_int|0
)braket
)paren
op_logical_and
id|isdigit
c_func
(paren
id|buffer
(braket
l_int|1
)braket
)paren
op_logical_and
op_logical_neg
id|buffer
(braket
l_int|2
)braket
)paren
)paren
r_return
op_minus
l_int|10
suffix:semicolon
id|bay
op_assign
l_int|10
op_star
(paren
id|buffer
(braket
l_int|0
)braket
op_minus
l_char|&squot;0&squot;
)paren
op_plus
(paren
id|buffer
(braket
l_int|1
)braket
op_minus
l_char|&squot;0&squot;
)paren
suffix:semicolon
r_if
c_cond
(paren
id|bay
OG
id|MODULE_BPOS_MASK
op_rshift
id|MODULE_BPOS_SHFT
)paren
r_return
op_minus
l_int|11
suffix:semicolon
id|m
op_assign
id|RBT_TO_MODULE
c_func
(paren
id|rack
comma
id|bay
comma
id|type
)paren
suffix:semicolon
multiline_comment|/* avoid sign extending the moduleid_t */
r_return
(paren
r_int
)paren
(paren
r_int
r_int
)paren
id|m
suffix:semicolon
)brace
macro_line|#else /* CONFIG_SGI_IP35 || CONFIG_IA64_SGI_SN1 */
multiline_comment|/*&n; * Format a module id for printing.&n; */
r_void
DECL|function|format_module_id
id|format_module_id
c_func
(paren
r_char
op_star
id|buffer
comma
id|moduleid_t
id|m
comma
r_int
id|fmt
)paren
(brace
r_if
c_cond
(paren
id|fmt
op_eq
id|MODULE_FORMAT_BRIEF
)paren
(brace
id|sprintf
c_func
(paren
id|buffer
comma
l_string|&quot;%d&quot;
comma
id|m
)paren
suffix:semicolon
)brace
r_else
r_if
c_cond
(paren
id|fmt
op_eq
id|MODULE_FORMAT_LONG
)paren
(brace
id|sprintf
c_func
(paren
id|buffer
comma
id|EDGE_LBL_MODULE
l_string|&quot;/%d&quot;
comma
id|m
)paren
suffix:semicolon
)brace
)brace
multiline_comment|/*&n; * Parse a module id, in either brief or long form.&n; * Returns &lt; 0 on error.&n; */
r_int
DECL|function|parse_module_id
id|parse_module_id
c_func
(paren
r_char
op_star
id|buffer
)paren
(brace
id|moduleid_t
id|m
suffix:semicolon
r_char
id|c
suffix:semicolon
r_if
c_cond
(paren
id|strstr
c_func
(paren
id|buffer
comma
id|EDGE_LBL_MODULE
l_string|&quot;/&quot;
)paren
op_eq
id|buffer
)paren
id|buffer
op_add_assign
id|strlen
c_func
(paren
id|EDGE_LBL_MODULE
l_string|&quot;/&quot;
)paren
suffix:semicolon
id|m
op_assign
l_int|0
suffix:semicolon
r_while
c_loop
(paren
id|c
op_assign
op_star
id|buffer
op_increment
)paren
(brace
r_if
c_cond
(paren
op_logical_neg
id|isdigit
c_func
(paren
id|c
)paren
)paren
r_return
op_minus
l_int|1
suffix:semicolon
id|m
op_assign
l_int|10
op_star
id|m
op_plus
(paren
id|c
op_minus
l_char|&squot;0&squot;
)paren
suffix:semicolon
)brace
multiline_comment|/* avoid sign extending the moduleid_t */
r_return
(paren
r_int
)paren
(paren
r_int
r_int
)paren
id|m
suffix:semicolon
)brace
macro_line|#endif /* CONFIG_SGI_IP35 || CONFIG_IA64_SGI_SN1 */
eof
