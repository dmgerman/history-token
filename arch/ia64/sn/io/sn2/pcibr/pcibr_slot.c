...
macro_line|#if 0
multiline_comment|/*&n; * pcibr_slot_reset&n; *&t;Reset the PCI device in the particular slot.&n; *&n; *      The Xbridge does not comply with the PCI Specification&n; *      when resetting an indiviaudl slot.  An individual slot is&n; *      is reset by toggling the slot&squot;s bit in the Xbridge Control&n; *      Register.  The Xbridge will assert the target slot&squot;s &n; *      (non-bussed) RST signal, but does not assert the (bussed) &n; *      REQ64 signal as required by the specification.   As&n; *      designed, the Xbridge cannot assert the REQ64 signal&n; *      becuase it may interfere with a bus transaction in progress.&n; *      The practical effects of this Xbridge implementation is&n; *      device dependent;  it probably will not adversely effect&n; *      32-bit cards, but may disable 64-bit data transfers by those&n; *      cards that normally support 64-bit data transfers.  &n; *&n; *      The Xbridge will assert REQ64 when all four slots are reset&n; *      by simultaneously toggling all four slot reset bits in the&n; *      Xbridge Control Register.  This is basically a PCI bus reset&n; *      and asserting the (bussed) REQ64 signal will not interfere&n; *      with any bus transactions in progress.&n; *&n; *      The Xbridge (and the SN0 Bridge) support resetting only&n; *      four PCI bus slots via the (X)bridge Control Register.&n; *&n; *      To reset an individual slot for the PCI Hot-Plug feature&n; *      use the L1 console commands to power-down and then &n; *      power-up the slot, or use the kernel infrastructure&n; *      functions to power-down/up the slot when they are&n; *      implemented for SN1.&n; */
r_int
id|pcibr_slot_reset
c_func
(paren
...
multiline_comment|/*&n; * pcibr_probe_slot_non_pic: read a config space word&n; * while trapping any errors; return zero if&n; * all went OK, or nonzero if there was an error.&n; * The value read, if any, is passed back&n; * through the valp parameter.&n; */
r_static
r_int
DECL|function|pcibr_probe_slot_non_pic
id|pcibr_probe_slot_non_pic
c_func
(paren
id|bridge_t
op_star
