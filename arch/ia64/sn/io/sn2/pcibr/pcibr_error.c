...
suffix:semicolon
macro_line|#endif
DECL|variable|pcibr_pioerr_dump
r_int
id|pcibr_pioerr_dump
op_assign
l_int|1
suffix:semicolon
multiline_comment|/* always dump pio errors */
...
c_func
(paren
l_string|&quot;&bslash;t    Bridge %sError Command Word Register &quot;
comma
id|errtype
)paren
suffix:semicolon
...
suffix:colon
multiline_comment|/* bit30    PMU_PAGE_FAULT */
id|printk
c_func
(paren
l_string|&quot;&bslash;t    Map Fault Address Reg: 0x%lx&bslash;n&quot;
comma
...
suffix:colon
multiline_comment|/* bit29    UNEXPECTED_RESP */
id|print_bridge_errcmd
c_func
(paren
id|pcibr_soft
...
suffix:colon
multiline_comment|/* bit28    BAD_RESP_PACKET */
r_case
id|BRIDGE_ISR_RESP_XTLK_ERR
suffix:colon
multiline_comment|/* bit26    RESP_XTALK_ERROR */
id|print_bridge_errcmd
c_func
(paren
id|pcibr_soft
...
suffix:semicolon
multiline_comment|/* PCI-X mode, DMA Request Error registers are valid.  But&n;&t;&t; * in PCI mode, Response Buffer Address register are valid.&n;&t;&t; */
r_if
c_cond
(paren
id|IS_PCIX
...
c_func
(paren
l_string|&quot;&bslash;t    Bridge Response Buf Error Addr Reg: 0x%lx&bslash;n&quot;
...
suffix:colon
multiline_comment|/* bit27    BAD_XREQ_PACKET */
r_case
id|BRIDGE_ISR_REQ_XTLK_ERR
suffix:colon
multiline_comment|/* bit25    REQ_XTALK_ERROR */
r_case
id|BRIDGE_ISR_INVLD_ADDR
suffix:colon
multiline_comment|/* bit24    INVALID_ADDRESS */
id|print_bridge_errcmd
c_func
(paren
id|pcibr_soft
...
c_func
(paren
l_string|&quot;&bslash;t    Bridge Error Address Register: 0x%lx&bslash;n&quot;
...
id|BRIDGE_ISR_UNSUPPORTED_XOP
suffix:colon
multiline_comment|/* bit23    UNSUPPORTED_XOP */
id|print_bridge_errcmd
c_func
(paren
id|pcibr_soft
...
suffix:colon
multiline_comment|/* bit22    XREQ_FIFO_OFLOW */
id|print_bridge_errcmd
c_func
(paren
id|pcibr_soft
...
suffix:colon
multiline_comment|/* bit15    PCI_ABORT */
r_case
id|BRIDGE_ISR_PCI_PARITY
suffix:colon
multiline_comment|/* bit14    PCI_PARITY */
r_case
id|BRIDGE_ISR_PCI_SERR
suffix:colon
multiline_comment|/* bit13    PCI_SERR */
r_case
id|BRIDGE_ISR_PCI_PERR
suffix:colon
multiline_comment|/* bit12    PCI_PERR */
r_case
id|BRIDGE_ISR_PCI_MST_TIMEOUT
suffix:colon
multiline_comment|/* bit11    PCI_MASTER_TOUT */
r_case
id|BRIDGE_ISR_PCI_RETRY_CNT
suffix:colon
multiline_comment|/* bit10    PCI_RETRY_CNT */
id|printk
c_func
(paren
l_string|&quot;&bslash;t    PCI Error Address Register: 0x%lx&bslash;n&quot;
...
suffix:colon
multiline_comment|/* bit09    XREAD_REQ_TOUT */
id|printk
c_func
(paren
l_string|&quot;&bslash;t    Bridge Response Buf Error Addr Reg: 0x%lx&bslash;n&quot;
...
multiline_comment|/*&n;     * pcibr_pioerr_dump is a systune that make be used to not&n;     * print bridge registers for interrupts generated by pio-errors.&n;     * Some customers do early probes and expect a lot of failed&n;     * pios.&n;     */
r_if
c_cond
(paren
op_logical_neg
...
suffix:semicolon
multiline_comment|/* machine_error_dump(&quot;&quot;); */
id|panic
c_func
(paren
...
multiline_comment|/*&n; * pcibr_pioerror&n; *      Handle PIO error that happened at the bridge pointed by pcibr_soft.&n; *&n; *      Queries the Bus interface attached to see if the device driver&n; *      mapping the device-number that caused error can handle the&n; *      situation. If so, it will clean up any error, and return&n; *      indicating the error was handled. If the device driver is unable&n; *      to handle the error, it expects the bus-interface to disable that&n; *      device, and takes any steps needed here to take away any resources&n; *      associated with this device.&n; *&n; * A note about slots:&n; *&n; * &t;PIC-based bridges use zero-based device numbering when devices to&n; * &t;internal registers.  However, the physical slots are numbered using a&n; *&t;one-based scheme because in PCI-X, device 0 is reserved (see comments&n; * &t;in pcibr_private.h for a better description).&n; *&n; * &t;When building up the hwgraph, we use the external (one-based) number&n; *&t;scheme when numbering slot components so that hwgraph more accuratly&n; * &t;reflects what is silkscreened on the bricks.&n; *&n; * &t;Since pciio_error_handler() needs to ultimatly be able to do a hwgraph&n; *&t;lookup, the ioerror that gets built up in pcibr_pioerror() encodes the&n; *&t;external (one-based) slot number.  However, loops in pcibr_pioerror() &n; * &t;which attempt to translate the virtual address into the correct&n; * &t;PCI physical address use the device (zero-based) numbering when &n; * &t;walking through bridge structures.&n; *&n; * &t;To that end, pcibr_pioerror() uses device to denote the &n; *&t;zero-based device number, and external_slot to denote the corresponding&n; *&t;one-based slot number.  Loop counters (eg. cs) are always device based.&n; */
...
id|device
suffix:semicolon
multiline_comment|/* final PCI device if appropriate */
id|pciio_slot_t
id|external_slot
suffix:semicolon
multiline_comment|/* external slot for device */
id|pciio_function_t
id|func
suffix:semicolon
...
c_func
(paren
l_string|&quot;pcibr_pioerror(): unknown bridge type&quot;
)paren
suffix:semicolon
r_return
...
id|pcibr_soft-&gt;bs_conn
comma
l_string|&quot;pcibr_pioerror: space=%d, offset=0x%lx, dev=0x%x, func=0x%x&bslash;n&quot;
comma
id|space
comma
id|offset
