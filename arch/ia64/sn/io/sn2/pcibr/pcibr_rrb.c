...
mdefine_line|#define RRB_ENABLE_BIT&t;&t;      (0x8)  /* [BRIDGE | PIC]_RRB_EN */
DECL|macro|NUM_PDEV_BITS
mdefine_line|#define NUM_PDEV_BITS&t;&t;      (1)
DECL|macro|NUMBER_VCHANNELS
mdefine_line|#define NUMBER_VCHANNELS&t;      (4)
DECL|macro|SLOT_2_PDEV
mdefine_line|#define SLOT_2_PDEV(slot)&t;&t;((slot) &gt;&gt; 1)
DECL|macro|SLOT_2_RRB_REG
mdefine_line|#define SLOT_2_RRB_REG(slot)  &t;&t;((slot) &amp; 0x1)
DECL|macro|RRB_VALID
mdefine_line|#define RRB_VALID(rrb)&t;&t;      (0x00010000 &lt;&lt; (rrb))
DECL|macro|RRB_INUSE
mdefine_line|#define RRB_INUSE(rrb)&t;&t;      (0x00000001 &lt;&lt; (rrb))
DECL|macro|RRB_CLEAR
mdefine_line|#define RRB_CLEAR(rrb)&t;&t;      (0x00000001 &lt;&lt; (rrb))
multiline_comment|/* validate that the slot and virtual channel are valid */
DECL|macro|VALIDATE_SLOT_n_VCHAN
mdefine_line|#define VALIDATE_SLOT_n_VCHAN(s, v) &bslash;&n;    (((((s) != PCIIO_SLOT_NONE) &amp;&amp; ((s) &lt;= (pciio_slot_t)3)) &amp;&amp; &bslash;&n;      (((v) &gt;= 0) &amp;&amp; ((v) &lt;= 3))) ? 1 : 0)
...
id|status
suffix:semicolon
multiline_comment|/* bridge_lock must be held;  this RRB must be disabled. */
...
multiline_comment|/*&n; * Initialize a slot with a given number of RRBs.  (this routine&n; * will also give back RRBs if the slot has more than we want).&n; */
r_void
DECL|function|pcibr_rrb_alloc_init
id|pcibr_rrb_alloc_init
c_func
(paren
id|pcibr_soft_t
...
multiline_comment|/* Free any extra RRBs that the slot may have allocated to it */
r_while
c_loop
(paren
id|have
...
comma
l_string|&quot;pcibr_rrb_alloc_init: had %d, added/removed %d, &quot;
l_string|&quot;(of requested %d) RRBs &quot;
...
c_func
(paren
l_string|&quot;pcibr_rrb_alloc_init&quot;
comma
id|pcibr_soft
)paren
suffix:semicolon
)brace
multiline_comment|/*&n; * Allocate more RRBs to a given slot (if the RRBs are available).&n; */
r_void
DECL|function|pcibr_rrb_alloc_more
id|pcibr_rrb_alloc_more
c_func
(paren
id|pcibr_soft_t
...
id|pcibr_soft-&gt;bs_vhdl
comma
l_string|&quot;pcibr_rrb_alloc_more: added %d (of %d requested) RRBs &quot;
l_string|&quot;to slot %d, vchan %d&bslash;n&quot;
comma
id|added
comma
...
suffix:semicolon
multiline_comment|/*&n;     * PIC BRINGUP WAR (PV# 856866, 859504, 861476, 861478):&n;     * Don&squot;t free RRBs we allocated to device[2|3]--vchan3 as&n;     * a WAR to those PVs mentioned above.  In pcibr_attach2&n;     * we allocate RRB0,8,1,9 to device[2|3]--vchan3.&n;     */
r_if
c_cond
(paren
...
suffix:semicolon
multiline_comment|/* Reserve RRBs for this empty slot for hot-plug */
r_for
c_loop
(paren
id|vchan
