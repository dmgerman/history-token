...
multiline_comment|/* In general, this file is organized in a hierarchy from lower-level&n; * to higher-level layers, as follows:&n; *&n; *&t;UART routines&n; *&t;Bedrock/L1 &quot;PPP-like&quot; protocol implementation&n; *&t;System controller &quot;message&quot; interface (allows multiplexing&n; *&t;&t;of various kinds of requests and responses with&n; *&t;&t;console I/O)&n; *&t;Console interfaces (there are two):&n; *&t;  (1) &quot;elscuart&quot;, used in the IP35prom and (maybe) some&n; *&t;&t;debugging situations elsewhere, and&n; *&t;  (2) &quot;l1_cons&quot;, the glue that allows the L1 to act&n; *&t;&t;as the system console for the stdio libraries&n; *&n; * Routines making use of the system controller &quot;message&quot;-style interface&n; * can be found in l1_command.c.  Their names are leftover from early SN0, &n; * when the &quot;module system controller&quot; (msc) was known as the &quot;entry level&n; * system controller&quot; (elsc).  The names and signatures of those functions &n; * remain unchanged in order to keep the SN0 -&gt; SN1 system controller&n; * changes fairly localized.&n; */
...
macro_line|#include &lt;asm/sn/sn1/uart16550.h&gt;
macro_line|#if defined(EEPROM_DEBUG)
DECL|macro|db_printf
mdefine_line|#define db_printf(x) printk x
macro_line|#else
DECL|macro|db_printf
mdefine_line|#define db_printf(x)
macro_line|#endif
singleline_comment|// From irix/kern/sys/SN/SN1/bdrkhspecregs.h
DECL|macro|HSPEC_UART_0
mdefine_line|#define    HSPEC_UART_0              0x00000080    /* UART Registers         */
...
DECL|macro|SD
mdefine_line|#define SD(x, v)        (LD(x) = (uint64_t) (v))
multiline_comment|/* location of uart receive/xmit data register */
DECL|macro|L1_UART_BASE
mdefine_line|#define L1_UART_BASE(n)&t;((ulong)REMOTE_HSPEC_ADDR((n), HSPEC_UART_0))
DECL|macro|LOCAL_HUB
mdefine_line|#define LOCAL_HUB&t;LOCAL_HUB_ADDR
DECL|macro|ADDR_L1_REG
mdefine_line|#define ADDR_L1_REG(n, r)&t;&bslash;&n;    (L1_UART_BASE(n) | ( (r) &lt;&lt; 3 ))
DECL|macro|READ_L1_UART_REG
mdefine_line|#define READ_L1_UART_REG(n, r) &bslash;&n;    ( LD(ADDR_L1_REG((n), (r))) )
DECL|macro|WRITE_L1_UART_REG
mdefine_line|#define WRITE_L1_UART_REG(n, r, v) &bslash;&n;    ( SD(ADDR_L1_REG((n), (r)), (v)) )
multiline_comment|/* Avoid conflicts with symmon...*/
DECL|macro|CONS_HW_LOCK
mdefine_line|#define CONS_HW_LOCK(x)
DECL|macro|CONS_HW_UNLOCK
mdefine_line|#define CONS_HW_UNLOCK(x)
DECL|macro|L1_CONS_HW_LOCK
mdefine_line|#define L1_CONS_HW_LOCK(sc)&t;CONS_HW_LOCK(sc-&gt;uart == BRL1_LOCALUART)
DECL|macro|L1_CONS_HW_UNLOCK
mdefine_line|#define L1_CONS_HW_UNLOCK(sc)&t;CONS_HW_UNLOCK(sc-&gt;uart == BRL1_LOCALUART)
macro_line|#if DEBUG
DECL|variable|debuglock_ospl
r_static
r_int
id|debuglock_ospl
suffix:semicolon
multiline_comment|/* For CONS_HW_LOCK macro */
macro_line|#endif
...
mdefine_line|#define UART_DELAY(x)&t;{ int i; i = x * 1000; while (--i); }
macro_line|#else
DECL|macro|UART_DELAY
mdefine_line|#define UART_DELAY(x)&t;us_delay(x)
macro_line|#endif
...
macro_line|#else&t;/* BIG_ENDIAN */
DECL|macro|COPY_INT_TO_BUFFER
mdefine_line|#define COPY_INT_TO_BUFFER(_b, _i, _n)&t;&t;&t;&bslash;&n;&t;{&t;&t;&t;&t;&t;&t;&bslash;&n;&t;&t;bcopy((char *)&amp;_n, _b, sizeof(_n));&t;&bslash;&n;&t;&t;_i += sizeof(_n);&t;&t;&t;&bslash;&n;&t;}
DECL|macro|COPY_BUFFER_TO_INT
mdefine_line|#define COPY_BUFFER_TO_INT(_b, _i, _n)&t;&t;&t;&bslash;&n;&t;{&t;&t;&t;&t;&t;&t;&bslash;&n;&t;&t;bcopy(&amp;_b[_i], &amp;_n, sizeof(_n));&t;&bslash;&n;&t;&t;_i += sizeof(_n);&t;&t;&t;&bslash;&n;&t;}
DECL|macro|COPY_BUFFER_TO_BUFFER
mdefine_line|#define COPY_BUFFER_TO_BUFFER(_b, _i, _bn)&t;&t;&bslash;&n;&t;{&t;&t;&t;&t;&t;&t;&bslash;&n;            bcopy(&amp;(_b[_i]), _bn, sizeof(int));&t;&t;&bslash;&n;            _i += sizeof(int);&t;&t;&t;&t;&bslash;&n;&t;}
macro_line|#endif&t;/* LITTLE_ENDIAN */
r_int
id|atomicAddInt
c_func
(paren
...
mdefine_line|#define UART_PUTC_READY(n)&t;(READ_L1_UART_REG((n), REG_LSR) &amp; LSR_XHRE)
r_static
r_int
...
(brace
macro_line|#ifdef BRINGUP
multiline_comment|/* need a delay to avoid dropping chars */
id|UART_DELAY
c_func
(paren
l_int|57
...
mdefine_line|#define RTR_UART_PUTC_TIMEOUT&t;0
DECL|macro|RTR_UART_DELAY_SPAN
mdefine_line|#define RTR_UART_DELAY_SPAN&t;0
DECL|macro|RTR_UART_INIT_TIMEOUT
mdefine_line|#define RTR_UART_INIT_TIMEOUT&t;0
macro_line|#else
DECL|macro|RTR_UART_PUTC_TIMEOUT
...
mdefine_line|#define L1SC_SEND_LOCK(l,pl)&t;&t;&t;&t;&t;&t;&bslash;&n;     { if( (l)-&gt;uart == BRL1_LOCALUART )&t;&t;&t;&t;&bslash;&n;&t; (pl) = mutex_spinlock_spl( &amp;((l)-&gt;send_lock), spl7 ); }
DECL|macro|L1SC_SEND_UNLOCK
mdefine_line|#define L1SC_SEND_UNLOCK(l,pl)&t;&t;&t;&t;&bslash;&n;     { if( (l)-&gt;uart == BRL1_LOCALUART )&t;&t;&bslash;&n;&t; mutex_spinunlock( &amp;((l)-&gt;send_lock), (pl)); }
DECL|macro|L1SC_RECV_LOCK
mdefine_line|#define L1SC_RECV_LOCK(l,pl)&t;&t;&t;&t;&t;&t;&bslash;&n;     { if( (l)-&gt;uart == BRL1_LOCALUART )&t;&t;&t;&t;&bslash;&n;&t; (pl) = mutex_spinlock_spl( &amp;((l)-&gt;recv_lock), spl7 ); }
DECL|macro|L1SC_RECV_UNLOCK
mdefine_line|#define L1SC_RECV_UNLOCK(l,pl)&t;&t;&t;&t;&bslash;&n;     { if( (l)-&gt;uart == BRL1_LOCALUART )&t;&t;&bslash;&n;&t; mutex_spinunlock( &amp;((l)-&gt;recv_lock), (pl)); }
...
macro_line|#ifdef SPINLOCKS_WORK
DECL|macro|SUBCH_LOCK
mdefine_line|#define SUBCH_LOCK(sc,pl) &bslash;&n;     (pl) = mutex_spinlock_spl( &amp;((sc)-&gt;subch_lock), spl7 )
DECL|macro|SUBCH_UNLOCK
mdefine_line|#define SUBCH_UNLOCK(sc,pl) &bslash;&n;     mutex_spinunlock( &amp;((sc)-&gt;subch_lock), (pl) )
DECL|macro|SUBCH_DATA_LOCK
mdefine_line|#define SUBCH_DATA_LOCK(sbch,pl) &bslash;&n;     (pl) = mutex_spinlock_spl( &amp;((sbch)-&gt;data_lock), spl7 )
DECL|macro|SUBCH_DATA_UNLOCK
mdefine_line|#define SUBCH_DATA_UNLOCK(sbch,pl) &bslash;&n;     mutex_spinunlock( &amp;((sbch)-&gt;data_lock), (pl) )
macro_line|#else
DECL|macro|SUBCH_LOCK
mdefine_line|#define SUBCH_LOCK(sc,pl) 
DECL|macro|SUBCH_UNLOCK
mdefine_line|#define SUBCH_UNLOCK(sc,pl)
DECL|macro|SUBCH_DATA_LOCK
mdefine_line|#define SUBCH_DATA_LOCK(sbch,pl)
DECL|macro|SUBCH_DATA_UNLOCK
mdefine_line|#define SUBCH_DATA_UNLOCK(sbch,pl)
macro_line|#endif&t;/* SPINLOCKS_WORK */
...
id|func
suffix:semicolon
multiline_comment|/* some upper layer is asking to be notified of low-water, but if the &n;     * send buffer isn&squot;t already in use, we&squot;re going to need to get the&n;     * interrupts going on the uart...&n;     */
r_if
c_cond
(paren
id|func
...
multiline_comment|/* get_myid is an internal function that reads the PI_CPU_NUM&n; * register of the local bedrock to determine which of the&n; * four possible CPU&squot;s &quot;this&quot; one is&n; */
r_static
r_int
DECL|function|get_myid
id|get_myid
c_func
(paren
r_void
...
(brace
multiline_comment|/* In the kernel, we track the depth of the C brick&squot;s UART&squot;s&n;     * fifo in software, and only check if the UART is accepting&n;     * characters when our count indicates that the fifo should&n;     * be full.&n;     *&n;     * For remote (router) UARTs, and also for the local (C brick)&n;     * UART in the prom, we check with the UART before sending every&n;     * character.&n;     */
r_if
c_cond
(paren
id|sc-&gt;uart
...
(brace
singleline_comment|//&t;    sc-&gt;fifo_space = UART_FIFO_DEPTH;
id|sc-&gt;fifo_space
op_assign
l_int|1000
suffix:semicolon
...
r_else
multiline_comment|/* The following applies to all UARTs in the prom, and to remote&n;     * (router) UARTs in the kernel...&n;     */
DECL|macro|TIMEOUT_RETRIES
mdefine_line|#define TIMEOUT_RETRIES&t;30
(brace
r_int
id|result
suffix:semicolon
...
l_int|0
suffix:semicolon
multiline_comment|/* couldn&squot;t send anything; wait for buffer to drain */
)brace
r_else
(brace
...
(brace
multiline_comment|/* success! release the send buffer */
id|sc-&gt;send_in_use
op_assign
l_int|0
suffix:semicolon
...
suffix:semicolon
multiline_comment|/* collect pointers to callups *before* unlocking */
id|subch
op_assign
id|sc-&gt;subch
suffix:semicolon
...
(brace
multiline_comment|/* call any upper layer that&squot;s asked for low-water notification */
id|callup
op_assign
id|callups
suffix:semicolon
...
mdefine_line|#define STATE_SET(l,s)&t;((l)-&gt;brl1_state = (s))
DECL|macro|STATE_GET
mdefine_line|#define STATE_GET(l)&t;((l)-&gt;brl1_state)
...
DECL|macro|SEQSTAMP_INCR
mdefine_line|#define SEQSTAMP_INCR(l)
DECL|macro|SEQSTAMP_GET
mdefine_line|#define SEQSTAMP_GET(l)
...
mdefine_line|#define IS_TTY_PKT(l) &bslash;&n;         ( SUBCH(LAST_HDR_GET(l)) &lt;= SC_CONS_SYSTEM ? 1 : 0 )
r_int
...
id|pl
suffix:semicolon
multiline_comment|/* priority level for UART receive lock */
r_int
id|done
suffix:semicolon
...
id|sch_pl
suffix:semicolon
multiline_comment|/* cookie for subchannel lock */
id|brl1_notif_t
id|callup
suffix:semicolon
...
suffix:semicolon
multiline_comment|/* bump the packet sequence counter */
...
multiline_comment|/*&n; * brl1_intrd is the function which is called in a loop by the&n; * xthread that services L1 interrupts.&n; */
...
id|ep
suffix:semicolon
multiline_comment|/* placate the compiler */
)brace
macro_line|#endif
...
(brace
multiline_comment|/* Disable the UART interrupt, giving the xthread time to respond.&n;     * When the daemon (xthread) finishes doing its thing, it will&n;     * unblock the interrupt.&n;     */
id|intr_block_bit
c_func
(paren
id|get_elsc
...
multiline_comment|/* set up uart interrupt handling for this node&squot;s uart&n; */
r_void
DECL|function|brl1_connect_intr
id|brl1_connect_intr
c_func
(paren
id|l1sc_t
op_star
...
id|CE_PANIC
comma
l_string|&quot;brl1_connect_intr: Can&squot;t connect UART interrupt.&quot;
)paren
suffix:semicolon
)brace
...
macro_line|#ifdef SABLE
multiline_comment|/* this function is called periodically to generate fake interrupts&n; * and allow brl1_intrd to send/receive characters&n; */
r_void
DECL|function|hubuart_service
id|hubuart_service
c_func
(paren
r_void
...
suffix:semicolon
multiline_comment|/* note that we&squot;ll lose error state by reading the lsr_reg.&n;     * This is probably ok in the frictionless domain of sable.&n;     */
r_int
id|lsr_reg
suffix:semicolon
id|nasid_t
...
multiline_comment|/*********************************************************************&n; * The following function allows the kernel to &quot;go around&quot; the&n; * uninitialized l1sc structure to allow console output during&n; * early system startup.&n; */
multiline_comment|/* These are functions to use from serial_in/out when in protocol&n; * mode to send and receive uart control regs.&n; */
r_void
DECL|function|brl1_send_control
id|brl1_send_control
c_func
(paren
r_int
id|offset
...
r_int
DECL|function|brl1_get_control
id|brl1_get_control
c_func
(paren
r_int
...
DECL|macro|PUTCHAR
mdefine_line|#define PUTCHAR(ch) &bslash;&n;    { &bslash;&n;        while( !(READ_L1_UART_REG( nasid, REG_LSR ) &amp; LSR_XHRE) );  &bslash;&n;        WRITE_L1_UART_REG( nasid, REG_DAT, (ch) ); &bslash;&n;    }
r_int
DECL|function|brl1_send_console_packet
id|brl1_send_console_packet
c_func
(paren
r_char
op_star
...
multiline_comment|/* &n; * Read as many characters from the system console receive queue as are&n; * available there (up to avail bytes).&n; */
r_int
DECL|function|l1_cons_read
id|l1_cons_read
c_func
(paren
id|l1sc_t
...
r_void
DECL|function|l1_cons_tx_notif
id|l1_cons_tx_notif
c_func
(paren
id|l1sc_t
...
r_void
DECL|function|l1_cons_rx_notif
id|l1_cons_rx_notif
c_func
(paren
id|l1sc_t
...
macro_line|#endif
multiline_comment|/* sc_data_ready is called to signal threads that are blocked on &n; * l1 input.&n; */
r_void
...
mdefine_line|#define __WAIT_SEND&t;( sc-&gt;uart != BRL1_LOCALUART )
...
multiline_comment|/* for now, sc_init just calls brl1_init&n; */
r_void
...
multiline_comment|/* write the event to syslog */
macro_line|#ifdef IRIX
id|cmn_err_tag
c_func
(paren
...
multiline_comment|/*********************************************************************&n; * elscuart functions.  These provide a uart-like interface to the&n; * bedrock/l1 protocol console channels.  They are similar in form&n; * and intent to the elscuart_* functions defined for SN0 in elsc.c.&n; *&n; */
r_int
id|_elscuart_flush
c_func
(paren
...
suffix:semicolon
multiline_comment|/* Leave room in queue for CR/LF */
DECL|macro|ELSCUART_LINE_MAX
mdefine_line|#define ELSCUART_LINE_MAX       (BRL1_QSIZE - 2)
multiline_comment|/*&n; * _elscuart_putc provides an entry point to the L1 interface driver;&n; * writes a single character to the output queue.  Flushes at the&n; * end of each line, and translates newlines into CR/LF.&n; *&n; * The kernel should generally use l1_cons_write instead, since it assumes&n; * buffering, translation, prefixing, etc. are done at a higher&n; * level.&n; *&n; */
r_int
DECL|function|_elscuart_putc
id|_elscuart_putc
c_func
(paren
id|l1sc_t
...
(brace
multiline_comment|/* flush buffered line */
r_if
c_cond
(paren
id|_elscuart_flush
...
multiline_comment|/*&n; * _elscuart_getc reads a character from the input queue.  This&n; * routine blocks.&n; */
r_int
DECL|function|_elscuart_getc
id|_elscuart_getc
c_func
(paren
id|l1sc_t
...
multiline_comment|/*&n; * _elscuart_poll returns 1 if characters are ready for the&n; * calling processor, 0 if they are not&n; */
r_int
DECL|function|_elscuart_poll
id|_elscuart_poll
c_func
(paren
id|l1sc_t
...
multiline_comment|/* _elscuart_readc is to be used only when _elscuart_poll has&n; * indicated that a character is waiting.  Pulls a character&n; * of this processor&squot;s console queue and returns it.&n; *&n; */
r_int
DECL|function|_elscuart_readc
id|_elscuart_readc
c_func
(paren
id|l1sc_t
...
(brace
multiline_comment|/* buffer queue contents */
id|r
op_assign
id|BRL1_QSIZE
op_minus
...
multiline_comment|/* attempt to send buffer contents */
id|r
op_assign
id|brl1_send
c_func
...
suffix:semicolon
multiline_comment|/* if no error, dequeue the sent characters; otherwise,&n;         * return the error&n;         */
r_if
c_cond
(paren
id|r
...
multiline_comment|/* _elscuart_probe returns non-zero if the L1 (and&n; * consequently the elscuart) can be accessed&n; */
r_int
DECL|function|_elscuart_probe
id|_elscuart_probe
c_func
(paren
id|l1sc_t
...
multiline_comment|/* _elscuart_init zeroes out the l1sc_t console&n; * queues for this processor&squot;s console subchannel.&n; */
r_void
DECL|function|_elscuart_init
id|_elscuart_init
c_func
(paren
id|l1sc_t
...
macro_line|#ifdef IRIX
multiline_comment|/* elscuart_syscon_listen causes the processor on which it&squot;s&n; * invoked to &quot;listen&quot; to the system console subchannel (that&n; * is, subchannel 4) for console input.&n; */
r_void
DECL|function|elscuart_syscon_listen
...
suffix:semicolon
multiline_comment|/* if we&squot;re already listening, don&squot;t bother */
r_if
c_cond
(paren
id|sc-&gt;cons_listen
