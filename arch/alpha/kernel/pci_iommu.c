...
id|pci_iommu_arena
op_star
DECL|function|iommu_arena_new_node
...
c_func
(paren
l_string|&quot;%s: couldn&squot;t allocate arena from node %d&bslash;n&quot;
l_string|&quot;    falling back to system-wide allocation&bslash;n&quot;
comma
id|__FUNCTION__
comma
id|nid
...
c_func
(paren
l_string|&quot;%s: couldn&squot;t allocate arena ptes from node %d&bslash;n&quot;
l_string|&quot;    falling back to system-wide allocation&bslash;n&quot;
comma
id|__FUNCTION__
comma
...
multiline_comment|/* Must be called with the arena lock held */
r_static
r_int
DECL|function|iommu_arena_find_pages
id|iommu_arena_find_pages
c_func
(paren
r_struct
id|pci_iommu_arena
...
id|nent
suffix:semicolon
multiline_comment|/* Search forward for the first mask-aligned sequence of N free ptes */
id|ptes
op_assign
id|arena-&gt;ptes
suffix:semicolon
...
multiline_comment|/* Success. It&squot;s the responsibility of the caller to mark them&n;&t;   in use before releasing the lock */
r_return
id|p
suffix:semicolon
)brace
...
suffix:semicolon
multiline_comment|/* Search for N empty ptes */
id|ptes
op_assign
id|arena-&gt;ptes
suffix:semicolon
...
multiline_comment|/* Success.  Mark them all in use, ie not zero and invalid&n;&t;   for the iommu tlb that could load them from under us.&n;&t;   The chip specific bits will fill this in with something&n;&t;   kosher when we return.  */
r_for
c_loop
(paren
id|i
...
(brace
multiline_comment|/* If we attempted a direct map above but failed, die.  */
r_if
c_cond
(paren
id|leader-&gt;dma_address
...
id|arena
suffix:semicolon
multiline_comment|/* If there exists a direct map, and the mask fits either&n;&t;   the entire direct mapped space or the total system memory as&n;&t;   shifted by the map base */
r_if
c_cond
(paren
id|__direct_map_size
...
"&f;"
multiline_comment|/*&n; * AGP GART extensions to the IOMMU&n; */
r_int
DECL|function|iommu_reserve
id|iommu_reserve
c_func
(paren
r_struct
...
multiline_comment|/* Success.  Mark them all reserved (ie not zero and invalid)&n;&t;   for the iommu tlb that could load them from under us.&n;&t;   They will be filled in with valid bits by _bind() */
r_for
c_loop
(paren
id|i
...
id|arena-&gt;ptes
suffix:semicolon
multiline_comment|/* Make sure they&squot;re all reserved first... */
r_for
c_loop
(paren
id|i
...
multiline_comment|/* Helper for generic DMA-mapping functions. */
r_struct
id|pci_dev
op_star
DECL|function|alpha_gendev_to_pci
...
suffix:semicolon
multiline_comment|/* Assume that non-PCI devices asking for DMA are either ISA or EISA,&n;&t;   BUG() otherwise. */
id|BUG_ON
c_func
(paren
op_logical_neg
...
suffix:semicolon
multiline_comment|/* Assume non-busmaster ISA DMA when dma_mask is not set (the ISA&n;&t;   bridge is bus master then). */
r_if
c_cond
(paren
op_logical_neg
...
id|isa_bridge
suffix:semicolon
multiline_comment|/* For EISA bus masters, return isa_bridge (it might have smaller&n;&t;   dma_mask due to wiring limitations). */
r_if
c_cond
(paren
op_star
id|dev-&gt;dma_mask
op_ge
id|isa_bridge-&gt;dma_mask
)paren
r_return
id|isa_bridge
suffix:semicolon
multiline_comment|/* This assumes ISA bus master with dma_mask 0xffffff. */
r_return
l_int|NULL
suffix:semicolon
)brace
