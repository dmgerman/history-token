...
l_int|0
suffix:semicolon
multiline_comment|/*&n;&t; * Set up the PCI to main memory translation windows.&n;&t; *&n;&t; * Mimic the SRM settings for the direct-map window.&n;&t; *   Window 0 is scatter-gather 8MB at 8MB (for isa).&n;&t; *   Window 1 is direct access 1GB at 1GB.&n;&t; *&n;&t; * Note that we do not try to save any of the DMA window CSRs&n;&t; * before setting them, since we cannot read those CSRs on LCA.&n;&t; */
id|hose-&gt;sg_isa
op_assign
id|iommu_arena_new
c_func
...
l_int|5
suffix:semicolon
multiline_comment|/*&n;&t; * Finally, set up for restoring the correct HAE if using SRM.&n;&t; * Again, since we cannot read many of the CSRs on the LCA,&n;&t; * one of which happens to be the HAE, we save the value that&n;&t; * the SRM will expect...&n;&t; */
r_if
c_cond
(paren
id|alpha_using_srm
