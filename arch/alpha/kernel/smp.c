multiline_comment|/*&n; *&t;linux/arch/alpha/kernel/smp.c&n; *&n; *      2001-07-09 Phil Ezolt (Phillip.Ezolt@compaq.com)&n; *            Renamed modified smp_call_function to smp_call_function_on_cpu()&n; *            Created an function that conforms to the old calling convention&n; *            of smp_call_function().&n; *&n; *            This is helpful for DCPI.&n; *&n; */
...
c_func
(paren
l_string|&quot;??, cpu 0x%x already present??&bslash;n&quot;
comma
id|cpuid
)paren
suffix:semicolon
...
suffix:semicolon
multiline_comment|/* Wait boot CPU to stop with irq enabled before running&n;&t;   calibrate_delay. */
id|wait_boot_cpu_to_stop
c_func
(paren
id|cpuid
...
suffix:semicolon
multiline_comment|/* Allow master to continue only after we written loops_per_jiffy.  */
id|wmb
c_func
(paren
)paren
...
(paren
(paren
l_string|&quot;smp_callin: commencing CPU %d current %p active_mm %p&bslash;n&quot;
comma
id|cpuid
comma
id|current
...
multiline_comment|/* Wait until hwrpb-&gt;txrdy is clear for cpu.  Return -1 on timeout.  */
r_static
r_int
id|__init
DECL|function|wait_for_txrdy
id|wait_for_txrdy
(paren
r_int
r_int
...
op_assign
op_amp
id|idle-&gt;thread_info-&gt;pcb
suffix:semicolon
multiline_comment|/* Initialize the CPU&squot;s HWPCB to something just good enough for&n;&t;   us to get started.  Immediately after starting, we&squot;ll swpctx&n;&t;   to the target idle task&squot;s pcb.  Reuse the stack in the mean&n;&t;   time.  Precalculate the target PCBB.  */
id|hwpcb-&gt;ksp
op_assign
(paren
r_int
...
suffix:semicolon
multiline_comment|/* Wait 10 seconds for an ACK from the console.  */
id|timeout
op_assign
id|jiffies
op_plus
...
suffix:semicolon
multiline_comment|/* Signal the secondary to wait a moment.  */
id|smp_secondary_alive
op_assign
op_minus
l_int|1
...
l_int|0
suffix:semicolon
multiline_comment|/* We&squot;ve been acked by the console; wait one second for&n;&t;   the task to start up for real.  */
id|timeout
op_assign
id|jiffies
op_plus
...
multiline_comment|/* We failed to boot the CPU.  */
id|printk
c_func
(paren
id|KERN_ERR
...
multiline_comment|/*&n; * Called by smp_init prepare the secondaries&n; */
r_void
id|__init
...
id|EBUSY
suffix:semicolon
multiline_comment|/* Send a message to the requested CPUs.  */
id|send_ipi_message
c_func
(paren
id|to_whom
...
suffix:semicolon
multiline_comment|/* If there&squot;s no response yet, log a message but allow a longer&n;&t; * timeout period -- if we get a response this time, log&n;&t; * a message saying when we got it.. &n;&t; */
r_if
c_cond
(paren
id|atomic_read
...
(paren
id|KERN_ERR
l_string|&quot;%s: initial timeout -- trying long wait&bslash;n&quot;
comma
id|__FUNCTION__
)paren
suffix:semicolon
...
(paren
id|KERN_ERR
l_string|&quot;%s: response %ld.%ld seconds into long wait&bslash;n&quot;
comma
id|__FUNCTION__
comma
id|delta
