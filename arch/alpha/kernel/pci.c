...
suffix:semicolon
multiline_comment|/* Just declaring that the power-of-ten prefixes are actually the&n;   power-of-two ones doesn&squot;t make it true :) */
DECL|macro|KB
...
multiline_comment|/* Create an __iomem token from a PCI BAR.  Copied from lib/iomap.c with&n;   no changes, since we don&squot;t want the other things in that object file.  */
...
(brace
multiline_comment|/* Not checking IORESOURCE_CACHEABLE because alpha does&n;&t;&t;   not distinguish between ioremap and ioremap_nocache.  */
r_return
id|ioremap
c_func
(paren
...
multiline_comment|/* Destroy that token.  Not copied from lib/iomap.c.  */
