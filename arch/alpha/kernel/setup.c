...
id|srm_hae
suffix:semicolon
DECL|variable|alpha_l1i_cacheshape
r_int
id|alpha_l1i_cacheshape
suffix:semicolon
DECL|variable|alpha_l1d_cacheshape
r_int
id|alpha_l1d_cacheshape
suffix:semicolon
DECL|variable|alpha_l2_cacheshape
r_int
id|alpha_l2_cacheshape
suffix:semicolon
DECL|variable|alpha_l3_cacheshape
r_int
...
l_int|0
suffix:semicolon
multiline_comment|/* Set AGP GART window size (0 means disabled). */
DECL|variable|alpha_agpgart_size
r_int
r_int
id|alpha_agpgart_size
op_assign
id|DEFAULT_AGP_APER_SIZE
suffix:semicolon
...
c_func
(paren
l_string|&quot;initrd moved to %p&bslash;n&quot;
comma
id|start
)paren
suffix:semicolon
...
suffix:semicolon
multiline_comment|/*&n;&t;&t; * If &quot;console=srm&quot; was specified, clear the srmcons_output&n;&t;&t; * flag now so that time.c won&squot;t unregister_srm_console&n;&t;&t; */
r_if
c_cond
(paren
id|srmcons_output
...
suffix:semicolon
multiline_comment|/* First guess at cpu cache sizes.  Do this before init_arch.  */
id|determine_cpu_caches
c_func
(paren
id|cpu-&gt;type
...
id|f
comma
l_string|&quot;%s&bslash;t&bslash;t: n/a&bslash;n&quot;
comma
id|which
)paren
suffix:semicolon
...
id|f
comma
l_string|&quot;%s&bslash;t&bslash;t: unknown&bslash;n&quot;
comma
id|which
)paren
suffix:semicolon
...
id|f
comma
l_string|&quot;%s&bslash;t&bslash;t: %dK, %d-way, %db line&bslash;n&quot;
comma
id|which
comma
id|shape
...
l_string|&quot;1:&t;ldl&t;%3,0(%2)&bslash;n&quot;
l_string|&quot;&t;subq&t;%1,1,%1&bslash;n&quot;
multiline_comment|/* Next two XORs introduce an explicit data dependency between&n;&t;   consecutive loads in the loop, which will give us true load&n;&t;   latency. */
l_string|&quot;&t;xor&t;%3,%2,%2&bslash;n&quot;
l_string|&quot;&t;xor&t;%3,%2,%2&bslash;n&quot;
l_string|&quot;&t;addq&t;%2,%4,%2&bslash;n&quot;
l_string|&quot;&t;bne&t;%1,1b&bslash;n&quot;
l_string|&quot;&t;rpcc&t;%3&bslash;n&quot;
l_string|&quot;&t;subl&t;%3,%0,%0&bslash;n&quot;
suffix:colon
l_string|&quot;=&amp;r&quot;
(paren
id|cnt
...
DECL|macro|CSHAPE
mdefine_line|#define CSHAPE(totalsize, linesize, assoc) &bslash;&n;  ((totalsize &amp; ~0xff) | (linesize &lt;&lt; 4) | assoc)
multiline_comment|/* ??? EV5 supports up to 64M, but did the systems with more than&n;   16M of BCACHE ever exist? */
DECL|macro|MAX_BCACHE_SIZE
mdefine_line|#define MAX_BCACHE_SIZE&t;16*1024*1024
multiline_comment|/* Note that the offchip caches are direct mapped on all Alphas. */
r_static
r_int
id|__init
DECL|function|external_cache_probe
id|external_cache_probe
c_func
(paren
r_int
id|minsize
...
suffix:semicolon
multiline_comment|/* Get the first block cached. */
id|read_mem_block
c_func
(paren
id|__va
...
(brace
multiline_comment|/* Get an average load latency in cycles. */
id|cycles
op_assign
id|read_mem_block
c_func
...
(brace
multiline_comment|/* Fine, we exceed the cache. */
id|printk
c_func
(paren
l_string|&quot;%ldK Bcache detected; load hit latency %d &quot;
l_string|&quot;cycles, load miss latency %d cycles&bslash;n&quot;
comma
id|size
op_rshift
l_int|11
...
multiline_comment|/* Try to get the next block cached. */
id|read_mem_block
c_func
(paren
id|__va
...
l_int|1
suffix:semicolon
multiline_comment|/* No BCACHE found. */
)brace
r_static
r_void
id|__init
DECL|function|determine_cpu_caches
id|determine_cpu_caches
(paren
r_int
r_int
...
l_int|1
suffix:semicolon
multiline_comment|/* BIU_CTL is a write-only Abox register.  PALcode has a&n;&t;&t;   shadow copy, and may be available from some versions&n;&t;&t;   of the CSERVE PALcall.  If we can get it, then&n;&n;&t;&t;&t;unsigned long biu_ctl, size;&n;&t;&t;&t;size = 128*1024 * (1 &lt;&lt; ((biu_ctl &gt;&gt; 28) &amp; 7));&n;&t;&t;&t;L2 = CSHAPE (size, 5, 1);&n;&n;&t;&t;   Unfortunately, we can&squot;t rely on that.&n;&t;&t;*/
id|L2
op_assign
id|external_cache_probe
c_func
...
suffix:semicolon
multiline_comment|/* No typo -- 8 byte cacheline size.  Whodathunk.  */
id|L2
op_assign
(paren
id|car
...
suffix:semicolon
multiline_comment|/* Check the line size of the Scache.  */
id|sc_ctl
op_assign
op_star
(paren
...
suffix:semicolon
multiline_comment|/* BC_CONTROL and BC_CONFIG are write-only IPRs.  PALcode&n;&t;&t;   has a shadow copy, and may be available from some versions&n;&t;&t;   of the CSERVE PALcall.  If we can get it, then&n;&n;&t;&t;&t;unsigned long bc_control, bc_config, size;&n;&t;&t;&t;size = 1024*1024 * (1 &lt;&lt; ((bc_config &amp; 7) - 1));&n;&t;&t;&t;L3 = (bc_control &amp; 1 ? CSHAPE (size, width, 1) : -1);&n;&n;&t;&t;   Unfortunately, we can&squot;t rely on that.&n;&t;&t;*/
id|L3
op_assign
id|external_cache_probe
c_func
...
r_default
suffix:colon
multiline_comment|/* Nothing known about this cpu type.  */
id|L1I
op_assign
id|L1D
op_assign
...
multiline_comment|/*&n; * We show only CPU #0 info.&n; */
r_static
r_void
op_star
