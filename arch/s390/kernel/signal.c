...
suffix:semicolon
multiline_comment|/* Copy a &squot;clean&squot; PSW mask to the user to avoid leaking&n;&t;   information about whether PER is currently on.  */
id|regs-&gt;psw.mask
op_assign
id|PSW_MASK_MERGE
c_func
...
suffix:semicolon
multiline_comment|/* Prepare for system call restart.  We do this here so that a&n;&t;&t;   debugger will see the already changed PSW. */
r_if
c_cond
(paren
id|retval
...
multiline_comment|/* Get signal to deliver.  When running under ptrace, at this point&n;&t;   the debugger may change all our registers ... */
id|signr
op_assign
...
suffix:semicolon
multiline_comment|/* Depending on the signal settings we may need to revert the&n;&t;   decision to restart the system call. */
r_if
c_cond
(paren
id|signr
...
multiline_comment|/* Restart a different system call. */
r_if
c_cond
(paren
id|retval
