multiline_comment|/*&n; * linux/arch/sh/kernel/io.c&n; *&n; * Copyright (C) 2000  Stuart Menefy&n; *&n; * Provide real functions which expand to whatever the header file defined.&n; * Also definitions of machine independent IO functions.&n; */
macro_line|#include &lt;asm/io.h&gt;
macro_line|#include &lt;linux/module.h&gt;
DECL|function|_inb
r_int
r_char
id|_inb
c_func
(paren
r_int
r_int
id|port
)paren
(brace
r_return
id|__inb
c_func
(paren
id|port
)paren
suffix:semicolon
)brace
DECL|variable|_inb
id|EXPORT_SYMBOL
c_func
(paren
id|_inb
)paren
suffix:semicolon
DECL|function|_inw
r_int
r_int
id|_inw
c_func
(paren
r_int
r_int
id|port
)paren
(brace
r_return
id|__inw
c_func
(paren
id|port
)paren
suffix:semicolon
)brace
DECL|variable|_inw
id|EXPORT_SYMBOL
c_func
(paren
id|_inw
)paren
suffix:semicolon
DECL|function|_inl
r_int
r_int
id|_inl
c_func
(paren
r_int
r_int
id|port
)paren
(brace
r_return
id|__inl
c_func
(paren
id|port
)paren
suffix:semicolon
)brace
DECL|variable|_inl
id|EXPORT_SYMBOL
c_func
(paren
id|_inl
)paren
suffix:semicolon
DECL|function|_outb
r_void
id|_outb
c_func
(paren
r_int
r_char
id|b
comma
r_int
r_int
id|port
)paren
(brace
id|__outb
c_func
(paren
id|b
comma
id|port
)paren
suffix:semicolon
)brace
DECL|variable|_outb
id|EXPORT_SYMBOL
c_func
(paren
id|_outb
)paren
suffix:semicolon
DECL|function|_outw
r_void
id|_outw
c_func
(paren
r_int
r_int
id|b
comma
r_int
r_int
id|port
)paren
(brace
id|__outw
c_func
(paren
id|b
comma
id|port
)paren
suffix:semicolon
)brace
DECL|variable|_outw
id|EXPORT_SYMBOL
c_func
(paren
id|_outw
)paren
suffix:semicolon
DECL|function|_outl
r_void
id|_outl
c_func
(paren
r_int
r_int
id|b
comma
r_int
r_int
id|port
)paren
(brace
id|__outl
c_func
(paren
id|b
comma
id|port
)paren
suffix:semicolon
)brace
DECL|variable|_outl
id|EXPORT_SYMBOL
c_func
(paren
id|_outl
)paren
suffix:semicolon
DECL|function|_inb_p
r_int
r_char
id|_inb_p
c_func
(paren
r_int
r_int
id|port
)paren
(brace
r_return
id|__inb_p
c_func
(paren
id|port
)paren
suffix:semicolon
)brace
DECL|variable|_inb_p
id|EXPORT_SYMBOL
c_func
(paren
id|_inb_p
)paren
suffix:semicolon
DECL|function|_inw_p
r_int
r_int
id|_inw_p
c_func
(paren
r_int
r_int
id|port
)paren
(brace
r_return
id|__inw_p
c_func
(paren
id|port
)paren
suffix:semicolon
)brace
DECL|variable|_inw_p
id|EXPORT_SYMBOL
c_func
(paren
id|_inw_p
)paren
suffix:semicolon
DECL|function|_outb_p
r_void
id|_outb_p
c_func
(paren
r_int
r_char
id|b
comma
r_int
r_int
id|port
)paren
(brace
id|__outb_p
c_func
(paren
id|b
comma
id|port
)paren
suffix:semicolon
)brace
DECL|variable|_outb_p
id|EXPORT_SYMBOL
c_func
(paren
id|_outb_p
)paren
suffix:semicolon
DECL|function|_outw_p
r_void
id|_outw_p
c_func
(paren
r_int
r_int
id|b
comma
r_int
r_int
id|port
)paren
(brace
id|__outw_p
c_func
(paren
id|b
comma
id|port
)paren
suffix:semicolon
)brace
DECL|variable|_outw_p
id|EXPORT_SYMBOL
c_func
(paren
id|_outw_p
)paren
suffix:semicolon
DECL|function|_insb
r_void
id|_insb
c_func
(paren
r_int
r_int
id|port
comma
r_void
op_star
id|buffer
comma
r_int
r_int
id|count
)paren
(brace
r_return
id|__insb
c_func
(paren
id|port
comma
id|buffer
comma
id|count
)paren
suffix:semicolon
)brace
DECL|variable|_insb
id|EXPORT_SYMBOL
c_func
(paren
id|_insb
)paren
suffix:semicolon
DECL|function|_insw
r_void
id|_insw
c_func
(paren
r_int
r_int
id|port
comma
r_void
op_star
id|buffer
comma
r_int
r_int
id|count
)paren
(brace
id|__insw
c_func
(paren
id|port
comma
id|buffer
comma
id|count
)paren
suffix:semicolon
)brace
DECL|variable|_insw
id|EXPORT_SYMBOL
c_func
(paren
id|_insw
)paren
suffix:semicolon
DECL|function|_insl
r_void
id|_insl
c_func
(paren
r_int
r_int
id|port
comma
r_void
op_star
id|buffer
comma
r_int
r_int
id|count
)paren
(brace
id|__insl
c_func
(paren
id|port
comma
id|buffer
comma
id|count
)paren
suffix:semicolon
)brace
DECL|variable|_insl
id|EXPORT_SYMBOL
c_func
(paren
id|_insl
)paren
suffix:semicolon
DECL|function|_outsb
r_void
id|_outsb
c_func
(paren
r_int
r_int
id|port
comma
r_const
r_void
op_star
id|buffer
comma
r_int
r_int
id|count
)paren
(brace
id|__outsb
c_func
(paren
id|port
comma
id|buffer
comma
id|count
)paren
suffix:semicolon
)brace
DECL|variable|_outsb
id|EXPORT_SYMBOL
c_func
(paren
id|_outsb
)paren
suffix:semicolon
DECL|function|_outsw
r_void
id|_outsw
c_func
(paren
r_int
r_int
id|port
comma
r_const
r_void
op_star
id|buffer
comma
r_int
r_int
id|count
)paren
(brace
id|__outsw
c_func
(paren
id|port
comma
id|buffer
comma
id|count
)paren
suffix:semicolon
)brace
DECL|variable|_outsw
id|EXPORT_SYMBOL
c_func
(paren
id|_outsw
)paren
suffix:semicolon
DECL|function|_outsl
r_void
id|_outsl
c_func
(paren
r_int
r_int
id|port
comma
r_const
r_void
op_star
id|buffer
comma
r_int
r_int
id|count
)paren
(brace
id|__outsl
c_func
(paren
id|port
comma
id|buffer
comma
id|count
)paren
suffix:semicolon
)brace
DECL|variable|_outsl
id|EXPORT_SYMBOL
c_func
(paren
id|_outsl
)paren
suffix:semicolon
DECL|function|___raw_readb
r_int
r_char
id|___raw_readb
c_func
(paren
r_int
r_int
id|addr
)paren
(brace
r_return
id|__readb
c_func
(paren
id|addr
)paren
suffix:semicolon
)brace
DECL|variable|___raw_readb
id|EXPORT_SYMBOL
c_func
(paren
id|___raw_readb
)paren
suffix:semicolon
DECL|function|___raw_readw
r_int
r_int
id|___raw_readw
c_func
(paren
r_int
r_int
id|addr
)paren
(brace
r_return
id|__readw
c_func
(paren
id|addr
)paren
suffix:semicolon
)brace
DECL|variable|___raw_readw
id|EXPORT_SYMBOL
c_func
(paren
id|___raw_readw
)paren
suffix:semicolon
DECL|function|___raw_readl
r_int
r_int
id|___raw_readl
c_func
(paren
r_int
r_int
id|addr
)paren
(brace
r_return
id|__readl
c_func
(paren
id|addr
)paren
suffix:semicolon
)brace
DECL|variable|___raw_readl
id|EXPORT_SYMBOL
c_func
(paren
id|___raw_readl
)paren
suffix:semicolon
DECL|function|_readb
r_int
r_char
id|_readb
c_func
(paren
r_int
r_int
id|addr
)paren
(brace
r_int
r_int
id|r
op_assign
id|__readb
c_func
(paren
id|addr
)paren
suffix:semicolon
id|mb
c_func
(paren
)paren
suffix:semicolon
r_return
id|r
suffix:semicolon
)brace
DECL|variable|_readb
id|EXPORT_SYMBOL
c_func
(paren
id|_readb
)paren
suffix:semicolon
DECL|function|_readw
r_int
r_int
id|_readw
c_func
(paren
r_int
r_int
id|addr
)paren
(brace
r_int
r_int
id|r
op_assign
id|__readw
c_func
(paren
id|addr
)paren
suffix:semicolon
id|mb
c_func
(paren
)paren
suffix:semicolon
r_return
id|r
suffix:semicolon
)brace
DECL|variable|_readw
id|EXPORT_SYMBOL
c_func
(paren
id|_readw
)paren
suffix:semicolon
DECL|function|_readl
r_int
r_int
id|_readl
c_func
(paren
r_int
r_int
id|addr
)paren
(brace
r_int
r_int
id|r
op_assign
id|__readl
c_func
(paren
id|addr
)paren
suffix:semicolon
id|mb
c_func
(paren
)paren
suffix:semicolon
r_return
id|r
suffix:semicolon
)brace
DECL|variable|_readl
id|EXPORT_SYMBOL
c_func
(paren
id|_readl
)paren
suffix:semicolon
DECL|function|___raw_writeb
r_void
id|___raw_writeb
c_func
(paren
r_int
r_char
id|b
comma
r_int
r_int
id|addr
)paren
(brace
id|__writeb
c_func
(paren
id|b
comma
id|addr
)paren
suffix:semicolon
)brace
DECL|function|___raw_writew
r_void
id|___raw_writew
c_func
(paren
r_int
r_int
id|b
comma
r_int
r_int
id|addr
)paren
(brace
id|__writew
c_func
(paren
id|b
comma
id|addr
)paren
suffix:semicolon
)brace
DECL|variable|___raw_writew
id|EXPORT_SYMBOL
c_func
(paren
id|___raw_writew
)paren
suffix:semicolon
DECL|function|___raw_writel
r_void
id|___raw_writel
c_func
(paren
r_int
r_int
id|b
comma
r_int
r_int
id|addr
)paren
(brace
id|__writel
c_func
(paren
id|b
comma
id|addr
)paren
suffix:semicolon
)brace
DECL|variable|___raw_writel
id|EXPORT_SYMBOL
c_func
(paren
id|___raw_writel
)paren
suffix:semicolon
DECL|function|_writeb
r_void
id|_writeb
c_func
(paren
r_int
r_char
id|b
comma
r_int
r_int
id|addr
)paren
(brace
id|__writeb
c_func
(paren
id|b
comma
id|addr
)paren
suffix:semicolon
id|mb
c_func
(paren
)paren
suffix:semicolon
)brace
DECL|variable|_writeb
id|EXPORT_SYMBOL
c_func
(paren
id|_writeb
)paren
suffix:semicolon
DECL|function|_writew
r_void
id|_writew
c_func
(paren
r_int
r_int
id|b
comma
r_int
r_int
id|addr
)paren
(brace
id|__writew
c_func
(paren
id|b
comma
id|addr
)paren
suffix:semicolon
id|mb
c_func
(paren
)paren
suffix:semicolon
)brace
DECL|variable|_writew
id|EXPORT_SYMBOL
c_func
(paren
id|_writew
)paren
suffix:semicolon
DECL|function|_writel
r_void
id|_writel
c_func
(paren
r_int
r_int
id|b
comma
r_int
r_int
id|addr
)paren
(brace
id|__writel
c_func
(paren
id|b
comma
id|addr
)paren
suffix:semicolon
id|mb
c_func
(paren
)paren
suffix:semicolon
)brace
DECL|variable|_writel
id|EXPORT_SYMBOL
c_func
(paren
id|_writel
)paren
suffix:semicolon
multiline_comment|/*&n; * Copy data from IO memory space to &quot;real&quot; memory space.&n; * This needs to be optimized.&n; */
DECL|function|memcpy_fromio
r_void
id|memcpy_fromio
c_func
(paren
r_void
op_star
id|to
comma
r_int
r_int
id|from
comma
r_int
r_int
id|count
)paren
(brace
r_while
c_loop
(paren
id|count
)paren
(brace
id|count
op_decrement
suffix:semicolon
op_star
(paren
r_char
op_star
)paren
id|to
op_assign
id|readb
c_func
(paren
id|from
)paren
suffix:semicolon
(paren
(paren
r_char
op_star
)paren
id|to
)paren
op_increment
suffix:semicolon
id|from
op_increment
suffix:semicolon
)brace
)brace
multiline_comment|/*&n; * Copy data from &quot;real&quot; memory space to IO memory space.&n; * This needs to be optimized.&n; */
DECL|function|memcpy_toio
r_void
id|memcpy_toio
c_func
(paren
r_int
r_int
id|to
comma
r_const
r_void
op_star
id|from
comma
r_int
r_int
id|count
)paren
(brace
r_while
c_loop
(paren
id|count
)paren
(brace
id|count
op_decrement
suffix:semicolon
id|writeb
c_func
(paren
op_star
(paren
r_char
op_star
)paren
id|from
comma
id|to
)paren
suffix:semicolon
(paren
(paren
r_char
op_star
)paren
id|from
)paren
op_increment
suffix:semicolon
id|to
op_increment
suffix:semicolon
)brace
)brace
multiline_comment|/*&n; * &quot;memset&quot; on IO memory space.&n; * This needs to be optimized.&n; */
DECL|function|memset_io
r_void
id|memset_io
c_func
(paren
r_int
r_int
id|dst
comma
r_int
id|c
comma
r_int
r_int
id|count
)paren
(brace
r_while
c_loop
(paren
id|count
)paren
(brace
id|count
op_decrement
suffix:semicolon
id|writeb
c_func
(paren
id|c
comma
id|dst
)paren
suffix:semicolon
id|dst
op_increment
suffix:semicolon
)brace
)brace
eof
