multiline_comment|/* $Id: irq_ipr.c,v 1.6 2000/05/14 08:41:25 gniibe Exp $&n; *&n; * linux/arch/sh/kernel/irq_ipr.c&n; *&n; * Copyright (C) 1999  Niibe Yutaka &amp; Takeshi Yaegashi&n; * Copyright (C) 2000  Kazumoto Kojima&n; *&n; * Interrupt handling for IPR-based IRQ.&n; *&n; * Supported system:&n; *&t;On-chip supporting modules (TMU, RTC, etc.).&n; *&t;On-chip supporting modules for SH7709/SH7709A/SH7729.&n; *&t;Hitachi SolutionEngine external I/O:&n; *&t;&t;MS7709SE01, MS7709ASE01, and MS7750SE01&n; *&n; */
macro_line|#include &lt;linux/config.h&gt;
macro_line|#include &lt;linux/init.h&gt;
macro_line|#include &lt;linux/irq.h&gt;
macro_line|#include &lt;asm/system.h&gt;
macro_line|#include &lt;asm/io.h&gt;
macro_line|#include &lt;asm/machvec.h&gt;
DECL|struct|ipr_data
r_struct
id|ipr_data
(brace
DECL|member|addr
r_int
r_int
id|addr
suffix:semicolon
multiline_comment|/* Address of Interrupt Priority Register */
DECL|member|shift
r_int
id|shift
suffix:semicolon
multiline_comment|/* Shifts of the 16-bit data */
DECL|member|priority
r_int
id|priority
suffix:semicolon
multiline_comment|/* The priority */
)brace
suffix:semicolon
DECL|variable|ipr_data
r_static
r_struct
id|ipr_data
id|ipr_data
(braket
id|NR_IRQS
)braket
suffix:semicolon
r_static
r_void
id|enable_ipr_irq
c_func
(paren
r_int
r_int
id|irq
)paren
suffix:semicolon
r_static
r_void
id|disable_ipr_irq
c_func
(paren
r_int
r_int
id|irq
)paren
suffix:semicolon
multiline_comment|/* shutdown is same as &quot;disable&quot; */
DECL|macro|shutdown_ipr_irq
mdefine_line|#define shutdown_ipr_irq disable_ipr_irq
r_static
r_void
id|mask_and_ack_ipr
c_func
(paren
r_int
r_int
)paren
suffix:semicolon
r_static
r_void
id|end_ipr_irq
c_func
(paren
r_int
r_int
id|irq
)paren
suffix:semicolon
DECL|function|startup_ipr_irq
r_static
r_int
r_int
id|startup_ipr_irq
c_func
(paren
r_int
r_int
id|irq
)paren
(brace
id|enable_ipr_irq
c_func
(paren
id|irq
)paren
suffix:semicolon
r_return
l_int|0
suffix:semicolon
multiline_comment|/* never anything pending */
)brace
DECL|variable|ipr_irq_type
r_static
r_struct
id|hw_interrupt_type
id|ipr_irq_type
op_assign
(brace
l_string|&quot;IPR-based-IRQ&quot;
comma
id|startup_ipr_irq
comma
id|shutdown_ipr_irq
comma
id|enable_ipr_irq
comma
id|disable_ipr_irq
comma
id|mask_and_ack_ipr
comma
id|end_ipr_irq
)brace
suffix:semicolon
DECL|function|disable_ipr_irq
r_static
r_void
id|disable_ipr_irq
c_func
(paren
r_int
r_int
id|irq
)paren
(brace
r_int
r_int
id|val
comma
id|flags
suffix:semicolon
r_int
r_int
id|addr
op_assign
id|ipr_data
(braket
id|irq
)braket
dot
id|addr
suffix:semicolon
r_int
r_int
id|mask
op_assign
l_int|0xffff
op_xor
(paren
l_int|0x0f
op_lshift
id|ipr_data
(braket
id|irq
)braket
dot
id|shift
)paren
suffix:semicolon
multiline_comment|/* Set the priority in IPR to 0 */
id|save_and_cli
c_func
(paren
id|flags
)paren
suffix:semicolon
id|val
op_assign
id|ctrl_inw
c_func
(paren
id|addr
)paren
suffix:semicolon
id|val
op_and_assign
id|mask
suffix:semicolon
id|ctrl_outw
c_func
(paren
id|val
comma
id|addr
)paren
suffix:semicolon
id|restore_flags
c_func
(paren
id|flags
)paren
suffix:semicolon
)brace
DECL|function|enable_ipr_irq
r_static
r_void
id|enable_ipr_irq
c_func
(paren
r_int
r_int
id|irq
)paren
(brace
r_int
r_int
id|val
comma
id|flags
suffix:semicolon
r_int
r_int
id|addr
op_assign
id|ipr_data
(braket
id|irq
)braket
dot
id|addr
suffix:semicolon
r_int
id|priority
op_assign
id|ipr_data
(braket
id|irq
)braket
dot
id|priority
suffix:semicolon
r_int
r_int
id|value
op_assign
(paren
id|priority
op_lshift
id|ipr_data
(braket
id|irq
)braket
dot
id|shift
)paren
suffix:semicolon
multiline_comment|/* Set priority in IPR back to original value */
id|save_and_cli
c_func
(paren
id|flags
)paren
suffix:semicolon
id|val
op_assign
id|ctrl_inw
c_func
(paren
id|addr
)paren
suffix:semicolon
id|val
op_or_assign
id|value
suffix:semicolon
id|ctrl_outw
c_func
(paren
id|val
comma
id|addr
)paren
suffix:semicolon
id|restore_flags
c_func
(paren
id|flags
)paren
suffix:semicolon
)brace
DECL|function|mask_and_ack_ipr
r_static
r_void
id|mask_and_ack_ipr
c_func
(paren
r_int
r_int
id|irq
)paren
(brace
id|disable_ipr_irq
c_func
(paren
id|irq
)paren
suffix:semicolon
macro_line|#if defined(CONFIG_CPU_SUBTYPE_SH7707) || defined(CONFIG_CPU_SUBTYPE_SH7709)
multiline_comment|/* This is needed when we use edge triggered setting */
multiline_comment|/* XXX: Is it really needed? */
r_if
c_cond
(paren
id|IRQ0_IRQ
op_le
id|irq
op_logical_and
id|irq
op_le
id|IRQ5_IRQ
)paren
(brace
multiline_comment|/* Clear external interrupt request */
r_int
id|a
op_assign
id|ctrl_inb
c_func
(paren
id|INTC_IRR0
)paren
suffix:semicolon
id|a
op_and_assign
op_complement
(paren
l_int|1
op_lshift
(paren
id|irq
op_minus
id|IRQ0_IRQ
)paren
)paren
suffix:semicolon
id|ctrl_outb
c_func
(paren
id|a
comma
id|INTC_IRR0
)paren
suffix:semicolon
)brace
macro_line|#endif
)brace
DECL|function|end_ipr_irq
r_static
r_void
id|end_ipr_irq
c_func
(paren
r_int
r_int
id|irq
)paren
(brace
id|enable_ipr_irq
c_func
(paren
id|irq
)paren
suffix:semicolon
)brace
DECL|function|make_ipr_irq
r_void
id|make_ipr_irq
c_func
(paren
r_int
r_int
id|irq
comma
r_int
r_int
id|addr
comma
r_int
id|pos
comma
r_int
id|priority
)paren
(brace
id|disable_irq_nosync
c_func
(paren
id|irq
)paren
suffix:semicolon
id|ipr_data
(braket
id|irq
)braket
dot
id|addr
op_assign
id|addr
suffix:semicolon
id|ipr_data
(braket
id|irq
)braket
dot
id|shift
op_assign
id|pos
op_star
l_int|4
suffix:semicolon
multiline_comment|/* POSition (0-3) x 4 means shift */
id|ipr_data
(braket
id|irq
)braket
dot
id|priority
op_assign
id|priority
suffix:semicolon
id|irq_desc
(braket
id|irq
)braket
dot
id|handler
op_assign
op_amp
id|ipr_irq_type
suffix:semicolon
id|disable_ipr_irq
c_func
(paren
id|irq
)paren
suffix:semicolon
)brace
macro_line|#if defined(CONFIG_CPU_SUBTYPE_SH7707) || defined(CONFIG_CPU_SUBTYPE_SH7709)
DECL|variable|pint_map
r_static
r_int
r_char
id|pint_map
(braket
l_int|256
)braket
suffix:semicolon
DECL|variable|portcr_mask
r_static
r_int
r_int
id|portcr_mask
op_assign
l_int|0
suffix:semicolon
r_static
r_void
id|enable_pint_irq
c_func
(paren
r_int
r_int
id|irq
)paren
suffix:semicolon
r_static
r_void
id|disable_pint_irq
c_func
(paren
r_int
r_int
id|irq
)paren
suffix:semicolon
multiline_comment|/* shutdown is same as &quot;disable&quot; */
DECL|macro|shutdown_pint_irq
mdefine_line|#define shutdown_pint_irq disable_pint_irq
r_static
r_void
id|mask_and_ack_pint
c_func
(paren
r_int
r_int
)paren
suffix:semicolon
r_static
r_void
id|end_pint_irq
c_func
(paren
r_int
r_int
id|irq
)paren
suffix:semicolon
DECL|function|startup_pint_irq
r_static
r_int
r_int
id|startup_pint_irq
c_func
(paren
r_int
r_int
id|irq
)paren
(brace
id|enable_pint_irq
c_func
(paren
id|irq
)paren
suffix:semicolon
r_return
l_int|0
suffix:semicolon
multiline_comment|/* never anything pending */
)brace
DECL|variable|pint_irq_type
r_static
r_struct
id|hw_interrupt_type
id|pint_irq_type
op_assign
(brace
l_string|&quot;PINT-IRQ&quot;
comma
id|startup_pint_irq
comma
id|shutdown_pint_irq
comma
id|enable_pint_irq
comma
id|disable_pint_irq
comma
id|mask_and_ack_pint
comma
id|end_pint_irq
)brace
suffix:semicolon
DECL|function|disable_pint_irq
r_static
r_void
id|disable_pint_irq
c_func
(paren
r_int
r_int
id|irq
)paren
(brace
r_int
r_int
id|val
comma
id|flags
suffix:semicolon
id|save_and_cli
c_func
(paren
id|flags
)paren
suffix:semicolon
id|val
op_assign
id|ctrl_inw
c_func
(paren
id|INTC_INTER
)paren
suffix:semicolon
id|val
op_and_assign
op_complement
(paren
l_int|1
op_lshift
(paren
id|irq
op_minus
id|PINT_IRQ_BASE
)paren
)paren
suffix:semicolon
id|ctrl_outw
c_func
(paren
id|val
comma
id|INTC_INTER
)paren
suffix:semicolon
multiline_comment|/* disable PINTn */
id|portcr_mask
op_and_assign
op_complement
(paren
l_int|3
op_lshift
(paren
id|irq
op_minus
id|PINT_IRQ_BASE
)paren
op_star
l_int|2
)paren
suffix:semicolon
id|restore_flags
c_func
(paren
id|flags
)paren
suffix:semicolon
)brace
DECL|function|enable_pint_irq
r_static
r_void
id|enable_pint_irq
c_func
(paren
r_int
r_int
id|irq
)paren
(brace
r_int
r_int
id|val
comma
id|flags
suffix:semicolon
id|save_and_cli
c_func
(paren
id|flags
)paren
suffix:semicolon
id|val
op_assign
id|ctrl_inw
c_func
(paren
id|INTC_INTER
)paren
suffix:semicolon
id|val
op_or_assign
l_int|1
op_lshift
(paren
id|irq
op_minus
id|PINT_IRQ_BASE
)paren
suffix:semicolon
id|ctrl_outw
c_func
(paren
id|val
comma
id|INTC_INTER
)paren
suffix:semicolon
multiline_comment|/* enable PINTn */
id|portcr_mask
op_or_assign
l_int|3
op_lshift
(paren
id|irq
op_minus
id|PINT_IRQ_BASE
)paren
op_star
l_int|2
suffix:semicolon
id|restore_flags
c_func
(paren
id|flags
)paren
suffix:semicolon
)brace
DECL|function|mask_and_ack_pint
r_static
r_void
id|mask_and_ack_pint
c_func
(paren
r_int
r_int
id|irq
)paren
(brace
id|disable_pint_irq
c_func
(paren
id|irq
)paren
suffix:semicolon
)brace
DECL|function|end_pint_irq
r_static
r_void
id|end_pint_irq
c_func
(paren
r_int
r_int
id|irq
)paren
(brace
id|enable_pint_irq
c_func
(paren
id|irq
)paren
suffix:semicolon
)brace
DECL|function|make_pint_irq
r_void
id|make_pint_irq
c_func
(paren
r_int
r_int
id|irq
)paren
(brace
id|disable_irq_nosync
c_func
(paren
id|irq
)paren
suffix:semicolon
id|irq_desc
(braket
id|irq
)braket
dot
id|handler
op_assign
op_amp
id|pint_irq_type
suffix:semicolon
id|disable_pint_irq
c_func
(paren
id|irq
)paren
suffix:semicolon
)brace
macro_line|#endif
DECL|function|init_IRQ
r_void
id|__init
id|init_IRQ
c_func
(paren
r_void
)paren
(brace
r_int
id|i
suffix:semicolon
id|make_ipr_irq
c_func
(paren
id|TIMER_IRQ
comma
id|TIMER_IPR_ADDR
comma
id|TIMER_IPR_POS
comma
id|TIMER_PRIORITY
)paren
suffix:semicolon
id|make_ipr_irq
c_func
(paren
id|RTC_IRQ
comma
id|RTC_IPR_ADDR
comma
id|RTC_IPR_POS
comma
id|RTC_PRIORITY
)paren
suffix:semicolon
macro_line|#ifdef SCI_ERI_IRQ
id|make_ipr_irq
c_func
(paren
id|SCI_ERI_IRQ
comma
id|SCI_IPR_ADDR
comma
id|SCI_IPR_POS
comma
id|SCI_PRIORITY
)paren
suffix:semicolon
id|make_ipr_irq
c_func
(paren
id|SCI_RXI_IRQ
comma
id|SCI_IPR_ADDR
comma
id|SCI_IPR_POS
comma
id|SCI_PRIORITY
)paren
suffix:semicolon
id|make_ipr_irq
c_func
(paren
id|SCI_TXI_IRQ
comma
id|SCI_IPR_ADDR
comma
id|SCI_IPR_POS
comma
id|SCI_PRIORITY
)paren
suffix:semicolon
macro_line|#endif
macro_line|#ifdef SCIF1_ERI_IRQ
id|make_ipr_irq
c_func
(paren
id|SCIF1_ERI_IRQ
comma
id|SCIF1_IPR_ADDR
comma
id|SCIF1_IPR_POS
comma
id|SCIF1_PRIORITY
)paren
suffix:semicolon
id|make_ipr_irq
c_func
(paren
id|SCIF1_RXI_IRQ
comma
id|SCIF1_IPR_ADDR
comma
id|SCIF1_IPR_POS
comma
id|SCIF1_PRIORITY
)paren
suffix:semicolon
id|make_ipr_irq
c_func
(paren
id|SCIF1_BRI_IRQ
comma
id|SCIF1_IPR_ADDR
comma
id|SCIF1_IPR_POS
comma
id|SCIF1_PRIORITY
)paren
suffix:semicolon
id|make_ipr_irq
c_func
(paren
id|SCIF1_TXI_IRQ
comma
id|SCIF1_IPR_ADDR
comma
id|SCIF1_IPR_POS
comma
id|SCIF1_PRIORITY
)paren
suffix:semicolon
macro_line|#endif
macro_line|#ifdef SCIF_ERI_IRQ
id|make_ipr_irq
c_func
(paren
id|SCIF_ERI_IRQ
comma
id|SCIF_IPR_ADDR
comma
id|SCIF_IPR_POS
comma
id|SCIF_PRIORITY
)paren
suffix:semicolon
id|make_ipr_irq
c_func
(paren
id|SCIF_RXI_IRQ
comma
id|SCIF_IPR_ADDR
comma
id|SCIF_IPR_POS
comma
id|SCIF_PRIORITY
)paren
suffix:semicolon
id|make_ipr_irq
c_func
(paren
id|SCIF_BRI_IRQ
comma
id|SCIF_IPR_ADDR
comma
id|SCIF_IPR_POS
comma
id|SCIF_PRIORITY
)paren
suffix:semicolon
id|make_ipr_irq
c_func
(paren
id|SCIF_TXI_IRQ
comma
id|SCIF_IPR_ADDR
comma
id|SCIF_IPR_POS
comma
id|SCIF_PRIORITY
)paren
suffix:semicolon
macro_line|#endif
macro_line|#ifdef IRDA_ERI_IRQ
id|make_ipr_irq
c_func
(paren
id|IRDA_ERI_IRQ
comma
id|IRDA_IPR_ADDR
comma
id|IRDA_IPR_POS
comma
id|IRDA_PRIORITY
)paren
suffix:semicolon
id|make_ipr_irq
c_func
(paren
id|IRDA_RXI_IRQ
comma
id|IRDA_IPR_ADDR
comma
id|IRDA_IPR_POS
comma
id|IRDA_PRIORITY
)paren
suffix:semicolon
id|make_ipr_irq
c_func
(paren
id|IRDA_BRI_IRQ
comma
id|IRDA_IPR_ADDR
comma
id|IRDA_IPR_POS
comma
id|IRDA_PRIORITY
)paren
suffix:semicolon
id|make_ipr_irq
c_func
(paren
id|IRDA_TXI_IRQ
comma
id|IRDA_IPR_ADDR
comma
id|IRDA_IPR_POS
comma
id|IRDA_PRIORITY
)paren
suffix:semicolon
macro_line|#endif
macro_line|#if defined(CONFIG_CPU_SUBTYPE_SH7707) || defined(CONFIG_CPU_SUBTYPE_SH7709)
multiline_comment|/*&n;&t; * Initialize the Interrupt Controller (INTC)&n;&t; * registers to their power on values&n;&t; */
macro_line|#if 0
multiline_comment|/*&n;&t; * XXX: I think that this is the job of boot loader. -- gniibe&n;&t; *&n;&t; * When Takeshi released new boot loader following setting&n;&t; * will be removed shortly.&n;&t; */
id|ctrl_outb
c_func
(paren
l_int|0
comma
id|INTC_IRR0
)paren
suffix:semicolon
id|ctrl_outb
c_func
(paren
l_int|0
comma
id|INTC_IRR1
)paren
suffix:semicolon
id|ctrl_outb
c_func
(paren
l_int|0
comma
id|INTC_IRR2
)paren
suffix:semicolon
id|ctrl_outw
c_func
(paren
l_int|0
comma
id|INTC_ICR0
)paren
suffix:semicolon
id|ctrl_outw
c_func
(paren
l_int|0
comma
id|INTC_ICR1
)paren
suffix:semicolon
multiline_comment|/* Really? 0x4000?*/
id|ctrl_outw
c_func
(paren
l_int|0
comma
id|INTC_ICR2
)paren
suffix:semicolon
id|ctrl_outw
c_func
(paren
l_int|0
comma
id|INTC_INTER
)paren
suffix:semicolon
id|ctrl_outw
c_func
(paren
l_int|0
comma
id|INTC_IPRA
)paren
suffix:semicolon
id|ctrl_outw
c_func
(paren
l_int|0
comma
id|INTC_IPRB
)paren
suffix:semicolon
id|ctrl_outw
c_func
(paren
l_int|0
comma
id|INTC_IPRC
)paren
suffix:semicolon
id|ctrl_outw
c_func
(paren
l_int|0
comma
id|INTC_IPRD
)paren
suffix:semicolon
id|ctrl_outw
c_func
(paren
l_int|0
comma
id|INTC_IPRE
)paren
suffix:semicolon
macro_line|#endif
multiline_comment|/*&n;&t; * Enable external irq (INTC IRQ mode).&n;&t; * You should set corresponding bits of PFC to &quot;00&quot;&n;&t; * to enable these interrupts.&n;&t; */
id|make_ipr_irq
c_func
(paren
id|IRQ0_IRQ
comma
id|IRQ0_IPR_ADDR
comma
id|IRQ0_IPR_POS
comma
id|IRQ0_PRIORITY
)paren
suffix:semicolon
id|make_ipr_irq
c_func
(paren
id|IRQ1_IRQ
comma
id|IRQ1_IPR_ADDR
comma
id|IRQ1_IPR_POS
comma
id|IRQ1_PRIORITY
)paren
suffix:semicolon
id|make_ipr_irq
c_func
(paren
id|IRQ2_IRQ
comma
id|IRQ2_IPR_ADDR
comma
id|IRQ2_IPR_POS
comma
id|IRQ2_PRIORITY
)paren
suffix:semicolon
id|make_ipr_irq
c_func
(paren
id|IRQ3_IRQ
comma
id|IRQ3_IPR_ADDR
comma
id|IRQ3_IPR_POS
comma
id|IRQ3_PRIORITY
)paren
suffix:semicolon
id|make_ipr_irq
c_func
(paren
id|IRQ4_IRQ
comma
id|IRQ4_IPR_ADDR
comma
id|IRQ4_IPR_POS
comma
id|IRQ4_PRIORITY
)paren
suffix:semicolon
id|make_ipr_irq
c_func
(paren
id|IRQ5_IRQ
comma
id|IRQ5_IPR_ADDR
comma
id|IRQ5_IPR_POS
comma
id|IRQ5_PRIORITY
)paren
suffix:semicolon
id|make_ipr_irq
c_func
(paren
id|PINT0_IRQ
comma
id|PINT0_IPR_ADDR
comma
id|PINT0_IPR_POS
comma
id|PINT0_PRIORITY
)paren
suffix:semicolon
id|make_ipr_irq
c_func
(paren
id|PINT8_IRQ
comma
id|PINT8_IPR_ADDR
comma
id|PINT8_IPR_POS
comma
id|PINT8_PRIORITY
)paren
suffix:semicolon
id|enable_ipr_irq
c_func
(paren
id|PINT0_IRQ
)paren
suffix:semicolon
id|enable_ipr_irq
c_func
(paren
id|PINT8_IRQ
)paren
suffix:semicolon
r_for
c_loop
(paren
id|i
op_assign
l_int|0
suffix:semicolon
id|i
OL
l_int|16
suffix:semicolon
id|i
op_increment
)paren
(brace
id|make_pint_irq
c_func
(paren
id|PINT_IRQ_BASE
op_plus
id|i
)paren
suffix:semicolon
)brace
r_for
c_loop
(paren
id|i
op_assign
l_int|0
suffix:semicolon
id|i
OL
l_int|256
suffix:semicolon
id|i
op_increment
)paren
(brace
r_if
c_cond
(paren
id|i
op_amp
l_int|1
)paren
(brace
id|pint_map
(braket
id|i
)braket
op_assign
l_int|0
suffix:semicolon
)brace
r_else
r_if
c_cond
(paren
id|i
op_amp
l_int|2
)paren
(brace
id|pint_map
(braket
id|i
)braket
op_assign
l_int|1
suffix:semicolon
)brace
r_else
r_if
c_cond
(paren
id|i
op_amp
l_int|4
)paren
(brace
id|pint_map
(braket
id|i
)braket
op_assign
l_int|2
suffix:semicolon
)brace
r_else
r_if
c_cond
(paren
id|i
op_amp
l_int|8
)paren
(brace
id|pint_map
(braket
id|i
)braket
op_assign
l_int|3
suffix:semicolon
)brace
r_else
r_if
c_cond
(paren
id|i
op_amp
l_int|0x10
)paren
(brace
id|pint_map
(braket
id|i
)braket
op_assign
l_int|4
suffix:semicolon
)brace
r_else
r_if
c_cond
(paren
id|i
op_amp
l_int|0x20
)paren
(brace
id|pint_map
(braket
id|i
)braket
op_assign
l_int|5
suffix:semicolon
)brace
r_else
r_if
c_cond
(paren
id|i
op_amp
l_int|0x40
)paren
(brace
id|pint_map
(braket
id|i
)braket
op_assign
l_int|6
suffix:semicolon
)brace
r_else
r_if
c_cond
(paren
id|i
op_amp
l_int|0x80
)paren
(brace
id|pint_map
(braket
id|i
)braket
op_assign
l_int|7
suffix:semicolon
)brace
)brace
macro_line|#endif /* CONFIG_CPU_SUBTYPE_SH7707 || CONFIG_CPU_SUBTYPE_SH7709 */
multiline_comment|/* Perform the machine specific initialisation */
r_if
c_cond
(paren
id|sh_mv.mv_init_irq
op_ne
l_int|NULL
)paren
(brace
id|sh_mv
dot
id|mv_init_irq
c_func
(paren
)paren
suffix:semicolon
)brace
)brace
macro_line|#if defined(CONFIG_CPU_SUBTYPE_SH7707) || defined(CONFIG_CPU_SUBTYPE_SH7709)
DECL|function|ipr_irq_demux
r_int
id|ipr_irq_demux
c_func
(paren
r_int
id|irq
)paren
(brace
r_int
r_int
id|creg
comma
id|dreg
comma
id|d
comma
id|sav
suffix:semicolon
r_if
c_cond
(paren
id|irq
op_eq
id|PINT0_IRQ
)paren
(brace
macro_line|#if defined(CONFIG_CPU_SUBTYPE_SH7707)
id|creg
op_assign
id|PORT_PACR
suffix:semicolon
id|dreg
op_assign
id|PORT_PADR
suffix:semicolon
macro_line|#else
id|creg
op_assign
id|PORT_PCCR
suffix:semicolon
id|dreg
op_assign
id|PORT_PCDR
suffix:semicolon
macro_line|#endif
id|sav
op_assign
id|ctrl_inw
c_func
(paren
id|creg
)paren
suffix:semicolon
id|ctrl_outw
c_func
(paren
id|sav
op_or
id|portcr_mask
comma
id|creg
)paren
suffix:semicolon
id|d
op_assign
(paren
op_complement
id|ctrl_inb
c_func
(paren
id|dreg
)paren
op_xor
id|ctrl_inw
c_func
(paren
id|INTC_ICR2
)paren
)paren
op_amp
id|ctrl_inw
c_func
(paren
id|INTC_INTER
)paren
op_amp
l_int|0xff
suffix:semicolon
id|ctrl_outw
c_func
(paren
id|sav
comma
id|creg
)paren
suffix:semicolon
r_if
c_cond
(paren
id|d
op_eq
l_int|0
)paren
(brace
r_return
id|irq
suffix:semicolon
)brace
r_return
id|PINT_IRQ_BASE
op_plus
id|pint_map
(braket
id|d
)braket
suffix:semicolon
)brace
r_else
r_if
c_cond
(paren
id|irq
op_eq
id|PINT8_IRQ
)paren
(brace
macro_line|#if defined(CONFIG_CPU_SUBTYPE_SH7707)
id|creg
op_assign
id|PORT_PBCR
suffix:semicolon
id|dreg
op_assign
id|PORT_PBDR
suffix:semicolon
macro_line|#else
id|creg
op_assign
id|PORT_PFCR
suffix:semicolon
id|dreg
op_assign
id|PORT_PFDR
suffix:semicolon
macro_line|#endif
id|sav
op_assign
id|ctrl_inw
c_func
(paren
id|creg
)paren
suffix:semicolon
id|ctrl_outw
c_func
(paren
id|sav
op_or
(paren
id|portcr_mask
op_rshift
l_int|16
)paren
comma
id|creg
)paren
suffix:semicolon
id|d
op_assign
(paren
op_complement
id|ctrl_inb
c_func
(paren
id|dreg
)paren
op_xor
(paren
id|ctrl_inw
c_func
(paren
id|INTC_ICR2
)paren
op_rshift
l_int|8
)paren
)paren
op_amp
(paren
id|ctrl_inw
c_func
(paren
id|INTC_INTER
)paren
op_rshift
l_int|8
)paren
op_amp
l_int|0xff
suffix:semicolon
id|ctrl_outw
c_func
(paren
id|sav
comma
id|creg
)paren
suffix:semicolon
r_if
c_cond
(paren
id|d
op_eq
l_int|0
)paren
(brace
r_return
id|irq
suffix:semicolon
)brace
r_return
id|PINT_IRQ_BASE
op_plus
l_int|8
op_plus
id|pint_map
(braket
id|d
)braket
suffix:semicolon
)brace
r_return
id|irq
suffix:semicolon
)brace
macro_line|#endif
eof
