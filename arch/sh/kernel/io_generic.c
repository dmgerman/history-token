multiline_comment|/* $Id: io_generic.c,v 1.2 2003/05/04 19:29:53 lethal Exp $&n; *&n; * linux/arch/sh/kernel/io_generic.c&n; *&n; * Copyright (C) 2000  Niibe Yutaka&n; *&n; * Generic I/O routine. These can be used where a machine specific version&n; * is not required.&n; *&n; * This file is subject to the terms and conditions of the GNU General Public&n; * License.  See the file &quot;COPYING&quot; in the main directory of this archive&n; * for more details.&n; *&n; */
macro_line|#include &lt;asm/io.h&gt;
macro_line|#include &lt;asm/machvec.h&gt;
macro_line|#include &lt;linux/module.h&gt;
macro_line|#if defined(CONFIG_CPU_SH3)
multiline_comment|/* I&squot;m not sure SH7709 has this kind of bug */
DECL|macro|SH3_PCMCIA_BUG_WORKAROUND
mdefine_line|#define SH3_PCMCIA_BUG_WORKAROUND 1
DECL|macro|DUMMY_READ_AREA6
mdefine_line|#define DUMMY_READ_AREA6&t;  0xba000000
macro_line|#endif
DECL|macro|PORT2ADDR
mdefine_line|#define PORT2ADDR(x) (sh_mv.mv_isa_port2addr(x))
DECL|variable|generic_io_base
r_int
r_int
id|generic_io_base
suffix:semicolon
DECL|function|delay
r_static
r_inline
r_void
id|delay
c_func
(paren
r_void
)paren
(brace
id|ctrl_inw
c_func
(paren
l_int|0xa0000000
)paren
suffix:semicolon
)brace
DECL|function|generic_inb
r_int
r_char
id|generic_inb
c_func
(paren
r_int
r_int
id|port
)paren
(brace
r_return
op_star
(paren
r_volatile
r_int
r_char
op_star
)paren
id|PORT2ADDR
c_func
(paren
id|port
)paren
suffix:semicolon
)brace
DECL|function|generic_inw
r_int
r_int
id|generic_inw
c_func
(paren
r_int
r_int
id|port
)paren
(brace
r_return
op_star
(paren
r_volatile
r_int
r_int
op_star
)paren
id|PORT2ADDR
c_func
(paren
id|port
)paren
suffix:semicolon
)brace
DECL|function|generic_inl
r_int
r_int
id|generic_inl
c_func
(paren
r_int
r_int
id|port
)paren
(brace
r_return
op_star
(paren
r_volatile
r_int
r_int
op_star
)paren
id|PORT2ADDR
c_func
(paren
id|port
)paren
suffix:semicolon
)brace
DECL|function|generic_inb_p
r_int
r_char
id|generic_inb_p
c_func
(paren
r_int
r_int
id|port
)paren
(brace
r_int
r_int
id|v
op_assign
op_star
(paren
r_volatile
r_int
r_char
op_star
)paren
id|PORT2ADDR
c_func
(paren
id|port
)paren
suffix:semicolon
id|delay
c_func
(paren
)paren
suffix:semicolon
r_return
id|v
suffix:semicolon
)brace
DECL|function|generic_inw_p
r_int
r_int
id|generic_inw_p
c_func
(paren
r_int
r_int
id|port
)paren
(brace
r_int
r_int
id|v
op_assign
op_star
(paren
r_volatile
r_int
r_int
op_star
)paren
id|PORT2ADDR
c_func
(paren
id|port
)paren
suffix:semicolon
id|delay
c_func
(paren
)paren
suffix:semicolon
r_return
id|v
suffix:semicolon
)brace
DECL|function|generic_inl_p
r_int
r_int
id|generic_inl_p
c_func
(paren
r_int
r_int
id|port
)paren
(brace
r_int
r_int
id|v
op_assign
op_star
(paren
r_volatile
r_int
r_int
op_star
)paren
id|PORT2ADDR
c_func
(paren
id|port
)paren
suffix:semicolon
id|delay
c_func
(paren
)paren
suffix:semicolon
r_return
id|v
suffix:semicolon
)brace
multiline_comment|/*&n; * insb/w/l all read a series of bytes/words/longs from a fixed port&n; * address. However as the port address doesn&squot;t change we only need to&n; * convert the port address to real address once.&n; */
DECL|function|generic_insb
r_void
id|generic_insb
c_func
(paren
r_int
r_int
id|port
comma
r_void
op_star
id|buffer
comma
r_int
r_int
id|count
)paren
(brace
r_volatile
r_int
r_char
op_star
id|port_addr
suffix:semicolon
r_int
r_char
op_star
id|buf
op_assign
id|buffer
suffix:semicolon
id|port_addr
op_assign
(paren
r_volatile
r_int
r_char
op_star
)paren
id|PORT2ADDR
c_func
(paren
id|port
)paren
suffix:semicolon
r_while
c_loop
(paren
id|count
op_decrement
)paren
(brace
op_star
id|buf
op_increment
op_assign
op_star
id|port_addr
suffix:semicolon
)brace
)brace
DECL|function|generic_insw
r_void
id|generic_insw
c_func
(paren
r_int
r_int
id|port
comma
r_void
op_star
id|buffer
comma
r_int
r_int
id|count
)paren
(brace
r_volatile
r_int
r_int
op_star
id|port_addr
suffix:semicolon
r_int
r_int
op_star
id|buf
op_assign
id|buffer
suffix:semicolon
id|port_addr
op_assign
(paren
r_volatile
r_int
r_int
op_star
)paren
id|PORT2ADDR
c_func
(paren
id|port
)paren
suffix:semicolon
r_while
c_loop
(paren
id|count
op_decrement
)paren
(brace
op_star
id|buf
op_increment
op_assign
op_star
id|port_addr
suffix:semicolon
)brace
macro_line|#ifdef SH3_PCMCIA_BUG_WORKAROUND
id|ctrl_inb
(paren
id|DUMMY_READ_AREA6
)paren
suffix:semicolon
macro_line|#endif
)brace
DECL|function|generic_insl
r_void
id|generic_insl
c_func
(paren
r_int
r_int
id|port
comma
r_void
op_star
id|buffer
comma
r_int
r_int
id|count
)paren
(brace
r_volatile
r_int
r_int
op_star
id|port_addr
suffix:semicolon
r_int
r_int
op_star
id|buf
op_assign
id|buffer
suffix:semicolon
id|port_addr
op_assign
(paren
r_volatile
r_int
r_int
op_star
)paren
id|PORT2ADDR
c_func
(paren
id|port
)paren
suffix:semicolon
r_while
c_loop
(paren
id|count
op_decrement
)paren
(brace
op_star
id|buf
op_increment
op_assign
op_star
id|port_addr
suffix:semicolon
)brace
macro_line|#ifdef SH3_PCMCIA_BUG_WORKAROUND
id|ctrl_inb
(paren
id|DUMMY_READ_AREA6
)paren
suffix:semicolon
macro_line|#endif
)brace
DECL|function|generic_outb
r_void
id|generic_outb
c_func
(paren
r_int
r_char
id|b
comma
r_int
r_int
id|port
)paren
(brace
op_star
(paren
r_volatile
r_int
r_char
op_star
)paren
id|PORT2ADDR
c_func
(paren
id|port
)paren
op_assign
id|b
suffix:semicolon
)brace
DECL|function|generic_outw
r_void
id|generic_outw
c_func
(paren
r_int
r_int
id|b
comma
r_int
r_int
id|port
)paren
(brace
op_star
(paren
r_volatile
r_int
r_int
op_star
)paren
id|PORT2ADDR
c_func
(paren
id|port
)paren
op_assign
id|b
suffix:semicolon
)brace
DECL|function|generic_outl
r_void
id|generic_outl
c_func
(paren
r_int
r_int
id|b
comma
r_int
r_int
id|port
)paren
(brace
op_star
(paren
r_volatile
r_int
r_int
op_star
)paren
id|PORT2ADDR
c_func
(paren
id|port
)paren
op_assign
id|b
suffix:semicolon
)brace
DECL|function|generic_outb_p
r_void
id|generic_outb_p
c_func
(paren
r_int
r_char
id|b
comma
r_int
r_int
id|port
)paren
(brace
op_star
(paren
r_volatile
r_int
r_char
op_star
)paren
id|PORT2ADDR
c_func
(paren
id|port
)paren
op_assign
id|b
suffix:semicolon
id|delay
c_func
(paren
)paren
suffix:semicolon
)brace
DECL|function|generic_outw_p
r_void
id|generic_outw_p
c_func
(paren
r_int
r_int
id|b
comma
r_int
r_int
id|port
)paren
(brace
op_star
(paren
r_volatile
r_int
r_int
op_star
)paren
id|PORT2ADDR
c_func
(paren
id|port
)paren
op_assign
id|b
suffix:semicolon
id|delay
c_func
(paren
)paren
suffix:semicolon
)brace
DECL|function|generic_outl_p
r_void
id|generic_outl_p
c_func
(paren
r_int
r_int
id|b
comma
r_int
r_int
id|port
)paren
(brace
op_star
(paren
r_volatile
r_int
r_int
op_star
)paren
id|PORT2ADDR
c_func
(paren
id|port
)paren
op_assign
id|b
suffix:semicolon
id|delay
c_func
(paren
)paren
suffix:semicolon
)brace
multiline_comment|/*&n; * outsb/w/l all write a series of bytes/words/longs to a fixed port&n; * address. However as the port address doesn&squot;t change we only need to&n; * convert the port address to real address once.&n; */
DECL|function|generic_outsb
r_void
id|generic_outsb
c_func
(paren
r_int
r_int
id|port
comma
r_const
r_void
op_star
id|buffer
comma
r_int
r_int
id|count
)paren
(brace
r_volatile
r_int
r_char
op_star
id|port_addr
suffix:semicolon
r_const
r_int
r_char
op_star
id|buf
op_assign
id|buffer
suffix:semicolon
id|port_addr
op_assign
(paren
r_volatile
r_int
r_char
op_star
)paren
id|PORT2ADDR
c_func
(paren
id|port
)paren
suffix:semicolon
r_while
c_loop
(paren
id|count
op_decrement
)paren
(brace
op_star
id|port_addr
op_assign
op_star
id|buf
op_increment
suffix:semicolon
)brace
)brace
DECL|function|generic_outsw
r_void
id|generic_outsw
c_func
(paren
r_int
r_int
id|port
comma
r_const
r_void
op_star
id|buffer
comma
r_int
r_int
id|count
)paren
(brace
r_volatile
r_int
r_int
op_star
id|port_addr
suffix:semicolon
r_const
r_int
r_int
op_star
id|buf
op_assign
id|buffer
suffix:semicolon
id|port_addr
op_assign
(paren
r_volatile
r_int
r_int
op_star
)paren
id|PORT2ADDR
c_func
(paren
id|port
)paren
suffix:semicolon
r_while
c_loop
(paren
id|count
op_decrement
)paren
(brace
op_star
id|port_addr
op_assign
op_star
id|buf
op_increment
suffix:semicolon
)brace
macro_line|#ifdef SH3_PCMCIA_BUG_WORKAROUND
id|ctrl_inb
(paren
id|DUMMY_READ_AREA6
)paren
suffix:semicolon
macro_line|#endif
)brace
DECL|function|generic_outsl
r_void
id|generic_outsl
c_func
(paren
r_int
r_int
id|port
comma
r_const
r_void
op_star
id|buffer
comma
r_int
r_int
id|count
)paren
(brace
r_volatile
r_int
r_int
op_star
id|port_addr
suffix:semicolon
r_const
r_int
r_int
op_star
id|buf
op_assign
id|buffer
suffix:semicolon
id|port_addr
op_assign
(paren
r_volatile
r_int
r_int
op_star
)paren
id|PORT2ADDR
c_func
(paren
id|port
)paren
suffix:semicolon
r_while
c_loop
(paren
id|count
op_decrement
)paren
(brace
op_star
id|port_addr
op_assign
op_star
id|buf
op_increment
suffix:semicolon
)brace
macro_line|#ifdef SH3_PCMCIA_BUG_WORKAROUND
id|ctrl_inb
(paren
id|DUMMY_READ_AREA6
)paren
suffix:semicolon
macro_line|#endif
)brace
DECL|function|generic_readb
r_int
r_char
id|generic_readb
c_func
(paren
r_int
r_int
id|addr
)paren
(brace
r_return
op_star
(paren
r_volatile
r_int
r_char
op_star
)paren
id|addr
suffix:semicolon
)brace
DECL|function|generic_readw
r_int
r_int
id|generic_readw
c_func
(paren
r_int
r_int
id|addr
)paren
(brace
r_return
op_star
(paren
r_volatile
r_int
r_int
op_star
)paren
id|addr
suffix:semicolon
)brace
DECL|function|generic_readl
r_int
r_int
id|generic_readl
c_func
(paren
r_int
r_int
id|addr
)paren
(brace
r_return
op_star
(paren
r_volatile
r_int
r_int
op_star
)paren
id|addr
suffix:semicolon
)brace
DECL|function|generic_writeb
r_void
id|generic_writeb
c_func
(paren
r_int
r_char
id|b
comma
r_int
r_int
id|addr
)paren
(brace
op_star
(paren
r_volatile
r_int
r_char
op_star
)paren
id|addr
op_assign
id|b
suffix:semicolon
)brace
DECL|function|generic_writew
r_void
id|generic_writew
c_func
(paren
r_int
r_int
id|b
comma
r_int
r_int
id|addr
)paren
(brace
op_star
(paren
r_volatile
r_int
r_int
op_star
)paren
id|addr
op_assign
id|b
suffix:semicolon
)brace
DECL|function|generic_writel
r_void
id|generic_writel
c_func
(paren
r_int
r_int
id|b
comma
r_int
r_int
id|addr
)paren
(brace
op_star
(paren
r_volatile
r_int
r_int
op_star
)paren
id|addr
op_assign
id|b
suffix:semicolon
)brace
DECL|function|generic_ioremap
r_void
op_star
id|generic_ioremap
c_func
(paren
r_int
r_int
id|offset
comma
r_int
r_int
id|size
)paren
(brace
r_return
(paren
r_void
op_star
)paren
id|P2SEGADDR
c_func
(paren
id|offset
)paren
suffix:semicolon
)brace
DECL|variable|generic_ioremap
id|EXPORT_SYMBOL
c_func
(paren
id|generic_ioremap
)paren
suffix:semicolon
DECL|function|generic_iounmap
r_void
id|generic_iounmap
c_func
(paren
r_void
op_star
id|addr
)paren
(brace
)brace
DECL|variable|generic_iounmap
id|EXPORT_SYMBOL
c_func
(paren
id|generic_iounmap
)paren
suffix:semicolon
DECL|function|generic_isa_port2addr
r_int
r_int
id|generic_isa_port2addr
c_func
(paren
r_int
r_int
id|offset
)paren
(brace
r_return
id|offset
op_plus
id|generic_io_base
suffix:semicolon
)brace
eof
