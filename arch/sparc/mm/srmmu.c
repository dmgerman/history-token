multiline_comment|/*&n; * srmmu.c:  SRMMU specific routines for memory management.&n; *&n; * Copyright (C) 1995 David S. Miller  (davem@caip.rutgers.edu)&n; * Copyright (C) 1995,2002 Pete Zaitcev (zaitcev@yahoo.com)&n; * Copyright (C) 1996 Eddie C. Dost    (ecd@skynet.be)&n; * Copyright (C) 1997,1998 Jakub Jelinek (jj@sunsite.mff.cuni.cz)&n; * Copyright (C) 1999,2000 Anton Blanchard (anton@samba.org)&n; */
...
suffix:semicolon
multiline_comment|/* these will be initialized in srmmu_nocache_calcsize() */
DECL|variable|srmmu_nocache_size
r_int
r_int
id|srmmu_nocache_size
suffix:semicolon
DECL|variable|srmmu_nocache_end
r_int
r_int
...
suffix:semicolon
DECL|variable|srmmu_nocache_map
r_static
r_struct
id|bit_map
id|srmmu_nocache_map
suffix:semicolon
DECL|function|srmmu_pte_pfn
r_static
r_int
r_int
id|srmmu_pte_pfn
...
id|ptp
suffix:semicolon
multiline_comment|/* Physical address, shifted right by 4 */
r_int
id|i
suffix:semicolon
id|ptp
...
DECL|function|srmmu_pmd_populate
r_static
r_void
id|srmmu_pmd_populate
c_func
(paren
id|pmd_t
op_star
...
suffix:semicolon
multiline_comment|/* watch for overflow */
r_for
c_loop
(paren
id|i
...
id|address
op_rshift
id|SRMMU_PMD_SHIFT_SOFT
)paren
op_amp
(paren
id|SRMMU_PTRS_PER_PMD_SOFT
op_minus
l_int|1
)paren
)paren
...
id|entry.val
op_rshift
id|SRMMU_SWP_TYPE_SHIFT
)paren
op_amp
id|SRMMU_SWP_TYPE_MASK
suffix:semicolon
)brace
DECL|function|srmmu_swp_offset
r_static
r_int
r_int
id|srmmu_swp_offset
...
id|entry.val
op_rshift
id|SRMMU_SWP_OFF_SHIFT
)paren
op_amp
id|SRMMU_SWP_OFF_MASK
suffix:semicolon
)brace
DECL|function|srmmu_swp_entry
r_static
id|swp_entry_t
id|srmmu_swp_entry
c_func
...
multiline_comment|/*&n; * size: bytes to allocate in the nocache area.&n; * align: bytes, number to align at.&n; * Returns the virtual address of the allocated area.&n; */
...
c_func
(paren
l_string|&quot;Size 0x%x too small for nocache request&bslash;n&quot;
comma
id|size
)paren
suffix:semicolon
...
c_func
(paren
l_string|&quot;Size 0x%x unaligned int nocache request&bslash;n&quot;
comma
id|size
)paren
suffix:semicolon
...
c_func
(paren
l_string|&quot;srmmu: out of nocache %d: %d/%d&bslash;n&quot;
comma
id|size
comma
(paren
...
c_func
(paren
l_string|&quot;Size 0x%x is not a power of 2&bslash;n&quot;
comma
id|size
)paren
suffix:semicolon
...
c_func
(paren
l_string|&quot;Size 0x%x is too small&bslash;n&quot;
comma
id|size
)paren
suffix:semicolon
...
suffix:semicolon
multiline_comment|/* in fault.c */
multiline_comment|/*&n; * Reserve nocache dynamically proportionally to the amount of&n; * system RAM. -- Tomas Szepe &lt;szepe@pinerecords.com&gt;, June 2002&n; */
DECL|function|srmmu_nocache_calcsize
r_void
...
suffix:semicolon
r_int
id|srmmu_nocache_npages
suffix:semicolon
id|srmmu_nocache_npages
op_assign
id|sysmemavail
op_div
id|SRMMU_NOCACHE_ALCRATIO
op_div
l_int|1024
op_star
l_int|256
suffix:semicolon
multiline_comment|/* P3 XXX The 4x overuse: corroborated by /proc/meminfo. */
singleline_comment|// if (srmmu_nocache_npages &lt; 256) srmmu_nocache_npages = 256;
r_if
c_cond
(paren
id|srmmu_nocache_npages
OL
id|SRMMU_MIN_NOCACHE_PAGES
)paren
id|srmmu_nocache_npages
op_assign
id|SRMMU_MIN_NOCACHE_PAGES
suffix:semicolon
multiline_comment|/* anything above 1280 blows up */
r_if
c_cond
(paren
id|srmmu_nocache_npages
OG
id|SRMMU_MAX_NOCACHE_PAGES
)paren
id|srmmu_nocache_npages
op_assign
id|SRMMU_MAX_NOCACHE_PAGES
...
DECL|function|srmmu_free_pgd_fast
r_static
r_void
id|srmmu_free_pgd_fast
c_func
(paren
id|pgd_t
op_star
...
DECL|function|srmmu_pmd_alloc_one
r_static
id|pmd_t
op_star
id|srmmu_pmd_alloc_one
c_func
(paren
r_struct
id|mm_struct
...
multiline_comment|/*&n; * Hardware needs alignment to 256 only, but we align to whole page size&n; * to reduce fragmentation problems due to the buddy principle.&n; * XXX Provide actual fragmentation statistics in /proc.&n; *&n; * Alignments up to the page size are the same for physical and virtual&n; * addresses of the nocache area.&n; */
r_static
id|pte_t
op_star
DECL|function|srmmu_pte_alloc_one_kernel
id|srmmu_pte_alloc_one_kernel
c_func
(paren
r_struct
id|mm_struct
...
id|page
op_star
DECL|function|srmmu_pte_alloc_one
id|srmmu_pte_alloc_one
c_func
(paren
r_struct
id|mm_struct
...
suffix:semicolon
multiline_comment|/* Cached address (for test) */
r_if
c_cond
(paren
id|p
...
suffix:semicolon
multiline_comment|/* Nocached virtual */
id|srmmu_free_nocache
c_func
(paren
id|p
...
DECL|function|srmmu_mapiorange
r_static
r_void
id|srmmu_mapiorange
c_func
...
DECL|function|srmmu_unmapiorange
r_static
r_void
id|srmmu_unmapiorange
c_func
(paren
r_int
r_int
...
DECL|function|srmmu_alloc_thread_info
r_struct
id|thread_info
op_star
id|srmmu_alloc_thread_info
c_func
(paren
r_void
)paren
...
(brace
multiline_comment|/*&n;&t;&t;&t; * We bend the rule where all 16 PTPs in a pmd_t point&n;&t;&t;&t; * inside the same PTE page, and we leak a perfectly&n;&t;&t;&t; * good hardware PTE piece. Alternatives seem worse.&n;&t;&t;&t; */
r_int
r_int
id|x
suffix:semicolon
multiline_comment|/* Index of HW PMD in soft cluster */
id|x
op_assign
(paren
id|start
...
suffix:semicolon
multiline_comment|/* Map &quot;low&quot; memory only */
r_const
r_int
r_int
id|min_vaddr
...
macro_line|#endif
DECL|function|srmmu_pgoff_to_pte
r_static
id|pte_t
id|srmmu_pgoff_to_pte
c_func
(paren
r_int
r_int
...
DECL|function|srmmu_pte_to_pgoff
r_static
r_int
r_int
id|srmmu_pte_to_pgoff
c_func
(paren
id|pte_t
id|pte
