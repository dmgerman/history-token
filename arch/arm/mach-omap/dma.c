multiline_comment|/*&n; * linux/arch/arm/omap/dma.c&n; *&n; * Copyright (C) 2003 Nokia Corporation&n; * Author: Juha Yrj&#xfffd;l&#xfffd; &lt;juha.yrjola@nokia.com&gt;&n; * DMA channel linking for 1610 by Samuel Ortiz &lt;samuel.ortiz@nokia.com&gt;&n; * Graphics DMA and LCD DMA graphics tranformations&n; * by Imre Deak &lt;imre.deak@nokia.com&gt;&n; * Some functions based on earlier dma-omap.c Copyright (C) 2001 RidgeRun, Inc.&n; *&n; * Support functions for the OMAP internal DMA channels.&n; *&n; * This program is free software; you can redistribute it and/or modify&n; * it under the terms of the GNU General Public License version 2 as&n; * published by the Free Software Foundation.&n; *&n; */
...
multiline_comment|/*&n; * Clears any DMA state so the DMA engine is ready to restart with new buffers&n; * through omap_start_dma(). Any buffers in flight are discarded.&n; */
DECL|function|omap_clear_dma
r_void
id|omap_clear_dma
c_func
(paren
...
multiline_comment|/*&n; * Returns current physical source address for the given DMA channel.&n; * If the channel is running the caller must disable interrupts prior calling&n; * this function and process the returned value before re-enabling interrupt to&n; * prevent races with the interrupt handler. Note that in continuous mode there&n; * is a chance for CSSA_L register overflow inbetween the two reads resulting&n; * in incorrect return value.&n; */
DECL|function|omap_get_dma_src_pos
id|dma_addr_t
id|omap_get_dma_src_pos
c_func
(paren
r_int
id|lch
...
multiline_comment|/*&n; * Returns current physical destination address for the given DMA channel.&n; * If the channel is running the caller must disable interrupts prior calling&n; * this function and process the returned value before re-enabling interrupt to&n; * prevent races with the interrupt handler. Note that in continuous mode there&n; * is a chance for CDSA_L register overflow inbetween the two reads resulting&n; * in incorrect return value.&n; */
DECL|function|omap_get_dma_dst_pos
id|dma_addr_t
id|omap_get_dma_dst_pos
c_func
(paren
r_int
id|lch
...
suffix:semicolon
DECL|variable|omap_get_dma_src_pos
id|EXPORT_SYMBOL
c_func
(paren
id|omap_get_dma_src_pos
...
suffix:semicolon
DECL|variable|omap_get_dma_dst_pos
id|EXPORT_SYMBOL
c_func
(paren
id|omap_get_dma_dst_pos
