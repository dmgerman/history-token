...
multiline_comment|/*&n; * ----------------------------------------------------------------------------&n; * OMAP revision check&n; *&n; * Since we use the cpu_is_omapnnnn() macros, there&squot;s a chance that a board&n; * switches to an updated core. We want to print out the OMAP revision early.&n; *&n; * We use the system_serial registers for the revision information so we&n; * can see it in /proc/cpuinfo.&n; *&n; * If the OMAP detection gets more complicated, we may want to expand this&n; * to store the OMAP version and replace the current cpu_is_omapnnnn() macros.&n; *&n; * ----------------------------------------------------------------------------&n; */
...
c_func
(paren
l_string|&quot;OMAP revision: %d.%d (0x%08x) id: 0x%08x detected as OMAP-&quot;
comma
(paren
id|system_serial_high
op_rshift
...
c_func
(paren
l_string|&quot;unknown, please add support!&bslash;n&quot;
)paren
suffix:semicolon
)brace
)brace
