...
multiline_comment|/*&n; * A note about masking IRQs:&n; *&n; * The GPIO IRQ edge detection only functions while the IRQ itself is&n; * enabled; edges are not detected while the IRQ is disabled.&n; *&n; * This is especially important for the PCMCIA signals, where we must&n; * pick up every transition.  We therefore do not disable the IRQs&n; * while processing them.&n; *&n; * However, since we are changed to a GPIO on the host processor,&n; * all SA1111 IRQs will be disabled while we&squot;re processing any SA1111&n; * IRQ.&n; *&n; * Note also that changing INTPOL while an IRQ is enabled will itself&n; * trigger an IRQ.&n; */
...
(paren
id|KERN_ERR
l_string|&quot;SA1111: unable to claim IRQ%d: %d&bslash;n&quot;
comma
id|irq_nr
comma
id|ret
