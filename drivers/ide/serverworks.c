multiline_comment|/*&n; * linux/drivers/ide/serverworks.c&t;&t;Version 0.6&t;05 April 2002&n; *&n; * Copyright (C) 1998-2000 Michel Aubry&n; * Copyright (C) 1998-2000 Andrzej Krzysztofowicz&n; * Copyright (C) 1998-2000 Andre Hedrick &lt;andre@linux-ide.org&gt;&n; * Portions copyright (c) 2001 Sun Microsystems&n; *&n; *&n; * RCC/ServerWorks IDE driver for Linux&n; *&n; *   OSB4: `Open South Bridge&squot; IDE Interface (fn 1)&n; *         supports UDMA mode 2 (33 MB/s)&n; *&n; *   CSB5: `Champion South Bridge&squot; IDE Interface (fn 1)&n; *         all revisions support UDMA mode 4 (66 MB/s)&n; *         revision A2.0 and up support UDMA mode 5 (100 MB/s)&n; *&n; *         *** The CSB5 does not provide ANY register ***&n; *         *** to detect 80-conductor cable presence. ***&n; *&n; *   CSB6: `Champion South Bridge&squot; IDE Interface (optional: third channel)&n; *&n; */
macro_line|#include &lt;linux/config.h&gt;
macro_line|#include &lt;linux/types.h&gt;
macro_line|#include &lt;linux/kernel.h&gt;
macro_line|#include &lt;linux/ioport.h&gt;
macro_line|#include &lt;linux/pci.h&gt;
macro_line|#include &lt;linux/hdreg.h&gt;
macro_line|#include &lt;linux/ide.h&gt;
macro_line|#include &lt;linux/init.h&gt;
macro_line|#include &lt;linux/delay.h&gt;
macro_line|#include &lt;asm/io.h&gt;
macro_line|#include &quot;ide_modes.h&quot;
DECL|macro|DISPLAY_SVWKS_TIMINGS
mdefine_line|#define DISPLAY_SVWKS_TIMINGS&t;1
DECL|macro|SVWKS_DEBUG_DRIVE_INFO
macro_line|#undef SVWKS_DEBUG_DRIVE_INFO
macro_line|#if defined(DISPLAY_SVWKS_TIMINGS) &amp;&amp; defined(CONFIG_PROC_FS)
macro_line|#include &lt;linux/stat.h&gt;
macro_line|#include &lt;linux/proc_fs.h&gt;
DECL|macro|SVWKS_MAX_DEVS
mdefine_line|#define SVWKS_MAX_DEVS&t;&t;2
DECL|variable|svwks_devs
r_static
r_struct
id|pci_dev
op_star
id|svwks_devs
(braket
id|SVWKS_MAX_DEVS
)braket
suffix:semicolon
DECL|variable|n_svwks_devs
r_static
r_int
id|n_svwks_devs
suffix:semicolon
DECL|variable|svwks_revision
r_static
id|byte
id|svwks_revision
op_assign
l_int|0
suffix:semicolon
r_static
r_int
id|svwks_get_info
c_func
(paren
r_char
op_star
comma
r_char
op_star
op_star
comma
id|off_t
comma
r_int
)paren
suffix:semicolon
r_extern
r_int
(paren
op_star
id|svwks_display_info
)paren
(paren
r_char
op_star
comma
r_char
op_star
op_star
comma
id|off_t
comma
r_int
)paren
suffix:semicolon
multiline_comment|/* ide-proc.c */
DECL|function|svwks_get_info
r_static
r_int
id|svwks_get_info
(paren
r_char
op_star
id|buffer
comma
r_char
op_star
op_star
id|addr
comma
id|off_t
id|offset
comma
r_int
id|count
)paren
(brace
r_char
op_star
id|p
op_assign
id|buffer
suffix:semicolon
r_int
id|i
suffix:semicolon
id|p
op_add_assign
id|sprintf
c_func
(paren
id|p
comma
l_string|&quot;&bslash;n                             &quot;
l_string|&quot;ServerWorks OSB4/CSB5/CSB6&bslash;n&quot;
)paren
suffix:semicolon
r_for
c_loop
(paren
id|i
op_assign
l_int|0
suffix:semicolon
id|i
OL
id|n_svwks_devs
suffix:semicolon
id|i
op_increment
)paren
(brace
r_struct
id|pci_dev
op_star
id|dev
op_assign
id|svwks_devs
(braket
id|i
)braket
suffix:semicolon
id|u32
id|bibma
op_assign
id|pci_resource_start
c_func
(paren
id|dev
comma
l_int|4
)paren
suffix:semicolon
id|u32
id|reg40
comma
id|reg44
suffix:semicolon
id|u16
id|reg48
comma
id|reg56
suffix:semicolon
id|u8
id|reg54
comma
id|c0
op_assign
l_int|0
comma
id|c1
op_assign
l_int|0
suffix:semicolon
id|pci_read_config_dword
c_func
(paren
id|dev
comma
l_int|0x40
comma
op_amp
id|reg40
)paren
suffix:semicolon
id|pci_read_config_dword
c_func
(paren
id|dev
comma
l_int|0x44
comma
op_amp
id|reg44
)paren
suffix:semicolon
id|pci_read_config_word
c_func
(paren
id|dev
comma
l_int|0x48
comma
op_amp
id|reg48
)paren
suffix:semicolon
id|pci_read_config_byte
c_func
(paren
id|dev
comma
l_int|0x54
comma
op_amp
id|reg54
)paren
suffix:semicolon
id|pci_read_config_word
c_func
(paren
id|dev
comma
l_int|0x56
comma
op_amp
id|reg56
)paren
suffix:semicolon
multiline_comment|/*&n;&t;&t; * at that point bibma+0x2 et bibma+0xa are byte registers&n;&t;&t; * to investigate:&n;&t;&t; */
id|c0
op_assign
id|inb_p
c_func
(paren
(paren
r_int
r_int
)paren
id|bibma
op_plus
l_int|0x02
)paren
suffix:semicolon
id|c1
op_assign
id|inb_p
c_func
(paren
(paren
r_int
r_int
)paren
id|bibma
op_plus
l_int|0x0a
)paren
suffix:semicolon
r_switch
c_cond
(paren
id|dev-&gt;device
)paren
(brace
r_case
id|PCI_DEVICE_ID_SERVERWORKS_CSB6IDE
suffix:colon
id|p
op_add_assign
id|sprintf
c_func
(paren
id|p
comma
l_string|&quot;&bslash;n                            &quot;
l_string|&quot;ServerWorks CSB6 Chipset (rev %02x)&bslash;n&quot;
comma
id|svwks_revision
)paren
suffix:semicolon
r_break
suffix:semicolon
r_case
id|PCI_DEVICE_ID_SERVERWORKS_CSB5IDE
suffix:colon
id|p
op_add_assign
id|sprintf
c_func
(paren
id|p
comma
l_string|&quot;&bslash;n                            &quot;
l_string|&quot;ServerWorks CSB5 Chipset (rev %02x)&bslash;n&quot;
comma
id|svwks_revision
)paren
suffix:semicolon
r_break
suffix:semicolon
r_case
id|PCI_DEVICE_ID_SERVERWORKS_OSB4IDE
suffix:colon
id|p
op_add_assign
id|sprintf
c_func
(paren
id|p
comma
l_string|&quot;&bslash;n                            &quot;
l_string|&quot;ServerWorks OSB4 Chipset (rev %02x)&bslash;n&quot;
comma
id|svwks_revision
)paren
suffix:semicolon
r_break
suffix:semicolon
r_default
suffix:colon
id|p
op_add_assign
id|sprintf
c_func
(paren
id|p
comma
l_string|&quot;&bslash;n                            &quot;
l_string|&quot;ServerWorks %04x Chipset (rev %02x)&bslash;n&quot;
comma
id|dev-&gt;device
comma
id|svwks_revision
)paren
suffix:semicolon
r_break
suffix:semicolon
)brace
id|p
op_add_assign
id|sprintf
c_func
(paren
id|p
comma
l_string|&quot;------------------------------- &quot;
l_string|&quot;General Status &quot;
l_string|&quot;---------------------------------&bslash;n&quot;
)paren
suffix:semicolon
id|p
op_add_assign
id|sprintf
c_func
(paren
id|p
comma
l_string|&quot;--------------- Primary Channel &quot;
l_string|&quot;---------------- Secondary Channel &quot;
l_string|&quot;-------------&bslash;n&quot;
)paren
suffix:semicolon
id|p
op_add_assign
id|sprintf
c_func
(paren
id|p
comma
l_string|&quot;                %sabled                         %sabled&bslash;n&quot;
comma
(paren
id|c0
op_amp
l_int|0x80
)paren
ques
c_cond
l_string|&quot;dis&quot;
suffix:colon
l_string|&quot; en&quot;
comma
(paren
id|c1
op_amp
l_int|0x80
)paren
ques
c_cond
l_string|&quot;dis&quot;
suffix:colon
l_string|&quot; en&quot;
)paren
suffix:semicolon
id|p
op_add_assign
id|sprintf
c_func
(paren
id|p
comma
l_string|&quot;--------------- drive0 --------- drive1 &quot;
l_string|&quot;-------- drive0 ---------- drive1 ------&bslash;n&quot;
)paren
suffix:semicolon
id|p
op_add_assign
id|sprintf
c_func
(paren
id|p
comma
l_string|&quot;DMA enabled:    %s              %s&quot;
l_string|&quot;             %s               %s&bslash;n&quot;
comma
(paren
id|c0
op_amp
l_int|0x20
)paren
ques
c_cond
l_string|&quot;yes&quot;
suffix:colon
l_string|&quot;no &quot;
comma
(paren
id|c0
op_amp
l_int|0x40
)paren
ques
c_cond
l_string|&quot;yes&quot;
suffix:colon
l_string|&quot;no &quot;
comma
(paren
id|c1
op_amp
l_int|0x20
)paren
ques
c_cond
l_string|&quot;yes&quot;
suffix:colon
l_string|&quot;no &quot;
comma
(paren
id|c1
op_amp
l_int|0x40
)paren
ques
c_cond
l_string|&quot;yes&quot;
suffix:colon
l_string|&quot;no &quot;
)paren
suffix:semicolon
id|p
op_add_assign
id|sprintf
c_func
(paren
id|p
comma
l_string|&quot;UDMA enabled:   %s              %s&quot;
l_string|&quot;             %s               %s&bslash;n&quot;
comma
(paren
id|reg54
op_amp
l_int|0x01
)paren
ques
c_cond
l_string|&quot;yes&quot;
suffix:colon
l_string|&quot;no &quot;
comma
(paren
id|reg54
op_amp
l_int|0x02
)paren
ques
c_cond
l_string|&quot;yes&quot;
suffix:colon
l_string|&quot;no &quot;
comma
(paren
id|reg54
op_amp
l_int|0x04
)paren
ques
c_cond
l_string|&quot;yes&quot;
suffix:colon
l_string|&quot;no &quot;
comma
(paren
id|reg54
op_amp
l_int|0x08
)paren
ques
c_cond
l_string|&quot;yes&quot;
suffix:colon
l_string|&quot;no &quot;
)paren
suffix:semicolon
id|p
op_add_assign
id|sprintf
c_func
(paren
id|p
comma
l_string|&quot;UDMA enabled:   %s                %s&quot;
l_string|&quot;               %s                 %s&bslash;n&quot;
comma
(paren
(paren
id|reg56
op_amp
l_int|0x0005
)paren
op_eq
l_int|0x0005
)paren
ques
c_cond
l_string|&quot;5&quot;
suffix:colon
(paren
(paren
id|reg56
op_amp
l_int|0x0004
)paren
op_eq
l_int|0x0004
)paren
ques
c_cond
l_string|&quot;4&quot;
suffix:colon
(paren
(paren
id|reg56
op_amp
l_int|0x0003
)paren
op_eq
l_int|0x0003
)paren
ques
c_cond
l_string|&quot;3&quot;
suffix:colon
(paren
(paren
id|reg56
op_amp
l_int|0x0002
)paren
op_eq
l_int|0x0002
)paren
ques
c_cond
l_string|&quot;2&quot;
suffix:colon
(paren
(paren
id|reg56
op_amp
l_int|0x0001
)paren
op_eq
l_int|0x0001
)paren
ques
c_cond
l_string|&quot;1&quot;
suffix:colon
(paren
(paren
id|reg56
op_amp
l_int|0x000F
)paren
)paren
ques
c_cond
l_string|&quot;?&quot;
suffix:colon
l_string|&quot;0&quot;
comma
(paren
(paren
id|reg56
op_amp
l_int|0x0050
)paren
op_eq
l_int|0x0050
)paren
ques
c_cond
l_string|&quot;5&quot;
suffix:colon
(paren
(paren
id|reg56
op_amp
l_int|0x0040
)paren
op_eq
l_int|0x0040
)paren
ques
c_cond
l_string|&quot;4&quot;
suffix:colon
(paren
(paren
id|reg56
op_amp
l_int|0x0030
)paren
op_eq
l_int|0x0030
)paren
ques
c_cond
l_string|&quot;3&quot;
suffix:colon
(paren
(paren
id|reg56
op_amp
l_int|0x0020
)paren
op_eq
l_int|0x0020
)paren
ques
c_cond
l_string|&quot;2&quot;
suffix:colon
(paren
(paren
id|reg56
op_amp
l_int|0x0010
)paren
op_eq
l_int|0x0010
)paren
ques
c_cond
l_string|&quot;1&quot;
suffix:colon
(paren
(paren
id|reg56
op_amp
l_int|0x00F0
)paren
)paren
ques
c_cond
l_string|&quot;?&quot;
suffix:colon
l_string|&quot;0&quot;
comma
(paren
(paren
id|reg56
op_amp
l_int|0x0500
)paren
op_eq
l_int|0x0500
)paren
ques
c_cond
l_string|&quot;5&quot;
suffix:colon
(paren
(paren
id|reg56
op_amp
l_int|0x0400
)paren
op_eq
l_int|0x0400
)paren
ques
c_cond
l_string|&quot;4&quot;
suffix:colon
(paren
(paren
id|reg56
op_amp
l_int|0x0300
)paren
op_eq
l_int|0x0300
)paren
ques
c_cond
l_string|&quot;3&quot;
suffix:colon
(paren
(paren
id|reg56
op_amp
l_int|0x0200
)paren
op_eq
l_int|0x0200
)paren
ques
c_cond
l_string|&quot;2&quot;
suffix:colon
(paren
(paren
id|reg56
op_amp
l_int|0x0100
)paren
op_eq
l_int|0x0100
)paren
ques
c_cond
l_string|&quot;1&quot;
suffix:colon
(paren
(paren
id|reg56
op_amp
l_int|0x0F00
)paren
)paren
ques
c_cond
l_string|&quot;?&quot;
suffix:colon
l_string|&quot;0&quot;
comma
(paren
(paren
id|reg56
op_amp
l_int|0x5000
)paren
op_eq
l_int|0x5000
)paren
ques
c_cond
l_string|&quot;5&quot;
suffix:colon
(paren
(paren
id|reg56
op_amp
l_int|0x4000
)paren
op_eq
l_int|0x4000
)paren
ques
c_cond
l_string|&quot;4&quot;
suffix:colon
(paren
(paren
id|reg56
op_amp
l_int|0x3000
)paren
op_eq
l_int|0x3000
)paren
ques
c_cond
l_string|&quot;3&quot;
suffix:colon
(paren
(paren
id|reg56
op_amp
l_int|0x2000
)paren
op_eq
l_int|0x2000
)paren
ques
c_cond
l_string|&quot;2&quot;
suffix:colon
(paren
(paren
id|reg56
op_amp
l_int|0x1000
)paren
op_eq
l_int|0x1000
)paren
ques
c_cond
l_string|&quot;1&quot;
suffix:colon
(paren
(paren
id|reg56
op_amp
l_int|0xF000
)paren
)paren
ques
c_cond
l_string|&quot;?&quot;
suffix:colon
l_string|&quot;0&quot;
)paren
suffix:semicolon
id|p
op_add_assign
id|sprintf
c_func
(paren
id|p
comma
l_string|&quot;DMA enabled:    %s                %s&quot;
l_string|&quot;               %s                 %s&bslash;n&quot;
comma
(paren
(paren
id|reg44
op_amp
l_int|0x00002000
)paren
op_eq
l_int|0x00002000
)paren
ques
c_cond
l_string|&quot;2&quot;
suffix:colon
(paren
(paren
id|reg44
op_amp
l_int|0x00002100
)paren
op_eq
l_int|0x00002100
)paren
ques
c_cond
l_string|&quot;1&quot;
suffix:colon
(paren
(paren
id|reg44
op_amp
l_int|0x00007700
)paren
op_eq
l_int|0x00007700
)paren
ques
c_cond
l_string|&quot;0&quot;
suffix:colon
(paren
(paren
id|reg44
op_amp
l_int|0x0000FF00
)paren
op_eq
l_int|0x0000FF00
)paren
ques
c_cond
l_string|&quot;X&quot;
suffix:colon
l_string|&quot;?&quot;
comma
(paren
(paren
id|reg44
op_amp
l_int|0x00000020
)paren
op_eq
l_int|0x00000020
)paren
ques
c_cond
l_string|&quot;2&quot;
suffix:colon
(paren
(paren
id|reg44
op_amp
l_int|0x00000021
)paren
op_eq
l_int|0x00000021
)paren
ques
c_cond
l_string|&quot;1&quot;
suffix:colon
(paren
(paren
id|reg44
op_amp
l_int|0x00000077
)paren
op_eq
l_int|0x00000077
)paren
ques
c_cond
l_string|&quot;0&quot;
suffix:colon
(paren
(paren
id|reg44
op_amp
l_int|0x000000FF
)paren
op_eq
l_int|0x000000FF
)paren
ques
c_cond
l_string|&quot;X&quot;
suffix:colon
l_string|&quot;?&quot;
comma
(paren
(paren
id|reg44
op_amp
l_int|0x20000000
)paren
op_eq
l_int|0x20000000
)paren
ques
c_cond
l_string|&quot;2&quot;
suffix:colon
(paren
(paren
id|reg44
op_amp
l_int|0x21000000
)paren
op_eq
l_int|0x21000000
)paren
ques
c_cond
l_string|&quot;1&quot;
suffix:colon
(paren
(paren
id|reg44
op_amp
l_int|0x77000000
)paren
op_eq
l_int|0x77000000
)paren
ques
c_cond
l_string|&quot;0&quot;
suffix:colon
(paren
(paren
id|reg44
op_amp
l_int|0xFF000000
)paren
op_eq
l_int|0xFF000000
)paren
ques
c_cond
l_string|&quot;X&quot;
suffix:colon
l_string|&quot;?&quot;
comma
(paren
(paren
id|reg44
op_amp
l_int|0x00200000
)paren
op_eq
l_int|0x00200000
)paren
ques
c_cond
l_string|&quot;2&quot;
suffix:colon
(paren
(paren
id|reg44
op_amp
l_int|0x00210000
)paren
op_eq
l_int|0x00210000
)paren
ques
c_cond
l_string|&quot;1&quot;
suffix:colon
(paren
(paren
id|reg44
op_amp
l_int|0x00770000
)paren
op_eq
l_int|0x00770000
)paren
ques
c_cond
l_string|&quot;0&quot;
suffix:colon
(paren
(paren
id|reg44
op_amp
l_int|0x00FF0000
)paren
op_eq
l_int|0x00FF0000
)paren
ques
c_cond
l_string|&quot;X&quot;
suffix:colon
l_string|&quot;?&quot;
)paren
suffix:semicolon
id|p
op_add_assign
id|sprintf
c_func
(paren
id|p
comma
l_string|&quot;PIO  enabled:   %s                %s&quot;
l_string|&quot;               %s                 %s&bslash;n&quot;
comma
(paren
(paren
id|reg40
op_amp
l_int|0x00002000
)paren
op_eq
l_int|0x00002000
)paren
ques
c_cond
l_string|&quot;4&quot;
suffix:colon
(paren
(paren
id|reg40
op_amp
l_int|0x00002200
)paren
op_eq
l_int|0x00002200
)paren
ques
c_cond
l_string|&quot;3&quot;
suffix:colon
(paren
(paren
id|reg40
op_amp
l_int|0x00003400
)paren
op_eq
l_int|0x00003400
)paren
ques
c_cond
l_string|&quot;2&quot;
suffix:colon
(paren
(paren
id|reg40
op_amp
l_int|0x00004700
)paren
op_eq
l_int|0x00004700
)paren
ques
c_cond
l_string|&quot;1&quot;
suffix:colon
(paren
(paren
id|reg40
op_amp
l_int|0x00005D00
)paren
op_eq
l_int|0x00005D00
)paren
ques
c_cond
l_string|&quot;0&quot;
suffix:colon
l_string|&quot;?&quot;
comma
(paren
(paren
id|reg40
op_amp
l_int|0x00000020
)paren
op_eq
l_int|0x00000020
)paren
ques
c_cond
l_string|&quot;4&quot;
suffix:colon
(paren
(paren
id|reg40
op_amp
l_int|0x00000022
)paren
op_eq
l_int|0x00000022
)paren
ques
c_cond
l_string|&quot;3&quot;
suffix:colon
(paren
(paren
id|reg40
op_amp
l_int|0x00000034
)paren
op_eq
l_int|0x00000034
)paren
ques
c_cond
l_string|&quot;2&quot;
suffix:colon
(paren
(paren
id|reg40
op_amp
l_int|0x00000047
)paren
op_eq
l_int|0x00000047
)paren
ques
c_cond
l_string|&quot;1&quot;
suffix:colon
(paren
(paren
id|reg40
op_amp
l_int|0x0000005D
)paren
op_eq
l_int|0x0000005D
)paren
ques
c_cond
l_string|&quot;0&quot;
suffix:colon
l_string|&quot;?&quot;
comma
(paren
(paren
id|reg40
op_amp
l_int|0x20000000
)paren
op_eq
l_int|0x20000000
)paren
ques
c_cond
l_string|&quot;4&quot;
suffix:colon
(paren
(paren
id|reg40
op_amp
l_int|0x22000000
)paren
op_eq
l_int|0x22000000
)paren
ques
c_cond
l_string|&quot;3&quot;
suffix:colon
(paren
(paren
id|reg40
op_amp
l_int|0x34000000
)paren
op_eq
l_int|0x34000000
)paren
ques
c_cond
l_string|&quot;2&quot;
suffix:colon
(paren
(paren
id|reg40
op_amp
l_int|0x47000000
)paren
op_eq
l_int|0x47000000
)paren
ques
c_cond
l_string|&quot;1&quot;
suffix:colon
(paren
(paren
id|reg40
op_amp
l_int|0x5D000000
)paren
op_eq
l_int|0x5D000000
)paren
ques
c_cond
l_string|&quot;0&quot;
suffix:colon
l_string|&quot;?&quot;
comma
(paren
(paren
id|reg40
op_amp
l_int|0x00200000
)paren
op_eq
l_int|0x00200000
)paren
ques
c_cond
l_string|&quot;4&quot;
suffix:colon
(paren
(paren
id|reg40
op_amp
l_int|0x00220000
)paren
op_eq
l_int|0x00220000
)paren
ques
c_cond
l_string|&quot;3&quot;
suffix:colon
(paren
(paren
id|reg40
op_amp
l_int|0x00340000
)paren
op_eq
l_int|0x00340000
)paren
ques
c_cond
l_string|&quot;2&quot;
suffix:colon
(paren
(paren
id|reg40
op_amp
l_int|0x00470000
)paren
op_eq
l_int|0x00470000
)paren
ques
c_cond
l_string|&quot;1&quot;
suffix:colon
(paren
(paren
id|reg40
op_amp
l_int|0x005D0000
)paren
op_eq
l_int|0x005D0000
)paren
ques
c_cond
l_string|&quot;0&quot;
suffix:colon
l_string|&quot;?&quot;
)paren
suffix:semicolon
)brace
id|p
op_add_assign
id|sprintf
c_func
(paren
id|p
comma
l_string|&quot;&bslash;n&quot;
)paren
suffix:semicolon
r_return
id|p
op_minus
id|buffer
suffix:semicolon
multiline_comment|/* =&gt; must be less than 4k! */
)brace
macro_line|#endif  /* defined(DISPLAY_SVWKS_TIMINGS) &amp;&amp; defined(CONFIG_PROC_FS) */
DECL|macro|SVWKS_CSB5_REVISION_NEW
mdefine_line|#define SVWKS_CSB5_REVISION_NEW&t;0x92 /* min PCI_REVISION_ID for UDMA5 (A2.0) */
DECL|macro|SVWKS_CSB6_REVISION
mdefine_line|#define SVWKS_CSB6_REVISION&t;0xa0 /* min PCI_REVISION_ID for UDMA4 (A1.0) */
DECL|variable|svwks_proc
id|byte
id|svwks_proc
op_assign
l_int|0
suffix:semicolon
DECL|variable|isa_dev
r_static
r_struct
id|pci_dev
op_star
id|isa_dev
suffix:semicolon
DECL|function|svwks_ratemask
r_static
id|byte
id|svwks_ratemask
(paren
id|ide_drive_t
op_star
id|drive
)paren
(brace
r_struct
id|pci_dev
op_star
id|dev
op_assign
id|HWIF
c_func
(paren
id|drive
)paren
op_member_access_from_pointer
id|pci_dev
suffix:semicolon
id|byte
id|mode
op_assign
l_int|0
suffix:semicolon
r_if
c_cond
(paren
id|dev-&gt;device
op_eq
id|PCI_DEVICE_ID_SERVERWORKS_OSB4IDE
)paren
(brace
id|u32
id|reg
op_assign
l_int|0
suffix:semicolon
id|mode
op_and_assign
op_complement
l_int|0x01
suffix:semicolon
r_if
c_cond
(paren
id|isa_dev
)paren
id|pci_read_config_dword
c_func
(paren
id|isa_dev
comma
l_int|0x64
comma
op_amp
id|reg
)paren
suffix:semicolon
r_if
c_cond
(paren
(paren
id|reg
op_amp
l_int|0x00004000
)paren
op_eq
l_int|0x00004000
)paren
id|mode
op_or_assign
l_int|0x01
suffix:semicolon
)brace
r_else
r_if
c_cond
(paren
id|svwks_revision
OL
id|SVWKS_CSB5_REVISION_NEW
)paren
(brace
id|mode
op_or_assign
l_int|0x01
suffix:semicolon
)brace
r_else
r_if
c_cond
(paren
id|svwks_revision
op_ge
id|SVWKS_CSB5_REVISION_NEW
)paren
(brace
id|u8
id|btr
op_assign
l_int|0
suffix:semicolon
id|pci_read_config_byte
c_func
(paren
id|dev
comma
l_int|0x5A
comma
op_amp
id|btr
)paren
suffix:semicolon
id|mode
op_or_assign
id|btr
suffix:semicolon
r_if
c_cond
(paren
op_logical_neg
id|eighty_ninty_three
c_func
(paren
id|drive
)paren
)paren
id|mode
op_and_assign
op_complement
l_int|0x02
suffix:semicolon
)brace
r_if
c_cond
(paren
(paren
id|dev-&gt;device
op_eq
id|PCI_DEVICE_ID_SERVERWORKS_CSB6IDE
)paren
op_logical_and
(paren
op_logical_neg
(paren
id|PCI_FUNC
c_func
(paren
id|dev-&gt;devfn
)paren
op_amp
l_int|1
)paren
)paren
)paren
id|mode
op_assign
l_int|0x02
suffix:semicolon
id|mode
op_and_assign
op_complement
l_int|0xFC
suffix:semicolon
r_return
(paren
id|mode
)paren
suffix:semicolon
)brace
DECL|function|svwks_ratefilter
r_static
id|byte
id|svwks_ratefilter
(paren
id|ide_drive_t
op_star
id|drive
comma
id|byte
id|speed
)paren
(brace
macro_line|#ifdef CONFIG_BLK_DEV_IDEDMA
id|byte
id|mode
op_assign
id|svwks_ratemask
c_func
(paren
id|drive
)paren
suffix:semicolon
r_switch
c_cond
(paren
id|mode
)paren
(brace
r_case
l_int|0x04
suffix:colon
r_while
c_loop
(paren
id|speed
OG
id|XFER_UDMA_6
)paren
id|speed
op_decrement
suffix:semicolon
r_break
suffix:semicolon
r_case
l_int|0x03
suffix:colon
r_while
c_loop
(paren
id|speed
OG
id|XFER_UDMA_5
)paren
id|speed
op_decrement
suffix:semicolon
r_break
suffix:semicolon
r_case
l_int|0x02
suffix:colon
r_while
c_loop
(paren
id|speed
OG
id|XFER_UDMA_4
)paren
id|speed
op_decrement
suffix:semicolon
r_break
suffix:semicolon
r_case
l_int|0x01
suffix:colon
r_while
c_loop
(paren
id|speed
OG
id|XFER_UDMA_2
)paren
id|speed
op_decrement
suffix:semicolon
r_break
suffix:semicolon
r_case
l_int|0x00
suffix:colon
r_default
suffix:colon
(brace
)brace
r_while
c_loop
(paren
id|speed
OG
id|XFER_MW_DMA_2
)paren
id|speed
op_decrement
suffix:semicolon
r_break
suffix:semicolon
r_break
suffix:semicolon
)brace
macro_line|#else
r_while
c_loop
(paren
id|speed
OG
id|XFER_PIO_4
)paren
id|speed
op_decrement
suffix:semicolon
macro_line|#endif /* CONFIG_BLK_DEV_IDEDMA */
singleline_comment|//&t;printk(&quot;%s: mode == %02x speed == %02x&bslash;n&quot;, drive-&gt;name, mode, speed);
r_return
id|speed
suffix:semicolon
)brace
DECL|function|svwks_csb_check
r_static
id|byte
id|svwks_csb_check
(paren
r_struct
id|pci_dev
op_star
id|dev
)paren
(brace
r_switch
c_cond
(paren
id|dev-&gt;device
)paren
(brace
r_case
id|PCI_DEVICE_ID_SERVERWORKS_CSB5IDE
suffix:colon
r_case
id|PCI_DEVICE_ID_SERVERWORKS_CSB6IDE
suffix:colon
r_return
l_int|1
suffix:semicolon
r_default
suffix:colon
r_break
suffix:semicolon
)brace
r_return
l_int|0
suffix:semicolon
)brace
DECL|function|svwks_tune_chipset
r_static
r_int
id|svwks_tune_chipset
(paren
id|ide_drive_t
op_star
id|drive
comma
id|byte
id|xferspeed
)paren
(brace
id|byte
id|udma_modes
(braket
)braket
op_assign
(brace
l_int|0x00
comma
l_int|0x01
comma
l_int|0x02
comma
l_int|0x03
comma
l_int|0x04
comma
l_int|0x05
)brace
suffix:semicolon
id|byte
id|dma_modes
(braket
)braket
op_assign
(brace
l_int|0x77
comma
l_int|0x21
comma
l_int|0x20
)brace
suffix:semicolon
id|byte
id|pio_modes
(braket
)braket
op_assign
(brace
l_int|0x5d
comma
l_int|0x47
comma
l_int|0x34
comma
l_int|0x22
comma
l_int|0x20
)brace
suffix:semicolon
id|ide_hwif_t
op_star
id|hwif
op_assign
id|HWIF
c_func
(paren
id|drive
)paren
suffix:semicolon
r_struct
id|pci_dev
op_star
id|dev
op_assign
id|hwif-&gt;pci_dev
suffix:semicolon
id|byte
id|unit
op_assign
(paren
id|drive-&gt;select.b.unit
op_amp
l_int|0x01
)paren
suffix:semicolon
id|byte
id|csb5
op_assign
id|svwks_csb_check
c_func
(paren
id|dev
)paren
suffix:semicolon
id|byte
id|drive_pci
op_assign
l_int|0x00
suffix:semicolon
id|byte
id|drive_pci2
op_assign
l_int|0x00
suffix:semicolon
id|byte
id|drive_pci3
op_assign
id|hwif-&gt;channel
ques
c_cond
l_int|0x57
suffix:colon
l_int|0x56
suffix:semicolon
id|byte
id|ultra_enable
op_assign
l_int|0x00
suffix:semicolon
id|byte
id|ultra_timing
op_assign
l_int|0x00
suffix:semicolon
id|byte
id|dma_timing
op_assign
l_int|0x00
suffix:semicolon
id|byte
id|pio_timing
op_assign
l_int|0x00
suffix:semicolon
r_int
r_int
id|csb5_pio
op_assign
l_int|0x00
suffix:semicolon
id|byte
id|pio
op_assign
id|ide_get_best_pio_mode
c_func
(paren
id|drive
comma
l_int|255
comma
l_int|5
comma
l_int|NULL
)paren
suffix:semicolon
id|byte
id|speed
op_assign
id|svwks_ratefilter
c_func
(paren
id|drive
comma
id|xferspeed
)paren
suffix:semicolon
r_switch
c_cond
(paren
id|drive-&gt;dn
)paren
(brace
r_case
l_int|0
suffix:colon
id|drive_pci
op_assign
l_int|0x41
suffix:semicolon
id|drive_pci2
op_assign
l_int|0x45
suffix:semicolon
r_break
suffix:semicolon
r_case
l_int|1
suffix:colon
id|drive_pci
op_assign
l_int|0x40
suffix:semicolon
id|drive_pci2
op_assign
l_int|0x44
suffix:semicolon
r_break
suffix:semicolon
r_case
l_int|2
suffix:colon
id|drive_pci
op_assign
l_int|0x43
suffix:semicolon
id|drive_pci2
op_assign
l_int|0x47
suffix:semicolon
r_break
suffix:semicolon
r_case
l_int|3
suffix:colon
id|drive_pci
op_assign
l_int|0x42
suffix:semicolon
id|drive_pci2
op_assign
l_int|0x46
suffix:semicolon
r_break
suffix:semicolon
r_default
suffix:colon
r_return
op_minus
l_int|1
suffix:semicolon
)brace
id|pci_read_config_byte
c_func
(paren
id|dev
comma
id|drive_pci
comma
op_amp
id|pio_timing
)paren
suffix:semicolon
id|pci_read_config_byte
c_func
(paren
id|dev
comma
id|drive_pci2
comma
op_amp
id|dma_timing
)paren
suffix:semicolon
id|pci_read_config_byte
c_func
(paren
id|dev
comma
id|drive_pci3
comma
op_amp
id|ultra_timing
)paren
suffix:semicolon
id|pci_read_config_word
c_func
(paren
id|dev
comma
l_int|0x4A
comma
op_amp
id|csb5_pio
)paren
suffix:semicolon
id|pci_read_config_byte
c_func
(paren
id|dev
comma
l_int|0x54
comma
op_amp
id|ultra_enable
)paren
suffix:semicolon
id|pio_timing
op_and_assign
op_complement
l_int|0xFF
suffix:semicolon
id|dma_timing
op_and_assign
op_complement
l_int|0xFF
suffix:semicolon
id|ultra_timing
op_and_assign
op_complement
(paren
l_int|0x0F
op_lshift
(paren
l_int|4
op_star
id|unit
)paren
)paren
suffix:semicolon
id|ultra_enable
op_and_assign
op_complement
(paren
l_int|0x01
op_lshift
id|drive-&gt;dn
)paren
suffix:semicolon
id|csb5_pio
op_and_assign
op_complement
(paren
l_int|0x0F
op_lshift
(paren
l_int|4
op_star
id|drive-&gt;dn
)paren
)paren
suffix:semicolon
r_switch
c_cond
(paren
id|speed
)paren
(brace
r_case
id|XFER_PIO_4
suffix:colon
r_case
id|XFER_PIO_3
suffix:colon
r_case
id|XFER_PIO_2
suffix:colon
r_case
id|XFER_PIO_1
suffix:colon
r_case
id|XFER_PIO_0
suffix:colon
id|pio_timing
op_or_assign
id|pio_modes
(braket
id|speed
op_minus
id|XFER_PIO_0
)braket
suffix:semicolon
id|csb5_pio
op_or_assign
(paren
(paren
id|speed
op_minus
id|XFER_PIO_0
)paren
op_lshift
(paren
l_int|4
op_star
id|drive-&gt;dn
)paren
)paren
suffix:semicolon
r_break
suffix:semicolon
macro_line|#ifdef CONFIG_BLK_DEV_IDEDMA
r_case
id|XFER_MW_DMA_2
suffix:colon
r_case
id|XFER_MW_DMA_1
suffix:colon
r_case
id|XFER_MW_DMA_0
suffix:colon
id|pio_timing
op_or_assign
id|pio_modes
(braket
id|pio
)braket
suffix:semicolon
id|csb5_pio
op_or_assign
(paren
id|pio
op_lshift
(paren
l_int|4
op_star
id|drive-&gt;dn
)paren
)paren
suffix:semicolon
id|dma_timing
op_or_assign
id|dma_modes
(braket
id|speed
op_minus
id|XFER_MW_DMA_0
)braket
suffix:semicolon
r_break
suffix:semicolon
r_case
id|XFER_UDMA_5
suffix:colon
r_case
id|XFER_UDMA_4
suffix:colon
r_case
id|XFER_UDMA_3
suffix:colon
r_case
id|XFER_UDMA_2
suffix:colon
r_case
id|XFER_UDMA_1
suffix:colon
r_case
id|XFER_UDMA_0
suffix:colon
id|pio_timing
op_or_assign
id|pio_modes
(braket
id|pio
)braket
suffix:semicolon
id|csb5_pio
op_or_assign
(paren
id|pio
op_lshift
(paren
l_int|4
op_star
id|drive-&gt;dn
)paren
)paren
suffix:semicolon
id|dma_timing
op_or_assign
id|dma_modes
(braket
l_int|2
)braket
suffix:semicolon
id|ultra_timing
op_or_assign
(paren
(paren
id|udma_modes
(braket
id|speed
op_minus
id|XFER_UDMA_0
)braket
)paren
op_lshift
(paren
l_int|4
op_star
id|unit
)paren
)paren
suffix:semicolon
id|ultra_enable
op_or_assign
(paren
l_int|0x01
op_lshift
id|drive-&gt;dn
)paren
suffix:semicolon
macro_line|#endif
r_default
suffix:colon
r_break
suffix:semicolon
)brace
id|pci_write_config_byte
c_func
(paren
id|dev
comma
id|drive_pci
comma
id|pio_timing
)paren
suffix:semicolon
r_if
c_cond
(paren
id|csb5
)paren
id|pci_write_config_word
c_func
(paren
id|dev
comma
l_int|0x4A
comma
id|csb5_pio
)paren
suffix:semicolon
macro_line|#ifdef CONFIG_BLK_DEV_IDEDMA
id|pci_write_config_byte
c_func
(paren
id|dev
comma
id|drive_pci2
comma
id|dma_timing
)paren
suffix:semicolon
id|pci_write_config_byte
c_func
(paren
id|dev
comma
id|drive_pci3
comma
id|ultra_timing
)paren
suffix:semicolon
id|pci_write_config_byte
c_func
(paren
id|dev
comma
l_int|0x54
comma
id|ultra_enable
)paren
suffix:semicolon
macro_line|#endif /* CONFIG_BLK_DEV_IDEDMA */
r_return
(paren
id|ide_config_drive_speed
c_func
(paren
id|drive
comma
id|speed
)paren
)paren
suffix:semicolon
)brace
DECL|function|config_chipset_for_pio
r_static
r_void
id|config_chipset_for_pio
(paren
id|ide_drive_t
op_star
id|drive
)paren
(brace
r_int
r_int
id|eide_pio_timing
(braket
l_int|6
)braket
op_assign
(brace
l_int|960
comma
l_int|480
comma
l_int|240
comma
l_int|180
comma
l_int|120
comma
l_int|90
)brace
suffix:semicolon
r_int
r_int
id|xfer_pio
op_assign
id|drive-&gt;id-&gt;eide_pio_modes
suffix:semicolon
id|byte
id|timing
comma
id|speed
comma
id|pio
suffix:semicolon
id|pio
op_assign
id|ide_get_best_pio_mode
c_func
(paren
id|drive
comma
l_int|255
comma
l_int|5
comma
l_int|NULL
)paren
suffix:semicolon
r_if
c_cond
(paren
id|xfer_pio
OG
l_int|4
)paren
id|xfer_pio
op_assign
l_int|0
suffix:semicolon
r_if
c_cond
(paren
id|drive-&gt;id-&gt;eide_pio_iordy
OG
l_int|0
)paren
r_for
c_loop
(paren
id|xfer_pio
op_assign
l_int|5
suffix:semicolon
id|xfer_pio
OG
l_int|0
op_logical_and
id|drive-&gt;id-&gt;eide_pio_iordy
OG
id|eide_pio_timing
(braket
id|xfer_pio
)braket
suffix:semicolon
id|xfer_pio
op_decrement
)paren
suffix:semicolon
r_else
id|xfer_pio
op_assign
(paren
id|drive-&gt;id-&gt;eide_pio_modes
op_amp
l_int|4
)paren
ques
c_cond
l_int|0x05
suffix:colon
(paren
id|drive-&gt;id-&gt;eide_pio_modes
op_amp
l_int|2
)paren
ques
c_cond
l_int|0x04
suffix:colon
(paren
id|drive-&gt;id-&gt;eide_pio_modes
op_amp
l_int|1
)paren
ques
c_cond
l_int|0x03
suffix:colon
(paren
id|drive-&gt;id-&gt;tPIO
op_amp
l_int|2
)paren
ques
c_cond
l_int|0x02
suffix:colon
(paren
id|drive-&gt;id-&gt;tPIO
op_amp
l_int|1
)paren
ques
c_cond
l_int|0x01
suffix:colon
id|xfer_pio
suffix:semicolon
id|timing
op_assign
(paren
id|xfer_pio
op_ge
id|pio
)paren
ques
c_cond
id|xfer_pio
suffix:colon
id|pio
suffix:semicolon
r_switch
c_cond
(paren
id|timing
)paren
(brace
r_case
l_int|4
suffix:colon
id|speed
op_assign
id|XFER_PIO_4
suffix:semicolon
r_break
suffix:semicolon
r_case
l_int|3
suffix:colon
id|speed
op_assign
id|XFER_PIO_3
suffix:semicolon
r_break
suffix:semicolon
r_case
l_int|2
suffix:colon
id|speed
op_assign
id|XFER_PIO_2
suffix:semicolon
r_break
suffix:semicolon
r_case
l_int|1
suffix:colon
id|speed
op_assign
id|XFER_PIO_1
suffix:semicolon
r_break
suffix:semicolon
r_default
suffix:colon
id|speed
op_assign
(paren
op_logical_neg
id|drive-&gt;id-&gt;tPIO
)paren
ques
c_cond
id|XFER_PIO_0
suffix:colon
id|XFER_PIO_SLOW
suffix:semicolon
r_break
suffix:semicolon
)brace
(paren
r_void
)paren
id|svwks_tune_chipset
c_func
(paren
id|drive
comma
id|speed
)paren
suffix:semicolon
id|drive-&gt;current_speed
op_assign
id|speed
suffix:semicolon
)brace
DECL|function|svwks_tune_drive
r_static
r_void
id|svwks_tune_drive
(paren
id|ide_drive_t
op_star
id|drive
comma
id|byte
id|pio
)paren
(brace
id|byte
id|speed
suffix:semicolon
r_switch
c_cond
(paren
id|pio
)paren
(brace
r_case
l_int|4
suffix:colon
id|speed
op_assign
id|XFER_PIO_4
suffix:semicolon
r_break
suffix:semicolon
r_case
l_int|3
suffix:colon
id|speed
op_assign
id|XFER_PIO_3
suffix:semicolon
r_break
suffix:semicolon
r_case
l_int|2
suffix:colon
id|speed
op_assign
id|XFER_PIO_2
suffix:semicolon
r_break
suffix:semicolon
r_case
l_int|1
suffix:colon
id|speed
op_assign
id|XFER_PIO_1
suffix:semicolon
r_break
suffix:semicolon
r_default
suffix:colon
id|speed
op_assign
id|XFER_PIO_0
suffix:semicolon
r_break
suffix:semicolon
)brace
(paren
r_void
)paren
id|svwks_tune_chipset
c_func
(paren
id|drive
comma
id|speed
)paren
suffix:semicolon
)brace
macro_line|#ifdef CONFIG_BLK_DEV_IDEDMA
DECL|function|config_chipset_for_dma
r_static
r_int
id|config_chipset_for_dma
(paren
id|ide_drive_t
op_star
id|drive
)paren
(brace
r_struct
id|hd_driveid
op_star
id|id
op_assign
id|drive-&gt;id
suffix:semicolon
id|byte
id|mode
op_assign
id|svwks_ratemask
c_func
(paren
id|drive
)paren
suffix:semicolon
id|byte
id|speed
comma
id|dma
op_assign
l_int|1
suffix:semicolon
r_if
c_cond
(paren
id|HWIF
c_func
(paren
id|drive
)paren
op_member_access_from_pointer
id|pci_dev-&gt;device
op_eq
id|PCI_DEVICE_ID_SERVERWORKS_OSB4IDE
)paren
id|mode
op_assign
l_int|0
suffix:semicolon
r_switch
c_cond
(paren
id|mode
)paren
(brace
r_case
l_int|0x04
suffix:colon
r_if
c_cond
(paren
id|id-&gt;dma_ultra
op_amp
l_int|0x0040
)paren
(brace
id|speed
op_assign
id|XFER_UDMA_6
suffix:semicolon
r_break
suffix:semicolon
)brace
r_case
l_int|0x03
suffix:colon
r_if
c_cond
(paren
id|id-&gt;dma_ultra
op_amp
l_int|0x0020
)paren
(brace
id|speed
op_assign
id|XFER_UDMA_5
suffix:semicolon
r_break
suffix:semicolon
)brace
r_case
l_int|0x02
suffix:colon
r_if
c_cond
(paren
id|id-&gt;dma_ultra
op_amp
l_int|0x0010
)paren
(brace
id|speed
op_assign
id|XFER_UDMA_4
suffix:semicolon
r_break
suffix:semicolon
)brace
r_if
c_cond
(paren
id|id-&gt;dma_ultra
op_amp
l_int|0x0008
)paren
(brace
id|speed
op_assign
id|XFER_UDMA_3
suffix:semicolon
r_break
suffix:semicolon
)brace
r_case
l_int|0x01
suffix:colon
r_if
c_cond
(paren
id|id-&gt;dma_ultra
op_amp
l_int|0x0004
)paren
(brace
id|speed
op_assign
id|XFER_UDMA_2
suffix:semicolon
r_break
suffix:semicolon
)brace
r_if
c_cond
(paren
id|id-&gt;dma_ultra
op_amp
l_int|0x0002
)paren
(brace
id|speed
op_assign
id|XFER_UDMA_1
suffix:semicolon
r_break
suffix:semicolon
)brace
r_if
c_cond
(paren
id|id-&gt;dma_ultra
op_amp
l_int|0x0001
)paren
(brace
id|speed
op_assign
id|XFER_UDMA_0
suffix:semicolon
r_break
suffix:semicolon
)brace
r_if
c_cond
(paren
id|id-&gt;dma_mword
op_amp
l_int|0x0004
)paren
(brace
id|speed
op_assign
id|XFER_MW_DMA_2
suffix:semicolon
r_break
suffix:semicolon
)brace
r_if
c_cond
(paren
id|id-&gt;dma_mword
op_amp
l_int|0x0002
)paren
(brace
id|speed
op_assign
id|XFER_MW_DMA_1
suffix:semicolon
r_break
suffix:semicolon
)brace
r_if
c_cond
(paren
id|id-&gt;dma_mword
op_amp
l_int|0x0001
)paren
(brace
id|speed
op_assign
id|XFER_MW_DMA_0
suffix:semicolon
r_break
suffix:semicolon
)brace
macro_line|#if 0
r_if
c_cond
(paren
id|id-&gt;dma_1word
op_amp
l_int|0x0004
)paren
(brace
id|speed
op_assign
id|XFER_SW_DMA_2
suffix:semicolon
r_break
suffix:semicolon
)brace
r_if
c_cond
(paren
id|id-&gt;dma_1word
op_amp
l_int|0x0002
)paren
(brace
id|speed
op_assign
id|XFER_SW_DMA_1
suffix:semicolon
r_break
suffix:semicolon
)brace
r_if
c_cond
(paren
id|id-&gt;dma_1word
op_amp
l_int|0x0001
)paren
(brace
id|speed
op_assign
id|XFER_SW_DMA_0
suffix:semicolon
r_break
suffix:semicolon
)brace
macro_line|#endif
r_default
suffix:colon
id|speed
op_assign
id|XFER_PIO_0
op_plus
id|ide_get_best_pio_mode
c_func
(paren
id|drive
comma
l_int|255
comma
l_int|5
comma
l_int|NULL
)paren
suffix:semicolon
id|dma
op_assign
l_int|0
suffix:semicolon
r_break
suffix:semicolon
)brace
(paren
r_void
)paren
id|svwks_tune_chipset
c_func
(paren
id|drive
comma
id|speed
)paren
suffix:semicolon
singleline_comment|//&t;return ((int) (dma) ? ide_dma_on : ide_dma_off_quietly);
r_return
(paren
(paren
r_int
)paren
(paren
(paren
id|id-&gt;dma_ultra
op_rshift
l_int|11
)paren
op_amp
l_int|7
)paren
ques
c_cond
id|ide_dma_on
suffix:colon
(paren
(paren
id|id-&gt;dma_ultra
op_rshift
l_int|8
)paren
op_amp
l_int|7
)paren
ques
c_cond
id|ide_dma_on
suffix:colon
(paren
(paren
id|id-&gt;dma_mword
op_rshift
l_int|8
)paren
op_amp
l_int|7
)paren
ques
c_cond
id|ide_dma_on
suffix:colon
(paren
(paren
id|id-&gt;dma_1word
op_rshift
l_int|8
)paren
op_amp
l_int|7
)paren
ques
c_cond
id|ide_dma_on
suffix:colon
id|ide_dma_off_quietly
)paren
suffix:semicolon
)brace
DECL|function|config_drive_xfer_rate
r_static
r_int
id|config_drive_xfer_rate
(paren
id|ide_drive_t
op_star
id|drive
)paren
(brace
r_struct
id|hd_driveid
op_star
id|id
op_assign
id|drive-&gt;id
suffix:semicolon
id|ide_dma_action_t
id|dma_func
op_assign
id|ide_dma_on
suffix:semicolon
id|drive-&gt;init_speed
op_assign
l_int|0
suffix:semicolon
r_if
c_cond
(paren
id|id
op_logical_and
(paren
id|id-&gt;capability
op_amp
l_int|1
)paren
op_logical_and
id|HWIF
c_func
(paren
id|drive
)paren
op_member_access_from_pointer
id|autodma
)paren
(brace
multiline_comment|/* Consult the list of known &quot;bad&quot; drives */
r_if
c_cond
(paren
id|ide_dmaproc
c_func
(paren
id|ide_dma_bad_drive
comma
id|drive
)paren
)paren
(brace
id|dma_func
op_assign
id|ide_dma_off
suffix:semicolon
r_goto
id|fast_ata_pio
suffix:semicolon
)brace
id|dma_func
op_assign
id|ide_dma_off_quietly
suffix:semicolon
r_if
c_cond
(paren
id|id-&gt;field_valid
op_amp
l_int|4
)paren
(brace
r_if
c_cond
(paren
id|id-&gt;dma_ultra
op_amp
l_int|0x003F
)paren
(brace
multiline_comment|/* Force if Capable UltraDMA */
id|dma_func
op_assign
id|config_chipset_for_dma
c_func
(paren
id|drive
)paren
suffix:semicolon
r_if
c_cond
(paren
(paren
id|id-&gt;field_valid
op_amp
l_int|2
)paren
op_logical_and
(paren
id|dma_func
op_ne
id|ide_dma_on
)paren
)paren
r_goto
id|try_dma_modes
suffix:semicolon
)brace
)brace
r_else
r_if
c_cond
(paren
id|id-&gt;field_valid
op_amp
l_int|2
)paren
(brace
id|try_dma_modes
suffix:colon
r_if
c_cond
(paren
(paren
id|id-&gt;dma_mword
op_amp
l_int|0x0007
)paren
op_logical_or
(paren
id|id-&gt;dma_1word
op_amp
l_int|0x007
)paren
)paren
(brace
multiline_comment|/* Force if Capable regular DMA modes */
id|dma_func
op_assign
id|config_chipset_for_dma
c_func
(paren
id|drive
)paren
suffix:semicolon
r_if
c_cond
(paren
id|dma_func
op_ne
id|ide_dma_on
)paren
r_goto
id|no_dma_set
suffix:semicolon
)brace
)brace
r_else
r_if
c_cond
(paren
id|ide_dmaproc
c_func
(paren
id|ide_dma_good_drive
comma
id|drive
)paren
)paren
(brace
r_if
c_cond
(paren
id|id-&gt;eide_dma_time
OG
l_int|150
)paren
(brace
r_goto
id|no_dma_set
suffix:semicolon
)brace
multiline_comment|/* Consult the list of known &quot;good&quot; drives */
id|dma_func
op_assign
id|config_chipset_for_dma
c_func
(paren
id|drive
)paren
suffix:semicolon
r_if
c_cond
(paren
id|dma_func
op_ne
id|ide_dma_on
)paren
r_goto
id|no_dma_set
suffix:semicolon
)brace
r_else
(brace
r_goto
id|fast_ata_pio
suffix:semicolon
)brace
)brace
r_else
r_if
c_cond
(paren
(paren
id|id-&gt;capability
op_amp
l_int|8
)paren
op_logical_or
(paren
id|id-&gt;field_valid
op_amp
l_int|2
)paren
)paren
(brace
id|fast_ata_pio
suffix:colon
id|dma_func
op_assign
id|ide_dma_off_quietly
suffix:semicolon
id|no_dma_set
suffix:colon
id|config_chipset_for_pio
c_func
(paren
id|drive
)paren
suffix:semicolon
singleline_comment|//&t;HWIF(drive)-&gt;tuneproc(drive, 5);
)brace
r_return
id|HWIF
c_func
(paren
id|drive
)paren
op_member_access_from_pointer
id|dmaproc
c_func
(paren
id|dma_func
comma
id|drive
)paren
suffix:semicolon
)brace
DECL|function|svwks_dmaproc
r_static
r_int
id|svwks_dmaproc
c_func
(paren
id|ide_dma_action_t
id|func
comma
id|ide_drive_t
op_star
id|drive
)paren
(brace
r_switch
c_cond
(paren
id|func
)paren
(brace
r_case
id|ide_dma_check
suffix:colon
r_return
id|config_drive_xfer_rate
c_func
(paren
id|drive
)paren
suffix:semicolon
r_case
id|ide_dma_end
suffix:colon
(brace
id|ide_hwif_t
op_star
id|hwif
op_assign
id|HWIF
c_func
(paren
id|drive
)paren
suffix:semicolon
r_int
r_int
id|dma_base
op_assign
id|hwif-&gt;dma_base
suffix:semicolon
r_if
c_cond
(paren
id|IN_BYTE
c_func
(paren
id|dma_base
op_plus
l_int|0x02
)paren
op_amp
l_int|1
)paren
(brace
macro_line|#if 0&t;&t;
r_int
id|i
suffix:semicolon
id|printk
c_func
(paren
id|KERN_ERR
l_string|&quot;Curious - OSB4 thinks the DMA is still running.&bslash;n&quot;
)paren
suffix:semicolon
r_for
c_loop
(paren
id|i
op_assign
l_int|0
suffix:semicolon
id|i
OL
l_int|10
suffix:semicolon
id|i
op_increment
)paren
(brace
r_if
c_cond
(paren
op_logical_neg
(paren
id|IN_BYTE
c_func
(paren
id|dma_base
op_plus
l_int|0x02
)paren
op_amp
l_int|1
)paren
)paren
(brace
id|printk
c_func
(paren
id|KERN_ERR
l_string|&quot;OSB4 now finished.&bslash;n&quot;
)paren
suffix:semicolon
r_break
suffix:semicolon
)brace
id|udelay
c_func
(paren
l_int|5
)paren
suffix:semicolon
)brace
macro_line|#endif&t;&t;
id|printk
c_func
(paren
id|KERN_CRIT
l_string|&quot;Serverworks OSB4 in impossible state.&bslash;n&quot;
)paren
suffix:semicolon
id|printk
c_func
(paren
id|KERN_CRIT
l_string|&quot;Disable UDMA or if you are using Seagate then try switching disk types&bslash;n&quot;
)paren
suffix:semicolon
id|printk
c_func
(paren
id|KERN_CRIT
l_string|&quot;on this controller. Please report this event to osb4-bug@ide.cabal.tm&bslash;n&quot;
)paren
suffix:semicolon
macro_line|#if 0&t;&t;
multiline_comment|/* Panic might sys_sync -&gt; death by corrupt disk */
id|panic
c_func
(paren
l_string|&quot;OSB4: continuing might cause disk corruption.&bslash;n&quot;
)paren
suffix:semicolon
macro_line|#else
id|printk
c_func
(paren
id|KERN_CRIT
l_string|&quot;OSB4: continuing might cause disk corruption.&bslash;n&quot;
)paren
suffix:semicolon
r_while
c_loop
(paren
l_int|1
)paren
(brace
id|cpu_relax
c_func
(paren
)paren
suffix:semicolon
)brace
macro_line|#endif&t;&t;&t;&t;
)brace
multiline_comment|/* and drop through */
)brace
r_default
suffix:colon
r_break
suffix:semicolon
)brace
multiline_comment|/* Other cases are done by generic IDE-DMA code. */
r_return
id|ide_dmaproc
c_func
(paren
id|func
comma
id|drive
)paren
suffix:semicolon
)brace
macro_line|#endif /* CONFIG_BLK_DEV_IDEDMA */
DECL|function|pci_init_svwks
r_int
r_int
id|__init
id|pci_init_svwks
(paren
r_struct
id|pci_dev
op_star
id|dev
comma
r_const
r_char
op_star
id|name
)paren
(brace
r_int
r_int
id|reg
suffix:semicolon
id|byte
id|btr
suffix:semicolon
multiline_comment|/* save revision id to determine DMA capability */
id|pci_read_config_byte
c_func
(paren
id|dev
comma
id|PCI_REVISION_ID
comma
op_amp
id|svwks_revision
)paren
suffix:semicolon
multiline_comment|/* force Master Latency Timer value to 64 PCICLKs */
id|pci_write_config_byte
c_func
(paren
id|dev
comma
id|PCI_LATENCY_TIMER
comma
l_int|0x40
)paren
suffix:semicolon
multiline_comment|/* OSB4 : South Bridge and IDE */
r_if
c_cond
(paren
id|dev-&gt;device
op_eq
id|PCI_DEVICE_ID_SERVERWORKS_OSB4IDE
)paren
(brace
id|isa_dev
op_assign
id|pci_find_device
c_func
(paren
id|PCI_VENDOR_ID_SERVERWORKS
comma
id|PCI_DEVICE_ID_SERVERWORKS_OSB4
comma
l_int|NULL
)paren
suffix:semicolon
r_if
c_cond
(paren
id|isa_dev
)paren
(brace
id|pci_read_config_dword
c_func
(paren
id|isa_dev
comma
l_int|0x64
comma
op_amp
id|reg
)paren
suffix:semicolon
id|reg
op_and_assign
op_complement
l_int|0x00002000
suffix:semicolon
multiline_comment|/* disable 600ns interrupt mask */
id|reg
op_or_assign
l_int|0x00004000
suffix:semicolon
multiline_comment|/* enable UDMA/33 support */
id|pci_write_config_dword
c_func
(paren
id|isa_dev
comma
l_int|0x64
comma
id|reg
)paren
suffix:semicolon
)brace
)brace
multiline_comment|/* setup CSB5/CSB6 : South Bridge and IDE option RAID */
r_else
r_if
c_cond
(paren
(paren
id|dev-&gt;device
op_eq
id|PCI_DEVICE_ID_SERVERWORKS_CSB5IDE
)paren
op_logical_or
(paren
id|dev-&gt;device
op_eq
id|PCI_DEVICE_ID_SERVERWORKS_CSB6IDE
)paren
)paren
(brace
multiline_comment|/* Third Channel Test */
r_if
c_cond
(paren
op_logical_neg
(paren
id|PCI_FUNC
c_func
(paren
id|dev-&gt;devfn
)paren
op_amp
l_int|1
)paren
)paren
(brace
macro_line|#if 1
r_struct
id|pci_dev
op_star
id|findev
op_assign
l_int|NULL
suffix:semicolon
r_int
r_int
id|reg4c
op_assign
l_int|0
suffix:semicolon
id|findev
op_assign
id|pci_find_device
c_func
(paren
id|PCI_VENDOR_ID_SERVERWORKS
comma
id|PCI_DEVICE_ID_SERVERWORKS_CSB5
comma
l_int|NULL
)paren
suffix:semicolon
r_if
c_cond
(paren
id|findev
)paren
(brace
id|pci_read_config_dword
c_func
(paren
id|findev
comma
l_int|0x4C
comma
op_amp
id|reg4c
)paren
suffix:semicolon
id|reg4c
op_and_assign
op_complement
l_int|0x000007FF
suffix:semicolon
id|reg4c
op_or_assign
l_int|0x00000040
suffix:semicolon
id|reg4c
op_or_assign
l_int|0x00000020
suffix:semicolon
id|pci_write_config_dword
c_func
(paren
id|findev
comma
l_int|0x4C
comma
id|reg4c
)paren
suffix:semicolon
)brace
macro_line|#endif
id|outb_p
c_func
(paren
l_int|0x06
comma
l_int|0x0c00
)paren
suffix:semicolon
id|dev-&gt;irq
op_assign
id|inb_p
c_func
(paren
l_int|0x0c01
)paren
suffix:semicolon
macro_line|#if 1
multiline_comment|/* WE need to figure out how to get the correct one */
id|printk
c_func
(paren
l_string|&quot;%s: interrupt %d&bslash;n&quot;
comma
id|name
comma
id|dev-&gt;irq
)paren
suffix:semicolon
r_if
c_cond
(paren
id|dev-&gt;irq
op_ne
l_int|0x0B
)paren
id|dev-&gt;irq
op_assign
l_int|0x0B
suffix:semicolon
macro_line|#endif
)brace
r_else
(brace
multiline_comment|/*&n;&t;&t;&t; * This is a device pin issue on CSB6.&n;&t;&t;&t; * Since there will be a future raid mode,&n;&t;&t;&t; * early versions of the chipset require the&n;&t;&t;&t; * interrupt pin to be set, and it is a compatablity&n;&t;&t;&t; * mode issue.&n;&t;&t;&t; */
id|dev-&gt;irq
op_assign
l_int|0
suffix:semicolon
)brace
id|pci_write_config_dword
c_func
(paren
id|dev
comma
l_int|0x40
comma
l_int|0x99999999
)paren
suffix:semicolon
id|pci_write_config_dword
c_func
(paren
id|dev
comma
l_int|0x44
comma
l_int|0xFFFFFFFF
)paren
suffix:semicolon
multiline_comment|/* setup the UDMA Control register&n;&t;&t; *&n;&t;&t; * 1. clear bit 6 to enable DMA&n;&t;&t; * 2. enable DMA modes with bits 0-1&n;&t;&t; * &t;00 : legacy&n;&t;&t; * &t;01 : udma2&n;&t;&t; * &t;10 : udma2/udma4&n;&t;&t; * &t;11 : udma2/udma4/udma5&n;&t;&t; */
id|pci_read_config_byte
c_func
(paren
id|dev
comma
l_int|0x5A
comma
op_amp
id|btr
)paren
suffix:semicolon
id|btr
op_and_assign
op_complement
l_int|0x40
suffix:semicolon
r_if
c_cond
(paren
op_logical_neg
(paren
id|PCI_FUNC
c_func
(paren
id|dev-&gt;devfn
)paren
op_amp
l_int|1
)paren
)paren
id|btr
op_or_assign
l_int|0x2
suffix:semicolon
r_else
id|btr
op_or_assign
(paren
id|svwks_revision
op_ge
id|SVWKS_CSB5_REVISION_NEW
)paren
ques
c_cond
l_int|0x3
suffix:colon
l_int|0x2
suffix:semicolon
id|pci_write_config_byte
c_func
(paren
id|dev
comma
l_int|0x5A
comma
id|btr
)paren
suffix:semicolon
)brace
id|svwks_devs
(braket
id|n_svwks_devs
op_increment
)braket
op_assign
id|dev
suffix:semicolon
macro_line|#if defined(DISPLAY_SVWKS_TIMINGS) &amp;&amp; defined(CONFIG_PROC_FS)
r_if
c_cond
(paren
op_logical_neg
id|svwks_proc
)paren
(brace
id|svwks_proc
op_assign
l_int|1
suffix:semicolon
id|svwks_display_info
op_assign
op_amp
id|svwks_get_info
suffix:semicolon
)brace
macro_line|#endif /* DISPLAY_SVWKS_TIMINGS &amp;&amp; CONFIG_PROC_FS */
r_return
(paren
id|dev-&gt;irq
)paren
ques
c_cond
id|dev-&gt;irq
suffix:colon
l_int|0
suffix:semicolon
)brace
DECL|function|ata66_svwks_svwks
r_static
r_int
r_int
id|__init
id|ata66_svwks_svwks
(paren
id|ide_hwif_t
op_star
id|hwif
)paren
(brace
singleline_comment|//&t;struct pci_dev *dev = hwif-&gt;pci_dev;
singleline_comment|//&t;return 0;
r_return
l_int|1
suffix:semicolon
)brace
multiline_comment|/* On Dell PowerEdge servers with a CSB5/CSB6, the top two bits&n; * of the subsystem device ID indicate presence of an 80-pin cable.&n; * Bit 15 clear = secondary IDE channel does not have 80-pin cable.&n; * Bit 15 set   = secondary IDE channel has 80-pin cable.&n; * Bit 14 clear = primary IDE channel does not have 80-pin cable.&n; * Bit 14 set   = primary IDE channel has 80-pin cable.&n; */
DECL|function|ata66_svwks_dell
r_static
r_int
r_int
id|__init
id|ata66_svwks_dell
(paren
id|ide_hwif_t
op_star
id|hwif
)paren
(brace
r_struct
id|pci_dev
op_star
id|dev
op_assign
id|hwif-&gt;pci_dev
suffix:semicolon
r_if
c_cond
(paren
id|dev-&gt;subsystem_vendor
op_eq
id|PCI_VENDOR_ID_DELL
op_logical_and
id|dev-&gt;vendor
op_eq
id|PCI_VENDOR_ID_SERVERWORKS
op_logical_and
(paren
id|dev-&gt;device
op_eq
id|PCI_DEVICE_ID_SERVERWORKS_CSB5IDE
op_logical_or
id|dev-&gt;device
op_eq
id|PCI_DEVICE_ID_SERVERWORKS_CSB6IDE
)paren
)paren
r_return
(paren
(paren
l_int|1
op_lshift
(paren
id|hwif-&gt;channel
op_plus
l_int|14
)paren
)paren
op_amp
id|dev-&gt;subsystem_device
)paren
ques
c_cond
l_int|1
suffix:colon
l_int|0
suffix:semicolon
r_return
l_int|0
suffix:semicolon
)brace
multiline_comment|/* Sun Cobalt Alpine hardware avoids the 80-pin cable&n; * detect issue by attaching the drives directly to the board.&n; * This check follows the Dell precedent (how scary is that?!)&n; *&n; * WARNING: this only works on Alpine hardware!&n; */
DECL|function|ata66_svwks_cobalt
r_static
r_int
r_int
id|__init
id|ata66_svwks_cobalt
(paren
id|ide_hwif_t
op_star
id|hwif
)paren
(brace
r_struct
id|pci_dev
op_star
id|dev
op_assign
id|hwif-&gt;pci_dev
suffix:semicolon
r_if
c_cond
(paren
id|dev-&gt;subsystem_vendor
op_eq
id|PCI_VENDOR_ID_SUN
op_logical_and
id|dev-&gt;vendor
op_eq
id|PCI_VENDOR_ID_SERVERWORKS
op_logical_and
id|dev-&gt;device
op_eq
id|PCI_DEVICE_ID_SERVERWORKS_CSB5IDE
)paren
r_return
(paren
(paren
l_int|1
op_lshift
(paren
id|hwif-&gt;channel
op_plus
l_int|14
)paren
)paren
op_amp
id|dev-&gt;subsystem_device
)paren
ques
c_cond
l_int|1
suffix:colon
l_int|0
suffix:semicolon
r_return
l_int|0
suffix:semicolon
)brace
DECL|function|ata66_svwks
r_int
r_int
id|__init
id|ata66_svwks
(paren
id|ide_hwif_t
op_star
id|hwif
)paren
(brace
r_struct
id|pci_dev
op_star
id|dev
op_assign
id|hwif-&gt;pci_dev
suffix:semicolon
multiline_comment|/* Server Works */
r_if
c_cond
(paren
id|dev-&gt;subsystem_vendor
op_eq
id|PCI_VENDOR_ID_SERVERWORKS
)paren
r_return
id|ata66_svwks_svwks
(paren
id|hwif
)paren
suffix:semicolon
multiline_comment|/* Dell PowerEdge */
r_if
c_cond
(paren
id|dev-&gt;subsystem_vendor
op_eq
id|PCI_VENDOR_ID_DELL
)paren
r_return
id|ata66_svwks_dell
(paren
id|hwif
)paren
suffix:semicolon
multiline_comment|/* Cobalt Alpine */
r_if
c_cond
(paren
id|dev-&gt;subsystem_vendor
op_eq
id|PCI_VENDOR_ID_SUN
)paren
r_return
id|ata66_svwks_cobalt
(paren
id|hwif
)paren
suffix:semicolon
r_return
l_int|0
suffix:semicolon
)brace
DECL|function|ide_init_svwks
r_void
id|__init
id|ide_init_svwks
(paren
id|ide_hwif_t
op_star
id|hwif
)paren
(brace
r_if
c_cond
(paren
op_logical_neg
id|hwif-&gt;irq
)paren
id|hwif-&gt;irq
op_assign
id|hwif-&gt;channel
ques
c_cond
l_int|15
suffix:colon
l_int|14
suffix:semicolon
id|hwif-&gt;tuneproc
op_assign
op_amp
id|svwks_tune_drive
suffix:semicolon
id|hwif-&gt;speedproc
op_assign
op_amp
id|svwks_tune_chipset
suffix:semicolon
id|hwif-&gt;drives
(braket
l_int|0
)braket
dot
id|autotune
op_assign
l_int|1
suffix:semicolon
id|hwif-&gt;drives
(braket
l_int|1
)braket
dot
id|autotune
op_assign
l_int|1
suffix:semicolon
id|hwif-&gt;autodma
op_assign
l_int|0
suffix:semicolon
r_if
c_cond
(paren
op_logical_neg
id|hwif-&gt;dma_base
)paren
r_return
suffix:semicolon
macro_line|#ifdef CONFIG_BLK_DEV_IDEDMA
id|hwif-&gt;dmaproc
op_assign
op_amp
id|svwks_dmaproc
suffix:semicolon
macro_line|# ifdef CONFIG_IDEDMA_AUTO
r_if
c_cond
(paren
op_logical_neg
id|noautodma
)paren
id|hwif-&gt;autodma
op_assign
l_int|1
suffix:semicolon
macro_line|# endif /* CONFIG_IDEDMA_AUTO */
macro_line|#endif /* !CONFIG_BLK_DEV_IDEDMA */
)brace
multiline_comment|/*&n; * We allow the BM-DMA driver to only work on enabled interfaces.&n; */
DECL|function|ide_dmacapable_svwks
r_void
id|__init
id|ide_dmacapable_svwks
(paren
id|ide_hwif_t
op_star
id|hwif
comma
r_int
r_int
id|dmabase
)paren
(brace
r_struct
id|pci_dev
op_star
id|dev
op_assign
id|hwif-&gt;pci_dev
suffix:semicolon
r_if
c_cond
(paren
(paren
id|dev-&gt;device
op_eq
id|PCI_DEVICE_ID_SERVERWORKS_CSB6IDE
)paren
op_logical_and
(paren
op_logical_neg
(paren
id|PCI_FUNC
c_func
(paren
id|dev-&gt;devfn
)paren
op_amp
l_int|1
)paren
)paren
op_logical_and
(paren
id|hwif-&gt;channel
)paren
)paren
r_return
suffix:semicolon
macro_line|#if 0
r_if
c_cond
(paren
id|svwks_revision
op_eq
(paren
id|SVWKS_CSB5_REVISION_NEW
op_plus
l_int|1
)paren
)paren
(brace
r_if
c_cond
(paren
id|hwif-&gt;mate
op_logical_and
id|hwif-&gt;mate-&gt;dma_base
)paren
(brace
id|dmabase
op_assign
id|hwif-&gt;mate-&gt;dma_base
op_minus
(paren
id|hwif-&gt;channel
ques
c_cond
l_int|0
suffix:colon
l_int|8
)paren
suffix:semicolon
)brace
r_else
(brace
id|dmabase
op_assign
id|pci_resource_start
c_func
(paren
id|dev
comma
l_int|4
)paren
suffix:semicolon
r_if
c_cond
(paren
op_logical_neg
id|dmabase
)paren
(brace
id|printk
c_func
(paren
l_string|&quot;%s: dma_base is invalid (0x%04lx)&bslash;n&quot;
comma
id|hwif-&gt;name
comma
id|dmabase
)paren
suffix:semicolon
id|dmabase
op_assign
l_int|0
suffix:semicolon
)brace
)brace
)brace
macro_line|#endif
id|ide_setup_dma
c_func
(paren
id|hwif
comma
id|dmabase
comma
l_int|8
)paren
suffix:semicolon
)brace
r_extern
r_void
id|ide_setup_pci_device
(paren
r_struct
id|pci_dev
op_star
id|dev
comma
id|ide_pci_device_t
op_star
id|d
)paren
suffix:semicolon
DECL|function|fixup_device_csb6
r_void
id|__init
id|fixup_device_csb6
(paren
r_struct
id|pci_dev
op_star
id|dev
comma
id|ide_pci_device_t
op_star
id|d
)paren
(brace
r_if
c_cond
(paren
op_logical_neg
(paren
id|PCI_FUNC
c_func
(paren
id|dev-&gt;devfn
)paren
op_amp
l_int|1
)paren
)paren
(brace
id|d-&gt;bootable
op_assign
id|NEVER_BOARD
suffix:semicolon
)brace
id|printk
c_func
(paren
l_string|&quot;%s: IDE controller on PCI bus %02x dev %02x&bslash;n&quot;
comma
id|d-&gt;name
comma
id|dev-&gt;bus-&gt;number
comma
id|dev-&gt;devfn
)paren
suffix:semicolon
id|ide_setup_pci_device
c_func
(paren
id|dev
comma
id|d
)paren
suffix:semicolon
)brace
eof
