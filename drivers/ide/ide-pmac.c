...
id|ide_dma_write
suffix:colon
multiline_comment|/* this almost certainly isn&squot;t needed since we don&squot;t&n;&t;&t;   appear to have a rwproc */
r_if
c_cond
(paren
id|drive-&gt;channel-&gt;rwproc
)paren
id|drive-&gt;channel
...
multiline_comment|/* We have to things to deal with here:&n;&t;&t; * &n;&t;&t; * - The dbdma won&squot;t stop if the command was started&n;&t;&t; * but completed with an error without transfering all&n;&t;&t; * datas. This happens when bad blocks are met during&n;&t;&t; * a multi-block transfer.&n;&t;&t; * &n;&t;&t; * - The dbdma fifo hasn&squot;t yet finished flushing to&n;&t;&t; * to system memory when the disk interrupt occurs.&n;&t;&t; * &n;&t;&t; * The trick here is to increment drive-&gt;waiting_for_dma,&n;&t;&t; * and return as if no interrupt occured. If the counter&n;&t;&t; * reach a certain timeout value, we then return 1. If&n;&t;&t; * we really got the interrupt, it will happen right away&n;&t;&t; * again.&n;&t;&t; * Apple&squot;s solution here may be more elegant. They issue&n;&t;&t; * a DMA channel interrupt (a separate irq line) via a DBDMA&n;&t;&t; * NOP command just before the STOP, and wait for both the&n;&t;&t; * disk and DBDMA interrupts to have completed.&n;&t;&t; */
multiline_comment|/* If ACTIVE is cleared, the STOP command have passed and&n;&t;&t; * transfer is complete.&n;&t;&t; */
r_if
c_cond
(paren
op_logical_neg
...
suffix:semicolon
multiline_comment|/* If dbdma didn&squot;t execute the STOP command yet, the&n;&t;&t; * active bit is still set */
id|drive-&gt;waiting_for_dma
op_increment
suffix:semicolon
r_if
