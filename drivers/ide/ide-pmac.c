...
multiline_comment|/* Note: We don&squot;t use the generic routine here because some of Apple&squot;s&n; * controller seem to be very sensitive about how things are done.&n; * We should probably set the NIEN bit, but that&squot;s an example of thing&n; * that can cause the controller to hang under some circumstances when&n; * done on the media-bay CD-ROM during boot. We do get occasional&n; * spurrious interrupts because of that.&n; * --BenH&n; */
r_static
r_int
...
l_int|150
suffix:semicolon
multiline_comment|/* For ata-4 controller */
r_if
c_cond
(paren
id|pmac_ide
...
l_int|1
suffix:semicolon
multiline_comment|/* That&squot;s a bit ugly and dangerous, but works in our case&n;&t;&t; * to workaround a problem with the channel status staying&n;&t;&t; * active if the drive returns an error&n;&t;&t; */
r_if
c_cond
(paren
id|IDE_CONTROL_REG
...
multiline_comment|/* In some edge cases, some datas may still be in the dbdma&n;&t;&t; * engine fifo, we wait a bit for dbdma to complete&n;&t;&t; */
r_while
c_loop
(paren
(paren
...
r_return
suffix:semicolon
multiline_comment|/* Disable and reset the bus */
id|feature_set
c_func
(paren
id|np
...
l_int|2
suffix:colon
multiline_comment|/* This one exists only for KL, I don&squot;t know&n;&t;&t;&t;   about any enable bit */
id|feature_set
c_func
(paren
id|np
...
suffix:semicolon
multiline_comment|/* Wait for the drive to come up and set it&squot;s DMA */
r_if
c_cond
(paren
id|drive-&gt;present
...
(brace
multiline_comment|/* Wait up to 20 seconds */
r_for
c_loop
(paren
id|j
...
suffix:semicolon
multiline_comment|/* Wake the device&n;&t;&t;&t; * We could handle the slave here&n;&t;&t;&t; */
r_if
c_cond
(paren
id|drive-&gt;present
