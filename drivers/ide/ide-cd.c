...
macro_line|#include &lt;scsi/scsi.h&gt;&t;/* For SCSI -&gt; ATAPI command conversion */
...
id|ILLEGAL_REQUEST
suffix:colon
multiline_comment|/*&n;&t;&t;&t; * don&squot;t log START_STOP unit with LoEj set, since&n;&t;&t;&t; * we cannot reliably check if drive can auto-close&n;&t;&t;&t; */
r_if
c_cond
(paren
id|rq-&gt;cmd
...
multiline_comment|/*&n; * Initialize a ide-cd packet command request&n; */
DECL|function|cdrom_prepare_request
r_static
r_void
...
id|REQ_SENSE
suffix:semicolon
multiline_comment|/* NOTE! Save the failed command in &quot;rq-&gt;buffer&quot; */
id|rq-&gt;buffer
op_assign
(paren
r_void
...
(brace
multiline_comment|/*&n;&t;&t; * For REQ_SENSE, &quot;rq-&gt;buffer&quot; points to the original failed&n;&t;&t; * request&n;&t;&t; */
r_struct
id|request
op_star
id|failed
...
multiline_comment|/*&n;&t;&t;&t; * now end failed request&n;&t;&t;&t; */
...
l_int|1
suffix:semicolon
multiline_comment|/* make sure it&squot;s fully ended */
r_if
c_cond
(paren
id|blk_pc_request
...
id|flags
suffix:semicolon
multiline_comment|/*&n;&t;&t; * if we have an error, pass back CHECK_CONDITION as the&n;&t;&t; * scsi status byte&n;&t;&t; */
r_if
c_cond
(paren
(paren
...
id|REQ_FAILED
suffix:semicolon
multiline_comment|/*&n;&t;&t; * instead of playing games with moving completions around,&n;&t;&t; * remove failed request completely and end it when the&n;&t;&t; * request sense has completed&n;&t;&t; */
r_if
c_cond
(paren
id|stat
...
id|drive-&gt;driver_data
suffix:semicolon
multiline_comment|/* allow the drive 5 seconds to recover, some&n;&t;&t;&t;&t; * devices will return this error while flushing&n;&t;&t;&t;&t; * data from cache */
r_if
c_cond
(paren
op_logical_neg
...
id|info-&gt;write_timeout
op_assign
id|jiffies
op_plus
id|ATAPI_WAIT_WRITE_BUSY
suffix:semicolon
id|rq-&gt;errors
op_assign
l_int|1
...
id|flags
suffix:semicolon
multiline_comment|/*&n;&t;&t;&t;&t;&t; * take a breather relying on the&n;&t;&t;&t;&t;&t; * unplug timer to kick us again&n;&t;&t;&t;&t;&t; */
id|spin_lock_irqsave
c_func
(paren
op_amp
...
(paren
id|KERN_INFO
l_string|&quot;ide-cd: cmd 0x%x timed out&bslash;n&quot;
comma
id|rq-&gt;cmd
(braket
l_int|0
...
id|rq
suffix:semicolon
multiline_comment|/*&n;&t; * handle dma case&n;&t; */
r_if
c_cond
(paren
id|dma
...
mdefine_line|#define IDECD_SEEK_TIMEOUT&t;(2 * WAIT_CMD)&t;&t;/* 20 sec */
...
id|time
suffix:semicolon
multiline_comment|/* FIXME: we should probably abort/retry or something &n;&t;&t; * in case of failure */
r_if
c_cond
(paren
id|rq-&gt;flags
...
(brace
multiline_comment|/* Two notes about IDE interrupt reason here - 0 means that&n;&t; * the drive wants to receive data from us, 2 means that&n;&t; * the drive is expecting to transfer data to us.&n;&t; */
r_if
c_cond
(paren
id|ireason
...
DECL|function|post_transform_command
r_static
r_void
id|post_transform_command
c_func
(paren
r_struct
id|request
...
r_return
suffix:semicolon
multiline_comment|/*&n;&t; * set ansi-revision and response data as atapi&n;&t; */
r_if
c_cond
(paren
id|c
...
suffix:semicolon
multiline_comment|/*&n; * best way to deal with dma that is not sector aligned right now... note&n; * that in this path we are not using -&gt;data or -&gt;buffer at all. this irs&n; * can replace cdrom_pc_intr, cdrom_read_intr, and cdrom_write_intr in the&n; * future.&n; */
DECL|function|cdrom_newpc_intr
r_static
id|ide_startstop_t
id|cdrom_newpc_intr
c_func
...
id|ide_stopped
suffix:semicolon
multiline_comment|/*&n;&t; * using dma, transfer is complete now&n;&t; */
r_if
c_cond
(paren
id|dma
...
multiline_comment|/*&n;&t; * ok we fall to pio :/&n;&t; */
id|ireason
op_assign
id|HWIF
c_func
...
id|len
suffix:semicolon
multiline_comment|/*&n;&t; * If DRQ is clear, the command has completed.&n;&t; */
r_if
c_cond
(paren
(paren
...
id|end_request
suffix:semicolon
multiline_comment|/*&n;&t; * check which way to transfer data&n;&t; */
r_if
c_cond
(paren
id|rq_data_dir
...
(brace
multiline_comment|/*&n;&t;&t; * write to drive&n;&t;&t; */
r_if
c_cond
(paren
id|cdrom_write_check_ireason
...
r_else
(brace
multiline_comment|/*&n;&t;&t; * read from drive&n;&t;&t; */
r_if
c_cond
(paren
id|cdrom_read_check_ireason
...
multiline_comment|/*&n;&t; * transfer data&n;&t; */
r_while
c_loop
(paren
id|thislen
...
id|rq-&gt;data
suffix:semicolon
multiline_comment|/*&n;&t;&t; * bio backed?&n;&t;&t; */
r_if
c_cond
(paren
id|rq-&gt;bio
...
multiline_comment|/*&n;&t; * pad, if necessary&n;&t; */
r_if
c_cond
(paren
id|len
...
id|SECTOR_SIZE
suffix:semicolon
multiline_comment|/*&n;&t; * now loop and write out the data&n;&t; */
r_while
c_loop
(paren
id|sectors_to_transfer
...
multiline_comment|/*&n;&t;&t; * current buffer complete, move on&n;&t;&t; */
r_if
c_cond
(paren
id|rq-&gt;current_nr_sectors
...
id|SECTOR_BITS
suffix:semicolon
multiline_comment|/*&n;&t; * writes *must* be hardware frame aligned&n;&t; */
r_if
c_cond
(paren
(paren
...
multiline_comment|/*&n;&t; * disk has become write protected&n;&t; */
r_if
c_cond
(paren
id|g-&gt;policy
...
l_int|0
suffix:semicolon
multiline_comment|/* use dma, if possible. we don&squot;t need to check more, since we&n;&t; * know that the transfer is always (at least!) frame aligned */
id|info-&gt;dma
op_assign
id|drive-&gt;using_dma
ques
...
DECL|function|cdrom_do_newpc_cont
r_static
id|ide_startstop_t
id|cdrom_do_newpc_cont
c_func
(paren
id|ide_drive_t
op_star
...
l_int|0
suffix:semicolon
multiline_comment|/*&n;&t; * sg request&n;&t; */
r_if
c_cond
(paren
id|rq-&gt;bio
...
id|drive-&gt;using_dma
suffix:semicolon
multiline_comment|/*&n;&t;&t; * check if dma is safe&n;&t;&t; */
r_if
c_cond
(paren
(paren
...
suffix:semicolon
multiline_comment|/* only tell drive to close tray if open, if it can do that */
r_if
c_cond
(paren
id|ejectflag
...
l_int|0
suffix:semicolon
multiline_comment|/* Try to get the total cdrom capacity and sector size. */
id|stat
op_assign
id|cdrom_read_capacity
c_func
...
(brace
multiline_comment|/* Re-read multisession information using MSF format */
id|stat
op_assign
id|cdrom_read_tocentry
c_func
...
multiline_comment|/*&n; * add logic to try GET_EVENT command first to check for media and tray&n; * status. this should be supported by newer cd-r/w and all DVD etc&n; * drives&n; */
r_static
...
id|CDS_DISC_OK
suffix:semicolon
multiline_comment|/*&n;&t; * If not using Mt Fuji extended media tray reports,&n;&t; * just return TRAY_OPEN since ATAPI doesn&squot;t provide&n;&t; * any other way to detect this...&n;&t; */
r_if
c_cond
(paren
id|sense.sense_key
...
id|drive-&gt;id-&gt;model
comma
l_string|&quot;STINGRAY 8422 IDE 8X CD-ROM 7-27-95&quot;
)paren
)paren
(brace
id|CDROM_CONFIG_FLAGS
...
multiline_comment|/*&n;&t; * we have to cheat a little here. the packet will eventually&n;&t; * be queued with ide_cdrom_packet(), which extracts the&n;&t; * drive from cdi-&gt;handle. Since this device hasn&squot;t been&n;&t; * registered with the Uniform layer yet, it can&squot;t do this.&n;&t; * Same goes for cdi-&gt;ops.&n;&t; */
id|cdi-&gt;handle
op_assign
(paren
id|ide_drive_t
...
id|drive-&gt;id-&gt;model
comma
l_string|&quot;MATSHITADVD-ROM SR-8176&quot;
)paren
op_eq
l_int|0
op_logical_or
...
id|drive-&gt;id-&gt;model
comma
l_string|&quot;MATSHITADVD-ROM SR-8174&quot;
)paren
op_eq
l_int|0
)paren
...
DECL|function|ide_cdrom_prep_fs
r_static
r_int
id|ide_cdrom_prep_fs
c_func
(paren
id|request_queue_t
op_star
...
multiline_comment|/*&n; * Most of the SCSI commands are supported directly by ATAPI devices.&n; * This transform handles the few exceptions.&n; */
DECL|function|ide_cdrom_prep_pc
r_static
r_int
id|ide_cdrom_prep_pc
c_func
(paren
r_struct
id|request
...
id|rq-&gt;cmd
suffix:semicolon
multiline_comment|/*&n;&t; * Transform 6-byte read/write commands to the 10-byte version&n;&t; */
r_if
c_cond
(paren
id|c
...
multiline_comment|/*&n;&t; * it&squot;s silly to pretend we understand 6-byte sense commands, just&n;&t; * reject with ILLEGAL_REQUEST and the caller should take the&n;&t; * appropriate action&n;&t; */
r_if
c_cond
(paren
id|c
...
DECL|function|ide_cdrom_prep_fn
r_static
r_int
...
(brace
multiline_comment|/* Vertos 300.&n;&t;&t;   Some versions of this drive like to talk BCD. */
id|CDROM_CONFIG_FLAGS
c_func
(paren
id|drive
...
multiline_comment|/* Old NEC260 (not R).&n;&t;&t;   This drive was released before the 1.2 version&n;&t;&t;   of the spec. */
id|CDROM_CONFIG_FLAGS
c_func
(paren
id|drive
...
(paren
id|KERN_ERR
l_string|&quot;%s: %s failed to unregister device from the cdrom &quot;
...
DECL|function|proc_idecd_read_capacity
r_static
r_int
id|proc_idecd_read_capacity
(paren
r_char
op_star
id|page
...
suffix:semicolon
macro_line|#else
DECL|macro|idecd_proc
macro_line|# define idecd_proc&t;NULL
macro_line|#endif
...
c_func
(paren
id|ide_cdrom_driver.drives
)paren
comma
)brace
suffix:semicolon
DECL|function|idecd_open
r_static
r_int
id|idecd_open
c_func
...
DECL|function|idecd_media_changed
r_static
r_int
id|idecd_media_changed
c_func
(paren
r_struct
id|gendisk
...
DECL|function|idecd_revalidate_disk
r_static
r_int
id|idecd_revalidate_disk
c_func
(paren
r_struct
id|gendisk
