...
suffix:semicolon
multiline_comment|/* Check whatever this error should be logged:&n;&t; */
r_switch
c_cond
(paren
id|sense-&gt;sense_key
...
id|NOT_READY
suffix:colon
multiline_comment|/* Don&squot;t care about tray state messages for e.g.&n;&t;&t;&t; * capacity commands or in-progress or becoming ready.&n;&t;&t;&t; */
r_if
c_cond
(paren
id|sense-&gt;asc
...
id|UNIT_ATTENTION
suffix:colon
multiline_comment|/* Make good and sure we&squot;ve seen this potential media&n;&t;&t;&t; * change. Some drives (i.e. Creative) fail to present&n;&t;&t;&t; * the correct sense key in the error register.&n;&t;&t;&t; */
id|cdrom_saw_media_change
c_func
(paren
id|drive
...
r_return
suffix:semicolon
multiline_comment|/*&n;&t; * If a read toc is executed for a CD-R or CD-RW medium where the first&n;&t; * toc has not been recorded yet, it will fail with 05/24/00 (which is&n;&t; * a confusing error).&n;&t; */
r_if
c_cond
(paren
id|fail_cmd
...
id|printk
(paren
l_string|&quot;%s: cdrom_read_intr: data underrun (%u blocks)&bslash;n&quot;
comma
id|drive-&gt;name
comma
id|rq-&gt;current_nr_sectors
...
multiline_comment|/*&n; * Routine to send a read packet command to the drive.  This is usually called&n; * directly from cdrom_start_read.  However, for drq_interrupt devices, it is&n; * called from an interrupt when the drive is ready to accept the command.&n; */
...
multiline_comment|/*&n; * Fix up a possibly partially-processed request so that we can&n; * start it over entirely -- remember to call prep_rq_fn again since we&n; * may have changed the layout&n; */
...
(brace
singleline_comment|//&t;&t;spin_lock_irqsave(DRIVE_LOCK(drive), flags);
...
suffix:semicolon
singleline_comment|//&t;&t;spin_unlock_irqrestore(DRIVE_LOCK(drive), flags);
)brace
id|restore_request
c_func
(paren
...
c_func
(paren
l_string|&quot;%s: write_intr: data underrun (%u blocks)&bslash;n&quot;
comma
id|drive-&gt;name
comma
id|rq-&gt;current_nr_sectors
...
r_static
id|ide_startstop_t
DECL|function|ide_cdrom_do_request
...
id|ar
suffix:semicolon
multiline_comment|/*&n;&t;&t;&t; * get a new command (push ar further down to avoid grabbing lock here&n;&t;&t;&t; */
...
suffix:semicolon
multiline_comment|/*&n;&t;&t;&t; * we&squot;ve reached maximum queue depth, bail&n;&t;&t;&t; */
r_if
c_cond
(paren
op_logical_neg
...
id|BLOCKS_PER_FRAME
suffix:semicolon
multiline_comment|/*&n;&t; * reset block size, ide_revalidate_disk incorrectly sets it to&n;&t; * 1024 even for CDROM&squot;s&n;&t; */
id|blk_size
(braket
id|drive-&gt;channel-&gt;major
)braket
