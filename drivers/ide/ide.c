...
mdefine_line|#define ata_dump_bits(msgs,nr,bits) do { } while (0)
macro_line|#endif
...
multiline_comment|/* FIXME: check this! It appears to act on the current channel! */
r_if
c_cond
(paren
id|ch-&gt;intrproc
...
multiline_comment|/*&n; * get_info_ptr() returns the (struct ata_device *) for a given device number.&n; * It returns NULL if the given device number does not match any present drives.&n; */
...
multiline_comment|/*&n; * Handle ioctls.&n; *&n; * NOTE: Due to ridiculous coding habbits in the hdparm utility we have to&n; * always return unsigned long in case we are returning simple values.&n; */
...
multiline_comment|/*&n; * Lookup ATA devices, which requested a particular driver.&n; */
...
suffix:semicolon
multiline_comment|/*&n; * Because most of the ATA adapters represent the timings in unit of bus&n; * clocks, and there is no known reliable way to detect the bus clock&n; * frequency, we assume 50 MHz for non-PCI (VLB, EISA) and 33 MHz for PCI based&n; * systems. Since assuming only hurts performance and not stability, this is&n; * OK. The user can change this on the command line by using the &quot;idebus=XX&quot;&n; * parameter. While the system_bus_speed variable is in kHz units, we accept&n; * both MHz and kHz entry on the command line for backward compatibility.&n; */
id|system_bus_speed
op_assign
l_int|50000
suffix:semicolon
