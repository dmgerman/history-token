...
(paren
id|KERN_INFO
l_string|&quot;%s: exceeded max command turn-around time (%d seconds)&bslash;n&quot;
comma
id|drive-&gt;name
comma
id|ATA_AR_MAX_TURNAROUND
...
suffix:semicolon
multiline_comment|/*&n;&t;&t; * IDE_SET_CUR_TAG(drive, IDE_INACTIVE_TAG) will do this&n;&t;&t; * too, but it really belongs here. assumes that the&n;&t;&t; * ended request is the active one.&n;&t;&t; */
id|HWGROUP
c_func
(paren
id|drive
...
comma
id|tmp-&gt;PADAM_service_start
op_plus
l_int|2
op_star
id|tmp-&gt;PADAM_service_time
)paren
)paren
)paren
(brace
...
(brace
multiline_comment|/*&n;&t;&t; * Take a short snooze, and then wake up this hwgroup&n;&t;&t; * again.  This gives other hwgroups on the same a&n;&t;&t; * chance to play fairly with us, just in case there&n;&t;&t; * are big differences in relative throughputs.. don&squot;t&n;&t;&t; * want to hog the cpu too much.&n;&t;&t; */
r_if
c_cond
(paren
l_int|0
...
suffix:semicolon
multiline_comment|/* we purposely leave hwgroup busy while&n;&t;&t; * sleeping */
)brace
r_else
(brace
multiline_comment|/* Ugly, but how can we sleep for the lock&n;&t;&t; * otherwise? perhaps from tq_disk? */
id|ide_release_lock
c_func
(paren
op_amp
...
c_func
(paren
l_string|&quot;%s: hwgroup not busy while queueing&bslash;n&quot;
comma
id|drive-&gt;name
)paren
suffix:semicolon
multiline_comment|/*&n;&t;&t; * abort early if we can&squot;t queue another command. for non&n;&t;&t; * tcq, ide_can_queue is always 1 since we never get here&n;&t;&t; * unless the drive is idle.&n;&t;&t; */
r_if
c_cond
(paren
op_logical_neg
...
multiline_comment|/*&n;&t;&t; * if there are queued commands, we can&squot;t start a non-fs&n;&t;&t; * request (really, a non-queuable command) until the&n;&t;&t; * queue is empty&n;&t;&t; */
r_if
c_cond
(paren
op_logical_neg
...
suffix:semicolon
multiline_comment|/*&n;&t;&t; * command started, we are busy&n;&t;&t; */
r_if
c_cond
(paren
id|startstop
...
r_break
suffix:semicolon
multiline_comment|/*&n;&t;&t; * start_request() can return either ide_stopped (no command&n;&t;&t; * was started), ide_started (command started, don&squot;t queue&n;&t;&t; * more), or ide_released (command started, try and queue&n;&t;&t; * more).&n;&t;&t; */
macro_line|#if 0
r_if
c_cond
(paren
...
(brace
multiline_comment|/*&n;&t;&t; * will clear IDE_BUSY, if appropriate&n;&t;&t; */
r_if
c_cond
(paren
(paren
...
id|drive
suffix:semicolon
multiline_comment|/*&n;&t;&t; * main queueing loop&n;&t;&t; */
id|ide_queue_commands
c_func
(paren
id|drive
...
id|channel-&gt;ata_read
op_assign
id|old_hwif.ata_read
suffix:semicolon
id|channel-&gt;ata_write
op_assign
id|old_hwif.ata_write
suffix:semicolon
id|channel-&gt;atapi_read
op_assign
id|old_hwif.atapi_read
suffix:semicolon
id|channel-&gt;atapi_write
op_assign
id|old_hwif.atapi_write
suffix:semicolon
id|channel-&gt;dmaproc
op_assign
id|old_hwif.dmaproc
...
op_increment
suffix:semicolon
multiline_comment|/*&n;&t; * for now, just don&squot;t shrink it...&n;&t; */
r_if
c_cond
(paren
id|drive-&gt;queue_depth
...
DECL|function|ide_init_commandlist
r_int
