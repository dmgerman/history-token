...
id|console_timer
suffix:semicolon
DECL|variable|blank_state
r_static
r_int
id|blank_state
suffix:semicolon
DECL|variable|blank_timer_expired
r_static
r_int
id|blank_timer_expired
suffix:semicolon
r_enum
(brace
DECL|enumerator|blank_off
id|blank_off
op_assign
l_int|0
comma
DECL|enumerator|blank_normal_wait
id|blank_normal_wait
comma
DECL|enumerator|blank_vesa_wait
id|blank_vesa_wait
comma
)brace
suffix:semicolon
...
DECL|macro|IS_FG
mdefine_line|#define IS_FG(vc)&t;((vc)-&gt;vc_num == fg_console)
macro_line|#ifdef VT_BUF_VRAM_ONLY
DECL|macro|DO_UPDATE
mdefine_line|#define DO_UPDATE(vc)&t;0
macro_line|#else
DECL|macro|DO_UPDATE
mdefine_line|#define DO_UPDATE(vc)&t;CON_IS_VISIBLE(vc)
macro_line|#endif
...
suffix:semicolon
multiline_comment|/*&n;&t;&t; * If console changed from mono&lt;-&gt;color, the best we can do&n;&t;&t; * is to clear the buffer attributes. As it currently stands,&n;&t;&t; * rebuilding new attributes from the old buffer is not doable&n;&t;&t; * without overly complex code.&n;&t;&t; */
r_if
c_cond
(paren
id|old_was_color
...
(brace
multiline_comment|/* ++Geert: vc-&gt;vc_sw-&gt;con_init determines console size */
r_if
c_cond
(paren
id|vc-&gt;vc_sw
...
(brace
multiline_comment|/* Resizes the resolution of the display adapater */
r_int
id|err
op_assign
l_int|0
...
DECL|macro|set_kbd
mdefine_line|#define set_kbd(vc, x)&t;set_vc_kbd_mode(kbd_table + (vc)-&gt;vc_num, (x))
DECL|macro|clr_kbd
mdefine_line|#define clr_kbd(vc, x)&t;clr_vc_kbd_mode(kbd_table + (vc)-&gt;vc_num, (x))
DECL|macro|is_kbd
mdefine_line|#define is_kbd(vc, x)&t;vc_kbd_mode(kbd_table + (vc)-&gt;vc_num, (x))
DECL|macro|decarm
...
id|vc-&gt;vc_y
suffix:semicolon
id|vc-&gt;vc_s_intensity
op_assign
id|vc-&gt;vc_intensity
suffix:semicolon
id|vc-&gt;vc_s_underline
op_assign
id|vc-&gt;vc_underline
suffix:semicolon
id|vc-&gt;vc_s_blink
...
macro_line|#else
mdefine_line|#define FLUSH if (draw_x &gt;= 0) { &bslash;&n;&t;vc-&gt;vc_sw-&gt;con_putcs(vc, (u16 *)draw_from, (u16 *)draw_to - (u16 *)draw_from, vc-&gt;vc_y, draw_x); &bslash;&n;&t;draw_x = -1; &bslash;&n;&t;}
macro_line|#endif
r_int
id|c
comma
...
(paren
id|KERN_ERR
l_string|&quot;vt: argh, driver_data is NULL !&bslash;n&quot;
)paren
suffix:semicolon
...
(paren
id|KERN_ERR
l_string|&quot;vt: argh, driver_data _became_ NULL !&bslash;n&quot;
)paren
suffix:semicolon
id|release_console_sem
c_func
...
c_func
(paren
l_string|&quot;Couldn&squot;t allocate console driver&bslash;n&quot;
)paren
suffix:semicolon
...
multiline_comment|/*&n; * We defer the timer blanking to work queue so it can take the console semaphore&n; * (console operations can still happen at irq time, but only from printk which&n; * has the console semaphore. Not perfect yet, but better than no locking&n; */
DECL|function|blank_screen_t
r_static
r_void
id|blank_screen_t
c_func
...
suffix:semicolon
multiline_comment|/* This isn&squot;t perfectly race free, but a race here would be mostly harmless,&n;&t; * at worse, we&squot;ll do a spurrious blank and it&squot;s unlikely&n;&t; */
id|del_timer
c_func
(paren
op_amp
...
id|tmp
suffix:semicolon
multiline_comment|/* If from KDFONTOP ioctl, don&squot;t allow things which can be done in userland,&n;&t;&t;   so that we can get rid of this soon */
r_if
c_cond
(paren
op_logical_neg
