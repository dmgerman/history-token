...
suffix:semicolon
multiline_comment|/* transmit holding buffer definitions*/
DECL|macro|MAX_TX_HOLDING_BUFFERS
mdefine_line|#define MAX_TX_HOLDING_BUFFERS 5
DECL|struct|tx_holding_buffer
r_struct
...
suffix:semicolon
DECL|member|num_tx_dma_buffers
r_int
id|num_tx_dma_buffers
suffix:semicolon
multiline_comment|/* number of tx dma frames required */
DECL|member|tx_dma_buffers_used
r_int
id|tx_dma_buffers_used
suffix:semicolon
...
DECL|member|start_tx_dma_buffer
r_int
id|start_tx_dma_buffer
suffix:semicolon
multiline_comment|/* tx dma buffer to start tx dma operation */
DECL|member|current_tx_buffer
r_int
id|current_tx_buffer
suffix:semicolon
multiline_comment|/* next tx dma buffer to be loaded */
...
suffix:semicolon
DECL|member|num_tx_holding_buffers
r_int
id|num_tx_holding_buffers
suffix:semicolon
multiline_comment|/* number of tx holding buffer allocated */
DECL|member|get_tx_holding_index
r_int
id|get_tx_holding_index
suffix:semicolon
multiline_comment|/* next tx holding buffer for adapter to load */
DECL|member|put_tx_holding_index
r_int
id|put_tx_holding_index
suffix:semicolon
multiline_comment|/* next tx holding buffer to store user request */
DECL|member|tx_holding_count
r_int
id|tx_holding_count
suffix:semicolon
multiline_comment|/* number of tx holding buffers waiting */
DECL|member|tx_holding_buffers
r_struct
id|tx_holding_buffer
id|tx_holding_buffers
(braket
id|MAX_TX_HOLDING_BUFFERS
)braket
suffix:semicolon
...
suffix:semicolon
multiline_comment|/* generic HDLC device parts */
...
(brace
multiline_comment|/* turn off receiver and rx DMA */
...
suffix:semicolon
multiline_comment|/* schedule BH handler to restart receiver */
id|info-&gt;pending_bh
op_or_assign
id|BH_RECEIVE
suffix:semicolon
...
id|status
suffix:semicolon
multiline_comment|/* clear interrupt pending and IUS bit for Tx DMA IRQ */
id|usc_OutDmaReg
c_func
(paren
id|info
...
suffix:semicolon
multiline_comment|/* Read the transmit DMA status to identify interrupt type. */
multiline_comment|/* This also clears the status bits. */
id|status
op_assign
id|usc_InDmaReg
...
c_func
(paren
l_string|&quot;%s(%d):mgsl_isr_transmit_dma(%s) status=%04X&bslash;n&quot;
comma
id|__FILE__
comma
id|__LINE__
...
(brace
op_decrement
id|info-&gt;tx_dma_buffers_used
suffix:semicolon
multiline_comment|/* if there are transmit frames queued,&n;&t;&t; *  try to load the next one&n;&t;&t; */
r_if
c_cond
(paren
id|load_next_tx_holding_buffer
...
(brace
multiline_comment|/* if call returns non-zero value, we have&n;&t;&t;&t; * at least one free tx holding buffer&n;&t;&t;&t; */
id|info-&gt;pending_bh
op_or_assign
id|BH_TRANSMIT
suffix:semicolon
...
multiline_comment|/* end of mgsl_isr_transmit_dma() */
...
multiline_comment|/* mgsl_write()&n; * &n; * &t;Send a block of data&n; * &t;&n; * Arguments:&n; * &n; * &t;tty&t;&t;pointer to tty information structure&n; * &t;buf&t;&t;pointer to buffer containing send data&n; * &t;count&t;&t;size of send data in bytes&n; * &t;&n; * Return Value:&t;number of characters written&n; */
...
multiline_comment|/* transmitter is actively sending data -&n;&t;&t;&t; * if we have multiple transmit dma and&n;&t;&t;&t; * holding buffers, attempt to queue this&n;&t;&t;&t; * frame for transmission at a later time.&n;&t;&t;&t; */
r_if
c_cond
(paren
id|info-&gt;tx_holding_count
op_ge
id|info-&gt;num_tx_holding_buffers
)paren
(brace
multiline_comment|/* no tx holding buffers available */
id|ret
op_assign
l_int|0
suffix:semicolon
...
multiline_comment|/* queue transmit frame request */
id|ret
op_assign
id|count
suffix:semicolon
...
suffix:semicolon
multiline_comment|/* if we have sufficient tx dma buffers,&n;&t;&t;&t; * load the next buffered tx request&n;&t;&t;&t; */
id|spin_lock_irqsave
c_func
(paren
op_amp
...
c_func
(paren
l_string|&quot;%s(%d):mgsl_open with invalid line #%d.&bslash;n&quot;
comma
id|__FILE__
comma
id|__LINE__
...
c_func
(paren
l_string|&quot;%s %s(%d)  allocating %d tx holding buffers&bslash;n&quot;
comma
id|info-&gt;device_name
comma
id|__FILE__
...
id|info-&gt;get_tx_holding_index
op_assign
l_int|0
suffix:semicolon
id|info-&gt;put_tx_holding_index
op_assign
l_int|0
suffix:semicolon
id|info-&gt;tx_holding_count
...
l_int|0
suffix:semicolon
multiline_comment|/* all buffers in use */
)brace
id|ptx
op_assign
op_amp
...
DECL|function|synclink_cleanup
r_static
r_void
id|synclink_cleanup
c_func
...
l_int|0x0001
suffix:semicolon
multiline_comment|/* Set Receive mode = external sync */
id|usc_OutReg
c_func
(paren
id|info
...
id|IOCR
comma
multiline_comment|/* Set IOCR DCD is RxSync Detect Input */
(paren
r_int
r_int
)paren
...
suffix:semicolon
multiline_comment|/*&n;&t;&t;&t; * TxSubMode:&n;&t;&t;&t; * &t;CMR &lt;15&gt;&t;&t;0&t;Don&squot;t send CRC on Tx Underrun&n;&t;&t;&t; * &t;CMR &lt;14&gt;&t;&t;x&t;undefined&n;&t;&t;&t; * &t;CMR &lt;13&gt;&t;&t;0&t;Send preamble before openning sync&n;&t;&t;&t; * &t;CMR &lt;12&gt;&t;&t;0&t;Send 8-bit syncs, 1=send Syncs per TxLength&n;&t;&t;&t; *&n;&t;&t;&t; * TxMode:&n;&t;&t;&t; * &t;CMR &lt;11-8)&t;0100&t;MonoSync&n;&t;&t;&t; *&n;&t;&t;&t; * &t;0x00 0100 xxxx xxxx  04xx&n;&t;&t;&t; */
id|RegValue
op_or_assign
l_int|0x0400
suffix:semicolon
...
suffix:semicolon
multiline_comment|/* arm RCC underflow interrupt */
id|usc_OutReg
c_func
(paren
id|info
...
id|info-&gt;tx_buffer_list
(braket
id|info-&gt;start_tx_dma_buffer
)braket
dot
id|rcc
suffix:semicolon
multiline_comment|/* if operating in Raw sync mode, reset the rcc component&n;&t;&t;&t; * of the tx dma buffer entry, otherwise, the serial controller&n;&t;&t;&t; * will send a closing sync char after this count.&n;&t;&t;&t; */
r_if
c_cond
(paren
id|info-&gt;params.mode
...
(brace
multiline_comment|/* When running external sync mode, attempt to &squot;stream&squot; transmit  */
multiline_comment|/* by filling tx dma buffers as they become available. To do this */
multiline_comment|/* we need to enable Tx DMA EOB Status interrupts :               */
multiline_comment|/*                                                                */
multiline_comment|/* 1. Arm End of Buffer (EOB) Transmit DMA Interrupt (BIT2 of TDIAR) */
multiline_comment|/* 2. Enable Transmit DMA Interrupts (BIT0 of DICR) */
id|usc_OutDmaReg
c_func
(paren
id|info
...
suffix:semicolon
multiline_comment|/*&n;&t; * if SyncLink WAN adapter is running in external sync mode, the&n;&t; * transmitter has been set to Monosync in order to try to mimic&n;&t; * a true raw outbound bit stream. Monosync still sends an open/close&n;&t; * sync char at the start/end of a frame. Try to match those sync&n;&t; * patterns to the idle mode set here&n;&t; */
r_if
c_cond
(paren
id|info-&gt;params.mode
...
c_func
(paren
l_string|&quot;%s(%d):mgsl_get_rx_frame(%s) rx frame status=%d&bslash;n&quot;
comma
id|__FILE__
comma
id|__LINE__
...
id|info-&gt;tty
suffix:semicolon
multiline_comment|/*&n; &t; * current_rx_buffer points to the 1st buffer of the next available&n;&t; * receive frame. The status field is set by the 16C32 after&n;&t; * completing a receive frame. If the status field of this buffer&n;&t; * is zero, either the USC is still filling this buffer or this&n;&t; * is one of a series of buffers making up a received frame.&n;&t; *&n;&t; * If the count field of this buffer is zero, the USC is either&n;&t; * using this buffer or has used this buffer. Look at the count&n;&t; * field of the next buffer. If that next buffer&squot;s count is&n;&t; * non-zero, the USC is still actively using the current buffer.&n;&t; * Otherwise, if the next buffer&squot;s count field is zero, the&n;&t; * current buffer is complete and the USC is using the next&n;&t; * buffer.&n;&t; */
id|CurrentIndex
op_assign
id|NextIndex
op_assign
...
(brace
multiline_comment|/*&n;&t; &t; * Either the status field of this dma buffer is non-zero&n;&t;&t; * (indicating the last buffer of a receive frame) or the next&n;&t; &t; * buffer is marked as in use -- implying this buffer is complete&n;&t;&t; * and an intermediate buffer for this received frame.&n;&t; &t; */
id|status
op_assign
id|info-&gt;rx_buffer_list
(braket
...
r_else
(brace
multiline_comment|/*&n;&t;&t;&t; * A receive frame is available, get frame size and status.&n;&t;&t;&t; *&n;&t;&t;&t; * The frame size is the starting value of the RCC (which was&n;&t;&t;&t; * set to 0xffff) minus the ending value of the RCC (decremented&n;&t;&t;&t; * once for each receive character) minus 2 or 4 for the 16-bit&n;&t;&t;&t; * or 32-bit CRC.&n;&t;&t;&t; *&n;&t;&t;&t; * If the status field is zero, this is an intermediate buffer.&n;&t;&t;&t; * It&squot;s size is 4K.&n;&t;&t;&t; *&n;&t;&t;&t; * If the DMA Buffer Entry&squot;s Status field is non-zero, the&n;&t;&t;&t; * receive operation completed normally (ie: DCD dropped). The&n;&t;&t;&t; * RCC field is valid and holds the received frame size.&n;&t;&t;&t; * It is possible that the RCC field will be zero on a DMA buffer&n;&t;&t;&t; * entry with a non-zero status. This can occur if the total&n;&t;&t;&t; * frame size (number of bytes between the time DCD goes active&n;&t;&t;&t; * to the time DCD goes inactive) exceeds 65535 bytes. In this&n;&t;&t;&t; * case the 16C32 has underrun on the RCC count and appears to&n;&t;&t;&t; * stop updating this counter to let us know the actual received&n;&t;&t;&t; * frame size. If this happens (non-zero status and zero RCC),&n;&t;&t;&t; * simply return the entire RxDMA Buffer&n;&t;&t;&t; */
r_if
c_cond
(paren
id|status
...
(brace
multiline_comment|/*&n;&t;&t;&t;&t; * In the event that the final RxDMA Buffer is&n;&t;&t;&t;&t; * terminated with a non-zero status and the RCC&n;&t;&t;&t;&t; * field is zero, we interpret this as the RCC&n;&t;&t;&t;&t; * having underflowed (received frame &gt; 65535 bytes).&n;&t;&t;&t;&t; *&n;&t;&t;&t;&t; * Signal the event to the user by passing back&n;&t;&t;&t;&t; * a status of RxStatus_CrcError returning the full&n;&t;&t;&t;&t; * buffer and let the app figure out what data is&n;&t;&t;&t;&t; * actually valid&n;&t;&t;&t;&t; */
r_if
c_cond
(paren
id|info-&gt;rx_buffer_list
...
(brace
multiline_comment|/*&n;&t;&t;&t; * if running in raw sync mode, ISR handler for&n;&t;&t;&t; * End Of Buffer events terminates all buffers at 4K.&n;&t;&t;&t; * If this frame size is said to be &gt;4K, get the&n;&t;&t;&t; * actual number of bytes of the frame in this buffer.&n;&t;&t;&t; */
id|framesize
op_assign
id|framesize
op_mod
...
c_func
(paren
l_string|&quot;%s(%d):mgsl_get_raw_rx_frame(%s) status=%04X size=%d&bslash;n&quot;
comma
id|__FILE__
comma
id|__LINE__
...
(brace
multiline_comment|/* copy dma buffer(s) to contiguous intermediate buffer */
multiline_comment|/* NOTE: we never copy more than DMABUFFERSIZE bytes&t;*/
id|pBufEntry
op_assign
op_amp
(paren
...
multiline_comment|/* begin loading the frame in the next available tx dma&n;&t; * buffer, remember it&squot;s starting location for setting&n;&t; * up tx dma operation&n;&t; */
id|i
op_assign
id|info-&gt;current_tx_buffer
suffix:semicolon
...
multiline_comment|/* remember next available tx dma buffer */
id|info-&gt;current_tx_buffer
op_assign
id|i
suffix:semicolon
