...
r_case
id|RADEON_EMIT_PP_CUBIC_OFFSETS_T0
suffix:colon
r_case
id|RADEON_EMIT_PP_CUBIC_OFFSETS_T1
suffix:colon
r_case
id|RADEON_EMIT_PP_CUBIC_OFFSETS_T2
suffix:colon
(brace
r_int
id|i
...
id|DRM_ERROR
(paren
l_string|&quot;Invalid R100 cubic texture offset&bslash;n&quot;
)paren
suffix:semicolon
r_return
id|DRM_ERR
...
suffix:colon
r_case
id|RADEON_EMIT_PP_CUBIC_FACES_0
suffix:colon
r_case
id|RADEON_EMIT_PP_CUBIC_FACES_1
suffix:colon
r_case
id|RADEON_EMIT_PP_CUBIC_FACES_2
suffix:colon
r_case
id|R200_EMIT_PP_TRI_PERF_CNTL
suffix:colon
...
comma
(brace
id|RADEON_PP_CUBIC_FACES_0
comma
l_int|1
comma
l_string|&quot;RADEON_PP_CUBIC_FACES_0&quot;
)brace
comma
(brace
id|RADEON_PP_CUBIC_OFFSET_T0_0
comma
l_int|5
comma
l_string|&quot;RADEON_PP_CUBIC_OFFSET_T0_0&quot;
)brace
comma
(brace
id|RADEON_PP_CUBIC_FACES_1
comma
l_int|1
comma
l_string|&quot;RADEON_PP_CUBIC_FACES_1&quot;
)brace
comma
(brace
id|RADEON_PP_CUBIC_OFFSET_T1_0
comma
l_int|5
comma
l_string|&quot;RADEON_PP_CUBIC_OFFSET_T1_0&quot;
)brace
comma
(brace
id|RADEON_PP_CUBIC_FACES_2
comma
l_int|1
comma
l_string|&quot;RADEON_PP_CUBIC_FACES_2&quot;
)brace
comma
(brace
id|RADEON_PP_CUBIC_OFFSET_T2_0
comma
l_int|5
comma
l_string|&quot;RADEON_PP_CUBIC_OFFSET_T2_0&quot;
)brace
comma
(brace
id|R200_PP_TRI_PERF
...
multiline_comment|/* hyper z clear */
multiline_comment|/* no docs available, based on reverse engeneering by Stephane Marchesin */
r_if
c_cond
(paren
(paren
...
suffix:semicolon
multiline_comment|/* Make sure we restore the 3D state next time.&n;&t;&t; * we haven&squot;t touched any &quot;normal&quot; state - still need this?&n;&t;&t; */
id|dev_priv-&gt;sarea_priv-&gt;ctx_owner
op_assign
l_int|0
suffix:semicolon
...
(brace
multiline_comment|/* FIXME : reverse engineer that for Rx00 cards */
multiline_comment|/* FIXME : the mask supposedly contains low-res z values. So can&squot;t set&n;&t;&t;   just to the max (0xff? or actually 0x3fff?), need to take z clear&n;&t;&t;   value into account? */
multiline_comment|/* pattern seems to work for r100, though get slight&n;&t;&t;   rendering errors with glxgears. If hierz is not enabled for r100,&n;&t;&t;   only 4 bits which indicate clear (15,16,31,32, all zero) matter, the&n;&t;&t;   other ones are ignored, and the same clear mask can be used. That&squot;s&n;&t;&t;   very different behaviour than R200 which needs different clear mask&n;&t;&t;   and different number of tiles to clear if hierz is enabled or not !?!&n;&t;&t;*/
id|clearmask
op_assign
(paren
l_int|0xff
...
r_else
(brace
multiline_comment|/* clear mask : chooses the clearing pattern.&n;&t;&t;   rv250: could be used to clear only parts of macrotiles&n;&t;&t;   (but that would get really complicated...)?&n;&t;&t;   bit 0 and 1 (either or both of them ?!?!) are used to&n;&t;&t;   not clear tile (or maybe one of the bits indicates if the tile is&n;&t;&t;   compressed or not), bit 2 and 3 to not clear tile 1,...,.&n;&t;&t;   Pattern is as follows:&n;&t;&t;        | 0,1 | 4,5 | 8,9 |12,13|16,17|20,21|24,25|28,29|&n;&t;&t;   bits -------------------------------------------------&n;&t;&t;        | 2,3 | 6,7 |10,11|14,15|18,19|22,23|26,27|30,31|&n;&t;&t;   rv100: clearmask covers 2x8 4x1 tiles, but one clear still&n;&t;&t;   covers 256 pixels ?!?&n;&t;&t;*/
id|clearmask
op_assign
l_int|0x0
suffix:semicolon
...
c_func
(paren
id|RADEON_RB3D_DEPTHCLEARVALUE
comma
id|tempRB3D_DEPTHCLEARVALUE
)paren
suffix:semicolon
multiline_comment|/* what offset is this exactly ? */
id|OUT_RING_REG
c_func
(paren
id|RADEON_RB3D_ZMASKOFFSET
comma
l_int|0
)paren
suffix:semicolon
multiline_comment|/* need ctlstat, otherwise get some strange black flickering */
id|OUT_RING_REG
c_func
(paren
id|RADEON_RB3D_ZCACHE_CTLSTAT
comma
id|RADEON_RB3D_ZC_FLUSH_ALL
)paren
suffix:semicolon
id|ADVANCE_RING
c_func
...
id|j
suffix:semicolon
multiline_comment|/* it looks like r200 needs rv-style clears, at least if hierz is not enabled? */
r_if
c_cond
(paren
(paren
...
(brace
multiline_comment|/* FIXME : figure this out for r200 (when hierz is enabled). Or&n;&t;&t;&t;&t;   maybe r200 actually doesn&squot;t need to put the low-res z value into&n;&t;&t;&t;&t;   the tile cache like r100, but just needs to clear the hi-level z-buffer?&n;&t;&t;&t;&t;   Works for R100, both with hierz and without.&n;&t;&t;&t;&t;   R100 seems to operate on 2x1 8x8 tiles, but...&n;&t;&t;&t;&t;   odd: offset/nrtiles need to be 64 pix (4 block) aligned? Potentially&n;&t;&t;&t;&t;   problematic with resolutions which are not 64 pix aligned? */
id|tileoffset
op_assign
(paren
(paren
...
c_func
(paren
id|RADEON_3D_CLEAR_ZMASK
comma
l_int|2
)paren
)paren
suffix:semicolon
multiline_comment|/* first tile */
id|OUT_RING
c_func
(paren
id|tileoffset
...
suffix:semicolon
multiline_comment|/* the number of tiles to clear */
id|OUT_RING
c_func
(paren
id|nrtilesx
...
suffix:semicolon
multiline_comment|/* clear mask : chooses the clearing pattern. */
id|OUT_RING
c_func
(paren
id|clearmask
...
(brace
multiline_comment|/* works for rv250. */
multiline_comment|/* find first macro tile (8x2 4x4 z-pixels on rv250) */
id|tileoffset
op_assign
(paren
(paren
...
suffix:semicolon
multiline_comment|/* first tile */
multiline_comment|/* judging by the first tile offset needed, could possibly&n;&t;&t;&t;&t;&t;   directly address/clear 4x4 tiles instead of 8x2 * 4x4&n;&t;&t;&t;&t;&t;   macro tiles, though would still need clear mask for&n;&t;&t;&t;&t;&t;   right/bottom if truely 4x4 granularity is desired ? */
id|OUT_RING
c_func
(paren
id|tileoffset
...
r_else
(brace
multiline_comment|/* rv 100 */
multiline_comment|/* rv100 might not need 64 pix alignment, who knows */
multiline_comment|/* offsets are, hmm, weird */
id|tileoffset
op_assign
(paren
(paren
...
multiline_comment|/* TODO don&squot;t always clear all hi-level z tiles */
r_if
c_cond
(paren
(paren
...
multiline_comment|/* r100 and cards without hierarchical z-buffer have no high-level z-buffer */
multiline_comment|/* FIXME : the mask supposedly contains low-res z values. So can&squot;t set&n;&t;&t;   just to the max (0xff? or actually 0x3fff?), need to take z clear&n;&t;&t;   value into account? */
(brace
id|BEGIN_RING
c_func
...
id|flags
op_amp
id|RADEON_USE_COMP_ZBUF
)paren
(brace
id|tempRB3D_ZSTENCILCNTL
op_or_assign
id|RADEON_Z_COMPRESSION_ENABLE
op_or
id|RADEON_Z_DECOMPRESSION_ENABLE
suffix:semicolon
)brace
r_if
c_cond
...
suffix:semicolon
multiline_comment|/* we got tiled coordinates, untile them */
id|image-&gt;x
op_mul_assign
l_int|2
suffix:semicolon
...
(brace
multiline_comment|/* texture micro tiling in use, minimum texture width is thus 16 bytes.&n;&t;&t;&t;   however, we cannot use blitter directly for texture width &lt; 64 bytes,&n;&t;&t;&t;   since minimum tex pitch is 64 bytes and we need this to match&n;&t;&t;&t;   the texture width, otherwise the blitter will tile it wrong.&n;&t;&t;&t;   Thus, tiling manually in this case. Additionally, need to special&n;&t;&t;&t;   case tex height = 1, since our actual image will have height 2&n;&t;&t;&t;   and we need to ensure we don&squot;t read beyond the texture size&n;&t;&t;&t;   from user space. */
r_if
c_cond
(paren
id|tex-&gt;height
...
(brace
multiline_comment|/* TODO: make sure this works when not fitting in one buffer&n;&t;&t;&t;&t;   (i.e. 32bytes x 2048...) */
r_for
c_loop
(paren
id|i
...
(brace
multiline_comment|/* Texture image width is larger than the minimum, so we&n;&t;&t;&t;&t; * can upload it directly.&n;&t;&t;&t;&t; */
r_if
c_cond
(paren
id|DRM_COPY_FROM_USER
...
r_else
(brace
multiline_comment|/* Texture image width is less than the minimum, so we&n;&t;&t;&t;&t; * need to pad out each image scanline to the minimum&n;&t;&t;&t;&t; * width.&n;&t;&t;&t;&t; */
r_for
c_loop
(paren
id|i
...
DECL|function|radeon_apply_surface_regs
r_static
r_void
...
c_func
(paren
id|RADEON_SURFACE0_INFO
op_plus
l_int|16
op_star
id|surf_index
comma
id|dev_priv-&gt;surfaces
(braket
id|surf_index
)braket
dot
...
c_func
(paren
id|RADEON_SURFACE0_LOWER_BOUND
op_plus
l_int|16
op_star
id|surf_index
...
c_func
(paren
id|RADEON_SURFACE0_UPPER_BOUND
op_plus
l_int|16
op_star
id|surf_index
...
multiline_comment|/* Allocates a virtual surface&n; * doesn&squot;t always allocate a real surface, will stretch an existing &n; * surface when possible.&n; *&n; * Note that refcount can be at most 2, since during a free refcount=3&n; * might mean we have to allocate a new surface which might not always&n; * be available.&n; * For example : we allocate three contigous surfaces ABC. If B is &n; * freed, we suddenly need two surfaces to store A and C, which might&n; * not always be available.&n; */
DECL|function|alloc_surface
r_static
r_int
id|alloc_surface
c_func
...
id|new_upper
op_amp
id|RADEON_SURF_ADDRESS_FIXED_MASK
)paren
op_ne
id|RADEON_SURF_ADDRESS_FIXED_MASK
)paren
...
l_int|1
suffix:semicolon
multiline_comment|/* make sure there is no overlap with existing surfaces */
r_for
c_loop
(paren
id|i
...
multiline_comment|/* find a virtual surface */
r_for
c_loop
(paren
id|i
...
id|i
suffix:semicolon
multiline_comment|/* try to reuse an existing surface */
r_for
c_loop
(paren
id|i
...
(brace
multiline_comment|/* extend before */
r_if
c_cond
(paren
(paren
...
multiline_comment|/* extend after */
r_if
c_cond
(paren
(paren
...
multiline_comment|/* okay, we need a new one */
r_for
c_loop
(paren
id|i
...
multiline_comment|/* we didn&squot;t find anything */
r_return
op_minus
l_int|1
suffix:semicolon
)brace
DECL|function|free_surface
r_static
r_int
id|free_surface
c_func
...
id|i
suffix:semicolon
multiline_comment|/* find the virtual surface */
r_for
c_loop
(paren
id|i
...
DECL|function|radeon_surfaces_release
r_static
r_void
id|radeon_surfaces_release
c_func
(paren
id|DRMFILE
id|filp
...
DECL|function|radeon_surface_alloc
r_static
r_int
...
DECL|function|radeon_surface_free
r_static
r_int
...
multiline_comment|/* Allocate an in-kernel area and copy in the cmdbuf.  Do this to avoid&n;&t; * races between checking values and using those values in other code,&n;&t; * and simply to avoid a lot of function calls to copy in data.&n;&t; */
id|orig_bufsz
op_assign
id|cmdbuf.bufsz
suffix:semicolon
...
suffix:semicolon
r_case
id|RADEON_SETPARAM_SWITCH_TILING
suffix:colon
r_if
c_cond
(paren
...
c_func
(paren
l_string|&quot;color tiling disabled&bslash;n&quot;
)paren
suffix:semicolon
id|dev_priv-&gt;front_pitch_offset
op_and_assign
op_complement
id|RADEON_DST_TILE_MACRO
suffix:semicolon
id|dev_priv-&gt;back_pitch_offset
op_and_assign
op_complement
id|RADEON_DST_TILE_MACRO
suffix:semicolon
id|dev_priv-&gt;sarea_priv-&gt;tiling_enabled
op_assign
l_int|0
suffix:semicolon
)brace
...
c_func
(paren
l_string|&quot;color tiling enabled&bslash;n&quot;
)paren
suffix:semicolon
id|dev_priv-&gt;front_pitch_offset
op_or_assign
...
suffix:semicolon
DECL|variable|radeon_max_ioctl
r_int
id|radeon_max_ioctl
op_assign
id|DRM_ARRAY_SIZE
