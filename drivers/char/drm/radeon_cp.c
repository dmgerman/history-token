...
multiline_comment|/* This code will reinit the Radeon CP hardware after a resume from disc.  &n; * AFAIK, it would be very difficult to pickle the state at suspend time, so &n; * here we make sure that all Radeon hardware initialisation is re-done without&n; * affecting running applications.  This function is called radeon_do_resume_cp()&n; * as it was derived from radeon_init_cp, where most of the initialisation takes&n; * place during DRI init.&n; *&n; * This patch is NOT to be confused with my and Michel Daenzer&squot;s earlier DRI&n; * reinit work, which de- and re-initialised the complete DRI at every VT&n; * switch.&n; *&n; * Charl P. Botha &lt;http://cpbotha.net&gt;&n; */
...
suffix:semicolon
multiline_comment|/* get the existing dev_private */
id|dev_priv
op_assign
id|dev-&gt;dev_private
suffix:semicolon
...
macro_line|#endif
(brace
multiline_comment|/* I&squot;m not so sure about this ati_picgart_init after at resume-time... */
r_if
c_cond
(paren
op_logical_neg
