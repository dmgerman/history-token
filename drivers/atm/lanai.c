...
DECL|macro|APRINTK
mdefine_line|#define APRINTK(truth, format, args...) &bslash;&n;&t;do { &bslash;&n;&t;&t;if (unlikely(!(truth))) &bslash;&n;&t;&t;&t;printk(KERN_ERR DEV_LABEL &quot;: &quot; format, ##args); &bslash;&n;&t;} while (0)
...
suffix:semicolon
multiline_comment|/* VCCs with tx backlog */
id|DECLARE_BITMAP
c_func
(paren
id|transmit_ready
...
suffix:semicolon
multiline_comment|/* VCCs with transmit space */
DECL|member|timer
r_struct
id|timer_list
id|timer
...
suffix:semicolon
multiline_comment|/*&n; * Each device has two bitmaps for each VCC (baclog_vccs and transmit_ready)&n; * This function iterates one of these, calling a given function for each&n; * vci with their bit set&n; */
...
multiline_comment|/*&n; * Lanai needs DMA buffers aligned to 256 bytes of at least 1024 bytes -&n; * usually any page allocation will do.  Just to be safe in case&n; * PAGE_SIZE is insanely tiny, though...&n; */
DECL|macro|LANAI_PAGE_SIZE
mdefine_line|#define LANAI_PAGE_SIZE   ((PAGE_SIZE &gt;= 1024) ? PAGE_SIZE : 1024)
multiline_comment|/*&n; * Allocate a buffer in host RAM for service list, RX, or TX&n; * Returns buf-&gt;start==NULL if no memory&n; * Note that the size will be rounded up 2^n bytes, and&n; * if we can&squot;t allocate that we&squot;ll settle for something smaller&n; * until minbytes&n; */
...
multiline_comment|/* max lanai buffer size */
id|bytes
op_assign
l_int|128
op_star
...
r_do
(brace
multiline_comment|/*&n;&t;&t; * Technically we could use non-consistent mappings for&n;&t;&t; * everything, but the way the lanai uses DMA memory would&n;&t;&t; * make that a terrific pain.  This is much simpler.&n;&t;&t; */
id|buf-&gt;start
op_assign
...
multiline_comment|/* Lanai requires 256-byte alignment of DMA bufs */
id|APRINTK
c_func
(paren
(paren
...
suffix:semicolon
multiline_comment|/* This can only happen if PAGE_SIZE is gigantic, but just in case */
r_if
c_cond
(paren
id|order
...
mdefine_line|#define     BOARD_ID_LANAI256&t;&t;(0)&t;/* 25.6M adapter card */
...
suffix:semicolon
multiline_comment|/*&n;&t; * We need to wait for the VCC to drain but don&squot;t wait forever.  We&n;&t; * give each 1K of buffer size 1/128th of a second to clear out.&n;&t; * TODO: maybe disable CBR if we&squot;re about to timeout?&n;&t; */
id|timeout
op_assign
id|jiffies
op_plus
...
id|KERN_ERR
id|DEV_LABEL
l_string|&quot;(itf %d): Timed out on &quot;
l_string|&quot;backlog closing vci %d&bslash;n&quot;
comma
...
multiline_comment|/* test if VCC is currently backlogged */
...
DECL|function|vcc_tx_add_aal5_trailer
r_static
r_inline
r_void
id|vcc_tx_add_aal5_trailer
c_func
(paren
r_struct
id|lanai_vcc
...
l_int|8
comma
l_string|&quot;vcc_tx_add_aal5_trailer: bad ptr=%p&bslash;n&quot;
comma
id|lvcc-&gt;tx.buf.ptr
)paren
suffix:semicolon
...
suffix:semicolon
multiline_comment|/*&n;&t; * Since the &quot;butt register&quot; is a shared resounce on the card we&n;&t; * serialize all accesses to it through this spinlock.  This is&n;&t; * mostly just paranoia sicne the register is rarely &quot;busy&quot; anyway&n;&t; * but is needed for correctness.&n;&t; */
id|spin_lock
c_func
(paren
op_amp
...
multiline_comment|/*&n;&t; * Before we tall the card to start work we need to be sure 100% of&n;&t; * the info in the service buffer has been written before we tell&n;&t; * the card about it&n;&t; */
id|wmb
c_func
(paren
)paren
...
multiline_comment|/*&n; * Add one AAL5 PDU to lvcc&squot;s transmit buffer.  Caller garauntees there&squot;s&n; * space available.  &quot;pdusize&quot; is the number of bytes the PDU will take&n; */
DECL|function|lanai_send_one_aal5
r_static
r_void
id|lanai_send_one_aal5
c_func
(paren
r_struct
id|lanai_dev
...
comma
l_string|&quot;lanai_send_one_aal5: wrong size packet (%d != %d)&bslash;n&quot;
comma
id|pdusize
comma
id|aal5_size
...
l_int|0
comma
l_string|&quot;pad is negative (%d)&bslash;n&quot;
comma
id|pad
)paren
suffix:semicolon
...
l_int|48
comma
l_string|&quot;pad is too big (%d)&bslash;n&quot;
comma
id|pad
)paren
suffix:semicolon
...
(brace
multiline_comment|/* No room for this packet - put it back on queue */
id|skb_queue_head
c_func
(paren
op_amp
...
l_int|64
comma
l_string|&quot;vcc_tx_aal5: n too small (%d)&bslash;n&quot;
comma
id|n
)paren
suffix:semicolon
...
(brace
multiline_comment|/* No space for this PDU */
id|__set_bit
c_func
(paren
id|lvcc-&gt;vci
...
suffix:semicolon
multiline_comment|/* Make sure that these copies don&squot;t get reordered */
id|barrier
c_func
(paren
)paren
...
suffix:semicolon
multiline_comment|/*&n;&t; * Before we actually read from the buffer, make sure the memory&n;&t; * changes have arrived&n;&t; */
id|rmb
c_func
(paren
)paren
...
suffix:semicolon
multiline_comment|/* Remember to get read_lock(&amp;vcc_sklist_lock) while looking up VC */
...
suffix:semicolon
multiline_comment|/* If we can grab the spinlock, check if any services need to be run */
r_if
c_cond
(paren
id|spin_trylock
...
multiline_comment|/* ...and see if any backlogged VCs can make progress */
multiline_comment|/* unfortunately linux has no read_trylock() currently */
id|read_lock
c_func
(paren
op_amp
...
id|KERN_ERR
id|DEV_LABEL
l_string|&quot;(itf %d): can&squot;t enable &quot;
l_string|&quot;PCI device&quot;
comma
id|lanai-&gt;number
)paren
...
id|KERN_WARNING
id|DEV_LABEL
l_string|&quot;(itf %d): No suitable DMA available.&bslash;n&quot;
comma
id|lanai-&gt;number
)paren
suffix:semicolon
...
id|DEV_LABEL
l_string|&quot;(itf %d): can&squot;t read &quot;
l_string|&quot;PCI_SUBSYSTEM_ID: %d&bslash;n&quot;
comma
id|lanai-&gt;number
comma
id|result
...
suffix:semicolon
multiline_comment|/* Make sure that all that made it */
id|intr_enable
c_func
(paren
id|lanai
...
suffix:semicolon
multiline_comment|/*&n;&t; * Make sure everything made it to RAM before we tell the card about&n;&t; * the VCC&n;&t; */
id|wmb
c_func
(paren
)paren
...
id|KERN_ERR
id|DEV_LABEL
l_string|&quot;: couldn&squot;t allocate dev_data structure!&bslash;n&quot;
)paren
suffix:semicolon
r_return
op_minus
...
id|KERN_ERR
id|DEV_LABEL
l_string|&quot;: couldn&squot;t register atm device!&bslash;n&quot;
)paren
suffix:semicolon
id|kfree
c_func
...
multiline_comment|/* terminal entry */
)brace
suffix:semicolon
...
suffix:semicolon
DECL|function|lanai_module_init
r_static
r_int
id|__init
id|lanai_module_init
c_func
(paren
r_void
)paren
...
DECL|function|lanai_module_exit
r_static
r_void
id|__exit
id|lanai_module_exit
c_func
(paren
r_void
)paren
...
DECL|variable|lanai_module_init
id|module_init
c_func
(paren
id|lanai_module_init
...
suffix:semicolon
DECL|variable|lanai_module_exit
id|module_exit
c_func
(paren
id|lanai_module_exit
