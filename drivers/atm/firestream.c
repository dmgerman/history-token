...
DECL|macro|NP
macro_line|#undef NP
multiline_comment|/* Small hardware gotcha:&n;&n;   The FS50 CAM (VP/VC match registers) always take the lowest channel&n;   number that matches. This is not a problem.&n;&n;   However, they also ignore whether the channel is enabled or&n;   not. This means that if you allocate channel 0 to 1.2 and then&n;   channel 1 to 0.0, then disabeling channel 0 and writing 0 to the&n;   match channel for channel 0 will &quot;steal&quot; the traffic from channel&n;   1, even if you correctly disable channel 0.&n;&n;   Workaround: &n;&n;   - When disabling channels, write an invalid VP/VC value to the&n;   match register. (We use 0xffffffff, which in the worst case &n;   matches VP/VC = &lt;maxVP&gt;/&lt;maxVC&gt;, but I expect it not to match&n;   anything as some &quot;when not in use, program to 0&quot; bits are now&n;   programmed to 1...)&n;&n;   - Don&squot;t initialize the match registers to 0, as 0.0 is a valid&n;   channel.&n;*/
