...
multiline_comment|/*&n;   Based on the NE2000 driver written by Donald Becker (1992-94).&n;   modified by Wim Dumon (Apr 1996)&n;&n;   This software may be used and distributed according to the terms&n;   of the GNU General Public License, incorporated herein by reference.&n;&n;   The author may be reached as wimpie@linux.cc.kuleuven.ac.be&n;&n;   Currently supported: NE/2&n;   This patch was never tested on other MCA-ethernet adapters, but it&n;   might work. Just give it a try and let me know if you have problems.&n;   Also mail me if it really works, please!&n;&n;   Changelog:&n;   Mon Feb  3 16:26:02 MET 1997&n;   - adapted the driver to work with the 2.1.25 kernel&n;   - multiple ne2 support (untested)&n;   - module support (untested)&n;&n;   Fri Aug 28 00:18:36 CET 1998 (David Weinehall)&n;   - fixed a few minor typos&n;   - made the MODULE_PARM conditional (it only works with the v2.1.x kernels)&n;   - fixed the module support (Now it&squot;s working...)&n;&n;   Mon Sep  7 19:01:44 CET 1998 (David Weinehall)&n;   - added support for Arco Electronics AE/2-card (experimental)&n;&n;   Mon Sep 14 09:53:42 CET 1998 (David Weinehall)&n;   - added support for Compex ENET-16MC/P (experimental) &n;&n;   Tue Sep 15 16:21:12 CET 1998 (David Weinehall, Magnus Jonsson, Tomas Ogren)&n;   - Miscellaneous bugfixes&n;&n;   Tue Sep 19 16:21:12 CET 1998 (Magnus Jonsson)&n;   - Cleanup&n;&n;   Wed Sep 23 14:33:34 CET 1998 (David Weinehall)&n;   - Restructuring and rewriting for v2.1.x compliance&n;&n;   Wed Oct 14 17:19:21 CET 1998 (David Weinehall)&n;   - Added code that unregisters irq and proc-info&n;   - Version# bump&n;&n;   Mon Nov 16 15:28:23 CET 1998 (Wim Dumon)&n;   - pass &squot;dev&squot; as last parameter of request_irq in stead of &squot;NULL&squot;   &n;&n;   Wed Feb  7 21:24:00 CET 2001 (Alfred Arnold)&n;   - added support for the D-Link DE-320CT&n;   &n;   *    WARNING&n;&t;-------&n;&t;This is alpha-test software.  It is not guaranteed to work. As a&n;&t;matter of fact, I&squot;m quite sure there are *LOTS* of bugs in here. I&n;&t;would like to hear from you if you use this driver, even if it works.&n;&t;If it doesn&squot;t work, be sure to send me a mail with the problems !&n;*/
...
suffix:semicolon
multiline_comment|/*&n; * special code to read the DE-320&squot;s MAC address EEPROM.  In contrast to a &n; * standard NE design, this is a serial EEPROM (93C46) that has to be read&n; * bit by bit.  The EEPROM cotrol port at base + 0x1e has the following &n; * layout:&n; *&n; * Bit 0 = Data out (read from EEPROM)&n; * Bit 1 = Data in  (write to EEPROM)&n; * Bit 2 = Clock&n; * Bit 3 = Chip Select&n; * Bit 7 = ~50 kHz clock for defined delays&n; *&n; */
...
id|v2
suffix:semicolon
multiline_comment|/* write the value to the NIC EEPROM register */
id|outb
c_func
(paren
id|value
...
suffix:semicolon
multiline_comment|/* now wait the clock line to toggle twice.  Effectively, we are&n;&t;   waiting (at least) for one clock cycle */
r_for
c_loop
(paren
id|z
...
(brace
multiline_comment|/* shift data bit into correct position */
id|bit
op_assign
id|bit
op_lshift
l_int|1
suffix:semicolon
multiline_comment|/* write value, keep clock line high for two cycles */
id|dlink_put_eeprom
c_func
(paren
l_int|0x09
...
id|z
suffix:semicolon
multiline_comment|/* adjust bits so that they are left-aligned in a 16-bit-word */
id|value
op_assign
id|value
op_lshift
...
suffix:semicolon
multiline_comment|/* shift bits out to the EEPROM */
r_for
c_loop
(paren
id|z
...
DECL|function|dlink_get_eeprom
r_static
r_int
r_int
id|__init
...
l_int|0
suffix:semicolon
multiline_comment|/* pull the CS line low for a moment.  This resets the EEPROM-&n;&t;   internal logic, and makes it ready for a new command. */
id|dlink_put_eeprom
c_func
(paren
l_int|0x01
...
suffix:semicolon
multiline_comment|/* send one start bit, read command (1 - 0), plus the address to&n;           the EEPROM */
id|dlink_send_eeprom_word
c_func
(paren
l_int|0x0180
...
suffix:semicolon
multiline_comment|/* get the data word.  We clock by sending 0s to the EEPROM, which&n;&t;   get ignored during the read process */
r_for
c_loop
(paren
id|z
...
multiline_comment|/* handle different POS register structure for D-Link card */
r_if
c_cond
(paren
id|mca_read_stored_pos
...
l_int|1
suffix:semicolon
multiline_comment|/* printk(&quot;Halleluja sdog, als er na de pijl een 1 staat is 1 - 1 == 0&quot;&n;&t;   &t;&quot; en zou het moeten werken -&gt; %d&bslash;n&quot;, i);&n;&t;   &t;The above line was for remote testing, thanx to sdog ... */
id|base_addr
op_assign
id|addresses
(braket
...
multiline_comment|/* I don&squot;t know whether the previous sequence includes the general&n;           board reset procedure, so better don&squot;t omit it and just overwrite&n;           the garbage read from a DE-320 with correct stuff. */
r_if
c_cond
(paren
id|mca_read_stored_pos
