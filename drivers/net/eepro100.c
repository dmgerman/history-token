...
id|mii_if
suffix:semicolon
multiline_comment|/* MII API hooks, info */
...
(paren
id|KERN_ALERT
l_string|&quot;%s: wait_for_cmd_done timeout!&bslash;n&quot;
comma
id|dev-&gt;name
)paren
suffix:semicolon
...
multiline_comment|/* when built-in, we only print version if device is found */
r_static
r_int
id|did_version
suffix:semicolon
...
id|dev
suffix:semicolon
id|sp-&gt;mii_if.mdio_read
op_assign
id|mdio_read
suffix:semicolon
id|sp-&gt;mii_if.mdio_write
op_assign
id|mdio_write
suffix:semicolon
id|sp-&gt;rx_bug
...
(paren
id|KERN_ERR
l_string|&quot;Command %4.4x was not accepted after %d polls!&quot;
l_string|&quot;  Current status %8.8x.&bslash;n&quot;
comma
id|cmd
comma
id|wait
...
suffix:semicolon
multiline_comment|/* Bogus, but it avoids the bug. */
multiline_comment|/* Note: these next two operations can take a while. */
id|do_slow_command
c_func
(paren
id|dev
...
multiline_comment|/* Note: RxStart should complete instantly. */
id|do_slow_command
c_func
(paren
id|dev
...
multiline_comment|/*&n; * Sometimes the receiver stops making progress.  This routine knows how to&n; * get it going again, without losing packets or being otherwise nasty like&n; * a chip reset would be.  Previously the driver had a whole sequence&n; * of if RxSuspended, if it&squot;s no buffers do one thing, if it&squot;s no resources,&n; * do another, etc.  But those things don&squot;t really matter.  Separate logic&n; * in the ISR provides for allocating buffers--the other half of operation&n; * is just making sure the receiver is active.  speedo_rx_soft_reset does that.&n; * This problem with the old, more involved algorithm is shown up under&n; * ping floods on the order of 60K packets/second on a 100Mbps fdx network.&n; */
r_static
r_void
DECL|function|speedo_rx_soft_reset
id|speedo_rx_soft_reset
c_func
(paren
r_struct
id|net_device
...
c_func
(paren
l_string|&quot;%s: previous command stalled&bslash;n&quot;
comma
id|dev-&gt;name
)paren
suffix:semicolon
...
c_func
(paren
l_string|&quot;%s: RxAbort command stalled&bslash;n&quot;
comma
id|dev-&gt;name
)paren
suffix:semicolon
...
suffix:semicolon
multiline_comment|/* XXX: do we really want to call this before the NULL check? --hch */
id|rx_align
c_func
(paren
id|skb
...
suffix:semicolon
multiline_comment|/*&n;&t;&t; * The chip may have suspended reception for various reasons.&n;&t;&t; * Check for that, and re-prime it should this be the case.&n;&t;&t; */
r_switch
c_cond
(paren
(paren
...
l_int|1
suffix:colon
multiline_comment|/* Suspended */
r_case
l_int|2
suffix:colon
multiline_comment|/* No resources (RxFDs) */
r_case
l_int|9
suffix:colon
multiline_comment|/* Suspended with no more RBDs */
r_case
l_int|10
suffix:colon
multiline_comment|/* No resources due to no RBDs */
r_case
l_int|12
suffix:colon
multiline_comment|/* Ready with no RBDs */
id|speedo_rx_soft_reset
c_func
(paren
id|dev
...
l_int|15
suffix:colon
multiline_comment|/* these are all reserved values */
r_break
suffix:semicolon
)brace
...
multiline_comment|/*&n;&t; * The chip requires a 10 microsecond quiet period.  Wait here!&n;&t; */
id|udelay
c_func
(paren
l_int|10
...
DECL|function|speedo_get_drvinfo
r_static
r_void
id|speedo_get_drvinfo
c_func
(paren
r_struct
id|net_device
...
DECL|function|speedo_get_settings
r_static
r_int
id|speedo_get_settings
c_func
(paren
r_struct
id|net_device
...
DECL|function|speedo_set_settings
r_static
r_int
id|speedo_set_settings
c_func
(paren
r_struct
id|net_device
...
DECL|function|speedo_nway_reset
r_static
r_int
id|speedo_nway_reset
c_func
(paren
r_struct
id|net_device
...
DECL|function|speedo_get_msglevel
r_static
id|u32
id|speedo_get_msglevel
c_func
(paren
r_struct
id|net_device
...
DECL|function|speedo_set_msglevel
r_static
r_void
id|speedo_set_msglevel
c_func
(paren
r_struct
id|net_device
...
"&f;"
DECL|variable|eepro100_pci_tbl
r_static
r_struct
id|pci_device_id
id|eepro100_pci_tbl
