multiline_comment|/*&n; *  ==FILEVERSION 980319==&n; *&n; * ppp_deflate.c - interface the zlib procedures for Deflate compression&n; * and decompression (as used by gzip) to the PPP code.&n; * This version is for use with Linux kernel 1.3.X.&n; *&n; * Copyright (c) 1994 The Australian National University.&n; * All rights reserved.&n; *&n; * Permission to use, copy, modify, and distribute this software and its&n; * documentation is hereby granted, provided that the above copyright&n; * notice appears in all copies.  This software is provided without any&n; * warranty, express or implied. The Australian National University&n; * makes no representations about the suitability of this software for&n; * any purpose.&n; *&n; * IN NO EVENT SHALL THE AUSTRALIAN NATIONAL UNIVERSITY BE LIABLE TO ANY&n; * PARTY FOR DIRECT, INDIRECT, SPECIAL, INCIDENTAL, OR CONSEQUENTIAL DAMAGES&n; * ARISING OUT OF THE USE OF THIS SOFTWARE AND ITS DOCUMENTATION, EVEN IF&n; * THE AUSTRALIAN NATIONAL UNIVERSITY HAS BEEN ADVISED OF THE POSSIBILITY&n; * OF SUCH DAMAGE.&n; *&n; * THE AUSTRALIAN NATIONAL UNIVERSITY SPECIFICALLY DISCLAIMS ANY WARRANTIES,&n; * INCLUDING, BUT NOT LIMITED TO, THE IMPLIED WARRANTIES OF MERCHANTABILITY&n; * AND FITNESS FOR A PARTICULAR PURPOSE.  THE SOFTWARE PROVIDED HEREUNDER IS&n; * ON AN &quot;AS IS&quot; BASIS, AND THE AUSTRALIAN NATIONAL UNIVERSITY HAS NO&n; * OBLIGATION TO PROVIDE MAINTENANCE, SUPPORT, UPDATES, ENHANCEMENTS,&n; * OR MODIFICATIONS.&n; *&n; * From: deflate.c,v 1.1 1996/01/18 03:17:48 paulus Exp&n; */
macro_line|#include &lt;linux/module.h&gt;
macro_line|#include &lt;linux/slab.h&gt;
macro_line|#include &lt;linux/vmalloc.h&gt;
macro_line|#include &lt;linux/init.h&gt;
macro_line|#include &lt;linux/string.h&gt;
macro_line|#include &lt;linux/ppp_defs.h&gt;
macro_line|#include &lt;linux/ppp-comp.h&gt;
macro_line|#include &lt;linux/zlib.h&gt;
multiline_comment|/*&n; * State for a Deflate (de)compressor.&n; */
DECL|struct|ppp_deflate_state
r_struct
id|ppp_deflate_state
(brace
DECL|member|seqno
r_int
id|seqno
suffix:semicolon
DECL|member|w_size
r_int
id|w_size
suffix:semicolon
DECL|member|unit
r_int
id|unit
suffix:semicolon
DECL|member|mru
r_int
id|mru
suffix:semicolon
DECL|member|debug
r_int
id|debug
suffix:semicolon
DECL|member|strm
id|z_stream
id|strm
suffix:semicolon
DECL|member|stats
r_struct
id|compstat
id|stats
suffix:semicolon
)brace
suffix:semicolon
DECL|macro|DEFLATE_OVHD
mdefine_line|#define DEFLATE_OVHD&t;2&t;&t;/* Deflate overhead/packet */
r_static
r_void
op_star
id|z_comp_alloc
c_func
(paren
r_int
r_char
op_star
id|options
comma
r_int
id|opt_len
)paren
suffix:semicolon
r_static
r_void
op_star
id|z_decomp_alloc
c_func
(paren
r_int
r_char
op_star
id|options
comma
r_int
id|opt_len
)paren
suffix:semicolon
r_static
r_void
id|z_comp_free
c_func
(paren
r_void
op_star
id|state
)paren
suffix:semicolon
r_static
r_void
id|z_decomp_free
c_func
(paren
r_void
op_star
id|state
)paren
suffix:semicolon
r_static
r_int
id|z_comp_init
c_func
(paren
r_void
op_star
id|state
comma
r_int
r_char
op_star
id|options
comma
r_int
id|opt_len
comma
r_int
id|unit
comma
r_int
id|hdrlen
comma
r_int
id|debug
)paren
suffix:semicolon
r_static
r_int
id|z_decomp_init
c_func
(paren
r_void
op_star
id|state
comma
r_int
r_char
op_star
id|options
comma
r_int
id|opt_len
comma
r_int
id|unit
comma
r_int
id|hdrlen
comma
r_int
id|mru
comma
r_int
id|debug
)paren
suffix:semicolon
r_static
r_int
id|z_compress
c_func
(paren
r_void
op_star
id|state
comma
r_int
r_char
op_star
id|rptr
comma
r_int
r_char
op_star
id|obuf
comma
r_int
id|isize
comma
r_int
id|osize
)paren
suffix:semicolon
r_static
r_void
id|z_incomp
c_func
(paren
r_void
op_star
id|state
comma
r_int
r_char
op_star
id|ibuf
comma
r_int
id|icnt
)paren
suffix:semicolon
r_static
r_int
id|z_decompress
c_func
(paren
r_void
op_star
id|state
comma
r_int
r_char
op_star
id|ibuf
comma
r_int
id|isize
comma
r_int
r_char
op_star
id|obuf
comma
r_int
id|osize
)paren
suffix:semicolon
r_static
r_void
id|z_comp_reset
c_func
(paren
r_void
op_star
id|state
)paren
suffix:semicolon
r_static
r_void
id|z_decomp_reset
c_func
(paren
r_void
op_star
id|state
)paren
suffix:semicolon
r_static
r_void
id|z_comp_stats
c_func
(paren
r_void
op_star
id|state
comma
r_struct
id|compstat
op_star
id|stats
)paren
suffix:semicolon
multiline_comment|/**&n; *&t;z_comp_free - free the memory used by a compressor&n; *&t;@arg:&t;pointer to the private state for the compressor.&n; */
DECL|function|z_comp_free
r_static
r_void
id|z_comp_free
c_func
(paren
r_void
op_star
id|arg
)paren
(brace
r_struct
id|ppp_deflate_state
op_star
id|state
op_assign
(paren
r_struct
id|ppp_deflate_state
op_star
)paren
id|arg
suffix:semicolon
r_if
c_cond
(paren
id|state
)paren
(brace
id|zlib_deflateEnd
c_func
(paren
op_amp
id|state-&gt;strm
)paren
suffix:semicolon
r_if
c_cond
(paren
id|state-&gt;strm.workspace
)paren
id|vfree
c_func
(paren
id|state-&gt;strm.workspace
)paren
suffix:semicolon
id|kfree
c_func
(paren
id|state
)paren
suffix:semicolon
)brace
)brace
multiline_comment|/**&n; *&t;z_comp_alloc - allocate space for a compressor.&n; *&t;@options: pointer to CCP option data&n; *&t;@opt_len: length of the CCP option at @options.&n; *&n; *&t;The @options pointer points to the a buffer containing the&n; *&t;CCP option data for the compression being negotiated.  It is&n; *&t;formatted according to RFC1979, and describes the window&n; *&t;size that the peer is requesting that we use in compressing&n; *&t;data to be sent to it.&n; *&n; *&t;Returns the pointer to the private state for the compressor,&n; *&t;or NULL if we could not allocate enough memory.&n; */
DECL|function|z_comp_alloc
r_static
r_void
op_star
id|z_comp_alloc
c_func
(paren
r_int
r_char
op_star
id|options
comma
r_int
id|opt_len
)paren
(brace
r_struct
id|ppp_deflate_state
op_star
id|state
suffix:semicolon
r_int
id|w_size
suffix:semicolon
r_if
c_cond
(paren
id|opt_len
op_ne
id|CILEN_DEFLATE
op_logical_or
(paren
id|options
(braket
l_int|0
)braket
op_ne
id|CI_DEFLATE
op_logical_and
id|options
(braket
l_int|0
)braket
op_ne
id|CI_DEFLATE_DRAFT
)paren
op_logical_or
id|options
(braket
l_int|1
)braket
op_ne
id|CILEN_DEFLATE
op_logical_or
id|DEFLATE_METHOD
c_func
(paren
id|options
(braket
l_int|2
)braket
)paren
op_ne
id|DEFLATE_METHOD_VAL
op_logical_or
id|options
(braket
l_int|3
)braket
op_ne
id|DEFLATE_CHK_SEQUENCE
)paren
r_return
l_int|NULL
suffix:semicolon
id|w_size
op_assign
id|DEFLATE_SIZE
c_func
(paren
id|options
(braket
l_int|2
)braket
)paren
suffix:semicolon
r_if
c_cond
(paren
id|w_size
template_param
id|DEFLATE_MAX_SIZE
)paren
r_return
l_int|NULL
suffix:semicolon
id|state
op_assign
(paren
r_struct
id|ppp_deflate_state
op_star
)paren
id|kmalloc
c_func
(paren
r_sizeof
(paren
op_star
id|state
)paren
comma
id|GFP_KERNEL
)paren
suffix:semicolon
r_if
c_cond
(paren
id|state
op_eq
l_int|NULL
)paren
r_return
l_int|NULL
suffix:semicolon
id|memset
(paren
id|state
comma
l_int|0
comma
r_sizeof
(paren
r_struct
id|ppp_deflate_state
)paren
)paren
suffix:semicolon
id|state-&gt;strm.next_in
op_assign
l_int|NULL
suffix:semicolon
id|state-&gt;w_size
op_assign
id|w_size
suffix:semicolon
id|state-&gt;strm.workspace
op_assign
id|vmalloc
c_func
(paren
id|zlib_deflate_workspacesize
c_func
(paren
)paren
)paren
suffix:semicolon
r_if
c_cond
(paren
id|state-&gt;strm.workspace
op_eq
l_int|NULL
)paren
r_goto
id|out_free
suffix:semicolon
r_if
c_cond
(paren
id|zlib_deflateInit2
c_func
(paren
op_amp
id|state-&gt;strm
comma
id|Z_DEFAULT_COMPRESSION
comma
id|DEFLATE_METHOD_VAL
comma
op_minus
id|w_size
comma
l_int|8
comma
id|Z_DEFAULT_STRATEGY
)paren
op_ne
id|Z_OK
)paren
r_goto
id|out_free
suffix:semicolon
r_return
(paren
r_void
op_star
)paren
id|state
suffix:semicolon
id|out_free
suffix:colon
id|z_comp_free
c_func
(paren
id|state
)paren
suffix:semicolon
r_return
l_int|NULL
suffix:semicolon
)brace
multiline_comment|/**&n; *&t;z_comp_init - initialize a previously-allocated compressor.&n; *&t;@arg:&t;pointer to the private state for the compressor&n; *&t;@options: pointer to the CCP option data describing the&n; *&t;&t;compression that was negotiated with the peer&n; *&t;@opt_len: length of the CCP option data at @options&n; *&t;@unit:&t;PPP unit number for diagnostic messages&n; *&t;@hdrlen: ignored (present for backwards compatibility)&n; *&t;@debug:&t;debug flag; if non-zero, debug messages are printed.&n; *&n; *&t;The CCP options described by @options must match the options&n; *&t;specified when the compressor was allocated.  The compressor&n; *&t;history is reset.  Returns 0 for failure (CCP options don&squot;t&n; *&t;match) or 1 for success.&n; */
DECL|function|z_comp_init
r_static
r_int
id|z_comp_init
c_func
(paren
r_void
op_star
id|arg
comma
r_int
r_char
op_star
id|options
comma
r_int
id|opt_len
comma
r_int
id|unit
comma
r_int
id|hdrlen
comma
r_int
id|debug
)paren
(brace
r_struct
id|ppp_deflate_state
op_star
id|state
op_assign
(paren
r_struct
id|ppp_deflate_state
op_star
)paren
id|arg
suffix:semicolon
r_if
c_cond
(paren
id|opt_len
OL
id|CILEN_DEFLATE
op_logical_or
(paren
id|options
(braket
l_int|0
)braket
op_ne
id|CI_DEFLATE
op_logical_and
id|options
(braket
l_int|0
)braket
op_ne
id|CI_DEFLATE_DRAFT
)paren
op_logical_or
id|options
(braket
l_int|1
)braket
op_ne
id|CILEN_DEFLATE
op_logical_or
id|DEFLATE_METHOD
c_func
(paren
id|options
(braket
l_int|2
)braket
)paren
op_ne
id|DEFLATE_METHOD_VAL
op_logical_or
id|DEFLATE_SIZE
c_func
(paren
id|options
(braket
l_int|2
)braket
)paren
op_ne
id|state-&gt;w_size
op_logical_or
id|options
(braket
l_int|3
)braket
op_ne
id|DEFLATE_CHK_SEQUENCE
)paren
r_return
l_int|0
suffix:semicolon
id|state-&gt;seqno
op_assign
l_int|0
suffix:semicolon
id|state-&gt;unit
op_assign
id|unit
suffix:semicolon
id|state-&gt;debug
op_assign
id|debug
suffix:semicolon
id|zlib_deflateReset
c_func
(paren
op_amp
id|state-&gt;strm
)paren
suffix:semicolon
r_return
l_int|1
suffix:semicolon
)brace
multiline_comment|/**&n; *&t;z_comp_reset - reset a previously-allocated compressor.&n; *&t;@arg:&t;pointer to private state for the compressor.&n; *&n; *&t;This clears the history for the compressor and makes it&n; *&t;ready to start emitting a new compressed stream.&n; */
DECL|function|z_comp_reset
r_static
r_void
id|z_comp_reset
c_func
(paren
r_void
op_star
id|arg
)paren
(brace
r_struct
id|ppp_deflate_state
op_star
id|state
op_assign
(paren
r_struct
id|ppp_deflate_state
op_star
)paren
id|arg
suffix:semicolon
id|state-&gt;seqno
op_assign
l_int|0
suffix:semicolon
id|zlib_deflateReset
c_func
(paren
op_amp
id|state-&gt;strm
)paren
suffix:semicolon
)brace
multiline_comment|/**&n; *&t;z_compress - compress a PPP packet with Deflate compression.&n; *&t;@arg:&t;pointer to private state for the compressor&n; *&t;@rptr:&t;uncompressed packet (input)&n; *&t;@obuf:&t;compressed packet (output)&n; *&t;@isize:&t;size of uncompressed packet&n; *&t;@osize:&t;space available at @obuf&n; *&n; *&t;Returns the length of the compressed packet, or 0 if the&n; *&t;packet is incompressible.&n; */
DECL|function|z_compress
r_int
id|z_compress
c_func
(paren
r_void
op_star
id|arg
comma
r_int
r_char
op_star
id|rptr
comma
r_int
r_char
op_star
id|obuf
comma
r_int
id|isize
comma
r_int
id|osize
)paren
(brace
r_struct
id|ppp_deflate_state
op_star
id|state
op_assign
(paren
r_struct
id|ppp_deflate_state
op_star
)paren
id|arg
suffix:semicolon
r_int
id|r
comma
id|proto
comma
id|off
comma
id|olen
comma
id|oavail
suffix:semicolon
r_int
r_char
op_star
id|wptr
suffix:semicolon
multiline_comment|/*&n;&t; * Check that the protocol is in the range we handle.&n;&t; */
id|proto
op_assign
id|PPP_PROTOCOL
c_func
(paren
id|rptr
)paren
suffix:semicolon
r_if
c_cond
(paren
id|proto
OG
l_int|0x3fff
op_logical_or
id|proto
op_eq
l_int|0xfd
op_logical_or
id|proto
op_eq
l_int|0xfb
)paren
r_return
l_int|0
suffix:semicolon
multiline_comment|/* Don&squot;t generate compressed packets which are larger than&n;&t;   the uncompressed packet. */
r_if
c_cond
(paren
id|osize
OG
id|isize
)paren
id|osize
op_assign
id|isize
suffix:semicolon
id|wptr
op_assign
id|obuf
suffix:semicolon
multiline_comment|/*&n;&t; * Copy over the PPP header and store the 2-byte sequence number.&n;&t; */
id|wptr
(braket
l_int|0
)braket
op_assign
id|PPP_ADDRESS
c_func
(paren
id|rptr
)paren
suffix:semicolon
id|wptr
(braket
l_int|1
)braket
op_assign
id|PPP_CONTROL
c_func
(paren
id|rptr
)paren
suffix:semicolon
id|wptr
(braket
l_int|2
)braket
op_assign
id|PPP_COMP
op_rshift
l_int|8
suffix:semicolon
id|wptr
(braket
l_int|3
)braket
op_assign
id|PPP_COMP
suffix:semicolon
id|wptr
op_add_assign
id|PPP_HDRLEN
suffix:semicolon
id|wptr
(braket
l_int|0
)braket
op_assign
id|state-&gt;seqno
op_rshift
l_int|8
suffix:semicolon
id|wptr
(braket
l_int|1
)braket
op_assign
id|state-&gt;seqno
suffix:semicolon
id|wptr
op_add_assign
id|DEFLATE_OVHD
suffix:semicolon
id|olen
op_assign
id|PPP_HDRLEN
op_plus
id|DEFLATE_OVHD
suffix:semicolon
id|state-&gt;strm.next_out
op_assign
id|wptr
suffix:semicolon
id|state-&gt;strm.avail_out
op_assign
id|oavail
op_assign
id|osize
op_minus
id|olen
suffix:semicolon
op_increment
id|state-&gt;seqno
suffix:semicolon
id|off
op_assign
(paren
id|proto
OG
l_int|0xff
)paren
ques
c_cond
l_int|2
suffix:colon
l_int|3
suffix:semicolon
multiline_comment|/* skip 1st proto byte if 0 */
id|rptr
op_add_assign
id|off
suffix:semicolon
id|state-&gt;strm.next_in
op_assign
id|rptr
suffix:semicolon
id|state-&gt;strm.avail_in
op_assign
(paren
id|isize
op_minus
id|off
)paren
suffix:semicolon
r_for
c_loop
(paren
suffix:semicolon
suffix:semicolon
)paren
(brace
id|r
op_assign
id|zlib_deflate
c_func
(paren
op_amp
id|state-&gt;strm
comma
id|Z_PACKET_FLUSH
)paren
suffix:semicolon
r_if
c_cond
(paren
id|r
op_ne
id|Z_OK
)paren
(brace
r_if
c_cond
(paren
id|state-&gt;debug
)paren
id|printk
c_func
(paren
id|KERN_ERR
l_string|&quot;z_compress: deflate returned %d&bslash;n&quot;
comma
id|r
)paren
suffix:semicolon
r_break
suffix:semicolon
)brace
r_if
c_cond
(paren
id|state-&gt;strm.avail_out
op_eq
l_int|0
)paren
(brace
id|olen
op_add_assign
id|oavail
suffix:semicolon
id|state-&gt;strm.next_out
op_assign
l_int|NULL
suffix:semicolon
id|state-&gt;strm.avail_out
op_assign
id|oavail
op_assign
l_int|1000000
suffix:semicolon
)brace
r_else
(brace
r_break
suffix:semicolon
multiline_comment|/* all done */
)brace
)brace
id|olen
op_add_assign
id|oavail
op_minus
id|state-&gt;strm.avail_out
suffix:semicolon
multiline_comment|/*&n;&t; * See if we managed to reduce the size of the packet.&n;&t; */
r_if
c_cond
(paren
id|olen
OL
id|isize
)paren
(brace
id|state-&gt;stats.comp_bytes
op_add_assign
id|olen
suffix:semicolon
id|state-&gt;stats.comp_packets
op_increment
suffix:semicolon
)brace
r_else
(brace
id|state-&gt;stats.inc_bytes
op_add_assign
id|isize
suffix:semicolon
id|state-&gt;stats.inc_packets
op_increment
suffix:semicolon
id|olen
op_assign
l_int|0
suffix:semicolon
)brace
id|state-&gt;stats.unc_bytes
op_add_assign
id|isize
suffix:semicolon
id|state-&gt;stats.unc_packets
op_increment
suffix:semicolon
r_return
id|olen
suffix:semicolon
)brace
multiline_comment|/**&n; *&t;z_comp_stats - return compression statistics for a compressor&n; *&t;&t;or decompressor.&n; *&t;@arg:&t;pointer to private space for the (de)compressor&n; *&t;@stats:&t;pointer to a struct compstat to receive the result.&n; */
DECL|function|z_comp_stats
r_static
r_void
id|z_comp_stats
c_func
(paren
r_void
op_star
id|arg
comma
r_struct
id|compstat
op_star
id|stats
)paren
(brace
r_struct
id|ppp_deflate_state
op_star
id|state
op_assign
(paren
r_struct
id|ppp_deflate_state
op_star
)paren
id|arg
suffix:semicolon
op_star
id|stats
op_assign
id|state-&gt;stats
suffix:semicolon
)brace
multiline_comment|/**&n; *&t;z_decomp_free - Free the memory used by a decompressor.&n; *&t;@arg:&t;pointer to private space for the decompressor.&n; */
DECL|function|z_decomp_free
r_static
r_void
id|z_decomp_free
c_func
(paren
r_void
op_star
id|arg
)paren
(brace
r_struct
id|ppp_deflate_state
op_star
id|state
op_assign
(paren
r_struct
id|ppp_deflate_state
op_star
)paren
id|arg
suffix:semicolon
r_if
c_cond
(paren
id|state
)paren
(brace
id|zlib_inflateEnd
c_func
(paren
op_amp
id|state-&gt;strm
)paren
suffix:semicolon
r_if
c_cond
(paren
id|state-&gt;strm.workspace
)paren
id|kfree
c_func
(paren
id|state-&gt;strm.workspace
)paren
suffix:semicolon
id|kfree
c_func
(paren
id|state
)paren
suffix:semicolon
)brace
)brace
multiline_comment|/**&n; *&t;z_decomp_alloc - allocate space for a decompressor.&n; *&t;@options: pointer to CCP option data&n; *&t;@opt_len: length of the CCP option at @options.&n; *&n; *&t;The @options pointer points to the a buffer containing the&n; *&t;CCP option data for the compression being negotiated.  It is&n; *&t;formatted according to RFC1979, and describes the window&n; *&t;size that we are requesting the peer to use in compressing&n; *&t;data to be sent to us.&n; *&n; *&t;Returns the pointer to the private state for the decompressor,&n; *&t;or NULL if we could not allocate enough memory.&n; */
DECL|function|z_decomp_alloc
r_static
r_void
op_star
id|z_decomp_alloc
c_func
(paren
r_int
r_char
op_star
id|options
comma
r_int
id|opt_len
)paren
(brace
r_struct
id|ppp_deflate_state
op_star
id|state
suffix:semicolon
r_int
id|w_size
suffix:semicolon
r_if
c_cond
(paren
id|opt_len
op_ne
id|CILEN_DEFLATE
op_logical_or
(paren
id|options
(braket
l_int|0
)braket
op_ne
id|CI_DEFLATE
op_logical_and
id|options
(braket
l_int|0
)braket
op_ne
id|CI_DEFLATE_DRAFT
)paren
op_logical_or
id|options
(braket
l_int|1
)braket
op_ne
id|CILEN_DEFLATE
op_logical_or
id|DEFLATE_METHOD
c_func
(paren
id|options
(braket
l_int|2
)braket
)paren
op_ne
id|DEFLATE_METHOD_VAL
op_logical_or
id|options
(braket
l_int|3
)braket
op_ne
id|DEFLATE_CHK_SEQUENCE
)paren
r_return
l_int|NULL
suffix:semicolon
id|w_size
op_assign
id|DEFLATE_SIZE
c_func
(paren
id|options
(braket
l_int|2
)braket
)paren
suffix:semicolon
r_if
c_cond
(paren
id|w_size
template_param
id|DEFLATE_MAX_SIZE
)paren
r_return
l_int|NULL
suffix:semicolon
id|state
op_assign
(paren
r_struct
id|ppp_deflate_state
op_star
)paren
id|kmalloc
c_func
(paren
r_sizeof
(paren
op_star
id|state
)paren
comma
id|GFP_KERNEL
)paren
suffix:semicolon
r_if
c_cond
(paren
id|state
op_eq
l_int|NULL
)paren
r_return
l_int|NULL
suffix:semicolon
id|memset
(paren
id|state
comma
l_int|0
comma
r_sizeof
(paren
r_struct
id|ppp_deflate_state
)paren
)paren
suffix:semicolon
id|state-&gt;w_size
op_assign
id|w_size
suffix:semicolon
id|state-&gt;strm.next_out
op_assign
l_int|NULL
suffix:semicolon
id|state-&gt;strm.workspace
op_assign
id|kmalloc
c_func
(paren
id|zlib_inflate_workspacesize
c_func
(paren
)paren
comma
id|GFP_KERNEL
)paren
suffix:semicolon
r_if
c_cond
(paren
id|state-&gt;strm.workspace
op_eq
l_int|NULL
)paren
r_goto
id|out_free
suffix:semicolon
r_if
c_cond
(paren
id|zlib_inflateInit2
c_func
(paren
op_amp
id|state-&gt;strm
comma
op_minus
id|w_size
)paren
op_ne
id|Z_OK
)paren
r_goto
id|out_free
suffix:semicolon
r_return
(paren
r_void
op_star
)paren
id|state
suffix:semicolon
id|out_free
suffix:colon
id|z_decomp_free
c_func
(paren
id|state
)paren
suffix:semicolon
r_return
l_int|NULL
suffix:semicolon
)brace
multiline_comment|/**&n; *&t;z_decomp_init - initialize a previously-allocated decompressor.&n; *&t;@arg:&t;pointer to the private state for the decompressor&n; *&t;@options: pointer to the CCP option data describing the&n; *&t;&t;compression that was negotiated with the peer&n; *&t;@opt_len: length of the CCP option data at @options&n; *&t;@unit:&t;PPP unit number for diagnostic messages&n; *&t;@hdrlen: ignored (present for backwards compatibility)&n; *&t;@mru:&t;maximum length of decompressed packets&n; *&t;@debug:&t;debug flag; if non-zero, debug messages are printed.&n; *&n; *&t;The CCP options described by @options must match the options&n; *&t;specified when the decompressor was allocated.  The decompressor&n; *&t;history is reset.  Returns 0 for failure (CCP options don&squot;t&n; *&t;match) or 1 for success.&n; */
DECL|function|z_decomp_init
r_static
r_int
id|z_decomp_init
c_func
(paren
r_void
op_star
id|arg
comma
r_int
r_char
op_star
id|options
comma
r_int
id|opt_len
comma
r_int
id|unit
comma
r_int
id|hdrlen
comma
r_int
id|mru
comma
r_int
id|debug
)paren
(brace
r_struct
id|ppp_deflate_state
op_star
id|state
op_assign
(paren
r_struct
id|ppp_deflate_state
op_star
)paren
id|arg
suffix:semicolon
r_if
c_cond
(paren
id|opt_len
OL
id|CILEN_DEFLATE
op_logical_or
(paren
id|options
(braket
l_int|0
)braket
op_ne
id|CI_DEFLATE
op_logical_and
id|options
(braket
l_int|0
)braket
op_ne
id|CI_DEFLATE_DRAFT
)paren
op_logical_or
id|options
(braket
l_int|1
)braket
op_ne
id|CILEN_DEFLATE
op_logical_or
id|DEFLATE_METHOD
c_func
(paren
id|options
(braket
l_int|2
)braket
)paren
op_ne
id|DEFLATE_METHOD_VAL
op_logical_or
id|DEFLATE_SIZE
c_func
(paren
id|options
(braket
l_int|2
)braket
)paren
op_ne
id|state-&gt;w_size
op_logical_or
id|options
(braket
l_int|3
)braket
op_ne
id|DEFLATE_CHK_SEQUENCE
)paren
r_return
l_int|0
suffix:semicolon
id|state-&gt;seqno
op_assign
l_int|0
suffix:semicolon
id|state-&gt;unit
op_assign
id|unit
suffix:semicolon
id|state-&gt;debug
op_assign
id|debug
suffix:semicolon
id|state-&gt;mru
op_assign
id|mru
suffix:semicolon
id|zlib_inflateReset
c_func
(paren
op_amp
id|state-&gt;strm
)paren
suffix:semicolon
r_return
l_int|1
suffix:semicolon
)brace
multiline_comment|/**&n; *&t;z_decomp_reset - reset a previously-allocated decompressor.&n; *&t;@arg:&t;pointer to private state for the decompressor.&n; *&n; *&t;This clears the history for the decompressor and makes it&n; *&t;ready to receive a new compressed stream.&n; */
DECL|function|z_decomp_reset
r_static
r_void
id|z_decomp_reset
c_func
(paren
r_void
op_star
id|arg
)paren
(brace
r_struct
id|ppp_deflate_state
op_star
id|state
op_assign
(paren
r_struct
id|ppp_deflate_state
op_star
)paren
id|arg
suffix:semicolon
id|state-&gt;seqno
op_assign
l_int|0
suffix:semicolon
id|zlib_inflateReset
c_func
(paren
op_amp
id|state-&gt;strm
)paren
suffix:semicolon
)brace
multiline_comment|/**&n; *&t;z_decompress - decompress a Deflate-compressed packet.&n; *&t;@arg:&t;pointer to private state for the decompressor&n; *&t;@ibuf:&t;pointer to input (compressed) packet data&n; *&t;@isize:&t;length of input packet&n; *&t;@obuf:&t;pointer to space for output (decompressed) packet&n; *&t;@osize:&t;amount of space available at @obuf&n; *&n; * Because of patent problems, we return DECOMP_ERROR for errors&n; * found by inspecting the input data and for system problems, but&n; * DECOMP_FATALERROR for any errors which could possibly be said to&n; * be being detected &quot;after&quot; decompression.  For DECOMP_ERROR,&n; * we can issue a CCP reset-request; for DECOMP_FATALERROR, we may be&n; * infringing a patent of Motorola&squot;s if we do, so we take CCP down&n; * instead.&n; *&n; * Given that the frame has the correct sequence number and a good FCS,&n; * errors such as invalid codes in the input most likely indicate a&n; * bug, so we return DECOMP_FATALERROR for them in order to turn off&n; * compression, even though they are detected by inspecting the input.&n; */
DECL|function|z_decompress
r_int
id|z_decompress
c_func
(paren
r_void
op_star
id|arg
comma
r_int
r_char
op_star
id|ibuf
comma
r_int
id|isize
comma
r_int
r_char
op_star
id|obuf
comma
r_int
id|osize
)paren
(brace
r_struct
id|ppp_deflate_state
op_star
id|state
op_assign
(paren
r_struct
id|ppp_deflate_state
op_star
)paren
id|arg
suffix:semicolon
r_int
id|olen
comma
id|seq
comma
id|r
suffix:semicolon
r_int
id|decode_proto
comma
id|overflow
suffix:semicolon
r_int
r_char
id|overflow_buf
(braket
l_int|1
)braket
suffix:semicolon
r_if
c_cond
(paren
id|isize
op_le
id|PPP_HDRLEN
op_plus
id|DEFLATE_OVHD
)paren
(brace
r_if
c_cond
(paren
id|state-&gt;debug
)paren
id|printk
c_func
(paren
id|KERN_DEBUG
l_string|&quot;z_decompress%d: short pkt (%d)&bslash;n&quot;
comma
id|state-&gt;unit
comma
id|isize
)paren
suffix:semicolon
r_return
id|DECOMP_ERROR
suffix:semicolon
)brace
multiline_comment|/* Check the sequence number. */
id|seq
op_assign
(paren
id|ibuf
(braket
id|PPP_HDRLEN
)braket
op_lshift
l_int|8
)paren
op_plus
id|ibuf
(braket
id|PPP_HDRLEN
op_plus
l_int|1
)braket
suffix:semicolon
r_if
c_cond
(paren
id|seq
op_ne
id|state-&gt;seqno
)paren
(brace
r_if
c_cond
(paren
id|state-&gt;debug
)paren
id|printk
c_func
(paren
id|KERN_DEBUG
l_string|&quot;z_decompress%d: bad seq # %d, expected %d&bslash;n&quot;
comma
id|state-&gt;unit
comma
id|seq
comma
id|state-&gt;seqno
)paren
suffix:semicolon
r_return
id|DECOMP_ERROR
suffix:semicolon
)brace
op_increment
id|state-&gt;seqno
suffix:semicolon
multiline_comment|/*&n;&t; * Fill in the first part of the PPP header.  The protocol field&n;&t; * comes from the decompressed data.&n;&t; */
id|obuf
(braket
l_int|0
)braket
op_assign
id|PPP_ADDRESS
c_func
(paren
id|ibuf
)paren
suffix:semicolon
id|obuf
(braket
l_int|1
)braket
op_assign
id|PPP_CONTROL
c_func
(paren
id|ibuf
)paren
suffix:semicolon
id|obuf
(braket
l_int|2
)braket
op_assign
l_int|0
suffix:semicolon
multiline_comment|/*&n;&t; * Set up to call inflate.  We set avail_out to 1 initially so we can&n;&t; * look at the first byte of the output and decide whether we have&n;&t; * a 1-byte or 2-byte protocol field.&n;&t; */
id|state-&gt;strm.next_in
op_assign
id|ibuf
op_plus
id|PPP_HDRLEN
op_plus
id|DEFLATE_OVHD
suffix:semicolon
id|state-&gt;strm.avail_in
op_assign
id|isize
op_minus
(paren
id|PPP_HDRLEN
op_plus
id|DEFLATE_OVHD
)paren
suffix:semicolon
id|state-&gt;strm.next_out
op_assign
id|obuf
op_plus
l_int|3
suffix:semicolon
id|state-&gt;strm.avail_out
op_assign
l_int|1
suffix:semicolon
id|decode_proto
op_assign
l_int|1
suffix:semicolon
id|overflow
op_assign
l_int|0
suffix:semicolon
multiline_comment|/*&n;&t; * Call inflate, supplying more input or output as needed.&n;&t; */
r_for
c_loop
(paren
suffix:semicolon
suffix:semicolon
)paren
(brace
id|r
op_assign
id|zlib_inflate
c_func
(paren
op_amp
id|state-&gt;strm
comma
id|Z_PACKET_FLUSH
)paren
suffix:semicolon
r_if
c_cond
(paren
id|r
op_ne
id|Z_OK
)paren
(brace
r_if
c_cond
(paren
id|state-&gt;debug
)paren
id|printk
c_func
(paren
id|KERN_DEBUG
l_string|&quot;z_decompress%d: inflate returned %d (%s)&bslash;n&quot;
comma
id|state-&gt;unit
comma
id|r
comma
(paren
id|state-&gt;strm.msg
ques
c_cond
id|state-&gt;strm.msg
suffix:colon
l_string|&quot;&quot;
)paren
)paren
suffix:semicolon
r_return
id|DECOMP_FATALERROR
suffix:semicolon
)brace
r_if
c_cond
(paren
id|state-&gt;strm.avail_out
op_ne
l_int|0
)paren
r_break
suffix:semicolon
multiline_comment|/* all done */
r_if
c_cond
(paren
id|decode_proto
)paren
(brace
id|state-&gt;strm.avail_out
op_assign
id|osize
op_minus
id|PPP_HDRLEN
suffix:semicolon
r_if
c_cond
(paren
(paren
id|obuf
(braket
l_int|3
)braket
op_amp
l_int|1
)paren
op_eq
l_int|0
)paren
(brace
multiline_comment|/* 2-byte protocol field */
id|obuf
(braket
l_int|2
)braket
op_assign
id|obuf
(braket
l_int|3
)braket
suffix:semicolon
op_decrement
id|state-&gt;strm.next_out
suffix:semicolon
op_increment
id|state-&gt;strm.avail_out
suffix:semicolon
)brace
id|decode_proto
op_assign
l_int|0
suffix:semicolon
)brace
r_else
r_if
c_cond
(paren
op_logical_neg
id|overflow
)paren
(brace
multiline_comment|/*&n;&t;&t;&t; * We&squot;ve filled up the output buffer; the only way to&n;&t;&t;&t; * find out whether inflate has any more characters&n;&t;&t;&t; * left is to give it another byte of output space.&n;&t;&t;&t; */
id|state-&gt;strm.next_out
op_assign
id|overflow_buf
suffix:semicolon
id|state-&gt;strm.avail_out
op_assign
l_int|1
suffix:semicolon
id|overflow
op_assign
l_int|1
suffix:semicolon
)brace
r_else
(brace
r_if
c_cond
(paren
id|state-&gt;debug
)paren
id|printk
c_func
(paren
id|KERN_DEBUG
l_string|&quot;z_decompress%d: ran out of mru&bslash;n&quot;
comma
id|state-&gt;unit
)paren
suffix:semicolon
r_return
id|DECOMP_FATALERROR
suffix:semicolon
)brace
)brace
r_if
c_cond
(paren
id|decode_proto
)paren
(brace
r_if
c_cond
(paren
id|state-&gt;debug
)paren
id|printk
c_func
(paren
id|KERN_DEBUG
l_string|&quot;z_decompress%d: didn&squot;t get proto&bslash;n&quot;
comma
id|state-&gt;unit
)paren
suffix:semicolon
r_return
id|DECOMP_ERROR
suffix:semicolon
)brace
id|olen
op_assign
id|osize
op_plus
id|overflow
op_minus
id|state-&gt;strm.avail_out
suffix:semicolon
id|state-&gt;stats.unc_bytes
op_add_assign
id|olen
suffix:semicolon
id|state-&gt;stats.unc_packets
op_increment
suffix:semicolon
id|state-&gt;stats.comp_bytes
op_add_assign
id|isize
suffix:semicolon
id|state-&gt;stats.comp_packets
op_increment
suffix:semicolon
r_return
id|olen
suffix:semicolon
)brace
multiline_comment|/**&n; *&t;z_incomp - add incompressible input data to the history.&n; *&t;@arg:&t;pointer to private state for the decompressor&n; *&t;@ibuf:&t;pointer to input packet data&n; *&t;@icnt:&t;length of input data.&n; */
DECL|function|z_incomp
r_static
r_void
id|z_incomp
c_func
(paren
r_void
op_star
id|arg
comma
r_int
r_char
op_star
id|ibuf
comma
r_int
id|icnt
)paren
(brace
r_struct
id|ppp_deflate_state
op_star
id|state
op_assign
(paren
r_struct
id|ppp_deflate_state
op_star
)paren
id|arg
suffix:semicolon
r_int
id|proto
comma
id|r
suffix:semicolon
multiline_comment|/*&n;&t; * Check that the protocol is one we handle.&n;&t; */
id|proto
op_assign
id|PPP_PROTOCOL
c_func
(paren
id|ibuf
)paren
suffix:semicolon
r_if
c_cond
(paren
id|proto
OG
l_int|0x3fff
op_logical_or
id|proto
op_eq
l_int|0xfd
op_logical_or
id|proto
op_eq
l_int|0xfb
)paren
r_return
suffix:semicolon
op_increment
id|state-&gt;seqno
suffix:semicolon
multiline_comment|/*&n;&t; * We start at the either the 1st or 2nd byte of the protocol field,&n;&t; * depending on whether the protocol value is compressible.&n;&t; */
id|state-&gt;strm.next_in
op_assign
id|ibuf
op_plus
l_int|3
suffix:semicolon
id|state-&gt;strm.avail_in
op_assign
id|icnt
op_minus
l_int|3
suffix:semicolon
r_if
c_cond
(paren
id|proto
OG
l_int|0xff
)paren
(brace
op_decrement
id|state-&gt;strm.next_in
suffix:semicolon
op_increment
id|state-&gt;strm.avail_in
suffix:semicolon
)brace
id|r
op_assign
id|zlib_inflateIncomp
c_func
(paren
op_amp
id|state-&gt;strm
)paren
suffix:semicolon
r_if
c_cond
(paren
id|r
op_ne
id|Z_OK
)paren
(brace
multiline_comment|/* gak! */
r_if
c_cond
(paren
id|state-&gt;debug
)paren
(brace
id|printk
c_func
(paren
id|KERN_DEBUG
l_string|&quot;z_incomp%d: inflateIncomp returned %d (%s)&bslash;n&quot;
comma
id|state-&gt;unit
comma
id|r
comma
(paren
id|state-&gt;strm.msg
ques
c_cond
id|state-&gt;strm.msg
suffix:colon
l_string|&quot;&quot;
)paren
)paren
suffix:semicolon
)brace
r_return
suffix:semicolon
)brace
multiline_comment|/*&n;&t; * Update stats.&n;&t; */
id|state-&gt;stats.inc_bytes
op_add_assign
id|icnt
suffix:semicolon
id|state-&gt;stats.inc_packets
op_increment
suffix:semicolon
id|state-&gt;stats.unc_bytes
op_add_assign
id|icnt
suffix:semicolon
id|state-&gt;stats.unc_packets
op_increment
suffix:semicolon
)brace
multiline_comment|/*************************************************************&n; * Module interface table&n; *************************************************************/
multiline_comment|/* These are in ppp_generic.c */
r_extern
r_int
id|ppp_register_compressor
(paren
r_struct
id|compressor
op_star
id|cp
)paren
suffix:semicolon
r_extern
r_void
id|ppp_unregister_compressor
(paren
r_struct
id|compressor
op_star
id|cp
)paren
suffix:semicolon
multiline_comment|/*&n; * Procedures exported to if_ppp.c.&n; */
DECL|variable|ppp_deflate
r_struct
id|compressor
id|ppp_deflate
op_assign
(brace
dot
id|compress_proto
op_assign
id|CI_DEFLATE
comma
dot
id|comp_alloc
op_assign
id|z_comp_alloc
comma
dot
id|comp_free
op_assign
id|z_comp_free
comma
dot
id|comp_init
op_assign
id|z_comp_init
comma
dot
id|comp_reset
op_assign
id|z_comp_reset
comma
dot
id|compress
op_assign
id|z_compress
comma
dot
id|comp_stat
op_assign
id|z_comp_stats
comma
dot
id|decomp_alloc
op_assign
id|z_decomp_alloc
comma
dot
id|decomp_free
op_assign
id|z_decomp_free
comma
dot
id|decomp_init
op_assign
id|z_decomp_init
comma
dot
id|decomp_reset
op_assign
id|z_decomp_reset
comma
dot
id|decompress
op_assign
id|z_decompress
comma
dot
id|incomp
op_assign
id|z_incomp
comma
dot
id|decomp_stat
op_assign
id|z_comp_stats
comma
dot
id|owner
op_assign
id|THIS_MODULE
)brace
suffix:semicolon
DECL|variable|ppp_deflate_draft
r_struct
id|compressor
id|ppp_deflate_draft
op_assign
(brace
dot
id|compress_proto
op_assign
id|CI_DEFLATE_DRAFT
comma
dot
id|comp_alloc
op_assign
id|z_comp_alloc
comma
dot
id|comp_free
op_assign
id|z_comp_free
comma
dot
id|comp_init
op_assign
id|z_comp_init
comma
dot
id|comp_reset
op_assign
id|z_comp_reset
comma
dot
id|compress
op_assign
id|z_compress
comma
dot
id|comp_stat
op_assign
id|z_comp_stats
comma
dot
id|decomp_alloc
op_assign
id|z_decomp_alloc
comma
dot
id|decomp_free
op_assign
id|z_decomp_free
comma
dot
id|decomp_init
op_assign
id|z_decomp_init
comma
dot
id|decomp_reset
op_assign
id|z_decomp_reset
comma
dot
id|decompress
op_assign
id|z_decompress
comma
dot
id|incomp
op_assign
id|z_incomp
comma
dot
id|decomp_stat
op_assign
id|z_comp_stats
comma
dot
id|owner
op_assign
id|THIS_MODULE
)brace
suffix:semicolon
DECL|function|deflate_init
r_int
id|__init
id|deflate_init
c_func
(paren
r_void
)paren
(brace
r_int
id|answer
op_assign
id|ppp_register_compressor
c_func
(paren
op_amp
id|ppp_deflate
)paren
suffix:semicolon
r_if
c_cond
(paren
id|answer
op_eq
l_int|0
)paren
id|printk
c_func
(paren
id|KERN_INFO
l_string|&quot;PPP Deflate Compression module registered&bslash;n&quot;
)paren
suffix:semicolon
id|ppp_register_compressor
c_func
(paren
op_amp
id|ppp_deflate_draft
)paren
suffix:semicolon
r_return
id|answer
suffix:semicolon
)brace
DECL|function|deflate_cleanup
r_void
id|__exit
id|deflate_cleanup
c_func
(paren
r_void
)paren
(brace
id|ppp_unregister_compressor
c_func
(paren
op_amp
id|ppp_deflate
)paren
suffix:semicolon
id|ppp_unregister_compressor
c_func
(paren
op_amp
id|ppp_deflate_draft
)paren
suffix:semicolon
)brace
DECL|variable|deflate_init
id|module_init
c_func
(paren
id|deflate_init
)paren
suffix:semicolon
DECL|variable|deflate_cleanup
id|module_exit
c_func
(paren
id|deflate_cleanup
)paren
suffix:semicolon
id|MODULE_LICENSE
c_func
(paren
l_string|&quot;Dual BSD/GPL&quot;
)paren
suffix:semicolon
eof
