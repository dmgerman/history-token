...
id|dead
suffix:semicolon
multiline_comment|/* unit/channel has been shut down */
)brace
suffix:semicolon
DECL|macro|PF_TO_X
mdefine_line|#define PF_TO_X(pf, X)&t;&t;((X *)((char *)(pf) - offsetof(X, file)))
...
id|file
suffix:semicolon
multiline_comment|/* stuff for read/write/poll 0 */
DECL|member|owner
r_struct
id|file
op_star
id|owner
suffix:semicolon
multiline_comment|/* file that owns this unit 48 */
DECL|member|channels
r_struct
id|list_head
id|channels
suffix:semicolon
multiline_comment|/* list of attached channels 4c */
DECL|member|n_channels
r_int
id|n_channels
suffix:semicolon
multiline_comment|/* how many channels are attached 54 */
DECL|member|rlock
id|spinlock_t
id|rlock
suffix:semicolon
multiline_comment|/* lock for receive side 58 */
DECL|member|wlock
id|spinlock_t
id|wlock
suffix:semicolon
multiline_comment|/* lock for transmit side 5c */
DECL|member|mru
r_int
id|mru
suffix:semicolon
multiline_comment|/* max receive unit 60 */
DECL|member|flags
r_int
r_int
id|flags
suffix:semicolon
multiline_comment|/* control bits 64 */
DECL|member|xstate
r_int
r_int
id|xstate
suffix:semicolon
multiline_comment|/* transmit state bits 68 */
DECL|member|rstate
r_int
r_int
id|rstate
suffix:semicolon
multiline_comment|/* receive state bits 6c */
DECL|member|debug
r_int
id|debug
suffix:semicolon
multiline_comment|/* debug flags 70 */
DECL|member|vj
r_struct
id|slcompress
op_star
...
suffix:semicolon
multiline_comment|/* what to do with each net proto 78 */
...
id|xmit_pending
suffix:semicolon
multiline_comment|/* a packet ready to go out 88 */
DECL|member|xcomp
r_struct
id|compressor
op_star
id|xcomp
suffix:semicolon
multiline_comment|/* transmit packet compressor 8c */
DECL|member|xc_state
r_void
op_star
id|xc_state
suffix:semicolon
multiline_comment|/* its internal state 90 */
DECL|member|rcomp
r_struct
id|compressor
op_star
id|rcomp
suffix:semicolon
multiline_comment|/* receive decompressor 94 */
DECL|member|rc_state
r_void
op_star
id|rc_state
suffix:semicolon
multiline_comment|/* its internal state 98 */
DECL|member|last_xmit
r_int
r_int
id|last_xmit
suffix:semicolon
multiline_comment|/* jiffies when last pkt sent 9c */
DECL|member|last_recv
r_int
r_int
id|last_recv
suffix:semicolon
multiline_comment|/* jiffies when last pkt rcvd a0 */
DECL|member|dev
r_struct
id|net_device
op_star
id|dev
suffix:semicolon
multiline_comment|/* network interface device a4 */
...
id|list
suffix:semicolon
multiline_comment|/* link in all/new_channels list */
DECL|member|chan
r_struct
id|ppp_channel
op_star
...
DECL|member|chan_sem
r_struct
id|rw_semaphore
id|chan_sem
suffix:semicolon
multiline_comment|/* protects `chan&squot; during chan ioctl */
DECL|member|downl
id|spinlock_t
id|downl
suffix:semicolon
...
id|upl
suffix:semicolon
multiline_comment|/* protects `ppp&squot; */
macro_line|#ifdef CONFIG_PPP_MULTILINK
DECL|member|avail
id|u8
id|avail
...
multiline_comment|/*&n; * A cardmap represents a mapping from unsigned integers to pointers,&n; * and provides a fast &quot;find lowest unused number&quot; operation.&n; * It uses a broad (32-way) tree with a bitmap at each level.&n; * It is designed to be space-efficient for small numbers of entries&n; * and time-efficient for large numbers of entries.&n; */
DECL|macro|CARDMAP_ORDER
mdefine_line|#define CARDMAP_ORDER&t;5
DECL|macro|CARDMAP_WIDTH
mdefine_line|#define CARDMAP_WIDTH&t;(1U &lt;&lt; CARDMAP_ORDER)
DECL|macro|CARDMAP_MASK
mdefine_line|#define CARDMAP_MASK&t;(CARDMAP_WIDTH - 1)
DECL|struct|cardmap
r_struct
id|cardmap
(brace
...
suffix:semicolon
multiline_comment|/*&n; * all_ppp_sem protects the all_ppp_units mapping.&n; * It also ensures that finding a ppp unit in the all_ppp_units map&n; * and updating its file.refcnt field is atomic.&n; */
r_static
id|DECLARE_MUTEX
c_func
(paren
...
(brace
multiline_comment|/*&n;&t;&t;&t; * Return 0 (EOF) on an interface that has no&n;&t;&t;&t; * channels connected, unless it is looping&n;&t;&t;&t; * network traffic (demand mode).&n;&t;&t;&t; */
r_struct
id|ppp
op_star
id|ppp
...
(brace
multiline_comment|/* see comment in ppp_read */
r_struct
id|ppp
op_star
id|ppp
...
(brace
multiline_comment|/*&n;&t;&t; * We have to be careful here... if the file descriptor&n;&t;&t; * has been dup&squot;d, we could have another process in the&n;&t;&t; * middle of a poll using the same file *, so we had&n;&t;&t; * better not free the interface data structures -&n;&t;&t; * instead we fail the ioctl.  Even in this case, we&n;&t;&t; * shut down the interface if we are the owner of it.&n;&t;&t; * Actually, we should get rid of PPPIOCDETACH, userland&n;&t;&t; * (i.e. pppd) could achieve the same effect by closing&n;&t;&t; * this fd and reopening /dev/ppp.&n;&t;&t; */
id|err
op_assign
op_minus
id|EINVAL
...
(paren
id|KERN_DEBUG
l_string|&quot;PPPIOCDETACH file-&gt;f_count=%d&bslash;n&quot;
comma
id|atomic_read
c_func
(paren
...
(paren
id|KERN_DEBUG
l_string|&quot;PPP: outbound frame not passed&bslash;n&quot;
)paren
suffix:semicolon
id|kfree_skb
c_func
...
id|err
suffix:semicolon
multiline_comment|/* Until we fix the decompressor need to make sure&n;&t;&t; * data portion is linear.&n;&t;&t; */
r_if
c_cond
(paren
op_logical_neg
...
(paren
id|KERN_DEBUG
l_string|&quot;PPP: inbound frame not passed&bslash;n&quot;
)paren
suffix:semicolon
id|kfree_skb
c_func
...
id|len
suffix:semicolon
multiline_comment|/* Until we fix all the decompressor&squot;s need to make sure&n;&t; * data portion is linear.&n;&t; */
r_if
c_cond
(paren
op_logical_neg
...
multiline_comment|/*&n; * Disconnect a channel from the generic layer.&n; * This must be called in process context.&n; */
r_void
...
l_int|2
suffix:semicolon
multiline_comment|/* don&squot;t count proto bytes */
r_for
c_loop
(paren
id|i
...
multiline_comment|/* Initialize the new ppp unit */
id|ppp-&gt;file.index
op_assign
id|unit
suffix:semicolon
...
(paren
id|KERN_ERR
l_string|&quot;PPP: couldn&squot;t register device %s (%d)&bslash;n&quot;
comma
id|dev-&gt;name
comma
id|ret
...
multiline_comment|/*&n; * Take down a ppp interface unit - called when the owning file&n; * (the one that created the unit) is closed or detached.&n; */
DECL|function|ppp_shutdown_interface
r_static
r_void
id|ppp_shutdown_interface
c_func
...
multiline_comment|/*&n; * Free the memory used by a ppp unit.  This is only called once&n; * there are no channels connected to the unit and no file structs&n; * that reference the unit.&n; */
...
(paren
id|KERN_ERR
l_string|&quot;ppp: destroying ppp struct %p but dead=%d &quot;
l_string|&quot;n_channels=%d !&bslash;n&quot;
comma
id|ppp
comma
id|ppp-&gt;file.dead
...
multiline_comment|/*&n; * Locate an existing ppp unit.&n; * The caller should have locked the all_ppp_sem.&n; */
r_static
r_struct
id|ppp
op_star
...
multiline_comment|/*&n; * Locate an existing ppp channel.&n; * The caller should have locked the all_channels_lock.&n; * First we look in the new_channels list, then in the&n; * all_channels list.  If found in the new_channels list,&n; * we move it to the all_channels list.  This is for speed&n; * when we have a lot of channels in use.&n; */
r_static
r_struct
id|channel
op_star
...
(paren
id|KERN_ERR
l_string|&quot;ppp: destroying undead channel %p !&bslash;n&quot;
comma
id|pch
)paren
suffix:semicolon
...
multiline_comment|/*&n; * Cardmap implementation.&n; */
DECL|function|cardmap_get
r_static
r_void
op_star
id|cardmap_get
...
r_do
(brace
multiline_comment|/* need a new top level */
r_struct
id|cardmap
op_star
id|np
...
DECL|function|cardmap_find_first_free
r_static
r_int
r_int
id|cardmap_find_first_free
...
multiline_comment|/* Module/initialization stuff */
