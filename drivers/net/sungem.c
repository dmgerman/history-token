multiline_comment|/* $Id: sungem.c,v 1.44.2.22 2002/03/13 01:18:12 davem Exp $&n; * sungem.c: Sun GEM ethernet driver.&n; *&n; * Copyright (C) 2000, 2001, 2002, 2003 David S. Miller (davem@redhat.com)&n; * &n; * Support for Apple GMAC and assorted PHYs, WOL, Power Management&n; * (C) 2001,2002,2003 Benjamin Herrenscmidt (benh@kernel.crashing.org)&n; * (C) 2004,2005 Benjamin Herrenscmidt, IBM Corp.&n; *&n; * NAPI and NETPOLL support&n; * (C) 2004 by Eric Lemoine (eric.lemoine@gmail.com)&n; * &n; * TODO: &n; *  - Now that the driver was significantly simplified, I need to rework&n; *    the locking. I&squot;m sure we don&squot;t need _2_ spinlocks, and we probably&n; *    can avoid taking most of them for so long period of time (and schedule&n; *    instead). The main issues at this point are caused by the netdev layer&n; *    though:&n; *    &n; *    gem_change_mtu() and gem_set_multicast() are called with a read_lock()&n; *    help by net/core/dev.c, thus they can&squot;t schedule. That means they can&squot;t&n; *    call netif_poll_disable() neither, thus force gem_poll() to keep a spinlock&n; *    where it could have been dropped. change_mtu especially would love also to&n; *    be able to msleep instead of horrid locked delays when resetting the HW,&n; *    but that read_lock() makes it impossible, unless I defer it&squot;s action to&n; *    the reset task, which means it&squot;ll be asynchronous (won&squot;t take effect until&n; *    the system schedules a bit).&n; *&n; *    Also, it would probably be possible to also remove most of the long-life&n; *    locking in open/resume code path (gem_reinit_chip) by beeing more careful&n; *    about when we can start taking interrupts or get xmit() called...&n; */
...
multiline_comment|/* Stripping FCS is causing problems, disabled for now */
DECL|macro|STRIP_FCS
macro_line|#undef STRIP_FCS
...
DECL|macro|ADVERTISE_MASK
mdefine_line|#define ADVERTISE_MASK&t;(SUPPORTED_10baseT_Half | SUPPORTED_10baseT_Full | &bslash;&n;&t;&t;&t; SUPPORTED_100baseT_Half | SUPPORTED_100baseT_Full | &bslash;&n;&t;&t;&t; SUPPORTED_1000baseT_Half | SUPPORTED_1000baseT_Full)
DECL|macro|DRV_NAME
mdefine_line|#define DRV_NAME&t;&quot;sungem&quot;
DECL|macro|DRV_VERSION
mdefine_line|#define DRV_VERSION&t;&quot;0.98&quot;
DECL|macro|DRV_RELDATE
mdefine_line|#define DRV_RELDATE&t;&quot;8/24/03&quot;
...
id|PCI_VENDOR_ID_APPLE
comma
id|PCI_DEVICE_ID_APPLE_UNI_N_GMAC2
comma
id|PCI_ANY_ID
comma
id|PCI_ANY_ID
...
id|PCI_VENDOR_ID_APPLE
comma
id|PCI_DEVICE_ID_APPLE_K2_GMAC
comma
id|PCI_ANY_ID
comma
id|PCI_ANY_ID
...
id|PCI_VENDOR_ID_APPLE
comma
id|PCI_DEVICE_ID_APPLE_SH_SUNGEM
comma
id|PCI_ANY_ID
comma
id|PCI_ANY_ID
...
(brace
multiline_comment|/* Enable all interrupts but TXDONE */
id|writel
c_func
(paren
id|GREG_STAT_TXDONE
...
DECL|function|gem_disable_ints
r_static
r_inline
r_void
id|gem_disable_ints
...
(brace
multiline_comment|/* Disable all interrupts, including TXDONE */
id|writel
c_func
(paren
id|GREG_STAT_NAPI
...
id|val
suffix:semicolon
multiline_comment|/* First, reset &amp; disable MAC RX. */
id|writel
c_func
(paren
id|MAC_RXRST_CMD
...
(paren
id|KERN_ERR
l_string|&quot;%s: RX MAC will not reset, resetting whole &quot;
...
(paren
id|KERN_ERR
l_string|&quot;%s: RX MAC fifo overflow smac[%08x].&bslash;n&quot;
comma
id|dev-&gt;name
comma
id|smac
...
r_break
suffix:semicolon
multiline_comment|/* When writing back RX descriptor, GEM writes status&n;&t;&t; * then buffer address, possibly in seperate transactions.&n;&t;&t; * If we don&squot;t wait for the chip to write both, we could&n;&t;&t; * post a new buffer to this descriptor then have GEM spam&n;&t;&t; * on the buffer address.  We sync on the RX completion&n;&t;&t; * register to prevent this from happening.&n;&t;&t; */
r_if
c_cond
(paren
id|entry
...
multiline_comment|/* We can now account for the work we&squot;re about to do */
id|work_done
op_increment
suffix:semicolon
id|skb
...
id|flags
suffix:semicolon
multiline_comment|/*&n;&t; * NAPI locking nightmare: See comment at head of driver &n;&t; */
...
id|work_done
suffix:semicolon
multiline_comment|/* Handle anomalies */
r_if
c_cond
(paren
id|gp-&gt;status
...
suffix:semicolon
multiline_comment|/* Run RX thread. We don&squot;t use any locking here, &n;&t;&t; * code willing to do bad things - like cleaning the &n;&t;&t; * rx ring - must call netif_poll_disable(), which&n;&t;&t; * schedule_timeout()&squot;s if polling is already disabled.&n;&t;&t; */
id|work_to_do
op_assign
id|min
c_func
...
id|flags
suffix:semicolon
multiline_comment|/* Swallow interrupts when shutting the chip down, though&n;&t; * that shouldn&squot;t happen, we should have done free_irq() at&n;&t; * this point...&n;&t; */
r_if
c_cond
(paren
op_logical_neg
...
suffix:semicolon
multiline_comment|/* If polling was disabled at the time we received that&n;&t; * interrupt, we may return IRQ_HANDLED here while we &n;&t; * should return IRQ_NONE. No big deal...&n;&t; */
r_return
id|IRQ_HANDLED
suffix:semicolon
)brace
...
(brace
multiline_comment|/* Tell upper layer to requeue */
...
multiline_comment|/* We raced with gem_do_stop() */
r_if
c_cond
(paren
op_logical_neg
...
(paren
id|KERN_ERR
l_string|&quot;%s: SW reset is ghetto.&bslash;n&quot;
comma
id|gp-&gt;dev-&gt;name
)paren
suffix:semicolon
...
multiline_comment|/* Must be invoked under gp-&gt;lock and gp-&gt;tx_lock. DMA won&squot;t be&n; * actually stopped before about 4ms tho ...&n; */
DECL|function|gem_stop_dma
r_static
r_void
id|gem_stop_dma
c_func
...
id|val
suffix:semicolon
multiline_comment|/* We are done rocking, turn everything off. */
id|val
op_assign
id|readl
c_func
...
suffix:semicolon
multiline_comment|/* Need to wait a bit ... done by the caller */
)brace
multiline_comment|/* Must be invoked under gp-&gt;lock and gp-&gt;tx_lock. */
...
id|non_mii
suffix:semicolon
multiline_comment|/* Setup advertise */
r_if
c_cond
(paren
id|found_mii_phy
...
id|start_aneg
suffix:colon
multiline_comment|/* Sanitize settings based on PHY capabilities */
r_if
c_cond
(paren
(paren
...
id|SPEED_10
suffix:semicolon
multiline_comment|/* If we are asleep, we don&squot;t try to actually setup the PHY, we&n;&t; * just store the settings&n;&t; */
r_if
c_cond
(paren
id|gp-&gt;asleep
...
id|link_aneg
suffix:colon
multiline_comment|/* We try forced modes after a failed aneg only on PHYs that don&squot;t&n;&t;&t; * have &quot;magic_aneg&quot; bit set, which means they internally do the&n;&t;&t; * while forced-mode thingy. On these, we just restart aneg&n;&t;&t; */
r_if
c_cond
(paren
id|gp-&gt;phy_mii.def-&gt;magic_aneg
)paren
r_return
l_int|1
suffix:semicolon
...
suffix:semicolon
multiline_comment|/* If the reset task is still pending, we just&n;&t; * reschedule the link timer&n;&t; */
r_if
c_cond
(paren
id|gp-&gt;reset_task_pending
...
(brace
multiline_comment|/* Ok, here we got a link. If we had it due to a forced&n;&t;&t; * fallback, and we were configured for autoneg, we do&n;&t;&t; * retry a short autoneg pass. If you know your hub is&n;&t;&t; * broken, use ethtool ;)&n;&t;&t; */
r_if
c_cond
(paren
id|gp-&gt;lstate
...
r_else
(brace
multiline_comment|/* If the link was previously up, we restart the&n;&t;&t; * whole process&n;&t;&t; */
r_if
c_cond
(paren
id|gp-&gt;lstate
...
multiline_comment|/* Init PHY interface and start link poll state machine */
...
id|i
suffix:semicolon
multiline_comment|/* Those delay sucks, the HW seem to love them though, I&squot;ll&n;&t;&t; * serisouly consider breaking some locks here to be able&n;&t;&t; * to schedule instead&n;&t;&t; */
r_for
c_loop
(paren
id|i
...
suffix:semicolon
macro_line|#endif
multiline_comment|/* Some PHYs used by apple have problem getting back to us,&n;&t;&t;&t; * we do an additional reset here&n;&t;&t;&t; */
id|phy_write
c_func
(paren
id|gp
...
(brace
singleline_comment|// XXX check for errors
id|mii_phy_probe
c_func
(paren
op_amp
...
suffix:semicolon
multiline_comment|/* Init PHY */
r_if
c_cond
(paren
id|gp-&gt;phy_mii.def
op_logical_and
id|gp-&gt;phy_mii.def-&gt;ops-&gt;init
)paren
id|gp-&gt;phy_mii.def-&gt;ops
op_member_access_from_pointer
id|init
...
suffix:semicolon
multiline_comment|/* Don&squot;t enable GEM&squot;s WOL in normal operations&n;&t; */
r_if
c_cond
(paren
id|gp-&gt;has_wol
...
multiline_comment|/* Configure the chip &quot;burst&quot; DMA mode &amp; enable some&n;&t; * HW bug fixes on Apple version&n;&t; */
id|cfg
op_assign
l_int|0
suffix:semicolon
...
id|cfg
op_or_assign
id|GREG_CFG_RONPAULBIT
op_or
id|GREG_CFG_ENBUG2FIX
suffix:semicolon
...
suffix:semicolon
multiline_comment|/* If Infinite Burst didn&squot;t stick, then use different&n;&t; * thresholds (and Apple bug fixes don&squot;t exist)&n;&t; */
r_if
c_cond
(paren
op_logical_neg
...
suffix:semicolon
multiline_comment|/* We hard-code the PHY address so we can properly bring it out of&n;&t;&t; * reset later on, we can&squot;t really probe it at this point, though&n;&t;&t; * that isn&squot;t an issue.&n;&t;&t; */
r_if
c_cond
(paren
id|gp-&gt;pdev-&gt;device
...
suffix:semicolon
multiline_comment|/* Make sure ints are disabled */
id|gem_disable_ints
c_func
(paren
id|gp
...
suffix:semicolon
multiline_comment|/* Configure pause thresholds */
id|gem_init_pause_thresholds
c_func
(paren
id|gp
...
suffix:semicolon
multiline_comment|/* Init DMA &amp; MAC engines */
id|gem_init_dma
c_func
(paren
id|gp
...
multiline_comment|/* Must be invoked with no lock held. */
...
id|csr
suffix:semicolon
multiline_comment|/* Setup wake-on-lan for MAGIC packet */
id|writel
c_func
(paren
id|MAC_RXCFG_HFE
...
suffix:semicolon
multiline_comment|/* No need to take the lock here */
r_if
c_cond
(paren
id|found_mii_phy
...
suffix:semicolon
multiline_comment|/* Enable the cell */
id|gem_get_cell
c_func
(paren
id|gp
...
l_int|0
suffix:semicolon
multiline_comment|/* Stop netif queue */
id|netif_stop_queue
c_func
(paren
id|dev
...
suffix:semicolon
multiline_comment|/* We can drop the lock now */
id|spin_unlock
c_func
(paren
op_amp
...
suffix:semicolon
multiline_comment|/* If we are going to sleep with WOL */
id|gem_stop_dma
c_func
(paren
id|gp
...
suffix:semicolon
multiline_comment|/* No irq needed anymore */
id|free_irq
c_func
(paren
id|gp-&gt;pdev-&gt;irq
...
suffix:semicolon
multiline_comment|/* Cell not needed neither if no WOL */
r_if
c_cond
(paren
op_logical_neg
...
suffix:semicolon
multiline_comment|/* We need the cell enabled */
r_if
c_cond
(paren
op_logical_neg
...
id|dev-&gt;priv
suffix:semicolon
multiline_comment|/* Note: we don&squot;t need to call netif_poll_disable() here because&n;&t; * our caller (dev_close) already did it for us&n;&t; */
id|down
c_func
(paren
op_amp
...
suffix:semicolon
multiline_comment|/* Keep the cell enabled during the entire operation */
id|spin_lock_irqsave
c_func
(paren
op_amp
...
suffix:semicolon
multiline_comment|/* If the driver is opened, we stop the MAC */
r_if
c_cond
(paren
id|gp-&gt;opened
...
suffix:semicolon
multiline_comment|/* Switch off MAC, remember WOL setting */
id|gp-&gt;asleep_wol
op_assign
id|gp-&gt;wake_on_lan
suffix:semicolon
...
l_int|0
suffix:semicolon
multiline_comment|/* Mark us asleep */
id|gp-&gt;asleep
op_assign
l_int|1
suffix:semicolon
...
suffix:semicolon
multiline_comment|/* Now we release the semaphore to not block the reset task who&n;&t; * can take it too. We are marked asleep, so there will be no&n;&t; * conflict here&n;&t; */
id|up
c_func
(paren
op_amp
...
suffix:semicolon
multiline_comment|/* Wait for a pending reset task to complete */
r_while
c_loop
(paren
id|gp-&gt;reset_task_pending
...
suffix:semicolon
multiline_comment|/* Shut the PHY down eventually and setup WOL */
id|gem_stop_phy
c_func
(paren
id|gp
...
suffix:semicolon
multiline_comment|/* Make sure bus master is disabled */
...
suffix:semicolon
multiline_comment|/* Release the cell, no need to take a lock at this point since&n;&t; * nothing else can happen now&n;&t; */
id|gem_put_cell
c_func
(paren
id|gp
...
suffix:semicolon
multiline_comment|/* Keep the cell enabled during the entire operation, no need to&n;&t; * take a lock here tho since nothing else can happen while we are&n;&t; * marked asleep&n;&t; */
id|gem_get_cell
c_func
(paren
id|gp
...
suffix:semicolon
multiline_comment|/* Make sure PCI access and bus master are enabled */
r_if
c_cond
(paren
...
(paren
id|KERN_ERR
l_string|&quot;%s: Can&squot;t re-enable chip !&bslash;n&quot;
comma
id|dev-&gt;name
)paren
suffix:semicolon
multiline_comment|/* Put cell and forget it for now, it will be considered as&n;&t;&t; * still asleep, a new sleep cycle may bring it back&n;&t;&t; */
id|gem_put_cell
c_func
(paren
id|gp
...
suffix:semicolon
multiline_comment|/* Mark us woken up */
id|gp-&gt;asleep
op_assign
l_int|0
suffix:semicolon
...
suffix:semicolon
multiline_comment|/* Bring the PHY back. Again, lock is useless at this point as&n;&t; * nothing can be happening until we restart the whole thing&n;&t; */
id|gem_init_phy
c_func
(paren
id|gp
...
suffix:semicolon
multiline_comment|/* If we were opened, bring everything back */
r_if
c_cond
(paren
id|gp-&gt;opened
...
suffix:semicolon
multiline_comment|/* Re-attach net device */
...
suffix:semicolon
multiline_comment|/* If we had WOL enabled, the cell clock was never turned off during&n;&t; * sleep, so we end up beeing unbalanced. Fix that here&n;&t; */
r_if
c_cond
(paren
id|gp-&gt;asleep_wol
...
suffix:semicolon
multiline_comment|/* This function doesn&squot;t need to hold the cell, it will be held if the&n;&t; * driver is open by gem_do_start().&n;&t; */
id|gem_put_cell
c_func
(paren
id|gp
...
suffix:semicolon
multiline_comment|/* I have seen this being called while the PM was in progress,&n;&t; * so we shield against this&n;&t; */
r_if
c_cond
(paren
id|gp-&gt;running
...
DECL|function|gem_get_settings
r_static
r_int
id|gem_get_settings
c_func
...
multiline_comment|/* Return current PHY settings */
id|spin_lock_irq
c_func
(paren
op_amp
...
id|gp-&gt;phy_mii.advertising
suffix:semicolon
multiline_comment|/* If we started with a forced mode, we don&squot;t have a default&n;&t;&t; * advertise set, we need to return something sensible so&n;&t;&t; * userland can re-enable autoneg properly.&n;&t;&t; */
r_if
c_cond
(paren
id|cmd-&gt;advertising
...
DECL|function|gem_set_settings
r_static
r_int
id|gem_set_settings
c_func
...
DECL|function|gem_get_msglevel
r_static
id|u32
id|gem_get_msglevel
c_func
...
DECL|function|gem_set_msglevel
r_static
r_void
id|gem_set_msglevel
c_func
...
multiline_comment|/* Add more when I understand how to program the chip */
multiline_comment|/* like WAKE_UCAST | WAKE_MCAST | WAKE_BCAST */
DECL|macro|WOL_SUPPORTED_MASK
mdefine_line|#define WOL_SUPPORTED_MASK&t;(WAKE_MAGIC)
DECL|function|gem_get_wol
r_static
r_void
id|gem_get_wol
c_func
...
id|flags
suffix:semicolon
multiline_comment|/* Hold the PM semaphore while doing ioctl&squot;s or we may collide&n;&t; * with power management.&n;&t; */
id|down
c_func
(paren
op_amp
...
suffix:semicolon
multiline_comment|/* We shouldn&squot;t need any locking here */
id|gem_get_cell
c_func
(paren
id|gp
...
suffix:semicolon
multiline_comment|/* Shut the PHY down */
id|gem_stop_phy
c_func
(paren
id|gp
...
multiline_comment|/* On Apple, we want a reference to the Open Firmware device-tree&n;&t; * node. We use it for clock control.&n;&t; */
macro_line|#ifdef CONFIG_PPC_PMAC
id|gp-&gt;of_node
op_assign
...
suffix:semicolon
macro_line|#endif
multiline_comment|/* Only Apple version supports WOL afaik */
r_if
c_cond
(paren
id|pdev-&gt;vendor
...
l_int|1
suffix:semicolon
multiline_comment|/* Make sure cell is enabled */
id|gem_get_cell
c_func
(paren
id|gp
...
suffix:semicolon
multiline_comment|/* Make sure everything is stopped and in init state */
id|gem_reset
c_func
(paren
id|gp
...
suffix:semicolon
multiline_comment|/* Fill up the mii_phy structure (even if we won&squot;t use it) */
id|gp-&gt;phy_mii.dev
op_assign
id|dev
suffix:semicolon
id|gp-&gt;phy_mii.mdio_read
op_assign
id|_phy_read
suffix:semicolon
id|gp-&gt;phy_mii.mdio_write
op_assign
id|_phy_write
suffix:semicolon
multiline_comment|/* By default, we start with autoneg */
id|gp-&gt;want_autoneg
op_assign
l_int|1
suffix:semicolon
multiline_comment|/* Check fifo sizes, PHY type, etc... */
r_if
c_cond
(paren
id|gem_check_invariants
...
suffix:semicolon
macro_line|#endif
multiline_comment|/* Set that now, in case PM kicks in now */
id|pci_set_drvdata
c_func
(paren
id|pdev
...
suffix:semicolon
multiline_comment|/* Detect &amp; init PHY, start autoneg, we release the cell now&n;&t; * too, it will be managed by whoever needs it&n;&t; */
id|gem_init_phy
c_func
(paren
id|gp
...
suffix:semicolon
multiline_comment|/* Register with kernel */
r_if
c_cond
(paren
id|register_netdev
...
(paren
id|KERN_INFO
l_string|&quot;%s: Found %s PHY&bslash;n&quot;
comma
id|dev-&gt;name
comma
id|gp-&gt;phy_mii.def
