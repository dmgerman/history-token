...
id|fail
suffix:semicolon
multiline_comment|/* Don&squot;t copy the network device structure! */
multiline_comment|/* copy the priv structure of dev[0] */
id|privN
op_assign
(paren
id|DGRS_PRIV
...
id|plxdma
suffix:semicolon
multiline_comment|/*&n;&t; * Get and check the bus-master and latency values.&n;&t; * Some PCI BIOSes fail to set the master-enable bit,&n;&t; * and the latency timer must be set to the maximum&n;&t; * value to avoid data corruption that occurs when the&n;&t; * timer expires during a transfer.  Yes, it&squot;s a bug.&n;&t; */
id|err
op_assign
...
l_int|15
suffix:semicolon
multiline_comment|/*&n;&t; * On some BIOSES, the PLX &quot;expansion rom&quot; (used for DMA)&n;&t; * address comes up as &quot;0&quot;.  This is probably because&n;&t; * the BIOS doesn&squot;t see a valid 55 AA ROM signature at&n;&t; * the &quot;ROM&quot; start and zeroes the address.  To get&n;&t; * around this problem the SE-6 is configured to ask&n;&t; * for 4 MB of space for the dual port memory.  We then&n;&t; * must set its range back to 2 MB, and use the upper&n;&t; * half for DMA register access&n;&t; */
id|OUTL
c_func
(paren
id|io
...
DECL|function|dgrs_eisa_remove
r_static
r_int
id|__devexit
id|dgrs_eisa_remove
...
DECL|variable|dgrs_eisa_driver
r_static
r_struct
id|eisa_driver
id|dgrs_eisa_driver
