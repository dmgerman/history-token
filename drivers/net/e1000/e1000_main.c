...
suffix:semicolon
DECL|variable|e1000_xmit_frame
id|EXPORT_SYMBOL
c_func
(paren
id|e1000_xmit_frame
...
suffix:semicolon
DECL|variable|e1000_change_mtu
id|EXPORT_SYMBOL
c_func
(paren
id|e1000_change_mtu
...
suffix:semicolon
multiline_comment|/* disable the transmit and receive units */
id|E1000_WRITE_REG
c_func
(paren
id|shared
...
suffix:semicolon
multiline_comment|/* delay to allow PCI transactions to complete */
id|msec_delay
c_func
(paren
l_int|10
...
suffix:semicolon
multiline_comment|/* 82542 2.0 needs MWI disabled while issuing a reset */
r_if
c_cond
(paren
id|shared-&gt;mac_type
...
suffix:semicolon
multiline_comment|/* EEPROM reload */
id|ctrl_ext
op_assign
id|E1000_READ_REG
c_func
...
id|rctl
suffix:semicolon
multiline_comment|/* Setup the Receive Control Register */
id|rctl
op_assign
id|E1000_RCTL_EN
op_or
...
suffix:semicolon
id|adapter-&gt;rx_ring.unused_count
op_assign
id|adapter-&gt;rx_ring.count
suffix:semicolon
...
multiline_comment|/**&n; * e1000_xmit_frame - Transmit entry point&n; * @skb: buffer with frame data to transmit&n; * @netdev: network interface device structure&n; *&n; * Returns 0 on success, 1 on error&n; *&n; * e1000_xmit_frame is called by the stack to initiate a transmit.&n; * The out of resource condition is checked after each successful Tx&n; * so that the stack can be notified, preventing the driver from&n; * ever needing to drop a frame.  The atomic operations on&n; * tx_ring.unused are used to syncronize with the transmit&n; * interrupt processing code without the need for a spinlock.&n; **/
DECL|macro|TXD_USE_COUNT
mdefine_line|#define TXD_USE_COUNT(x) (((x) &gt;&gt; 12) + ((x) &amp; 0x0fff ? 1 : 0))
DECL|macro|SETUP_TXD_PAGE
mdefine_line|#define SETUP_TXD_PAGE(L, P, O) do { &bslash;&n;&t;tx_ring-&gt;buffer_info[i].length = (L); &bslash;&n;&t;tx_ring-&gt;buffer_info[i].dma = &bslash;&n;&t;&t;pci_map_page(pdev, (P), (O), (L), PCI_DMA_TODEVICE); &bslash;&n;&t;tx_desc-&gt;buffer_addr = cpu_to_le64(tx_ring-&gt;buffer_info[i].dma); &bslash;&n;&t;tx_desc-&gt;lower.data = cpu_to_le32(txd_lower | (L)); &bslash;&n;&t;tx_desc-&gt;upper.data = cpu_to_le32(txd_upper); &bslash;&n;} while (0)
DECL|macro|SETUP_TXD_PTR
mdefine_line|#define SETUP_TXD_PTR(L, P) &bslash;&n;&t;SETUP_TXD_PAGE((L), virt_to_page(P), (unsigned long)(P) &amp; ~PAGE_MASK)
DECL|macro|QUEUE_TXD
mdefine_line|#define QUEUE_TXD() do { i = (i + 1) % tx_ring-&gt;count; &bslash;&n;                         atomic_dec(&amp;tx_ring-&gt;unused); } while (0)
r_static
r_int
...
id|adapter-&gt;txd_cmd
suffix:semicolon
multiline_comment|/* If controller appears hung, force transmit timeout */
r_if
c_cond
(paren
id|time_after
...
op_logical_and
multiline_comment|/* If transmitting XOFFs, we&squot;re not really hung */
op_logical_neg
(paren
id|E1000_READ_REG
c_func
...
multiline_comment|/* make sure there are enough Tx descriptors available in the ring */
r_if
c_cond
(paren
id|atomic_read
...
multiline_comment|/* EOP and SKB pointer go with the last fragment */
...
id|i
suffix:semicolon
multiline_comment|/* Move the HW Tx Tail Pointer */
id|E1000_WRITE_REG
c_func
(paren
op_amp
...
DECL|macro|TXD_USE_COUNT
macro_line|#undef TXD_USE_COUNT
DECL|macro|SETUP_TXD
macro_line|#undef SETUP_TXD
DECL|macro|QUEUE_TXD
macro_line|#undef QUEUE_TXD
...
id|max_frame
op_le
id|MAXIMUM_ETHERNET_PACKET_SIZE
op_plus
id|CRC_LENGTH
)paren
(brace
