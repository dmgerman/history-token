...
multiline_comment|/* Change Log&n; *&n; *   o Feature: merged in modified NAPI patch from Robert Olsson&n; *     &lt;Robert.Olsson@its.uu.se&gt; Uppsala Univeristy, Sweden.&n; *&n; * 4.3.15      8/9/02&n; *   o Converted from Dual BSD/GPL license to GPL license.&n; *   o Clean up: use pci_[clear|set]_mwi rather than direct calls to&n; *     pci_write_config_word.&n; *   o Bug fix: added read-behind-write calls to post writes before delays.&n; *   o Bug fix: removed mdelay busy-waits in interrupt context.&n; *   o Clean up: direct clear of descriptor bits rather than using memset.&n; *   o Bug fix: added wmb() for ia-64 between descritor writes and advancing&n; *     descriptor tail.&n; *   o Feature: added locking mechanism for asf functionality.&n; *   o Feature: exposed two Tx and one Rx interrupt delay knobs for finer&n; *     control over interurpt rate tuning.&n; *   o Misc ethtool bug fixes.&n; *         &n; * 4.3.2       7/5/02&n; *   o Bug fix: perform controller reset using I/O rather than mmio because&n; *     some chipsets try to perform a 64-bit write, but the controller ignores&n; *     the upper 32-bit write once the reset is intiated by the lower 32-bit&n; *     write, causing a master abort.&n; *   o Bug fix: fixed jumbo frames sized from 1514 to 2048.&n; *   o ASF support: disable ARP when driver is loaded or resumed; enable when &n; *     driver is removed or suspended.&n; *   o Bug fix: changed default setting for RxIntDelay to 0 for 82542/3/4&n; *     controllers to workaround h/w errata where controller will hang when&n; *     RxIntDelay &lt;&gt; 0 under certian network conditions.&n; *   o Clean up: removed unused and undocumented user-settable settings for&n; *     PHY.&n; *   o Bug fix: ethtool GEEPROM was using byte address rather than word&n; *     addressing.&n; *   o Feature: added support for ethtool SEEPROM.&n; *   o Feature: added support for entropy pool.&n; *&n; * 4.2.17      5/30/02&n; */
...
suffix:semicolon
DECL|variable|e1000_notifier_netdev
r_struct
id|notifier_block
id|e1000_notifier_netdev
op_assign
(brace
dot
id|notifier_call
...
r_static
r_int
DECL|function|e1000_notify_netdev
id|e1000_notify_netdev
c_func
(paren
r_struct
...
id|netdev-&gt;priv
suffix:semicolon
multiline_comment|/* rename the proc nodes the easy way */
id|e1000_proc_dev_free
c_func
(paren
id|adapter
