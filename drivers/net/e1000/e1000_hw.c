...
id|IGP01E1000_PHY_PAGE_SELECT
comma
id|IGP01E1000_ANALOG_REGS_PAGE
)paren
suffix:semicolon
...
multiline_comment|/* Return to first page of registers */
id|e1000_write_phy_reg
c_func
(paren
id|hw
...
multiline_comment|/******************************************************************************&n; * Sets up link for a fiber based adapter&n; *&n; * hw - Struct containing variables accessed by shared code&n; *&n; * Manipulates Physical Coding Sublayer functions in order to configure&n; * link. Assumes the hardware has been previously reset and the transmitter&n; * and receiver are not enabled.&n; *****************************************************************************/
r_static
r_int32
...
multiline_comment|/* Force MDI for IGP PHY */
id|phy_data
op_and_assign
op_complement
(paren
...
multiline_comment|/* Read the data.  The address of the eeprom internally increments with&n;     * each word (microwire) or byte (spi) being read, saving on the overhead&n;     * of eeprom setup and tear-down.  The address counter will roll over if&n;     * reading beyond the size of the eeprom, thus allowing the entire memory&n;     * to be read starting from any offset. */
r_for
c_loop
(paren
id|i
...
multiline_comment|/* Return to page 0 */
r_if
c_cond
(paren
id|e1000_write_phy_reg
...
multiline_comment|/******************************************************************************&n; * Check the cable polarity&n; *&n; * hw - Struct containing variables accessed by shared code&n; * polarity - output parameter : 0 - Polarity is not reversed&n; *                               1 - Polarity is reversed.&n; *&n; * returns: E1000_SUCCESS / -E1000_ERR_XXX&n; *&n; * For phy&squot;s older then IGP, this function simply reads the polarity bit in the&n; * Phy Status register.  For IGP phy&squot;s, this bit is valid only if link speed is&n; * 10 Mbps.  If the link speed is 100 Mbps there is no polarity so this bit will&n; * return 0.  If the link speed is 1000 Mbps the polarity status is in the&n; * IGP01E1000_PHY_PCS_INIT_REG.&n; *****************************************************************************/
r_int32
