...
mdefine_line|#define DEVICE_NAME &quot;%s: Gianfar Ethernet Controller Version 1.0, &quot;
...
suffix:semicolon
multiline_comment|/* Called by the ocp code to initialize device data structures&n; * required for bringing up the device&n; * returns 0 on success */
...
multiline_comment|/* get a pointer to the register memory which can&n;&t; * configure the PHYs.  If it&squot;s different from this set,&n;&t; * get the device which has those regs */
r_if
c_cond
(paren
(paren
...
suffix:semicolon
multiline_comment|/* If the device which holds the MDIO regs isn&squot;t&n;&t;&t; * up, wait for it to come up */
r_if
c_cond
(paren
id|mdiodev
...
multiline_comment|/* and into the address registers MAC_STNADDR1,2. */
multiline_comment|/* Backwards, because little endian MACs are dumb. */
multiline_comment|/* Don&squot;t set the regs if the firmware already did */
id|memcpy
c_func
(paren
id|dev-&gt;dev_addr
...
suffix:semicolon
multiline_comment|/* If there is no RMON support in this device, we don&squot;t&n;&t; * want to expose non-existant statistics */
r_if
c_cond
(paren
(paren
...
(brace
id|dev_ethtool_ops-&gt;get_strings
op_assign
id|gfar_gstrings_normon
suffix:semicolon
id|dev_ethtool_ops-&gt;get_stats_count
op_assign
id|gfar_stats_count_normon
suffix:semicolon
id|dev_ethtool_ops-&gt;get_ethtool_stats
op_assign
id|gfar_fill_stats_normon
suffix:semicolon
)brace
...
(brace
id|dev_ethtool_ops-&gt;set_coalesce
op_assign
l_int|NULL
suffix:semicolon
id|dev_ethtool_ops-&gt;get_coalesce
op_assign
l_int|NULL
suffix:semicolon
)brace
...
suffix:semicolon
multiline_comment|/* Initially, coalescing is disabled */
...
id|curphy
suffix:semicolon
multiline_comment|/* Run the commands which configure the PHY */
id|phy_run_commands
c_func
(paren
id|dev
...
c_func
(paren
id|priv-&gt;einfo-&gt;interruptTransmit
comma
id|dev
)paren
suffix:semicolon
...
c_func
(paren
id|priv-&gt;einfo-&gt;interruptReceive
comma
id|dev
)paren
suffix:semicolon
...
suffix:semicolon
multiline_comment|/* Free the buffer descriptors */
id|kfree
c_func
(paren
id|priv-&gt;tx_bd_base
...
multiline_comment|/* Grab the PHY interrupt */
r_if
c_cond
(paren
id|priv-&gt;einfo-&gt;flags
...
multiline_comment|/* Set up the bottom half queue */
id|INIT_WORK
c_func
(paren
op_amp
...
suffix:semicolon
multiline_comment|/* Configure the PHY interrupt */
id|phy_run_commands
c_func
(paren
id|dev
comma
id|priv-&gt;phyinfo-&gt;startup
)paren
suffix:semicolon
multiline_comment|/* Tell the kernel the link is up, and determine the&n;&t; * negotiated features (speed, duplex) */
id|adjust_link
c_func
(paren
id|dev
...
(paren
id|KERN_INFO
l_string|&quot;%s: No link detected&bslash;n&quot;
comma
id|dev-&gt;name
)paren
suffix:semicolon
...
multiline_comment|/**********************************************************************&n; * gfar_accept_fastpath&n; *&n; * Used to authenticate to the kernel that a fast path entry can be&n; * added to device&squot;s routing table cache&n; *&n; * Input : pointer to ethernet interface network device structure and&n; *         a pointer to the designated entry to be added to the cache.&n; * Output : zero upon success, negative upon failure&n; **********************************************************************/
macro_line|#ifdef CONFIG_NET_FASTROUTE
DECL|function|gfar_accept_fastpath
r_static
r_int
id|gfar_accept_fastpath
c_func
...
macro_line|#endif
multiline_comment|/* try_fastroute() -- Checks the fastroute cache to see if a given packet&n; *   can be routed immediately to another device.  If it can, we send it.&n; *   If we used a fastroute, we return 1.  Otherwise, we return 0.&n; *   Returns 0 if CONFIG_NET_FASTROUTE is not on&n; */
DECL|function|try_fastroute
r_static
r_inline
r_int
id|try_fastroute
...
suffix:semicolon
multiline_comment|/* Only route ethernet IP packets */
r_if
c_cond
(paren
id|eth-&gt;h_proto
...
suffix:semicolon
multiline_comment|/* Generate the hash value */
id|hash
op_assign
(paren
(paren
...
op_increment
suffix:semicolon
multiline_comment|/* Make sure the packet is:&n;&t;&t;&t; * 1) IPv4&n;&t;&t;&t; * 2) without any options (header length of 5)&n;&t;&t;&t; * 3) Not a multicast packet&n;&t;&t;&t; * 4) going to a valid destination&n;&t;&t;&t; * 5) Not out of time-to-live&n;&t;&t;&t; */
r_if
c_cond
(paren
id|iph-&gt;version
...
(brace
multiline_comment|/* Fast Route Path: Taken if the outgoing device is ready to transmit the packet now */
r_if
c_cond
(paren
(paren
...
c_func
(paren
l_string|&quot;%s: FastRoute path corrupted&quot;
comma
id|dev-&gt;name
)paren
suffix:semicolon
...
multiline_comment|/* Semi Fast Route Path: Mark the packet as needing fast routing, but let the&n;&t;&t;&t;&t; * stack handle getting it to the device */
r_else
(brace
id|skb-&gt;pkt_type
op_assign
...
macro_line|#endif /* CONFIG_NET_FASTROUTE */
r_return
l_int|0
suffix:semicolon
)brace
...
multiline_comment|/* gfar_process_frame() -- handle one incoming packet if skb&n; * isn&squot;t NULL.  Try the fastroute before using the stack */
...
multiline_comment|/* gfar_clean_rx_ring() -- Processes each frame in the rx ring&n; *   until all are gone (or, in the case of NAPI, the budget/quota&n; *   has been reached). Returns the number of frames handled&n; */
macro_line|#ifdef CONFIG_GFAR_NAPI
...
suffix:semicolon
macro_line|#ifdef CONFIG_GFAR_NAPI
DECL|macro|GFAR_RXDONE
mdefine_line|#define GFAR_RXDONE() ((bdp-&gt;status &amp; RXBD_EMPTY) || (--rx_work_limit &lt; 0))
macro_line|#else
mdefine_line|#define GFAR_RXDONE() (bdp-&gt;status &amp; RXBD_EMPTY)
macro_line|#endif
r_while
c_loop
(paren
...
suffix:semicolon
multiline_comment|/* Run the commands which acknowledge the interrupt */
id|phy_run_commands
c_func
(paren
id|dev
comma
id|priv-&gt;phyinfo-&gt;ack_int
)paren
suffix:semicolon
multiline_comment|/* Schedule the bottom half */
id|schedule_work
c_func
(paren
op_amp
...
suffix:semicolon
multiline_comment|/* Run the commands which check the link state */
id|phy_run_commands
c_func
(paren
id|dev
comma
id|priv-&gt;phyinfo-&gt;handle_int
)paren
suffix:semicolon
multiline_comment|/* React to the change in state */
id|adjust_link
c_func
(paren
id|dev
