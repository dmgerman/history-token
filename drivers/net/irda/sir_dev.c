...
l_int|3
comma
l_string|&quot;%s(), raw-tx started&bslash;n&quot;
comma
id|__FUNCTION__
)paren
suffix:semicolon
...
id|len
suffix:semicolon
multiline_comment|/* all data is going to be sent */
)brace
id|spin_unlock_irqrestore
c_func
(paren
...
multiline_comment|/* remaining stuff gets flushed when re-enabling normal rx */
r_return
id|count
suffix:semicolon
)brace
DECL|function|sirdev_set_dtr_rts
r_int
...
(brace
multiline_comment|/* in raw mode we are just done now after the buffer was sent&n;&t;&t; * completely. Since this was requested by some dongle driver&n;&t;&t; * running under the control of the irda-thread we must take&n;&t;&t; * care here not to re-enable the queue. The queue will be&n;&t;&t; * restarted when the irda-thread has completed the request.&n;&t;&t; */
id|IRDA_DEBUG
c_func
(paren
l_int|3
comma
l_string|&quot;%s(), raw-tx done&bslash;n&quot;
comma
id|__FUNCTION__
)paren
suffix:semicolon
