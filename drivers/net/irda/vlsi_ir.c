...
l_string|&quot;vlsi_ir&quot;
suffix:semicolon
DECL|macro|PCI_CLASS_IRDA_GENERIC
mdefine_line|#define PCI_CLASS_IRDA_GENERIC 0x0d00
...
suffix:semicolon
multiline_comment|/*&t;ringsize: size of the tx and rx descriptor rings&n; *&t;&t;independent for tx and rx&n; *&t;&t;specify as ringsize=tx[,rx]&n; *&t;&t;allowed values: 4, 8, 16, 32, 64&n; */
...
suffix:semicolon
multiline_comment|/* default is tx=rx=16 */
id|MODULE_PARM
c_func
(paren
id|sirpulse
...
suffix:semicolon
multiline_comment|/*&t;sirpulse: tuning of the sir pulse width within IrPHY 1.3 limits&n; *&t;&t;0: real short, 1.5us (exception 6us at 2.4kb/s)&n; *&t;&t;1: nominal 3/16 bittime width&n; */
...
suffix:semicolon
multiline_comment|/*&t;mtt_bit: encoded min-turn-time values we accept for connections&n; *&t;&t; according to IrLAP definition (section 6.6.8)&n; *&t;&t; the widespreadly used HP HDLS-1100 requires 1 msec&n; */
DECL|variable|mtt_bits
r_static
r_int
id|mtt_bits
op_assign
...
(brace
multiline_comment|/* overlayed - order is important! */
id|rd-&gt;rd_addr
op_assign
id|a
suffix:semicolon
...
multiline_comment|/* advancing indices pointing into descriptor rings */
DECL|function|ring_ptr_inc
r_static
r_inline
r_void
id|ring_ptr_inc
...
multiline_comment|/********************************************************/
DECL|macro|MAX_PACKET_LEN
mdefine_line|#define MAX_PACKET_LEN&t;&t;2048&t;/* IrDA MTU */
multiline_comment|/* increase transfer buffer size somewhat so we have enough space left&n; * when packet size increases during wrapping due to XBOFs and escapes.&n; * well, this wastes some memory - anyway, later we will&n; * either map skb&squot;s directly or use pci_pool allocator...&n; */
DECL|macro|XFER_BUF_SIZE
mdefine_line|#define XFER_BUF_SIZE&t;&t;(MAX_PACKET_LEN+512)
...
suffix:semicolon
multiline_comment|/* protocol to detect PLL lock synchronisation */
id|udelay
c_func
(paren
l_int|500
...
r_else
(brace
multiline_comment|/* got succesful PLL lock */
id|clksrc
op_assign
l_int|1
suffix:semicolon
...
multiline_comment|/* we get here if either no PLL detected in auto-mode or&n;&t;   the external clock source explicitly specified */
id|clkctl
op_assign
id|CLKCTL_EXTCLK
op_or
...
DECL|function|vlsi_alloc_buffers_init
r_static
r_int
id|vlsi_alloc_buffers_init
c_func
(paren
id|vlsi_irda_dev_t
op_star
...
(paren
id|KERN_CRIT
l_string|&quot;%s: rx still active!!!&bslash;n&quot;
comma
id|__FUNCTION__
)paren
suffix:semicolon
...
(brace
multiline_comment|/* only rewrite ENRX, if tx not running!&n;&t;&t; * rewriting ENRX during tx in progress wouldn&squot;t hurt&n;&t;&t; * but would be racy since we would also have to rewrite&n;&t;&t; * ENTX then (same register) - which might get disabled meanwhile.&n;&t;&t; */
id|outw
c_func
(paren
l_int|0
...
(brace
multiline_comment|/* tx ring empty */
multiline_comment|/* sth more to do here? */
r_break
suffix:semicolon
)brace
id|ring_ptr_inc
...
(paren
id|KERN_CRIT
l_string|&quot;%s: tx still active!!!&bslash;n&quot;
comma
id|__FUNCTION__
)paren
suffix:semicolon
...
(brace
multiline_comment|/* tx ring not empty */
id|outw
c_func
(paren
id|config
...
DECL|function|vlsi_act_interrupt
r_static
r_int
...
suffix:semicolon
multiline_comment|/* still needed? - we have SET_MODULE_OWNER! */
r_if
c_cond
(paren
...
(paren
id|KERN_ERR
l_string|&quot;%s: sending 0-size packet???&bslash;n&quot;
comma
id|__FUNCTION__
)paren
suffix:semicolon
...
suffix:semicolon
macro_line|#endif
multiline_comment|/*&n; *&t;race window due to concurrent controller processing!&n; *&n; *&t;we may loose ENTX at any time when the controller&n; *&t;fetches an inactive descr or one with CLR_ENTX set.&n; *&t;therefore we only rely on the controller telling us&n; *&t;tx is already stopped because (cannot restart without PROMPT).&n; *&t;instead we depend on the tx-complete-isr to detect the&n; *&t;false negatives and retrigger the tx ring.&n; *&t;that&squot;s why we need interrupts disabled till tx has been&n; *&t;kicked, so the tx-complete-isr was either already finished&n; *&t;before we&squot;ve put the new active descriptor on the ring - or&n; *&t;the isr will be called after the new active descr is on the&n; *&t;ring _and_ the ring was prompted. Making these two steps&n; *&t;atomic allows to resolve the race.&n; */
id|iobase
op_assign
id|ndev-&gt;base_addr
suffix:semicolon
...
singleline_comment|//&t;vlsi_reg_debug(iobase, __FUNCTION__);
r_return
l_int|0
suffix:semicolon
)brace
...
suffix:semicolon
multiline_comment|/* currently no media definitions for SIR/MIR/FIR */
id|ndev-&gt;flags
op_or_assign
id|IFF_PORTSEL
op_or
...
(paren
id|KERN_INFO
l_string|&quot;%s: found IrDA PCI controler %s&bslash;n&quot;
comma
id|drivername
comma
id|pdev-&gt;name
...
(paren
id|KERN_INFO
l_string|&quot;%s: invalid tx ringsize %d - using default=16&bslash;n&quot;
comma
id|__FUNCTION__
comma
id|ringsize
...
(paren
id|KERN_INFO
l_string|&quot;%s: invalid rx ringsize %d - using default=16&bslash;n&quot;
comma
id|__FUNCTION__
comma
id|ringsize
