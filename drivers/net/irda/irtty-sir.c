...
id|TIOCM_DTR
suffix:semicolon
multiline_comment|/*&n;&t; * The ioctl() function, or actually set_modem_info() in serial.c&n;&t; * expects a pointer to the argument in user space. This is working&n;&t; * here because we are always called from the kIrDAd thread which&n;&t; * has set_fs(KERNEL_DS) permanently set. Therefore copy_from_user()&n;&t; * is happy with our arg-parameter being local here in kernel space.&n;&t; */
id|lock_kernel
c_func
(paren
)paren
...
multiline_comment|/* ------------------------------------------------------- */
multiline_comment|/* called from sir_dev when there is more data to send&n; * context is either netdev-&gt;hard_xmit or some transmit-completion bh&n; * i.e. we are under spinlock here and must not sleep.&n; *&n; * Note: as of 2.5.44 the usb-serial driver calls down() on a semaphore&n; * hence we are hitting the might_sleep bugcatcher. IMHO the whole tty-api&n; * would be pretty pointless if write_room/write would be allowed to sleep.&n; * Furthermore other tty ldiscs (like ppp) do also require the driver not&n; * to sleep there. Hence this is considered a current limitation of&n; * usb-serial.&n; */
...
r_return
suffix:semicolon
multiline_comment|/* Please use ASSERT - Fix ASSERT as needed - Jean II */
r_if
c_cond
(paren
id|unlikely
