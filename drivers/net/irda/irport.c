...
l_int|16
suffix:semicolon
multiline_comment|/* 16550A and compatible */
...
(paren
l_string|&quot;%s(), can&squot;t allocate memory for &quot;
l_string|&quot;receive buffer!&bslash;n&quot;
comma
id|__FUNCTION__
)paren
suffix:semicolon
...
op_assign
id|self-&gt;rx_buff.skb-&gt;data
suffix:semicolon
multiline_comment|/* No need to memset the buffer, unless you are really pedantic */
multiline_comment|/* Finish setup the Rx buffer descriptor */
...
(paren
l_string|&quot;%s(), can&squot;t allocate memory for &quot;
l_string|&quot;transmit buffer!&bslash;n&quot;
comma
id|__FUNCTION__
)paren
suffix:semicolon
...
suffix:semicolon
multiline_comment|/* We can&squot;t lock, we may be called from a FIR driver - Jean II */
multiline_comment|/* We are not transmitting any more */
id|self-&gt;transmitting
op_assign
l_int|0
suffix:semicolon
...
multiline_comment|/*&n; * Function irport_get_fcr (speed)&n; *&n; *    Compute value of fcr&n; *&n; */
DECL|function|irport_get_fcr
r_static
r_inline
r_int
r_int
...
suffix:semicolon
r_else
singleline_comment|//fcr |= UART_FCR_TRIGGER_14;
id|fcr
op_or_assign
...
id|speed
suffix:semicolon
multiline_comment|/* Get proper fifo configuration */
id|fcr
op_assign
id|irport_get_fcr
c_func
...
multiline_comment|/* Turn on interrups */
multiline_comment|/* This will generate a fatal interrupt storm.&n;&t; * People calling us will do that properly - Jean II */
...
multiline_comment|/* Put stuff in the state we found them - Jean II */
r_if
c_cond
(paren
id|wasunlocked
...
suffix:semicolon
multiline_comment|/* Debug what&squot;s happening... */
multiline_comment|/* Get interrupt status */
id|lsr
op_assign
id|inb
c_func
...
l_int|0
comma
l_string|&quot;%s(), transmitting=%d, remain=%d, done=%d&bslash;n&quot;
comma
id|__FUNCTION__
comma
id|self-&gt;transmitting
...
suffix:semicolon
multiline_comment|/* Now, restart the port */
id|irport_start
c_func
(paren
id|self
...
id|self-&gt;tx_buff.len
suffix:semicolon
multiline_comment|/* We are transmitting */
id|self-&gt;transmitting
op_assign
l_int|1
suffix:semicolon
...
multiline_comment|/*&n; * Function irport_write (driver)&n; *&n; *    Fill Tx FIFO with transmit data&n; *&n; * Called only from irport_write_wakeup()&n; */
...
multiline_comment|/*&n; * Function irport_write_wakeup (tty)&n; *&n; *    Called by the driver when there&squot;s room for more data.  If we have&n; *    more packets to send, we send them here.&n; *&n; * Called only from irport_interrupt()&n; * Make sure this function is *not* called while we are receiving,&n; * otherwise we will reset fifo and loose data :-(&n; */
...
l_int|1
comma
l_string|&quot;%s() : finished Tx&bslash;n&quot;
comma
id|__FUNCTION__
)paren
suffix:semicolon
...
multiline_comment|/*&n; * Function irport_receive (self)&n; *&n; *    Receive one frame from the infrared port&n; *&n; * Called only from irport_interrupt()&n; */
...
id|self-&gt;io.sir_base
suffix:semicolon
multiline_comment|/* Cut&squot;n&squot;paste interrupt routine from serial.c&n;&t; * This version try to minimise latency and I/O operations.&n;&t; * Simplified and modified to enforce half duplex operation.&n;&t; * - Jean II */
multiline_comment|/* Check status even is iir reg is cleared, more robust and&n;&t; * eliminate a read on the I/O bus - Jean II */
r_do
(brace
multiline_comment|/* Get interrupt status ; Clear interrupt */
id|lsr
op_assign
id|inb
c_func
...
suffix:semicolon
multiline_comment|/* Are we receiving or transmitting ? */
r_if
c_cond
(paren
op_logical_neg
...
(brace
multiline_comment|/* Received something ? */
r_if
c_cond
(paren
id|lsr
...
r_else
(brace
multiline_comment|/* Room in Tx fifo ? */
r_if
c_cond
(paren
id|lsr
...
multiline_comment|/* A bit hackish, but working as expected... Jean II */
r_if
c_cond
(paren
id|lsr
...
suffix:semicolon
multiline_comment|/* Enable this debug only when no other options and at low&n;&t;&t; * bit rates, otherwise it may cause Rx overruns (lsr=63).&n;&t;&t; * - Jean II */
id|IRDA_DEBUG
c_func
(paren
l_int|6
...
suffix:semicolon
multiline_comment|/* As long as interrupt pending... */
)brace
r_while
c_loop
(paren
...
suffix:semicolon
multiline_comment|/* Set 9600 bauds per default, including at the dongle */
id|irda_task_execute
c_func
(paren
id|self
...
multiline_comment|/* Locking :&n;&t;&t; * irda_device_dongle_init() can&squot;t be locked.&n;&t;&t; * irda_task_execute() doesn&squot;t need to be locked.&n;&t;&t; * Jean II&n;&t;&t; */
