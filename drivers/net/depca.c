...
DECL|macro|DE210_ID
mdefine_line|#define DE210_ID 0x628d
DECL|macro|DE212_ID
mdefine_line|#define DE212_ID 0x6def
DECL|variable|depca_mca_adapter_ids
r_static
r_int
id|depca_mca_adapter_ids
(braket
)braket
op_assign
(brace
...
suffix:semicolon
DECL|variable|depca_mca_adapter_name
r_static
r_char
op_star
id|depca_mca_adapter_name
(braket
)braket
op_assign
(brace
l_string|&quot;DEC EtherWORKS MC Adapter (DE210)&quot;
comma
l_string|&quot;DEC EtherWORKS MC Adapter (DE212)&quot;
comma
l_int|NULL
)brace
suffix:semicolon
DECL|variable|depca_mca_adapter_type
r_static
r_enum
id|depca_type
id|depca_mca_adapter_type
(braket
)braket
op_assign
(brace
...
suffix:semicolon
DECL|variable|depca_mca_driver
r_static
r_struct
id|mca_driver
id|depca_mca_driver
...
suffix:semicolon
DECL|variable|depca_isa_driver
r_static
r_struct
id|device_driver
id|depca_isa_driver
...
r_enum
(brace
DECL|enumerator|DEPCA_BUS_MCA
id|DEPCA_BUS_MCA
op_assign
l_int|1
comma
DECL|enumerator|DEPCA_BUS_ISA
id|DEPCA_BUS_ISA
comma
DECL|enumerator|DEPCA_BUS_EISA
id|DEPCA_BUS_EISA
comma
DECL|member|depca_bus
)brace
id|depca_bus
suffix:semicolon
multiline_comment|/* type of bus */
...
id|mem_start
suffix:semicolon
multiline_comment|/* Bus address of device RAM (before remap) */
DECL|member|mem_len
id|u_long
id|mem_len
suffix:semicolon
multiline_comment|/* device memory size */
...
id|mem
comma
l_string|&quot;DEPCA shared memory address&quot;
)paren
suffix:semicolon
id|MODULE_LICENSE
c_func
...
id|mem_start
suffix:semicolon
multiline_comment|/*&n;&t; * We are now supposed to enter this function with the&n;&t; * following fields filled with proper values :&n;&t; *&n;&t; * dev-&gt;base_addr&n;&t; * lp-&gt;mem_start&n;&t; * lp-&gt;depca_bus&n;&t; * lp-&gt;adapter&n;&t; *&n;&t; * dev-&gt;irq can be set if known from device configuration (on&n;&t; * MCA or EISA) or module option. Otherwise, it will be auto&n;&t; * detected.&n;&t; */
id|ioaddr
op_assign
id|dev-&gt;base_addr
suffix:semicolon
...
c_func
(paren
l_string|&quot;Unknown DEPCA bus %d&bslash;n&quot;
comma
id|lp-&gt;depca_bus
)paren
suffix:semicolon
...
l_int|32
suffix:semicolon
multiline_comment|/* Only EISA/ISA needs start address to be re-computed */
r_if
c_cond
(paren
id|lp-&gt;depca_bus
...
(paren
id|KERN_ERR
l_string|&quot;depca: cannot request ISA memory, aborting&bslash;n&quot;
)paren
suffix:semicolon
r_goto
id|out_priv
...
DECL|function|depca_common_init
r_static
r_int
id|__init
id|depca_common_init
(paren
id|u_long
id|ioaddr
comma
...
DECL|function|depca_platform_release
r_static
r_void
id|depca_platform_release
(paren
r_struct
id|device
op_star
...
DECL|function|depca_platform_probe
r_static
r_void
id|__init
id|depca_platform_probe
(paren
r_void
)paren
(brace
...
l_int|NULL
suffix:semicolon
multiline_comment|/* if an address has been specified on the command&n;&t;&t; * line, use it (if valid) */
r_if
c_cond
(paren
id|io
...
(brace
multiline_comment|/* The driver was not bound to this device, there was&n;&t;&t; * no hardware at this address. Unregister it, as the&n;&t;&t; * release fuction will take care of freeing the&n;&t;&t; * allocated structure */
id|depca_io_ports
(braket
id|i
)braket
...
DECL|function|depca_shmem_probe
r_static
r_enum
id|depca_type
id|__init
id|depca_shmem_probe
(paren
id|ulong
op_star
id|mem_start
...
id|irq
suffix:semicolon
multiline_comment|/* Use whatever value the user gave&n;&t;&t;&t;&t; * us, and 0 if he didn&squot;t. */
id|lp
op_assign
id|dev-&gt;priv
suffix:semicolon
...
multiline_comment|/*&n;** EISA callbacks from sysfs.&n;*/
macro_line|#ifdef CONFIG_EISA
DECL|function|depca_eisa_probe
r_static
r_int
id|__init
id|depca_eisa_probe
...
id|out
suffix:semicolon
multiline_comment|/* It would have been nice to get card configuration from the&n;&t; * card. Unfortunately, this register is write-only (shares&n;&t; * it&squot;s address with the ethernet prom)... As we don&squot;t parse&n;&t; * the EISA configuration structures (yet... :-), just rely on&n;&t; * the ISA probing to sort it out... */
id|depca_shmem_probe
(paren
op_amp
id|mem_start
...
macro_line|#endif
DECL|function|depca_device_remove
r_static
r_int
id|__devexit
id|depca_device_remove
...
l_int|0x8000
suffix:semicolon
multiline_comment|/* 32KB */
multiline_comment|/* Can&squot;t reserve the prom region, it is already marked as&n;&t; * used, at least on x86. Instead, reserve a memory region a&n;&t; * board would certainly use. If it works, go ahead. If not,&n;&t; * run like hell... */
r_if
c_cond
(paren
op_logical_neg
...
DECL|function|depca_module_init
r_static
r_int
id|__init
id|depca_module_init
(paren
r_void
)paren
(brace
...
DECL|function|depca_module_exit
r_static
r_void
id|__exit
id|depca_module_exit
(paren
r_void
)paren
(brace
...
DECL|variable|depca_module_init
id|module_init
(paren
id|depca_module_init
)paren
suffix:semicolon
DECL|variable|depca_module_exit
id|module_exit
(paren
id|depca_module_exit
)paren
