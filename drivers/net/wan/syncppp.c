...
id|debug
suffix:semicolon
multiline_comment|/* Flush global outgoing packet queue to dev_queue_xmit().&n; *&n; * dev_queue_xmit() must be called with interrupts enabled&n; * which means it can&squot;t be called with spinlocks held.&n; * If a packet needs to be sent while a spinlock is held,&n; * then put the packet into tx_queue, and call sppp_flush_xmit()&n; * after spinlock is released.&n; */
DECL|function|sppp_flush_xmit
r_static
r_void
id|sppp_flush_xmit
c_func
