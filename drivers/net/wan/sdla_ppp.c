...
mdefine_line|#define MAX_IP_ERRORS 100 
...
mdefine_line|#define TMR_INT_ENABLED_UPDATE  &t;0x01
DECL|macro|TMR_INT_ENABLED_PPP_EVENT
mdefine_line|#define TMR_INT_ENABLED_PPP_EVENT&t;0x02
DECL|macro|TMR_INT_ENABLED_UDP
mdefine_line|#define TMR_INT_ENABLED_UDP&t;&t;0x04
DECL|macro|TMR_INT_ENABLED_CONFIG
mdefine_line|#define TMR_INT_ENABLED_CONFIG&t;&t;0x20
...
id|interface_down
suffix:semicolon
multiline_comment|/* Brind down interface when channel &n;                                                   goes down */
DECL|member|config_wait_timeout
r_int
r_int
id|config_wait_timeout
suffix:semicolon
multiline_comment|/* After if_open() if in dynamic if mode,&n;&t;&t;&t;&t;&t;&t;   wait a few seconds before configuring */
...
id|ENODEV
suffix:semicolon
multiline_comment|/* Shutdown bug fix. This function can be&n;         * called with NULL dev pointer during&n;         * shutdown &n;&t; */
r_if
c_cond
(paren
(paren
...
id|conf-&gt;chap
suffix:semicolon
multiline_comment|/* Option to bring down the interface when &n;         * the link goes down */
r_if
c_cond
(paren
id|conf-&gt;if_down
...
comma
op_amp
id|ppp_priv_area-&gt;interface_down
)paren
suffix:semicolon
id|printk
c_func
...
suffix:semicolon
id|ppp_priv_area-&gt;poll_delay_timer.data
op_assign
(paren
r_int
r_int
...
id|dev
suffix:semicolon
id|ppp_priv_area-&gt;poll_delay_timer.function
op_assign
id|ppp_poll_delay
suffix:semicolon
multiline_comment|/* Since we start with dummy IP addresses we can say&n;&t; * that route exists */
id|printk
c_func
(paren
id|KERN_INFO
...
id|tv
suffix:semicolon
singleline_comment|//unsigned long smp_flags;
r_if
c_cond
(paren
id|is_dev_running
...
id|tv.tv_sec
suffix:semicolon
multiline_comment|/* We cannot configure the card here because we don&squot;t&n;&t; * have access to the interface IP addresses.&n;         * Once the interface initilization is complete, we will be&n;         * able to access the IP addresses.  Therefore,&n;         * configure the ppp link in the poll routine */
id|set_bit
c_func
(paren
l_int|0
comma
op_amp
id|ppp_priv_area-&gt;config_ppp
)paren
suffix:semicolon
id|ppp_priv_area-&gt;config_wait_timeout
op_assign
id|jiffies
suffix:semicolon
...
(paren
id|KERN_INFO
l_string|&quot;%s: Transmit times out on %s&bslash;n&quot;
comma
id|card-&gt;devname
comma
id|dev-&gt;name
...
(paren
id|KERN_INFO
l_string|&quot;%s: PIPEMON UDP request too long : %li&bslash;n&quot;
comma
id|card-&gt;devname
comma
id|skb-&gt;len
...
DECL|function|ppp_comm_disable_shutdown
r_static
r_int
id|ppp_comm_disable_shutdown
c_func
...
id|CMD_TIMEOUT
suffix:semicolon
multiline_comment|/* Disable communicatinons */
id|memset
c_func
(paren
op_amp
...
(brace
multiline_comment|/* Make sure we are not already sending */
r_if
c_cond
(paren
op_logical_neg
...
(brace
multiline_comment|/* Initialize the polling timer and set the state&n;                                 * to WAN_CONNNECTED */
multiline_comment|/* BUG FIX: When the protocol restarts, during heavy &n;                                 * traffic, board tx buffers and driver tx buffers&n;                                 * can go out of sync.  This checks the condition&n;                                 * and if the tx buffers are out of sync, the &n;                                 * protocols are restarted. &n;                                 * I don&squot;t know why the board tx buffer is out&n;                                 * of sync. It could be that a packets is tx&n;                                 * while the link is down, but that is not &n;                                 * possible. The other possiblility is that the&n;                                 * firmware doesn&squot;t reinitialize properly.&n;                                 * FIXME: A better fix should be found.&n;                                 */
r_if
c_cond
(paren
id|detect_and_fix_tx_bug
...
(paren
id|KERN_INFO
l_string|&quot;CON: L Tx: %lx  B Tx: %lx || L Rx %lx B Rx %lx&bslash;n&quot;
comma
(paren
r_int
r_int
...
id|PPP_INTR_TIMER
suffix:semicolon
multiline_comment|/* If we are in PEER mode, we must first obtain the&n;&t;&t;&t;&t; * IP information and then go into the poll routine */
r_if
c_cond
(paren
id|card-&gt;u.p.ip_mode
...
(paren
id|KERN_INFO
l_string|&quot;%s: Failed to read PEER IP Addresses&bslash;n&quot;
comma
id|card-&gt;devname
)paren
suffix:semicolon
...
r_else
(brace
singleline_comment|//FIXME Put the comment back int
id|ppp_priv_area-&gt;timer_int_enabled
op_and_assign
op_complement
id|TMR_INT_ENABLED_PPP_EVENT
...
(brace
multiline_comment|/* We get ip_local from the firmware in PEER mode.&n;&t;         * Therefore, if ip_local is 0, we failed to obtain&n;         &t; * the remote IP address. */
r_if
c_cond
(paren
...
(paren
id|KERN_INFO
l_string|&quot;%s: Error: Failed to add a route for PPP interface %s&bslash;n&quot;
comma
id|card-&gt;devname
comma
id|dev-&gt;name
...
(paren
id|KERN_INFO
l_string|&quot;%s: Enabling IPX Protocol&bslash;n&quot;
comma
id|card-&gt;devname
)paren
suffix:semicolon
...
(paren
id|KERN_INFO
l_string|&quot;%s: PPP IP Mode: STATIC&bslash;n&quot;
comma
id|card-&gt;devname
)paren
suffix:semicolon
...
(paren
id|KERN_INFO
l_string|&quot;%s: PPP IP Mode: HOST&bslash;n&quot;
comma
id|card-&gt;devname
)paren
suffix:semicolon
...
(paren
id|KERN_INFO
l_string|&quot;%s: PPP IP Mode: PEER&bslash;n&quot;
comma
id|card-&gt;devname
)paren
suffix:semicolon
...
suffix:semicolon
macro_line|#else
multiline_comment|/* No PEER support for 2.0.X kernels, drop down to default&n;   &t;&t;&t; * condition */
id|printk
c_func
(paren
id|KERN_INFO
l_string|&quot;%s: ERROR, PEER mode is not supported in 2.0.X kernels&bslash;n&quot;
comma
id|card-&gt;devname
)paren
suffix:semicolon
...
(paren
id|KERN_INFO
l_string|&quot;%s:        PPP IP Modes: STATIC, PEER or HOST&bslash;n&quot;
comma
id|card-&gt;devname
)paren
suffix:semicolon
...
suffix:colon
r_case
id|PPP_READ_STATISTICS
suffix:colon
r_case
id|PPP_READ_ERROR_STATS
suffix:colon
r_case
id|PPP_READ_PACKET_STATS
suffix:colon
r_case
id|PPP_READ_LCP_STATS
suffix:colon
r_case
id|PPP_READ_IPCP_STATS
suffix:colon
r_case
id|PPP_READ_IPXCP_STATS
suffix:colon
r_case
id|PPP_READ_PAP_STATS
suffix:colon
r_case
id|PPP_READ_CHAP_STATS
suffix:colon
r_case
id|PPP_READ_CODE_VERSION
suffix:colon
...
suffix:semicolon
op_increment
id|ppp_priv_area-&gt;pipe_mgmt_stat.UDP_PIPE_mgmt_direction_err
suffix:semicolon
r_if
c_cond
(paren
...
id|udp_dflt_cmd
suffix:semicolon
multiline_comment|/* WARNING: FIXME: This should be fixed.&n;&t;&t; * The FT1 Status Ctrl doesn&squot;t have a break&n;                 * statment.  Thus, no code must be inserted&n;                 * HERE: between default and above case statement */
r_default
suffix:colon
id|udp_dflt_cmd
suffix:colon
...
suffix:semicolon
macro_line|#else
macro_line|#ifdef _DYNAMIC_ROUTE_20X_SUPPORT_
r_struct
id|rtentry
id|route
suffix:semicolon
...
macro_line|#else
multiline_comment|/* FIXME: Dynamic Routing in 2.0.X kernels is not&n;         * supported. Sorry ! I&squot;ll come back to it when I get&n;         * a chance. */
id|printk
c_func
(paren
id|KERN_INFO
l_string|&quot;%s: ERROR, Dynamic routing is not supported in 2.0.X kernels&bslash;n&quot;
comma
id|card-&gt;devname
)paren
suffix:semicolon
...
(paren
id|KERN_INFO
l_string|&quot;%s:        Please use the STATIC IP mode!&bslash;n&quot;
comma
id|card-&gt;devname
)paren
suffix:semicolon
...
(paren
id|KERN_INFO
l_string|&quot;%s: Adding of route failed: %i&bslash;n&quot;
comma
id|card-&gt;devname
comma
id|err
...
(paren
id|KERN_INFO
l_string|&quot;READ CONNECTION GOT IP ADDRESS %x, %x&bslash;n&quot;
comma
id|ppp_priv_area-&gt;ip_remote
comma
id|ppp_priv_area-&gt;ip_local
...
multiline_comment|/*===============================================================================&n; * config_ppp&n; *&n; *&t;Configure the ppp protocol and enable communications.&t;&t;&n; *&n; *   &t;The if_open function binds this function to the poll routine.&n; *      Therefore, this function will run every time the ppp interface&n; *      is brought up.  &n; *      &n; *&t;If the communications are not enabled, proceed to configure&n; *      the card and enable communications.&n; *&n; *      If the communications are enabled, it means that the interface&n; *      was shutdown by ether the user or driver. In this case, we &n; *      have to check that the IP addresses have not changed.  If&n; *      the IP addresses changed, we have to reconfigure the firmware&n; *      and update the changed IP addresses.  Otherwise, just exit.&n; */
DECL|function|config_ppp
r_static
r_int
id|config_ppp
(paren
...
c_cond
(paren
id|ppp_priv_area-&gt;ip_local_tmp
op_ne
id|ppp_priv_area-&gt;ip_local
op_logical_or
id|ppp_priv_area-&gt;ip_remote_tmp
op_ne
id|ppp_priv_area-&gt;ip_remote
)paren
(brace
...
multiline_comment|/* Record the new IP addreses */
id|ppp_priv_area-&gt;ip_local
op_assign
id|ppp_priv_area-&gt;ip_local_tmp
suffix:semicolon
...
(paren
id|KERN_INFO
l_string|&quot;%s: Failed to configure PPP device&bslash;n&quot;
comma
id|card-&gt;devname
)paren
suffix:semicolon
...
(paren
id|KERN_INFO
l_string|&quot;%s: Failed to configure board interrupts !&bslash;n&quot;
comma
id|card-&gt;devname
)paren
suffix:semicolon
...
(paren
id|KERN_INFO
l_string|&quot;%s: Outbound authentication failed !&bslash;n&quot;
comma
id|card-&gt;devname
)paren
suffix:semicolon
...
(paren
id|KERN_INFO
l_string|&quot;%s: Inbound authentication failed !&bslash;n&quot;
comma
id|card-&gt;devname
)paren
suffix:semicolon
...
multiline_comment|/* If we fail to enable communications here it&squot;s OK,&n;&t; * since the DTR timer will cause a disconnected, which&n;&t; * will retrigger communication in timer_intr() */
r_if
c_cond
(paren
id|ppp_comm_enable
...
id|card-&gt;flags
suffix:semicolon
multiline_comment|/* Shutdown is in progress, stop what you are &n;&t; * doing and get out */
r_if
c_cond
(paren
id|test_bit
...
r_else
(brace
multiline_comment|/* We need to check if the local IP address is&n;               &t;   &t; * zero. If it is, we shouldn&squot;t try to remove it.&n;                 &t; * For some reason the kernel crashes badly if &n;                 &t; * we try to remove the route twice */
r_if
c_cond
(paren
id|card-&gt;wandev.dev-&gt;flags
...
multiline_comment|/*============================================================&n; * trigger_ppp_poll&n; *&n; * Description:&n; * &t;Add a ppp_poll() task into a tq_scheduler bh handler&n; *      for a specific interface.  This will kick&n; *      the ppp_poll() routine at a later time. &n; *&n; * Usage:&n; * &t;Interrupts use this to defer a taks to &n; *      a polling routine.&n; *&n; */
DECL|function|trigger_ppp_poll
r_static
r_void
id|trigger_ppp_poll
(paren
...
multiline_comment|/*============================================================&n; * detect_and_fix_tx_bug&n; *&n; * Description:&n; *&t;On connect, if the board tx buffer ptr is not the same&n; *      as the driver tx buffer ptr, we found a firmware bug.&n; *      Report the bug to the above layer.  To fix the&n; *      error restart communications again.&n; *&n; * Usage:&n; *&n; */
DECL|function|detect_and_fix_tx_bug
r_static
r_int
id|detect_and_fix_tx_bug
(paren
...
(paren
id|KERN_INFO
l_string|&quot;Major Error, Fix the bug&bslash;n&quot;
)paren
suffix:semicolon
r_return
l_int|1
