...
DECL|macro|TX_QUEUE_LEN_RESTART
mdefine_line|#define TX_QUEUE_LEN_RESTART&t;5
...
DECL|enumerator|AlwaysFDX
DECL|enumerator|FDXOnNoMII
id|CanHaveMII
op_assign
l_int|1
comma
...
comma
multiline_comment|/* Sometime a Level-One switch card. */
(brace
l_int|0x08401050
comma
l_int|0xffffffff
...
suffix:semicolon
multiline_comment|/* MII device addresses, but only the first is used */
DECL|member|mii
id|u32
id|mii
suffix:semicolon
...
(paren
id|KERN_INFO
l_string|&quot;%s: MII PHY %8.8xh found at address %d, status &quot;
...
DECL|macro|MII_DAVICOM_DM9101
mdefine_line|#define MII_DAVICOM_DM9101&t;0x0181b800
DECL|function|update_link
r_static
r_int
id|update_link
c_func
...
id|np-&gt;csr6
suffix:semicolon
multiline_comment|/* reread: the link status bit is sticky */
id|mii_reg
op_assign
id|mdio_read
c_func
...
(paren
id|KERN_INFO
l_string|&quot;%s: MII #%d reports no link. Disabling watchdog.&bslash;n&quot;
comma
id|dev-&gt;name
comma
id|np-&gt;phys
...
(paren
id|KERN_INFO
l_string|&quot;%s: MII #%d link is back. Enabling watchdog.&bslash;n&quot;
comma
id|dev-&gt;name
comma
id|np-&gt;phys
...
op_eq
id|MII_DAVICOM_DM9101
)paren
(brace
multiline_comment|/* If the link partner doesn&squot;t support autonegotiation&n;&t;&t; * the MII detects it&squot;s abilities with the &quot;parallel detection&quot;.&n;&t;&t; * Some MIIs update the LPA register to the result of the parallel&n;&t;&t; * detection, some don&squot;t.&n;&t;&t; * The Davicom PHY [at least 0181b800] doesn&squot;t.&n;&t;&t; * Instead bit 9 and 13 of the BMCR are updated to the result&n;&t;&t; * of the negotiation..&n;&t;&t; */
id|mii_reg
op_assign
id|mdio_read
c_func
...
(paren
id|KERN_INFO
l_string|&quot;%s: Setting %dMBit-%s-duplex based on MII#%d&bslash;n&quot;
comma
id|dev-&gt;name
comma
id|fasteth
...
DECL|macro|RXTX_TIMEOUT
mdefine_line|#define RXTX_TIMEOUT&t;2000
DECL|function|update_csr6
r_static
r_inline
r_void
id|update_csr6
...
r_return
suffix:semicolon
multiline_comment|/* stop both Tx and Rx processes */
id|writel
c_func
(paren
id|np-&gt;csr6
...
suffix:semicolon
multiline_comment|/* wait until they have really stopped */
r_for
c_loop
(paren
suffix:semicolon
...
(paren
id|KERN_INFO
l_string|&quot;%s: couldn&squot;t stop rxtx, IntrStatus %xh.&bslash;n&quot;
comma
id|dev-&gt;name
comma
id|csr5
...
r_new
suffix:semicolon
multiline_comment|/* and restart them with the new configuration */
id|writel
c_func
(paren
id|np-&gt;csr6
...
suffix:semicolon
multiline_comment|/* Big-endian descriptors */
macro_line|#else
id|i
op_assign
l_int|0
...
l_string|&quot;alignment to 8 longwords.&bslash;n&quot;
comma
id|dev-&gt;name
)paren
suffix:semicolon
...
l_int|0
suffix:semicolon
multiline_comment|/* 128 byte Tx threshold; &n;&t;&t;Transmit on; Receive on; */
id|update_csr6
c_func
(paren
id|dev
...
multiline_comment|/* Now acquire the irq spinlock.&n;&t; * The difficult race is the the ordering between&n;&t; * increasing np-&gt;cur_tx and setting DescOwn:&n;&t; * - if np-&gt;cur_tx is increased first the interrupt&n;&t; *   handler could consider the packet as transmitted&n;&t; *   since DescOwn is cleared.&n;&t; * - If DescOwn is set first the NIC could report the&n;&t; *   packet as sent, but the interrupt handler would ignore it&n;&t; *   since the np-&gt;cur_tx was not yet increased.&n;&t; */
id|spin_lock_irq
c_func
(paren
op_amp
...
(paren
id|KERN_DEBUG
l_string|&quot;%s: Transmit slot %d ok, Tx status %8.8x.&bslash;n&quot;
comma
id|dev-&gt;name
comma
id|entry
...
(paren
id|KERN_DEBUG
l_string|&quot;%s: Tx underflow, new csr6 %8.8x.&bslash;n&quot;
comma
id|dev-&gt;name
comma
r_new
...
multiline_comment|/*&n; * suspend/resume synchronization:&n; * - open, close, do_ioctl:&n; * &t;rtnl_lock, &amp; netif_device_detach after the rtnl_unlock.&n; * - get_stats:&n; * &t;spin_lock_irq(np-&gt;lock), doesn&squot;t touch hw if not present&n; * - hard_start_xmit:&n; * &t;netif_stop_queue + spin_unlock_wait(&amp;dev-&gt;xmit_lock);&n; * - tx_timeout:&n; * &t;netif_device_detach + spin_unlock_wait(&amp;dev-&gt;xmit_lock);&n; * - set_multicast_list&n; * &t;netif_device_detach + spin_unlock_wait(&amp;dev-&gt;xmit_lock);&n; * - interrupt handler&n; * &t;doesn&squot;t touch hw if not present, synchronize_irq waits for&n; * &t;running instances of the interrupt handler.&n; *&n; * Disabling hw requires clearing csr6 &amp; IntrEnable.&n; * update_csr6 &amp; all function that write IntrEnable check netif_device_present&n; * before settings any bits.&n; *&n; * Detach must occur under spin_unlock_irq(), interrupts from a detached&n; * device would cause an irq storm.&n; */
DECL|function|w840_suspend
r_static
r_int
id|w840_suspend
(paren
...
l_int|0xffff
suffix:semicolon
multiline_comment|/* no more hardware accesses behind this line. */
r_if
c_cond
(paren
id|np-&gt;csr6
