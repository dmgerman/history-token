...
multiline_comment|/* Buffer sizes:&n; * The nic writes 32-bit values, even if the upper bytes of&n; * a 32-bit value are beyond the end of the buffer.&n; */
DECL|macro|NATSEMI_HEADERS
mdefine_line|#define NATSEMI_HEADERS&t;&t;22&t;/* 2*mac,type,vlan,crc */
DECL|macro|NATSEMI_PADDING
mdefine_line|#define NATSEMI_PADDING&t;&t;16&t;/* 2 bytes should be sufficient */
DECL|macro|NATSEMI_LONGPKT
mdefine_line|#define NATSEMI_LONGPKT&t;&t;1518&t;/* limit for normal packets */
DECL|macro|NATSEMI_RX_LIMIT
mdefine_line|#define NATSEMI_RX_LIMIT&t;2046&t;/* maximum supported by hardware */
...
multiline_comment|/*&n; * Support for fibre connections on Am79C874:&n; * This phy needs a special setup when connected to a fibre cable.&n; * http://www.amd.com/files/connectivitysolutions/networking/archivednetworking/22235.pdf&n; */
DECL|macro|PHYID_AM79C874
mdefine_line|#define PHYID_AM79C874&t;0x0022561b
DECL|macro|MII_MCTRL
mdefine_line|#define MII_MCTRL&t;0x15&t;/* mode control register */
DECL|macro|MII_FX_SEL
mdefine_line|#define MII_FX_SEL&t;0x0001&t;/* 100BASE-FX (fiber) */
DECL|macro|MII_EN_SCRM
mdefine_line|#define MII_EN_SCRM&t;0x0004&t;/* enable scrambler (tp) */
...
suffix:semicolon
multiline_comment|/* &n; * Tx Configuration:&n; * - 256 byte DMA burst length&n; * - fill threshold 512 bytes (i.e. restart DMA when 512 bytes are free)&n; * - 64 bytes initial drain threshold (i.e. begin actual transmission&n; *   when 64 byte are in the fifo)&n; * - on tx underruns, increase drain threshold by 64.&n; * - at most use a drain threshold of 1472 bytes: The sum of the fill&n; *   threshold and the drain threshold must be less than 2016 bytes.&n; *&n; */
DECL|macro|TX_FLTH_VAL
mdefine_line|#define TX_FLTH_VAL&t;&t;((512/32) &lt;&lt; 8)
DECL|macro|TX_DRTH_VAL_START
mdefine_line|#define TX_DRTH_VAL_START&t;(64/32)
DECL|macro|TX_DRTH_VAL_INC
mdefine_line|#define TX_DRTH_VAL_INC&t;&t;2
DECL|macro|TX_DRTH_VAL_LIMIT
mdefine_line|#define TX_DRTH_VAL_LIMIT&t;(1472/32)
...
suffix:semicolon
DECL|macro|RX_DRTH_VAL
mdefine_line|#define RX_DRTH_VAL&t;&t;(128/8)
...
suffix:semicolon
DECL|macro|PHY_ADDR_NONE
mdefine_line|#define PHY_ADDR_NONE&t;&t;32
DECL|macro|PHY_ADDR_INTERNAL
mdefine_line|#define PHY_ADDR_INTERNAL&t;1
...
id|hands_off
suffix:semicolon
multiline_comment|/* external phy that is used: only valid if dev-&gt;if_port != PORT_TP */
DECL|member|mii
r_int
id|mii
suffix:semicolon
DECL|member|phy_addr_external
r_int
id|phy_addr_external
suffix:semicolon
...
id|dspcfg
suffix:semicolon
multiline_comment|/* parms saved in ethtool format */
DECL|member|speed
id|u16
id|speed
suffix:semicolon
...
id|autoneg
suffix:semicolon
multiline_comment|/* Autonegotiation enabled */
...
l_int|31
suffix:semicolon
multiline_comment|/* &n;&t; * The internal phy is visible on the external mii bus. Therefore we must&n;&t; * move it away before we can send commands to an external phy.&n;&t; * There are two addresses we must avoid:&n;&t; * - the address on the external phy that is used for transmission.&n;&t; * - the address that we want to access. User space can access phys&n;&t; *   on the mii bus with SIOCGMIIREG/SIOCSMIIREG, independant from the&n;&t; *   phy that is used for transmission.&n;&t; */
r_if
c_cond
(paren
id|target
...
l_int|0
suffix:semicolon
multiline_comment|/* Initial port:&n;&t; * - If the nic was configured to use an external phy and if find_mii&n;&t; *   finds a phy: use external port, first phy that replies.&n;&t; * - Otherwise: internal port.&n;&t; * Note that the phy address for the internal phy doesn&squot;t matter:&n;&t; * The address would be used to access a phy over the mii bus, but&n;&t; * the internal phy is accessed through mapped registers.&n;&t; */
r_if
c_cond
(paren
id|readl
...
(paren
id|KERN_INFO
l_string|&quot;natsemi %s: ignoring user supplied media type %d&quot;
comma
id|pci_name
c_func
(paren
...
suffix:semicolon
multiline_comment|/* get the initial settings from hardware */
id|tmp
op_assign
id|mdio_read
c_func
...
(paren
id|KERN_INFO
l_string|&quot;natsemi %s: Transceiver default autonegotiation %s &quot;
...
(paren
id|KERN_INFO
l_string|&quot;natsemi %s: Transceiver status %#04x advertising %#04x.&bslash;n&quot;
comma
id|pci_name
c_func
(paren
...
(paren
id|KERN_INFO
l_string|&quot;natsemi %s: silicon revision %#04x.&bslash;n&quot;
comma
id|pci_name
c_func
(paren
...
(paren
id|KERN_INFO
l_string|&quot;natsemi %s: %s at %#08lx (%s), &quot;
comma
id|dev-&gt;name
comma
id|natsemi_pci_info
...
c_func
(paren
l_string|&quot;, port MII, phy ad %d.&bslash;n&quot;
comma
id|np-&gt;phy_addr_external
)paren
suffix:semicolon
...
multiline_comment|/* MII transceiver control section.&n; * The 83815 series has an internal transceiver, and we present the&n; * internal management registers as if they were MII connected.&n; * External Phy registers are referenced through the MII interface.&n; */
multiline_comment|/* clock transitions &gt;= 20ns (25MHz)&n; * One readl should be good to PCI @ 100MHz&n; */
DECL|macro|mii_delay
mdefine_line|#define mii_delay(ioaddr)  readl(ioaddr + EECtrl)
...
suffix:semicolon
multiline_comment|/* The 83815 series has two ports:&n;&t; * - an internal transceiver&n;&t; * - an external mii bus&n;&t; */
r_if
c_cond
(paren
id|dev-&gt;if_port
...
suffix:semicolon
multiline_comment|/* The 83815 series has an internal transceiver; handle separately */
r_if
c_cond
(paren
id|dev-&gt;if_port
...
id|tmp
suffix:semicolon
multiline_comment|/* restore stuff lost when power was out */
id|tmp
op_assign
id|mdio_read
c_func
...
(brace
multiline_comment|/* renegotiate if something changed */
r_if
c_cond
(paren
(paren
...
(brace
multiline_comment|/* turn on autonegotiation and force negotiation */
id|tmp
op_or_assign
(paren
id|BMCR_ANENABLE
...
id|BMCR_FULLDPLX
suffix:semicolon
multiline_comment|/* &n;&t;&t; * Note: there is no good way to inform the link partner&n;&t;&t; * that our capabilities changed. The user has to unplug&n;&t;&t; * and replug the network cable after some changes, e.g.&n;&t;&t; * after switching from 10HD, autoneg off to 100 HD,&n;&t;&t; * autoneg off.&n;&t;&t; */
)brace
id|mdio_write
c_func
(paren
...
suffix:semicolon
multiline_comment|/* find out what phy this is */
id|np-&gt;mii
op_assign
(paren
id|mdio_read
...
suffix:semicolon
multiline_comment|/* handle external phys here */
r_switch
c_cond
(paren
id|np-&gt;mii
...
id|PHYID_AM79C874
suffix:colon
multiline_comment|/* phy specific configuration for fibre/tp operation */
id|tmp
op_assign
id|mdio_read
c_func
...
(paren
id|KERN_INFO
l_string|&quot;%s: DSPCFG mismatch after retrying for %d usec.&bslash;n&quot;
comma
id|dev-&gt;name
comma
id|i
...
(paren
id|KERN_INFO
l_string|&quot;%s: DSPCFG accepted after %d usec.&bslash;n&quot;
comma
id|dev-&gt;name
comma
id|i
...
multiline_comment|/*&n;&t; * Enable PHY Specific event based interrupts.  Link state change&n;&t; * and Auto-Negotiation Completion are among the affected.&n;&t; * Read the intr status to clear it (needed for wake events).&n;&t; */
id|readw
c_func
(paren
id|ioaddr
...
DECL|function|switch_port_external
r_static
r_int
id|switch_port_external
c_func
(paren
r_struct
id|net_device
...
(paren
id|KERN_INFO
l_string|&quot;%s: switching to external transceiver.&bslash;n&quot;
comma
id|dev-&gt;name
)paren
suffix:semicolon
)brace
multiline_comment|/* 1) switch back to external phy */
id|writel
c_func
(paren
id|cfg
...
suffix:semicolon
multiline_comment|/* 2) reset the external phy: */
multiline_comment|/* resetting the external PHY has been known to cause a hub supplying&n;&t; * power over Ethernet to kill the power.  We don&squot;t want to kill&n;&t; * power to this computer, so we avoid resetting the phy.&n;&t; */
multiline_comment|/* 3) reinit the phy fixup, it got lost during power down. */
id|move_int_phy
c_func
(paren
id|dev
...
DECL|function|switch_port_internal
r_static
r_int
id|switch_port_internal
c_func
(paren
r_struct
id|net_device
...
(paren
id|KERN_INFO
l_string|&quot;%s: switching to internal transceiver.&bslash;n&quot;
comma
id|dev-&gt;name
)paren
suffix:semicolon
)brace
multiline_comment|/* 1) switch back to internal phy: */
id|cfg
op_assign
id|cfg
op_amp
...
suffix:semicolon
multiline_comment|/* 2) reset the internal phy: */
id|bmcr
op_assign
id|readw
c_func
...
(paren
id|KERN_INFO
l_string|&quot;%s: phy reset did not complete in %d usec.&bslash;n&quot;
comma
id|dev-&gt;name
comma
id|i
...
multiline_comment|/* Scan for a PHY on the external mii bus.&n; * There are two tricky points:&n; * - Do not scan while the internal phy is enabled. The internal phy will&n; *   crash: e.g. reads from the DSPCFG register will return odd values and&n; *   the nasty random phy reset code will reset the nic every few seconds.&n; * - The internal phy must be moved around, an external phy could&n; *   have the same address as the internal phy.&n; */
DECL|function|find_mii
r_static
r_int
id|find_mii
c_func
...
id|did_switch
suffix:semicolon
multiline_comment|/* Switch to external phy */
id|did_switch
op_assign
id|switch_port_external
c_func
...
suffix:semicolon
multiline_comment|/* Scan the possible phy addresses:&n;&t; *&n;&t; * PHY address 0 means that the phy is in isolate mode. Not yet&n;&t; * supported due to lack of test hardware. User space should&n;&t; * handle it through ethtool.&n;&t; */
r_for
c_loop
(paren
id|i
...
(paren
id|KERN_INFO
l_string|&quot;natsemi %s: found external phy %08x at address %d.&bslash;n&quot;
comma
id|pci_name
c_func
(paren
...
multiline_comment|/* And switch back to internal phy: */
r_if
c_cond
(paren
id|did_switch
...
id|CFG_RESET_SAVE
suffix:semicolon
multiline_comment|/* turn on external phy if it was selected */
r_if
c_cond
(paren
id|dev-&gt;if_port
...
(paren
id|KERN_WARNING
l_string|&quot;natsemi %s: EEPROM did not reload in %d usec.&bslash;n&quot;
comma
id|pci_name
c_func
(paren
...
(paren
id|KERN_DEBUG
l_string|&quot;natsemi %s: EEPROM reloaded in %d usec.&bslash;n&quot;
comma
id|pci_name
c_func
(paren
...
id|bmsr
suffix:semicolon
multiline_comment|/* The link status field is latched: it remains low after a temporary&n;&t; * link failure until it&squot;s read. We need the current link status,&n;&t; * thus read twice.&n;&t; */
id|mdio_read
c_func
(paren
id|dev
...
id|RX_DRTH_VAL
suffix:semicolon
multiline_comment|/* if receive ring now has bigger buffers than normal, enable jumbo */
r_if
c_cond
(paren
id|np-&gt;rx_buf_sz
...
(brace
multiline_comment|/* if this is the tail of a double buffer&n;&t;&t;&t; * packet, we&squot;ve already counted the error&n;&t;&t;&t; * on the first part.  Ignore the second half.&n;&t;&t;&t; */
)brace
r_else
(brace
r_struct
...
(paren
id|KERN_NOTICE
l_string|&quot;%s: tx underrun with maximum tx threshold, txcfg %#08x.&bslash;n&quot;
comma
id|dev-&gt;name
comma
id|np-&gt;tx_config
...
DECL|function|natsemi_change_mtu
r_static
r_int
id|natsemi_change_mtu
c_func
...
id|new_mtu
suffix:semicolon
multiline_comment|/* synchronized against open : rtnl_lock() held by caller */
r_if
c_cond
(paren
id|netif_running
...
suffix:semicolon
multiline_comment|/* stop engines */
id|natsemi_stop_rxtx
c_func
(paren
id|dev
...
suffix:semicolon
multiline_comment|/* drain rx queue */
id|drain_rx
c_func
(paren
id|dev
...
suffix:semicolon
multiline_comment|/* change buffers */
id|set_bufsize
c_func
(paren
id|dev
...
suffix:semicolon
multiline_comment|/* restart engines */
id|writel
c_func
(paren
id|RxOn
...
id|np-&gt;phy_addr_external
suffix:semicolon
multiline_comment|/*&n;&t; * We intentionally report the phy address of the external&n;&t; * phy, even if the internal phy is used. This is necessary&n;&t; * to work around a deficiency of the ethtool interface:&n;&t; * It&squot;s only possible to query the settings of the active&n;&t; * port. Therefore &n;&t; * # ethtool -s ethX port mii&n;&t; * actually sends an ioctl to switch to port mii with the&n;&t; * settings that are used for the current active port.&n;&t; * If we would report a different phy address in this&n;&t; * command, then&n;&t; * # ethtool -s ethX port tp;ethtool -s ethX port mii&n;&t; * would unintentionally change the phy address.&n;&t; *&n;&t; * Fortunately the phy address doesn&squot;t matter with the&n;&t; * internal phy...&n;&t; */
multiline_comment|/* set information based on active port type */
r_switch
c_cond
(paren
id|ecmd-&gt;port
...
multiline_comment|/* if autonegotiation is on, try to return the active speed/duplex */
r_if
c_cond
(paren
id|ecmd-&gt;autoneg
...
multiline_comment|/*&n;&t; * maxtxpkt, maxrxpkt: ignored for now.&n;&t; *&n;&t; * transceiver:&n;&t; * PORT_TP is always XCVR_INTERNAL, PORT_MII and PORT_FIBRE are always&n;&t; * XCVR_EXTERNAL. The implementation thus ignores ecmd-&gt;transceiver and&n;&t; * selects based on ecmd-&gt;port.&n;&t; *&n;&t; * Actually PORT_FIBRE is nearly identical to PORT_MII: it&squot;s for fibre&n;&t; * phys that are connected to the mii bus. It&squot;s used to apply fibre&n;&t; * specific updates.&n;&t; */
multiline_comment|/* WHEW! now lets bang some bits */
multiline_comment|/* save the parms */
id|dev-&gt;if_port
op_assign
id|ecmd-&gt;port
suffix:semicolon
...
id|ecmd-&gt;duplex
suffix:semicolon
multiline_comment|/* user overriding the initial full duplex parm? */
r_if
c_cond
(paren
id|np-&gt;duplex
...
multiline_comment|/* get the right phy enabled */
r_if
c_cond
(paren
id|ecmd-&gt;port
...
suffix:semicolon
multiline_comment|/* set parms and see how this affected our link status */
id|init_phy_fixup
c_func
(paren
id|dev
...
suffix:semicolon
multiline_comment|/* read non-mii page 0 of registers */
r_for
c_loop
(paren
id|i
...
multiline_comment|/* read current mii registers */
r_for
c_loop
(paren
id|i
...
suffix:colon
multiline_comment|/* for binary compat, remove in 2.5 */
multiline_comment|/* The phy_id is not enough to uniquely identify&n;&t;&t; * the intended target. Therefore the command is sent to&n;&t;&t; * the given mii on the current port.&n;&t;&t; */
r_if
c_cond
(paren
id|dev-&gt;if_port
