...
id|reset_timer
suffix:semicolon
DECL|member|reset_timer_armed
r_int
id|reset_timer_armed
suffix:semicolon
DECL|member|crvalue_sv
r_int
r_int
id|crvalue_sv
suffix:semicolon
DECL|member|imrvalue_sv
r_int
r_int
id|imrvalue_sv
suffix:semicolon
...
id|mdio_read
suffix:semicolon
id|np-&gt;mii.mdio_write
op_assign
id|mdio_write
suffix:semicolon
id|np-&gt;mii.phy_id_mask
op_assign
l_int|0x1f
suffix:semicolon
id|np-&gt;mii.reg_num_mask
op_assign
l_int|0x1f
suffix:semicolon
id|ring_space
...
id|dev
suffix:semicolon
id|np-&gt;reset_timer.function
op_assign
op_amp
id|reset_timer
suffix:semicolon
id|np-&gt;reset_timer_armed
op_assign
l_int|0
suffix:semicolon
r_return
...
multiline_comment|/* Take lock before calling */
multiline_comment|/* Reset chip and disable rx, tx and interrupts */
DECL|function|reset_and_disable_rxtx
r_static
r_void
id|reset_and_disable_rxtx
c_func
(paren
r_struct
id|net_device
...
suffix:semicolon
multiline_comment|/* Ueimor: wait for 50 PCI cycles (and flush posted writes btw). &n;&t;   We surely wait too long (address+data phase). Who cares? */
r_while
c_loop
(paren
op_decrement
...
multiline_comment|/* Take lock before calling */
multiline_comment|/* Restore chip after reset */
DECL|function|enable_rxtx
r_static
r_void
id|enable_rxtx
c_func
...
suffix:semicolon
multiline_comment|/* changes np-&gt;crvalue, writes it into TCRRCR */
multiline_comment|/* Clear and Enable interrupts by setting the interrupt mask. */
id|writel
c_func
(paren
...
(paren
id|KERN_WARNING
l_string|&quot;%s: resetting tx and rx machinery&bslash;n&quot;
comma
id|dev-&gt;name
)paren
suffix:semicolon
...
suffix:semicolon
multiline_comment|/* works for me without this:&n;&t;reset_tx_descriptors(dev); */
id|enable_rxtx
c_func
(paren
id|dev
...
suffix:semicolon
multiline_comment|/* FIXME: or netif_wake_queue(dev); ? */
id|np-&gt;reset_timer_armed
op_assign
l_int|0
suffix:semicolon
...
l_int|0
suffix:semicolon
multiline_comment|/* do we need np-&gt;tx_ring[i].control = XXX; ?? */
id|np-&gt;tx_ring
(braket
id|i
)braket
...
multiline_comment|/* Take lock before calling */
multiline_comment|/* Chip probably hosed tx ring. Clean up. */
DECL|function|reset_tx_descriptors
r_static
r_void
id|reset_tx_descriptors
c_func
...
suffix:semicolon
multiline_comment|/* or dev_kfree_skb_irq(cur-&gt;skbuff); ? */
id|cur-&gt;skbuff
op_assign
l_int|NULL
suffix:semicolon
...
multiline_comment|/* probably not needed. We do it for purely paranoid reasons */
id|cur-&gt;next_desc
op_assign
id|np-&gt;tx_ring_dma
op_plus
...
suffix:semicolon
multiline_comment|/* or netif_tx_disable(dev); ?? */
multiline_comment|/* Prevent other paths from enabling tx,rx,intrs */
id|np-&gt;crvalue_sv
op_assign
id|np-&gt;crvalue
suffix:semicolon
...
(paren
id|KERN_DEBUG
l_string|&quot;%s: I&squot;m scared&bslash;n&quot;
comma
id|dev-&gt;name
)paren
suffix:semicolon
...
r_else
(brace
multiline_comment|/* rx error, need to reset this chip */
id|stop_nic_rx
c_func
(paren
id|ioaddr
...
multiline_comment|/* for dev-&gt;set_multicast_list */
