...
suffix:semicolon
macro_line|#ifdef CONFIG_TULIP_NAPI_HW_MITIGATION
DECL|macro|MIT_SIZE
...
macro_line|#endif
multiline_comment|/* Check if the packet is long enough to accept without copying&n;                                  to a minimally-sized skbuff. */
r_if
c_cond
(paren
id|pkt_len
...
multiline_comment|/* New ack strategy... irq does not ack Rx any longer&n;                  hopefully this helps */
multiline_comment|/* Really bad things can happen here... If new packet arrives&n;                * and an irq arrives (tx or just due to occasionally unset&n;                * mask), it will be acked by irq handler, but new thread&n;                * is not scheduled. It is major hole in design.&n;                * No idea how to fix this if &quot;playing with fire&quot; will fail&n;                * tomorrow (night 011029). If it will not fail, we won&n;                * finally: amount of IO did not increase at all. */
)brace
r_while
c_loop
(paren
...
macro_line|#endif /* CONFIG_TULIP_NAPI_HW_MITIGATION */
id|dev-&gt;quota
op_sub_assign
id|received
suffix:semicolon
...
suffix:semicolon
multiline_comment|/* If RX ring is not full we are out of memory. */
r_if
c_cond
(paren
id|tp-&gt;rx_buffers
...
id|oom
suffix:semicolon
multiline_comment|/* Remove us from polling list and enable RX intr. */
...
suffix:semicolon
multiline_comment|/* The last op happens after poll completion. Which means the following:&n;          * 1. it can race with disabling irqs in irq handler&n;          * 2. it can race with dise/enabling irqs in other poll threads&n;          * 3. if an irq raised after beginning loop, it will be immediately&n;          *    triggered here.&n;          *&n;          * Summarizing: the logic results in some redundant irqs both&n;          * due to races in masking and due to too late acking of already&n;          * processed irqs. But it must not result in losing events.&n;          */
r_return
l_int|0
suffix:semicolon
id|not_done
...
id|dev-&gt;quota
suffix:semicolon
multiline_comment|/* Not to happen */
)brace
id|dev-&gt;quota
op_sub_assign
id|received
...
id|oom
suffix:colon
multiline_comment|/* Executed with RX ints disabled */
multiline_comment|/* Start timer, stop polling, but do not enable rx interrupts. */
id|mod_timer
c_func
(paren
op_amp
...
suffix:semicolon
multiline_comment|/* Think: timer_pending() was an explicit signature of bug.&n;          * Timer can be pending now but fired and completed&n;          * before we did netif_rx_complete(). See? We would lose it. */
multiline_comment|/* remove ourselves from the polling list */
id|netif_rx_complete
c_func
(paren
id|dev
...
macro_line|#else /* CONFIG_TULIP_NAPI */
...
macro_line|#endif  /* CONFIG_TULIP_NAPI */
...
id|TimerInt
op_or
multiline_comment|/* Abnormal intr. */
id|RxDied
op_or
id|TxFIFOUnderflow
op_or
