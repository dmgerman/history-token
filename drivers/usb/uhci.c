...
multiline_comment|/* Two stage unlink so we don&squot;t deadlock on urb_list_lock */
id|INIT_LIST_HEAD
c_func
(paren
op_amp
...
id|tmp-&gt;next
suffix:semicolon
multiline_comment|/* Make sure we block waiting on these to die */
id|urb-&gt;transfer_flags
op_and_assign
op_complement
id|USB_ASYNC_UNLINK
suffix:semicolon
multiline_comment|/* uhci_unlink_urb will unlink from the temp list */
id|uhci_unlink_urb
c_func
(paren
id|urb
...
id|status
suffix:semicolon
multiline_comment|/*  The HC may continue using the current QH if it finished */
multiline_comment|/* all of the TD&squot;s in this URB and may have started on the */
multiline_comment|/* next URB&squot;s TD&squot;s already, so we&squot;ll take over ownership */
multiline_comment|/* of this QH and use it instead. Don&squot;t forget to delete */
multiline_comment|/* the old QH first */
id|uhci_free_qh
c_func
(paren
id|uhci
...
l_int|NULL
suffix:semicolon
multiline_comment|/* If the last TD from the first (this) urb didn&squot;t */
multiline_comment|/*  complete, reset qh-&gt;element to the first TD in the */
multiline_comment|/*  next urb */
id|pltd
op_assign
id|list_entry
c_func
...
multiline_comment|/* FIXME: If we forcefully unlink an urb, we should reset the toggle for */
multiline_comment|/*  that pipe to match what actually completed */
