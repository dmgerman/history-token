...
macro_line|#ifdef&t;EHCI_SOFT_RETRIES
DECL|variable|soft_retries
r_static
r_int
id|soft_retries
op_assign
id|EHCI_SOFT_RETRIES
suffix:semicolon
id|MODULE_PARM
(paren
id|soft_retries
...
id|soft_retries
comma
l_string|&quot;Number of software retries for endpoint i/o&quot;
)paren
suffix:semicolon
macro_line|#endif
multiline_comment|/*-------------------------------------------------------------------------*/
...
id|dbg
(paren
l_string|&quot;devpath %s ep %d-%s qtd token %x --&gt; status %d&quot;
comma
id|urb-&gt;dev-&gt;devpath
comma
id|usb_pipeendpoint
...
id|err
(paren
l_string|&quot;must CLEAR_TT_BUFFER, hub %s port %d%s addr %d ep %d&quot;
comma
id|urb-&gt;dev-&gt;tt-&gt;hub-&gt;devpath
comma
id|urb-&gt;dev-&gt;ttport
comma
id|urb-&gt;dev-&gt;tt-&gt;multi
...
(brace
macro_line|#ifdef&t;EHCI_SOFT_RETRIES
multiline_comment|/* extra soft retries for protocol errors */
r_if
c_cond
(paren
op_logical_neg
...
id|dbg
(paren
l_string|&quot;soft retry, qh %p qtd %p&quot;
comma
id|qh
comma
id|qtd
...
suffix:semicolon
multiline_comment|/* qtd update not needed */
id|qh-&gt;hw_token
op_assign
id|cpu_to_le32
(paren
...
id|dbg
(paren
l_string|&quot;retried %d times, qh %p qtd %p&quot;
comma
id|qh-&gt;retries
comma
id|qh
...
macro_line|#endif&t;/* EHCI_SOFT_RETRIES */
id|halted
op_assign
l_int|1
suffix:semicolon
...
(brace
multiline_comment|/* stop scan if qtd is visible to the HC */
r_if
c_cond
(paren
op_logical_neg
...
multiline_comment|/* continue cleanup if HC is halted */
r_if
c_cond
(paren
id|ehci-&gt;hcd.state
...
multiline_comment|/* stall? some other urb was unlinked? */
r_if
c_cond
(paren
id|urb-&gt;status
...
id|dbg
(paren
l_string|&quot;?why? qh %p, qtd %p halted, urb %p, token %8x, len %d&quot;
comma
id|qh
comma
id|qtd
...
id|retval
suffix:semicolon
multiline_comment|/*&n;&t;     * FIXME: write this code.  When one queued urb is unlinked,&n;&t;     * unlink every succeeding urb.&n;&t;     */
r_continue
suffix:semicolon
)brace
multiline_comment|/* else stopped for some other reason */
)brace
id|scrub
suffix:colon
id|spin_lock
...
suffix:semicolon
multiline_comment|/*&n;&t;&t; * NOTE:  this won&squot;t work right with interrupt urbs that&n;&t;&t; * need multiple qtds ... only the first scan of qh-&gt;qtd_list&n;&t;&t; * starts at the right qtd, yet multiple scans could happen&n;&t;&t; * for transfers that are scheduled across multiple uframes. &n;&t;&t; */
r_if
c_cond
(paren
id|likely
...
suffix:semicolon
multiline_comment|/* another queued urb? */
r_if
c_cond
(paren
id|unlikely
...
suffix:semicolon
singleline_comment|// FIXME unlink any urb should unlink all following urbs,
singleline_comment|// so that this will never happen
r_if
c_cond
(paren
op_logical_neg
