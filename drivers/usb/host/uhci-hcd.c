...
l_int|NULL
suffix:semicolon
multiline_comment|/*&n;&t; * Only go through the hoops if it&squot;s actually linked in&n;&t; * Queued QHs are removed in uhci_delete_queued_urb,&n;&t; * since (for queued URBs) the pqh is pointed to the next&n;&t; * QH in the queue, not the next endpoint&squot;s QH.&n;&t; */
id|spin_lock_irqsave
c_func
(paren
op_amp
...
l_int|0
suffix:semicolon
multiline_comment|/*&n;&t;&t; * Fixup the previous QH&squot;s queue to link to the new head&n;&t;&t; * of this queue.&n;&t;&t; */
id|pqh
op_assign
id|list_entry
c_func
(paren
id|urbp-&gt;qh-&gt;list.prev
comma
r_struct
id|uhci_qh
comma
...
multiline_comment|/*&n; * If control was short, then end status packet wasn&squot;t sent, so this&n; * reorganize s so it&squot;s sent to finish the transfer.  The original QH is&n; * removed from the skel and discarded; all TDs except the last (status)&n; * are deleted; the last (status) TD is put on a new QH which is reinserted&n; * into the skel.  Since the last TD and urb_priv are reused, the TD-&gt;link&n; * and urb_priv maintain any queued QHs.&n; */
...
(brace
multiline_comment|/* Interrupt-IN can&squot;t be more than 1 packet */
r_if
c_cond
(paren
id|usb_pipein
...
id|urb-&gt;hcpriv
suffix:semicolon
multiline_comment|/* If this is an interrupt URB that is being killed in urb-&gt;complete, */
multiline_comment|/* then just set its status and return */
r_if
c_cond
(paren
op_logical_neg
...
multiline_comment|/* Note that this doesn&squot;t do what usb_hcd_giveback_urb() */
multiline_comment|/* normally does, so that doesn&squot;t ever get done. */
r_if
c_cond
(paren
id|urb-&gt;status
...
id|KERN_ERR
id|__FILE__
l_string|&quot;: could not resubmit interrupt URB : %d&bslash;n&quot;
comma
id|ret
)paren
suffix:semicolon
...
multiline_comment|/*&n;&t; * 8 Interrupt queues; link int2 to int1, int4 to int2, etc&n;&t; * then link int1 to control and control to bulk&n;&t; */
id|uhci-&gt;skel_int128_qh-&gt;link
op_assign
id|cpu_to_le32
c_func
(paren
id|uhci-&gt;skel_int64_qh-&gt;dma_handle
)paren
op_or
id|UHCI_PTR_QH
suffix:semicolon
...
c_func
(paren
id|uhci-&gt;skel_int32_qh-&gt;dma_handle
)paren
op_or
id|UHCI_PTR_QH
suffix:semicolon
...
c_func
(paren
id|uhci-&gt;skel_int16_qh-&gt;dma_handle
)paren
op_or
id|UHCI_PTR_QH
suffix:semicolon
...
c_func
(paren
id|uhci-&gt;skel_int8_qh-&gt;dma_handle
)paren
op_or
id|UHCI_PTR_QH
suffix:semicolon
...
c_func
(paren
id|uhci-&gt;skel_int4_qh-&gt;dma_handle
)paren
op_or
id|UHCI_PTR_QH
suffix:semicolon
...
c_func
(paren
id|uhci-&gt;skel_int2_qh-&gt;dma_handle
)paren
op_or
id|UHCI_PTR_QH
suffix:semicolon
