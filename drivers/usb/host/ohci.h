...
multiline_comment|/* PSW is only for ISO.  Only 1 PSW entry is used, but on&n;&t; * big-endian PPC hardware that&squot;s the second entry.&n;&t; */
DECL|macro|MAXPSW
mdefine_line|#define MAXPSW&t;2
DECL|member|hwPSW
id|__hc16
id|hwPSW
(braket
...
multiline_comment|/* &n;&t; * OHCI defines u16 frame_no, followed by u16 zero pad.&n;&t; * Since some processors can&squot;t do 16 bit bus accesses,&n;&t; * portable access must be a 32 bits wide.&n;&t; */
...
multiline_comment|/*-------------------------------------------------------------------------*/
multiline_comment|/* HCCA frame number is 16 bits, but is accessed as 32 bits since not all&n; * hardware handles 16 bit reads.  That creates a different confusion on&n; * some big-endian SOC implementations.  Same thing happens with PSW access.&n; */
macro_line|#ifdef CONFIG_STB03xxx
DECL|macro|OHCI_BE_FRAME_NO_SHIFT
mdefine_line|#define OHCI_BE_FRAME_NO_SHIFT&t;16
macro_line|#else
DECL|macro|OHCI_BE_FRAME_NO_SHIFT
mdefine_line|#define OHCI_BE_FRAME_NO_SHIFT&t;0
macro_line|#endif
DECL|function|ohci_frame_no
r_static
r_inline
id|u16
id|ohci_frame_no
