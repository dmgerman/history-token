...
mdefine_line|#define DRIVER_VERSION &quot;v5.3&quot;
...
(brace
multiline_comment|/* ISOC, BULK, INTR data buffer starts at td 0 &n;&t;&t; * CTRL setup starts at td 0 */
id|td
op_assign
id|urb_priv-&gt;td
(braket
...
suffix:semicolon
multiline_comment|/* unmap CTRL URB setup */
r_if
c_cond
(paren
id|usb_pipecontrol
...
id|PIPE_ISOCHRONOUS
suffix:colon
singleline_comment|// for (urbt = urb-&gt;next; urbt &amp;&amp; (urbt != urb); urbt = urbt-&gt;next);
r_if
c_cond
(paren
id|urbt
...
l_int|1
suffix:semicolon
multiline_comment|/* If the transfer size is multiple of the pipe mtu,&n;&t;&t;&t; * we may need an extra TD to create a empty frame&n;&t;&t;&t; * Jean II */
r_if
c_cond
(paren
(paren
...
id|entry
suffix:semicolon
singleline_comment|// FIXME:  usb-uhci uses relative timeouts (like this),
singleline_comment|// while uhci uses absolute ones (probably better).
singleline_comment|// Pick one solution and change the affected drivers.
id|urb-&gt;timeout
op_add_assign
id|jiffies
suffix:semicolon
...
suffix:semicolon
multiline_comment|/* drive timeouts by SF (messy, but works) */
id|writel
(paren
id|OHCI_INTR_SF
comma
...
suffix:semicolon
multiline_comment|/* ED might have been unlinked through another path */
r_while
c_loop
(paren
op_star
...
op_increment
suffix:semicolon
multiline_comment|/* If the transfer size is multiple of the pipe mtu,&n;&t;&t;&t; * we may need an extra TD to create a empty frame&n;&t;&t;&t; * Note : another way to check this condition is&n;&t;&t;&t; * to test if(urb_priv-&gt;length &gt; cnt) - Jean II */
r_if
c_cond
(paren
(paren
...
multiline_comment|/*-------------------------------------------------------------------------*/
multiline_comment|/* called only from interrupt handler */
DECL|function|check_timeouts
r_static
r_void
id|check_timeouts
(paren
...
suffix:semicolon
singleline_comment|// outside the interrupt handler (in a timer...)
singleline_comment|// this reference would race interrupts
id|sohci_unlink_urb
(paren
id|urb
)paren
...
suffix:semicolon
singleline_comment|// FIXME:  enable SF as needed in a timer;
singleline_comment|// don&squot;t make lots of 1ms interrupts
singleline_comment|// On unloaded USB, think 4k ~= 4-5msec
r_if
c_cond
(paren
op_logical_neg
...
multiline_comment|/*-------------------------------------------------------------------------*/
multiline_comment|/*&n; * Host bus independent add one OHCI host controller.&n; */
r_int
DECL|function|hc_add_ohci
id|hc_add_ohci
c_func
(paren
r_struct
...
multiline_comment|/*&n; * Host bus independent remove one OHCI host controller.&n; */
DECL|function|hc_remove_ohci
r_void
id|hc_remove_ohci
c_func
(paren
...
suffix:semicolon
DECL|variable|dl_reverse_done_list
id|EXPORT_SYMBOL
c_func
(paren
id|dl_reverse_done_list
