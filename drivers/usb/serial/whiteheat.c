...
suffix:semicolon
multiline_comment|/*&n;&t; * We ignore the return code. In the case where rmmod/insmod is&n;&t; * performed with a WhiteHEAT connected, the above times out&n;&t; * because the endpoint is already prepped, meaning the below succeeds&n;&t; * regardless. All other cases the above succeeds.&n;&t; */
id|pipe
op_assign
id|usb_sndbulkpipe
(paren
...
suffix:semicolon
multiline_comment|/*&n;&t; * Return that we&squot;ve claimed the interface. A failure here may be&n;&t; * due to interception by the command_callback routine or other&n;&t; * causes that don&squot;t mean that the firmware isn&squot;t running. This may&n;&t; * change in the future. Probably should actually.&n;&t; */
r_return
l_int|0
suffix:semicolon
)brace
