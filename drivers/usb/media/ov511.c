...
mdefine_line|#define MAX_FRAME_SIZE(w, h) ((w) * (h) * 3 / 2)
...
l_int|22
comma
l_string|&quot;Lifeview USB Life TV (PAL D/K+B/G)&quot;
)brace
comma
(brace
l_int|36
...
l_int|38
comma
l_string|&quot;Lifeview USB Life TV (PAL)&quot;
)brace
comma
(brace
l_int|41
...
l_int|70
comma
l_string|&quot;Lifeview USB Life TV (PAL/SECAM)&quot;
)brace
comma
(brace
l_int|100
...
multiline_comment|/**********************************************************************&n; *&n; * Low-level I2C I/O functions&n; *&n; **********************************************************************/
...
c_func
(paren
l_string|&quot;Couldn&squot;t restore primary I2C slave&quot;
)paren
suffix:semicolon
id|up
c_func
...
id|out
suffix:semicolon
singleline_comment|// FIXME: Is this actually necessary?
id|rc
op_assign
id|ov51x_reset
c_func
...
multiline_comment|/* Note: Unlike the OV511/OV511+, the size argument does NOT include the&n; * optional packet number byte. The actual size *is* stored in ov-&gt;packet_size,&n; * though. */
r_static
r_int
DECL|function|ov518_set_packet_size
...
(brace
multiline_comment|/* Program ISO FIFO size reg (packet number isn&squot;t included) */
id|ov518_reg_w32
c_func
(paren
id|ov
...
multiline_comment|/* Matches the sensor&squot;s internal frame rate to the lighting frequency.&n; * Valid frequencies are:&n; *&t;50 - 50Hz, for European and Asian lighting&n; *&t;60 - 60Hz, for American lighting&n; *&n; * Tested with: OV7610, OV7620, OV76BE, OV6620&n; * Unsupported: KS0127, KS0127B, SAA7111A&n; * Returns: 0 for success&n; */
r_static
r_int
...
multiline_comment|/* Modifies the sensor&squot;s exposure algorithm to allow proper exposure of objects&n; * that are illuminated from behind.&n; *&n; * Tested with: OV6620, OV7620&n; * Unsupported: OV7610, OV76BE, KS0127, KS0127B, SAA7111A&n; * Returns: 0 for success&n; */
r_static
r_int
...
c_func
(paren
l_string|&quot;Sensor not supported for set_mirror&quot;
)paren
suffix:semicolon
r_return
op_minus
...
(brace
multiline_comment|/* Note: OV518(+) does downsample on its own) */
r_if
c_cond
(paren
(paren
...
(brace
multiline_comment|/* Set the OV511 up as 320x480, but keep the&n;&t;&t;&t; * V4L resolution as 640x480 */
id|width
op_assign
l_int|320
suffix:semicolon
...
c_func
(paren
l_string|&quot;SAA7111A only allows 320x240 or 640x480&quot;
)paren
suffix:semicolon
r_return
op_minus
...
multiline_comment|/* Sets up the OV518/OV518+ with the given image parameters&n; *&n; * OV518 needs a completely different approach, until we can figure out what&n; * the individual registers do. Also, only 15 FPS is supported now.&n; *&n; * Do not put any sensor-specific code in here (including I2C I/O functions)&n; */
r_static
r_int
...
c_func
(paren
l_string|&quot;Invalid width/height combination (%d, %d)&quot;
comma
id|width
comma
id|height
...
suffix:semicolon
multiline_comment|/* Clock divider; lower==faster */
id|reg_w
c_func
(paren
id|ov
...
multiline_comment|/* This sets the default image parameters. This is useful for apps that use&n; * read() and do not set these.&n; */
r_static
r_int
...
suffix:semicolon
multiline_comment|/* Initialize to max width/height, YUV420 or RGB24 (if supported) */
r_if
c_cond
(paren
id|mode_init_regs
...
r_static
r_void
DECL|function|ov51x_postprocess_grey
id|ov51x_postprocess_grey
c_func
(paren
r_struct
id|usb_ov511
...
multiline_comment|/* Process raw YUV420 data into standard YUV420P */
r_static
r_void
DECL|function|ov51x_postprocess_yuv420
id|ov51x_postprocess_yuv420
c_func
(paren
r_struct
id|usb_ov511
...
id|ts
suffix:semicolon
multiline_comment|/* Don&squot;t copy the packet number byte */
r_if
c_cond
(paren
id|ov-&gt;packet_numbering
...
c_func
(paren
l_string|&quot;ph: %2x %2x %2x %2x %2x %2x %2x %2x&quot;
comma
id|in
(braket
l_int|0
...
multiline_comment|/* This initializes the OV7610, OV7620, or OV76BE sensor. The OV76BE uses&n; * the same register settings as the OV7610, since they are very similar.&n; */
r_static
r_int
...
comma
multiline_comment|/* Color Procesing Parameter */
(brace
id|OV511_I2C_BUS
comma
l_int|0x34
...
macro_line|#endif
multiline_comment|/* 640x480 not supported with PAL */
r_if
c_cond
(paren
id|ov-&gt;pal
...
(brace
multiline_comment|/* For 518 and 518+ */
r_static
r_struct
id|ov511_regvals
id|aRegvalsInit518
...
suffix:semicolon
multiline_comment|/* Give it the default description */
id|ov-&gt;desc
op_assign
id|symbolic
c_func
...
comma
op_amp
id|class_device_attr_custom_id
)paren
suffix:semicolon
id|video_device_create_file
c_func
...
comma
op_amp
id|class_device_attr_brightness
)paren
suffix:semicolon
id|video_device_create_file
c_func
...
comma
op_amp
id|class_device_attr_saturation
)paren
suffix:semicolon
id|video_device_create_file
c_func
...
comma
op_amp
id|class_device_attr_exposure
)paren
suffix:semicolon
)brace
multiline_comment|/****************************************************************************&n; *  USB routines&n; ***************************************************************************/
r_static
r_int
