...
suffix:semicolon
multiline_comment|/* This must be used if and only if the sensor doesn&squot;t implement the standard&n;   I2C protocol, like the TASC sensors. There a number of good reasons why you&n;   must use the single-byte versions of this function: do not abuse. It writes&n;   n bytes, from data0 to datan, (registers 0x09 - 0x09+n of SN9C10X chip) */
r_extern
r_int
...
suffix:semicolon
multiline_comment|/* NOTE: there are no debugging functions here. To uniform the output you must&n;   use the dev_info()/dev_warn()/dev_err() macros defined in device.h, already&n;   included here, the argument being the struct device &squot;dev&squot; of the sensor&n;   structure. Do NOT use these macros before the sensor is attached or the&n;   kernel will crash! However you should not need to notify the user about&n;   common errors or other messages, since this is done by the master module. */
multiline_comment|/*****************************************************************************/
...
multiline_comment|/* These sensor capabilities must be provided if the SN9C10X controller&n;&t;   needs to communicate through the sensor serial interface by using&n;&t;   at least one of the i2c functions available */
...
id|interface
suffix:semicolon
multiline_comment|/* These identifiers must be provided if the image sensor implements&n;&t;   the standard I2C protocol. TASC sensors don&squot;t, although they have a&n;&t;   serial interface: so this is a case where the &quot;raw&quot; I2C version&n;&t;   could be helpful. */
...
multiline_comment|/* NOTE: Where not noted,most of the functions below are not mandatory.&n;&t;         Set to null if you do not implement them. If implemented,&n;&t;         they must return 0 on success, the proper error otherwise. */
DECL|member|init
r_int
(paren
op_star
...
suffix:semicolon
multiline_comment|/* Optional list of default controls, defined as indicated in the &n;&t;   V4L2 API. Menu type controls are not handled by this interface. */
...
id|dev
suffix:semicolon
multiline_comment|/* This is the argument for dev_err(), dev_info() and dev_warn(). It&n;&t;   is used for debugging purposes. You must not access the struct&n;&t;   before the sensor is attached. */
DECL|member|usbdev
r_const
r_struct
id|usb_device
...
id|usbdev
suffix:semicolon
multiline_comment|/* Points to the usb_device struct after the sensor is attached.&n;&t;   Do not touch unless you know what you are doing. */
multiline_comment|/* Do NOT write to the data below, it&squot;s READ ONLY. It is used by the&n;&t;   core module to store successfully updated values of the above&n;&t;   settings, for rollbacks..etc..in case of errors during atomic I/O */
DECL|member|_qctrl
r_struct
id|v4l2_queryctrl
id|_qctrl
