...
suffix:semicolon
multiline_comment|/* We disable the UDC -- and its 48 MHz clock -- whenever it&squot;s not&n; * in active use.  &n; */
DECL|function|pullup
r_static
r_int
id|pullup
c_func
...
multiline_comment|/* VBUS reporting logically comes from a transceiver */
DECL|function|pxa2xx_udc_vbus_session
r_static
r_int
id|pxa2xx_udc_vbus_session
c_func
(paren
r_struct
id|usb_gadget
...
multiline_comment|/* drivers may have software control over D+ pullup */
DECL|function|pxa2xx_udc_pullup
r_static
r_int
id|pxa2xx_udc_pullup
c_func
(paren
r_struct
id|usb_gadget
...
suffix:semicolon
multiline_comment|/* not all boards support pullup control */
r_if
c_cond
(paren
op_logical_neg
id|udc-&gt;mach-&gt;udc_command
)paren
r_return
op_minus
id|EOPNOTSUPP
...
id|pxa2xx_udc_pullup
comma
singleline_comment|// .vbus_draw ... boards may consume current from VBUS, up to
singleline_comment|// 100-500mA based on config.  the 500uA suspend ceiling means
singleline_comment|// that exclusively vbus-powered PXA designs violate USB specs.
)brace
suffix:semicolon
multiline_comment|/*-------------------------------------------------------------------------*/
...
id|UICR0_IM0
suffix:semicolon
multiline_comment|/* if hardware supports it, pullup D+ and wait for reset */
id|let_usb_appear
c_func
(paren
)paren
...
multiline_comment|/* Lubbock has separate connect and disconnect irqs.  More typical designs&n; * use one GPIO as the VBUS IRQ, and another to control the D+ pullup.&n; */
r_static
id|irqreturn_t
DECL|function|lubbock_vbus_irq
id|lubbock_vbus_irq
c_func
(paren
r_int
...
id|DBG_VERBOSE
comma
l_string|&quot;USB reset end&bslash;n&quot;
)paren
suffix:semicolon
id|dev-&gt;gadget.speed
op_assign
...
multiline_comment|/* with U-Boot (but not BLOB), hex is off by default */
id|HEX_DISPLAY
c_func
(paren
id|dev-&gt;stats.irqs
...
multiline_comment|/* USB suspend (controlled by the host) and system suspend (controlled&n; * by the PXA) don&squot;t necessarily work well together.  If USB is active,&n; * the 48 MHz clock is required; so the system can&squot;t enter 33 MHz idle&n; * mode, or any deeper PM saving state.&n; *&n; * For now, we punt and forcibly disconnect from the USB host when PXA&n; * enters any suspend state.  While we&squot;re disconnected, we always disable&n; * the 48MHz USB clock ... allowing PXA sleep and/or 33 MHz idle states. &n; * Boards without software pullup control shouldn&squot;t use those states.&n; * VBUS IRQs should probably be ignored so that the PXA device just acts&n; * &quot;dead&quot; to USB hosts until system resume.&n; */
DECL|function|pxa2xx_udc_suspend
r_static
r_int
id|pxa2xx_udc_suspend
c_func
(paren
r_struct
id|device
...
c_func
(paren
l_string|&quot;USB host won&squot;t detect disconnect!&bslash;n&quot;
)paren
suffix:semicolon
id|pullup
c_func
...
DECL|function|pxa2xx_udc_resume
r_static
r_int
id|pxa2xx_udc_resume
c_func
(paren
r_struct
id|device
...
macro_line|#else
DECL|macro|pxa2xx_udc_suspend
mdefine_line|#define&t;pxa2xx_udc_suspend&t;NULL
DECL|macro|pxa2xx_udc_resume
mdefine_line|#define&t;pxa2xx_udc_resume&t;NULL
macro_line|#endif
multiline_comment|/*-------------------------------------------------------------------------*/
