...
DECL|macro|VERBOSE
macro_line|#undef&t;VERBOSE&t;&t;/* extra debug messages (success too) */
...
suffix:semicolon
multiline_comment|/* use_dma -- general goodness, fewer interrupts, less cpu load (vs PIO)&n; * use_dma_chaining -- dma descriptor queueing gives even more irq reduction&n; *&n; * The net2280 DMA engines are not tightly integrated with their FIFOs;&n; * not all cases are (yet) handled well in this driver or the silicon.&n; * Some gadget drivers work better with the dma support here than others.&n; * These two parameters let you use PIO or more aggressive DMA.&n; */
...
l_int|1
suffix:semicolon
DECL|variable|use_dma_chaining
r_static
r_int
id|use_dma_chaining
op_assign
...
id|ep-&gt;dev
comma
l_string|&quot;%s, no dma for maxpacket %d&bslash;n&quot;
comma
id|ep-&gt;ep.name
comma
id|ep-&gt;ep.maxpacket
...
(brace
multiline_comment|/* erratum 0105 workaround prevents hs NYET */
r_if
c_cond
(paren
id|dev-&gt;chiprev
...
multiline_comment|/* last fifo entry is &quot;short&quot; unless we wrote a full packet.&n;&t; * also explicitly validate last word in (periodic) transfers&n;&t; * when maxpacket is not a multiple of 4 bytes.&n;&t; */
r_if
c_cond
(paren
id|count
...
multiline_comment|/* work around erratum 0106: PCI and USB race over the OUT fifo.&n; * caller guarantees chiprev 0100, out endpoint is NAKing, and&n; * there&squot;s no real data in the fifo.&n; *&n; * NOTE:  also used in cases where that erratum doesn&squot;t apply:&n; * where the host wrote &quot;too much&quot; data to us.&n; */
...
suffix:semicolon
multiline_comment|/* handled that data already? */
r_if
c_cond
(paren
id|count
...
l_int|1
suffix:semicolon
multiline_comment|/* NAK_OUT_PACKETS will be set, so flushing is safe;&n;&t;&t;&t; * the next read will start with the next packet&n;&t;&t;&t; */
)brace
multiline_comment|/* else it&squot;s a ZLP, no worries */
id|count
op_assign
id|tmp
suffix:semicolon
...
suffix:semicolon
multiline_comment|/* LE conversion is implicit here: */
r_do
(brace
op_star
id|buf
...
multiline_comment|/* on this path we &quot;know&quot; there&squot;s no dma active (yet) */
id|WARN_ON
(paren
id|readl
(paren
...
r_break
suffix:semicolon
multiline_comment|/* SHORT_PACKET_TRANSFERRED_INTERRUPT handles &quot;usb-short&quot;&n;&t;&t; * cases where DMA must be aborted; this code handles&n;&t;&t; * all non-abort DMA completions.&n;&t;&t; */
r_if
c_cond
(paren
id|unlikely
...
r_break
suffix:semicolon
multiline_comment|/* single transfer mode */
id|dma_done
(paren
id|ep
comma
...
suffix:semicolon
multiline_comment|/* AVOID TROUBLE HERE by not issuing short reads from&n;&t;&t;&t; * your gadget driver.  That helps avoids errata 0121,&n;&t;&t;&t; * 0122, and 0124; not all cases trigger the warning.&n;&t;&t;&t; */
r_if
c_cond
(paren
(paren
...
id|ep-&gt;dev
comma
l_string|&quot;%s lost packet sync!&bslash;n&quot;
comma
id|ep-&gt;ep.name
)paren
suffix:semicolon
...
(brace
multiline_comment|/* fifo gets flushed later */
id|ep-&gt;out_overflow
op_assign
l_int|1
suffix:semicolon
...
multiline_comment|/* the 2280 will be processing the queue unless queue hiccups after&n;&t; * the previous transfer:&n;&t; *  IN:   wanted automagic zlp, head doesn&squot;t (or vice versa)&n;&t; *        DMA_FIFO_VALIDATE doesn&squot;t init from dma descriptors.&n;&t; *  OUT:  was &quot;usb-short&quot;, we must restart.&n;&t; */
r_if
c_cond
(paren
id|ep-&gt;is_in
...
r_else
(brace
multiline_comment|/* force a hiccup */
id|prev-&gt;td-&gt;dmacount
op_or_assign
id|dma_done_ie
suffix:semicolon
...
(brace
multiline_comment|/* FIXME work around errata 0121, 0122, 0124 */
id|writel
(paren
(paren
l_int|1
...
id|ep-&gt;stopped
suffix:semicolon
multiline_comment|/* quiesce dma while we patch the queue */
id|dmactl
op_assign
l_int|0
suffix:semicolon
...
suffix:semicolon
multiline_comment|/* WARNING erratum 0127 may kick in ... */
id|stop_dma
(paren
id|ep-&gt;dma
)paren
...
multiline_comment|/* make sure it&squot;s still queued on this endpoint */
id|list_for_each_entry
(paren
id|req
comma
...
(brace
singleline_comment|// NOTE: misreports single-transfer mode
id|req-&gt;td-&gt;dmacount
op_assign
l_int|0
suffix:semicolon
...
DECL|function|net2280_set_selfpowered
r_static
r_int
id|net2280_set_selfpowered
(paren
r_struct
id|usb_gadget
op_star
...
l_string|&quot;%s version &quot;
id|DRIVER_VERSION
l_string|&quot;, chiprev %04x, dma %s&bslash;n&bslash;n&quot;
...
id|size
comma
l_string|&quot;&bslash;n%s&bslash;tcfg %05x rsp (%02x) %s%s%s%s%s%s%s%s&quot;
...
id|size
comma
l_string|&quot;&bslash;n%s (ep%d%s-%s) max %04x %s fifo %d&bslash;n&quot;
comma
id|ep-&gt;ep.name
comma
id|t
...
(brace
multiline_comment|/* any preceding dma transfers must finish.&n;&t;&t;&t;&t; * dma handles (M &gt;= N), may empty the queue&n;&t;&t;&t;&t; */
id|scan_dma_completions
(paren
id|ep
)paren
...
suffix:semicolon
multiline_comment|/* here either (M &lt; N), a &quot;real&quot; short rx;&n;&t;&t;&t;&t; * or (M == N) and the queue didn&squot;t empty&n;&t;&t;&t;&t; */
r_if
c_cond
(paren
id|likely
...
multiline_comment|/* (re)start dma if needed, stop NAKing */
id|ep-&gt;stopped
op_assign
id|stopped
suffix:semicolon
...
suffix:semicolon
multiline_comment|/* VBUS disconnect is indicated by VBUS_PIN and VBUS_INTERRUPT set.&n;&t; * Root Port Reset is indicated by ROOT_PORT_RESET_INTERRRUPT set and&n;&t; * both HIGH_SPEED and FULL_SPEED clear (as ROOT_PORT_RESET_INTERRUPT &n;&t; * only indicates a change in the reset state).&n;&t; */
r_if
c_cond
(paren
id|stat
...
suffix:semicolon
multiline_comment|/* chaining should stop on abort, short OUT from fifo,&n;&t;&t; * or (stat0 codepath) short OUT transfer.&n;&t;&t; */
r_if
c_cond
(paren
op_logical_neg
...
id|dmacount
suffix:semicolon
multiline_comment|/* the descriptor at the head of the chain&n;&t;&t;&t;&t; * may still have VALID_BIT clear; that&squot;s&n;&t;&t;&t;&t; * used to trigger changing DMA_FIFO_VALIDATE&n;&t;&t;&t;&t; * (affects automagic zlp writes).&n;&t;&t;&t;&t; */
id|req
op_assign
id|list_entry
(paren
...
singleline_comment|// 256 write retries may not be enough...
singleline_comment|// | (1 &lt;&lt; PCI_RETRY_ABORT_ENABLE)
op_or
(paren
l_int|1
op_lshift
