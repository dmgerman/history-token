...
multiline_comment|/**&n; * usb_get_device_descriptor - (re)reads the device descriptor&n; * @dev: the device whose device descriptor is being updated&n; * @size: how much of the descriptor to read&n; * Context: !in_interrupt ()&n; *&n; * Updates the copy of the device descriptor stored in the device structure,&n; * which dedicates space for this purpose.  Note that several fields are&n; * converted to the host CPU&squot;s byte order:  the USB version (bcdUSB), and&n; * vendors product and version fields (idVendor, idProduct, and bcdDevice).&n; * That lets device drivers compare against non-byteswapped constants.&n; *&n; * Not exported, only for use by the core.  If drivers really want to read&n; * the device descriptor directly, they can call usb_get_descriptor() with&n; * type = USB_DT_DEVICE and index = 0.&n; *&n; * This call is synchronous, and may not be used in an interrupt context.&n; *&n; * Returns the number of bytes received on success, or else the status code&n; * returned by the underlying usb_control_msg() call.&n; */
