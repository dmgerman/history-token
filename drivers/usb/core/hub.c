...
DECL|function|hub_remove_children_work
r_static
r_void
id|hub_remove_children_work
c_func
(paren
r_void
op_star
...
multiline_comment|/* If there are any children then this is unbind only, not a&n;&t; * physical disconnection.  The active ports must be disabled&n;&t; * and later on we must call usb_disconnect().  We can&squot;t call&n;&t; * it here because we already own the usb bus writelock.&n;&t; */
id|n
op_assign
l_int|0
suffix:semicolon
...
r_else
(brace
multiline_comment|/* Reuse hub-&gt;leds to disconnect the children */
id|INIT_WORK
c_func
(paren
op_amp
