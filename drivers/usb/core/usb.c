...
suffix:semicolon
DECL|function|usb_register_minors
r_static
r_int
id|usb_register_minors
(paren
r_struct
id|usb_driver
op_star
...
c_func
(paren
l_string|&quot;registering %d minors, starting at %d&quot;
comma
id|num_minors
comma
id|start_minor
...
c_func
(paren
l_string|&quot;minor %d is already in use, error registering %s driver&quot;
comma
id|i
comma
id|driver-&gt;name
...
DECL|function|usb_deregister_minors
r_static
r_void
id|usb_deregister_minors
(paren
r_struct
id|usb_driver
op_star
...
id|dbg
(paren
l_string|&quot;%s is removing %d minors starting at %d&quot;
comma
id|driver-&gt;name
comma
id|num_minors
...
multiline_comment|/**&n; * usb_register_dev - register a USB device, and ask for a minor number&n; * @new_driver: USB operations for the driver&n; * @num_minors: number of minor numbers requested for this device&n; * @start_minor: place to put the new starting minor number&n; *&n; * Used to ask the USB core for a new minor number for a device that has&n; * just showed up.  This is used to dynamically allocate minor numbers&n; * from the pool of USB reserved minor numbers.&n; *&n; * This should be called by all drivers that use the USB major number.&n; * This only returns a good value of CONFIG_USB_DYNAMIC_MINORS is&n; * selected by the user.&n; *&n; * usb_deregister_dev() should be called when the driver is done with&n; * the minor numbers given out by this function.&n; *&n; * Returns -ENODEV if CONFIG_USB_DYNAMIC_MINORS is not enabled in this&n; * kernel, -EINVAL if something bad happens with trying to register a&n; * device, and 0 on success, alone with a value that the driver should&n; * use in start_minor.&n; */
...
id|dbg
(paren
l_string|&quot;%s is asking for %d minors&quot;
comma
id|new_driver-&gt;name
comma
id|num_minors
...
(brace
multiline_comment|/* someone snuck in here, so let&squot;s start looking all over again */
id|spin_lock
(paren
op_amp
id|minor_lock
...
multiline_comment|/**&n; * usb_deregister_dev - deregister a USB device&squot;s dynamic minor.&n; * @driver: USB operations for the driver&n; * @num_minors: number of minor numbers to put back.&n; * @start_minor: the starting minor number&n; *&n; * Used in conjunction with usb_register_dev().  This function is called&n; * when the USB driver is finished with the minor numbers gotten from a&n; * call to usb_register_dev() (usually when the device is disconnected&n; * from the system.)&n; * &n; * This should be called by all drivers that use the USB major number.&n; */
...
macro_line|#endif&t;/* CONFIG_USB_DYNAMIC_MINORS */
