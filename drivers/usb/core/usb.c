...
multiline_comment|/**&n; *&t;usb_unbind_driver - disconnects a driver from a device (usbcore-internal)&n; *&t;@device: usb device to be disconnected&n; *&t;@intf: interface of the device to be disconnected&n; *&t;Context: BKL held&n; *&n; *&t;Handles module usage count correctly&n; */
DECL|function|usb_unbind_driver
r_void
...
r_return
suffix:semicolon
multiline_comment|/* as soon as we increase the module use count we drop the BKL&n;&t;   before that we must not sleep */
r_if
c_cond
(paren
id|driver-&gt;owner
...
suffix:semicolon
multiline_comment|/* if we sleep here on an umanaged driver&n;&t;&t;&t;&t;&t;   the holder of the lock guards against&n;&t;&t;&t;&t;&t;   module unload */
id|driver
...
multiline_comment|/**&n; *&t;usb_bind_driver - connect a driver to a device&squot;s interface (usbcore-internal)&n; *&t;@driver: device driver to be bound to interface&n; *&t;@interface: interface that the driver will be using&n; *&t;Context: BKL held&n; *&n; *&t;Does a safe binding of a driver to one of a device&squot;s interfaces.&n; *&t;Returns the driver&squot;s data for the binding, or null indicating&n; *&t;that the driver did not bind to this interface.&n; *&n; *&t;This differs from usb_driver_claim_interface(), which is called from&n; *&t;drivers and neither calls the driver&squot;s probe() entry nor does any&n; *&t;locking to guard against removing driver modules.&n; */
r_void
op_star
DECL|function|usb_bind_driver
id|usb_bind_driver
(paren
r_struct
id|usb_driver
...
l_int|NULL
suffix:semicolon
multiline_comment|/* this horse is dead - don&squot;t ride*/
id|unlock_kernel
c_func
(paren
)paren
...
singleline_comment|// START TEMPORARY
singleline_comment|// driver-&gt;probe() hasn&squot;t yet changed to take interface not dev+ifnum,
singleline_comment|// so we still need ifnum here.
r_for
c_loop
(paren
id|ifnum
...
suffix:semicolon
singleline_comment|// END TEMPORARY
id|id
op_assign
id|driver-&gt;id_table
suffix:semicolon
...
suffix:semicolon
multiline_comment|/* FIXME It&squot;s just luck that for some devices with drivers that set&n;&t; * configuration in probe(), the interface numbers still make sense.&n;&t; * That&squot;s one of several unsafe assumptions involved in configuring&n;&t; * devices, and in binding drivers to their interfaces.&n;&t; */
r_for
c_loop
(paren
id|ifnum
...
id|prod_str
suffix:semicolon
multiline_comment|/* set default; keep it if there are no strings, or kmalloc fails */
id|sprintf
(paren
id|dev-&gt;dev.name
comma
