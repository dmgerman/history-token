...
multiline_comment|/**&n; * usb_epnum_to_ep_desc - get the endpoint object with a given endpoint number&n; * @dev: the device whose current configuration+altsettings is considered&n; * @epnum: the desired endpoint, masked with USB_DIR_IN as appropriate.&n; *&n; * This walks the device descriptor for the currently active configuration,&n; * and returns a pointer to the endpoint with that particular endpoint&n; * number, or null.&n; *&n; * Note that interface descriptors are not required to list endpoint&n; * numbers in any standardized order, so that it would be wrong to&n; * assume that ep2in precedes either ep5in, ep2out, or even ep1out.&n; * This routine helps device drivers avoid such mistakes.&n; */
r_struct
...
id|alt
suffix:semicolon
multiline_comment|/* only endpoints in current altseting are active */
id|intf
op_assign
id|dev-&gt;actconfig-&gt;interface
(braket
...
DECL|function|usb_device_suspend
r_static
r_int
id|usb_device_suspend
c_func
(paren
r_struct
id|device
...
DECL|function|usb_device_resume
r_static
r_int
id|usb_device_resume
c_func
(paren
r_struct
id|device
