...
DECL|macro|VERSION
mdefine_line|#define VERSION &quot;1.2b2&quot;
DECL|macro|DEBUG
...
id|critical_state
suffix:semicolon
DECL|variable|rackmac
r_static
r_int
id|rackmac
suffix:semicolon
DECL|variable|dimm_output_clamp
r_static
id|s32
id|dimm_output_clamp
suffix:semicolon
r_static
id|DECLARE_MUTEX
c_func
...
suffix:semicolon
multiline_comment|/*&n; * We have 3 types of CPU PID control. One is &quot;split&quot; old style control&n; * for intake &amp; exhaust fans, the other is &quot;combined&quot; control for both&n; * CPUs that also deals with the pumps when present. To be &quot;compatible&quot;&n; * with OS X at this point, we only use &quot;COMBINED&quot; on the machines that&n; * are identified as having the pumps (though that identification is at&n; * least dodgy). Ultimately, we could probably switch completely to this&n; * algorithm provided we hack it to deal with the UP case&n; */
...
DECL|macro|CPU_PID_TYPE_RACKMAC
mdefine_line|#define CPU_PID_TYPE_RACKMAC&t;2
...
id|loc
op_assign
l_string|&quot;BACKSIDE,SYS CTRLR FAN&quot;
comma
dot
id|type
op_assign
...
comma
multiline_comment|/* Xserve fans */
(braket
id|CPU_A1_FAN_RPM_INDEX
)braket
op_assign
(brace
dot
...
c_func
(paren
l_string|&quot;Error reading LM87, retrying...&bslash;n&quot;
)paren
suffix:semicolon
r_if
c_cond
...
(paren
id|KERN_ERR
l_string|&quot;therm_pm72: Error reading LM87 !&bslash;n&quot;
)paren
suffix:semicolon
r_return
op_minus
...
DECL|function|fetch_cpu_pumps_minmax
r_static
r_void
id|fetch_cpu_pumps_minmax
c_func
(paren
r_void
)paren
...
suffix:semicolon
multiline_comment|/* Try to fetch pumps min/max infos from eeprom */
id|memcpy
c_func
(paren
op_amp
...
comma
op_amp
id|state0-&gt;mpu.processor_part_num
comma
l_int|8
)paren
suffix:semicolon
...
multiline_comment|/* Double check the values, this _IS_ needed as the EEPROM on&n;&t; * some dual 2.5Ghz G5s seem, at least, to have both min &amp; max&n;&t; * same to the same value ... (grrrr)&n;&t; */
r_if
c_cond
(paren
id|pump_min
...
c_func
(paren
id|dimms_temperature
comma
id|dimms_state.last_temp
)paren
r_static
id|DEVICE_ATTR
c_func
...
id|state0-&gt;mpu.rminn_exhaust_fan
)paren
suffix:semicolon
id|state0-&gt;rpm
op_assign
...
suffix:semicolon
multiline_comment|/* Calculate intake fan */
id|intake
op_assign
(paren
id|state-&gt;rpm
...
DECL|function|do_monitor_cpu_rack
r_static
r_void
id|do_monitor_cpu_rack
c_func
(paren
r_struct
id|cpu_pid_state
...
suffix:semicolon
multiline_comment|/* Check clamp from dimms */
id|fan_min
op_assign
id|dimm_output_clamp
suffix:semicolon
...
c_func
(paren
l_string|&quot;** CPU %d RPM: %d overtemp: %d&bslash;n&quot;
comma
id|state-&gt;index
comma
(paren
...
id|sum
suffix:semicolon
multiline_comment|/* Check for clamp */
id|fan_min
op_assign
(paren
id|dimm_output_clamp
...
id|backside_params.G_d
op_assign
id|BACKSIDE_PID_RACK_G_d
suffix:semicolon
id|backside_params.input_target
op_assign
id|BACKSIDE_PID_RACK_INPUT_TARGET
suffix:semicolon
id|backside_params.output_min
op_assign
...
id|backside_params.interval
op_assign
id|BACKSIDE_PID_RACK_INTERVAL
suffix:semicolon
id|backside_params.G_p
op_assign
id|BACKSIDE_PID_RACK_G_p
suffix:semicolon
id|backside_params.G_r
op_assign
id|BACKSIDE_PID_G_r
...
multiline_comment|/*&n; * DIMMs temp control loop&n; */
DECL|function|do_monitor_dimms
r_static
r_void
id|do_monitor_dimms
c_func
...
id|state-&gt;ticks
op_assign
id|DIMM_PID_INTERVAL
suffix:semicolon
id|DBG
c_func
(paren
l_string|&quot;DIMM:&bslash;n&quot;
)paren
suffix:semicolon
id|DBG
c_func
(paren
l_string|&quot;  current value: %d&bslash;n&quot;
comma
id|state-&gt;output
)paren
suffix:semicolon
...
c_func
(paren
l_string|&quot;** DIMM clamp value: %d&bslash;n&quot;
comma
(paren
r_int
)paren
...
suffix:semicolon
multiline_comment|/* Backside PID is only every 5 seconds, force backside fan clamping now */
id|fan_min
op_assign
(paren
id|dimm_output_clamp
...
c_func
(paren
l_string|&quot; -&gt; applying clamp to backside fan now: %d  !&bslash;n&quot;
comma
id|fan_min
)paren
suffix:semicolon
...
multiline_comment|/*&n; * Initialize the state structure for the DIMM temp control loop&n; */
DECL|function|init_dimms_state
r_static
r_int
id|init_dimms_state
c_func
...
multiline_comment|/*&n; * Dispose of the state data for the drives control loop&n; */
DECL|function|dispose_dimms_state
r_static
r_void
id|dispose_dimms_state
c_func
(paren
r_struct
id|dimm_pid_state
...
suffix:semicolon
multiline_comment|/* First, we always calculate the new DIMMs state on an Xserve */
r_if
c_cond
(paren
id|rackmac
...
suffix:semicolon
multiline_comment|/* Then, the CPUs */
r_if
c_cond
(paren
id|cpu_pid_type
...
suffix:semicolon
singleline_comment|// better deal with UP
)brace
r_else
(brace
id|do_monitor_cpu_split
...
DECL|function|fan_check_loc_match
r_static
r_int
id|fan_check_loc_match
c_func
(paren
r_const
r_char
...
c_func
(paren
l_string|&quot;Driver for Apple&squot;s PowerMac G5 thermal control&quot;
)paren
suffix:semicolon
id|MODULE_LICENSE
c_func
