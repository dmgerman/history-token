...
id|flags
suffix:semicolon
multiline_comment|/* cuda_interrupt only takes a normal lock, we disable&n;     * interrupts here to avoid re-entering and thus deadlocking.&n;     * An option would be to disable only the IRQ source with&n;     * disable_irq(), would that work on m68k ? --BenH&n;     */
id|local_irq_save
c_func
(paren
id|flags
