...
multiline_comment|/**&n; *&t;device_unregister_wait - Unregister device and wait for it to be freed.&n; *&t;@dev: Device to unregister.&n; *&n; *&t;For the cases where the caller needs to wait for all references to&n; *&t;be dropped from the device before continuing (e.g. modules with&n; *&t;statically allocated devices), this function uses a completion struct&n; *&t;to wait, along with a matching complete() in device_release() above.&n; */
DECL|function|device_unregister_wait
r_void
...
suffix:semicolon
DECL|variable|device_unregister_wait
id|EXPORT_SYMBOL
c_func
(paren
id|device_unregister_wait
