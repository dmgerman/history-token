multiline_comment|/*&n; * linux/drivers/video/neofb.c -- NeoMagic Framebuffer Driver&n; *&n; * Copyright (c) 2001-2002  Denis Oliver Kropp &lt;dok@directfb.org&gt;&n; *&n; *&n; * Card specific code is based on XFree86&squot;s neomagic driver.&n; * Framebuffer framework code is based on code of cyber2000fb.&n; *&n; * This file is subject to the terms and conditions of the GNU General&n; * Public License.  See the file COPYING in the main directory of this&n; * archive for more details.&n; *&n; *&n; * 0.4.1&n; *  - Cosmetic changes (dok)&n; *&n; * 0.4&n; *  - Toshiba Libretto support, allow modes larger than LCD size if&n; *    LCD is disabled, keep BIOS settings if internal/external display&n; *    haven&squot;t been enabled explicitly&n; *                          (Thomas J. Moore &lt;dark@mama.indstate.edu&gt;)&n; *&n; * 0.3.3&n; *  - Porting over to new fbdev api. (jsimmons)&n; *  &n; * 0.3.2&n; *  - got rid of all floating point (dok) &n; *&n; * 0.3.1&n; *  - added module license (dok)&n; *&n; * 0.3&n; *  - hardware accelerated clear and move for 2200 and above (dok)&n; *  - maximum allowed dotclock is handled now (dok)&n; *&n; * 0.2.1&n; *  - correct panning after X usage (dok)&n; *  - added module and kernel parameters (dok)&n; *  - no stretching if external display is enabled (dok)&n; *&n; * 0.2&n; *  - initial version (dok)&n; *&n; *&n; * TODO&n; * - ioctl for internal/external switching&n; * - blanking&n; * - 32bit depth support, maybe impossible&n; * - disable pan-on-sync, need specs&n; *&n; * BUGS&n; * - white margin on bootup like with tdfxfb (colormap problem?)&n; *&n; */
...
mdefine_line|#define NEOFB_VERSION &quot;0.4.2&quot;
multiline_comment|/* --------------------------------------------------------------------- */
...
id|libretto
comma
l_string|&quot;Force Libretto 100/110 800x480 LCD.&quot;
)paren
suffix:semicolon
id|module_param
c_func
...
id|mode_option
comma
l_string|&quot;Preferred video mode (&squot;640x480-8@60&squot;, etc)&quot;
)paren
suffix:semicolon
macro_line|#endif
multiline_comment|/* --------------------------------------------------------------------- */
...
multiline_comment|/*&n; * VGA Palette management&n; */
...
multiline_comment|/* BOGUS  vga_wgfx(NULL, 0x09, 0x26); */
...
l_int|16
suffix:colon
multiline_comment|/* TrueColor, 64k */
id|info-&gt;fix.visual
op_assign
...
(brace
multiline_comment|/*&n;&t; *  Blank the screen if blank_mode != 0, else unblank.&n;&t; *  Return 0 if blanking succeeded, != 0 if un-/blanking failed due to&n;&t; *  e.g. a video mode which doesn&squot;t support it. Implements VESA suspend&n;&t; *  and powerdown modes for monitors, and backlight control on LCDs.&n;&t; *    blank_mode == 0: unblanked (backlight on)&n;&t; *    blank_mode == 1: blank (backlight on)&n;&t; *    blank_mode == 2: suspend vsync (backlight off)&n;&t; *    blank_mode == 3: suspend hsync (backlight off)&n;&t; *    blank_mode == 4: powerdown (backlight off)&n;&t; *&n;&t; *  wms...Enable VESA DPMS compatible powerdown mode&n;&t; *  run &quot;setterm -powersave powerdown&quot; to take advantage&n;&t; */
r_struct
id|neofb_par
op_star
id|par
...
l_int|0
suffix:semicolon
multiline_comment|/* LCD off */
id|dpmsflags
op_assign
id|NEO_GR01_SUPPRESS_HSYNC
op_or
id|NEO_GR01_SUPPRESS_VSYNC
suffix:semicolon
macro_line|#ifdef CONFIG_TOSHIBA
multiline_comment|/* Do we still need this ? */
multiline_comment|/* attempt to turn off backlight on toshiba; also turns off external */
(brace
id|SMMRegisters
id|regs
suffix:semicolon
...
l_int|0xff00
suffix:semicolon
multiline_comment|/* HCI_SET */
id|regs.ebx
op_assign
l_int|0x0002
suffix:semicolon
multiline_comment|/* HCI_BACKLIGHT */
id|regs.ecx
op_assign
l_int|0x0000
suffix:semicolon
multiline_comment|/* HCI_DISABLE */
id|tosh_smm
c_func
(paren
op_amp
...
l_int|0x02
suffix:semicolon
multiline_comment|/* LCD normal */
id|dpmsflags
op_assign
l_int|0
suffix:semicolon
multiline_comment|/* no hsync/vsync suppression */
r_break
suffix:semicolon
r_case
id|FB_BLANK_UNBLANK
...
l_int|0
suffix:semicolon
multiline_comment|/* Enable sequencer */
id|lcdflags
op_assign
id|par-&gt;PanelDispCntlReg1
op_amp
...
macro_line|#ifdef CONFIG_TOSHIBA
multiline_comment|/* Do we still need this ? */
multiline_comment|/* attempt to re-enable backlight/external on toshiba */
(brace
id|SMMRegisters
id|regs
suffix:semicolon
...
r_default
suffix:colon
multiline_comment|/* Anything else we don&squot;t understand; return 1 to tell&n;&t;&t;&t; * fb_blank we didn&squot;t aactually do anything */
r_return
l_int|1
suffix:semicolon
)brace
...
r_static
r_void
DECL|function|neo2200_fillrect
id|neo2200_fillrect
c_func
(paren
r_struct
...
op_or
singleline_comment|//               NEO_BC3_DST_XY_ADDR  |
singleline_comment|//               NEO_BC3_SRC_XY_ADDR  |
id|rop
comma
op_amp
id|par-&gt;neo2200-&gt;bltCntl
)paren
suffix:semicolon
r_switch
c_cond
...
r_static
r_void
DECL|function|neo2200_copyarea
id|neo2200_copyarea
c_func
(paren
r_struct
...
(brace
multiline_comment|/* Start with the lower right corner */
id|sy
op_add_assign
(paren
id|area-&gt;height
...
id|NEO_BC0_X_DEC
op_or
id|NEO_BC0_DST_Y_DEC
op_or
id|NEO_BC0_SRC_Y_DEC
suffix:semicolon
)brace
id|src
op_assign
...
r_static
r_void
DECL|function|neo2200_imageblit
id|neo2200_imageblit
c_func
(paren
r_struct
id|fb_info
...
(brace
multiline_comment|/* FIXME. There is a bug with accelerated color-expanded&n;&t;&t;&t; * transfers in 24 bit mode if the image being transferred&n;&t;&t;&t; * is less than 16 bits wide. This is due to insufficient&n;&t;&t;&t; * padding when writing the image. We need to adjust&n;&t;&t;&t; * struct fb_pixmap. Not yet done. */
r_return
id|cfb_imageblit
c_func
(paren
...
r_else
(brace
multiline_comment|/* We don&squot;t currently support hardware acceleration if image&n;&t;&t; * depth is different from display */
r_return
id|cfb_imageblit
c_func
(paren
...
suffix:semicolon
singleline_comment|//      par-&gt;neo2200-&gt;dstStart = (image-&gt;dy &lt;&lt; 16) | (image-&gt;dx &amp; 0xffff);
id|writel
c_func
(paren
(paren
...
multiline_comment|/*&n;static void&n;neofb_draw_cursor(struct fb_info *info, u8 *dst, u8 *src, unsigned int width)&n;{&n;&t;//memset_io(info-&gt;sprite.addr, 0xff, 1);&n;}&n;&n;static int&n;neofb_cursor(struct fb_info *info, struct fb_cursor *cursor)&n;{&n;&t;struct neofb_par *par = (struct neofb_par *) info-&gt;par;&n;&n;&t;* Disable cursor *&n;&t;write_le32(NEOREG_CURSCNTL, ~NEO_CURS_ENABLE, par);&n;&n;&t;if (cursor-&gt;set &amp; FB_CUR_SETPOS) {&n;&t;&t;u32 x = cursor-&gt;image.dx;&n;&t;&t;u32 y = cursor-&gt;image.dy;&n;&n;&t;&t;info-&gt;cursor.image.dx = x;&n;&t;&t;info-&gt;cursor.image.dy = y;&n;&t;&t;write_le32(NEOREG_CURSX, x, par);&n;&t;&t;write_le32(NEOREG_CURSY, y, par);&n;&t;}&n;&n;&t;if (cursor-&gt;set &amp; FB_CUR_SETSIZE) {&n;&t;&t;info-&gt;cursor.image.height = cursor-&gt;image.height;&n;&t;&t;info-&gt;cursor.image.width = cursor-&gt;image.width;&n;&t;}&n;&n;&t;if (cursor-&gt;set &amp; FB_CUR_SETHOT)&n;&t;&t;info-&gt;cursor.hot = cursor-&gt;hot;&n;&n;&t;if (cursor-&gt;set &amp; FB_CUR_SETCMAP) {&n;&t;&t;if (cursor-&gt;image.depth == 1) {&n;&t;&t;&t;u32 fg = cursor-&gt;image.fg_color;&n;&t;&t;&t;u32 bg = cursor-&gt;image.bg_color;&n;&n;&t;&t;&t;info-&gt;cursor.image.fg_color = fg;&n;&t;&t;&t;info-&gt;cursor.image.bg_color = bg;&n;&n;&t;&t;&t;fg = ((fg &amp; 0xff0000) &gt;&gt; 16) | ((fg &amp; 0xff) &lt;&lt; 16) | (fg &amp; 0xff00);&n;&t;&t;&t;bg = ((bg &amp; 0xff0000) &gt;&gt; 16) | ((bg &amp; 0xff) &lt;&lt; 16) | (bg &amp; 0xff00);&n;&t;&t;&t;write_le32(NEOREG_CURSFGCOLOR, fg, par);&n;&t;&t;&t;write_le32(NEOREG_CURSBGCOLOR, bg, par);&n;&t;&t;}&n;&t;}&n;&n;&t;if (cursor-&gt;set &amp; FB_CUR_SETSHAPE)&n;&t;&t;fb_load_cursor_image(info);&n;&n;&t;if (info-&gt;cursor.enable)&n;&t;&t;write_le32(NEOREG_CURSCNTL, NEO_CURS_ENABLE, par);&n;&t;return 0;&n;}&n;*/
...
(brace
singleline_comment|//unsigned long addr;
id|DBG
c_func
(paren
...
suffix:semicolon
multiline_comment|/* Allocate Cursor drawing pad.&n;&t;info-&gt;fix.smem_len -= PAGE_SIZE;&n;&t;addr = info-&gt;fix.smem_start + info-&gt;fix.smem_len;&n;&t;write_le32(NEOREG_CURSMEMPOS, ((0x000f &amp; (addr &gt;&gt; 10)) &lt;&lt; 8) |&n;&t;&t;&t;&t;&t;((0x0ff0 &amp; (addr &gt;&gt; 10)) &gt;&gt; 4), par);&n;&t;addr = (unsigned long) info-&gt;screen_base + info-&gt;fix.smem_len;&n;&t;info-&gt;sprite.addr = (u8 *) addr; */
r_return
l_int|0
suffix:semicolon
)brace
...
DECL|function|neo_scan_monitor
r_static
r_int
id|__devinit
id|neo_scan_monitor
...
id|w
suffix:semicolon
singleline_comment|// Eventually we will have i2c support.
...
(paren
id|KERN_INFO
l_string|&quot;Autodetected %s display&bslash;n&quot;
comma
id|par-&gt;internal_display
op_logical_and
id|par-&gt;external_display
...
l_int|0x02
suffix:colon
singleline_comment|// 1024x768@60
id|par-&gt;NeoPanelWidth
op_assign
l_int|1024
suffix:semicolon
...
l_int|0x03
suffix:colon
multiline_comment|/* 1280x1024@60 panel support needs to be added */
...
(paren
id|KERN_ERR
l_string|&quot;neofb: Only 640x480, 800x600/480 and 1024x768 panels are currently supported&bslash;n&quot;
)paren
suffix:semicolon
r_return
op_minus
...
multiline_comment|/*&n;&t;info-&gt;sprite.size = CursorMem;&n;&t;info-&gt;sprite.scan_align = 1;&n;&t;info-&gt;sprite.buf_align = 1;&n;&t;info-&gt;sprite.flags = FB_PIXMAP_IO;&n;&t;info-&gt;sprite.outbuf = neofb_draw_cursor;&n;*/
id|par-&gt;maxClock
op_assign
id|maxClock
suffix:semicolon
...
(paren
id|KERN_ERR
l_string|&quot;neofb: Unable to find usable video mode.&bslash;n&quot;
)paren
suffix:semicolon
r_goto
id|err_map_video
