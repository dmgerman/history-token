multiline_comment|/*&n; *&n; * Hardware accelerated Matrox Millennium I, II, Mystique, G100, G200 and G400&n; *&n; * (c) 1998-2002 Petr Vandrovec &lt;vandrove@vc.cvut.cz&gt;&n; *&n; * Portions Copyright (c) 2001 Matrox Graphics Inc.&n; *&n; * Version: 1.65 2002/08/14&n; *&n; * MTRR stuff: 1998 Tom Rini &lt;trini@kernel.crashing.org&gt;&n; *&n; * Contributors: &quot;menion?&quot; &lt;menion@mindless.com&gt;&n; *                     Betatesting, fixes, ideas&n; *&n; *               &quot;Kurt Garloff&quot; &lt;garloff@suse.de&gt;&n; *                     Betatesting, fixes, ideas, videomodes, videomodes timmings&n; *&n; *               &quot;Tom Rini&quot; &lt;trini@kernel.crashing.org&gt;&n; *                     MTRR stuff, PPC cleanups, betatesting, fixes, ideas&n; *&n; *               &quot;Bibek Sahu&quot; &lt;scorpio@dodds.net&gt;&n; *                     Access device through readb|w|l and write b|w|l&n; *                     Extensive debugging stuff&n; *&n; *               &quot;Daniel Haun&quot; &lt;haund@usa.net&gt;&n; *                     Testing, hardware cursor fixes&n; *&n; *               &quot;Scott Wood&quot; &lt;sawst46+@pitt.edu&gt;&n; *                     Fixes&n; *&n; *               &quot;Gerd Knorr&quot; &lt;kraxel@goldbach.isdn.cs.tu-berlin.de&gt;&n; *                     Betatesting&n; *&n; *               &quot;Kelly French&quot; &lt;targon@hazmat.com&gt;&n; *               &quot;Fernando Herrera&quot; &lt;fherrera@eurielec.etsit.upm.es&gt;&n; *                     Betatesting, bug reporting&n; *&n; *               &quot;Pablo Bianucci&quot; &lt;pbian@pccp.com.ar&gt;&n; *                     Fixes, ideas, betatesting&n; *&n; *               &quot;Inaky Perez Gonzalez&quot; &lt;inaky@peloncho.fis.ucm.es&gt;&n; *                     Fixes, enhandcements, ideas, betatesting&n; *&n; *               &quot;Ryuichi Oikawa&quot; &lt;roikawa@rr.iiij4u.or.jp&gt;&n; *                     PPC betatesting, PPC support, backward compatibility&n; *&n; *               &quot;Paul Womar&quot; &lt;Paul@pwomar.demon.co.uk&gt;&n; *               &quot;Owen Waller&quot; &lt;O.Waller@ee.qub.ac.uk&gt;&n; *                     PPC betatesting&n; *&n; *               &quot;Thomas Pornin&quot; &lt;pornin@bolet.ens.fr&gt;&n; *                     Alpha betatesting&n; *&n; *               &quot;Pieter van Leuven&quot; &lt;pvl@iae.nl&gt;&n; *               &quot;Ulf Jaenicke-Roessler&quot; &lt;ujr@physik.phy.tu-dresden.de&gt;&n; *                     G100 testing&n; *&n; *               &quot;H. Peter Arvin&quot; &lt;hpa@transmeta.com&gt;&n; *                     Ideas&n; *&n; *               &quot;Cort Dougan&quot; &lt;cort@cs.nmt.edu&gt;&n; *                     CHRP fixes and PReP cleanup&n; *&n; *               &quot;Mark Vojkovich&quot; &lt;mvojkovi@ucsd.edu&gt;&n; *                     G400 support&n; *&n; *               &quot;David C. Hansen&quot; &lt;haveblue@us.ibm.com&gt;&n; *                     Fixes&n; *&n; * (following author is not in any relation with this code, but his code&n; *  is included in this driver)&n; *&n; * Based on framebuffer driver for VBE 2.0 compliant graphic boards&n; *     (c) 1998 Gerd Knorr &lt;kraxel@cs.tu-berlin.de&gt;&n; *&n; * (following author is not in any relation with this code, but his ideas&n; *  were used when writting this driver)&n; *&n; *&t;&t; FreeVBE/AF (Matrox), &quot;Shawn Hargreaves&quot; &lt;shawn@talula.demon.co.uk&gt;&n; *&n; */
multiline_comment|/* make checkconfig does not check includes for this... */
macro_line|#include &lt;linux/config.h&gt;
macro_line|#include &quot;matroxfb_misc.h&quot;
macro_line|#include &lt;linux/interrupt.h&gt;
macro_line|#include &lt;linux/matroxfb.h&gt;
DECL|function|matroxfb_DAC_out
r_void
id|matroxfb_DAC_out
c_func
(paren
id|CPMINFO
r_int
id|reg
comma
r_int
id|val
)paren
(brace
id|DBG_REG
c_func
(paren
id|__FUNCTION__
)paren
id|mga_outb
c_func
(paren
id|M_RAMDAC_BASE
op_plus
id|M_X_INDEX
comma
id|reg
)paren
suffix:semicolon
id|mga_outb
c_func
(paren
id|M_RAMDAC_BASE
op_plus
id|M_X_DATAREG
comma
id|val
)paren
suffix:semicolon
)brace
DECL|function|matroxfb_DAC_in
r_int
id|matroxfb_DAC_in
c_func
(paren
id|CPMINFO
r_int
id|reg
)paren
(brace
id|DBG_REG
c_func
(paren
id|__FUNCTION__
)paren
id|mga_outb
c_func
(paren
id|M_RAMDAC_BASE
op_plus
id|M_X_INDEX
comma
id|reg
)paren
suffix:semicolon
r_return
id|mga_inb
c_func
(paren
id|M_RAMDAC_BASE
op_plus
id|M_X_DATAREG
)paren
suffix:semicolon
)brace
DECL|function|matroxfb_var2my
r_void
id|matroxfb_var2my
c_func
(paren
r_struct
id|fb_var_screeninfo
op_star
id|var
comma
r_struct
id|my_timming
op_star
id|mt
)paren
(brace
r_int
r_int
id|pixclock
op_assign
id|var-&gt;pixclock
suffix:semicolon
id|DBG
c_func
(paren
id|__FUNCTION__
)paren
r_if
c_cond
(paren
op_logical_neg
id|pixclock
)paren
id|pixclock
op_assign
l_int|10000
suffix:semicolon
multiline_comment|/* 10ns = 100MHz */
id|mt-&gt;pixclock
op_assign
l_int|1000000000
op_div
id|pixclock
suffix:semicolon
r_if
c_cond
(paren
id|mt-&gt;pixclock
OL
l_int|1
)paren
id|mt-&gt;pixclock
op_assign
l_int|1
suffix:semicolon
id|mt-&gt;mnp
op_assign
op_minus
l_int|1
suffix:semicolon
id|mt-&gt;dblscan
op_assign
id|var-&gt;vmode
op_amp
id|FB_VMODE_DOUBLE
suffix:semicolon
id|mt-&gt;interlaced
op_assign
id|var-&gt;vmode
op_amp
id|FB_VMODE_INTERLACED
suffix:semicolon
id|mt-&gt;HDisplay
op_assign
id|var-&gt;xres
suffix:semicolon
id|mt-&gt;HSyncStart
op_assign
id|mt-&gt;HDisplay
op_plus
id|var-&gt;right_margin
suffix:semicolon
id|mt-&gt;HSyncEnd
op_assign
id|mt-&gt;HSyncStart
op_plus
id|var-&gt;hsync_len
suffix:semicolon
id|mt-&gt;HTotal
op_assign
id|mt-&gt;HSyncEnd
op_plus
id|var-&gt;left_margin
suffix:semicolon
id|mt-&gt;VDisplay
op_assign
id|var-&gt;yres
suffix:semicolon
id|mt-&gt;VSyncStart
op_assign
id|mt-&gt;VDisplay
op_plus
id|var-&gt;lower_margin
suffix:semicolon
id|mt-&gt;VSyncEnd
op_assign
id|mt-&gt;VSyncStart
op_plus
id|var-&gt;vsync_len
suffix:semicolon
id|mt-&gt;VTotal
op_assign
id|mt-&gt;VSyncEnd
op_plus
id|var-&gt;upper_margin
suffix:semicolon
id|mt-&gt;sync
op_assign
id|var-&gt;sync
suffix:semicolon
)brace
DECL|function|matroxfb_PLL_calcclock
r_int
id|matroxfb_PLL_calcclock
c_func
(paren
r_const
r_struct
id|matrox_pll_features
op_star
id|pll
comma
r_int
r_int
id|freq
comma
r_int
r_int
id|fmax
comma
r_int
r_int
op_star
id|in
comma
r_int
r_int
op_star
id|feed
comma
r_int
r_int
op_star
id|post
)paren
(brace
r_int
r_int
id|bestdiff
op_assign
op_complement
l_int|0
suffix:semicolon
r_int
r_int
id|bestvco
op_assign
l_int|0
suffix:semicolon
r_int
r_int
id|fxtal
op_assign
id|pll-&gt;ref_freq
suffix:semicolon
r_int
r_int
id|fwant
suffix:semicolon
r_int
r_int
id|p
suffix:semicolon
id|DBG
c_func
(paren
id|__FUNCTION__
)paren
id|fwant
op_assign
id|freq
suffix:semicolon
macro_line|#ifdef DEBUG
id|printk
c_func
(paren
id|KERN_ERR
l_string|&quot;post_shift_max: %d&bslash;n&quot;
comma
id|pll-&gt;post_shift_max
)paren
suffix:semicolon
id|printk
c_func
(paren
id|KERN_ERR
l_string|&quot;ref_freq: %d&bslash;n&quot;
comma
id|pll-&gt;ref_freq
)paren
suffix:semicolon
id|printk
c_func
(paren
id|KERN_ERR
l_string|&quot;freq: %d&bslash;n&quot;
comma
id|freq
)paren
suffix:semicolon
id|printk
c_func
(paren
id|KERN_ERR
l_string|&quot;vco_freq_min: %d&bslash;n&quot;
comma
id|pll-&gt;vco_freq_min
)paren
suffix:semicolon
id|printk
c_func
(paren
id|KERN_ERR
l_string|&quot;in_div_min: %d&bslash;n&quot;
comma
id|pll-&gt;in_div_min
)paren
suffix:semicolon
id|printk
c_func
(paren
id|KERN_ERR
l_string|&quot;in_div_max: %d&bslash;n&quot;
comma
id|pll-&gt;in_div_max
)paren
suffix:semicolon
id|printk
c_func
(paren
id|KERN_ERR
l_string|&quot;feed_div_min: %d&bslash;n&quot;
comma
id|pll-&gt;feed_div_min
)paren
suffix:semicolon
id|printk
c_func
(paren
id|KERN_ERR
l_string|&quot;feed_div_max: %d&bslash;n&quot;
comma
id|pll-&gt;feed_div_max
)paren
suffix:semicolon
id|printk
c_func
(paren
id|KERN_ERR
l_string|&quot;fmax: %d&bslash;n&quot;
comma
id|fmax
)paren
suffix:semicolon
macro_line|#endif
r_for
c_loop
(paren
id|p
op_assign
l_int|1
suffix:semicolon
id|p
op_le
id|pll-&gt;post_shift_max
suffix:semicolon
id|p
op_increment
)paren
(brace
r_if
c_cond
(paren
id|fwant
op_star
l_int|2
OG
id|fmax
)paren
r_break
suffix:semicolon
id|fwant
op_mul_assign
l_int|2
suffix:semicolon
)brace
r_if
c_cond
(paren
id|fwant
OL
id|pll-&gt;vco_freq_min
)paren
id|fwant
op_assign
id|pll-&gt;vco_freq_min
suffix:semicolon
r_if
c_cond
(paren
id|fwant
OG
id|fmax
)paren
id|fwant
op_assign
id|fmax
suffix:semicolon
r_for
c_loop
(paren
suffix:semicolon
id|p
op_decrement
OG
l_int|0
suffix:semicolon
id|fwant
op_rshift_assign
l_int|1
comma
id|bestdiff
op_rshift_assign
l_int|1
)paren
(brace
r_int
r_int
id|m
suffix:semicolon
r_if
c_cond
(paren
id|fwant
OL
id|pll-&gt;vco_freq_min
)paren
r_break
suffix:semicolon
r_for
c_loop
(paren
id|m
op_assign
id|pll-&gt;in_div_min
suffix:semicolon
id|m
op_le
id|pll-&gt;in_div_max
suffix:semicolon
id|m
op_increment
)paren
(brace
r_int
r_int
id|diff
comma
id|fvco
suffix:semicolon
r_int
r_int
id|n
suffix:semicolon
id|n
op_assign
(paren
id|fwant
op_star
(paren
id|m
op_plus
l_int|1
)paren
op_plus
(paren
id|fxtal
op_rshift
l_int|1
)paren
)paren
op_div
id|fxtal
op_minus
l_int|1
suffix:semicolon
r_if
c_cond
(paren
id|n
OG
id|pll-&gt;feed_div_max
)paren
r_break
suffix:semicolon
r_if
c_cond
(paren
id|n
OL
id|pll-&gt;feed_div_min
)paren
id|n
op_assign
id|pll-&gt;feed_div_min
suffix:semicolon
id|fvco
op_assign
(paren
id|fxtal
op_star
(paren
id|n
op_plus
l_int|1
)paren
)paren
op_div
(paren
id|m
op_plus
l_int|1
)paren
suffix:semicolon
r_if
c_cond
(paren
id|fvco
OL
id|fwant
)paren
id|diff
op_assign
id|fwant
op_minus
id|fvco
suffix:semicolon
r_else
id|diff
op_assign
id|fvco
op_minus
id|fwant
suffix:semicolon
r_if
c_cond
(paren
id|diff
OL
id|bestdiff
)paren
(brace
id|bestdiff
op_assign
id|diff
suffix:semicolon
op_star
id|post
op_assign
id|p
suffix:semicolon
op_star
id|in
op_assign
id|m
suffix:semicolon
op_star
id|feed
op_assign
id|n
suffix:semicolon
id|bestvco
op_assign
id|fvco
suffix:semicolon
)brace
)brace
)brace
id|dprintk
c_func
(paren
id|KERN_ERR
l_string|&quot;clk: %02X %02X %02X %d %d %d&bslash;n&quot;
comma
op_star
id|in
comma
op_star
id|feed
comma
op_star
id|post
comma
id|fxtal
comma
id|bestvco
comma
id|fwant
)paren
suffix:semicolon
r_return
id|bestvco
suffix:semicolon
)brace
DECL|function|matroxfb_vgaHWinit
r_int
id|matroxfb_vgaHWinit
c_func
(paren
id|WPMINFO
r_struct
id|my_timming
op_star
id|m
)paren
(brace
r_int
r_int
id|hd
comma
id|hs
comma
id|he
comma
id|hbe
comma
id|ht
suffix:semicolon
r_int
r_int
id|vd
comma
id|vs
comma
id|ve
comma
id|vt
comma
id|lc
suffix:semicolon
r_int
r_int
id|wd
suffix:semicolon
r_int
r_int
id|divider
suffix:semicolon
r_int
id|i
suffix:semicolon
r_int
id|fwidth
suffix:semicolon
r_struct
id|matrox_hw_state
op_star
r_const
id|hw
op_assign
op_amp
id|ACCESS_FBINFO
c_func
(paren
id|hw
)paren
suffix:semicolon
id|fwidth
op_assign
l_int|8
suffix:semicolon
id|DBG
c_func
(paren
id|__FUNCTION__
)paren
id|hw-&gt;SEQ
(braket
l_int|0
)braket
op_assign
l_int|0x00
suffix:semicolon
r_if
c_cond
(paren
id|fwidth
op_eq
l_int|9
)paren
id|hw-&gt;SEQ
(braket
l_int|1
)braket
op_assign
l_int|0x00
suffix:semicolon
r_else
id|hw-&gt;SEQ
(braket
l_int|1
)braket
op_assign
l_int|0x01
suffix:semicolon
multiline_comment|/* or 0x09 */
id|hw-&gt;SEQ
(braket
l_int|2
)braket
op_assign
l_int|0x0F
suffix:semicolon
multiline_comment|/* bitplanes */
id|hw-&gt;SEQ
(braket
l_int|3
)braket
op_assign
l_int|0x00
suffix:semicolon
id|hw-&gt;SEQ
(braket
l_int|4
)braket
op_assign
l_int|0x0E
suffix:semicolon
multiline_comment|/* CRTC 0..7, 9, 16..19, 21, 22 are reprogrammed by Matrox Millennium code... Hope that by MGA1064 too */
r_if
c_cond
(paren
id|m-&gt;dblscan
)paren
(brace
id|m-&gt;VTotal
op_lshift_assign
l_int|1
suffix:semicolon
id|m-&gt;VDisplay
op_lshift_assign
l_int|1
suffix:semicolon
id|m-&gt;VSyncStart
op_lshift_assign
l_int|1
suffix:semicolon
id|m-&gt;VSyncEnd
op_lshift_assign
l_int|1
suffix:semicolon
)brace
r_if
c_cond
(paren
id|m-&gt;interlaced
)paren
(brace
id|m-&gt;VTotal
op_rshift_assign
l_int|1
suffix:semicolon
id|m-&gt;VDisplay
op_rshift_assign
l_int|1
suffix:semicolon
id|m-&gt;VSyncStart
op_rshift_assign
l_int|1
suffix:semicolon
id|m-&gt;VSyncEnd
op_rshift_assign
l_int|1
suffix:semicolon
)brace
multiline_comment|/* GCTL is ignored when not using 0xA0000 aperture */
id|hw-&gt;GCTL
(braket
l_int|0
)braket
op_assign
l_int|0x00
suffix:semicolon
id|hw-&gt;GCTL
(braket
l_int|1
)braket
op_assign
l_int|0x00
suffix:semicolon
id|hw-&gt;GCTL
(braket
l_int|2
)braket
op_assign
l_int|0x00
suffix:semicolon
id|hw-&gt;GCTL
(braket
l_int|3
)braket
op_assign
l_int|0x00
suffix:semicolon
id|hw-&gt;GCTL
(braket
l_int|4
)braket
op_assign
l_int|0x00
suffix:semicolon
id|hw-&gt;GCTL
(braket
l_int|5
)braket
op_assign
l_int|0x40
suffix:semicolon
id|hw-&gt;GCTL
(braket
l_int|6
)braket
op_assign
l_int|0x05
suffix:semicolon
id|hw-&gt;GCTL
(braket
l_int|7
)braket
op_assign
l_int|0x0F
suffix:semicolon
id|hw-&gt;GCTL
(braket
l_int|8
)braket
op_assign
l_int|0xFF
suffix:semicolon
multiline_comment|/* Whole ATTR is ignored in PowerGraphics mode */
r_for
c_loop
(paren
id|i
op_assign
l_int|0
suffix:semicolon
id|i
OL
l_int|16
suffix:semicolon
id|i
op_increment
)paren
id|hw-&gt;ATTR
(braket
id|i
)braket
op_assign
id|i
suffix:semicolon
id|hw-&gt;ATTR
(braket
l_int|16
)braket
op_assign
l_int|0x41
suffix:semicolon
id|hw-&gt;ATTR
(braket
l_int|17
)braket
op_assign
l_int|0xFF
suffix:semicolon
id|hw-&gt;ATTR
(braket
l_int|18
)braket
op_assign
l_int|0x0F
suffix:semicolon
r_if
c_cond
(paren
id|fwidth
op_eq
l_int|9
)paren
id|hw-&gt;ATTR
(braket
l_int|19
)braket
op_assign
l_int|0x08
suffix:semicolon
r_else
id|hw-&gt;ATTR
(braket
l_int|19
)braket
op_assign
l_int|0x00
suffix:semicolon
id|hw-&gt;ATTR
(braket
l_int|20
)braket
op_assign
l_int|0x00
suffix:semicolon
id|hd
op_assign
id|m-&gt;HDisplay
op_rshift
l_int|3
suffix:semicolon
id|hs
op_assign
id|m-&gt;HSyncStart
op_rshift
l_int|3
suffix:semicolon
id|he
op_assign
id|m-&gt;HSyncEnd
op_rshift
l_int|3
suffix:semicolon
id|ht
op_assign
id|m-&gt;HTotal
op_rshift
l_int|3
suffix:semicolon
multiline_comment|/* standard timmings are in 8pixels, but for interleaved we cannot */
multiline_comment|/* do it for 4bpp (because of (4bpp &gt;&gt; 1(interleaved))/4 == 0) */
multiline_comment|/* using 16 or more pixels per unit can save us */
id|divider
op_assign
id|ACCESS_FBINFO
c_func
(paren
id|curr.final_bppShift
)paren
suffix:semicolon
r_while
c_loop
(paren
id|divider
op_amp
l_int|3
)paren
(brace
id|hd
op_rshift_assign
l_int|1
suffix:semicolon
id|hs
op_rshift_assign
l_int|1
suffix:semicolon
id|he
op_rshift_assign
l_int|1
suffix:semicolon
id|ht
op_rshift_assign
l_int|1
suffix:semicolon
id|divider
op_lshift_assign
l_int|1
suffix:semicolon
)brace
id|divider
op_assign
id|divider
op_div
l_int|4
suffix:semicolon
multiline_comment|/* divider can be from 1 to 8 */
r_while
c_loop
(paren
id|divider
OG
l_int|8
)paren
(brace
id|hd
op_lshift_assign
l_int|1
suffix:semicolon
id|hs
op_lshift_assign
l_int|1
suffix:semicolon
id|he
op_lshift_assign
l_int|1
suffix:semicolon
id|ht
op_lshift_assign
l_int|1
suffix:semicolon
id|divider
op_rshift_assign
l_int|1
suffix:semicolon
)brace
id|hd
op_assign
id|hd
op_minus
l_int|1
suffix:semicolon
id|hs
op_assign
id|hs
op_minus
l_int|1
suffix:semicolon
id|he
op_assign
id|he
op_minus
l_int|1
suffix:semicolon
id|ht
op_assign
id|ht
op_minus
l_int|1
suffix:semicolon
id|vd
op_assign
id|m-&gt;VDisplay
op_minus
l_int|1
suffix:semicolon
id|vs
op_assign
id|m-&gt;VSyncStart
op_minus
l_int|1
suffix:semicolon
id|ve
op_assign
id|m-&gt;VSyncEnd
op_minus
l_int|1
suffix:semicolon
id|vt
op_assign
id|m-&gt;VTotal
op_minus
l_int|2
suffix:semicolon
id|lc
op_assign
id|vd
suffix:semicolon
multiline_comment|/* G200 cannot work with (ht &amp; 7) == 6 */
r_if
c_cond
(paren
(paren
(paren
id|ht
op_amp
l_int|0x07
)paren
op_eq
l_int|0x06
)paren
op_logical_or
(paren
(paren
id|ht
op_amp
l_int|0x0F
)paren
op_eq
l_int|0x04
)paren
)paren
id|ht
op_increment
suffix:semicolon
id|hbe
op_assign
id|ht
suffix:semicolon
id|wd
op_assign
id|ACCESS_FBINFO
c_func
(paren
id|fbcon
)paren
dot
id|var.xres_virtual
op_star
id|ACCESS_FBINFO
c_func
(paren
id|curr.final_bppShift
)paren
op_div
l_int|64
suffix:semicolon
id|hw-&gt;CRTCEXT
(braket
l_int|0
)braket
op_assign
l_int|0
suffix:semicolon
id|hw-&gt;CRTCEXT
(braket
l_int|5
)braket
op_assign
l_int|0
suffix:semicolon
r_if
c_cond
(paren
id|m-&gt;interlaced
)paren
(brace
id|hw-&gt;CRTCEXT
(braket
l_int|0
)braket
op_assign
l_int|0x80
suffix:semicolon
id|hw-&gt;CRTCEXT
(braket
l_int|5
)braket
op_assign
(paren
id|hs
op_plus
id|he
op_minus
id|ht
)paren
op_rshift
l_int|1
suffix:semicolon
r_if
c_cond
(paren
op_logical_neg
id|m-&gt;dblscan
)paren
id|wd
op_lshift_assign
l_int|1
suffix:semicolon
id|vt
op_and_assign
op_complement
l_int|1
suffix:semicolon
)brace
id|hw-&gt;CRTCEXT
(braket
l_int|0
)braket
op_or_assign
(paren
id|wd
op_amp
l_int|0x300
)paren
op_rshift
l_int|4
suffix:semicolon
id|hw-&gt;CRTCEXT
(braket
l_int|1
)braket
op_assign
(paren
(paren
(paren
id|ht
op_minus
l_int|4
)paren
op_amp
l_int|0x100
)paren
op_rshift
l_int|8
)paren
op_or
(paren
(paren
id|hd
op_amp
l_int|0x100
)paren
op_rshift
l_int|7
)paren
op_or
multiline_comment|/* blanking */
(paren
(paren
id|hs
op_amp
l_int|0x100
)paren
op_rshift
l_int|6
)paren
op_or
multiline_comment|/* sync start */
(paren
id|hbe
op_amp
l_int|0x040
)paren
suffix:semicolon
multiline_comment|/* end hor. blanking */
multiline_comment|/* FIXME: Enable vidrst only on G400, and only if TV-out is used */
r_if
c_cond
(paren
id|ACCESS_FBINFO
c_func
(paren
id|outputs
(braket
l_int|1
)braket
)paren
dot
id|src
op_eq
id|MATROXFB_SRC_CRTC1
)paren
id|hw-&gt;CRTCEXT
(braket
l_int|1
)braket
op_or_assign
l_int|0x88
suffix:semicolon
multiline_comment|/* enable horizontal and vertical vidrst */
id|hw-&gt;CRTCEXT
(braket
l_int|2
)braket
op_assign
(paren
(paren
id|vt
op_amp
l_int|0xC00
)paren
op_rshift
l_int|10
)paren
op_or
(paren
(paren
id|vd
op_amp
l_int|0x400
)paren
op_rshift
l_int|8
)paren
op_or
multiline_comment|/* disp end */
(paren
(paren
id|vd
op_amp
l_int|0xC00
)paren
op_rshift
l_int|7
)paren
op_or
multiline_comment|/* vblanking start */
(paren
(paren
id|vs
op_amp
l_int|0xC00
)paren
op_rshift
l_int|5
)paren
op_or
(paren
(paren
id|lc
op_amp
l_int|0x400
)paren
op_rshift
l_int|3
)paren
suffix:semicolon
id|hw-&gt;CRTCEXT
(braket
l_int|3
)braket
op_assign
(paren
id|divider
op_minus
l_int|1
)paren
op_or
l_int|0x80
suffix:semicolon
id|hw-&gt;CRTCEXT
(braket
l_int|4
)braket
op_assign
l_int|0
suffix:semicolon
id|hw-&gt;CRTC
(braket
l_int|0
)braket
op_assign
id|ht
op_minus
l_int|4
suffix:semicolon
id|hw-&gt;CRTC
(braket
l_int|1
)braket
op_assign
id|hd
suffix:semicolon
id|hw-&gt;CRTC
(braket
l_int|2
)braket
op_assign
id|hd
suffix:semicolon
id|hw-&gt;CRTC
(braket
l_int|3
)braket
op_assign
(paren
id|hbe
op_amp
l_int|0x1F
)paren
op_or
l_int|0x80
suffix:semicolon
id|hw-&gt;CRTC
(braket
l_int|4
)braket
op_assign
id|hs
suffix:semicolon
id|hw-&gt;CRTC
(braket
l_int|5
)braket
op_assign
(paren
(paren
id|hbe
op_amp
l_int|0x20
)paren
op_lshift
l_int|2
)paren
op_or
(paren
id|he
op_amp
l_int|0x1F
)paren
suffix:semicolon
id|hw-&gt;CRTC
(braket
l_int|6
)braket
op_assign
id|vt
op_amp
l_int|0xFF
suffix:semicolon
id|hw-&gt;CRTC
(braket
l_int|7
)braket
op_assign
(paren
(paren
id|vt
op_amp
l_int|0x100
)paren
op_rshift
l_int|8
)paren
op_or
(paren
(paren
id|vd
op_amp
l_int|0x100
)paren
op_rshift
l_int|7
)paren
op_or
(paren
(paren
id|vs
op_amp
l_int|0x100
)paren
op_rshift
l_int|6
)paren
op_or
(paren
(paren
id|vd
op_amp
l_int|0x100
)paren
op_rshift
l_int|5
)paren
op_or
(paren
(paren
id|lc
op_amp
l_int|0x100
)paren
op_rshift
l_int|4
)paren
op_or
(paren
(paren
id|vt
op_amp
l_int|0x200
)paren
op_rshift
l_int|4
)paren
op_or
(paren
(paren
id|vd
op_amp
l_int|0x200
)paren
op_rshift
l_int|3
)paren
op_or
(paren
(paren
id|vs
op_amp
l_int|0x200
)paren
op_rshift
l_int|2
)paren
suffix:semicolon
id|hw-&gt;CRTC
(braket
l_int|8
)braket
op_assign
l_int|0x00
suffix:semicolon
id|hw-&gt;CRTC
(braket
l_int|9
)braket
op_assign
(paren
(paren
id|vd
op_amp
l_int|0x200
)paren
op_rshift
l_int|4
)paren
op_or
(paren
(paren
id|lc
op_amp
l_int|0x200
)paren
op_rshift
l_int|3
)paren
suffix:semicolon
r_if
c_cond
(paren
id|m-&gt;dblscan
op_logical_and
op_logical_neg
id|m-&gt;interlaced
)paren
id|hw-&gt;CRTC
(braket
l_int|9
)braket
op_or_assign
l_int|0x80
suffix:semicolon
r_for
c_loop
(paren
id|i
op_assign
l_int|10
suffix:semicolon
id|i
OL
l_int|16
suffix:semicolon
id|i
op_increment
)paren
id|hw-&gt;CRTC
(braket
id|i
)braket
op_assign
l_int|0x00
suffix:semicolon
id|hw-&gt;CRTC
(braket
l_int|16
)braket
op_assign
id|vs
multiline_comment|/* &amp; 0xFF */
suffix:semicolon
id|hw-&gt;CRTC
(braket
l_int|17
)braket
op_assign
(paren
id|ve
op_amp
l_int|0x0F
)paren
op_or
l_int|0x20
suffix:semicolon
id|hw-&gt;CRTC
(braket
l_int|18
)braket
op_assign
id|vd
multiline_comment|/* &amp; 0xFF */
suffix:semicolon
id|hw-&gt;CRTC
(braket
l_int|19
)braket
op_assign
id|wd
multiline_comment|/* &amp; 0xFF */
suffix:semicolon
id|hw-&gt;CRTC
(braket
l_int|20
)braket
op_assign
l_int|0x00
suffix:semicolon
id|hw-&gt;CRTC
(braket
l_int|21
)braket
op_assign
id|vd
multiline_comment|/* &amp; 0xFF */
suffix:semicolon
id|hw-&gt;CRTC
(braket
l_int|22
)braket
op_assign
(paren
id|vt
op_plus
l_int|1
)paren
multiline_comment|/* &amp; 0xFF */
suffix:semicolon
id|hw-&gt;CRTC
(braket
l_int|23
)braket
op_assign
l_int|0xC3
suffix:semicolon
id|hw-&gt;CRTC
(braket
l_int|24
)braket
op_assign
id|lc
suffix:semicolon
r_return
l_int|0
suffix:semicolon
)brace
suffix:semicolon
DECL|function|matroxfb_vgaHWrestore
r_void
id|matroxfb_vgaHWrestore
c_func
(paren
id|WPMINFO2
)paren
(brace
r_int
id|i
suffix:semicolon
r_struct
id|matrox_hw_state
op_star
r_const
id|hw
op_assign
op_amp
id|ACCESS_FBINFO
c_func
(paren
id|hw
)paren
suffix:semicolon
id|CRITFLAGS
id|DBG
c_func
(paren
id|__FUNCTION__
)paren
id|dprintk
c_func
(paren
id|KERN_INFO
l_string|&quot;MiscOutReg: %02X&bslash;n&quot;
comma
id|hw-&gt;MiscOutReg
)paren
suffix:semicolon
id|dprintk
c_func
(paren
id|KERN_INFO
l_string|&quot;SEQ regs:   &quot;
)paren
suffix:semicolon
r_for
c_loop
(paren
id|i
op_assign
l_int|0
suffix:semicolon
id|i
OL
l_int|5
suffix:semicolon
id|i
op_increment
)paren
id|dprintk
c_func
(paren
l_string|&quot;%02X:&quot;
comma
id|hw-&gt;SEQ
(braket
id|i
)braket
)paren
suffix:semicolon
id|dprintk
c_func
(paren
l_string|&quot;&bslash;n&quot;
)paren
suffix:semicolon
id|dprintk
c_func
(paren
id|KERN_INFO
l_string|&quot;GDC regs:   &quot;
)paren
suffix:semicolon
r_for
c_loop
(paren
id|i
op_assign
l_int|0
suffix:semicolon
id|i
OL
l_int|9
suffix:semicolon
id|i
op_increment
)paren
id|dprintk
c_func
(paren
l_string|&quot;%02X:&quot;
comma
id|hw-&gt;GCTL
(braket
id|i
)braket
)paren
suffix:semicolon
id|dprintk
c_func
(paren
l_string|&quot;&bslash;n&quot;
)paren
suffix:semicolon
id|dprintk
c_func
(paren
id|KERN_INFO
l_string|&quot;CRTC regs: &quot;
)paren
suffix:semicolon
r_for
c_loop
(paren
id|i
op_assign
l_int|0
suffix:semicolon
id|i
OL
l_int|25
suffix:semicolon
id|i
op_increment
)paren
id|dprintk
c_func
(paren
l_string|&quot;%02X:&quot;
comma
id|hw-&gt;CRTC
(braket
id|i
)braket
)paren
suffix:semicolon
id|dprintk
c_func
(paren
l_string|&quot;&bslash;n&quot;
)paren
suffix:semicolon
id|dprintk
c_func
(paren
id|KERN_INFO
l_string|&quot;ATTR regs: &quot;
)paren
suffix:semicolon
r_for
c_loop
(paren
id|i
op_assign
l_int|0
suffix:semicolon
id|i
OL
l_int|21
suffix:semicolon
id|i
op_increment
)paren
id|dprintk
c_func
(paren
l_string|&quot;%02X:&quot;
comma
id|hw-&gt;ATTR
(braket
id|i
)braket
)paren
suffix:semicolon
id|dprintk
c_func
(paren
l_string|&quot;&bslash;n&quot;
)paren
suffix:semicolon
id|CRITBEGIN
id|mga_inb
c_func
(paren
id|M_ATTR_RESET
)paren
suffix:semicolon
id|mga_outb
c_func
(paren
id|M_ATTR_INDEX
comma
l_int|0
)paren
suffix:semicolon
id|mga_outb
c_func
(paren
id|M_MISC_REG
comma
id|hw-&gt;MiscOutReg
)paren
suffix:semicolon
r_for
c_loop
(paren
id|i
op_assign
l_int|1
suffix:semicolon
id|i
OL
l_int|5
suffix:semicolon
id|i
op_increment
)paren
id|mga_setr
c_func
(paren
id|M_SEQ_INDEX
comma
id|i
comma
id|hw-&gt;SEQ
(braket
id|i
)braket
)paren
suffix:semicolon
id|mga_setr
c_func
(paren
id|M_CRTC_INDEX
comma
l_int|17
comma
id|hw-&gt;CRTC
(braket
l_int|17
)braket
op_amp
l_int|0x7F
)paren
suffix:semicolon
r_for
c_loop
(paren
id|i
op_assign
l_int|0
suffix:semicolon
id|i
OL
l_int|25
suffix:semicolon
id|i
op_increment
)paren
id|mga_setr
c_func
(paren
id|M_CRTC_INDEX
comma
id|i
comma
id|hw-&gt;CRTC
(braket
id|i
)braket
)paren
suffix:semicolon
r_for
c_loop
(paren
id|i
op_assign
l_int|0
suffix:semicolon
id|i
OL
l_int|9
suffix:semicolon
id|i
op_increment
)paren
id|mga_setr
c_func
(paren
id|M_GRAPHICS_INDEX
comma
id|i
comma
id|hw-&gt;GCTL
(braket
id|i
)braket
)paren
suffix:semicolon
r_for
c_loop
(paren
id|i
op_assign
l_int|0
suffix:semicolon
id|i
OL
l_int|21
suffix:semicolon
id|i
op_increment
)paren
(brace
id|mga_inb
c_func
(paren
id|M_ATTR_RESET
)paren
suffix:semicolon
id|mga_outb
c_func
(paren
id|M_ATTR_INDEX
comma
id|i
)paren
suffix:semicolon
id|mga_outb
c_func
(paren
id|M_ATTR_INDEX
comma
id|hw-&gt;ATTR
(braket
id|i
)braket
)paren
suffix:semicolon
)brace
id|mga_outb
c_func
(paren
id|M_PALETTE_MASK
comma
l_int|0xFF
)paren
suffix:semicolon
id|mga_outb
c_func
(paren
id|M_DAC_REG
comma
l_int|0x00
)paren
suffix:semicolon
r_for
c_loop
(paren
id|i
op_assign
l_int|0
suffix:semicolon
id|i
OL
l_int|768
suffix:semicolon
id|i
op_increment
)paren
id|mga_outb
c_func
(paren
id|M_DAC_VAL
comma
id|hw-&gt;DACpal
(braket
id|i
)braket
)paren
suffix:semicolon
id|mga_inb
c_func
(paren
id|M_ATTR_RESET
)paren
suffix:semicolon
id|mga_outb
c_func
(paren
id|M_ATTR_INDEX
comma
l_int|0x20
)paren
suffix:semicolon
id|CRITEND
)brace
DECL|function|get_pins
r_static
r_void
id|get_pins
c_func
(paren
r_int
r_char
op_star
id|pins
comma
r_struct
id|matrox_bios
op_star
id|bd
)paren
(brace
r_int
r_int
id|b0
op_assign
id|readb
c_func
(paren
id|pins
)paren
suffix:semicolon
r_if
c_cond
(paren
id|b0
op_eq
l_int|0x2E
op_logical_and
id|readb
c_func
(paren
id|pins
op_plus
l_int|1
)paren
op_eq
l_int|0x41
)paren
(brace
r_int
r_int
id|pins_len
op_assign
id|readb
c_func
(paren
id|pins
op_plus
l_int|2
)paren
suffix:semicolon
r_int
r_int
id|i
suffix:semicolon
r_int
r_char
id|cksum
suffix:semicolon
r_int
r_char
op_star
id|dst
op_assign
id|bd-&gt;pins
suffix:semicolon
r_if
c_cond
(paren
id|pins_len
template_param
l_int|128
)paren
(brace
r_return
suffix:semicolon
)brace
op_star
id|dst
op_increment
op_assign
l_int|0x2E
suffix:semicolon
op_star
id|dst
op_increment
op_assign
l_int|0x41
suffix:semicolon
op_star
id|dst
op_increment
op_assign
id|pins_len
suffix:semicolon
id|cksum
op_assign
l_int|0x2E
op_plus
l_int|0x41
op_plus
id|pins_len
suffix:semicolon
r_for
c_loop
(paren
id|i
op_assign
l_int|3
suffix:semicolon
id|i
OL
id|pins_len
suffix:semicolon
id|i
op_increment
)paren
(brace
id|cksum
op_add_assign
op_star
id|dst
op_increment
op_assign
id|readb
c_func
(paren
id|pins
op_plus
id|i
)paren
suffix:semicolon
)brace
r_if
c_cond
(paren
id|cksum
)paren
(brace
r_return
suffix:semicolon
)brace
id|bd-&gt;pins_len
op_assign
id|pins_len
suffix:semicolon
)brace
r_else
r_if
c_cond
(paren
id|b0
op_eq
l_int|0x40
op_logical_and
id|readb
c_func
(paren
id|pins
op_plus
l_int|1
)paren
op_eq
l_int|0x00
)paren
(brace
r_int
r_int
id|i
suffix:semicolon
r_int
r_char
op_star
id|dst
op_assign
id|bd-&gt;pins
suffix:semicolon
op_star
id|dst
op_increment
op_assign
l_int|0x40
suffix:semicolon
op_star
id|dst
op_increment
op_assign
l_int|0
suffix:semicolon
r_for
c_loop
(paren
id|i
op_assign
l_int|2
suffix:semicolon
id|i
OL
l_int|0x40
suffix:semicolon
id|i
op_increment
)paren
(brace
op_star
id|dst
op_increment
op_assign
id|readb
c_func
(paren
id|pins
op_plus
id|i
)paren
suffix:semicolon
)brace
id|bd-&gt;pins_len
op_assign
l_int|0x40
suffix:semicolon
)brace
)brace
DECL|function|get_bios_version
r_static
r_void
id|get_bios_version
c_func
(paren
r_int
r_char
op_star
id|vbios
comma
r_struct
id|matrox_bios
op_star
id|bd
)paren
(brace
r_int
r_int
id|pcir_offset
suffix:semicolon
id|pcir_offset
op_assign
id|readb
c_func
(paren
id|vbios
op_plus
l_int|24
)paren
op_or
(paren
id|readb
c_func
(paren
id|vbios
op_plus
l_int|25
)paren
op_lshift
l_int|8
)paren
suffix:semicolon
r_if
c_cond
(paren
id|pcir_offset
op_ge
l_int|26
op_logical_and
id|pcir_offset
OL
l_int|0xFFE0
op_logical_and
id|readb
c_func
(paren
id|vbios
op_plus
id|pcir_offset
)paren
op_eq
l_char|&squot;P&squot;
op_logical_and
id|readb
c_func
(paren
id|vbios
op_plus
id|pcir_offset
op_plus
l_int|1
)paren
op_eq
l_char|&squot;C&squot;
op_logical_and
id|readb
c_func
(paren
id|vbios
op_plus
id|pcir_offset
op_plus
l_int|2
)paren
op_eq
l_char|&squot;I&squot;
op_logical_and
id|readb
c_func
(paren
id|vbios
op_plus
id|pcir_offset
op_plus
l_int|3
)paren
op_eq
l_char|&squot;R&squot;
)paren
(brace
r_int
r_char
id|h
suffix:semicolon
id|h
op_assign
id|readb
c_func
(paren
id|vbios
op_plus
id|pcir_offset
op_plus
l_int|0x12
)paren
suffix:semicolon
id|bd-&gt;version.vMaj
op_assign
(paren
id|h
op_rshift
l_int|4
)paren
op_amp
l_int|0xF
suffix:semicolon
id|bd-&gt;version.vMin
op_assign
id|h
op_amp
l_int|0xF
suffix:semicolon
id|bd-&gt;version.vRev
op_assign
id|readb
c_func
(paren
id|vbios
op_plus
id|pcir_offset
op_plus
l_int|0x13
)paren
suffix:semicolon
)brace
r_else
(brace
r_int
r_char
id|h
suffix:semicolon
id|h
op_assign
id|readb
c_func
(paren
id|vbios
op_plus
l_int|5
)paren
suffix:semicolon
id|bd-&gt;version.vMaj
op_assign
(paren
id|h
op_rshift
l_int|4
)paren
op_amp
l_int|0xF
suffix:semicolon
id|bd-&gt;version.vMin
op_assign
id|h
op_amp
l_int|0xF
suffix:semicolon
id|bd-&gt;version.vRev
op_assign
l_int|0
suffix:semicolon
)brace
)brace
DECL|function|get_bios_output
r_static
r_void
id|get_bios_output
c_func
(paren
r_int
r_char
op_star
id|vbios
comma
r_struct
id|matrox_bios
op_star
id|bd
)paren
(brace
r_int
r_char
id|b
suffix:semicolon
id|b
op_assign
id|readb
c_func
(paren
id|vbios
op_plus
l_int|0x7FF1
)paren
suffix:semicolon
r_if
c_cond
(paren
id|b
op_eq
l_int|0xFF
)paren
(brace
id|b
op_assign
l_int|0
suffix:semicolon
)brace
id|bd-&gt;output.state
op_assign
id|b
suffix:semicolon
)brace
DECL|function|get_bios_tvout
r_static
r_void
id|get_bios_tvout
c_func
(paren
r_int
r_char
op_star
id|vbios
comma
r_struct
id|matrox_bios
op_star
id|bd
)paren
(brace
r_int
r_int
id|i
suffix:semicolon
multiline_comment|/* Check for &squot;IBM .*(V....TVO&squot; string - it means TVO BIOS */
id|bd-&gt;output.tvout
op_assign
l_int|0
suffix:semicolon
r_if
c_cond
(paren
id|readb
c_func
(paren
id|vbios
op_plus
l_int|0x1D
)paren
op_ne
l_char|&squot;I&squot;
op_logical_or
id|readb
c_func
(paren
id|vbios
op_plus
l_int|0x1E
)paren
op_ne
l_char|&squot;B&squot;
op_logical_or
id|readb
c_func
(paren
id|vbios
op_plus
l_int|0x1F
)paren
op_ne
l_char|&squot;M&squot;
op_logical_or
id|readb
c_func
(paren
id|vbios
op_plus
l_int|0x20
)paren
op_ne
l_char|&squot; &squot;
)paren
(brace
r_return
suffix:semicolon
)brace
r_for
c_loop
(paren
id|i
op_assign
l_int|0x2D
suffix:semicolon
id|i
OL
l_int|0x2D
op_plus
l_int|128
suffix:semicolon
id|i
op_increment
)paren
(brace
r_int
r_char
id|b
op_assign
id|readb
c_func
(paren
id|vbios
op_plus
id|i
)paren
suffix:semicolon
r_if
c_cond
(paren
id|b
op_eq
l_char|&squot;(&squot;
op_logical_and
id|readb
c_func
(paren
id|vbios
op_plus
id|i
op_plus
l_int|1
)paren
op_eq
l_char|&squot;V&squot;
)paren
(brace
r_if
c_cond
(paren
id|readb
c_func
(paren
id|vbios
op_plus
id|i
op_plus
l_int|6
)paren
op_eq
l_char|&squot;T&squot;
op_logical_and
id|readb
c_func
(paren
id|vbios
op_plus
id|i
op_plus
l_int|7
)paren
op_eq
l_char|&squot;V&squot;
op_logical_and
id|readb
c_func
(paren
id|vbios
op_plus
id|i
op_plus
l_int|8
)paren
op_eq
l_char|&squot;O&squot;
)paren
(brace
id|bd-&gt;output.tvout
op_assign
l_int|1
suffix:semicolon
)brace
r_return
suffix:semicolon
)brace
r_if
c_cond
(paren
id|b
op_eq
l_int|0
)paren
r_break
suffix:semicolon
)brace
)brace
DECL|function|parse_bios
r_static
r_void
id|parse_bios
c_func
(paren
r_int
r_char
op_star
id|vbios
comma
r_struct
id|matrox_bios
op_star
id|bd
)paren
(brace
r_int
r_int
id|pins_offset
suffix:semicolon
r_if
c_cond
(paren
id|readb
c_func
(paren
id|vbios
)paren
op_ne
l_int|0x55
op_logical_or
id|readb
c_func
(paren
id|vbios
op_plus
l_int|1
)paren
op_ne
l_int|0xAA
)paren
(brace
r_return
suffix:semicolon
)brace
id|bd-&gt;bios_valid
op_assign
l_int|1
suffix:semicolon
id|get_bios_version
c_func
(paren
id|vbios
comma
id|bd
)paren
suffix:semicolon
id|get_bios_output
c_func
(paren
id|vbios
comma
id|bd
)paren
suffix:semicolon
id|get_bios_tvout
c_func
(paren
id|vbios
comma
id|bd
)paren
suffix:semicolon
id|pins_offset
op_assign
id|readb
c_func
(paren
id|vbios
op_plus
l_int|0x7FFC
)paren
op_or
(paren
id|readb
c_func
(paren
id|vbios
op_plus
l_int|0x7FFD
)paren
op_lshift
l_int|8
)paren
suffix:semicolon
r_if
c_cond
(paren
id|pins_offset
op_le
l_int|0xFF80
)paren
(brace
id|get_pins
c_func
(paren
id|vbios
op_plus
id|pins_offset
comma
id|bd
)paren
suffix:semicolon
)brace
)brace
DECL|macro|get_u16
mdefine_line|#define get_u16(x) (le16_to_cpu(get_unaligned((__u16*)(x))))
DECL|macro|get_u32
mdefine_line|#define get_u32(x) (le32_to_cpu(get_unaligned((__u32*)(x))))
DECL|function|parse_pins1
r_static
r_int
id|parse_pins1
c_func
(paren
id|WPMINFO
r_const
r_struct
id|matrox_bios
op_star
id|bd
)paren
(brace
r_int
r_int
id|maxdac
suffix:semicolon
r_switch
c_cond
(paren
id|bd-&gt;pins
(braket
l_int|22
)braket
)paren
(brace
r_case
l_int|0
suffix:colon
id|maxdac
op_assign
l_int|175000
suffix:semicolon
r_break
suffix:semicolon
r_case
l_int|1
suffix:colon
id|maxdac
op_assign
l_int|220000
suffix:semicolon
r_break
suffix:semicolon
r_default
suffix:colon
id|maxdac
op_assign
l_int|240000
suffix:semicolon
r_break
suffix:semicolon
)brace
r_if
c_cond
(paren
id|get_u16
c_func
(paren
id|bd-&gt;pins
op_plus
l_int|24
)paren
)paren
(brace
id|maxdac
op_assign
id|get_u16
c_func
(paren
id|bd-&gt;pins
op_plus
l_int|24
)paren
op_star
l_int|10
suffix:semicolon
)brace
id|MINFO-&gt;limits.pixel.vcomax
op_assign
id|maxdac
suffix:semicolon
id|MINFO-&gt;values.pll.system
op_assign
id|get_u16
c_func
(paren
id|bd-&gt;pins
op_plus
l_int|28
)paren
ques
c_cond
id|get_u16
c_func
(paren
id|bd-&gt;pins
op_plus
l_int|28
)paren
op_star
l_int|10
suffix:colon
l_int|50000
suffix:semicolon
multiline_comment|/* ignore 4MB, 8MB, module clocks */
id|MINFO-&gt;features.pll.ref_freq
op_assign
l_int|14318
suffix:semicolon
id|MINFO-&gt;values.reg.mctlwtst
op_assign
l_int|0x00030101
suffix:semicolon
r_return
l_int|0
suffix:semicolon
)brace
DECL|function|default_pins1
r_static
r_void
id|default_pins1
c_func
(paren
id|WPMINFO2
)paren
(brace
multiline_comment|/* Millennium */
id|MINFO-&gt;limits.pixel.vcomax
op_assign
l_int|220000
suffix:semicolon
id|MINFO-&gt;values.pll.system
op_assign
l_int|50000
suffix:semicolon
id|MINFO-&gt;features.pll.ref_freq
op_assign
l_int|14318
suffix:semicolon
id|MINFO-&gt;values.reg.mctlwtst
op_assign
l_int|0x00030101
suffix:semicolon
)brace
DECL|function|parse_pins2
r_static
r_int
id|parse_pins2
c_func
(paren
id|WPMINFO
r_const
r_struct
id|matrox_bios
op_star
id|bd
)paren
(brace
id|MINFO-&gt;limits.pixel.vcomax
op_assign
id|MINFO-&gt;limits.system.vcomax
op_assign
(paren
id|bd-&gt;pins
(braket
l_int|41
)braket
op_eq
l_int|0xFF
)paren
ques
c_cond
l_int|230000
suffix:colon
(paren
(paren
id|bd-&gt;pins
(braket
l_int|41
)braket
op_plus
l_int|100
)paren
op_star
l_int|1000
)paren
suffix:semicolon
id|MINFO-&gt;values.reg.mctlwtst
op_assign
(paren
(paren
id|bd-&gt;pins
(braket
l_int|51
)braket
op_amp
l_int|0x01
)paren
ques
c_cond
l_int|0x00000001
suffix:colon
l_int|0
)paren
op_or
(paren
(paren
id|bd-&gt;pins
(braket
l_int|51
)braket
op_amp
l_int|0x02
)paren
ques
c_cond
l_int|0x00000100
suffix:colon
l_int|0
)paren
op_or
(paren
(paren
id|bd-&gt;pins
(braket
l_int|51
)braket
op_amp
l_int|0x04
)paren
ques
c_cond
l_int|0x00010000
suffix:colon
l_int|0
)paren
op_or
(paren
(paren
id|bd-&gt;pins
(braket
l_int|51
)braket
op_amp
l_int|0x08
)paren
ques
c_cond
l_int|0x00020000
suffix:colon
l_int|0
)paren
suffix:semicolon
id|MINFO-&gt;values.pll.system
op_assign
(paren
id|bd-&gt;pins
(braket
l_int|43
)braket
op_eq
l_int|0xFF
)paren
ques
c_cond
l_int|50000
suffix:colon
(paren
(paren
id|bd-&gt;pins
(braket
l_int|43
)braket
op_plus
l_int|100
)paren
op_star
l_int|1000
)paren
suffix:semicolon
id|MINFO-&gt;features.pll.ref_freq
op_assign
l_int|14318
suffix:semicolon
r_return
l_int|0
suffix:semicolon
)brace
DECL|function|default_pins2
r_static
r_void
id|default_pins2
c_func
(paren
id|WPMINFO2
)paren
(brace
multiline_comment|/* Millennium II, Mystique */
id|MINFO-&gt;limits.pixel.vcomax
op_assign
id|MINFO-&gt;limits.system.vcomax
op_assign
l_int|230000
suffix:semicolon
id|MINFO-&gt;values.reg.mctlwtst
op_assign
l_int|0x00030101
suffix:semicolon
id|MINFO-&gt;values.pll.system
op_assign
l_int|50000
suffix:semicolon
id|MINFO-&gt;features.pll.ref_freq
op_assign
l_int|14318
suffix:semicolon
)brace
DECL|function|parse_pins3
r_static
r_int
id|parse_pins3
c_func
(paren
id|WPMINFO
r_const
r_struct
id|matrox_bios
op_star
id|bd
)paren
(brace
id|MINFO-&gt;limits.pixel.vcomax
op_assign
id|MINFO-&gt;limits.system.vcomax
op_assign
(paren
id|bd-&gt;pins
(braket
l_int|36
)braket
op_eq
l_int|0xFF
)paren
ques
c_cond
l_int|230000
suffix:colon
(paren
(paren
id|bd-&gt;pins
(braket
l_int|36
)braket
op_plus
l_int|100
)paren
op_star
l_int|1000
)paren
suffix:semicolon
id|MINFO-&gt;values.reg.mctlwtst
op_assign
id|get_u32
c_func
(paren
id|bd-&gt;pins
op_plus
l_int|48
)paren
op_eq
l_int|0xFFFFFFFF
ques
c_cond
l_int|0x01250A21
suffix:colon
id|get_u32
c_func
(paren
id|bd-&gt;pins
op_plus
l_int|48
)paren
suffix:semicolon
multiline_comment|/* memory config */
id|MINFO-&gt;values.reg.memrdbk
op_assign
(paren
(paren
id|bd-&gt;pins
(braket
l_int|57
)braket
op_lshift
l_int|21
)paren
op_amp
l_int|0x1E000000
)paren
op_or
(paren
(paren
id|bd-&gt;pins
(braket
l_int|57
)braket
op_lshift
l_int|22
)paren
op_amp
l_int|0x00C00000
)paren
op_or
(paren
(paren
id|bd-&gt;pins
(braket
l_int|56
)braket
op_lshift
l_int|1
)paren
op_amp
l_int|0x000001E0
)paren
op_or
(paren
id|bd-&gt;pins
(braket
l_int|56
)braket
op_amp
l_int|0x0000000F
)paren
suffix:semicolon
id|MINFO-&gt;values.reg.opt
op_assign
(paren
id|bd-&gt;pins
(braket
l_int|54
)braket
op_amp
l_int|7
)paren
op_lshift
l_int|10
suffix:semicolon
id|MINFO-&gt;values.reg.opt2
op_assign
id|bd-&gt;pins
(braket
l_int|58
)braket
op_lshift
l_int|12
suffix:semicolon
id|MINFO-&gt;features.pll.ref_freq
op_assign
(paren
id|bd-&gt;pins
(braket
l_int|52
)braket
op_amp
l_int|0x20
)paren
ques
c_cond
l_int|14318
suffix:colon
l_int|27000
suffix:semicolon
r_return
l_int|0
suffix:semicolon
)brace
DECL|function|default_pins3
r_static
r_void
id|default_pins3
c_func
(paren
id|WPMINFO2
)paren
(brace
multiline_comment|/* G100, G200 */
id|MINFO-&gt;limits.pixel.vcomax
op_assign
id|MINFO-&gt;limits.system.vcomax
op_assign
l_int|230000
suffix:semicolon
id|MINFO-&gt;values.reg.mctlwtst
op_assign
l_int|0x01250A21
suffix:semicolon
id|MINFO-&gt;values.reg.memrdbk
op_assign
l_int|0x00000000
suffix:semicolon
id|MINFO-&gt;values.reg.opt
op_assign
l_int|0x00000C00
suffix:semicolon
id|MINFO-&gt;values.reg.opt2
op_assign
l_int|0x00000000
suffix:semicolon
id|MINFO-&gt;features.pll.ref_freq
op_assign
l_int|27000
suffix:semicolon
)brace
DECL|function|parse_pins4
r_static
r_int
id|parse_pins4
c_func
(paren
id|WPMINFO
r_const
r_struct
id|matrox_bios
op_star
id|bd
)paren
(brace
id|MINFO-&gt;limits.pixel.vcomax
op_assign
(paren
id|bd-&gt;pins
(braket
l_int|39
)braket
op_eq
l_int|0xFF
)paren
ques
c_cond
l_int|230000
suffix:colon
id|bd-&gt;pins
(braket
l_int|39
)braket
op_star
l_int|4000
suffix:semicolon
id|MINFO-&gt;limits.system.vcomax
op_assign
(paren
id|bd-&gt;pins
(braket
l_int|38
)braket
op_eq
l_int|0xFF
)paren
ques
c_cond
id|MINFO-&gt;limits.pixel.vcomax
suffix:colon
id|bd-&gt;pins
(braket
l_int|38
)braket
op_star
l_int|4000
suffix:semicolon
id|MINFO-&gt;values.reg.mctlwtst
op_assign
id|get_u32
c_func
(paren
id|bd-&gt;pins
op_plus
l_int|71
)paren
suffix:semicolon
id|MINFO-&gt;values.reg.memrdbk
op_assign
(paren
(paren
id|bd-&gt;pins
(braket
l_int|87
)braket
op_lshift
l_int|21
)paren
op_amp
l_int|0x1E000000
)paren
op_or
(paren
(paren
id|bd-&gt;pins
(braket
l_int|87
)braket
op_lshift
l_int|22
)paren
op_amp
l_int|0x00C00000
)paren
op_or
(paren
(paren
id|bd-&gt;pins
(braket
l_int|86
)braket
op_lshift
l_int|1
)paren
op_amp
l_int|0x000001E0
)paren
op_or
(paren
id|bd-&gt;pins
(braket
l_int|86
)braket
op_amp
l_int|0x0000000F
)paren
suffix:semicolon
id|MINFO-&gt;values.reg.opt
op_assign
(paren
(paren
id|bd-&gt;pins
(braket
l_int|53
)braket
op_lshift
l_int|15
)paren
op_amp
l_int|0x00400000
)paren
op_or
(paren
(paren
id|bd-&gt;pins
(braket
l_int|53
)braket
op_lshift
l_int|22
)paren
op_amp
l_int|0x10000000
)paren
op_or
(paren
(paren
id|bd-&gt;pins
(braket
l_int|53
)braket
op_lshift
l_int|7
)paren
op_amp
l_int|0x00001C00
)paren
suffix:semicolon
id|MINFO-&gt;values.reg.opt3
op_assign
id|get_u32
c_func
(paren
id|bd-&gt;pins
op_plus
l_int|67
)paren
suffix:semicolon
id|MINFO-&gt;values.pll.system
op_assign
(paren
id|bd-&gt;pins
(braket
l_int|65
)braket
op_eq
l_int|0xFF
)paren
ques
c_cond
l_int|200000
suffix:colon
id|bd-&gt;pins
(braket
l_int|65
)braket
op_star
l_int|4000
suffix:semicolon
id|MINFO-&gt;features.pll.ref_freq
op_assign
(paren
id|bd-&gt;pins
(braket
l_int|92
)braket
op_amp
l_int|0x01
)paren
ques
c_cond
l_int|14318
suffix:colon
l_int|27000
suffix:semicolon
r_return
l_int|0
suffix:semicolon
)brace
DECL|function|default_pins4
r_static
r_void
id|default_pins4
c_func
(paren
id|WPMINFO2
)paren
(brace
multiline_comment|/* G400 */
id|MINFO-&gt;limits.pixel.vcomax
op_assign
id|MINFO-&gt;limits.system.vcomax
op_assign
l_int|252000
suffix:semicolon
id|MINFO-&gt;values.reg.mctlwtst
op_assign
l_int|0x04A450A1
suffix:semicolon
id|MINFO-&gt;values.reg.memrdbk
op_assign
l_int|0x000000E7
suffix:semicolon
id|MINFO-&gt;values.reg.opt
op_assign
l_int|0x10000400
suffix:semicolon
id|MINFO-&gt;values.reg.opt3
op_assign
l_int|0x0190A419
suffix:semicolon
id|MINFO-&gt;values.pll.system
op_assign
l_int|200000
suffix:semicolon
id|MINFO-&gt;features.pll.ref_freq
op_assign
l_int|27000
suffix:semicolon
)brace
DECL|function|parse_pins5
r_static
r_int
id|parse_pins5
c_func
(paren
id|WPMINFO
r_const
r_struct
id|matrox_bios
op_star
id|bd
)paren
(brace
r_int
r_int
id|mult
suffix:semicolon
id|mult
op_assign
id|bd-&gt;pins
(braket
l_int|4
)braket
ques
c_cond
l_int|8000
suffix:colon
l_int|6000
suffix:semicolon
id|MINFO-&gt;limits.pixel.vcomax
op_assign
(paren
id|bd-&gt;pins
(braket
l_int|38
)braket
op_eq
l_int|0xFF
)paren
ques
c_cond
l_int|600000
suffix:colon
id|bd-&gt;pins
(braket
l_int|38
)braket
op_star
id|mult
suffix:semicolon
id|MINFO-&gt;limits.system.vcomax
op_assign
(paren
id|bd-&gt;pins
(braket
l_int|36
)braket
op_eq
l_int|0xFF
)paren
ques
c_cond
id|MINFO-&gt;limits.pixel.vcomax
suffix:colon
id|bd-&gt;pins
(braket
l_int|36
)braket
op_star
id|mult
suffix:semicolon
id|MINFO-&gt;limits.video.vcomax
op_assign
(paren
id|bd-&gt;pins
(braket
l_int|37
)braket
op_eq
l_int|0xFF
)paren
ques
c_cond
id|MINFO-&gt;limits.system.vcomax
suffix:colon
id|bd-&gt;pins
(braket
l_int|37
)braket
op_star
id|mult
suffix:semicolon
id|MINFO-&gt;limits.pixel.vcomin
op_assign
(paren
id|bd-&gt;pins
(braket
l_int|123
)braket
op_eq
l_int|0xFF
)paren
ques
c_cond
l_int|256000
suffix:colon
id|bd-&gt;pins
(braket
l_int|123
)braket
op_star
id|mult
suffix:semicolon
id|MINFO-&gt;limits.system.vcomin
op_assign
(paren
id|bd-&gt;pins
(braket
l_int|121
)braket
op_eq
l_int|0xFF
)paren
ques
c_cond
id|MINFO-&gt;limits.pixel.vcomin
suffix:colon
id|bd-&gt;pins
(braket
l_int|121
)braket
op_star
id|mult
suffix:semicolon
id|MINFO-&gt;limits.video.vcomin
op_assign
(paren
id|bd-&gt;pins
(braket
l_int|122
)braket
op_eq
l_int|0xFF
)paren
ques
c_cond
id|MINFO-&gt;limits.system.vcomin
suffix:colon
id|bd-&gt;pins
(braket
l_int|122
)braket
op_star
id|mult
suffix:semicolon
id|MINFO-&gt;values.pll.system
op_assign
id|MINFO-&gt;values.pll.video
op_assign
(paren
id|bd-&gt;pins
(braket
l_int|92
)braket
op_eq
l_int|0xFF
)paren
ques
c_cond
l_int|284000
suffix:colon
id|bd-&gt;pins
(braket
l_int|92
)braket
op_star
l_int|4000
suffix:semicolon
id|MINFO-&gt;values.reg.opt
op_assign
id|get_u32
c_func
(paren
id|bd-&gt;pins
op_plus
l_int|48
)paren
suffix:semicolon
id|MINFO-&gt;values.reg.opt2
op_assign
id|get_u32
c_func
(paren
id|bd-&gt;pins
op_plus
l_int|52
)paren
suffix:semicolon
id|MINFO-&gt;values.reg.opt3
op_assign
id|get_u32
c_func
(paren
id|bd-&gt;pins
op_plus
l_int|94
)paren
suffix:semicolon
id|MINFO-&gt;values.reg.mctlwtst
op_assign
id|get_u32
c_func
(paren
id|bd-&gt;pins
op_plus
l_int|98
)paren
suffix:semicolon
id|MINFO-&gt;values.reg.memmisc
op_assign
id|get_u32
c_func
(paren
id|bd-&gt;pins
op_plus
l_int|102
)paren
suffix:semicolon
id|MINFO-&gt;values.reg.memrdbk
op_assign
id|get_u32
c_func
(paren
id|bd-&gt;pins
op_plus
l_int|106
)paren
suffix:semicolon
id|MINFO-&gt;features.pll.ref_freq
op_assign
(paren
id|bd-&gt;pins
(braket
l_int|110
)braket
op_amp
l_int|0x01
)paren
ques
c_cond
l_int|14318
suffix:colon
l_int|27000
suffix:semicolon
id|MINFO-&gt;values.memory.ddr
op_assign
(paren
id|bd-&gt;pins
(braket
l_int|114
)braket
op_amp
l_int|0x60
)paren
op_eq
l_int|0x20
suffix:semicolon
id|MINFO-&gt;values.memory.dll
op_assign
(paren
id|bd-&gt;pins
(braket
l_int|115
)braket
op_amp
l_int|0x02
)paren
op_ne
l_int|0
suffix:semicolon
id|MINFO-&gt;values.memory.emrswen
op_assign
(paren
id|bd-&gt;pins
(braket
l_int|115
)braket
op_amp
l_int|0x01
)paren
op_ne
l_int|0
suffix:semicolon
id|MINFO-&gt;values.reg.maccess
op_assign
id|MINFO-&gt;values.memory.emrswen
ques
c_cond
l_int|0x00004000
suffix:colon
l_int|0x00000000
suffix:semicolon
r_if
c_cond
(paren
id|bd-&gt;pins
(braket
l_int|115
)braket
op_amp
l_int|4
)paren
(brace
id|MINFO-&gt;values.reg.mctlwtst_core
op_assign
id|MINFO-&gt;values.reg.mctlwtst
suffix:semicolon
)brace
r_else
(brace
id|u_int32_t
id|wtst_xlat
(braket
)braket
op_assign
(brace
l_int|0
comma
l_int|1
comma
l_int|5
comma
l_int|6
comma
l_int|7
comma
l_int|5
comma
l_int|2
comma
l_int|3
)brace
suffix:semicolon
id|MINFO-&gt;values.reg.mctlwtst_core
op_assign
(paren
id|MINFO-&gt;values.reg.mctlwtst
op_amp
op_complement
l_int|7
)paren
op_or
id|wtst_xlat
(braket
id|MINFO-&gt;values.reg.mctlwtst
op_amp
l_int|7
)braket
suffix:semicolon
)brace
r_return
l_int|0
suffix:semicolon
)brace
DECL|function|default_pins5
r_static
r_void
id|default_pins5
c_func
(paren
id|WPMINFO2
)paren
(brace
multiline_comment|/* Mine 16MB G450 with SDRAM DDR */
id|MINFO-&gt;limits.pixel.vcomax
op_assign
id|MINFO-&gt;limits.system.vcomax
op_assign
id|MINFO-&gt;limits.video.vcomax
op_assign
l_int|600000
suffix:semicolon
id|MINFO-&gt;limits.pixel.vcomin
op_assign
id|MINFO-&gt;limits.system.vcomin
op_assign
id|MINFO-&gt;limits.video.vcomin
op_assign
l_int|256000
suffix:semicolon
id|MINFO-&gt;values.pll.system
op_assign
id|MINFO-&gt;values.pll.video
op_assign
l_int|284000
suffix:semicolon
id|MINFO-&gt;values.reg.opt
op_assign
l_int|0x404A1160
suffix:semicolon
id|MINFO-&gt;values.reg.opt2
op_assign
l_int|0x0000AC00
suffix:semicolon
id|MINFO-&gt;values.reg.opt3
op_assign
l_int|0x0090A409
suffix:semicolon
id|MINFO-&gt;values.reg.mctlwtst_core
op_assign
id|MINFO-&gt;values.reg.mctlwtst
op_assign
l_int|0x0C81462B
suffix:semicolon
id|MINFO-&gt;values.reg.memmisc
op_assign
l_int|0x80000004
suffix:semicolon
id|MINFO-&gt;values.reg.memrdbk
op_assign
l_int|0x01001103
suffix:semicolon
id|MINFO-&gt;features.pll.ref_freq
op_assign
l_int|27000
suffix:semicolon
id|MINFO-&gt;values.memory.ddr
op_assign
l_int|1
suffix:semicolon
id|MINFO-&gt;values.memory.dll
op_assign
l_int|1
suffix:semicolon
id|MINFO-&gt;values.memory.emrswen
op_assign
l_int|1
suffix:semicolon
id|MINFO-&gt;values.reg.maccess
op_assign
l_int|0x00004000
suffix:semicolon
)brace
DECL|function|matroxfb_set_limits
r_static
r_int
id|matroxfb_set_limits
c_func
(paren
id|WPMINFO
r_const
r_struct
id|matrox_bios
op_star
id|bd
)paren
(brace
r_int
r_int
id|pins_version
suffix:semicolon
r_static
r_const
r_int
r_int
id|pinslen
(braket
)braket
op_assign
(brace
l_int|64
comma
l_int|64
comma
l_int|64
comma
l_int|128
comma
l_int|128
)brace
suffix:semicolon
r_switch
c_cond
(paren
id|ACCESS_FBINFO
c_func
(paren
id|chip
)paren
)paren
(brace
r_case
id|MGA_2064
suffix:colon
id|default_pins1
c_func
(paren
id|PMINFO2
)paren
suffix:semicolon
r_break
suffix:semicolon
r_case
id|MGA_2164
suffix:colon
r_case
id|MGA_1064
suffix:colon
r_case
id|MGA_1164
suffix:colon
id|default_pins2
c_func
(paren
id|PMINFO2
)paren
suffix:semicolon
r_break
suffix:semicolon
r_case
id|MGA_G100
suffix:colon
r_case
id|MGA_G200
suffix:colon
id|default_pins3
c_func
(paren
id|PMINFO2
)paren
suffix:semicolon
r_break
suffix:semicolon
r_case
id|MGA_G400
suffix:colon
id|default_pins4
c_func
(paren
id|PMINFO2
)paren
suffix:semicolon
r_break
suffix:semicolon
r_case
id|MGA_G450
suffix:colon
r_case
id|MGA_G550
suffix:colon
id|default_pins5
c_func
(paren
id|PMINFO2
)paren
suffix:semicolon
r_break
suffix:semicolon
)brace
r_if
c_cond
(paren
op_logical_neg
id|bd-&gt;bios_valid
)paren
(brace
id|printk
c_func
(paren
id|KERN_INFO
l_string|&quot;matroxfb: Your Matrox device does not have BIOS&bslash;n&quot;
)paren
suffix:semicolon
r_return
op_minus
l_int|1
suffix:semicolon
)brace
r_if
c_cond
(paren
id|bd-&gt;pins_len
OL
l_int|64
)paren
(brace
id|printk
c_func
(paren
id|KERN_INFO
l_string|&quot;matroxfb: BIOS on your Matrox device does not contain powerup info&bslash;n&quot;
)paren
suffix:semicolon
r_return
op_minus
l_int|1
suffix:semicolon
)brace
r_if
c_cond
(paren
id|bd-&gt;pins
(braket
l_int|0
)braket
op_eq
l_int|0x2E
op_logical_and
id|bd-&gt;pins
(braket
l_int|1
)braket
op_eq
l_int|0x41
)paren
(brace
id|pins_version
op_assign
id|bd-&gt;pins
(braket
l_int|5
)braket
suffix:semicolon
r_if
c_cond
(paren
id|pins_version
template_param
l_int|5
)paren
(brace
id|printk
c_func
(paren
id|KERN_INFO
l_string|&quot;matroxfb: Unknown version (%u) of powerup info&bslash;n&quot;
comma
id|pins_version
)paren
suffix:semicolon
r_return
op_minus
l_int|1
suffix:semicolon
)brace
)brace
r_else
(brace
id|pins_version
op_assign
l_int|1
suffix:semicolon
)brace
r_if
c_cond
(paren
id|bd-&gt;pins_len
op_ne
id|pinslen
(braket
id|pins_version
op_minus
l_int|1
)braket
)paren
(brace
id|printk
c_func
(paren
id|KERN_INFO
l_string|&quot;matroxfb: Invalid powerup info&bslash;n&quot;
)paren
suffix:semicolon
r_return
op_minus
l_int|1
suffix:semicolon
)brace
r_switch
c_cond
(paren
id|pins_version
)paren
(brace
r_case
l_int|1
suffix:colon
r_return
id|parse_pins1
c_func
(paren
id|PMINFO
id|bd
)paren
suffix:semicolon
r_case
l_int|2
suffix:colon
r_return
id|parse_pins2
c_func
(paren
id|PMINFO
id|bd
)paren
suffix:semicolon
r_case
l_int|3
suffix:colon
r_return
id|parse_pins3
c_func
(paren
id|PMINFO
id|bd
)paren
suffix:semicolon
r_case
l_int|4
suffix:colon
r_return
id|parse_pins4
c_func
(paren
id|PMINFO
id|bd
)paren
suffix:semicolon
r_case
l_int|5
suffix:colon
r_return
id|parse_pins5
c_func
(paren
id|PMINFO
id|bd
)paren
suffix:semicolon
r_default
suffix:colon
id|printk
c_func
(paren
id|KERN_DEBUG
l_string|&quot;matroxfb: Powerup info version %u is not yet supported&bslash;n&quot;
comma
id|pins_version
)paren
suffix:semicolon
r_return
op_minus
l_int|1
suffix:semicolon
)brace
)brace
DECL|function|matroxfb_read_pins
r_void
id|matroxfb_read_pins
c_func
(paren
id|WPMINFO2
)paren
(brace
id|u32
id|opt
suffix:semicolon
id|u32
id|biosbase
suffix:semicolon
id|u32
id|fbbase
suffix:semicolon
r_struct
id|pci_dev
op_star
id|pdev
op_assign
id|ACCESS_FBINFO
c_func
(paren
id|pcidev
)paren
suffix:semicolon
id|memset
c_func
(paren
op_amp
id|ACCESS_FBINFO
c_func
(paren
id|bios
)paren
comma
l_int|0
comma
r_sizeof
(paren
id|ACCESS_FBINFO
c_func
(paren
id|bios
)paren
)paren
)paren
suffix:semicolon
id|pci_read_config_dword
c_func
(paren
id|pdev
comma
id|PCI_OPTION_REG
comma
op_amp
id|opt
)paren
suffix:semicolon
id|pci_write_config_dword
c_func
(paren
id|pdev
comma
id|PCI_OPTION_REG
comma
id|opt
op_or
id|PCI_OPTION_ENABLE_ROM
)paren
suffix:semicolon
id|pci_read_config_dword
c_func
(paren
id|pdev
comma
id|PCI_ROM_ADDRESS
comma
op_amp
id|biosbase
)paren
suffix:semicolon
id|pci_read_config_dword
c_func
(paren
id|pdev
comma
id|ACCESS_FBINFO
c_func
(paren
id|devflags.fbResource
)paren
comma
op_amp
id|fbbase
)paren
suffix:semicolon
id|pci_write_config_dword
c_func
(paren
id|pdev
comma
id|PCI_ROM_ADDRESS
comma
(paren
id|fbbase
op_amp
id|PCI_ROM_ADDRESS_MASK
)paren
op_or
id|PCI_ROM_ADDRESS_ENABLE
)paren
suffix:semicolon
id|parse_bios
c_func
(paren
id|vaddr_va
c_func
(paren
id|ACCESS_FBINFO
c_func
(paren
id|video
)paren
dot
id|vbase
)paren
comma
op_amp
id|ACCESS_FBINFO
c_func
(paren
id|bios
)paren
)paren
suffix:semicolon
id|pci_write_config_dword
c_func
(paren
id|pdev
comma
id|PCI_ROM_ADDRESS
comma
id|biosbase
)paren
suffix:semicolon
id|pci_write_config_dword
c_func
(paren
id|pdev
comma
id|PCI_OPTION_REG
comma
id|opt
)paren
suffix:semicolon
macro_line|#ifdef CONFIG_X86
r_if
c_cond
(paren
op_logical_neg
id|ACCESS_FBINFO
c_func
(paren
id|bios
)paren
dot
id|bios_valid
)paren
(brace
r_int
r_char
op_star
id|b
suffix:semicolon
id|b
op_assign
id|ioremap
c_func
(paren
l_int|0x000C0000
comma
l_int|65536
)paren
suffix:semicolon
r_if
c_cond
(paren
op_logical_neg
id|b
)paren
(brace
id|printk
c_func
(paren
id|KERN_INFO
l_string|&quot;matroxfb: Unable to map legacy BIOS&bslash;n&quot;
)paren
suffix:semicolon
)brace
r_else
(brace
r_int
r_int
id|ven
op_assign
id|readb
c_func
(paren
id|b
op_plus
l_int|0x64
op_plus
l_int|0
)paren
op_or
(paren
id|readb
c_func
(paren
id|b
op_plus
l_int|0x64
op_plus
l_int|1
)paren
op_lshift
l_int|8
)paren
suffix:semicolon
r_int
r_int
id|dev
op_assign
id|readb
c_func
(paren
id|b
op_plus
l_int|0x64
op_plus
l_int|2
)paren
op_or
(paren
id|readb
c_func
(paren
id|b
op_plus
l_int|0x64
op_plus
l_int|3
)paren
op_lshift
l_int|8
)paren
suffix:semicolon
r_if
c_cond
(paren
id|ven
op_ne
id|pdev-&gt;vendor
op_logical_or
id|dev
op_ne
id|pdev-&gt;device
)paren
(brace
id|printk
c_func
(paren
id|KERN_INFO
l_string|&quot;matroxfb: Legacy BIOS is for %04X:%04X, while this device is %04X:%04X&bslash;n&quot;
comma
id|ven
comma
id|dev
comma
id|pdev-&gt;vendor
comma
id|pdev-&gt;device
)paren
suffix:semicolon
)brace
r_else
(brace
id|parse_bios
c_func
(paren
id|b
comma
op_amp
id|ACCESS_FBINFO
c_func
(paren
id|bios
)paren
)paren
suffix:semicolon
)brace
id|iounmap
c_func
(paren
id|b
)paren
suffix:semicolon
)brace
)brace
macro_line|#endif
id|matroxfb_set_limits
c_func
(paren
id|PMINFO
op_amp
id|ACCESS_FBINFO
c_func
(paren
id|bios
)paren
)paren
suffix:semicolon
)brace
DECL|variable|matroxfb_DAC_in
id|EXPORT_SYMBOL
c_func
(paren
id|matroxfb_DAC_in
)paren
suffix:semicolon
DECL|variable|matroxfb_DAC_out
id|EXPORT_SYMBOL
c_func
(paren
id|matroxfb_DAC_out
)paren
suffix:semicolon
DECL|variable|matroxfb_var2my
id|EXPORT_SYMBOL
c_func
(paren
id|matroxfb_var2my
)paren
suffix:semicolon
DECL|variable|matroxfb_PLL_calcclock
id|EXPORT_SYMBOL
c_func
(paren
id|matroxfb_PLL_calcclock
)paren
suffix:semicolon
macro_line|#ifndef CONFIG_FB_MATROX_MULTIHEAD
DECL|variable|matroxfb_global_mxinfo
r_struct
id|matrox_fb_info
id|matroxfb_global_mxinfo
suffix:semicolon
DECL|variable|matroxfb_global_mxinfo
id|EXPORT_SYMBOL
c_func
(paren
id|matroxfb_global_mxinfo
)paren
suffix:semicolon
macro_line|#endif
DECL|variable|matroxfb_vgaHWinit
id|EXPORT_SYMBOL
c_func
(paren
id|matroxfb_vgaHWinit
)paren
suffix:semicolon
multiline_comment|/* DAC1064, Ti3026 */
DECL|variable|matroxfb_vgaHWrestore
id|EXPORT_SYMBOL
c_func
(paren
id|matroxfb_vgaHWrestore
)paren
suffix:semicolon
multiline_comment|/* DAC1064, Ti3026 */
DECL|variable|matroxfb_read_pins
id|EXPORT_SYMBOL
c_func
(paren
id|matroxfb_read_pins
)paren
suffix:semicolon
id|MODULE_AUTHOR
c_func
(paren
l_string|&quot;(c) 1999-2002 Petr Vandrovec &lt;vandrove@vc.cvut.cz&gt;&quot;
)paren
suffix:semicolon
id|MODULE_DESCRIPTION
c_func
(paren
l_string|&quot;Miscellaneous support for Matrox video cards&quot;
)paren
suffix:semicolon
id|MODULE_LICENSE
c_func
(paren
l_string|&quot;GPL&quot;
)paren
suffix:semicolon
eof
