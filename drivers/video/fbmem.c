...
multiline_comment|/*&n; * Three (3) kinds of logo maps exist.  linux_logo (&gt;16 colors), linux_logo_16&n; * (16 colors) and linux_logo_bw (2 colors).  Depending on the visual format and&n; * color depth of the framebuffer, the DAC, the pseudo_palette, and the logo data&n; * will be adjusted accordingly.&n; *&n; * Case 1 - linux_logo:&n; * Color exceeds the number of console colors (16), thus we set the hardware DAC&n; * using fb_set_cmap() appropriately.  The &quot;needs_cmapreset&quot;  flag will be set.&n; *&n; * For visuals that require color info from the pseudo_palette, we also construct&n; * one for temporary use. The &quot;needs_directpalette&quot; or &quot;needs_truepalette&quot; flags&n; * will be set.&n; *&n; * Case 2 - linux_logo_16:&n; * The number of colors just matches the console colors, thus there is no need&n; * to set the DAC or the pseudo_palette.  However, the bitmap is packed, ie,&n; * each byte contains color information for two pixels (upper and lower nibble).&n; * To be consistent with fb_imageblit() usage, we therefore separate the two&n; * nibbles into separate bytes. The &quot;needs_logo&quot; flag will be set to 4.&n; *&n; * Case 3 - linux_logo_bw:&n; * This is similar with Case 2.  Each byte contains information for 8 pixels.&n; * We isolate each bit and expand each into a byte. The &quot;needs_logo&quot; flag will&n; * be set to 1.&n; */
...
id|FB_VISUAL_MONO01
suffix:colon
multiline_comment|/* reversed 0 = fg, 1 = bg */
id|needs_logo
op_assign
op_complement
l_int|1
