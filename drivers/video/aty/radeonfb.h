...
multiline_comment|/* This function is required to workaround a hardware bug in some (all?)&n; * revisions of the R300.  This workaround should be called after every&n; * CLOCK_CNTL_INDEX register access.  If not, register reads afterward&n; * may not be correct.&n; */
...
multiline_comment|/*&n; * PLL accesses suffer from various HW issues on the different chip&n; * families. Some R300&squot;s need the above workaround, rv200 &amp; friends&n; * need a couple of dummy reads after any write of CLOCK_CNTL_INDEX,&n; * and some RS100/200 need a dummy read before writing to&n; * CLOCK_CNTL_INDEX as well. Instead of testing every chip revision,&n; * we just unconditionally do  the workarounds at once since PLL&n; * accesses are far from beeing performance critical. Except for R300&n; * one which stays separate for now&n; */
DECL|function|radeon_pll_workaround_before
r_static
r_inline
r_void
...
DECL|function|radeon_pll_workaround_after
r_static
r_inline
r_void
