...
multiline_comment|/*&n;&t; * We must always set the mode. The mode of the previous console&n;&t; * driver could be in the same resolution but we are using different&n;&t; * hardware so we have to initialize the hardware.&n;&t; */
r_if
c_cond
(paren
...
multiline_comment|/* okay, we&squot;ll use software version of accel funcs... */
id|p-&gt;scrollmode
op_assign
id|SCROLL_ACCEL
suffix:semicolon
...
suffix:semicolon
DECL|variable|fbcon_event_notifer
r_static
r_struct
id|notifier_block
id|fbcon_event_notifer
op_assign
(brace
dot
id|notifier_call
