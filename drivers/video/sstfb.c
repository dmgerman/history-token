...
multiline_comment|/*&n; * misc notes, TODOs, toASKs, and deep thoughts&n;&n;-TODO: at one time or another test that the mode is acceptable by the monitor&n;-ASK: Can I choose different ordering for the color bitfields (rgba argb ...)&n;      wich one should i use ? is there any preferred one ? It seems ARGB is&n;      the one ...&n;-TODO: in  set_var check the validity of timings (hsync vsync)...&n;-TODO: check and recheck the use of sst_wait_idle : we dont flush the fifo via&n;       a nop command. so it&squot;s ok as long as the commands we pass don&squot;t go&n;       through the fifo. warning: issuing a nop command seems to need pci_fifo&n;-FIXME: in case of failure in the init sequence, be sure we return to a safe&n;        state.&n;-FIXME: 4MB boards have banked memory (FbiInit2 bits 1 &amp; 20)&n; */
