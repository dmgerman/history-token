...
multiline_comment|/*&n; * misc notes, TODOs, toASKs, and deep thoughts&n;&n;-TODO: at one time or another test that the mode is acceptable by the monitor&n;-ASK: I can choose different ordering for the color bitfields (rgba argb ...)&n;      wich one should i use ? is there any preferred one ? It seems ARGB is&n;      the one ...&n;-ASK: later: how to cope with endianness ? the fbi chip has builtin functions&n;      to do byte swizling /swapping, maybe use that ...&n;-TODO: check the error paths . if something get wrong, the error doesn&squot;t seem&n;      to be very well handled...if handled at all.. not good.&n;-TODO: in  set_var check the validity of timings (hsync vsync)...&n;-TODO: check and recheck the use of sst_wait_idle : we dont flush the fifo via&n;       a nop command . so it&squot;s ok as long as the commands we pass don&squot;t go&n;       through the fifo. warning: issuing a nop command seems to need pci_fifo&n;-FIXME: in case of failure in the init sequence, be sure we return to a safe&n;        state.&n;-FIXME: 4MB boards have banked memory (FbiInit2 bits 1 &amp; 20)&n;-ASK: I stole &quot;inverse&quot; but seems it doesn&squot;t work... check what it realy does...&n;-TODO: change struct sst_info fb_info from static to array/dynamic&n;&n; *&n; */
...
mdefine_line|#define SST_DEBUG
singleline_comment|//#undef SST_DEBUG
...
mdefine_line|#define Dump_regs __Dump_regs(sst_info)  
...
mdefine_line|#define sst_wait_idle()&t; __sst_wait_idle(sst_info-&gt;mmio.vbase)
r_static
r_int
...
mdefine_line|#define IS_VOODOO2(info) ((info)-&gt;type == ID_VOODOO2 )
...
DECL|macro|sst_read
mdefine_line|#define sst_read(reg)&t;&t;__sst_read(sst_info-&gt;mmio.vbase, reg)
DECL|macro|sst_write
mdefine_line|#define sst_write(reg,val)&t;__sst_write(sst_info-&gt;mmio.vbase, reg, val)
DECL|macro|sst_set_bits
mdefine_line|#define sst_set_bits(reg,val)&t;__sst_set_bits(sst_info-&gt;mmio.vbase, reg, val)
DECL|macro|sst_unset_bits
mdefine_line|#define sst_unset_bits(reg,val)&t;__sst_unset_bits(sst_info-&gt;mmio.vbase, reg, val)
DECL|macro|sst_dac_read
mdefine_line|#define sst_dac_read(reg)&t;__sst_dac_read(sst_info-&gt;mmio.vbase, reg)
DECL|macro|sst_dac_write
mdefine_line|#define sst_dac_write(reg,val)&t;__sst_dac_write(sst_info-&gt;mmio.vbase, reg, val)
DECL|macro|dac_i_read
mdefine_line|#define dac_i_read(reg)&t;&t;__dac_i_read(sst_info-&gt;mmio.vbase, reg)
DECL|macro|dac_i_write
mdefine_line|#define dac_i_write(reg,val)&t;__dac_i_write(sst_info-&gt;mmio.vbase, reg, val)
...
c_func
(paren
l_string|&quot;Pixclock %d out of range&bslash;n&quot;
comma
id|var-&gt;pixclock
)paren
suffix:semicolon
...
id|par-&gt;bpp
suffix:semicolon
multiline_comment|/* {x|y}offset = 0 */
id|var-&gt;height
op_assign
op_minus
l_int|1
...
l_int|1
suffix:semicolon
singleline_comment|//XXX  do something to avoid hanging the machine if the voodoo is out
)brace
)brace
...
(brace
singleline_comment|//XXX
singleline_comment|//XXX&t;while ((this_opt = strsep(&amp;options, &quot;,&quot;)) != NULL) {
r_if
c_cond
(paren
op_logical_neg
...
comma
op_amp
id|sst_info-&gt;revision
)paren
suffix:semicolon
id|sst_info-&gt;mmio.base
op_assign
...
suffix:semicolon
id|sst_info-&gt;info.node
op_assign
op_minus
l_int|1
suffix:semicolon
id|sst_info-&gt;info.flags
op_assign
id|FBINFO_FLAG_DEFAULT
suffix:semicolon
id|sst_info-&gt;info.fbops
op_assign
op_amp
id|sstfb_ops
suffix:semicolon
id|sst_info-&gt;info.disp
op_assign
op_amp
id|sst_info-&gt;disp
suffix:semicolon
...
l_int|NULL
suffix:semicolon
id|sst_info-&gt;info.switch_con
op_assign
op_amp
id|sstfbcon_switch
suffix:semicolon
id|sst_info-&gt;info.updatevar
op_assign
op_amp
id|sstfbcon_updatevar
suffix:semicolon
id|sst_info-&gt;info.blank
op_assign
op_amp
id|sstfbcon_blank
suffix:semicolon
...
r_else
(brace
multiline_comment|/* set the new default */
id|sstfb_default
op_assign
id|var
suffix:semicolon
...
l_int|1
suffix:semicolon
multiline_comment|/* no need to set the mode. */
)brace
)brace
r_if
c_cond
