...
DECL|macro|pm_access
mdefine_line|#define pm_access(pm)&t;&t;do { } while (0)
DECL|macro|pm_unregister
mdefine_line|#define pm_unregister(pm)&t;do { } while (0)
macro_line|#endif
...
DECL|function|uart_update_altspeed
r_static
r_inline
r_void
id|uart_update_altspeed
c_func
(paren
r_struct
id|uart_info
...
multiline_comment|/*&n; * Startup the port.  This will be called once per open.  All calls&n; * will be serialised by the global port semaphore.&n; */
...
multiline_comment|/*&n; * This routine will shutdown a serial port; interrupts are disabled, and&n; * DTR is dropped if the hangup on close termio flag is on.  Calls to&n; * uart_shutdown are serialised by port_sem.&n; */
...
id|quot
suffix:semicolon
multiline_comment|/* Special case: B0 rate */
r_if
c_cond
(paren
id|baud
...
l_int|9600
suffix:semicolon
multiline_comment|/* Old HI/VHI/custom speed handling */
r_if
c_cond
(paren
id|baud
...
multiline_comment|/*&n; * uart_get_lsr_info - get line status register info&n; */
...
DECL|function|uart_get_modem_info
r_static
r_int
id|uart_get_modem_info
c_func
(paren
r_struct
id|uart_port
...
r_static
r_int
DECL|function|uart_set_modem_info
id|uart_set_modem_info
c_func
(paren
r_struct
id|uart_port
...
id|EPERM
suffix:semicolon
multiline_comment|/*&n;&t; * Take the &squot;count&squot; lock.  This prevents count&n;&t; * from incrementing, and hence any extra opens&n;&t; * of the port while we&squot;re auto-configging.&n;&t; */
r_if
c_cond
(paren
id|down_interruptible
...
c_func
(paren
l_string|&quot;uart_close() called&bslash;n&quot;
)paren
suffix:semicolon
multiline_comment|/*&n;&t; * This is safe, as long as the BKL exists in&n;&t; * do_tty_hangup(), and we&squot;re protected by the BKL.&n;&t; */
r_if
c_cond
(paren
id|tty_hung_up_p
...
multiline_comment|/*&n;&t; * The UIF_CLOSING flag protects us against further opens&n;&t; * of this port.&n;&t; */
id|info-&gt;flags
op_or_assign
id|UIF_CLOSING
suffix:semicolon
...
multiline_comment|/*&n;&t;&t; * Put device into D3 state.&n;&t;&t; */
id|pm_send
c_func
(paren
id|info-&gt;state-&gt;pm
...
r_break
suffix:semicolon
multiline_comment|/*&n;&t;&t; * If the device is in the middle of being closed, block&n;&t;&t; * until it&squot;s done.  We will need to re-initialise the&n;&t;&t; * port.  Hmm, is it legal to block a non-blocking open?&n;&t;&t; */
r_if
c_cond
(paren
id|info-&gt;flags
...
multiline_comment|/*&n;&t; * Start up the serial port.  We have this semaphore here to&n;&t; * prevent uart_startup or uart_shutdown being re-entered if&n;&t; * we sleep while requesting an IRQ.&n;&t; */
id|down
c_func
(paren
op_amp
...
id|fail
suffix:semicolon
multiline_comment|/*&n;&t; * Wait until the port is ready.&n;&t; */
id|retval
op_assign
id|uart_block_til_ready
c_func
...
suffix:semicolon
multiline_comment|/*&n; * Central &quot;initialise all serial consoles&quot; container.  Needs to be killed.&n; */
DECL|function|uart_console_init
r_void
id|__init
...
macro_line|#ifdef CONFIG_PM
multiline_comment|/*&n; *  Serial port power management.&n; *&n; * This is pretty coarse at the moment - either all on or all off.  We&n; * should probably some day do finer power management here some day.&n; *&n; * We don&squot;t actually save any state; the serial driver already has the&n; * state held internally to re-setup the port when we come out of D3.&n; */
DECL|function|uart_pm_set_state
r_static
r_int
id|uart_pm_set_state
c_func
(paren
r_struct
id|uart_state
...
c_func
(paren
l_string|&quot;pm: %08x: %d -&gt; %d, %srunning&bslash;n&quot;
comma
id|port-&gt;iobase
comma
id|dev-&gt;state
...
(brace
multiline_comment|/*&n;&t;&t;&t; * The port lock isn&squot;t taken here -&n;&t;&t;&t; * the port isn&squot;t initialised.&n;&t;&t;&t; */
id|ops
op_member_access_from_pointer
id|set_mctrl
c_func
...
multiline_comment|/*&n; *  Wakeup support.&n; */
DECL|function|uart_pm_set_wakeup
r_static
r_int
id|uart_pm_set_wakeup
c_func
(paren
r_struct
id|uart_state
...
suffix:semicolon
macro_line|#ifdef CONFIG_PM
multiline_comment|/*&n;&t;&t; * Power down all ports by default, except the&n;&t;&t; * console if we have one.  We need to drop the&n;&t;&t; * port semaphore here.&n;&t;&t; */
r_if
c_cond
(paren
id|state-&gt;pm
...
multiline_comment|/*&n; *&t;Hangup the port.  This must be done outside the port_sem&n; *&t;since uart_hangup() grabs this same semaphore.  Grr.&n; */
r_static
r_void
DECL|function|__uart_hangup_port
id|__uart_hangup_port
c_func
(paren
r_struct
id|uart_driver
...
multiline_comment|/*&n; * This reverses the affects of __uart_register_port.&n; */
r_static
r_void
...
id|uart_put_char
suffix:semicolon
id|normal-&gt;flush_chars
op_assign
id|uart_flush_chars
suffix:semicolon
id|normal-&gt;write_room
op_assign
id|uart_write_room
suffix:semicolon
id|normal-&gt;chars_in_buffer
op_assign
id|uart_chars_in_buffer
suffix:semicolon
id|normal-&gt;flush_buffer
op_assign
id|uart_flush_buffer
suffix:semicolon
id|normal-&gt;ioctl
...
id|uart_throttle
suffix:semicolon
id|normal-&gt;unthrottle
op_assign
id|uart_unthrottle
suffix:semicolon
id|normal-&gt;send_xchar
op_assign
id|uart_send_xchar
suffix:semicolon
id|normal-&gt;set_termios
op_assign
id|uart_set_termios
suffix:semicolon
id|normal-&gt;stop
