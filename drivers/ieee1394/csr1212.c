...
multiline_comment|/* The keyval key id is not used for the root node, but a valid key id&n;&t; * that can be used for a directory needs to be passed to&n;&t; * csr1212_new_directory(). */
id|csr-&gt;root_kv
op_assign
...
l_int|1
suffix:semicolon
id|cache-&gt;ext_rom-&gt;value.leaf.data
op_assign
id|cache-&gt;data
suffix:semicolon
...
id|dkv-&gt;prev
suffix:semicolon
singleline_comment|//if (dkv == *layout_tail)
singleline_comment|//&t;*layout_tail = dkv-&gt;prev;
...
(brace
multiline_comment|/* Set the length and CRC of the extended ROM. */
r_struct
id|csr1212_keyval_img
op_star
id|kvi
...
suffix:semicolon
macro_line|#if 0
multiline_comment|/* Apparently there are too many differnt wrong implementations of the&n;&t; * CRC algorithm that verifying them is moot. */
r_if
c_cond
(paren
(paren
...
suffix:semicolon
multiline_comment|/* Some devices put null entries in their unit&n;&t;&t;&t; * directories.  If we come across such an entry,&n;&t;&t;&t; * then skip it. */
r_if
c_cond
(paren
id|ki
...
id|q
suffix:semicolon
multiline_comment|/* Don&squot;t read the entire extended ROM now.  Pieces of it will&n;&t;&t; * be read when entries inside it are read. */
r_return
id|csr1212_parse_keyval
c_func
(paren
...
suffix:semicolon
multiline_comment|/* Scan through the Root directory finding all extended ROM regions&n;&t; * and make cache regions for them */
r_for
c_loop
(paren
id|dentry
op_assign
id|csr-&gt;root_kv-&gt;value.directory.dentries_head
suffix:semicolon
id|dentry
suffix:semicolon
id|dentry
