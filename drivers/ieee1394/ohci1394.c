...
suffix:semicolon
multiline_comment|/* Initialize IR Legacy DMA */
...
id|KERN_ERR
comma
l_string|&quot;pci_pool_create failed for %s&quot;
comma
id|pool_name
)paren
suffix:semicolon
...
suffix:semicolon
multiline_comment|/* Allocate the IR DMA context right here so we don&squot;t have&n;&t; * to do it in interrupt path - note that this doesn&squot;t&n;&t; * waste much memory and avoids the jugglery required to&n;&t; * allocate it in IRQ path. */
r_if
c_cond
(paren
id|alloc_dma_rcv_ctx
...
id|ENOMEM
comma
l_string|&quot;Cannot allocate IR Legacy DMA context&quot;
)paren
suffix:semicolon
)brace
multiline_comment|/* We hopefully don&squot;t have to pre-allocate IT DMA like we did&n;&t; * for IR DMA above. Allocate it on-demand and mark inactive. */
id|ohci-&gt;it_legacy_context.ohci
op_assign
l_int|NULL
suffix:semicolon
...
suffix:semicolon
multiline_comment|/* Free IR legacy dma */
id|free_dma_rcv_ctx
c_func
(paren
op_amp
