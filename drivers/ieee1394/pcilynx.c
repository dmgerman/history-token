...
multiline_comment|/*&n; * Contributions:&n; *&n; * Manfred Weihs &lt;weihs@ict.tuwien.ac.at&gt;&n; *        reading bus info block (containing GUID) from serial &n; *            eeprom via i2c and storing it in config ROM&n; *        Reworked code for initiating bus resets&n; *            (long, short, with or without hold-off)&n; */
...
multiline_comment|/* Check for EOF before we trust wrap */
r_return
l_int|0
suffix:semicolon
...
suffix:semicolon
r_int
id|got_valid_bus_info_block
op_assign
l_int|0
suffix:semicolon
multiline_comment|/* set to 1, if we were able to get a valid bus info block from serial eeprom */
id|error
op_assign
op_minus
id|ENXIO
...
id|lynx-&gt;id
comma
l_string|&quot;since we did not get a bus info block from serial eeprom, we use a generic one with a hard coded GUID&quot;
)paren
suffix:semicolon
id|memcpy
c_func
