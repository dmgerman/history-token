...
id|EBUSY
suffix:semicolon
multiline_comment|/* Not going to allow setting the MAC address, we really need to use&n;&t; * the real one suppliled by the hardware */
r_return
op_minus
id|EINVAL
suffix:semicolon
...
suffix:semicolon
multiline_comment|/* This check should be unnecessary, but we&squot;ll keep it for safety for&n;&t; * a while longer. */
r_if
c_cond
(paren
id|max_payload
...
id|dev-&gt;name
comma
l_string|&quot;max_payload too small: %d   (setting to 512)&bslash;n&quot;
comma
id|max_payload
)paren
suffix:semicolon
...
id|addr
suffix:semicolon
multiline_comment|/* This test is just temporary until ConfigROM support has&n;&t;&t; * been added to eth1394.  Until then, we need an ARP packet&n;&t;&t; * after a bus reset from the current destination node so that&n;&t;&t; * we can get FIFO information. */
r_if
c_cond
(paren
id|priv-&gt;fifo
