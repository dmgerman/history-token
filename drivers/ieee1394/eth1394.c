...
multiline_comment|/* This driver intends to support RFC 2734, which describes a method for&n; * transporting IPv4 datagrams over IEEE-1394 serial busses. This driver&n; * will ultimately support that method, but currently falls short in&n; * several areas.&n; *&n; * TODO:&n; * RFC 2734 related:&n; * - Add support for broadcast messages&n; * - Use EUI instead of node id in internal ARP tables&n; * - Add Config ROM entry&n; * - Add MCAP and multicast&n; *&n; * Non-RFC 2734 related:&n; * - Move generic GASP reception to core 1394 code&n; * - Convert kmalloc/kfree for link fragments to use kmem_cache_* instead&n; * - Stability improvements&n; * - Performance enhancements&n; * - Change hardcoded 1394 bus address region to a dynamic memory space allocation&n; * - Consider garbage collecting old partial datagrams after X amount of time&n; */
...
suffix:semicolon
multiline_comment|/* The max_partial_datagrams parameter is the maximum number of fragmented datagrams&n; * per node that eth1394 will keep in memory.  Providing an upper bound allows us to&n; * limit the amount of memory that partial datagrams consume in the event that some&n; * partial datagrams are never completed.  This should probably change to a sysctl&n; * item or the like if possible.&n; */
id|MODULE_PARM
c_func
(paren
...
id|max_partial_datagrams
comma
l_string|&quot;Maximum number of partially received fragmented datagrams (default = 25).&quot;
)paren
suffix:semicolon
...
id|dev-&gt;priv
suffix:semicolon
multiline_comment|/* First time sending?  Need a broadcast channel for ARP and for&n;&t; * listening on */
r_if
c_cond
(paren
id|priv-&gt;bc_state
...
id|bc
suffix:semicolon
multiline_comment|/* Get the local copy of the broadcast channel and check its&n;&t;&t; * validity (the IRM should validate it for us) */
id|bc
op_assign
...
(brace
multiline_comment|/* broadcast channel not validated yet */
id|ETH1394_PRINT
c_func
(paren
id|KERN_WARNING
...
id|dev-&gt;name
comma
l_string|&quot;Error BROADCAST_CHANNEL register valid &quot;
...
id|dev-&gt;name
comma
l_string|&quot;Changing to broadcast channel %d...&bslash;n&quot;
comma
id|priv-&gt;broadcast_channel
)paren
suffix:semicolon
...
id|dev-&gt;name
comma
l_string|&quot;failed to change broadcast &quot;
...
id|dev-&gt;name
comma
l_string|&quot;Cannot send packet, no broadcast channel available.&bslash;n&quot;
)paren
suffix:semicolon
id|ret
op_assign
...
multiline_comment|/* Set the transmission type for the packet.  Right now only ARP&n;&t; * packets are sent via GASP.  IP broadcast and IP multicast are not&n;&t; * yet supported properly, they too should use GASP. */
r_switch
c_cond
(paren
id|proto
