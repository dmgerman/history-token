...
suffix:semicolon
multiline_comment|/*&n; *&t;initialize_SCp&t;&t;-&t;init the scsi pointer field&n; *&t;@cmd: command block to set up&n; *&n; *&t;Set up the internal fields in the SCSI command.&n; */
...
multiline_comment|/**&n; *&t;NCR5380_poll_politely&t;-&t;wait for NCR5380 status bits&n; *&t;@instance: controller to poll&n; *&t;@reg: 5380 register to poll&n; *&t;@bit: Bitmask to check&n; *&t;@val: Value required to exit&n; *&n; *&t;Polls the NCR5380 in a reasonably efficient manner waiting for&n; *&t;an event to occur, after a short quick poll we begin giving the&n; *&t;CPU back in non IRQ contexts&n; *&n; *&t;Returns the value of the register or a negative error code.&n; */
DECL|function|NCR5380_poll_politely
r_static
r_int
id|NCR5380_poll_politely
c_func
(paren
r_struct
id|Scsi_Host
...
l_int|500
suffix:semicolon
multiline_comment|/* At about 8uS a cycle for the cpu access */
r_int
r_int
id|end
op_assign
...
suffix:semicolon
multiline_comment|/**&n; *&t;NCR5380_print&t;-&t;print scsi bus signals&n; *&t;@instance:&t;adapter state to dump&n; *&n; *&t;Print the SCSI bus signals for debugging purposes&n; *&n; *&t;Locks: caller holds hostdata lock (not essential)&n; */
...
multiline_comment|/**&n; *&t;NCR58380_print_options&t;-&t;show options&n; *&t;@instance: unused for now&n; *&n; *&t;Called by probe code indicating the NCR5380 driver options that &n; *&t;were selected. At some point this will switch to runtime options&n; *&t;read from the adapter in question&n; *&n; *&t;Locks: none&n; */
...
(paren
id|KERN_ERR
l_string|&quot;NCR5380_init called with interrupts off!&bslash;n&quot;
)paren
suffix:semicolon
)brace
...
multiline_comment|/**&n; *&t;NCR5380_exit&t;-&t;remove an NCR5380&n; *&t;@instance: adapter to remove&n; */
DECL|function|NCR5380_exit
r_static
r_void
id|__devexit
id|NCR5380_exit
...
multiline_comment|/* lock held here still */
)brace
multiline_comment|/* if target/lun is not busy */
)brace
multiline_comment|/* for */
multiline_comment|/* exited locked */
)brace
...
id|hostdata-&gt;selecting
suffix:semicolon
multiline_comment|/* Selection will drop and retake the lock */
r_if
c_cond
(paren
op_logical_neg
...
suffix:semicolon
multiline_comment|/* FIXME: we need to poll briefly then defer a workqueue task ! */
id|NCR5380_poll_politely
c_func
(paren
id|hostdata
...
suffix:semicolon
multiline_comment|/* We can be relaxed here, interrupts are on, we are&n;&t;   in workqueue context, the birds are singing in the trees */
id|spin_unlock_irq
c_func
(paren
id|instance-&gt;host_lock
...
op_logical_and
multiline_comment|/* RvC: DTC3181E has some trouble with this&n;&t;     *      so we simply removed it. Seems to work with&n;&t;     *      only Mustek scanner attached&n;&t;     */
(paren
id|NCR5380_read
c_func
(paren
...
id|part2
suffix:colon
multiline_comment|/* RvC: here we enter after a sleeping period, or immediately after&n;&t;   execution of part 1&n;&t;   we poll only once ech clock tick */
id|value
op_assign
id|NCR5380_read
c_func
...
l_int|0
suffix:semicolon
multiline_comment|/* Selection failed */
id|failed
suffix:colon
r_return
op_minus
...
id|data
suffix:semicolon
multiline_comment|/*&n;&t; *      RvC: some administrative data to process polling time&n;&t; */
r_int
id|break_allowed
op_assign
l_int|0
...
suffix:semicolon
multiline_comment|/* RvC: don&squot;t know if this is necessary, but other SCSI I/O is short&n;&t; *      so breaks are not necessary there&n;&t; */
r_if
c_cond
(paren
(paren
...
multiline_comment|/* RvC: we simply poll once, after that we stop temporarily&n;&t;&t; *      and let the device buffer fill up&n;&t;&t; *      if breaking is not allowed, we keep polling as long as needed&n;&t;&t; */
...
multiline_comment|/* FIXME - if this fails bus reset ?? */
id|NCR5380_poll_politely
c_func
(paren
id|instance
...
multiline_comment|/*&n; * Function : do_abort (Scsi_Host *host)&n; * &n; * Purpose : abort the currently established nexus.  Should only be &n; *      called from a routine which can drop into a &n; * &n; * Returns : 0 on success, -1 on failure.&n; *&n; * Locks: queue lock held by caller&n; *&t;FIXME: sort this out and get new_eh running&n; */
...
suffix:semicolon
multiline_comment|/* &n;&t; *&t;On the PAS16 at least I/O recovery delays are not needed here.&n;&t; *&t;Everyone else seems to want them.&n;&t; */
r_if
c_cond
(paren
id|p
...
(brace
multiline_comment|/*&n;&t;&t;&t; * We can&squot;t disable DMA mode after successfully transferring &n;&t;&t;&t; * what we plan to be the last byte, since that would open up&n;&t;&t;&t; * a race condition where if the target asserted REQ before &n;&t;&t;&t; * we got the DMA mode reset, the NCR5380 would have latched&n;&t;&t;&t; * an additional byte into the INPUT DATA register and we&squot;d&n;&t;&t;&t; * have dropped it.&n;&t;&t;&t; * &n;&t;&t;&t; * The workaround was to transfer one fewer bytes than we &n;&t;&t;&t; * intended to with the pseudo-DMA read function, wait for &n;&t;&t;&t; * the chip to latch the last byte, read it, and then disable&n;&t;&t;&t; * pseudo-DMA mode.&n;&t;&t;&t; * &n;&t;&t;&t; * After REQ is asserted, the NCR5380 asserts DRQ and ACK.&n;&t;&t;&t; * REQ is deasserted when ACK is asserted, and not reasserted&n;&t;&t;&t; * until ACK goes false.  Since the NCR5380 won&squot;t lower ACK&n;&t;&t;&t; * until DACK is asserted, which won&squot;t happen unless we twiddle&n;&t;&t;&t; * the DMA port or we take the NCR5380 out of DMA mode, we &n;&t;&t;&t; * can guarantee that we won&squot;t handshake another extra &n;&t;&t;&t; * byte.&n;&t;&t;&t; */
r_if
c_cond
(paren
op_logical_neg
...
(brace
multiline_comment|/*&n;&t;&t;&t; * Wait for the last byte to be sent.  If REQ is being asserted for &n;&t;&t;&t; * the byte we&squot;re interested, we&squot;ll ACK it and it will go false.  &n;&t;&t;&t; */
r_if
c_cond
(paren
op_logical_neg
...
suffix:semicolon
multiline_comment|/* Wait for bus free to avoid nasty timeouts - FIXME timeout !*/
multiline_comment|/* NCR538_poll_politely(instance, STATUS_REG, SR_BSY, 0, 30 * HZ); */
r_while
c_loop
(paren
(paren
...
multiline_comment|/*&n; * Function : void NCR5380_reselect (struct Scsi_Host *instance)&n; *&n; * Purpose : does reselection, initializing the instance-&gt;connected &n; *      field to point to the Scsi_Cmnd for which the I_T_L or I_T_L_Q &n; *      nexus has been reestablished,&n; *      &n; * Inputs : instance - this instance of the NCR5380.&n; *&n; * Locks: io_request_lock held by caller if IRQ driven&n; */
...
suffix:semicolon
multiline_comment|/* FIXME: timeout too long, must fail to workqueue */
r_if
c_cond
(paren
id|NCR5380_poll_politely
...
suffix:semicolon
multiline_comment|/*&n;&t; * Wait for target to go into MSGIN.&n;&t; * FIXME: timeout needed and fail to work queeu&n;&t; */
r_if
c_cond
(paren
id|NCR5380_poll_politely
...
suffix:semicolon
multiline_comment|/*&n;&t; * XXX this might not be right.&n;&t; *&n;&t; * Wait for final byte to transfer, ie wait for ACK to go false.&n;&t; *&n;&t; * We should use the Last Byte Sent bit, unfortunately this is &n;&t; * not available on the 5380/5381 (only the various CMOS chips)&n;&t; *&n;&t; * FIXME: timeout, and need to handle long timeout/irq case&n;&t; */
id|NCR5380_poll_politely
c_func
(paren
id|instance
...
multiline_comment|/* &n; * Function : int NCR5380_bus_reset (Scsi_Cmnd *cmd)&n; * &n; * Purpose : reset the SCSI bus.&n; *&n; * Returns : SUCCESS&n; *&n; * Locks: host lock taken by caller&n; */
...
multiline_comment|/* &n; * Function : int NCR5380_device_reset (Scsi_Cmnd *cmd)&n; * &n; * Purpose : reset a SCSI device&n; *&n; * Returns : FAILED&n; *&n; * Locks: io_request_lock held by caller&n; */
DECL|function|NCR5380_device_reset
r_static
r_int
...
multiline_comment|/* &n; * Function : int NCR5380_host_reset (Scsi_Cmnd *cmd)&n; * &n; * Purpose : reset a SCSI device&n; *&n; * Returns : FAILED&n; *&n; * Locks: io_request_lock held by caller&n; */
DECL|function|NCR5380_host_reset
r_static
r_int
