...
(paren
op_amp
id|scmd-&gt;host-&gt;host_active
)paren
comma
id|scmd-&gt;host-&gt;host_busy
comma
id|scmd-&gt;host-&gt;host_failed
)paren
)paren
suffix:semicolon
)brace
...
macro_line|#endif
multiline_comment|/**&n; * scsi_eh_get_failed - Gather failed cmds.&n; * @sc_list:&t;A pointer to a list for failed cmds.&n; * @shost:&t;Scsi host being recovered.&n; *&n; * XXX Add opaque interator for device / shost. Investigate direct&n; * addition to per eh list on error allowing skipping of this step.&n; **/
DECL|function|scsi_eh_get_failed
r_static
r_void
id|scsi_eh_get_failed
c_func
(paren
id|Scsi_Cmnd
op_star
...
r_else
(brace
multiline_comment|/*&n;&t;&t;&t;&t; * FIXME Verify how this can happen and if&n;&t;&t;&t;&t; * this is still needed??&n;&t;&t;&t;&t; */
r_if
c_cond
(paren
id|scmd-&gt;state
...
(brace
multiline_comment|/*&n;&t;&t;&t;&t; * Rats.  Something is still floating&n;&t;&t;&t;&t; * around out there This could be the&n;&t;&t;&t;&t; * result of the fact that the upper level&n;&t;&t;&t;&t; * drivers are still frobbing commands&n;&t;&t;&t;&t; * that might have succeeded.  There are&n;&t;&t;&t;&t; * two outcomes. One is that the command&n;&t;&t;&t;&t; * block will eventually be freed, and the&n;&t;&t;&t;&t; * other one is that the command will be&n;&t;&t;&t;&t; * queued and will be finished along the&n;&t;&t;&t;&t; * way.&n;&t;&t;&t;&t; */
id|SCSI_LOG_ERROR_RECOVERY
c_func
(paren
l_int|1
...
c_cond
(paren
id|scmd-&gt;host-&gt;hostt-&gt;eh_abort_handler
)paren
id|scmd-&gt;host-&gt;hostt
op_member_access_from_pointer
id|eh_abort_handler
...
(paren
op_logical_neg
id|scmd-&gt;host-&gt;hostt-&gt;unchecked_isa_dma
)paren
ques
c_cond
op_amp
...
multiline_comment|/**&n; * scsi_eh_retry_cmd - Retry the original command&n; * @scmd:&t;Original failed SCSI cmd.&n; *&n; * Notes:&n; *    This function will *not* return until the command either times out,&n; *    or it completes.&n; **/
DECL|function|scsi_eh_retry_cmd
r_static
r_int
...
c_func
(paren
l_string|&quot;%s: checking to see if we need&quot;
l_string|&quot; to request sense&bslash;n&quot;
comma
id|__FUNCTION__
)paren
)paren
...
multiline_comment|/**&n; * scsi_eh_abort_cmd - abort a timed-out cmd.&n; * @sc_todo:&t;A list of cmds that have failed.&n; * @shost:&t;scsi host being recovered.&n; *&n; * Decription:&n; *    Try and see whether or not it makes sense to try and abort the&n; *    running command.  this only works out to be the case if we have one&n; *    command that has timed out.  if the command simply failed, it makes&n; *    no sense to try and abort the command, since as far as the shost&n; *    adapter is concerned, it isn&squot;t running.&n; **/
DECL|function|scsi_eh_abort_cmd
r_static
r_int
id|scsi_eh_abort_cmd
c_func
(paren
id|Scsi_Cmnd
op_star
...
(paren
l_string|&quot;%s: checking to see if we need&quot;
l_string|&quot; to abort cmd&bslash;n&quot;
comma
id|__FUNCTION__
)paren
)paren
...
c_cond
(paren
id|scmd-&gt;host-&gt;hostt-&gt;eh_device_reset_handler
op_eq
l_int|NULL
)paren
(brace
...
c_func
(paren
l_string|&quot;%s: Trying BDR&bslash;n&quot;
comma
id|__FUNCTION__
)paren
)paren
...
r_continue
suffix:semicolon
multiline_comment|/*&n;&t;&t; * ok, we have a device that is having problems.  try and send&n;&t;&t; * a bus device reset to it.&n;&t;&t; */
id|rtn
op_assign
id|scsi_try_bus_device_reset
c_func
...
c_cond
(paren
id|scmd-&gt;host-&gt;hostt-&gt;eh_bus_reset_handler
op_eq
l_int|NULL
)paren
r_return
...
c_cond
(paren
id|scmd-&gt;host-&gt;hostt-&gt;eh_host_reset_handler
op_eq
l_int|NULL
)paren
r_return
...
multiline_comment|/**&n; * scsi_eh_bus_host_reset - send a bus reset and on failure try host reset&n; * @sc_todo:&t;a list of cmds that have failed.&n; * @shost:&t;scsi host being recovered.&n; **/
DECL|function|scsi_eh_bus_host_reset
r_static
r_int
id|scsi_eh_bus_host_reset
c_func
(paren
id|Scsi_Cmnd
op_star
...
c_func
(paren
l_string|&quot;%s: Try Bus/Host RST&bslash;n&quot;
comma
id|__FUNCTION__
)paren
)paren
...
r_continue
suffix:semicolon
multiline_comment|/*&n;&t;&t; * we now know that we are able to perform a reset for the&n;&t;&t; * channel that scmd points to.&n;&t;&t; */
id|rtn
op_assign
id|scsi_try_bus_reset
c_func
...
multiline_comment|/**&n; * scsi_eh_offline_sdevs - offline scsi devices that fail to recover&n; * @sc_todo:&t;a list of cmds that have failed.&n; * @shost:&t;scsi host being recovered.&n; *&n; **/
...
c_func
(paren
l_string|&quot;scsi%d (%d,%d,%d) : reservation conflict&bslash;n&quot;
comma
...
id|scmd
suffix:semicolon
multiline_comment|/*&n;&t; * Is this assert really ok anymore (andmike). Should we at least&n;&t; * be using spin_lock_unlocked.&n;&t; */
id|ASSERT_LOCK
c_func
(paren
id|shost-&gt;host_lock
...
suffix:semicolon
multiline_comment|/*&n;&t; * We are currently holding these things in a linked list - we&n;&t; * didn&squot;t put them in the bottom half queue because we wanted to&n;&t; * keep things quiet while we were working on recovery, and&n;&t; * passing them up to the top level could easily cause the top&n;&t; * level to try and queue something else again.&n;&t; *&n;&t; * start by marking that the host is no longer in error recovery.&n;&t; */
...
l_int|NULL
suffix:semicolon
multiline_comment|/*&n;&t;&t; * Oh, this is a vile hack.  scsi_done() expects a timer&n;&t;&t; * to be running on the command.  If there isn&squot;t, it assumes&n;&t;&t; * that the command has actually timed out, and a timer&n;&t;&t; * handler is running.  That may well be how we got into&n;&t;&t; * this fix, but right now things are stable.  We add&n;&t;&t; * a timer back again so that we can report completion.&n;&t;&t; * scsi_done() will immediately remove said timer from&n;&t;&t; * the command, and then process it.&n;&t;&t; */
id|scsi_add_timer
c_func
(paren
id|scmd
...
multiline_comment|/**&n; * scsi_new_reset - Send reset to a bus or device at any phase.&n; * @scmd:&t;Cmd to send reset with (usually a dummy)&n; * @flag:&t;Reset type.&n; *&n; * Description:&n; *    This is used by the SCSI Generic driver to provide Bus/Device reset&n; *    capability.&n; *&n; * Return value:&n; *    SUCCESS/FAILED.&n; **/
DECL|function|scsi_new_reset
r_static
r_int
id|scsi_new_reset
c_func
