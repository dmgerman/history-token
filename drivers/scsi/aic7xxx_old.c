...
macro_line|#endif
macro_line|#if defined(__powerpc__) || defined(__i386__) || defined(__x86_64__)
DECL|macro|MMAPIO
macro_line|#  define MMAPIO
macro_line|#endif
multiline_comment|/*&n; * You can try raising me for better performance or lowering me if you have&n; * flaky devices that go off the scsi bus when hit with too many tagged&n; * commands (like some IBM SCSI-3 LVD drives).&n; */
...
suffix:semicolon
multiline_comment|/*&n; * Make a define that will tell the driver not to the default tag depth&n; * everywhere.&n; */
...
multiline_comment|/*&n; * Get out private data area from a scsi cmd pointer&n; */
DECL|macro|AIC_DEV
mdefine_line|#define AIC_DEV(cmd)&t;((struct aic_dev_data *)(cmd)-&gt;device-&gt;hostdata)
...
id|pad
suffix:semicolon
multiline_comment|/*&n;                                     * Unused by the kernel, but we require&n;                                     * the padding so that the array of&n;                                     * hardware SCBs is aligned on 32 byte&n;                                     * boundaries so the sequencer can index&n;                                     */
)brace
suffix:semicolon
r_typedef
r_enum
...
l_int|0x4000
comma
DECL|enumerator|SCB_QUEUE_FULL
id|SCB_QUEUE_FULL
op_assign
l_int|0x8000
...
r_enum
(brace
DECL|enumerator|AHC_BUG_NONE
id|AHC_BUG_NONE
op_assign
l_int|0x0000
comma
DECL|enumerator|AHC_BUG_TMODE_WIDEODD
id|AHC_BUG_TMODE_WIDEODD
op_assign
l_int|0x0001
comma
DECL|enumerator|AHC_BUG_AUTOFLUSH
id|AHC_BUG_AUTOFLUSH
op_assign
l_int|0x0002
comma
DECL|enumerator|AHC_BUG_CACHETHEN
id|AHC_BUG_CACHETHEN
op_assign
l_int|0x0004
comma
DECL|enumerator|AHC_BUG_CACHETHEN_DIS
id|AHC_BUG_CACHETHEN_DIS
op_assign
l_int|0x0008
comma
DECL|enumerator|AHC_BUG_PCI_2_1_RETRY
id|AHC_BUG_PCI_2_1_RETRY
op_assign
l_int|0x0010
comma
DECL|enumerator|AHC_BUG_PCI_MWI
id|AHC_BUG_PCI_MWI
op_assign
l_int|0x0020
comma
DECL|enumerator|AHC_BUG_SCBCHAN_UPLOAD
id|AHC_BUG_SCBCHAN_UPLOAD
op_assign
l_int|0x0040
comma
DECL|typedef|ahc_bugs
)brace
id|ahc_bugs
suffix:semicolon
...
id|delayed_scbs
suffix:semicolon
DECL|member|temp_q_depth
r_volatile
r_int
r_int
id|temp_q_depth
suffix:semicolon
DECL|member|max_q_depth
r_int
r_int
id|max_q_depth
suffix:semicolon
...
id|active_cmds
suffix:semicolon
multiline_comment|/*&n;   * Statistics Kept:&n;   *&n;   * Total Xfers (count for each command that has a data xfer),&n;   * broken down by reads &amp;&amp; writes.&n;   *&n;   * Further sorted into a few bins for keeping tabs on how many commands&n;   * we get of various sizes.&n;   *&n;   */
DECL|member|w_total
r_int
id|w_total
suffix:semicolon
...
DECL|member|barrier_total
r_int
id|barrier_total
suffix:semicolon
multiline_comment|/* total num of REQ_BARRIER commands */
DECL|member|ordered_total
r_int
id|ordered_total
suffix:semicolon
multiline_comment|/* How many REQ_BARRIER commands we&n;&t;&t;&t;&t;&t;    used ordered tags to satisfy */
DECL|member|w_bins
r_int
id|w_bins
(braket
...
mdefine_line|#define  BUS_DEVICE_RESET_PENDING       0x01
DECL|macro|DEVICE_RESET_DELAY
mdefine_line|#define  DEVICE_RESET_DELAY             0x02
DECL|macro|DEVICE_PRINT_DTR
mdefine_line|#define  DEVICE_PRINT_DTR               0x04
DECL|macro|DEVICE_WAS_BUSY
mdefine_line|#define  DEVICE_WAS_BUSY                0x08
DECL|macro|DEVICE_DTR_SCANNED
mdefine_line|#define  DEVICE_DTR_SCANNED&t;&t;0x10
DECL|macro|DEVICE_SCSI_3
mdefine_line|#define  DEVICE_SCSI_3&t;&t;&t;0x20
DECL|member|flags
r_volatile
r_int
r_char
...
id|ultraenb
suffix:semicolon
multiline_comment|/* Gets downloaded to card as a&n;&t;&t;&t;&t;&t;&t;bitmap */
...
DECL|member|aic_devs
r_struct
id|list_head
id|aic_devs
suffix:semicolon
multiline_comment|/* all aic_dev structs on host */
DECL|member|host_no
r_int
id|host_no
suffix:semicolon
...
multiline_comment|/*&n; * Use this as the default queue depth when setting tagged queueing on.&n; */
DECL|variable|aic7xxx_default_queue_depth
r_static
r_int
r_int
...
suffix:semicolon
multiline_comment|/*&n;         * Clear any residual information since the normal aic7xxx_done() path&n;         * doesn&squot;t touch the residuals.&n;         */
id|scb-&gt;hscb-&gt;residual_SG_segment_count
op_assign
l_int|0
suffix:semicolon
id|scb-&gt;hscb-&gt;residual_data_count
(braket
l_int|0
)braket
op_assign
...
suffix:semicolon
multiline_comment|/*&n;   * If we are still scanning devices, this device may not be linked into&n;   * our host controller yet.  So, we grab the temporary aic_dev that we&n;   * allocate during the device scan (by getting device-&gt;hostdata for the&n;   * active device) and if the list struct in the aic_dev is empty, we&n;   * add it to p-&gt;aic_devs.  Then at the end of the function, if we added&n;   * the aic_dev to our host struct so our loop would work, then we just&n;   * remove it.&n;   */
id|scb_tag
op_assign
id|aic_inb
c_func
...
(paren
id|INFO_LEAD
l_string|&quot;Reset device, hardware_scb %d, aic_dev %p,&bslash;n&quot;
comma
id|p-&gt;host_no
comma
id|channel
...
(paren
id|INFO_LEAD
l_string|&quot;Current scb %d, SEQADDR 0x%x, LASTPHASE &quot;
...
(paren
id|INFO_LEAD
l_string|&quot;SG_CACHEPTR 0x%x, SG_COUNT %d, SCSISIGI 0x%x&bslash;n&quot;
comma
id|p-&gt;host_no
comma
id|channel
...
(paren
id|INFO_LEAD
l_string|&quot;SSTAT0 0x%x, SSTAT1 0x%x, SSTAT2 0x%x&bslash;n&quot;
comma
id|p-&gt;host_no
comma
id|channel
...
(paren
id|INFO_LEAD
l_string|&quot;processing aic_dev %p&bslash;n&quot;
comma
id|p-&gt;host_no
comma
id|channel
...
multiline_comment|/*&n;       * In 2.4, only send back the residual information, don&squot;t flag this&n;       * as an error.  Before 2.4 we had to flag this as an error because&n;       * the mid layer didn&squot;t check residual data counts to see if the&n;       * command needs retried.&n;       */
id|cmd-&gt;resid
op_assign
id|scb-&gt;sg_length
op_minus
...
id|old_verbose
suffix:semicolon
multiline_comment|/*&n;             * Hmmmm....the device is flaking out on tagged commands.&n;             */
id|scsi_adjust_queue_depth
c_func
(paren
id|scb-&gt;cmd-&gt;device
...
(paren
id|INFO_LEAD
l_string|&quot;CHECK_CONDITION on REQUEST_SENSE, returning &quot;
l_string|&quot;an error.&bslash;n&quot;
comma
id|p-&gt;host_no
comma
id|CTL_OF_SCB
...
id|scb_index
suffix:semicolon
multiline_comment|/*&n;               * We have to look three places for queued commands:&n;               *  1: p-&gt;waiting_scbs queue&n;               *  2: QINFIFO&n;               *  3: WAITING_SCBS list on card (for commands that are started&n;               *     but haven&squot;t yet made it to the device)&n;&t;       *&n;&t;       * Of special note here is that commands on 2 or 3 above will&n;&t;       * have already been marked as active, while commands on 1 will&n;&t;       * not.  The aic7xxx_done() function will want to unmark them&n;&t;       * from active, so any commands we pull off of 1 need to&n;&t;       * up the active count.&n;               */
id|next_scbp
op_assign
id|p-&gt;waiting_scbs.head
suffix:semicolon
...
id|result
suffix:semicolon
multiline_comment|/* temp_q_depth could have been dropped to 1 for an untagged&n;&t;&t;   * command that might be coming up */
r_if
c_cond
(paren
id|aic_dev-&gt;temp_q_depth
...
multiline_comment|/* We should free up the no unused SCB entries.  But, that&squot;s&n;&t;&t; * a difficult thing to do because we use a direct indexed&n;&t;&t; * array, so we can&squot;t just take any entries and free them,&n;&t;&t; * we *have* to free the ones at the end of the array, and&n;&t;&t; * they very well could be in use right now, which means&n;&t;&t; * in order to do this right, we have to add a delayed&n;&t;&t; * freeing mechanism tied into the scb_free() code area.&n;&t;&t; * We&squot;ll add that later.&n;&t;&t; */
)brace
r_break
suffix:semicolon
)brace
...
(paren
id|INFO_LEAD
l_string|&quot;Got WIDE_RESIDUE message, patching up data &quot;
...
suffix:semicolon
multiline_comment|/*&n;         * If resid_dcnt == native_length, then we just loaded this SG&n;         * segment and we need to back it up one...&n;         */
r_if
c_cond
(paren
id|resid_dcnt
...
(brace
multiline_comment|/*&n;             * Oops, this isn&squot;t right, we can&squot;t back up to before the&n;             * beginning.  This must be a bogus message, ignore it.&n;             */
r_break
suffix:semicolon
)brace
id|resid_dcnt
...
r_else
(brace
multiline_comment|/*&n;           * resid_dcnt != native_length, so we are in the middle of a SG&n;           * element.  Back it up one byte and leave the rest alone.&n;           */
id|resid_dcnt
op_add_assign
l_int|1
suffix:semicolon
...
multiline_comment|/*&n;         * Output the new addresses and counts to the right places on the&n;         * card.&n;         */
id|aic_outb
c_func
(paren
id|p
...
suffix:semicolon
multiline_comment|/*&n;         * The sequencer actually wants to find the new address&n;         * in the SHADDR register set.  On the Ultra2 and later controllers&n;         * this register set is readonly.  In order to get the right number&n;         * into the register, you actually have to enter it in HADDR and then&n;         * use the PRELOADEN bit of DFCNTRL to drop it through from the&n;         * HADDR register to the SHADDR register.  On non-Ultra2 controllers,&n;         * we simply write it direct.&n;         */
r_if
c_cond
(paren
id|p-&gt;features
...
(brace
multiline_comment|/*&n;           * We might as well be accurate and drop both the resid_dcnt and&n;           * cur_addr into HCNT and HADDR and have both of them drop&n;           * through to the shadow layer together.&n;           */
id|aic_outb
c_func
(paren
id|p
...
(paren
id|WARN_LEAD
l_string|&quot;invalid scb_index during SEQ_SG_FIXUP.&bslash;n&quot;
comma
id|p-&gt;host_no
comma
op_minus
...
(paren
id|INFO_LEAD
l_string|&quot;SG_CACHEPTR 0x%x, SSTAT2 0x%x, STCNT 0x%x&bslash;n&quot;
comma
id|p-&gt;host_no
comma
op_minus
...
(paren
id|INFO_LEAD
l_string|&quot;Fixing up SG address for sequencer.&bslash;n&quot;
comma
id|p-&gt;host_no
comma
id|CTL_OF_SCB
...
multiline_comment|/*&n;       * Advance the SG pointer to the next element in the list&n;       */
id|tmp
op_assign
id|aic_inb
c_func
...
suffix:semicolon
multiline_comment|/*&n;       * Now stuff the element we just advanced past down onto the&n;       * card so it can be stored in the residual area.&n;       */
id|aic_outb
c_func
(paren
id|p
...
id|WIDEXFER
suffix:semicolon
multiline_comment|/*&n;         * If our current max syncrate is in the Ultra3 range, bump it back&n;         * down to Ultra2 since we can&squot;t negotiate DT transfers using SDTR&n;         */
r_if
c_cond
(paren
id|maxsync
...
id|aic_dev-&gt;flags
op_amp
id|DEVICE_DTR_SCANNED
)paren
)paren
(brace
multiline_comment|/*&n;             * We shouldn&squot;t get here unless this is a narrow drive, wide&n;             * devices should trigger this same section of code in the WDTR&n;             * handler first instead.&n;             */
id|aic_dev-&gt;goal.width
op_assign
id|MSG_EXT_WDTR_BUS_8_BIT
suffix:semicolon
...
(brace
multiline_comment|/*&n;             * This is a preemptive message from the target, we&squot;ve already&n;             * scanned this target and set our options for it, and we&n;             * don&squot;t need a SDTR with this target (for whatever reason),&n;             * so reject this incoming SDTR&n;             */
id|reject
op_assign
id|TRUE
suffix:semicolon
...
multiline_comment|/* The device is sending this message first and we have to reply */
id|reply
op_assign
id|TRUE
suffix:semicolon
...
multiline_comment|/*&n;         * Use our new_period, new_offset, bus_width, and card options&n;         * to determine the actual syncrate settings&n;         */
id|syncrate
op_assign
id|aic7xxx_find_syncrate
c_func
...
suffix:semicolon
multiline_comment|/*&n;         * Did we drop to async?  If so, send a reply regardless of whether&n;         * or not we initiated this negotiation.&n;         */
r_if
c_cond
(paren
(paren
...
multiline_comment|/*&n;         * Did we start this, if not, or if we went too low and had to&n;         * go async, then send an SDTR back to the target&n;         */
r_if
c_cond
(paren
id|reply
...
(brace
multiline_comment|/* when sending a reply, make sure that the goal settings are&n;           * updated along with current and active since the code that&n;           * will actually build the message for the sequencer uses the&n;           * goal settings as its guidelines.&n;           */
id|aic7xxx_set_syncrate
c_func
(paren
id|p
...
multiline_comment|/*&n;             * Devices that support DT transfers don&squot;t start WDTR requests&n;             */
id|aic_dev-&gt;goal.options
op_assign
l_int|0
suffix:semicolon
...
(brace
multiline_comment|/*&n;             * This is a preemptive message from the target, we&squot;ve already&n;             * scanned this target and set our options for it, and we&n;             * don&squot;t need a WDTR with this target (for whatever reason),&n;             * so reject this incoming WDTR&n;             */
id|reject
op_assign
id|TRUE
suffix:semicolon
...
(brace
multiline_comment|/* there is no other command with SCB_DTR_SCB already set that will&n;             * trigger the release of the dtr_pending bit.  Both set the bit&n;             * and set scb-&gt;flags |= SCB_DTR_SCB&n;             */
id|aic_dev-&gt;dtr_pending
op_assign
l_int|1
suffix:semicolon
...
(brace
multiline_comment|/* Have we scanned the device yet? */
r_if
c_cond
(paren
op_logical_neg
...
(brace
multiline_comment|/* The device is electing to use PPR messages, so we will too until&n;             * we know better */
id|aic_dev-&gt;needppr
op_assign
id|aic_dev-&gt;needppr_copy
op_assign
...
l_int|0
suffix:semicolon
multiline_comment|/* We know the device is SCSI-3 compliant due to PPR */
id|aic_dev-&gt;flags
op_or_assign
id|DEVICE_SCSI_3
suffix:semicolon
...
(brace
multiline_comment|/*&n;             * This is a preemptive message from the target, we&squot;ve already&n;             * scanned this target and set our options for it, and we&n;             * don&squot;t need a PPR with this target (for whatever reason),&n;             * so reject this incoming PPR&n;             */
id|reject
op_assign
id|TRUE
suffix:semicolon
...
multiline_comment|/*&n;         * As it turns out, if we don&squot;t *have* to have PPR messages, then&n;         * configure ourselves not to use them since that makes some&n;         * external drive chassis work (those chassis can&squot;t parse PPR&n;         * messages and they mangle the SCSI bus until you send a WDTR&n;         * and SDTR that they can understand).&n;         */
r_if
c_cond
(paren
id|new_trans_options
...
(brace
multiline_comment|/*&n;           * Oops, the syncrate went to low for this card and we fell off&n;           * to async (should never happen with a device that uses PPR&n;           * messages, but have to be complete)&n;           */
id|reply
op_assign
id|TRUE
suffix:semicolon
...
(brace
multiline_comment|/*&n;       * Hmmm...error during a negotiation command.  Either we have a&n;       * borken bus, or the device doesn&squot;t like our negotiation message.&n;       * Since we check the INQUIRY data of a device before sending it&n;       * negotiation messages, assume the bus is borken for whatever&n;       * reason.  Complete the command.&n;       */
id|printerror
op_assign
l_int|0
suffix:semicolon
...
(brace
multiline_comment|/*&n;         * Signal that we need to re-negotiate things.&n;         */
id|aic_dev-&gt;needppr
op_assign
id|aic_dev-&gt;needppr_copy
suffix:semicolon
...
r_static
r_void
DECL|function|aic7xxx_init_transinfo
id|aic7xxx_init_transinfo
c_func
(paren
r_struct
id|aic7xxx_host
...
multiline_comment|/*+F*************************************************************************&n; * Function:&n; *   aic7xxx_alloc_aic_dev&n; *&n; * Description:&n; *   Set up the initial aic_dev struct pointers&n; *-F*************************************************************************/
r_static
r_struct
id|aic_dev_data
op_star
DECL|function|aic7xxx_alloc_aic_dev
id|aic7xxx_alloc_aic_dev
c_func
(paren
r_struct
id|aic7xxx_host
...
r_return
suffix:semicolon
singleline_comment|// We&squot;ve already enabled this device
r_if
c_cond
(paren
...
(paren
id|INFO_LEAD
l_string|&quot;Tagged queuing enabled, queue depth %d.&bslash;n&quot;
comma
id|p-&gt;host_no
comma
id|device-&gt;channel
...
(paren
id|INFO_LEAD
l_string|&quot;Tagged queuing disabled, queue depth %d.&bslash;n&quot;
comma
id|p-&gt;host_no
comma
id|device-&gt;channel
...
multiline_comment|/*+F*************************************************************************&n; * Function:&n; *   aic7xxx_slave_detach&n; *&n; * Description:&n; *   prepare for this device to go away&n; *-F*************************************************************************/
r_void
DECL|function|aic7xxx_slave_detach
...
multiline_comment|/*+F*************************************************************************&n; * Function:&n; *   aic7xxx_slave_attach&n; *&n; * Description:&n; *   Configure the device we are attaching to the controller.  This is&n; *   where we get to do things like scan the INQUIRY data, set queue&n; *   depths, allocate command structs, etc.&n; *-F*************************************************************************/
r_int
DECL|function|aic7xxx_slave_attach
...
suffix:semicolon
multiline_comment|/*&n;   * In the future, we may call this function as a last resort for&n;   * error handling.  Let&squot;s be nice and not do any unnecessary delays.&n;   */
id|wait
op_assign
l_int|1000
suffix:semicolon
...
multiline_comment|/*+F*************************************************************************&n; * Function:&n; *   aic7xxx_configure_bugs&n; *&n; * Description:&n; *   Take the card passed in and set the appropriate bug flags based upon&n; *   the card model.  Also make any changes needed to device registers or&n; *   PCI registers while we are here.&n; *-F*************************************************************************/
r_static
r_void
DECL|function|aic7xxx_configure_bugs
id|aic7xxx_configure_bugs
c_func
(paren
r_struct
id|aic7xxx_host
...
multiline_comment|/*&n;   * Now handle the bugs that require PCI register or card register tweaks&n;   */
...
multiline_comment|/*&n;           * Call our function to fixup any bugs that exist on this chipset.&n;           * This may muck with PCI settings and other device settings, so&n;           * make sure it&squot;s after all the other PCI and device register&n;           * tweaks so it can back out bad settings on specific broken cards.&n;           */
id|aic7xxx_configure_bugs
c_func
(paren
id|temp_p
...
multiline_comment|/*&n;     * All the 7770 based chipsets have this bug&n;     */
id|temp_p-&gt;bugs
op_or_assign
id|AHC_BUG_TMODE_WIDEODD
suffix:semicolon
...
id|DISCENB
suffix:semicolon
multiline_comment|/* We always force TEST_UNIT_READY to untagged */
r_if
c_cond
(paren
id|cmd-&gt;cmnd
...
(paren
id|KERN_ERR
l_string|&quot;aic7xxx_bus_device_reset: called with NULL cmd!&bslash;n&quot;
)paren
suffix:semicolon
r_return
id|FAILED
...
suffix:semicolon
multiline_comment|/* If the command was already complete or just completed, then we didn&squot;t&n;   * do a reset, return FAILED */
r_if
c_cond
(paren
op_logical_neg
...
multiline_comment|/*&n;   * Simply set the MK_MESSAGE flag and the SEQINT handler will do&n;   * the rest on a reconnect/connect.&n;   */
id|scb-&gt;hscb-&gt;control
op_or_assign
id|MK_MESSAGE
suffix:semicolon
...
id|BUS_DEVICE_RESET_PENDING
suffix:semicolon
multiline_comment|/*&n;   * Check to see if the command is on the qinfifo.  If it is, then we will&n;   * not need to queue the command again since the card should start it soon&n;   */
r_if
c_cond
(paren
id|aic7xxx_search_qinfifo
...
suffix:semicolon
multiline_comment|/*&n;       * If the DISCONNECTED bit is not set in SCB_CONTROL, then we are&n;       * actually on the waiting list, not disconnected, and we don&squot;t&n;       * need to requeue the command.&n;       */
id|disconnected
op_assign
(paren
id|scb_control
...
(brace
multiline_comment|/*&n;       * Actually requeue this SCB in case we can select the&n;       * device before it reconnects.  This can result in the command&n;       * being on the qinfifo twice, but we don&squot;t care because it will&n;       * all get cleaned up if/when the reset takes place.&n;       */
r_if
c_cond
(paren
id|aic7xxx_verbose
...
(paren
id|INFO_LEAD
l_string|&quot;Queueing device reset command.&bslash;n&quot;
comma
id|p-&gt;host_no
comma
id|CTL_OF_SCB
...
(paren
id|KERN_ERR
l_string|&quot;aic7xxx_abort: called with NULL cmd!&bslash;n&quot;
)paren
suffix:semicolon
r_return
id|FAILED
...
(paren
id|INFO_LEAD
l_string|&quot;Aborting scb %d, flags 0x%x, SEQADDR 0x%x, LASTPHASE &quot;
l_string|&quot;0x%x&bslash;n&quot;
comma
id|p-&gt;host_no
comma
...
(paren
id|INFO_LEAD
l_string|&quot;SCB found in QINFIFO and aborted.&bslash;n&quot;
comma
id|p-&gt;host_no
comma
id|CTL_OF_SCB
...
id|DISCONNECTED
suffix:semicolon
multiline_comment|/*&n;     * If the DISCONNECTED bit is not set in SCB_CONTROL, then we are&n;     * either currently active or on the waiting list.&n;     */
r_if
c_cond
(paren
op_logical_neg
...
suffix:semicolon
multiline_comment|/* If we are the only waiting command, stop the selection engine */
r_if
c_cond
(paren
id|aic_inb
...
(brace
multiline_comment|/*&n;       * We are the currently active command&n;       */
r_if
c_cond
(paren
(paren
...
(brace
multiline_comment|/*&n;&t; * Message buffer busy, unable to abort&n;&t; */
id|printk
c_func
(paren
id|INFO_LEAD
l_string|&quot;message buffer busy, unable to abort.&bslash;n&quot;
comma
id|p-&gt;host_no
comma
id|CTL_OF_SCB
...
multiline_comment|/* Fallthrough to below, set ATNO after we set SCB_CONTROL */
)brace
id|aic_outb
c_func
(paren
...
r_else
(brace
multiline_comment|/*&n;     * The scb isn&squot;t in the card at all and it is active and it isn&squot;t in&n;     * any of the queues, so it must be disconnected and paged out.  Fall&n;     * through to the code below.&n;     */
id|disconnected
op_assign
l_int|1
suffix:semicolon
...
(paren
id|INFO_LEAD
l_string|&quot;Abort never delivered, returning FAILED&bslash;n&quot;
comma
id|p-&gt;host_no
comma
id|CTL_OF_CMD
...
(paren
id|INFO_LEAD
l_string|&quot;Abort successful.&bslash;n&quot;
comma
id|p-&gt;host_no
comma
id|CTL_OF_CMD
...
(brace
multiline_comment|/*&n;     * We just completed the command when we ran the isr stuff, so we no&n;     * longer have it.&n;     */
id|unpause_sequencer
c_func
(paren
id|p
