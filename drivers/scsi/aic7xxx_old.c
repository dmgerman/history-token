...
mdefine_line|#define  DEVICE_WAS_BUSY                0x10
DECL|macro|DEVICE_SCSI_3
mdefine_line|#define  DEVICE_SCSI_3                  0x20
DECL|macro|DEVICE_DTR_SCANNED
mdefine_line|#define  DEVICE_DTR_SCANNED             0x40
...
suffix:semicolon
DECL|member|dev_lun_queue_depth
r_int
r_char
id|dev_lun_queue_depth
(braket
id|MAX_TARGETS
)braket
suffix:semicolon
DECL|member|dev_scbs_needed
r_int
r_int
id|dev_scbs_needed
(braket
...
(paren
id|INFO_LEAD
l_string|&quot;Current scb_tag %d, SEQADDR 0x%x, LASTPHASE &quot;
...
l_int|0
suffix:semicolon
multiline_comment|/*&n;         * The QUEUE_FULL/BUSY handler in aic7xxx_seqint takes care of putting&n;         * this command on a timer and allowing us to retry it.  Here, we&n;         * just 0 out a few values so that they don&squot;t carry through to when&n;         * the command finally does complete.&n;         */
r_break
suffix:semicolon
r_default
suffix:colon
...
(brace
multiline_comment|/*&n;     * We&squot;ve already scanned some lun on this device and enabled tagged&n;     * queueing on it.  So, as long as this lun also supports tagged&n;     * queueing, enable it here with the same depth.  Call SCSI mid layer&n;     * to adjust depth on this device, and add enough to the max_queue_depth&n;     * to cover the commands for this lun.&n;     *&n;     * Note: there is a shortcoming here.  The aic7xxx driver really assumes&n;     * that if any lun on a device supports tagged queueing, then they *all*&n;     * do.  Our p-&gt;tagenable field is on a per target id basis and doesn&squot;t&n;     * differentiate for different luns.  If we end up with one lun that&n;     * doesn&squot;t support tagged queueing, it&squot;s going to disable tagged queueing&n;     * on *all* the luns on that target ID :-(&n;     */
r_if
c_cond
(paren
...
id|lun
suffix:semicolon
multiline_comment|/*&n;       * Uh ohh, this is what I was talking about.  All the other devices on&n;       * this target ID that support tagged queueing are going to end up&n;       * getting tagged queueing turned off because of this device.  Print&n;       * out a message to this effect for the user, then disable tagged&n;       * queueing on all the devices on this ID.&n;       */
id|printk
c_func
(paren
id|WARN_LEAD
l_string|&quot;does not support tagged queuing while other luns on&bslash;n&quot;
l_string|&quot;          the same target ID do!!  Tagged queueing will be disabled for&bslash;n&quot;
l_string|&quot;          all luns on this target ID!!&bslash;n&quot;
comma
id|p-&gt;host_no
comma
id|device-&gt;channel
...
(paren
id|WARN_LEAD
l_string|&quot;disabling tagged queuing.&bslash;n&quot;
comma
id|p-&gt;host_no
comma
id|p-&gt;Scsi_Dev
...
multiline_comment|/*&n;   * We only need to check INQUIRY data on one lun of multi lun devices&n;   * since speed negotiations are not lun specific.  Once we&squot;ve check this&n;   * particular target id once, the DEVICE_PRESENT flag will be set.&n;   */
r_if
c_cond
(paren
op_logical_neg
...
(brace
multiline_comment|/*&n;     * Well, all SCBs are currently active on the bus.  So, we spin here&n;     * running the interrupt handler until one completes and becomes free.&n;     * We can do this safely because we either A) hold the driver lock (in&n;     * 2.0 kernels) or we have the io_request_lock held (in 2.2 and later&n;     * kernels) and so either way, we won&squot;t take any other interrupts and&n;     * the queue path will block until we release it.  Also, we would worry&n;     * about running the completion queues, but obviously there are plenty&n;     * of commands outstanding to trigger a later interrupt that will do&n;     * that for us, so skip it here.&n;     */
id|DRIVER_LOCK
id|aic7xxx_isr
c_func
(paren
...
mdefine_line|#define RESET_DELAY  0x08
r_int
id|action
suffix:semicolon
r_if
...
(paren
id|INFO_LEAD
l_string|&quot;Reset called for a SYNCHRONOUS reset, flags 0x%x, &quot;
l_string|&quot;cmd-&gt;result 0x%x.&bslash;n&quot;
comma
id|p-&gt;host_no
comma
id|CTL_OF_CMD
...
l_string|&quot;-&gt;SCB mapping, failing.&bslash;n&quot;
comma
id|p-&gt;host_no
comma
id|CTL_OF_CMD
