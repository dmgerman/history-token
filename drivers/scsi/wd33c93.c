...
mdefine_line|#define WD33C93_DATE       &quot;22/Feb/2003&quot;
id|MODULE_AUTHOR
c_func
(paren
l_string|&quot;John Shifflett&quot;
)paren
suffix:semicolon
id|MODULE_DESCRIPTION
c_func
(paren
l_string|&quot;Generic WD33C93 SCSI driver&quot;
)paren
suffix:semicolon
id|MODULE_LICENSE
c_func
...
DECL|macro|write_wd33c93_cmd
mdefine_line|#define write_wd33c93_cmd(regs, cmd) &bslash;&n;&t;write_wd33c93((regs), WD_COMMAND, (cmd))
r_static
r_inline
r_void
DECL|function|write_wd33c93_cdb
id|write_wd33c93_cdb
c_func
(paren
r_const
id|wd33c93_regs
...
macro_line|#else /* CONFIG_WD33C93_PIO */
r_static
r_inline
id|uchar
DECL|function|read_wd33c93
...
macro_line|#endif /* CONFIG_WD33C93_PIO */
r_static
r_inline
id|uchar
...
suffix:semicolon
multiline_comment|/*&n;&t; * Add the cmd to the end of &squot;input_Q&squot;. Note that REQUEST SENSE&n;&t; * commands are added to the head of the queue so that the desired&n;&t; * sense data is not lost before REQUEST_SENSE executes.&n;&t; */
id|spin_lock_irq
c_func
(paren
op_amp
...
suffix:semicolon
macro_line|#endif
multiline_comment|/*&n;&t; * Start the selection process&n;&t; */
r_if
c_cond
(paren
...
suffix:semicolon
multiline_comment|/* When doing a non-disconnect command with DMA, we can save&n;&t;&t; * ourselves a DATA phase interrupt later by setting everything&n;&t;&t; * up ahead of time.&n;&t;&t; */
r_if
c_cond
(paren
(paren
...
suffix:semicolon
multiline_comment|/* From esp.c:&n;&t;&t; * There is a window of time within the scsi_done() path&n;&t;&t; * of execution where interrupts are turned back on full&n;&t;&t; * blast and left that way.  During that time we could&n;&t;&t; * reconnect to a disconnected command, then we&squot;d bomb&n;&t;&t; * out below.  We could also end up executing two commands&n;&t;&t; * at _once_.  ...just so you know why the restore_flags()&n;&t;&t; * is here...&n;&t;&t; */
...
multiline_comment|/* Old chips (pre -A ???) don&squot;t have advanced features and will&n;&t;&t;     * generate CSR_RESEL.  In that case we have to extract the LUN the&n;&t;&t;     * hard way (see below).&n;&t;&t;     * First we have to make sure this reselection didn&squot;t&n;&t;&t;     * happen during Arbitration/Selection of some other device.&n;&t;&t;     * If yes, put losing command back on top of input_Q.&n;&t;&t;     */
r_if
c_cond
(paren
id|hostdata-&gt;level2
...
suffix:semicolon
multiline_comment|/*&n; &t; * there are scsi devices out there, which manage to lock up&n;&t; * the wd33c93 in a busy condition. In this state it won&squot;t&n;&t; * accept the reset command. The only way to solve this is to&n; &t; * give the chip a hardware reset (if possible). The code below&n;&t; * does this for the SGI Indy, where this is possible&n;&t; */
...
id|p2
suffix:semicolon
multiline_comment|/* The kernel does some processing of the command-line before calling&n;&t; * this function: If it begins with any decimal or hex number arguments,&n;&t; * ints[0] = how many numbers found and ints[1] through [n] are the values&n;&t; * themselves. str points to where the non-numeric arguments (if any)&n;&t; * start: We do our own parsing of those. We construct synthetic &squot;nosync&squot;&n;&t; * keywords out of numeric args (to maintain compatibility with older&n;&t; * versions) and then add the rest of the arguments.&n;&t; */
id|p1
op_assign
id|setup_buffer
suffix:semicolon
...
suffix:semicolon
DECL|variable|wd33c93_queuecommand
id|EXPORT_SYMBOL
c_func
(paren
id|wd33c93_queuecommand
