...
DECL|variable|ncr53c8xx_transport_template
r_static
r_struct
id|scsi_transport_template
op_star
id|ncr53c8xx_transport_template
op_assign
l_int|NULL
suffix:semicolon
r_struct
...
suffix:semicolon
macro_line|#endif
multiline_comment|/* User settable limits and options.  */
DECL|member|usrsync
id|u_char
id|usrsync
suffix:semicolon
...
macro_line|#ifdef CONFIG_NCR53C8XX_PREFETCH
...
comma
(brace
multiline_comment|/*&n;**&t;Copy the CCB address to the next done entry.&n;**&t;Because the size depends on the&n;**&t;#define MAX_DONE parameter, it is filled&n;**&t;in at runtime.&n;**&n;**-----------------------------------------------------------&n;**&n;**  ##===========&lt; I=0; i&lt;MAX_DONE &gt;===========&n;**  ||&t;SCR_COPY (sizeof(struct ccb *),&n;**  ||&t;&t;NADDR (header.cp),&n;**  ||&t;&t;NADDR (ccb_done[i]),&n;**  ||&t;SCR_CALL,&n;**  ||&t;&t;PADDR (done_end),&n;**  ##==========================================&n;**&n;**-----------------------------------------------------------&n;*/
l_int|0
)brace
...
multiline_comment|/*&n; *&t;Burst code from io register bits.  Burst enable is ctest0 for c720&n; */
DECL|macro|burst_code
mdefine_line|#define burst_code(dmode, ctest0) &bslash;&n;&t;(ctest0) &amp; 0x80 ? 0 : (((dmode) &amp; 0xc0) &gt;&gt; 6) + 1
...
multiline_comment|/*&n;&t;**&t;Configure targets according to driver setup.&n;&t;**&t;Allow to override sync, wide and NOSCAN from &n;&t;**&t;boot command line.&n;&t;*/
r_for
c_loop
(paren
id|i
...
(paren
id|KERN_INFO
l_string|&quot;%s: ID %d, Fast-%d%s%s&bslash;n&quot;
comma
id|ncr_name
c_func
(paren
...
id|DMA_NONE
suffix:semicolon
multiline_comment|/*&n;&t;**&t;If data direction is BIDIRECTIONAL, speculate FROM_DEVICE&n;&t;**&t;but prepare alternate pointers for TO_DEVICE in case &n;&t;**&t;of our speculation will be just wrong.&n;&t;**&t;SCRIPTS will swap values if needed.&n;&t;*/
r_switch
c_cond
(paren
id|direction
...
multiline_comment|/*&n; * Start reset process.&n; * If reset in progress do nothing.&n; * The interrupt handler will reinitialize the chip.&n; * The timeout handler will wait for settle_time before &n; * clearing it and so resuming command processing.&n; */
...
macro_line|#if 0 /* unused and broken.. */
...
multiline_comment|/* Free allocated tp(s) */
r_for
c_loop
(paren
id|target
...
(brace
multiline_comment|/*&n;&t;&t; *&t;All went well (GOOD status).&n;&t;&t; *&t;CONDITION MET status is returned on &n;&t;&t; *&t;`Pre-Fetch&squot; or `Search data&squot; success.&n;&t;&t; */
id|cmd-&gt;result
op_assign
id|ScsiResult
c_func
...
id|cp
suffix:semicolon
multiline_comment|/*&n;&t;**&t;This allows list_del to work for the default ccb.&n;&t;*/
id|INIT_LIST_HEAD
c_func
(paren
op_amp
...
suffix:semicolon
multiline_comment|/*&n;&t;**&t;Select queue depth from driver setup.&n;&t;**&t;Donnot use more than configured by user.&n;&t;**&t;Use at least 2.&n;&t;**&t;Donnot use more than our maximum.&n;&t;*/
id|numtags
op_assign
...
suffix:semicolon
multiline_comment|/*&n;&t;**&t;Since the queue depth is not tunable under Linux,&n;&t;**&t;we need to know this value in order not to &n;&t;**&t;announce stupid things to user.&n;&t;**&n;&t;**&t;XXX(hch): As of Linux 2.6 it certainly _is_ tunable..&n;&t;**&t;&t;  In fact we just tuned it, or did I miss&n;&t;**&t;&t;  something important? :)&n;&t;*/
r_if
c_cond
(paren
id|lp
...
DECL|function|show_ncr53c8xx_revision
r_static
id|ssize_t
id|show_ncr53c8xx_revision
c_func
(paren
r_struct
id|class_device
...
comma
id|host_data-&gt;ncb-&gt;revision_id
)paren
suffix:semicolon
)brace
DECL|variable|ncr53c8xx_revision_attr
r_static
r_struct
id|class_device_attribute
id|ncr53c8xx_revision_attr
op_assign
(brace
dot
id|attr
...
suffix:semicolon
DECL|variable|ncr53c8xx_host_attrs
r_static
r_struct
id|class_device_attribute
op_star
id|ncr53c8xx_host_attrs
(braket
)braket
op_assign
(brace
...
(paren
id|KERN_INFO
l_string|&quot;ncr53c720-%d: rev 0x%x irq %d&bslash;n&quot;
comma
id|unit
comma
...
id|attach_error
suffix:semicolon
multiline_comment|/* Store input information in the host data structure.  */
id|np-&gt;unit
op_assign
id|unit
suffix:semicolon
...
id|device-&gt;host_id
suffix:semicolon
multiline_comment|/* Allocate SCRIPTS areas.  */
id|np-&gt;script0
op_assign
id|m_calloc_dma
c_func
...
id|ncr53c8xx_timeout
suffix:semicolon
multiline_comment|/* Try to map the controller chip to virtual and physical memory. */
id|np-&gt;paddr
op_assign
id|device-&gt;slot.base
suffix:semicolon
...
multiline_comment|/* Make the controller&squot;s registers available.  Now the INB INW INL&n;&t; * OUTB OUTW OUTL macros can be used safely.&n;&t; */
id|np-&gt;reg
op_assign
(paren
r_struct
...
id|np-&gt;vaddr
suffix:semicolon
multiline_comment|/* Do chip dependent initialization.  */
id|ncr_prepare_setting
c_func
(paren
id|np
...
suffix:semicolon
multiline_comment|/* This can happen if you forget to call ncr53c8xx_init from&n;&t; * your module_init */
id|BUG_ON
c_func
(paren
op_logical_neg
...
suffix:semicolon
multiline_comment|/* Patch script to physical addresses */
id|ncr_script_fill
c_func
(paren
op_amp
...
suffix:semicolon
multiline_comment|/* Patch the script for LED support.  */
r_if
c_cond
(paren
id|np-&gt;features
...
multiline_comment|/*&n;&t; * Look for the target control block of this nexus.&n;&t; * For i = 0 to 3&n;&t; *   JUMP ^ IFTRUE (MASK (i, 3)), @(next_lcb)&n;&t; */
r_for
c_loop
(paren
id|i
...
suffix:semicolon
multiline_comment|/* Now check the cache handling of the chipset.  */
r_if
c_cond
(paren
id|ncr_snooptest
...
id|device-&gt;slot.irq
suffix:semicolon
multiline_comment|/* Initialize the fixed part of the default ccb.  */
id|ncr_init_ccb
c_func
(paren
id|np
...
suffix:semicolon
multiline_comment|/*&n;&t; * After SCSI devices have been opened, we cannot reset the bus&n;&t; * safely, so we do it here.  Interrupt handler does the real work.&n;&t; * Process the reset exception if interrupts are not enabled yet.&n;&t; * Then enable disconnects.&n;&t; */
id|spin_lock_irqsave
c_func
(paren
op_amp
...
l_int|1
suffix:semicolon
multiline_comment|/*&n;&t; * The middle-level SCSI driver does not wait for devices to settle.&n;&t; * Wait synchronously if more than 2 seconds.&n;&t; */
r_if
c_cond
(paren
id|driver_setup.settle_delay
...
suffix:semicolon
multiline_comment|/* use SIMPLE TAG messages by default */
...
DECL|function|ncr53c8xx_set_period
r_static
r_void
id|ncr53c8xx_set_period
c_func
(paren
r_struct
id|scsi_target
...
DECL|function|ncr53c8xx_set_offset
r_static
r_void
id|ncr53c8xx_set_offset
c_func
(paren
r_struct
id|scsi_target
...
DECL|function|ncr53c8xx_set_width
r_static
r_void
id|ncr53c8xx_set_width
c_func
(paren
r_struct
id|scsi_target
...
id|ncr53c8xx_transport_functions
op_assign
(brace
dot
id|set_period
