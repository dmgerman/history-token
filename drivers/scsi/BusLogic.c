...
multiline_comment|/*static void BusLogic_ReportTargetDeviceInfo(BusLogic_HostAdapter_T&n;&t;&t;&t;&t;&t;    *HostAdapter)&n;{&n;  int TargetID;&n;*/
multiline_comment|/*&n;    Inhibit the Target Device Inquiry and Reporting if requested.&n;  */
multiline_comment|/*  if (BusLogic_MultiMasterHostAdapterP(HostAdapter) &amp;&amp;&n;      HostAdapter-&gt;DriverOptions != NULL &amp;&amp;&n;      HostAdapter-&gt;DriverOptions-&gt;LocalOptions.InhibitTargetInquiry)&n;    return;&n;*/
multiline_comment|/*&n;    Report on the Target Devices found.&n;  */
multiline_comment|/*  for (TargetID = 0; TargetID &lt; HostAdapter-&gt;MaxTargetDevices; TargetID++)&n;    {&n;      BusLogic_TargetFlags_T *TargetFlags = &amp;HostAdapter-&gt;TargetFlags[TargetID];&n;      if (TargetFlags-&gt;TargetExists &amp;&amp; !TargetFlags-&gt;TargetInfoReported)&n;&t;{&n;&t;  int SynchronousTransferRate = 0;&n;&t;  if (BusLogic_FlashPointHostAdapterP(HostAdapter))&n;&t;    {&n;&t;      unsigned char WideTransfersActive;&n;&t;      FlashPoint_InquireTargetInfo(&n;&t;&t;HostAdapter-&gt;CardHandle, TargetID,&n;&t;&t;&amp;HostAdapter-&gt;SynchronousPeriod[TargetID],&n;&t;&t;&amp;HostAdapter-&gt;SynchronousOffset[TargetID],&n;&t;&t;&amp;WideTransfersActive);&n;&t;      TargetFlags-&gt;WideTransfersActive = WideTransfersActive;&n;&t;    }&n;&t;  else if (TargetFlags-&gt;WideTransfersSupported &amp;&amp;&n;&t;&t;   (HostAdapter-&gt;WidePermitted &amp; (1 &lt;&lt; TargetID)) &amp;&amp;&n;&t;&t;   strcmp(HostAdapter-&gt;FirmwareVersion, &quot;5.06L&quot;) &lt; 0)&n;&t;    TargetFlags-&gt;WideTransfersActive = true;&n;&t;  if (HostAdapter-&gt;SynchronousPeriod[TargetID] &gt; 0)&n;&t;    SynchronousTransferRate =&n;&t;      100000 / HostAdapter-&gt;SynchronousPeriod[TargetID];&n;&t;  if (TargetFlags-&gt;WideTransfersActive)&n;&t;    SynchronousTransferRate &lt;&lt;= 1;&n;&t;  if (SynchronousTransferRate &gt;= 9950)&n;&t;    {&n;&t;      SynchronousTransferRate = (SynchronousTransferRate + 50) / 100;&n;&t;      BusLogic_Info(&quot;Target %d: Queue Depth %d, %sSynchronous at &quot;&n;&t;&t;&t;    &quot;%d.%01d MB/sec, offset %d&bslash;n&quot;,&n;&t;&t;&t;    HostAdapter, TargetID,&n;&t;&t;&t;    HostAdapter-&gt;QueueDepth[TargetID],&n;&t;&t;&t;    (TargetFlags-&gt;WideTransfersActive ? &quot;Wide &quot; : &quot;&quot;),&n;&t;&t;&t;    SynchronousTransferRate / 10,&n;&t;&t;&t;    SynchronousTransferRate % 10,&n;&t;&t;&t;    HostAdapter-&gt;SynchronousOffset[TargetID]);&n;&t;    }&n;&t;  else if (SynchronousTransferRate &gt; 0)&n;&t;    {&n;&t;      SynchronousTransferRate = (SynchronousTransferRate + 5) / 10;&n;&t;      BusLogic_Info(&quot;Target %d: Queue Depth %d, %sSynchronous at &quot;&n;&t;&t;&t;    &quot;%d.%02d MB/sec, offset %d&bslash;n&quot;,&n;&t;&t;&t;    HostAdapter, TargetID,&n;&t;&t;&t;    HostAdapter-&gt;QueueDepth[TargetID],&n;&t;&t;&t;    (TargetFlags-&gt;WideTransfersActive ? &quot;Wide &quot; : &quot;&quot;),&n;&t;&t;&t;    SynchronousTransferRate / 100,&n;&t;&t;&t;    SynchronousTransferRate % 100,&n;&t;&t;&t;    HostAdapter-&gt;SynchronousOffset[TargetID]);&n;&t;    }&n;&t;  else BusLogic_Info(&quot;Target %d: Queue Depth %d, Asynchronous&bslash;n&quot;,&n;&t;&t;&t;     HostAdapter, TargetID,&n;&t;&t;&t;     HostAdapter-&gt;QueueDepth[TargetID]);&n;&t;  TargetFlags-&gt;TargetInfoReported = true;&n;&t;}&n;    }&n;}&n;*/
...
multiline_comment|/* static void BusLogic_SelectQueueDepths(SCSI_Host_T *Host,&n;&t;&t;&t;&t;       SCSI_Device_T *DeviceList)&n;{&n;  BusLogic_HostAdapter_T *HostAdapter =&n;    (BusLogic_HostAdapter_T *) Host-&gt;hostdata;&n;  int TaggedDeviceCount = 0, AutomaticTaggedDeviceCount = 0;&n;  int UntaggedDeviceCount = 0, AutomaticTaggedQueueDepth = 0;&n;  int AllocatedQueueDepth = 0;&n;  SCSI_Device_T *Device;&n;  int TargetID;&n;  for (TargetID = 0; TargetID &lt; HostAdapter-&gt;MaxTargetDevices; TargetID++)&n;    if (HostAdapter-&gt;TargetFlags[TargetID].TargetExists)&n;      {&n;&t;int QueueDepth = HostAdapter-&gt;QueueDepth[TargetID];&n;&t;if (HostAdapter-&gt;TargetFlags[TargetID].TaggedQueuingSupported &amp;&amp;&n;&t;    (HostAdapter-&gt;TaggedQueuingPermitted &amp; (1 &lt;&lt; TargetID)))&n;&t;  {&n;&t;    TaggedDeviceCount++;&n;&t;    if (QueueDepth == 0) AutomaticTaggedDeviceCount++;&n;&t;  }&n;&t;else&n;&t;  {&n;&t;    UntaggedDeviceCount++;&n;&t;    if (QueueDepth == 0 ||&n;&t;&t;QueueDepth &gt; HostAdapter-&gt;UntaggedQueueDepth)&n;&t;      {&n;&t;&t;QueueDepth = HostAdapter-&gt;UntaggedQueueDepth;&n;&t;&t;HostAdapter-&gt;QueueDepth[TargetID] = QueueDepth;&n;&t;      }&n;&t;  }&n;&t;AllocatedQueueDepth += QueueDepth;&n;&t;if (QueueDepth == 1)&n;&t;  HostAdapter-&gt;TaggedQueuingPermitted &amp;= ~(1 &lt;&lt; TargetID);&n;      }&n;  HostAdapter-&gt;TargetDeviceCount = TaggedDeviceCount + UntaggedDeviceCount;&n;  if (AutomaticTaggedDeviceCount &gt; 0)&n;    {&n;      AutomaticTaggedQueueDepth =&n;&t;(HostAdapter-&gt;HostAdapterQueueDepth - AllocatedQueueDepth)&n;&t;/ AutomaticTaggedDeviceCount;&n;      if (AutomaticTaggedQueueDepth &gt; BusLogic_MaxAutomaticTaggedQueueDepth)&n;&t;AutomaticTaggedQueueDepth = BusLogic_MaxAutomaticTaggedQueueDepth;&n;      if (AutomaticTaggedQueueDepth &lt; BusLogic_MinAutomaticTaggedQueueDepth)&n;&t;AutomaticTaggedQueueDepth = BusLogic_MinAutomaticTaggedQueueDepth;&n;      for (TargetID = 0; TargetID &lt; HostAdapter-&gt;MaxTargetDevices; TargetID++)&n;&t;if (HostAdapter-&gt;TargetFlags[TargetID].TargetExists &amp;&amp;&n;&t;    HostAdapter-&gt;QueueDepth[TargetID] == 0)&n;&t;  {&n;&t;    AllocatedQueueDepth += AutomaticTaggedQueueDepth;&n;&t;    HostAdapter-&gt;QueueDepth[TargetID] = AutomaticTaggedQueueDepth;&n;&t;  }&n;    }&n;  for (Device = DeviceList; Device != NULL; Device = Device-&gt;next)&n;    if (Device-&gt;host == Host)&n;      Device-&gt;queue_depth = HostAdapter-&gt;QueueDepth[Device-&gt;id];&n;*/
multiline_comment|/* Allocate an extra CCB for each Target Device for a Bus Device Reset. */
multiline_comment|/*  AllocatedQueueDepth += HostAdapter-&gt;TargetDeviceCount;&n;  if (AllocatedQueueDepth &gt; HostAdapter-&gt;DriverQueueDepth)&n;    AllocatedQueueDepth = HostAdapter-&gt;DriverQueueDepth;&n;  BusLogic_CreateAdditionalCCBs(HostAdapter,&n;&t;&t;&t;&t;AllocatedQueueDepth&n;&t;&t;&t;&t;- HostAdapter-&gt;AllocatedCCBs,&n;&t;&t;&t;&t;false);&n;  if (HostAdapter == BusLogic_LastRegisteredHostAdapter)&n;    for (HostAdapter = BusLogic_FirstRegisteredHostAdapter;&n;&t; HostAdapter != NULL;&n;&t; HostAdapter = HostAdapter-&gt;Next)&n;      BusLogic_ReportTargetDeviceInfo(HostAdapter);&n;}&n;*/
...
id|Host-&gt;host_no
suffix:semicolon
multiline_comment|/*&n;       * This function is deprecated&n;      Host-&gt;select_queue_depths = BusLogic_SelectQueueDepths;&n;       */
