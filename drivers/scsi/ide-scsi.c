...
(brace
multiline_comment|/* is cmd active?&n;&t;&t; *  need to lock so this stuff doesn&squot;t change under us */
id|spin_lock_irqsave
c_func
(paren
op_amp
...
(brace
multiline_comment|/* yep - let&squot;s give it some more time - &n;&t;&t;&t; * we can do that, we&squot;re in _our_ error kernel thread */
id|spin_unlock_irqrestore
c_func
(paren
op_amp
...
multiline_comment|/* no, but is it queued in the ide subsystem? */
r_if
c_cond
(paren
id|elv_queue_empty
...
suffix:semicolon
multiline_comment|/* first null the handler for the drive and let any process&n;&t; * doing IO (on another CPU) run to (partial) completion&n;&t; * the lock prevents processing new requests */
id|spin_lock_irqsave
c_func
(paren
op_amp
...
multiline_comment|/* FIXME - this will probably leak memory */
id|HWGROUP
c_func
(paren
id|drive
...
suffix:semicolon
multiline_comment|/* finally, reset the drive (and its partner on the bus...) */
id|ide_do_reset
(paren
id|drive
)paren
