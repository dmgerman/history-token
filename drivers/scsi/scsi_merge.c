...
c_func
(paren
id|scsi_front_merge_fn_d
comma
id|front
comma
l_int|1
)paren
multiline_comment|/*&n; * Function:    __scsi_merge_requests_fn()&n; *&n; * Purpose:     Prototype for queue merge function.&n; *&n; * Arguments:   q       - Queue for which we are merging request.&n; *              req     - request into which we wish to merge.&n; *              next    - 2nd request that we might want to combine with req&n; *              dma_host - 1 if this host has ISA DMA issues (bus doesn&squot;t&n; *                      expose all of the address lines, so that DMA cannot&n; *                      be done from an arbitrary address).&n; *&n; * Returns:     1 if it is OK to merge the two requests.  0&n; *              if it is not OK.&n; *&n; * Lock status: queue lock is assumed to be held here.&n; *&n; * Notes:       Some drivers have limited scatter-gather table sizes, and&n; *              thus they cannot queue an infinitely large command.  This&n; *              function is called from ll_rw_blk before it attempts to merge&n; *              a new block into a request to make sure that the request will&n; *              not become too large.&n; */
...
multiline_comment|/*&n; * Function:    scsi_merge_requests_fn_()&n; *&n; * Purpose:     queue merge function.&n; *&n; * Arguments:   q       - Queue for which we are merging request.&n; *              req     - request into which we wish to merge.&n; *              bio     - Block which we may wish to merge into request&n; *&n; * Returns:     1 if it is OK to merge the block into the request.  0&n; *              if it is not OK.&n; *&n; * Lock status: queue lock is assumed to be held here.&n; *&n; * Notes:       Optimized for different cases depending upon whether&n; *              ISA DMA is in use and whether clustering should be used.&n; */
DECL|macro|MERGEREQFCT
mdefine_line|#define MERGEREQFCT(_FUNCTION, _DMA)&t;&t;&t;&bslash;&n;static int _FUNCTION(request_queue_t * q,&t;&t;&bslash;&n;&t;&t;     struct request * req,&t;&t;&bslash;&n;&t;&t;     struct request * next)&t;&t;&bslash;&n;{&t;&t;&t;&t;&t;&t;&t;&bslash;&n;    int ret;&t;&t;&t;&t;&t;&t;&bslash;&n;    ret =  __scsi_merge_requests_fn(q, req, next, _DMA); &bslash;&n;    return ret;&t;&t;&t;&t;&t;&t;&bslash;&n;}
id|MERGEREQFCT
c_func
(paren
