...
r_static
r_int
id|cpqfc_alloc_private_data_pool
c_func
(paren
id|CPQFCHBA
op_star
...
suffix:semicolon
multiline_comment|/* local function to load our per-HBA (local) data for chip&n;   registers, FC link state, all FC exchanges, etc.&n;&n;   We allocate space and compute address offsets for the&n;   most frequently accessed addresses; others (like World Wide&n;   Name) are not necessary.&n;*/
...
(paren
id|KERN_WARNING
l_string|&quot;cpqfc: unable to allocate pool for passthru ioctls.  &quot;
l_string|&quot;Passthru ioctls disabled.&bslash;n&quot;
)paren
suffix:semicolon
)brace
)brace
...
DECL|function|cpqfc_alloc_private_data_pool
r_static
r_int
id|cpqfc_alloc_private_data_pool
c_func
...
(brace
id|hba-&gt;private_data_bits
op_assign
l_int|NULL
suffix:semicolon
id|hba-&gt;private_data_pool
op_assign
l_int|NULL
suffix:semicolon
id|hba-&gt;private_data_bits
...
DECL|function|cpqfc_free_private_data_pool
r_static
r_void
id|cpqfc_free_private_data_pool
c_func
(paren
id|CPQFCHBA
op_star
...
DECL|function|is_private_data_of_cpqfc
r_int
...
(brace
multiline_comment|/* Is pointer within our private data pool?&n;&t;   We use Scsi_Request-&gt;upper_private_data (normally&n;&t;   reserved for upper layer drivers, e.g. the sg driver)&n;&t;   We check to see if the pointer is ours by looking at&n;&t;   its address.  Is this ok?   Hmm, it occurs to me that&n;&t;   a user app might do something bad by using sg to send&n;&t;   a cpqfc passthrough ioctl with upper_data_private&n;&t;   forged to be somewhere in our pool..., though they&squot;d&n;&t;   normally have to be root already to do this.  */
r_return
(paren
id|pointer
op_ne
...
DECL|function|cpqfc_alloc_private_data
id|cpqfc_passthru_private_t
op_star
id|cpqfc_alloc_private_data
c_func
(paren
id|CPQFCHBA
op_star
...
DECL|function|cpqfc_free_private_data
r_void
...
multiline_comment|/* Check that our pool got allocated ok. */
r_if
c_cond
(paren
id|cpqfcHBAdata-&gt;private_data_pool
op_eq
l_int|NULL
)paren
r_return
...
singleline_comment|// Now build a Scsi_Request to pass down...
id|ScsiPassThruReq
op_assign
...
id|ScsiPassThruReq-&gt;upper_private_data
op_assign
id|cpqfc_alloc_private_data
c_func
(paren
...
id|ScsiPassThruReq-&gt;sr_data_direction
op_assign
id|SCSI_DATA_WRITE
suffix:semicolon
)brace
...
r_else
singleline_comment|// maybe this means a bug in the user app
id|ScsiPassThruReq-&gt;sr_data_direction
op_assign
id|SCSI_DATA_NONE
suffix:semicolon
id|ScsiPassThruReq-&gt;sr_cmd_len
op_assign
l_int|0
suffix:semicolon
singleline_comment|// set correctly by scsi_do_req()
...
l_int|0
suffix:semicolon
singleline_comment|// We copy the scheme used by sd.c:spinup_disk() to submit commands
...
suffix:semicolon
singleline_comment|// retries
id|result
op_assign
id|ScsiPassThruReq-&gt;sr_result
suffix:semicolon
...
id|SDpnt
op_assign
id|ScsiPassThruReq-&gt;sr_device
suffix:semicolon
multiline_comment|/* upper_private_data is already freed in call_scsi_done() */
id|scsi_release_request
c_func
(paren
id|ScsiPassThruReq
...
id|SDpnt
suffix:semicolon
singleline_comment|// FIXME, cpqfcTS_TargetDeviceReset needs to be fixed 
singleline_comment|// similarly to how the passthrough ioctl was fixed 
singleline_comment|// around the 2.5.30 kernel.  Scsi_Cmnd replaced with 
singleline_comment|// Scsi_Request, etc.
singleline_comment|// For now, so people don&squot;t fall into a hole...
r_return
op_minus
id|ENOTSUPP
suffix:semicolon
multiline_comment|/*&n;  // printk(&quot;   ENTERING cpqfcTS_TargetDeviceReset() - flag=%d &bslash;n&quot;,reset_flags);&n;&n;  if (ScsiDev-&gt;host-&gt;eh_active) return FAILED;&n;&n;  memset( scsi_cdb, 0, sizeof( scsi_cdb));&n;&n;  scsi_cdb[0] = RELEASE;&n;&n;  SCpnt = scsi_get_command(ScsiDev, GFP_KERNEL);&n;  {&n;    CPQFC_DECLARE_COMPLETION(wait);&n;    &n;    SCpnt-&gt;SCp.buffers_residual = FCP_TARGET_RESET;&n;&n;&t;// FIXME: this would panic, SCpnt-&gt;request would be NULL.&n;&t;SCpnt-&gt;request-&gt;CPQFC_WAITING = &amp;wait;&n;&t;scsi_do_cmd(SCpnt,  scsi_cdb, NULL,  0, my_ioctl_done,  timeout, retries);&n;&t;CPQFC_WAIT_FOR_COMPLETION(&amp;wait);&n;&t;SCpnt-&gt;request-&gt;CPQFC_WAITING = NULL;&n;  }&n;    &n;&n;      if(driver_byte(SCpnt-&gt;result) != 0)&n;&t;  switch(SCpnt-&gt;sense_buffer[2] &amp; 0xf) {&n;&t;case ILLEGAL_REQUEST:&n;&t;    if(cmd[0] == ALLOW_MEDIUM_REMOVAL) dev-&gt;lockable = 0;&n;&t;    else printk(&quot;SCSI device (ioctl) reports ILLEGAL REQUEST.&bslash;n&quot;);&n;&t;    break;&n;&t;case NOT_READY: // This happens if there is no disc in drive &n;&t;    if(dev-&gt;removable &amp;&amp; (cmd[0] != TEST_UNIT_READY)){&n;&t;&t;printk(KERN_INFO &quot;Device not ready.  Make sure there is a disc in the drive.&bslash;n&quot;);&n;&t;&t;break;&n;&t;    }&n;&t;case UNIT_ATTENTION:&n;&t;    if (dev-&gt;removable){&n;&t;&t;dev-&gt;changed = 1;&n;&t;&t;SCpnt-&gt;result = 0; // This is no longer considered an error&n;&t;&t;// gag this error, VFS will log it anyway /axboe &n;&t;&t;// printk(KERN_INFO &quot;Disc change detected.&bslash;n&quot;); &n;&t;&t;break;&n;&t;    };&n;&t;default: // Fall through for non-removable media&n;&t;    printk(&quot;SCSI error: host %d id %d lun %d return code = %x&bslash;n&quot;,&n;&t;&t;   dev-&gt;host-&gt;host_no,&n;&t;&t;   dev-&gt;id,&n;&t;&t;   dev-&gt;lun,&n;&t;&t;   SCpnt-&gt;result);&n;&t;    printk(&quot;&bslash;tSense class %x, sense error %x, extended sense %x&bslash;n&quot;,&n;&t;&t;   sense_class(SCpnt-&gt;sense_buffer[0]),&n;&t;&t;   sense_error(SCpnt-&gt;sense_buffer[0]),&n;&t;&t;   SCpnt-&gt;sense_buffer[2] &amp; 0xf);&n;&t;    &n;      };&n;*/
id|result
op_assign
id|SCpnt-&gt;result
suffix:semicolon
...
c_func
(paren
l_string|&quot;cpqfc: ISR = 0x%02x&bslash;n&quot;
comma
id|IntStat
)paren
suffix:semicolon
...
id|pcistat
suffix:semicolon
multiline_comment|/* read the pci status register */
...
c_func
(paren
l_string|&quot;PCI status register is 0x%04x&bslash;n&quot;
comma
id|pcistat
)paren
suffix:semicolon
...
c_func
(paren
l_string|&quot;Parity Error Detected.&bslash;n&quot;
)paren
suffix:semicolon
r_if
c_cond
...
c_func
(paren
l_string|&quot;Signalled System Error&bslash;n&quot;
)paren
suffix:semicolon
r_if
c_cond
...
c_func
(paren
l_string|&quot;Received Master Abort&bslash;n&quot;
)paren
suffix:semicolon
r_if
c_cond
...
c_func
(paren
l_string|&quot;Received Target Abort&bslash;n&quot;
)paren
suffix:semicolon
r_if
c_cond
...
c_func
(paren
l_string|&quot;Signalled Target Abort&bslash;n&quot;
)paren
suffix:semicolon
)brace
r_if
