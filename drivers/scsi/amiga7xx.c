multiline_comment|/*&n; * Detection routine for the NCR53c710 based Amiga SCSI Controllers for Linux.&n; *&t;&t;Amiga MacroSystemUS WarpEngine SCSI controller.&n; *&t;&t;Amiga Technologies A4000T SCSI controller.&n; *&t;&t;Amiga Technologies/DKB A4091 SCSI controller.&n; *&n; * Written 1997 by Alan Hourihane &lt;alanh@fairlite.demon.co.uk&gt;&n; * plus modifications of the 53c7xx.c driver to support the Amiga.&n; */
macro_line|#include &lt;linux/types.h&gt;
macro_line|#include &lt;linux/mm.h&gt;
macro_line|#include &lt;linux/blkdev.h&gt;
macro_line|#include &lt;linux/sched.h&gt;
macro_line|#include &lt;linux/version.h&gt;
macro_line|#include &lt;linux/config.h&gt;
macro_line|#include &lt;linux/zorro.h&gt;
macro_line|#include &lt;linux/stat.h&gt;
macro_line|#include &lt;asm/setup.h&gt;
macro_line|#include &lt;asm/page.h&gt;
macro_line|#include &lt;asm/pgtable.h&gt;
macro_line|#include &lt;asm/amigaints.h&gt;
macro_line|#include &lt;asm/amigahw.h&gt;
macro_line|#include &lt;asm/dma.h&gt;
macro_line|#include &lt;asm/irq.h&gt;
macro_line|#include &quot;scsi.h&quot;
macro_line|#include &quot;hosts.h&quot;
macro_line|#include &quot;53c7xx.h&quot;
macro_line|#include &quot;amiga7xx.h&quot;
DECL|function|amiga7xx_register_one
r_static
r_int
id|amiga7xx_register_one
c_func
(paren
id|Scsi_Host_Template
op_star
id|tpnt
comma
r_int
r_int
id|address
)paren
(brace
r_int
r_int
id|options
suffix:semicolon
r_int
id|clock
suffix:semicolon
r_if
c_cond
(paren
op_logical_neg
id|request_mem_region
c_func
(paren
id|address
comma
l_int|0x1000
comma
l_string|&quot;ncr53c710&quot;
)paren
)paren
r_return
l_int|0
suffix:semicolon
id|address
op_assign
(paren
r_int
r_int
)paren
id|z_ioremap
c_func
(paren
id|address
comma
l_int|0x1000
)paren
suffix:semicolon
id|options
op_assign
id|OPTION_MEMORY_MAPPED
op_or
id|OPTION_DEBUG_TEST1
op_or
id|OPTION_INTFLY
op_or
id|OPTION_SYNCHRONOUS
op_or
id|OPTION_ALWAYS_SYNCHRONOUS
op_or
id|OPTION_DISCONNECT
suffix:semicolon
id|clock
op_assign
l_int|50000000
suffix:semicolon
multiline_comment|/* 50 MHz SCSI Clock */
id|ncr53c7xx_init
c_func
(paren
id|tpnt
comma
l_int|0
comma
l_int|710
comma
id|address
comma
l_int|0
comma
id|IRQ_AMIGA_PORTS
comma
id|DMA_NONE
comma
id|options
comma
id|clock
)paren
suffix:semicolon
r_return
l_int|1
suffix:semicolon
)brace
macro_line|#ifdef CONFIG_ZORRO
r_static
r_struct
(brace
DECL|member|id
id|zorro_id
id|id
suffix:semicolon
DECL|member|offset
r_int
r_int
id|offset
suffix:semicolon
DECL|member|absolute
r_int
id|absolute
suffix:semicolon
multiline_comment|/* offset is absolute address */
DECL|variable|amiga7xx_table
)brace
id|amiga7xx_table
(braket
)braket
op_assign
(brace
(brace
dot
id|id
op_assign
id|ZORRO_PROD_PHASE5_BLIZZARD_603E_PLUS
comma
dot
id|offset
op_assign
l_int|0xf40000
comma
dot
id|absolute
op_assign
l_int|1
)brace
comma
(brace
dot
id|id
op_assign
id|ZORRO_PROD_MACROSYSTEMS_WARP_ENGINE_40xx
comma
dot
id|offset
op_assign
l_int|0x40000
)brace
comma
(brace
dot
id|id
op_assign
id|ZORRO_PROD_CBM_A4091_1
comma
dot
id|offset
op_assign
l_int|0x800000
)brace
comma
(brace
dot
id|id
op_assign
id|ZORRO_PROD_CBM_A4091_2
comma
dot
id|offset
op_assign
l_int|0x800000
)brace
comma
(brace
dot
id|id
op_assign
id|ZORRO_PROD_GVP_GFORCE_040_060
comma
dot
id|offset
op_assign
l_int|0x40000
)brace
comma
(brace
l_int|0
)brace
)brace
suffix:semicolon
DECL|function|amiga7xx_zorro_detect
r_static
r_int
id|__init
id|amiga7xx_zorro_detect
c_func
(paren
id|Scsi_Host_Template
op_star
id|tpnt
)paren
(brace
r_int
id|num
op_assign
l_int|0
comma
id|i
suffix:semicolon
r_struct
id|zorro_dev
op_star
id|z
op_assign
l_int|NULL
suffix:semicolon
r_int
r_int
id|address
suffix:semicolon
r_while
c_loop
(paren
(paren
id|z
op_assign
id|zorro_find_device
c_func
(paren
id|ZORRO_WILDCARD
comma
id|z
)paren
)paren
)paren
(brace
r_for
c_loop
(paren
id|i
op_assign
l_int|0
suffix:semicolon
id|amiga7xx_table
(braket
id|i
)braket
dot
id|id
suffix:semicolon
id|i
op_increment
)paren
r_if
c_cond
(paren
id|z-&gt;id
op_eq
id|amiga7xx_table
(braket
id|i
)braket
dot
id|id
)paren
r_break
suffix:semicolon
r_if
c_cond
(paren
op_logical_neg
id|amiga7xx_table
(braket
id|i
)braket
dot
id|id
)paren
r_continue
suffix:semicolon
r_if
c_cond
(paren
id|amiga7xx_table
(braket
id|i
)braket
dot
id|absolute
)paren
id|address
op_assign
id|amiga7xx_table
(braket
id|i
)braket
dot
id|offset
suffix:semicolon
r_else
id|address
op_assign
id|z-&gt;resource.start
op_plus
id|amiga7xx_table
(braket
id|i
)braket
dot
id|offset
suffix:semicolon
id|num
op_add_assign
id|amiga7xx_register_one
c_func
(paren
id|tpnt
comma
id|address
)paren
suffix:semicolon
)brace
r_return
id|num
suffix:semicolon
)brace
macro_line|#endif /* CONFIG_ZORRO */
DECL|function|amiga7xx_detect
r_int
id|__init
id|amiga7xx_detect
c_func
(paren
id|Scsi_Host_Template
op_star
id|tpnt
)paren
(brace
r_static
r_int
r_char
id|called
op_assign
l_int|0
suffix:semicolon
r_int
id|num
op_assign
l_int|0
suffix:semicolon
r_if
c_cond
(paren
id|called
op_logical_or
op_logical_neg
id|MACH_IS_AMIGA
)paren
r_return
l_int|0
suffix:semicolon
id|tpnt-&gt;proc_name
op_assign
l_string|&quot;Amiga7xx&quot;
suffix:semicolon
r_if
c_cond
(paren
id|AMIGAHW_PRESENT
c_func
(paren
id|A4000_SCSI
)paren
)paren
id|num
op_add_assign
id|amiga7xx_register_one
c_func
(paren
id|tpnt
comma
l_int|0xdd0040
)paren
suffix:semicolon
macro_line|#ifdef CONFIG_ZORRO
id|num
op_add_assign
id|amiga7xx_zorro_detect
c_func
(paren
id|tpnt
)paren
suffix:semicolon
macro_line|#endif
id|called
op_assign
l_int|1
suffix:semicolon
r_return
id|num
suffix:semicolon
)brace
DECL|function|amiga7xx_release
r_static
r_int
id|amiga7xx_release
c_func
(paren
r_struct
id|Scsi_Host
op_star
id|shost
)paren
(brace
r_if
c_cond
(paren
id|shost-&gt;irq
)paren
id|free_irq
c_func
(paren
id|shost-&gt;irq
comma
l_int|NULL
)paren
suffix:semicolon
r_if
c_cond
(paren
id|shost-&gt;dma_channel
op_ne
l_int|0xff
)paren
id|free_dma
c_func
(paren
id|shost-&gt;dma_channel
)paren
suffix:semicolon
r_if
c_cond
(paren
id|shost-&gt;io_port
op_logical_and
id|shost-&gt;n_io_port
)paren
id|release_region
c_func
(paren
id|shost-&gt;io_port
comma
id|shost-&gt;n_io_port
)paren
suffix:semicolon
id|scsi_unregister
c_func
(paren
id|shost
)paren
suffix:semicolon
r_return
l_int|0
suffix:semicolon
)brace
DECL|variable|driver_template
r_static
id|Scsi_Host_Template
id|driver_template
op_assign
(brace
dot
id|name
op_assign
l_string|&quot;Amiga NCR53c710 SCSI&quot;
comma
dot
id|detect
op_assign
id|amiga7xx_detect
comma
dot
id|release
op_assign
id|amiga7xx_release
comma
dot
id|queuecommand
op_assign
id|NCR53c7xx_queue_command
comma
dot
m_abort
op_assign
id|NCR53c7xx_abort
comma
dot
id|reset
op_assign
id|NCR53c7xx_reset
comma
dot
id|can_queue
op_assign
l_int|24
comma
dot
id|this_id
op_assign
l_int|7
comma
dot
id|sg_tablesize
op_assign
l_int|63
comma
dot
id|cmd_per_lun
op_assign
l_int|3
comma
dot
id|use_clustering
op_assign
id|DISABLE_CLUSTERING
)brace
suffix:semicolon
macro_line|#include &quot;scsi_module.c&quot;
eof
