...
multiline_comment|/* This is the end routine we get to if a command was never attached&n; * to the request.  Simply complete the request without changing&n; * rq_status; this will cause a DRIVER_ERROR. */
DECL|function|scsi_wait_req_end_io
r_static
r_void
id|scsi_wait_req_end_io
c_func
(paren
r_struct
id|request
...
multiline_comment|/*&n; * Function:    scsi_end_request()&n; *&n; * Purpose:     Post-processing of completed commands (usually invoked at end&n; *&t;&t;of upper level post-processing and scsi_io_completion).&n; *&n; * Arguments:   cmd&t; - command that is complete.&n; *              uptodate - 1 if I/O indicates success, &lt;= 0 for I/O error.&n; *              bytes    - number of bytes of completed I/O&n; *&t;&t;requeue  - indicates whether we should requeue leftovers.&n; *&n; * Lock status: Assumed that lock is not held upon entry.&n; *&n; * Returns:     cmd if requeue done or required, NULL otherwise&n; *&n; * Notes:       This is called for block device requests in order to&n; *              mark some number of sectors as complete.&n; * &n; *&t;&t;We are guaranteeing that the request queue will be goosed&n; *&t;&t;at some point during this call.&n; */
...
(brace
multiline_comment|/* detected disc change.  set a bit &n;&t;&t;&t;&t; * and quietly refuse further access.&n;&t;&t;&t;&t; */
...
id|ILLEGAL_REQUEST
suffix:colon
multiline_comment|/*&n;&t;&t; &t;* If we had an ILLEGAL REQUEST returned, then we may&n;&t;&t; &t;* have performed an unsupported command.  The only&n;&t;&t; &t;* thing this should be would be a ten byte read where&n;&t;&t;&t;* only a six byte read was supported.  Also, on a&n;&t;&t;&t;* system where READ CAPACITY failed, we may have read&n;&t;&t;&t;* past the end of the disk.&n;&t;&t; &t;*/
r_if
c_cond
(paren
...
DECL|function|scsi_prepare_flush_fn
r_static
r_int
id|scsi_prepare_flush_fn
c_func
(paren
id|request_queue_t
op_star
...
DECL|function|scsi_end_flush_fn
r_static
r_void
id|scsi_end_flush_fn
c_func
(paren
id|request_queue_t
op_star
...
c_func
(paren
l_string|&quot;scsi: barrier error, disabling flush support&bslash;n&quot;
)paren
suffix:semicolon
...
multiline_comment|/*&n; * Kill requests for a dead device&n; */
DECL|function|scsi_kill_requests
r_static
r_void
id|scsi_kill_requests
c_func
(paren
id|request_queue_t
op_star
...
c_func
(paren
l_string|&quot;scsi: killing requests for dead queue&bslash;n&quot;
)paren
suffix:semicolon
id|scsi_kill_requests
c_func
...
suffix:semicolon
multiline_comment|/*&n;&t; * ordered tags are superior to flush ordering&n;&t; */
r_if
c_cond
(paren
...
r_void
DECL|function|scsi_target_block
...
DECL|variable|scsi_target_block
id|EXPORT_SYMBOL_GPL
c_func
(paren
id|scsi_target_block
...
r_void
DECL|function|scsi_target_unblock
...
DECL|variable|scsi_target_unblock
id|EXPORT_SYMBOL_GPL
c_func
(paren
id|scsi_target_unblock
