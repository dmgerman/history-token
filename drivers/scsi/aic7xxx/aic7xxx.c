...
r_static
r_void
id|ahc_update_pending_syncrates
c_func
(paren
r_struct
id|ahc_softc
...
id|modnext
suffix:semicolon
multiline_comment|/*&n;&t;&t;&t; * Clear 32bits of QOUTFIFO at a time&n;&t;&t;&t; * so that we don&squot;t clobber an incomming&n;&t;&t;&t; * byte DMA to the array on architectures&n;&t;&t;&t; * that only support 32bit load and store&n;&t;&t;&t; * operations.&n;&t;&t;&t; */
id|modnext
op_assign
id|ahc-&gt;qoutfifonext
op_amp
...
suffix:semicolon
multiline_comment|/*&n;&t;&t;&t;&t; * Save off the residual if there is one.&n;&t;&t;&t;&t; */
r_if
c_cond
(paren
id|ahc_check_residual
...
l_int|0
suffix:semicolon
multiline_comment|/*&n;&t;&t;&t;&t; * XXX Still true???&n;&t;&t;&t;&t; * Would be nice to preserve DISCENB here,&n;&t;&t;&t;&t; * but due to the way we manage busy targets,&n;&t;&t;&t;&t; * we can&squot;t.&n;&t;&t;&t;&t; */
id|hscb-&gt;control
op_assign
l_int|0
suffix:semicolon
multiline_comment|/*&n;&t;&t;&t;&t; * This request sense could be because the&n;&t;&t;&t;&t; * the device lost power or in some other&n;&t;&t;&t;&t; * way has lost our transfer negotiations.&n;&t;&t;&t;&t; * Renegotiate if appropriate.  Unit attention&n;&t;&t;&t;&t; * errors will be reported before any data&n;&t;&t;&t;&t; * phases occur.&n;&t;&t;&t;&t; */
r_if
c_cond
(paren
id|ahc_get_residual
...
multiline_comment|/*&n;&t;&t;&t;&t; * Ensure we have enough time to actually&n;&t;&t;&t;&t; * retrieve the sense.&n;&t;&t;&t;&t; */
id|untimeout
c_func
(paren
id|ahc_timeout
...
id|curphase
suffix:semicolon
multiline_comment|/*&n;&t;&t;&t; * The hardware will only let you ack bytes&n;&t;&t;&t; * if the expected phase in SCSISIGO matches&n;&t;&t;&t; * the current phase.  Make sure this is&n;&t;&t;&t; * currently the case.&n;&t;&t;&t; */
id|curphase
op_assign
id|ahc_inb
c_func
...
suffix:semicolon
id|tinfo-&gt;current.transport_version
op_assign
l_int|2
suffix:semicolon
...
id|tstate
suffix:semicolon
multiline_comment|/* Don&squot;t clean up the entry for our initiator role */
r_if
c_cond
(paren
(paren
...
r_break
suffix:semicolon
multiline_comment|/* Skip any DT entries if DT is not available */
r_if
c_cond
(paren
(paren
...
r_void
DECL|function|ahc_update_target_msg_request
id|ahc_update_target_msg_request
c_func
(paren
r_struct
...
(brace
multiline_comment|/* Update the message request bit for this target */
r_if
c_cond
(paren
op_logical_neg
...
id|ppr_options
suffix:semicolon
multiline_comment|/* Update the syncrates in any pending scbs */
id|ahc_update_pending_syncrates
c_func
(paren
id|ahc
...
multiline_comment|/*&n; * Update the user/goal/current tables of wide negotiation&n; * parameters as well as, in the case of a current or active update,&n; * any data structures on the host controller.  In the case of an&n; * active update, the specified target is currently talking to us on&n; * the bus, so the transfer parameter update must take effect&n; * immediately.&n; */
r_void
...
r_static
r_void
DECL|function|ahc_update_pending_syncrates
id|ahc_update_pending_syncrates
c_func
(paren
r_struct
...
multiline_comment|/*&n; * Wait for a complete incomming message, parse it, and respond accordingly.&n; */
r_static
r_int
...
c_func
(paren
l_string|&quot;(%s:%c:%d:%d): refuses tagged commands.  Performing &quot;
l_string|&quot;non-tagged I/O&bslash;n&quot;
comma
id|ahc_name
c_func
(paren
...
multiline_comment|/* XXX What about high address byte??? */
id|ahc_outb
c_func
(paren
id|ahc
...
suffix:semicolon
multiline_comment|/* XXX Perhaps better to just keep the saved address in sram */
r_if
c_cond
(paren
(paren
...
id|PAUSE
suffix:semicolon
multiline_comment|/* XXX The shared scb data stuff should be depricated */
r_if
c_cond
(paren
id|ahc-&gt;scb_data
...
id|list_ahc
suffix:semicolon
macro_line|#ifdef AHC_SUPPORT_PCI
multiline_comment|/*&n;&t; * Second Function PCI devices need to inherit some&n;&t; * settings from function 0.  We assume that function 0&n;&t; * will always be found prior to function 1.&n;&t; */
r_if
c_cond
(paren
(paren
...
r_void
DECL|function|ahc_init_probe_config
...
op_increment
suffix:semicolon
multiline_comment|/* Allocation for our ccbs */
r_if
c_cond
(paren
id|ahc_dmamem_alloc
...
id|buf
comma
l_string|&quot;%s Channel %c, SCSI Id=%d, &quot;
comma
id|type
comma
id|ahc-&gt;channel
...
c_func
(paren
l_string|&quot;%s: unable to allocate tmode_tstate.  &quot;
...
l_int|2
suffix:semicolon
id|tinfo-&gt;current.protocol_version
op_assign
l_int|2
suffix:semicolon
id|tinfo-&gt;current.transport_version
...
l_int|0
suffix:semicolon
multiline_comment|/* Wait until the XPT says its okay */
)brace
...
suffix:semicolon
multiline_comment|/* Don&squot;t have any special messages to send to targets */
id|ahc_outb
c_func
(paren
id|ahc
...
multiline_comment|/*&n;&t; * Go through the pending CCB list and look for&n;&t; * commands for this target that are still active.&n;&t; * These are other tagged commands that were&n;&t; * disconnected when the reset occured.&n;&t; */
id|scbp_next
op_assign
id|LIST_FIRST
c_func
...
c_func
(paren
l_string|&quot;%s: Dumping Card State at SEQADDR 0x%x&bslash;n&quot;
comma
id|ahc_name
c_func
(paren
...
c_func
(paren
l_string|&quot;SCSISEQ = 0x%x, SBLKCTL = 0x%x, SSTAT0 0x%x&bslash;n&quot;
comma
id|ahc_inb
c_func
(paren
...
l_int|0
suffix:semicolon
multiline_comment|/*&n;&t;&t; * Lazily update our position in the target mode incomming&n;&t;&t; * command queue as seen by the sequencer.&n;&t;&t; */
r_if
c_cond
(paren
(paren
