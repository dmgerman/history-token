...
l_int|0x01
suffix:semicolon
multiline_comment|/* SCSI_3 and higher, don&squot;t touch */
id|scsi_cmd
(braket
l_int|4
)braket
...
multiline_comment|/**&n; * scsi_scan_selected_lun - probe and add one LUN&n; *&n; * Description:&n; *     Probe a single LUN on @shost, @channel, @id and @lun. If the LUN is&n; *     found, set the queue depth, allocate command blocks, and call&n; *     init/attach/finish of the upper level (sd, sg, etc.) drivers.&n; **/
DECL|function|scsi_scan_selected_lun
r_static
r_void
id|scsi_scan_selected_lun
c_func
(paren
r_struct
id|Scsi_Host
...
(brace
multiline_comment|/*&n;&t;&t;&t;&t; * Low level driver failed to attach this&n;&t;&t;&t;&t; * device, we&squot;ve got to kick it back out&n;&t;&t;&t;&t; * now as a result :-(&n;&t;&t;&t;&t; */
id|printk
c_func
(paren
l_string|&quot;scsi_scan_selected_lun: slave_attach &quot;
l_string|&quot;failed, marking device OFFLINE.&bslash;n&quot;
)paren
suffix:semicolon
id|sdev-&gt;online
op_assign
...
multiline_comment|/**&n; * scan_scsis - scan the given adapter, or scan a single LUN&n; * @shost:&t;adapter to scan&n; * @hardcoded:&t;1 if a single channel/id/lun should be scanned, else 0&n; * @hchannel:&t;channel to scan for hardcoded case&n; * @hid:&t;target id to scan for hardcoded case&n; * @hlun:&t;lun to scan for hardcoded case&n; *&n; * Description:&n; *     If @hardcoded is 1, call scsi_scan_selected_lun to scan a single&n; *     LUN; else, iterate and call scsi_scan_target to scan all possible&n; *     target id&squot;s on all possible channels.&n; **/
...
(brace
multiline_comment|/*&n;&t;&t; * XXX Overload hchannel/hid/hlun to figure out what to&n;&t;&t; * scan, and use the standard scanning code rather than&n;&t;&t; * this function - that way, an entire bus (or fabric), or&n;&t;&t; * target id can be scanned. There are problems with queue&n;&t;&t; * depth and the init/attach/finish that must be resolved&n;&t;&t; * before (re-)scanning can handle finding more than one new&n;&t;&t; * LUN.&n;&t;&t; *&n;&t;&t; * For example, set hchannel 0 and hid to 5, and hlun to -1&n;&t;&t; * in order to scan all LUNs on channel 0, target id 5.&n;&t;&t; */
id|scsi_scan_selected_lun
c_func
(paren
id|shost
...
id|order_id
suffix:semicolon
multiline_comment|/*&n;&t;&t; * The blk layer queue allocation is a bit expensive to&n;&t;&t; * repeat for each channel and id - for FCP max_id is near&n;&t;&t; * 255: each call to scsi_alloc_sdev() implies a call to&n;&t;&t; * blk_init_queue, and then blk_init_free_list, where 2 *&n;&t;&t; * queue_nr_requests requests are allocated. Don&squot;t do so&n;&t;&t; * here for scsi_scan_selected_lun, since we end up&n;&t;&t; * calling select_queue_depths with an extra Scsi_Device&n;&t;&t; * on the host_queue list.&n;&t;&t; */
id|sdevscan
op_assign
id|scsi_alloc_sdev
c_func
...
r_return
suffix:semicolon
multiline_comment|/*&n;&t;&t; * The sdevscan host, channel, id and lun are filled in as&n;&t;&t; * needed to scan.&n;&t;&t; */
r_for
c_loop
(paren
id|channel
...
multiline_comment|/*&n;&t;&t;&t;&t;&t; * Scan from high to low id.&n;&t;&t;&t;&t;&t; */
id|order_id
op_assign
id|shost-&gt;max_id
op_minus
