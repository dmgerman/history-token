...
suffix:semicolon
DECL|variable|NCR_700_transport_template
id|STATIC
r_struct
id|scsi_transport_template
op_star
id|NCR_700_transport_template
op_assign
l_int|NULL
suffix:semicolon
...
id|STATUS_OFFSET
suffix:semicolon
multiline_comment|/* all of these offsets are L1_CACHE_BYTES separated.  It is fatal&n;&t; * if this isn&squot;t sufficient separation to avoid dma flushing issues */
id|BUG_ON
c_func
(paren
op_logical_neg
...
id|NCR_700_slave_destroy
suffix:semicolon
id|tpnt-&gt;change_queue_depth
op_assign
id|NCR_700_change_queue_depth
suffix:semicolon
id|tpnt-&gt;change_queue_type
op_assign
id|NCR_700_change_queue_type
suffix:semicolon
r_if
...
id|dev
comma
l_string|&quot;53c700: irq %lu request failed&bslash;n &quot;
comma
id|irq
)paren
suffix:semicolon
...
id|dev
comma
l_string|&quot;53c700: scsi_add_host failed&bslash;n&quot;
)paren
suffix:semicolon
r_goto
id|out_release_irq
...
id|SCp-&gt;device
comma
id|NCR_700_DEV_PRINT_SYNC_NEGOTIATION
)paren
)paren
(brace
r_if
...
(paren
id|KERN_INFO
l_string|&quot;scsi%d: (%d:%d) Asynchronous&bslash;n&quot;
comma
id|host-&gt;host_no
comma
id|pun
...
suffix:semicolon
multiline_comment|/* we&squot;re done negotiating */
...
suffix:semicolon
multiline_comment|/* OK, if TCQ still under negotiation, we now know it works */
r_if
c_cond
(paren
id|NCR_700_get_tag_neg_state
...
(paren
id|KERN_ERR
l_string|&quot;scsi%d: (%d:%d) HAS NO device&bslash;n&quot;
comma
id|host-&gt;host_no
comma
id|reselection_id
...
(paren
id|KERN_ERR
l_string|&quot;scsi%d: (%d:%d) no saved request for tag %d&bslash;n&quot;
comma
id|host-&gt;host_no
comma
id|reselection_id
...
(paren
(paren
l_string|&quot;53c700: %d:%d:%d, reselection is tag %d, slot %p(%d)&bslash;n&quot;
comma
id|host-&gt;host_no
comma
id|SDp-&gt;id
...
(paren
id|KERN_ERR
l_string|&quot;scsi%d: (%d:%d) no saved request for untagged cmd&bslash;n&quot;
comma
id|host-&gt;host_no
comma
id|reselection_id
...
multiline_comment|/* check for untagged commands.  We cannot have any outstanding&n;&t; * commands if we accept them.  Commands could be untagged because:&n;&t; *&n;&t; * - The tag negotiated bitmap is clear&n;&t; * - The blk layer sent and untagged command&n;&t; */
r_if
c_cond
(paren
id|NCR_700_get_depth
...
multiline_comment|/* here we may have to process an untagged command.  The gate&n;&t; * above ensures that this will be the only one outstanding,&n;&t; * so clear the tag negotiated bit.&n;&t; *&n;&t; * FIXME: This will royally screw up on multiple LUN devices&n;&t; * */
r_if
c_cond
(paren
op_logical_neg
...
(paren
id|KERN_INFO
l_string|&quot;scsi%d: (%d:%d) Disabling Tag Command Queuing&bslash;n&quot;
comma
id|SCp-&gt;device-&gt;host-&gt;host_no
comma
id|SCp-&gt;device-&gt;id
...
id|SCSI_NO_TAG
suffix:semicolon
multiline_comment|/* must populate current_cmnd for scsi_find_tag to work */
id|SCp-&gt;device-&gt;current_cmnd
op_assign
id|SCp
suffix:semicolon
)brace
...
l_int|NULL
suffix:semicolon
multiline_comment|/* Revalidate the transport parameters of the failing device */
r_if
c_cond
(paren
id|hostdata-&gt;fast
...
id|STATIC
r_void
DECL|function|NCR_700_set_period
id|NCR_700_set_period
c_func
(paren
r_struct
id|scsi_target
...
id|STATIC
r_void
DECL|function|NCR_700_set_offset
id|NCR_700_set_offset
c_func
(paren
r_struct
id|scsi_target
...
multiline_comment|/* if we&squot;re currently async, make sure the period is reasonable */
r_if
c_cond
(paren
id|spi_offset
...
suffix:semicolon
multiline_comment|/* to do here: allocate memory; build a queue_full list */
r_if
c_cond
(paren
...
r_else
(brace
multiline_comment|/* initialise to default depth */
...
(brace
multiline_comment|/* Find the correct offset and period via domain validation */
r_if
c_cond
(paren
op_logical_neg
...
(brace
multiline_comment|/* to do here: deallocate memory */
)brace
r_static
r_int
DECL|function|NCR_700_change_queue_depth
id|NCR_700_change_queue_depth
c_func
(paren
r_struct
...
DECL|function|NCR_700_change_queue_type
r_static
r_int
id|NCR_700_change_queue_type
c_func
...
suffix:semicolon
multiline_comment|/* We have a global (per target) flag to track whether TCQ is&n;&t; * enabled, so we&squot;ll be turning it off for the entire target here.&n;&t; * our tag algorithm will fail if we mix tagged and untagged commands,&n;&t; * so quiesce the device before doing this */
r_if
c_cond
(paren
id|change_tag
...
(brace
multiline_comment|/* shift back to the default unqueued number of commands&n;&t;&t; * (the user can still raise this) */
id|scsi_deactivate_tcq
c_func
(paren
id|SDp
...
r_else
(brace
multiline_comment|/* Here, we cleared the negotiation flag above, so this&n;&t;&t; * will force the driver to renegotiate */
id|scsi_activate_tcq
c_func
(paren
id|SDp
...
r_static
id|ssize_t
DECL|function|NCR_700_show_active_tags
id|NCR_700_show_active_tags
c_func
(paren
r_struct
id|device
...
DECL|variable|NCR_700_active_tags_attr
r_static
r_struct
id|device_attribute
...
id|NCR_700_transport_functions
op_assign
(brace
dot
id|set_period
