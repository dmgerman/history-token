multiline_comment|/*&n; * sim710.c - Copyright (C) 1999 Richard Hirst &lt;richard@sleepie.demon.co.uk&gt;&n; *&n; *----------------------------------------------------------------------------&n; *  This program is free software; you can redistribute it and/or modify&n; *  it under the terms of the GNU General Public License as published by &n; *  the Free Software Foundation; either version 2 of the License, or&n; *  (at your option) any later version.&n; *&n; *  This program is distributed in the hope that it will be useful,&n; *  but WITHOUT ANY WARRANTY; without even the implied warranty of&n; *  MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the&n; *  GNU General Public License for more details.&n; *&n; *  You should have received a copy of the GNU General Public License&n; *  along with this program; if not, write to the Free Software&n; *  Foundation, Inc., 675 Mass Ave, Cambridge, MA 02139, USA.&n; *----------------------------------------------------------------------------&n; *&n; * MCA card detection code by Trent McNair.&n; * Fixes to not explicitly nul bss data from Xavier Bestel.&n; * Some multiboard fixes from Rolf Eike Beer.&n; * Auto probing of EISA config space from Trevor Hemsley.&n; *&n; * Various bits of code in this driver have been copied from 53c7,8xx,c,&n; * which is coyright Drew Eckhardt.  The scripts for the SCSI chip are&n; * compiled with the script compiler written by Drew.&n; *&n; * This is a simple driver for the NCR53c710.  More complex drivers&n; * for this chip (e.g. 53c7xx.c) require that the scsi chip be able to&n; * do DMA block moves between memory and on-chip registers, which can&n; * be a problem if those registers are in the I/O address space.  There&n; * can also be problems on hardware where the registers are memory&n; * mapped, if the design is such that memory-to-memory transfers initiated&n; * by the scsi chip cannot access the chip registers.&n; *&n; * This driver is designed to avoid these problems and is intended to&n; * work with any Intel machines using 53c710 chips, including various&n; * Compaq and NCR machines.  It was initially written for the Tadpole&n; * TP34V VME board which is 68030 based.&n; *&n; * The driver supports boot-time parameters similar to&n; *&t;sim710=addr:0x9000,irq:15&n; * and insmod parameters similar to&n; *&t;sim710=&quot;addr:0x9000 irq:15&quot;&n; *&n; * Multiple controllers can also be set up by command line, provided the&n; * addr: parameter is specified first for each controller.  e.g.&n; *      sim710=&quot;addr:0x9000 irq:15 addr:0x8000 irq:14&quot;&n; *&n; * To separate the different options, &squot; &squot;, &squot;+&squot;, and &squot;,&squot; can be used, except&n; * that &squot;,&squot; can not be used in module parameters.  &squot; &squot; can be a pain, because&n; * it needs to be quoted, which causes problems with some installers.&n; * The command line above is completely equivalent to&n; *      sim710=&quot;addr:0x9000+irq:15+addr:0x8000+irq:14&quot;&n; *&n; * The complete list of options are:&n; *&n; * addr:0x9000&t;&t;Specifies the base I/O port (or address) of the 53C710.&n; * irq:15&t;&t;Specifies the IRQ number used by the 53c710.&n; * debug:0xffff&t;&t;Generates lots of debug output.&n; * ignore:0x0a&t;&t;Makes the driver ignore SCSI IDs 0 and 2.&n; * nodisc:0x70&t;&t;Prevents disconnects from IDs 6, 5 and 4.&n; * noneg:0x10&t;&t;Prevents SDTR negotiation on ID 4.&n; * disabled:1&t;&t;Completely disables the driver. When present, overrides&n; *&t;&t;&t;all other options.&n; *&n; * The driver will auto-probe chip addresses and IRQs now, so typically no&n; * parameters are needed.  Auto-probing of addresses is disabled if any addr:&n; * parameters are specified.&n; *&n; * Current limitations:&n; *&n; * o  Async only&n; * o  Severely lacking in error recovery&n; * o  &squot;debug:&squot; should be per host really.&n; *&n; */
...
multiline_comment|/* All targets are I/O mapped at the moment */
...
mdefine_line|#define MAXBOARDS 4&t;/* Increase this and the sizes of the&n;&t;&t;&t;   arrays below, if you need more.. */
...
DECL|variable|hostdata_order
r_static
r_int
id|hostdata_order
suffix:semicolon
multiline_comment|/* Encoded size of hostdata for free_pages() */
DECL|variable|no_of_boards
r_static
r_int
id|no_of_boards
suffix:semicolon
multiline_comment|/* Actual number of boards/chips */
DECL|variable|bases
r_static
r_int
r_int
...
suffix:semicolon
multiline_comment|/* Base addresses of chips */
...
suffix:semicolon
multiline_comment|/* IRQ vectors used by chips */
...
suffix:semicolon
multiline_comment|/*&n; * Function : static void ncr_dump (struct Scsi_Host *host)&n; *&n; * Purpose :  Dump (possibly) useful info&n; *&n; * Inputs : host - pointer to this host adapter&squot;s structure&n; */
r_static
r_void
DECL|function|ncr_dump
id|ncr_dump
(paren
r_struct
id|Scsi_Host
...
c_func
(paren
l_string|&quot;scsi%d: Chip register contents:&bslash;n&quot;
comma
id|host-&gt;host_no
)paren
suffix:semicolon
...
c_func
(paren
l_string|&quot; (script at virt %p, bus %lx)&bslash;n&quot;
comma
id|hostdata-&gt;script
comma
id|virt_to_bus
...
c_func
(paren
l_string|&quot; 00  sien:  %02x  sdid:  %02x  scntl1:%02x  scntl0:%02x&bslash;n&quot;
l_string|&quot; 04  socl:  %02x  sodl:  %02x  sxfer: %02x  scid:  %02x&bslash;n&quot;
l_string|&quot; 08  sbcl:  %02x  sbdl:  %02x  sidl:  %02x  sfbr:  %02x&bslash;n&quot;
l_string|&quot; 0C  sstat2:%02x  sstat1:%02x  sstat0:%02x  dstat: %02x&bslash;n&quot;
l_string|&quot; 10  dsa:   %08x&bslash;n&quot;
l_string|&quot; 14  ctest3:%02x  ctest2:%02x  ctest1:%02x  ctest0:%02x&bslash;n&quot;
l_string|&quot; 18  ctest7:%02x  ctest6:%02x  ctest5:%02x  ctest4:%02x&bslash;n&quot;
l_string|&quot; 1C  temp:  %08x&bslash;n&quot;
l_string|&quot; 20  lcrc:  %02x  ctest8:%02x  istat: %02x  dfifo: %02x&bslash;n&quot;
l_string|&quot; 24  dbc:   %08x  dnad:  %08x  dsp:   %08x&bslash;n&quot;
l_string|&quot; 30  dsps:  %08x  scratch:%08x&bslash;n&quot;
l_string|&quot; 38  dcntl: %02x  dwt:   %02x  dien:  %02x  dmode: %02x&bslash;n&quot;
l_string|&quot; 3C  adder: %08x&bslash;n&quot;
comma
id|NCR_read8
c_func
(paren
...
c_func
(paren
l_string|&quot;sim710: Invalid parameters, addr: must come first&bslash;n&quot;
)paren
suffix:semicolon
id|no_of_boards
op_assign
...
c_func
(paren
l_string|&quot;sim710: unexpected boot option &squot;%.*s&squot;&bslash;n&quot;
comma
(paren
r_int
)paren
...
multiline_comment|/* Allow &squot;,&squot;, &squot; &squot;, or &squot;+&squot; separators.  Used to be &squot;,&squot; at boot and&n;&t; * &squot; &squot; for module load, some installers crap out on the space and&n;&t; * insmod doesn&squot;t like the comma.&n;&t; */
r_if
c_cond
(paren
(paren
...
suffix:semicolon
multiline_comment|/* Stage 0 : eat all interrupts&n;       Stage 1 : set ABORT&n;       Stage 2 : eat all but abort interrupts&n;       Stage 3 : eat all interrupts&n;       We loop for 50000 times with a delay of 10us which should give us&n;       about half a second.&n;     */
r_for
c_loop
(paren
id|stage
...
multiline_comment|/* Handle incoming Synchronous data transfer request.  If our negotiate&n; * flag is set then this is a response to our request, otherwise it is&n; * spurious request from the target.  Don&squot;t really expect target initiated&n; * SDTRs, because we always negotiate on the first command.  Could still&n; * get them though..&n; * The chip is currently paused with ACK asserted on the last byte of the&n; * SDTR.&n; * resa is the resume address if the message is in response to our outgoing&n; * SDTR.  Only possible on initial identify.&n; * resb is the resume address if the message exchange is initiated by the&n; * target.&n; */
r_static
id|u32
...
(brace
multiline_comment|/* Some devices do this on parity error, at least */
id|printk
c_func
(paren
l_string|&quot;scsi%d: Unexpected switch to STATUSIN on initial message out&bslash;n&quot;
comma
id|host-&gt;host_no
)paren
suffix:semicolon
...
(brace
multiline_comment|/* Change to Status In at some random point; probably wants to report a&n;&t; * parity error or similar.&n;&t; */
id|printk
c_func
(paren
l_string|&quot;scsi%d: Unexpected phase change to STATUSIN at index 0x%x&bslash;n&quot;
comma
id|host-&gt;host_no
comma
id|index
...
c_func
(paren
l_string|&quot;  msgin_buf: %02x %02x %02x %02x&bslash;n&quot;
comma
id|p
(braket
l_int|0
...
multiline_comment|/* Auto probe, if no boards specified in boot parameters */
r_if
c_cond
(paren
id|no_of_boards
...
id|io_addr
suffix:semicolon
multiline_comment|/* reverse probe, so my on-board controller at 0x9000 is always scsi0 */
r_for
c_loop
(paren
id|io_addr
...
suffix:semicolon
multiline_comment|/* The on-board controller on my Proliant 2000 is 0x1044,&n;&t;&t; * my EISA card is 0x1144.&n;&t;&t; */
r_if
c_cond
(paren
id|id0
...
suffix:semicolon
macro_line|#if 0
multiline_comment|/* This should detect the IRQ, but I havn&squot;t proved it for&n;&t;&t;     * myself.  Leave the old probe code active for now, as&n;&t;&t;     * no-one has reported problems with it.&n;&t;&t;     */
r_switch
c_cond
(paren
id|inb
...
c_func
(paren
l_string|&quot;scsi%d: Configuring Sim710 (SCSI-ID %d) at %x, IRQ %d&bslash;n&quot;
comma
id|host-&gt;host_no
comma
id|scsi_id
...
c_func
(paren
l_string|&quot;scsi%d: Failed to initialise 53c710 at address %x&bslash;n&quot;
comma
id|host-&gt;host_no
comma
id|base_addr
...
suffix:semicolon
multiline_comment|/* Now run test1 */
id|hostdata-&gt;test1_src
op_assign
l_int|0x53c710aa
suffix:semicolon
...
c_func
(paren
l_string|&quot;scsi%d: Chip is using IRQ %d&bslash;n&quot;
comma
id|host-&gt;host_no
comma
id|probed_irq
...
c_func
(paren
l_string|&quot;scsi%d: Failed to probe for IRQ (returned %d)&bslash;n&quot;
comma
id|host-&gt;host_no
comma
id|probed_irq
...
c_func
(paren
l_string|&quot;scsi%d: WARNING requested IRQ %d, but probed as %d&bslash;n&quot;
comma
id|host-&gt;host_no
comma
id|requested_irq
...
c_func
(paren
l_string|&quot;scsi%d: WARNING IRQ probe failed, (returned %d)&bslash;n&quot;
comma
id|host-&gt;host_no
comma
id|probed_irq
...
c_func
(paren
l_string|&quot;scsi%d: test 1 FAILED: data: exp 0x53c710aa, got 0x%08x&bslash;n&quot;
comma
id|host-&gt;host_no
comma
id|hostdata-&gt;test1_dst
...
c_func
(paren
l_string|&quot;scsi%d: test 1 FAILED: dsps: exp 0x%08x, got 0x%08x&bslash;n&quot;
comma
id|host-&gt;host_no
comma
id|A_int_test1
...
c_func
(paren
l_string|&quot;scsi%d: test 1 completed ok.&bslash;n&quot;
comma
id|host-&gt;host_no
)paren
suffix:semicolon
...
multiline_comment|/* OK, now run down our_hosts[] calling request_irq(... SA_SHIRQ ...).&n;     * Couldn&squot;t call request_irq earlier, as probing would have failed.&n;     */
r_for
c_loop
(paren
id|indx
...
c_func
(paren
l_string|&quot;scsi%d: istat = %02x, sstat0 = %02x, sstat1 = %02x, dstat = %02x&bslash;n&quot;
comma
id|host-&gt;host_no
comma
id|istat
...
c_func
(paren
l_string|&quot;scsi%d: dsp = %08x (script[0x%04x]), dsps = %08x, scratch = %08x&bslash;n&quot;
comma
id|host-&gt;host_no
comma
id|dsp
