...
r_static
r_int
id|megaraid_sysfs_alloc_resources
c_func
(paren
id|adapter_t
op_star
...
r_static
r_void
id|megaraid_sysfs_free_resources
c_func
(paren
id|adapter_t
op_star
...
r_static
id|ssize_t
id|megaraid_sysfs_show_app_hndl
c_func
(paren
r_struct
id|class_device
...
id|PCI_VENDOR_ID_LSI_LOGIC
comma
id|PCI_DEVICE_ID_MEGARAID_NEC_ROMB_2E
comma
id|PCI_VENDOR_ID_NEC
comma
id|PCI_SUBSYS_ID_MEGARAID_NEC_ROMB_2E
comma
)brace
comma
(brace
...
suffix:semicolon
singleline_comment|// definitions for the device attributes for exporting logical drive number
singleline_comment|// for a scsi address (Host, Channel, Id, Lun)
id|CLASS_DEVICE_ATTR
c_func
(paren
id|megaraid_mbox_app_hndl
comma
id|S_IRUSR
comma
id|megaraid_sysfs_show_app_hndl
...
suffix:semicolon
singleline_comment|// Host template initializer for megaraid mbox sysfs device attributes
DECL|variable|megaraid_shost_attrs
r_static
r_struct
id|class_device_attribute
op_star
id|megaraid_shost_attrs
(braket
)braket
op_assign
(brace
op_amp
id|class_device_attr_megaraid_mbox_app_hndl
comma
l_int|NULL
comma
)brace
...
suffix:semicolon
singleline_comment|// Host template initializer for megaraid mbox sysfs device attributes
DECL|variable|megaraid_sdev_attrs
r_static
r_struct
id|device_attribute
op_star
id|megaraid_sdev_attrs
(braket
)braket
op_assign
(brace
op_amp
id|dev_attr_megaraid_mbox_ld
comma
l_int|NULL
comma
)brace
...
l_int|0xFF
suffix:semicolon
id|raid_dev-&gt;random_del_supported
op_assign
l_int|1
suffix:semicolon
)brace
...
id|megaraid_cmd_per_lun
suffix:semicolon
multiline_comment|/*&n;&t; * Allocate resources required to issue FW calls, when sysfs is&n;&t; * accessed&n;&t; */
r_if
c_cond
(paren
id|megaraid_sysfs_alloc_resources
...
id|scb-&gt;dev_target
suffix:semicolon
singleline_comment|// 0=6sec, 1=60sec, 2=10min, 3=3hrs, 4=NO timeout
id|pthru-&gt;timeout
op_assign
l_int|4
suffix:semicolon
...
multiline_comment|/**&n; * megaraid_sysfs_alloc_resources - allocate sysfs related resources&n; *&n; * Allocate packets required to issue FW calls whenever the sysfs attributes&n; * are read. These attributes would require up-to-date information from the&n; * FW. Also set up resources for mutual exclusion to share these resources and&n; * the wait queue.&n; *&n; * @param adapter : controller&squot;s soft state&n; *&n; * @return 0 on success&n; * @return -ERROR_CODE on failure&n; */
r_static
r_int
DECL|function|megaraid_sysfs_alloc_resources
id|megaraid_sysfs_alloc_resources
c_func
(paren
id|adapter_t
...
multiline_comment|/**&n; * megaraid_sysfs_free_resources - free sysfs related resources&n; *&n; * Free packets allocated for sysfs FW commands&n; *&n; * @param adapter : controller&squot;s soft state&n; */
r_static
r_void
DECL|function|megaraid_sysfs_free_resources
id|megaraid_sysfs_free_resources
c_func
(paren
id|adapter_t
...
multiline_comment|/**&n; * megaraid_sysfs_get_ldmap_done - callback for get ldmap&n; *&n; * Callback routine called in the ISR/tasklet context for get ldmap call&n; *&n; * @param uioc : completed packet&n; */
r_static
r_void
DECL|function|megaraid_sysfs_get_ldmap_done
id|megaraid_sysfs_get_ldmap_done
c_func
(paren
id|uioc_t
op_star
...
multiline_comment|/**&n; * megaraid_sysfs_get_ldmap_timeout - timeout handling for get ldmap&n; *&n; * Timeout routine to recover and return to application, in case the adapter&n; * has stopped responding. A timeout of 60 seconds for this command seem like&n; * a good value&n; *&n; * @param uioc : timed out packet&n; */
r_static
r_void
DECL|function|megaraid_sysfs_get_ldmap_timeout
id|megaraid_sysfs_get_ldmap_timeout
c_func
(paren
r_int
r_int
...
multiline_comment|/**&n; * megaraid_sysfs_get_ldmap - get update logical drive map&n; *&n; * This routine will be called whenever user reads the logical drive&n; * attributes, go get the current logical drive mapping table from the&n; * firmware. We use the managment API&squot;s to issue commands to the controller.&n; *&n; * NOTE: The commands issuance functionality is not generalized and&n; * implemented in context of &quot;get ld map&quot; command only. If required, the&n; * command issuance logical can be trivially pulled out and implemented as a&n; * standalone libary. For now, this should suffice since there is no other&n; * user of this interface.&n; *&n; * @param adapter : controller&squot;s soft state&n; *&n; * @return 0 on success&n; * @return -1 on failure&n; */
r_static
r_int
DECL|function|megaraid_sysfs_get_ldmap
id|megaraid_sysfs_get_ldmap
c_func
(paren
id|adapter_t
op_star
...
l_int|0
suffix:semicolon
multiline_comment|/*&n;&t; * Allow only one read at a time to go through the sysfs attributes&n;&t; */
id|down
c_func
(paren
op_amp
...
id|megaraid_sysfs_get_ldmap_done
suffix:semicolon
multiline_comment|/*&n;&t; * Prepare the mailbox packet to get the current logical drive mapping&n;&t; * table&n;&t; */
id|mbox-&gt;xferaddr
op_assign
(paren
r_uint32
...
id|OP_GET_LDID_MAP
suffix:semicolon
multiline_comment|/*&n;&t; * Setup a timer to recover from a non-responding controller&n;&t; */
id|timerp
op_assign
op_amp
id|sysfs_timer
...
suffix:semicolon
multiline_comment|/*&n;&t; * Send the command to the firmware&n;&t; */
id|rval
op_assign
id|megaraid_mbox_mm_command
c_func
...
(brace
singleline_comment|// command successfully issued
id|wait_event
c_func
(paren
id|raid_dev-&gt;sysfs_wait_q
...
suffix:semicolon
multiline_comment|/*&n;&t;&t; * Check if the command timed out&n;&t;&t; */
r_if
c_cond
(paren
id|uioc-&gt;status
...
(paren
id|KERN_NOTICE
l_string|&quot;megaraid: sysfs get ld map timed out&bslash;n&quot;
)paren
)paren
suffix:semicolon
id|rval
...
(paren
id|KERN_NOTICE
l_string|&quot;megaraid: get ld map failed with %x&bslash;n&quot;
comma
id|rval
)paren
)paren
...
(paren
id|KERN_NOTICE
l_string|&quot;megaraid: could not issue ldmap command:%x&bslash;n&quot;
comma
id|rval
)paren
)paren
...
multiline_comment|/**&n; * megaraid_sysfs_show_app_hndl - display application handle for this adapter&n; *&n; * Display the handle used by the applications while executing management&n; * tasks on the adapter. We invoke a management module API to get the adapter&n; * handle, since we do not interface with applications directly.&n; *&n; * @param cdev&t;: class device object representation for the host&n; * @param buf&t;: buffer to send data to&n; */
r_static
id|ssize_t
DECL|function|megaraid_sysfs_show_app_hndl
id|megaraid_sysfs_show_app_hndl
c_func
(paren
r_struct
...
multiline_comment|/**&n; * megaraid_sysfs_show_ldnum - display the logical drive number for this device&n; *&n; * Display the logical drive number for the device in question, if it a valid&n; * logical drive. For physical devices, &quot;-1&quot; is returned&n; * The logical drive number is displayed in following format&n; *&n; * &lt;SCSI ID&gt; &lt;LD NUM&gt; &lt;LD STICKY ID&gt; &lt;APP ADAPTER HANDLE&gt;&n; *   &lt;int&gt;     &lt;int&gt;       &lt;int&gt;            &lt;int&gt;&n; *&n; * @param dev&t;: device object representation for the scsi device&n; * @param buf&t;: buffer to send data to&n; */
r_static
id|ssize_t
DECL|function|megaraid_sysfs_show_ldnum
id|megaraid_sysfs_show_ldnum
c_func
(paren
r_struct
...
(paren
id|KERN_NOTICE
l_string|&quot;megaraid: sysfs get ld map failed: %x&bslash;n&quot;
comma
id|rval
)paren
)paren
