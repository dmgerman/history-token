multiline_comment|/* WARNING: GENERATED FILE (from 53c700.c), DO NOT MODIFY */
...
(paren
id|KERN_ERR
l_string|&quot;53c700: Failed to allocate script, detatching&bslash;n&quot;
)paren
suffix:semicolon
id|scsi_unregister
c_func
...
(paren
id|KERN_ERR
l_string|&quot;53c700: Failed to allocate command slots, detatching&bslash;n&quot;
)paren
suffix:semicolon
id|scsi_unregister
c_func
...
multiline_comment|/* bus physical address of script */
id|pScript
op_assign
id|virt_to_bus
c_func
...
multiline_comment|/*&n; * Function : static int datapath_residual (Scsi_Host *host)&n; *&n; * Purpose : return residual data count of what&squot;s in the chip.  If you&n; * really want to know what this function is doing, it&squot;s almost a&n; * direct transcription of the algorithm described in the 53c710&n; * guide, except that the DBC and DFIFO registers are only 6 bits&n; * wide.&n; *&n; * Inputs : host - SCSI host */
r_static
r_inline
r_int
...
(paren
id|KERN_WARNING
l_string|&quot;53c700: Period %dns is less than SCSI-2 minimum, setting to %d&bslash;n&quot;
comma
id|period
op_star
l_int|4
...
(paren
id|KERN_WARNING
l_string|&quot;53c700: Offset %d exceeds maximum, setting to %d&bslash;n&quot;
comma
id|offset
comma
id|NCR_700_MAX_OFFSET
...
c_func
(paren
l_string|&quot; ORIGINAL CMD %p RETURNED %d, new return is %d sense is&quot;
comma
id|SCp
comma
id|SCp-&gt;cmnd
...
suffix:semicolon
multiline_comment|/* we can destroy the command here because the&n;&t;&t;&t;&t; * contingent allegiance condition will cause a &n;&t;&t;&t;&t; * retry which will re-copy the command from the&n;&t;&t;&t;&t; * saved data_cmnd */
id|SCp-&gt;cmnd
(braket
l_int|0
)braket
...
l_int|6
suffix:semicolon
multiline_comment|/* Here&squot;s a quiet hack: the REQUEST_SENSE command is&n;&t;&t;&t;&t; * six bytes, so store a flag indicating that this&n;&t;&t;&t;&t; * was an internal sense request and the original&n;&t;&t;&t;&t; * status at the end of the command */
id|SCp-&gt;cmnd
(braket
l_int|6
)braket
...
(brace
multiline_comment|/* Piggy back the tag queueing support&n;&t;&t;&t;&t; * on this command */
r_if
c_cond
(paren
(paren
...
(paren
id|KERN_INFO
l_string|&quot;scsi%d: (%d:%d) RESELECTION DURING SELECTION, dsp=%p[%04x] state=%d, count=%d&bslash;n&quot;
comma
id|host-&gt;host_no
comma
id|reselection_id
...
(paren
id|KERN_INFO
l_string|&quot;IDENTIFIED SG segment as being %p in slot %p, cmd %p, slot-&gt;resume_offset=%p&bslash;n&quot;
comma
(paren
r_void
op_star
...
(paren
id|KERN_NOTICE
l_string|&quot;scsi%d (%d:%d) DEBUG INTERRUPT %d AT %p[%04x], continuing&bslash;n&quot;
comma
id|host-&gt;host_no
comma
id|pun
...
multiline_comment|/* REQUEST_SENSE is asking for contingent I_T_L status.  If a&n;&t; * contingent allegiance condition exists, the device will&n;&t; * refuse all tags, so send the request sense as untagged */
r_if
c_cond
(paren
(paren
...
(paren
id|KERN_ERR
l_string|&quot;scsi%d: Bus Reset detected, executing command %p, slot %p, dsp %p[%04x]&bslash;n&quot;
comma
id|host-&gt;host_no
comma
id|SCp
...
(paren
id|KERN_ERR
l_string|&quot;scsi%d: (%d:%d) Illegal Instruction detected at 0x%p[0x%x]!!!&bslash;n&quot;
...
(paren
id|KERN_ERR
l_string|&quot;scsi%d: Driver error: resume at %p [%04x] with non busy host!&bslash;n&quot;
comma
id|host-&gt;host_no
comma
(paren
...
(paren
id|KERN_WARNING
l_string|&quot;scsi%d (%d:%d) Tag clock back to current, queueing&bslash;n&quot;
comma
id|SCp-&gt;host-&gt;host_no
comma
id|SCp-&gt;target
...
l_int|NULL
suffix:semicolon
multiline_comment|/* This is f****g ridiculous; every low level HBA driver has&n;&t; * to determine the direction of the commands, why isn&squot;t this&n;&t; * done inside the scsi_lib !!??? */
r_switch
c_cond
(paren
id|SCp-&gt;cmnd
