...
multiline_comment|/**&n; *&t;ata_chk_spurious_int - Check for spurious interrupts&n; *&t;@ap: port to which command is being issued&n; *&n; *&t;Examines the DMA status registers and clears&n; *      unexpected interrupts.  Created to work around&n; *&t;hardware bug on Intel ICH5, but is applied to all&n; *&t;chipsets using the standard irq handler, just for safety.&n; *&t;If the bug is not present, this is simply a single&n; *&t;PIO or MMIO read addition to the irq handler.&n; *&n; *&t;LOCKING:&n; */
DECL|function|ata_chk_spurious_int
r_static
r_inline
r_void
id|ata_chk_spurious_int
c_func
(paren
r_struct
id|ata_port
...
c_func
(paren
l_string|&quot;ata%u: Caught spurious interrupt, status 0x%X&bslash;n&quot;
comma
id|ap-&gt;id
comma
id|host_stat
