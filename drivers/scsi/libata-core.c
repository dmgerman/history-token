...
(brace
multiline_comment|/* straddles boundary, need two s/g entries */
id|sg2len
op_assign
(paren
id|addr
...
r_else
(brace
multiline_comment|/* no split, just one s/g entry */
id|sg2len
op_assign
l_int|0
suffix:semicolon
...
multiline_comment|/* fill first S/G list entry */
id|ap-&gt;prd
(braket
id|idx
)braket
...
op_increment
suffix:semicolon
multiline_comment|/* if we had to break S/G entry across segment boundary,&n;&t;&t; * due to iommu merging, fill second S/G list entry&n;&t;&t; */
r_if
c_cond
(paren
id|sg2len
