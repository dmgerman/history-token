...
suffix:semicolon
multiline_comment|/* export the attention callback registration methods */
DECL|variable|acpiphp_register_attention
...
suffix:semicolon
DECL|variable|acpiphp_unregister_attention
id|EXPORT_SYMBOL_GPL
c_func
(paren
...
suffix:semicolon
multiline_comment|/**&n; * acpiphp_register_attention - set attention LED callback&n; * @info: must be completely filled with LED callbacks&n; *&n; * Description: this is used to register a hardware specific ACPI&n; * driver that manipulates the attention LED.  All the fields in&n; * info must be set.&n; **/
DECL|function|acpiphp_register_attention
r_int
id|acpiphp_register_attention
c_func
(paren
...
multiline_comment|/**&n; * acpiphp_unregister_attention - unset attention LED callback&n; * @info: must match the pointer used to register&n; *&n; * Description: this is used to un-register a hardware specific acpi&n; * driver that manipulates the attention LED.  The pointer to the &n; * info struct must be the same as the one used to set it.&n; **/
DECL|function|acpiphp_unregister_attention
r_int
id|acpiphp_unregister_attention
c_func
(paren
...
multiline_comment|/**&n;  * set_attention_status - set attention LED&n; * @hotplug_slot: slot to set attention LED on&n; * @status: value to set attention LED to (0 or 1)&n; *&n; * attention status LED, so we use a callback that&n; * was registered with us.  This allows hardware specific&n; * ACPI implementations to blink the light for us.&n; **/
...
multiline_comment|/**&n; * get_attention_status - get attention LED status&n; * @hotplug_slot: slot to get status from&n; * @value: returns with value of attention LED&n; *&n; * ACPI doesn&squot;t have known method to determine the state&n; * of the attention status LED, so we use a callback that&n; * was registered with us.  This allows hardware specific&n; * ACPI implementations to determine its state&n; **/
