...
multiline_comment|/**&n; * pci_match_one_device - Tell if a PCI device structure has a matching PCI device id structure&n; * @id: single PCI device id structure to match&n; * @dev: the PCI device structure to match against&n; * &n; * Returns the matching pci_device_id structure or %NULL if there is no match.&n; */
r_static
r_inline
r_const
r_struct
...
multiline_comment|/*&n; * Attribute to force driver probe for devices&n; * If __pci_device_probe() returns 0, it matched at least one previously&n; * unclaimed device.  If it returns -ENODEV, it didn&squot;t match.  Both are&n; * alright in this case, just keep searching for new devices.&n; */
r_static
r_int
DECL|function|probe_each_pci_dev
id|probe_each_pci_dev
c_func
(paren
r_struct
id|pci_driver
