...
multiline_comment|/* If we&squot;re in D3, force entire word to 0, since we can&squot;t access the&n;&t; * PCI config space for the device&n;&t; */
r_if
c_cond
(paren
id|dev-&gt;current_state
...
multiline_comment|/**&n; * pci_disable_device - Disable PCI device after use&n; * @dev: PCI device to be disabled&n; *&n; * Signal to the system that the PCI device is not in use by the system&n; * anymore.  Currently this only involves disabling PCI busmastering,&n; * if active.&n; */
r_void
...
id|EIO
suffix:semicolon
multiline_comment|/* this device cannot poweroff - up to bridge to cut power */
multiline_comment|/* make sure device supports wake events (from any state) */
id|pci_read_config_word
c_func
(paren
id|dev
...
id|EINVAL
suffix:semicolon
multiline_comment|/* doesn&squot;t support wake events */
multiline_comment|/* &n;&t; * XXX - We&squot;re assuming that device can generate wake events from whatever &n;&t; * state it may be entering. &n;&t; * We&squot;re not actually checking what state we&squot;re going into to.&n;&t; */
id|pci_read_config_word
c_func
(paren
id|dev
