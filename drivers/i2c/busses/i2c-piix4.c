...
suffix:semicolon
multiline_comment|/* Note: We assume all devices are identical&n;         to the Intel PIIX4; we only mention it during detection.   */
...
macro_line|#endif
multiline_comment|/* Detect whether a PIIX4 can be found, and initialize it, where necessary.&n;   Note the differences between kernels with the old PCI BIOS interface and&n;   newer kernels with the real PCI interface. In compat.h some things are&n;   defined to make the transition easier. */
...
multiline_comment|/* Look for a supported device/function */
r_do
(brace
r_if
c_cond
...
(paren
id|KERN_ERR
l_string|&quot;i2c-piix4.o: Error: Can&squot;t detect PIIX4 or compatible device!&bslash;n&quot;
)paren
suffix:semicolon
id|error_return
op_assign
