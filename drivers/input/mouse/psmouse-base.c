...
r_break
suffix:semicolon
multiline_comment|/*&n;&t;&t;&t; * Workaround for mice which don&squot;t ACK the Get ID command.&n;&t;&t;&t; * These are valid mouse IDs that we recognize.&n;&t;&t;&t; */
r_case
l_int|0x00
suffix:colon
r_case
...
multiline_comment|/*&n; * psmouse_sendbyte() sends a byte to the mouse, and waits for acknowledge.&n; * It doesn&squot;t handle retransmission, though it could - because when there would&n; * be need for retransmissions, the mouse has to be replaced anyway.&n; *&n; * psmouse_sendbyte() can only be called from a process context&n; */
...
multiline_comment|/*&n; * psmouse_command() sends a command and its parameters to the mouse,&n; * then waits for the response and puts it in the param array.&n; *&n; * psmouse_command() can only be called from a process context&n; */
