multiline_comment|/*&n; * $Id: spaceball.c,v 1.17 2002/01/22 20:29:03 vojtech Exp $&n; *&n; *  Copyright (c) 1999-2001 Vojtech Pavlik&n; *&n; *  Based on the work of:&n; *  &t;David Thompson&n; *  &t;Joseph Krahn&n; */
multiline_comment|/*&n; * SpaceTec SpaceBall 2003/3003/4000 FLX driver for Linux&n; */
multiline_comment|/*&n; * This program is free software; you can redistribute it and/or modify&n; * it under the terms of the GNU General Public License as published by&n; * the Free Software Foundation; either version 2 of the License, or&n; * (at your option) any later version.&n; *&n; * This program is distributed in the hope that it will be useful,&n; * but WITHOUT ANY WARRANTY; without even the implied warranty of&n; * MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the&n; * GNU General Public License for more details.&n; *&n; * You should have received a copy of the GNU General Public License&n; * along with this program; if not, write to the Free Software&n; * Foundation, Inc., 59 Temple Place, Suite 330, Boston, MA 02111-1307 USA&n; *&n; *  Should you need to contact me, the author, you can do so either by&n; * e-mail - mail your message to &lt;vojtech@ucw.cz&gt;, or by paper mail:&n; * Vojtech Pavlik, Simunkova 1594, Prague 8, 182 00 Czech Republic&n; */
macro_line|#include &lt;linux/kernel.h&gt;
macro_line|#include &lt;linux/slab.h&gt;
macro_line|#include &lt;linux/module.h&gt;
macro_line|#include &lt;linux/init.h&gt;
macro_line|#include &lt;linux/input.h&gt;
macro_line|#include &lt;linux/serio.h&gt;
DECL|macro|DRIVER_DESC
mdefine_line|#define DRIVER_DESC&t;&quot;SpaceTec SpaceBall 2003/3003/4000 FLX driver&quot;
id|MODULE_AUTHOR
c_func
(paren
l_string|&quot;Vojtech Pavlik &lt;vojtech@ucw.cz&gt;&quot;
)paren
suffix:semicolon
DECL|variable|DRIVER_DESC
id|MODULE_DESCRIPTION
c_func
(paren
id|DRIVER_DESC
)paren
suffix:semicolon
id|MODULE_LICENSE
c_func
(paren
l_string|&quot;GPL&quot;
)paren
suffix:semicolon
multiline_comment|/*&n; * Constants.&n; */
DECL|macro|SPACEBALL_MAX_LENGTH
mdefine_line|#define SPACEBALL_MAX_LENGTH&t;128
DECL|macro|SPACEBALL_MAX_ID
mdefine_line|#define SPACEBALL_MAX_ID&t;8
DECL|macro|SPACEBALL_1003
mdefine_line|#define SPACEBALL_1003      1
DECL|macro|SPACEBALL_2003B
mdefine_line|#define SPACEBALL_2003B     3
DECL|macro|SPACEBALL_2003C
mdefine_line|#define SPACEBALL_2003C     4
DECL|macro|SPACEBALL_3003C
mdefine_line|#define SPACEBALL_3003C     7
DECL|macro|SPACEBALL_4000FLX
mdefine_line|#define SPACEBALL_4000FLX   8
DECL|macro|SPACEBALL_4000FLX_L
mdefine_line|#define SPACEBALL_4000FLX_L 9
DECL|variable|spaceball_axes
r_static
r_int
id|spaceball_axes
(braket
)braket
op_assign
(brace
id|ABS_X
comma
id|ABS_Z
comma
id|ABS_Y
comma
id|ABS_RX
comma
id|ABS_RZ
comma
id|ABS_RY
)brace
suffix:semicolon
DECL|variable|spaceball_names
r_static
r_char
op_star
id|spaceball_names
(braket
)braket
op_assign
(brace
l_string|&quot;?&quot;
comma
l_string|&quot;SpaceTec SpaceBall 1003&quot;
comma
l_string|&quot;SpaceTec SpaceBall 2003&quot;
comma
l_string|&quot;SpaceTec SpaceBall 2003B&quot;
comma
l_string|&quot;SpaceTec SpaceBall 2003C&quot;
comma
l_string|&quot;SpaceTec SpaceBall 3003&quot;
comma
l_string|&quot;SpaceTec SpaceBall SpaceController&quot;
comma
l_string|&quot;SpaceTec SpaceBall 3003C&quot;
comma
l_string|&quot;SpaceTec SpaceBall 4000FLX&quot;
comma
l_string|&quot;SpaceTec SpaceBall 4000FLX Lefty&quot;
)brace
suffix:semicolon
multiline_comment|/*&n; * Per-Ball data.&n; */
DECL|struct|spaceball
r_struct
id|spaceball
(brace
DECL|member|dev
r_struct
id|input_dev
id|dev
suffix:semicolon
DECL|member|serio
r_struct
id|serio
op_star
id|serio
suffix:semicolon
DECL|member|idx
r_int
id|idx
suffix:semicolon
DECL|member|escape
r_int
id|escape
suffix:semicolon
DECL|member|data
r_int
r_char
id|data
(braket
id|SPACEBALL_MAX_LENGTH
)braket
suffix:semicolon
DECL|member|phys
r_char
id|phys
(braket
l_int|32
)braket
suffix:semicolon
)brace
suffix:semicolon
multiline_comment|/*&n; * spaceball_process_packet() decodes packets the driver receives from the&n; * SpaceBall.&n; */
DECL|function|spaceball_process_packet
r_static
r_void
id|spaceball_process_packet
c_func
(paren
r_struct
id|spaceball
op_star
id|spaceball
comma
r_struct
id|pt_regs
op_star
id|regs
)paren
(brace
r_struct
id|input_dev
op_star
id|dev
op_assign
op_amp
id|spaceball-&gt;dev
suffix:semicolon
r_int
r_char
op_star
id|data
op_assign
id|spaceball-&gt;data
suffix:semicolon
r_int
id|i
suffix:semicolon
r_if
c_cond
(paren
id|spaceball-&gt;idx
OL
l_int|2
)paren
r_return
suffix:semicolon
id|input_regs
c_func
(paren
id|dev
comma
id|regs
)paren
suffix:semicolon
r_switch
c_cond
(paren
id|spaceball-&gt;data
(braket
l_int|0
)braket
)paren
(brace
r_case
l_char|&squot;D&squot;
suffix:colon
multiline_comment|/* Ball data */
r_if
c_cond
(paren
id|spaceball-&gt;idx
op_ne
l_int|15
)paren
r_return
suffix:semicolon
r_for
c_loop
(paren
id|i
op_assign
l_int|0
suffix:semicolon
id|i
OL
l_int|6
suffix:semicolon
id|i
op_increment
)paren
id|input_report_abs
c_func
(paren
id|dev
comma
id|spaceball_axes
(braket
id|i
)braket
comma
(paren
id|__s16
)paren
(paren
(paren
id|data
(braket
l_int|2
op_star
id|i
op_plus
l_int|3
)braket
op_lshift
l_int|8
)paren
op_or
id|data
(braket
l_int|2
op_star
id|i
op_plus
l_int|2
)braket
)paren
)paren
suffix:semicolon
r_break
suffix:semicolon
r_case
l_char|&squot;K&squot;
suffix:colon
multiline_comment|/* Button data */
r_if
c_cond
(paren
id|spaceball-&gt;idx
op_ne
l_int|3
)paren
r_return
suffix:semicolon
id|input_report_key
c_func
(paren
id|dev
comma
id|BTN_1
comma
(paren
id|data
(braket
l_int|2
)braket
op_amp
l_int|0x01
)paren
op_logical_or
(paren
id|data
(braket
l_int|2
)braket
op_amp
l_int|0x20
)paren
)paren
suffix:semicolon
id|input_report_key
c_func
(paren
id|dev
comma
id|BTN_2
comma
id|data
(braket
l_int|2
)braket
op_amp
l_int|0x02
)paren
suffix:semicolon
id|input_report_key
c_func
(paren
id|dev
comma
id|BTN_3
comma
id|data
(braket
l_int|2
)braket
op_amp
l_int|0x04
)paren
suffix:semicolon
id|input_report_key
c_func
(paren
id|dev
comma
id|BTN_4
comma
id|data
(braket
l_int|2
)braket
op_amp
l_int|0x08
)paren
suffix:semicolon
id|input_report_key
c_func
(paren
id|dev
comma
id|BTN_5
comma
id|data
(braket
l_int|1
)braket
op_amp
l_int|0x01
)paren
suffix:semicolon
id|input_report_key
c_func
(paren
id|dev
comma
id|BTN_6
comma
id|data
(braket
l_int|1
)braket
op_amp
l_int|0x02
)paren
suffix:semicolon
id|input_report_key
c_func
(paren
id|dev
comma
id|BTN_7
comma
id|data
(braket
l_int|1
)braket
op_amp
l_int|0x04
)paren
suffix:semicolon
id|input_report_key
c_func
(paren
id|dev
comma
id|BTN_8
comma
id|data
(braket
l_int|1
)braket
op_amp
l_int|0x10
)paren
suffix:semicolon
r_break
suffix:semicolon
r_case
l_char|&squot;.&squot;
suffix:colon
multiline_comment|/* Advanced button data */
r_if
c_cond
(paren
id|spaceball-&gt;idx
op_ne
l_int|3
)paren
r_return
suffix:semicolon
id|input_report_key
c_func
(paren
id|dev
comma
id|BTN_1
comma
id|data
(braket
l_int|2
)braket
op_amp
l_int|0x01
)paren
suffix:semicolon
id|input_report_key
c_func
(paren
id|dev
comma
id|BTN_2
comma
id|data
(braket
l_int|2
)braket
op_amp
l_int|0x02
)paren
suffix:semicolon
id|input_report_key
c_func
(paren
id|dev
comma
id|BTN_3
comma
id|data
(braket
l_int|2
)braket
op_amp
l_int|0x04
)paren
suffix:semicolon
id|input_report_key
c_func
(paren
id|dev
comma
id|BTN_4
comma
id|data
(braket
l_int|2
)braket
op_amp
l_int|0x08
)paren
suffix:semicolon
id|input_report_key
c_func
(paren
id|dev
comma
id|BTN_5
comma
id|data
(braket
l_int|2
)braket
op_amp
l_int|0x10
)paren
suffix:semicolon
id|input_report_key
c_func
(paren
id|dev
comma
id|BTN_6
comma
id|data
(braket
l_int|2
)braket
op_amp
l_int|0x20
)paren
suffix:semicolon
id|input_report_key
c_func
(paren
id|dev
comma
id|BTN_7
comma
id|data
(braket
l_int|2
)braket
op_amp
l_int|0x80
)paren
suffix:semicolon
id|input_report_key
c_func
(paren
id|dev
comma
id|BTN_8
comma
id|data
(braket
l_int|1
)braket
op_amp
l_int|0x01
)paren
suffix:semicolon
id|input_report_key
c_func
(paren
id|dev
comma
id|BTN_9
comma
id|data
(braket
l_int|1
)braket
op_amp
l_int|0x02
)paren
suffix:semicolon
id|input_report_key
c_func
(paren
id|dev
comma
id|BTN_A
comma
id|data
(braket
l_int|1
)braket
op_amp
l_int|0x04
)paren
suffix:semicolon
id|input_report_key
c_func
(paren
id|dev
comma
id|BTN_B
comma
id|data
(braket
l_int|1
)braket
op_amp
l_int|0x08
)paren
suffix:semicolon
id|input_report_key
c_func
(paren
id|dev
comma
id|BTN_C
comma
id|data
(braket
l_int|1
)braket
op_amp
l_int|0x10
)paren
suffix:semicolon
id|input_report_key
c_func
(paren
id|dev
comma
id|BTN_MODE
comma
id|data
(braket
l_int|1
)braket
op_amp
l_int|0x20
)paren
suffix:semicolon
r_break
suffix:semicolon
r_case
l_char|&squot;E&squot;
suffix:colon
multiline_comment|/* Device error */
id|spaceball-&gt;data
(braket
id|spaceball-&gt;idx
op_minus
l_int|1
)braket
op_assign
l_int|0
suffix:semicolon
id|printk
c_func
(paren
id|KERN_ERR
l_string|&quot;spaceball: Device error. [%s]&bslash;n&quot;
comma
id|spaceball-&gt;data
op_plus
l_int|1
)paren
suffix:semicolon
r_break
suffix:semicolon
r_case
l_char|&squot;?&squot;
suffix:colon
multiline_comment|/* Bad command packet */
id|spaceball-&gt;data
(braket
id|spaceball-&gt;idx
op_minus
l_int|1
)braket
op_assign
l_int|0
suffix:semicolon
id|printk
c_func
(paren
id|KERN_ERR
l_string|&quot;spaceball: Bad command. [%s]&bslash;n&quot;
comma
id|spaceball-&gt;data
op_plus
l_int|1
)paren
suffix:semicolon
r_break
suffix:semicolon
)brace
id|input_sync
c_func
(paren
id|dev
)paren
suffix:semicolon
)brace
multiline_comment|/*&n; * Spaceball 4000 FLX packets all start with a one letter packet-type decriptor,&n; * and end in 0x0d. It uses &squot;^&squot; as an escape for CR, XOFF and XON characters which&n; * can occur in the axis values.&n; */
DECL|function|spaceball_interrupt
r_static
id|irqreturn_t
id|spaceball_interrupt
c_func
(paren
r_struct
id|serio
op_star
id|serio
comma
r_int
r_char
id|data
comma
r_int
r_int
id|flags
comma
r_struct
id|pt_regs
op_star
id|regs
)paren
(brace
r_struct
id|spaceball
op_star
id|spaceball
op_assign
id|serio_get_drvdata
c_func
(paren
id|serio
)paren
suffix:semicolon
r_switch
c_cond
(paren
id|data
)paren
(brace
r_case
l_int|0xd
suffix:colon
id|spaceball_process_packet
c_func
(paren
id|spaceball
comma
id|regs
)paren
suffix:semicolon
id|spaceball-&gt;idx
op_assign
l_int|0
suffix:semicolon
id|spaceball-&gt;escape
op_assign
l_int|0
suffix:semicolon
r_break
suffix:semicolon
r_case
l_char|&squot;^&squot;
suffix:colon
r_if
c_cond
(paren
op_logical_neg
id|spaceball-&gt;escape
)paren
(brace
id|spaceball-&gt;escape
op_assign
l_int|1
suffix:semicolon
r_break
suffix:semicolon
)brace
id|spaceball-&gt;escape
op_assign
l_int|0
suffix:semicolon
r_case
l_char|&squot;M&squot;
suffix:colon
r_case
l_char|&squot;Q&squot;
suffix:colon
r_case
l_char|&squot;S&squot;
suffix:colon
r_if
c_cond
(paren
id|spaceball-&gt;escape
)paren
(brace
id|spaceball-&gt;escape
op_assign
l_int|0
suffix:semicolon
id|data
op_and_assign
l_int|0x1f
suffix:semicolon
)brace
r_default
suffix:colon
(brace
)brace
r_if
c_cond
(paren
id|spaceball-&gt;escape
)paren
id|spaceball-&gt;escape
op_assign
l_int|0
suffix:semicolon
r_if
c_cond
(paren
id|spaceball-&gt;idx
OL
id|SPACEBALL_MAX_LENGTH
)paren
id|spaceball-&gt;data
(braket
id|spaceball-&gt;idx
op_increment
)braket
op_assign
id|data
suffix:semicolon
r_break
suffix:semicolon
)brace
r_return
id|IRQ_HANDLED
suffix:semicolon
)brace
multiline_comment|/*&n; * spaceball_disconnect() is the opposite of spaceball_connect()&n; */
DECL|function|spaceball_disconnect
r_static
r_void
id|spaceball_disconnect
c_func
(paren
r_struct
id|serio
op_star
id|serio
)paren
(brace
r_struct
id|spaceball
op_star
id|spaceball
op_assign
id|serio_get_drvdata
c_func
(paren
id|serio
)paren
suffix:semicolon
id|input_unregister_device
c_func
(paren
op_amp
id|spaceball-&gt;dev
)paren
suffix:semicolon
id|serio_close
c_func
(paren
id|serio
)paren
suffix:semicolon
id|serio_set_drvdata
c_func
(paren
id|serio
comma
l_int|NULL
)paren
suffix:semicolon
id|kfree
c_func
(paren
id|spaceball
)paren
suffix:semicolon
)brace
multiline_comment|/*&n; * spaceball_connect() is the routine that is called when someone adds a&n; * new serio device that supports Spaceball protocol and registers it as&n; * an input device.&n; */
DECL|function|spaceball_connect
r_static
r_int
id|spaceball_connect
c_func
(paren
r_struct
id|serio
op_star
id|serio
comma
r_struct
id|serio_driver
op_star
id|drv
)paren
(brace
r_struct
id|spaceball
op_star
id|spaceball
suffix:semicolon
r_int
id|i
comma
id|t
comma
id|id
suffix:semicolon
r_int
id|err
suffix:semicolon
r_if
c_cond
(paren
(paren
id|id
op_assign
id|serio-&gt;id.id
)paren
OG
id|SPACEBALL_MAX_ID
)paren
r_return
op_minus
id|ENODEV
suffix:semicolon
r_if
c_cond
(paren
op_logical_neg
(paren
id|spaceball
op_assign
id|kmalloc
c_func
(paren
r_sizeof
(paren
r_struct
id|spaceball
)paren
comma
id|GFP_KERNEL
)paren
)paren
)paren
r_return
op_minus
id|ENOMEM
suffix:semicolon
id|memset
c_func
(paren
id|spaceball
comma
l_int|0
comma
r_sizeof
(paren
r_struct
id|spaceball
)paren
)paren
suffix:semicolon
id|spaceball-&gt;dev.evbit
(braket
l_int|0
)braket
op_assign
id|BIT
c_func
(paren
id|EV_KEY
)paren
op_or
id|BIT
c_func
(paren
id|EV_ABS
)paren
suffix:semicolon
r_switch
c_cond
(paren
id|id
)paren
(brace
r_case
id|SPACEBALL_4000FLX
suffix:colon
r_case
id|SPACEBALL_4000FLX_L
suffix:colon
id|spaceball-&gt;dev.keybit
(braket
id|LONG
c_func
(paren
id|BTN_0
)paren
)braket
op_or_assign
id|BIT
c_func
(paren
id|BTN_9
)paren
suffix:semicolon
id|spaceball-&gt;dev.keybit
(braket
id|LONG
c_func
(paren
id|BTN_A
)paren
)braket
op_or_assign
id|BIT
c_func
(paren
id|BTN_A
)paren
op_or
id|BIT
c_func
(paren
id|BTN_B
)paren
op_or
id|BIT
c_func
(paren
id|BTN_C
)paren
op_or
id|BIT
c_func
(paren
id|BTN_MODE
)paren
suffix:semicolon
r_default
suffix:colon
id|spaceball-&gt;dev.keybit
(braket
id|LONG
c_func
(paren
id|BTN_0
)paren
)braket
op_or_assign
id|BIT
c_func
(paren
id|BTN_2
)paren
op_or
id|BIT
c_func
(paren
id|BTN_3
)paren
op_or
id|BIT
c_func
(paren
id|BTN_4
)paren
op_or
id|BIT
c_func
(paren
id|BTN_5
)paren
op_or
id|BIT
c_func
(paren
id|BTN_6
)paren
op_or
id|BIT
c_func
(paren
id|BTN_7
)paren
op_or
id|BIT
c_func
(paren
id|BTN_8
)paren
suffix:semicolon
r_case
id|SPACEBALL_3003C
suffix:colon
id|spaceball-&gt;dev.keybit
(braket
id|LONG
c_func
(paren
id|BTN_0
)paren
)braket
op_or_assign
id|BIT
c_func
(paren
id|BTN_1
)paren
op_or
id|BIT
c_func
(paren
id|BTN_8
)paren
suffix:semicolon
)brace
r_for
c_loop
(paren
id|i
op_assign
l_int|0
suffix:semicolon
id|i
OL
l_int|6
suffix:semicolon
id|i
op_increment
)paren
(brace
id|t
op_assign
id|spaceball_axes
(braket
id|i
)braket
suffix:semicolon
id|set_bit
c_func
(paren
id|t
comma
id|spaceball-&gt;dev.absbit
)paren
suffix:semicolon
id|spaceball-&gt;dev.absmin
(braket
id|t
)braket
op_assign
id|i
OL
l_int|3
ques
c_cond
op_minus
l_int|8000
suffix:colon
op_minus
l_int|1600
suffix:semicolon
id|spaceball-&gt;dev.absmax
(braket
id|t
)braket
op_assign
id|i
OL
l_int|3
ques
c_cond
l_int|8000
suffix:colon
l_int|1600
suffix:semicolon
id|spaceball-&gt;dev.absflat
(braket
id|t
)braket
op_assign
id|i
OL
l_int|3
ques
c_cond
l_int|40
suffix:colon
l_int|8
suffix:semicolon
id|spaceball-&gt;dev.absfuzz
(braket
id|t
)braket
op_assign
id|i
OL
l_int|3
ques
c_cond
l_int|8
suffix:colon
l_int|2
suffix:semicolon
)brace
id|spaceball-&gt;serio
op_assign
id|serio
suffix:semicolon
id|spaceball-&gt;dev
dot
r_private
op_assign
id|spaceball
suffix:semicolon
id|sprintf
c_func
(paren
id|spaceball-&gt;phys
comma
l_string|&quot;%s/input0&quot;
comma
id|serio-&gt;phys
)paren
suffix:semicolon
id|init_input_dev
c_func
(paren
op_amp
id|spaceball-&gt;dev
)paren
suffix:semicolon
id|spaceball-&gt;dev.name
op_assign
id|spaceball_names
(braket
id|id
)braket
suffix:semicolon
id|spaceball-&gt;dev.phys
op_assign
id|spaceball-&gt;phys
suffix:semicolon
id|spaceball-&gt;dev.id.bustype
op_assign
id|BUS_RS232
suffix:semicolon
id|spaceball-&gt;dev.id.vendor
op_assign
id|SERIO_SPACEBALL
suffix:semicolon
id|spaceball-&gt;dev.id.product
op_assign
id|id
suffix:semicolon
id|spaceball-&gt;dev.id.version
op_assign
l_int|0x0100
suffix:semicolon
id|spaceball-&gt;dev.dev
op_assign
op_amp
id|serio-&gt;dev
suffix:semicolon
id|serio_set_drvdata
c_func
(paren
id|serio
comma
id|spaceball
)paren
suffix:semicolon
id|err
op_assign
id|serio_open
c_func
(paren
id|serio
comma
id|drv
)paren
suffix:semicolon
r_if
c_cond
(paren
id|err
)paren
(brace
id|serio_set_drvdata
c_func
(paren
id|serio
comma
l_int|NULL
)paren
suffix:semicolon
id|kfree
c_func
(paren
id|spaceball
)paren
suffix:semicolon
r_return
id|err
suffix:semicolon
)brace
id|input_register_device
c_func
(paren
op_amp
id|spaceball-&gt;dev
)paren
suffix:semicolon
id|printk
c_func
(paren
id|KERN_INFO
l_string|&quot;input: %s on serio%s&bslash;n&quot;
comma
id|spaceball_names
(braket
id|id
)braket
comma
id|serio-&gt;phys
)paren
suffix:semicolon
r_return
l_int|0
suffix:semicolon
)brace
multiline_comment|/*&n; * The serio driver structure.&n; */
DECL|variable|spaceball_serio_ids
r_static
r_struct
id|serio_device_id
id|spaceball_serio_ids
(braket
)braket
op_assign
(brace
(brace
dot
id|type
op_assign
id|SERIO_RS232
comma
dot
id|proto
op_assign
id|SERIO_SPACEBALL
comma
dot
id|id
op_assign
id|SERIO_ANY
comma
dot
id|extra
op_assign
id|SERIO_ANY
comma
)brace
comma
(brace
l_int|0
)brace
)brace
suffix:semicolon
id|MODULE_DEVICE_TABLE
c_func
(paren
id|serio
comma
id|spaceball_serio_ids
)paren
suffix:semicolon
DECL|variable|spaceball_drv
r_static
r_struct
id|serio_driver
id|spaceball_drv
op_assign
(brace
dot
id|driver
op_assign
(brace
dot
id|name
op_assign
l_string|&quot;spaceball&quot;
comma
)brace
comma
dot
id|description
op_assign
id|DRIVER_DESC
comma
dot
id|id_table
op_assign
id|spaceball_serio_ids
comma
dot
id|interrupt
op_assign
id|spaceball_interrupt
comma
dot
id|connect
op_assign
id|spaceball_connect
comma
dot
id|disconnect
op_assign
id|spaceball_disconnect
comma
)brace
suffix:semicolon
multiline_comment|/*&n; * The functions for inserting/removing us as a module.&n; */
DECL|function|spaceball_init
r_static
r_int
id|__init
id|spaceball_init
c_func
(paren
r_void
)paren
(brace
id|serio_register_driver
c_func
(paren
op_amp
id|spaceball_drv
)paren
suffix:semicolon
r_return
l_int|0
suffix:semicolon
)brace
DECL|function|spaceball_exit
r_static
r_void
id|__exit
id|spaceball_exit
c_func
(paren
r_void
)paren
(brace
id|serio_unregister_driver
c_func
(paren
op_amp
id|spaceball_drv
)paren
suffix:semicolon
)brace
DECL|variable|spaceball_init
id|module_init
c_func
(paren
id|spaceball_init
)paren
suffix:semicolon
DECL|variable|spaceball_exit
id|module_exit
c_func
(paren
id|spaceball_exit
)paren
suffix:semicolon
eof
