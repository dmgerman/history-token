...
suffix:semicolon
multiline_comment|/*&n;&t; * This barrier makes sure that all updates to ownership bits&n;&t; * done by set_eqe_hw() hit memory before the consumer index&n;&t; * is updated.  set_eq_ci() allows the HCA to possibly write&n;&t; * more EQ entries, and we want to avoid the exceedingly&n;&t; * unlikely possibility of the HCA writing an entry and then&n;&t; * having set_eqe_hw() overwrite the owner field.&n;&t; */
id|wmb
c_func
(paren
)paren
...
(brace
multiline_comment|/* See comment in tavor_set_eq_ci() above. */
id|wmb
c_func
(paren
)paren
...
comma
id|dev-&gt;eq_regs.arbel.eq_set_ci_base
op_plus
id|eq-&gt;eqn
op_star
l_int|8
...
suffix:semicolon
multiline_comment|/* We still want ordering, just not swabbing, so add a barrier */
id|mb
c_func
(paren
)paren
...
DECL|function|tavor_eq_req_not
r_static
r_inline
r_void
id|tavor_eq_req_not
...
DECL|function|arbel_eq_req_not
r_static
r_inline
r_void
id|arbel_eq_req_not
...
(brace
multiline_comment|/*&n;&t;&t;&t; * Conditional on hca_type is OK here because&n;&t;&t;&t; * this is a rare case, not the fast path.&n;&t;&t;&t; */
id|set_eq_ci
c_func
(paren
id|dev
...
multiline_comment|/*&n;&t; * Rely on caller to set consumer index so that we don&squot;t have&n;&t; * to test hca_type in our interrupt handling fast path.&n;&t; */
r_return
id|eqes_found
suffix:semicolon
)brace
DECL|function|mthca_tavor_interrupt
r_static
id|irqreturn_t
id|mthca_tavor_interrupt
c_func
(paren
r_int
id|irq
...
DECL|function|mthca_tavor_msi_x_interrupt
r_static
id|irqreturn_t
id|mthca_tavor_msi_x_interrupt
c_func
(paren
r_int
id|irq
...
DECL|function|mthca_arbel_interrupt
r_static
id|irqreturn_t
id|mthca_arbel_interrupt
c_func
(paren
r_int
id|irq
...
DECL|function|mthca_arbel_msi_x_interrupt
r_static
id|irqreturn_t
id|mthca_arbel_msi_x_interrupt
c_func
(paren
r_int
id|irq
...
DECL|function|mthca_map_eq_regs
r_static
r_int
id|__devinit
id|mthca_map_eq_regs
c_func
(paren
r_struct
id|mthca_dev
...
(brace
multiline_comment|/*&n;&t;&t; * We assume that the EQ arm and EQ set CI registers&n;&t;&t; * fall within the first BAR.  We can&squot;t trust the&n;&t;&t; * values firmware gives us, since those addresses are&n;&t;&t; * valid on the HCA&squot;s side of the PCI bus but not&n;&t;&t; * necessarily the host side.&n;&t;&t; */
r_if
c_cond
(paren
id|mthca_map_reg
...
multiline_comment|/*&n;&t;&t; * Add 4 because we limit ourselves to EQs 0 ... 31,&n;&t;&t; * so we only need the low word of the register.&n;&t;&t; */
r_if
c_cond
(paren
id|mthca_map_reg
...
DECL|function|mthca_unmap_eq_regs
r_static
r_void
id|__devexit
id|mthca_unmap_eq_regs
c_func
(paren
r_struct
id|mthca_dev
