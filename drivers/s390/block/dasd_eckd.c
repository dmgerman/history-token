multiline_comment|/* &n; * File...........: linux/drivers/s390/block/dasd_eckd.c&n; * Author(s)......: Holger Smolinski &lt;Holger.Smolinski@de.ibm.com&gt;&n; * Bugreports.to..: &lt;Linux390@de.ibm.com&gt;&n; * (C) IBM Corporation, IBM Deutschland Entwicklung GmbH, 1999,2000&n;&n; * History of changes (starts July 2000)&n; * 07/11/00 Enabled rotational position sensing&n; * 07/14/00 Reorganized the format process for better ERP&n; * 07/20/00 added experimental support for 2105 control unit (ESS)&n; * 07/24/00 increased expiration time and added the missing zero&n; * 08/07/00 added some bits to define_extent for ESS support&n; * 09/20/00 added reserve and release ioctls&n; * 10/04/00 changed RW-CCWS to R/W Key and Data&n; * 10/10/00 reverted last change according to ESS exploitation&n; * 10/10/00 now dequeuing init_cqr before freeing *ouch*&n; * 26/10/00 fixed ITPM20144ASC (problems when accesing a device formatted by VIF)&n; */
...
id|KERN_INFO
id|PRINTK_HEADER
l_string|&quot;%04X on sch %d: %04X/%02X(CU:%04X/%02X) Cyl:%d Head:%d Sec:%d&bslash;n&quot;
comma
...
id|KERN_WARNING
id|PRINTK_HEADER
l_string|&quot;sizes of configuration data mismatch %d (read) vs %ld (expected)&bslash;n&quot;
comma
id|conf_len
comma
r_sizeof
...
id|KERN_INFO
id|PRINTK_HEADER
l_string|&quot;%04X on sch %d: %04X/%02X(CU:%04X/%02X): Configuration data read&bslash;n&quot;
comma
id|device-&gt;devinfo.devno
comma
id|device-&gt;devinfo.irq
...
id|KERN_INFO
id|PRINTK_HEADER
l_string|&quot;/dev/%s (%04X): invalid blocksize %d&bslash;n&quot;
id|KERN_INFO
id|PRINTK_HEADER
l_string|&quot;/dev/%s (%04X): capacity (at 4kB blks): %dkB at %dkB/trk&bslash;n&quot;
comma
id|device-&gt;name
comma
id|device-&gt;devinfo.devno
...
id|KERN_INFO
id|PRINTK_HEADER
l_string|&quot;/dev/%s (%04X): capacity (%dkB blks): %dkB at %dkB/trk&bslash;n&quot;
comma
id|device-&gt;name
comma
id|device-&gt;devinfo.devno
...
l_int|0x2105
suffix:colon
multiline_comment|/*  return dasd_3990_erp_action; */
r_case
l_int|0x9343
suffix:colon
...
id|PRINT_WARN
(paren
l_string|&quot;Cannot fulfill small request %ld vs. %d (%ld sects)&bslash;n&quot;
comma
id|size
comma
id|byt_per_blk
