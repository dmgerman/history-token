...
mdefine_line|#define ZFCP_SCSI_REVISION &quot;$Revision: 1.38 $&quot;
...
multiline_comment|/*&n; * function:    zfcp_scsi_insert_into_fake_queue&n; *&n; * purpose:&n; *&t;&t;&n; *&n; * returns:&n; *&n; * FIXME:&t;Is the following scenario possible and - even more interesting -&n; *&t;&t;a problem? It reminds me of the famous &squot;no retry for tape&squot; fix&n; *&t;&t;(no problem for disks, but what is about tapes...)&n; *&n; *&t;&t;device is unaccessable,&n; *&t;&t;command A is put into the fake queue,&n; *&t;&t;device becomes accessable again,&n; *&t;&t;command B is queued to the device,&n; *&t;&t;fake queue timer expires&n; *&t;&t;command A is returned to the mid-layer&n; *&t;&t;command A is queued to the device&n; */
r_void
DECL|function|zfcp_scsi_insert_into_fake_queue
...
c_func
(paren
l_string|&quot;Faking SCSI command:&bslash;n&quot;
)paren
suffix:semicolon
id|ZFCP_HEX_DUMP
c_func
...
id|new_cmnd
suffix:semicolon
id|adapter-&gt;fake_scsi_timer.function
op_assign
id|zfcp_scsi_process_and_clear_fake_queue
suffix:semicolon
id|adapter-&gt;fake_scsi_timer.data
op_assign
(paren
r_int
r_int
...
id|adapter
suffix:semicolon
id|adapter-&gt;fake_scsi_timer.expires
op_assign
id|jiffies
op_plus
id|ZFCP_FAKE_SCSI_COMPLETION_TIME
suffix:semicolon
id|add_timer
c_func
(paren
op_amp
id|adapter-&gt;fake_scsi_timer
)paren
suffix:semicolon
)brace
r_else
...
multiline_comment|/*&n; * function:    zfcp_scsi_process_and_clear_fake_queue&n; *&n; * purpose:&n; *&t;&t;&n; *&n; * returns:&n; */
r_void
DECL|function|zfcp_scsi_process_and_clear_fake_queue
id|zfcp_scsi_process_and_clear_fake_queue
c_func
(paren
r_int
...
id|next
suffix:semicolon
multiline_comment|/*&n;&t; * We need a common lock for scsi_req on command completion&n;&t; * as well as on command abort to avoid race conditions&n;&t; * during completions and aborts taking place at the same time.&n;&t; * It needs to be the outer lock as in the eh_abort_handler.&n;&t; */
id|read_lock_irqsave
c_func
(paren
op_amp
...
multiline_comment|/* Sends command on a round-trip using SCSI stack */
r_static
r_void
DECL|function|zfcp_scsi_queuecommand_fake
id|zfcp_scsi_queuecommand_fake
c_func
(paren
id|Scsi_Cmnd
op_star
...
c_func
(paren
l_string|&quot;Looping SCSI IO on the adapter %s.&bslash;n&quot;
comma
...
suffix:semicolon
multiline_comment|/* &n;&t; * Reset everything for devices with retries, allow at least one retry&n;&t; * for others, e.g. tape.&n;&t; */
id|scpnt-&gt;retries
op_assign
l_int|0
suffix:semicolon
...
multiline_comment|/* set when we have a unit/port list modification */
r_if
c_cond
(paren
id|atomic_test_mask
c_func
(paren
id|ZFCP_STATUS_ADAPTER_QUEUECOMMAND_BLOCK
comma
op_amp
id|adapter-&gt;status
)paren
...
multiline_comment|/*&n; * function:    zfcp_scsi_potential_abort_on_fake&n; *&n; * purpose:&n; *&n; * returns:     0 - no fake request aborted&n; *              1 - fake request was aborted&n; *&n; * context:&t;both the adapter-&gt;abort_lock and the &n; *              adapter-&gt;fake_list_lock are assumed to be held write lock&n; *              irqsave&n; */
r_int
DECL|function|zfcp_scsi_potential_abort_on_fake
id|zfcp_scsi_potential_abort_on_fake
c_func
(paren
r_struct
id|zfcp_adapter
...
suffix:semicolon
multiline_comment|/*&n;&t; * Race condition between normal (late) completion and abort has&n;&t; * to be avoided.&n;&t; * The entirity of all accesses to scsi_req have to be atomic.&n;&t; * scsi_req is usually part of the fsf_req (for requests which&n;&t; * are not faked) and thus we block the release of fsf_req&n;&t; * as long as we need to access scsi_req.&n;&t; * For faked commands we use the same lock even if they are not&n;&t; * put into the fsf_req queue. This makes implementation&n;&t; * easier. &n;&t; */
id|write_lock_irqsave
c_func
(paren
op_amp
...
suffix:semicolon
multiline_comment|/*&n;&t; * Check if we deal with a faked command, which we may just forget&n;&t; * about from now on&n;&t; */
id|write_lock
c_func
(paren
op_amp
...
suffix:semicolon
multiline_comment|/* only need to go through list if there are faked requests */
r_if
c_cond
(paren
id|adapter-&gt;first_fake_cmnd
...
c_func
(paren
l_string|&quot;A faked command was aborted&bslash;n&quot;
)paren
suffix:semicolon
id|retval
op_assign
...
suffix:semicolon
multiline_comment|/*&n;&t; * We should not be called to reset a target which we &squot;sent&squot; faked SCSI&n;&t; * commands since the abort of faked SCSI commands should always&n;&t; * succeed (simply delete timer). &n;&t; */
r_if
c_cond
(paren
op_logical_neg
