...
l_int|0
suffix:semicolon
multiline_comment|/*&n;&t; * this list iteration is done without any locking in md?!&n;&t; */
id|ITERATE_RDEV
c_func
(paren
id|mddev
...
multiline_comment|/* csum_partial is not consistent between different architectures.&n; * Some (i386) do a 32bit csum.  Some (alpha) do 16 bit.&n; * This makes it hard for user-space to know what to do.&n; * So we use calc_sb_csum to set the checksum to allow working&n; * with older kernels, but allow calc_sb_csum_common to&n; * be used when checking if a checksum is correct, to&n; * make life easier for user-space tools that might write&n; * a superblock.&n; */
DECL|function|calc_sb_csum_common
r_static
r_int
r_int
id|calc_sb_csum_common
c_func
(paren
id|mdp_super_t
op_star
