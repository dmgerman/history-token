...
suffix:semicolon
multiline_comment|/* Command line args */
DECL|variable|mpt_dv
r_static
r_int
id|mpt_dv
op_assign
...
id|mpt_saf_te
comma
l_string|&quot; Force enabling SEP Processor: enable=1  (default=MPTSCSIH_SAF_TE=0)&quot;
)paren
suffix:semicolon
DECL|variable|mpt_pq_filter
r_static
r_int
id|mpt_pq_filter
op_assign
...
id|mpt_pq_filter
comma
l_string|&quot; Enable peripheral qualifier filter: enable=1  (default=0)&quot;
)paren
suffix:semicolon
multiline_comment|/*=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=*/
...
r_static
r_int
id|mptscsih_tm_wait_for_completion
c_func
(paren
id|MPT_SCSI_HOST
op_star
...
(paren
id|MYIOC_s_ERR_FMT
l_string|&quot;_wait_for_completion FAILED!&quot;
l_string|&quot; (hd %p, ioc %p, mf %p) &bslash;n&quot;
comma
id|hd-&gt;ioc-&gt;name
comma
...
multiline_comment|/*=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=*/
multiline_comment|/**&n; *&t;mptscsih_tm_wait_for_completion - wait for completion of TM task&n; *&t;@hd: Pointer to MPT host structure.&n; *&n; *&t;Returns {SUCCESS,FAILED}.&n; */
r_static
r_int
DECL|function|mptscsih_tm_wait_for_completion
id|mptscsih_tm_wait_for_completion
c_func
(paren
id|MPT_SCSI_HOST
...
suffix:semicolon
multiline_comment|/*&n;&t; * If the peripheral qualifier filter is enabled then if the target reports a 0x1&n;&t; * (i.e. The targer is capable of supporting the specified peripheral device type&n;&t; * on this logical unit; however, the physical device is not currently connected&n;&t; * to this logical unit) it will be converted to a 0x3 (i.e. The target is not &n;&t; * capable of supporting a physical device on this logical unit). This is to work&n;&t; * around a bug in th emid-layer in some distributions in which the mid-layer will&n;&t; * continue to try to communicate to the LUN and evntually create a dummy LUN.&n;&t;*/
r_if
c_cond
(paren
id|mpt_pq_filter
