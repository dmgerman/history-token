...
(paren
id|MYIOC_s_INFO_FMT
l_string|&quot;complete (%p, %ld)&bslash;n&quot;
comma
id|ioc-&gt;name
comma
id|sc
...
r_static
r_void
DECL|function|mptscsih_reset_timeouts
id|mptscsih_reset_timeouts
(paren
id|MPT_SCSI_HOST
op_star
id|hd
...
(paren
id|MYIOC_s_WARN_FMT
l_string|&quot;resetting SCpnt=%p timeout + 60HZ&quot;
comma
(paren
id|hd
op_logical_and
...
multiline_comment|/* Search pendingQ, if found,&n;&t;&t;&t; * delete from Q.&n;&t;&t;&t; */
id|mptscsih_search_pendingQ
c_func
(paren
id|hd
...
suffix:semicolon
macro_line|#if 1
multiline_comment|/* Post to doneQ, do not reply until POST phase&n;&t;&t;&t; * of reset handler....prevents new commands from&n;&t;&t;&t; * being queued.&n;&t;&t;&t; */
id|spin_lock_irqsave
c_func
(paren
op_amp
...
id|SCpnt
suffix:semicolon
multiline_comment|/* Add to the doneQ&n;&t;&t;&t;&t; */
id|Q_ADD_TAIL
c_func
(paren
op_amp
...
multiline_comment|/* Allocate memory for free and doneQ&squot;s&n;&t; */
id|sz
op_assign
id|sh-&gt;can_queue
op_star
...
id|mem
suffix:semicolon
multiline_comment|/* Initialize the free, done and pending Qs.&n;&t; */
id|Q_INIT
c_func
(paren
op_amp
...
multiline_comment|/* Initialize this Scsi_Host&n;&t; * internal task Q.&n;&t; */
id|Q_INIT
c_func
(paren
op_amp
...
macro_line|#ifdef MPTSCSIH_DBG_TIMEOUT
id|hd-&gt;ioc-&gt;timeout_hard
op_assign
l_int|0
suffix:semicolon
id|hd-&gt;ioc-&gt;timeout_delta
op_assign
l_int|30
op_star
id|HZ
suffix:semicolon
id|hd-&gt;ioc-&gt;timeout_maxcnt
op_assign
l_int|0
suffix:semicolon
id|hd-&gt;ioc-&gt;timeout_cnt
op_assign
l_int|0
suffix:semicolon
r_for
...
macro_line|#ifndef MPTSCSIH_DBG_TIMEOUT
...
DECL|macro|UC_DBG_TIMEOUT
mdefine_line|#define UC_DBG_TIMEOUT&t;&t;0x01
DECL|macro|UC_DBG_HARDRESET
mdefine_line|#define UC_DBG_HARDRESET&t;0x02
...
c_func
(paren
l_string|&quot;user_command:  ioc %d, buffer %s, length %d&bslash;n&quot;
comma
id|ioc-&gt;id
comma
id|btmp
...
c_func
(paren
l_string|&quot;user_command: cnt=%ld delta=%ld&bslash;n&quot;
comma
id|number
comma
id|delta
...
(brace
multiline_comment|/* process this command ...&n;&t;&t;&t; */
id|ioc-&gt;timeout_maxcnt
op_assign
l_int|0
suffix:semicolon
id|ioc-&gt;timeout_delta
op_assign
id|delta
OL
l_int|2
...
(brace
multiline_comment|/* Prevent new commands from being issued&n;&t;&t; * while reloading the FW. Reset timer to 60 seconds,&n;&t;&t; * as the FW can take some time to come ready.&n;&t;&t; * For New EH, cmds on doneQ posted to FW.&n;&t;&t; */
id|did_errcode
op_assign
l_int|1
suffix:semicolon
...
op_increment
suffix:semicolon
singleline_comment|//mod_timer(&amp;SCpnt-&gt;eh_timeout, jiffies + hd-&gt;ioc-&gt;timeout_delta);
id|issueCmd
op_assign
l_int|0
suffix:semicolon
...
(paren
id|MYIOC_s_WARN_FMT
l_string|&quot;to pendingQ: (sc=%p, mf=%p, time=%ld)&bslash;n&quot;
comma
id|hd-&gt;ioc-&gt;name
comma
id|SCpnt
...
(paren
id|MYIOC_s_WARN_FMT
l_string|&quot;SCpnt=%p to doneQ&bslash;n&quot;
comma
(paren
id|hd
op_logical_and
...
id|MYNAM
l_string|&quot;: %s: mptscsih_abort: &quot;
l_string|&quot;Posting pended cmd! (sc=%p)&bslash;n&quot;
comma
id|hd-&gt;ioc-&gt;name
comma
id|SCpnt
...
suffix:semicolon
multiline_comment|/* Unsupported for SCSI. Supported for FCP&n;&t; */
r_if
c_cond
(paren
id|hd-&gt;is_spi
...
suffix:semicolon
singleline_comment|//&t;printk(&quot;testing start : mptscsih_schedule_reset&bslash;n&quot;);
singleline_comment|//&t;mptscsih_schedule_reset(hd);
singleline_comment|//&t;printk(&quot;testing end: mptscsih_schedule_reset&bslash;n&quot;);
r_if
c_cond
(paren
id|mptscsih_TMHandler
...
(brace
multiline_comment|/* error case - don&squot;t know about this device */
id|scsi_adjust_queue_depth
c_func
(paren
id|device
...
suffix:semicolon
macro_line|#if 0&t;&t;
multiline_comment|/* calling mod_timer() panics in 2.6 kernel...&n;&t;&t; * need to investigate&n;&t;&t; */
id|mptscsih_reset_timeouts
(paren
id|hd
)paren
...
suffix:semicolon
multiline_comment|/* Is LUN supported? If so, upper 3 bits will be 0&n;&t;* in first byte of inquiry data.&n;&t;*/
r_if
c_cond
(paren
id|data
...
id|MPT_TARGET_FLAGS_VALID_INQUIRY
suffix:semicolon
multiline_comment|/* If LUN 0, tape and have not done DV, set the DV flag.&n;&t;&t; */
r_if
c_cond
(paren
id|hd-&gt;is_spi
...
r_else
(brace
multiline_comment|/* Treat all Processors as SAF-TE if&n;&t;&t;&t;&t; * command line option is set */
r_if
c_cond
(paren
id|hd-&gt;ioc-&gt;spi_data.Saf_Te
...
id|MPT_TARGET_NO_NEGO_QAS
suffix:semicolon
multiline_comment|/* Disable QAS in a mixed configuration case&n;&t;&t; &t;&t;*/
singleline_comment|//&t;&t;&t;&t;ddvtprintk((KERN_INFO &quot;Disabling QAS!&bslash;n&quot;));
r_for
c_loop
(paren
id|ii
...
(paren
id|MYIOC_s_INFO_FMT
l_string|&quot;writeIOCPage4 (mf=%p, id=%d)&bslash;n&quot;
comma
id|ioc-&gt;name
comma
id|mf
...
(paren
id|MYIOC_s_INFO_FMT
l_string|&quot;writeIOCPage4: pgaddr 0x%x&bslash;n&quot;
comma
id|ioc-&gt;name
comma
(paren
...
macro_line|#endif /* ~MPTSCSIH_ENABLE_DOMAIN_VALIDATION */
multiline_comment|/*=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=*/
multiline_comment|/* Commandline Parsing routines and defines.&n; *&n; * insmod format:&n; *&t;insmod mptscsih mptscsih=&quot;width:1 dv:n factor:0x09 saf-te:1&quot;&n; *  boot format:&n; *&t;mptscsih=width:1,dv:n,factor:0x8,saf-te:1&n; *&n; */
macro_line|#ifdef MODULE
DECL|macro|ARG_SEP
...
l_string|&quot;saf-te:&quot;
suffix:semicolon
multiline_comment|/* DO NOT REMOVE THIS &squot;;&squot; */
DECL|macro|OPT_DV
...
DECL|macro|OPT_SAF_TE
mdefine_line|#define OPT_SAF_TE&t;&t;4
multiline_comment|/*=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=*/
r_static
r_int
