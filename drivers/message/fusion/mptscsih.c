...
macro_line|#if LINUX_VERSION_CODE &gt;= KERNEL_VERSION(2,5,52)
r_int
...
macro_line|#else /* LINUX_VERSION_CODE &gt;= KERNEL_VERSION(2,5,52) */
r_void
...
(paren
id|MYIOC_s_INFO_FMT
l_string|&quot;target = %d, sync factor = %#x, queue depth = %d&bslash;n&quot;
comma
id|hd-&gt;ioc-&gt;name
comma
id|pTarget-&gt;target_id
comma
id|pTarget-&gt;minSyncFactor
comma
id|device-&gt;queue_depth
...
macro_line|#endif /* LINUX_VERSION_CODE &gt;= KERNEL_VERSION(2,5,52) */
multiline_comment|/*=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=*/
...
suffix:semicolon
multiline_comment|/* If LUN 0, tape and have not done DV, set the DV flag.&n;&t;&t;&t; */
r_if
c_cond
(paren
(paren
