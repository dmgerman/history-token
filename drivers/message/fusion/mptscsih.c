...
multiline_comment|/* mptscsih_io_direction() */
multiline_comment|/*=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=*/
...
(paren
id|MYIOC_s_INFO_FMT
l_string|&quot;SCSI data underrun: underflow=%02x, xfercnt=%02x&bslash;n&quot;
comma
id|ioc-&gt;name
comma
id|sc-&gt;underflow
...
(brace
multiline_comment|/* Command found.&n;&t;&t;&t; *&n;&t;&t;&t; * Search pendingQ, if found,&n;&t;&t;&t; * delete from Q. If found, do not decrement&n;&t;&t;&t; * queue_depth, command never posted.&n;&t;&t;&t; */
r_if
c_cond
(paren
id|mptscsih_search_pendingQ
...
id|KERN_INFO
id|MYNAM
l_string|&quot;: search_running target %d lun %d max %d numIos %d&bslash;n&quot;
comma
id|target
comma
id|lun
...
r_continue
suffix:semicolon
multiline_comment|/* If cmd pended, do not decrement queue_depth, command never posted.&n;&t;&t;&t; */
r_if
c_cond
(paren
id|mptscsih_search_pendingQ
...
multiline_comment|/* &t;mptscsih_flush_drop_test - Free resources and do callback if&n; *&t;&t;DROP_TEST enabled.&n; *&n; *&t;@hd: Pointer to a SCSI HOST structure&n; *&n; *&t;Returns: None.&n; *&n; *&t;Must be called while new I/Os are being queued.&n; */
r_static
r_void
DECL|function|mptscsih_flush_drop_test
id|mptscsih_flush_drop_test
(paren
id|MPT_SCSI_HOST
op_star
id|hd
...
id|req_idx
suffix:semicolon
multiline_comment|/* Free resources for the drop test MF&n;&t; * and chain buffers.&n;&t; */
r_if
c_cond
(paren
id|dropMfPtr
...
r_else
(brace
multiline_comment|/* unmap OS resources, set status, do callback&n;&t;&t;&t; * free driver resources&n;&t;&t;&t; */
r_if
c_cond
(paren
id|sc-&gt;use_sg
...
multiline_comment|/*&t;memset(mem, 0xFF, sz); */
r_for
c_loop
(paren
id|ii
...
suffix:semicolon
multiline_comment|/* Attach the SCSI Host to the IOC structure&n;&t;&t;&t; */
id|ioc-&gt;sh
op_assign
id|sh
suffix:semicolon
...
l_int|16
suffix:semicolon
multiline_comment|/* Yikes!  This is important!&n;&t;&t;&t; * Otherwise, by default, linux&n;&t;&t;&t; * only scans target IDs 0-7!&n;&t;&t;&t; * pfactsN-&gt;MaxDevices unreliable&n;&t;&t;&t; * (not supported in early&n;&t;&t;&t; *&t;versions of the FW).&n;&t;&t;&t; * max_id = 1 + actual max id,&n;&t;&t;&t; * max_lun = 1 + actual last lun,&n;&t;&t;&t; *&t;see hosts.h :o(&n;&t;&t;&t; */
r_if
c_cond
(paren
(paren
...
r_else
(brace
multiline_comment|/* For FC, increase the queue depth&n;&t;&t;&t; * from MPT_SCSI_CAN_QUEUE (31)&n;&t;&t;&t; * to MPT_FC_CAN_QUEUE (63).&n;&t;&t;&t; */
id|sh-&gt;can_queue
op_assign
id|MPT_FC_CAN_QUEUE
suffix:semicolon
...
id|PortSCSIID
suffix:semicolon
multiline_comment|/* Required entry.&n;&t;&t;&t; */
id|sh-&gt;unique_id
op_assign
id|ioc-&gt;id
suffix:semicolon
multiline_comment|/* Verify that we won&squot;t exceed the maximum&n;&t;&t;&t; * number of chain buffers&n;&t;&t;&t; * We can optimize:  ZZ = req_sz/sizeof(SGE)&n;&t;&t;&t; * For 32bit SGE&squot;s:&n;&t;&t;&t; *  numSGE = 1 + (ZZ-1)*(maxChain -1) + ZZ&n;&t;&t;&t; *               + (req_sz - 64)/sizeof(SGE)&n;&t;&t;&t; * A slightly different algorithm is required for&n;&t;&t;&t; * 64bit SGEs.&n;&t;&t;&t; */
id|scale
op_assign
id|ioc-&gt;req_sz
op_div
...
multiline_comment|/* Set the pci device pointer in Scsi_Host structure.&n;&t;&t;&t; */
id|scsi_set_device
c_func
(paren
id|sh
...
id|mptscsih_probe_failed
suffix:semicolon
multiline_comment|/* Allocate memory for free and doneQ&squot;s&n;&t;&t;&t; */
id|sz
op_assign
id|sh-&gt;can_queue
op_star
...
id|mem
suffix:semicolon
multiline_comment|/* Initialize the free, done and pending Qs.&n;&t;&t;&t; */
id|Q_INIT
c_func
(paren
op_amp
...
multiline_comment|/* Initialize this Scsi_Host&n;&t;&t;&t; * internal task Q.&n;&t;&t;&t; */
id|Q_INIT
c_func
(paren
op_amp
...
l_int|0
suffix:semicolon
multiline_comment|/* Allocate memory for the device structures.&n;&t;&t;&t; * A non-Null pointer at an offset&n;&t;&t;&t; * indicates a device exists.&n;&t;&t;&t; * max_id = 1 + maximum id (hosts.h)&n;&t;&t;&t; */
id|sz
op_assign
id|sh-&gt;max_id
op_star
...
suffix:semicolon
multiline_comment|/* Clear the TM flags&n;&t;&t;&t; */
id|hd-&gt;tmPending
op_assign
l_int|0
suffix:semicolon
...
l_int|0
suffix:semicolon
multiline_comment|/* Clear the pointer used to store&n;&t;&t;&t; * single-threaded commands, i.e., those&n;&t;&t;&t; * issued during a bus scan, dv and&n;&t;&t;&t; * configuration pages.&n;&t;&t;&t; */
id|hd-&gt;cmdPtr
op_assign
l_int|NULL
suffix:semicolon
multiline_comment|/* Initialize this SCSI Hosts&squot; timers&n;&t;&t;&t; * To use, set the timer expires field&n;&t;&t;&t; * and add_timer&n;&t;&t;&t; */
id|init_timer
c_func
(paren
op_amp
...
(brace
multiline_comment|/* Update with the driver setup&n;&t;&t;&t;&t; * values.&n;&t;&t;&t;&t; */
r_if
c_cond
(paren
...
multiline_comment|/* scsi_host_alloc */
)brace
...
suffix:semicolon
macro_line|#endif
multiline_comment|/* probing for devices */
r_for
c_loop
(paren
id|ioc
...
id|ioc
suffix:semicolon
multiline_comment|/* removing devices */
r_for
c_loop
(paren
id|ioc
...
suffix:semicolon
DECL|macro|UC_GET_SPEED
mdefine_line|#define UC_GET_SPEED&t;0x10
DECL|function|mptscsih_exec_user_cmd
r_static
r_void
id|mptscsih_exec_user_cmd
c_func
(paren
id|MPT_ADAPTER
op_star
...
(paren
(paren
l_string|&quot;exec_user_command: ioc %p cmd %ld target=%ld&bslash;n&quot;
comma
id|ioc
comma
id|uc-&gt;cmd
...
id|uc-&gt;target
suffix:semicolon
multiline_comment|/* bus &lt;&lt; 8 | target */
id|cfg.physAddr
op_assign
id|cfg_dma_addr
suffix:semicolon
...
mdefine_line|#define SKIP_SPACES(min_spaces)&t;&t;&t;&t;&t;&t;&bslash;&n;&t;if ((arg_len = skip_spaces(ptr,len)) &lt; (min_spaces))&t;&t;&bslash;&n;&t;&t;return -EINVAL;&t;&t;&t;&t;&t;&t;&bslash;&n;&t;ptr += arg_len;&t;&t;&t;&t;&t;&t;&t;&bslash;&n;&t;len -= arg_len;
DECL|macro|GET_INT_ARG
mdefine_line|#define GET_INT_ARG(v)&t;&t;&t;&t;&t;&t;&t;&bslash;&n;&t;if (!(arg_len = get_int_arg(ptr,len, &amp;(v))))&t;&t;&t;&bslash;&n;&t;&t;return -EINVAL;&t;&t;&t;&t;&t;&t;&bslash;&n;&t;ptr += arg_len;&t;&t;&t;&t;&t;&t;&t;&bslash;&n;&t;len -= arg_len;
DECL|function|mptscsih_user_command
r_static
r_int
id|mptscsih_user_command
c_func
(paren
id|MPT_ADAPTER
op_star
...
(paren
(paren
l_string|&quot;user_command:  arg_len=%d, cmd=%ld&bslash;n&quot;
comma
id|arg_len
comma
id|uc-&gt;cmd
...
(paren
(paren
l_string|&quot;user_command: target=%ld len=%d&bslash;n&quot;
comma
id|uc-&gt;target
comma
id|len
...
r_else
(brace
multiline_comment|/* process this command ...&n;&t;&t; */
id|mptscsih_exec_user_cmd
c_func
(paren
id|ioc
...
(paren
(paren
l_string|&quot;buffer %p, start=%p (%p) offset=%ld length = %d&bslash;n&quot;
comma
id|buffer
comma
id|start
...
suffix:semicolon
multiline_comment|/*&n;&t; *  Write SCSI CDB into the message&n;&t; *  Should write from cmd_len up to 16, but skip for performance reasons.&n;&t; */
id|cmd_len
op_assign
id|SCpnt-&gt;cmd_len
suffix:semicolon
...
(paren
id|MYIOC_s_INFO_FMT
l_string|&quot;Pending cmd=%p idx %d&bslash;n&quot;
comma
id|hd-&gt;ioc-&gt;name
comma
id|SCpnt
...
(paren
id|MYIOC_s_ERR_FMT
l_string|&quot;slave_alloc kmalloc(%d) FAILED!&bslash;n&quot;
comma
id|hd-&gt;ioc-&gt;name
comma
(paren
...
suffix:semicolon
multiline_comment|/* Free memory and reset all flags for this target&n;&t; */
r_if
c_cond
(paren
(paren
...
suffix:semicolon
multiline_comment|/* Free device structure only if number of luns is 0.&n;&t;&t; */
r_if
c_cond
(paren
id|vdev-&gt;num_luns
...
id|MPT_SCSICFG_DV_NOT_DONE
suffix:semicolon
multiline_comment|/* Don&squot;t alter isRaid, not allowed to move&n;&t;&t;&t;&t; * volumes on a running system.&n;&t;&t;&t;&t; */
r_if
c_cond
(paren
...
(paren
id|KERN_INFO
l_string|&quot;slave_configure: device @ %p, id=%d, LUN=%d, channel=%d&bslash;n&quot;
comma
id|device
comma
id|device-&gt;id
...
suffix:semicolon
multiline_comment|/*&t;set target parameters, queue depths, set dv flags ?  */
r_if
c_cond
(paren
id|hd
...
id|vdev-&gt;tflags
op_amp
id|MPT_TARGET_FLAGS_CONFIGURED
)paren
)paren
(brace
multiline_comment|/* Configure only the first discovered LUN&n;&t;&t;&t; */
id|vdev-&gt;raidVolume
op_assign
l_int|0
suffix:semicolon
...
(brace
multiline_comment|/* set the queue depth for all devices&n;&t;&t;&t; */
r_if
c_cond
(paren
op_logical_neg
...
r_static
r_void
DECL|function|mptscsih_target_settings
id|mptscsih_target_settings
c_func
(paren
id|MPT_SCSI_HOST
...
(paren
id|KERN_INFO
l_string|&quot;set Target: (id %d) &bslash;n&quot;
comma
id|id
)paren
)paren
...
(brace
multiline_comment|/* FC - only care about QTag support&n;&t; &t; */
r_if
c_cond
(paren
...
multiline_comment|/* SCSI - Set flags based on Inquiry data&n;&t; */
r_if
c_cond
(paren
id|sdev-&gt;scsi_level
...
(brace
multiline_comment|/* U320 requires IU capability */
r_if
c_cond
(paren
(paren
...
multiline_comment|/* Disable QAS if mixed configuration case&n;&t; */
r_if
c_cond
(paren
(paren
...
(paren
id|KERN_INFO
l_string|&quot;Final settings id %d: dvstatus 0x%x&bslash;n&quot;
comma
id|sdev-&gt;id
comma
id|pspi_data-&gt;dvStatus
...
(paren
(paren
l_string|&quot;wide %d, factor 0x%x offset 0x%x neg flags 0x%x flags 0x%x&bslash;n&quot;
comma
id|width
comma
id|factor
...
(paren
id|MYIOC_s_INFO_FMT
l_string|&quot;: post_pendingQ_commands&bslash;n&quot;
comma
id|hd-&gt;ioc-&gt;name
)paren
)paren
...
(paren
id|MYIOC_s_INFO_FMT
l_string|&quot;Issued SCSI cmd (sc=%p) idx=%d (mf=%p)&bslash;n&quot;
comma
id|hd-&gt;ioc-&gt;name
comma
id|sc
...
suffix:semicolon
multiline_comment|/* 8. Set flag to force DV and re-read IOC Page 3&n;&t;&t; */
r_if
c_cond
(paren
id|hd-&gt;is_spi
...
multiline_comment|/* Prevent the system from continually writing to the log&n;&t; * if a medium is not found: 02 3A 00&n;&t; * Changer issues: TUR, Read Capacity, Table of Contents continually&n;&t; */
r_if
c_cond
(paren
id|sk
...
multiline_comment|/*=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=*/
multiline_comment|/*&n; * If no Target (old) or Target unconfigured (new) and bus reset on 1st I/O,&n; * set the flag to prevent any future negotiations to this device.&n; */
...
id|header1
suffix:semicolon
multiline_comment|/* Double writes to SDP1 can cause problems,&n;&t;&t; * skip save of the final negotiated settings to&n;&t;&t; * SCSI device page 1.&n;&t;&t; */
id|cfg.physAddr
op_assign
id|cfg1_dma_addr
suffix:semicolon
