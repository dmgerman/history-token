...
suffix:semicolon
DECL|variable|i2ob_backlog_tail
r_static
r_struct
id|i2ob_request
op_star
id|i2ob_backlog_tail
(braket
id|MAX_I2O_CONTROLLERS
)braket
suffix:semicolon
...
multiline_comment|/*&n;&t;&t; *&t;Heuristic for now since the block layer doesnt give&n;&t;&t; *&t;us enough info. If its a big write assume sequential&n;&t;&t; *&t;readahead on controller. If its small then don&squot;t read&n;&t;&t; *&t;ahead but do use the controller cache.&n;&t;&t; */
r_if
c_cond
(paren
id|size
...
r_else
multiline_comment|/* &n;&t;&t;&t;&t; * Allow replies to come back once data is cached in the controller&n;&t;&t;&t;&t; * This allows us to handle writes quickly thus giving more of the&n;&t;&t;&t;&t; * queue to reads.&n;&t;&t;&t;&t; */
id|__raw_writel
c_func
(paren
l_int|16
...
r_else
(brace
multiline_comment|/* Large write, don&squot;t cache */
r_if
c_cond
(paren
id|size
...
id|m
suffix:semicolon
multiline_comment|/*&n;&t; *&t;Ask the controller to write the cache back. This sorts out&n;&t; *&t;the supertrak firmware flaw and also does roughly the right&n;&t; *&t;thing for other cases too.&n;&t; */
id|__raw_writel
c_func
(paren
...
(paren
(paren
l_string|&quot;unconstipated&bslash;n&quot;
)paren
)paren
suffix:semicolon
r_if
...
(brace
multiline_comment|/*&n;&t;&t;&t; *&t;Time to uncork stuff&n;&t;&t;&t; */
r_if
c_cond
(paren
op_logical_neg
...
(paren
(paren
l_string|&quot;constipated&bslash;n&quot;
)paren
)paren
suffix:semicolon
multiline_comment|/* Now pull the chain */
r_if
c_cond
(paren
id|i2ob_flush
...
(paren
(paren
l_string|&quot;i2ob: Unable to queue flush. Retrying I/O immediately.&bslash;n&quot;
)paren
)paren
suffix:semicolon
id|dev-&gt;constipated
...
multiline_comment|/*&n;&t;&t;&t; *&t;Recycle the request&n;&t;&t;&t; */
singleline_comment|//&t;&t;&t;i2ob_unhook_request(ireq, c-&gt;unit);
multiline_comment|/*&n;&t;&t;&t; *&t;Place it on the recycle queue&n;&t;&t;&t; */
id|ireq-&gt;next
op_assign
l_int|NULL
suffix:semicolon
...
suffix:semicolon
multiline_comment|/*&n;&t;&t;&t; *&t;If the constipator flush failed we want to&n;&t;&t;&t; *&t;poke the queue again. &n;&t;&t;&t; */
id|i2ob_request
c_func
(paren
id|dev-&gt;req_queue
...
suffix:semicolon
multiline_comment|/*&n;&t;&t;&t; *&t;and out&n;&t;&t;&t; */
r_return
suffix:semicolon
)brace
id|spin_unlock_irqrestore
...
(paren
id|KERN_INFO
l_string|&quot;%s: Received event %d we didn&squot;t register for&bslash;n&quot;
id|KERN_INFO
...
id|flags
suffix:semicolon
multiline_comment|/*&n;&t; * We cannot touch the request queue or the timer&n;         * flag without holding the queue_lock&n;&t; */
id|spin_lock_irqsave
c_func
(paren
op_amp
...
DECL|function|i2ob_backlog_request
r_static
r_int
id|i2ob_backlog_request
c_func
...
multiline_comment|/*&n;&t;&t; *&t;Is the channel constipated ?&n;&t;&t; */
r_if
c_cond
(paren
id|i2ob_backlog
...
id|I2O_TYPE_PCI
op_logical_and
id|d-&gt;controller-&gt;bus.pci.queue_buggy
op_eq
l_int|2
)paren
(brace
...
multiline_comment|/*&n;&t; *    The device list connected to the I2O Controller is doubly linked&n;&t; * Here we traverse the end of the list , and start claiming devices&n;&t; * from that end. This assures that within an I2O controller atleast&n;&t; * the newly created volumes get claimed after the older ones, thus&n;&t; * mapping to same major/minor (and hence device file name) after &n;&t; * every reboot.&n;&t; * The exception being: &n;&t; * 1. If there was a TID reuse.&n;&t; * 2. There was more than one I2O controller. &n;&t; */
r_if
c_cond
(paren
op_logical_neg
...
suffix:semicolon
multiline_comment|/*&n;&t; *&t;Adding i2ob_gendisk into the gendisk list.&n;&t; */
id|add_gendisk
c_func
(paren
op_amp
