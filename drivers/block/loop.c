...
DECL|function|do_bh_filebacked
r_static
r_int
id|do_bh_filebacked
c_func
...
(brace
multiline_comment|/*&n;&t; * check b_end_io, may just be a remapped bh and not an allocated one&n;&t; */
r_if
c_cond
(paren
id|bh
...
multiline_comment|/*&n; * when buffer i/o has completed. if BH_Dirty is set, this was a WRITE&n; * and lo-&gt;transfer stuff has already been done. if not, it was a READ&n; * so queue it for the loop thread and let it do the transfer out of&n; * b_end_io context (we don&squot;t want to do decrypt of a page with irqs&n; * disabled)&n; */
...
(paren
id|KERN_ERR
l_string|&quot;loop: unknown command (%d)&bslash;n&quot;
comma
id|rw
)paren
suffix:semicolon
...
c_func
(paren
l_string|&quot;loop: missing bh&bslash;n&quot;
)paren
suffix:semicolon
r_continue
suffix:semicolon
