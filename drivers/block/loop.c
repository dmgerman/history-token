...
multiline_comment|/**&n; * do_lo_send_aops - helper for writing data to a loop device&n; *&n; * This is the fast version for backing filesystems which implement the address&n; * space operations prepare_write and commit_write.&n; */
DECL|function|do_lo_send_aops
r_static
r_int
id|do_lo_send_aops
c_func
...
multiline_comment|/**&n; * __do_lo_send_write - helper for writing data to a loop device&n; *&n; * This helper just factors out common code between do_lo_send_direct_write()&n; * and do_lo_send_write().&n; */
DECL|function|__do_lo_send_write
r_static
r_inline
r_int
id|__do_lo_send_write
c_func
(paren
r_struct
id|file
...
(paren
id|KERN_ERR
l_string|&quot;loop: Write error at byte offset %llu, length %i.&bslash;n&quot;
comma
(paren
r_int
r_int
...
multiline_comment|/**&n; * do_lo_send_direct_write - helper for writing data to a loop device&n; *&n; * This is the fast, non-transforming version for backing filesystems which do&n; * not implement the address space operations prepare_write and commit_write.&n; * It uses the write file operation which should be present on all writeable&n; * filesystems.&n; */
DECL|function|do_lo_send_direct_write
r_static
r_int
id|do_lo_send_direct_write
c_func
(paren
r_struct
id|loop_device
...
multiline_comment|/**&n; * do_lo_send_write - helper for writing data to a loop device&n; *&n; * This is the slow, transforming version for filesystems which do not&n; * implement the address space operations prepare_write and commit_write.  It&n; * uses the write file operation which should be present on all writeable&n; * filesystems.&n; *&n; * Using fops-&gt;write is slower than using aops-&gt;{prepare,commit}_write in the&n; * transforming case because we need to double buffer the data as we cannot do&n; * the transformations in place as we do not have direct access to the&n; * destination pages of the backing file.&n; */
DECL|function|do_lo_send_write
r_static
r_int
id|do_lo_send_write
c_func
...
(paren
id|KERN_ERR
l_string|&quot;loop: Transfer error at byte offset %llu, &quot;
...
(paren
id|KERN_ERR
l_string|&quot;loop: Failed to allocate temporary page for write.&bslash;n&quot;
)paren
suffix:semicolon
id|ret
op_assign
