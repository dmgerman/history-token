...
multiline_comment|/*&n; * This task is about to go to sleep on IO.  Increment nr_iowait_tasks so&n; * that process accounting knows that this is a task in IO wait state.&n; *&n; * But don&squot;t do that if it is a deliberate, throttling IO wait (this task&n; * has set its backing_dev_info: the queue against which it should throttle)&n; */
...
(paren
id|KERN_ERR
l_string|&quot;request %p for device [02%x:02%x] already tagged %d&quot;
comma
id|rq
comma
id|major
...
multiline_comment|/**&n; * blk_congestion_wait - wait for a queue to become uncongested&n; * @rw: READ or WRITE&n; * @timeout: timeout in jiffies&n; *&n; * Waits for up to @timeout jiffies for a queue (any queue) to exit congestion.&n; * If no queues are congested then just return, in the hope that the caller&n; * will submit some more IO.&n; */
...
multiline_comment|/*&n;&t;&t; * we are ending the last part of the bio, advance req pointer&n;&t;&t; */
r_if
c_cond
(paren
(paren
...
id|nsect
suffix:semicolon
multiline_comment|/*&n;&t;&t; * if we didn&squot;t advance the req-&gt;bio pointer, advance bi_idx&n;&t;&t; * to indicate we are now on the next bio_vec&n;&t;&t; */
r_if
c_cond
(paren
op_logical_neg
