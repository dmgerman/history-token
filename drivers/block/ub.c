...
multiline_comment|/*&n; * Downcount for deallocation. This rides on two assumptions:&n; *  - once something is poisoned, its refcount cannot grow&n; *  - opens cannot happen at this time (del_gendisk was done)&n; * If the above is true, we can drop the lock, which we need for&n; * blk_cleanup_queue(): the silly thing may attempt to sleep.&n; * [Actually, it never needs to sleep for us, but it calls might_sleep()]&n; */
DECL|function|ub_put
r_static
r_void
id|ub_put
c_func
...
multiline_comment|/*&n; * Final cleanup and deallocation.&n; */
...
DECL|function|ub_cmd_build_block
r_static
r_int
id|ub_cmd_build_block
c_func
...
(paren
id|KERN_WARNING
l_string|&quot;%s: unmapped block request&quot;
l_string|&quot; flags 0x%lx sectors %lu&bslash;n&quot;
comma
id|sc-&gt;name
comma
id|rq-&gt;flags
...
DECL|function|ub_cmd_build_packet
r_static
r_int
id|ub_cmd_build_packet
c_func
...
(paren
id|KERN_WARNING
l_string|&quot;%s: unmapped packet request&quot;
l_string|&quot; flags 0x%lx length %d&bslash;n&quot;
comma
id|sc-&gt;name
comma
id|rq-&gt;flags
...
(brace
multiline_comment|/* UB_SENSE_SIZE is smaller than SCSI_SENSE_BUFFERSIZE */
id|memcpy
c_func
(paren
id|rq-&gt;sense
...
id|sc-&gt;work_bcb
suffix:semicolon
multiline_comment|/*&n;&t; * ``If the allocation length is eighteen or greater, and a device&n;&t; * server returns less than eithteen bytes of data, the application&n;&t; * client should assume that the bytes not transferred would have been&n;&t; * zeroes had the device server returned those bytes.&squot;&squot;&n;&t; *&n;&t; * We zero sense for all commands so that when a packet request&n;&t; * fails it does not return a stale sense.&n;&t; */
id|memset
c_func
(paren
op_amp
...
id|flags
suffix:semicolon
multiline_comment|/*&n;&t; * Prevent ub_bd_release from pulling the rug from under us.&n;&t; * XXX This is starting to look like a kref.&n;&t; * XXX Why not to take this ref at probe time?&n;&t; */
id|spin_lock_irqsave
c_func
(paren
op_amp
...
suffix:semicolon
multiline_comment|/*&n;&t; * Unregister the upper layer.&n;&t; */
r_if
c_cond
(paren
id|disk-&gt;flags
...
suffix:semicolon
multiline_comment|/*&n;&t; * I wish I could do:&n;&t; *    set_bit(QUEUE_FLAG_DEAD, &amp;q-&gt;queue_flags);&n;&t; * As it is, we rely on our internal poisoning and let&n;&t; * the upper levels to spin furiously failing all the I/O.&n;&t; */
