...
(paren
id|KERN_DEBUG
l_string|&quot;%s: Using software-emulated EPP mode&bslash;n&quot;
comma
id|port-&gt;name
)paren
suffix:semicolon
...
id|IEEE1284_MODE_COMPAT
suffix:colon
multiline_comment|/* if we can tri-state use BYTE mode instead of NIBBLE mode,&n;&t;&t; * if that fails, revert to NIBBLE mode -- ought to store somewhere&n;&t;&t; * the device&squot;s ability to do BYTE mode reverse transfers, so we don&squot;t&n;&t;&t; * end up needlessly calling negotiate(BYTE) repeately..  (fb)&n;&t;&t; */
r_if
c_cond
(paren
(paren
id|port-&gt;physport-&gt;modes
op_amp
...
(brace
multiline_comment|/* got into BYTE mode OK */
id|DPRINTK
(paren
id|KERN_DEBUG
...
multiline_comment|/* fall through to NIBBLE */
r_case
id|IEEE1284_MODE_NIBBLE
suffix:colon
id|DPRINTK
