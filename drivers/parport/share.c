...
(brace
multiline_comment|/* caller has exclusive registration_lock */
r_struct
id|parport_driver
op_star
id|drv
...
multiline_comment|/* Call the driver&squot;s detach routine for each&n;&t; * port to clean up any resources that the&n;&t; * attach routine acquired. */
id|list_for_each_entry
c_func
(paren
id|port
...
suffix:semicolon
multiline_comment|/*&n;&t; * Now that the portnum is known finish doing the Init.&n;&t; */
id|sprintf
c_func
(paren
id|name
...
suffix:semicolon
multiline_comment|/* Let drivers know that new port(s) has arrived. */
id|attach_driver_chain
(paren
id|port
)paren
...
DECL|function|parport_remove_port
r_void
...
multiline_comment|/* We up our own module reference count, and that of the port&n;           on which a device is to be registered, to ensure that&n;           neither of us gets unloaded while we sleep in (e.g.)&n;           kmalloc.&n;         */
r_if
c_cond
(paren
op_logical_neg
...
suffix:semicolon
DECL|variable|parport_remove_port
id|EXPORT_SYMBOL
c_func
(paren
id|parport_remove_port
