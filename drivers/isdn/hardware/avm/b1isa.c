multiline_comment|/* $Id: b1isa.c,v 1.1.2.3 2004/02/10 01:07:12 keil Exp $&n; * &n; * Module for AVM B1 ISA-card.&n; * &n; * Copyright 1999 by Carsten Paeth &lt;calle@calle.de&gt;&n; * &n; * This software may be used and distributed according to the terms&n; * of the GNU General Public License, incorporated herein by reference.&n; *&n; */
macro_line|#include &lt;linux/module.h&gt;
macro_line|#include &lt;linux/kernel.h&gt;
macro_line|#include &lt;linux/skbuff.h&gt;
macro_line|#include &lt;linux/delay.h&gt;
macro_line|#include &lt;linux/mm.h&gt;
macro_line|#include &lt;linux/interrupt.h&gt;
macro_line|#include &lt;linux/ioport.h&gt;
macro_line|#include &lt;linux/capi.h&gt;
macro_line|#include &lt;linux/init.h&gt;
macro_line|#include &lt;linux/pci.h&gt;
macro_line|#include &lt;asm/io.h&gt;
macro_line|#include &lt;linux/isdn/capicmd.h&gt;
macro_line|#include &lt;linux/isdn/capiutil.h&gt;
macro_line|#include &lt;linux/isdn/capilli.h&gt;
macro_line|#include &quot;avmcard.h&quot;
multiline_comment|/* ------------------------------------------------------------- */
DECL|variable|revision
r_static
r_char
op_star
id|revision
op_assign
l_string|&quot;$Revision: 1.1.2.3 $&quot;
suffix:semicolon
multiline_comment|/* ------------------------------------------------------------- */
id|MODULE_DESCRIPTION
c_func
(paren
l_string|&quot;CAPI4Linux: Driver for AVM B1 ISA card&quot;
)paren
suffix:semicolon
id|MODULE_AUTHOR
c_func
(paren
l_string|&quot;Carsten Paeth&quot;
)paren
suffix:semicolon
id|MODULE_LICENSE
c_func
(paren
l_string|&quot;GPL&quot;
)paren
suffix:semicolon
multiline_comment|/* ------------------------------------------------------------- */
DECL|function|b1isa_remove
r_static
r_void
id|b1isa_remove
c_func
(paren
r_struct
id|pci_dev
op_star
id|pdev
)paren
(brace
id|avmctrl_info
op_star
id|cinfo
op_assign
id|pci_get_drvdata
c_func
(paren
id|pdev
)paren
suffix:semicolon
id|avmcard
op_star
id|card
suffix:semicolon
r_if
c_cond
(paren
op_logical_neg
id|cinfo
)paren
r_return
suffix:semicolon
id|card
op_assign
id|cinfo-&gt;card
suffix:semicolon
id|b1_reset
c_func
(paren
id|card-&gt;port
)paren
suffix:semicolon
id|b1_reset
c_func
(paren
id|card-&gt;port
)paren
suffix:semicolon
id|detach_capi_ctr
c_func
(paren
op_amp
id|cinfo-&gt;capi_ctrl
)paren
suffix:semicolon
id|free_irq
c_func
(paren
id|card-&gt;irq
comma
id|card
)paren
suffix:semicolon
id|release_region
c_func
(paren
id|card-&gt;port
comma
id|AVMB1_PORTLEN
)paren
suffix:semicolon
id|b1_free_card
c_func
(paren
id|card
)paren
suffix:semicolon
)brace
multiline_comment|/* ------------------------------------------------------------- */
r_static
r_char
op_star
id|b1isa_procinfo
c_func
(paren
r_struct
id|capi_ctr
op_star
id|ctrl
)paren
suffix:semicolon
DECL|function|b1isa_probe
r_static
r_int
id|b1isa_probe
c_func
(paren
r_struct
id|pci_dev
op_star
id|pdev
)paren
(brace
id|avmctrl_info
op_star
id|cinfo
suffix:semicolon
id|avmcard
op_star
id|card
suffix:semicolon
r_int
id|retval
suffix:semicolon
id|card
op_assign
id|b1_alloc_card
c_func
(paren
l_int|1
)paren
suffix:semicolon
r_if
c_cond
(paren
op_logical_neg
id|card
)paren
(brace
id|printk
c_func
(paren
id|KERN_WARNING
l_string|&quot;b1isa: no memory.&bslash;n&quot;
)paren
suffix:semicolon
id|retval
op_assign
op_minus
id|ENOMEM
suffix:semicolon
r_goto
id|err
suffix:semicolon
)brace
id|cinfo
op_assign
id|card-&gt;ctrlinfo
suffix:semicolon
id|card-&gt;port
op_assign
id|pci_resource_start
c_func
(paren
id|pdev
comma
l_int|0
)paren
suffix:semicolon
id|card-&gt;irq
op_assign
id|pdev-&gt;irq
suffix:semicolon
id|card-&gt;cardtype
op_assign
id|avm_b1isa
suffix:semicolon
id|sprintf
c_func
(paren
id|card-&gt;name
comma
l_string|&quot;b1isa-%x&quot;
comma
id|card-&gt;port
)paren
suffix:semicolon
r_if
c_cond
(paren
id|card-&gt;port
op_ne
l_int|0x150
op_logical_and
id|card-&gt;port
op_ne
l_int|0x250
op_logical_and
id|card-&gt;port
op_ne
l_int|0x300
op_logical_and
id|card-&gt;port
op_ne
l_int|0x340
)paren
(brace
id|printk
c_func
(paren
id|KERN_WARNING
l_string|&quot;b1isa: invalid port 0x%x.&bslash;n&quot;
comma
id|card-&gt;port
)paren
suffix:semicolon
id|retval
op_assign
op_minus
id|EINVAL
suffix:semicolon
r_goto
id|err_free
suffix:semicolon
)brace
r_if
c_cond
(paren
id|b1_irq_table
(braket
id|card-&gt;irq
op_amp
l_int|0xf
)braket
op_eq
l_int|0
)paren
(brace
id|printk
c_func
(paren
id|KERN_WARNING
l_string|&quot;b1isa: irq %d not valid.&bslash;n&quot;
comma
id|card-&gt;irq
)paren
suffix:semicolon
id|retval
op_assign
op_minus
id|EINVAL
suffix:semicolon
r_goto
id|err_free
suffix:semicolon
)brace
r_if
c_cond
(paren
op_logical_neg
id|request_region
c_func
(paren
id|card-&gt;port
comma
id|AVMB1_PORTLEN
comma
id|card-&gt;name
)paren
)paren
(brace
id|printk
c_func
(paren
id|KERN_WARNING
l_string|&quot;b1isa: ports 0x%03x-0x%03x in use.&bslash;n&quot;
comma
id|card-&gt;port
comma
id|card-&gt;port
op_plus
id|AVMB1_PORTLEN
)paren
suffix:semicolon
id|retval
op_assign
op_minus
id|EBUSY
suffix:semicolon
r_goto
id|err_free
suffix:semicolon
)brace
id|retval
op_assign
id|request_irq
c_func
(paren
id|card-&gt;irq
comma
id|b1_interrupt
comma
l_int|0
comma
id|card-&gt;name
comma
id|card
)paren
suffix:semicolon
r_if
c_cond
(paren
id|retval
)paren
(brace
id|printk
c_func
(paren
id|KERN_ERR
l_string|&quot;b1isa: unable to get IRQ %d.&bslash;n&quot;
comma
id|card-&gt;irq
)paren
suffix:semicolon
r_goto
id|err_release_region
suffix:semicolon
)brace
id|b1_reset
c_func
(paren
id|card-&gt;port
)paren
suffix:semicolon
r_if
c_cond
(paren
(paren
id|retval
op_assign
id|b1_detect
c_func
(paren
id|card-&gt;port
comma
id|card-&gt;cardtype
)paren
)paren
op_ne
l_int|0
)paren
(brace
id|printk
c_func
(paren
id|KERN_NOTICE
l_string|&quot;b1isa: NO card at 0x%x (%d)&bslash;n&quot;
comma
id|card-&gt;port
comma
id|retval
)paren
suffix:semicolon
id|retval
op_assign
op_minus
id|ENODEV
suffix:semicolon
r_goto
id|err_free_irq
suffix:semicolon
)brace
id|b1_reset
c_func
(paren
id|card-&gt;port
)paren
suffix:semicolon
id|b1_getrevision
c_func
(paren
id|card
)paren
suffix:semicolon
id|cinfo-&gt;capi_ctrl.owner
op_assign
id|THIS_MODULE
suffix:semicolon
id|cinfo-&gt;capi_ctrl.driver_name
op_assign
l_string|&quot;b1isa&quot;
suffix:semicolon
id|cinfo-&gt;capi_ctrl.driverdata
op_assign
id|cinfo
suffix:semicolon
id|cinfo-&gt;capi_ctrl.register_appl
op_assign
id|b1_register_appl
suffix:semicolon
id|cinfo-&gt;capi_ctrl.release_appl
op_assign
id|b1_release_appl
suffix:semicolon
id|cinfo-&gt;capi_ctrl.send_message
op_assign
id|b1_send_message
suffix:semicolon
id|cinfo-&gt;capi_ctrl.load_firmware
op_assign
id|b1_load_firmware
suffix:semicolon
id|cinfo-&gt;capi_ctrl.reset_ctr
op_assign
id|b1_reset_ctr
suffix:semicolon
id|cinfo-&gt;capi_ctrl.procinfo
op_assign
id|b1isa_procinfo
suffix:semicolon
id|cinfo-&gt;capi_ctrl.ctr_read_proc
op_assign
id|b1ctl_read_proc
suffix:semicolon
id|strcpy
c_func
(paren
id|cinfo-&gt;capi_ctrl.name
comma
id|card-&gt;name
)paren
suffix:semicolon
id|retval
op_assign
id|attach_capi_ctr
c_func
(paren
op_amp
id|cinfo-&gt;capi_ctrl
)paren
suffix:semicolon
r_if
c_cond
(paren
id|retval
)paren
(brace
id|printk
c_func
(paren
id|KERN_ERR
l_string|&quot;b1isa: attach controller failed.&bslash;n&quot;
)paren
suffix:semicolon
r_goto
id|err_free_irq
suffix:semicolon
)brace
id|printk
c_func
(paren
id|KERN_INFO
l_string|&quot;b1isa: AVM B1 ISA at i/o %#x, irq %d, revision %d&bslash;n&quot;
comma
id|card-&gt;port
comma
id|card-&gt;irq
comma
id|card-&gt;revision
)paren
suffix:semicolon
id|pci_set_drvdata
c_func
(paren
id|pdev
comma
id|cinfo
)paren
suffix:semicolon
r_return
l_int|0
suffix:semicolon
id|err_free_irq
suffix:colon
id|free_irq
c_func
(paren
id|card-&gt;irq
comma
id|card
)paren
suffix:semicolon
id|err_release_region
suffix:colon
id|release_region
c_func
(paren
id|card-&gt;port
comma
id|AVMB1_PORTLEN
)paren
suffix:semicolon
id|err_free
suffix:colon
id|b1_free_card
c_func
(paren
id|card
)paren
suffix:semicolon
id|err
suffix:colon
r_return
id|retval
suffix:semicolon
)brace
DECL|function|b1isa_procinfo
r_static
r_char
op_star
id|b1isa_procinfo
c_func
(paren
r_struct
id|capi_ctr
op_star
id|ctrl
)paren
(brace
id|avmctrl_info
op_star
id|cinfo
op_assign
(paren
id|avmctrl_info
op_star
)paren
(paren
id|ctrl-&gt;driverdata
)paren
suffix:semicolon
r_if
c_cond
(paren
op_logical_neg
id|cinfo
)paren
r_return
l_string|&quot;&quot;
suffix:semicolon
id|sprintf
c_func
(paren
id|cinfo-&gt;infobuf
comma
l_string|&quot;%s %s 0x%x %d r%d&quot;
comma
id|cinfo-&gt;cardname
(braket
l_int|0
)braket
ques
c_cond
id|cinfo-&gt;cardname
suffix:colon
l_string|&quot;-&quot;
comma
id|cinfo-&gt;version
(braket
id|VER_DRIVER
)braket
ques
c_cond
id|cinfo-&gt;version
(braket
id|VER_DRIVER
)braket
suffix:colon
l_string|&quot;-&quot;
comma
id|cinfo-&gt;card
ques
c_cond
id|cinfo-&gt;card-&gt;port
suffix:colon
l_int|0x0
comma
id|cinfo-&gt;card
ques
c_cond
id|cinfo-&gt;card-&gt;irq
suffix:colon
l_int|0
comma
id|cinfo-&gt;card
ques
c_cond
id|cinfo-&gt;card-&gt;revision
suffix:colon
l_int|0
)paren
suffix:semicolon
r_return
id|cinfo-&gt;infobuf
suffix:semicolon
)brace
multiline_comment|/* ------------------------------------------------------------- */
DECL|macro|MAX_CARDS
mdefine_line|#define MAX_CARDS 4
DECL|variable|isa_dev
r_static
r_struct
id|pci_dev
id|isa_dev
(braket
id|MAX_CARDS
)braket
suffix:semicolon
DECL|variable|io
r_static
r_int
id|io
(braket
id|MAX_CARDS
)braket
suffix:semicolon
DECL|variable|irq
r_static
r_int
id|irq
(braket
id|MAX_CARDS
)braket
suffix:semicolon
id|MODULE_PARM
c_func
(paren
id|io
comma
l_string|&quot;1-&quot;
id|__MODULE_STRING
c_func
(paren
id|MAX_CARDS
)paren
l_string|&quot;i&quot;
)paren
suffix:semicolon
id|MODULE_PARM
c_func
(paren
id|irq
comma
l_string|&quot;1-&quot;
id|__MODULE_STRING
c_func
(paren
id|MAX_CARDS
)paren
l_string|&quot;i&quot;
)paren
suffix:semicolon
id|MODULE_PARM_DESC
c_func
(paren
id|io
comma
l_string|&quot;I/O base address(es)&quot;
)paren
suffix:semicolon
id|MODULE_PARM_DESC
c_func
(paren
id|irq
comma
l_string|&quot;IRQ number(s) (assigned)&quot;
)paren
suffix:semicolon
DECL|function|b1isa_add_card
r_static
r_int
id|b1isa_add_card
c_func
(paren
r_struct
id|capi_driver
op_star
id|driver
comma
id|capicardparams
op_star
id|data
)paren
(brace
r_int
id|i
suffix:semicolon
r_for
c_loop
(paren
id|i
op_assign
l_int|0
suffix:semicolon
id|i
OL
id|MAX_CARDS
suffix:semicolon
id|i
op_increment
)paren
(brace
r_if
c_cond
(paren
id|isa_dev
(braket
id|i
)braket
dot
id|resource
(braket
l_int|0
)braket
dot
id|start
)paren
r_continue
suffix:semicolon
id|isa_dev
(braket
id|i
)braket
dot
id|resource
(braket
l_int|0
)braket
dot
id|start
op_assign
id|data-&gt;port
suffix:semicolon
id|isa_dev
(braket
id|i
)braket
dot
id|irq
op_assign
id|data-&gt;irq
suffix:semicolon
r_if
c_cond
(paren
id|b1isa_probe
c_func
(paren
op_amp
id|isa_dev
(braket
id|i
)braket
)paren
op_eq
l_int|0
)paren
r_return
l_int|0
suffix:semicolon
)brace
r_return
op_minus
id|ENODEV
suffix:semicolon
)brace
DECL|variable|capi_driver_b1isa
r_static
r_struct
id|capi_driver
id|capi_driver_b1isa
op_assign
(brace
dot
id|name
op_assign
l_string|&quot;b1isa&quot;
comma
dot
id|revision
op_assign
l_string|&quot;1.0&quot;
comma
dot
id|add_card
op_assign
id|b1isa_add_card
comma
)brace
suffix:semicolon
DECL|function|b1isa_init
r_static
r_int
id|__init
id|b1isa_init
c_func
(paren
r_void
)paren
(brace
r_char
op_star
id|p
suffix:semicolon
r_char
id|rev
(braket
l_int|32
)braket
suffix:semicolon
r_int
id|i
suffix:semicolon
r_if
c_cond
(paren
(paren
id|p
op_assign
id|strchr
c_func
(paren
id|revision
comma
l_char|&squot;:&squot;
)paren
)paren
op_ne
l_int|0
op_logical_and
id|p
(braket
l_int|1
)braket
)paren
(brace
id|strlcpy
c_func
(paren
id|rev
comma
id|p
op_plus
l_int|2
comma
l_int|32
)paren
suffix:semicolon
r_if
c_cond
(paren
(paren
id|p
op_assign
id|strchr
c_func
(paren
id|rev
comma
l_char|&squot;$&squot;
)paren
)paren
op_ne
l_int|0
op_logical_and
id|p
OG
id|rev
)paren
op_star
(paren
id|p
op_minus
l_int|1
)paren
op_assign
l_int|0
suffix:semicolon
)brace
r_else
id|strcpy
c_func
(paren
id|rev
comma
l_string|&quot;1.0&quot;
)paren
suffix:semicolon
r_for
c_loop
(paren
id|i
op_assign
l_int|0
suffix:semicolon
id|i
OL
id|MAX_CARDS
suffix:semicolon
id|i
op_increment
)paren
(brace
r_if
c_cond
(paren
op_logical_neg
id|io
(braket
id|i
)braket
)paren
r_break
suffix:semicolon
id|isa_dev
(braket
id|i
)braket
dot
id|resource
(braket
l_int|0
)braket
dot
id|start
op_assign
id|io
(braket
id|i
)braket
suffix:semicolon
id|isa_dev
(braket
id|i
)braket
dot
id|irq
op_assign
id|irq
(braket
id|i
)braket
suffix:semicolon
r_if
c_cond
(paren
id|b1isa_probe
c_func
(paren
op_amp
id|isa_dev
(braket
id|i
)braket
)paren
op_ne
l_int|0
)paren
r_return
op_minus
id|ENODEV
suffix:semicolon
)brace
id|strlcpy
c_func
(paren
id|capi_driver_b1isa.revision
comma
id|rev
comma
l_int|32
)paren
suffix:semicolon
id|register_capi_driver
c_func
(paren
op_amp
id|capi_driver_b1isa
)paren
suffix:semicolon
id|printk
c_func
(paren
id|KERN_INFO
l_string|&quot;b1isa: revision %s&bslash;n&quot;
comma
id|rev
)paren
suffix:semicolon
r_return
l_int|0
suffix:semicolon
)brace
DECL|function|b1isa_exit
r_static
r_void
id|__exit
id|b1isa_exit
c_func
(paren
r_void
)paren
(brace
r_int
id|i
suffix:semicolon
r_for
c_loop
(paren
id|i
op_assign
l_int|0
suffix:semicolon
id|i
OL
id|MAX_CARDS
suffix:semicolon
id|i
op_increment
)paren
(brace
r_if
c_cond
(paren
op_logical_neg
id|io
(braket
id|i
)braket
)paren
r_break
suffix:semicolon
id|b1isa_remove
c_func
(paren
op_amp
id|isa_dev
(braket
id|i
)braket
)paren
suffix:semicolon
)brace
id|unregister_capi_driver
c_func
(paren
op_amp
id|capi_driver_b1isa
)paren
suffix:semicolon
)brace
DECL|variable|b1isa_init
id|module_init
c_func
(paren
id|b1isa_init
)paren
suffix:semicolon
DECL|variable|b1isa_exit
id|module_exit
c_func
(paren
id|b1isa_exit
)paren
suffix:semicolon
eof
