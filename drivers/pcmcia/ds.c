...
suffix:semicolon
DECL|function|pcmcia_release_bus_socket
r_static
r_void
id|pcmcia_release_bus_socket
c_func
(paren
r_struct
id|kref
...
id|EINVAL
suffix:semicolon
multiline_comment|/* initialize common fields */
id|driver-&gt;drv.bus
op_assign
op_amp
id|pcmcia_bus_type
...
l_int|1
comma
l_string|&quot;releasing dev %p&bslash;n&quot;
comma
id|p_dev
)paren
suffix:semicolon
...
DECL|function|pcmcia_device_probe
r_static
r_int
id|pcmcia_device_probe
c_func
...
DECL|function|pcmcia_device_remove
r_static
r_int
id|pcmcia_device_remove
c_func
...
id|p_drv
suffix:semicolon
multiline_comment|/* detach the &quot;instance&quot; */
id|p_dev
op_assign
id|to_pcmcia_dev
c_func
...
suffix:semicolon
multiline_comment|/* from pcmcia_probe_device */
id|put_device
c_func
(paren
op_amp
...
multiline_comment|/*&n; * pcmcia_device_query -- determine information about a pcmcia device&n; */
DECL|function|pcmcia_device_query
r_static
r_int
id|pcmcia_device_query
c_func
(paren
r_struct
id|pcmcia_device
...
id|manf_id.card
suffix:semicolon
id|p_dev-&gt;has_manf_id
op_assign
l_int|1
suffix:semicolon
id|p_dev-&gt;has_card_id
op_assign
l_int|1
suffix:semicolon
)brace
...
r_else
(brace
multiline_comment|/* rule of thumb: cards with no FUNCID, but with&n;&t;&t; * common memory device geometry information, are&n;&t;&t; * probably memory cards (from pcmcia-cs) */
...
l_int|0
comma
l_string|&quot;mem device geometry probably means &quot;
l_string|&quot;FUNCID_MEMORY&bslash;n&quot;
)paren
suffix:semicolon
id|p_dev-&gt;func_id
op_assign
...
multiline_comment|/* device_add_lock is needed to avoid double registration by cardmgr and kernel.&n; * Serializes pcmcia_device_add; will most likely be removed in future.&n; *&n; * While it has the caveat that adding new PCMCIA devices inside(!) device_register()&n; * won&squot;t work, this doesn&squot;t matter much at the moment: the driver core doesn&squot;t&n; * support it either.&n; */
r_static
id|DECLARE_MUTEX
c_func
(paren
...
suffix:semicolon
DECL|function|pcmcia_device_add
r_static
r_struct
id|pcmcia_device
op_star
id|pcmcia_device_add
c_func
(paren
r_struct
id|pcmcia_bus_socket
...
id|p_dev-&gt;client.client_magic
op_assign
id|CLIENT_MAGIC
suffix:semicolon
id|p_dev-&gt;client.Socket
op_assign
id|s-&gt;parent
suffix:semicolon
id|p_dev-&gt;client.Function
op_assign
id|function
suffix:semicolon
id|p_dev-&gt;client.state
...
id|CLIENT_UNBOUND
suffix:semicolon
multiline_comment|/* Add to the list in pcmcia_bus_socket */
...
id|EAGAIN
suffix:semicolon
multiline_comment|/* try again, but later... */
...
l_int|0
comma
l_string|&quot;invalid CIS or invalid resources&bslash;n&quot;
)paren
suffix:semicolon
r_return
op_minus
...
l_int|1
suffix:semicolon
multiline_comment|/* this doesn&squot;t handle multifunction devices on one pcmcia function&n;&t; * yet. */
r_for
c_loop
(paren
id|i
...
DECL|function|pcmcia_bus_match
r_static
r_int
id|pcmcia_bus_match
c_func
...
suffix:semicolon
multiline_comment|/* matching by cardmgr */
r_if
c_cond
(paren
id|p_dev-&gt;cardmgr
...
multiline_comment|/************************ per-device sysfs output ***************************/
DECL|macro|pcmcia_device_attr
mdefine_line|#define pcmcia_device_attr(field, test, format)&t;&t;&t;&t;&bslash;&n;static ssize_t field##_show (struct device *dev, char *buf)&t;&t;&bslash;&n;{&t;&t;&t;&t;&t;&t;&t;&t;&t;&bslash;&n;&t;struct pcmcia_device *p_dev = to_pcmcia_dev(dev);&t;&t;&bslash;&n;&t;return p_dev-&gt;test ? sprintf (buf, format, p_dev-&gt;field) : -ENODEV; &bslash;&n;}
DECL|macro|pcmcia_device_stringattr
mdefine_line|#define pcmcia_device_stringattr(name, field)&t;&t;&t;&t;&t;&bslash;&n;static ssize_t name##_show (struct device *dev, char *buf)&t;&t;&bslash;&n;{&t;&t;&t;&t;&t;&t;&t;&t;&t;&bslash;&n;&t;struct pcmcia_device *p_dev = to_pcmcia_dev(dev);&t;&t;&bslash;&n;&t;return p_dev-&gt;field ? sprintf (buf, &quot;%s&bslash;n&quot;, p_dev-&gt;field) : -ENODEV; &bslash;&n;}
id|pcmcia_device_attr
c_func
(paren
id|func
comma
...
suffix:semicolon
DECL|variable|pcmcia_dev_attrs
r_static
r_struct
id|device_attribute
id|pcmcia_dev_attrs
...
suffix:semicolon
DECL|function|send_event_callback
r_static
r_int
id|send_event_callback
c_func
(paren
r_struct
id|device
...
id|_data
suffix:semicolon
multiline_comment|/* we get called for all sockets, but may only pass the event&n;&t; * for drivers _on the affected socket_ */
r_if
c_cond
(paren
id|p_dev-&gt;socket
...
multiline_comment|/*======================================================================&n;&n;    bind_request() and bind_device() are merged by now. Register_client()&n;    is called right at the end of bind_request(), during the driver&squot;s&n;    -&gt;attach() call. Individual descriptions:&n;&n;    bind_request() connects a socket to a particular client driver.&n;    It looks up the specified device ID in the list of registered&n;    drivers, binds it to the socket, and tries to create an instance&n;    of the device.  unbind_request() deletes a driver instance.&n;    &n;    Bind_device() associates a device driver with a particular socket.&n;    It is normally called by Driver Services after it has identified&n;    a newly inserted card.  An instance of that driver will then be&n;    eligible to register as a client of this socket.&n;&n;    Register_client() uses the dev_info_t handle to match the&n;    caller with a socket.  The driver must have already been bound&n;    to a socket with bind_device() -- in fact, bind_device()&n;    allocates the client structure that will be used.&n;&n;======================================================================*/
...
(brace
multiline_comment|/* if there&squot;s already a device&n;&t;&t;&t;&t;&t; * registered, and it was registered&n;&t;&t;&t;&t;&t; * by userspace before, we need to&n;&t;&t;&t;&t;&t; * return the &quot;instance&quot;. */
id|spin_unlock_irqrestore
c_func
(paren
op_amp
...
r_else
(brace
multiline_comment|/* the correct driver managed to bind&n;&t;&t;&t;&t;&t; * itself magically to the correct&n;&t;&t;&t;&t;&t; * device. */
id|spin_unlock_irqrestore
c_func
(paren
op_amp
...
(brace
multiline_comment|/* there&squot;s already a device available where&n;&t;&t;&t;&t; * no device has been bound to yet. So we don&squot;t&n;&t;&t;&t;&t; * need to register a device! */
id|spin_unlock_irqrestore
c_func
(paren
op_amp
...
suffix:semicolon
multiline_comment|/*&n;&t; * Prevent this racing with a card insertion.&n;&t; */
id|down
c_func
(paren
op_amp
...
suffix:semicolon
multiline_comment|/* check whether the driver indeed matched. I don&squot;t care if this&n;&t; * is racy or not, because it can only happen on cardmgr access&n;&t; * paths...&n;&t; */
r_if
c_cond
(paren
op_logical_neg
...
suffix:semicolon
multiline_comment|/* safe, as we already hold a reference from bind_device */
op_star
id|handle
op_assign
id|client
...
multiline_comment|/*&n;&t; * Some unbelievably ugly code to associate the PCI cardbus&n;&t; * device and its driver with the PCMCIA &quot;bind&quot; information.&n;&t; */
(brace
r_struct
id|pci_bus
op_star
...
multiline_comment|/*====================================================================*/
multiline_comment|/* unbind _all_ devices attached to a given pcmcia_bus_socket. The&n; * drivers have been called with EVENT_CARD_REMOVAL before.&n; */
...
l_int|2
comma
l_string|&quot;unbind_request(%d)&bslash;n&quot;
comma
id|s-&gt;parent-&gt;sock
)paren
suffix:semicolon
...
(brace
multiline_comment|/* unregister all pcmcia_devices registered with this socket*/
id|spin_lock_irqsave
c_func
(paren
op_amp
...
(paren
id|KERN_WARNING
l_string|&quot;2.6. kernels use pcmciamtd instead of memory_cs.c and do not require special&bslash;n&quot;
)paren
suffix:semicolon
id|printk
c_func
(paren
id|KERN_WARNING
l_string|&quot;MTD handling any more.&bslash;n&quot;
)paren
suffix:semicolon
id|printed
op_increment
