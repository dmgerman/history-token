...
multiline_comment|/* sa1100_pcmcia_inquire_socket()&n; * ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^&n; * Implements the inquire_socket() operation for the in-kernel PCMCIA&n; * service (formerly SS_InquireSocket in Card Services).  We set&n; * SS_CAP_STATIC_MAP, which disables the memory resource database&n; * check. (Mapped memory is set up within the socket driver itself.)&n; *&n; * In conjunction with the STATIC_MAP capability is a new field,&n; * `io_offset&squot;, recommended by David Hinds. Rather than go through&n; * the SetIOMap interface (which is not quite suited for communicating&n; * window locations up from the socket driver), we just pass up&n; * an offset which is applied to client-requested base I/O addresses&n; * in alloc_io_space().&n; *&n; * SS_CAP_STATIC_MAP: don&squot;t bother with the (user-configured) memory&n; *   resource database; we instead pass up physical address ranges&n; *   and allow other parts of Card Services to deal with remapping.&n; *&n; * SS_CAP_PCCARD: we can deal with 16-bit PCMCIA &amp; CF cards, but&n; *   not 32-bit CardBus devices.&n; *&n; * Return value is irrelevant; the pcmcia subsystem ignores it.&n; */
r_static
r_int
