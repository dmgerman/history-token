...
multiline_comment|/*&n; * Call the shrink functions to age shrinkable caches&n; *&n; * Here we assume it costs one seek to replace a lru page and that it also&n; * takes a seek to recreate a cache object.  With this in mind we age equal&n; * percentages of the lru and ageable caches.  This should balance the seeks&n; * generated by these structures.&n; *&n; * If the vm encounted mapped pages on the LRU it increase the pressure on&n; * slab to avoid swapping.&n; *&n; * FIXME: do not do for zone highmem&n; *&n; * We do weird things to avoid (scanned*seeks*entries) overflowing 32 bits.&n; */
