...
multiline_comment|/*&n; * FIXME: don&squot;t do this for ZONE_HIGHMEM&n; */
multiline_comment|/*&n; * Here we assume it costs one seek to replace a lru page and that it also&n; * takes a seek to recreate a cache object.  With this in mind we age equal&n; * percentages of the lru and ageable caches.  This should balance the seeks&n; * generated by these structures.&n; *&n; * NOTE: for now I do this for all zones.  If we find this is too aggressive&n; * on large boxes we may want to exclude ZONE_HIGHMEM.&n; *&n; * If we&squot;re encountering mapped pages on the LRU then increase the pressure on&n; * slab to avoid swapping.&n; */
...
multiline_comment|/*&n; * kswapd will work across all this node&squot;s zones until they are all at&n; * pages_high.&n; */
...
r_break
suffix:semicolon
multiline_comment|/* All zones are at pages_high */
id|blk_congestion_wait
c_func
(paren
id|WRITE
