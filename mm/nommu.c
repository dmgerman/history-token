...
multiline_comment|/*&n; * determine whether a mapping should be permitted and, if so, what sort of&n; * mapping we&squot;re capable of supporting&n; */
DECL|function|validate_mmap_request
r_static
r_int
...
id|capabilities
op_assign
id|mapping-&gt;backing_dev_info-&gt;capabilities
suffix:semicolon
r_if
c_cond
(paren
...
(brace
multiline_comment|/* no explicit capabilities set, so assume some&n;&t;&t;&t; * defaults */
r_switch
c_cond
(paren
...
multiline_comment|/* eliminate any capabilities that we can&squot;t support on this&n;&t;&t; * device */
r_if
c_cond
(paren
op_logical_neg
...
multiline_comment|/* we mustn&squot;t privatise shared mappings */
id|capabilities
op_and_assign
op_complement
id|BDI_CAP_MAP_COPY
...
r_else
(brace
multiline_comment|/* we&squot;re going to read the file into private memory we&n;&t;&t;&t; * allocate */
r_if
c_cond
(paren
op_logical_neg
...
id|ENODEV
suffix:semicolon
multiline_comment|/* we don&squot;t permit a private writable mapping to be&n;&t;&t;&t; * shared with the backing device */
r_if
c_cond
(paren
id|prot
...
multiline_comment|/* handle executable mappings and implied executable&n;&t;&t; * mappings */
r_if
c_cond
(paren
...
(brace
multiline_comment|/* handle implication of PROT_EXEC by PROT_READ */
r_if
c_cond
(paren
...
(brace
multiline_comment|/* backing file is not executable, try to copy */
id|capabilities
op_and_assign
op_complement
id|BDI_CAP_MAP_DIRECT
...
r_else
(brace
multiline_comment|/* anonymous mappings are always memory backed and can be&n;&t;&t; * privately mapped&n;&t;&t; */
id|capabilities
op_assign
id|BDI_CAP_MAP_COPY
suffix:semicolon
...
multiline_comment|/*&n; * we&squot;ve determined that we can make the mapping, now translate what we&n; * now know into VMA flags&n; */
DECL|function|determine_vm_flags
r_static
r_int
r_int
id|determine_vm_flags
c_func
(paren
r_struct
id|file
...
id|VM_MAYEXEC
suffix:semicolon
multiline_comment|/* vm_flags |= mm-&gt;def_flags; */
r_if
c_cond
(paren
op_logical_neg
...
(brace
multiline_comment|/* attempt to share read-only copies of mapped file chunks */
r_if
c_cond
(paren
id|file
...
r_else
(brace
multiline_comment|/* overlay a shareable mapping on the backing device or inode&n;&t;&t; * if possible - used for chardevs, ramfs/tmpfs/shmfs and&n;&t;&t; * romfs/cramfs */
r_if
c_cond
(paren
id|flags
...
multiline_comment|/* refuse to let anyone share private mappings with this process if&n;&t; * it&squot;s being traced - otherwise breakpoints set in it may interfere&n;&t; * with another untraced process&n;&t; */
r_if
c_cond
(paren
(paren
...
multiline_comment|/*&n; * set up a shared mapping on a file&n; */
DECL|function|do_mmap_shared_file
r_static
r_int
id|do_mmap_shared_file
c_func
(paren
r_struct
id|vm_area_struct
...
id|ret
suffix:semicolon
multiline_comment|/* getting an ENOSYS error indicates that direct mmap isn&squot;t&n;&t; * possible (as opposed to tried but failed) so we&squot;ll fall&n;&t; * through to making a private copy of the data and mapping&n;&t; * that if we can */
r_return
op_minus
id|ENODEV
suffix:semicolon
)brace
multiline_comment|/*&n; * set up a private mapping or an anonymous shared mapping&n; */
DECL|function|do_mmap_private
r_static
r_int
id|do_mmap_private
c_func
...
id|ret
suffix:semicolon
multiline_comment|/* invoke the file&squot;s mapping function so that it can keep track of&n;&t; * shared mappings on devices or memory&n;&t; * - VM_MAYSHARE will be set if it may attempt to share&n;&t; */
r_if
c_cond
(paren
id|vma-&gt;vm_file
...
(brace
multiline_comment|/* shouldn&squot;t return success if we&squot;re not sharing */
id|BUG_ON
c_func
(paren
id|ret
...
id|ret
suffix:semicolon
multiline_comment|/* success or a real error */
)brace
multiline_comment|/* getting an ENOSYS error indicates that direct mmap isn&squot;t&n;&t;&t; * possible (as opposed to tried but failed) so we&squot;ll try to&n;&t;&t; * make a private copy of the data and map that instead */
)brace
...
(brace
multiline_comment|/* read the contents of a file into the copy */
id|mm_segment_t
id|old_fs
suffix:semicolon
id|loff_t
...
id|error_free
suffix:semicolon
multiline_comment|/* clear the last little bit */
r_if
c_cond
(paren
id|ret
...
r_else
(brace
multiline_comment|/* if it&squot;s an anonymous mapping, then just clear it */
id|memset
c_func
(paren
id|base
...
id|ret
suffix:semicolon
multiline_comment|/* decide whether we should attempt the mapping, and if so what sort of&n;&t; * mapping */
id|ret
op_assign
id|validate_mmap_request
c_func
...
id|ret
suffix:semicolon
multiline_comment|/* we&squot;ve determined that we can make the mapping, now translate what we&n;&t; * now know into VMA flags */
id|vm_flags
op_assign
id|determine_vm_flags
c_func
...
r_continue
suffix:semicolon
multiline_comment|/* handle inexactly overlapping matches between mappings */
r_if
c_cond
(paren
id|vma-&gt;vm_pgoff
...
l_int|NULL
suffix:semicolon
multiline_comment|/* obtain the address at which to make a shared mapping&n;&t;&t; * - this is the hook for quasi-memory character devices to&n;&t;&t; *   tell us the location of a shared mapping&n;&t;&t; */
r_if
c_cond
(paren
id|file
...
id|error
suffix:semicolon
multiline_comment|/* the driver refused to tell us where to site&n;&t;&t;&t;&t; * the mapping so we&squot;ll have to attempt to copy&n;&t;&t;&t;&t; * it */
id|ret
op_assign
(paren
r_int
...
id|vma
suffix:semicolon
multiline_comment|/* set up the mapping */
r_if
c_cond
(paren
id|file
...
id|error
suffix:semicolon
multiline_comment|/* okay... we have a mapping; now we have to register it */
id|result
op_assign
(paren
r_void
