...
multiline_comment|/*&n; * shmem_recalc_inode - recalculate the size of an inode&n; *&n; * @inode: inode to recalc&n; *&n; * We have to calculate the free blocks since the mm can drop&n; * undirtied hole pages behind our back.  Later we should be&n; * able to use the releasepage method to handle this better.&n; *&n; * But normally   info-&gt;alloced == inode-&gt;i_mapping-&gt;nrpages + info-&gt;swapped&n; * So mm freed is info-&gt;alloced - (inode-&gt;i_mapping-&gt;nrpages + info-&gt;swapped)&n; *&n; * It has to be called with the spinlock held.&n; */
...
multiline_comment|/*&n; * shmem_swp_alloc - get the position of the swap entry for the page.&n; *                   If it does not exist allocate the entry.&n; *&n; * @info:&t;info structure for the inode&n; * @index:&t;index of the page to find&n; */
...
multiline_comment|/*&n; * shmem_truncate_direct - free the swap entries of a whole doubly&n; *                         indirect block&n; *&n; * @info:&t;the info structure of the inode&n; * @dir:&t;pointer to the pointer to the block&n; * @start:&t;offset to start from (in pages)&n; * @len:&t;how many pages are stored in this block&n; *&n; * Returns the number of freed swap entries.&n; */
r_static
r_inline
r_int
r_int
...
DECL|function|shmem_vm_writeback
r_static
r_int
id|shmem_vm_writeback
c_func
(paren
r_struct
id|page
...
id|EFBIG
suffix:semicolon
multiline_comment|/*&n;&t; * When writing, i_sem is held against truncation and other&n;&t; * writing, so next_index will remain as set here; but when&n;&t; * reading, idx must always be checked against next_index&n;&t; * after sleeping, lest truncation occurred meanwhile.&n;&t; */
id|spin_lock
c_func
(paren
op_amp
...
suffix:semicolon
multiline_comment|/*&n;&t; * Somehow, it feels wrong for truncation down to cause any&n;&t; * allocation: so instead of a blind shmem_getpage, check that&n;&t; * the page has actually been instantiated before holding it.&n;&t; */
id|spin_lock
c_func
(paren
op_amp
