...
multiline_comment|/*&n; * Readahead design.&n; *&n; * The fields in struct file_ra_state represent the most-recently-executed&n; * readahead attempt:&n; *&n; * start:&t;Page index at which we started the readahead&n; * size:&t;Number of pages in that read&n; *              Together, these form the &quot;current window&quot;.&n; *              Together, start and size represent the `readahead window&squot;.&n; * prev_page:   The page which the readahead algorithm most-recently inspected.&n; *              It is mainly used to detect sequential file reading.&n; *              If page_cache_readahead sees that it is again being called for&n; *              a page which it just looked at, it can return immediately without&n; *              making any state changes.&n; * ahead_start,&n; * ahead_size:  Together, these form the &quot;ahead window&quot;.&n; * ra_pages:&t;The externally controlled max readahead for this fd.&n; *&n; * When readahead is in the off state (size == 0), readahead is disabled.&n; * In this state, prev_page is used to detect the resumption of sequential I/O.&n; *&n; * The readahead code manages two windows - the &quot;current&quot; and the &quot;ahead&quot;&n; * windows.  The intent is that while the application is walking the pages&n; * in the current window, I/O is underway on the ahead window.  When the&n; * current window is fully traversed, it is replaced by the ahead window&n; * and the ahead window is invalidated.  When this copying happens, the&n; * new current window&squot;s pages are probably still locked.  So&n; * we submit a new batch of I/O immediately, creating a new ahead window.&n; *&n; * So:&n; *&n; *   ----|----------------|----------------|-----&n; *       ^start           ^start+size&n; *                        ^ahead_start     ^ahead_start+ahead_size&n; *&n; *         ^ When this page is read, we submit I/O for the&n; *           ahead window.&n; *&n; * A `readahead hit&squot; occurs when a read request is made against a page which is&n; * the next sequential page. Ahead window calculations are done only when it&n; * is time to submit a new IO.  The code ramps up the size agressively at first,&n; * but slow down as it approaches max_readhead.&n; *&n; * Any seek/ramdom IO will result in readahead being turned off.  It will resume&n; * at the first sequential access.&n; *&n; * There is a special-case: if the first page which the application tries to&n; * read happens to be the first page of the file, it is assumed that a linear&n; * read is about to happen and the window is immediately set to the initial size&n; * based on I/O request size and the max_readahead.&n; *&n; * This function is to be called for every read request, rather than when&n; * it is time to perform readahead.  It is called only once for the entire I/O&n; * regardless of size unless readahead is unable to start enough I/O to satisfy&n; * the request (I/O request &gt; max_readahead).&n; */
...
DECL|function|make_ahead_window
r_static
r_int
id|make_ahead_window
c_func
(paren
r_struct
id|address_space
...
(brace
multiline_comment|/* A read failure in blocking mode, implies pages are&n;&t;&t; * all cached. So we can safely assume we have taken&n;&t;&t; * care of all the pages requested in this call.&n;&t;&t; * A read failure in non-blocking mode, implies we are&n;&t;&t; * reading more pages than requested in this call.  So&n;&t;&t; * we safely assume we have taken care of all the pages&n;&t;&t; * requested in this call.&n;&t;&t; *&n;&t;&t; * Just reset the ahead window in case we failed due to&n;&t;&t; * congestion.  The ahead window will any way be closed&n;&t;&t; * in case we failed due to excessive page cache hits.&n;&t;&t; */
id|ra-&gt;ahead_start
op_assign
l_int|0
suffix:semicolon
...
id|sequential
suffix:semicolon
multiline_comment|/*&n;&t; * We avoid doing extra work and bogusly perturbing the readahead&n;&t; * window expansion logic.&n;&t; */
r_if
c_cond
(paren
id|offset
...
id|offset
suffix:semicolon
multiline_comment|/* Note that prev_page == -1 if it is a first read */
id|sequential
op_assign
(paren
id|offset
...
suffix:semicolon
multiline_comment|/* No readahead or sub-page sized read or file already in cache */
r_if
c_cond
(paren
id|newsize
...
l_int|1
suffix:semicolon
multiline_comment|/*&n;&t; * Special case - first read at start of file. We&squot;ll assume it&squot;s&n;&t; * a whole-file read and grow the window fast.  Or detect first&n;&t; * sequential access&n;&t; */
r_if
c_cond
(paren
id|sequential
