...
multiline_comment|/*&n; * Always set the resulting pte to be nowrite (the same as COW pages&n; * after one process has exited).  We don&squot;t know just how many PTEs will&n; * share this swap entry, so be cautious and let do_wp_page work out&n; * what to do if a write is requested later.&n; *&n; * vma-&gt;vm_mm-&gt;page_table_lock is held.&n; */
...
suffix:semicolon
multiline_comment|/*&n;&t; * Move the page to the active list so it is not&n;&t; * immediately swapped out again after swapon.&n;&t; */
id|activate_page
c_func
(paren
id|page
...
suffix:semicolon
multiline_comment|/*&n;&t; * Currently unuse_mm cannot fail, but leave error handling&n;&t; * at call sites for now, since we change it from time to time.&n;&t; */
r_return
l_int|0
suffix:semicolon
)brace
