...
id|PAGE_SHIFT
suffix:semicolon
multiline_comment|/*&n;&t; * Setting and clearing PG_anon must always happen inside&n;&t; * page_map_lock to avoid races between mapping and&n;&t; * unmapping on different processes of the same&n;&t; * shared cow swapcache page. And while we take the&n;&t; * page_map_lock PG_anon cannot change from under us.&n;&t; * Actually PG_anon cannot change under fork either&n;&t; * since fork holds a reference on the page so it cannot&n;&t; * be unmapped under fork and in turn copy_page_range is&n;&t; * allowed to read PG_anon outside the page_map_lock.&n;&t; */
id|page_map_lock
c_func
(paren
id|page
