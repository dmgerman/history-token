...
multiline_comment|/*&n; * Locking:&n; * - the page-&gt;pte_chain is protected by the PG_chainlock bit,&n; *   which nests within the pagemap_lru_lock, then the&n; *   mm-&gt;page_table_lock, and then the page lock.&n; * - because swapout locking is opposite to the locking order&n; *   in the page fault path, the swapout path uses trylocks&n; *   on the mm-&gt;page_table_lock&n; */
