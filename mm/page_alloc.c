...
suffix:semicolon
multiline_comment|/*&n;&t;&t; * We let real-time tasks dip their real-time paws a little&n;&t;&t; * deeper into reserves.&n;&t;&t; */
r_if
c_cond
(paren
id|rt_task
...
multiline_comment|/*&n; * Page buddy system uses &quot;index &gt;&gt; (i+1)&quot;, where &quot;index&quot; is&n; * at most &quot;size-1&quot;.&n; *&n; * The extra &quot;+3&quot; is to round down to byte size (8 bits per byte&n; * assumption). Thus we get &quot;(size-1) &gt;&gt; (i+4)&quot; as the last byte&n; * we can access.&n; *&n; * The &quot;+1&quot; is because we want to round the byte allocation up&n; * rather than down. So we should have had a &quot;+7&quot; before we shifted&n; * down by three. Also, we have to add one as we actually _use_ the&n; * last bit (it&squot;s [0,n] inclusive, not [0,n[).&n; *&n; * So we actually had +7+1 before we shift down by 3. But&n; * (n+8) &gt;&gt; 3 == (n &gt;&gt; 3) + 1 (modulo overflows, which we do not have).&n; *&n; * Finally, we LONG_ALIGN because all bitmap operations are on longs.&n; */
DECL|function|pages_to_bitmap_size
r_int
r_int
id|pages_to_bitmap_size
c_func
(paren
r_int
r_int
...
id|pages
suffix:semicolon
multiline_comment|/* there is no higher zone to get a contribution from */
r_if
c_cond
(paren
id|z_idx
...
suffix:semicolon
multiline_comment|/* We always start with the higher zone&squot;s protection value */
id|pages
op_assign
id|higherzone-&gt;protection
(braket
id|alloc_type
)braket
suffix:semicolon
multiline_comment|/*&n;&t; * We get a lower-zone-protection contribution only if there are&n;&t; * pages in the higher zone and if we&squot;re not the highest zone&n;&t; * in the current zonelist.  e.g., never happens for GFP_DMA. Happens&n;&t; * only for ZONE_DMA in a GFP_KERNEL allocation and happens for ZONE_DMA&n;&t; * and ZONE_NORMAL for a GFP_HIGHMEM allocation.&n;&t; */
r_if
c_cond
(paren
id|higherzone-&gt;present_pages
op_logical_and
id|z_idx
OL
id|alloc_type
...
multiline_comment|/*&n; * setup_per_zone_protection - called whenver min_free_kbytes or&n; *&t;sysctl_lower_zone_protection changes.  Ensures that each zone&n; *&t;has a correct pages_protected value, so an adequate number of&n; *&t;pages are left in the zone after a successful __alloc_pages().&n; *&n; *&t;This algorithm is way confusing.  I tries to keep the same behavior&n; *&t;as we had with the incremental min iterative algorithm.&n; */
DECL|function|setup_per_zone_protection
r_static
r_void
...
id|i
suffix:semicolon
multiline_comment|/*&n;&t;&t; * For each of the different allocation types:&n;&t;&t; * GFP_DMA -&gt; GFP_KERNEL -&gt; GFP_HIGHMEM&n;&t;&t; */
r_for
c_loop
(paren
id|i
...
(brace
multiline_comment|/*&n;&t;&t;&t; * For each of the zones:&n;&t;&t;&t; * ZONE_HIGHMEM -&gt; ZONE_NORMAL -&gt; ZONE_DMA&n;&t;&t;&t; */
r_for
c_loop
(paren
id|j
...
suffix:semicolon
multiline_comment|/*&n;&t;&t;&t;&t; * We never protect zones that don&squot;t have memory&n;&t;&t;&t;&t; * in them (j&gt;max_zone) or zones that aren&squot;t in&n;&t;&t;&t;&t; * the zonelists for a certain type of&n;&t;&t;&t;&t; * allocation (j&gt;i).  We have to assign these to&n;&t;&t;&t;&t; * zero because the lower zones take&n;&t;&t;&t;&t; * contributions from the higher zones.&n;&t;&t;&t;&t; */
r_if
c_cond
(paren
id|j
...
multiline_comment|/*&n;&t;&t;&t;&t; * The contribution of the next higher zone&n;&t;&t;&t;&t; */
id|zone-&gt;protection
(braket
id|i
)braket
...
multiline_comment|/*&n; * lower_zone_protection_sysctl_handler - just a wrapper around&n; *&t;proc_dointvec() so that we can call setup_per_zone_protection()&n; *&t;whenever sysctl_lower_zone_protection changes.&n; */
DECL|function|lower_zone_protection_sysctl_handler
r_int
...
multiline_comment|/*&n; * allocate a large system hash table from bootmem&n; * - it is assumed that the hash table must contain an exact power-of-2&n; *   quantity of entries&n; */
