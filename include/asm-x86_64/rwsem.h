multiline_comment|/* rwsem.h: R/W semaphores implemented using XADD/CMPXCHG for x86_64+&n; *&n; * Written by David Howells (dhowells@redhat.com).&n; * Ported by Andi Kleen &lt;ak@suse.de&gt; to x86-64.&n; *&n; * Derived from asm-i386/semaphore.h and asm-i386/rwsem.h&n; *&n; *&n; * The MSW of the count is the negated number of active writers and waiting&n; * lockers, and the LSW is the total number of active locks&n; *&n; * The lock count is initialized to 0 (no active and no waiting lockers).&n; *&n; * When a writer subtracts WRITE_BIAS, it&squot;ll get 0xffff0001 for the case of an&n; * uncontended lock. This can be determined because XADD returns the old value.&n; * Readers increment by 1 and see a positive value when uncontended, negative&n; * if there are writers (and maybe) readers waiting (in which case it goes to&n; * sleep).&n; *&n; * The value of WAITING_BIAS supports up to 32766 waiting processes. This can&n; * be extended to 65534 by manually checking the whole MSW rather than relying&n; * on the S flag.&n; *&n; * The value of ACTIVE_BIAS supports up to 65535 active processes.&n; *&n; * This should be totally fair - if anything is waiting, a process that wants a&n; * lock will go to the back of the queue. When the currently active lock is&n; * released, if there&squot;s a writer at the front of the queue, then that and only&n; * that will be woken up; if there&squot;s a bunch of consecutive readers at the&n; * front, then they&squot;ll all be woken up, but no other readers will be.&n; */
...
l_string|&quot;  movl     %[bias],%[tmp]&bslash;n&bslash;t&quot;
id|LOCK_PREFIX
l_string|&quot;  xaddl     %[tmp],(%%rdi)&bslash;n&bslash;t&quot;
...
id|LOCK_PREFIX
l_string|&quot;  addl      %[bias],(%%rdi)&bslash;n&bslash;t&quot;
...
l_string|&quot;2:&bslash;n&bslash;t&quot;
l_string|&quot;  call&t;     rwsem_downgrade_thunk&bslash;n&quot;
l_string|&quot;  jmp       1b&bslash;n&quot;
id|LOCK_SECTION_END
