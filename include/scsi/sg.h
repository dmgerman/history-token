...
multiline_comment|/*&n;   History:&n;    Started: Aug 9 by Lawrence Foard (entropy@world.std.com), to allow user&n;     process control of SCSI devices.&n;    Development Sponsored by Killy Corp. NY NY&n;Original driver (sg.h):&n;*       Copyright (C) 1992 Lawrence Foard&n;Version 2 and 3 extensions to driver:&n;*       Copyright (C) 1998 - 2001 Douglas Gilbert&n;&n;    Version: 3.1.20 (20010814)&n;    This version is for 2.4 series kernels.&n;&n;    Changes since 3.1.19 (20010623)&n;&t;- add SG_GET_ACCESS_COUNT ioctl &n;&t;- make open() increment and close() decrement access_count&n;&t;- only register first 256 devices, reject subsequent devices&n;    Changes since 3.1.18 (20010505)&n;&t;- fix bug that caused long wait when large buffer requested&n;&t;- fix leak in error case of sg_new_read() [report: Eric Barton]&n;&t;- add &squot;online&squot; column to /proc/scsi/sg/devices&n;    Changes since 3.1.17 (20000921)&n;    &t;- add CAP_SYS_RAWIO capability for sensitive stuff&n;    &t;- compile in dio stuff, procfs &squot;allow_dio&squot; defaulted off (0)&n;&t;- make premature close and detach more robust&n;&t;- lun masked into commands &lt;= SCSI_2&n;&t;- poll() and async notification now yield POLL_HUP on detach&n;&t;- various 3rd party tweaks tracking lk 2.4 internal changes&n;&n;Map of SG verions to the Linux kernels in which they appear:&n;       ----------        ----------------------------------&n;       original          all kernels &lt; 2.2.6&n;       2.1.38            2.2.16&n;       2.1.39            2.2.17 - 2.2.19&n;       3.0.x             optional version 3 sg driver for 2.2 series&n;       3.1.17            2.4.0 ++&n;&n;Major new features in SG 3.x driver (cf SG 2.x drivers)&n;&t;- SG_IO ioctl() combines function if write() and read()&n;&t;- new interface (sg_io_hdr_t) but still supports old interface&n;&t;- scatter/gather in user space and direct IO supported&n;&n; The term &quot;indirect IO&quot; refers a method by which data is DMAed into kernel&n; buffers from the hardware and afterwards is transferred into the user&n; space (or vice versa if you are writing). Transfer speeds of up to 20 to&n; 30MBytes/sec have been measured using indirect IO. For faster throughputs&n; &quot;direct IO&quot; which cuts out the double handling of data is required.&n; Direct IO is supported by the SG 3.x drivers on 2.4 series Linux kernels&n; and requires the use of the new interface.&n;&n; Requests for direct IO with the new interface will automatically fall back&n; to indirect IO mode if they cannot be fulfilled. An example of such a case&n; is an ISA SCSI adapter which is only capable of DMAing to the lower 16MB of&n; memory due to the architecture of ISA. The &squot;info&squot; field in the new&n; interface indicates whether a direct or indirect data transfer took place.&n;&n; Obtaining memory for the kernel buffers used in indirect IO is done by&n; first checking if the &quot;reserved buffer&quot; for the current file descriptor&n; is available and large enough. If these conditions are _not_ met then&n; kernel memory is obtained on a per SCSI command basis. This corresponds&n; to a write(), read() sequence or a SG_IO ioctl() call. Further, the&n; kernel memory that is suitable for DMA may be constrained by the&n; architecture of the SCSI adapter (e.g. ISA adapters).&n;&n; ** N.B. To use direct IO &squot;echo 1 &gt; /proc/scsi/sg/allow_dio&squot; may be&n;         needed. That pseudo file&squot;s content is defaulted to 0. **&n;&n; Documentation&n; =============&n; A web site for SG device drivers can be found at:&n;&t;http://www.torque.net/sg  [alternatively check the MAINTAINERS file]&n; The main documents are still based on 2.x versions:&n;&t;http://www.torque.net/sg/p/scsi-generic.txt&n;&t;http://www.torque.net/sg/p/scsi-generic_long.txt&n; Documentation on the changes and additions in 3.x version of the sg driver&n; can be found at: http://www.torque.net/sg/p/scsi-generic_v3.txt&n; A version of this document (potentially out of date) may also be found in&n; the kernel source tree, probably at:&n;        /usr/src/linux/Documentation/scsi-generic.txt .&n; Utility and test programs are available at the sg web site.&n;*/
