...
multiline_comment|/*&n; * Normally we use the spinlock functions in arch/ppc64/lib/locks.c.&n; * For special applications such as profiling, we can have the&n; * spinlock functions inline by defining CONFIG_SPINLINE.&n; * This is not recommended on partitioned systems with shared&n; * processors, since the inline spinlock functions don&squot;t include&n; * the code for yielding the CPU to the lock holder.&n; */
...
multiline_comment|/*&n; * Note: if we ever want to inline the spinlocks on iSeries,&n; * we will have to change the irq enable/disable stuff in here.&n; */
...
mdefine_line|#define spin_unlock_wait(x)&t;do { cpu_relax(); } while (spin_is_locked(x))
