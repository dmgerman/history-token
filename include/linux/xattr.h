multiline_comment|/*&n;  File: linux/xattr.h&n;&n;  Extended attributes handling.&n;&n;  Copyright (C) 2001 by Andreas Gruenbacher &lt;a.gruenbacher@computer.org&gt;&n;  Copyright (c) 2001-2002 Silicon Graphics, Inc.  All Rights Reserved.&n;*/
macro_line|#ifndef _LINUX_XATTR_H
DECL|macro|_LINUX_XATTR_H
mdefine_line|#define _LINUX_XATTR_H
DECL|macro|XATTR_CREATE
mdefine_line|#define XATTR_CREATE&t;&t;0x1&t;/* fail if attr already exists */
DECL|macro|XATTR_REPLACE
mdefine_line|#define XATTR_REPLACE&t;&t;0x2&t;/* fail if attr does not exist */
DECL|macro|XATTR_KERNEL_CONTEXT
mdefine_line|#define XATTR_KERNEL_CONTEXT&t;0x4&t;/* called from kernel context */
macro_line|#endif&t;/* _LINUX_XATTR_H */
eof
