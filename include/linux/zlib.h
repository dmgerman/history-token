...
multiline_comment|/*   &n;extern int inflateInit2 OF((z_streamp strm,&n;                                     int  windowBits));&n;&n;     This is another version of inflateInit with an extra parameter. The&n;   fields next_in, avail_in, zalloc, zfree and opaque must be initialized&n;   before by the caller.&n;&n;     The windowBits parameter is the base two logarithm of the maximum window&n;   size (the size of the history buffer).  It should be in the range 8..15 for&n;   this version of the library. The default value is 15 if inflateInit is used&n;   instead. If a compressed stream with a larger window size is given as&n;   input, inflate() will return with the error code Z_DATA_ERROR instead of&n;   trying to allocate a larger window.&n;&n;      inflateInit2 returns Z_OK if success, Z_MEM_ERROR if there was not enough&n;   memory, Z_STREAM_ERROR if a parameter is invalid (such as a negative&n;   memLevel). msg is set to null if there is no error message.  inflateInit2&n;   does not perform any decompression apart from reading the zlib header if&n;   present: this will be done by inflate(). (So next_in and avail_in may be&n;   modified, but next_out and avail_out are unchanged.)&n;*/
r_extern
r_int
