...
multiline_comment|/**&n; * wait_event - sleep until a condition gets true&n; * @wq: the waitqueue to wait on&n; * @condition: a C expression for the event to wait for&n; *&n; * The process is put to sleep (TASK_UNINTERRUPTIBLE) until the&n; * @condition evaluates to true. The @condition is checked each time&n; * the waitqueue @wq is woken up.&n; *&n; * wake_up() has to be called after changing any variable that could&n; * change the result of the wait condition.&n; */
...
multiline_comment|/**&n; * wait_event_timeout - sleep until a condition gets true or a timeout elapses&n; * @wq: the waitqueue to wait on&n; * @condition: a C expression for the event to wait for&n; * @timeout: timeout, in jiffies&n; *&n; * The process is put to sleep (TASK_UNINTERRUPTIBLE) until the&n; * @condition evaluates to true. The @condition is checked each time&n; * the waitqueue @wq is woken up.&n; *&n; * wake_up() has to be called after changing any variable that could&n; * change the result of the wait condition.&n; *&n; * The function returns 0 if the @timeout elapsed, and the remaining&n; * jiffies if the condition evaluated to true before the timeout elapsed.&n; */
...
multiline_comment|/**&n; * wait_event_interruptible - sleep until a condition gets true&n; * @wq: the waitqueue to wait on&n; * @condition: a C expression for the event to wait for&n; *&n; * The process is put to sleep (TASK_INTERRUPTIBLE) until the&n; * @condition evaluates to true or a signal is received.&n; * The @condition is checked each time the waitqueue @wq is woken up.&n; *&n; * wake_up() has to be called after changing any variable that could&n; * change the result of the wait condition.&n; *&n; * The function will return -ERESTARTSYS if it was interrupted by a&n; * signal and 0 if @condition evaluated to true.&n; */
...
multiline_comment|/**&n; * wait_event_interruptible_timeout - sleep until a condition gets true or a timeout elapses&n; * @wq: the waitqueue to wait on&n; * @condition: a C expression for the event to wait for&n; * @timeout: timeout, in jiffies&n; *&n; * The process is put to sleep (TASK_INTERRUPTIBLE) until the&n; * @condition evaluates to true or a signal is received.&n; * The @condition is checked each time the waitqueue @wq is woken up.&n; *&n; * wake_up() has to be called after changing any variable that could&n; * change the result of the wait condition.&n; *&n; * The function returns 0 if the @timeout elapsed, -ERESTARTSYS if it&n; * was interrupted by a signal, and the remaining jiffies otherwise&n; * if the condition evaluated to true before the timeout elapsed.&n; */
