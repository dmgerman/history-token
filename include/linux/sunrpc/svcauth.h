...
multiline_comment|/* Authentication is done in the context of a domain.&n; *&n; * Currently, the nfs server uses the auth_domain to stand&n; * for the &quot;client&quot; listed in /etc/exports.&n; *&n; * More generally, a domain might represent a group of clients using&n; * a common mechanism for authentication and having a common mapping&n; * between local identity (uid) and network identity.  All clients&n; * in a domain have similar general access rights.  Each domain can&n; * contain multiple principals which will have different specific right&n; * based on normal Discretionary Access Control.&n; *&n; * A domain is created by an authentication flavour module based on name&n; * only.  Userspace then fills in detail on demand.&n; *&n; * In the case of auth_unix and auth_null, the auth_domain is also&n; * associated with entries in another cache representing the mapping&n; * of ip addresses to the given client.&n; */
