...
multiline_comment|/*&n; * The zone field is never updated after free_area_init_core()&n; * sets it, so none of the operations on it need to be atomic.&n; * We&squot;ll have up to log2(MAX_NUMNODES * MAX_NR_ZONES) zones&n; * total, so we use NODES_SHIFT here to get enough bits.&n; */
DECL|macro|ZONE_SHIFT
mdefine_line|#define ZONE_SHIFT (BITS_PER_LONG - NODES_SHIFT - MAX_NR_ZONES_SHIFT)
r_struct
id|zone
suffix:semicolon
r_extern
