...
suffix:semicolon
multiline_comment|/* Same as above two parameters, except that these values&n;&t; * apply while an IRQ is being serviced by the host.  Not&n;&t; * all cards support this feature and the values are ignored&n;&t; * in that case.&n;&t; */
...
id|net_device
suffix:semicolon
multiline_comment|/* Some generic methods drivers may use in their ethtool_ops */
id|u32
...
suffix:semicolon
multiline_comment|/**&n; * &amp;ethtool_ops - Alter and report network device settings&n; * get_settings: Get device-specific settings&n; * set_settings: Set device-specific settings&n; * get_drvinfo: Report driver information&n; * get_regs: Get device registers&n; * get_wol: Report whether Wake-on-Lan is enabled&n; * set_wol: Turn Wake-on-Lan on or off&n; * get_msglevel: Report driver message level&n; * set_msglevel: Set driver message level&n; * nway_reset: Restart autonegotiation&n; * get_link: Get link status&n; * get_eeprom: Read data from the device EEPROM&n; * set_eeprom: Write data to the device EEPROM&n; * get_coalesce: Get interrupt coalescing parameters&n; * set_coalesce: Set interrupt coalescing parameters&n; * get_ringparam: Report ring sizes&n; * set_ringparam: Set ring sizes&n; * get_pauseparam: Report pause parameters&n; * set_pauseparam: Set pause paramters&n; * get_rx_csum: Report whether receive checksums are turned on or off&n; * set_rx_csum: Turn receive checksum on or off&n; * get_tx_csum: Report whether transmit checksums are turned on or off&n; * set_tx_csum: Turn transmit checksums on or off&n; * get_sg: Report whether scatter-gather is enabled&n; * set_sg: Turn scatter-gather on or off&n; * get_tso: Report whether TCP segmentation offload is enabled&n; * set_tso: Turn TCP segmentation offload on or off&n; * self_test: Run specified self-tests&n; * get_strings: Return a set of strings that describe the requested objects &n; * phys_id: Identify the device&n; * get_stats: Return statistics about the device&n; *&n; * Description:&n; *&n; * get_settings:&n; *&t;@get_settings is passed an &amp;ethtool_cmd to fill in.  It returns&n; *&t;an negative errno or zero.&n; *&n; * set_settings:&n; *&t;@set_settings is passed an &amp;ethtool_cmd and should attempt to set&n; *&t;all the settings this device supports.  It may return an error value&n; *&t;if something goes wrong (otherwise 0).&n; *&n; * get_eeprom:&n; *&t;Should fill in the magic field.  Don&squot;t need to check len for zero&n; *&t;or wraparound but must check offset + len &lt; size.  Fill in the data&n; *&t;argument with the eeprom values from offset to offset + len.  Update&n; *&t;len to the amount read.  Returns an error or zero.&n; *&n; * set_eeprom:&n; *&t;Should validate the magic field.  Don&squot;t need to check len for zero&n; *&t;or wraparound but must check offset + len &lt; size.  Update len to&n; *&t;the amount written.  Returns an error or zero.&n; */
DECL|struct|ethtool_ops
r_struct
id|ethtool_ops
(brace
