...
macro_line|#else
macro_line|#include &lt;s390x/s390-regs-common.h&gt;
macro_line|#endif
...
DECL|macro|ACR0_OFFSET
mdefine_line|#define ACR0_OFFSET ((PSW_MASK_SIZE+PSW_ADDR_SIZE)+(GPR_SIZE*NUM_GPRS))
DECL|macro|CR0_OFFSET
mdefine_line|#define CR0_OFFSET (ACR0_OFFSET+(ACR_SIZE+NUM_ACRS))
DECL|macro|FPC_OFFSET
mdefine_line|#define FPC_OFFSET (CR0_OFFSET+(CR_SIZE*NUM_CRS))
DECL|macro|FP0_OFFSET
mdefine_line|#define FP0_OFFSET (FPC_OFFSET+(FPC_SIZE+FPC_PAD_SIZE))
...
mdefine_line|#define REGISTER_BYTE(N)  ((N)&lt;=GP_LAST_REGNUM ? (N)*8: &bslash;&n;(N) &lt;= LAST_ACR ? (ACR0_OFFSET+(((N)-FIRST_ACR)*ACR_SIZE)): &bslash;&n;(N) &lt;= LAST_CR ? (CR0_OFFSET+(((N)-FIRST_CR)*CR_SIZE)): &bslash;&n;(N) == FPC_REGNUM ? FPC_OFFSET:(FP0_OFFSET+(((N)-FP0_REGNUM)*FPR_SIZE)))
macro_line|#endif
eof
