...
macro_line|#endif
multiline_comment|/*&n; * Here we define all the compile-time &squot;special&squot; virtual&n; * addresses. The point is to have a constant address at&n; * compile time, but to set the physical address only&n; * in the boot process. We allocate these special  addresses&n; * from the top of unused virtual memory (0xfd000000 - 1 page) backwards.&n; * Also this lets us do fail-safe vmalloc(), we&n; * can guarantee that these special addresses and&n; * vmalloc()-ed addresses never overlap.&n; *&n; * these &squot;compile-time allocated&squot; memory buffers are&n; * fixed-size 4k pages. (or larger if used with an increment&n; * highger than 1) use fixmap_set(idx,phys) to associate&n; * physical memory with fixmap indices.&n; *&n; * TLB entries of such buffers will not be flushed across&n; * task switches.&n; */
multiline_comment|/*&n; * on UP currently we will have no trace of the fixmap mechanism,&n; * no page table allocations, etc. This might change in the&n; * future, say framebuffers for the console driver(s) could be&n; * fix-mapped?&n; */
...
multiline_comment|/*&n; * used by vmalloc.c.&n; *&n; * Leave one empty page between IO pages at 0xfd000000 and&n; * the start of the fixmap.&n; */
DECL|macro|FIXADDR_TOP
mdefine_line|#define FIXADDR_TOP&t;(0xfcfff000UL)
