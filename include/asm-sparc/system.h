...
macro_line|#endif
singleline_comment|// #define prepare_arch_schedule(prev)&t;task_lock(prev)
singleline_comment|// #define finish_arch_schedule(prev)&t;task_unlock(prev)
DECL|macro|prepare_arch_schedule
mdefine_line|#define prepare_arch_schedule(prev)&t;do{ }while(0)
DECL|macro|finish_arch_schedule
mdefine_line|#define finish_arch_schedule(prev)&t;do{ }while(0)
multiline_comment|/*&n; * Flush windows so that the VM switch which follows&n; * would not pull the stack from under us.&n; *&n; * SWITCH_ENTER and SWITH_DO_LAZY_FPU do not work yet (e.g. SMP does not work)&n; * XXX WTF is the above comment? Found in late teen 2.4.x.&n; *&n; * XXX prepare_arch_switch() is much smarter than this in sparc64, are we sure?&n; * XXX Cosider if doing it the flush_user_windows way is faster (by uwinmask).&n; */
