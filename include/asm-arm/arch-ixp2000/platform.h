...
multiline_comment|/*&n; * Boards may multiplex different devices on the 2nd channel of &n; * the slowport interface that each need different configuration &n; * settings.  For example, the IXDP2400 uses channel 2 on the interface &n; * to access the CPLD, the switch fabric card, and the media card.  Each&n; * one needs a different mode so drivers must save/restore the mode &n; * before and after each operation.  &n; *&n; * acquire_slowport(&amp;your_config);&n; * ...&n; * do slowport operations&n; * ...&n; * release_slowport();&n; *&n; * Note that while you have the slowport, you are holding a spinlock,&n; * so your code should be written as if you explicitly acquired a lock.&n; *&n; * The configuration only affects device 2 on the slowport, so the&n; * MTD map driver does not acquire/release the slowport.  &n; */
